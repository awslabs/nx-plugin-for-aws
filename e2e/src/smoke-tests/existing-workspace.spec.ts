...
describe('smoke test - existing-workspace', () => {
  ...
  beforeEach(() => {
    ...
    // allowBuilds mirrors the documented user step for pnpm 11's strict dep
    // builds: the first install (during `nx init`, before the plugin's init
    // generator can allow-list anything) hard-fails on nx's postinstall with
    // ERR_PNPM_IGNORED_BUILDS unless the user has approved it (see the
    // "add to an existing project" guide's troubleshooting section).
    writeFileSync(
