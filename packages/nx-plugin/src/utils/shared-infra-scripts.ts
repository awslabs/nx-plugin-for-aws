...
import {
  PACKAGES_DIR,
  SHARED_INFRA_SCRIPTS_NAME,
  SHARED_INFRA_SCRIPTS_DIR,
} from './shared-constructs-constants';

/**
 * Lazily creates the shared infra-scripts package at packages/common/infra-scripts/.
...
export async function sharedInfraScriptsGenerator(tree: Tree): Promise<void> {
  ...
  if (!tree.exists(pkgJsonPath)) {
    tree.write(
      ...
      JSON.stringify(
        {
          name: `${npmScopePrefix}${SHARED_INFRA_SCRIPTS_NAME}`,
          version: '0.0.0',
          type: 'module',
          bin: {
  ...
  addDependenciesToPackageJson(
    tree,
    { [`${npmScopePrefix}${SHARED_INFRA_SCRIPTS_NAME}`]: 'workspace:*' },
    {},
  );
  ...
  generateFiles(
    ...
    scriptsDir,
    {
      fullyQualifiedName: `${npmScopePrefix}${SHARED_INFRA_SCRIPTS_NAME}`,
      name: SHARED_INFRA_SCRIPTS_NAME,
