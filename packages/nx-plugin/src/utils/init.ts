 ...
 *
 * It assumes an ESM-oriented Nx workspace (the mainstream shape all modern Nx
 * presets produce). It does NOT rewrite an existing `tsconfig.base.json`'s
 * compiler options, migrate a workspace's package manager, restructure existing
 * projects, or convert CommonJS projects to ESM — those are decisions for the
 * user (see the "add to an existing workspace" guide).
 */
