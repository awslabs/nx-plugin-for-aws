...
// Matches `from <module> import <names>` re-export lines in the barrel.
const RE_EXPORT_IMPORT = /^from (\S+) import (.+)$/;
 ...
 *
 * The barrel is rebuilt in a canonical, ruff-clean form on each call (imports
 * sorted and merged per module, single blank line separation) so it passes the
 * project's `ruff check` (I001) regardless of whether generation-time
 * formatting runs — it does not in a freshly generated standalone workspace.
 */
export async function addPythonReExport(
  ...
  const existing = tree.exists(initFilePath)
    ? (tree.read(initFilePath, 'utf-8') ?? '')
    : '';

  // Preserve a leading module docstring if present.
  const docstringMatch = existing.match(/^\s*("""[\s\S]*?"""|'''[\s\S]*?''')/);
  const docstring =
    docstringMatch?.[1] ?? '"""Automatically generated by Nx."""';

  // Collect existing re-exported names grouped by source module.
  const importsByModule = new Map<string, Set<string>>();
  for (const line of existing.split('\n')) {
    const match = line.trim().match(RE_EXPORT_IMPORT);
    if (!match) {
      continue;
    }
    const names = importsByModule.get(match[1]) ?? new Set<string>();
    for (const name of match[2].split(',').map((n) => n.trim())) {
      if (name) {
        names.add(name);
      }
    }
    importsByModule.set(match[1], names);
  }

  // Add the new re-export.
  const names = importsByModule.get(fromModule) ?? new Set<string>();
  names.add(importName);
  importsByModule.set(fromModule, names);

  // Emit imports sorted by module, with names sorted within each module, and
  // __all__ in the order the names appear across the sorted import lines.
  const allNames: string[] = [];
  const importLines = [...importsByModule.keys()].sort().map((mod) => {
    const sortedNames = [...importsByModule.get(mod)!].sort();
    allNames.push(...sortedNames);
    return `from ${mod} import ${sortedNames.join(', ')}`;
  });

  const allEntries = allNames.map((name) => `    "${name}",`).join('\n');
  tree.write(
    initFilePath,
    `${docstring}\n\n${importLines.join('\n')}\n\n__all__ = [\n${allEntries}\n]\n`,
  );
