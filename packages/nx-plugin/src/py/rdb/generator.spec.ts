    ...
    expect(projectConfig.targets['bundle-migration']).toEqual({
      ...
      options: {
        commands: [
          ...
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "dist/packages/db/bundle-arm" "dist/packages/db/docker/migration"',
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "packages/db/migrations" "dist/packages/db/docker/migration/migrations"',
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "packages/db/alembic.ini" "dist/packages/db/docker/migration/alembic.ini"',
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "packages/db/Dockerfile.migration" "dist/packages/db/docker/migration/Dockerfile"',
        ],
        parallel: false,
      },
    ...
    expect(projectConfig.targets['bundle-create-db-user']).toEqual({
      ...
      options: {
        commands: [
          ...
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "dist/packages/db/bundle-arm" "dist/packages/db/docker/create-db-user"',
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "packages/db/Dockerfile.create-db-user" "dist/packages/db/docker/create-db-user/Dockerfile"',
        ],
        parallel: false,
      },
      dependsOn: ['bundle-arm'],
    ...
    expect(projectConfig.targets.trivy).toEqual({
      ...
      options: {
        commands: [
          ...
          'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "packages/db/.trivyignore" "dist/packages/db/trivy/proj-db-migration-latest/.trivyignore"',
