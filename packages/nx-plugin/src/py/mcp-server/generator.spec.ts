...
`,
  ...
  it('should add MCP server to existing Python project with default name', async () => {
    ...
    expect(projectConfig.targets['mcp-server-serve'].options.commands).toEqual([
      'uv run uvicorn --reload proj_test_project.mcp_server.http:app --host 0.0.0.0 --port 8000',
    ]);
  ...
  it('should generate MCP server with BedrockAgentCoreRuntime and default name', async () => {
    ...
    expect(projectConfig.targets['mcp-server-docker'].options.commands).toEqual(
      [
        ...
        'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "dist/apps/test-project/bundle-arm" "dist/apps/test-project/docker/test-project-mcp-server"',
        'node -e "require(\'fs\').cpSync(process.argv[1],process.argv[2],{recursive:true,verbatimSymlinks:true})" "apps/test-project/proj_test_project/mcp_server/Dockerfile" "dist/apps/test-project/docker/test-project-mcp-server/Dockerfile"',
