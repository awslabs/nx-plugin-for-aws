...
describe('runCheck', () => {
  ...
  it('formatReport uses collector traceCommand for dependency chain hints', async () => {
    writePkg(join(dir, 'node_modules'), 'gpl-pkg', '1.0.0', 'GPL-3.0');
    const failing = await runCheck({
      projectRoot: dir,
      config: { allow },
    });
    const { npmCollector } = await import('./collector');
    const report = formatReport(failing, { collectors: [npmCollector()] });
