  ...
  const applicationTargets: {
    ...
    bootstrap: {
      ...
      options: {
        forwardAllArgs: true,
        commands: [
          `aws s3 cp s3://$(aws sts get-caller-identity --query Account --output text)-tf-state-\${AWS_REGION:-\${AWS_DEFAULT_REGION:-$(aws configure get region)}}/bootstrap.tfstate ${tfDistDir}/bootstrap.tfstate || true`,
          'terraform init',
          `terraform apply -auto-approve -state=${tfDistDir}/bootstrap.tfstate -var="aws_region=\${AWS_REGION:-\${AWS_DEFAULT_REGION:-$(aws configure get region)}}"`,
          `aws s3 cp ${tfDistDir}/bootstrap.tfstate s3://$(aws sts get-caller-identity --query Account --output text)-tf-state-\${AWS_REGION:-\${AWS_DEFAULT_REGION:-$(aws configure get region)}}/bootstrap.tfstate`,
        ],
        parallel: false,
    ...
    init: {
      ...
      configurations: {
        dev: {
          command: `terraform init -reconfigure -backend-config="region=\${AWS_REGION:-\${AWS_DEFAULT_REGION:-$(aws configure get region)}}" -backend-config="bucket=$(aws sts get-caller-identity --query Account --output text)-tf-state-\${AWS_REGION:-\${AWS_DEFAULT_REGION:-$(aws configure get region)}}" -backend-config="key=${kebabCase(lib.fullyQualifiedName)}/dev/terraform.tfstate"`,
        },
      },
      options: {
        forwardAllArgs: true,
