...
3. **Harness `auth` field & JWT forwarding:** the harness metadata hardcodes `auth: 'iam'`. Decide whether to support JWT inbound + bearer forwarding (needed for per-user downstream authz) in v1 or defer.
   ...
   **Recommendation:** defer. Ship IAM inbound in v1 with per-user scoping enforced in the handler (actor id from JWT). Add JWT inbound + bearer forwarding as a follow-up when a use case needs per-user downstream (Token Vault / Gateway / Cedar) authorization.
