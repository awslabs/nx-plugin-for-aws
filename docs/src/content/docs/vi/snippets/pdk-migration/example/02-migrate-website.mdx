...
Với Nx Plugin for AWS, tích hợp API được hỗ trợ thông qua <Link path="/guides/api-connection">trình tạo `api-connection`</Link>. Tiếp theo, chúng ta sử dụng trình tạo này để trang web của chúng ta có thể gọi Smithy API:
