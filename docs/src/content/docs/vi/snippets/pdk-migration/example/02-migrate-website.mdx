...
Chạy <Link path="/guides/react-website">trình tạo `ts#website`</Link> với `framework` được đặt thành `react` để thiết lập dự án trang web của bạn trong `packages/website`:
...
Máy chủ cục bộ của Smithy API chạy với [`tsx`](https://github.com/privatenumber/tsx), vì vậy hãy thêm nó như một phụ thuộc phát triển trước khi khởi động các máy chủ cục bộ:

<InstallCommand dev pkg="tsx" />

Bây giờ hãy khởi động máy chủ trang web cục bộ:
