...
1. Thêm trait `@cursor` mới vào operation `GetShoppingLists` trong `get-shopping-lists.smithy`:
    ...
    Bất kỳ operation `@paginated` nào cũng nên sử dụng `@cursor` nếu bạn đang sử dụng client generator được cung cấp bởi Nx Plugin for AWS (thông qua generator <Link path="guides/api-connection/react-smithy">`api-connection`</Link>).
