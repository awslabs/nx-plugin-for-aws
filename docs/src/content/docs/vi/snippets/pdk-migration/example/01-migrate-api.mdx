...
1. Thêm trait `@cursor` mới vào thao tác `GetShoppingLists` trong `get-shopping-lists.smithy`:
    ...
    Bất kỳ thao tác `@paginated` nào cũng nên sử dụng `@cursor` nếu bạn đang sử dụng generator client được cung cấp bởi Nx Plugin for AWS (thông qua generator <Link path="/guides/connection/react-smithy">`api-connection`</Link>).
