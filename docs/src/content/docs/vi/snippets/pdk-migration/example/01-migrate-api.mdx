...
1. Thêm trait `@cursor` mới vào operation `GetShoppingLists` trong `get-shopping-lists.smithy`:
    ...
    Bất kỳ operation `@paginated` nào cũng nên sử dụng `@cursor` nếu bạn đang sử dụng client generator được cung cấp bởi Nx Plugin for AWS (thông qua generator <Link path="guides/connection/react-smithy">`api-connection`</Link>).
...
Generator `ts#smithy-api` tạo một operation `Echo` mẫu. Vì chúng ta đã xóa nó khỏi mô hình, hãy xóa handler tương ứng trong `backend/src/operations/echo.ts`. Chúng ta sẽ đăng ký các operation đã di chuyển trong `service.ts` ở phần dưới.
