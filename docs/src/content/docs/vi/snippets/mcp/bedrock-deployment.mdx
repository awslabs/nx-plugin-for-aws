...
Nếu bạn đã chọn `agentcore` cho `infra`, cơ sở hạ tầng CDK hoặc Terraform tương ứng sẽ được tạo ra, bạn có thể sử dụng để triển khai MCP server của mình lên [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Truyền các output của module <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> được chia sẻ vào module MCP server:
...
Bạn có thể cấp quyền truy cập để gọi MCP server của mình trên Bedrock AgentCore Runtime bằng phương thức `grantInvokeAccess`. Ví dụ, bạn có thể muốn một agent được tạo bằng trình tạo <Link path="/guides/py-agent">`py#agent`</Link> gọi MCP server của bạn:
...
Construct `UserIdentity` có thể được tạo bằng <Link path="/guides/react-website-auth">trình tạo `ts#website#auth`</Link>, hoặc bạn có thể tự tạo CDK `UserPool` và `UserPoolClient` của riêng mình.
</Fragment>
<Fragment slot="terraform">
