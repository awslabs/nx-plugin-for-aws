...
Khai báo `module "runtime_config_appconfig_deployment"` một lần ở cuối root module của bạn (với `depends_on` bao gồm mọi module đóng góp một runtime-config entry) — nó tổng hợp các entry được đóng góp và tạo các AppConfig hosted configuration versions + deployments. Xem <Link path="/guides/terraform-project#shared-runtime-config-appconfig">mẫu đầy đủ</Link>.
</Fragment>
</Infrastructure>
