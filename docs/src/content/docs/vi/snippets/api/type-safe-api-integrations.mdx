...
Nếu bạn chỉ định `environment` trong các tùy chọn mặc định của mình, điều này sẽ __ghi đè__ bất kỳ biến môi trường nào được định nghĩa trong construct API đã được tạo của bạn. Thông thường tốt hơn là sử dụng phương thức `addEnvironment` cho các Lambda function. Bạn có thể dễ dàng đạt được điều này bằng cách lặp qua các tích hợp của mình:
