...
Hỗ trợ từ generator hiện tại khác nhau tùy theo backend:

- `ts#trpc-api` hỗ trợ cả `isolated` và `shared` cho `ServerlessApiGatewayRestApi` và `ServerlessApiGatewayHttpApi` thông qua tùy chọn `integrationPattern`.
- `py#fast-api` và `ts#smithy-api` hiện tại tạo `isolated`.

Bạn cũng có thể chuyển đổi một construct CDK API đã được tạo giữa hai pattern bằng cách chỉnh sửa trường `pattern` trong phương thức `defaultIntegrations` của nó:

```ts {3}
return IntegrationBuilder.rest({
  // Thay đổi giá trị này thành 'isolated' hoặc 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Các module Terraform đã sử dụng router pattern theo mặc định, với một Lambda function duy nhất xử lý tất cả các operation API.
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
Nếu generator của bạn không expose router pattern trực tiếp, hoặc bạn muốn tùy chỉnh nó thêm, bạn có thể chỉnh sửa phương thức `defaultIntegrations` cho API của mình để kết nối các tích hợp theo cách bạn muốn:
