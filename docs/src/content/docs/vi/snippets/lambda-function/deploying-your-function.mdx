...
Generator này tạo ra mã infrastructure as code bằng CDK hoặc Terraform dựa trên `iac` bạn đã chọn. Bạn có thể sử dụng nó để triển khai function của mình.
...
Đảm bảo event source khớp với tùy chọn `event` đã chọn để đảm bảo event được xử lý đúng cách trong handler function của bạn.
:::
...
Generator này tạo ra một Terraform module để triển khai function của bạn trong thư mục `common/terraform`. Khởi tạo module `core/asset-bucket` được chia sẻ một lần cho mỗi deployment và truyền `bucket_name` của nó vào input `asset_bucket_name`:
