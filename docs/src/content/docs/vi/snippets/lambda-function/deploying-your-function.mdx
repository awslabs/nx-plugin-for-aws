...
Generator này tạo ra mã cơ sở hạ tầng CDK hoặc Terraform dựa trên `iac` bạn đã chọn. Bạn có thể sử dụng nó để triển khai function của mình.
...
Generator này tạo ra một Terraform module để triển khai function của bạn trong thư mục `common/terraform`. Khởi tạo module `core/asset-bucket` được chia sẻ một lần cho mỗi lần triển khai và truyền `bucket_name` của nó vào input `asset_bucket_name`:
