...
Trình tạo tRPC hỗ trợ hai mô hình cơ sở hạ tầng thông qua tùy chọn `integrationPattern`:

- `isolated` là mặc định. Nó tạo một Lambda function cho mỗi procedure.
- `shared` tạo một Lambda integration `$router` mặc định duy nhất cho toàn bộ API và sử dụng lại nó cho mọi procedure trừ khi bạn cung cấp các override cho từng procedure.

`isolated` cung cấp cho bạn cơ sở hạ tầng chi tiết hơn và giúp dễ dàng gắn các quyền hoặc cấu hình khác nhau cho mỗi operation. `shared` giảm số lượng Lambda và integration, đồng thời vẫn cho phép bạn override các procedure cụ thể với các integration riêng biệt khi cần thiết.

Cả hai integration pattern hiện đang được hỗ trợ cho `ServerlessApiGatewayRestApi` và `ServerlessApiGatewayHttpApi`.
