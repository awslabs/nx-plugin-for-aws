...
[tRPC](https://trpc.io/) là một framework để xây dựng API trong TypeScript với tính an toàn kiểu từ đầu đến cuối. Sử dụng tRPC, các cập nhật đối với đầu vào và đầu ra của các thao tác API được phản ánh ngay lập tức trong mã client và hiển thị trong IDE của bạn mà không cần phải rebuild dự án.

Trình tạo API tRPC tạo một API tRPC mới với cấu hình cơ sở hạ tầng AWS CDK hoặc Terraform. Backend được tạo sử dụng AWS Lambda để triển khai serverless, được expose thông qua AWS API Gateway API, và bao gồm xác thực schema sử dụng [Zod](https://zod.dev/). Nó thiết lập [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) để quan sát, bao gồm logging, AWS X-Ray tracing và Cloudwatch Metrics.
...
Ở mức độ cao, các API tRPC bao gồm một router ủy quyền các request đến các procedure cụ thể. Mỗi procedure có một đầu vào và đầu ra, được định nghĩa dưới dạng schema Zod.
...
Zod cung cấp các tiện ích mạnh mẽ để kết hợp hoặc suy ra schema như `.merge`, `.pick`, `.omit` và nhiều hơn nữa. Bạn có thể tìm thêm thông tin trên [trang tài liệu Zod](https://zod.dev/?id=basic-usage).

### Router và Procedures

Router tRPC của bạn được định nghĩa trong `src/router.ts`, đăng ký tất cả các procedure. Mỗi procedure định nghĩa đầu vào, đầu ra và triển khai mong đợi. Entry point của Lambda handler nằm trong `src/handler.ts`, chuyển tiếp các request đến router của bạn.

Router mẫu được tạo cho bạn có một thao tác duy nhất, được gọi là `echo`:
...
- `input` chấp nhận một schema Zod định nghĩa đầu vào mong đợi cho thao tác. Các request được gửi cho thao tác này được tự động xác thực theo schema này.
...
tRPC subscriptions cho phép bạn stream dữ liệu từ server đến client bằng [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Khi bạn chọn `rest-lambda` làm loại compute của mình, generator tự động cấu hình cơ sở hạ tầng cần thiết cho streaming, cũng như một streaming Lambda handler và schema helper `ZodAsyncIterable`.
...
Bạn có thể nhóm các thao tác lại với nhau bằng cách sử dụng nested router, ví dụ:
...
AWS Lambda Powertools tracer được cấu hình trong `src/middleware/tracer.ts`, và có thể được truy cập trong triển khai API thông qua `opts.ctx.tracer`. Bạn có thể sử dụng điều này để thêm trace với AWS X-Ray để cung cấp thông tin chi tiết về hiệu suất và luồng của các request API. Ví dụ:
...
Ví dụ này hướng dẫn identity middleware cho xác thực `IAM`. Chúng ta tra cứu người gọi trong Cognito bằng cách sử dụng sub được trích xuất từ API Gateway event.

Việc tra cứu sử dụng Cognito Identity Provider client, không phải là dependency của API tRPC được tạo. Cài đặt nó vào dự án API của bạn trước:
...
Trong trường hợp của chúng ta, chúng ta muốn trích xuất chi tiết về người dùng Cognito đang gọi. Chúng ta sẽ làm điều đó bằng cách trích xuất ID chủ thể của người dùng (hoặc "sub") từ API Gateway event, và truy xuất chi tiết người dùng từ Cognito. Việc triển khai khác nhau tùy thuộc vào việc event được cung cấp cho hàm của chúng ta bởi REST API hay HTTP API:
...
Khi bạn triển khai với `auth: 'cognito'`, API Gateway Cognito authorizer xác minh JWT mà người gọi cung cấp trong header `Authorization` và đặt các claim đã xác minh trên Lambda event. Middleware của chúng ta chỉ đọc các claim đó — không có cuộc gọi AWS SDK bổ sung, không có xác minh JWT thủ công.
...
Lưu ý rằng chúng ta định nghĩa một thuộc tính _tùy chọn_ bổ sung trên ngữ cảnh. tRPC quản lý việc đảm bảo rằng điều này được định nghĩa trong các procedure đã cấu hình middleware này một cách chính xác.

Tiếp theo, middleware. Loại event và vị trí của các claim khác nhau giữa REST API và HTTP API, vì vậy việc triển khai phụ thuộc vào `infra` bạn đã chọn:
...
<TabItem label="HTTP API" _filter={{ infra: 'http-lambda' }}>
JWT authorizer của HTTP API cung cấp một payload-v2 event có các claim nằm sâu hơn một cấp, tại `event.requestContext.authorizer.jwt.claims`. Ngữ cảnh phải được gõ trên `APIGatewayProxyEventV2WithJWTAuthorizer` để khớp với ngữ cảnh mà `publicProcedure` được tạo sử dụng — nếu không `.concat()` sẽ thất bại với lỗi `Context mismatch` của tRPC:
...
Sau đó, bạn có thể trộn plugin vào bất kỳ procedure nào cần danh tính của người gọi:
...
Bạn không cần `aws-jwt-verify` hoặc bất kỳ thư viện xác minh JWT nào khác ở đây — API Gateway Cognito User Pools authorizer đã xác minh chữ ký, issuer, scopes và expiry vào thời điểm Lambda của bạn chạy. Nếu bất kỳ kiểm tra nào trong số đó thất bại, API Gateway trả về `401 Unauthorized` và handler của bạn không bao giờ được gọi.
:::
</OptionFilter>

## Triển khai API tRPC của bạn

Trình tạo API tRPC tạo mã cơ sở hạ tầng CDK hoặc Terraform dựa trên `iac` bạn đã chọn. Bạn có thể sử dụng điều này để triển khai API tRPC của mình.

<Infrastructure>
<Fragment slot="cdk">
CDK construct để triển khai API của bạn nằm trong thư mục `common/constructs`. Bạn có thể sử dụng điều này trong một ứng dụng CDK, ví dụ:
...
Kết quả của Authorizer được cache trong 5 phút, được khóa trên header `Authorization`, vì vậy một token bị thu hồi có thể được chấp nhận cho đến khi kết quả được cache của nó hết hạn. Giảm thời gian cache trong cơ sở hạ tầng được tạo nếu bạn cần thu hồi nhanh hơn.
:::
</OptionFilter>
...
Construct `UserIdentity` có thể được tạo bằng <Link path="/guides/react-website-auth">generator `ts#website#auth`</Link>.
</OptionFilter>
...
Module API stages Lambda deployment zip của nó trong một S3 asset bucket được chia sẻ — xem <Link path="/guides/terraform-project">hướng dẫn cơ sở hạ tầng Terraform</Link> để biết chi tiết. Khởi tạo module `core/asset-bucket` một lần cho mỗi triển khai và truyền đầu ra `bucket_name` của nó vào mọi module API / Lambda thông qua đầu vào `asset_bucket_name`:
...
Nếu bạn đã chọn CDK cho `iac` của mình, khi bạn thêm hoặc xóa một procedure trong API tRPC của mình, những thay đổi này sẽ được phản ánh ngay lập tức trong CDK construct mà không cần rebuild.
:::
...
Các đầu ra chính từ module API mà bạn có thể sử dụng cho các IAM policy là:
...
Entry point cho server cục bộ là `src/local-server.ts`.
...
Nếu bạn đang gọi API của mình từ một website React, hãy xem xét sử dụng generator <Link path="guides/connection/react-trpc">Connection</Link> để cấu hình client.
...
Sử dụng generator <Link path="guides/connection">`connection`</Link> để tích hợp dự án này với các dự án khác trong workspace của bạn. Các connection sau liên quan đến dự án này:
