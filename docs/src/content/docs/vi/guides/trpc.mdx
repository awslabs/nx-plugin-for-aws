...
:::tip
Tùy chọn `integrationStyle` mặc định là `Individual Functions`, tạo một Lambda cho mỗi tRPC procedure. Chọn `Router` nếu bạn muốn một Lambda handler được chia sẻ duy nhất cho toàn bộ API.
:::
...
### Chọn Integration Style

Trình tạo tRPC hỗ trợ hai mô hình cơ sở hạ tầng thông qua tùy chọn `integrationStyle`:

- `Individual Functions` là mặc định. Nó tạo một Lambda function cho mỗi procedure.
- `Router` tạo một Lambda function được chia sẻ duy nhất cho toàn bộ API và sử dụng lại nó cho mọi integration.

`Individual Functions` cung cấp cho bạn cơ sở hạ tầng chi tiết hơn và giúp dễ dàng gắn các quyền hoặc cấu hình khác nhau cho mỗi operation. `Router` giảm số lượng Lambda và integration, điều này có thể hữu ích cho các API nhỏ hơn hoặc khi bạn muốn một handler trung tâm duy nhất.

Cả hai integration style hiện đang được hỗ trợ cho `ServerlessApiGatewayRestApi` và `ServerlessApiGatewayHttpApi`.
