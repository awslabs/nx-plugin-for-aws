...
:::tip[Loại API]
Chọn `rest-lambda` (mặc định) làm `infra` của bạn nếu bạn muốn sử dụng [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) để stream các phản hồi.
:::
...
<FileTree>
  ...
  - package.json Manifest dự án định nghĩa tên package và các dependency của dự án
...
Construct `UserIdentity` có thể được tạo bằng cách sử dụng <Link path="/vi/guides/react-website-auth">trình tạo `ts#website#auth`</Link>.
</OptionFilter>
...
Nếu bạn đã chọn CDK cho `iac` của mình, khi bạn thêm hoặc xóa một procedure trong API tRPC của bạn, những thay đổi này sẽ được phản ánh ngay lập tức trong CDK construct mà không cần rebuild.
:::
