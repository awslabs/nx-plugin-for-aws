...
:::tip[Loại API]
Chọn `ServerlessApiGatewayRestApi` (mặc định) làm `computeType` của bạn nếu bạn muốn sử dụng [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) để stream các phản hồi. Chọn `BedrockAgentCoreRuntimeWebSocket` để triển khai máy chủ tRPC dưới dạng container trên Bedrock AgentCore với streaming WebSocket subscriptions — xem [Triển khai lên Bedrock AgentCore](#deploying-to-bedrock-agentcore) bên dưới.
:::

:::tip[Mẫu tích hợp]
Tùy chọn `integrationPattern` mặc định là `isolated`, tạo một Lambda cho mỗi tRPC procedure. Chọn `shared` nếu bạn muốn một Lambda handler được chia sẻ duy nhất cho toàn bộ API, với các override tùy chọn cho từng procedure. Tùy chọn `integrationPattern` không áp dụng khi `computeType` là `BedrockAgentCoreRuntimeWebSocket`.
:::
...
<FileTree>
  - src
    ...
    - handler-agentcore.ts Điểm vào máy chủ WebSocket (chỉ loại compute AgentCore)
...
:::caution[Loại tính toán đăng ký]
...
## Triển khai lên Bedrock AgentCore

Đặt `computeType` thành `BedrockAgentCoreRuntimeWebSocket` triển khai máy chủ tRPC dưới dạng máy chủ WebSocket được container hóa trên [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) thay vì API Gateway + Lambda. AgentCore cung cấp cho bạn:

- **Streaming subscriptions qua WebSocket** — nhắn tin song công đầy đủ cho tRPC subscriptions mà không có thời gian chờ 30 giây của Lambda hoặc API Gateway.
- **Session affinity** — header `x-amzn-bedrock-agentcore-runtime-session-id` được truyền vào tRPC context của bạn dưới dạng `ctx.sessionId`, vì vậy bạn có thể khóa trạng thái theo phiên.
- **Managed runtime** — AgentCore xử lý scaling, health checks (`/ping`), và authorization.

:::caution[Tùy chọn xác thực]
AgentCore chỉ hỗ trợ xác thực `IAM` hoặc `Cognito`. Chọn `auth=None` với `computeType=BedrockAgentCoreRuntimeWebSocket` sẽ thất bại khi tạo.
:::

### Cách hoạt động

Khi bạn chọn `BedrockAgentCoreRuntimeWebSocket`, trình tạo sẽ tạo ra:

- `src/handler-agentcore.ts` — một máy chủ Node WebSocket + HTTP expose `/ping` cho health checks của AgentCore và `/ws` cho tRPC subscriptions thông qua `@trpc/server/adapters/ws`.
- `Dockerfile` — container hóa máy chủ đã được bundle và bao gồm [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) cho khả năng quan sát được quản lý.
- Một CDK construct (hoặc Terraform module) cung cấp Bedrock AgentCore `Runtime`, cấp quyền cho nó để gọi các mô hình Bedrock và đọc cấu hình runtime, và xuất bản runtime ARN vào `runtimeConfig.apis.<YourApiName>` để frontend nhận.
- Một build target `docker` được kết nối vào build target `build` của dự án để `nx build` tạo ra container image.

### Truy cập session id

Mỗi procedure nhận `ctx.sessionId` khi API được triển khai lên AgentCore:
...
Ở local, `nx serve` chạy một máy chủ HTTP + WebSocket độc lập để client có thể kết nối theo cách tương tự như trong production. Session id mặc định là một UUID ngẫu nhiên khi header không có.
