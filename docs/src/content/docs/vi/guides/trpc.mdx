...
Ví dụ này hướng dẫn về identity middleware cho xác thực `IAM`, nơi chúng ta tra cứu người gọi trong Cognito bằng cách sử dụng sub được trích xuất từ API Gateway event. Đối với xác thực `Cognito`, identity middleware đơn giản hơn — trích xuất các claim trực tiếp từ `event` thay thế.
