...
<FileTree>
  - \<name>
    - \_\_init\_\_.py Package exports (`get_engine`, `session_context`, `get_session`)
...
Import `session_context` từ database package của bạn và sử dụng nó như một context manager để có được một SQLModel `Session`:
...
Khi kết nối trực tiếp đến Aurora cluster (không có RDS Proxy), truyền đường dẫn Amazon RDS CA bundle bằng tham số tùy chọn `rds_ca`:
...
Giá trị `rds_ca` phải trỏ đến một CA certificate bundle có sẵn trong hệ thống tệp runtime của bạn. Đối với Amazon RDS, sử dụng global CA bundle từ:

```text
https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
```

Cách bạn làm cho tệp đó có sẵn phụ thuộc vào runtime kết nối đến cơ sở dữ liệu.

#### Runtime Container Images

Nếu bạn chuẩn bị container image của riêng mình cho runtime, thêm RDS CA bundle trong Dockerfile của bạn và truyền đường dẫn đó cho `session_context`:
...
Nếu bạn triển khai một zipped Lambda function, đính kèm `RdsCaLayer` Lambda layer (CDK) hoặc `ca-layer` Terraform module. CDK layer sử dụng Docker bundling để thêm CA bundle, và layer đã triển khai cung cấp nó tại `/opt/global-bundle.pem`, mà bạn có thể truyền cho `session_context`:
