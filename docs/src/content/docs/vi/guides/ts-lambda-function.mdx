...
Nếu bạn đã chọn `Any` cho `eventSource`, `parser` middleware sẽ không được kết nối và tham số `event` được type là `any`. Hãy tạo lại function với một `eventSource` cụ thể nếu bạn muốn có type safety tại thời điểm biên dịch và xác thực runtime.
</OptionFilter>
