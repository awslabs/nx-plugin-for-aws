...
Nếu bạn đã chọn một `event` khi tạo lambda function, function của bạn sẽ được trang bị [`parser` middleware từ AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Ví dụ:
...
Nếu bạn đã chọn `Any` cho `event`, `parser` middleware sẽ không được kết nối và tham số `event` được type là `any`. Hãy tạo lại function với một `event` cụ thể nếu bạn muốn có type safety tại thời điểm biên dịch và xác thực runtime.
</OptionFilter>
