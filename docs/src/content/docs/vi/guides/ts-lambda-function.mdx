...
Bạn đã chọn `Any` cho `eventSource`, điều này có nghĩa là `parser` middleware không được kết nối và tham số `event` được type là `any`. Nếu sau này bạn muốn có type safety tại thời điểm biên dịch và xác thực runtime, hãy tạo lại function với một `eventSource` cụ thể.
</OptionFilter>
