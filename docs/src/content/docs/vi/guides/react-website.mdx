...
:::note[UX Provider]
`ux` mặc định là [shadcn/ui](https://ui.shadcn.com/). Bạn cũng có thể chọn [Cloudscape](http://cloudscape.design/) hoặc `none` (tự mang thư viện component của bạn).
:::
...
<FileTree>
  ...
  - src
    ...
    - hooks
      - useAppLayout.tsx Hook để điều chỉnh AppLayout từ các component lồng nhau (chỉ Cloudscape)
    - routes
  ...
  - package.json Manifest dự án xác định tên package và các dependency của dự án
</FileTree>

:::note[Không dùng TanStack Router]
...
Trang web được triển khai có kiến trúc như sau:
...
Theo mặc định, distribution sử dụng tên miền CloudFront mặc định (`*.cloudfront.net`) và chứng chỉ mặc định của nó, không hỗ trợ việc thực thi phiên bản TLS tối thiểu là 1.2. Để phục vụ trang web của bạn từ tên miền riêng của bạn, hãy cung cấp một [chứng chỉ ACM](https://docs.aws.amazon.com/acm/latest/userguide/acm-overview.html) (phải nằm trong `us-east-1` để sử dụng với CloudFront) và tên miền của bạn — phiên bản TLS tối thiểu là 1.2 sau đó sẽ được thực thi cho người xem:

<Infrastructure>
<Fragment slot="cdk">
Truyền các props `certificate` và `domainNames` trong construct website được tạo của bạn trong `packages/common/constructs/src/app/static-websites`:
...
<Fragment slot="terraform">
Đặt các biến `custom_domain_names` và `acm_certificate_arn` trong module website được tạo của bạn trong `packages/common/terraform/src/app/static-websites`:
...
Trang web của bạn đi kèm với [TanStack Router](https://tanstack.com/router/v1) được cấu hình mặc định. Điều này giúp dễ dàng thêm các route mới:
...
Cấu hình từ cơ sở hạ tầng của bạn được cung cấp cho trang web của bạn thông qua <Link href="guides/runtime-config">Runtime Configuration</Link>. Điều này cho phép trang web của bạn truy cập các chi tiết như URL API mà không được biết cho đến khi ứng dụng của bạn được triển khai.
...
Construct CDK của trang web của bạn sẽ triển khai namespace `connection` của cấu hình runtime dưới dạng file `runtime-config.json` vào thư mục gốc của bucket S3 của bạn.
...
export class ApplicationStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    // Website có thể được khai báo tại bất kỳ điểm nào — runtime config được resolve một cách lazy
...
Với CDK, construct website có thể được khai báo tại bất kỳ điểm nào trong stack của bạn. Cấu hình runtime được resolve một cách lazy tại thời điểm synth, vì vậy tất cả các giá trị sẽ được bao gồm bất kể thứ tự khai báo.
:::
</Fragment>
<Fragment slot="terraform">
...
Module Terraform của trang web của bạn sẽ triển khai namespace `connection` của cấu hình runtime dưới dạng file `runtime-config.json` vào thư mục gốc của bucket S3 của bạn.
...
Để biết chi tiết về cách cấu hình runtime được lưu trữ trong AWS AppConfig và cách các consumer phía server (Lambda functions, agents) có thể truy xuất nó, hãy xem hướng dẫn <Link path="guides/runtime-config">Runtime Configuration</Link>.
:::
...
Lệnh chuẩn cho phát triển local là `dev`, khởi động trang web của bạn (và bất kỳ server local nào cho các API mà bạn đã kết nối với nó) bằng một lệnh duy nhất:

<NxCommands commands={['dev <my-website>']} />

Target `serve` cũng có sẵn khi bạn cần kiểm soát mức độ ứng dụng của bạn chạy local so với việc trỏ đến cơ sở hạ tầng AWS đã triển khai. Để có cái nhìn tổng quan rộng hơn về phát triển local trên các dự án được kết nối, bao gồm cách `dev` hoạt động cho các dự án có nhiều thành phần, hãy xem hướng dẫn <Link path="guides/local-development">Local Development</Link>.
...
Target `dev` khởi động một local development server cho trang web của bạn (với [Vite `MODE`](https://vite.dev/guide/env-and-mode) được đặt thành `local-dev`), cũng như khởi động bất kỳ server local nào cho các API mà bạn đã kết nối trang web của mình thông qua <Link path="/guides/connection">Connection generator</Link>.
...
:::note[Script `dev` của Workspace]
`package.json` gốc của workspace của bạn bao gồm một script `dev` chạy target `dev` cho mọi dự án có target đó:

<PackageManagerShortCommand commands={["dev"]} />

Điều này khởi động local development server cho trang web của bạn (và bất kỳ dự án nào khác có target `dev`) trong một lệnh. Để chỉ chạy trang web này, hãy sử dụng target `dev` riêng của nó (ví dụ: `nx dev <my-website>`).
:::

:::warning[Xác thực giả lập]
Khi chạy ở chế độ này và không có `runtime-config.json`, nếu bạn đã cấu hình Cognito Authentication (thông qua <Link path="/guides/react-website-auth">generator `ts#website#auth`</Link>), đăng nhập sẽ được bỏ qua và các yêu cầu đến server local của bạn sẽ không bao gồm header xác thực.
...
Bạn có thể xây dựng trang web của mình bằng cách sử dụng target `build`. Điều này chạy các target `bundle`, `compile`, `test` và `lint`, kiểm tra kiểu, đóng gói, testing và lint trang web của bạn.

<NxCommands commands={['build <my-website>']} />

Target `bundle` sử dụng Vite để tạo một bundle production trong thư mục gốc `dist/packages/<my-website>/bundle`. Đây là artifact có thể triển khai được sử dụng bởi cơ sở hạ tầng trang web của bạn. Bạn có thể chạy nó độc lập:
...
<Fragment slot="terraform">
Để triển khai trang web của bạn, chúng tôi khuyên bạn nên sử dụng <Link path="/guides/terraform-project">generator `terraform#project`</Link> để tạo một dự án Terraform.
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Kết nối một trang web React với các agent thông qua AgentCore Gateway"
