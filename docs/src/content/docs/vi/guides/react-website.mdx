...
:::note[Script `dev`]
Nếu không có script `dev` trong `package.json` gốc của bạn khi generator này chạy, một script `dev` sẽ được thêm vào để gọi `serve-local` cho trang web được tạo, có nghĩa là bạn cũng có thể khởi động local development server với:

<PackageManagerShortCommand commands={["dev"]} />

Điều này làm cho trang web React đầu tiên bạn tạo trở thành target `dev` mặc định cho workspace. Các trang web tiếp theo không ghi đè script `dev` hiện có — bạn có thể gọi trực tiếp các target `serve-local` của chúng hoặc cập nhật script theo cách thủ công.
:::
...
<CardGrid>
  <ConnectionCard
    title="React to tRPC"
    description="Gọi tRPC API từ trang web React"
  ...
  <ConnectionCard
    ...
    description="Gọi Python FastAPI từ trang web React"
  ...
  <ConnectionCard
    ...
    description="Gọi Smithy API từ trang web React"
  ...
  <ConnectionCard
    ...
    description="Gọi Python Agent từ trang web React"
  ...
  <ConnectionCard
    ...
    description="Gọi TypeScript Agent từ trang web React"
  ...
  <ConnectionCard
    ...
    description="Gọi Agent hiển thị giao thức AG-UI từ trang web React thông qua CopilotKit"
