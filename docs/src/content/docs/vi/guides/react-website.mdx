...
`$id` trong đường dẫn `to` là một [path parameter](https://tanstack.com/router/latest/docs/framework/react/guide/path-params), được cung cấp thông qua `params`. Thay thế `<button>` thông thường bằng button component của bộ UX của bạn nếu cần.
...
Target `checkov` truyền `--config-file`, điều này tắt khả năng tự động phát hiện `.checkov.yaml` của chính `checkov` — một file cấu hình được đặt ở bất kỳ đâu khác sẽ bị bỏ qua.
:::
...
Cấu hình từ cơ sở hạ tầng của bạn được cung cấp cho website của bạn qua <Link path="guides/runtime-config">Runtime Configuration</Link>. Điều này cho phép website của bạn truy cập các chi tiết như API URLs mà không được biết cho đến khi ứng dụng của bạn được triển khai.
...
Website của bạn được tạo với một `RuntimeConfigProvider` (trong `src/components/RuntimeConfig`) tải `runtime-config.json` trước khi render ứng dụng của bạn, và một hook `useRuntimeConfig` để đọc các giá trị từ nó:
...
Provider sẽ fallback về một cấu hình rỗng khi không có `runtime-config.json` được phục vụ, vì vậy một website chưa triển khai gì vẫn render được. `IRuntimeConfig` được gõ là `any` theo mặc định — thu hẹp nó trong `src/components/RuntimeConfig/index.tsx` nếu bạn muốn truy cập an toàn kiểu.
