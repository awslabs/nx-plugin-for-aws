...
Trên pnpm, workspace cũng đặt [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) để `pnpm add` ghi lại các dependencies mới trong catalog tự động. yarn và bun không có cài đặt tương đương, vì vậy hãy thêm các dependencies mới với protocol `catalog:` để ghi lại chúng trong catalog, ví dụ `yarn workspace @my-scope/my-library add some-npm-package@catalog:` hoặc `bun add some-npm-package@catalog: --cwd packages/my-library`.
