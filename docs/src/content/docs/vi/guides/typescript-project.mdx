...
:::tip[Hàm Lambda]
Nếu bạn đang xây dựng một hàm AWS Lambda, hãy xem trình tạo <Link path="/guides/ts-lambda-function">`ts#lambda-function`</Link> vì nó cấu hình bundling cho bạn, cũng như tạo cơ sở hạ tầng và thêm khả năng quan sát và type-safety.
:::
