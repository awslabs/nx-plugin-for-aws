...
Khi catalogs bị tắt, các generators ghi phiên bản dependency trực tiếp vào `package.json` của mỗi dự án, và việc giữ các phiên bản căn chỉnh giữa các dự án là trách nhiệm của bạn (ví dụ sử dụng [syncpack](https://github.com/JamieMason/syncpack)). Khi catalogs được bật trên pnpm, workspace cũng đặt [`catalogMode: prefer`](https://pnpm.io/settings#catalogmode) để `pnpm add` tiếp tục thêm các dependencies mới vào catalog.
