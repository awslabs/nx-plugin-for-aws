...
<FileTree>
  ...
  - src Mã nguồn TypeScript
    - index.ts
  - package.json Manifest dự án tối thiểu định nghĩa tên package của dự án
...
:::note[Bí danh dự án]
Aliases khớp với tên package của các dự án, vì vậy bất kỳ dự án nào trong workspace của bạn đều có thể được import bằng tên được khai báo trong `package.json` của nó — không cần khai báo các dự án cục bộ làm dependencies.
:::
...
Mỗi dự án TypeScript khai báo các **runtime dependencies của bên thứ ba mà mã nguồn của nó import** trong `package.json` riêng của nó. Điều này giúp mọi dự án tự mô tả — manifest của nó liệt kê chính xác những gì nó cần — và cho phép quy tắc lint [`noUndeclaredDependencies`](https://biomejs.dev/linter/rules/no-undeclared-dependencies/) của Biome đảm bảo rằng một dự án không bao giờ import một package mà nó chưa khai báo.

**Các công cụ build và test** (ví dụ `vitest`, `tsx`, `rolldown`, Nx toolchain (`nx`, `@nx/js`, ...), `typescript`, `@biomejs/biome`) được chia sẻ trên toàn workspace, vì vậy chúng nằm trong `devDependencies` của `package.json` **root** thay vì trong mỗi dự án. Các công cụ này chỉ được tham chiếu từ các file cấu hình, build scripts và tests, mà quy tắc lint miễn trừ.

Để thêm một runtime dependency vào một dự án, hãy thêm nó vào `package.json` của dự án đó. Sử dụng flag filter/workspace của package manager để nhắm đúng dự án, ví dụ với pnpm:

```bash
pnpm add some-npm-package --filter @my-scope/my-library
```

Để thêm một công cụ dev dùng chung, hãy thêm nó vào workspace root:

<InstallCommand pkg="-D some-dev-tool" />

Điều này tuân theo [single version policy](https://nx.dev/concepts/decisions/dependency-management#single-version-policy), có nghĩa là tất cả các dự án sử dụng cùng một phiên bản của mỗi dependency — tránh xung đột kiểu và nhiều bản sao của các packages trong deployment bundles. Đối với các package managers hỗ trợ catalogs ([pnpm](https://pnpm.io/catalogs), [yarn](https://yarnpkg.com/features/catalogs) và [bun](https://bun.com/docs/install/catalogs)), các generators ghi lại mỗi phiên bản trong catalog và tham chiếu nó bằng protocol `catalog:` (ví dụ `"zod": "catalog:"`), vì vậy catalog là nguồn duy nhất về phiên bản bất kể dự án nào khai báo dependency. `biome.json` được cung cấp kích hoạt resolver [`experimentalPnpmCatalogs`](https://biomejs.dev/reference/configuration/#javascriptresolverexperimentalpnpmcatalogs) của Biome để quy tắc lint hiểu các tham chiếu `catalog:`.

:::tip[Căn chỉnh phiên bản trên npm]
npm (và yarn classic) không có tính năng catalog, vì vậy các phiên bản được khai báo trực tiếp trong mỗi `package.json`. Để giữ chúng nhất quán giữa các dự án, chúng tôi khuyên dùng [syncpack](https://github.com/JamieMason/syncpack) (`npx syncpack lint`).
:::

:::note[bun]
Trên bun, một số packages mà các Nx generators đọc phiên bản từ manifest root (`vite`, `react`, `react-dom`) giữ các version ranges trực tiếp thay vì tham chiếu `catalog:` — Nx chưa resolve bun catalogs, và một tham chiếu `catalog:` sẽ làm hỏng các generators đó.
:::
...
Khi catalogs bị tắt, các generators ghi phiên bản dependency trực tiếp vào `package.json` của mỗi dự án, và việc giữ các phiên bản căn chỉnh giữa các dự án là trách nhiệm của bạn (ví dụ sử dụng [syncpack](https://github.com/JamieMason/syncpack)). Khi catalogs được bật trên pnpm, workspace cũng đặt [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) để `pnpm add` ghi lại các dependencies mới trong catalog, duy trì chính sách phiên bản đơn mà không cần chỉnh sửa thủ công.

Khi một dự án import một dự án khác trong workspace của bạn, bạn không cần khai báo nó thủ công: các path aliases TypeScript resolve import để build, và generator `ts#sync` (chạy trước `compile`, hoặc theo yêu cầu qua `nx sync`) khai báo dự án được import làm workspace dependency trong `package.json` của dự án import (`workspace:*` khi package manager hỗ trợ workspace protocol, `*` trên npm và yarn classic). Điều này giữ cho manifest của mỗi dự án là một bản ghi chính xác về những gì nó sử dụng — thỏa mãn quy tắc lint `noUndeclaredDependencies` — mà không cần ghi chép thủ công.
