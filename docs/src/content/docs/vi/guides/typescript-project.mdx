...
Dự án của bạn cũng có một target `package`, là target chỉ tạo artifact của `build`: nó tạo ra bất cứ thứ gì dự án của bạn đóng góp cho một triển khai (đầu ra đã biên dịch, và bất kỳ bundle, container image hoặc đặc tả được tạo mà nó khai báo) mà không chạy các cổng kiểm tra lint, test hoặc type-check.

<NxCommands commands={['package <project-name>']} />

Các <Link path="guides/typescript-infrastructure#deploying-to-aws">target deploy</Link> phụ thuộc vào `package` thay vì `build`, vì vậy việc triển khai chỉ build những gì nó sắp triển khai. `build` không thay đổi và vẫn làm mọi thứ, vì vậy nó vẫn là thứ bạn chạy để kiểm tra một dự án — và là thứ bạn nên chạy trong CI.

:::caution
Vì `package` bỏ qua các cổng kiểm tra chất lượng, một triển khai không còn thất bại khi có lỗi lint hoặc một bài kiểm thử đơn vị thất bại. Chạy `build` (hoặc pipeline CI của bạn) để kiểm tra mã của bạn.
:::
