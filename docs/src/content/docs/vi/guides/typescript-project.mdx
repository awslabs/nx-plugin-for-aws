...
Trên pnpm, workspace cũng đặt [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), vì vậy `pnpm add` ghi lại phiên bản trong catalog và tham chiếu nó tự động.

yarn và bun không có tương đương, và protocol `catalog:` của chúng chỉ *tham chiếu* một entry catalog hiện có — nó không thể tạo một entry mới. Để thêm một dependency mới trên yarn hoặc bun, trước tiên hãy ghi lại phiên bản của nó trong catalog, sau đó tham chiếu nó từ dự án:

- **yarn** — thêm phiên bản dưới `catalog` trong `.yarnrc.yml`, sau đó `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — thêm phiên bản dưới `catalog` trong `package.json` root, sau đó `bun add some-npm-package@catalog: --cwd packages/my-library`
