...
<FileTree>
  ...
  - vitest.config.mts cấu hình Vitest dùng chung mà vitest.config.mts của dự án bạn kế thừa (được tạo bởi dự án TypeScript đầu tiên)
  - package.json các công cụ build và test dùng chung được thêm vào
  - pnpm-workspace.yaml dự án được thêm vào workspace, và các phiên bản phụ thuộc vào [catalog](#catalogs) (tệp tương đương cho trình quản lý package của bạn)
  - .gitignore các đường dẫn đầu ra build và test được bỏ qua
...
`cwd` là thứ cho phép `rolldown` tìm thấy cấu hình của nó: nếu không có nó, lệnh sẽ chạy từ thư mục gốc workspace, nơi `rolldown.config.ts` không tồn tại.
...
Dự án của bạn cũng có một target `assemble`, tạo ra bất cứ thứ gì dự án của bạn đóng góp cho một triển khai (ví dụ như kết quả biên dịch hoặc đóng gói của nó), mà không chạy các cổng kiểm tra lint, test hoặc type-check.

<NxCommands commands={['assemble <project-name>']} />

Các <Link path="guides/typescript-infrastructure#deploying-to-aws">target deploy</Link> phụ thuộc vào `assemble`, vì vậy việc triển khai chỉ build những gì nó sắp triển khai.

:::caution[Sử dụng `build` trong pipeline của bạn]
`assemble` được sử dụng để lặp lại nhanh hơn trong quá trình phát triển. Chạy `build` trong pipeline CI của bạn để đảm bảo dự án của bạn vượt qua các cổng chất lượng trước khi triển khai production.
:::
