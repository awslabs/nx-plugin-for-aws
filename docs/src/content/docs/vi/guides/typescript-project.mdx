...
**Các công cụ build và test** (ví dụ `vitest`, `tsx`, `rolldown`, `@nx/*`, `typescript`, `@biomejs/biome`) được chia sẻ trên toàn workspace, vì vậy chúng nằm trong `devDependencies` của `package.json` **root** thay vì trong mỗi dự án. Các công cụ này chỉ được tham chiếu từ các file cấu hình, build scripts và tests, mà quy tắc lint miễn trừ.
...
npm không có tính năng catalog, vì vậy các phiên bản được khai báo trực tiếp trong mỗi `package.json`. Để giữ chúng nhất quán giữa các dự án trong một npm workspace, chúng tôi khuyên dùng [syncpack](https://github.com/JamieMason/syncpack).
:::
...
Nếu bạn đang publish dự án TypeScript của mình lên NPM, `package.json` của nó phải khai báo các dependencies mà dự án của bạn tham chiếu, cùng với các entry points (`main`/`exports`). Vì tại thời điểm build, dự án của bạn resolve các dependencies được cài đặt qua `package.json` ở workspace root, quy tắc `noUndeclaredDependencies` của Biome sẽ cảnh báo nếu dự án của bạn import một package không được liệt kê trong `package.json` của nó.
