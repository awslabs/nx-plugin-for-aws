...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Thêm phụ thuộc với [`--save-catalog`](https://pnpm.io/settings#catalogmode), để pnpm ghi lại phiên bản trong catalog và tham chiếu nó:
    ...
    :::note
    Thiết lập [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) trong `pnpm-workspace.yaml` làm cho đây trở thành mặc định cho mọi `pnpm add`. Nó cũng làm cho `pnpm add some-npm-package@<version>` thất bại bất cứ khi nào catalog ghim một phiên bản khác, điều này ngăn `nx migrate` nâng cấp Nx — vì vậy workspace được tạo ra để nó không được thiết lập.
    :::
  </TabItem>
  <TabItem label="yarn">
