...
File này phải khai báo các dependencies mà dự án của bạn tham chiếu. Vì tại thời điểm build, dự án của bạn sẽ resolve các dependencies được cài đặt qua `package.json` ở workspace root, quy tắc `noUndeclaredDependencies` của Biome sẽ cảnh báo nếu dự án của bạn import một package không được liệt kê trong `package.json` của nó.
