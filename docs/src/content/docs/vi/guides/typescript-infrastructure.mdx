...
| `deploy`         | Triển khai bất kỳ stage nào trong quá trình phát triển, bằng cách đặt tên stage hoặc stack bạn muốn. Sử dụng express mode. |
...
Các target `deploy-sandbox` và `deploy` sử dụng [CloudFormation express mode](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) theo mặc định (thông qua cờ `--express` của CDK). Express mode kết thúc khi cấu hình tài nguyên được xác nhận áp dụng thay vì chờ đợi ổn định đầy đủ, với việc lan truyền tiếp tục ở chế độ nền, cho các chu kỳ lặp lại nhanh hơn đáng kể.

Để chờ đợi ổn định tài nguyên đầy đủ thay vì, truyền `--no-express`:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox` và `deploy` chỉ dành cho phát triển]
Vì express mode trả về trước khi tài nguyên hoàn tất ổn định, `deploy-sandbox` và `deploy` được dành cho phát triển và lặp lại, không dành cho CI/CD hoặc triển khai production. Sử dụng [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) trong pipeline của bạn — nó không bao giờ sử dụng express mode, vì vậy nó luôn chờ đợi ổn định đầy đủ.
:::
...
Target này khác với các target `deploy` và `deploy-sandbox` theo hai cách. Thứ nhất, nó triển khai một cloud assembly đã được synthesized trước thay vì synthesizing ngay lập tức, tránh khả năng không xác định tiềm ẩn từ các thay đổi phiên bản gói và đảm bảo rằng mọi giai đoạn pipeline triển khai sử dụng cùng một cloud assembly. Thứ hai, nó không sử dụng express mode, vì vậy nó chờ đợi mọi tài nguyên ổn định đầy đủ trước khi báo cáo thành công.

:::caution[Luôn sử dụng `deploy-ci` cho production]
`deploy` và `deploy-sandbox` sử dụng express mode và trả về trước khi tài nguyên hoàn tất ổn định, điều này khiến chúng không phù hợp cho CI/CD và triển khai production. Sử dụng `deploy-ci` cho những trường hợp đó.
:::
