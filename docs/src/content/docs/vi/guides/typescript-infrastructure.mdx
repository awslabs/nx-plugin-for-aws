...
Nếu bạn tạo với `--stageConfig=true`, trình tạo cũng tạo hai gói chia sẻ để quản lý thông tin xác thực tập trung (nếu chúng chưa tồn tại):
...
Phần này áp dụng khi bạn tạo với `stageConfig`:

<NxCommands commands={['g @aws/nx-plugin:ts#infra --name=infra --stageConfig=true']} />

Nếu không có nó, trình tạo tạo ra một thiết lập đơn giản hơn nơi bạn tự quản lý thông tin xác thực AWS (ví dụ: bằng cách export `AWS_PROFILE` trước khi triển khai).
:::
...
`synth` phụ thuộc vào `^assemble`, vì vậy việc synthesizing xây dựng các artifact mà stack của bạn triển khai, chẳng hạn như các Lambda bundle, container image và specification được tạo ra, mà không chạy các cổng kiểm tra lint, test và type-check của mọi dự án upstream. Xem <Link path="guides/typescript-project#assembling">Assembling</Link> để biết thêm chi tiết về `assemble`.
...
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `deploy-sandbox` | Triển khai sandbox stage của riêng bạn trong quá trình phát triển. Không cần đối số stage. Sử dụng express mode.                    |
| `deploy`         | Triển khai bất kỳ stage nào, bằng cách đặt tên stage hoặc stack bạn muốn. Sử dụng express mode trừ khi bạn tạo với `stageConfig`.   |
| `deploy-ci`      | Triển khai từ một CI/CD pipeline, sử dụng một cloud assembly đã được synthesized trước. Chờ đợi ổn định đầy đủ.                     |

`deploy-sandbox` và `deploy` phụ thuộc vào `^assemble`, vì vậy chúng xây dựng các artifact mà chúng sắp triển khai và không có gì khác. `deploy-ci` triển khai một assembly mà pipeline của bạn đã xây dựng, vì vậy nó không có bất kỳ phụ thuộc xây dựng nào cả.

:::caution[Sử dụng `build` trong pipeline của bạn]
`assemble` được sử dụng để lặp lại nhanh hơn trong quá trình phát triển. Chạy `build` trong CI pipeline của bạn để đảm bảo dự án của bạn vượt qua các cổng chất lượng trước khi triển khai production.
:::
...
Target `deploy-sandbox` sử dụng [CloudFormation express mode](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) theo mặc định (thông qua cờ `--express` của CDK). Express mode kết thúc khi cấu hình tài nguyên được xác nhận áp dụng thay vì chờ đợi ổn định đầy đủ, với việc lan truyền tiếp tục ở chế độ nền, cho các chu kỳ lặp lại nhanh hơn đáng kể.

Vì nó trả về trước khi tài nguyên hoàn tất ổn định, express mode phù hợp với sandbox stage mà bạn lặp lại, không dành cho CI/CD hoặc triển khai production — sử dụng [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) cho những trường hợp đó. Để chờ đợi ổn định tài nguyên đầy đủ ở đây, truyền `--no-express`:
...
Việc target này có sử dụng express mode hay không phụ thuộc vào cách bạn tạo dự án:

- **Với `stageConfig`** nó chờ đợi ổn định tài nguyên đầy đủ, vì target có thể đặt tên bất kỳ stage nào bao gồm cả stage production của bạn. Truyền `--express` để chọn vào express mode cho một stage mà bạn đang lặp lại.
- **Không có `stageConfig`** nó sử dụng express mode, như `deploy-sandbox` làm. Vì stage bạn đặt tên có thể là một stage production, truyền `--no-express` để chờ đợi ổn định tài nguyên đầy đủ:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Sử dụng [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) từ một pipeline thay thế — nó không bao giờ sử dụng express mode.
...
Target này khác với các target `deploy` và `deploy-sandbox` theo hai cách. Thứ nhất, nó triển khai một cloud assembly đã được synthesized trước thay vì synthesizing ngay lập tức, tránh khả năng không xác định tiềm ẩn từ các thay đổi phiên bản gói và đảm bảo rằng mọi giai đoạn pipeline triển khai sử dụng cùng một cloud assembly. Thứ hai, nó không bao giờ sử dụng express mode, vì vậy nó luôn chờ đợi mọi tài nguyên ổn định đầy đủ trước khi báo cáo thành công.
