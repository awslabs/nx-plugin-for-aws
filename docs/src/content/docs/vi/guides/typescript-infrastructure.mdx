...
| `deploy`         | Triển khai bất kỳ stage nào, bằng cách đặt tên stage hoặc stack bạn muốn. Chờ đợi ổn định đầy đủ.               |
...
Vì target này có thể đặt tên bất kỳ stage nào, bao gồm cả stage production của bạn, nó chờ đợi ổn định tài nguyên đầy đủ. Truyền `--express` để chọn vào express mode cho một stage mà bạn đang lặp lại.
...
Target này triển khai một cloud assembly đã được synthesized trước thay vì synthesizing ngay lập tức, tránh khả năng không xác định tiềm ẩn từ các thay đổi phiên bản gói và đảm bảo rằng mọi giai đoạn pipeline triển khai sử dụng cùng một cloud assembly. Giống như `deploy`, nó chờ đợi mọi tài nguyên ổn định đầy đủ trước khi báo cáo thành công, và không giống như `deploy` nó không chấp nhận tùy chọn `--express` để từ chối.
