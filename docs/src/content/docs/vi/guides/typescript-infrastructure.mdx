...
Nếu bạn cần triển khai tiêu chuẩn chờ đợi ổn định tài nguyên hoàn toàn, hãy từ chối với `--no-express`:

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

Target `deploy-ci` không sử dụng chế độ express, vì vậy các triển khai pipeline CI/CD luôn chờ đợi ổn định hoàn toàn.
:::
