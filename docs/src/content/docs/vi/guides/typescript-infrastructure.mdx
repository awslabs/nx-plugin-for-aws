...
Với CDK, construct website có thể được khai báo tại bất kỳ điểm nào trong stack của bạn — cấu hình runtime được phân giải một cách lazy tại thời điểm synth. Đối với Terraform, hãy đảm bảo module website có `depends_on` phù hợp cho các module thêm vào <Link path="guides/react-website#runtime-configuration">Runtime Config</Link>.
:::
...
:::caution[Quy tắc Checkov bị tắt]
...
Target `deploy` sử dụng [chế độ express của CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) theo mặc định (thông qua cờ `--express` của CDK) để tăng tốc độ triển khai trong quá trình phát triển. Chế độ express hoàn thành ngay khi cấu hình tài nguyên được xác nhận đã áp dụng thay vì chờ đợi ổn định hoàn toàn, với việc lan truyền tiếp tục ở chế độ nền, giúp chu kỳ lặp nhanh hơn đáng kể.

Chế độ express được thiết kế cho phát triển và lặp lại. Để triển khai mà không có nó và chờ đợi ổn định tài nguyên hoàn toàn, hãy xóa cờ `--express` khỏi lệnh của target `deploy` trong `project.json` của dự án cơ sở hạ tầng của bạn.

Đối với các triển khai thực tế đến các giai đoạn production, hãy sử dụng target `deploy-ci` (xem [bên dưới](#deploying-to-aws-in-a-cicd-pipeline)) như một phần của pipeline CI/CD. Target này không sử dụng chế độ express, vì vậy nó luôn chờ đợi ổn định hoàn toàn.
:::
