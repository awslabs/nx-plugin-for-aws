...
`synth` phụ thuộc vào `^assemble` thay vì `^build`, vì vậy synthesizing xây dựng các artifact mà stack của bạn triển khai — các Lambda bundle, container image và specification được tạo ra — mà không cần chạy các cổng lint, test và type-check của mọi dự án upstream. Xem <Link path="guides/typescript-project#assembling">Assembling</Link> để biết cách `assemble` liên quan đến `build`.
