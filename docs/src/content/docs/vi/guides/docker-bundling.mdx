...
Một số generator (như <Link path="/vi/guides/ts-strands-agent">`ts#strands-agent`</Link> và <Link path="/vi/guides/py-strands-agent">`py#strands-agent`</Link>) tạo ra Docker image được đẩy lên Amazon ECR và được sử dụng bởi hạ tầng AWS. Hướng dẫn này mô tả mẫu thiết kế mà chúng tuân theo để bạn có thể áp dụng cho các trường hợp sử dụng khác — ví dụ, chạy dự án <Link path="/vi/guides/fastapi">`py#fast-api`</Link> trên Amazon ECS, hoặc triển khai máy chủ Express được container hóa.
...
Cấu hình target `bundle` gọi Rolldown. Nếu bạn bắt đầu từ <Link path="/vi/guides/typescript-project">`ts#project`</Link>, thêm phần sau vào `project.json` của bạn:
...
Cấu hình target `bundle` sử dụng `uv` để export và cài đặt dependency cho nền tảng đích của bạn. Generator <Link path="/vi/guides/python-project">`py#project`</Link> và generator <Link path="/vi/guides/python-lambda-function">`py#lambda-function`</Link> đều cấu hình điều này cho bạn. Cấu hình target trông như sau:
...
3. Một `null_resource` với provisioner `local-exec` xác thực với ECR, re-tag image được build locally, và đẩy nó lên.
4. Resource downstream (ví dụ `aws_ecs_task_definition`) tham chiếu `"${aws_ecr_repository.repo.repository_url}:latest"`.
...
- <Link path="/vi/guides/ts-strands-agent">`ts#strands-agent` generator</Link> — một ví dụ hoàn chỉnh về mẫu này cho TypeScript agent được triển khai lên Bedrock AgentCore Runtime.
- <Link path="/vi/guides/py-strands-agent">`py#strands-agent` generator</Link> — tương đương cho Python.
