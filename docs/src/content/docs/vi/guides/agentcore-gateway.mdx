...
<FileTree>
  - packages/\<name>/
    - policies/ Các tệp nguồn chính sách Cedar (bỏ qua khi `cedarPolicy: false`)
      - permit-all.cedar Chính sách Cedar mặc định cho phép người gọi đã xác thực (người gọi IAM từ cùng tài khoản AWS, hoặc bất kỳ người dùng Cognito nào — xem [Authentication](#authentication))
...
- Một `AgentCore::Gateway` được cấu hình cho giao thức MCP với xác thực IAM đầu vào (mặc định) hoặc xác thực JWT Cognito (xem <a href="#authentication">Authentication</a>)
