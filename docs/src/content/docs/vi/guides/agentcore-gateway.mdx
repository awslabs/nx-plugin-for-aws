...
Truyền role ARN vào dưới dạng biến template (xem <a href="#template-variables">Biến template</a> bên dưới) thay vì hard-code hoặc pattern-matching nó. Chạy lại synth hoặc plan để triển khai chính sách mới.
...
Thêm biến mới tại nơi các chính sách được render, ví dụ để truyền ARN vai trò thực thi của agent vào ví dụ `ts-agent-divide.cedar` ở trên:

<Infrastructure>
<Fragment slot="cdk">
Trong `packages/common/constructs/src/app/gateways/<name>/<name>.ts`, thêm vào object được truyền cho `ejs.render`:
...
query = {
  ...
  gatewayArn     = awscc_bedrockagentcore_gateway.this.gateway_arn
...
Thuộc tính `id` của principal chứa IAM ARN của người gọi. Người gọi thường đến dưới dạng STS assumed-role ARN với hậu tố session được tạo, vì vậy hãy truyền chính xác role ARN vào dưới dạng biến template và khớp hậu tố session với `like`:
