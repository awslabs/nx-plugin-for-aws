...
Nếu bạn chọn `None` cho `computeType`, không có CDK construct hoặc Terraform module nào được tạo — máy chủ MCP được cấu hình chỉ để sử dụng STDIO / HTTP cục bộ. Tùy chọn `auth` bị bỏ qua trong chế độ này vì không có endpoint được lưu trữ để xác thực.
</OptionFilter>
