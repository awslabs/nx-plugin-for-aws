...
Nếu bạn chọn `None` cho `infra`, không có CDK construct hoặc Terraform module nào được tạo — máy chủ MCP được cấu hình chỉ để sử dụng STDIO / HTTP cục bộ. Tùy chọn `auth` bị bỏ qua trong chế độ này vì không có endpoint được lưu trữ để xác thực.
</OptionFilter>
...
:::tip
Để kiểm tra máy chủ bằng giao vận STDIO thay thế, hãy sử dụng target `<your-server-name>-inspect-stdio`, khởi chạy inspector với một phiên bản STDIO của máy chủ của bạn.
:::
...
Một target `docker` cụ thể cho máy chủ MCP của bạn cũng được thêm vào, sao chép `Dockerfile` và các artifact đã bundle vào một thư mục ngữ cảnh docker. Điều này đặt `Dockerfile` cùng vị trí với kết quả build, cho phép CDK xây dựng Docker image trực tiếp bằng cách sử dụng `AgentRuntimeArtifact.fromAsset`.

### Quét Image

<Snippet name="trivy-image-scan" parentHeading="Target Bundle và Docker" />
