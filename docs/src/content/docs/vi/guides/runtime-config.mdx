...
Các construct được tạo tự động ghi cấu hình liên quan vào namespace `connection`. Bạn cũng có thể ghi các giá trị của riêng mình vào bất kỳ namespace nào.
...
- Một **Deployment** tức thì vào môi trường `default`
</Fragment>
...
1. **`core/runtime-config/appconfig`** — được khai báo một lần, gần đầu root module. Tạo ứng dụng AppConfig, môi trường, chiến lược triển khai và một configuration profile cho mỗi namespace. Các output `application_id` và `application_arn` của nó được truyền vào mọi module đọc cấu hình runtime tại runtime (agent, MCP server, hàm Lambda, v.v.).
...
2. **`core/runtime-config/entry`** — một lần gọi cho mỗi đóng góp. Các module API, agent và MCP được tạo gọi nó nội bộ để xuất bản URL và ARN của chúng. Gọi trực tiếp để xuất bản cấu hình tùy chỉnh.
...
3. **`core/runtime-config/appconfig-deployment`** — được khai báo một lần, ở cuối root module, với `depends_on` bao gồm mọi module đóng góp một entry. Tổng hợp các entry được đóng góp thành một JSON cho mỗi namespace và tạo hosted configuration version + deployment đối với ứng dụng được chia sẻ.
...
Các consumer phía server cần **AppConfig Application ID** và quyền IAM để đọc cấu hình tại runtime. Các construct được tạo xử lý điều này tự động.
...
Các construct API và agent được tạo thực hiện điều này tự động — bạn chỉ cần làm thủ công nếu bạn tạo các hàm Lambda hoặc tài nguyên tính toán của riêng mình cần đọc cấu hình runtime.
:::
</Fragment>
<Fragment slot="terraform">
Tham chiếu output `application_id` của module `runtime_config_appconfig` được chia sẻ để lấy AppConfig Application ID, và thêm các câu lệnh IAM policy thích hợp:
...
:::note[Đăng Ký Tự Động]
Các module agent và MCP được tạo chấp nhận các input `appconfig_application_id` + `appconfig_application_arn` — truyền các output `runtime_config_appconfig` được chia sẻ và chúng sẽ kết nối biến môi trường và quyền IAM cho bạn. Bạn chỉ cần làm thủ công cho các hàm Lambda hoặc tài nguyên tính toán tùy chỉnh.
:::
</Fragment>
</Infrastructure>

:::tip[Phát Triển Cục Bộ]
Nếu bạn sử dụng runtime config với các namespace tùy chỉnh (ví dụ: `tables`, `features`), bạn chỉ cần định nghĩa một biến môi trường `RUNTIME_CONFIG_APP_ID` duy nhất cục bộ để trỏ đến ứng dụng AppConfig đã triển khai của bạn. Điều này cho phép các API hoặc agent cục bộ của bạn truy xuất cùng cấu hình như môi trường đã triển khai mà không cần sao chép tất cả các biến môi trường riêng lẻ.
:::

## Truy Cập Phía Server qua AppConfig

Các consumer phía server như hàm Lambda và agent có thể truy xuất cấu hình runtime từ **AWS AppConfig** bằng cách sử dụng [AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/).
...
`getAppConfig` trả về bất kỳ giá trị JSON hợp lệ nào, vì vậy hãy khai báo hình dạng bạn mong đợi và ép kiểu kết quả — nếu không, các truy cập thuộc tính sẽ không biên dịch được dưới `tsconfig` nghiêm ngặt của workspace:
...
:::tip[Xác thực tại ranh giới]
Một phép ép kiểu cho trình biên dịch biết hình dạng nhưng không kiểm tra nó tại runtime. Đối với cấu hình bạn không kiểm soát, hãy phân tích nó bằng schema [Zod](https://zod.dev/) thay vì vậy, để một giá trị không đúng định dạng thất bại rõ ràng thay vì xuất hiện dưới dạng `undefined` sau này.
:::
...
Đối với các website, namespace `connection` được triển khai dưới dạng file `runtime-config.json` vào S3 bucket. Xem hướng dẫn <Link path="guides/react-website#runtime-configuration">React Website Runtime Configuration</Link> để biết chi tiết về cách truy cập các giá trị này từ mã frontend của bạn.