...
Generator cung cấp tùy chọn `auth` để cấu hình xác thực cho Strands Agent của bạn. Bạn có thể chọn giữa xác thực `IAM` (mặc định) hoặc `Cognito` khi tạo agent của bạn.

#### IAM

Mặc định, Strands Agent của bạn sẽ được bảo mật bằng xác thực IAM, chỉ cần triển khai nó mà không cần bất kỳ tham số nào:
...
Bạn có thể cấp quyền truy cập để gọi agent của bạn trên Bedrock AgentCore Runtime bằng phương thức `grantInvokeAccess`, ví dụ:
...
Để cấp quyền truy cập để gọi agent của bạn, bạn sẽ cần thêm một policy như sau, tham chiếu output `module.my_project_agent.agent_core_runtime_arn`:
...
Khi bạn chọn xác thực `Cognito`, generator tự động tạo construct `UserIdentity` và cấu hình agent để sử dụng Cognito cho xác thực.

<Infrastructure>
<Fragment slot="cdk">
Construct được tạo ra chấp nhận prop `identity` để cấu hình xác thực Cognito:
...
Construct `UserIdentity` được tự động tạo ra khi bạn chọn xác thực `Cognito`.
</Fragment>
<Fragment slot="terraform">
...
Bedrock AgentCore Runtime hỗ trợ [xác thực OAuth/JWT từ trình duyệt](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-get-started-websocket.html#:~:text=Browser%20JavaScript%20client%20with%20OAuth) bằng cách chấp nhận bearer token được nhúng trong header `Sec-WebSocket-Protocol` trong quá trình WebSocket handshake. Token phải được mã hóa base64url và có tiền tố `base64UrlBearerAuthorization.`, theo sau là subprotocol sentinel `base64UrlBearerAuthorization`.

Ví dụ sau đây cho thấy cách tạo một tRPC client trong ứng dụng React sử dụng xác thực Cognito:
...
:::note
Ví dụ trên sử dụng [`react-oidc-context`](https://github.com/authts/react-oidc-context) để lấy access token, đây là cùng một thư viện được sử dụng bởi generator <Link path="/guides/react-website-auth">`ts#react-website#auth`</Link>. Bạn có thể lấy token từ bất kỳ OAuth/OIDC provider nào.
:::
...
:::tip
Nếu bạn muốn có trải nghiệm tương tự như <Link path="/guides/connection">Connection generator</Link> cho các agent, vui lòng +1 [GitHub issue này](https://github.com/awslabs/nx-plugin-for-aws/issues/326).
:::