...
Khi `protocol` được đặt thành `A2A`, điểm vào sử dụng [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) thay vì tRPC:
...
`A2AAgent` của Strands SDK bao bọc một endpoint A2A từ xa để bạn có thể gọi nó như một `Agent` local. `invoke` resolve thành một `AgentResult`; gọi `.toString()` trên nó sẽ nối các text block từ reply của remote agent.
...
Các request đến một A2A agent đã triển khai trên AgentCore (bao gồm cả việc fetch agent card) phải được ký SigV4. Truyền một `clientFactory` vào `A2AAgent` sử dụng fetch ký SigV4 cho cả card resolver và JSON-RPC transport:
...
:::note[Quyền IAM]
IAM role của người gọi cần cả `bedrock-agentcore:InvokeAgentRuntime` và `bedrock-agentcore:GetAgentCard` trên ARN runtime đích. Helper `grantInvokeAccess` của construct được tạo ra cấp cả hai quyền.
:::

:::note[Dependency bổ sung]
Ví dụ SigV4 client ở trên yêu cầu `@a2a-js/sdk`, `aws4fetch`, và `@aws-sdk/credential-providers`. Thêm chúng vào `package.json` của bạn trước khi chạy ví dụ.
:::

#### Sử dụng một A2A Agent như một Công cụ

`A2AAgent` triển khai cùng interface `InvokableAgent` như một `Agent` local, vì vậy bạn có thể bao bọc nó trong một `tool` của Strands và đưa nó cho một agent khác. Đây là cách template HTTP/tRPC agent được tạo ra ủy quyền cho một remote A2A agent:
...
Trong CDK stack của bạn, cấp quyền invoke access cho host agent đến remote agent — điều này kết nối cả hai IAM action mà A2A client cần:
