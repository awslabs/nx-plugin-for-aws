...
Đối với các agent **A2A**, target chỉ đơn giản chạy `agent-chat-cli` như một binary — không có code nào được tạo vào workspace của bạn.

Đối với các agent **HTTP** (tRPC qua WebSocket), generator tạo ra một file `scripts/<your-agent-name>/chat.ts` nhỏ gọn bao bọc `<Agent>Client.local({ url })` được tạo ra như một `ChatAdapter` và truyền nó vào `chatLoop` của `agent-chat-cli`.
