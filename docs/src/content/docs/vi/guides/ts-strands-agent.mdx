...
:::tip
`Dockerfile` của bạn được đặt cùng với dự án Strands Agent của bạn. Đối với CDK, construct sử dụng `buildContexts` để build image trực tiếp từ Dockerfile này. Đối với Terraform, docker image được pre-built sử dụng một tag (ví dụ `my-scope-my-project-agent:latest`) được tham chiếu bởi hạ tầng Terraform của bạn.
:::
