...
:::caution[AG-UI chưa được hỗ trợ cho TypeScript]
[Giao thức AG-UI](https://docs.ag-ui.com/) chưa được hỗ trợ cho TypeScript Strands Agents. Nếu bạn cần hỗ trợ AG-UI, vui lòng sử dụng generator <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> với `protocol=AG-UI`. Xem [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) để biết trạng thái hỗ trợ AG-UI cho TypeScript.
:::
...
Generator cấu hình một target có tên `<your-agent-name>-invoke` khởi động một cuộc trò chuyện tương tác với agent của bạn từ terminal. Mặc định nó kết nối với local server:

<NxCommands commands={['agent-invoke your-project']} />

Để trò chuyện với một agent đã triển khai thay vào đó, đặt `RUNTIME_CONFIG_APP_ID` thành AppConfig application ID của workspace của bạn — CLI sẽ resolve runtime ARN và gọi deployed endpoint:
...
Script CLI nằm tại `scripts/<agent>/cli.ts` và tự động chọn giao thức phù hợp (HTTP/A2A) cho agent của bạn.
