...
Một số tùy chọn chỉ áp dụng cho các giá trị nhất định của tùy chọn khác. Sử dụng `x-when` để chỉ định điều đó, để trang tài liệu bỏ qua tùy chọn ở nơi nó không áp dụng và nêu rõ điều kiện ở nơi nó áp dụng:
...
Nhiều khóa được kết hợp bằng AND và một danh sách các giá trị được kết hợp bằng OR trong một khóa, khớp với `<OptionFilter when={{…}}>`:
...
`x-when` ghi lại tùy chọn; nó không thực thi bất cứ điều gì, vì vậy một generator vẫn xác thực hoặc bỏ qua một tùy chọn không áp dụng cho lần chạy mà nó được cung cấp.
