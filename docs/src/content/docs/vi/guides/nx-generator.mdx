...
:::tip[Sân chơi GritQL]
Bạn có thể thử nghiệm các pattern GritQL trong [Sân chơi GritQL](https://docs.grit.io/playground), hoặc cài đặt GritQL CLI cục bộ để kiểm tra các pattern với các tệp của bạn:
...
export const myGenerator = async (tree: Tree, schema: MyGeneratorSchema) => {
  ...
  generateFiles(
    tree,
    joinPathFragments(import.meta.dirname, 'files'), // Thư mục template
    'path/to/output', // Thư mục đầu ra
    data,
  );
};
