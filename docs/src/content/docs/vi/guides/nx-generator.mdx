...
Thêm một [Nx Generator](https://nx.dev/recipes/generators/local-generators) vào dự án TypeScript, giúp bạn tự động hóa các tác vụ lặp đi lặp lại như tạo scaffold cho các component hoặc thực thi các cấu trúc dự án cụ thể.
...
#### Biến đổi Code với GritQL

Bạn có thể sử dụng [GritQL](https://docs.grit.io/) để tìm kiếm và biến đổi mã nguồn một cách khai báo trong các generator của bạn. GritQL hỗ trợ nhiều ngôn ngữ bao gồm TypeScript, JavaScript, Python, HCL (Terraform), và nhiều hơn nữa — vì vậy bạn có thể sử dụng cùng một cú pháp pattern trên toàn bộ stack của mình.
...
:::tip
Bạn có thể thử nghiệm các pattern GritQL trong [GritQL Playground](https://docs.grit.io/playground), hoặc cài đặt GritQL CLI cục bộ để kiểm tra các pattern với các tệp của bạn:
...
:::tip
Nếu bạn sử dụng một AI coding agent để giúp viết generators, hãy thêm [Context7 MCP server](https://context7.com/) để cung cấp cho nó tài liệu cập nhật. Nhắc nó tra cứu `gritql` và `docs.grit.io` để tham khảo cú pháp pattern GritQL, và cung cấp quyền truy cập vào GritQL CLI để nó có thể kiểm tra các pattern trực tiếp.
:::
