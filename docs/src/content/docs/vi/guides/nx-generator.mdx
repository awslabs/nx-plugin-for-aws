...
Chúng tôi khuyến nghị sử dụng generator <Link path="/vi/guides/ts-nx-plugin">`ts#nx-plugin`</Link> làm dự án cơ sở cho bất kỳ generator nào bạn tạo, vì nó cũng cung cấp một MCP Server giúp AI sử dụng các generator của bạn.
:::
...
:::warning[Sửa đổi dự án]
Generator này sẽ cập nhật `project` đã chọn để sử dụng CommonJS, vì Nx Generators hiện chỉ hỗ trợ CommonJS ([tham khảo vấn đề GitHub này để biết về hỗ trợ ESM](https://github.com/nrwl/nx/issues/15682)).
:::
...
Để có hướng dẫn chuyên sâu hơn về việc đóng góp cho Nx Plugin for AWS, vui lòng tham khảo <Link path="/vi/get_started/tutorials/contribute-generator">hướng dẫn tại đây</Link>.
:::