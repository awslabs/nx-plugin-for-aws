...
`A2AAgent` của Strands SDK bao bọc một endpoint A2A từ xa để bạn có thể gọi nó như một `Agent` local.
...
#### Gọi trên AgentCore với Xác thực IAM

Các request tới một A2A agent đã triển khai trên AgentCore (bao gồm cả việc lấy agent card) phải được ký SigV4. Truyền một `client_config` cho `A2AAgent` với một `httpx.AsyncClient` sử dụng luồng xác thực SigV4:
...
:::note[Quyền IAM]
IAM role của caller cần cả `bedrock-agentcore:InvokeAgentRuntime` và `bedrock-agentcore:GetAgentCard` trên ARN runtime đích. Helper `grantInvokeAccess` của construct được tạo cấp cả hai.
:::

:::note[Dependency bổ sung]
A2A generator thêm `strands-agents[a2a]` cho bạn. Nếu bạn đang thêm mã A2A client vào một HTTP agent, hãy thêm `strands-agents[a2a]` và `httpx` vào `pyproject.toml` của bạn.
:::

#### Sử dụng A2A Agent như một Tool

`A2AAgent` kế thừa từ `AgentBase` giống như một `Agent` local, vì vậy bạn có thể bao bọc nó trong một `@tool` và đưa nó cho một agent khác:

```python
from strands import Agent, tool

remote = # A2AAgent built with client_config as above
...
Trong CDK stack của bạn, cấp quyền truy cập invoke cho host agent tới remote agent — điều này thiết lập cả hai IAM action mà A2A client cần:
