...
Generator cấu hình một target Nx `<your-agent-name>-chat` phụ thuộc vào `<your-agent-name>-serve-local`. Khi chạy nó sẽ khởi động agent locally và đưa bạn vào một terminal chat tương tác được hỗ trợ bởi [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Đối với các agent **A2A** và **AG-UI**, target chat chỉ đơn giản gọi `agent-chat-cli` như một binary đối với agent đang chạy locally — không có code nào được tạo vào workspace của bạn.

Đối với các agent **HTTP**, generator tạo ra:
...
- Một target Nx `<your-agent-name>-generate-client` chạy `open-api#ts-client` để tạo một TypeScript client type-safe trong `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — một adapter mỏng import generated client và kết nối nó vào `chatLoop` của `agent-chat-cli`

Script chat là một wrapper nhỏ xung quanh generated client. Khi bạn tùy chỉnh input shape của agent (ví dụ: thêm các trường mới vào `InvokeInput`), hãy cập nhật `sendMessage` của adapter để truyền các trường mới và phần còn lại sẽ hoạt động tự động.
