...
Generator cũng cung cấp một CLI chat tương tác tối giản trong `scripts/<your-agent-name>/chat.ts` và một target `<your-agent-name>-chat` tương ứng. Target này phụ thuộc vào `<your-agent-name>-serve-local`, vì vậy khi chạy nó sẽ khởi động agent locally và đưa bạn vào một terminal chat tương tác với agent:

<NxCommands commands={['run your-project:agent-chat']} />

CLI là một script TypeScript (chạy qua `tsx`) sử dụng client phù hợp với giao thức để stream các response từng token thông qua [`@clack/prompts`](https://github.com/bombshell-dev/clack):

- **HTTP**: `POST /invocations` với JSONL streaming
- **A2A**: `@a2a-js/sdk` `ClientFactory` với `sendMessageStream`
