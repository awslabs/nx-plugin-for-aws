...
:::tip
`Dockerfile` của bạn được đặt cùng với Strands Agent project của bạn. Đối với CDK, construct sử dụng `buildContexts` để build image trực tiếp từ Dockerfile này. Đối với Terraform, docker image được pre-built bằng một tag (ví dụ `my-scope-my-project-agent:latest`) được tham chiếu bởi hạ tầng Terraform của bạn.
:::
