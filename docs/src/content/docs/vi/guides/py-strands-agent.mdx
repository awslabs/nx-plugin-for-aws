...
Generator cung cấp một tùy chọn `auth` để cấu hình xác thực cho Strands Agent của bạn. Bạn có thể chọn giữa xác thực `IAM` (mặc định) hoặc `Cognito` khi tạo agent của bạn.
...
<Infrastructure>
<Fragment slot="cdk">
Construct được tạo chấp nhận một prop `identity` để cấu hình xác thực Cognito:
...
Construct `UserIdentity` được tự động tạo khi bạn chọn xác thực `Cognito`.
</Fragment>
<Fragment slot="terraform">
