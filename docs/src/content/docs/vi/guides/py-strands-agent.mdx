...
Bạn có thể định nghĩa một Pydantic model để yield dữ liệu có cấu trúc thay thế. Để làm điều này, bạn sẽ cần serialize các pydantic model được yield bởi `handle_invoke`:
...
:::caution
Lưu ý ở đây rằng server được tạo (và phương thức `serialize_stream` ở trên) hiện không tuân theo tiêu chuẩn Server-Sent Events (SSE), do đó nếu muốn Agent có thể được gọi bởi TypeScript client được tạo của chúng ta từ frontend, hiện không mong đợi các streaming event ở định dạng SSE. Vui lòng +1 [GitHub issue này](https://github.com/awslabs/nx-plugin-for-aws/issues/369) nếu điều này ảnh hưởng đến bạn để giúp chúng tôi ưu tiên giải quyết vấn đề này.
:::
