...
Khi `protocol` được đặt thành `AG-UI`, entry point sử dụng tích hợp [ag-ui-strands](https://docs.ag-ui.com/), expose agent của bạn qua giao thức AG-UI để tích hợp trực tiếp với frontend thông qua [CopilotKit](https://docs.copilotkit.ai/aws-strands):
...
Khi `protocol=AG-UI`, file `main.py` được tạo wrap Strands `Agent` của bạn trong một `ag_ui_strands.StrandsAgent` và tạo một ứng dụng FastAPI thông qua `create_strands_app()`. Ứng dụng kết quả expose một POST endpoint duy nhất stream các sự kiện [AG-UI](https://docs.ag-ui.com/) qua Server-Sent Events (SSE), cũng như `/ping` cho health check của AgentCore runtime.
...
Generator cấu hình một target có tên `<your-agent-name>-invoke` để bắt đầu một cuộc trò chuyện tương tác với agent của bạn từ terminal. Nó được cung cấp dưới dạng một script TypeScript (thông qua `tsx`) và tự động chọn giao thức phù hợp (HTTP, A2A, AG-UI) cho agent của bạn. Mặc định, nó kết nối với local server:

<NxCommands commands={['agent-invoke your-project']} />

Để trò chuyện với một agent đã triển khai thay vì local, hãy đặt `RUNTIME_CONFIG_APP_ID` thành ID ứng dụng AppConfig của workspace của bạn — CLI sẽ resolve runtime ARN và gọi deployed endpoint với các request được ký SigV4:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Đối với các agent được xác thực bằng Cognito, truyền JWT với `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

Script CLI nằm tại `scripts/<agent>/cli.ts` trong project của bạn.
