...
- **Chuyển đổi mã nguồn bằng GritQL.** Sử dụng các <Link path="/vi/guides/nx-generator">helper GritQL</Link> (`applyGritQL`, `matchGritQL`) để chỉnh sửa nguồn thay vì regex hoặc thay thế chuỗi. GritQL khớp với AST, vì vậy một pattern giữ được trên toàn bộ định dạng, khoảng trắng và dấu ngoặc kép mà bản sao của người dùng có thể đã trôi vào, trong khi khớp văn bản âm thầm bỏ lỡ code tương đương hoặc làm hỏng tệp. Sử dụng `updateJson` cho JSON và parser phù hợp cho cấu hình có cấu trúc khác.
...
Để biết luồng đầy đủ từ góc độ người dùng đang nâng cấp — bao gồm đầu ra ví dụ cho các migrations deterministic và agentic — xem <Link path="/vi/get_started/upgrading">Nâng cấp Workspace của bạn</Link>.
