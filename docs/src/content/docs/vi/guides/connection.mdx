...
Generator connection sử dụng <Link path="guides/runtime-config">Runtime Configuration</Link> để truyền các giá trị tại thời điểm triển khai (chẳng hạn như API URLs, cài đặt Cognito, và agent runtime ARNs) giữa các dự án và thành phần được tạo ra tại runtime để chúng có thể khám phá và kết nối với nhau.
:::