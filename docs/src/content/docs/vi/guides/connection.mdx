...
:::tip[Phác thảo trực quan]
Muốn thiết kế workspace của bạn trước? <Link path="get_started/graph-builder">Graph Builder</Link> cho phép bạn kéo thả các dự án và vẽ các kết nối giữa chúng, sau đó cung cấp cho bạn các lệnh để tạo tất cả.
:::
...
Generator connection sử dụng <Link path="guides/runtime-config">Runtime Configuration</Link> để truyền các giá trị thời điểm triển khai (chẳng hạn như URL API, cài đặt Cognito và ARN runtime của agent) giữa các dự án và thành phần được tạo tại runtime để chúng có thể khám phá và kết nối với nhau.
:::

:::tip[Phát triển cục bộ]
Các dự án được kết nối có thể chạy trên máy của bạn với các target `serve` và `dev`. Xem hướng dẫn <Link path="guides/local-development">Local Development</Link> để biết chi tiết.
:::
