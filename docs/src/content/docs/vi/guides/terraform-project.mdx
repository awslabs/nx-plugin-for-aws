...
<FileTree>
  ...
  - scripts Các trình trợ giúp Node được chạy bởi các target nx `bootstrap` và `init`
    - aws-config.ts Phân giải tài khoản + vùng thông qua chuỗi thông tin xác thực AWS SDK
    - bootstrap.ts Kéo/đẩy bootstrap tfstate và chạy `terraform apply`
    - init.ts Chạy `terraform init` với cấu hình backend S3
    - paths.ts Xác định vị trí workspace + project root tại thời điểm chạy
  - project.json Cấu hình dự án và các target build

</FileTree>


:::note[Module Terraform chia sẻ]
...
Trước khi triển khai cơ sở hạ tầng của bạn, bạn sẽ cần bootstrap backend trạng thái từ xa. Điều này tạo ra một S3 bucket để lưu trữ các tệp trạng thái Terraform của bạn:

<NxCommands commands={['bootstrap tf-infra']} />

:::note[Cấu hình hồ sơ AWS]
...
:::tip[Tự động phê duyệt]
Lệnh trên áp dụng kế hoạch được tạo bởi target `plan`. Hãy đảm bảo chạy `plan` trước để xem xét các thay đổi.
:::

:::note[Bí danh Deploy]
Target `deploy` cũng có sẵn như một bí danh cho `apply`. Bạn có thể sử dụng <NxCommands commands={['deploy tf-infra']} /> hoặc <NxCommands commands={['apply tf-infra']} /> thay thế cho nhau.
:::
