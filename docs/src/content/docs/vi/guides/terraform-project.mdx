...
:::note[Variable names]
Mọi tên bạn đặt ở đây phải được khai báo trong `src/variables.tf` — trình tạo khai báo `aws_region` và `environment`.
:::
</TabItem>
...
Mọi target chạy `terraform init` đều tái sử dụng một provider cache dưới `.terraform/plugin-cache` trong workspace root của bạn, vì vậy các provider được tải xuống một lần thay vì mỗi lần chạy. Mỗi dự án có thư mục riêng ở đó. Xem [tài liệu Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) để biết thêm thông tin.

Target `install-providers` là thứ điền vào cache đó, và mọi target chạy `terraform init` đều phụ thuộc vào nó:

<NxCommands commands={['install-providers tf-infra']} />

Bạn thường sẽ không tự chạy nó. Nó tồn tại vì hai lần chạy `terraform init` cùng điền vào một cache có thể mỗi cái tính toán một hash khác nhau cho cùng một provider, sau đó terraform sẽ từ chối so với `.terraform.lock.hcl` của bạn với "the cached package ... does not match any of the checksums recorded in the dependency lock file". Tải xuống các provider một lần, trước các target cần chúng, để lại các target đó chỉ đọc từ một cache đã được làm ấm sẵn.

Đặt `TF_PLUGIN_CACHE_DIR` trong môi trường của bạn để trỏ script `init` được cung cấp đến một cache mà bạn tự quản lý — ví dụ như một volume được chia sẻ giữa các workspace. Lưu ý rằng các target `install-providers`, `test` và `validate` đọc đường dẫn của chúng từ `project.json`, vì vậy hãy thay đổi nó ở đó nữa.
