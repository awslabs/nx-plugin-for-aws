...
Checkov chạy như một phần của cả `build` và `assemble`, vì vậy một `plan` tương tác quét hạ tầng của bạn trước khi bạn áp dụng nó.
...
`plan` phụ thuộc vào `assemble` thay vì `build`, vì vậy nó tạo ra các artifact mà các module của bạn tham chiếu — các bundle Lambda và metadata operations được tạo — và chạy Checkov, mà không cần chạy các cổng lint, test và type-check của mọi dự án.
