...
Checkov chạy như một phần của cả `build` và `package`, vì vậy một `plan` tương tác sẽ quét hạ tầng của bạn trước khi bạn áp dụng nó.
...
`plan` phụ thuộc vào `package` thay vì `build`, vì vậy nó tạo ra các artifact mà các module của bạn tham chiếu — các bundle Lambda và metadata operations được tạo — và chạy Checkov, mà không cần chạy các cổng lint, test và type-check của mọi dự án.

:::caution
Bởi vì `plan` không còn chạy các test module Terraform hoặc các unit test của dự án của bạn, hãy chạy `build` để kiểm tra code của bạn trước khi áp dụng bất cứ điều gì bạn quan tâm, và đặt cổng pipeline của bạn trên nó:
