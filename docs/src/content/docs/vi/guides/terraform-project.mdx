...
<FileTree>
  ...
  - scripts Node helpers chạy bởi các target nx `bootstrap`, `bootstrap-destroy` và `init`
    ...
    - plugin-cache.ts Trỏ `terraform init` đến shared provider cache
...
Mọi target chạy `terraform init` đều chia sẻ một provider cache tại `.terraform/plugin-cache` trong workspace root của bạn, thông qua [`TF_PLUGIN_CACHE_DIR`](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) của Terraform. Các provider được tải xuống một lần và được liên kết từ đó sau này, điều này quan trọng nhất đối với target `test`: nó giữ thư mục làm việc của nó dưới `dist`, vì vậy một `dist` đã được dọn dẹp hoặc cache miss sẽ tải lại AWS provider — vài trăm megabyte — trong mỗi lần chạy.

Thư mục được tạo tự động trước mỗi `terraform init`, và `.terraform` được gitignore. Không giống như `.nx/cache`, nó không bị xóa bởi `nx reset`, vì vậy một reset không loại bỏ các provider. Trong CI, cache `.terraform/plugin-cache` giữa các job để có được cùng một tiết kiệm ở đó.

`env` của Nx được ưu tiên hơn shell của bạn, vì vậy việc đặt `TF_PLUGIN_CACHE_DIR` trong môi trường của bạn không thay đổi nơi các target này tìm kiếm. Để trỏ chúng đến nơi khác — một vị trí được chia sẻ giữa nhiều workspace, chẳng hạn — thay đổi `TF_PLUGIN_CACHE_DIR` trong `project.json` của dự án của bạn, và đường dẫn trong `scripts/plugin-cache.ts` cho các target `bootstrap`, `bootstrap-destroy` và `init`. Các đường dẫn trong `project.json` là tương đối với `cwd` của target, vì Nx không mở rộng `{workspaceRoot}` bên trong `env`.

:::note[Lock Files]
Cache không thay đổi phiên bản provider nào bạn nhận được: `.terraform.lock.hcl` vẫn ghim chúng, và Terraform ghi lại cùng các hash cho dù provider đến từ cache hay registry. Commit lock file như thường lệ.
:::
