...
Mọi target chạy `terraform init` đều chia sẻ một provider cache tại `.terraform/plugin-cache` trong workspace root của bạn. Xem [tài liệu Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) để biết thêm thông tin.
