...
Target `fmt` kiểm tra xem code Terraform của bạn có được định dạng hay không, và thất bại nếu không. Nó chạy như một phần của `build`:

<NxCommands commands={['fmt tf-infra']} />

Để định dạng lại code của bạn, chạy với cấu hình `fix`:

<NxCommands commands={['fmt tf-infra --configuration=fix']} />

Target `lint` điều phối kiểm tra định dạng, vì vậy `run-many` tiếp cận các dự án Terraform của bạn cùng với các dự án TypeScript và Python:

<NxCommands commands={['run-many --target lint --all --configuration=fix']} />

:::tip[Why the check is the default]
`terraform fmt` ghi đè các tệp mà nó đọc — chính những tệp mà target `fmt` khai báo là cache inputs của nó. Ghi từ target cơ sở sẽ thay đổi cache key của chính nó trên mỗi lần chạy, vì vậy nó không bao giờ có thể đăng ký cache hit. Thay vào đó, kiểm tra giữ cho target có thể cache được, phản ánh cách các target `format` của TypeScript và Python hoạt động.
:::

Để bỏ qua định dạng và linting trong quá trình phát triển, sử dụng cấu hình `skip-lint`:
