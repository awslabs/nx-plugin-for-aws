...
- **Kết nối** — `BaseModel.Meta` đặt `host`, `region`, `aws_access_key_id`, và `aws_secret_access_key` từ `config.json` khi `is_local()` là `True`, trỏ PynamoDB đến phiên bản DynamoDB cục bộ. Trong AWS, các giá trị này không được đặt để PynamoDB sử dụng chuỗi thông tin xác thực mặc định.
