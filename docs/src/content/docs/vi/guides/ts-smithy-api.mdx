...
Để gọi API của bạn từ một React website, bạn có thể sử dụng trình tạo <Link path="guides/api-connection/react-smithy">`api-connection`</Link>, cung cấp tạo client an toàn kiểu từ mô hình Smithy của bạn.