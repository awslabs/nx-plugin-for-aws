...
Construct `UserIdentity` có thể được tạo bằng trình tạo <Link path="/guides/react-website-auth">`ts#website#auth`</Link>
:::
</OptionFilter>
...
Module API đặt zip triển khai Lambda của nó trong một S3 asset bucket được chia sẻ — xem <Link path="/guides/terraform-project">hướng dẫn cơ sở hạ tầng Terraform</Link> để biết chi tiết. Khởi tạo module `core/asset-bucket` một lần cho mỗi triển khai và truyền đầu ra `bucket_name` của nó vào mọi module API / Lambda qua đầu vào `asset_bucket_name`:
