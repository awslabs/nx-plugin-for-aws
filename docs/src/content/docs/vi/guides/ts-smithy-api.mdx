...
CLI được giải quyết bởi [mise](https://mise.jdx.dev/), được thêm vào workspace của bạn như một dev dependency, vì vậy không có gì cần cài đặt — nó tải xuống và lưu vào bộ nhớ cache phiên bản đã được ghim lần đầu tiên bạn build.

:::caution[Windows]
`mise` không xuất bản gói Windows lên npm, vì vậy hãy làm theo [hướng dẫn cài đặt Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) để tự cài đặt trước khi build.
:::

:::tip[Building in CI]
`mise` đọc metadata phát hành của CLI từ GitHub API, có giới hạn tốc độ không xác thực được chia sẻ theo địa chỉ IP và dễ dàng bị cạn kiệt trên CI runner — việc tải xuống sau đó sẽ thất bại với `403 Forbidden`. Đặt `GITHUB_TOKEN` trong môi trường build của bạn để nâng giới hạn này. Các lần build tiếp theo sẽ sử dụng bộ nhớ cache cục bộ và không thực hiện bất kỳ yêu cầu nào.
:::
