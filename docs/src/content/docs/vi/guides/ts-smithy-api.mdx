...
Trên macOS và Linux, CLI được giải quyết bởi [mise](https://mise.jdx.dev/), mà quá trình build tải về theo yêu cầu, vì vậy không có gì cần cài đặt — nó tải xuống và lưu vào bộ nhớ cache phiên bản đã được ghim lần đầu tiên bạn build. Trên Windows, CLI là một điều kiện tiên quyết mà bạn tự cài đặt, và việc nâng cấp workspace không nâng cấp nó — xem <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> và <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> trong hướng dẫn dự án Smithy.
