...
:::note[Dự án được tạo]
Dự án này được tạo bằng trình tạo <Link path="/guides/typescript-project">`ts#project`</Link> và do đó cấu hình các target build giống nhau.
:::
</Fragment>
...
:::note[Dự án được tạo]
Dự án này được tạo bằng trình tạo <Link path="/guides/terraform-project">`terraform#project`</Link> và do đó cấu hình các target build giống nhau.
:::
</Fragment>
</Infrastructure>
...
Khi API của bạn được bảo vệ bởi xác thực, các thao tác của bạn thường cần biết ai đang gọi. Cách tiếp cận được khuyến nghị là giải quyết danh tính của người gọi một lần trong handler và truyền nó qua [ngữ cảnh dịch vụ](#ngữ-cảnh-dịch-vụ) để các thao tác cụ thể sử dụng.
...
Bạn không cần `aws-jwt-verify` hoặc bất kỳ thư viện xác minh JWT nào khác ở đây — bộ ủy quyền Cognito User Pools của API Gateway đã xác minh chữ ký, nhà phát hành, phạm vi và thời gian hết hạn trước khi Lambda của bạn chạy. Nếu bất kỳ kiểm tra nào trong số đó thất bại, API Gateway trả về `401 Unauthorized` và handler của bạn không bao giờ được gọi.
:::
</OptionFilter>
...
`getIdentity` là một trường bắt buộc trên `ServiceContext`, và như [lưu ý ở trên](#ngữ-cảnh-dịch-vụ) ghi chú, ngữ cảnh được xây dựng trong **cả hai** điểm vào — vì vậy `src/local-server.ts` cũng cần nó. Không có bộ ủy quyền API Gateway ở phía trước máy chủ cục bộ, vì vậy hãy cung cấp một danh tính stub cho phát triển cục bộ:
...
Trên macOS và Linux, CLI được giải quyết bởi [mise](https://mise.jdx.dev/), mà quá trình build tải theo yêu cầu, vì vậy không có gì để cài đặt — nó tải xuống và lưu vào bộ nhớ cache phiên bản được ghim lần đầu tiên bạn build.
...
`mise` không xuất bản package Windows lên npm, vì vậy trên Windows, Smithy CLI là một điều kiện tiên quyết bạn tự cài đặt. Cài đặt nó một lần theo [hướng dẫn cài đặt Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) (ví dụ `winget install smithy` hoặc `scoop install smithy`), và đảm bảo `smithy` nằm trên `PATH` của bạn. Một dự án Smithy được tạo trên Windows chạy `smithy` trực tiếp thay vì thông qua `mise`.

:::caution[Nâng cấp CLI]
Vì CLI được cài đặt toàn cục thay vì được ghim trong workspace của bạn, việc nâng cấp workspace bằng `nx migrate` **không** nâng cấp nó. Khi một migration di chuyển các phụ thuộc Smithy về phía trước, hãy tự nâng cấp Smithy CLI toàn cục của bạn lên phiên bản phù hợp.
:::

Ngoài ra, phát triển bên trong [WSL](https://learn.microsoft.com/en-us/windows/wsl/install), nơi quá trình build chạy đường dẫn Linux và `mise` giải quyết CLI cho bạn — không có gì để cài đặt.

Một dự án được tạo trên Windows commit một target `compile` gọi `smithy` trực tiếp, vì vậy bất kỳ ai khác làm việc trên nó — bao gồm trên macOS hoặc Linux — cũng cần Smithy CLI trên `PATH` của họ. Để các máy đó giải quyết CLI thông qua `mise` thay thế, chuyển target sang lệnh `mise` như [mô tả bên dưới](#chọn-cách-cli-được-giải-quyết).
...
:::tip[Tải lại nóng]
Máy chủ cục bộ sẽ không chỉ tải lại nóng khi bạn thực hiện thay đổi TypeScript đối với mã backend của bạn, mà còn sẽ tải lại khi bạn thực hiện thay đổi đối với dự án mô hình Smithy của bạn, cho phép bạn liên tục lặp lại trên mô hình Smithy và máy chủ của bạn cùng nhau.
:::
...
Nếu bạn đã chọn xác thực `Cognito`, bạn sẽ cần cung cấp thuộc tính `identity` cho construct API:
...
Kết quả của Authorizer được lưu vào bộ nhớ cache trong 5 phút, được khóa trên header `Authorization`, vì vậy một token bị thu hồi có thể được chấp nhận cho đến khi kết quả được lưu trong bộ nhớ cache của nó hết hạn. Giảm thời lượng bộ nhớ cache trong cơ sở hạ tầng được tạo nếu bạn cần thu hồi nhanh hơn.
:::
</OptionFilter>
</Fragment>
<Fragment slot="terraform">
...
<Snippet name="api/custom-domain" parentHeading="Tên miền tùy chỉnh" />

### Tích hợp

<Infrastructure>
<Fragment slot="cdk">
:::caution[Tên thao tác là camelCase trong CDK]
Các thao tác Smithy được khai báo trong PascalCase (`Echo`), và backend của bạn xuất chúng dưới tên đó. Các khóa tích hợp CDK là dạng **camelCase** của cùng một tên, vì vậy một thao tác `Echo` là `echo` ở mọi nơi trong stack của bạn:
...
<Fragment slot="terraform">
:::note[Tên thao tác là camelCase trong Terraform]
Các đầu ra được khóa theo thao tác của module được tạo và `operations.json` của nó sử dụng dạng **camelCase** của tên thao tác Smithy, vì vậy một thao tác Smithy `Echo` được khóa `echo`:
...
Bạn sẽ cần chạy một build bất cứ khi nào bạn thay đổi mô hình Smithy của mình để đảm bảo các kiểu được sử dụng bởi CDK construct được cập nhật.

<PackageManagerShortCommand commands={["build"]} />
:::

:::tip[Tải lại nóng]
Nếu bạn đang tích cực làm việc trên cả cơ sở hạ tầng CDK và Smithy API của bạn cùng nhau, bạn có thể sử dụng [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) để tái tạo các kiểu này mỗi khi bạn thực hiện thay đổi mô hình:
...
Chúng tôi không hỗ trợ các tích hợp an toàn kiểu cho Terraform, và do đó không có target tạo mã nào được cấu hình nếu bạn đã chọn Terraform cho `iac` của bạn.
:::
</Fragment>
</Infrastructure>
...
<CardGrid>
  <ConnectionCard
    ...
    description="Gọi một Smithy API từ một website React"
  ...
  <ConnectionCard
    ...
    description="Kết nối một Smithy API với cơ sở dữ liệu quan hệ Aurora"
  ...
  <ConnectionCard
    ...
    description="Kết nối một Smithy API với bảng DynamoDB"
