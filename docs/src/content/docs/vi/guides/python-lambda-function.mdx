...
Bạn đã chọn `Any` cho `eventSource`, vì vậy decorator `@event_parser` không được áp dụng và handler nhận một event dict không có kiểu. Tạo lại với một `eventSource` cụ thể nếu bạn muốn xác thực Pydantic và truy cập trường có kiểu.
</OptionFilter>
