...
- **`iac.provider`** — nhà cung cấp infrastructure-as-code mặc định (`CDK` hoặc `Terraform`) được sử dụng bởi các trình tạo phát ra cơ sở hạ tầng (ví dụ: `ts#infra`, `ts#trpc-api`, `py#fast-api`). Các trình tạo chấp nhận flag `--iac` mặc định là `Inherit`, đọc giá trị này.
