...
Sau khi có dự án plugin, bạn có thể thêm các generator bằng cách sử dụng generator <Link path="/guides/nx-generator">`ts#nx-generator`</Link>:
...
Đọc <Link path="/guides/nx-generator">hướng dẫn `ts#nx-generator`</Link> để biết chi tiết về cách triển khai các generator.
:::
...
- Các manifest `generators.json`, `executors.json` và `migrations.json` mà Nx đọc từ một plugin đã xuất bản
- Các file tài liệu và giấy phép

## Tích hợp MCP Server

Plugin cấu hình một MCP server bằng cách sử dụng generator <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>.
...
Để biết thêm chi tiết về việc viết MCP Servers, hãy tham khảo <Link path="/guides/ts-mcp-server">hướng dẫn `ts#mcp-server`</Link>.

## Xuất bản Plugin của bạn

### Chuẩn bị để Xuất bản

1. **Cập nhật Thông tin Package**: Chỉnh sửa [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) của plugin với tên, phiên bản, mô tả và thông tin repository phù hợp. Cũng xóa `"private": true`.

2. **Build Package**:
   <NxCommands commands={['package your-plugin']} />

   Package sẽ nằm trong `dist/<thư mục gốc dự án của plugin>/package` — đối với một plugin được tạo với `--name=nx-plugin --directory=tools`, đó là `dist/tools/nx-plugin/package`.
