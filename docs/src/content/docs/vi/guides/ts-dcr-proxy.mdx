...
Nếu bạn đã có User Pool từ trình tạo <Link path="guides/react-website-auth">`ts#website#auth`</Link>, bạn có thể đặt proxy trước một máy chủ MCP cho cùng những người dùng trang web. Xem <Link path="guides/ts-dcr-proxy#reusing-a-useridentity-user-pool">Tái sử dụng UserIdentity User Pool</Link> bên dưới.
:::
...
// User pool được tạo bởi ts#website#auth cho người dùng trang web của bạn
const identity = new UserIdentity(this, 'Identity');
...
new DcrProxy(this, 'DcrProxy', {
  ...
  userPoolClientId: proxyClient.userPoolClientId,
  cognitoClientSecretArn: clientSecret.secretArn,
  // Construct UserIdentity luôn tạo một domain
  cognitoHostedUiBase: identity.userPoolDomain.baseUrl(),
  upstreamUrl: mcpServer.invocationUrl,
});
```
...
<Fragment slot="terraform">
```hcl
# Module user pool được tạo bởi ts#website#auth cho người dùng trang web của bạn
