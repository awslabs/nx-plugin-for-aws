...
Trình tạo tạo một <Link path="/guides/typescript-project">dự án TypeScript</Link> độc lập chứa các Lambda handler, và cơ sở hạ tầng để triển khai chúng dựa trên `iac` bạn đã chọn.
...
Nếu bạn đã có một User Pool từ trình tạo <Link path="/guides/react-website-auth">`ts#website#auth`</Link>, bạn có thể đặt trước một máy chủ MCP cho cùng những người dùng website. Xem <Link path="/guides/ts-dcr-proxy#tai-su-dung-mot-useridentity-user-pool">Tái sử dụng một UserIdentity User Pool</Link> bên dưới.
:::

### Đặt trước một Máy chủ MCP

Để đặt trước một máy chủ MCP được tạo bằng trình tạo <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> (hoặc <Link path="/guides/py-mcp-server">`py#mcp-server`</Link>) sử dụng `--auth cognito`, truyền **cùng** User Pool và App Client cho cả máy chủ MCP và proxy, và sử dụng `invocationUrl` của construct máy chủ MCP làm `upstreamUrl` của proxy.
