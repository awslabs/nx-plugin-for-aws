...
- \<dcr-proxy-name>
  ...
  - rolldown.config.ts Cấu hình bundle, một entry cho mỗi handler
  - README.md README của dự án
  - package.json Khai báo các dependency của handler
  - project.json Thêm một target bundle cho mỗi handler
  - tsconfig.json / tsconfig.lib.json / tsconfig.spec.json Cấu hình TypeScript
