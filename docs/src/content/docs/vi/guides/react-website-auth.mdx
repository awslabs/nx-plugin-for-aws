...
Cognito từ chối xác minh số điện thoại dựa trên SMS trừ khi SMS cũng là một phương thức MFA được bật bất cứ khi nào MFA không bị tắt, trong cả CDK và Terraform. Do đó, việc vô hiệu hóa `mfaSecondFactor.sms` / `mfa_second_factor_sms` trong khi `mfa` đang bật cũng vô hiệu hóa xác minh số điện thoại dựa trên SMS cho user pool.
:::
...
Construct `UserIdentity` tự động thêm <Link path="guides/react-website#runtime-configuration">Runtime Configuration</Link> cần thiết để đảm bảo rằng website của bạn có thể trỏ đến Cognito User Pool chính xác để xác thực.
...
Module user identity tự động thêm <Link path="guides/react-website#runtime-configuration">Runtime Configuration</Link> cần thiết để đảm bảo rằng website của bạn có thể trỏ đến Cognito User Pool chính xác để xác thực.
</Fragment>
</Infrastructure>
