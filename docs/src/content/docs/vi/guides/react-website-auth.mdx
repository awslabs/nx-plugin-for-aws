...
Không giống như các tùy chọn `enableWaf` và MFA bên dưới, chế độ thực thi không được hiển thị dưới dạng prop hoặc biến, vì vậy chỉnh sửa này áp dụng cho mọi `UserIdentity` trong workspace của bạn.
...
`mfa` chấp nhận `"OFF"` / `"OPTIONAL"` / `"ON"`. `mfa_second_factor_sms` và `mfa_second_factor_otp` bật hoặc tắt từng phương thức yếu tố thứ hai một cách độc lập; chúng không có hiệu lực khi `mfa` là `"OFF"`.
</Fragment>
</Infrastructure>

:::caution[SMS MFA được liên kết với xác minh số điện thoại]
Cognito từ chối xác minh số điện thoại dựa trên SMS trừ khi SMS cũng là một phương thức MFA được bật bất cứ khi nào MFA không tắt, trong cả CDK và Terraform. Do đó, việc tắt `mfaSecondFactor.sms` / `mfa_second_factor_sms` trong khi `mfa` đang bật cũng sẽ tắt xác minh số điện thoại dựa trên SMS cho user pool.
:::

#### Web Application Firewall (WAF)

Theo mặc định, User Pool được liên kết với một [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) Web ACL sử dụng các nhóm quy tắc được quản lý `AWSManagedRulesCommonRuleSet` và `AWSManagedRulesKnownBadInputsRuleSet`. Bạn có thể tắt điều này nếu muốn quản lý Web ACL của riêng mình hoặc không yêu cầu:
...
User Pool client được tạo cho phép URL phân phối CloudFront của bạn (và bất kỳ tên miền tùy chỉnh nào được cấu hình trên đó) làm URL callback/logout OAuth, cộng với các URL dev-server cục bộ và preview của mỗi website được kết nối (ví dụ: `http://localhost:4200` và `http://localhost:4300` cho website đầu tiên được tạo trong workspace) để phát triển cục bộ với pool đã triển khai. Mỗi website được tạo bằng `ts#website` được gán các cổng riêng biệt, vì vậy một workspace có nhiều website sẽ có nhiều hơn một cặp `http://localhost:<port>` được cho phép ở đây.
