...
`mfa` chấp nhận `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` trong CDK, và `"OFF"` / `"OPTIONAL"` / `"ON"` trong Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` và tương đương `otp` bật hoặc tắt từng phương thức yếu tố thứ hai một cách độc lập; chúng không có hiệu lực khi `mfa` bị tắt.

:::caution[Terraform liên kết SMS MFA với xác minh số điện thoại]
Không giống như CDK construct, AWS provider của Terraform không có cách nào để cung cấp SMS như một phương thức MFA độc lập với việc sử dụng nó để xác minh số điện thoại. Do đó, việc đặt `mfa_second_factor_sms = false` cũng vô hiệu hóa xác minh số điện thoại dựa trên SMS (`auto_verified_attributes` loại bỏ `phone_number`).
:::
