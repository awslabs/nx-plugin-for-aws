...
:::note[Callback URLs]
User Pool client được tạo cho phép `http://localhost:4200` và `http://localhost:4300` làm OAuth callback/logout URLs (để phát triển cục bộ với pool đã triển khai), cộng với URL phân phối CloudFront của bạn. Giữ danh sách cho phép này ở mức tối thiểu: đối với các triển khai production được bảo mật, hãy xem xét việc xóa các URL localhost (hoặc sử dụng một client riêng cho phát triển cục bộ) sau khi bạn không còn chạy cục bộ với User Pool đã triển khai nữa.
:::
