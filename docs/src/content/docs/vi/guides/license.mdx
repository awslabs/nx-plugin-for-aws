...
Để vô hiệu hóa vĩnh viễn, xóa khóa `license.dependencies` khỏi cấu hình của bạn trong `aws-nx-plugin.config.mts`. Bạn cũng có thể chạy lại generator `license` với `--dependencyCheck=false` để scaffold mà không có nó.