...
Quản lý cấp phép trong toàn bộ workspace của bạn: đồng bộ các tệp `LICENSE` và header mã nguồn cho code của riêng bạn (`license.source`), đồng thời kiểm tra rằng mọi dependency tuân thủ danh sách giấy phép được phép (`license.dependencies`).
...
<FileTree>
  ...
  - aws-nx-plugin.config.mts Cấu hình cho đồng bộ nguồn giấy phép (`license.source`) và kiểm tra dependency (`license.dependencies`)
</FileTree>
...
Đồng bộ nguồn giấy phép được kích hoạt bởi sự hiện diện của khóa `license.source` trong cấu hình của bạn. Để vô hiệu hóa nó:
...
2. Nếu bạn cũng muốn xóa hoàn toàn sync generator, hãy xóa generator `@aws/nx-plugin:license#sync` khỏi `targetDefaults.lint.syncGenerators`
...
Các `inputs` được tính toán cho workspace của bạn: chỉ các lockfile thực sự có mặt mới được bao gồm, cùng với `aws-nx-plugin.config.mts`, cộng thêm một glob `{workspaceRoot}/**/uv.lock` khi kiểm tra dependency Python được bật (tức là một Python collector được cấu hình).

Bạn có thể chạy kiểm tra trực tiếp:

<NxCommands commands={['license-check']} />

Kết quả được cache dựa trên các lockfile và `aws-nx-plugin.config.mts` của bạn — việc chạy lại sẽ tức thì khi không có gì thay đổi.
...
Kiểm tra giấy phép dependency chạy tự động mỗi khi bạn `lint` hoặc `build` bất kỳ dự án nào trong workspace của bạn. Generator `license` kết nối target `lint` của mỗi dự án để phụ thuộc vào target `license-check` gốc, và các project generator (`ts#*` và `py#*`) cũng làm điều tương tự khi chúng chạy — vì vậy kiểm tra được kết nối bất kể thứ tự các generator được chạy.

Điều này có nghĩa là bạn không cần chạy kiểm tra một cách rõ ràng, mặc dù bạn vẫn có thể làm như vậy với target NxCommands `license-check`:

<NxCommands commands={['license-check']} />

Kết nối là một `dependsOn` cross-project trên target `lint` của mỗi dự án trỏ đến target `license-check` gốc. Để bỏ qua kiểm tra trong quá trình lint hoặc build, hãy đặt biến môi trường `LICENSE_DEPENDENCY_CHECK=skip`:
...
`license.dependencies` chấp nhận một callback `onDependency` tùy chọn được gọi một lần cho mỗi dependency được phát hiện, bất kể nó pass hay fail kiểm tra. Nó nhận `{ package, spdx }`, trong đó `package` là tên package và `spdx` là biểu thức giấy phép SPDX đã được phân giải. `spdx` của một ngoại lệ được ưu tiên hơn giấy phép được khai báo thô, và `spdx` có thể là một chuỗi rỗng nếu không có giấy phép nào được khai báo.
...
Kiểm tra giấy phép dependency được kích hoạt bởi sự hiện diện của khóa `license.dependencies` trong cấu hình của bạn.

Để vô hiệu hóa kiểm tra cho một lần chạy duy nhất, đặt biến môi trường `LICENSE_DEPENDENCY_CHECK=skip`:

<PackageManagerShortCommand commands={["LICENSE_DEPENDENCY_CHECK=skip lint"]} />

Để vô hiệu hóa vĩnh viễn, xóa khóa `license.dependencies` khỏi cấu hình của bạn trong `aws-nx-plugin.config.mts`. Bạn cũng có thể chạy lại generator `license` với `--dependencyCheck=false` để scaffold mà không có nó — cờ generator vẫn được đặt tên là `--dependencyCheck` và kiểm soát việc generator có ghi block `dependencies` hay không.