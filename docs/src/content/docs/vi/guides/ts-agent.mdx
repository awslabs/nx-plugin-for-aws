...
Một target `<your-agent-name>-serve` cũng được tạo, chạy agent với hạ tầng đã triển khai của bạn và do đó yêu cầu `RUNTIME_CONFIG_APP_ID` phải được thiết lập. Xem hướng dẫn <Link path="guides/local-development">Local Development</Link> để biết sự khác biệt giữa `dev` và `serve`.
