...
Nếu bạn chọn `None` cho `computeType`, không có CDK construct hoặc Terraform module nào được tạo ra — Agent chỉ có thể chạy locally. Tùy chọn `auth` sẽ bị bỏ qua trong chế độ này vì không có endpoint được host để xác thực.
</OptionFilter>
