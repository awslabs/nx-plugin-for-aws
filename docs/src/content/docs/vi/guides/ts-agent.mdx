...
<Snippet name="agent/dev-vs-serve" parentHeading="Phát triển cục bộ" />
...
Agent của bạn được tự động cấu hình với khả năng quan sát sử dụng [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT).
...
:::caution[Sử dụng `-dev`, không phải `-serve`]
Sử dụng `<your-agent-name>-dev` cho việc này, không phải `<your-agent-name>-serve`. `-serve` chạy đường dẫn mã đã triển khai, vì vậy nó cần `RUNTIME_CONFIG_APP_ID` được thiết lập — xem <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Nếu không có nó, mọi lần gọi sẽ thất bại với `RUNTIME_CONFIG_APP_ID is not set`.
:::
