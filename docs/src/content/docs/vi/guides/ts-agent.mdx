...
Để gọi Strands Agent của bạn từ một React website, bạn có thể sử dụng <Link path="/guides/connection/react-ts-agent">`connection` generator</Link>, generator này tự động thiết lập tRPC WebSocket client với xác thực đúng (IAM hoặc Cognito).
