...
Generator cấu hình một target Nx `<your-agent-name>-chat` đưa bạn vào một terminal chat tương tác với agent của bạn, được hỗ trợ bởi [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli).

Target chat chạy độc lập — nó **không** khởi động agent cho bạn. Mặc định nó kết nối với agent đang chạy locally của bạn, vì vậy hãy khởi động `<your-agent-name>-serve-local` trước (trong một terminal riêng):
