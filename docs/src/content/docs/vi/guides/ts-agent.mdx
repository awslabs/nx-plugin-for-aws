...
File `index.ts` được tạo bọc Strands `Agent` của bạn trong một [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands) `StrandsAgent` và xây dựng một Express app. App kết quả expose một POST endpoint duy nhất stream các sự kiện [AG-UI](https://docs.ag-ui.com/) qua Server-Sent Events (SSE), cũng như `/ping` cho health check của AgentCore runtime.
