...
Nếu bạn muốn mỗi người dùng chỉ truy cập các cuộc hội thoại của riêng họ, hãy ghi lại ai sở hữu mỗi session trong ứng dụng của bạn và kiểm tra quyền sở hữu đó khi vào. Session ID sau đó là thứ mà ứng dụng của bạn phát hành thay vì thứ mà người gọi chọn.

**1. Tạo session thông qua API của bạn**

Thêm một thủ tục bắt đầu một cuộc hội thoại. Nó tự tạo session ID, giải quyết người gọi từ context yêu cầu của chính nó, và lưu trữ cặp đó — ví dụ trong một bảng được tạo bằng generator <Link path="guides/py-dynamodb">`py#dynamodb`</Link>:
...
API có thể ở bất kỳ ngôn ngữ nào; ví dụ trên sử dụng generator <Link path="guides/trpc">`ts#trpc-api`</Link>.

Giải quyết người gọi từ context yêu cầu đã được xác minh thay vì từ một trường đầu vào, vì vậy nó không thể được cung cấp bởi trình duyệt. <Link path="guides/trpc">Implementing Custom Middleware</Link> cho thấy cách thực hiện điều này cho cả API được xác thực bằng IAM và Cognito.

Một thủ tục thứ hai truy vấn index đó cho phép UI cung cấp cho người dùng các cuộc hội thoại của chính họ mà không bao giờ thấy của người khác.

**2. Xác thực quyền sở hữu trong agent**

Agent của bạn đã nhận được session ID, và với `auth=cognito` nó cũng nhận được JWT của người gọi — trình ủy quyền AgentCore Runtime xác thực nó và hạ tầng được tạo ra cho phép header `Authorization` đi qua code của bạn. Tra cứu session, so sánh chủ sở hữu của nó với người gọi, và từ chối yêu cầu nếu chúng khác nhau:
...
:::tip[Tại sao xác thực trong agent cũng như trong API]
API quyết định ai sở hữu một session; agent thực thi nó. Kiểm tra ở cả hai nơi có nghĩa là một session ID bị rò rỉ — được sao chép từ trình duyệt, hoặc được sử dụng lại từ một cuộc hội thoại cũ — vẫn bị từ chối trong lần gọi tiếp theo, và bất kỳ client tương lai nào của agent đều kế thừa cùng một quy tắc.
:::

:::caution[Session cũng chọn microVM]
Một session ID chọn [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) phục vụ yêu cầu, vì vậy hệ thống tệp, `/tmp` và bộ nhớ cache trong tiến trình của nó được chia sẻ bởi mọi yêu cầu giải quyết đến session đó. Điều này đáng chú ý nếu agent hoặc công cụ của nó ghi vào đĩa cục bộ: hai người gọi gửi cùng một session ID chia sẻ các tài nguyên đó ngay cả khi các cuộc hội thoại được lưu trữ của họ là riêng biệt.

Tạo các session ID không rõ ràng trong API của bạn tránh điều này, vì người gọi không thể đến một session mà nó không được cung cấp. Lấy một từ các giá trị do người dùng cung cấp — tên cuộc hội thoại, email, tên trò chơi — có thể dự đoán được và cho phép hai người gọi va chạm.
:::
