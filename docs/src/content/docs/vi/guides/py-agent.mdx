...
Entry point expose agent của bạn qua giao thức A2A (Strands sử dụng [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent); LangChain wrap graph trong một executor [`a2a-sdk`](https://a2a-protocol.org/)), được mount lên một ứng dụng FastAPI:
...
Hầu hết người dùng sẽ không cần sửa đổi file này; chỉnh sửa `agent.py` để thay đổi các công cụ hoặc system prompt. A2A server điền thông tin agent card (`/.well-known/agent-card.json`) từ `name` và `description` của agent.
</OptionFilter>
