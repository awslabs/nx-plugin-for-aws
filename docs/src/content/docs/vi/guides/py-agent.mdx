...
Định nghĩa công cụ, sau đó thêm nó vào danh sách `tools` bên trong `get_agent()`:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
Strands cung cấp một bộ sưu tập các công cụ dựng sẵn thông qua gói `strands-agents-tools`, mà generator đã thêm vào `pyproject.toml` của dự án bạn. Import các công cụ bạn muốn và thêm chúng vào `get_agent()`:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain cung cấp một hệ sinh thái lớn các [công cụ và tích hợp](https://docs.langchain.com/oss/python/integrations/tools), mỗi công cụ được phân phối trong gói riêng của nó. Generator không thêm những công cụ này, vì vậy hãy cài đặt công cụ bạn cần trước — ví dụ `langchain-community`, nơi công cụ tìm kiếm bên dưới đến từ:

<NxCommands commands={['run <project-name>:add langchain-community']} />

Sau đó import các công cụ và thêm chúng vào `get_agent()`:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
Agent được tạo ra sử dụng mô hình Strands mặc định trên Amazon Bedrock. Để cấu hình nó, truyền một `model` vào `Agent`. Xem [tài liệu Strands về nhà cung cấp mô hình](https://strandsagents.com/docs/user-guide/concepts/model-providers/) để biết các nhà cung cấp có sẵn và các tùy chọn của chúng:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
Các agent LangChain sử dụng mô hình [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse). Agent được tạo ra đọc id mô hình và region từ các biến môi trường `MODEL_ID` và `AWS_REGION`, mà hạ tầng đặt cho agent đã triển khai. Để cấu hình mô hình thêm, thêm các đối số nơi nó được khởi tạo trong `get_agent()`:
...
Agent lắng nghe trên một cổng được gán từ pool workspace khi nó được tạo ra. Đọc nó từ `metadata.ports` trong `project.json` của dự án, hoặc từ cờ `--port` trong lệnh của target `<your-agent-name>-dev`. Các ví dụ dưới đây sử dụng `8081`, cổng mà một agent HTTP đầu tiên được gán trong một workspace mới.

Một target `<your-agent-name>-serve` cũng được tạo ra, chạy agent đối với hạ tầng đã triển khai của bạn và do đó yêu cầu `RUNTIME_CONFIG_APP_ID` được đặt. Xem hướng dẫn <Link path="guides/local-development">Phát triển Cục bộ</Link> để biết sự khác biệt giữa `dev` và `serve`.
...
- `scripts/<your_agent_name>_openapi.py` — một script nhỏ xuất đặc tả OpenAPI của agent (được đặt tên với tên agent của bạn trong `snake_case`)
...
Sau đó gửi một yêu cầu POST đến `/invocations` trên cổng mà agent cục bộ của bạn đang chạy. Thay thế cổng được gán cho agent của bạn — đọc nó từ `metadata.ports` trong `project.json` của dự án, hoặc từ cờ `--port` trong lệnh của target `<your-agent-name>-dev`. Một agent HTTP đầu tiên trong một workspace mới được gán `8081`:
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'vi'}/guides/connection/py-agent-mcp`}
    source="strands"
    sourceBadge="python"
    target="mcp"
  />
