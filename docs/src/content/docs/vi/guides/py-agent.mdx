...
Vì adapter `ag-ui-strands` của AG-UI nhân bản một `Agent` template cho mỗi conversation thread, `get_session_manager` được kết nối như một `session_manager_provider` trên `StrandsAgentConfig` được truyền cho `StrandsAgent` trong `main.py`, vì vậy một `SessionManager` mới được resolve cho mỗi thread thay vì được gắn vào agent template được chia sẻ.
</OptionFilter>

<OptionFilter when={{ protocol: ['http', 'a2a'] }} description="HTTP/A2A session wiring">
Vì `with_session_id` đã cache một instance `Agent` cho mỗi session, `get_session_manager` được gọi trực tiếp bên trong lời gọi `Agent(session_manager=get_session_manager())` của `get_agent` trong `agent.py`.
</OptionFilter>
...
- **`s3`**: Agent đã triển khai sử dụng một `S3CheckpointSaver` với session bucket được cung cấp, lưu trữ các checkpoint và pending write dưới prefix `checkpoints/`. Class này (được điều chỉnh từ [`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3), giấy phép MIT) nằm trong `s3_checkpoint_saver_langchain.py` trong project agent-connection được chia sẻ.
...
- **`in-memory`** (mặc định): Không có table hoặc bucket nào được cung cấp. Trạng thái hội thoại chỉ được giữ trong bộ nhớ trong suốt thời gian tồn tại của process đang chạy và không tồn tại qua các lần khởi động lại hoặc scale-in.
