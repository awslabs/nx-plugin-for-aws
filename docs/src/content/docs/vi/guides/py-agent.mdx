...
Agent lắng nghe trên một cổng được gán từ pool workspace khi nó được tạo ra. Đọc nó từ `metadata.ports` trong `project.json` của dự án, hoặc từ cờ `--port` trong lệnh của target `<your-agent-name>-dev`. Các ví dụ dưới đây sử dụng `8081`, cổng mà một HTTP agent đầu tiên được gán trong một workspace mới.
...
- `scripts/<your_agent_name>_openapi.py` — một script nhỏ xuất đặc tả OpenAPI của agent (được đặt tên theo tên agent của bạn trong `snake_case`)
...
Sử dụng `<your-agent-name>-dev` cho việc này, không phải `<your-agent-name>-serve`. `-serve` chạy đường dẫn mã đã triển khai, vì vậy nó cần `RUNTIME_CONFIG_APP_ID` được đặt — xem <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Nếu không có nó, streaming đã bắt đầu vào thời điểm agent cố gắng giải quyết lưu trữ session của nó, vì vậy yêu cầu trả về một `200` trống không có lỗi và lỗi chỉ xuất hiện trong log máy chủ.
:::
