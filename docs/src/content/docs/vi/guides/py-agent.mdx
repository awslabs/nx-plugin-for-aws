...
Đối với các Strands agent, generator sử dụng [FastAPI](https://fastapi.tiangolo.com/) để expose một HTTP server theo mặc định. Ngoài ra, bạn có thể chọn giao thức [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) để tương tác với các agent tương thích A2A khác, hoặc giao thức [AG-UI](https://docs.ag-ui.com/) để tích hợp trực tiếp với frontend thông qua [CopilotKit](https://docs.copilotkit.ai/).

:::note[LangChain chỉ hỗ trợ AG-UI]
Framework `langchain` hiện chỉ hỗ trợ giao thức `ag-ui`. Chọn framework `strands` mặc định nếu bạn cần giao thức `http` hoặc `a2a`. Các LangChain agent vẫn có thể kết nối với MCP server, AgentCore Gateway và A2A agent thông qua <Link path="/guides/connection/py-agent-mcp">`connection` generator</Link>.
:::
...
Các ví dụ dưới đây sử dụng Strands. Một LangChain agent hoạt động theo cách tương tự, sử dụng `create_agent` với decorator `@tool` từ `langchain_core.tools` và một model `ChatBedrockConverse` (các biến môi trường `MODEL_ID` và `AWS_REGION` thiết lập model và region). Xem [tài liệu LangChain](https://docs.langchain.com/oss/python/langchain/overview) để biết chi tiết.
...
Để sử dụng các MCP Server mà bạn đã tạo bằng generator <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> hoặc <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>, bạn có thể sử dụng <Link path="/guides/connection/py-agent-mcp">`connection` generator</Link>.
...
Để có hướng dẫn chuyên sâu hơn về viết Strands agent, hãy tham khảo [tài liệu Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Đối với các LangChain agent, <Link path="/guides/connection/py-agent-mcp">`connection` generator</Link> cũng kết nối các công cụ MCP server, AgentCore Gateway và A2A agent vào `create_agent`.
