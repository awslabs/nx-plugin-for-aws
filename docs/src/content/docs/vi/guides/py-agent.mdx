...
AG-UI agents có thể được kết nối với React frontend bằng cách sử dụng <Link path="/guides/connection/react-agui">`connection` generator</Link>, sẽ thiết lập các component [CopilotKit](https://docs.copilotkit.ai/) cho trải nghiệm chat phong phú.
:::
</OptionFilter>
...
Để có hướng dẫn chuyên sâu hơn về viết Strands agent, hãy tham khảo [tài liệu Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Đối với các LangChain agent, <Link path="/guides/connection/py-agent-mcp">`connection` generator</Link> cũng kết nối các công cụ từ MCP server và A2A agent vào `create_agent`.
