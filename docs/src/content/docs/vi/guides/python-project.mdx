...
Target `build` sẽ biên dịch, lint, kiểm tra và kiểm tra kiểu dự án của bạn.
...
Để bỏ qua một chẩn đoán cụ thể cho một dòng đơn, thêm một comment `# ty: ignore[<rule>]` ở cuối dòng, ví dụ:

```python
value: int = "not an int"  # ty: ignore[invalid-assignment]
```

Để cấu hình hành vi kiểm tra kiểu trong toàn bộ dự án của bạn, thêm một phần `[tool.ty]` vào `pyproject.toml` của dự án. Tham khảo [tài liệu tham khảo cấu hình ty](https://docs.astral.sh/ty/reference/configuration/) để biết các tùy chọn có sẵn.
