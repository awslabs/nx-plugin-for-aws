...
Nếu bạn chọn `None` cho `computeType`, không có CDK constructs hoặc Terraform modules nào được tạo ra — máy chủ MCP được cấu hình chỉ để sử dụng STDIO / HTTP cục bộ. Tùy chọn `auth` bị bỏ qua trong chế độ này vì không có endpoint được lưu trữ để xác thực.
</OptionFilter>
