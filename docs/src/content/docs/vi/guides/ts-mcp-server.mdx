...
Sau đó đăng ký nó trong `server.ts`:
...
### Phạm vi Dữ liệu Công cụ theo Người gọi

`createServer` nhận một `McpRequestContext` mô tả yêu cầu hiện tại. Qua Streamable HTTP, điều này bao gồm session id của AgentCore Runtime của người gọi, mà một <Link path="guides/ts-agent">agent</Link> tự động chuyển tiếp trên mọi cuộc gọi MCP đi ra.

Truyền nó cho một công cụ lưu trữ dữ liệu theo từng người gọi:
...
:::caution
Phạm vi dữ liệu được lưu trữ trên ngữ cảnh yêu cầu thay vì trên đối số công cụ. Một mô hình có thể bị thuyết phục để truyền bất kỳ giá trị nào nó thích, vì vậy các đối số như tên người dùng hoặc tên người chơi không phải là ranh giới tin cậy. Ngữ cảnh không có mặt qua STDIO, nơi không có yêu cầu.
:::
