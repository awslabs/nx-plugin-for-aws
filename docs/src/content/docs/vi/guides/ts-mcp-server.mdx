...
Generator cấu hình một target `<your-server-name>-docker` để sao chép `Dockerfile` từ thư mục nguồn máy chủ MCP của bạn vào thư mục đầu ra của bundle. Điều này đặt `Dockerfile` cùng vị trí với các artifact đã được đóng gói, cho phép CDK xây dựng Docker image trực tiếp bằng cách sử dụng `AgentRuntimeArtifact.fromAsset`.
