...
:::tip
Để chạy máy chủ MCP của bạn (và mọi phụ thuộc được kết nối) mà không cần inspector, hãy sử dụng target `dev` của dự án. Nếu bạn đã tạo nhiều component vào cùng một dự án, hãy chạy một máy chủ MCP riêng lẻ với target `<your-server-name>-dev` của nó.
:::
