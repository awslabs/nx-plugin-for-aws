...
:::note[Tên dự án]
Tên dự án Python sử dụng dấu gạch dưới, vì vậy một API được tạo với `--name=MyApi` được gọi là `my_api` (hoặc bằng tên đầy đủ của nó, `<scope>.my_api`). Cổng mà server lắng nghe được gán cho mỗi dự án — đọc nó từ target `serve` trong `project.json` của dự án của bạn.
:::
