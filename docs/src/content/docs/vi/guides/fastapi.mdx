...
- Thêm OpenAPI vendor extension `x-streaming: true` nếu bạn định sử dụng <Link path="guides/connection/react-fastapi">API Connection</Link>.
...
Để sử dụng một stream của responses, bạn có thể tận dụng <Link path="guides/connection/react-fastapi#consuming-a-stream">API Connection Generator</Link> sẽ cung cấp một phương thức type-safe để lặp qua các streamed chunks của bạn.
...
Để gọi API của bạn từ một React website, bạn có thể sử dụng generator <Link path="guides/connection/react-fastapi">`connection`</Link>.