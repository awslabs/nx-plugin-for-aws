...
Khi được đặt, client sử dụng URL Gateway đã triển khai và ký SigV4 các yêu cầu với thông tin xác thực AWS cục bộ của bạn thay vì quay lại ghép kênh cục bộ.

### Độ trung thực cục bộ

Chế độ ghép kênh cục bộ bỏ qua chính Gateway, do đó:

- **Không đánh giá chính sách Cedar.** Mọi công cụ đều hiển thị với agent bất kể chính sách. Sử dụng `GATEWAY_URL_OVERRIDE` để thực thi chính sách với Gateway đã triển khai.
