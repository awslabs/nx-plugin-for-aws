...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py Strands MCP client cho Gateway đã triển khai
      - auth/ SigV4 / chuyển tiếp session `httpx.Auth` không phụ thuộc framework
    - app/
      - \<gateway\_snake>\_client\_strands.py Wrapper client Strands cho mỗi Gateway
