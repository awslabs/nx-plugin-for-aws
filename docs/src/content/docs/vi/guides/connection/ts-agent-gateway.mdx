...
Khi triển khai (`LOCAL_DEV` không được đặt), client trỏ tới MCP endpoint của Gateway và xác thực bằng SigV4. Khi `LOCAL_DEV=true`, nó trỏ tới local gateway được khởi động bởi target `dev` của dự án Gateway, vì vậy cùng một `agent.ts` hoạt động đồng nhất trong cả hai chế độ.
...
Để chạy agent cục bộ **với Gateway đã triển khai** thay thế (ví dụ: để thực thi các chính sách Cedar), hãy sử dụng target `serve` của agent. Khi không đặt `LOCAL_DEV`, client sẽ phân giải Gateway URL đã triển khai từ cấu hình runtime và ký các yêu cầu bằng SigV4 với thông tin xác thực AWS cục bộ của bạn:
