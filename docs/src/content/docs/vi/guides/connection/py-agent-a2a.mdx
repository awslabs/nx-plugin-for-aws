...
1. Một dự án Python với component <Link path="guides/py-agent">Strands Agent</Link> (bất kỳ giao thức nào)
2. Một dự án với component Strands Agent được tạo với `--protocol=A2A` và `--auth=IAM` (có thể là <Link path="guides/ts-agent">`ts#agent`</Link> hoặc <Link path="guides/py-agent">`py#agent`</Link>)
