...
1. Một dự án Python với component <Link path="guides/py-agent">Agent</Link> (bất kỳ giao thức nào)
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_a2a\_client\_strands.py Client A2A Strands bọc cấu hình
      - auth/ SigV4 không phụ thuộc framework / chuyển tiếp session `httpx.Auth`
    - app
      - \<target\_agent\_name>\_client\_strands.py Client Strands theo từng kết nối cho mỗi agent A2A
...
Bên trong, `RemoteAgentClientStrands.create(session_id=...)` trả về một Strands `A2AAgent` được cấu hình với một `httpx.AsyncClient` ký các yêu cầu bằng SigV4 khi triển khai lên AWS, và một endpoint `http://localhost:<port>/` đơn giản khi `SERVE_LOCAL=true`.
