...
`appconfig_application_id`/`appconfig_application_arn` đến từ ứng dụng AppConfig <Link path="guides/runtime-config">cấu hình runtime</Link> được chia sẻ được khai báo một lần trong root module của bạn, không phải từ module cơ sở dữ liệu.
