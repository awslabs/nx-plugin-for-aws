...
:::caution
Khi `securityGroups` bị bỏ qua, `RuntimeNetworkConfiguration.usingVpc` tạo một security group với construct id cố định (`SecurityGroup`) trên scope đã cho. Truyền một security group rõ ràng như được hiển thị sẽ tránh xung đột construct id khi nhiều runtime (ví dụ: một agent và một MCP server) được định nghĩa trong cùng một stack.
:::
