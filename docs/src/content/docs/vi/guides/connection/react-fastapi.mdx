...
:::tip[Tự động tạo lại]
Nếu bạn đang làm việc tích cực với cả ứng dụng React và FastAPI cùng lúc, sử dụng target `dev` của ứng dụng React (một alias cho `serve-local`), nó sẽ tự động sinh lại client mỗi khi API của bạn thay đổi, cũng như hot-reload website và local FastAPI server của bạn:
