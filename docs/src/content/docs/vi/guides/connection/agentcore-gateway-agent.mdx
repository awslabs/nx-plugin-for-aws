...
Triển khai AgentCore Runtime VPC chỉ được hỗ trợ trong một số availability zone nhất định. Nếu triển khai thất bại với lỗi `subnets are in unsupported availability zones`, hãy giới hạn VPC (hoặc lựa chọn subnet) vào các zone được hỗ trợ cho region của bạn.
:::
</Fragment>
