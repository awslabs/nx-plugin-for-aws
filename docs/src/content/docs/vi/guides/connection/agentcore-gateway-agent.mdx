...
description: Đặt một agent phía sau AgentCore Gateway làm runtime target
when:
...
Generator `connection` có thể đăng ký một agent (có thể là <Link path="guides/ts-agent">TypeScript</Link> hoặc <Link path="guides/py-agent">Python</Link>) làm [AgentCore Runtime target](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-target-http-runtime.html) của một <Link path="guides/agentcore-gateway">AgentCore Gateway</Link> được tạo với `protocol: http`.
...
2. Một component agent (<Link path="guides/ts-agent">`ts#agent`</Link> hoặc <Link path="guides/py-agent">`py#agent`</Link>) được tạo với `infra: agentcore`. Cả `auth: iam` (Gateway gọi nó bằng role của chính nó) hoặc `auth: cognito` (Gateway chuyển tiếp JWT của người gọi — xem [Chuyển tiếp danh tính người gọi](#chuyen-tiep-danh-tinh-nguoi-goi-den-runtime)) đều hoạt động.

:::caution[Cognito agents cần Cognito Gateway]
Một agent `auth: cognito` được gọi bằng cách chuyển tiếp token mà Gateway đã xác thực khi vào, vì vậy nó cũng yêu cầu một Gateway được tạo với `auth: cognito` — một Gateway `auth: iam` xác thực người gọi của nó bằng SigV4 và không có bearer token để chuyển tiếp. Generator từ chối cặp đó. Một Cognito Gateway có thể đặt phía trước một agent `auth: iam`, mà nó gọi bằng role của chính nó.
:::

:::caution[TypeScript HTTP agents]
Một TypeScript `http` agent phục vụ tRPC qua WebSocket, và AgentCore Gateway không hỗ trợ WebSocket hoặc bidirectional streaming — generator từ chối kết hợp này. Hãy tạo agent với giao thức `ag-ui` thay thế.
:::
...
Đặt một agent phía sau Gateway là điều giúp agent được triển khai trong VPC có thể tiếp cận được: người gọi giao tiếp với endpoint công khai của Gateway, và Gateway tiếp cận runtime bên trong. Truyền cấu hình mạng VPC cho agent construct:
...
Sử dụng **private subnet với egress**, không phải private isolated subnet: runtime cần truy cập internet đi ra để tiếp cận Bedrock và đọc cấu hình của nó từ AWS AppConfig.

Triển khai AgentCore Runtime VPC chỉ được hỗ trợ trong một số availability zone nhất định. Nếu triển khai thất bại với `subnets are in unsupported availability zones`, hãy giới hạn VPC (hoặc lựa chọn subnet) vào các zone được hỗ trợ cho region của bạn.
:::
</Fragment>
...
:::tip[Triển khai agents vào VPC]
Đặt một agent phía sau Gateway là điều giúp agent được triển khai trong VPC có thể tiếp cận được: người gọi giao tiếp với endpoint công khai của Gateway, và Gateway tiếp cận runtime bên trong. Đặt các biến VPC của agent module:
...
Sử dụng private subnet với egress (một route đến NAT gateway), không phải private isolated subnet: runtime cần truy cập internet đi ra để tiếp cận Bedrock và đọc cấu hình của nó từ AWS AppConfig.
:::
</Fragment>
</Infrastructure>
...
Theo mặc định, Gateway ký các lệnh gọi đi ra bằng IAM role của chính nó (credential provider `GATEWAY_IAM_ROLE`), vì vậy runtime thấy danh tính của _Gateway_, không phải của người gọi. Nếu thay vào đó bạn muốn agent ủy quyền dựa trên người gọi — ví dụ để đọc các claim `sub` hoặc `scope` của người dùng — hãy đặt một agent **Cognito** phía sau một Gateway **Cognito**. Gateway sau đó chuyển tiếp JWT của người gọi đến runtime không thay đổi (credential provider `JWT_PASSTHROUGH`), và runtime xác thực lại nó.
...
:::note
JWT passthrough phù hợp với một nhà cung cấp danh tính duy nhất có token audience đã bao phủ runtime. Nếu một Gateway đặt phía trước các agent trên nhiều tenant hoặc audience, hãy sử dụng OAuth [on-behalf-of token exchange](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-building-adding-targets-authorization.html) thay thế.
:::
