...
4. Cognito Auth được thêm thông qua <Link path="/guides/react-website-auth">`ts#website#auth` generator</Link> nếu kết nối một API sử dụng Cognito hoặc IAM auth
...
Subscriptions chỉ được hỗ trợ khi tRPC API sử dụng `rest-lambda` (REST API) làm loại compute. API Gateway HTTP APIs không hỗ trợ response streaming.
:::
...
Để biết thông tin về cách định nghĩa các subscription procedure trong backend của bạn, xem <Link path="guides/trpc">hướng dẫn tRPC API generator</Link>.
...
Nếu tRPC API của bạn sử dụng xác thực `Custom` (Lambda Authorizer), client provider được tạo ra sẽ bao gồm các `headers` placeholder nơi bạn phải thêm các authorization header mà authorizer của bạn mong đợi. Tìm các comment `// TODO: Add headers required by your custom authorizer` trong file `<ApiName>ClientProvider.tsx` được tạo ra và thay thế chúng bằng logic token hoặc API key của bạn.
