...
- **`ServerlessApiGatewayRestApi`** — HTTP link với `httpSubscriptionLink` qua Server-Sent Events cho các tRPC subscription.
- **`ServerlessApiGatewayHttpApi`** — HTTP link thuần túy (không có subscription).
- **`BedrockAgentCoreRuntimeWebSocket`** — WebSocket link kết nối với AgentCore runtime URL, với handshake được ký SigV4 (IAM) hoặc JWT subprotocol được mã hóa base64url (Cognito). Runtime ARN được đọc từ `runtimeConfig.apis.<ApiName>` và chuyển đổi thành URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` một cách tự động.
...
Ngoài ra, nó cài đặt các dependencies bắt buộc:
  ...
  - `event-source-polyfill` (nếu sử dụng REST API backend, để hỗ trợ subscription)
