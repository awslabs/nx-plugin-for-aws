...
Generator kiểm tra `computeType` của backend và tạo ra client tương ứng:

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Một HTTP link với `httpSubscriptionLink` qua Server-Sent Events cho các tRPC subscription.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
Một HTTP link đơn giản. Subscriptions không được hỗ trợ vì API Gateway HTTP APIs không hỗ trợ response streaming.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Một WebSocket link kết nối đến AgentCore runtime URL, với handshake được ký SigV4 (IAM) hoặc JWT được mã hóa base64url subprotocol (Cognito). Runtime ARN được đọc từ `runtimeConfig.apis.<ApiName>` và được chuyển đổi thành URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` một cách tự động.
</TabItem>
</Tabs>
...
Ngoài ra, nó cài đặt các dependencies bắt buộc:
  ...
  - `event-source-polyfill` (nếu sử dụng backend REST API, để hỗ trợ subscription)
  - `react-oidc-context` (nếu sử dụng Cognito auth)
...
:::caution[Không có subscription trên HTTP API]
API Gateway HTTP APIs không hỗ trợ response streaming. Chọn `ServerlessApiGatewayRestApi` hoặc `BedrockAgentCoreRuntimeWebSocket` làm `computeType` backend của bạn nếu bạn cần subscriptions.
:::
</OptionFilter>
...
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Client được tạo ra sử dụng một `splitLink` định tuyến các subscription operations thông qua `httpSubscriptionLink` (Server-Sent Events) và các query/mutation thông thường thông qua `httpLink`. Subscriptions hoạt động ngay lập tức mà không cần cấu hình bổ sung.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Client được tạo ra sử dụng `wsLink` kết nối đến AgentCore runtime URL — tất cả các tRPC operations (queries, mutations, và subscriptions) đều truyền qua một kết nối WebSocket duy nhất. Handshake được xác thực tự động: ký SigV4 cho `IAM` auth, hoặc mang JWT dưới dạng base64url-encoded subprotocol cho `Cognito`.
</TabItem>
</Tabs>
