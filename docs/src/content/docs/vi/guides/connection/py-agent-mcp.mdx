...
1. Một dự án Python với component <Link path="guides/py-agent">Agent</Link>
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_strands.py Client MCP Strands bao bọc truyền tải
      - auth/ `httpx.Auth` SigV4 / chuyển tiếp session không phụ thuộc framework
    - app
      - \<mcp\_server\_name>\_client\_strands.py Client Strands theo từng kết nối cho mỗi MCP server
