...
Prisma client được khởi tạo bên trong `getAgent()`. Vì generator `ts#strands-agent` cấu hình một Agent duy nhất cho mỗi phiên, client cũng được tái sử dụng trong suốt vòng đời của phiên.
