...
Nếu agent của bạn sử dụng xác thực IAM, authenticated role của Cognito Identity Pool phải được cấp quyền để gọi agent. Bạn có thể sử dụng phương thức `grantInvokeAccess` để cấp cả hai quyền `InvokeAgentRuntime` và `InvokeAgentRuntimeWithWebSocketStream`:
