...
2. Một backend Smithy TypeScript API đang hoạt động (được tạo bằng <Link path="/guides/ts-smithy-api">generator `ts#api`</Link> với `--framework=smithy`)
3. Cognito Auth được thêm vào thông qua <Link path="/guides/react-website-auth">generator `ts#website#auth`</Link> nếu kết nối với API sử dụng xác thực Cognito hoặc IAM
...
:::tip[Tự động tạo lại]
Nếu bạn đang làm việc tích cực với cả ứng dụng React và Smithy API cùng lúc, hãy sử dụng target `dev` của ứng dụng React (một alias cho `serve-local`) để tự động tạo lại client bất cứ khi nào API của bạn thay đổi, cũng như hot-reload website và local Smithy API server:
...
Nếu Smithy API của bạn sử dụng xác thực `Custom` (Lambda Authorizer), bạn sẽ cần chỉnh sửa client provider được tạo để thêm các authorization headers mà authorizer của bạn mong đợi. Tìm cấu hình `fetch` trong file `<ApiName>Provider.tsx` được tạo và thêm token hoặc API key của bạn vào request headers.
