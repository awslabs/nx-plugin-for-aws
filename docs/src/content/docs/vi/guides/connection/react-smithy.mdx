...
:::tip[Tự động tạo lại]
Nếu bạn đang làm việc tích cực với cả ứng dụng React và Smithy API cùng lúc, hãy sử dụng target `dev` của ứng dụng React để tự động tạo lại client bất cứ khi nào API của bạn thay đổi, cũng như hot-reload website và local Smithy API server:
