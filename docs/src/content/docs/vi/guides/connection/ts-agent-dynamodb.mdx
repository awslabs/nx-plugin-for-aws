---
title: TypeScript Agent tới DynamoDB
description: Kết nối TypeScript Agent với dự án TypeScript DynamoDB
when:
  sourceType: ts#agent
...
Generator `connection` kết nối một <Link path="guides/ts-agent">TypeScript Agent</Link> với dự án <Link path="guides/ts-dynamodb">TypeScript DynamoDB</Link>, cấu hình phát triển cục bộ để cả hai khởi động cùng nhau tự động.
...
Import các entity factory từ gói DynamoDB và sử dụng chúng bên trong định nghĩa agent của bạn:
