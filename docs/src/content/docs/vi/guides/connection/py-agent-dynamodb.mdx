...
Trình tạo cập nhật target `<agent-name>-serve-local` của agent trong `project.json` để phụ thuộc vào target `serve-local` của dự án DynamoDB, và thêm gói DynamoDB làm phụ thuộc workspace. Không có tệp nguồn nào được sửa đổi.
