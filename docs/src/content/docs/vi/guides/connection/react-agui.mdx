...
`targetComponent` chấp nhận `--name` mà bạn đã cung cấp cho agent generator ở bất kỳ dạng viết hoa nào (`StoryAgent`, `story-agent`), đường dẫn của component tương đối với thư mục gốc của target project, hoặc generator id của nó.
