...
AG-UI và AgentCore Runtime mỗi cái xác định một cuộc hội thoại theo cách khác nhau, và hook được tạo liên kết chúng với nhau:
...
Cả hai giá trị đều đến từ trình duyệt, vì vậy chúng đặt tên cho một cuộc hội thoại mà không nói đó là của ai. Để hạn chế mỗi người dùng chỉ truy cập các cuộc hội thoại của riêng họ, hãy xem <Link path="guides/ts-agent">Restricting sessions to their owner</Link>.
