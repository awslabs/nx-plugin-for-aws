...
module "inner_gateway" {
  ...
  # Target ids của các target riêng của inner gateway (ví dụ: các MCP server của nó), để
  # gateway_url của nó không được sử dụng cho đến khi nó phục vụ các công cụ của chúng.
...
:::note[Thứ tự target]
AgentCore tìm nạp các công cụ của inner gateway khi `aws_bedrockagentcore_gateway_target` được tạo, vì vậy inner gateway phải đã phục vụ chúng. Đặt `tool_dependencies` của inner gateway thành các id của các target riêng của nó: `gateway_url` của nó sau đó sẽ chảy qua một readiness probe thăm dò `tools/list` cho đến khi các công cụ đó được phục vụ, vì vậy target của outer gateway sẽ đợi inner gateway sẵn sàng.
:::
</Fragment>
</Infrastructure>
