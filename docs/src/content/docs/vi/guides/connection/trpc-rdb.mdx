...
Ngoài ra, nó cập nhật target `dev` của tRPC API để tự động khởi động cơ sở dữ liệu khi chạy cục bộ.
...
Trình tạo cấu hình target `dev` của tRPC API để phụ thuộc vào target `dev` của cơ sở dữ liệu, vì vậy khi chạy:
