...
Trình tạo `connection` có thể kết nối <Link path="guides/ts-agent">TypeScript Strands Agent</Link> của bạn với máy chủ MCP (có thể là <Link path="guides/ts-mcp-server">TypeScript</Link> hoặc <Link path="guides/py-mcp-server">Python</Link>).
