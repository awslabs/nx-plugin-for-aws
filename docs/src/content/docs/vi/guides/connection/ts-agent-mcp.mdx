...
1. Một dự án TypeScript với thành phần <Link path="guides/ts-agent">Agent</Link>
...
- packages/common/agent-connection
  - src
    - app
      - \<mcp-server-name>-client-strands.ts Client Strands cấp cao cho máy chủ MCP được kết nối
    - core
      - agentcore-endpoints.ts Giải quyết ARN/URL không phụ thuộc framework
      - agentcore-fetch.ts Fetch với SigV4 / JWT / chuyển tiếp session không phụ thuộc framework
      - agentcore-mcp-transport.ts Transport MCP không phụ thuộc framework
      - agentcore-mcp-client-strands.ts Client MCP Strands bao bọc transport
