...
`my_scope_my_db` là tên module của dự án database của bạn — một module phẳng duy nhất có tên `<scope>_<name>`, hiển thị dưới dạng thư mục package bên trong dự án.

`.scalars()` giải nén các hàng kết quả thành các instance `ExampleModel`, do đó `model_dump()` có sẵn trên mỗi item.
