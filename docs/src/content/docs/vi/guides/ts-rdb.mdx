...
Dừng `serve-local` (ví dụ với `Ctrl+C`) sẽ tự động xóa container database local, nhưng vẫn giữ nguyên named volume để dữ liệu của bạn được lưu trữ qua các lần khởi động lại.

:::caution[Windows]
Do giới hạn xử lý tín hiệu trên Windows, container không bị tự động xóa khi `serve-local` bị dừng. Bạn cần xóa thủ công:
...
Local database image được cấu hình trong trường `serveLocal.image` của file `config.json` được tạo ra trong thư mục gốc dự án database của bạn. Cập nhật giá trị đó khi bạn thay đổi engine version.
