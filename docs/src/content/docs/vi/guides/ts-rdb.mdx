...
Aurora cluster xuất database logs đến CloudWatch Logs (`postgresql` cho PostgreSQL, `error` và `slowquery` cho MySQL) và bật Performance Insights trên writer instance (được mã hóa bằng KMS key của cluster) để các truy vấn có thể quan sát được nhằm mục đích khắc phục sự cố và kiểm toán. Lưu ý rằng CloudWatch Logs và Performance Insights sẽ phát sinh thêm chi phí.
