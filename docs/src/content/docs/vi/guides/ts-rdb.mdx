...
<FileTree>
  ...
  - scripts
    - pull-image.ts Kéo database container image cho phát triển cục bộ
...
Local database image được cấu hình trong target `serve-local` của dự án cơ sở dữ liệu được tạo ra trong `project.json`. Cập nhật tham số image được truyền cho `scripts/start-container.ts` khi bạn thay đổi engine version.
