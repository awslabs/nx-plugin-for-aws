...
Terraform chỉ in ra các outputs được khai báo trong `packages/infra/src`, vì vậy bất cứ thứ gì bạn muốn đọc sau khi triển khai phải được xuất lại từ module tạo ra nó — do đó có hai khối `output` ở trên.
:::
</Fragment>
</Infrastructure>
...
Website của bạn cũng được phục vụ từ CloudFront. Mở distribution domain name được in trong deployment outputs của bạn — output kết thúc bằng `DistributionDomainName` cho CDK, hoặc `website_url` cho Terraform — để sử dụng phiên bản chạy hoàn toàn trên AWS.
...
Cognito user pool được tạo với tính năng bảo vệ xóa được bật, vì vậy nó tồn tại sau các lệnh trên. Tắt tính năng bảo vệ xóa trong [Amazon Cognito console](https://console.aws.amazon.com/cognito/) trước khi xóa nó.

<Infrastructure>
<Fragment slot="cdk">
:::note[Tài Nguyên Được Giữ Lại]
Xóa các stacks sẽ để lại user pool, cùng với IAM role mà Cognito sử dụng để gửi tin nhắn SMS, IAM role mà API Gateway sử dụng để ghi vào CloudWatch Logs, các KMS keys mã hóa website bucket và các log groups của API và WAF, và chính các log groups. Xóa pool, roles và log groups, và lên lịch xóa các keys, khi bạn không còn cần chúng nữa.
:::
</Fragment>
<Fragment slot="terraform">
:::caution[Destroy Thất Bại Cho Đến Khi Tắt Deletion Protection]
Terraform cố gắng xóa user pool, vì vậy `destroy` dừng lại với thông báo `The user pool cannot be deleted because deletion protection is activated` và để lại các tài nguyên mà nó chưa đạt tới. Tắt tính năng bảo vệ xóa, sau đó chạy lại lệnh destroy để xóa mọi thứ — bao gồm cả user pool và IAM role mà Cognito sử dụng để gửi tin nhắn SMS.
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">Add to an Existing Project</Link> — áp dụng plugin trong Nx workspace hoặc monorepo mà bạn đã có.
