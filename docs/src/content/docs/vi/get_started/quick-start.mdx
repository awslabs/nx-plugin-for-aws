...
:::note[Tài Nguyên Được Giữ Lại]
Cognito user pool được tạo với tính năng bảo vệ xóa được bật, vì vậy nó không được xóa tự động (lệnh destroy sẽ báo lỗi). Xóa nó (và IAM role mà Cognito sử dụng để gửi tin nhắn SMS) thủ công nếu bạn không còn cần chúng nữa.
:::
</Fragment>
</Infrastructure>

Để xóa các tài nguyên bootstrap, làm theo hướng dẫn bên dưới:
...
:::note[Asset Bucket]
Asset bucket được giữ lại có chủ đích, vì vậy hãy xóa nó riêng.
:::
</Fragment>
<Fragment slot="terraform">
...
Lệnh này làm trống và xóa S3 bucket chứa Terraform state của bạn, vì vậy chỉ chạy nó khi bạn đã gỡ bỏ tất cả cơ sở hạ tầng đã triển khai.
:::
</Fragment>
</Infrastructure>
