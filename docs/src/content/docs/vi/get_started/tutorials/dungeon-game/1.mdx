...
Điều này cấu hình một CDK `AgentRuntimeArtifact` tải image Docker agent của bạn lên ECR và lưu trữ nó bằng AgentCore Runtime. Nó sử dụng `buildContexts` của CDK để tham chiếu gốc workspace, cho phép Dockerfile được đặt cùng vị trí với mã nguồn agent.
