...
Vì `agent-serve-local` của Story Agent đã phụ thuộc vào `mcp-server-serve-local` của máy chủ MCP Inventory (thông qua kết nối `story → inventory` ở trên), và cả Game API và máy chủ MCP hiện phụ thuộc vào `dungeon-db:serve-local`, việc chạy mục tiêu `serve-local` của bất kỳ dự án nào sẽ khởi động mọi phụ thuộc mà nó cần theo đúng thứ tự.
</Aside>
