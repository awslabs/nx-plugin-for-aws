...
Chúng ta sử dụng `--iac=cdk` vì chúng ta sẽ sử dụng CDK cho cơ sở hạ tầng dưới dạng mã trong hướng dẫn này. Nx Plugin cho AWS cũng hỗ trợ `Terraform`.
:::
...
Bạn sẽ thấy một số tệp mới xuất hiện trong cây tệp của mình.
<Drawer title="Các tệp được cập nhật bởi py#agent" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
...
Điều này tạo một Strands agent ví dụ và định nghĩa một công cụ trừ.
...
## Nhiệm vụ 5: Tạo cơ sở dữ liệu trò chơi
...
Trình tạo `ts#dynamodb` cung cấp một mục tiêu `serve-local` chạy [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) trong một container. Kết hợp với trình tạo `connection` (mà chúng ta chạy bên dưới), điều này có nghĩa là **toàn bộ trò chơi chạy trên máy của bạn mà không có cuộc gọi AWS nào** cho đến khi bạn chọn triển khai vào cuối hướng dẫn. Bạn sẽ cần [một container engine](https://docs.docker.com/get-started/get-docker/) như Docker hoặc Finch được cài đặt và đang chạy.
:::

Bạn sẽ thấy một số tệp mới xuất hiện trong cây tệp của mình.
<Drawer title="Các tệp được cập nhật bởi ts#dynamodb" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - constants.ts các hằng số phát triển local và khóa cấu hình runtime
      - client.ts DynamoDB client singleton và phân giải tên bảng
      - gsi.ts nguồn chân lý cho Global Secondary Indexes
      - entities/
        - example.ts entity ElectroDB ví dụ (chúng ta sẽ thay thế cái này)
        - index.ts entity exports
    - project.json thêm các mục tiêu `serve-local` và `pull-image`
  - common/constructs/
    - src/
      - app/dynamodb/
...
<Drawer title="Các tệp được cập nhật bởi ts#website" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
...
- packages/
  ...
  - game-ui/
    ...
    - ...
  - common/
    - shadcn/ thư viện shadcn/ui được chia sẻ (token chủ đề, `Button`, `Card`, `Input`, `Sidebar`, …) được import bởi mọi website `uxProvider=Shadcn`
      - src/components/ui/*
...
<Drawer title="Các tệp được cập nhật bởi ts#website#auth" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
...
Trình tạo:
- Phát hiện `uxProvider` của website React (Shadcn ở đây) và cung cấp các component chat phù hợp.
...
Cả Game API và máy chủ MCP Inventory đều đọc và ghi vào bảng DynamoDB của chúng ta, vì vậy hãy kết nối chúng với dự án `DungeonDb`. Trình tạo `connection` phát hiện rằng mục tiêu là một dự án `ts#dynamodb` và kết nối mục tiêu `serve-local` của mỗi dự án nguồn để tự động khởi động DynamoDB Local.
...
Vì `agent-serve-local` của Story Agent đã phụ thuộc vào `mcp-server-serve-local` của máy chủ MCP Inventory (thông qua kết nối `story → inventory` ở trên), và cả Game API và máy chủ MCP hiện phụ thuộc vào `dungeon-db:serve-local`, việc chạy mục tiêu `serve-local` của bất kỳ dự án nào sẽ khởi động mọi phụ thuộc mà nó cần — bao gồm cả DynamoDB Local — theo đúng thứ tự. Chúng ta sẽ dựa vào điều này trong suốt phần còn lại của hướng dẫn.
</Aside>
...
## Nhiệm vụ 7: Cập nhật cơ sở hạ tầng của chúng ta
...
<Aside type="note" title="Môi trường không tương tác">
Trong một shell không tương tác (ví dụ: CI), không có lời nhắc — thay vào đó, quá trình build sẽ thất bại với lỗi "workspace is out of sync". Chạy <NxCommands commands={['sync']} /> để áp dụng các trình tạo sync đang chờ xử lý, sau đó build lại, hoặc đặt `sync.applyChanges` thành `true` trong `nx.json` để tự động áp dụng các thay đổi trên mỗi lần build. Bạn sẽ gặp lại kiểm tra out-of-sync này bất cứ khi nào các tham chiếu dự án thay đổi (ví dụ: sau khi thêm các procedure trong module tiếp theo).
</Aside>
