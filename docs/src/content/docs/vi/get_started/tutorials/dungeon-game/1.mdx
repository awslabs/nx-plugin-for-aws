...
- packages/
  - story/
    - dungeon\_adventure\_story/ module python
      - agent/
        - main.py điểm vào ứng dụng AG-UI FastAPI cho Bedrock AgentCore Runtime
...
Vì chúng ta đã chọn `protocol=AG-UI`, trình tạo bọc Strands agent của chúng ta trong một adapter [AG-UI](https://ag-ui.com/). `create_strands_app` xây dựng một ứng dụng FastAPI mà expose một endpoint `/invocations` duy nhất - các instance agent theo phiên được tạo tự động, và lịch sử hội thoại được giữ trong bộ nhớ theo `threadId` trong suốt thời gian tồn tại của máy chủ. Chúng ta sẽ không cần phải sửa đổi `main.py` ở bất kỳ đâu trong hướng dẫn này.
