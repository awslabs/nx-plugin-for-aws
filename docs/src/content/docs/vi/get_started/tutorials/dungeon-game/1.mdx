...
<Aside type="tip" title="Commit thường xuyên">Cách tốt nhất là đảm bảo tất cả các tệp chưa được stage của bạn được commit trong Git trước khi chạy bất kỳ trình tạo nào. Điều này cho phép bạn xem những gì đã thay đổi sau khi chạy trình tạo thông qua `git diff`.</Aside>
...
- packages/
  - story/
    - dungeon\_adventure\_story/ module python
      - agent/
        - main.py điểm vào ứng dụng AG-UI Starlette cho Bedrock AgentCore Runtime
        - agent.py định nghĩa Strands agent và các công cụ của nó
        - session.py lưu trữ session id của AgentCore runtime đầu vào
...
Hãy xem xét các tệp chính chi tiết:
...
@contextmanager
def get_agent():
    # Khi AgentCore-managed session storage được gắn vào runtime, đường dẫn
    # mount được expose qua SESSION_STORAGE_PATH. Việc lưu trữ thông qua
    # FileSessionManager cho phép agent tiếp tục toàn bộ lịch sử hội thoại
    # sau một chu kỳ dừng/tiếp tục. Nếu không có biến môi trường (ví dụ: serve cục bộ),
    # agent chỉ chạy trong bộ nhớ.
...
`agent.py` định nghĩa một Strands agent mẫu. Chúng ta sẽ thay thế system prompt và các công cụ bằng logic dungeon-adventure của chúng ta trong <Link path="get_started/tutorials/dungeon-game/3">Mô-đun 3</Link>.
...
class _LazyStrandsApp:
    """Xây dựng ứng dụng Strands bên trong khi yêu cầu ``/invocations`` đầu tiên để
    các health check ``/ping`` vẫn nhẹ và session id AgentCore đầu vào được
    lưu trữ trước khi bất kỳ MCP / A2A client downstream nào đọc nó."""
...
Vì chúng ta đã chọn `protocol=AG-UI`, trình tạo bọc Strands agent của chúng ta trong một adapter [AG-UI](https://ag-ui.com/) được expose qua một ứng dụng Starlette. AgentCore định tuyến mỗi `runtimeSessionId` đến một microVM chuyên dụng, vì vậy một `Agent` tồn tại lâu dài cho mỗi container là thời gian tồn tại chính xác - `main.py` xây dựng nó một cách lazy khi yêu cầu `/invocations` đầu tiên sau khi lưu trữ session id đầu vào. Khi AgentCore-managed session storage được gắn vào, `agent.py` kết nối một `FileSessionManager` với `SESSION_STORAGE_PATH` để agent có thể tiếp tục toàn bộ lịch sử hội thoại sau một lần dừng/tiếp tục. Chúng ta sẽ không cần phải sửa đổi `main.py` ở bất kỳ đâu trong hướng dẫn này.
...
Đây là điểm vào nơi React được gắn kết. Ban đầu nó chỉ cấu hình một `@tanstack/react-router` trong cấu hình [`file-based-routing`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) - miễn là máy chủ dev của bạn đang chạy, việc tạo một tệp trong `src/routes/` sẽ tự động thêm một tuyến đường và tạo lại `routeTree.gen.ts`. Điều này giữ cho `<Link to="...">` hoàn toàn an toàn kiểu.
...
Một component sẽ được render khi điều hướng đến tuyến đường `/`. Vì chúng ta đã chọn `uxProvider=Shadcn`, trình tạo cũng tạo một sidebar + app shell trong `components/AppLayout` sử dụng các primitive [shadcn/ui](https://ui.shadcn.com/) được chia sẻ dưới `packages/common/shadcn`.
...
Hook `useGameApi` không yêu cầu build để các thay đổi được phản ánh, nhờ vào việc sử dụng [suy luận TypeScript](https://trpc.io/docs/concepts) của tRPC. Điều này cho phép các nhà phát triển thực hiện thay đổi đối với backend của họ và ngay lập tức thấy chúng được phản ánh trong frontend.
</Aside>
...
<Drawer title="Các tệp được cập nhật bởi kết nối Story Agent -> Inventory MCP" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
  ...
  - common/
    - agent\_connection/
      - dungeon\_adventure\_agent\_connection/
        ...
        - app/
          - **inventory\_mcp\_server\_client.py** Client để kết nối với máy chủ MCP Inventory
...
- Cập nhật mục tiêu `serve-local` để tự động khởi động máy chủ MCP khi chạy cục bộ

Để biết thêm chi tiết, hãy tham khảo <Link path="guides/connection/py-strands-agent-mcp">hướng dẫn kết nối Python Strands Agent với MCP</Link>.
</Drawer>

### Game UI: Kết nối với Story Agent

Hãy kết nối Game UI của chúng ta với Story Agent để website React có thể trò chuyện với agent qua giao thức AG-UI.
...
<Drawer title="Các tệp được cập nhật bởi kết nối UI -> Story Agent" trigger="Nhấp vào đây để xem xét các tệp này chi tiết hơn.">
Vì Story Agent được tạo với `protocol=AG-UI`, trình tạo `connection` kết nối website bằng cách sử dụng [CopilotKit](https://copilotkit.ai/) (nói giao thức [AG-UI](https://ag-ui.com/)). Các tệp này được tạo/cập nhật:
...
- packages/
  - game-ui/
    - src/
      - components/
        - **AguiProvider.tsx** CopilotKitProvider đăng ký mỗi AG-UI agent
        - **copilot/** Các component chat CopilotKit có chủ đề (assistant/user bubbles, input, cursor) - được chọn tự động dựa trên UX provider của website (Shadcn ở đây)
      - hooks/
        - **useAguiStoryAgent.tsx** Hook theo agent xây dựng một AG-UI `HttpAgent` được xác thực Cognito nhắm vào AgentCore runtime
      - **main.tsx** Được cập nhật để thêm `<AguiProvider>` xung quanh app

</FileTree>

Trình tạo:
- Thêm một `AguiProvider` được chia sẻ (nếu chưa tồn tại) bọc app trong `CopilotKitProvider` của CopilotKit, và đăng ký hook của mỗi AG-UI agent
- Tạo một hook theo agent (`useAguiStoryAgent`) giải quyết AgentCore runtime ARN từ `runtime-config.json`, xây dựng một `HttpAgent` được xác thực, và đính kèm header `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` trên mỗi yêu cầu
- Cung cấp các primitive chat có chủ đề dưới `src/components/copilot/` để chat trông giống với UX provider của website

Để biết thêm chi tiết, hãy tham khảo <Link path="guides/connection/react-py-strands-agent">hướng dẫn kết nối React với Python Strands Agent</Link>.
</Drawer>
...
<Aside type="tip" title="Shell không tương tác">
Nếu bạn đang chạy trong shell không tương tác (CI, hoặc môi trường headless), bạn sẽ không thấy prompt - `build` sẽ thất bại ngay lập tức với thông báo "The workspace is out of sync". Trong trường hợp đó, hãy chạy <NxCommands commands={['sync']} /> thủ công một lần, sau đó chạy lại build.
</Aside>
