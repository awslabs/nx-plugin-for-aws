...
## Nhiệm vụ 2: Scaffold trò chơi Dungeon Adventure

Với workspace đã sẵn sàng, chúng ta scaffold các dự án con của trò chơi — Game API, Story Agent, máy chủ MCP Inventory, cơ sở dữ liệu trò chơi và website — cùng với các kết nối kết nối chúng lại với nhau. Có hai cách để làm điều này:

- **Nhanh** — sao chép các lệnh trực tiếp từ sơ đồ bên dưới và chạy chúng. Đây là cách nhanh nhất để đạt được cùng một điểm xuất phát.
- **Từng bước** — mở rộng phần bên dưới để chạy từng trình tạo và kiểm tra chính xác những gì mỗi trình tạo tạo ra.

Sơ đồ bên dưới _chính là_ workspace Dungeon Adventure: mọi dự án, component và kết nối mà bạn sẽ xây dựng trong module này. Nhấn **Copy commands** để lấy toàn bộ chuỗi lệnh, sau đó chạy chúng từ trong thư mục `dungeon-adventure` mà bạn đã tạo trong Nhiệm vụ 1.

<EmbeddedGraph preset="dungeon-adventure" workspace="dungeon-adventure" iac="cdk" skipWorkspace />

<Drawer title="Từng bước" trigger="Từng bước: chạy từng trình tạo và xem những gì nó tạo ra">

Thay vì sao chép tất cả các lệnh cùng một lúc, bạn có thể chạy từng trình tạo riêng lẻ. Đây là cách tốt nhất để hiểu những gì mỗi trình tạo thêm vào workspace của bạn. Chạy từng trình tạo lần lượt, từ trong thư mục `dungeon-adventure` mà bạn đã tạo trong Nhiệm vụ 1.
