...
Hook `useGameApi` khác với hook `useStoryAgent` vì nó không yêu cầu build để các thay đổi được phản ánh nhờ vào việc sử dụng [suy luận Typescript](https://trpc.io/docs/concepts) của tRPC. Điều này cho phép các nhà phát triển thực hiện thay đổi đối với backend của họ và ngay lập tức được phản ánh trong frontend của họ.
</Aside>
...
Hãy kết nối Game UI của chúng ta với Story Agent để chúng ta có thể gọi API của agent từ website React với một client an toàn kiểu, được tạo tự động.
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** Script để tạo đặc tả OpenAPI từ ứng dụng FastAPI của agent
    - project.json Được cập nhật với mục tiêu `agent-openapi`
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** Provider cho OpenAPI client an toàn kiểu
        - **QueryClientProvider.tsx** TanStack React Query client provider (nếu chưa có)
      - hooks/
        - **useStoryAgent.tsx** Hook cho TanStack Query options proxy
        - **useStoryAgentClient.tsx** Hook cho vanilla API client
      - generated/
        - story-agent/ Được tạo tại thời điểm build từ đặc tả OpenAPI
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json Được cập nhật với các mục tiêu tạo client
    - **main.tsx** Được cập nhật để thêm StoryAgentProvider

</FileTree>

Trình tạo:
- Tạo một script tạo đặc tả OpenAPI có phạm vi agent trong dự án story
- Tạo một TypeScript client an toàn kiểu từ đặc tả OpenAPI của agent tại thời điểm build
- Thêm một provider, hooks và client vào website để gọi API của agent
- Cấu hình `serve-local` để tự động khởi động máy chủ cục bộ của agent và tạo lại client khi có thay đổi
