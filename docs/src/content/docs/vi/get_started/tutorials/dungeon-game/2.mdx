...
:::note[Không có thủ tục lịch sử trò chuyện]
Bạn có thể mong đợi một thủ tục để lưu mỗi lượt user/assistant của cuộc trò chuyện. Chúng ta không cần điều đó: agent AG-UI được lưu trữ trên Bedrock AgentCore Runtime, tự động duy trì lịch sử hội thoại cho mỗi phiên miễn là cùng một thread id được sử dụng.
:::
