...
Dependency `@aws-sdk/client-s3` đã có sẵn trong workspace, vì vậy không cần cài đặt gì thêm. Tên bucket đến từ cùng một cơ chế Runtime Configuration được sử dụng cho `tables` — chúng ta đăng ký nó dưới namespace `buckets` mới trong infrastructure stack bên dưới.
  </TabItem>
</Tabs>
