...
Chúng ta sẽ triển khai cơ sở dữ liệu trong DynamoDB và sẽ sử dụng thư viện client DynamoDB [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) để đơn giản hóa mọi thứ. `queryActions` của Game API cũng cần S3 client để đọc các lượt hội thoại mà agent đã ghi. Cài đặt tất cả bằng lệnh:
