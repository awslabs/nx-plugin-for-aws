...
:::note[Lịch sử hội thoại được lưu cùng với agent]
Chúng ta không lưu trữ các tin nhắn trò chuyện thông qua Game API. Trong <Link path="get_started/tutorials/dungeon-game/3">Module 3</Link>, Story Agent sẽ kết nối với `FileSessionManager` để ghi lịch sử của mỗi thread vào đĩa, được khóa bởi thread id mà UI gửi ở mỗi lượt chơi — do đó mỗi cặp `(player, genre)` sẽ tiếp tục câu chuyện của riêng mình mà không cần UI phải phát lại các tin nhắn.
:::
