...
Bạn đã thấy hai đầu của phổ: mọi thứ cục bộ (`dev`) và mọi thứ đã triển khai. Trong quá trình phát triển hàng ngày, thường hữu ích khi kết hợp cả hai — ví dụ, lặp lại trên code website với API và agent *thực* đã triển khai, hoặc chạy API cục bộ với bảng DynamoDB *thực*.
