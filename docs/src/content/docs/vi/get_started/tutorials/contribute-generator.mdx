...
Chúng ta có một bộ "smoke tests" chạy các generators trong một workspace mới và đảm bảo rằng mọi thứ đều build thành công. Ở mức tối thiểu, generator mới của bạn nên được thêm vào generator matrix trong `e2e/src/smoke-tests/generator-matrix.ts` để nó được thực thi bởi các smoke tests.
