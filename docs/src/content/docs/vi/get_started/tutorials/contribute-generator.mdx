...
:::tip[Học qua ví dụ]
Hướng dẫn này giúp bạn đóng góp một generator cho Nx Plugin for AWS thông qua một ví dụ thực tế. Nó cũng có thể đóng vai trò như một tài liệu tham khảo hữu ích về các cách suy nghĩ và kiểm thử generators. Bạn có thể bỏ qua phần hướng dẫn chung để xem một số mẹo hàng đầu khi làm việc với generators.
:::
...
:::tip[Định dạng tệp]
Chúng ta cũng đã gọi `formatFilesInSubtree` ở cuối generator, điều này đảm bảo rằng bất kỳ file nào chúng ta tạo hoặc sửa đổi đều được định dạng theo cấu hình [Biome](https://biomejs.dev/) của workspace.
:::
...
Chúng ta sử dụng [GritQL](https://docs.grit.io/) để khai báo tìm kiếm và biến đổi mã nguồn. Helper `addDestructuredImport` thêm các named imports, và `applyGritQL` áp dụng một pattern GritQL để thêm procedure vào object literal của router.
...
:::tip[Mẫu GritQL]
Trong đoạn code trên, `applyGritQL` sử dụng một pattern [GritQL](https://docs.grit.io/) để tìm lời gọi `router({ ... })` và thêm procedure mới vào object literal của nó. Mệnh đề `where` đảm bảo tính idempotent — nếu procedure đã tồn tại, không có thay đổi nào được thực hiện.

Bạn có thể thử các pattern GritQL trong [Sân chơi GritQL](https://docs.grit.io/playground), hoặc cài đặt GritQL CLI để kiểm tra các pattern cục bộ:
...
:::note[Sử dụng dự án của bạn]
Nếu bạn đã hoàn thành <Link path="get_started/tutorials/dungeon-game/overview">tutorial dungeon adventure</Link>, hoặc đã có một Nx workspace hiện có khác sử dụng tRPC API, bạn có thể bỏ qua bước này.
:::
...
:::note[Đường dẫn plugin đã biên dịch]
...
Generator của chúng ta nên bảo vệ chống lại các vấn đề tiềm ẩn, chẳng hạn như người dùng chọn một `project` không phải là tRPC API. Hãy xem generator `connection` để xem ví dụ về điều này.
...
Chúng ta có một bộ "smoke tests" chạy các generators trong một workspace mới và đảm bảo rằng mọi thứ đều build thành công. Ở mức tối thiểu, generator mới của bạn nên được thêm vào **cả hai** generator matrices để nó được thực thi bởi các smoke tests:

- `e2e/src/smoke-tests/generator-matrix.ts` — chạy mỗi generator thông qua CLI, từng lần gọi một, chính xác như người dùng sẽ làm.
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — một generator ẩn kết hợp tất cả các generators khác trong cùng một process trên một tree duy nhất.

Cả hai đều đáng giữ lại. CLI matrix kiểm tra đường dẫn mà người dùng thực sự thực hiện. Composed matrix được đóng gói cùng với plugin, vì vậy mỗi phiên bản phát hành mang theo matrix của các generators mà _nó_ có — cho phép các tests nâng cấp một workspace cũ hơn có thể scaffold nó với matrix của chính phiên bản đó. Vì nó gọi các generators trực tiếp thay vì thông qua CLI, các options của nó được kiểm tra kiểu so với schema của mỗi generator, vì vậy một required option mới hoặc một enum đã thay đổi sẽ làm build thất bại ở đó thay vì âm thầm giảm coverage.

Lưu ý rằng generator matrix chỉ chạy các generators và build workspace — nó không khởi tạo bất kỳ infrastructure nào. Đối với các generators triển khai infrastructure, hãy xem xét mở rộng các deployment e2e tests (`e2e/src/smoke-tests/cdk-deploy.spec.ts` và `terraform-deploy.spec.ts`) để thực sự triển khai các resources của bạn (thông qua `cdk deploy` / `terraform apply`), sau đó thêm một assertion vào `deploy-invocations.ts` để gọi deployed resource và xác minh nó hoạt động như mong đợi.

Nếu generator của bạn cung cấp một local development server, hãy xem xét thêm nó vào local development e2e test (`e2e/src/smoke-tests/local-dev.spec.ts`), test này khởi động `dev` target và thực thi running server.
...
#### Làm việc ngược từ một dự án thực tế

Một cách hữu ích để xây dựng generators mới hoặc thêm tính năng/sửa lỗi cho một generator hiện có là _xây dựng nó thực tế trước_. Bằng cách này, bạn có thể xác thực ý tưởng của mình và lặp lại nhanh chóng để đạt được chức năng bạn cần. Sau khi bạn đã quyết định kết quả mong muốn, bạn có thể cập nhật generator.
...
1. Chạy bất kỳ generators nào có thể là điều kiện tiên quyết cho generator/tính năng/sửa lỗi mới của bạn
1. Commit các thay đổi của bạn (`git commit`)
1. Thực hiện các thay đổi mong muốn và kiểm tra chúng khi cần
1. Sử dụng `git diff` của các thay đổi để xác định những thay đổi nào cần được thực hiện đối với Nx Plugin for AWS
1. Thực hiện một lần kiểm tra end to end cuối cùng (liên kết `@aws/nx-plugin` của bạn) để đảm bảo generator của bạn tạo ra những thay đổi bạn cần
