...
Chúng ta có một bộ "smoke tests" chạy các generators trong một workspace mới và đảm bảo rằng mọi thứ đều build thành công. Ở mức tối thiểu, generator mới của bạn nên được thêm vào generator matrix trong `packages/nx-plugin/src/internal/test-matrix/generator.ts` để nó được thực thi bởi các smoke tests.

Matrix tự nó là một generator (ẩn) kết hợp tất cả các generators khác, vì vậy nó được đóng gói cùng với plugin. Điều đó có nghĩa là mỗi phiên bản phát hành mang theo matrix của các generators mà _nó_ có, cho phép các tests nâng cấp một workspace cũ hơn có thể scaffold nó với matrix của chính phiên bản đó. Vì nó gọi các generators trực tiếp thay vì thông qua CLI, các options được kiểm tra kiểu so với schema của mỗi generator — vì vậy một required option mới hoặc một enum đã thay đổi sẽ làm build thất bại ở đây thay vì âm thầm giảm coverage.
