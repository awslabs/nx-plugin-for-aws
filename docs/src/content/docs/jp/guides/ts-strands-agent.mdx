...
`protocol` が `A2A` に設定されている場合、エントリーポイントは tRPC の代わりに [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) を使用します:
...
`protocol=A2A` の場合、生成された `index.ts` は [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) を Express アプリにマウントするため、生成されたエージェントは A2A プロトコルエンドポイントを `/ping` ヘルスチェックと共に公開します。AgentCore にデプロイされると、エントリーポイントは AppConfig からランタイムのパブリック ARN を解決し、それをエージェントカードでアドバタイズします。
...
Strands SDK の `A2AAgent` は、リモート A2A エンドポイントをラップするため、ローカルの `Agent` のように呼び出すことができます。`invoke` は `AgentResult` に解決され、`.toString()` を呼び出すと、リモートエージェントの応答からテキストブロックが結合されます。
...
#### IAM 認証を使用した AgentCore での呼び出し

AgentCore にデプロイされた A2A エージェントへのリクエスト (エージェントカードの取得を含む) は、SigV4 署名が必要です。カードリゾルバーと JSON-RPC トランスポートの両方に SigV4 署名 fetch を使用する `clientFactory` を `A2AAgent` に渡します:
...
:::note[IAM 権限]
呼び出し側の IAM ロールには、ターゲットランタイム ARN に対して `bedrock-agentcore:InvokeAgentRuntime` と `bedrock-agentcore:GetAgentCard` の両方が必要です。生成されたコンストラクトの `grantInvokeAccess` ヘルパーは両方を付与します。
:::

:::note[追加の依存関係]
上記の SigV4 クライアントの例では、`@a2a-js/sdk`、`aws4fetch`、および `@aws-sdk/credential-providers` が必要です。例を実行する前に、これらを `package.json` に追加してください。
:::

#### A2A エージェントをツールとして使用

`A2AAgent` はローカルの `Agent` と同じ `InvokableAgent` インターフェースを実装しているため、Strands `tool` でラップして別のエージェントに渡すことができます。これは、生成された HTTP/tRPC エージェントテンプレートがリモート A2A エージェントに委任する方法です:

```typescript
import { Agent, tool } from '@strands-agents/sdk';
import { z } from 'zod';

const remote = /* 上記のように clientFactory で構築された A2AAgent */;
...
CDK スタックで、ホストエージェントにリモートエージェントへの呼び出しアクセスを付与します — これにより、A2A クライアントが必要とする両方の IAM アクションが設定されます:
