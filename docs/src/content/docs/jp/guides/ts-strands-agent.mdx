...
:::caution[TypeScript では AG-UI はまだサポートされていません]
[AG-UI プロトコル](https://docs.ag-ui.com/)は、TypeScript Strands Agent ではまだサポートされていません。AG-UI サポートが必要な場合は、`protocol=AG-UI` を指定して <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> ジェネレーターを使用してください。TypeScript の AG-UI サポート状況については、[strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) を参照してください。
:::
...
ジェネレーターは `<your-agent-name>-invoke` という名前のターゲットを設定し、ターミナルからエージェントとのインタラクティブなチャットを開始します。デフォルトではローカルサーバーに接続します:

<NxCommands commands={['agent-invoke your-project']} />

デプロイされたエージェントとチャットするには、`RUNTIME_CONFIG_APP_ID` をワークスペースの AppConfig アプリケーション ID に設定します — CLI はランタイム ARN を解決し、デプロイされたエンドポイントを呼び出します:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Cognito 認証されたエージェントの場合は、`--accessToken` で JWT を渡します:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

CLI スクリプトは `scripts/<agent>/cli.ts` にあり、エージェントに適したプロトコル (HTTP/A2A) を自動的に選択します。
