...
ジェネレーターは、`scripts/<your-agent-name>/chat.ts` の下に最小限のインタラクティブなチャット CLI と、対応する `<your-agent-name>-chat` ターゲットも提供します。このターゲットは `<your-agent-name>-serve-local` に依存しているため、実行するとエージェントがローカルで起動し、インタラクティブなターミナルチャットに入ります:

<NxCommands commands={['run your-project:agent-chat']} />

CLI は、プロトコルに適したクライアント (HTTP/tRPC の場合は `AgentClient.local`、A2A の場合は `@a2a-js/sdk`) を使用し、[`@clack/prompts`](https://github.com/bombshell-dev/clack) を介してエージェントの応答をトークンごとにストリーミングします。
