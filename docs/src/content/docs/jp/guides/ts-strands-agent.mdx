...
ジェネレーターは、`<your-agent-name>-serve-local` に依存する `<your-agent-name>-chat` Nx ターゲットを設定します。実行すると、エージェントがローカルで起動し、[`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) を使用したインタラクティブなターミナルチャットに入ります:

<NxCommands commands={['run your-project:agent-chat']} />

**A2A** エージェントの場合、ターゲットは単に `agent-chat-cli` をバイナリとして実行します — ワークスペースにコードは生成されません。

**HTTP** (tRPC over WebSocket) エージェントの場合、ジェネレーターは、生成された `<Agent>Client.local({ url })` を `ChatAdapter` としてラップし、`agent-chat-cli` の `chatLoop` に渡す小さな `scripts/<your-agent-name>/chat.ts` を生成します。
