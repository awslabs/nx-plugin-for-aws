...
このコマンドは `uv run uvicorn --reload` を使用して、HTTP トランスポートで MCP サーバーを実行し、ファイルが変更されると自動的に再起動します。

各 MCP サーバーには `8000` から始まる独自のポートが割り当てられるため、同じワークスペース内で複数のサーバーを並行して実行できます。サーバーがリッスンするポートは、`project.json` 内の `<your-server-name>-serve` ターゲットから確認してください。
