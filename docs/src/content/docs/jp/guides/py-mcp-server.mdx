...
ジェネレータは`<your-server-name>-inspect`ターゲットを設定します。このターゲットはMCPサーバーをローカルで起動し（`<your-server-name>-serve-local`ターゲット経由で、ローカルデータベースなどの接続された依存関係を含む）、ストリーミング可能HTTPトランスポートで接続するように事前設定された[MCP Inspector](https://github.com/modelcontextprotocol/inspector)を起動します。
...
<CardGrid>
  <ConnectionCard
    ...
    description="TypeScript AgentをMCPサーバーに接続"
  ...
  <ConnectionCard
    ...
    description="Python MCPサーバーをDynamoDBテーブルに接続"
