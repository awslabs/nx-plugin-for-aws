---
title: TypeScript Agent から DynamoDB へ
...
`connection` ジェネレーターは、<Link path="guides/ts-agent">TypeScript Agent</Link> を <Link path="guides/ts-dynamodb">TypeScript DynamoDB</Link> プロジェクトに接続し、両方が自動的に一緒に起動するようにローカル開発を構成します。
