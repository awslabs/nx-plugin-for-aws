...
`connection` ジェネレーターは、<Link path="guides/ts-smithy-api">Smithy API</Link> を <Link path="guides/ts-dynamodb">TypeScript DynamoDB</Link> プロジェクトに接続し、両方が自動的に一緒に起動するようにローカル開発を設定します。
...
ジェネレーターは、Smithy API の `project.json` を更新して、その `dev` ターゲットから DynamoDB プロジェクトの `dev` ターゲットへの依存関係を追加します。ソースファイルは変更されません。
