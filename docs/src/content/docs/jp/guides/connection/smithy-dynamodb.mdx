...
`connection` ジェネレーターは、<Link path="guides/ts-smithy-api">Smithy API</Link> を <Link path="guides/ts-dynamodb">DynamoDB</Link> プロジェクトに接続し、ローカル開発環境で両方が自動的に一緒に起動するように設定します。
