...
ジェネレータはバックエンドの`computeType`を検査し、対応するクライアントを生成します:

- **`ServerlessApiGatewayRestApi`** — tRPCサブスクリプション用にServer-Sent Events上で`httpSubscriptionLink`を使用するHTTPリンク。
- **`ServerlessApiGatewayHttpApi`** — プレーンなHTTPリンク（サブスクリプションなし）。
- **`BedrockAgentCoreRuntimeWebSocket`** — AgentCoreランタイムURLに対するWebSocketリンク。SigV4署名ハンドシェイク（IAM）またはbase64urlエンコードされたJWTサブプロトコル（Cognito）を使用。ランタイムARNは`runtimeConfig.apis.<ApiName>`から読み取られ、`wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws`URLに動的に変換されます。
