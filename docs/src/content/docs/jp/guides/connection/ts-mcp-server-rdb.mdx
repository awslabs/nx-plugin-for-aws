...
MCPサーバーをデータベースと同じVPC内で実行し、`additional_iam_policy_statements`を介して`rds-db:connect`を付与し、セキュリティグループルールのペアでネットワークパスを開きます：
...
`appconfig_application_id`/`appconfig_application_arn`は、データベースモジュールからではなく、ルートモジュールで一度宣言された共有<Link path="guides/runtime-config">ランタイム設定</Link> AppConfigアプリケーションから取得されます。
