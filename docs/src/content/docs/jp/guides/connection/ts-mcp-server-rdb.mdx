...
`appconfig_application_id`/`appconfig_application_arn`は、データベースモジュールからではなく、ルートモジュールで一度宣言された共有<Link path="guides/runtime-config">ランタイム設定</Link> AppConfigアプリケーションから取得されます。データベースモジュールのランタイム設定エントリがデプロイされるように、インスタンス化する際に`database`名前空間を含めます：
