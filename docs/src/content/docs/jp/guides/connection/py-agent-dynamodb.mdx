...
description: Python Agent を Python DynamoDB プロジェクトに接続する
when:
  sourceType: py#agent
...
ジェネレーターは、エージェントの `<agent-name>-dev` ターゲットを `project.json` 内で更新し、DynamoDB プロジェクトの `dev` ターゲットに依存するようにし、DynamoDB パッケージをワークスペース依存関係として追加します。ソースファイルは変更されません。
