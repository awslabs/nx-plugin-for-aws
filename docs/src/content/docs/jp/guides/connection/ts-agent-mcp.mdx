---
title: "TypeScript Agent から MCP への接続"
...
`connection` ジェネレーターを使用すると、<Link path="guides/ts-agent">TypeScript Agent</Link> を MCP サーバー（<Link path="guides/ts-mcp-server">TypeScript</Link> または <Link path="guides/py-mcp-server">Python</Link>）に接続できます。
...
1. <Link path="guides/ts-agent">Agent</Link> コンポーネントを持つ TypeScript プロジェクト
...
- エージェントの `dev` ターゲットを更新して MCP サーバーの serve ターゲットに依存
- 必要な依存関係をインストール
