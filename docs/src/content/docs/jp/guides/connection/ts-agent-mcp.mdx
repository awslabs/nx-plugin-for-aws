...
`connection` ジェネレーターを使用すると、<Link path="guides/ts-agent">TypeScript Strands Agent</Link> を MCP サーバー（<Link path="guides/ts-mcp-server">TypeScript</Link> または <Link path="guides/py-mcp-server">Python</Link>）に接続できます。
