...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py デプロイされたGateway用のStrands MCPクライアント
      - auth/ フレームワークに依存しないSigV4 / セッション転送 `httpx.Auth`
    - app/
      - \<gateway\_snake>\_client\_strands.py GatewayごとのStrandsクライアントラッパー
...
`MyGatewayClientStrands.create()`は、`list_tools_sync()`がGatewayを通じて利用可能なすべてのツールを生成する、単一のコンテキスト管理可能なクライアントを返します：
