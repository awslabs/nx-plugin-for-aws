...
2. 動作する Smithy TypeScript API バックエンドが存在すること（<Link path="/guides/ts-smithy-api">`ts#api` ジェネレータ</Link>を `--framework=smithy` で使用して生成）
3. Cognito または IAM 認証を使用する API に接続する場合、<Link path="/guides/react-website-auth">`ts#website#auth` ジェネレータ</Link>で Cognito 認証が追加されていること
...
:::tip[自動再生成]
ReactアプリとSmithy APIを同時に開発する場合、Reactアプリの`dev`ターゲット（`serve-local`のエイリアス）を使用すると、API変更時にクライアントの自動再生成、ウェブサイトとローカルSmithy APIサーバーのホットリロードが有効になります:
