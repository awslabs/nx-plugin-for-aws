...
2. 動作するSmithy TypeScript APIバックエンド（<Link path="/guides/ts-smithy-api">`ts#api`ジェネレーター</Link>を`--framework=smithy`で使用して生成）
3. CognitoまたはIAM認証を使用するAPIに接続する場合は、<Link path="/guides/react-website-auth">`ts#website#auth`ジェネレーター</Link>を介して追加されたCognito Auth
