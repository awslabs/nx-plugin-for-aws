...
内部的には、`RemoteAgentClientStrands.create(sessionId)` は、AWS にデプロイされた場合は SigV4 署名付きの `clientFactory` で構成された Strands `A2AAgent` を返し、`LOCAL_DEV=true` の場合はプレーンな `http://localhost:<port>/` エンドポイントを返します。署名とエンドポイント解決はフレームワーク非依存の `agentcore-a2a-client-config.ts` に存在し、薄い `agentcore-a2a-client-strands.ts` のみが Strands に依存します。
