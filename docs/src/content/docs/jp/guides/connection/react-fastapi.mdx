...
3. Cognito または IAM 認証を使用する API に接続する場合、<Link path="/guides/react-website-auth">`ts#website#auth` ジェネレータ</Link>経由で Cognito 認証が追加されていること
...
:::tip[自動再生成]
React アプリケーションと FastAPI を同時に開発している場合は、React アプリケーションの `dev` ターゲット (`serve-local` のエイリアス) を使用してください。API が変更されるたびにクライアントの自動再生成、ウェブサイトとローカル FastAPI サーバーのホットリロードが行われます:
