...
1. <Link path="guides/py-agent">Python Agent</Link> コンポーネント（Strands または LangChain）を含む Python プロジェクト
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_\<framework>.py エージェントのフレームワーク用にトランスポートをラップする MCP クライアント
      - auth/ フレームワークに依存しない SigV4 / セッション転送 `httpx.Auth`
    - app
      - \<mcp\_server\_name>\_client\_\<framework>.py 各 MCP サーバーの接続ごとのクライアント
...
Strands クライアントはコンテキストマネージャーであり、エージェントを囲む `with` ブロック内で使用されます。`session_id` パラメーターは呼び出し元から渡され、[Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html) の一貫性を保証します。
</TabItem>
...
LangChain クライアントは、[`langchain-mcp-adapters`](https://docs.langchain.com/oss/python/langchain/mcp) 経由で読み込まれたツールのリストを返します。各ツールは呼び出しごとに新しい MCP セッションを開くため、ツールはエージェントの生存期間中使用可能な状態を維持します — `with` ブロックは不要です。
</TabItem>
</Tabs>

AgentCore セッション ID は、両方のフレームワークで `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` ヘッダーを介して MCP サーバーに自動的に伝播され、[Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html) の一貫性を保証します。
...
ジェネレーターは、エージェントの `dev` ターゲットを以下のように設定します：
