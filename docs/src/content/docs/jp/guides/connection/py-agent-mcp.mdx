...
`connection` ジェネレーターは、<Link path="guides/py-agent">Python Strands Agent</Link> を MCP サーバー（<Link path="guides/ts-mcp-server">TypeScript</Link> または <Link path="guides/py-mcp-server">Python</Link>）に接続できます。
...
1. <Link path="guides/py-agent">Strands Agent</Link> コンポーネントを含む Python プロジェクト
