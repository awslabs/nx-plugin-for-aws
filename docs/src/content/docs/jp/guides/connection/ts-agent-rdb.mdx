---
title: TypeScript Agent からリレーショナルデータベースへの接続
description: TypeScript Agent をリレーショナルデータベースに接続する
when:
  sourceType: ts#agent
  targetType: ts#rdb
---
...
`connection` ジェネレーターは、<Link path="guides/ts-agent">TypeScript Agent</Link> を <Link path="guides/ts-rdb">リレーショナルデータベース</Link> プロジェクトに接続し、エージェントファクトリー内で Prisma クライアントを利用できるようにします。
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="RDS Proxy を使用せずに接続する場合の SSL 要件" />
