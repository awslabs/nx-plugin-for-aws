---
title: TypeScript Agent から Relational Database への接続
description: TypeScript Agent を Relational Database に接続する
when:
  sourceType: ts#agent
  targetType: ts#rdb
---
...
`connection` ジェネレーターは、<Link path="guides/ts-agent">TypeScript Agent</Link> を <Link path="guides/ts-rdb">Relational Database</Link> プロジェクトに接続し、エージェントファクトリー内で Prisma クライアントを利用可能にします。
...
:::caution
`securityGroups` を省略すると、`RuntimeNetworkConfiguration.usingVpc` は指定されたスコープ上に固定のコンストラクト ID（`SecurityGroup`）を持つセキュリティグループを作成します。示されているように明示的なセキュリティグループを渡すことで、複数のランタイム（例えばエージェントと MCP サーバー）が同じスタック内で定義されている場合のコンストラクト ID の衝突を回避できます。
:::

</Fragment>
<Fragment slot="terraform">

エージェントをデータベースと同じ VPC 内で実行し、`additional_iam_policy_statements` を介して `rds-db:connect` を付与し、セキュリティグループルールのペアでネットワークパスを開きます。`aws_vpc.main` と `aws_subnet` リソースは、データベースデプロイメントガイドで定義されています：
