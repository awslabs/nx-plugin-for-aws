...
`connection` ジェネレーターは、<Link path="guides/ts-agent">TypeScript Strands Agent</Link> を <Link path="guides/ts-rdb">Relational Database</Link> プロジェクトに接続し、エージェントファクトリー内で Prisma クライアントを利用可能にします。
