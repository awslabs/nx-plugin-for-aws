...
このジェネーターは、エージェントが MCP サーバーのツールを検出して呼び出すために必要なすべての配線を設定します。これは、AWS にデプロイされた場合（Bedrock AgentCore 経由）とローカルで実行される場合の両方で機能します。
...
MCP サーバーの AgentCore ランタイム ARN は、生成された CDK コンストラクトによって <Link path="guides/runtime-config">ランタイム設定</Link> の `connection` 名前空間に自動的に登録されるため、エージェントは実行時にそれを検出できます。
</Fragment>
...
MCP サーバーの AgentCore ランタイム ARN は、生成された Terraform モジュールによって <Link path="guides/runtime-config">ランタイム設定</Link> の `connection` 名前空間に自動的に登録されるため、エージェントは実行時にそれを検出できます。
</Fragment>
</Infrastructure>
