...
2. <Link path="guides/agentcore-gateway">`agentcore-gateway`</Link>プロジェクト（`auth: iam`）

GatewayはIAM認証を使用する必要があります — エージェントは自身の実行ロールを使用してSigV4でリクエストに署名します。ジェネレーターはCognito認証されたゲートウェイを拒否します。
