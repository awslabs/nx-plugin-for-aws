---
title: FastAPI から リレーショナルデータベースへ
...
1. FastAPI プロジェクト（`py#api --framework=fastapi` で生成）、<Link path="guides/fastapi">`py#api`</Link> ガイドを参照
...
`.scalars()` は結果の行を `ExampleModel` インスタンスにアンラップし、FastAPI がシリアライズできるようにします。
...
API Lambda 関数を**エグレス付きプライベートサブネット**にデプロイし、プライベート分離サブネットにはデプロイしないでください。実行時に、`session_context()` は AWS AppConfig からデータベース設定を取得しますが、これはパブリック AWS サービスエンドポイントであり、アウトバウンドインターネットアクセスが必要です。
