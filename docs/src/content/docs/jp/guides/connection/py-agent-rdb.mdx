...
データベースと同じ VPC 内でエージェントを実行し、`additional_iam_policy_statements` を介して `rds-db:connect` を付与し、セキュリティグループルールのペアでネットワークパスを開きます。`aws_vpc.main` および `aws_subnet` リソースは、データベースデプロイメントガイドで定義されています：
