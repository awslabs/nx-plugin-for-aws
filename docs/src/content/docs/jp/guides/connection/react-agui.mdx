...
フックはスレッド ID からセッション ID を導出し、AgentCore Runtime が必要とする 33 文字に右パディングします：
...
両方の値はブラウザから送信されるため、誰の会話であるかを示さずに会話に名前を付けます。各ユーザーを自分の会話に制限するには、<Link path="guides/ts-agent">セッションを所有者に制限する</Link>を参照してください。
