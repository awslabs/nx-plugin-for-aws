...
MCP サーバーをデータベースと同じ VPC 内で実行し、`additional_iam_policy_statements` を介して `rds-db:connect` を付与し、セキュリティグループルールのペアでネットワークパスを開きます。`aws_vpc.main` と `aws_subnet` リソースは、データベースデプロイメントガイドで定義されています：
