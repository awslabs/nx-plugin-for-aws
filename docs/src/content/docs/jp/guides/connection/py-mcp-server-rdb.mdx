...
`.scalars()` は結果行を `ExampleModel` インスタンスにアンラップするため、各アイテムで `model_dump()` が利用可能になります。
...
生成された MCP サーバーコンストラクトは `IGrantable` と `IConnectable` を実装しているため、コンストラクト上で直接データベースへのネットワークアクセスと IAM アクセスを許可できます。
