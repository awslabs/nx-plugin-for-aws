...
Pythonプロジェクトジェネレーターは、[Python](https://www.python.org/)のベストプラクティスで構成されたモダンなライブラリまたはアプリケーションを作成するために使用でき、[UV](https://docs.astral.sh/uv/)で管理され、単一のロックファイルと仮想環境を[UVワークスペース](https://docs.astral.sh/uv/concepts/projects/workspaces/)に配置し、テスト実行には[pytest](https://docs.pytest.org/en/stable/)、静的解析には[Ruff](https://docs.astral.sh/ruff/)を使用します。
...
<FileTree>
  ...
  - .python-version プロジェクトのPythonバージョンを含む

</FileTree>

また、ワークスペースのルートに次のファイルが作成/更新されていることに気づくでしょう：

<FileTree>

  - pyproject.toml UVのワークスペースレベルのパッケージング設定
  - .python-version ワークスペースのPythonバージョンを含む
...
テストは、プロジェクト内の `test` ディレクトリに、`test_` が先頭に付いたPythonファイルで作成する必要があります。例えば：
...
テストは `test_` で始まるメソッドで、期待する結果を検証するアサーションを行います。例：
...
同様に、ワークスペース内のすべてのパッケージのすべてのリント問題を修正する場合は、次を実行できます：
