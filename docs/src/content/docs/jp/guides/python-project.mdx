...
プロジェクトには `assemble` ターゲットもあります。これは `build` のアーティファクトのみの兄弟ターゲットです：リント、テスト、型チェックのゲートを実行せずに、プロジェクトがデプロイに貢献するもの（コンパイルされた出力、およびバンドル、コンテナイメージ、または宣言された生成仕様）を生成します。

<NxCommands commands={['assemble <project-name>']} />

<Link path="guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `build` ではなく `assemble` に依存しているため、デプロイはこれからデプロイするものだけをビルドします。`build` は変更されず、すべてを実行するため、プロジェクトをチェックするために実行するもの、そして CI で実行すべきものであり続けます。

:::caution
`assemble` は品質ゲートをスキップするため、デプロイはリントエラー、失敗したユニットテスト、または型エラーで失敗しなくなります。コードをチェックするには `build`（または CI パイプライン）を実行してください。
:::
