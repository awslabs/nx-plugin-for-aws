...
`type` オプションは、アプリケーション（デプロイ可能なプロジェクト、例えば Lambda 関数やコンテナイメージ）またはライブラリ（他のプロジェクトによってインポートされる再利用可能なコード）のどちらを取得するかを選択します。デフォルトは `application` なので、ライブラリの場合は `--type=library` を渡します：
...
`assemble` は開発中の高速な反復のために使用されます。本番環境へのデプロイメント前にプロジェクトが品質ゲートを通過することを保証するために、CI パイプラインでは `build` を実行してください。
:::
