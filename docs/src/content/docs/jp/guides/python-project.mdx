...
プロジェクトには `package` ターゲットもあります。これは `build` のアーティファクトのみの兄弟ターゲットです：リント、テスト、型チェックのゲートを実行せずに、プロジェクトがデプロイに貢献するもの（コンパイル済み出力、およびバンドル、コンテナイメージ、または宣言された生成仕様）を生成します。

<NxCommands commands={['package <project-name>']} />

<Link path="guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `build` ではなく `package` に依存するため、デプロイはデプロイしようとしているもののみをビルドします。`build` は変更されず、すべてを実行するため、プロジェクトをチェックするために実行するもの、そして CI で実行すべきものであり続けます。

:::caution
`package` は品質ゲートをスキップするため、デプロイはリントエラー、失敗したユニットテスト、または型エラーで失敗しなくなります。コードをチェックするには `build`（または CI パイプライン）を実行してください。
:::
