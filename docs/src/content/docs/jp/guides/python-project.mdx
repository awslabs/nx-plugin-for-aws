...
Python プロジェクトをランタイムコードとして使用する場合（例えば AWS Lambda 関数のハンドラーとして）、ソースコードとすべての依存関係のバンドルを作成する必要があります。<Link path="guides/python-lambda-function">`py#lambda-function`</Link>、<Link path="guides/fastapi">`py#api`</Link>、<Link path="guides/py-mcp-server">`py#mcp-server`</Link> ジェネレーターはこれを自動的に追加します。手動で追加するには、これらのジェネレーターが提供する形式に合わせて、`project.json` ファイルに以下のようなターゲットを追加します：
