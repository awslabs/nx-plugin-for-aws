...
生成されたコンストラクトは、関連する設定を`connection`名前空間に自動的に書き込みます。任意の名前空間に独自の値を書き込むこともできます。
...
- `default`環境への即座の**デプロイメント**
</Fragment>
<Fragment slot="terraform">
Terraformは、3つの`core/runtime-config/*`モジュールを通じてランタイム設定を配線します。ルートモジュールで次の順序で宣言します：

<Steps>

1. **`core/runtime-config/appconfig`** — ルートモジュールの上部付近で一度宣言します。AppConfigアプリケーション、環境、デプロイメント戦略、および名前空間ごとに1つの設定プロファイルを作成します。その`application_id`と`application_arn`出力は、ランタイムでランタイム設定を読み取るすべてのモジュール（エージェント、MCPサーバー、Lambda関数など）に渡されます。
...
2. **`core/runtime-config/entry`** — コントリビューションごとに1回の呼び出し。生成されたAPI、エージェント、MCPモジュールは、URLとARNを公開するために内部的にこれを呼び出します。カスタム設定を公開するには直接呼び出します。
...
3. **`core/runtime-config/appconfig-deployment`** — ルートモジュールの最後に一度宣言し、エントリを提供するすべてのモジュールをカバーする`depends_on`を指定します。提供されたエントリを名前空間ごとに1つのJSONに集約し、共有アプリケーションに対してホスト型設定バージョン+デプロイメントを作成します。
...
サーバーサイドのコンシューマーは、ランタイムで設定を読み取るために**AppConfigアプリケーションID**とIAM権限が必要です。生成されたコンストラクトはこれを自動的に処理します。

<Infrastructure>
<Fragment slot="cdk">
`appConfigApplicationId`を使用してAppConfigアプリケーションIDを取得し、`grantReadAppConfig()`を使用して読み取り権限を付与します：
...
:::note[自動登録]
生成されたAPIおよびエージェントコンストラクトはこれを自動的に行います — ランタイム設定を読み取る必要がある独自のLambda関数やコンピュートリソースを作成する場合にのみ、手動で行う必要があります。
:::
</Fragment>
<Fragment slot="terraform">
...
:::note[自動登録]
生成されたエージェントおよびMCPモジュールは`appconfig_application_id` + `appconfig_application_arn`入力を受け入れます — 共有`runtime_config_appconfig`出力を渡すと、環境変数とIAM権限が自動的に配線されます。カスタムLambda関数やコンピュートリソースの場合にのみ、手動で行う必要があります。
:::
</Fragment>
</Infrastructure>

:::tip[ローカル開発]
カスタム名前空間（例：`tables`、`features`）でランタイム設定を使用する場合、デプロイされたAppConfigアプリケーションを指すために、ローカルで単一の`RUNTIME_CONFIG_APP_ID`環境変数を定義するだけで済みます。これにより、ローカルのAPIやエージェントは、すべての個別の環境変数を複製することなく、デプロイされた環境と同じ設定を取得できます。
:::

## AppConfig経由のサーバーサイドアクセス

Lambda関数やエージェントなどのサーバーサイドコンシューマーは、[AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/)を使用して**AWS AppConfig**からランタイム設定を取得できます。
...
[`@aws-lambda-powertools/parameters`](https://docs.aws.amazon.com/powertools/typescript/latest/features/parameters)の`getAppConfig`を使用します：

`getAppConfig`は任意の有効なJSON値を返すため、期待する形状を宣言して結果をキャストします — そうしないと、ワークスペースの厳格な`tsconfig`の下でプロパティアクセスがコンパイルされません：
...
:::tip[境界でバリデーション]
キャストはコンパイラに形状を伝えますが、ランタイムでチェックしません。制御できない設定の場合は、代わりに[Zod](https://zod.dev/)スキーマで解析することで、不正な値が後で`undefined`として現れるのではなく、明確に失敗するようにします。
:::

  </TabItem>
  <TabItem label="Python" icon="seti:python">

[`aws_lambda_powertools.utilities.parameters`](https://docs.aws.amazon.com/powertools/python/latest/utilities/parameters/)の`get_app_config`を使用します：
...
ウェブサイトの場合、`connection`名前空間はS3バケットに`runtime-config.json`ファイルとしてデプロイされます。フロントエンドコードからこれらの値にアクセスする方法の詳細については、<Link path="guides/react-website#runtime-configuration">Reactウェブサイトのランタイム設定</Link>ガイドを参照してください。
