...
:::note[shadcn/ui]
`uxProvider`として[shadcn/ui](https://ui.shadcn.com/)を選択しました。生成されたコンポーネントとスタイルはTanStack Router / Viteと共に配置されます。さらにコンポーネントを追加する場合はShadcnのドキュメントを参照してください。
:::
</OptionFilter>

<OptionFilter when={{ uxProvider: 'None' }} description="No UX provider — bring your own component library">
:::note[UX Providerなし]
`uxProvider`として`None`を選択したため、コンポーネントライブラリやデザインシステムは組み込まれていません。お好みのコンポーネントライブラリを自由に追加できます。
:::
</OptionFilter>
