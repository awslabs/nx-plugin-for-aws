...
`serve-local`ターゲットは[Viteの`MODE`](https://vite.dev/guide/env-and-mode)を`serve-local`に設定したローカル開発サーバーを起動し、<Link path="/guides/connection">API接続ジェネレータ</Link>で接続したAPIのローカルサーバーも同時に起動します。
