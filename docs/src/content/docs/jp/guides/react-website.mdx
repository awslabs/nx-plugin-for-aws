...
:::note[`dev`スクリプト]
このジェネレータの実行時にルートの`package.json`に`dev`スクリプトが存在しない場合、生成されたウェブサイトの`serve-local`を呼び出す`dev`スクリプトが追加されます。これにより、以下のコマンドでもローカル開発サーバーを起動できます：

<PackageManagerShortCommand commands={["dev"]} />

これにより、最初に生成したReactウェブサイトがワークスペースのデフォルトの`dev`ターゲットになります。後続のウェブサイトは既存の`dev`スクリプトを上書きしません。それらの`serve-local`ターゲットを直接呼び出すか、スクリプトを手動で更新してください。
:::
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="ReactウェブサイトからPythonエージェントを呼び出す"
  ...
  <ConnectionCard
    ...
    description="ReactウェブサイトからTypeScriptエージェントを呼び出す"
  ...
  <ConnectionCard
    ...
    description="CopilotKit経由でAG-UIプロトコルを公開するエージェントをReactウェブサイトから呼び出す"
