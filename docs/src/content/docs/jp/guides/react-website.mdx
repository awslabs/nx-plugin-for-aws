...
export class ApplicationStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    // ウェブサイトは任意のタイミングで宣言可能 — ランタイム設定は遅延解決される
...
:::note
CDKでは、ウェブサイトコンストラクトはスタック内の任意のタイミングで宣言できます。ランタイム設定はsynth時に遅延解決されるため、宣言順序に関係なくすべての値が含まれます。
:::
</Fragment>
<Fragment slot="terraform">
...
:::note
ランタイム設定がAWS AppConfigにどのように保存され、サーバーサイドのコンシューマー（Lambda関数、エージェント）がどのように取得できるかについては、<Link path="guides/runtime-config">ランタイム設定</Link>ガイドを参照してください。
:::
