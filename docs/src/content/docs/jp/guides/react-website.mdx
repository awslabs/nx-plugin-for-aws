...
:::warning
このモードで`runtime-config.json`が存在しない場合、<Link path="/guides/react-website-auth">CloudScape Website Authジェネレータ</Link>でCognito認証を設定していても、ログインがスキップされ認証ヘッダーが付加されません。
