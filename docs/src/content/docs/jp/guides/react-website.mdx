...
<FileTree>
  ...
  - src
    ...
    - components
      ...
      - RuntimeConfig 起動時に`runtime-config.json`をロードするプロバイダー
    ...
    - hooks
      - useRuntimeConfig.tsx [ランタイム設定](#runtime-configuration)を読み取るためのフック
...
`S3_MANAGED`を選択すると、ウェブサイトおよびディストリビューションログバケットで、プロジェクトの`checkov`ターゲットが`CKV_AWS_145`で失敗します。ターゲットはプロジェクト独自の`checkov.yml`に対して`checkov`を実行するため、その`skip-check`リストにチェックを追加してください：
...
`checkov`ターゲットは`--config-file`を渡すため、`checkov`独自の`.checkov.yaml`の検出がオフになります — 他の場所に配置された設定ファイルは無視されます。
:::
...
ウェブサイトは、アプリケーションをレンダリングする前に`runtime-config.json`をロードする`RuntimeConfigProvider`（`src/components/RuntimeConfig`内）と、そこから値を読み取るための`useRuntimeConfig`フックとともに生成されます：
...
プロバイダーは、`runtime-config.json`が提供されていない場合は空の設定にフォールバックするため、まだ何もデプロイされていないウェブサイトでもレンダリングされます。`IRuntimeConfig`はデフォルトで`any`型です — 型安全なアクセスが必要な場合は、`src/components/RuntimeConfig/index.tsx`で絞り込んでください。
