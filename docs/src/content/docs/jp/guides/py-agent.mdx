...
- **`s3`**（デフォルト）：デプロイされたエージェントは、プロビジョニングされたセッションバケットで`S3CheckpointSaver`を使用し、`checkpoints/`プレフィックスの下にチェックポイントと保留中の書き込みを保存します。このクラス（[`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3)から適応、MITライセンス）は、共有エージェント接続プロジェクトの`s3_checkpoint_saver_langchain.py`にあります。
