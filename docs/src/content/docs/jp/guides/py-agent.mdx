...
Strandsエージェントの場合、ジェネレーターはデフォルトで[FastAPI](https://fastapi.tiangolo.com/)を使用してHTTPサーバーを公開します。または、他のA2A互換エージェントとの相互運用性のために[Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html)プロトコル、または[CopilotKit](https://docs.copilotkit.ai/)を介した直接的なフロントエンド統合のために[AG-UI](https://docs.ag-ui.com/)プロトコルを選択することもできます。

:::note[LangChainはAG-UIのみをサポート]
`langchain`フレームワークは現在`ag-ui`プロトコルのみをサポートしています。`http`または`a2a`プロトコルが必要な場合は、デフォルトの`strands`フレームワークを選択してください。LangChainエージェントは、<Link path="/guides/connection/py-agent-mcp">`connection`ジェネレーター</Link>を介してMCPサーバー、AgentCore Gateway、A2Aエージェントに接続できます。
:::
...
以下の例ではStrandsを使用しています。LangChainエージェントも同様に動作し、`langchain_core.tools`の`@tool`デコレータと`ChatBedrockConverse`モデルを使用した`create_agent`を使用します（`MODEL_ID`と`AWS_REGION`環境変数でモデルとリージョンを設定）。詳細は[LangChainドキュメント](https://docs.langchain.com/oss/python/langchain/overview)を参照してください。
...
<Link path="/guides/py-mcp-server">`py#mcp-server`</Link>や<Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>ジェネレーターで作成したMCPサーバーを利用する場合、<Link path="/guides/connection/py-agent-mcp">`connection`ジェネレーター</Link>を使用できます。
...
Strandsエージェントの詳細な作成方法は[Strandsドキュメント](https://strandsagents.com/docs/user-guide/quickstart/overview/)を参照してください。LangChainエージェントについては、<Link path="/guides/connection/py-agent-mcp">`connection`ジェネレーター</Link>がMCPサーバー、AgentCore Gateway、A2Aエージェントツールを`create_agent`に接続します。
...
チャットターゲットは単独で実行されます。デフォルトではローカルで実行中のエージェントに接続するため、まず`<your-agent-name>-serve-local`を（別のターミナルで）起動してください：
