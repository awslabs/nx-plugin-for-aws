...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChainは大規模な [ツールと統合](https://docs.langchain.com/oss/python/integrations/tools) のエコシステムを提供しており、それぞれが独自のパッケージで配布されています。ジェネレーターはこれらを追加しないため、まず必要なものをインストールしてください — 例えば、以下の検索ツールが含まれる `langchain-community`:

<NxCommands commands={['run <project-name>:add langchain-community']} />

次にツールをインポートして `get_agent()` に追加します:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
生成されたエージェントはAmazon Bedrock上のデフォルトのStrandsモデルを使用します。設定するには、`Agent` に `model` を渡します。利用可能なプロバイダーとそのオプションについては、[Strandsのモデルプロバイダーに関するドキュメント](https://strandsagents.com/docs/user-guide/concepts/model-providers/) を参照してください:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChainエージェントは [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse) モデルを使用します。生成されたエージェントは `MODEL_ID` および `AWS_REGION` 環境変数からモデルIDとリージョンを読み取ります。これらはインフラストラクチャがデプロイされたエージェント用に設定します。モデルをさらに設定するには、`get_agent()` 内で構築される場所に引数を追加します:
