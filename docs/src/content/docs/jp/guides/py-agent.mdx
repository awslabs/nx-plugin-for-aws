...
次に、ローカルエージェントが実行されているポートの `/invocations` にPOSTリクエストを送信します。エージェントに割り当てられたポートに置き換えてください — プロジェクトの `project.json` の `metadata.ports` から、または `<your-agent-name>-dev` ターゲットのコマンドの `--port` フラグから読み取ることができます。新しいワークスペースの最初のHTTPエージェントには `8081` が割り当てられます:
...
:::caution[`-dev` を使用し、`-serve` は使用しない]
これには `<your-agent-name>-dev` を使用し、`<your-agent-name>-serve` は使用しないでください。`-serve` はデプロイされたコードパスを実行するため、`RUNTIME_CONFIG_APP_ID` の設定が必要です — <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link> を参照してください。これがないと、エージェントがセッションストレージを解決しようとする時点でストリーミングが既に開始されているため、リクエストはエラーなしで空の `200` を返し、失敗はサーバーログにのみ表示されます。
:::
