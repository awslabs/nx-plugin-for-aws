...
AG-UIの`ag-ui-strands`アダプターは会話スレッドごとにテンプレート`Agent`をクローンするため、`get_session_manager`は`main.py`の`StrandsAgent`に渡される`StrandsAgentConfig`の`session_manager_provider`として配線され、共有テンプレートエージェントに組み込まれるのではなく、各スレッドに対して新しい`SessionManager`が解決されます。
</OptionFilter>

<OptionFilter when={{ protocol: ['http', 'a2a'] }} description="HTTP/A2A session wiring">
`with_session_id`はすでにセッションごとに1つの`Agent`インスタンスをキャッシュしているため、`get_session_manager`は`agent.py`の`get_agent`の`Agent(session_manager=get_session_manager())`呼び出し内で直接呼び出されます。
</OptionFilter>
...
- **`s3`**：デプロイされたエージェントは、プロビジョニングされたセッションバケットで`S3CheckpointSaver`を使用し、`checkpoints/`プレフィックスの下にチェックポイントと保留中の書き込みを保存します。このクラス（[`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3)から適応、MITライセンス）は、共有エージェント接続プロジェクトの`s3_checkpoint_saver_langchain.py`にあります。
...
- **`in-memory`**（デフォルト）：テーブルまたはバケットはプロビジョニングされません。会話状態は、実行中のプロセスの存続期間中のみメモリに保持され、再起動やスケールインには耐えられません。
