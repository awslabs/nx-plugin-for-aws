...
FastAPIジェネレータは、AWS CDKまたはTerraformのインフラストラクチャ設定を備えた新しいFastAPIを作成します。生成されるバックエンドはサーバーレスデプロイ用にAWS Lambdaを使用し、AWS API Gateway APIを介して公開されます。[AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) を設定し、ロギング、AWS X-Rayトレーシング、Cloudwatchメトリクスを含むオブザーバビリティを実現します。
...
また、`packages/common`ディレクトリにAPIをデプロイするためのCDKまたはTerraformのインフラストラクチャコードも生成されます。
...
構造化ロギングをAWS Lambda Powertoolsで設定します。ルートハンドラでロガーにアクセス:
...
リクエスト毎にCloudWatchメトリクスを自動収集。カスタムメトリクスを追加可能:
...
AWS API Gatewayはストリーミング応答をサポートしていないため、ストリーミングをサポートするプラットフォームにデプロイする必要があります。最も簡単なオプションはAWS Lambda Function URLを使用することです。
...
Terraformでは、生成されたAPI Gatewayインフラをストリーミング応答をサポートするLambda Function URLに置き換えます。

<details>
<summary>ストリーミング用Lambda Function URL設定例</summary>
...
# Function URLの出力
output "my_api_url" {
  description = "ストリーミングFastAPI Lambda関数のURL"
  value       = aws_lambda_function_url.my_api_url.function_url
}

# オプション: ランタイム設定用SSMパラメータ作成
...
resource "aws_iam_policy" "my_api_invoke_policy" {
  ...
  description = "ストリーミングFastAPI Lambda関数URLの呼び出しを許可するポリシー"
...
# 例: ポリシーをロールにアタッチ（必要に応じてコメント解除し修正）
...
ストリーミングレスポンスを消費するには、<Link path="guides/api-connection/react-fastapi#consuming-a-stream">API接続ジェネレータ</Link>を使用してチャンクを反復処理する型安全なメソッドを利用できます。
...
<Infrastructure>
<Fragment slot="cdk">
`common/constructs`フォルダにAPIデプロイ用CDKコンストラクトがあります。CDKアプリケーションで使用可能:
...
<Fragment slot="terraform">
`common/terraform`フォルダにTerraformモジュールがあります。Terraform設定で使用可能:
...
適切なTerraformリソースまたはモジュールでCognitoユーザープールとクライアントを設定可能
:::
...
```
Check: CKV_AWS_309: "API GatewayV2ルートで認証タイプを指定"
 FAILED for resource: aws_apigatewayv2_route.proxy_routes["PUT"]
```
</TabItem>
<TabItem label="REST API">
```
Check: CKV_AWS_59: "API経由のバックエンドリソースへのオープンアクセス防止"
 FAILED for resource: aws_api_gateway_method.proxy_method
```
</TabItem>
</Tabs>

APIを公開したい場合は[抑制コメントを追加](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html)可能
:::
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
FastAPIの操作はPythonで定義され、CDKインフラはTypeScriptで記述されるため、型安全な統合を提供するためメタデータをCDKコンストラクトに供給するコード生成を実装しています。

`common/constructs`の`project.json`に`generate:<ApiName>-metadata`ターゲットを追加し、`packages/common/constructs/src/generated/my-api/metadata.gen.ts`のようなファイルを生成します。ビルド時に生成されるためバージョン管理対象外です。

:::note
APIを変更する際は、CDKコンストラクトが使用する型を最新化するためビルドを実行する必要があります。

<NxCommands commands={["run-many --target build --all"]} />
:::

:::tip
CDKインフラとFastAPIを同時に開発する場合、[`nx watch`](https://nx.dev/nx-api/nx/documents/watch)を使用してAPI変更の度に型を再生成可能:
...
<Fragment slot="terraform">
:::note
Terraformでは型安全な統合をサポートしていないため、`iacProvider`にTerraformを選択した場合コード生成ターゲットは設定されません
:::
</Fragment>
</Infrastructure>
...
`IAM`認証を選択した場合、`grantInvokeAccess`メソッドでAPIへのアクセスを許可可能:
...
# IAMロールにポリシーをアタッチ（例: 認証済みユーザーロール）
...
ジェネレータが設定したローカル開発サーバを以下で起動:

<NxCommands commands={['run my-api:serve']} />

以下を含むローカルFastAPI開発サーバが起動:
...
ReactウェブサイトからAPIを呼び出すには<Link path="guides/api-connection/react-fastapi">`api-connection`</Link>ジェネレータを使用可能。