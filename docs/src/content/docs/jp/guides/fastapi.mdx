...
:::caution[`boto3`を上記のバージョンに固定]
他のジェネレーターは、正確にこのバージョンに固定された`boto3`を追加します。ワークスペースは単一のUV仮想環境とロックファイルを共有しているため、固定せずに追加すると（`add boto3`）、最新リリースが解決され、`boto3>=<latest>`制約が書き込まれます。これは`uv`が固定と並行して満たすことができません。その結果、ワークスペース内のすべてのPythonターゲットが依存関係の解決に失敗します。
:::
