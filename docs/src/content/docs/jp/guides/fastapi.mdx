...
- <Link path="guides/connection/react-fastapi">Connection</Link> ジェネレータを使用する場合は OpenAPI ベンダー拡張 `x-streaming: true` を追加する
...
ストリームレスポンスを消費するには、<Link path="guides/connection/react-fastapi#consuming-a-stream">Connection Generator</Link> を使用してストリームチャンクを反復処理するタイプセーフなメソッドを利用できます。
