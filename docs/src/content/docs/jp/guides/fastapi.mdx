...
:::tip[インテグレーションパターン]
`integrationPattern` オプションはデフォルトで `isolated` に設定されており、FastAPI の操作ごとに1つの Lambda を作成します。API 全体で単一の共有 Lambda ハンドラーを使用し、必要に応じて操作ごとにオーバーライドする場合は `shared` を選択してください。
:::
...
FastAPI ジェネレータは選択した `iac` に基づき CDK または Terraform のインフラストラクチャコードを作成します。これを使用して FastAPI をデプロイできます。
...
`UserIdentity` コンストラクトは <Link path="/guides/react-website-auth">`ts#website#auth` ジェネレータ</Link> を使用して生成できます
:::
</OptionFilter>
...
API モジュールは Lambda デプロイ zip を共有 S3 アセットバケットにステージングします — 詳細は <Link path="/guides/terraform-project">Terraform インフラストラクチャガイド</Link>を参照してください。デプロイごとに `core/asset-bucket` モジュールを一度インスタンス化し、その `bucket_name` 出力を `asset_bucket_name` 入力を介してすべての API / Lambda モジュールに渡します:
...
<Fragment slot="terraform">
:::note[Terraformの制限]
Terraform のタイプセーフ統合はサポートしていないため、`iac` に Terraform を選択した場合コード生成ターゲットは設定されません
:::
</Fragment>
</Infrastructure>
