...
`stageConfig`オプションを設定した場合、ジェネレータは集中認証情報管理用の2つの共有パッケージも作成します（まだ存在しない場合）:
...
:::note[ステージ設定]
このセクションは`stageConfig`で生成した場合に適用されます。これを使用しない場合、ジェネレータはAWS認証情報を自分で管理する（例: デプロイ前に`AWS_PROFILE`をエクスポートする）よりシンプルなセットアップを生成します。
:::
...
CDKの詳細については[CDK Developer Guide](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html)と[API Reference](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)を参照してください。de/core_concepts.html)と[API Reference](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)を参照してください。