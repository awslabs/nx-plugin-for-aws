...
:::note
CDKでは、ウェブサイトコンストラクトはスタック内の任意の位置で宣言できます — ランタイム設定は合成時に遅延解決されます。Terraformの場合、<Link path="guides/react-website#runtime-configuration">ランタイム設定</Link>に追加するモジュールに対して、ウェブサイトモジュールに適切な`depends_on`が設定されていることを確認してください。
:::
