...
`synth`は`^build`ではなく`^assemble`に依存しているため、合成はスタックがデプロイするアーティファクト（Lambdaバンドル、コンテナイメージ、生成された仕様）をビルドしますが、すべての上流プロジェクトのリント、テスト、型チェックゲートは実行しません。`assemble`が`build`とどのように関連するかについては、<Link path="guides/typescript-project#assembling">アセンブル</Link>を参照してください。
...
��発者ガイド](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html)と[APIリファレンス](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)を参照してください。
