...
`synth`は`^build`ではなく`^package`に依存しているため、合成はスタックがデプロイするアーティファクト（Lambdaバンドル、コンテナイメージ、生成された仕様）をビルドしますが、すべての上流プロジェクトのリント、テスト、型チェックゲートは実行しません。`package`が`build`とどのように関連しているかについては、<Link path="guides/typescript-project#packaging">パッケージング</Link>を参照してください。
...
`deploy-sandbox`と`deploy`は`^package`に依存しているため、デプロイしようとしているアーティファクトのみをビルドし、それ以外は何もビルドしません。`deploy-ci`はパイプラインが既にビルドしたアセンブリをデプロイするため、ビルド依存関係は全くありません。

:::caution
デプロイターゲットはリント、テスト、型チェックゲートをスキップするため、リントエラーや失敗したユニットテストでデプロイが失敗することはありません。重要なものをデプロイする前に`build`を実行し、パイプラインでそれをゲートにしてください：
