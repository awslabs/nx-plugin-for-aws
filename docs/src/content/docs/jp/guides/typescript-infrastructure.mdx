...
CDKでは、ウェブサイトコンストラクトはスタック内の任意の位置で宣言できます — ランタイム設定は合成時に遅延して解決されます。Terraformの場合は、<Link path="guides/react-website#runtime-configuration">ランタイム設定</Link>に追加するモジュールに対して、ウェブサイトモジュールに適切な`depends_on`を設定してください。
:::
...
`deploy`ターゲットは、開発中のデプロイを高速化するために、デフォルトで[CloudFormation expressモード](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)を使用します（CDKの`--express`フラグ経由）。Expressモードは、完全な安定化を待つのではなく、リソース設定が適用されたことが確認されると終了し、伝播はバックグラウンドで継続されるため、イテレーションサイクルが大幅に高速化されます。

Expressモードは開発とイテレーションを目的としています。Expressモードを使用せずに完全なリソース安定化を待つには、インフラストラクチャプロジェクトの`project.json`内の`deploy`ターゲットのコマンドから`--express`フラグを削除してください。

本番ステージへの実際のデプロイには、CI/CDパイプラインの一部として`deploy-ci`ターゲット（[下記](#deploying-to-aws-in-a-cicd-pipeline)を参照）を使用してください。このターゲットはexpressモードを使用しないため、常に完全な安定化を待ちます。
:::
