...
### 呼び出し元ごとのツールデータのスコープ設定

`createServer` は、現在のリクエストを記述する `McpRequestContext` を受け取ります。Streamable HTTP 経由では、これには呼び出し元の AgentCore Runtime セッション ID が含まれており、<Link path="guides/ts-agent">エージェント</Link>はすべての送信 MCP 呼び出しで自動的にこれを転送します。

呼び出し元ごとにデータを保存するツールに渡します：
...
:::caution
保存されたデータは、ツールの引数ではなく、リクエストコンテキストでスコープを設定してください。モデルは任意の値を渡すように説得される可能性があるため、ユーザー名やプレイヤー名などの引数は信頼境界ではありません。STDIO 経由ではリクエストが存在しないため、コンテキストは存在しません。
:::
