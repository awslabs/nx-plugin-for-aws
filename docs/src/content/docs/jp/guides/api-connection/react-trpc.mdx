...
Nx向けAWSプラグインは、<Link path="guides/trpc">tRPC API</Link>をReactウェブサイトに迅速に統合するためのジェネレーターを提供します。AWS IAM認証サポートや適切なエラーハンドリングを含む、tRPCバックエンド接続に必要なすべての設定を構成します。この統合により、フロントエンドとtRPCバックエンド間の完全なエンドツーエンド型安全性が実現されます。

## 前提条件

このジェネレーターを使用する前に、Reactアプリケーションが以下を満たしていることを確認してください：

1. アプリケーションをレンダリングする`main.tsx`ファイルが存在する
2. tRPCプロバイダーが自動注入される`<App/>` JSX要素が存在する
3. tRPCバックエンドジェネレーターで生成された動作可能なtRPCバックエンドが存在する
...
ジェネレーターはReactアプリケーション内に以下の構造を作成します：
...
- src
  ...
  - hooks
    - useSigV4.tsx SigV4を使用したHTTPリクエスト署名用フック（IAM専用）
...
ジェネレーターは型安全なtRPCクライアントにアクセスするための`use<ApiName>`フックを提供します：
...
ユーザーエクスペリエンス向上のため、ローディングとエラー状態を常に処理します：
...
ユーザーエクスペリエンス向上のためオプティミスティック更新を使用します：
...
型はバックエンドのルーターとスキーマ定義から自動的に推論されるため、APIに変更があってもビルドなしですぐにフロントエンドコードに反映されます。
