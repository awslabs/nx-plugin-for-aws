...
AWS Plugin for Nxは、<Link path="guides/trpc">tRPC API</Link>をReactウェブサイトに迅速に統合するためのジェネレーターを提供します。AWS IAM認証のサポートと適切なエラーハンドリングを含む、バックエンドへの接続に必要なすべての設定を行います。この統合により、フロントエンドとtRPCバックエンド間の完全なエンドツーエンドの型安全性が提供されます。
...
型は、バックエンドのルーターとスキーマ定義から自動的に推論され、APIへの変更がビルドせずに即座にフロントエンドコードに反映されることを保証します。

## 詳細情報

詳細については、[tRPC TanStack React Query のドキュメント](https://trpc.io/docs/client/tanstack-react-query/usage)を参照してください。

また、[TanStack Query のドキュメント](https://tanstack.com/query/v5)を直接参照することもできます。