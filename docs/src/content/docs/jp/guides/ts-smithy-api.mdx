...
CLIは[mise](https://mise.jdx.dev/)によって解決され、開発依存関係としてワークスペースに追加されるため、インストールする必要はありません。初回ビルド時に固定されたバージョンをダウンロードしてキャッシュします。

:::caution[Windows]
`mise`はnpmにWindowsパッケージを公開していないため、ビルド前に[Smithy CLIインストールガイド](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html)に従って自分でインストールしてください。
:::

:::tip[CIでのビルド]
`mise`はGitHub APIからCLIのリリースメタデータを読み取りますが、認証なしのレート制限はIPアドレスごとに共有されており、CIランナーで簡単に使い果たされます。その場合、ダウンロードは`403 Forbidden`で失敗します。ビルド環境に`GITHUB_TOKEN`を設定することで制限を解除できます。その後のビルドはローカルキャッシュを使用し、リクエストは一切行われません。
:::
