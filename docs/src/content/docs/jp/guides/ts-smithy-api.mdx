...
CLIは[mise](https://mise.jdx.dev/)によって解決され、開発依存関係としてワークスペースに追加されているため、インストールは不要です — 初回ビルド時にピン留めされたバージョンをダウンロードしてキャッシュします。

:::caution[Windows]
`mise`はnpmにWindowsパッケージを公開していないため、ビルド前にSmithy CLIを自分でインストールしてください：
...
他のオプションについては[Smithy CLIインストールガイド](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html)を参照してください。
:::
