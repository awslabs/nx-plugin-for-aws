...
macOSとLinuxでは、CLIは[mise](https://mise.jdx.dev/)によって解決され、ビルドがオンデマンドで取得するため、インストールする必要はありません。初回ビルド時に固定されたバージョンをダウンロードしてキャッシュします。Windowsでは、CLIは自分でインストールする前提条件であり、ワークスペースのアップグレードではアップグレードされません。詳細はSmithyプロジェクトガイドの<Link path="guides/smithy-project#building-on-windows">Windowsでのビルド</Link>と<Link path="guides/smithy-project#using-the-smithy-cli-directly">Smithy CLIの直接使用</Link>を参照してください。
