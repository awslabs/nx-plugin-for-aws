...
[Terraform](https://www.terraform.io/)は、インフラストラクチャを安全かつ予測可能に作成・変更・改善できるオープンソースのInfrastructure as Codeツールです。
...
アプリケーションプロジェクト（`--type=application`）の場合、リモートステート管理を含む完全なTerraformアプリケーションが生成されます:
...
:::tip
アプリケーションプロジェクトにはリモートステート管理を含む完全なデプロイ機能が含まれますが、ライブラリプロジェクトは他のプロジェクトで消費できる再利用可能なTerraformモジュール作成用に設計されています。
:::
...
新しい環境を追加するには、環境固有の変数を含む`src/env/<environment>.tfvars`ファイルを作成し、`project.json`の`apply, destroy, init, plan`に新しいエントリを追加します。例: `prod`環境を追加する場合:
...
環境間でtfstateが自動的に分割されるため、いつでも環境を切り替え可能です。
:::
...
ブートストラップターゲットはアプリケーションタイププロジェクトでのみ利用可能です。ライブラリプロジェクトは再利用可能モジュール用のためリモートステート管理を必要としません。
:::
...
プラン作成後、`apply`ターゲットでインフラストラクチャをAWSにデプロイ:

<NxCommands commands={['run tf-infra:apply']} />

:::tip
上記コマンドは`plan`ターゲットで作成されたプランを適用します。変更を確認するためまず`plan`を実行してください。
:::
