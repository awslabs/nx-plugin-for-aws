...
<FileTree>
  ...
  - scripts nxの`bootstrap`と`init`ターゲットで実行されるNodeヘルパー
    - aws-config.ts AWS SDK認証情報チェーンを介してアカウント+リージョンを解決
    - bootstrap.ts ブートストラップtfstateをプル/プッシュし、`terraform apply`を実行
    - init.ts S3バックエンド設定で`terraform init`を実行
    - paths.ts 実行時にワークスペース+プロジェクトルートを特定
  - project.json プロジェクト設定とビルドターゲット

</FileTree>

:::note[共有Terraformモジュール]
...
:::tip[デフォルト環境]
デフォルトではすべてのターゲットが`dev`環境を想定しますが、特定の環境を指定できます: <NxCommands commands={['apply tf-infra --configuration=prod']} />
...
インフラをデプロイする前にリモートステートバックエンドのブートストラップが必要です。これによりTerraformステートファイルを保存するS3バケットが作成されます:
...
:::note[Deployエイリアス]
`deploy`ターゲットは`apply`のエイリアスとしても利用できます。<NxCommands commands={['deploy tf-infra']} />と<NxCommands commands={['apply tf-infra']} />は同じように使用できます。
:::
