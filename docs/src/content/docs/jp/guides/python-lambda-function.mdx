...
Lambdaファンクションの生成時に `eventSource` を選択した場合、関数は [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/) の `@event_parser` で計装されます。例：
...
イベントを型付けしたくない場合は、`eventSource` に `Any` を選択できます。
