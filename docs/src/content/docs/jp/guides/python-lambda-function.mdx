...
AWS X-Rayトレースが自動的に設定されます。トレースにカスタムサブセグメントを追加できます：
...
Lambdaファンクション生成時に `eventSource` を選択した場合、関数は [AWS Lambda Powertoolsの `@event_parser`](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/) で計装されます。例：
...
これにより、[Pydantic](https://docs.pydantic.dev/latest/) を使用してデータモデルを定義でき、<Link path="guides/fastapi">Fast API</Link> での作業と同様の方法で行えます。

:::tip
DynamoDBストリームやEventBridgeイベントなど、イベント内にカスタムデータがネストされている場合、そのカスタムデータの型安全性を提供するために[エンベロープ](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/#envelopes)の使用が有益な場合があります。
:::
...
:::note
イベントソースが選択された`eventSource`オプションと一致することを確認し、ハンドラー関数内でイベントが適切に処理されるようにしてください。
:::

以下の例は、Event Bridgeを使用してスケジュールに基づいてラムダ関数を呼び出すためのCDKコードを示しています：
