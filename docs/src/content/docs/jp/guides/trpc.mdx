...
この例では、`IAM` 認証用の identity ミドルウェアについて説明します。API Gateway イベントから抽出した sub を使用して Cognito で呼び出し元を検索します。`Cognito` 認証の場合、identity ミドルウェアはより簡単です — 代わりに `event` から直接クレームを抽出してください。
...
Cognito User Pool と Client を接続するには、関連する変数を指定します:
