...
:::tip
`integrationStyle` オプションはデフォルトで `Individual Functions` に設定されており、tRPC プロシージャごとに1つの Lambda を作成します。API 全体で単一の共有 Lambda ハンドラーを使用したい場合は `Router` を選択してください。
:::
...
tRPC ジェネレータは `integrationStyle` オプションを通じて2つのインフラストラクチャパターンをサポートしています:

- `Individual Functions` がデフォルトです。プロシージャごとに1つの Lambda 関数を生成します。
- `Router` は API 全体で単一の共有 Lambda 関数を生成し、すべてのインテグレーションでそれを再利用します。

`Individual Functions` はより詳細なインフラストラクチャを提供し、操作ごとに異なる権限や設定を付与しやすくします。`Router` は Lambda とインテグレーションの増加を抑制し、小規模な API や単一の中央ハンドラーを好む場合に便利です。

両方のインテグレーションスタイルは現在 `ServerlessApiGatewayRestApi` と `ServerlessApiGatewayHttpApi` でサポートされています。
...
:::warning
この例は `auth` が `IAM` に設定されていることを想定しています。Cognito 認証の場合、`event` から関連するクレームを抽出するため、identity ミドルウェアはより簡単です。
:::
