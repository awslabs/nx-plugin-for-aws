...
tRPC ジェネレータは `integrationPattern` オプションを通じて2つのインフラストラクチャパターンをサポートしています:

- `isolated` がデフォルトです。プロシージャごとに1つの Lambda 関数を生成します。
- `shared` は API 全体で単一のデフォルト `$router` Lambda インテグレーションを生成し、プロシージャごとのオーバーライドを提供しない限り、すべてのプロシージャでそれを再利用します。

`isolated` はより詳細なインフラストラクチャを提供し、操作ごとに異なる権限や設定を付与しやすくします。`shared` は Lambda とインテグレーションの増加を抑制しながら、必要に応じて特定のプロシージャを専用のインテグレーションでオーバーライドできます。

両方のインテグレーションパターンは現在 `ServerlessApiGatewayRestApi` と `ServerlessApiGatewayHttpApi` でサポートされています。
