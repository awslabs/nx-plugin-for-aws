...
2. 共有 `core/asset-ecr` リポジトリがイメージを保持します。ワークスペース内のすべてのコンテナがここに公開されるため、独自のリポジトリをプロビジョニングするものはありません — デプロイごとにモジュールを 1 回インスタンス化し、その `repository_url` をイメージを公開する各モジュールにスレッドします。
