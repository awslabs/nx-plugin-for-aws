...
`shx` パッケージは、クロスプラットフォームの `cp`、`rm`、`mkdir` コマンドを提供し、シェルビルトイン（Windows では利用できません）を回避します。ワークスペースのルートで `pnpm add -D -w shx` を使用してインストールします。

ディレクトリをコピーするには、`-R` とソースの末尾に `/.` が必要です — `shx cp -R src/. dst` — これにより、内容が `dst` にマージされます。`/.` がない場合、ソースは `dst` の内部にネストされ、`src/*` グロブとは異なり、ドットファイルも含まれます。
:::
