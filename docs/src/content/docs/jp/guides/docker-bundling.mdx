...
- Nx はバンドルターゲットを**キャッシュ**できるため、繰り返しのビルドが高速になります。
...
イメージはプロジェクトソースによって完全に決定されるため、`inputs`（ここでは `default` と `^production`、`bundle` ターゲットと一致）を宣言することで、Nx はスキャンをキャッシュし、変更されていないイメージの再スキャンをスキップできます。Trivy イメージバージョンをピン留めして（例：<code>trivy:<TrivyVersion /></code>）、スキャンを再現可能にします。
:::
