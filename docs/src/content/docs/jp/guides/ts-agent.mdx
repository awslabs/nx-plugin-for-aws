...
これは `tsx --watch` を使用して、ファイルが変更されたときにサーバーを自動的に再起動します。エージェントは `http://localhost:8081`（または複数のエージェントがある場合は割り当てられたポート — プロジェクトの `project.json` の `metadata.ports` から読み取ります）で利用可能になります。

`<your-agent-name>-serve` ターゲットも生成されます。これはデプロイされたインフラストラクチャに対してエージェントを実行するため、`RUNTIME_CONFIG_APP_ID` を設定する必要があります。`dev` と `serve` の違いについては、<Link path="guides/local-development">Local Development</Link> ガイドを参照してください。
...
エージェントに割り当てられたポートに置き換えてください — プロジェクトの `project.json` の `metadata.ports` から読み取ります。
...
<CardGrid>
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'jp'}/guides/connection/react-ts-agent`}
    source="react"
    target="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'jp'}/guides/connection/ts-agent-mcp`}
    source="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'jp'}/guides/connection/ts-agent-dynamodb`}
    source="strands"
    sourceBadge="typescript"
    target="dynamodb"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'jp'}/guides/connection/ts-agent-gateway`}
    source="strands"
    sourceBadge="typescript"
    target="agentcore"
  />
