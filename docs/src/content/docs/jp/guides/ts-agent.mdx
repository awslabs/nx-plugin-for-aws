...
ジェネレーターは、[`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) を使用してエージェントとのインタラクティブなターミナルチャットを可能にする `<your-agent-name>-chat` Nx ターゲットを設定します。

チャットターゲットはスタンドアロンで実行されます — エージェントを自動的に起動**しません**。デフォルトではローカルで実行中のエージェントに接続するため、まず `<your-agent-name>-serve-local` を (別のターミナルで) 起動してください:
