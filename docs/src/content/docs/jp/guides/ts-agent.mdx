...
React ウェブサイトから Strands Agent を呼び出す場合は、<Link path="/guides/connection/react-ts-agent">`connection` ジェネレーター</Link>を利用できます。これにより、正しい認証 (IAM または Cognito) を使用した tRPC WebSocket クライアントが自動的にセットアップされます。
