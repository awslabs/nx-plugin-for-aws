...
:::note[コールバック URL]
生成されたユーザープールクライアントは、OAuth コールバック/ログアウト URL として `http://localhost:4200` と `http://localhost:4300`（デプロイされたプールに対するローカル開発用）、および CloudFront ディストリビューション URL を許可します。この許可リストは最小限に保ってください: 本番環境のセキュリティを強化するには、デプロイされたユーザープールに対してローカルで実行しなくなったら、localhost URL を削除する（またはローカル開発用に別のクライアントを使用する）ことを検討してください。
:::
