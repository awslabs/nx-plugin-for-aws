...
ジェネレータは選択した`iac`に基づいて関数をデプロイするためのインフラストラクチャコードを作成します:
...
Lambda関数生成時に`event`を選択した場合、関数は[AWS Lambda Powertoolsの`parser`ミドルウェア](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/)で計装されます。例:
...
`event`に`Any`を選択した場合、`parser`ミドルウェアは組み込まれず、`event`パラメータは`any`型になります。コンパイル時の型安全性とランタイムバリデーションが必要な場合は、特定の`event`を指定して関数を再生成してください。
</OptionFilter>
