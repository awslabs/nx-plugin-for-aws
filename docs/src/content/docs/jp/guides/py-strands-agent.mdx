...
`protocol`が`AG-UI`に設定されている場合、エントリーポイントは[ag-ui-strands](https://docs.ag-ui.com/)統合を使用し、[CopilotKit](https://docs.copilotkit.ai/aws-strands)との直接的なフロントエンド統合のためにAG-UIプロトコル経由でエージェントを公開します：
...
`protocol=AG-UI`の場合、生成される`main.py`はStrandsの`Agent`を`ag_ui_strands.StrandsAgent`でラップし、`create_strands_app()`を介してFastAPIアプリを作成します。作成されたアプリは、Server-Sent Events（SSE）経由で[AG-UI](https://docs.ag-ui.com/)イベントをストリーミングする単一のPOSTエンドポイント、およびAgentCoreランタイムのヘルスチェック用の`/ping`を公開します。
...
ジェネレーターは`<your-agent-name>-invoke`という名前のターゲットを設定し、ターミナルからエージェントとのインタラクティブなチャットを開始します。TypeScriptスクリプト（`tsx`経由）として提供され、エージェントに適したプロトコル（HTTP、A2A、AG-UI）を自動的に選択します。デフォルトではローカルサーバーに接続します：

<NxCommands commands={['agent-invoke your-project']} />

代わりにデプロイされたエージェントとチャットするには、`RUNTIME_CONFIG_APP_ID`をワークスペースのAppConfigアプリケーションIDに設定します。CLIはランタイムARNを解決し、SigV4署名付きリクエストでデプロイされたエンドポイントを呼び出します：

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Cognito認証されたエージェントの場合、`--accessToken`でJWTを渡します：

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

CLIスクリプトはプロジェクトの`scripts/<agent>/cli.ts`にあります。
