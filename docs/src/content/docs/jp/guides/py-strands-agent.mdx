...
ジェネレーターは、`<your-agent-name>-serve-local`に依存する`<your-agent-name>-chat` Nxターゲットを設定します。実行するとエージェントがローカルで起動し、[`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)を使用したインタラクティブなターミナルチャットに入ります：

<NxCommands commands={['run your-project:agent-chat']} />

**A2A**および**AG-UI**エージェントの場合、チャットターゲットはローカルで実行中のエージェントに対して`agent-chat-cli`をバイナリとして呼び出すだけで、ワークスペースにコードは生成されません。

**HTTP**エージェントの場合、ジェネレーターは以下を生成します：

- `scripts/<your-agent-name>_openapi.py` — エージェントのOpenAPI仕様をエクスポートする小さなスクリプト
- `<your-agent-name>-openapi` Nxターゲット（これを実行）
- `<your-agent-name>-generate-client` Nxターゲット（`open-api#ts-client`を実行して`scripts/<your-agent-name>/generated/`に型安全なTypeScriptクライアントを生成）
- `scripts/<your-agent-name>/chat.ts` — 生成されたクライアントをインポートし、`agent-chat-cli`の`chatLoop`に接続する薄いアダプター

チャットスクリプトは生成されたクライアントの小さなラッパーです。エージェントの入力形状をカスタマイズする場合（例：`InvokeInput`に新しいフィールドを追加）、アダプターの`sendMessage`を更新して新しいフィールドを渡すと、残りは自動的に機能します。
