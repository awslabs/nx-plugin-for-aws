...
`protocol`が`A2A`に設定されている場合、エントリーポイントはFastAPIの代わりに[Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)を使用します：
...
エージェントのサーバープロトコルは通信方法を決定します。両方のオプションは[FastAPI](https://fastapi.tiangolo.com/)で提供されますが、エントリーポイントが異なります：
...
両方のプロトコルは、AgentCoreランタイムのヘルスチェック契約用に`/ping`を公開します。A2Aエージェントはポート`9000`でリッスンします（HTTPは`8080`）。生成されるDockerfileとインフラストラクチャは自動的に設定されます。
...
`protocol=A2A`の場合、生成される`main.py`は`A2AServer.to_fastapi_app()`を親FastAPIアプリにマウントし、`/ping`も公開します。AgentCoreにデプロイすると、エントリーポイントはAppConfigからランタイムのパブリックARNを解決し、エージェントカードに通知します。
...
Strands SDKの`A2AAgent`はリモートA2Aエンドポイントをラップし、ローカル`Agent`のように呼び出すことができます。
...
#### IAM認証を使用したAgentCoreでの呼び出し

AgentCoreにデプロイされたA2Aエージェントへのリクエスト（エージェントカードの取得を含む）はSigV4署名が必要です。SigV4認証フローを使用する`httpx.AsyncClient`を含む`client_config`を`A2AAgent`に渡します：
...
:::note[IAM権限]
呼び出し元のIAMロールには、ターゲットランタイムARNに対する`bedrock-agentcore:InvokeAgentRuntime`と`bedrock-agentcore:GetAgentCard`の両方が必要です。生成されたコンストラクトの`grantInvokeAccess`ヘルパーは両方を付与します。
:::

:::note[追加の依存関係]
A2Aジェネレーターは`strands-agents[a2a]`を追加します。HTTPエージェントにA2Aクライアントコードを追加する場合は、`pyproject.toml`に`strands-agents[a2a]`と`httpx`を追加してください。
:::

#### A2Aエージェントをツールとして使用

`A2AAgent`はローカル`Agent`と同様に`AgentBase`を継承しているため、`@tool`でラップして別のエージェントに渡すことができます：

```python
from strands import Agent, tool

remote = # 上記のclient_configで構築されたA2AAgent

async def ask_remote_agent(prompt: str) -> str:
    """質問をリモートA2Aエージェントに委譲し、その返答を返す。"""
    result = await remote.invoke_async(prompt)
    return str(result)

ask_remote = tool(
    name="ask_remote_agent",
    description="質問をリモートA2Aエージェントに委譲し、その返答を返す。",
)(ask_remote_agent)

agent = Agent(
    system_prompt="適切な場合はask_remote_agentに委譲する。",
    tools=[ask_remote],
)
```

CDKスタックで、ホストエージェントにリモートエージェントへの呼び出しアクセスを付与します。これにより、A2Aクライアントが必要とする両方のIAMアクションが設定されます：
