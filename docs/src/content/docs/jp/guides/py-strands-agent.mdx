...
:::tip[Reactウェブサイトへの接続]
AG-UIエージェントは、<Link path="/guides/connection/react-agui">`react` → AG-UIエージェント接続ジェネレーター</Link>を使用してReactフロントエンドに接続でき、リッチなチャット体験のための[CopilotKit](https://docs.copilotkit.ai/aws-strands)コンポーネントがセットアップされます。
:::
...
:::tip[Reactウェブサイトへの接続]
AG-UIエージェントは、フロントエンドから直接利用されるように設計されています。<Link path="/guides/connection/react-agui">`react` → AG-UIエージェント接続ジェネレーター</Link>を使用して、[CopilotKit](https://docs.copilotkit.ai/aws-strands)プロバイダーと[AG-UI HttpAgent](https://docs.ag-ui.com/)クライアントでReactウェブサイトをエージェントに接続してください。
:::
