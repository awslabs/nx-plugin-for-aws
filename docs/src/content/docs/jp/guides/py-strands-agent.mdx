...
代わりに構造化データを生成するPydanticモデルを定義できます。これを行うには、`handle_invoke`が生成するPydanticモデルをシリアライズする必要があります：
...
:::caution
ここで注意すべき点は、提供されるサーバー（および上記の`serialize_stream`メソッド）は現在Server-Sent Events（SSE）標準に準拠していないため、必要に応じてフロントエンドから生成されたTypeScriptクライアントでエージェントを呼び出すことができますが、現在はSSE形式のストリーミングイベントを想定していません。この問題が影響する場合は、優先順位付けのために[このGitHub issue](https://github.com/awslabs/nx-plugin-for-aws/issues/369)に+1をお願いします。
:::
