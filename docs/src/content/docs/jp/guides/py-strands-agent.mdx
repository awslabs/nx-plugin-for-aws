...
ジェネレーターは、`scripts/<your-agent-name>/chat.ts`の下に最小限のインタラクティブチャットCLIと、対応する`<your-agent-name>-chat`ターゲットも提供します。このターゲットは`<your-agent-name>-serve-local`に依存しているため、実行するとエージェントがローカルで起動し、インタラクティブなターミナルチャットに入ります：

<NxCommands commands={['run your-project:agent-chat']} />

このCLIは（`tsx`経由で実行される）TypeScriptスクリプトで、プロトコルに適したクライアントを使用して[`@clack/prompts`](https://github.com/bombshell-dev/clack)経由でレスポンスをトークンごとにストリーミングします：

- **HTTP**: JSONLストリーミングを使用した`POST /invocations`
- **A2A**: `sendMessageStream`を使用した`@a2a-js/sdk` `ClientFactory`
