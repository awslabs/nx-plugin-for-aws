...
- **`http`** — パスベースのルーティング（`/<targetName>/invocations`）を介して、[AgentCore Runtimeターゲット](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-target-http-runtime.html)（エージェント）にリクエストを直接プロキシします。集約やプロトコル変換は行いません。これを使用して、単一の管理されたエンドポイントでエージェントをフロントします — たとえば、ウェブサイトがVPC内にデプロイされたエージェントにGatewayを介して到達できるようにします。
...
<FileTree>
  - packages/\<name>/
    ...
    - local-dev.ts ローカル開発用のローカルゲートウェイ — アタッチされたMCPサーバーを集約（`mcp`）またはアタッチされたエージェントをプロキシ（`http`）
    - project.json `serve`および`dev`ターゲットを追加
</FileTree>
...
- インバウンドIAM認証（デフォルト）またはCognito JWT認証（<a href="#authentication">認証</a>を参照）を使用した`AgentCore::Gateway`。`mcp`ゲートウェイはMCPプロトコル用に構成されます；`http`ゲートウェイはプロトコルタイプを持たず、これはAgentCoreがランタイムターゲットに必要とします
- `ENFORCE`モードで実行され、Gatewayにアタッチされた`AgentCore::PolicyEngine`（`mcp`ゲートウェイのみ；`cedarPolicy: false`の場合は省略）
...
`auth`オプションは、Gatewayがインバウンドリクエストを認証する方法を設定します。`IAM`（デフォルト）と`Cognito`から選択します。

### IAM

デフォルトでは、Gatewayは`GatewayAuthorizer.usingAwsIam()`で構成されています。呼び出し元はSigV4でリクエストに署名し、呼び出し元のIAMアイデンティティは`AgentCore::IamEntity`プリンシパルとしてCedarポリシーで利用できます。これは、呼び出し元がAWSで実行されているエージェントまたはサービスである場合に推奨されるオプションです — 例えば、<Link path="guides/connection/ts-agent-gateway">エージェントからGatewayへの接続ジェネレーター</Link>を介して接続された<Link path="/guides/ts-agent">エージェント</Link>は、独自の実行ロールで呼び出しに署名します。

### Cognito

`cognito`を選択すると、GatewayはCognitoユーザープールを指すカスタムJWTオーソライザーで構成されます。呼び出し元はJWTベアラートークンを提示して認証し、トークンの`sub`クレームは`AgentCore::OAuthUser`プリンシパルとしてCedarポリシーで利用できます。呼び出し元がCognitoを介して認証する場合に使用します — 例えば、ウェブサイトや<Link path="guides/ts-dcr-proxy">`ts#dcr-proxy`ジェネレーター</Link>を介して接続するコーディングエージェントなどです。

生成されたインフラストラクチャは、既存のCognitoユーザープールとクライアントを使用します — それらを作成しません。<Link path="/guides/react-website-auth">`ts#website#auth`ジェネレーター</Link>を使用して`UserIdentity`を生成するか、独自のものを提供できます。

<Infrastructure>
<Fragment slot="cdk">
生成されたコンストラクトには、ユーザープールとクライアントを提供する`identity`プロパティが必要です：
...
<Fragment slot="terraform">
生成されたモジュールには、`user_pool_id`および`user_pool_client_ids`変数が必要です：
...
:::note[カスタムOIDCプロバイダー]
Cognito以外のOIDCプロバイダーで認証するには、生成されたGatewayコンストラクトまたはTerraformモジュールを直接変更します — オーソライザーのディスカバリーURLと許可されたクライアントをプロバイダーのものに交換します。エージェントからGatewayへの接続ジェネレーターは、エージェントがSigV4で呼び出しに署名するため、`IAM`認証されたゲートウェイのみをサポートします。
:::
...
ジェネレーターは、Gatewayの認証タイプに応じたデフォルトポリシーを提供します。

**IAM** Gatewayの場合、Gatewayがデプロイされる先のAWSアカウントからの任意のIAM呼び出し元を許可します：
...
**Cognito** Gatewayの場合、認証された任意のOAuthユーザーを許可します（JWTオーソライザーは、ポリシーが評価される前にトークンのユーザープールとクライアントを既に検証しています）：
...
プリンシパルタイプは、Gatewayが呼び出し元を認証する方法によって異なります（<a href="#authentication">認証</a>を参照）。
...
**Cognito** Gatewayの場合、呼び出し元はJWTトークンの`sub`クレームから構築されたOAuthユーザーであり、JWTクレーム（ユーザー名、スコープなど）はプリンシパルタグとして利用できます：

```
principal is AgentCore::OAuthUser
```

`when`句で個々のクレームを一致させます — 例えば、スコープを要求するには：
...
ジェネレーターは、Gatewayプロジェクトに`dev`ターゲットを追加します。このターゲットは`local-dev.ts`を実行します。これを実行すると、ローカルゲートウェイとアタッチされたすべてのターゲットが一緒に起動します：

<NxCommands commands={["dev <name>"]} />

`mcp`ゲートウェイの場合、ローカルゲートウェイは、デプロイされたGatewayに一致するように、ツールに`<target>___<tool>`のプレフィックスが付けられた、アタッチされたすべてのMCPサーバー（<Link path="guides/connection/agentcore-gateway-mcp">`agentcore-gateway#mcp-connection`ジェネレーター</Link>を介して接続）を集約する単一のMCPエンドポイントを公開します。

`http`ゲートウェイの場合、ローカルゲートウェイは、デプロイされたGatewayのパスベースルーティングに一致するように、`/<targetName>/...`パスを各アタッチされたエージェントのローカルサーバー（<Link path="guides/connection/agentcore-gateway-agent">`agentcore-gateway#agent-connection`ジェネレーター</Link>を介して接続）にプロキシします。
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="AgentCore Gatewayをランタイムターゲットとしてエージェントをフロントする"
  ...
  <ConnectionCard
    ...
    description="ReactウェブサイトをAgentCore Gatewayを介してエージェントに接続する"
