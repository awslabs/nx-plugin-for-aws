...
`--languages` を指定せず、`targetLanguages` が空のままの場合、ターゲットは `ERROR No target languages configured` で非ゼロで終了します。
:::
...
3. 結果の翻訳を PR ブランチにコミットバックします。コミットメッセージには `docs: update translations` を使用してください。これにより、後続の増分実行がベースラインコミットを検出し、それ以降に変更されたファイルのみを再翻訳できます。別のメッセージを使用する場合は、`scripts/translate.config.json` に `translationCommitMessage` キーを追加し、その値と一致させてください。
