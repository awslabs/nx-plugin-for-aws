...
つまり、チェックを明示的に実行する必要はありませんが、NxCommandsの`license-check`ターゲットで実行することもできます：
...
恒久的に無効にするには、`aws-nx-plugin.config.mts`の設定から`license.dependencies`キーを削除します。また、`--dependencyCheck=false`で`license`ジェネレータを再実行して、それなしでスキャフォールドすることもできます — ジェネレータフラグは引き続き`--dependencyCheck`という名前で、ジェネレータが`dependencies`ブロックを書き込むかどうかを制御します。