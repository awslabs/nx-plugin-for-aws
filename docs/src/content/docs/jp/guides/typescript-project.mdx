...
pnpmでは、ワークスペースは[`catalogMode: strict`](https://pnpm.io/settings#catalogmode)も設定するため、`pnpm add`がバージョンをカタログに記録し、自動的に参照します。

yarnとbunには同等の機能がなく、それらの`catalog:`プロトコルは既存のカタログエントリを*参照*するだけで、新しいエントリを作成できません。yarnまたはbunで新しい依存関係を追加するには、まずカタログにバージョンを記録してから、プロジェクトから参照します:

- **yarn** — `.yarnrc.yml`の`catalog`にバージョンを追加してから、`yarn workspace @my-scope/my-library add some-npm-package@catalog:`を実行
- **bun** — ルートの`package.json`の`catalog`にバージョンを追加してから、`bun add some-npm-package@catalog: --cwd packages/my-library`を実行
