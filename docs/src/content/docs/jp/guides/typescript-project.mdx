...
プロジェクトには `package` ターゲットもあります。これは `build` のアーティファクトのみの兄弟です：リント、テスト、型チェックのゲートを実行せずに、プロジェクトがデプロイメントに貢献するもの（コンパイルされた出力、およびそれが宣言するバンドル、コンテナイメージ、生成された仕様）を生成します。

<NxCommands commands={['package <project-name>']} />

<Link path="/jp/guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `build` ではなく `package` に依存しているため、デプロイはこれからデプロイしようとしているもののみをビルドします。`build` は変更されず、すべてを実行するため、プロジェクトをチェックするために実行するもの — そして CI で実行すべきもの — のままです。

:::caution
`package` は品質ゲートをスキップするため、デプロイはリントエラーや失敗したユニットテストで失敗しなくなります。コードをチェックするには `build`（または CI パイプライン）を実行してください。
:::
