...
このファイルにはプロジェクトが参照する依存関係を宣言する必要があります。ビルド時にはワークスペースルートの`package.json`経由でインストールされた依存関係が解決されるため、Biomeの`noUndeclaredDependencies`ルールは、プロジェクトが`package.json`にリストされていないパッケージをインポートした場合に警告します。
