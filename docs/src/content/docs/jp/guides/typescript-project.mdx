...
pnpmでは、ワークスペースは[`catalogMode: strict`](https://pnpm.io/settings#catalogmode)も設定するため、`pnpm add`が新しい依存関係を自動的にカタログに記録します。yarnとbunには同等の設定がないため、新しい依存関係を`catalog:`プロトコルで追加してカタログに記録します。例：`yarn workspace @my-scope/my-library add some-npm-package@catalog:`または`bun add some-npm-package@catalog: --cwd packages/my-library`。
