...
:::note[プロジェクトエイリアス]
エイリアスはプロジェクトのパッケージ名と一致するため、ワークスペース内のどのプロジェクトも`package.json`で宣言された名前でインポートできます — ローカルプロジェクトを依存関係として宣言する必要はありません。
:::
...
各TypeScriptプロジェクトは、**ソースコードがインポートするサードパーティのランタイム依存関係**を独自の`package.json`で宣言します。これにより、すべてのプロジェクトが自己記述的になり（マニフェストに必要なものが正確にリストされる）、Biomeの[`noUndeclaredDependencies`](https://biomejs.dev/linter/rules/no-undeclared-dependencies/)リントルールがプロジェクトが宣言していないパッケージをインポートしないことを強制できます。

**ビルドおよびテストツール**（例：`vitest`、`tsx`、`rolldown`、Nxツールチェーン（`nx`、`@nx/js`、...）、`typescript`、`@biomejs/biome`）はワークスペース全体で共有されるため、各プロジェクトではなく**ルート**の`package.json`の`devDependencies`に配置されます。これらのツールは設定ファイル、ビルドスクリプト、テストからのみ参照され、リントルールはこれらを除外します。

プロジェクトにランタイム依存関係を追加するには、そのプロジェクトの`package.json`に追加します。パッケージマネージャのフィルター/ワークスペースフラグを使用して適切なプロジェクトをターゲットにします。例えばpnpmの場合:

```bash
pnpm add some-npm-package --filter @my-scope/my-library
```

共有開発ツールを追加するには、ワークスペースルートに追加します:

<InstallCommand pkg="-D some-dev-tool" />

これは[単一バージョンポリシー](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)に従っており、すべてのプロジェクトが各依存関係の同じバージョンを使用します。これにより、型の競合やデプロイメントバンドル内のパッケージの複数コピーを回避できます。カタログをサポートするパッケージマネージャ（[pnpm](https://pnpm.io/catalogs)、[yarn](https://yarnpkg.com/features/catalogs)、[bun](https://bun.com/docs/install/catalogs)）の場合、ジェネレータは各バージョンをカタログに記録し、`catalog:`プロトコル（例：`"zod": "catalog:"`）で参照するため、どのプロジェクトが依存関係を宣言しているかに関係なく、カタログがバージョンの唯一の信頼できる情報源となります。提供される`biome.json`はBiomeの[`experimentalPnpmCatalogs`](https://biomejs.dev/reference/configuration/#javascriptresolverexperimentalpnpmcatalogs)リゾルバを有効にするため、リントルールは`catalog:`参照を理解します。

:::tip[npmでのバージョン調整]
npm（およびyarn classic）にはカタログ機能がないため、バージョンは各`package.json`に直接宣言されます。プロジェクト間でバージョンの一貫性を保つには、[syncpack](https://github.com/JamieMason/syncpack)（`npx syncpack lint`）の使用をお勧めします。
:::

:::note[bun]
bunでは、Nxジェネレータがルートマニフェストからバージョンを読み取るいくつかのパッケージ（`vite`、`react`、`react-dom`）は、`catalog:`参照の代わりに直接バージョン範囲を保持します — Nxはまだbunカタログを解決せず、`catalog:`参照はそれらのジェネレータを壊してしまいます。
:::
...
カタログを無効にすると、ジェネレータは依存関係のバージョンを各プロジェクトの`package.json`に直接書き込み、プロジェクト間でバージョンを揃えることはあなたの責任になります（例えば[syncpack](https://github.com/JamieMason/syncpack)を使用）。pnpmでカタログが有効な場合、ワークスペースは[`catalogMode: strict`](https://pnpm.io/settings#catalogmode)も設定するため、`pnpm add`は新しい依存関係をカタログに記録し、手動編集なしに単一バージョンポリシーを維持します。

ワークスペース内のあるプロジェクトが別のプロジェクトをインポートする場合、手動で宣言する必要はありません。TypeScriptパスエイリアスがビルド用にインポートを解決し、`ts#sync`ジェネレータ（`compile`の前に実行されるか、`nx sync`経由でオンデマンドで実行）がインポートされたプロジェクトをワークスペース依存関係としてインポート元プロジェクトの`package.json`に宣言します（パッケージマネージャがワークスペースプロトコルをサポートする場合は`workspace:*`、npmとyarn classicでは`*`）。これにより、各プロジェクトのマニフェストが使用するものの正確な記録となり、手動での管理なしに`noUndeclaredDependencies`リントルールを満たします。
...
TypeScriptプロジェクトをNPMに公開する場合、その`package.json`には依存関係に加えてエントリポイント（`main`/`exports`）を宣言する必要があります。依存関係はジェネレータが既に正確に保持しています（[依存関係](#dependencies)を参照）。
