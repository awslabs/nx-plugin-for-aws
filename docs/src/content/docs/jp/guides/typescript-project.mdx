...
**ビルドおよびテストツール**（例：`vitest`、`tsx`、`rolldown`、`@nx/*`、`typescript`、`@biomejs/biome`）はワークスペース全体で共有されるため、各プロジェクトではなく**ルート**の`package.json`の`devDependencies`に配置されます。これらのツールは設定ファイル、ビルドスクリプト、テストからのみ参照され、リントルールはこれらを除外します。
...
:::tip[npmでのバージョン調整]
npmにはカタログ機能がないため、バージョンは各`package.json`に直接宣言されます。npmワークスペース内のプロジェクト間でバージョンの一貫性を保つには、[syncpack](https://github.com/JamieMason/syncpack)の使用をお勧めします。
:::
...
TypeScriptプロジェクトをNPMに公開する場合、その`package.json`にはプロジェクトが参照する依存関係とエントリポイント（`main`/`exports`）を宣言する必要があります。ビルド時にはワークスペースルートの`package.json`経由でインストールされた依存関係が解決されるため、Biomeの`noUndeclaredDependencies`ルールは、プロジェクトが`package.json`にリストされていないパッケージをインポートした場合に警告します。
