...
プロジェクトには `assemble` ターゲットもあります。これは `build` のアーティファクトのみの兄弟です：リント、テスト、型チェックのゲートを実行せずに、プロジェクトがデプロイメントに貢献するもの（コンパイルされた出力、およびバンドル、コンテナイメージ、または宣言された生成された仕様）を生成します。

<NxCommands commands={['assemble <project-name>']} />

<Link path="guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `build` ではなく `assemble` に依存するため、デプロイはこれからデプロイしようとしているものだけをビルドします。`build` は変更されず、すべてを実行するため、プロジェクトをチェックするために実行するもの、そして CI で実行すべきものとして残ります。

:::caution
`assemble` は品質ゲートをスキップするため、デプロイはリントエラーや失敗したユニットテストで失敗しなくなります。コードをチェックするには `build`（または CI パイプライン）を実行してください。
:::
