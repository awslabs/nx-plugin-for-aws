...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    [`--save-catalog`](https://pnpm.io/settings#catalogmode) を付けて依存関係を追加すると、pnpm がバージョンをカタログに記録し、参照します：
    ...
    :::note
    `pnpm-workspace.yaml` で [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) を設定すると、すべての `pnpm add` でこれがデフォルトになります。また、カタログが異なるバージョンを固定している場合は常に `pnpm add some-npm-package@<version>` が失敗するようになり、`nx migrate` が Nx をアップグレードできなくなります — そのため、生成されたワークスペースでは設定されていません。
    :::
  </TabItem>
  <TabItem label="yarn">
