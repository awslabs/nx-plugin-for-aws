...
カタログを無効にすると、ジェネレータは依存関係のバージョンを各プロジェクトの`package.json`に直接書き込み、プロジェクト間でバージョンを揃えることはあなたの責任になります（例えば[syncpack](https://github.com/JamieMason/syncpack)を使用）。pnpmでカタログが有効な場合、ワークスペースは[`catalogMode: prefer`](https://pnpm.io/settings#catalogmode)も設定するため、`pnpm add`は新しい依存関係をカタログに追加し続けます。
