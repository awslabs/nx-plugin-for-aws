...
:::tip[Lambda 関数]
AWS Lambda 関数を構築している場合は、<Link path="/guides/ts-lambda-function">`ts#lambda-function`</Link> ジェネレーターをチェックしてください。これにより、バンドルが設定され、インフラストラクチャの生成、可観測性と型安全性の追加が行われます。
:::
...
<Link path="/guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `assemble` に依存しているため、デプロイはこれからデプロイしようとしているもののみをビルドします。
