...
<FileTree>
  ...
  - vitest.config.mts プロジェクト独自の vitest.config.mts が拡張する共有 Vitest 設定（最初の TypeScript プロジェクトによって作成されます）
  - package.json 共有ビルドおよびテストツールが追加されます
  - pnpm-workspace.yaml プロジェクトがワークスペースに追加され、依存関係のバージョンが[カタログ](#catalogs)に追加されます（お使いのパッケージマネージャーの同等のファイル）
  - .gitignore ビルドおよびテスト出力パスが無視されます
...
`cwd` は `rolldown` が設定を見つけるために必要です：これがないと、コマンドはワークスペースルートから実行され、そこには `rolldown.config.ts` が存在しません。
...
次のコマンドで TypeScript 依存関係をインストールします：
...
プロジェクトには `assemble` ターゲットもあり、これはリント、テスト、型チェックのゲートを実行せずに、プロジェクトがデプロイメントに貢献するもの（例：コンパイルまたはバンドルされた出力）を生成します。

<NxCommands commands={['assemble <project-name>']} />

<Link path="/jp/guides/typescript-infrastructure#deploying-to-aws">デプロイターゲット</Link>は `assemble` に依存しているため、デプロイはこれからデプロイしようとしているもののみをビルドします。

:::caution[パイプラインでは `build` を使用]
`assemble` は開発中の高速な反復のために使用されます。本番デプロイメントの前にプロジェクトが品質ゲートを通過することを確認するために、CI パイプラインでは `build` を実行してください。
:::
