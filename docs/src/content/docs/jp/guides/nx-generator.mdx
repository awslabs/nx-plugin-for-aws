...
TypeScriptプロジェクトに[Nx Generator](https://nx.dev/recipes/generators/local-generators)を追加し、コンポーネントのスキャフォールディングや特定のプロジェクト構造の強制など、反復的なタスクの自動化を支援します。
...
:::tip
GritQLパターンは[GritQL Playground](https://docs.grit.io/playground)で試すことができます。また、GritQL CLIをローカルにインストールして、ファイルに対してパターンをテストすることもできます：
