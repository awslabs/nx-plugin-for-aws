...
:::tip[推奨セットアップ]
作成するすべてのGeneratorのベースプロジェクトとして<Link path="/guides/ts-nx-plugin">`ts#nx-plugin`</Link> Generatorを使用することをお勧めします。これにより、AIがGeneratorを使用するのに役立つMCP Serverも提供されます。
:::
...
<FileTree>
  - src/\<name>/
    ...
    - files/
      - hello.ts.template スタブGeneratorによってレンダリングされるテンプレートの例
  - src/index.ts Generatorをエクスポートするように更新
  - generators.json Generatorを定義するために作成または更新
  - project.json プロジェクトに対してGeneratorを記録するように更新
  - package.json 存在しない場合は作成され、`generators.json`を指すように設定
</FileTree>
...
一部のオプションは、別のオプションの特定の値に対してのみ適用されます。`x-when`を使用してどの値かを指定すると、ドキュメントサイトは適用されない場所ではオプションを省略し、適用される場所では条件を記載します：
...
複数のキーはANDで結合され、キー内の値のリストはORで結合されます。これは`<OptionFilter when={{…}}>`と一致します：
...
`x-when`はオプションを文書化するものであり、何も強制しません。そのため、Generatorは、与えられた実行に適用されないオプションを検証または無視する必要があります。
...
- `@aws/nx-plugin/sdk/utils/test`の`createTreeUsingTsSolutionSetup()`を使用して仮想ファイルシステムを作成します。これは、プリセットが書き込むワークスペースマーカー（`tsconfig.base.json`、`pnpm-workspace.yaml`、`biome.json`、`type: module`のルート`package.json`）をシードし、`@aws/nx-plugin` Generatorを構成するすべてのGeneratorに必要です
...
:::tip[Generatorの貢献]
Nx Plugin for AWSへの貢献に関するより詳細なガイドについては、<Link path="get_started/tutorials/contribute-generator">こちらのチュートリアル</Link>を参照してください。
:::