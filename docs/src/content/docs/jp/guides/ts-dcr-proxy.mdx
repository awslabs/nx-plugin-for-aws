...
<Link path="guides/react-website-auth">`ts#website#auth`</Link>ジェネレーターから既にUser Poolを持っている場合、同じWebサイトユーザーのためにMCPサーバーを前段に配置できます。以下の<Link path="guides/ts-dcr-proxy#reusing-a-useridentity-user-pool">UserIdentity User Poolの再利用</Link>を参照してください。
:::
...
<Fragment slot="terraform">
```hcl
# ts#website#authによってWebサイトユーザー用に作成されたUser Poolモジュール
