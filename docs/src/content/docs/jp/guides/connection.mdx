---
title: Connection
description: コネクションのリファレンスドキュメント
---
...
:::tip[視覚的にスケッチする]
まずワークスペースを設計したいですか？<Link path="get_started/graph-builder">Graph Builder</Link>を使用すると、プロジェクトをドラッグして配置し、それらの間の接続を描画してから、すべてをスキャフォールドするコマンドを取得できます。
:::
...
:::note[ランタイム設定]
connectionジェネレーターは<Link path="guides/runtime-config">Runtime Configuration</Link>を使用して、デプロイ時の値（API URL、Cognito設定、エージェントランタイムARNなど）を生成されたプロジェクトとコンポーネント間で渡し、実行時に相互に検出して接続できるようにします。
:::

:::tip[ローカル開発]
接続されたプロジェクトは、`serve`および`dev`ターゲットを使用してマシン上で実行できます。詳細については、<Link path="guides/local-development">Local Development</Link>ガイドを参照してください。
:::
