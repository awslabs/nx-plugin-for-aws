...
:::tip[視覚的にスケッチする]
まずワークスペースを設計したいですか？<Link path="get_started/graph-builder">Graph Builder</Link>を使用すると、プロジェクトをドラッグして配置し、それらの間の接続を描画してから、すべてをスキャフォールドするためのコマンドを取得できます。
:::
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="FastAPIをPython Auroraリレーショナルデータベースに接続する"
  ...
  <ConnectionCard
    ...
    description="Python AgentをPython Auroraリレーショナルデータベースに接続する"
  ...
  <ConnectionCard
    ...
    description="Python MCP ServerをPython Auroraリレーショナルデータベースに接続する"
  ...
  <ConnectionCard
    ...
    description="ReactウェブサイトをAgentCore Gateway経由でエージェントに接続する"
...
:::tip[ローカル開発]
接続されたプロジェクトは、`serve` および `dev` ターゲットを使用してマシン上で実行できます。詳細については、<Link path="guides/local-development">ローカル開発</Link>ガイドを参照してください。
:::
