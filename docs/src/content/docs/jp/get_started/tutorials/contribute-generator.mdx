...
新しいワークスペースでジェネレータを実行し、すべてがビルドされることを確認する「smoke test」のスイートがあります。最低限、新しいジェネレータを`e2e/src/smoke-tests/generator-matrix.ts`のジェネレータマトリックスに追加して、smoke testで実行されるようにする必要があります。
