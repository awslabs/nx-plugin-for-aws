...
- このチュートリアルではアプリケーションのデプロイとAmazon Bedrockの呼び出しを行うため、ターゲットAWSアカウントに設定された[AWS Credentials](https://docs.aws.amazon.com/sdkref/latest/guide/access.html)が必要です
- ローカルDynamoDB開発とAgentCoreコンポーネントのビルドには[Docker](https://www.docker.com/)（または[Finch](https://github.com/runfinch/finch)）が必要です
