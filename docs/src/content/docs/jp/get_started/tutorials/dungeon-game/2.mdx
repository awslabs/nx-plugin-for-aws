...
4. `queryActions` - ゲームに関連する全てのアクションのページネーション付きリストを返す

### APIスキーマ

APIの入力と出力を定義するため、[Zod](https://zod.dev/)を使用して`packages/game-api/src/schema`ディレクトリ内に以下のようにスキーマを作成します：
...
また、このプロジェクトでは使用しないため、`packages/game-api/src/schema/echo.ts`ファイルを削除できます。
...
DynamoDBでデータベースを実装し、[ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDBクライアントライブラリを使用して簡素化します。始めるには、まず次のコマンドを実行して`electrodb`をインストールします：

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
全ての依存関係はルート`package.json`に追加されます。これは`@aws/nx-plugin`が[シングルバージョンポリシー](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)原則に従っているためです。詳細は<Link path="guides/typescript-project#dependencies">ts#projectガイド</Link>を参照してください。
</Aside>

次に、上記のER図に従ってElectroDBエンティティを定義するため、`packages/game-api/src/entities`フォルダ内に以下のファイルを作成します：
...
ElectroDBは非常に強力で、型定義だけでなく、上記のタイムスタンプのような特定の値のデフォルトも提供できます。さらにElectroDBはDynamoDB使用時のベストプラクティスである[シングルテーブル設計](https://electrodb.dev/en/core-concepts/single-table-relationships/)に従っています。
...
各プロシージャでDynamoDBクライアントへのアクセスが必要なため、コンテキスト経由で渡せる単一のクライアントインスタンスを作成する必要があります。これを行うため、`packages/game-api/src`内で以下の変更を加えます：

<Tabs>
  <TabItem label="middleware/dynamodb.ts">
<E2ECode lang="typescript" path="dungeon-adventure/2/middleware/dynamodb.ts.template" />

これは`DynamoDBClient`を作成しコンテキストに注入するプラグインです。
  </TabItem>
...
次にAPIメソッドを実装します。これを行うため、`packages/game-api/src/procedures`内で以下の変更を加えます：
...
また、このプロジェクトでは使用しないため、`echo.ts`ファイル（`packages/game-api/src/procedures`内）を削除できます。

### ルーター設定

プロシージャを定義したので、APIに接続します。以下のファイルを次のように更新します：

<E2EDiff lang="typescript" before="dungeon-adventure/2/router.ts.old.template" after="dungeon-adventure/2/router.ts.template" />

### インフラストラクチャ

最後のステップは、DynamoDBテーブルを作成しGame APIからの操作権限を付与するため、インフラストラクチャを更新します。これを行うため、`packages/infra/src`を以下のように更新します：
...
<Aside type="tip">
リンターエラーが発生した場合、以下のコマンドを実行して自動修正できます。
...
以下のスクリプトを`.bashrc`ファイルに追加（`source`で反映）するか、コマンドを実行するターミナルに直接貼り付けます。
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
<Tabs>
  <TabItem label="ローカル">
    ...
    <Aside type="caution">
    CDKデプロイ出力値`dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX`を使用して、ハイライトされたプレースホルダーを置き換えてください。
    </Aside>

    サーバーが起動したら、次のコマンドで呼び出せます：
...
```
    <Aside type="caution">
    CDKデプロイ出力値`dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX`を使用してハイライトされたプレースホルダーを置き換え、リージョンを適切に設定してください。
    </Aside>
  </TabItem>
</Tabs>
...
コマンドが正常に実行されると、以下のようなレスポンスが表示されます：

```json
{"result":{"data":{"items":[],"cursor":null}}}
```

おめでとうございます！tRPCを使用して最初のAPIを構築・デプロイできました！ 🎉🎉🎉