...
3. `saveAction` - 指定したゲームのアクションを保存します
4. `queryActions` - ゲームに関連する全てのアクションのページネーション付きリストを返します
...
<Aside type="tip">
上記のように、Zodで定義した各スキーマに対して`z.TypeOf`構文を使用してインターフェースをエクスポートしています。これにより、重複作業なしでZod定義からTypescriptインターフェースを生成できます！
</Aside>

### エンティティモデリング

アプリケーションのER図は次のようになります：

<Image class="centered-image white-bg" src={dungeonAdventureErPng} alt="dungeon-adventure-er.png" width="400" height="300" />

DynamoDBでデータベースを実装するため、[ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDBクライアントライブラリを使用します。まず次のコマンドで`electrodb`をインストールします：

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
全ての依存関係はルート`package.json`に追加されます。`@aws/nx-plugin`は[単一バージョンポリシー](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)原則に従っています。詳細は<Link path="guides/typescript-project#dependencies">ts#projectガイド</Link>を参照してください。
</Aside>

`packages/game-api/backend/src/entities`フォルダ内に次のファイルを作成し、上記ER図に基づいてElectroDBエンティティを定義します：
...
ElectroDBは非常に強力で、タイプ定義に加え、タイムスタンプのデフォルト値設定などが可能です。またElectroDBはDynamoDB使用時のベストプラクティスである[シングルテーブル設計](https://electrodb.dev/en/core-concepts/single-table-relationships/)を採用しています。
...
DynamoDBClientを作成してコンテキストに注入するプラグインです。
  </TabItem>
...
<Aside>
新しいAPIであるため`unstable_concat`を使用していますが、安全に使用できます。詳細は[concatガイド](https://trpc.io/docs/server/middlewares#concat)を参照してください。
</Aside>
  </TabItem>
</Tabs>
...
`packages/game-api/backend/src/procedures`内の`echo.ts`ファイルは削除できます。
...
DynamoDBテーブルの作成とGame APIへの権限付与を行うため、`packages/infra/src`を更新します：
...
初回デプロイは約8分かかります。以降のデプロイは約2分で完了します。

:::tip
Lambda関数のコード変更を反復する場合、`--hotswap`フラグを使用してデプロイすると大幅に短縮（2-3秒）できます。
...
次のコマンドでCDKアプリケーション内の全スタックをデプロイできます：

<NxCommands commands={['run @dungeon-adventure/infra:deploy --all']} />

ただし`infra-prod`のようにデプロイステージを分離する場合、`--all`フラグは意図しないデプロイを引き起こす可能性があるため**推奨しません**。

</Drawer>

デプロイが完了すると次のような出力が表示されます（一部値は編集済み）：
...
<Tabs>
  <TabItem label="ローカル環境">
    ...
    <Aside type="caution">
    `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX`のCDKデプロイ出力値でプレースホルダーを置換してください。
    </Aside>
...
```
    <Aside type="caution">
    `dungeon-adventure-infra-sandbox.GameApiGameApiUrlXXX`のCDK出力値でプレースホルダーを置換し、リージョンを適切に設定してください。
    </Aside>
  </TabItem>
</Tabs>
