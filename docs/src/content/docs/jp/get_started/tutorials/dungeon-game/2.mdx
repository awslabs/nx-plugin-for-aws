...
:::tip[ランタイム設定]
DynamoDBテーブル名は<Link path="guides/runtime-config">ランタイム設定</Link>の`tables`名前空間に登録され、AWS AppConfigに保存されます。Lambda関数とMCPサーバーは、環境変数として渡される必要なく、Lambda Powertoolsを使用して実行時に取得します。このアプローチの利点は、テーブルやその他のインフラストラクチャ識別子の数が増えても、ローカル実行時に必要な環境変数は`RUNTIME_CONFIG_APP_ID`1つだけで済むことです。
:::
  </TabItem>
</Tabs>
