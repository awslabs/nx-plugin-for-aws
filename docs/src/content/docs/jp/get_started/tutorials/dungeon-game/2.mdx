...
DynamoDBでデータベースを実装し、[ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDBクライアントライブラリを使用して簡素化します。Game APIの`queryActions`はエージェントが書き込んだ会話ターンを読み取るためにS3クライアントも必要とします。以下のコマンドですべてをインストールします:
