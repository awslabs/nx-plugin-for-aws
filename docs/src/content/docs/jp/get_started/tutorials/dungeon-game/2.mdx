...
## モジュール2: Game APIとInventory MCP Serverの実装

まずGame APIの実装から始めます。これには合計5つのAPIを作成する必要があります：
...
APIの入力と出力を定義するため、[Zod](https://zod.dev/)を使用して`packages/game-api/src/schema`ディレクトリ内に以下のスキーマを作成します：
...
<Aside type="tip">
上記のように、Zodで定義する各スキーマに対して`z.TypeOf`構文を使用してインターフェースをエクスポートしています。これにより、Typescriptインターフェースを重複せずに生成できます！
</Aside>
...
<Aside>
全ての依存関係はルート`package.json`に追加されます。これは`@aws/nx-plugin`が[シングルバージョンポリシー](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)原則に従っているためです。詳細は<Link path="guides/typescript-project#dependencies">tsプロジェクトガイド</Link>を参照してください。
</Aside>
...
ElectroDBではタイプ定義だけでなく、タイムスタンプなどのデフォルト値も設定可能です。またElectroDBはDynamoDB使用時のベストプラクティスである[シングルテーブル設計](https://electrodb.dev/en/core-concepts/single-table-relationships/)を採用しています。
...
:::note
エンティティを共有プロジェクトにリファクタリングすれば、MCPサーバーがAPIに依存せず、より良いパッケージ構造になります。
:::

### tRPCコンテキストへのDynamoDBクライアント追加

各プロシージャでDynamoDBクライアントにアクセスするため、コンテキスト経由で単一インスタンスを渡せるようにします。`packages/game-api/src`内で以下の変更を加えます：
...
エージェントがプレイヤーのインベントリを管理するためのMCPサーバーを作成します。
...
ツールが増えた場合は、必要に応じて別ファイルにリファクタリングできます。
...
:::note
各プロシージャが個別のLambda関数で提供されるため、最小権限の原則に従い、プロシージャの実装に基づいて必要な読み書き権限のみを付与できます。
:::
  </TabItem>
</Tabs>
...
:::tip
Lambda関数コードの変更を反復する場合、コードビルド後に`--hotswap`フラグ付きでデプロイすると時間（2-3秒）を短縮できます：
...
`.bashrc`ファイルに以下のスクリプトを追加（`source`で反映）するか、直接ターミナルに貼り付けます：
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
PowerShellプロファイルに関数を追加するか、直接セッションに貼り付けます：
```powershell
function acurl {
    param(
...
<Tabs>
  <TabItem label="ローカル">
    ...
    <Aside type="caution">
    `dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX`のCDKデプロイ出力値でプレースホルダーを置き換えてください。
    </Aside>
...
```
    <Aside type="caution">
    `dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX`のCDKデプロイ出力値でプレースホルダーを置き換え、リージョンを適切に設定してください。
    </Aside>
  </TabItem>
</Tabs>

:::note
テスト用に渡す`%7B%7D`はURIエンコードされた空のJSONオブジェクト（`{}`）です。
:::
