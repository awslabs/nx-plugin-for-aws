...
:::note[会話履歴はエージェントと共に存在]
Game APIを通じてチャットメッセージを永続化することはありません。<Link path="get_started/tutorials/dungeon-game/3">モジュール3</Link>では、Story Agentが`FileSessionManager`をプラグインし、UIがターンごとに送信するスレッドIDをキーとして各スレッドの履歴をディスクに書き込みます。これにより、各`(player, genre)`ペアはUIがメッセージを再生する必要なく独自のストーリーを再開できます。
:::
