...
先ほど書いたプロシージャと MCP サーバーは `@dungeon-adventure/dungeon-db` をインポートしており、これは新しいプロジェクト間インポートです。まず、ワークスペースを同期します：

<NxCommands commands={["sync"]} />

<Aside type="tip" title="このステップが必要な理由">
`nx sync` は、`game-api` と `inventory` の `package.json` ファイルに `@dungeon-adventure/dungeon-db` を宣言し、対応する TypeScript プロジェクト参照を追加します。これにより、プロジェクト間の依存関係を宣言する際に手動で追加する必要がなくなります。
</Aside>
