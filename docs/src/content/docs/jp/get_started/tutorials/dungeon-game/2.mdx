...
:::note[チャット履歴プロシージャは不要]
チャットのユーザー/アシスタントの各ターンを保存するプロシージャが必要だと思われたかもしれません。しかし、それは不要です。AG-UIエージェントはBedrock AgentCore Runtimeでホストされており、同じスレッドIDが使用されている限り、セッションごとの会話履歴が自動的に永続化されます。
:::
