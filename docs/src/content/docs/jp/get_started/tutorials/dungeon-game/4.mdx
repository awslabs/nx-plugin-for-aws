...
:::tip[Nxコマンドの使用]
代わりにNxコマンドを実行して、接続されたAPIローカルサーバーを起動することで、tRPC APIとウェブサイトの同時開発が可能になります。<NxCommands commands={["serve-local game-ui"]} env={{RUNTIME_CONFIG_APP_ID:"xxxx"}} />

`RUNTIME_CONFIG_APP_ID`環境変数には、CDKデプロイ出力の`RuntimeConfigApplicationId`の値を使用してください。
:::
...
<Link path="get_started/tutorials/dungeon-game/1#task-7-connect-your-components">モジュール1</Link>で、`connection`ジェネレータを使用してGame UIをStory Agentに接続したことを思い出してください。これにより、エージェントとやり取りするための型安全なOpenAPIクライアントと、フックおよびプロバイダーが設定されました。

connectionジェネレータは以下を作成しました:
- `main.tsx`でアプリをラップする`StoryAgentProvider`コンポーネント
- TanStack Query統合のための`useStoryAgent`フック
- 直接クライアントアクセスのための`useStoryAgentClient`フック
- エージェントのOpenAPI仕様から生成されたTypeScript型

ゲームコンポーネントで`useStoryAgentClient`フックを使用して、エージェントからストーリーレスポンスをストリーミングします。
