...
:::tip
代わりにNxコマンドを実行して、接続されたAPIローカルサーバーを起動することで、tRPC APIとウェブサイトの同時開発が可能になります。<NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} /> 
:::
...
:::tip
`api-connection`ジェネレータを使用したため、Game APIとの統合は既に設定済みです。同様のエージェント体験を希望される場合は[このGitHubイシュー](https://github.com/awslabs/nx-plugin-for-aws/issues/326)へ+1をお願いします。
:::
