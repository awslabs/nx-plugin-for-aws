...
:::tip
代わりにNxコマンドを実行して、接続されたAPIローカルサーバーを起動することで、tRPC APIとウェブサイトの同時開発が可能になります。<NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} env={{RUNTIME_CONFIG_APP_ID:"xxxx"}} />
