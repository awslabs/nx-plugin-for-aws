---
title: "AIダンジョンゲーム"
description: "@aws/nx-pluginを使用してAIを活用したダンジョン冒険ゲームを構築する方法のチュートリアル"
---
...
UIの構築を開始するには、ローカル開発サーバーをデプロイされたサンドボックスに接続する必要があります。次のコマンドを実行してください:

<NxCommands commands={["run @dungeon-adventure/game-ui:load:runtime-config"]} />

このコマンドはデプロイ済みの`runtime-config.json`を取得し、`packages/game-ui/public`フォルダ内に保存します。

開発サーバーを起動するには次のコマンドを実行します:

<NxCommands commands={["run @dungeon-adventure/game-ui:serve"]} />

ブラウザでローカルサイト（http://localhost:4200）を開くと、ログインを求められ、新規ユーザー作成のプロンプトが表示されます。完了するとベースラインサイトが表示されます:

<Image src={baselineWebsitePng} alt="baseline-website.png" width="800" height="600" />

<Aside type="caution">
_本モジュールの残りの作業中は開発サーバーを起動したままにしてください。変更を加えると自動的にホットリロードされます。_
</Aside>

### 新しい'/game'ルートの作成

`@tanstack/react-router`の機能を活用して型安全な新しいルートを作成します。次の場所に空ファイルを作成してください: `packages/game-ui/src/routes/game/index.tsx`。開発サーバーのログを確認します:
...
デフォルトのレイアウトはSaaS型ビジネスアプリケーション向けです。ダンジョンスタイルのゲームに適したテーマに再構成します。
...
function RouteComponent() {
  ...
  // 最後のゲームが可視状態で次のページがある場合、追加データ取得
  useEffect(() => {
...
<Aside type="tip">
`$playerName`構文は`@tanstack/react-router`に`playerName`を[パスパラメータ](https://tanstack.com/router/v1/docs/framework/react/guide/path-params)として扱うよう指示します。また`validateSearch`メソッドで`genre`クエリパラメータを厳密に型チェックします。
</Aside>
</TabItem>
</Tabs>

変更を加えると、ローカル開発サーバー（http://localhost:4200/）でゲームをプレイできるようになります!

<Drawer title="ビルド＆デプロイ" trigger="Cloudfrontへのビルド＆デプロイも可能です">
...
デプロイ後、cdk deployの出力からCloudfront URLを確認できます。
