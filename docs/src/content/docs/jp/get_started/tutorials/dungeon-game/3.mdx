...
<Tabs>
  <TabItem label="ローカル">
  エージェントジェネレーターは、エージェントのOpenAPI仕様から生成された型安全なTypeScriptクライアントを使用するインタラクティブなターミナルチャットCLI（`agent-chat`）を提供します。エージェントの入力形式を`playerName`、`genre`、`actions`を受け入れるように変更したため、これらのフィールドを提供するように生成されたチャットスクリプトを更新する必要があります。

  `packages/story/scripts/agent/chat.ts`を更新します：
  ...
  次に、ローカルで実行中のStoryエージェントとのインタラクティブなチャットを開始します：
    ...
    `agent-chat`ターゲットは自動的に`agent-serve-local`を起動し（エージェントと任意のMCPサーバー依存関係を`SERVE_LOCAL=true`で起動）、OpenAPI仕様とクライアント生成を実行してからチャットに入ります。

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat`は`agent-serve-local`に依存しており、これはエージェントを`SERVE_LOCAL=true`で起動し、任意のMCPサーバー依存関係もローカルで起動します。エージェントを**デプロイ済み**のMCPサーバーに接続させたい場合（`RUNTIME_CONFIG_APP_ID`経由）は、別のターミナルで`agent-serve`ターゲットを実行し、`/invocations`エンドポイントに対して`curl`でエージェントを手動で呼び出してください。
    :::
  </TabItem>
