...
<Tabs>
  <TabItem label="ローカル">
  以下のコマンドを実行して、ローカルでエージェントサーバーを起動します：
    ...
    `agent-serve`ターゲットはエージェントをローカルで起動しますが、**デプロイ済み**のMCPサーバーに接続します（`RUNTIME_CONFIG_APP_ID`経由）。MCPサーバーもローカルで実行したい場合は、代わりに`agent-serve-local`を使用してください。これにより、すべての依存サービスが起動され、直接HTTP接続のために`SERVE_LOCAL=true`が設定されます。
    :::

    エージェントサーバーが起動して実行中になったら（出力は表示されません）、以下のコマンドを実行して呼び出します：
...
コマンドが正常に実行されると、ストーリーの冒頭テキストがJSON Lines形式でストリーミングされるのが確認できます：
