...
<Aside type="caution">
AWSアカウントで対象リージョンの[Strandsデフォルトモデル](https://strandsagents.com/latest/documentation/docs/user-guide/concepts/model-providers/amazon-bedrock/)へのアクセスが有効化されていることを確認してください。本ガイド執筆時点ではAnthropic Claude Sonnet 4が該当します。設定手順は[こちらのガイド](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html)を参照してください。
</Aside>

ストーリーエージェントは[Strands](https://strandsagents.com/)のエージェントで、`Game`と`Action`のリストをコンテキストとして物語を進行します。Inventory MCPサーバーと連携してプレイヤーの所持アイテムを管理するよう設定します。

:::note
本チュートリアルでは簡略化のため、各呼び出しが完全なメッセージ履歴を含む独立セッションとなる「ステートレス」な実装としています。本番環境では[Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html)との統合を検討してください。StrandsとAgentCore Memoryの連携に関する詳細は[こちら](https://github.com/aws/bedrock-agentcore-sdk-python/tree/main/src/bedrock_agentcore/memory/integrations/strands)を参照ください。
:::
...
- システムプロンプトとMCPサーバーツールを備えたエージェントの構成
...
<Aside type="tip">
リンターエラーが発生した場合は、以下のコマンドで自動修正可能です。
...
<li>デプロイ済みAPIをJWTトークン付きで`curl`呼び出す</li>
</ul>
...
<Tabs>
  ...
  以下のコマンドでローカルエージェントサーバーを起動：
    ...
    :::caution
    上記コマンドではCDKデプロイ出力の`InventoryMcpArn`値と対象`AWS_REGION`を指定してください
    :::
