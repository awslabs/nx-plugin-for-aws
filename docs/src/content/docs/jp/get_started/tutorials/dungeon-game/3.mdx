...
<Tabs>
  <TabItem label="ローカル">
  エージェントジェネレーターは、エージェントのOpenAPI仕様から生成された型安全なTypeScriptクライアントを使用するインタラクティブなターミナルチャットCLI（`story-chat`）を提供します。エージェントの入力形式を`playerName`、`genre`、`actions`を受け入れるように変更したため、まずこれらのフィールドを提供するように生成されたチャットスクリプトを更新する必要があります。

  `packages/story/scripts/story/chat.ts`の内容を以下に置き換えます：
    ...
    次に、ローカルで実行中のStoryエージェントとのインタラクティブなチャットを開始します：
    ...
    `story-chat`ターゲットは自動的に`story-serve-local`を起動し（エージェントと任意のMCPサーバー依存関係を`SERVE_LOCAL=true`で起動）、OpenAPI仕様とクライアント生成を実行してからチャットに入ります。

    :::note[`story-serve` vs `story-serve-local`]
    `story-chat`は`story-serve-local`に依存しており、これはエージェントを`SERVE_LOCAL=true`で起動し、任意のMCPサーバー依存関係もローカルで起動します。エージェントを**デプロイ済み**のMCPサーバーに接続させたい場合（`RUNTIME_CONFIG_APP_ID`経由）は、別のターミナルで`story-serve`ターゲットを実行し、`agent-chat-cli`または`curl`でエージェントを手動で呼び出してください。
    :::
  </TabItem>
...
すべてが正しく設定されていれば、ローカルチャットでは最初のプロンプトを入力した後にストーリーの冒頭テキストがトークンごとにストリーミングされ、デプロイ済みの`curl`コマンドではJSON Lines形式のストリームが返されます：
