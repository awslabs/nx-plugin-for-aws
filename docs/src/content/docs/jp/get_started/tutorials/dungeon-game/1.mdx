...
<Aside>
ルートの`package.json`に`type`が`module`として設定され、`@aws/nx-plugin`が提供する全NodeベースサブプロジェクトでESMがデフォルトのモジュールタイプになります。詳細は<Link path="guides/typescript-project">ts#projectガイド</Link>を参照してください。
</Aside>

<Drawer title="ts#trpc-apiで更新されたファイル" trigger="詳細を確認するにはここをクリック">
`ts#trpc-api`ジェネレーターで生成された全ファイルのリストです。ファイルツリー内の主要ファイルを確認します:
<FileTree>
- packages/
  - common/
    - constructs/
...
GameApiのCDKコンストラクトです。`defaultIntegrations`メソッドにより、tRPC APIの各プロシージャ用Lambda関数が自動生成されます。バンドリングはバックエンドプロジェクトのビルドターゲット時に実行されます。
...
`py#project`ジェネレーターで生成されたファイルリスト:
...
共有仮想環境を持つ[UV Workspace](https://docs.astral.sh/uv/concepts/projects/workspaces/)が設定されます。

</Drawer>

### ストーリーエージェント: Strandsエージェント

`py#strands-agent`ジェネレーターでStrandsエージェントを追加:
...
`py#strands-agent`ジェネレーターで生成されたファイルリスト:
...
[Amazon Bedrock AgentCore SDK](https://github.com/aws/bedrock-agentcore-sdk-python)を使用したエージェントのエントリーポイント。ストリーミングイベントをクライアントに返します。
...
ECRへのDockerイメージアップロードとAgentCore Runtimeでのホスティングを設定します。
...
`ts#project`ジェネレーターで生成されたファイルリスト:
...
`ts#mcp-server`ジェネレーターで生成されたファイルリスト:
...
ゲーム操作用UIを作成します。以下の手順で`GameUI`ウェブサイトを作成:
...
ファイルツリーが更新されます。
<Drawer title="UI → tRPC api-connection更新ファイル" trigger="詳細を確認するにはここをクリック">
接続関連ファイルの更新内容:
...
`run-many`コマンドは複数サブプロジェクトでターゲットを実行します。依存関係順に実行されます。
...
Nxは[キャッシュ](https://nx.dev/concepts/how-caching-works)を使用してビルドを高速化します。キャッシュ無効化:
...
ビルド成果物はモノレポルートの`dist/`フォルダに生成されます。クリーン時は`dist/`を削除。
