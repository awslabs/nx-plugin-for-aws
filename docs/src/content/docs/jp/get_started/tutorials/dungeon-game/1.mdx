...
- packages/
  - story/
    - dungeon\_adventure\_story/ Pythonモジュール
      - agent/
        - main.py Bedrock AgentCore Runtime用のAG-UI FastAPIアプリエントリーポイント
...
`protocol=AG-UI`を選択したため、ジェネレーターはStrandsエージェントを[AG-UI](https://ag-ui.com/)アダプターでラップします。`create_strands_app`は単一の`/invocations`エンドポイントを公開するFastAPIアプリを構築します - セッションごとのエージェントインスタンスは自動的に作成され、会話履歴はサーバーの存続期間中`threadId`ごとにメモリに保持されます。このチュートリアルでは`main.py`を変更する必要はありません。
