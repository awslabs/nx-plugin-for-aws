...
- .gitignore
- biome.json リンティングとフォーマットのためにBiomeを設定
...
<Drawer title="ts#apiで更新されたファイル" trigger="詳細を確認するにはここをクリック">
以下は`ts#api`ジェネレーターによって生成されたすべてのファイルのリストです。ファイルツリーで強調表示されている主要なファイルを確認します:
<FileTree>
- packages/
  - common/
    - constructs/
...
これはエージェントのエントリーポイントです。`--protocol=AG-UI`を選択したため、ジェネレーターはStrandsの`Agent`を[`ag_ui_strands`](https://docs.copilotkit.ai/aws-strands/integration)の`StrandsAgent`でラップし、[AG-UIプロトコル](https://docs.copilotkit.ai/aws-strands/protocol)を話すFastAPIアプリにマウントします。これがReactウェブサイトからCopilotKitが通信する相手です。`_SessionIdMiddleware`は、インバウンドのAgentCoreランタイムセッションIDを`ContextVar`にバインドするため、後で配線する下流のMCP/A2Aクライアント(例えば<Link path="get_started/tutorials/dungeon-game/2">モジュール2</Link>のInventory MCPサーバー)が、アウトバウンド呼び出しで自動的にそれを転送します。<Link path="get_started/tutorials/dungeon-game/3">モジュール3</Link>では、各スレッドIDが独自の`S3SessionManager`を取得し、会話履歴がターン間で永続化されるように`session_manager_provider`も追加します。
...
:::tip[ローカルファースト開発]
`ts#dynamodb`ジェネレーターは、コンテナ内で[DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)を実行する`serve-local`ターゲットを提供します。`connection`ジェネレーター(以下で実行します)と組み合わせることで、**チュートリアルの最後にデプロイすることを選択するまで、ゲーム全体がAWS呼び出しなしでマシン上で実行されます**。[コンテナエンジン](https://docs.docker.com/get-started/get-docker/)(DockerやFinchなど)がインストールされ、実行されている必要があります。
:::

ファイルツリーに新しいファイルが表示されます。
<Drawer title="ts#dynamodbで更新されたファイル" trigger="詳細を確認するにはここをクリック">
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - constants.ts ローカル開発定数とランタイム設定キー
      - client.ts DynamoDBクライアントシングルトンとテーブル名解決
      - gsi.ts グローバルセカンダリインデックスの信頼できる情報源
      - entities/
        - example.ts サンプルElectroDBエンティティ(これを置き換えます)
        - index.ts エンティティエクスポート
    - project.json `serve-local`と`pull-image`ターゲットを追加
  - common/constructs/
    - src/
      - app/dynamodb/
...
生成された`src/client.ts`は`getDynamoDBClient()`と`resolveTableName()`をエクスポートします。`SERVE_LOCAL=true`の場合(`serve-local`ターゲットによって自動的に設定されます)、これらはDynamoDB Localに接続します。それ以外の場合はAWSに接続し、<Link path="guides/runtime-config">Runtime Configuration</Link>からデプロイされたテーブル名を解決します。<Link path="get_started/tutorials/dungeon-game/2">モジュール2</Link>でこのプロジェクトに`Game`と`Inventory`エンティティをモデル化します。

詳細については、<Link path="guides/ts-dynamodb">ts#dynamodbジェネレーターガイド</Link>を参照してください。

</Drawer>

## タスク6: ユーザーインターフェース(UI)を作成する
...
:::note[Shadcn UI]
`--ux=shadcn`を選択することで、生成されたウェブサイトがTailwindでスタイリングされた[shadcn/ui](https://ui.shadcn.com/)コンポーネントを使用するようになります。すべてのルートコードは`Card`、`Button`、`Input`などのshadcnプリミティブを使用し、後の`connection`ジェネレーターがshadcnテーマの[CopilotKit](https://docs.copilotkit.ai/)チャットサーフェスを配線します。
:::
...
次の手順を使用して、Amazon Cognito経由の認証アクセスを要求するようにゲームUIを設定しましょう:
...
以前に作成したGame APIに接続するようにゲームUIを設定しましょう。
...
### ストーリーエージェント: Inventory MCPサーバーに接続する

ストーリーエージェントをInventory MCPサーバーに接続して、エージェントがMCPサーバーのツールを検出して呼び出せるようにしましょう。

<RunGenerator generator="connection" requiredParameters={{sourceProject:"story", targetProject:"inventory"}} noInteractive />

<Drawer title="ストーリーエージェント → Inventory MCP接続で更新されたファイル" trigger="詳細を確認するにはここをクリック">
...
詳細については、<Link path="guides/connection/py-agent-mcp">PythonエージェントからMCPへの接続ガイド</Link>を参照してください。
</Drawer>

### ゲームUI: ストーリーエージェントに接続する

ゲームUIをストーリーエージェントに接続しましょう。エージェントがAG-UIを話すため、`connection`ジェネレーターは[CopilotKit](https://docs.copilotkit.ai/)を配線します。テーマ付きチャットコンポーネントと、レンダリング準備が整った`@ag-ui/client`の`HttpAgent`が得られます。
...
### Game APIとInventory MCPサーバーをデータベースに接続する

Game APIとInventory MCPサーバーの両方がDynamoDBテーブルを読み書きするため、`DungeonDb`プロジェクトに接続しましょう。`connection`ジェネレーターはターゲットが`ts#dynamodb`プロジェクトであることを検出し、各ソースプロジェクトの`serve-local`ターゲットがDynamoDB Localを自動的に起動するように配線します。
...
ストーリーエージェントの`agent-serve-local`は既にInventory MCPサーバーの`mcp-server-serve-local`に依存しており(上記の`story → inventory`接続経由)、Game APIとMCPサーバーの両方が`dungeon-db:serve-local`に依存するようになったため、いずれかのプロジェクトの`serve-local`ターゲットを実行すると、DynamoDB Localを含む必要なすべての依存関係が正しい順序で起動されます。チュートリアルの残りの部分でこれを活用します。
</Aside>
...
<Aside type="note" title="非対話型環境">
非対話型シェル(例: CI)では、プロンプトは表示されません。代わりに、ビルドは「workspace is out of sync」エラーで失敗します。<NxCommands commands={['sync']} />を実行して保留中の同期ジェネレーターを適用してから再度ビルドするか、`nx.json`で`sync.applyChanges`を`true`に設定して、ビルドごとに変更を自動的に適用します。プロジェクト参照が変更されるたび(例: 次のモジュールでプロシージャを追加した後)に、この同じ同期外れチェックが発生します。
</Aside>
