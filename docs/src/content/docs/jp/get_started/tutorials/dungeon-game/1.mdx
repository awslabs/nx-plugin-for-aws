...
<Aside type="tip">ジェネレーターを実行する前に、未ステージングのファイルをすべてGitにコミットすることがベストプラクティスです。これにより、`git diff`を通じてジェネレーター実行後の変更を確認できます</Aside>
...
<Aside>
ルートの`package.json`に`type`が`module`として設定され、`@aws/nx-plugin`が提供するすべてのnodeベースサブプロジェクトでESMがデフォルトのモジュールタイプになります。TypeScriptプロジェクトの詳細については<Link path="guides/typescript-project">ts#projectガイド</Link>を参照してください。
</Aside>
...
- packages/
  ...
  - game-api/ tRPC API
    - src/
      - client/ TypeScriptマシン間通信で使用されるバニラクライアント
        - index.ts
        - sigv4.ts
...
`echo`メソッドの実装で、入力出力データ構造を厳密に型定義しています。
...
GameApiを定義するCDKコンストラクトです。`defaultIntegrations`メソッドは、tRPC APIの各プロシージャに対応するLambda関数を自動生成します。バックエンドプロジェクトのビルドターゲットの一部としてバンドル済みの実装を指すため、`cdk synth`時にバンドリングが発生しません。
...
`py#fast-api`ジェネレーターによって生成/更新されたファイル:
<FileTree>
- .venv/ モノレポ用単一仮想環境
- packages/
  - common/
...
`ts#react-website`ジェネレーターによって生成されたファイル:
<FileTree>
- packages/
  - common/
    - constructs/
...
Reactのマウントポイント。[ファイルベースルーティング](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing)を設定します。
...
`ts#react-website#auth`ジェネレーターによる変更:
<FileTree>
- packages/
  - common/
    - constructs/
  ...
  - game-ui/
    - src/
      - components/
        ...
        - RuntimeConfig/
          - index.tsx `runtime-config.json`取得とコンテキスト提供
      - hooks/
...
`RuntimeConfigProvider`と`CognitoAuth`コンポーネントを追加。`runtime-config.json`から認証設定を取得します。
...
認証済みAPIリクエスト用フック。ビルド時に生成される`StoryApi`クライアントを使用します。
...
`StoryApiOptionsProxy`を使用してTanStack Queryオプションを構築します。ストリーミングAPIにはバニラクライアントを直接使用します。
...
tRPCの[React Query統合](https://trpc.io/blog/introducing-tanstack-react-query-client)を使用。バックエンドの変更がフロントエンドに即時反映されます。
...
tRPCプロバイダーを追加。AST変換により`main.tsx`を更新。
...
<Aside type="tip">IDEにインポートエラーが表示される場合、`nx sync`を実行してTypeScript参照を更新してください。詳細は<Link path="guides/typescript-project#importing-your-library-code-in-other-projects">TypeScriptガイド</Link>を参照。</Aside>
...
<Drawer title="Nxコマンド" trigger="初めてのコードビルドを実行">
###### 単一 vs 複数ターゲット
...
Nxは[キャッシュ](https://nx.dev/concepts/how-caching-works)を使用してビルドを高速化します。キャッシュを無効にするには`--skip-nx-cache`を追加:
...
**Yes, sync the changes and run the tasks**を選択してください。TypeScript参照が自動的に追加され、IDEエラーが解消されます。
...
ビルド成果物はモノレポルートの`dist/`フォルダに生成されます。クリーンアップ時は`dist/`を削除します。
