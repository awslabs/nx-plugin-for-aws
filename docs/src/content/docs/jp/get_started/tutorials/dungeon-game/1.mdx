...
:::tip[ローカルファースト開発]
`ts#dynamodb`ジェネレーターは、コンテナ内で[DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)を実行する`serve-local`ターゲットを提供します。`connection`ジェネレーター(以下で実行します)と組み合わせることで、**チュートリアルの最後にデプロイすることを選択するまで、ゲーム全体がAWS呼び出しなしでマシン上で実行されます**。
:::
