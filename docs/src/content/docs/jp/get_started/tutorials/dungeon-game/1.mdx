...
これは、エージェントDockerイメージをECRにアップロードし、AgentCore Runtimeを使用してホストするCDK `AgentRuntimeArtifact`を設定します。CDKの`buildContexts`を使用してワークスペースルートを参照し、Dockerfileをエージェントソースコードと同じ場所に配置できるようにします。
