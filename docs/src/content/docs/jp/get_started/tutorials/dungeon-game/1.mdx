...
ReactウェブサイトからエージェントのAPIを型安全な自動生成クライアントで呼び出せるように、Game UIをストーリーエージェントに接続しましょう。
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** エージェントのFastAPIアプリからOpenAPI仕様を生成するスクリプト
    - project.json `agent-openapi`ターゲットで更新
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** 型安全なOpenAPIクライアントのプロバイダー
        - **QueryClientProvider.tsx** TanStack React Queryクライアントプロバイダー(まだ存在しない場合)
      - hooks/
        - **useStoryAgent.tsx** TanStack Queryオプションプロキシ用フック
        - **useStoryAgentClient.tsx** バニラAPIクライアント用フック
      - generated/
        - story-agent/ ビルド時にOpenAPI仕様から生成
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json クライアント生成ターゲットで更新
    - **main.tsx** StoryAgentProviderを追加するように更新

</FileTree>

ジェネレーターは:
- storyプロジェクトにエージェントスコープのOpenAPI仕様生成スクリプトを作成します
- ビルド時にエージェントのOpenAPI仕様から型安全なTypeScriptクライアントを生成します
- エージェントのAPIを呼び出すためのプロバイダー、フック、クライアントをウェブサイトに追加します
- エージェントのローカルサーバーを自動的に起動し、変更時にクライアントを再生成するように`serve-local`を設定します
