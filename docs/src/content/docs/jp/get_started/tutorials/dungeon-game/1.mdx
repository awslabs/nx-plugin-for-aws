...
`echo`メソッドの実装で、入力・出力データ構造を厳密に型定義しています。
...
/**
 * GameApi用AWS API Gateway REST APIを構成するCDKコンストラクト
 * @template TIntegrations - 操作名とインテグレーションのマップ
 */
...
GameApiを定義するCDKコンストラクトです。`defaultIntegrations`メソッドにより、tRPC APIの各プロシージャ用に個別のLambda関数が自動生成されます。
...
/**
 * StoryApi用AWS API Gateway REST APIを構成するCDKコンストラクト
 * @template TIntegrations - 操作名とインテグレーションのマップ
 */
...
StoryApiを定義するCDKコンストラクトです。`defaultIntegrations`メソッドにより、FastAPIの各操作用に個別のLambda関数が自動生成されます。
...
APIメソッドを定義する場所です。[Pydantic](https://docs.pydantic.dev/latest/)を使用して入出力を型安全に定義できます。
...
Reactのマウントポイントです。[ファイルベースルーティング](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing)が設定されています。
...
`/welcome`ルート用コンポーネントです。後続のセクションで説明されます。
...
`RuntimeConfigProvider`と`CognitoAuth`コンポーネントが追加され、Cognito認証が有効になります。
...
tRPCの[React Query統合](https://trpc.io/blog/introducing-tanstack-react-query-client)を使用するフックです。

<Aside>
`useGameApi`フックはビルド不要で変更が即時反映されます。これはtRPCの[TypeScript推論](https://trpc.io/docs/concepts)によるものです。
</Aside>
...
<Aside type="tip">IDEにインポートエラーが表示される場合、インフラプロジェクトのtsconfig.jsonにTypeScript参照が設定されていないためです。Nxはビルド/コンパイル時に[動的に参照を生成](https://nx.dev/nx-api/js/generators/typescript-sync)します。詳細は<Link path="guides/typescript-project#importing-your-library-code-in-other-projects">TypeScriptガイド</Link>を参照。</Aside>
...
<Drawer title="Nxコマンド" trigger="初めてのコードビルド">
###### 単一 vs 複数ターゲット

`run-many`コマンドは複数サブプロジェクトでターゲットを実行します（`--all`で全対象）。依存関係を正しい順序で実行します。
...
`@dungeon-adventure/infra`プロジェクトでエラーが発生する場合、`cfn-guard`がWindows未対応のためです。`packages/infra/src/main.ts`を修正：
