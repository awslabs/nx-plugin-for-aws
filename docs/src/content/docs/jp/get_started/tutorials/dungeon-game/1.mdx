...
- packages/
  - story/
    - dungeon_adventure_story/ Pythonモジュール
      - agent/
        - init.py FastAPIアプリとミドルウェアをセットアップ
...
:::tip[ローカルファースト開発]
`ts#dynamodb`ジェネレーターは、コンテナ内で[DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)を実行する`serve-local`ターゲットを提供します。`connection`ジェネレーター(以下で実行します)と組み合わせることで、**チュートリアルの最後まで、デプロイなしでゲーム全体がマシン上で実行されます**。
:::
...
- packages/
  - dungeon-db/
    ...
    - dynamodb.config.json グローバルセカンダリインデックスの信頼できる情報源
    - src/
...
ストーリーエージェントの`agent-serve-local`は既にInventory MCPサーバーの`mcp-server-serve-local`に依存しており（上記の`story → inventory`接続経由）、Game APIとMCPサーバーの両方が`dungeon-db:serve-local`に依存するようになったため、いずれかのプロジェクトの`serve-local`ターゲットを実行すると、必要なすべての依存関係が正しい順序で起動されます。
</Aside>
