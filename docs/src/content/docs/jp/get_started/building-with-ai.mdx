...
Nx Plugin for AWSには[Skill](https://code.claude.com/docs/en/skills)/[Power](https://kiro.dev/docs/powers/)と[MCPサーバー](https://modelcontextprotocol.io/introduction)が含まれており、これらを組み合わせることでAIアシスタントがプラグインと効果的に連携できるようになります。これらを使用することで、AIによる開発ワークフローを加速しながら、決定論的な方法でプロジェクトをスキャフォールディングできるため、主要コンポーネントの設定に費やす時間とコンテキストウィンドウを削減できます。
