...
他のAIアシスタントの場合は、MCPサーバーを手動で設定できます。ほとんどのAIアシスタントでは、MCPサーバー設定用のJSONファイルが用意されています。Amazon Q Developerの場合、`~/.aws/amazonq/mcp.json`に配置されます。Nx Plugin for AWSのMCPサーバー用エントリを追加してください：
