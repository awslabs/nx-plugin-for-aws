...
1. **生成** — `nx migrate <package>@latest` は `package.json` 内のパッケージのバージョンを更新し、実行するマイグレーションをリストした `migrations.json` を書き込みます。まだ何もインストールされず、他のファイルも変更されないため、最初にバージョンの変更を確認できます。

2. **インストール** — パッケージマネージャーで新しいバージョンをインストールします（例：`pnpm install --no-frozen-lockfile`）。

3. **実行** — `nx migrate --run-migrations` は保留中の各マイグレーションをワークスペースに一つずつ適用します。

4. **コミット** — ワークスペースがビルドできることを確認し、`migrations.json` を削除してコミットします。
...
次に、以前と同様にマイグレーションの実行、確認、コミットを行います：

<NxCommands commands={['migrate --run-migrations']} />

<Aside type="tip">
`@aws/nx-plugin` のアップグレードは Nx をアップグレードしません（逆も同様）。`nx migrate` は指定したパッケージのみを更新します。複数の Nx メジャーバージョンをまたぐ場合は、一度に一つのメジャーバージョンずつアップグレードしてください。詳細については、[Automate Updating Dependencies](https://nx.dev/docs/features/automate-updating-dependencies) を参照してください。
</Aside>
...
ions.json` を削除してコミットし、次のパッケージのアップグレードを開始します。
