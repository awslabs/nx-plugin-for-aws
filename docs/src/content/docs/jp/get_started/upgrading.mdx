...
<Aside type="caution" title="実験的なジェネレーターはマイグレーションされません">
一部のジェネレーターは実験的とマークされています。これらのガイドページにはその旨を示すバナーが表示されています。これらのジェネレーターへの変更は、既存のワークスペースに適用するマイグレーションなしで提供される場合があります。そのため、使用している場合は、アップグレード後にそのガイドを確認し、プロジェクトとの違いを自分で調整してください。
</Aside>
