...
:::tip[IaC プロバイダーの選択]
ワークスペースを作成すると、優先する Infrastructure as Code (IaC) プロバイダーとして、[CDK](https://docs.aws.amazon.com/cdk/) または [Terraform](https://developer.hashicorp.com/terraform) のいずれかを選択するよう求められます。`--iac` を渡すことでプロンプトをスキップできます：
...
また、コンテナイメージをビルドするジェネレーターで使用されるコンテナエンジンについても尋ねられます。デフォルト（`infer`）を受け入れると、Docker がインストールされている場合は Docker が選択され、そうでない場合は Finch が選択されます。
:::

### AI を使用する

<Steps>
1. お好みのパッケージマネージャーで <Link path="guides/workspace">Nx ワークスペース</Link>を作成し、作成されたディレクトリを開きます：
...
2. AI コーディングアシスタントでワークスペースを開きます。
...
3. アプリケーションを依頼します：
    ...
    アシスタントは MCP サーバーを通じてジェネレーターを発見し、依存関係が必要とする順序で実行します。変更内容を確認してから、ステップ 2 に進んでください。
</Steps>

:::tip[アシスタント用のプラグインまたはパワーをインストール]
Claude Plugin と Kiro Power は、MCP サーバーをステアリングドキュメントとワークフローガイドとバンドルしており、MCP サーバー単体よりも優れた結果が得られます。インストール方法、またはワークスペースごとではなくグローバルに MCP サーバーを設定する方法については、<Link path="get_started/building-with-ai">AI で構築</Link>を参照してください。
:::
...
2. アプリケーションをスキャフォールドするジェネレーターを実行します。以下のボタンを使用してすべてをコピーするか、図の下に説明されているように各ジェネレーターを自分で実行してください。
...
## ステップ 3: クラウドリソースを定義して AWS にデプロイする
...
## ステップ 4: デプロイされたクラウドリソースでウェブサイトをテストする
...
ウェブサイトは CloudFront からも提供されます。そのディストリビューションドメイン名（デプロイ後にターミナルに出力されます）を開いて、完全に AWS 上で実行されているバージョンを使用してください。
...
:::note[保持されるリソース]
Cognito ユーザープールは削除保護が有効になった状態で作成されるため、自動的には削除されません。不要になった場合は、手動で削除してください（Cognito が SMS メッセージの送信に使用する IAM ロールも含む）。
:::
