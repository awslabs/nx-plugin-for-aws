...
:::note[保持されるリソース]
Cognito ユーザープールは削除保護が有効になった状態で作成されるため、自動的には削除されません（destroy コマンドは失敗を報告します）。不要になったら、ユーザープールと Cognito が SMS メッセージの送信に使用する IAM ロールを手動で削除してください。
:::
</Fragment>
</Infrastructure>
...
:::note[アセットバケット]
アセットバケットは意図的に保持されるため、別途削除してください。
:::
</Fragment>
<Fragment slot="terraform">
...
これにより、Terraform ステートを保持している S3 バケットが空にされて削除されるため、デプロイされたすべてのインフラストラクチャを削除した後にのみ実行してください。
:::
</Fragment>
</Infrastructure>
