...
ウェブサイトは Amazon Cognito によって保護されており、セルフサインアップはデフォルトで無効になっているため、デプロイしたユーザープールに最初のユーザーを作成します。ユーザープール ID は、デプロイメント出力または [Amazon Cognito コンソール](https://console.aws.amazon.com/cognito/)で確認できます：
...
`--message-action SUPPRESS` を削除すると、自分でユーザーと共有するのではなく、Cognito が指定したアドレスに一時パスワードをメールで送信します。Cognito コンソールのユーザープールの **Users** タブからユーザーを作成することもできます。

このユーザーとして初めてサインインすると、新しいパスワードを選択するよう求められ、次に SMS で送信されるコードまたは認証アプリからの時間ベースのワンタイムパスワードを使用して多要素認証を設定するよう求められます。

:::tip[セルフサインアップ]
各ユーザーを自分で作成するのではなく、ユーザーがサインインページから自分で登録できるようにしたい場合は、<Link path="guides/react-website-auth">`ts#website#auth`</Link> ジェネレーターを `--allowSignup=true` で実行してください。
:::

## ステップ 6: デプロイされたクラウドリソースでウェブサイトをテストする
...
ウェブサイトは `http://localhost:4200` で利用でき、API と認証用にデプロイしたリソースを指します。作成したユーザーでサインインして、動作を確認してください。

ウェブサイトは CloudFront からも提供されます — そのディストリビューションドメイン名を開いて、完全に AWS 上で実行されているバージョンを使用してください。
