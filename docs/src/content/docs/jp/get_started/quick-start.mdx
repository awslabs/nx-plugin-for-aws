...
以下のコマンドを実行して、任意のパッケージマネージャーで<Link path="guides/workspace">Nxワークスペース</Link>を作成します：

<CreateNxWorkspaceCommand workspace="my-project" />

:::note[既存のGitリポジトリ]
ワークスペースディレクトリが既にGitリポジトリ内にある場合、Git初期化はスキップされるため、既存のリポジトリはそのまま保持されます。
:::

:::tip[IaCプロバイダーの選択]
IaC（Infrastructure as Code）プロバイダーとして[CDK](https://docs.aws.amazon.com/cdk/)または[Terraform](https://developer.hashicorp.com/terraform)を選択するプロンプトが表示されます。`--iac`オプションを付けて実行することでプロンプトをスキップできます：
...
# 共有アセットバケット — すべてのLambda / APIモジュールのLambdaデプロイzipをステージング
module "asset_bucket" {
