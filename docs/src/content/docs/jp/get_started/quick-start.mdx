...
このクイックスタートガイドでは、tRPC API、Reactウェブサイト、Cognito認証、CDKインフラストラクチャを追加します。構築しているプロジェクトの種類に応じて、任意の組み合わせのジェネレーターを選択して、プロジェクトを迅速にブートストラップできます。左側のナビゲーションバーの__ガイド__を確認して、利用可能なオプションの全リストを参照してください。
