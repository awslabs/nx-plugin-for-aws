...
:::note[ルートモジュールの出力]
Terraform は `packages/infra/src` で宣言された出力のみを表示するため、デプロイ後に読み取りたいものは、それを生成するモジュールから再エクスポートする必要があります。そのため、上記の 2 つの `output` ブロックがあります。
:::
</Fragment>
</Infrastructure>
...
ウェブサイトは CloudFront からも提供されます。デプロイメント出力に表示されるディストリビューションドメイン名（CDK の場合は `DistributionDomainName` で終わる出力、Terraform の場合は `website_url`）を開いて、完全に AWS 上で実行されているバージョンを使用してください。
...
Cognito ユーザープールは削除保護が有効になった状態で作成されるため、上記のコマンドでは削除されません。削除する前に、[Amazon Cognito コンソール](https://console.aws.amazon.com/cognito/)で削除保護をオフにしてください。

<Infrastructure>
<Fragment slot="cdk">
:::note[保持されるリソース]
スタックを削除すると、ユーザープール、Cognito が SMS メッセージの送信に使用する IAM ロール、API Gateway が CloudWatch Logs への書き込みに使用する IAM ロール、ウェブサイトバケットと API および WAF ログ グループを暗号化する KMS キー、およびログ グループ自体が残ります。不要になったら、プール、ロール、ログ グループを削除し、キーの削除をスケジュールしてください。
:::
</Fragment>
<Fragment slot="terraform">
:::caution[削除保護がオフになるまで Destroy は失敗します]
Terraform はユーザープールを削除しようとするため、`destroy` は `The user pool cannot be deleted because deletion protection is activated` で停止し、まだ到達していないリソースが残ります。削除保護をオフにしてから、destroy コマンドを再度実行してすべてを削除してください。ユーザープールと Cognito が SMS メッセージの送信に使用する IAM ロールも含まれます。
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">既存のプロジェクトに追加する</Link> — すでにお持ちの Nx ワークスペースまたはモノレポでプラグインを採用します。
