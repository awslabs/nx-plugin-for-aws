...
以下はエージェント用にCognito認証を設定する方法を示します。

<Infrastructure>
<Fragment slot="cdk">
CognitoでJWT認証を設定するには、`RuntimeAuthorizerConfiguration.usingCognito()`ファクトリーメソッドを使用します：
...
または、独自のOIDCプロバイダーでカスタムJWT認証を行う場合は、`RuntimeAuthorizerConfiguration.usingJWT()`を使用します：
...
<Fragment slot="terraform">
JWT認証を設定するには、MCPサーバーモジュールの`authorizer_configuration`変数を以下のように設定：
