...
このジェネレータは選択した `iac` に基づいてCDKまたはTerraformのインフラストラクチャーコードを生成します。生成されたコードを使用して関数をデプロイできます。
...
:::note[イベントソースの一致]
ハンドラー関数内でイベントが適切に処理されるよう、イベントソースが選択した `event` オプションと一致していることを確認してください。
:::
