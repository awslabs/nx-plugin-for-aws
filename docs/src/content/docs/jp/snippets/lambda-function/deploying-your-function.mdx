...
このジェネレータは `common/terraform` フォルダに関数をデプロイするためのTerraformモジュールを作成します。Terraform設定で使用できます。

関数モジュールは、Lambda API経由でインラインアップロードする代わりに（70 MBのリクエスト制限があります）、共有S3アセットバケットにデプロイzipをステージングします。デプロイごとに `core/asset-bucket` モジュールを一度インスタンス化し、その `bucket_name` 出力を `asset_bucket_name` 入力を介してすべてのlambda / APIモジュールに渡します：
...
`core/asset-bucket` モジュールは、S3管理（AES256）暗号化、バージョニング、TLS専用バケットポリシー、パブリックアクセスブロック、およびS3アクセスログ記録が有効化されたS3バケットをプロビジョニングします（ログは兄弟アクセスログバケットに配信されます）。デプロイ内のすべてのLambda / APIモジュールで同じモジュールインスタンスを再利用してください。各呼び出し元はモジュールスコープのキープレフィックスの下に書き込むため、衝突は発生しません。
