...
TypeScriptクライアントの場合、<Link path="/guides/react-website">`ts#website`ジェネレーター</Link>と<Link path="/guides/connection">`connection`ジェネレーター</Link>を`ts#api`の例（`framework`を`smithy`に設定）と組み合わせて使用し、クライアントの生成とウェブサイトへの統合方法を確認できます。これは`open-api#ts-client`または`open-api#ts-hooks`ジェネレーターを呼び出すビルドターゲットを設定します。これらのジェネレーターをOpenAPI仕様を指すように自分で使用できます。
...
同等の機能は、`type`を`shapes`に設定した<Link path="/guides/smithy-project">`smithy#project`ジェネレーター</Link>です：

<RunGenerator generator="smithy#project" requiredParameters={{ name: 'my-shapes', type: 'shapes' }} />

`SmithyShapeLibraryProject`からshapeを生成されたプロジェクトの`src`フォルダーに移動し、<Link path="/guides/smithy-project#depending-on-a-shape-library">Smithyプロジェクトガイド</Link>を参照してライブラリをAPIのモデルの依存関係として接続する方法を確認してください。
