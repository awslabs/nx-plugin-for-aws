...
TypeScriptクライアントの場合、<Link path="/guides/react-website">`ts#react-website`ジェネレーター</Link>と<Link path="/guides/connection">`connection`ジェネレーター</Link>を`ts#smithy-api`の例と組み合わせて使用し、クライアントの生成とウェブサイトへの統合方法を確認できます。これは`open-api#ts-client`または`open-api#ts-hooks`ジェネレーターを呼び出すビルドターゲットを設定します。これらのジェネレーターをOpenAPI仕様を指すように自分で使用できます。
