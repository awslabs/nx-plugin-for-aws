...
1. `get-shopping-lists.smithy`の`GetShoppingLists`オペレーションに新しい`@cursor`トレイトを追加:
    ...
    Nx Plugin for AWS（<Link path="guides/api-connection/react-smithy">`api-connection`</Link>ジェネレーター経由）のクライアントジェネレーターを使用する場合、`@paginated`オペレーションには`@cursor`も使用する必要があります。
