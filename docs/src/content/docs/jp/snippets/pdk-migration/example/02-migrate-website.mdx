...
:::tip
ここでは `dev` ターゲット（`serve-local` のエイリアス）を使用しています。これは `connection` で接続されたAPIのローカルサーバーも起動し、ウェブサイト・モデル・バックエンドに変更があるとホットリロードします。CDKコードを書く前にAPIとウェブサイトをローカルでテストできます！
:::
