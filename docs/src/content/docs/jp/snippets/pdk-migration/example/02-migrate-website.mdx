...
このプロジェクトタイプは [`create-react-app`](https://github.com/facebook/create-react-app) を活用していましたが、これは現在非推奨となっています。このガイドでウェブサイトを移行するために、より現代的でサポートされている技術、すなわち [Vite](https://vite.dev/) を使用する <Link path="/guides/react-website">`ts#website` ジェネレーター</Link> を使用します。
