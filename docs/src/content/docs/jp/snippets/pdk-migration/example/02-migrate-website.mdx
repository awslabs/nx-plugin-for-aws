...
<Link path="/guides/react-website">`ts#website` ジェネレータ</Link> を実行し、`framework` を `react` に設定して、`packages/website` にウェブサイトプロジェクトをセットアップします：
...
</Drawer>          navigationItemAriaLabel: () => "",
            }}
            empty={<></>}
          />
        )}
...
</Drawer>ncementDndItemReordered: () => "",
              liveAnnouncementDndItemResized: () => "",
              liveAnnouncementDndStarted: () => "",
              liveAnnouncementItemRemoved: () => "",
              navigationAriaLabel: "",
