...
APIをデータベースと同じVPCにデプロイし、`additional_iam_policy_statements`を介して`rds-db:connect`を付与し、セキュリティグループルールのペアでネットワークパスを開きます：
...
API Lambda関数は、プライベート分離サブネットではなく、**エグレス付きプライベートサブネット**にデプロイしてください。`RUNTIME_CONFIG_APP_ID`は、データベースモジュールの出力ではなく、共有<Link path="guides/runtime-config">ランタイム設定</Link> AppConfigアプリケーションを指す必要があります。
