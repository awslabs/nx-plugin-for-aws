...
API Lambda関数は、プライベート分離サブネットではなく、**エグレス付きプライベートサブネット**にデプロイしてください。`RUNTIME_CONFIG_APP_ID`は、データベースモジュールの出力ではなく、共有<Link path="guides/runtime-config">ランタイム設定</Link> AppConfigアプリケーションを指す必要があります。データベースモジュールのランタイム設定エントリがデプロイされるように、インスタンス化する際に`database`名前空間を含めてください：
