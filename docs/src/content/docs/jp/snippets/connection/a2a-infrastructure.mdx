...
リモートエージェントの AgentCore ランタイム ARN は、生成された CDK コンストラクトによって <Link path="guides/runtime-config">Runtime Configuration</Link> の `agentcore` 名前空間に自動的に登録されるため、ホストエージェントは実行時にそれを検出できます。
</Fragment>
...
リモートエージェントの AgentCore ランタイム ARN は、生成された Terraform モジュールによって <Link path="guides/runtime-config">Runtime Configuration</Link> の `agentcore` 名前空間に自動的に登録されるため、ホストエージェントは実行時にそれを検出できます。
</Fragment>
</Infrastructure>