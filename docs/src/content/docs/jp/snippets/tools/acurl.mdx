...
次の関数を PowerShell プロファイルに追加するか、コマンドを実行したい同じ PowerShell セッションに以下を貼り付けることができます。
```powershell
...
:::caution[Curl バージョン要件]
AWS Signature v4 のサポートは、`curl` の最新バージョンで追加されており、近年このサポートにいくつかのバグ修正が行われています。`curl` バージョン 8.17.0 以上をインストールすることをお勧めします。OS に付属しているバージョンは古い可能性があります。
:::