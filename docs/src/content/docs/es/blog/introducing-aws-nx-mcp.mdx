...
Si estás interesado en explorar el desarrollo AWS asistido por IA con más estructura y confiabilidad, prueba el **Nx Plugin para AWS MCP Server**. Puedes configurarlo en tu asistente de IA favorito (Kiro, Kiro CLI, Cline, Claude Code, etc) con la siguiente configuración del servidor MCP:
