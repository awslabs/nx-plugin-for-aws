...
Los Strands Agents leen su configuración de tiempo de ejecución desde una aplicación compartida de AWS AppConfig — consulta la <Link path="/guides/terraform-project#shared-runtime-config-appconfig">guía de infraestructura Terraform</Link> para más detalles. Instancia el módulo `core/runtime-config/appconfig` una vez por despliegue y pasa sus salidas a cada módulo de agente / MCP:
...
Declara `module "runtime_config_appconfig_deployment"` una vez al final de tu módulo raíz (con `depends_on` cubriendo cada módulo que contribuye una entrada de runtime-config) — agrega las entradas contribuidas y crea las versiones de configuración alojada de AppConfig + despliegues. Consulta el <Link path="/guides/terraform-project#shared-runtime-config-appconfig">patrón completo</Link>.
</Fragment>
</Infrastructure>
