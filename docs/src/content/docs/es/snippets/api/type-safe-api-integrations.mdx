...
El soporte del generador actualmente varía según el backend:

- `ts#trpc-api` admite tanto `isolated` como `shared` para `ServerlessApiGatewayRestApi` y `ServerlessApiGatewayHttpApi` a través de la opción `integrationPattern`.
- `py#fast-api` y `ts#smithy-api` actualmente generan `isolated`.

También puedes cambiar un constructo de API CDK generado existente entre los dos patrones editando el campo `pattern` en su método `defaultIntegrations`:

```ts {3}
return IntegrationBuilder.rest({
  // Cambia esto a 'isolated' o 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Los módulos de Terraform ya usan el patrón router por defecto, con una única función Lambda que maneja todas las operaciones de la API.
</Fragment>
</Infrastructure>
...
#### Patrón Router Personalizado

<Infrastructure>
<Fragment slot="cdk">
Si necesitas personalizar el patrón router más allá de los valores por defecto generados, puedes modificar libremente el método `defaultIntegrations` de tu API para configurar las integraciones como prefieras:
