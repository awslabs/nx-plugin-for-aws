...
:::caution[Variables de Entorno Lambda]
Si especificas `environment` en tus opciones por defecto esto __sobrescribirá__ cualquier variable de entorno que esté definida dentro de tu constructo de API generado. Usualmente es preferible usar el método `addEnvironment` para las funciones Lambda. Puedes lograr esto fácilmente recorriendo tus integraciones:
