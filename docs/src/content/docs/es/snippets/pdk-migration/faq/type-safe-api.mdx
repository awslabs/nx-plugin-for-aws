...
Para clientes TypeScript, puedes usar el <Link path="/guides/react-website">generador `ts#react-website`</Link> y el <Link path="/guides/api-connection">generador `api-connection`</Link> con un ejemplo de `ts#smithy-api` para ver cómo se generan e integran los clientes con un sitio web. Esto configura objetivos de compilación que generan clientes invocando nuestros generadores `open-api#ts-client` o `open-api#ts-hooks`. Puedes usar estos generadores tú mismo apuntándolos a tu Especificación OpenAPI.
