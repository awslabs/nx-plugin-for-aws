...
Con el Plugin Nx para AWS, la integración de API se realiza mediante el <Link path="/guides/api-connection">generador `api-connection`</Link>. A continuación, usamos este generador para que nuestro sitio web pueda invocar nuestra API Smithy:
...
:::tip
Usamos el objetivo `serve-local` que también inicia servidores locales para APIs conectadas con `api-connection`, y recarga en caliente si cambian el sitio web, modelo o backend. ¡Esto nos permite probar la API y el sitio web localmente antes de escribir código CDK!
:::
