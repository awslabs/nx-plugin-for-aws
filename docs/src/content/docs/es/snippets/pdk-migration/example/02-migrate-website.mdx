...
Iniciemos el servidor local del sitio web:

<NxCommands commands={["dev website"]} />

:::tip
Usamos el objetivo `dev` aquí, que también inicia servidores locales para APIs conectadas con `connection`, y recarga en caliente si cambian el sitio web, modelo o backend. ¡Esto nos permite probar la API y el sitio web localmente antes de escribir código CDK!
:::
