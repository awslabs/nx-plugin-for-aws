...
1. Añade `createFileRoute` para registrar cada ruta:
    ...
    Al guardar el archivo, notarás que los errores de tipos con la llamada a `createFileRoute` desaparecen.
...
:::tip
Consulta la [guía detallada en la documentación de TanStack Router](https://tanstack.com/router/latest/docs/installation/migrate-from-react-router) para escenarios más complejos.
:::

#### Corregir importaciones de componentes

Como nuestros archivos de ruta no están tan profundamente anidados en el árbol de archivos como en el proyecto PDK, necesitamos ajustar la importación de `CreateItem` en ambos `routes/index.tsx` y `routes/$shoppingListId.tsx`:
