...
Por lo tanto, podemos usar el <Link path="/es/guides/ts-smithy-api">generador `ts#smithy-api`</Link> para proporcionar funcionalidad equivalente.
...
Ejecuta el <Link path="/es/guides/ts-smithy-api">generador `ts#api`</Link> con `framework` establecido en `smithy` para configurar tu proyecto de api en `packages/api`:
...
1. Agrega el nuevo trait `@cursor` a la operación `GetShoppingLists` en `get-shopping-lists.smithy`:
    ...
    Cualquier operación `@paginated` también debe usar `@cursor` si estás utilizando el generador de cliente proporcionado por el Nx Plugin for AWS (a través del generador <Link path="es/guides/connection/react-smithy">`api-connection`</Link>).
