...
:::nota
Si utilizaste OpenAPI o TypeSpec como lenguaje de modelado, o tienes manejadores implementados en Python o Java, consulta las [Preguntas Frecuentes](#frequently-asked-questions) para conocer opciones potenciales.
:::
...
Ahora que tenemos la estructura básica para nuestro proyecto Smithy API, podemos migrar el modelo:
...
1. Actualiza el nombre del servicio y el namespace en `smithy-build.json` para que coincidan con la aplicación PDK:
...
1. Agrega el nuevo trait `@cursor` a la operación `GetShoppingLists` en `get-shopping-lists.smithy`:
    ...
    Cualquier operación `@paginated` también debe usar `@cursor` si estás usando el generador de clientes proporcionado por el Nx Plugin para AWS (a través del generador <Link path="guides/api-connection/react-smithy">`api-connection`</Link>).

1. Finalmente, elimina el trait `@handler` de todas las operaciones ya que no es compatible con el Nx Plugin para AWS. Al usar `ts#smithy-api`, no necesitamos las construcciones CDK de funciones lambda generadas automáticamente ni los objetivos de bundling generados por este trait, ya que usamos un solo bundle para todas las funciones lambda.

</Steps>

En este punto, ejecutemos un build para verificar los cambios en nuestro modelo y asegurarnos de tener código de servidor generado con el que trabajar. Habrá algunos errores en el proyecto backend (`@shopping-list/api`) pero los abordaremos a continuación.
...
Nuestros manejadores lambda dependen del paquete `@aws-sdk/client-dynamodb`, así que instalémoslo primero:
...
1. Elimina las importaciones de `my-api-typescript-runtime` e importa el tipo de operación desde el SDK de servidor TypeScript generado, así como el `ServiceContext` por ejemplo:
...
1. Actualiza las referencias a los parámetros de entrada. Dado que el SSDK proporciona tipos que coinciden exactamente con tu modelo Smithy (en lugar de agrupar parámetros de path/query/header separados del parámetro body), actualiza cualquier referencia de entrada:
...
1. Elimina el uso de `Response`. En su lugar, simplemente devolvemos objetos planos en el SSDK.
...
1. Actualiza cualquier importación para usar sintaxis ESM, es decir, agregando la extensión `.js` a las importaciones relativas.
...
<Drawer title="Migración del manejador de lista de compras" trigger="Haz clic aquí para ver ejemplos completos antes/después de las tres operaciones de lista de compras del tutorial">
 ...
 * El método deleteShoppingListHandler envuelve el manejador type-safe y gestiona inputs/outputs
 */
export const handler = deleteShoppingListHandler(
  ...INTERCEPTORS,
  deleteShoppingList,
 ...
 * El método getShoppingListsHandler envuelve el manejador type-safe y gestiona inputs/outputs
 */
 ...
 * El método putShoppingListHandler envuelve el manejador type-safe y gestiona inputs/outputs
 */
...
Generamos el proyecto Smithy API con el nombre `api` inicialmente para que se agregue a `packages/api` y sea consistente con el proyecto PDK. Dado que nuestra Smithy API ahora define `service MyApi` en lugar de `service Api`, necesitamos actualizar cualquier instancia de `getApiServiceHandler` con `getMyApiServiceHandler`.
