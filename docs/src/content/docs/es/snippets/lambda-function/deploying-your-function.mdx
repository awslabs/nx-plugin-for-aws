...
:::note[Coincidencia de origen de eventos]
...
El módulo de función almacena su zip de implementación en un bucket S3 de activos compartido en lugar de cargarlo en línea a través de la API de Lambda (que tiene un límite de solicitud de 70 MB). Instancia el módulo `core/asset-bucket` una vez por implementación y pasa su salida `bucket_name` a cada módulo de lambda / API a través de la entrada `asset_bucket_name`:
...
El módulo `core/asset-bucket` aprovisiona un bucket S3 con cifrado administrado por S3 (AES256), versionado, una política de bucket solo TLS, bloqueo de acceso público y registro de acceso S3 habilitado (los registros se entregan a un bucket de registros de acceso hermano). Reutiliza la misma instancia del módulo en cada módulo de Lambda / API en tu implementación: cada llamador escribe bajo un prefijo de clave con alcance de módulo, por lo que no hay colisiones.
