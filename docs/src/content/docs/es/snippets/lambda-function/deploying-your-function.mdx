...
Este generador crea infraestructura como código de CDK o Terraform según el `iac` seleccionado. Puedes usarlo para implementar tu función.
...
Asegúrate de que el origen del evento coincida con la opción `event` seleccionada para garantizar un manejo adecuado del evento dentro de tu función handler.
:::
...
Este generador crea un módulo de Terraform para implementar tu función en la carpeta `common/terraform`. Instancia el módulo compartido `core/asset-bucket` una vez por implementación y pasa su `bucket_name` a la entrada `asset_bucket_name`:
