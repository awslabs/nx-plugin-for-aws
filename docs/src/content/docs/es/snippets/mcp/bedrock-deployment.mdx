...
Puedes otorgar acceso para invocar tu servidor MCP en Bedrock AgentCore Runtime usando el método `grantInvokeAccess`. Por ejemplo, quizás quieras que un agente generado con el generador <Link path="/guides/py-agent">`py#agent`</Link> pueda llamar a tu servidor MCP:
