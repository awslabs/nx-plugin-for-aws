...
Si seleccionaste `agentcore` para `infra`, se genera la infraestructura relevante de CDK o Terraform que puedes usar para desplegar tu servidor MCP en [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Pasa las salidas del módulo compartido <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> al módulo del servidor MCP:
...
[Docker](https://www.docker.com/) es necesario para construir y desplegar tu servidor MCP. Asegúrate de haber instalado e iniciado Docker para evitar errores como:
