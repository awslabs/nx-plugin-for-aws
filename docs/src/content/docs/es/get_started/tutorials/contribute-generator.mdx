...
Si has completado el <Link path="get_started/tutorials/dungeon-game/overview">tutorial de aventura dungeon</Link>, o ya tienes otro workspace Nx existente que usa una API tRPC, puedes saltar este paso.
:::
...
Tenemos un conjunto de "pruebas de humo" que ejecutan generadores en un workspace nuevo y verifican que todo se construya. Como mínimo, tu nuevo generador debe agregarse a **ambas** matrices de generadores para que sea ejercitado por las pruebas de humo:

- `e2e/src/smoke-tests/generator-matrix.ts` — ejecuta cada generador a través de la CLI, una invocación a la vez, exactamente como lo haría un usuario.
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — un generador oculto que compone todos los demás en proceso en un solo tree.

Ambas valen la pena mantener. La matriz CLI prueba el camino que los usuarios realmente toman. La matriz compuesta se envía con el plugin, por lo que cada versión lanzada lleva la matriz de los generadores que _ella_ tenía — lo que permite que las pruebas que actualizan un workspace antiguo lo estructuren con la matriz de esa versión. Debido a que llama a los generadores directamente en lugar de a través de la CLI, sus opciones se verifican por tipo contra el esquema de cada generador, por lo que una nueva opción requerida o un enum modificado falla la construcción allí en lugar de reducir silenciosamente la cobertura.
