...
Tenemos un conjunto de "pruebas de humo" que ejecutan generadores en un workspace nuevo y verifican que todo se construya. Como mínimo, tu nuevo generador debe agregarse a la matriz de generadores en `packages/nx-plugin/src/internal/test-matrix/generator.ts` para que sea ejercitado por las pruebas de humo.

La matriz es en sí misma un generador (oculto) que compone todos los demás, por lo que se envía con el plugin. Esto significa que cada versión lanzada lleva la matriz de los generadores que _ella_ tenía, lo que permite que las pruebas que actualizan un workspace antiguo lo estructuren con la matriz de esa versión. Dado que llama a los generadores directamente en lugar de a través de la CLI, las opciones se verifican por tipo contra el esquema de cada generador — por lo que una nueva opción requerida o un enum modificado fallará la construcción aquí en lugar de reducir silenciosamente la cobertura.
