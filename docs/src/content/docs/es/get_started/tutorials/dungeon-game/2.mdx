...
Ampliamos nuestro `IMiddlewareContext` para añadir el `IDynamoDBContext`.
  </TabItem>
...
:::tip
Si estás iterando en cambios de código de funciones lambda, puedes desplegar con el flag `--hotswap` después de construir el código base para un tiempo de despliegue mucho más corto (2-3 segundos).
...
Puedes agregar el siguiente script a tu archivo `.bashrc` (y hacer `source`) o simplemente pegarlo en la misma terminal donde ejecutarás el comando.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
<Tabs>
  <TabItem label="Local">
    ...
    <Aside type="caution">
    Usa el valor de salida `dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX` del despliegue CDK para reemplazar el marcador resaltado.
    </Aside>
...
```
    <Aside type="caution">
    Usa el valor de salida `dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX` del despliegue CDK para reemplazar el marcador y ajustar la región correspondiente.
    </Aside>
  </TabItem>
</Tabs>
