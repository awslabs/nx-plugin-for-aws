...
Los procedimientos y el servidor MCP que acabamos de escribir importan `@dungeon-adventure/dungeon-db`, una nueva importación entre proyectos, así que comienza sincronizando el espacio de trabajo:

<NxCommands commands={["sync"]} />

<Aside type="tip" title="Por qué se necesita este paso">
`nx sync` declara `@dungeon-adventure/dungeon-db` en los archivos `package.json` de `game-api` e `inventory` y agrega las referencias de proyecto de TypeScript correspondientes por ti, ahorrándote la necesidad de agregar esto manualmente al declarar dependencias entre proyectos.
</Aside>

Luego corrige cualquier problema de lint:

<PackageManagerShortCommand commands={["lint"]} />

Y construye la base de código:
