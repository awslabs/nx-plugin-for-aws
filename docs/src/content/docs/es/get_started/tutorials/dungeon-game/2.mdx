...
El cliente S3 se agrega al `package.json` propio del proyecto `game-api`, ya que el código fuente de ese proyecto lo importa. Las versiones aún se alinean en todo el espacio de trabajo a través del catálogo de dependencias del gestor de paquetes (cuando es compatible) ([política de versión única](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)). Para más información, consulta la <Link path="guides/typescript-project#dependencies">guía ts#project</Link>.
</Aside>
