...
Para cada uno de los esquemas que definimos en Zod, también exportamos una interfaz usando la sintaxis `z.TypeOf`. Esto convierte nuestra definición de Zod en una interfaz de Typescript sin tener que duplicar esfuerzos.
</Aside>
...
El cliente de S3 se agrega al `package.json` propio del proyecto `game-api`, ya que el código fuente de ese proyecto lo importa. Las versiones aún están alineadas en todo el espacio de trabajo a través del catálogo de dependencias del gestor de paquetes (donde sea compatible) ([política de versión única](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)). Para más información, consulta la <Link path="guides/typescript-project#dependencies">guía de ts#project</Link>.
</Aside>
