...
La dependencia `@aws-sdk/client-s3` ya está en el workspace, por lo que no hay nada nuevo que instalar. El nombre del bucket proviene del mismo mecanismo de Configuración de Runtime utilizado para `tables` — lo registramos bajo un nuevo namespace `buckets` en el stack de infraestructura a continuación.
  </TabItem>
</Tabs>
