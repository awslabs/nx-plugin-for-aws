...
:::note[El historial de conversación vive con el agente]
No persistimos mensajes de chat a través de la Game API. En el <Link path="get_started/tutorials/dungeon-game/3">Módulo 3</Link>, el Story Agent conecta un `FileSessionManager` que escribe el historial de cada hilo en disco, indexado por el id del hilo que la UI envía en cada turno — así cada par `(jugador, género)` reanuda su propia historia sin que la UI tenga que reproducir mensajes.
:::
