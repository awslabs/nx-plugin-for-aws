...
description: Un tutorial de cómo construir un juego de aventuras de mazmorra con IA agéntica usando el @aws/nx-plugin.
---
...
Usamos `--iac=cdk` ya que usaremos CDK para infraestructura como código en este tutorial. El Nx Plugin para AWS también soporta `terraform`.
:::
...
<Drawer title="Archivos actualizados por ts#api" trigger="Haz clic aquí para examinar estos archivos en más detalle.">
A continuación se muestra una lista de todos los archivos generados por el generador `ts#api`. Examinaremos algunos de los archivos clave resaltados en el árbol de archivos:
<FileTree>
- packages/
  - common/
    - constructs/
...
Verás que aparecen algunos archivos nuevos en tu árbol de directorios.
<Drawer title="Archivos actualizados por py#agent" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
Esto crea un agente Strands de ejemplo y define una herramienta de resta.
...
## Tarea 5: Crear la base de datos del juego
...
El generador `ts#dynamodb` proporciona un target `serve-local` que ejecuta [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) en un contenedor. Combinado con el generador `connection` (que ejecutamos a continuación), esto significa que **todo el juego se ejecuta en tu máquina sin llamadas a AWS** hasta que elijas desplegarlo al final del tutorial. Necesitarás [un motor de contenedores](https://docs.docker.com/get-started/get-docker/) como Docker o Finch instalado y en ejecución.
:::

Verás que aparecen algunos archivos nuevos en tu árbol de directorios.
<Drawer title="Archivos actualizados por ts#dynamodb" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - gsi.ts fuente de verdad para índices secundarios globales
      - entities/
        - example.ts entidad ElectroDB de ejemplo (reemplazaremos esto)
        - index.ts exportaciones de entidades
    - project.json agrega los targets `serve-local` y `pull-image`
  - common/constructs/
    - src/
      - app/dynamodb/
...
<Drawer title="Archivos actualizados por ts#website" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
- packages/
  ...
  - game-ui/
    ...
    - ...
  - common/
    - shadcn/ biblioteca compartida shadcn/ui (tokens de tema, `Button`, `Card`, `Input`, `Sidebar`, …) importada por cada sitio web con `uxProvider=Shadcn`
      - src/components/ui/*
...
<Drawer title="Archivos actualizados por ts#website#auth" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
El generador:
- Detecta el `uxProvider` del sitio web React (Shadcn aquí) y proporciona componentes de chat coincidentes.
...
Debido a que el `agent-serve-local` del Story Agent ya depende del `mcp-server-serve-local` del servidor MCP de Inventario (a través de la conexión `story → inventory` anterior), y tanto la Game API como el servidor MCP ahora dependen de `dungeon-db:serve-local`, ejecutar el target `serve-local` de cualquier proyecto inicia todas las dependencias que necesita — incluyendo DynamoDB Local — en el orden correcto. Confiaremos en esto durante el resto del tutorial.
</Aside>
...
## Tarea 7: Actualizar nuestra infraestructura
...
<Aside type="note" title="Entornos no interactivos">
En un shell no interactivo (CI, por ejemplo) no hay prompt — en su lugar, el build falla con un error "workspace is out of sync". Ejecuta <NxCommands commands={['sync']} /> para aplicar los generadores de sincronización pendientes, luego construye nuevamente, o establece `sync.applyChanges` a `true` en `nx.json` para aplicar los cambios automáticamente en cada build. Encontrarás esta misma verificación de desincronización cada vez que cambien las referencias de proyecto (por ejemplo, después de agregar procedimientos en el siguiente módulo).
</Aside>

Todos los artefactos construidos están ahora disponibles dentro de la carpeta `dist/` ubicada en la raíz del monorepo. Esta es una práctica estándar cuando se usan proyectos generados por el `@aws/nx-plugin` ya que no contamina tu árbol de archivos con archivos generados. En caso de que quieras limpiar tus archivos, elimina la carpeta `dist/` sin preocuparte por artefactos de build esparcidos por todo el árbol de archivos.
