...
El hook `useGameApi` es diferente al hook `useStoryAgent` ya que no requiere un build para que los cambios se reflejen gracias al uso de [inferencia de TypeScript](https://trpc.io/docs/concepts) por parte de tRPC. Esto permite a los desarrolladores hacer cambios en su backend que se reflejan instantáneamente en su frontend.
</Aside>
...
Conectemos nuestra Game UI al Story Agent para que podamos llamar a la API del agente desde nuestro sitio web React con un cliente auto-generado y con tipos seguros.
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** Script para generar la especificación OpenAPI desde la aplicación FastAPI del agente
    - project.json Actualizado con target `agent-openapi`
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** Provider para el cliente OpenAPI con tipos seguros
        - **QueryClientProvider.tsx** Provider del cliente TanStack React Query (si no está ya presente)
      - hooks/
        - **useStoryAgent.tsx** Hook para proxy de opciones de TanStack Query
        - **useStoryAgentClient.tsx** Hook para el cliente vanilla de la API
      - generated/
        - story-agent/ Generado en tiempo de build desde la especificación OpenAPI
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json Actualizado con targets de generación del cliente
    - **main.tsx** Actualizado para agregar StoryAgentProvider

</FileTree>

El generador:
- Crea un script de generación de especificación OpenAPI con alcance de agente en el proyecto story
- Genera un cliente TypeScript con tipos seguros desde la especificación OpenAPI del agente en tiempo de build
- Agrega un provider, hooks y cliente al sitio web para llamar a la API del agente
- Configura `serve-local` para iniciar automáticamente el servidor local del agente y regenerar el cliente en cambios
