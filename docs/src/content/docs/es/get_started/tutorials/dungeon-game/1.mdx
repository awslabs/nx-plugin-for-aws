...
- packages/
  - story/
    - dungeon\_adventure\_story/ módulo Python
      - agent/
        - main.py punto de entrada de aplicación FastAPI AG-UI para Bedrock AgentCore Runtime
...
Como elegimos `protocol=AG-UI`, el generador envuelve nuestro agente Strands en un adaptador [AG-UI](https://ag-ui.com/). `create_strands_app` construye una aplicación FastAPI que expone un único endpoint `/invocations` - las instancias de agente por sesión se crean automáticamente, y el historial de conversación se mantiene en memoria por `threadId` durante la vida del servidor. No necesitaremos modificar `main.py` en ninguna parte de este tutorial.
