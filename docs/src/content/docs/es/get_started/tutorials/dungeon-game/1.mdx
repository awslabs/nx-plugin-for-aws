...
- tsconfig.json
- aws-nx-plugin.config.mts configuración para el Nx Plugin de AWS
</FileTree>

Ahora estamos listos para comenzar a crear nuestros diferentes subproyectos usando `@aws/nx-plugin`.
  ...
  - game-api/ API tRPC
    - src/
      ...
      - schema/ definiciones de entradas y salidas para tu API
        - **echo.ts**
      - procedures/ implementaciones específicas de tus procedimientos/rutas de API
        - **echo.ts**
      - index.ts
...
```
El router define el punto de entrada para tu API tRPC y es donde declararás todos tus métodos de API. Como se ve arriba, tenemos un método llamado `echo` con su implementación en el archivo `./procedures/echo.ts`.
...
/**
 * Un constructo CDK que crea y configura un API Gateway REST de AWS
...
> extends RestApi<Operations, TIntegrations> {
  /**
   * Crea integraciones por defecto para todas las operaciones, implementando cada operación como
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          ...
          // Abrimos OPTIONS para permitir solicitudes preflight sin autenticación desde navegadores
          new PolicyStatement({
            effect: Effect.ALLOW,
...
Deberías ver nuevos archivos en tu árbol de directorios.
<Drawer title="py#fast-api archivos actualizados" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
- packages/
  - common/
    - constructs/
      ...
      - project.json actualizado para agregar dependencia de compilación en story_api
    - types/ tipos compartidos
      - src/
...
Ahora creemos la interfaz que permitirá interactuar con el juego. Para esto, creemos un sitio web llamado `GameUI` siguiendo estos pasos:

<RunGenerator generator="ts#react-website" requiredParameters={{name:"GameUI"}} noInteractive />

Deberías ver nuevos archivos en tu árbol de directorios.

<Drawer title="ts#react-website archivos actualizados" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
Este es el constructo CDK que define nuestro GameUI. Como se ve, ya está configurada la ruta al paquete generado para nuestra UI basada en Vite. Esto significa que en el momento de `build`, el empaquetado ocurre dentro del objetivo de compilación del proyecto game-ui y su salida se usa aquí.
...
Este es el punto de entrada donde se monta React. Como se muestra, inicialmente solo configura un `@tanstack/react-router` en una configuración de [`enrutado basado en archivos`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing). Esto significa que, mientras el servidor de desarrollo esté en ejecución, puedes simplemente crear archivos en la carpeta `routes` y `@tanstack/react-router` creará la configuración necesaria y actualizará el archivo `routeTree.gen.ts`. Este archivo mantiene todas las rutas de manera segura en tipos, lo que significa que cuando usas `<Link>`, la opción `to` solo mostrará rutas válidas. Para más información, consulta la [documentación de `@tanstack/react-router`](https://tanstack.com/router/v1/docs/framework/react/quick-start).
...
Deberías ver archivos nuevos/cambiados en tu árbol de directorios.

<Drawer title="ts#react-website#auth archivos actualizados" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
- packages/
  ...
  - game-ui/
    - src/
      - components/
        - AppLayout/
          - index.tsx agrega usuario conectado/cierre de sesión al encabezado
        - CognitoAuth/
          - index.ts gestiona el inicio de sesión en Cognito
        - RuntimeConfig/
          - index.tsx obtiene el `runtime-config.json` y lo provee a hijos vía contexto
      - hooks/
...
<Drawer title="UI -> FastAPI archivos actualizados de api-connection" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
Este hook puede usarse para hacer solicitudes API autenticadas a `StoryApi`. Como se ve en la implementación, usa el `StoryApi` generado en tiempo de compilación, por lo que verás un error en tu IDE hasta que compilemos nuestro código. Para más detalles sobre cómo se genera el cliente o cómo consumir la API, consulta la <Link path="guides/api-connection/react-fastapi">guía de React a FastAPI</Link>.
...
<Drawer title="UI -> tRPC archivos actualizados de api-connection" trigger="Haz clic aquí para examinar estos archivos en detalle.">
A continuación se muestra una lista de todos los archivos generados/actualizados por el generador `api-connection`. Examinaremos algunos archivos clave resaltados en el árbol:
<FileTree>
- packages/
  - game-ui/
...
<Drawer title="ts#infra archivos actualizados" trigger="Haz clic aquí para examinar estos archivos en detalle.">
...
...
// Suprime la RULE_NAME para el constructo o cualquiera de sus descendientes si es instancia de Bucket
...
Hagamos una actualización en nuestro `packages/infra/src/stacks/application-stack.ts` para instanciar algunos de nuestros constructos ya generados:
...
Nota que aquí proveemos integraciones por defecto para nuestras dos APIs. Por defecto, cada operación en nuestra API está mapeada a una función lambda individual para manejar esa operación.
...
También puedes disparar una compilación (o cualquier otra tarea) para un objetivo de proyecto único ejecutando el objetivo en el proyecto directamente. Por ejemplo, si queremos construir el proyecto `@dungeon-adventure/infra`, puedes ejecutar:
...
Nx depende del [caché](https://nx.dev/concepts/how-caching-works) para reutilizar artefactos de compilaciones previas y acelerar el desarrollo. Se requiere cierta configuración para que esto funcione correctamente y puede haber casos donde quieras realizar una compilación **sin usar el caché**. Para eso, simplemente agrega el argumento `--skip-nx-cache` a tu comando. Por ejemplo:

<NxCommands commands={['run @dungeon-adventure/infra:build --skip-nx-cache']} />
Si por alguna razón quisieras limpiar tu caché (almacenado en la carpeta `.nx`), puedes ejecutar:
...
Este mensaje indica que NX ha detectado archivos que pueden actualizarse automáticamente. En este caso, se refiere a los archivos `tsconfig.json` que no tienen referencias TypeScript configuradas en proyectos referenciados. Selecciona la opción **Yes, sync the changes and run the tasks** para continuar. ¡Deberías notar que todos los errores de importación en tu IDE se resuelven automáticamente ya que el generador de sincronización agregará las referencias TypeScript faltantes!
...
Si encuentras un error de compilación/síntesis para el proyecto `@dungeon-adventure/infra`, esto es esperado ya que la librería que instrumenta `cfn-guard` actualmente no soporta Windows. Hay una solicitud de función para esto, pero mientras tanto podemos deshabilitar `cfn-guard` modificando el archivo `packages/infra/src/main.ts` como sigue:
...
Todos los artefactos construidos están ahora disponibles en la carpeta `dist/` ubicada en la raíz del monorepo. Esta es una práctica estándar cuando se usan proyectos generados por `@aws/nx-plugin` ya que no contamina tu árbol de archivos con archivos generados. Si deseas limpiar tus archivos, simplemente borra la carpeta `dist/` sin preocuparte por archivos generados esparcidos.

¡Felicidades! Has creado todos los subproyectos necesarios para comenzar a implementar el núcleo de nuestro juego de aventuras.  🎉🎉🎉