...
Observemos algunos de los archivos clave:
...
```
El router define el punto de entrada para tu API tRPC y es donde declararás todos los métodos de tu API. Como se muestra arriba, tenemos un método llamado `echo` con su implementación en el archivo `./procedures/echo.ts`.
...
Este archivo es la implementación del método `echo` y como se ve está fuertemente tipado al declarar sus estructuras de datos de entrada y salida.
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          ...
          // Se puede definir acceso máquina a máquina granular aquí usando principios más específicos (ej. roles o
...
Aquí es donde se definirán todos tus métodos de API. Como se ve aquí, tenemos un método `read_root` mapeado a la ruta `GET /`. Puedes usar [Pydantic](https://docs.pydantic.dev/latest/) para declarar las entradas y salidas de tus métodos y asegurar la seguridad de tipos.
...
- packages/
  ...
  - infra
    - src/
      - stages/
        - **application-stage.ts** pilas CDK definidas aquí
      - stacks/
...
export class ApplicationStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // El código que define tu pila va aquí
  }
}
```
...
Nota aquí que proveemos integraciones predeterminadas para nuestras dos APIs. Por defecto, cada operación en nuestra API está mapeada a una función lambda individual para manejar esa operación.
...
También puedes disparar un build (o cualquier otra tarea) para un objetivo de proyecto único ejecutando el objetivo directamente en el proyecto. Por ejemplo, si queremos construir el proyecto `@dungeon-adventure/infra`, puedes ejecutar:
...
Nx depende del [caching](https://nx.dev/concepts/how-caching-works) para que puedas reutilizar artefactos de builds previos y acelerar el desarrollo. Se requiere cierta configuración para que esto funcione correctamente y puede haber casos donde quieras realizar un build **sin usar la caché**. Para eso, simplemente agrega el argumento `--skip-nx-cache` a tu comando. Por ejemplo:

<NxCommands commands={['run @dungeon-adventure/infra:build --skip-nx-cache']} />
Si por cualquier motivo quisieras limpiar tu caché (almacenada en la carpeta `.nx`), puedes ejecutar:
...
<Aside type="caution" title="Fallo de compilación en Windows">
<Drawer trigger="Si estás en Windows y encuentras un error de compilación, haz clic aquí." title="Fallo de compilación en Windows">
