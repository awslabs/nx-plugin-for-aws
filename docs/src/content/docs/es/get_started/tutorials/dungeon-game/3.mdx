...
<Tabs>
  <TabItem label="Local">
  El generador de agentes proporciona una CLI de chat interactiva de terminal (`agent-chat`) que utiliza un cliente TypeScript con seguridad de tipos generado a partir de la especificación OpenAPI del agente. Debido a que cambiamos la forma de entrada del agente para aceptar `playerName`, `genre` y `actions`, necesitamos actualizar el script de chat generado para proporcionar esos campos.

  Actualiza `packages/story/scripts/agent/chat.ts`:
  ...
  Luego inicia un chat interactivo con tu agente Story ejecutándose localmente:
    ...
    El objetivo `agent-chat` inicia automáticamente `agent-serve-local` (que inicia el agente y cualquier dependencia de servidor MCP con `SERVE_LOCAL=true`) y ejecuta la generación de especificación OpenAPI + cliente antes de llevarte al chat.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat` depende de `agent-serve-local`, que inicia el agente con `SERVE_LOCAL=true` y también inicia cualquier dependencia de servidor MCP localmente. Si prefieres que el agente se conecte a tu servidor MCP **desplegado** en su lugar (a través de `RUNTIME_CONFIG_APP_ID`), ejecuta el objetivo `agent-serve` en una terminal separada e invoca el agente manualmente a través de `curl` contra su endpoint `/invocations`.
    :::
  </TabItem>
