...
<Tabs>
  <TabItem label="Local">
  Inicia tu servidor de Agente local ejecutando el siguiente comando:
    ...
    El objetivo `agent-serve` inicia el agente localmente pero se conecta a tu servidor MCP **desplegado** (a través de `RUNTIME_CONFIG_APP_ID`). Si prefieres ejecutar el servidor MCP localmente también, usa `agent-serve-local` en su lugar, que inicia todos los servicios dependientes y establece `SERVE_LOCAL=true` para conexiones HTTP directas.
    :::

    Una vez que el servidor del Agente esté en funcionamiento (no verás ninguna salida), llámalo ejecutando el siguiente comando:
