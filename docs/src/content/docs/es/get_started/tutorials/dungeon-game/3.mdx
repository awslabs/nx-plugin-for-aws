...
## Tarea 2: Prueba tu Agente localmente
...
`agent-chat` tiene `dependsOn: ['agent-serve-local']`, por lo que no es necesario iniciar el servidor del agente por separado — Nx generará `agent-serve-local` (que a su vez inicia el servidor Inventory MCP, y a través de él DynamoDB Local), luego lanzará el REPL AG-UI de [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) contra `http://localhost:8081/invocations`. Todo el stack del agente se ejecuta localmente sin llamadas a AWS.
...
¡Felicidades! Has construido y probado tu primer Agente de Strands localmente. 🎉🎉🎉