...
- **Cualquier otro lenguaje** — agrega un `project.json` a cada proyecto y define sus objetivos para ejecutar tus comandos de construcción existentes, para que `nx build <project>` (y `nx run-many`) los impulsen. Consulta la <Link path="guides/workspace">guía del espacio de trabajo</Link> para ver cómo se configuran `project.json` y los objetivos.
...
- package.json scripts de conveniencia para tareas comunes, además de las dependencias de desarrollo `@aws/nx-plugin`, `nx`, `@nx/js`, `@nx/workspace`, `typescript` y Biome
- biome.json configuración predeterminada del formateador y linter de Biome
- .mcp.json configura el servidor MCP para agentes de codificación compatibles a menos que esté deshabilitado (también equivalentes .cursor/, .kiro/, .gemini/, .vscode/ y .codex/). Ejecuta el bin `aws-nx-mcp` del propio `@aws/nx-plugin` del espacio de trabajo, por lo que el servidor siempre coincide con la versión del plugin instalado
</FileTree>
...
La primera ejecución de `ts#project` configura el plugin `@nx/js/typescript` para inferir el objetivo de construcción TypeScript como `compile` (el plugin reserva `build` para orquestar `lint`, `compile` y `test`). Las bibliotecas existentes que dependen del objetivo `build` inferido también se infieren como `compile` — ejecuta `nx run <project>:compile`, o usa `nx run-many --target build,compile`. Las bibliotecas con un objetivo `build` explícito en su `project.json` no se ven afectadas.
