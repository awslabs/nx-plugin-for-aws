...
Se te pedirá tu proveedor de infraestructura como código (IaC) preferido, ya sea [CDK](https://docs.aws.amazon.com/cdk/) o [Terraform](https://developer.hashicorp.com/terraform). Puedes omitir la solicitud ejecutando el comando con `--iac`, por ejemplo:
...
El <Link path="get_started/building-with-ai">Servidor MCP de Nx Plugin para AWS</Link> ya está configurado en tu espacio de trabajo, por lo que puedes pedirle a tu agente de codificación de IA favorito como Kiro o Claude que construya estos proyectos por ti en lugar de escribir los comandos CLI tú mismo.
:::
...
El bootstrapping de CDK es necesario tanto para tu región de AWS de despliegue objetivo como para `us-east-1` para permitir el despliegue del AWS WAF WebACL para el sitio web.
:::
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap infra']} />
...
El bootstrapping de Terraform crea un bucket S3 para almacenar tus archivos de estado de Terraform. Esto solo necesita hacerse una vez por combinación de cuenta/región de AWS.
:::
</Fragment>
</Infrastructure>
