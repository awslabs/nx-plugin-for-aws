...
## Paso 5: Crear un Usuario

Tu sitio web está protegido por Amazon Cognito, y el auto-registro está deshabilitado por defecto, así que crea tu primer usuario en el grupo de usuarios que desplegaste. Puedes encontrar el id del grupo de usuarios en las salidas de tu despliegue, o en la [consola de Amazon Cognito](https://console.aws.amazon.com/cognito/):
...
Omite `--message-action SUPPRESS` para que Cognito envíe por correo electrónico la contraseña temporal a la dirección que proporcionaste, en lugar de compartirla con el usuario tú mismo. También puedes crear usuarios desde la pestaña **Users** de tu grupo de usuarios en la consola de Cognito.
...
:::tip[Auto-Registro]
Ejecuta el generador <Link path="guides/react-website-auth">`ts#website#auth`</Link> con `--allowSignup=true` si prefieres que los usuarios se registren ellos mismos desde la página de inicio de sesión en lugar de crear cada usuario tú mismo.
:::

## Paso 6: Probar el Sitio Web con Recursos en la Nube Desplegados
...
Tu sitio web estará disponible en `http://localhost:4200`, y apuntará a los recursos que desplegaste para la API y la autenticación. Inicia sesión con el usuario que creaste para verlo en acción.

Tu sitio web también se sirve desde CloudFront — abre el nombre de dominio de su distribución para usar la versión que se ejecuta completamente en AWS.
