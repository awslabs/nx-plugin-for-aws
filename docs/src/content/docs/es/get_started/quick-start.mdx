...
### Eliminar los Recursos de Bootstrap

Los recursos que inicializaste son compartidos por cada proyecto que se despliega en esa cuenta y región de AWS, y cuestan casi nada dejarlos en su lugar. Elimínalos solo si nada más se despliega allí.
...
El bucket de activos se retiene a propósito, así que elimínalo por separado. Está versionado, lo cual `aws s3 rb --force` no puede manejar, así que elimina primero cada versión y marcador de eliminación:
...
Cada `delete-objects` reporta un error en lugar de no hacer nada cuando no queda nada de ese tipo para eliminar, lo cual es seguro ignorar.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Tu Estado de Terraform También Se Elimina]
Esto vacía y elimina el bucket S3 que contiene tu estado de Terraform, así que ejecútalo solo una vez que nada que te importe esté aún desplegado desde ese estado.
:::
</Fragment>
</Infrastructure>
