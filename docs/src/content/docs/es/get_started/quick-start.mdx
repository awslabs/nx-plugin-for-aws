...
Esta guía te lleva a través de los conceptos básicos para instalar y usar `@aws/nx-plugin` y construir proyectos en AWS rápidamente. Prueba.
...
## Paso 1: Inicializar un nuevo espacio de trabajo de Nx

Ejecuta el siguiente comando para crear un espacio de trabajo de Nx con el gestor de paquetes de tu elección:

<CreateNxWorkspaceCommand workspace="my-project" />

:::tip
Se te pedirá que elijas tu proveedor de infraestructura como código (IaC), ya sea [CDK](https://docs.aws.amazon.com/cdk/) o [Terraform](https://developer.hashicorp.com/terraform). Puedes omitir esta pregunta ejecutando el comando con `--iacProvider`, ej:
...
Añade el proyecto de infraestructura según tu proveedor de IaC elegido.
...
<Fragment slot="terraform">
Abre `packages/infra/src/main.tf` y añade el siguiente código:
...
:::tip
Se requiere inicialización de CDK tanto para tu región objetivo de despliegue en AWS como para `us-east-1` para permitir el despliegue del WebACL de AWS WAF para el sitio web.
:::
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['run infra:bootstrap']} />
