...
<FileTree>
  - packages/\<name>/
    ...
    - package.json Declara las dependencias que importa `local-dev.ts`
    - tsconfig.json Configuración de TypeScript utilizada por el target `typecheck`
    - project.json Agrega los targets `serve`, `dev`, `build`, `lint`, `format` y `typecheck`
</FileTree>
...
Selecciona `cognito` cuando el Gateway frontaliza un agente con `auth: cognito`: el Gateway reenvía el token del llamador al runtime del agente, por lo que debe haber recibido uno. Ver <Link path="guides/connection/agentcore-gateway-agent">Gateway a Agente</Link>.
...
Las políticas son plantillas [EJS](https://ejs.co/) renderizadas en tiempo de synth/plan para que permanezcan portables entre cuentas y redespliegues de Gateway:
