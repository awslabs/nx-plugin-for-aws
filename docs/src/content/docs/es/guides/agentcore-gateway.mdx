...
El gateway local redirige rutas `/<targetName>/...` al servidor local de cada agente adjunto (conectado a través del <Link path="guides/connection/agentcore-gateway-agent">generador `agentcore-gateway#agent-connection`</Link>), coincidiendo con el enrutamiento basado en rutas del Gateway desplegado.
</OptionFilter>
