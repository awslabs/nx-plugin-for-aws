...
El atributo `id` del principal contiene el ARN IAM del llamador. Los llamadores generalmente llegan como ARNs de assumed-role de STS con un sufijo de sesión generado, así que pasa el ARN exacto del rol como una variable de plantilla y coincide el sufijo de sesión con `like`:
