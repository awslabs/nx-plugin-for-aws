...
Genera un proyecto [Amazon Bedrock AgentCore Gateway](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html). Un AgentCore Gateway es un punto de entrada administrado que agrega uno o más destinos de servidor MCP detrás de un único endpoint MCP, autentica solicitudes entrantes (IAM o Cognito), evalúa cada llamada de herramienta contra un [motor de políticas Cedar](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/policy.html), y firma el tráfico saliente a servidores MCP con IAM SigV4.
...
<FileTree>
  - packages/\<name>/
    - policies/ Archivos fuente de políticas Cedar (omitidos cuando `cedarPolicy: false`)
      - permit-all.cedar Política Cedar predeterminada que permite llamadores autenticados (llamadores IAM de la misma cuenta de AWS, o cualquier usuario de Cognito — consulta [Authentication](#authentication))
...
- Un `AgentCore::Gateway` configurado para el protocolo MCP con autenticación IAM entrante (predeterminada) o autenticación JWT de Cognito (consulta <a href="#authentication">Authentication</a>)
