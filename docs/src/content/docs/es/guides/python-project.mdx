...
Tu proyecto también tiene un objetivo `assemble`, que es el hermano solo de artefactos de `build`: produce lo que tu proyecto contribuye a un despliegue (su salida compilada, y cualquier paquete, imagen de contenedor o especificación generada que declare) sin ejecutar las compuertas de análisis, prueba o verificación de tipos.

<NxCommands commands={['assemble <project-name>']} />

Los objetivos de despliegue dependen de `assemble` en lugar de `build`, por lo que el despliegue construye solo lo que está a punto de desplegar. `build` permanece sin cambios y sigue haciendo todo, por lo que sigue siendo lo que ejecutas para verificar un proyecto, y lo que deberías ejecutar en CI.
