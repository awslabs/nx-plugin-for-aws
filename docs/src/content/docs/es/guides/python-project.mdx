...
Los <Link path="guides/typescript-infrastructure#deploying-to-aws">objetivos de despliegue</Link> dependen de `assemble` en lugar de `build`, por lo que el despliegue construye solo lo que está a punto de desplegar. `build` permanece sin cambios y aún hace todo, por lo que sigue siendo lo que ejecutas para verificar un proyecto, y lo que debes ejecutar en CI.

:::caution
Debido a que `assemble` omite las compuertas de calidad, un despliegue ya no falla por un error de análisis, una prueba unitaria fallida o un error de tipo. Ejecuta `build` (o tu pipeline de CI) para verificar tu código.
:::
