...
Tu proyecto también tiene un objetivo `package`, que es el hermano de solo artefactos de `build`: produce lo que tu proyecto contribuye a un despliegue (su salida compilada, y cualquier paquete, imagen de contenedor o especificación generada que declare) sin ejecutar las compuertas de análisis, pruebas o verificación de tipos.

<NxCommands commands={['package <project-name>']} />

Los <Link path="guides/typescript-infrastructure#deploying-to-aws">objetivos de despliegue</Link> dependen de `package` en lugar de `build`, por lo que el despliegue construye solo lo que está a punto de desplegar. `build` permanece sin cambios y aún hace todo, por lo que sigue siendo lo que ejecutas para verificar un proyecto, y lo que deberías ejecutar en CI.

:::caution
Debido a que `package` omite las compuertas de calidad, un despliegue ya no falla por un error de análisis, una prueba unitaria fallida o un error de tipo. Ejecuta `build` (o tu pipeline de CI) para verificar tu código.
:::
