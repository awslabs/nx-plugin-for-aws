...
La opción `type` selecciona si obtienes una aplicación (un proyecto desplegable, por ejemplo una función lambda o imagen de contenedor) o una biblioteca (código reutilizable, importado por otros proyectos). Por defecto es `application`, así que pasa `--type=library` para una biblioteca:
