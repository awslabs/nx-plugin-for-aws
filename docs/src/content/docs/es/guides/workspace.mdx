...
El valor predeterminado propio de Nx es 3, lo que deja núcleos inactivos en estos workspaces — cada proyecto contribuye varios targets (`lint`, `compile`, `test`, y a menudo `bundle` y `docker`), produciendo un amplio grafo de tareas con mucho trabajo independiente. Aumentar esto más allá de 8 tiende a no ayudar, ya que la ruta crítica del grafo se convierte en el límite en lugar de los núcleos disponibles.
