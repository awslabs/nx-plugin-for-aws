---
title: API Smithy a Base de Datos Relacional
description: Conectar una API Smithy a una Base de Datos Relacional
when:
  sourceType: ts#smithy-api
  targetType: ts#rdb
---
import { FileTree } from '@astrojs/starlight/components';
import Link from '@components/link.astro';
import RunGenerator from '@components/run-generator.astro';
import GeneratorParameters from '@components/generator-parameters.astro';
import NxCommands from '@components/nx-commands.astro';
import Snippet from '@components/snippet.astro';

El generador `connection` conecta una <Link path="guides/ts-smithy-api">API Smithy</Link> a un proyecto de <Link path="guides/ts-rdb">Base de Datos Relacional</Link>, inyectando un cliente Prisma en el contexto del servicio para que todas las implementaciones de operaciones puedan acceder a la base de datos.

## Requisitos Previos

Antes de usar este generador, asegúrate de tener:

1. Un proyecto de <Link path="guides/ts-smithy-api">API TypeScript Smithy</Link> (generado con `ts#api` usando `--framework=smithy`)
2. Un proyecto <Link path="guides/ts-rdb">`ts#rdb`</Link>

## Uso

### Ejecutar el Generador

<RunGenerator generator="connection" />

Selecciona tu proyecto backend de API Smithy como origen y tu proyecto de base de datos relacional como destino.

### Opciones

<GeneratorParameters generator="connection" />

## Salida del Generador

El generador modifica tres archivos existentes en tu backend de API Smithy:

<FileTree>

- packages/api/src
  - context.ts propiedad `db` agregada a `ServiceContext`
  - handler.ts Cliente Prisma creado dentro de `lambdaHandler`, pasado a `serviceHandler.handle`
  - local-server.ts Cliente Prisma creado dentro del manejador de solicitudes, pasado a `serviceHandler.handle`

</FileTree>

Además, actualiza el target `dev` de la API para iniciar la base de datos automáticamente.

## Cómo Funciona

### ServiceContext

El generador agrega una propiedad `db` tipada a `ServiceContext` en `context.ts`:

```ts title="packages/api/src/context.ts" {1,7}
import { getPrisma as getMyDb } from '@my-scope/my-db';

export interface ServiceContext {
  tracer: Tracer;
  logger: Logger;
  metrics: Metrics;
  myDb: Awaited<ReturnType<typeof getMyDb>>;
}
```

### Lambda Handler

El cliente Prisma se instancia dentro de `lambdaHandler` y se pasa a través del contexto del servicio:

```ts title="packages/api/src/handler.ts" {1,5,10}
import { getPrisma as getMyDb } from '@my-scope/my-db';

export const lambdaHandler = async (event: APIGatewayProxyEvent) => {
  const httpRequest = convertEvent(event);
  const myDb = await getMyDb();
  const httpResponse = await serviceHandler.handle(httpRequest, {
    tracer,
    logger,
    metrics,
    myDb,
  });
  return convertVersion1Response(httpResponse);
};
```

### Usar la Base de Datos en Operaciones

Accede a `db` desde el contexto en tus implementaciones de operaciones:

```ts title="packages/api/src/operations/list-users.ts" {8}
import { ListUsersOperationInput, ListUsersOperationOutput } from '../generated/ssdk/index.js';
import { ServiceContext } from '../context.js';

export const listUsers = async (
  input: ListUsersOperationInput,
  ctx: ServiceContext,
): Promise<ListUsersOperationOutput> => {
  const users = await ctx.myDb.user.findMany();
  return { users };
};
```

## Múltiples Bases de Datos

Ejecutar el generador nuevamente con un destino diferente agrega la segunda base de datos junto a la primera. Ambos clientes se agregan a `ServiceContext` y se instancian en `handler.ts`:

```ts title="packages/api/src/context.ts" {5,6}
export interface ServiceContext {
  tracer: Tracer;
  logger: Logger;
  metrics: Metrics;
  myDb: Awaited<ReturnType<typeof getMyDb>>;
  otherDb: Awaited<ReturnType<typeof getOtherDb>>;
}
```

```ts title="packages/api/src/handler.ts" {1,2,7,8}
const myDb = await getMyDb();
const otherDb = await getOtherDb();
const httpResponse = await serviceHandler.handle(httpRequest, {
  tracer,
  logger,
  metrics,
  myDb,
  otherDb,
});
```

## Infraestructura

<Snippet name="connection/rdb-api-infrastructure" parentHeading="Infraestructura" />

### Requisitos SSL al Conectarse sin RDS Proxy

<Snippet name="connection/ts-lambda-rdb-ssl-requirements" parentHeading="Requisitos SSL al Conectarse sin RDS Proxy" />

## Desarrollo Local

El generador aplica la misma inyección de cliente Prisma dentro del manejador de solicitudes en `local-server.ts`:

```ts title="packages/api/src/local-server.ts" {1,5,10}
import { getPrisma as getMyDb } from '@my-scope/my-db';

const server = createServer(async function (req, res) {
  const httpRequest = convertRequest(req);
  const myDb = await getMyDb();
  const httpResponse = await serviceHandler.handle(httpRequest, {
    tracer,
    logger,
    metrics,
    myDb,
  });
  return writeResponse(httpResponse, res);
});
```

<NxCommands commands={["dev <api-project-name>"]} />

Esto inicia tanto la API como la base de datos local. La variable de entorno `LOCAL_DEV=true` se establece automáticamente, por lo que el cliente Prisma se conecta a la base de datos Docker local en lugar de Aurora.
