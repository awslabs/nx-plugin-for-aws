...
El generador configura el target `serve-local` de tu API tRPC para que dependa del target `serve-local` de la base de datos, por lo que al ejecutar:

<NxCommands commands={["serve-local <api-project-name>"]} />

se iniciará automáticamente la base de datos local junto con tu API.
