...
3. Autenticación Cognito añadida mediante el <Link path="/guides/react-website-auth">generador `ts#react-website-auth`</Link> si conectas una API que use autenticación Cognito o IAM
...
Si estás trabajando simultáneamente en tu aplicación React y FastAPI, usa el target `serve-local` de la aplicación React, que regenerará automáticamente el cliente cuando cambie tu API, además de recargar en caliente tu sitio y el servidor FastAPI local:
