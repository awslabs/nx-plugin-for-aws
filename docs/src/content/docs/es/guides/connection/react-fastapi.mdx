...
3. Autenticación Cognito añadida mediante el <Link path="/guides/react-website-auth">generador `ts#website#auth`</Link> si conectas una API que use autenticación Cognito o IAM
...
- project.json Añade un nuevo target al build que invoca el script de generación
...
El generador también añadirá Runtime Config a la infraestructura de tu sitio web si no existe, asegurando que la URL de la API para tu FastAPI esté disponible en el sitio y configurada automáticamente por el hook `use<ApiName>.tsx`.

### Generación de Código

Durante el build, se genera un cliente tipado desde la especificación OpenAPI de tu FastAPI. Esto añadirá tres nuevos archivos a tu aplicación React:
...
El cliente tipado generado puede usarse para llamar a tu FastAPI desde tu aplicación React. Se recomienda usar los hooks de TanStack Query, pero también puedes usar el cliente vanilla si lo prefieres.

:::note[Regenerar código del cliente]
...
Si estás trabajando simultáneamente en tu aplicación React y FastAPI, usa el target `dev` de la aplicación React (un alias para `serve-local`), que regenerará automáticamente el cliente cuando cambie tu API, además de recargar en caliente tu sitio y el servidor FastAPI local:
...
:::warning[Dependencia del file watcher]
`watch-generate:<ApiName>-client` depende del comando [`nx watch`](https://nx.dev/docs/guides/tasks--caching/workspace-watching), que requiere que el [Nx Daemon](https://nx.dev/docs/concepts/nx-daemon) esté en ejecución. Por lo tanto, si has deshabilitado el daemon, el cliente no se regenerará automáticamente al hacer cambios en tu FastAPI.
:::
...
<Steps>
  ...
  3. Se reciben fragmentos posteriores
      ...
      - `fetchStatus` permanece `'fetching'`
      - `data` se actualiza con cada fragmento recibido
...
Si tienes una API de streaming que acepta un parámetro `cursor`, cuando uses el hook `useInfiniteQuery`, cada página esperará a que finalice el stream antes de cargarse.
:::
...
Los hooks y métodos del cliente generado se organizan automáticamente según los tags OpenAPI en tus endpoints FastAPI. Esto ayuda a mantener organizadas las llamadas a la API y facilita encontrar operaciones relacionadas.
...
Esta agrupación facilita organizar tus llamadas a la API y provee mejor autocompletado en tu IDE.
...
function ItemList() {
  ...
  const deleteMutation = useMutation({
    ...
    onMutate: async (itemId) => {
      ...
      // Toma una instantánea del valor previo
...
function ItemList() {
  ...
  const handleDelete = async (itemId) => {
    // Elimina optimistamente el ítem
    ...
    try {
      ...
      // Restaura los ítems previos en caso de error
...
Los tipos se generan automáticamente desde el esquema OpenAPI de tu FastAPI, asegurando que cualquier cambio en tu API se refleje en tu código frontend después de un build.

## Autenticación Personalizada

Si tu FastAPI usa autenticación `Custom` (Lambda Authorizer), necesitarás editar el proveedor del cliente generado para añadir los encabezados de autorización que tu autorizador espera. Busca la configuración `fetch` en el archivo `<ApiName>Provider.tsx` generado y añade tu token o clave API a los encabezados de la petición.
