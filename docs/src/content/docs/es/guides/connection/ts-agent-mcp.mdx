...
1. Un proyecto TypeScript con un componente <Link path="guides/ts-agent">Agent</Link>
...
- packages/common/agent-connection
  - src
    - app
      - \<mcp-server-name>-client-strands.ts Cliente Strands de alto nivel para el servidor MCP conectado
    - core
      - agentcore-endpoints.ts Resolución de ARN/URL agnóstica del framework
      - agentcore-fetch.ts Fetch agnóstico del framework con SigV4 / JWT / reenvío de sesión
      - agentcore-mcp-transport.ts Transporte MCP agnóstico del framework
      - agentcore-mcp-client-strands.ts Cliente MCP Strands que envuelve el transporte
...
- Actualiza el objetivo `dev` del agente para que dependa del objetivo serve del servidor MCP
