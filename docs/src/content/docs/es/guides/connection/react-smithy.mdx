...
3. Auth con Cognito añadido mediante el <Link path="/guides/react-website-auth">generador `ts#website#auth`</Link> si conectas una API que use autenticación Cognito o IAM
...
Si estás trabajando activamente en tu aplicación React y API Smithy simultáneamente, usa el target `dev` de la aplicación React (un alias para `serve-local`) que regenerará automáticamente el cliente cuando cambie la API, además de recargar en caliente el website y el servidor local de Smithy:
...
Si tu API Smithy usa autenticación `Custom` (Lambda Authorizer), necesitarás editar el proveedor del cliente generado para añadir los encabezados de autorización que tu autorizador espera. Busca la configuración `fetch` en el archivo generado `<ApiName>Provider.tsx` y añade tu token o clave API a los encabezados de la solicitud.
