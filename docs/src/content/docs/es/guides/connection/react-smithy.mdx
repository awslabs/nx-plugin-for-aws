...
Si tu API Smithy usa autenticación `Custom` (Lambda Authorizer), necesitarás editar el proveedor del cliente generado para añadir los encabezados de autorización que tu autorizador espera. Busca la configuración `fetch` en el `<ApiName>Provider.tsx` generado y añade tu token o clave API a los encabezados de la petición.
