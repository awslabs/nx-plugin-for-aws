...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_transport.py Transporte MCP Gateway independiente del framework
      - agentcore\_gateway\_mcp\_client\_\<framework>.py Cliente MCP Gateway para el framework de tu agente
      - auth/ `httpx.Auth` SigV4 / reenvío de sesión independiente del framework
    - app/
      - \<gateway\_snake>\_client\_\<framework>.py Envoltorio de cliente por Gateway
...
- **Modo desplegado** (`SERVE_LOCAL` no establecido): herramientas apuntando al endpoint MCP del Gateway, firmado con SigV4.
- **Modo local** (`SERVE_LOCAL=true`): herramientas HTTP simples apuntando al gateway local iniciado por el objetivo `serve-local` del proyecto Gateway.
