...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py Cliente MCP Strands para el Gateway desplegado
      - auth/ `httpx.Auth` SigV4 / reenvío de sesión independiente del framework
    - app/
      - \<gateway\_snake>\_client\_strands.py Envoltorio de cliente Strands por Gateway
...
`MyGatewayClientStrands.create()` devuelve un único cliente administrable por contexto cuyo `list_tools_sync()` produce cada herramienta disponible a través del Gateway:
