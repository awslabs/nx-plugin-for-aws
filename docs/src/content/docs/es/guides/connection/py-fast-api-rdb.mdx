...
La aplicación AppConfig debe exponer el espacio de nombres `database` para que se despliegue la entrada de configuración en tiempo de ejecución del módulo de base de datos. Inclúyelo en los `namespaces` al instanciar el módulo `runtime-config/appconfig`:
...
Al invocar la API directamente, ten en cuenta que la salida `api_endpoint` es la URL base de API Gateway sin la ruta del stage — agrega `/prod` (o usa la salida `stage_invoke_url`) al llamar a la API desplegada.
