...
1. Un proyecto FastAPI (generado con `py#api --framework=fastapi`), consulta la guía de <Link path="guides/fastapi">`py#api`</Link>
...
- packages/my_api
  - project.json Agrega una dependencia desde `dev` al objetivo `dev` de la base de datos
  - pyproject.toml Agrega el paquete de la base de datos como una dependencia del workspace
  - my_api
    - dependencies
...
`.scalars()` desenvuelve las filas de resultados en instancias de `ExampleModel`, que FastAPI puede serializar.
