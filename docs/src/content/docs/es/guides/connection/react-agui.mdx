...
| ------------ | ------------------------------------------------------------------------ |
| `Cloudscape` | Los mensajes del asistente y del usuario se renderizan dentro de componentes `ChatBubble` de Cloudscape con `Avatar`s gen-AI, coincidiendo con el [patrón de chat de IA generativa de Cloudscape](https://cloudscape.design/patterns/genai/generative-AI-chat/). La barra de herramientas de mensajes se convierte en un `ButtonGroup` de Cloudscape en el slot `actions` de la burbuja (copiar, pulgares arriba/abajo, regenerar), el Markdown se renderiza a través del renderizador predeterminado de CopilotKit dentro de la burbuja, el indicador de escritura se convierte en un `LoadingBar` gen-AI, y el input se reemplaza con `PromptInput` de Cloudscape. Construido con `@cloudscape-design/components` y `@cloudscape-design/chat-components`. |
| `Shadcn` | Los mensajes del asistente se renderizan con un avatar `Sparkles` y una burbuja estilo tarjeta `bg-muted` atenuada usando el estilizado `prose` de Tailwind; una barra de herramientas de `Button` revelada al pasar el cursor proporciona copiar/pulgares/regenerar. Los mensajes del usuario se renderizan alineados a la derecha en una burbuja `bg-primary` con un avatar `User`. El input se reemplaza con un `Textarea` redondeado + `Button` de icono en forma de píldora (Enter envía, Shift+Enter inserta un salto de línea; crece automáticamente hasta 8 filas), y el indicador de escritura es una animación de pulso de tres puntos. Usa primitivos de shadcn (`Avatar`, `Button`, `Textarea`) del paquete compartido `common-shadcn`. |
| `None` (o cualquier otro) | No se aplica tema — el wrapper simplemente re-exporta los componentes predeterminados de CopilotKit. |
