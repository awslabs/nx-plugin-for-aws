...
AG-UI aún no es compatible con TypeScript Agents. Consulta [agents/sdk-typescript#347](https://github.com/agents/sdk-typescript/issues/347) para conocer el estado más reciente. Por ahora, usa el <Link path="guides/py-agent">generador `py#agent`</Link> con `protocol=AG-UI`.
:::
...
2. Un Python Agent con `protocol=AG-UI` (generado usando el <Link path="guides/py-agent">generador `py#agent`</Link>)
