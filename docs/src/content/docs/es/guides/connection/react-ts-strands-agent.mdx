...
Si tu agente usa autenticación IAM, el rol autenticado del Cognito Identity Pool debe tener permiso para invocar el agente. Puedes usar el método `grantInvokeAccess` que otorga tanto los permisos `InvokeAgentRuntime` como `InvokeAgentRuntimeWithWebSocketStream`:
