...
El generador inspecciona el `computeType` del backend y produce el cliente correspondiente:

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Un enlace HTTP con `httpSubscriptionLink` sobre Server-Sent Events para suscripciones tRPC.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
Un enlace HTTP simple. Las suscripciones no son compatibles porque las APIs HTTP de API Gateway no soportan streaming de respuestas.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Un enlace WebSocket contra la URL de runtime de AgentCore, con handshake firmado con SigV4 (IAM) o subprotocolo JWT codificado en base64url (Cognito). El ARN de runtime se lee desde `runtimeConfig.apis.<ApiName>` y se convierte a una URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` sobre la marcha.
</TabItem>
</Tabs>
...
Adicionalmente, instala las dependencias requeridas:
  ...
  - `event-source-polyfill` (si se usa un backend de REST API, para soporte de suscripciones)
  - `react-oidc-context` (si se usa autenticación Cognito)
...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayHttpApi' }} description="Los backends de HTTP API no soportan suscripciones">
:::caution[Sin suscripciones en HTTP API]
Las APIs HTTP de API Gateway no soportan streaming de respuestas. Selecciona `ServerlessApiGatewayRestApi` o `BedrockAgentCoreRuntimeWebSocket` como el `computeType` de tu backend si necesitas suscripciones.
:::
</OptionFilter>

<OptionFilter when={{ computeType: ['ServerlessApiGatewayRestApi', 'BedrockAgentCoreRuntimeWebSocket'] }} description="Cómo las suscripciones llegan al cliente — el transporte difiere según el tipo de cómputo del backend">
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
El cliente generado usa un `splitLink` que enruta las operaciones de suscripción a través de `httpSubscriptionLink` (Server-Sent Events) y las consultas/mutaciones regulares a través de `httpLink`. Las suscripciones funcionan de inmediato sin configuración adicional.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
El cliente generado usa `wsLink` contra la URL de runtime de AgentCore — todas las operaciones tRPC (consultas, mutaciones y suscripciones) viajan sobre una única conexión WebSocket. El handshake se autentica automáticamente: firmado con SigV4 para autenticación `IAM`, o portando el JWT como un subprotocolo codificado en base64url para `Cognito`.
</TabItem>
</Tabs>
