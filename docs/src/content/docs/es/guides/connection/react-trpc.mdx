...
- **`ServerlessApiGatewayRestApi`** — enlace HTTP con `httpSubscriptionLink` sobre Server-Sent Events para suscripciones tRPC.
- **`ServerlessApiGatewayHttpApi`** — enlace HTTP simple (sin suscripciones).
- **`BedrockAgentCoreRuntimeWebSocket`** — enlace WebSocket contra la URL del runtime de AgentCore, con handshake firmado con SigV4 (IAM) o subprotocolo JWT codificado en base64url (Cognito). El ARN del runtime se lee desde `runtimeConfig.apis.<ApiName>` y se convierte a una URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` sobre la marcha.
