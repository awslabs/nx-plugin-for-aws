...
4. Cognito Auth agregado mediante el <Link path="/guides/react-website-auth">generador `ts#website#auth`</Link> si conectas una API que use autenticación Cognito o IAM
...
Las suscripciones solo son compatibles cuando la API tRPC usa `rest-lambda` (REST API) como tipo de cómputo. Las APIs HTTP de API Gateway no soportan streaming de respuestas.
:::
...
Si tu API tRPC usa autenticación `Custom` (Lambda Authorizer), el proveedor de cliente generado incluye `headers` de marcador de posición donde debes agregar los encabezados de autorización que tu autorizador espera. Busca los comentarios `// TODO: Add headers required by your custom authorizer` en el archivo generado `<ApiName>ClientProvider.tsx` y reemplázalos con tu lógica de token o clave de API.
