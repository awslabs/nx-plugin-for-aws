...
1. Un proyecto Python con un componente <Link path="guides/py-agent">Agent</Link>
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_strands.py Cliente MCP Strands que envuelve el transporte
      - auth/ `httpx.Auth` agnóstico del framework para SigV4 / reenvío de sesión
    - app
      - \<mcp\_server\_name>\_client\_strands.py Cliente Strands por conexión para cada servidor MCP
...
El cliente Strands es un administrador de contexto, ingresado en un bloque `with` alrededor del agente. El parámetro `session_id` se pasa desde el llamador, asegurando consistencia para [Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
</TabItem>
