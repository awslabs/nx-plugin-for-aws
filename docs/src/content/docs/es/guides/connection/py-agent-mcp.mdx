...
El generador `connection` puede conectar tu <Link path="guides/py-agent">Python Agent</Link> a un servidor MCP (ya sea <Link path="guides/ts-mcp-server">TypeScript</Link> o <Link path="guides/py-mcp-server">Python</Link>).
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      - agentcore\_endpoints.py Resolución de ARN/URL agnóstica del framework
      - agentcore\_mcp\_transport.py Transporte MCP agnóstico del framework
      - agentcore\_mcp\_client\_\<framework>.py Cliente MCP que envuelve el transporte para el framework de tu agente
      - auth/ `httpx.Auth` agnóstico del framework para SigV4 / reenvío de sesión
    - app
...
- Actualiza el target `dev` del agente para que dependa del target serve del servidor MCP
...
El cliente LangChain devuelve una lista de herramientas cargadas a través de [`langchain-mcp-adapters`](https://docs.langchain.com/oss/python/langchain/mcp). Cada herramienta abre una nueva sesión MCP por llamada, por lo que las herramientas permanecen utilizables durante la vida útil del agente — no se necesita un bloque `with`.
</TabItem>
</Tabs>
