...
Importa `session_context` desde tu paquete de base de datos y úsalo dentro de las herramientas de tu servidor MCP:
...
`my_scope_my_db` es el nombre del módulo de tu proyecto de base de datos — un único módulo plano llamado `<scope>_<name>`, visible como el directorio del paquete dentro del proyecto.

`.scalars()` desenvuelve las filas de resultado en instancias de `ExampleModel`, por lo que `model_dump()` está disponible en cada elemento.
...
Ejecuta el servidor MCP dentro de la misma VPC que la base de datos, otórgale `rds-db:connect` a través de `additional_iam_policy_statements`, y abre la ruta de red con un par de reglas de grupo de seguridad. Los recursos `aws_vpc.main` y `aws_subnet` están definidos en la guía de implementación de la base de datos:
...
Cuando el servidor MCP se conecta directamente al clúster Aurora (sin RDS Proxy), el generador de conexión actualiza el Dockerfile del servidor MCP generado para instalar el paquete de CA de Amazon RDS en el almacén de confianza del sistema:
...
Cuando uses RDS Proxy, no necesitas configurar el paquete de CA de RDS en el tiempo de ejecución del servidor MCP.
