...
El generador `connection` conecta un <Link path="guides/ts-agent">TypeScript Strands Agent</Link> a un proyecto de <Link path="guides/ts-rdb">Base de Datos Relacional</Link>, haciendo que un cliente Prisma esté disponible dentro de tu fábrica de agentes.
