...
El generador `connection` conecta un <Link path="guides/ts-agent">TypeScript Agent</Link> a un proyecto de <Link path="guides/ts-rdb">Base de Datos Relacional</Link>, haciendo que un cliente Prisma esté disponible dentro de tu factory de agente.
...
Además, el target `<agent-name>-dev` del agente se actualiza para depender del target `dev` de la base de datos.
...
El construct de agente generado implementa `IGrantable` e `IConnectable`, por lo que puedes otorgar acceso de red e IAM a la base de datos directamente en el construct.
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="Requisitos de SSL al Conectarse sin RDS Proxy" />
