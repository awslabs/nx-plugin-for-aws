...
Cuando `protocol` se establece en `AG-UI`, el punto de entrada usa la integración [ag-ui-strands](https://docs.ag-ui.com/), que expone tu agente mediante el protocolo AG-UI para integración directa con el frontend usando [CopilotKit](https://docs.copilotkit.ai/aws-strands):
...
El generador configura un target llamado `<your-agent-name>-invoke` que inicia un chat interactivo con tu agente desde la terminal. Se distribuye como un script TypeScript (mediante `tsx`) y selecciona automáticamente el protocolo correcto (HTTP, A2A, AG-UI) para tu agente. Por defecto se conecta al servidor local:

<NxCommands commands={['agent-invoke your-project']} />

Para chatear con un agente desplegado en su lugar, establece `RUNTIME_CONFIG_APP_ID` al ID de aplicación AppConfig de tu workspace — el CLI resuelve el ARN del runtime e invoca el endpoint desplegado con solicitudes firmadas con SigV4:
...
El script CLI se encuentra en `scripts/<agent>/cli.ts` en tu proyecto.
