...
- **Listo para producción**: Observabilidad completa, tracing y opciones de despliegue para escalar
- **Agnóstico a modelos y proveedores**: Soporta múltiples modelos de distintos proveedores
- **Herramientas comunitarias**: Potente conjunto de herramientas contribuidas por la comunidad
...
# Añade herramientas a tu agente
agent = Agent(
...
Para consumir servidores MCP creados con los generadores <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> o <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> (u otros alojados en Bedrock AgentCore Runtime), se genera una fábrica de clientes en `agentcore_mcp_client.py`.
...
El constructo CDK se nombra según el `name` que elegiste al ejecutar el generador, o `<ProjectName>Agent` por defecto.
...
:::note
Al desplegar en Bedrock AgentCore Runtime, debes asegurarte de que los modelos base apropiados estén habilitados en tu cuenta de AWS. Por defecto, los agentes Strands usan Claude 4 Sonnet, pero esto se puede personalizar como se describió anteriormente.
...
:::tip
La imagen docker se construye usando un tag (por ejemplo `my-scope-my-project-agent:latest`), que el `Dockerfile` del constructo CDK hereda, permitiendo que tu `Dockerfile` esté ubicado junto a tu proyecto Strands Agent.
:::
...
Puedes encontrar trazas en la consola AWS de CloudWatch, seleccionando "GenAI Observability" en el menú. Nota que para que las trazas aparezcan, debes habilitar [Transaction Search](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Transaction-Search.html).
