...
El generador proporciona una opción `auth` para configurar la autenticación de tu Agente Strands. Puedes elegir entre autenticación `IAM` (por defecto) o `Cognito` al generar tu agente.

#### IAM

Por defecto, tu Agente Strands estará protegido usando autenticación IAM, simplemente despliégalo sin argumentos:
...
Para otorgar acceso para invocar tu agente, necesitarás añadir una política como la siguiente, referenciando la salida `module.my_project_agent.agent_core_runtime_arn`:
...
<Fragment slot="terraform">
El módulo generado acepta variables `user_pool_id` y `user_pool_client_ids` para autenticación Cognito:
