...
El generador también proporciona una CLI de chat interactiva mínima en `scripts/<your-agent-name>/chat.ts` y un target correspondiente `<your-agent-name>-chat`. Este target depende de `<your-agent-name>-serve-local`, por lo que al ejecutarlo se inicia el agente localmente y te lleva a un chat interactivo en la terminal con él:

<NxCommands commands={['run your-project:agent-chat']} />

La CLI es un script TypeScript (ejecutado mediante `tsx`) que usa el cliente apropiado para el protocolo para transmitir respuestas token por token mediante [`@clack/prompts`](https://github.com/bombshell-dev/clack):
