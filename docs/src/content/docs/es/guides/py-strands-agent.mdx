...
El generador configura un target Nx `<your-agent-name>-chat` que depende de `<your-agent-name>-serve-local`. Al ejecutarlo se inicia el agente localmente y te lleva a un chat interactivo en la terminal impulsado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Para agentes **A2A** y **AG-UI**, el target de chat simplemente invoca `agent-chat-cli` como un binario contra el agente ejecutándose localmente — no se genera código en tu espacio de trabajo.

Para agentes **HTTP**, el generador emite:
...
- Un target Nx `<your-agent-name>-generate-client` que ejecuta `open-api#ts-client` para producir un cliente TypeScript con tipos bajo `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — un adaptador delgado que importa el cliente generado y lo conecta al `chatLoop` de `agent-chat-cli`

El script de chat es un pequeño envoltorio alrededor del cliente generado. Cuando personalizas la forma de entrada del agente (por ejemplo, añades nuevos campos a `InvokeInput`), actualiza el `sendMessage` del adaptador para pasar los nuevos campos y el resto funciona automáticamente.
