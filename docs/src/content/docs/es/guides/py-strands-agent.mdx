...
Para respuestas de streaming, el generador proporciona `JsonStreamingResponse` que serializa automáticamente modelos Pydantic a formato JSON Lines (`application/jsonl`). Este formato es compatible con la especificación de streaming de OpenAPI 3.2 y funciona perfectamente con el cliente TypeScript generado.
...
:::note
Ten en cuenta que como FastAPI actualmente no soporta generar una especificación OpenAPI para respuestas de streaming, debemos establecer explícitamente el modelo de respuesta para cada operación de streaming, es decir:
...
Existe una [solicitud de funcionalidad abierta para soporte nativo en FastAPI](https://github.com/fastapi/fastapi/discussions/14362).
