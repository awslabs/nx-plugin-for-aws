...
Cuando `protocol` se establece en `A2A`, el punto de entrada usa el [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) en lugar de FastAPI:
...
El protocolo de servidor de tu agente determina cómo se comunica. Ambas opciones son servidas por [FastAPI](https://fastapi.tiangolo.com/) — el punto de entrada difiere:
...
Ambos protocolos exponen `/ping` para el contrato de verificación de salud del runtime de AgentCore. Los agentes A2A escuchan en el puerto `9000` (vs `8080` para HTTP); el Dockerfile y la infraestructura generados están configurados para ti.
...
### Invocando un Agente A2A

El `A2AAgent` del SDK de Strands envuelve un endpoint A2A remoto para que puedas llamarlo como un `Agent` local.
...
#### Invocando en AgentCore con Autenticación IAM

Las solicitudes a un agente A2A desplegado en AgentCore (incluyendo la obtención de la tarjeta del agente) deben estar firmadas con SigV4. Pasa un `client_config` a `A2AAgent` con un `httpx.AsyncClient` que use un flujo de autenticación SigV4:
...
:::note[Permisos IAM]
El rol IAM del llamador necesita tanto `bedrock-agentcore:InvokeAgentRuntime` como `bedrock-agentcore:GetAgentCard` en el ARN del runtime objetivo. El helper `grantInvokeAccess` del constructo generado otorga ambos.
:::

:::note[Dependencia adicional]
El generador A2A añade `strands-agents[a2a]` por ti. Si estás añadiendo el código del cliente A2A a un agente HTTP, añade `strands-agents[a2a]` y `httpx` a tu `pyproject.toml`.
:::

#### Usando un Agente A2A como Herramienta

`A2AAgent` hereda de `AgentBase` como un `Agent` local, así que puedes envolverlo en un `@tool` y pasarlo a otro agente:

```python
from strands import Agent, tool

remote = # A2AAgent construido con client_config como arriba

async def ask_remote_agent(prompt: str) -> str:
    """Delega una pregunta al agente A2A remoto y devuelve su respuesta."""
    result = await remote.invoke_async(prompt)
    return str(result)

ask_remote = tool(
    name="ask_remote_agent",
    description="Delega una pregunta al agente A2A remoto y devuelve su respuesta.",
)(ask_remote_agent)

agent = Agent(
    system_prompt="Delega a ask_remote_agent cuando sea apropiado.",
    tools=[ask_remote],
)
```
