...
Puedes definir un modelo Pydantic para producir datos estructurados en su lugar. Para hacer esto, necesitarás serializar los modelos pydantic producidos por `handle_invoke`:
...
:::caution
Ten en cuenta que el servidor generado (y el método `serialize_stream` anterior) actualmente no se ajusta al estándar Server-Sent Events (SSE), de modo que si se desea el Agente puede ser invocado por nuestro cliente TypeScript generado desde un frontend, el cual actualmente no espera eventos de streaming en formato SSE. Por favor dale +1 a [este issue de GitHub](https://github.com/awslabs/nx-plugin-for-aws/issues/369) si esto te afecta para ayudarnos a priorizar su resolución.
:::
