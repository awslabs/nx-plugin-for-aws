...
Para respuestas de streaming, la anotación de tipo de retorno en tu endpoint corresponde al tipo de cada valor producido por tu función generadora. Por defecto, el agente produce strings que contienen el texto de respuesta del agente mientras se transmite desde Strands:
...
Puedes definir un modelo Pydantic para producir datos estructurados en su lugar:
...
<Infrastructure>
<Fragment slot="cdk">
Se genera un constructo CDK para tu agente, nombrado según el parámetro `name` usado al generar, o `<ProjectName>Agent` por defecto.
...
:::note
Al desplegar en Bedrock AgentCore Runtime, asegúrate de habilitar los modelos necesarios en tu cuenta AWS. Por defecto Strands usa Claude 4 Sonnet, pero esto es configurable.

Para habilitar modelos en Amazon Bedrock, sigue [estas instrucciones](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html).
:::

:::caution
Se requiere [Docker](https://www.docker.com/) para construir y desplegar tu agente. Asegúrate de tener Docker instalado para evitar errores como:
