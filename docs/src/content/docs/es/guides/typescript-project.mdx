...
**Las herramientas de compilación y prueba** (por ejemplo `vitest`, `tsx`, `rolldown`, la cadena de herramientas de Nx (`nx`, `@nx/js`, ...), `typescript`, `@biomejs/biome`) se comparten en todo el workspace, por lo que residen en las `devDependencies` del `package.json` **raíz** en lugar de en cada proyecto. Estas herramientas solo se referencian desde archivos de configuración, scripts de compilación y pruebas, que la regla de lint exime.
...
npm (y yarn classic) no tiene función de catálogo, por lo que las versiones se declaran directamente en cada `package.json`. Para mantenerlas consistentes entre proyectos, recomendamos [syncpack](https://github.com/JamieMason/syncpack) (`npx syncpack lint`).
:::

:::note[bun]
En bun, algunos paquetes cuyas versiones los generadores de Nx leen del manifiesto raíz (`vite`, `react`, `react-dom`) mantienen rangos de versión directos en lugar de referencias `catalog:` — Nx aún no resuelve catálogos de bun, y una referencia `catalog:` rompería esos generadores.
:::
...
Con los catálogos desactivados, los generadores escriben las versiones de dependencias directamente en el `package.json` de cada proyecto, y mantener las versiones alineadas entre proyectos es tu responsabilidad (por ejemplo usando [syncpack](https://github.com/JamieMason/syncpack)). Cuando los catálogos están habilitados en pnpm, el workspace también establece [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) para que `pnpm add` registre nuevas dependencias en el catálogo, manteniendo la política de versión única sin edición manual.
