...
El `cwd` es lo que permite que `rolldown` encuentre su configuración: sin él, el comando se ejecuta desde la raíz del workspace, donde `rolldown.config.ts` no existe.
...
Si estás construyendo una función AWS Lambda, consulta el generador <Link path="/es/guides/ts-lambda-function">`ts#lambda-function`</Link> ya que este configura el empaquetado por ti, además de generar infraestructura y agregar observabilidad y seguridad de tipos.
:::
...
Los <Link path="/es/guides/typescript-infrastructure#deploying-to-aws">objetivos de despliegue</Link> dependen de `assemble`, por lo que desplegar solo construye lo que está a punto de desplegar.
