...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Agrega la dependencia con [`--save-catalog`](https://pnpm.io/settings#catalogmode), para que pnpm registre la versión en el catálogo y la referencie:
    ...
    :::note
    Establecer [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) en `pnpm-workspace.yaml` hace que esto sea el valor predeterminado para cada `pnpm add`. También hace que `pnpm add some-npm-package@<version>` falle cuando el catálogo fija una versión diferente, lo que impide que `nx migrate` actualice Nx — por lo que el workspace generado lo deja sin configurar.
    :::
  </TabItem>
  <TabItem label="yarn">
