...
Tu proyecto también tiene un objetivo `package`, que es el hermano solo de artefactos de `build`: produce lo que tu proyecto contribuye a un despliegue (su salida compilada, y cualquier bundle, imagen de contenedor o especificación generada que declare) sin ejecutar las compuertas de lint, test o verificación de tipos.

<NxCommands commands={['package <project-name>']} />

Los <Link path="/es/guides/typescript-infrastructure#deploying-to-aws">objetivos de despliegue</Link> dependen de `package` en lugar de `build`, por lo que desplegar compila solo lo que está a punto de desplegar. `build` no cambia y sigue haciendo todo, por lo que sigue siendo lo que ejecutas para verificar un proyecto — y lo que deberías ejecutar en CI.

:::caution
Debido a que `package` omite las compuertas de calidad, un despliegue ya no falla en un error de lint o una prueba unitaria fallida. Ejecuta `build` (o tu pipeline de CI) para verificar tu código.
:::
