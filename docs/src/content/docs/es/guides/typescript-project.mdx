...
Tu proyecto también tiene un objetivo `assemble`, que es el hermano solo de artefactos de `build`: produce lo que tu proyecto contribuye a un despliegue (su salida compilada, y cualquier bundle, imagen de contenedor o especificación generada que declare) sin ejecutar las compuertas de lint, prueba o verificación de tipos.

<NxCommands commands={['assemble <project-name>']} />

Los <Link path="/es/guides/typescript-infrastructure#deploying-to-aws">objetivos de despliegue</Link> dependen de `assemble` en lugar de `build`, por lo que desplegar solo construye lo que está a punto de desplegar. `build` permanece sin cambios y sigue haciendo todo, por lo que sigue siendo lo que ejecutas para verificar un proyecto — y lo que deberías ejecutar en CI.

:::caution
Debido a que `assemble` omite las compuertas de calidad, un despliegue ya no falla por un error de lint o una prueba unitaria fallida. Ejecuta `build` (o tu pipeline de CI) para verificar tu código.
:::
