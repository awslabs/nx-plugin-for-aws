...
En pnpm, el workspace también establece [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) para que `pnpm add` registre nuevas dependencias en el catálogo automáticamente. yarn y bun no tienen una configuración equivalente, por lo que añade nuevas dependencias con el protocolo `catalog:` para registrarlas en el catálogo, por ejemplo `yarn workspace @my-scope/my-library add some-npm-package@catalog:` o `bun add some-npm-package@catalog: --cwd packages/my-library`.
