...
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea un Lambda por operación de Smithy. Selecciona `shared` si prefieres un único manejador Lambda compartido para toda la API, con anulaciones opcionales por operación.
:::
...
- **model/** Proyecto de modelo Smithy
...
La API de Smithy desplegada tiene la siguiente arquitectura, con una ACL Web de [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) frente a la etapa de API Gateway:
...
Las operaciones se definen en archivos Smithy dentro del proyecto de modelo. La definición principal del servicio está en `main.smithy`:
...
Puedes referenciar el registrador desde tus implementaciones de operaciones a través del contexto:
...
El rastreo de AWS X-Ray se configura automáticamente a través del middleware `captureLambdaHandler`.
...
Puedes agregar subsegmentos personalizados a tus rastros en tus operaciones:
...
Cuando tu API está protegida por autenticación, tus operaciones a menudo necesitan saber quién está llamando. El enfoque recomendado es resolver la identidad del llamador una vez en el manejador y pasarla a través del [contexto del servicio](#contexto-del-servicio) para su consumo por operaciones específicas.
...
Primero, expón la identidad resuelta en el contexto del servicio en `src/context.ts`. La proporcionamos como una función para que el `UnauthorizedError` se lance desde dentro de una operación (donde el Server SDK lo serializa a un `403`), en lugar de desde el manejador:
...
Para la autenticación `IAM`, buscamos el llamador en Cognito usando el sub extraído del evento de API Gateway. La búsqueda utiliza el cliente Cognito Identity Provider, que no es una dependencia de un backend Smithy generado, así que instálalo primero en el proyecto backend:
...
Con `auth: 'cognito'`, el autorizador de Cognito User Pools de API Gateway verifica el JWT que el llamador proporciona en el encabezado `Authorization` y coloca las reclamaciones verificadas en el evento en `event.requestContext.authorizer.claims`:
...
El proyecto de modelo Smithy utiliza la [CLI de Smithy](https://smithy.io/2.0/guides/smithy-cli/index.html) para compilar los artefactos de Smithy y generar el TypeScript Server SDK:
...
Un proyecto generado en Windows confirma un objetivo `compile` que invoca `smithy` directamente, por lo que cualquier otra persona que trabaje en él — incluso en macOS o Linux — necesita la CLI de Smithy en su `PATH` también. Para que esas máquinas resuelvan la CLI a través de `mise` en su lugar, cambia el objetivo al comando `mise` como [se describe a continuación](#elegir-cómo-se-resuelve-la-cli).

### Elegir cómo se resuelve la CLI

macOS y Linux resuelven la CLI a través de `mise` y Windows usa una CLI instalada globalmente, pero puedes elegir cualquiera en cualquier plataforma editando el comando del objetivo `compile` en el `project.json` del proyecto de modelo.
...
El servidor local no solo recargará cuando hagas cambios de TypeScript en tu código backend, sino que también recargará cuando hagas cambios en tu proyecto de modelo Smithy, permitiéndote iterar continuamente en tu modelo Smithy y servidor juntos.
:::
...
4. Grupo de registros de CloudWatch
5. Configuración de rastreo de X-Ray
...
El módulo de la API coloca su zip de despliegue Lambda en un bucket S3 de activos compartido — consulta la <Link path="/guides/terraform-project">guía de infraestructura Terraform</Link> para más detalles. Instancia el módulo `core/asset-bucket` una vez por despliegue y pasa su salida `bucket_name` a cada módulo de API / Lambda a través de la entrada `asset_bucket_name`:
...
<Snippet name="api/custom-domain" parentHeading="Dominio personalizado" />
...
:::tip[Recarga en Caliente]
Si estás trabajando activamente tanto en tu infraestructura CDK como en la API de Smithy juntas, puedes usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar estos tipos cada vez que hagas cambios en el modelo:
