...
[Smithy](https://smithy.io/) es un lenguaje de definición de interfaces agnóstico al protocolo para crear APIs mediante un enfoque basado en modelos.

El generador de API TypeScript para Smithy crea una nueva API utilizando Smithy para la definición del servicio y el [SDK de servidor Smithy TypeScript](https://github.com/awslabs/smithy-typescript) para la implementación. El generador provee infraestructura como código con CDK o Terraform para desplegar tu servicio en AWS Lambda, expuesto mediante una API REST de AWS API Gateway. Ofrece desarrollo de APIs con seguridad de tipos mediante generación automática de código a partir de modelos Smithy. El manejador generado utiliza [AWS Lambda Powertools para TypeScript](https://docs.powertools.aws.dev/lambda/typescript/latest/) para observabilidad, incluyendo registro, trazado con AWS X-Ray y métricas de CloudWatch.
...
- **model/** Proyecto del modelo Smithy
  ...
  - build.Dockerfile Configuración Docker para construir artefactos Smithy
  - src/
    - main.smithy Definición principal del servicio
    - operations/
      - echo.smithy Definición de operación de ejemplo
- **backend/** Implementación TypeScript del backend
  - project.json Configuración del proyecto y objetivos de build
  - rolldown.config.ts Configuración de empaquetado
  - src/
...
Dado que este generador crea infraestructura como código basada en tu elección de `iacProvider`, creará un proyecto en `packages/common` que incluye los constructos CDK o módulos Terraform relevantes.
...
<FileTree>
  - packages/common/constructs
    - src
      ...
      - core/ Constructos genéricos reutilizables
        - api/
...
<FileTree>
  - packages/common/terraform
    - src
      ...
      - core/ Módulos genéricos reutilizables
        - api/
          - rest-api/
...
export const Echo: EchoOperation<ServiceContext> = async (input) => {
  ...
  return {
    message: `Echo: ${input.message}` // Seguro de tipos basado en tu modelo Smithy
  };
};
```
...
export const Service: YourServiceService<ServiceContext> = {
  Echo,
  // Agrega otras operaciones aquí
};
```

### Contexto del Servicio

Puedes definir contexto compartido para tus operaciones en `context.ts`:
...
export interface ServiceContext {
  ...
  logger: Logger;
  metrics: Metrics;
  // Agrega dependencias compartidas, conexiones a bases de datos, etc.
  dbClient: any;
  userIdentity: string;
}
```

Este contexto se pasa a todas las implementaciones de operaciones y puede usarse para compartir recursos como conexiones a bases de datos, configuración o utilidades de logging.

:::caution
Debes construir el contexto manualmente tanto en `handler.ts` (punto de entrada de la función Lambda) como en `local-server.ts` (punto de entrada para ejecución local mediante el objetivo `serve`).
:::
...
El generador configura logging estructurado usando AWS Lambda Powertools con inyección automática de contexto mediante middleware Middy.
...
Puedes acceder al logger desde tus implementaciones de operaciones mediante el contexto:
...
El proyecto de modelo Smithy usa [Docker](https://www.docker.com/) para construir los artefactos Smithy y generar el SDK de servidor TypeScript:
...
2. **Genera especificación OpenAPI** a partir del modelo Smithy
3. **Crea SDK de servidor TypeScript** con interfaces de operación seguras en tipos
...
El proyecto backend usa [Rolldown](https://rolldown.rs/) para empaquetar tu manejador de servicio en un paquete de despliegue para AWS Lambda.
...
:::tip
El servidor local no solo recargará cuando hagas cambios en el código TypeScript de tu backend, sino que también se recargará cuando modifiques tu proyecto de modelo Smithy, permitiéndote iterar continuamente en tu modelo Smithy y servidor juntos.
:::

## Desplegando tu API Smithy

El generador crea infraestructura CDK o Terraform basada en tu elección de `iacProvider`.
...
module "my_api" {
  ...
  additional_iam_policy_statements = [
    # Agrega permisos adicionales que necesite tu API
  ]
...
<Infrastructure>
<Fragment slot="cdk">
Dado que las operaciones se definen en Smithy, usamos generación de código para proveer metadatos al constructo CDK para integraciones seguras en tipos.

Se agrega un objetivo `generate:<ApiName>-metadata` al `project.json` de los constructos comunes para facilitar esta generación de código, que produce un archivo como `packages/common/constructs/src/generated/my-api/metadata.gen.ts`. Como esto se genera en tiempo de build, se ignora en control de versiones.
...
:::tip
Si estás trabajando activamente en tu infraestructura CDK y API Smithy juntas, puedes usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar estos tipos cada vez que hagas cambios en el modelo:
...
<Fragment slot="terraform">
:::note
No soportamos integraciones seguras en tipos para Terraform, por lo que no se configuran objetivos de generación de código si seleccionaste Terraform como tu `iacProvider`.
:::
</Fragment>
</Infrastructure>
...
Para invocar tu API desde un sitio web React, puedes usar el generador <Link path="guides/api-connection/react-smithy">`api-connection`</Link>, que provee generación de cliente segura en tipos a partir de tu modelo Smithy.