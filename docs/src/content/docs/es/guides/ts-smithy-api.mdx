...
El CLI se resuelve mediante [mise](https://mise.jdx.dev/), añadido a tu workspace como dependencia de desarrollo, por lo que no hay nada que instalar — descarga y almacena en caché la versión fijada la primera vez que construyes.

:::caution[Windows]
`mise` no publica ningún paquete de Windows en npm, así que sigue la [guía de instalación del Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) para instalarlo tú mismo antes de construir.
:::

:::tip[Construir en CI]
`mise` lee los metadatos de lanzamiento del CLI desde la API de GitHub, cuyo límite de velocidad no autenticado se comparte por dirección IP y se agota fácilmente en un runner de CI — la descarga entonces falla con `403 Forbidden`. Establece `GITHUB_TOKEN` en tu entorno de build para eliminarlo. Las construcciones posteriores utilizan la caché local y no realizan ninguna solicitud.
:::
