...
En macOS y Linux el CLI se resuelve mediante [mise](https://mise.jdx.dev/), que la construcción obtiene bajo demanda, por lo que no hay nada que instalar — descarga y almacena en caché la versión fijada la primera vez que construyes. En Windows el CLI es un prerrequisito que instalas tú mismo, y actualizar el workspace no lo actualiza — consulta <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> y <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> en la guía del proyecto Smithy.
