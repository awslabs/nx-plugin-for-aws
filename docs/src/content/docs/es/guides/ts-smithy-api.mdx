...
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea una Lambda por operación Smithy. Selecciona `shared` si prefieres un único manejador Lambda compartido para toda la API, con opciones de sobrescritura por operación.
:::
...
- **model/** Proyecto del modelo Smithy
  ...
  - ssdk.rolldown.config.mjs Empaqueta el SDK de servidor TypeScript generado
  - src/
...
Dado que este generador crea infraestructura como código según el `iac` seleccionado, generará un proyecto en `packages/common` que incluye los constructos CDK o módulos Terraform relevantes.
...
La API Smithy desplegada tiene la siguiente arquitectura, con un Web ACL de [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) frente al stage de API Gateway:
...
### Añadiendo una Biblioteca de Formas

Si tienes varias APIs Smithy que comparten los mismos tipos de datos, puedes definir esos tipos una vez en una biblioteca de formas en lugar de duplicarlos en cada modelo. Una biblioteca de formas es un proyecto Smithy sin servicio — solo formas reutilizables — del cual cualquier número de proyectos Smithy puede depender.
...
Consulta la <Link path="guides/smithy-project">guía de proyectos Smithy</Link> para saber cómo crear una biblioteca de formas y conectarla como dependencia del modelo de tu API.
...
El proyecto del modelo Smithy usa el [Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/index.html) para construir los artefactos Smithy y generar el SDK de servidor TypeScript:

<NxCommands commands={['build <model-project>']} />

En macOS y Linux el CLI se resuelve mediante [mise](https://mise.jdx.dev/), que la construcción obtiene bajo demanda, por lo que no hay nada que instalar — descarga y almacena en caché la versión fijada la primera vez que construyes.
...
`mise` no publica ningún paquete de Windows en npm, por lo que en Windows el Smithy CLI es un prerrequisito que instalas tú mismo. Instálalo una vez siguiendo la [guía de instalación del Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) (por ejemplo `winget install smithy` o `scoop install smithy`), y asegúrate de que `smithy` esté en tu `PATH`. Un proyecto Smithy generado en Windows ejecuta `smithy` directamente en lugar de a través de `mise`.

:::caution[Actualización del CLI]
Debido a que el CLI se instala globalmente en lugar de estar fijado en tu workspace, actualizar el workspace con `nx migrate` **no** lo actualiza. Cuando una migración mueve las dependencias de Smithy hacia adelante, actualiza tu Smithy CLI global a una versión coincidente tú mismo.
:::

Alternativamente, desarrolla dentro de [WSL](https://learn.microsoft.com/en-us/windows/wsl/install), donde la construcción ejecuta la ruta de Linux y `mise` resuelve el CLI por ti — nada que instalar.

Un proyecto generado en Windows confirma un objetivo `compile` que invoca `smithy` directamente, por lo que cualquier otra persona que trabaje en él — incluyendo en macOS o Linux — necesita el Smithy CLI en su `PATH` también. Para que esas máquinas resuelvan el CLI a través de `mise` en su lugar, cambia el objetivo al comando `mise` como [se describe a continuación](#choosing-how-the-cli-is-resolved).

### Elegir cómo se resuelve el CLI

macOS y Linux resuelven el CLI a través de `mise` y Windows usa un CLI instalado globalmente, pero puedes elegir cualquiera de los dos en cualquier plataforma editando el comando del objetivo `compile` en el `project.json` del proyecto del modelo.

Para usar un Smithy CLI instalado globalmente en lugar de `mise`, reemplaza el prefijo `mise` con un `smithy` simple:
...
Para volver a que `mise` resuelva el CLI, restaura el prefijo `npx -y mise@<version> exec smithy@<version> --`.
...
El generador crea infraestructura como código con CDK o Terraform según el `iac` seleccionado.
...
No soportamos integraciones con seguridad de tipos para Terraform, por lo que no se configuran objetivos de generación de código si seleccionaste Terraform como tu `iac`.
:::
</Fragment>
</Infrastructure>
...
Para invocar tu API desde un sitio web React, puedes usar el generador <Link path="guides/connection/react-smithy">`connection`</Link>, que provee generación de cliente con seguridad de tipos a partir de tu modelo Smithy.
