...
El constructo `UserIdentity` puede generarse usando el <Link path="/es/guides/react-website-auth">generador `ts#website#auth`</Link>
:::
</OptionFilter>
...
El módulo de la API almacena su zip de despliegue de Lambda en un bucket S3 de activos compartido — consulta la <Link path="/es/guides/terraform-project">guía de infraestructura Terraform</Link> para más detalles. Instancia el módulo `core/asset-bucket` una vez por despliegue y pasa su salida `bucket_name` a cada módulo de API / Lambda a través de la entrada `asset_bucket_name`:
