...
Para invocar tu API desde un sitio web React, puedes usar el generador <Link path="guides/connection/react-smithy">`connection`</Link>, que provee generación de cliente con seguridad de tipos a partir de tu modelo Smithy.uridad de tipos a partir de tu modelo Smithy.