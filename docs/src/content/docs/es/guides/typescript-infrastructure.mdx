...
El [AWS CDK](https://docs.aws.amazon.com/cdk/v2/guide/home.html) es un marco para definir infraestructura en la nube mediante código y aprovisionarla a través de AWS CloudFormation.
...
Si has usado el generador de <Link path="guides/cloudscape-website">sitios web CloudScape</Link>, notarás que ya tienes un construct en `packages/common/constructs` para desplegarlo. Por ejemplo:
...
:::warning
Es importante asegurarse de que el sitio web se declare _después_ de cualquier construct de API para que la <Link path="guides/cloudscape-website#runtime-configuration">Configuración en Tiempo de Ejecución</Link> del sitio web incluya toda la configuración de las API.
:::

## Sintetizando tu infraestructura

Como parte de tu target `build`, además de ejecutar los <Link path="guides/typescript-project#building">targets predeterminados de compilación, lint y pruebas</Link>, tu proyecto de infraestructura se _sintetiza_ a CloudFormation. Esto también puede ejecutarse de forma independiente con el target `synth`:
...
Si es la primera vez que despliegas una aplicación CDK en una cuenta AWS, deberás hacer bootstrap primero.
...
:::tip
El comando anterior despliega _todos_ los stacks definidos en `main.ts`. Puedes querer desplegar un stack individual, especialmente si has configurado múltiples etapas de una aplicación:
