...
| `deploy`         | Desplegar cualquier stage durante el desarrollo, nombrando el stage o stacks que deseas. Usa modo express. |
...
Los targets `deploy-sandbox` y `deploy` usan el [modo express de CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) por defecto (a través de la bandera `--express` de CDK). El modo express finaliza una vez que se confirma que la configuración del recurso está aplicada en lugar de esperar la estabilización completa, con la propagación continuando en segundo plano, para ciclos de iteración significativamente más rápidos.

Para esperar la estabilización completa del recurso en su lugar, pasa `--no-express`:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox` y `deploy` son solo para desarrollo]
Debido a que el modo express retorna antes de que los recursos hayan terminado de estabilizarse, `deploy-sandbox` y `deploy` están destinados para desarrollo e iteración, no para CI/CD o despliegues de producción. Usa [`deploy-ci`](#desplegar-en-aws-en-un-pipeline-de-cicd) en tu pipeline — nunca usa el modo express, por lo que siempre espera la estabilización completa.
:::
...
Este target difiere de los targets `deploy` y `deploy-sandbox` de dos maneras. Primero, despliega un ensamblaje en la nube pre-sintetizado en lugar de sintetizar sobre la marcha, evitando el potencial no determinismo de los cambios de versión de paquetes y asegurando que cada etapa del pipeline despliegue usando el mismo ensamblaje en la nube. Segundo, no usa el modo express, por lo que espera a que cada recurso se estabilice completamente antes de reportar éxito.

:::caution[Siempre usa `deploy-ci` para producción]
`deploy` y `deploy-sandbox` usan el modo express y retornan antes de que los recursos hayan terminado de estabilizarse, lo que los hace inadecuados para CI/CD y despliegues de producción. Usa `deploy-ci` para esos.
:::
