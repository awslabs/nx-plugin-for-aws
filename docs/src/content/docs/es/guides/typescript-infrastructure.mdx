...
Si generas con `--stageConfig=true`, el generador también crea dos paquetes compartidos para la gestión centralizada de credenciales (si aún no existen):
...
El stage sandbox es el que los targets <Link path="es/guides/typescript-infrastructure#desplegando-tu-stage-sandbox">`deploy-sandbox`</Link> y <Link path="es/guides/typescript-infrastructure#desmantelando-tu-stage-sandbox">`destroy-sandbox`</Link> actúan sobre.
...
Esta sección aplica cuando generas con `stageConfig`:

<NxCommands commands={['g @aws/nx-plugin:ts#infra --name=infra --stageConfig=true']} />

Sin ella, el generador produce una configuración más simple donde gestionas las credenciales de AWS tú mismo (por ejemplo, exportando `AWS_PROFILE` antes de desplegar).
:::
...
`synth` depende de `^assemble`, por lo que sintetizar construye los artefactos que tus stacks despliegan, como los bundles de Lambda, imágenes de contenedor y especificaciones generadas, sin ejecutar las compuertas de lint, prueba y verificación de tipos de cada proyecto upstream. Consulta <Link path="guides/typescript-project#ensamblar">Ensamblar</Link> para obtener más detalles sobre `assemble`.
...
| Target           | Úsalo para                                                                                                            |
| ---------------- | --------------------------------------------------------------------------------------------------------------------- |
| `deploy-sandbox` | Desplegar tu propio stage sandbox durante el desarrollo. No se necesita argumento de stage. Usa modo express.                       |
| `deploy`         | Desplegar cualquier stage, nombrando el stage o stacks que deseas. Usa modo express a menos que generes con `stageConfig`.  |
| `deploy-ci`      | Desplegar desde un pipeline de CI/CD, usando un ensamblaje en la nube pre-sintetizado. Espera la estabilización completa.                  |

`deploy-sandbox` y `deploy` dependen de `^assemble`, por lo que construyen los artefactos que están a punto de desplegar y nada más. `deploy-ci` despliega un ensamblaje que tu pipeline ya construyó, por lo que no tiene dependencias de construcción en absoluto.

:::caution[Usa `build` en tu pipeline]
`assemble` se usa para una iteración más rápida durante el desarrollo. Ejecuta `build` en tu pipeline de CI para asegurar que tu proyecto pase las compuertas de calidad antes de un despliegue de producción.
:::
...
El target `deploy-sandbox` usa el [modo express de CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) por defecto (a través de la bandera `--express` de CDK). El modo express finaliza una vez que se confirma que la configuración del recurso está aplicada en lugar de esperar la estabilización completa, con la propagación continuando en segundo plano, para ciclos de iteración significativamente más rápidos.

Dado que retorna antes de que los recursos hayan terminado de estabilizarse, el modo express es adecuado para el stage sandbox en el que iteras, no para CI/CD o despliegues de producción — usa [`deploy-ci`](#desplegar-en-aws-en-un-pipeline-de-cicd) para esos. Para esperar la estabilización completa del recurso aquí también, pasa `--no-express`:
...
Si este target usa modo express depende de cómo generaste el proyecto:

- **Con `stageConfig`** espera la estabilización completa del recurso, ya que el target puede nombrar cualquier stage incluido el de producción. Pasa `--express` para optar por el modo express para un stage en el que estés iterando.
- **Sin `stageConfig`** usa modo express, como lo hace `deploy-sandbox`. Dado que el stage que nombres puede ser uno de producción, pasa `--no-express` para esperar la estabilización completa del recurso:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Usa [`deploy-ci`](#desplegar-en-aws-en-un-pipeline-de-cicd) desde un pipeline en su lugar — nunca usa modo express.
...
Este target difiere de los targets `deploy` y `deploy-sandbox` de dos maneras. Primero, despliega un ensamblaje en la nube pre-sintetizado en lugar de sintetizar sobre la marcha, evitando el potencial no determinismo de los cambios de versión de paquetes y asegurando que cada etapa del pipeline despliegue usando el mismo ensamblaje en la nube. Segundo, nunca usa el modo express, por lo que siempre espera a que cada recurso se estabilice completamente antes de reportar éxito.
