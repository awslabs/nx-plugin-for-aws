...
Si necesitas un despliegue estándar que espere la estabilización completa de los recursos, desactívalo con `--no-express`:

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

El objetivo `deploy-ci` no utiliza el modo express, por lo que los despliegues de pipelines CI/CD siempre esperan la estabilización completa.
:::
