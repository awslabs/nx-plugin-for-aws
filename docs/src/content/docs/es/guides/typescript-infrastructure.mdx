...
Si estableces la opción `stageConfig`, el generador también crea dos paquetes compartidos para gestión centralizada de credenciales (si aún no existen):
...
```ts title="src/main.ts (con stageConfig)"
import { resolveStage } from ':my-scope/common-infra-config';

// Busca la etapa bajo este proyecto (packages/infra), recurriendo a
// etapas compartidas. Devuelve undefined cuando no existe configuración para la etapa.
...
Esta sección aplica cuando generas con `stageConfig`. Sin ella, el generador produce una configuración más simple donde gestionas las credenciales AWS tú mismo (por ejemplo, exportando `AWS_PROFILE` antes de desplegar).
:::
...
Para más información sobre CDK, consulta la [Guía para desarrolladores de CDK](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html) y [Referencia de API](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html). y [Referencia de API](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html).