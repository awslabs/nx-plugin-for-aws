...
:::tip[Confirmar configuración de etapas]
...
Con CDK, el constructo del sitio web puede declararse en cualquier punto de tu stack — la configuración en tiempo de ejecución se resuelve de forma diferida en el momento de síntesis. Para Terraform, asegúrate de que el módulo del sitio web tenga los `depends_on` apropiados para módulos que agregan a la <Link path="guides/react-website#runtime-configuration">Configuración en tiempo de ejecución</Link>.
:::
...
El objetivo `deploy` utiliza el [modo express de CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) por defecto (mediante la bandera `--express` de CDK) para acelerar los despliegues durante el desarrollo. El modo express finaliza una vez que se confirma que la configuración de recursos se ha aplicado en lugar de esperar la estabilización completa, con la propagación continuando en segundo plano, para ciclos de iteración significativamente más rápidos.

El modo express está destinado al desarrollo y la iteración. Para desplegar sin él y esperar la estabilización completa de los recursos, elimina la bandera `--express` del comando del objetivo `deploy` en el `project.json` de tu proyecto de infraestructura.

Para despliegues reales a etapas de producción, utiliza el objetivo `deploy-ci` (ver [más abajo](#deploying-to-aws-in-a-cicd-pipeline)) como parte de un pipeline CI/CD. Este objetivo no utiliza el modo express, por lo que siempre espera la estabilización completa.
:::
