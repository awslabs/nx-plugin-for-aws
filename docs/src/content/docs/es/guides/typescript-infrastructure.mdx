...
`synth` depende de `^package` en lugar de `^build`, por lo que sintetizar construye los artefactos que tus stacks despliegan — los bundles de Lambda, imágenes de contenedor y especificaciones generadas — sin ejecutar también las compuertas de lint, test y verificación de tipos de cada proyecto upstream. Consulta <Link path="guides/typescript-project#empaquetar">Empaquetar</Link> para ver cómo `package` se relaciona con `build`.
...
`deploy-sandbox` y `deploy` dependen de `^package`, por lo que construyen los artefactos que están a punto de desplegar y nada más. `deploy-ci` despliega un ensamblaje que tu pipeline ya construyó, por lo que no tiene dependencias de construcción en absoluto.

:::caution
Debido a que los targets de despliegue omiten las compuertas de lint, test y verificación de tipos, desplegar no fallará por un error de lint o una prueba unitaria fallida. Ejecuta `build` antes de desplegar cualquier cosa que te importe, y condiciona tu pipeline a ello:
