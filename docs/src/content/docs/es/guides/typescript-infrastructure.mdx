...
| `deploy`         | Desplegar cualquier stage, nombrando el stage o stacks que deseas. Espera la estabilización completa.               |
...
Dado que este target puede nombrar cualquier stage, incluido el de producción, espera la estabilización completa del recurso. Pasa `--express` para optar por el modo express para un stage en el que estés iterando.
...
Este target despliega un ensamblaje en la nube pre-sintetizado en lugar de sintetizar sobre la marcha, evitando el potencial no determinismo de los cambios de versión de paquetes y asegurando que cada etapa del pipeline despliegue usando el mismo ensamblaje en la nube. Al igual que `deploy`, espera a que cada recurso se estabilice completamente antes de reportar éxito, y a diferencia de `deploy` no acepta ninguna exclusión de `--express`.
