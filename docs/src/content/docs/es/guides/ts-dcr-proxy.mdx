...
Pasa `identity.userPool.userPoolId`, `proxyClient.userPoolClientId` e `identity.userPoolDomain.baseUrl()` al proxy. La construcción `UserIdentity` siempre crea un dominio, por lo que no necesitas agregar uno.
</Fragment>
<Fragment slot="terraform">
```hcl
...
Pasa el id del user pool, el id de este cliente y el dominio del user pool existente al módulo del proxy.
</Fragment>
</Infrastructure>
:::
