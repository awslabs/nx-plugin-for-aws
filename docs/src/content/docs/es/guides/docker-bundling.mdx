...
:::tip[Trivy no es parte de `build` — ejecútalo en CI]
El escaneo intencionalmente **no** está conectado a `build`. El resultado de Trivy depende de su base de datos de vulnerabilidades, que se actualiza constantemente: un escaneo que pasa hoy puede fallar mañana cuando se publique un nuevo CVE contra una imagen base o dependencia que no has cambiado. Acoplarlo a `build` haría que las construcciones locales y de CI sean no deterministas y bloquearía trabajo no relacionado.

En su lugar, ejecútalo como un paso dedicado en tu pipeline de CI (y localmente antes de un lanzamiento):

<PackageManagerShortCommand commands={['trivy']} />

Cuando un escaneo comience a fallar por un CVE recién publicado, actualiza el paquete afectado o la imagen base en tu `Dockerfile`, o suprímelo en `.trivyignore` si aún no hay una solución (ver abajo).
:::
