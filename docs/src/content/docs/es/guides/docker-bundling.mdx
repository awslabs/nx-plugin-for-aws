...
Es una buena práctica escanear tus imágenes en busca de vulnerabilidades conocidas. Los generadores que siguen este patrón agregan un target `trivy` que escanea la imagen construida con [Trivy](https://trivy.dev/), ejecutándose desde la [imagen Trivy alojada en ECR](https://gallery.ecr.aws/aquasecurity/trivy), y falla la construcción en hallazgos `HIGH` o `CRITICAL`.
...
Luego haz que `build` dependa de `trivy` para que el escaneo se ejecute como parte de una construcción:
