...
Fija cada dependencia instalada dentro del `Dockerfile` a una versión exacta (como arriba) — de lo contrario `npm install` tomará silenciosamente una versión más nueva la próxima vez que construyas, produciendo imágenes que se comportan de manera diferente dependiendo de cuándo fueron construidas. El mismo consejo aplica a la imagen base: prefiere un digest o tag específico (por ejemplo `node:22.14.0`) sobre un tag flotante como `lts`.
:::
...
1. Un `aws_ecr_repository` para almacenar la imagen.
2. Un `null_resource` con un provisionador `local-exec` que autentica en ECR, re-etiqueta la imagen construida localmente y la envía.
3. El recurso downstream (por ejemplo, `aws_ecs_task_definition`) referencia `"${aws_ecr_repository.repo.repository_url}:latest"`.
