...
Si elegiste un `event` al generar tu función, esta se instrumenta con [`@event_parser` de AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/). Por ejemplo:
...
Si seleccionaste `Any` para tu `event`, el decorador `@event_parser` no se aplica y el manejador recibe un diccionario de evento sin tipar. Regenera con un `event` específico si deseas validación con Pydantic y acceso a campos tipados.
</OptionFilter>
...
2. [`uv pip install`](https://docs.astral.sh/uv/reference/cli/#uv-pip-install) para instalar dependencias para la plataforma objetivo (`x86_64-manylinux_2_28`)

:::note[Cambios de arquitectura]
Si cambias la [arquitectura Lambda a ARM](https://docs.aws.amazon.com/lambda/latest/dg/foundation-arch.html), edita el target de empaquetado a `aarch64-manylinux_2_28`.
:::
