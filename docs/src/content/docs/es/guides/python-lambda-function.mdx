...
Si seleccionaste `Any` para tu `eventSource`, el decorador `@event_parser` no se aplica y el manejador recibe un diccionario de evento sin tipar. Regenera con un `eventSource` específico si deseas validación con Pydantic y acceso a campos tipados.
</OptionFilter>
