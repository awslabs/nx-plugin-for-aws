...
- **Formatea lo que escribes.** `updateJson` y `writeJson` re-serializan un archivo completo, expandiendo arrays en línea en una forma que el objetivo `format` vendido rechaza — dejando al usuario con una compilación fallida después de una actualización por lo demás limpia. Termina con `formatFilesInSubtree(tree)` para que el diff permanezca limitado a las líneas que pretendías cambiar.
...
io que actualiza — incluyendo salida de ejemplo para migraciones deterministas y agénticas — consulta <Link path="get_started/upgrading">Actualizar Tu Workspace</Link>.
rante `nx migrate --run-migrations` cuando uno está disponible.
