...
El `migration.spec.ts` generado usa `createTreeWithEmptyWorkspace()` para construir un workspace virtual. Prueba que la migración lleva un workspace del estado "antes" al estado objetivo, omite (y reporta) archivos personalizados, y es idempotente:
...
`nx migrate` es un flujo de dos pasos — los dos comandos hacen cosas diferentes y deben ejecutarse en orden, con una instalación en el medio:

<NxCommands commands={['migrate my-plugin@latest']} />

Este primer paso solo actualiza la versión de tu plugin (y cualquier paquete del que dependa) en `package.json` y escribe un `migrations.json` listando las migraciones a ejecutar. No se instalan paquetes y no se tocan otros archivos todavía, por lo que el usuario puede revisar los cambios primero.

El usuario luego instala las nuevas versiones con su gestor de paquetes (`pnpm install`, `npm install`, `yarn`, o `bun install`), luego aplica las migraciones:

<NxCommands commands={['migrate --run-migrations']} />

Este segundo paso ejecuta cada migración pendiente para actualizar la configuración y el código fuente, una a la vez. Después, el usuario elimina `migrations.json` y hace commit.

:::note[Nx y tu plugin se actualizan independientemente]
`nx migrate <package>@<version>` apunta a cualquier paquete que nombres. `nx migrate my-plugin@latest` actualiza tu plugin y ejecuta sus migraciones; `nx migrate latest` (o `nx migrate nx@latest`) actualiza Nx mismo. Son actualizaciones separadas — ejecuta cada una cuando elijas mover ese paquete.

`nx migrate` apunta a un solo paso de versión de paquete a la vez, por lo que los dos pasos anteriores no están intercalados: completa el paso run-migrations para una actualización (y haz commit) antes de comenzar la siguiente. Al saltar a través de múltiples versiones principales, actualiza una versión principal a la vez.
:::
