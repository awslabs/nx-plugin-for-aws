...
Para algunos ejemplos de operaciones que puedes realizar en tu migración (generar archivos, modificar archivos existentes usando GritQL, leer y actualizar JSON), consulta la <Link path="/guides/nx-generator">guía de Generador de Nx</Link>.
...
Para el flujo completo desde la perspectiva del usuario que actualiza — incluyendo salida de ejemplo para migraciones deterministas y agénticas — consulta <Link path="/get_started/upgrading">Actualizar Tu Espacio de Trabajo</Link>.
