...
Checkov se ejecuta como parte de `build` y `package`, por lo que un `plan` interactivo escanea tu infraestructura antes de aplicarla.
...
`plan` depende de `package` en lugar de `build`, por lo que produce los artefactos que tus módulos referencian — los bundles de Lambda y los metadatos de operaciones generados — y ejecuta Checkov, sin ejecutar también las compuertas de lint, test y type-check de cada proyecto.
