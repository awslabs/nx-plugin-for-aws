...
<FileTree>
  ...
  - scripts Node helpers run by the nx `bootstrap`, `bootstrap-destroy` and `init` targets
    ...
    - plugin-cache.ts Points `terraform init` at the shared provider cache
...
#### Caché de Proveedores Compartido

Cada target que ejecuta `terraform init` comparte un caché de proveedores en `.terraform/plugin-cache` en la raíz de tu workspace, a través de [`TF_PLUGIN_CACHE_DIR`](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) de Terraform. Los proveedores se descargan una vez y se enlazan desde allí en adelante, lo cual es más importante para el target `test`: mantiene su directorio de trabajo bajo `dist`, por lo que un `dist` limpiado o una falla de caché de lo contrario volvería a descargar el proveedor de AWS — varios cientos de megabytes — en cada ejecución.

El directorio se crea automáticamente antes de cada `terraform init`, y `.terraform` está en gitignore. A diferencia de `.nx/cache`, no se elimina con `nx reset`, por lo que un reset no descarta los proveedores. En CI, almacena en caché `.terraform/plugin-cache` entre trabajos para obtener el mismo ahorro allí.

El `env` de Nx tiene precedencia sobre tu shell, por lo que establecer `TF_PLUGIN_CACHE_DIR` en tu entorno no cambia dónde buscan estos targets. Para apuntarlos a otro lugar — una ubicación compartida entre varios workspaces, digamos — cambia `TF_PLUGIN_CACHE_DIR` en el `project.json` de tu proyecto, y la ruta en `scripts/plugin-cache.ts` para los targets `bootstrap`, `bootstrap-destroy` e `init`. Las rutas en `project.json` son relativas al `cwd` del target, ya que Nx no expande `{workspaceRoot}` dentro de `env`.

:::note[Archivos de Bloqueo]
El caché no cambia qué versiones de proveedores obtienes: `.terraform.lock.hcl` aún las fija, y Terraform registra los mismos hashes ya sea que un proveedor provenga del caché o del registro. Haz commit del archivo de bloqueo como de costumbre.
:::
