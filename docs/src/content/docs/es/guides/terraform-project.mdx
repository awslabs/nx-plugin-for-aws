...
El target `fmt` verifica que tu código Terraform esté formateado, y falla si no lo está. Se ejecuta como parte de `build`:

<NxCommands commands={['fmt tf-infra']} />

Para reformatear tu código, ejecuta con la configuración `fix`:

<NxCommands commands={['fmt tf-infra --configuration=fix']} />

Un target `lint` orquesta la verificación de formato, por lo que `run-many` alcanza tus proyectos Terraform junto con tus proyectos TypeScript y Python:

<NxCommands commands={['run-many --target lint --all --configuration=fix']} />

:::tip[Por qué la verificación es la predeterminada]
`terraform fmt` reescribe los archivos que lee — los mismos archivos que el target `fmt` declara como sus entradas de caché. Escribir desde el target base cambiaría su propia clave de caché en cada ejecución, por lo que nunca podría registrar un acierto de caché. Verificar en su lugar mantiene el target cacheable, reflejando cómo funcionan los targets `format` de TypeScript y Python.
:::

Para omitir el formateo y linting durante el desarrollo, usa la configuración `skip-lint`:
