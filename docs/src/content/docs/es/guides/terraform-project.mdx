...
Checkov se ejecuta como parte de `build` y `assemble`, por lo que un `plan` interactivo escanea tu infraestructura antes de aplicarla.
...
`plan` depende de `assemble` en lugar de `build`, por lo que produce los artefactos que tus módulos referencian — los bundles de Lambda y los metadatos de operaciones generados — y ejecuta Checkov, sin ejecutar también las compuertas de lint, test y type-check de cada proyecto.

:::caution
Debido a que `plan` ya no ejecuta las pruebas de módulos de Terraform ni las pruebas unitarias de tus proyectos, ejecuta `build` para verificar tu código antes de aplicar cualquier cosa que te importe, y condiciona tu pipeline a ello:
