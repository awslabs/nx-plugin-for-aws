...
Cada target que ejecuta `terraform init` comparte un caché de proveedores en `.terraform/plugin-cache` en la raíz de tu workspace. Consulta la [documentación de Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) para más información.
