...
<FileTree>
  ...
  - scripts Ayudantes Node ejecutados por los objetivos nx `bootstrap` e `init`
    - aws-config.ts Resuelve cuenta + región a través de la cadena de credenciales del AWS SDK
    - bootstrap.ts Extrae/empuja el tfstate de bootstrap y ejecuta `terraform apply`
    - init.ts Ejecuta `terraform init` con la configuración del backend S3
    - paths.ts Localiza la raíz del workspace + proyecto en tiempo de ejecución
...
Por defecto, todos los objetivos asumen el entorno `dev`, pero puedes especificar un entorno con: <NxCommands commands={['apply tf-infra --configuration=prod']} />
...
Antes de desplegar tu infraestructura, necesitas ejecutar el bootstrap del backend de estado remoto. Esto crea un bucket S3 para almacenar tus archivos de estado de Terraform:

<NxCommands commands={['bootstrap tf-infra']} />

:::note[Configuración del perfil de AWS]
...
:::note[Alias de Deploy]
Un objetivo `deploy` también está disponible como alias para `apply`. Puedes usar <NxCommands commands={['deploy tf-infra']} /> o <NxCommands commands={['apply tf-infra']} /> indistintamente.
:::
