...
[Terraform](https://www.terraform.io/) es una herramienta de código abierto para infraestructura como código que te permite crear, modificar y mejorar infraestructura de manera segura y predecible.
...
### Generar un proyecto de Terraform

Puedes generar un nuevo proyecto de Terraform de dos maneras:
...
:::note
El generador también crea una biblioteca `packages/common/terraform` que inicialmente solo agrega un módulo de métricas (crea un stack CloudFormation vacío con metadatos) que se instrumenta automáticamente para que el equipo `nx-plugin-for-aws` pueda rastrear métricas de uso.
:::

### Tipo biblioteca

Para proyectos de biblioteca (`--type=library`), el generador crea una estructura más simple para módulos reutilizables de Terraform:
...
:::tip
Los proyectos de aplicación incluyen capacidades completas de despliegue con gestión de estado remoto, mientras que los proyectos de biblioteca están diseñados para crear módulos reutilizables de Terraform que pueden ser consumidos por otros proyectos.
:::
...
Configura variables específicas de entorno en los archivos `src/env/*.tfvars`.
...
:::tip
Por defecto, todos los objetivos asumen el entorno `dev`, pero puedes especificar un entorno particular así: <NxCommands commands={['run tf-infra:apply --configuration=prod']} />
...
El objetivo bootstrap solo está disponible para proyectos de tipo aplicación. Los proyectos de biblioteca no requieren gestión de estado remoto ya que están diseñados para ser módulos reutilizables.
:::
...
#### Validación de tu infraestructura

Puedes validar tu configuración de Terraform usando el objetivo `validate`:
...
Encontrarás los resultados de las pruebas de seguridad en la carpeta raíz `dist`, bajo `dist/packages/<mi-proyecto-terraform>/checkov`.
...
:::caution
¡Esto eliminará permanentemente todos los recursos gestionados por esta configuración de Terraform. ¡Usa con precaución!
:::

#### Destrucción de recursos de bootstrap
