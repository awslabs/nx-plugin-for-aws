...
Si deseas ejecutar un módulo de otro proyecto (lib), puedes hacerlo de la siguiente manera:
...
Para proyectos de aplicación, antes de desplegar tu infraestructura, necesitas preparar el backend de estado remoto. Esto crea un bucket S3 para almacenar tus archivos de estado de Terraform:
...
El objetivo bootstrap solo está disponible para proyectos de tipo aplicación. Los proyectos de biblioteca no requieren gestión de estado remoto al ser módulos reutilizables.
:::
...
Ejecuta verificaciones de seguridad con Checkov usando el objetivo `test`:

<NxCommands commands={['run tf-infra:test']} />

Encontrarás los resultados en la carpeta `dist` raíz, bajo `dist/packages/<mi-proyecto-terraform>/checkov`.
...
Antes de aplicar cambios, puedes ver lo que hará Terraform con el objetivo `plan`:
...
:::tip
Este comando aplica el plan creado por `plan`. Asegúrate de ejecutar `plan` primero para revisar los cambios.
:::
...
Para eliminar tu infraestructura, usa el objetivo `destroy`:

<NxCommands commands={['run tf-infra:destroy']} />

:::caution
¡Esto eliminará permanentemente todos los recursos gestionados por esta configuración! Usa con precaución.
:::

#### Destruir recursos de bootstrap

Para eliminar los recursos de bootstrap (bucket S3 de estado):
