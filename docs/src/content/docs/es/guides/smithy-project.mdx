...
El CLI se resuelve mediante [mise](https://mise.jdx.dev/), agregado a tu espacio de trabajo como una dependencia de desarrollo, por lo que no hay nada que instalar — descarga y almacena en caché la versión fijada la primera vez que compilas.

:::caution[Windows]
`mise` no publica ningún paquete de Windows en npm, así que sigue la [guía de instalación del Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) para instalarlo tú mismo antes de compilar.
:::
