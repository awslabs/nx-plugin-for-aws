...
`project.json`. Las configuraciones regionales a las que traduce provienen de `targetLanguages` en
`scripts/translate.config.json`, que se envía vacío — así que comienza nombrando las
configuraciones regionales que deseas en la línea de comandos:
...
Una vez que hayas completado `targetLanguages` (consulta [Configurar la
traducción](#configurar-la-traducción)), `--languages` se vuelve opcional y se
utilizan las configuraciones regionales configuradas:
...
:::caution[Configura primero tus idiomas de destino]
Sin `--languages` y con `targetLanguages` aún vacío, el target sale con
código distinto de cero con `ERROR No target languages configured`.
:::
...
| `translationCommitMessage` | Marcador de mensaje de commit para commits de traducción. No está en la configuración generada — agrega la clave para anular su valor predeterminado `docs: update translations`. |
...
3. Haga commit de las traducciones resultantes de vuelta a la rama del PR, usando
   `docs: update translations` como mensaje de commit, para que las ejecuciones
   incrementales posteriores puedan detectar el commit de referencia y solo volver a
   traducir los archivos que cambiaron desde entonces. Para usar un mensaje diferente,
   agrega una clave `translationCommitMessage` a `scripts/translate.config.json` y
   haz coincidir ese valor en su lugar.
