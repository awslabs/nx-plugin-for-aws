...
3. Haga commit de las traducciones resultantes de vuelta a la rama del PR, usando
   `docs: update translations` como mensaje de commit, para que las
   ejecuciones incrementales posteriores puedan detectar el commit de referencia y solo
   volver a traducir los archivos que cambiaron desde entonces. Para usar un mensaje diferente, agrega una
   clave `translationCommitMessage` a `scripts/translate.config.json` y coincide con
   ese valor en su lugar.
