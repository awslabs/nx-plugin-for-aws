...
El generador también provee una CLI de chat interactiva mínima bajo `scripts/<your-agent-name>/chat.ts` y un target correspondiente `<your-agent-name>-chat`. Este target depende de `<your-agent-name>-serve-local`, por lo que ejecutarlo inicia el agente localmente y te lleva a un chat interactivo de terminal con él:

<NxCommands commands={['run your-project:agent-chat']} />

La CLI usa el cliente apropiado para el protocolo (`AgentClient.local` para HTTP/tRPC o `@a2a-js/sdk` para A2A) y transmite las respuestas del agente token por token a través de [`@clack/prompts`](https://github.com/bombshell-dev/clack).
