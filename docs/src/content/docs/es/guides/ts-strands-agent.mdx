...
El generador configura un target Nx `<your-agent-name>-chat` que depende de `<your-agent-name>-serve-local`. Ejecutarlo inicia el agente localmente y te lleva a un chat interactivo de terminal impulsado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Para agentes **A2A**, el target simplemente ejecuta `agent-chat-cli` como un binario — no se provee código en tu workspace.

Para agentes **HTTP** (tRPC sobre WebSocket), el generador emite un pequeño `scripts/<your-agent-name>/chat.ts` que envuelve el `<Agent>Client.local({ url })` generado como un `ChatAdapter` y lo pasa al `chatLoop` de `agent-chat-cli`.
