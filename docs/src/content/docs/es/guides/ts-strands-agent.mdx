...
:::caution[AG-UI aún no soportado para TypeScript]
El [protocolo AG-UI](https://docs.ag-ui.com/) aún no está soportado para TypeScript Strands Agents. Si necesitas soporte AG-UI, por favor usa el generador <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> con `protocol=AG-UI`. Consulta [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) para el estado del soporte AG-UI en TypeScript.
:::
...
El generador configura un target llamado `<your-agent-name>-invoke` que inicia un chat interactivo con tu agente desde la terminal. Por defecto se conecta al servidor local:

<NxCommands commands={['agent-invoke your-project']} />

Para chatear con un agente desplegado en su lugar, establece `RUNTIME_CONFIG_APP_ID` al ID de aplicación AppConfig de tu workspace — el CLI resuelve el ARN del runtime e invoca el endpoint desplegado:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Para agentes autenticados con Cognito, pasa un JWT con `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

El script CLI se encuentra en `scripts/<agent>/cli.ts` y selecciona el protocolo correcto (HTTP/A2A) para tu agente automáticamente.
