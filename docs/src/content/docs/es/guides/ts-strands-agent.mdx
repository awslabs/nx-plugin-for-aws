...
Para otorgar acceso para invocar tu agente, necesitarás agregar una política como la siguiente, referenciando la salida `module.my_project_agent.agent_core_runtime_arn`:
...
Cuando seleccionas autenticación `Cognito`, el generador crea automáticamente el constructo `UserIdentity` y configura el agente para usar Cognito para autenticación.
...
El constructo `UserIdentity` se genera automáticamente cuando seleccionas autenticación `Cognito`.
</Fragment>
<Fragment slot="terraform">
El módulo generado acepta las variables `user_pool_id` y `user_pool_client_ids` para autenticación Cognito:
