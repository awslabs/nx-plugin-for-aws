...
Cuando `protocol` está establecido en `A2A`, el punto de entrada usa el [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) en lugar de tRPC:
...
:::tip[Etiquetas de imagen Docker]
...
### Invocar un Agente A2A

El `A2AAgent` del SDK de Strands envuelve un endpoint A2A remoto para que puedas invocarlo como un `Agent` local. `invoke` resuelve a un `AgentResult`; llamar a `.toString()` en él une los bloques de texto de la respuesta del agente remoto.
...
#### Invocar en AgentCore con Autenticación IAM

Las solicitudes a un agente A2A desplegado en AgentCore (incluyendo la obtención de la tarjeta del agente) deben estar firmadas con SigV4. Pasa un `clientFactory` a `A2AAgent` que use un fetch con firma SigV4 tanto para el resolvedor de tarjetas como para el transporte JSON-RPC:
...
:::note[Permisos IAM]
El rol IAM del invocador necesita tanto `bedrock-agentcore:InvokeAgentRuntime` como `bedrock-agentcore:GetAgentCard` en el ARN del runtime de destino. El helper `grantInvokeAccess` del constructo generado otorga ambos.
:::

:::note[Dependencias adicionales]
El ejemplo del cliente SigV4 anterior requiere `@a2a-js/sdk`, `aws4fetch` y `@aws-sdk/credential-providers`. Agrégalos a tu `package.json` antes de ejecutar el ejemplo.
:::

#### Usar un Agente A2A como Herramienta

`A2AAgent` implementa la misma interfaz `InvokableAgent` que un `Agent` local, por lo que puedes envolverlo en una `tool` de Strands y pasarlo a otro agente. Así es como la plantilla del agente HTTP/tRPC generado delega a un agente A2A remoto:

```typescript
import { Agent, tool } from '@strands-agents/sdk';
import { z } from 'zod';

const remote = /* A2AAgent construido con clientFactory como arriba */;
...
En tu stack CDK, otorga al agente host acceso de invocación al agente remoto — esto conecta ambas acciones IAM que el cliente A2A necesita:
