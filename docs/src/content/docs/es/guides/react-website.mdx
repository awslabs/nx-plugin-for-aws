...
:::warning
Al ejecutarse en este modo y si no existe `runtime-config.json`, si has configurado Autenticación Cognito (mediante el <Link path="/guides/react-website-auth">generador CloudScape Website Auth</Link>), el login se omitirá y las peticiones a servidores locales no incluirán headers de autenticación.
