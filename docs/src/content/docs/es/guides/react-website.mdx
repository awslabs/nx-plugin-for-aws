...
`$id` en la ruta `to` es un [parámetro de ruta](https://tanstack.com/router/latest/docs/framework/react/guide/path-params), proporcionado a través de `params`. Intercambia el `<button>` simple por el componente de botón propio de tu kit de UX según sea necesario.
