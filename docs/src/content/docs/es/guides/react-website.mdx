...
:::note
Si bien este generador te configura con CloudScape, en última instancia es un generador de proyectos React, y puedes modificar tu código para migrar a un sistema de diseño o biblioteca de componentes alternativo si lo deseas.
:::
...
## Implementación de tu sitio web React
...
<Steps>
  ...
  2. Crear un nuevo archivo `<nombre-página>.tsx` en `src/routes`, donde su posición en el árbol de archivos representa la ruta
...
La configuración de tu infraestructura AWS CDK se provee a tu sitio web mediante Runtime Configuration. Esto permite que tu sitio web acceda a detalles como URLs de APIs que no se conocen hasta que la aplicación es desplegada.

### Infraestructura

El constructo CDK `RuntimeConfig` se puede usar para añadir y recuperar configuración en tu infraestructura CDK. Los constructos CDK generados por `@aws/nx-plugin` (como <Link path="guides/trpc">APIs tRPC</Link> y <Link path="guides/fastapi">FastAPIs</Link>) añadirán automáticamente valores apropiados al `RuntimeConfig`.
...
:::warning
Debes asegurarte de declarar tu sitio web _después_ de cualquier constructo que añada al `RuntimeConfig`, de lo contrario faltarán en tu archivo `runtime-config.json`.
:::
...
Al ejecutar el [servidor de desarrollo local](#local-development-server), necesitarás un archivo `runtime-config.json` en tu directorio `public` para que tu sitio local conozca las URLs de backend, configuración de identidad, etc.
...
:::warning
Si cambias el prefijo de tus stage names en el `src/main.ts` de tu proyecto de infraestructura, deberás actualizar el target `load:runtime-config` en el archivo `project.json` de tu sitio web.

Además, es importante notar que el target `load:runtime-config` asume que un único stage de tu aplicación está desplegado en el entorno para el que tienes credenciales AWS. Deberás ajustar el comando si despliegas múltiples stages en la misma cuenta y región.
:::
...
Este target es útil para trabajar en cambios del sitio web apuntando a APIs "reales" desplegadas y otra infraestructura.

### Target Serve Local

El target `serve-local` inicia un servidor de desarrollo local para tu sitio web (con [Vite `MODE`](https://vite.dev/guide/env-and-mode) establecido en `serve-local`), además de iniciar servidores locales para APIs conectadas a tu sitio web mediante el <Link path="/guides/api-connection">generador de Conexión API</Link>.

Cuando el servidor local se ejecuta mediante este target, el `runtime-config.json` se sobrescribe automáticamente para apuntar a las URLs de tus APIs locales.

Puedes ejecutar este target con el siguiente comando:

<NxCommands commands={['run <my-website>:serve-local']} />

Este target es útil cuando trabajas simultáneamente en tu sitio web y API, y deseas iterar rápidamente sin desplegar infraestructura.

:::warning
Cuando se ejecuta en este modo y no existe `runtime-config.json`, si has configurado Autenticación Cognito (mediante el <Link path="/guides/react-website-auth">generador CloudScape Website Auth</Link>), el login se omitirá y las peticiones a tus servidores locales no incluirán headers de autenticación.
...
Probar tu sitio web es similar a escribir tests en un proyecto TypeScript estándar, por favor consulta la <Link path="guides/typescript-project#testing">guía de proyectos TypeScript</Link> para más detalles.
...
Puedes ejecutar tus tests usando el target `test`:
