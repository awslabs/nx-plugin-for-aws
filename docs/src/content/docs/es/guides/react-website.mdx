...
El target `serve-local` inicia un servidor local (con [Vite `MODE`](https://vite.dev/guide/env-and-mode) en `serve-local`) e inicia servidores locales para APIs conectadas mediante el generador <Link path="/guides/connection">Connection</Link>.
...
:::note[Script `dev`]
Si no existe un script `dev` en tu `package.json` raíz cuando se ejecuta este generador, se añade un script `dev` que invoca `serve-local` para el sitio web generado, lo que significa que también puedes iniciar el servidor de desarrollo local con:

<PackageManagerShortCommand commands={["dev"]} />

Esto hace que el primer sitio web React que generes sea el target `dev` predeterminado para el workspace. Los sitios web subsiguientes no sobrescriben el script `dev` existente — puedes invocar sus targets `serve-local` directamente, o actualizar el script manualmente.
:::

:::warning[Autenticación simulada]
En este modo, si no hay `runtime-config.json` y usas Autenticación Cognito (via <Link path="/guides/react-website-auth">generador `ts#website#auth`</Link>), el login se omitirá y las peticiones no incluirán headers de autenticación.

Para habilitar login en `serve-local`, despliega tu infraestructura y carga la configuración.
:::

:::tip[Comportamiento de Serve-Local]
