...
`src/hooks/useRuntimeConfig.tsx` y `src/components/RuntimeConfig` se agregan a tu sitio web por el generador <Link path="/guides/react-website-auth">`ts#website#auth`</Link> y los generadores <Link path="guides/connection">`connection`</Link>, ya que son los que ponen valores en la configuración en tiempo de ejecución. Un sitio web sin ninguno de estos no tiene configuración en tiempo de ejecución para leer, por lo que el hook no se proporciona hasta que uno se haya ejecutado.
:::
