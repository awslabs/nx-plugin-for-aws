...
Elegir `S3_MANAGED` falla el objetivo `checkov` de tu proyecto con `CKV_AWS_145` en los buckets del sitio web y de logs de distribución. El objetivo ejecuta `checkov` contra el `checkov.yml` de tu propio proyecto, así que agrega la verificación a su lista `skip-check`:
...
El objetivo `checkov` pasa `--config-file`, lo que desactiva el descubrimiento propio de `checkov` de un `.checkov.yaml` — un archivo de configuración colocado en cualquier otro lugar es ignorado.
:::
...
Deshabilitar la rotación de clave en la clave creada automáticamente falla el objetivo `checkov` de tu proyecto con `CKV_AWS_7`. Agrégalo a la misma lista `skip-check` en `packages/infra/checkov.yml`:
...
Tu sitio web se genera con un `RuntimeConfigProvider` (en `src/components/RuntimeConfig`) que carga `runtime-config.json` antes de renderizar tu aplicación, y un hook `useRuntimeConfig` para leer valores de él:
...
El proveedor recurre a una configuración vacía cuando no se sirve `runtime-config.json`, por lo que un sitio web sin nada desplegado aún se renderiza. `IRuntimeConfig` está tipado como `any` por defecto — restringe el tipo en `src/components/RuntimeConfig/index.tsx` si deseas acceso con seguridad de tipos.
