...
- .gitignore Ignora por defecto los archivos del cliente generado

</FileTree>

El generador también añadirá Runtime Config a tu infraestructura web si no existe, asegurando que la URL de la API para FastAPI esté disponible en el sitio y configurada automáticamente por el hook `use<ApiName>.tsx`.
...
:::note
Cuando hagas cambios en tu FastAPI, debes reconstruir tu proyecto para que se reflejen en el cliente generado. Por ejemplo:
...
const createItem = useMutation({
  ...
  onSettled: () => {
    ...
    // Buen lugar para invalidar queries afectadas
...
function ItemList() {
  ...
  const items = useInfiniteQuery({
    ...api.listItems.infiniteQueryOptions({
      limit: 10, // Número de items por página
    }, {
      // Asegúrate de definir una función getNextPageParam que devuelva
      // el parámetro que se pasará como 'cursor' para la siguiente página
  ...
  return (
    <div>
      {/* Aplana el array de páginas para renderizar todos los items */}
      <ul>
...
Los hooks generados manejan automáticamente la paginación basada en cursor si tu API lo soporta. El valor `nextCursor` se extrae de la respuesta y se usa para obtener la siguiente página.
...
Si has <Link path="guides/fastapi#streaming">configurado tu FastAPI para transmitir respuestas</Link>, tu hook `useQuery` se actualizará automáticamente con los nuevos fragmentos del stream a medida que llegan.
...
<Steps>
  ...
  3. Se reciben fragmentos posteriores
      ...
      - `fetchStatus` permanece `'fetching'`
      - `data` se actualiza con cada nuevo fragmento inmediatamente
...
Si has <Link path="guides/fastapi#streaming">configurado tu FastAPI para transmitir respuestas</Link>, el cliente generado incluirá métodos tipados para iterar asíncronamente sobre los fragmentos del stream usando sintaxis `for await`.
...
:::note
Si tienes una API de streaming que acepta un parámetro `cursor`, cuando uses el hook `useInfiniteQuery`, cada página esperará a que finalice el stream antes de considerarse cargada.
:::
...
Los hooks y métodos del cliente generado se organizan automáticamente según los tags OpenAPI en tus endpoints FastAPI. Esto ayuda a mantener organizadas tus llamadas API y facilita encontrar operaciones relacionadas.
...
function ItemsAndUsers() {
  ...
  const handleCreateItem = async () => {
    try {
      // Crear item usando el método agrupado
...
Finalmente, especifica los modelos de respuesta para diferentes códigos de error en tus endpoints:
...
function ItemComponent() {
  ...
  // Crear item con manejo de errores
...
function ItemList() {
  ...
  // Mutación para eliminar items con actualizaciones optimistas
  const deleteMutation = useMutation({
    ...api.deleteItem.mutationOptions(),
    onMutate: async (itemId) => {
      // Cancela cualquier recarga pendiente
      await queryClient.cancelQueries({ queryKey: api.listItems.queryKey() });

      // Toma snapshot del valor anterior
      ...
      // Retorna un objeto de contexto con el snapshot
...
function ItemList() {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      ...
      // Restaura items anteriores en caso de error
...
function ItemForm() {
  ...
  // Mutación tipada para crear items
  const createItem = useMutation({
    ...api.createItem.mutationOptions(),
