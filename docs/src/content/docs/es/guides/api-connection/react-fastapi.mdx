...
2. Un backend FastAPI funcionando (generado usando el generador FastAPI)
3. Cognito Auth añadido mediante el <Link path="/guides/react-website-auth">generador `ts#react-website-auth`</Link> si conectas una API que use autenticación Cognito o IAM
...
- project.json Se añade un nuevo target al build que invoca el script de generación anterior
...
- project.json Se añade un nuevo target al build que genera un cliente type-safe
...
En tiempo de build, se genera un cliente type-safe a partir de la especificación OpenAPI de tu FastAPI. Esto añadirá tres nuevos archivos a tu aplicación React:
...
:::tip[Consejo]
Por defecto, el cliente generado se ignora en control de versiones. Si prefieres incluirlo, puedes eliminar la entrada del archivo `.gitignore` de tu aplicación React, pero ten en cuenta que cualquier cambio manual en los archivos `.gen.ts` será sobrescrito al construir tu proyecto.
:::

## Usando el código generado

El cliente type-safe generado puede usarse para llamar a tu FastAPI desde tu aplicación React. Se recomienda usar los hooks de TanStack Query, pero también puedes usar el cliente vanilla si lo prefieres.
...
:::tip[Consejo]
Si estás trabajando simultáneamente en tu aplicación React y FastAPI, usa el target `serve-local` de la aplicación React, que regenerará automáticamente el cliente cuando cambie tu API, además de recargar en caliente tu sitio y servidor FastAPI local:
...
function MyComponent() {
  ...
  if (item.isLoading) return <div>Cargando...</div>;
...
function CreateItemForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    createItem.mutate({ name: 'Nuevo elemento', description: 'Un nuevo elemento' });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {createItem.isPending ? 'Creando...' : 'Crear elemento'}
      </button>

      {createItem.isSuccess && (
        <div className="success">
          Elemento creado con ID: {createItem.data.id}
        </div>
      )}
...
const createItem = useMutation({
  ...
  onSuccess: (data) => {
    ...
    // Puedes navegar al nuevo elemento
...
function CreateItemForm() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {createdItem && (
        <div className="success">
          Elemento creado con ID: {createdItem.id}
        </div>
      )}
...
function ItemList() {
  ...
  const items = useInfiniteQuery({
    ...api.listItems.infiniteQueryOptions({
      ...
      // Asegúrate de definir una función getNextPageParam que devuelva
  ...
  return (
    <div>
      {/* Aplana el array de páginas para renderizar todos los elementos */}
      <ul>
...
La integración incluye manejo de errores integrado con respuestas de error tipadas. Se genera un tipo `<operation-name>Error` que encapsula las posibles respuestas de error definidas en la especificación OpenAPI. Cada error tiene una propiedad `status` y `error`, y al verificar el valor de `status` puedes restringir a un tipo específico de error.
...
function MyComponent() {
  ...
  const handleClick = () => {
    createItem.mutate({ name: 'Nuevo elemento' });
  };
  ...
  return <button onClick={handleClick}>Crear elemento</button>;
}
```
...
function MyComponent() {
  ...
  const handleClick = async () => {
    try {
      await api.createItem({ name: 'Nuevo elemento' });
    } catch (e) {
...
Si has <Link path="guides/fastapi#streaming">configurado tu FastAPI para transmitir respuestas</Link>, tu hook `useQuery` actualizará automáticamente sus datos a medida que lleguen nuevos fragmentos del stream.
...
<Steps>
  ...
  3. Se reciben fragmentos posteriores
      ...
      - `fetchStatus` permanece `'fetching'`
      - `data` se actualiza con cada nuevo fragmento tan pronto se recibe
...
Si has <Link path="guides/fastapi#streaming">configurado tu FastAPI para transmitir respuestas</Link>, el cliente generado incluirá métodos type-safe para iterar asincrónicamente sobre fragmentos del stream usando sintaxis `for await`.
...
:::note[Nota]
Si tienes una API de streaming que acepta un parámetro `cursor`, al usar el hook `useInfiniteQuery`, cada página esperará a que finalice el stream antes de cargarse.
:::
...
Los hooks y métodos del cliente generado se organizan automáticamente basados en los tags OpenAPI de tus endpoints FastAPI. Esto ayuda a mantener organizadas tus llamadas API y facilita encontrar operaciones relacionadas.
...
function ItemsAndUsers() {
  ...
  return (
    <div>
      ...
      <button onClick={handleCreateItem}>Añadir elemento</button>
...
function ItemsAndUsers() {
  ...
  const handleCreateItem = async () => {
    try {
      // Crear elemento usando el método agrupado
      const newItem = await api.items.create({ name: 'Nuevo elemento' });
...
#### Añadiendo manejadores de excepción
...
def get_item(item_id: str) -> Item:
    ...
    if not item:
        raise NotFoundException(message=f"Elemento con ID {item_id} no encontrado")
    return item
...
def create_item(item: Item) -> Item:
    if not is_valid(item):
        raise ValidationException(
            ValidationError(
                message="Datos de elemento inválidos",
...
function ItemList() {
  ...
  // Mutación para eliminar elementos con actualizaciones optimistas
  const deleteMutation = useMutation({
    ...
    onMutate: async (itemId) => {
      ...
      // Devolver un objeto de contexto con el snapshot
      return { previousItems };
    },
    onError: (err, itemId, context) => {
      // Si la mutación falla, usa el contexto devuelto de onMutate para revertir
      queryClient.setQueryData(api.listItems.queryKey(), context.previousItems);
      console.error('Error al eliminar elemento:', err);
    },
    onSettled: () => {
      // Siempre recargar después de error o éxito para asegurar sincronización
      queryClient.invalidateQueries({ queryKey: api.listItems.queryKey() });
    },
  });
  ...
  if (itemsQuery.isError) {
    return <ErrorMessage message="Error al cargar elementos" />;
  }
...
function ItemList() {
  ...
  const handleDelete = async (itemId) => {
    // Eliminar optimistamente el elemento
    ...
    try {
      ...
      // Restaurar elementos anteriores en caso de error
...
La integración provee seguridad de tipos de extremo a extremo. Tu IDE proveerá autocompletado completo y verificación de tipos para todas tus llamadas API:
...
function ItemForm() {
  ...
  // Mutación type-safe para crear elementos
  const createItem = useMutation({
    ...api.createItem.mutationOptions(),
  ...
  return (
    <form onSubmit={(e) => {
      e.preventDefault();
      handleSubmit({ name: 'Nuevo elemento' });
    }}>
      {/* Campos del formulario */}
      <button
        type="submit"
