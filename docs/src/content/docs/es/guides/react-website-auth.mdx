...
:::note[URLs de Callback]
El cliente del User Pool generado permite `http://localhost:4200` y `http://localhost:4300` como URLs de callback/logout de OAuth (para desarrollo local contra el pool implementado), además de la URL de tu distribución CloudFront. Mantén esta lista de permitidos mínima: para implementaciones de producción reforzadas, considera eliminar las URLs de localhost (o usar un cliente separado para desarrollo local) una vez que ya no ejecutes localmente contra el User Pool implementado.
:::
