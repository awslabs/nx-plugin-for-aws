...
El generador de Autenticación de Sitio Web React añade autenticación a tu sitio web React usando [Amazon Cognito](https://aws.amazon.com/cognito/).
...
`mfa` acepta `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` en CDK, y `"OFF"` / `"OPTIONAL"` / `"ON"` en Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` y el equivalente `otp` habilitan o deshabilitan cada método de segundo factor de forma independiente; no tienen efecto cuando `mfa` está desactivado.

:::caution[Terraform vincula SMS MFA a la verificación de número de teléfono]
A diferencia del construct CDK, el proveedor AWS de Terraform no tiene forma de ofrecer SMS como método MFA independientemente de usarlo para la verificación de número de teléfono. Establecer `mfa_second_factor_sms = false` por lo tanto también deshabilita la verificación de número de teléfono basada en SMS (`auto_verified_attributes` elimina `phone_number`).
:::
...
<Infrastructure>
<Fragment slot="cdk">
Necesitarás añadir la infraestructura de identidad de usuario a tu stack, declarándola _antes_ del sitio web:
...
El construct `UserIdentity` añade automáticamente la <Link path="guides/react-website#runtime-configuration">Configuración de Tiempo de Ejecución</Link> necesaria para asegurar que tu sitio web pueda apuntar al Cognito User Pool correcto para la autenticación.

:::note[Retención del rol SMS]
El User Pool está respaldado por un rol IAM que Cognito asume para enviar mensajes SMS. La política de eliminación del rol refleja la del User Pool — por defecto, el `UserPool` de CDK usa `RemovalPolicy.RETAIN`, por lo que en `cdk destroy` tanto el User Pool como su rol SMS se retienen y necesitarán ser limpiados manualmente si ya no se necesitan.
:::
</Fragment>
<Fragment slot="terraform">
...
El módulo de identidad de usuario añade automáticamente la <Link path="guides/react-website#runtime-configuration">Configuración de Tiempo de Ejecución</Link> necesaria para asegurar que tu sitio web pueda apuntar al Cognito User Pool correcto para la autenticación.
</Fragment>
</Infrastructure>

:::caution[Elimina las URLs de callback localhost para producción]
El cliente User Pool generado permite la URL de tu distribución CloudFront (y cualquier nombre de dominio personalizado configurado en ella) como URLs de callback/logout de OAuth, más `http://localhost:4200` y `http://localhost:4300` para desarrollo local contra el pool desplegado.
