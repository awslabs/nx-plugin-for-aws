...
Una vez que tengas tu proyecto de plugin, puedes agregar generadores usando el generador <Link path="/guides/nx-generator">`ts#nx-generator`</Link>:
...
Lee la <Link path="/guides/nx-generator">guía de `ts#nx-generator`</Link> para obtener detalles sobre cómo implementar generadores.
:::
...
- Los manifiestos `generators.json`, `executors.json` y `migrations.json` que Nx lee de un plugin publicado
...
1. **Actualizar Información del Paquete**: Edita el [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) de tu plugin con el nombre, versión, descripción e información del repositorio adecuados. También elimina `"private": true`.

2. **Construir el Paquete**:
   <NxCommands commands={['package your-plugin']} />

   El paquete se coloca en `dist/<raíz del proyecto de tu plugin>/package` — para un plugin generado con `--name=nx-plugin --directory=tools`, eso es `dist/tools/nx-plugin/package`.
