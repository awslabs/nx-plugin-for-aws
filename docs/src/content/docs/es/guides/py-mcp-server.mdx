...
Si seleccionaste `None` para `infra`, no se generan constructos CDK ni módulos Terraform — el servidor MCP está configurado solo para uso local STDIO / HTTP. La opción `auth` se ignora en este modo ya que no hay un endpoint alojado para autenticar.
</OptionFilter>
...
Si prefieres ejecutar tu servidor MCP localmente usando [transporte HTTP transmitible](https://modelcontextprotocol.io/docs/concepts/transports#streamable-http), puedes usar el objetivo `<your-server-name>-serve`.
...
También se añade un objetivo `docker` específico para tu servidor MCP, que copia el `Dockerfile` y los artefactos empaquetados en un directorio de contexto de Docker. Esto ubica el `Dockerfile` junto con la salida construida, permitiendo que CDK construya la imagen Docker directamente usando `AgentRuntimeArtifact.fromAsset`.

### Observabilidad

<Snippet name="mcp/observability" parentHeading="Observabilidad" />
</OptionFilter>="Observabilidad" />
</OptionFilter>