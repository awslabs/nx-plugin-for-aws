...
Si seleccionaste `None` para `computeType`, no se generan constructos CDK ni módulos Terraform — el servidor MCP está configurado solo para uso local STDIO / HTTP. La opción `auth` se ignora en este modo ya que no hay un endpoint alojado para autenticar.
</OptionFilter>
