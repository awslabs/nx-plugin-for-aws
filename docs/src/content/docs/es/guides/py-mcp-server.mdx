...
:::tip
Primero utiliza el generador <Link path="/guides/python-project">`py#project`</Link> para crear un proyecto al que añadir tu servidor MCP.
:::
  ...
  - your-project/
    - your_module/
      - mcp_server/ (o nombre personalizado si se especifica)
        ...
        - Dockerfile Punto de entrada para alojar tu servidor MCP (excluido cuando `computeType` es `None`)
...
Las herramientas son funciones que el asistente de IA puede llamar para realizar acciones. El servidor MCP Python utiliza la biblioteca [MCP Python SDK (FastMCP)](https://github.com/modelcontextprotocol/python-sdk), que provee un enfoque basado en decoradores para definir herramientas.
