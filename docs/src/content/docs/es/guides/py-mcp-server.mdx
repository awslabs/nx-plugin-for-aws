...
Genera un servidor Python del [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) para proporcionar contexto a modelos de lenguaje grandes (LLMs), con la opción de desplegarlo en [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
:::note
A menos que hayas seleccionado `None` para `computeType`, el generador también creará la infraestructura CDK o Terraform para desplegar tu servidor MCP.
:::
...
Las herramientas son funciones que el asistente de IA puede llamar para realizar acciones. El servidor MCP en Python utiliza la biblioteca [MCP Python SDK (FastMCP)](https://github.com/modelcontextprotocol/python-sdk), que provee un enfoque basado en decoradores para definir herramientas.
...
Los recursos proveen contexto al asistente de IA. Puedes añadirlos usando el decorador `@mcp.resource`:
...
La forma más sencilla de probar y usar un servidor MCP es mediante el inspector o configurándolo con un asistente de IA (como se mencionó anteriormente).
...
:::tip
La imagen Docker se construye usando una etiqueta (por ejemplo `my-scope-my-project-mcp-server:latest`), referenciada por tu infraestructura CDK o Terraform, permitiendo que tu `Dockerfile` esté ubicado junto a tu proyecto de servidor MCP.
:::
