...
<OptionFilter when={{ infra: 'agentcore' }} description="Despliegue de Bedrock AgentCore Runtime">
...
### Delimitar Datos de Herramientas por Llamador

`createServer` recibe un `McpRequestContext` que describe la solicitud actual. A través de Streamable HTTP esto incluye el id de sesión de AgentCore Runtime del llamador, que un <Link path="guides/ts-agent">agente</Link> reenvía automáticamente en cada llamada MCP saliente.

Pásalo a una herramienta que almacena datos por llamador:
...
:::caution
Delimita los datos almacenados en el contexto de la solicitud en lugar de en un argumento de herramienta. Un modelo puede ser persuadido para pasar cualquier valor que desee, por lo que argumentos como un nombre de usuario o jugador no son un límite de confianza. El contexto está ausente a través de STDIO, donde no hay solicitud.
:::
...
<OptionFilter when={{ infra: 'agentcore' }} description="Detalles del despliegue de Bedrock AgentCore Runtime">
