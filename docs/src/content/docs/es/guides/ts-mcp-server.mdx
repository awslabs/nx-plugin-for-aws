...
<FileTree>
  - your-project/
    ...
    - project.json Actualizado con objetivo de servicio del servidor MCP
</FileTree>
...
:::tip
La imagen docker se construye usando un tag (por ejemplo `my-scope-my-project-mcp-server:latest`), del cual el constructo CDK hereda en su `Dockerfile`, permitiendo que tu `Dockerfile` esté ubicado junto a tu proyecto de servidor MCP.
:::
