...
Genera un servidor TypeScript del [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) para proporcionar contexto a modelos de lenguaje grandes (LLMs), y opcionalmente desplegarlo en [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
Una vez que [este issue del SDK MCP](https://github.com/modelcontextprotocol/typescript-sdk/issues/164) se resuelva, podrás eliminar la dependencia adicional de `zod-v3` e importar directamente desde `zod`.
:::
...
La forma más fácil de probar y usar un servidor MCP es configurándolo con un asistente de IA (como se mencionó anteriormente).
...
Por defecto, tu servidor MCP estará asegurado usando autenticación IAM. Simplemente despliégalo sin argumentos:
...
Para configurar autenticación JWT, puedes pasar la propiedad `authorizerConfiguration` a tu constructo de servidor MCP. Aquí un ejemplo que configura un user pool y cliente de Cognito para asegurar el servidor MCP:
