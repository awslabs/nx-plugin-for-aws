...
Las herramientas son funciones que el asistente de IA puede llamar para realizar acciones. Cada herramienta vive en su propio archivo bajo `tools/` que exporta una función `register<Name>Tool`, la cual luego llamas desde `server.ts`. Por ejemplo, añade `tools/my-tool.ts`:
