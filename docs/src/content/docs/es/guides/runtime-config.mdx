...
En tiempo de synth/deploy, `RuntimeConfig` crea una aplicación **AWS AppConfig** que contiene:
- Un **Perfil de Configuración** para cada espacio de nombres
- Una **Versión de Configuración Alojada** con los datos JSON para cada perfil
- Un **Despliegue** instantáneo al entorno `default`
</Fragment>
<Fragment slot="terraform">
Terraform conecta la configuración en tiempo de ejecución a través de tres módulos `core/runtime-config/*`. Declara estos en el siguiente orden en tu módulo raíz:

<Steps>

1. **`core/runtime-config/appconfig`** — declarado una vez, cerca del inicio del módulo raíz. Crea la aplicación AppConfig, el entorno, la estrategia de despliegue y un perfil de configuración por espacio de nombres. Sus salidas `application_id` y `application_arn` se pasan a cada módulo que lee la configuración en tiempo de ejecución en tiempo de ejecución (agentes, servidores MCP, funciones Lambda, etc.).
...
2. **`core/runtime-config/entry`** — una invocación por contribución. Los módulos generados de API, agente y MCP llaman a esto internamente para publicar sus URLs y ARNs. Llámalo directamente para publicar configuración personalizada.
...
3. **`core/runtime-config/appconfig-deployment`** — declarado una vez, al final del módulo raíz, con `depends_on` cubriendo cada módulo que contribuye una entrada. Agrega las entradas contribuidas en un JSON por espacio de nombres y crea la versión de configuración alojada + despliegue contra la aplicación compartida.
...
Los consumidores del lado del servidor necesitan el **ID de Aplicación AppConfig** y permisos IAM para leer la configuración en tiempo de ejecución. Los constructos generados manejan esto automáticamente.

<Infrastructure>
<Fragment slot="cdk">
Usa `appConfigApplicationId` para obtener el ID de Aplicación AppConfig, y `grantReadAppConfig()` para otorgar permisos de lectura:
...
Los constructos generados de API y agente hacen esto automáticamente — solo necesitas hacer esto manualmente si creas tus propias funciones Lambda o recursos de cómputo que necesitan leer la configuración en tiempo de ejecución.
:::
</Fragment>
<Fragment slot="terraform">
Referencia la salida `application_id` del módulo compartido `runtime_config_appconfig` para obtener el ID de Aplicación AppConfig, y agrega las declaraciones de política IAM apropiadas:
...
:::note[Registro Automático]
Los módulos generados de agente y MCP aceptan entradas `appconfig_application_id` + `appconfig_application_arn` — pasa las salidas compartidas de `runtime_config_appconfig` y ellos configuran la variable de entorno y los permisos IAM por ti. Solo necesitas hacer esto manualmente para funciones Lambda personalizadas o recursos de cómputo.
:::
</Fragment>
</Infrastructure>

:::tip[Desarrollo Local]
Si usas configuración en tiempo de ejecución con espacios de nombres personalizados (por ejemplo, `tables`, `features`), solo necesitas definir una única variable de entorno `RUNTIME_CONFIG_APP_ID` localmente para apuntar a tu aplicación AppConfig desplegada. Esto permite que tus APIs o agentes locales recuperen la misma configuración que el entorno desplegado sin necesidad de replicar todas las variables de entorno individuales.
:::

## Acceso del Lado del Servidor vía AppConfig

Los consumidores del lado del servidor como funciones Lambda y agentes pueden recuperar la configuración en tiempo de ejecución desde **AWS AppConfig** usando [AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/).

Todos los constructos generados de API y agente se configuran automáticamente con:
- La variable de entorno `RUNTIME_CONFIG_APP_ID` (el ID de Aplicación AppConfig)
...
`getAppConfig` devuelve cualquier valor JSON válido, así que declara la forma que esperas y convierte el resultado — de lo contrario, los accesos a propiedades no compilarán bajo el `tsconfig` estricto del workspace:
...
:::tip[Valida en el límite]
Una conversión le dice al compilador la forma pero no la verifica en tiempo de ejecución. Para configuración que no controlas, analízala con un esquema [Zod](https://zod.dev/) en su lugar, para que un valor malformado falle ruidosamente en lugar de aparecer como `undefined` más tarde.
:::
...
Para sitios web, el espacio de nombres `connection` se despliega como un archivo `runtime-config.json` en el bucket S3. Consulta la guía <Link path="guides/react-website#runtime-configuration">Configuración en Tiempo de Ejecución del Sitio Web React</Link> para detalles sobre cómo acceder a estos valores desde tu código frontend.
