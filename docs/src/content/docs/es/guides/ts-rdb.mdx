...
<FileTree>
  ...
  - scripts
    - pull-image.ts Descarga la imagen de contenedor de la base de datos para desarrollo local
...
Para tipos de proyecto específicos, usa el generador `connection` para conectar automáticamente la base de datos — maneja importaciones, inyección de contexto y dependencias de desarrollo local:

- <Link path="guides/connection/trpc-rdb">API tRPC → Base de Datos Relacional</Link>
- <Link path="guides/connection/smithy-rdb">API Smithy → Base de Datos Relacional</Link>
- <Link path="guides/connection/ts-agent-rdb">Agente TypeScript Strands → Base de Datos Relacional</Link>
- <Link path="guides/connection/ts-mcp-server-rdb">Servidor MCP → Base de Datos Relacional</Link>
...
La imagen de base de datos local se configura en el objetivo `serve-local` del proyecto de base de datos generado en `project.json`. Actualiza el argumento de imagen pasado a `scripts/start-container.ts` cuando cambies las versiones del motor.
