...
Usa `exceptions` para paquetes que fallan la verificación — ya sea porque su licencia no está en la lista de permitidas, o porque se envían sin metadatos de licencia detectables.

Cada entrada acepta:

- `package` (requerido): el nombre del paquete al que se aplica la excepción
- `reason` (requerido): por qué se otorgó la excepción, para que los revisores puedan verlo
- `version` (opcional): restringe la excepción a una sola versión, para que una versión posterior se verifique nuevamente
- `spdx` (opcional): la licencia que se debe tratar como declarada por el paquete, para un paquete que incluye texto de licencia sin declararlo en sus metadatos. Esto es lo que reporta el hook `onDependency` a continuación
...
Los recolectores descubren dependencias y extraen metadatos de licencia. Los recolectores integrados son `npmCollector()` (escanea `node_modules`) y `pythonCollector()` (escanea entornos virtuales de Python), ambos exportados desde `@aws/nx-plugin/sdk/license`. El generador de licencias configura `npmCollector()` por defecto y agrega `pythonCollector()` cuando hay proyectos de Python presentes.
