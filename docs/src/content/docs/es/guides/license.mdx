...
Administra las licencias en todo tu espacio de trabajo: sincroniza archivos `LICENSE` y cabeceras de código fuente, y verifica que cada dependencia cumpla con una lista de licencias permitidas.
...
<FileTree>
  ...
  - aws-nx-plugin.config.mts Configuración para sincronización de licencias y verificación de dependencias
</FileTree>
...
Los resultados se almacenan en caché contra tu lockfile y `aws-nx-plugin.config.mts` — las re-ejecuciones son instantáneas cuando nada ha cambiado.
...
Para desactivar las verificaciones de dependencias en una sola ejecución, establece la variable de entorno `LICENSE_DEPENDENCY_CHECK=skip`:

<PackageManagerShortCommand commands={["LICENSE_DEPENDENCY_CHECK=skip lint"]} />

Para desactivar permanentemente, establece `dependencyCheck` a `false` en tu configuración o vuelve a ejecutar el generador con `--dependencyCheck=false`.
