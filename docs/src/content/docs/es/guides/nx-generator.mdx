...
Algunas opciones solo aplican para ciertos valores de otra. Usa `x-when` para indicar cuáles,
de modo que el sitio de documentación omita la opción donde no aplica y declare
la condición donde sí aplica:
...
Múltiples claves se combinan con AND y una lista de valores se combina con OR dentro de una clave, coincidiendo
con `<OptionFilter when={{…}}>`:
...
`x-when` documenta la opción; no impone nada, por lo que un generador aún
valida o ignora una opción que no aplica a la ejecución que se le dio.
