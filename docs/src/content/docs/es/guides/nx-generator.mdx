...
Recomendamos usar el generador <Link path="/es/guides/ts-nx-plugin">`ts#nx-plugin`</Link> como proyecto base para cualquier generador que crees, ya que también proporciona un Servidor MCP que ayuda a la IA a usar tus generadores.
:::
...
:::warning[Modificación del Proyecto]
Este generador actualizará el `project` seleccionado para usar CommonJS, ya que los Generadores Nx solo soportan CommonJS en la actualidad ([consulta este issue de GitHub para soporte ESM](https://github.com/nrwl/nx/issues/15682)).
:::
...
Para una guía más detallada sobre cómo contribuir al Nx Plugin for AWS, consulta el <Link path="/es/get_started/tutorials/contribute-generator">tutorial aquí</Link>.
:::