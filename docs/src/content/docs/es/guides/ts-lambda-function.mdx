...
Seleccionaste `Any` para tu `eventSource`, lo que significa que el middleware `parser` no está conectado y el parámetro `event` está tipado como `any`. Si más adelante deseas seguridad de tipos en tiempo de compilación y validación en tiempo de ejecución, regenera la función con un `eventSource` específico.
</OptionFilter>
