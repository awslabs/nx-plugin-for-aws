...
Si seleccionaste `Any` para tu `eventSource`, el middleware `parser` no está conectado y el parámetro `event` está tipado como `any`. Regenera la función con un `eventSource` específico si deseas seguridad de tipos en tiempo de compilación y validación en tiempo de ejecución.
</OptionFilter>
