...
El generador crea infraestructura como código para desplegar tu función según el `iac` seleccionado:
...
Si seleccionaste un `event` al generar tu función Lambda, esta se instrumenta con el middleware [`parser` de AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Por ejemplo:
...
Si tienes datos personalizados anidados en un evento, como un stream de DynamoDB o evento de EventBridge, puedes beneficiarte usando [Envelopes](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#envelopes) para proveer seguridad de tipos a esos datos.
:::
</OptionFilter>
