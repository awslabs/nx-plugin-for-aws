...
:::tip
La opción `integrationStyle` tiene como valor predeterminado `Individual Functions`, que crea un Lambda por procedimiento tRPC. Selecciona `Router` si prefieres un único manejador Lambda compartido para toda la API.
:::
...
### Eligiendo un Estilo de Integración

El generador tRPC admite dos patrones de infraestructura mediante la opción `integrationStyle`:

- `Individual Functions` es el predeterminado. Genera una función Lambda por procedimiento.
- `Router` genera una única función Lambda compartida para toda la API y la reutiliza para cada integración.

`Individual Functions` te da una infraestructura más granular y facilita adjuntar diferentes permisos o configuración por operación. `Router` reduce la proliferación de Lambdas e integraciones, lo que puede ser útil para APIs más pequeñas o cuando prefieres un único manejador central.

Ambos estilos de integración son actualmente compatibles con `ServerlessApiGatewayRestApi` y `ServerlessApiGatewayHttpApi`.
