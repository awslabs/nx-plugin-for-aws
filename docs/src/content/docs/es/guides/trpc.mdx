...
<FileTree>
  - src
    ...
    - router.ts Definición del enrutador tRPC (punto de entrada de la API en el handler Lambda)
    - schema Definiciones de esquemas usando Zod
      - echo.ts Definiciones de ejemplo para la entrada y salida del procedimiento "echo"
...
A alto nivel, las APIs tRPC consisten en un enrutador que delega solicitudes a procedimientos específicos. Cada procedimiento tiene una entrada y salida definidas como esquemas Zod.
...
=======
:::note
Se agrega un alias en `tsconfig.base.json` en la raíz de tu monorepo para resolver Zod a [versión 4.x](https://zod.dev/v4). Esto prepara para cuando Zod v4 se lance en NPM, permitiendo una actualización sin cambiar las rutas de importación.
:::
...
=======
El punto de entrada de tu API se encuentra en `src/router.ts`. Este archivo contiene el handler Lambda que enruta las solicitudes a "procedimientos" basados en la operación invocada. Cada procedimiento define la entrada esperada, salida e implementación.
>>>>>>> c5375c6 (docs: update translations)

El enrutador de ejemplo generado tiene una sola operación llamada `echo`:
...
=======
El tracer de AWS Lambda Powertools se configura en `src/middleware/tracer.ts`, y puede accederse en una implementación de API via `opts.ctx.tracer`. Puedes usarlo para agregar trazas con AWS X-Ray que proporcionen insights detallados sobre el rendimiento y flujo de las solicitudes a la API. Por ejemplo:
>>>>>>> c5375c6 (docs: update translations)
...
=======
Este ejemplo asume que `auth` fue configurado como `IAM`. Para autenticación con Cognito, el middleware de identidad es más directo, extrayendo los claims relevantes del `event`.
>>>>>>> c5375c6 (docs: update translations)
:::
...
=======
Esto configura la infraestructura de tu API, incluyendo una AWS API Gateway REST o HTTP API, funciones AWS Lambda para lógica de negocio, y autenticación basada en tu método `auth` elegido.
>>>>>>> c5375c6 (docs: update translations)

:::note
Si seleccionaste usar autenticación `Cognito`, necesitarás proporcionar la propiedad `identity` al construct de la API:
...
Si seleccionaste usar autenticación `IAM`, puedes usar el método `grantInvokeAccess` para otorgar acceso a tu API, por ejemplo podrías querer dar acceso a usuarios autenticados de Cognito a tu API:
...
Puedes crear un cliente tRPC para invocar tu API de forma type-safe. Si estás llamando a tu API tRPC desde otro backend, puedes usar el cliente en `src/client/index.ts`, por ejemplo:
>>>>>>> c5375c6 (docs: update translations)
...
Si estás llamando a tu API desde un sitio web React, considera usar el generador <Link path="guides/api-connection/react-trpc">Conexión a API</Link> para configurar el cliente.
