...
:::tip[Tipo de API]
Selecciona `ServerlessApiGatewayRestApi` (predeterminado) como tu `computeType` si deseas usar [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) para transmitir respuestas. Selecciona `BedrockAgentCoreRuntimeWebSocket` para desplegar el servidor tRPC como un contenedor en Bedrock AgentCore con subscripciones WebSocket de streaming — consulta [Desplegando en Bedrock AgentCore](#desplegando-en-bedrock-agentcore) más abajo.
:::

:::tip[Patrón de integración]
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea un Lambda por procedimiento tRPC. Selecciona `shared` si prefieres un único manejador Lambda compartido para toda la API, con opciones de sobrescritura por procedimiento. La opción `integrationPattern` no aplica cuando `computeType` es `BedrockAgentCoreRuntimeWebSocket`.
:::
...
<FileTree>
  ...
  - Dockerfile Imagen de contenedor para despliegues en AgentCore (solo tipo de cómputo AgentCore)
...
Las subscripciones solo son compatibles cuando `computeType` es `ServerlessApiGatewayRestApi` (predeterminado), ya que las HTTP APIs de API Gateway no admiten streaming de respuestas.
:::

Las subscripciones de tRPC te permiten transmitir datos del servidor al cliente usando [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Cuando seleccionas `ServerlessApiGatewayRestApi` como tu tipo de cómputo, el generador configura automáticamente la infraestructura requerida para streaming, así como un manejador Lambda de streaming y el helper de esquema `ZodAsyncIterable`.
...
## Desplegando en Bedrock AgentCore

Configurar `computeType` como `BedrockAgentCoreRuntimeWebSocket` despliega el servidor tRPC como un servidor WebSocket en contenedor en [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) en lugar de API Gateway + Lambda. AgentCore te proporciona:

- **Subscripciones de streaming sobre WebSocket** — mensajería full duplex para subscripciones tRPC sin los timeouts de 30 segundos de Lambda o API Gateway.
- **Afinidad de sesión** — el encabezado `x-amzn-bedrock-agentcore-runtime-session-id` se propaga a tu contexto tRPC como `ctx.sessionId`, para que puedas asociar estado por sesión.
- **Runtime administrado** — AgentCore maneja el escalado, verificaciones de salud (`/ping`) y autorización.

:::caution[Opciones de autenticación]
AgentCore solo admite autenticación `IAM` o `Cognito`. Seleccionar `auth=None` con `computeType=BedrockAgentCoreRuntimeWebSocket` fallará la generación.
:::

### Cómo funciona

Cuando eliges `BedrockAgentCoreRuntimeWebSocket`, el generador emite:

- `src/handler-agentcore.ts` — un servidor Node WebSocket + HTTP que expone `/ping` para verificaciones de salud de AgentCore y `/ws` para subscripciones tRPC vía `@trpc/server/adapters/ws`.
- `Dockerfile` — conteneriza el servidor empaquetado e incluye el [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) para observabilidad administrada.
- Un constructo CDK (o módulo Terraform) que aprovisiona un `Runtime` de Bedrock AgentCore, le otorga permiso para invocar modelos de Bedrock y leer la configuración del runtime, y publica el ARN del runtime en `runtimeConfig.apis.<YourApiName>` para que el frontend lo recoja.
- Un objetivo de build `docker` conectado al objetivo `build` del proyecto para que `nx build` produzca la imagen de contenedor.

### Accediendo al id de sesión

Cada procedimiento recibe `ctx.sessionId` cuando la API se despliega en AgentCore:
...
:::warning[Suposición de autenticación]
