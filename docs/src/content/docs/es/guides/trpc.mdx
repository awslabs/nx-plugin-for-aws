...
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea un Lambda por procedimiento tRPC. Selecciona `shared` si prefieres un único manejador Lambda compartido para toda la API, con anulaciones opcionales por procedimiento.
:::
...
A alto nivel, las APIs tRPC consisten en un enrutador que delega solicitudes a procedimientos específicos. Cada procedimiento tiene una entrada y salida, definidas como un esquema Zod.
...
Los esquemas son compartidos tanto por el código del servidor como del cliente, proporcionando un único lugar para actualizar al realizar cambios en las estructuras utilizadas en tu API.
...
### Enrutador y Procedimientos

Tu enrutador tRPC está definido en `src/router.ts`, que registra todos los procedimientos. Cada procedimiento define la entrada, salida e implementación esperadas. El punto de entrada del manejador Lambda está en `src/handler.ts`, que reenvía las solicitudes a tu enrutador.

El enrutador de ejemplo generado para ti tiene una única operación, llamada `echo`:
...
- `query` acepta una función que define la implementación para tu API. Esta implementación recibe `opts`, que contiene la `input` pasada a tu operación, así como otro contexto configurado por el middleware, disponible en `opts.ctx`. La función pasada a `query` debe devolver una salida que se ajuste al esquema `output`.
...
Si agregas un nuevo procedimiento, asegúrate de registrarlo agregándolo al enrutador en `src/router.ts`.

<OptionFilter when={{ infra: 'rest-lambda' }} description="Suscripciones de streaming — solo API REST, usa SSE">
...
Registra la suscripción en tu enrutador como cualquier otro procedimiento:
...
La infraestructura generada usa un manejador Lambda de streaming con `ResponseTransferMode.STREAM` en API Gateway para todas las operaciones de API REST, lo que permite que las suscripciones funcionen junto con consultas y mutaciones regulares.
</OptionFilter>
...
Puedes agrupar operaciones usando enrutadores anidados, por ejemplo:
...
Las métricas de AWS Lambda Powertools están configuradas en `src/middleware/metrics.ts`, y se pueden acceder en una implementación de API a través de `opts.ctx.metrics`. Puedes usar esto para registrar métricas en CloudWatch sin necesidad de importar y usar el AWS SDK, por ejemplo:
...
El trazador de AWS Lambda Powertools está configurado en `src/middleware/tracer.ts`, y se puede acceder en una implementación de API a través de `opts.ctx.tracer`. Puedes usar esto para agregar trazas con AWS X-Ray para proporcionar información detallada sobre el rendimiento y flujo de las solicitudes de API. Por ejemplo:
...
Como ejemplo, implementemos algo de middleware para extraer algunos detalles sobre el usuario que llama a nuestra API en `src/middleware/identity.ts`.

<OptionFilter when={{ auth: 'iam' }} description="Ejemplo de middleware de identidad para APIs autenticadas con IAM">
...
La búsqueda usa el cliente Cognito Identity Provider, que no es una dependencia de una API tRPC generada. Instálalo primero en tu proyecto de API:
...
Ten en cuenta que definimos una propiedad adicional _opcional_ al contexto. tRPC se encarga de asegurar que esto esté definido en procedimientos que hayan configurado correctamente este middleware.
...
<OptionFilter when={{ auth: 'cognito' }} description="Ejemplo de middleware de identidad para APIs autenticadas con Cognito">
Cuando despliegas con `auth: 'cognito'`, el autorizador Cognito de API Gateway verifica el JWT que el llamador proporciona en el encabezado `Authorization` y coloca las reclamaciones verificadas en el evento Lambda. Nuestro middleware solo lee esas reclamaciones — sin llamadas adicionales al AWS SDK, sin verificación manual de JWT.
...
Ten en cuenta que definimos una propiedad adicional _opcional_ en el contexto. tRPC se encarga de asegurar que esto esté definido en procedimientos que hayan configurado correctamente este middleware.
...
<OptionFilter when={{ auth: ['iam', 'custom'] }} description="Uso de CDK para autenticación IAM o Custom">
```ts {6-8}
...
<OptionFilter when={{ auth: 'cognito' }} description="Uso de CDK con autenticación Cognito — pasa el constructo de identidad">
```ts {6,10}
...
El módulo de API almacena su zip de despliegue Lambda en un bucket S3 de activos compartido — consulta la <Link path="/guides/terraform-project">guía de infraestructura Terraform</Link> para más detalles. Instancia el módulo `core/asset-bucket` una vez por despliegue y pasa su salida `bucket_name` a cada módulo de API / Lambda a través de la entrada `asset_bucket_name`:

<OptionFilter when={{ auth: ['iam', 'custom'] }} description="Uso de Terraform para autenticación IAM o Custom">
```hcl {1-3, 8}
module "asset_bucket" {
...
<OptionFilter when={{ auth: 'cognito' }} description="Uso de Terraform con autenticación Cognito — proporciona user pool y client">
```hcl {1-3, 8, 10-11}
module "asset_bucket" {
  source = "../../common/terraform/src/core/asset-bucket"
}
...
<OptionFilter when={{ auth: 'iam' }} description="Otorgando acceso de invocación de API — solo APIs autenticadas con IAM">
