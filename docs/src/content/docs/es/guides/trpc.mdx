...
El generador también creará constructs de CDK que pueden usarse para desplegar tu API, ubicados en el directorio `packages/common/constructs`.
...
El directorio `src/schema` contiene los tipos compartidos entre tu código cliente y servidor. En este paquete, estos tipos se definen usando [Zod](https://zod.dev/), una biblioteca de declaración y validación de esquemas con enfoque en TypeScript.
...
- `query` acepta una función que define la implementación de tu API. Esta implementación recibe `opts`, que contiene la `input` pasada a tu operación, así como otro contexto configurado por el middleware, disponible en `opts.ctx`. La función pasada a `query` debe devolver una salida que cumpla con el esquema de `output`.
...
Los clientes entonces reciben esta agrupación, por ejemplo, invocar la operación `listUsers` se vería así:
...
Las métricas de AWS Lambda Powertools se configuran en `src/middleware/metrics.ts`, y pueden accederse via `opts.ctx.metrics`. Puedes usarlas para registrar métricas en CloudWatch sin necesidad de importar y usar el AWS SDK, por ejemplo:
...
El tracer de AWS Lambda Powertools se configura en `src/middleware/tracer.ts`, y puede accederse via `opts.ctx.tracer`. Puedes usarlo para agregar trazas con AWS X-Ray y obtener insights detallados sobre el rendimiento y flujo de las solicitudes a la API. Por ejemplo:
...
En nuestro caso, queremos extraer detalles del usuario de Cognito que llama. Hacemos esto extrayendo el ID de sujeto ("sub") del usuario del evento de API Gateway, y recuperando detalles del usuario desde Cognito. La implementación varía ligeramente dependiendo de si el evento fue proporcionado por una API REST o HTTP:
