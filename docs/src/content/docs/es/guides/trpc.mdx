...
:::caution
Si seleccionaste `None` para `auth` al ejecutar el generador, puedes ver fallos en las verificaciones de Checkov como:
...
:::tip
Si seleccionaste CDK como tu `iacProvider`, cuando agregues o elimines un procedimiento en tu API tRPC, estos cambios se reflejarán inmediatamente en el constructo CDK sin necesidad de reconstruir.
:::
