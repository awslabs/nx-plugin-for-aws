...
:::tip
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea un Lambda por procedimiento tRPC. Selecciona `shared` si prefieres un único manejador Lambda predeterminado `$router` para toda la API, con opciones de sobrescritura por procedimiento.
:::
...
### Eligiendo un Patrón de Integración

El generador tRPC admite dos patrones de infraestructura mediante la opción `integrationPattern`:

- `isolated` es el predeterminado. Genera una función Lambda por procedimiento.
- `shared` genera una única integración Lambda predeterminada `$router` para toda la API y la reutiliza para cada procedimiento a menos que proporciones sobrescrituras por procedimiento.

`isolated` te da una infraestructura más granular y facilita adjuntar diferentes permisos o configuración por operación. `shared` reduce la proliferación de Lambdas e integraciones, mientras permite sobrescribir procedimientos específicos con integraciones dedicadas cuando sea necesario.

Ambos patrones de integración son actualmente compatibles con `ServerlessApiGatewayRestApi` y `ServerlessApiGatewayHttpApi`.
