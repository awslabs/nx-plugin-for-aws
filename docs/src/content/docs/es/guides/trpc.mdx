...
:::tip[Tipo de API]
Selecciona `ServerlessApiGatewayRestApi` (predeterminado) como tu `computeType` si deseas usar [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) para transmitir respuestas.
:::
...
Este ejemplo recorre el middleware de identidad para autenticación `IAM`, donde buscamos al llamante en Cognito usando el sub extraído del evento de API Gateway. Para autenticación `Cognito`, el middleware de identidad es más directo — extrae las claims directamente del `event` en su lugar.
...
Para conectar un User Pool y cliente de Cognito, proporciona las variables relevantes:
