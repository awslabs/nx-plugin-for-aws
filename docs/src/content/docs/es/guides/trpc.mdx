...
El generador de API tRPC crea una nueva API tRPC con configuración de infraestructura usando AWS CDK o Terraform. El backend generado puede desplegarse como funciones AWS Lambda expuestas a través de un API Gateway de AWS, o como un servidor WebSocket en contenedor ejecutándose en [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html). Incluye validación de esquemas usando [Zod](https://zod.dev/) y configura [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) para observabilidad, incluyendo logging, trazado con AWS X-Ray y métricas de CloudWatch.
...
:::tip[Cuándo usar cada `computeType`]
- **`ServerlessApiGatewayHttpApi`** — la opción más rentable. Elige esta cuando solo necesites queries y mutations y no necesites [WAF](https://aws.amazon.com/waf/) en el nivel de API. Las [HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html) son más baratas y de menor latencia que las REST APIs, pero no pueden transmitir respuestas (por lo que no hay [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions)).
- **`ServerlessApiGatewayRestApi`** (predeterminado) — elige esta cuando necesites [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) sobre [Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events) y/o un [WAF Web ACL](https://aws.amazon.com/waf/) en la API. Las [REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-rest-api.html) cuestan más que las HTTP APIs pero soportan streaming de respuestas e integración con WAF.
- **`BedrockAgentCoreRuntimeWebSocket`** — elige esta cuando necesites [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) sobre un WebSocket para streaming en tiempo real sin el timeout de 30 segundos de Lambda o API Gateway. [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) aloja tu servidor tRPC como un servidor WebSocket en contenedor con autenticación gestionada, afinidad de sesión y escalado.

Para una comparación más profunda de REST vs HTTP API Gateway, consulta la [Guía del Desarrollador de API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html).
:::
...
La opción `integrationPattern` tiene como valor predeterminado `isolated`, que crea un Lambda por procedimiento tRPC. Selecciona `shared` si prefieres un único manejador Lambda compartido para toda la API, con opciones de sobrescritura por procedimiento.
:::
</OptionFilter>
...
<FileTree>
  - src
    ...
    - handler.ts Punto de entrada del manejador Lambda (solo tipos de cómputo Lambda)
    - handler-agentcore.ts Punto de entrada del servidor WebSocket (solo tipo de cómputo AgentCore)
    ...
    - schema Definiciones de esquemas usando Zod
      ...
      - z-async-iterable.ts Helper de Zod para subscripciones (solo REST API y AgentCore)
  ...
  - Dockerfile Imagen de contenedor para despliegues AgentCore (solo tipo de cómputo AgentCore)
...
### Subscripciones (Streaming)

Las subscripciones de tRPC transmiten datos del servidor al cliente. El generador configura automáticamente el helper de esquema `ZodAsyncIterable` y el manejador para respuestas de streaming.

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Las subscripciones usan [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). La infraestructura generada usa un manejador Lambda de streaming con `ResponseTransferMode.STREAM` en API Gateway para todas las operaciones de REST API, lo que permite que las subscripciones funcionen junto con queries y mutations regulares.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Las subscripciones usan [WebSockets](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) contra el endpoint del runtime de AgentCore. El manejador generado expone `/ws` vía `@trpc/server/adapters/ws`, que transporta queries, mutations y subscripciones sobre una única conexión full-duplex sin el timeout de 30 segundos de Lambda o API Gateway.
</TabItem>
</Tabs>
...
Cuando se despliega en AgentCore, cada procedimiento recibe `ctx.sessionId` — el valor del encabezado `x-amzn-bedrock-agentcore-runtime-session-id` que AgentCore establece en la actualización del WebSocket. Úsalo para asociar estado por sesión:
...
export const chat = publicProcedure
  ...
  .subscription(async function* (opts) {
    ...
    // ...transmite fragmentos que se construyen sobre `history`
  });
```

Localmente, `nx serve` ejecuta un servidor HTTP + WebSocket standalone para que los clientes puedan conectarse de la misma manera que lo hacen en producción. El ID de sesión tiene como valor predeterminado un UUID aleatorio cuando el encabezado no está presente.
</OptionFilter>
...
En AgentCore, el runtime ya ha aplicado la autenticación `IAM` o `Cognito` en la actualización del WebSocket antes de que tu manejador vea cualquier tráfico. El contexto generado expone:

- `ctx.sessionId` — el valor del encabezado `x-amzn-bedrock-agentcore-runtime-session-id` establecido por AgentCore en la actualización del WebSocket.

Puedes leer `ctx.sessionId` directamente en cualquier procedimiento — no se requiere middleware adicional:
...
Si deseas exponer campos adicionales (por ejemplo, claims JWT personalizadas reenviadas a través de encabezados) a los procedimientos, agrega middleware que aumente `ctx` en el mismo patrón que los ejemplos de Lambda anteriores — la única diferencia es que la fuente de verdad son los encabezados de actualización del WebSocket, no un evento de API Gateway.
</OptionFilter>
...
El módulo de API almacena temporalmente su zip de despliegue de Lambda en un bucket S3 de activos compartido — consulta la <Link path="/guides/terraform-project">guía de infraestructura de Terraform</Link> para más detalles. Instancia el módulo `core/asset-bucket` una vez por despliegue y pasa su salida `bucket_name` a cada módulo de API / Lambda a través de la entrada `asset_bucket_name`:
...
:::caution[Sin autenticación]
Puedes ver fallos en los checks de Checkov como:
...
El generador emite un constructo CDK (o módulo Terraform) que aprovisiona un `Runtime` de Bedrock AgentCore, le otorga permiso para invocar modelos de Bedrock y leer la configuración del runtime, y publica el ARN del runtime en `runtimeConfig.apis.<YourApiName>` para que el frontend lo recoja. Un objetivo de build `docker` está conectado al objetivo `build` del proyecto para que `nx build` produzca la imagen de contenedor.
...
El constructo aprovisiona el runtime de AgentCore desde la imagen de contenedor construida por el objetivo `docker` y expone el ARN del runtime vía `api.agentCoreRuntime.agentRuntimeArn`.
</Fragment>
<Fragment slot="terraform">
```hcl
module "my_api" {
...
El módulo aprovisiona el runtime de AgentCore desde la imagen de contenedor construida por el objetivo `docker` y publica el ARN del runtime vía la salida `agent_runtime_arn`.
</Fragment>
</Infrastructure>
</OptionFilter>
...
:::tip[Integraciones CDK con tipos seguros]
Si seleccionaste CDK para tu `iacProvider`, cuando agregues o quites un procedimiento en tu API tRPC, estos cambios se reflejarán inmediatamente en el constructo CDK sin necesidad de reconstruir.
:::
</OptionFilter>
...
Si estás llamando a tu API desde un sitio web React, considera usar el generador <Link path="guides/connection/react-trpc">Connection</Link> para configurar el cliente.
