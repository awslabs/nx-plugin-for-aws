...
:::tip
Se recomienda especificar modelos de respuesta para tus operaciones API para una mejor generación de código si usas el generador `connection`. <Link path="guides/connection/react-fastapi">Consulta aquí para más detalles</Link>.
:::
...
- Añadir la extensión de proveedor OpenAPI `x-streaming: true` si planeas usar <Link path="guides/connection/react-fastapi">API Connection</Link>.
...
Para consumir un stream de respuestas, puedes usar el <Link path="guides/connection/react-fastapi#consuming-a-stream">generador API Connection</Link> que provee un método tipado para iterar sobre los fragmentos transmitidos.
