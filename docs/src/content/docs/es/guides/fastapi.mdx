...
Para invocar tu API desde un sitio React, puedes usar el <Link path="guides/connection/react-fastapi">generador `connection`</Link>./react-fastapi">generador `connection`</Link>.