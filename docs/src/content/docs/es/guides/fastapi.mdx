...
Se recopilan métricas de CloudWatch automáticamente por cada solicitud. Puedes añadir métricas personalizadas:
...
:::tip
Se recomienda especificar modelos de respuesta para tus operaciones API para mejor generación de código si usas el generador `api-connection`. <Link path="guides/api-connection/react-fastapi#errors">Ver más detalles aquí</Link>.
:::
...
Dado que AWS API Gateway no admite respuestas en streaming, necesitarás desplegar tu FastAPI en una plataforma que lo soporte. La opción más simple es usar una Function URL de AWS Lambda.

<Infrastructure>
<Fragment slot="cdk">
Para lograrlo, puedes reemplazar el constructo generado `common/constructs/src/app/apis/<name>-api.ts` por uno que despliegue una Function URL.
...
Para lograrlo con Terraform, puedes reemplazar la infraestructura de API Gateway generada por una Function URL de Lambda que soporte streaming.
...
# Function URL con soporte para streaming
...
resource "aws_iam_policy" "my_api_invoke_policy" {
  ...
  description = "Política para invocar la Function URL de FastAPI"
...
Para consumir un stream de respuestas, puedes usar el <Link path="guides/api-connection/react-fastapi#consuming-a-stream">Generador de API Connection</Link> que provee un método tipado para iterar sobre los fragmentos.

## Despliegue de tu FastAPI

El generador crea código de infraestructura CDK o Terraform según tu selección de `iacProvider`. Puedes usarlo para desplegar tu FastAPI.
...
1. Función AWS Lambda por cada operación de la API
...
:::note
Si seleccionaste autenticación con `Cognito`, debes proveer la propiedad `identity`:
...
Puedes configurar User Pool y Client usando recursos o módulos Terraform.
:::
...
Puedes personalizar CORS con variables:
...
<Infrastructure>
<Fragment slot="cdk">
Dado que las operaciones en FastAPI se definen en Python y la infraestructura CDK en TypeScript, instrumentamos generación de código para proveer metadatos al constructo CDK y ofrecer una interfaz tipada.
...
:::note
Debes ejecutar un build tras cambios en tu API para actualizar los tipos en CDK.
...
Si seleccionaste autenticación `IAM`, puedes usar el método `grantInvokeAccess` para conceder acceso:
...
Outputs clave del módulo para políticas IAM:
