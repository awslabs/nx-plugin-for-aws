...
<Infrastructure>
<Fragment slot="cdk">
Para esto, puedes reemplazar el constructo generado `common/constructs/src/app/apis/<name>-api.ts` por uno que despliegue una URL de función.
...
Para esto con Terraform, puedes reemplazar la infraestructura generada de API Gateway por una Lambda Function URL que soporte streaming de respuestas.
...
- Añadir la extensión de proveedor OpenAPI `x-streaming: true` si planeas usar el generador <Link path="guides/connection/react-fastapi">Connection</Link>.
...
Para invocar tu API desde un sitio React, puedes usar el generador <Link path="guides/connection/react-fastapi">`connection`</Link>.