...
:::caution[Fija `boto3` a la versión anterior]
Otros generadores agregan `boto3` fijado exactamente a esta versión. Dado que tu espacio de trabajo comparte un único entorno virtual UV y archivo de bloqueo, agregarlo sin fijar (`add boto3`) resuelve la última versión y escribe una restricción `boto3>=<latest>`, que `uv` no puede satisfacer junto con la fijación. Todos los objetivos de Python en el espacio de trabajo fallan entonces al resolver dependencias.
:::
