...
- \<module_name>
  ...
  - init.py Configura la aplicación FastAPI y middleware de powertools
...
El generador también creará infraestructura como código con CDK o Terraform para desplegar tu API, ubicada en el directorio `packages/common`.
...
El generador configura logging estructurado usando AWS Lambda Powertools. Accede al logger en tus handlers:
...
El trazado con AWS X-Ray se configura automáticamente. Añade subsegmentos personalizados:
...
Métricas de CloudWatch se recolectan automáticamente. Añade métricas personalizadas:
...
:::tip
Se recomienda especificar modelos de respuesta para las operaciones de la API si usas el generador `api-connection`. <Link path="guides/api-connection/react-fastapi#errors">Ver detalles aquí</Link>.
:::
...
Como API Gateway de AWS no soporta streaming, necesitas desplegar tu FastAPI en una plataforma que lo soporte. La opción más simple es usar una URL de función Lambda.

<Infrastructure>
<Fragment slot="cdk">
Para esto, reemplaza el constructo generado `common/constructs/src/app/apis/<name>-api.ts` por uno que use Function URL.

<details>
<summary>Ejemplo de Constructo FunctionURL con Streaming</summary>
...
Para Terraform, reemplaza la infraestructura de API Gateway por una Function URL con streaming.
...
resource "aws_lambda_function" "my_api_handler" {
  ...
  # Variables para Lambda Web Adapter
  environment {
    variables = {
      AWS_CONNECTION_REUSE_ENABLED = "1"
      PORT                        = "8000"
...
# Opcional: Parámetro SSM para configuración
...
# Política IAM para acceso de invocación
resource "aws_iam_policy" "my_api_invoke_policy" {
  name        = "my-api-invoke-policy"
  description = "Política para invocar Function URL"
...
# Ejemplo: Adjuntar política a un rol
...
- Declarar el tipo de cada chunk
...
Para consumir streams, usa el <Link path="guides/api-connection/react-fastapi#consuming-a-stream">Generador API Connection</Link>.

## Desplegar tu FastAPI

El generador crea infraestructura CDK o Terraform según tu `iacProvider`. Úsala para desplegar:

<Infrastructure>
<Fragment slot="cdk">
El constructo CDK en `common/constructs` se usa en aplicaciones CDK:
...
5. Configuración X-Ray
6. Namespace de métricas CloudWatch

:::note
Si usas autenticación Cognito, provee la propiedad `identity`:
...
<Fragment slot="terraform">
Los módulos Terraform en `common/terraform` se usan en configuraciones:
...
1. Función Lambda para todas las rutas
2. API Gateway HTTP/REST
3. Roles IAM
4. Grupo de logs
5. Configuración X-Ray
...
:::note
Para Cognito, provee la configuración:
...
Configura Cognito con recursos Terraform.
:::
...
:::caution
Si seleccionaste `None` en `auth`, Checkov puede mostrar errores:
...
Puedes [añadir comentarios de supresión](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html) si deseas una API pública.
:::
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
Como las operaciones se definen en Python y CDK en TypeScript, generamos código para proveer metadatos al constructo CDK.

El target `generate:<ApiName>-metadata` en `project.json` genera `packages/common/constructs/src/generated/my-api/metadata.gen.ts`, ignorado en control de versiones.

:::note
Ejecuta un build tras cambios en la API:

<NxCommands commands={["run-many --target build --all"]} />
:::

:::tip
Usa [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar tipos automáticamente:
...
<Fragment slot="terraform">
:::note
No hay soporte para integraciones type-safe en Terraform.
:::
</Fragment>
</Infrastructure>

### Otorgar Acceso (Solo IAM)

Para autenticación IAM, usa `grantInvokeAccess`:
...
Salidas clave:
- `module.my_api.api_execution_arn` - Permisos execute-api:Invoke
- `module.my_api.api_arn` - ARN de API Gateway
- `module.my_api.lambda_function_arn` - ARN de Lambda
</Fragment>
</Infrastructure>
...
Para invocar desde un sitio React, usa el <Link path="guides/api-connection/react-fastapi">generador `api-connection`</Link>.