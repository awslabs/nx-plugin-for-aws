...
Para autenticación `IAM`, buscamos al llamador en Cognito usando el sub extraído del contexto de solicitud de API Gateway, lo cual requiere `boto3`. Agrégalo a tu proyecto de API:
...
El constructo `UserIdentity` se puede generar usando el <Link path="/es/guides/react-website-auth">generador `ts#website#auth`</Link>
:::
</OptionFilter>
...
El módulo de API almacena su zip de despliegue de Lambda en un bucket S3 de activos compartido — consulta la <Link path="/es/guides/terraform-project">guía de infraestructura de Terraform</Link> para más detalles. Instancia el módulo `core/asset-bucket` una vez por despliegue y pasa su salida `bucket_name` a cada módulo de API / Lambda a través de la entrada `asset_bucket_name`:
...
`nx watch` requiere el [demonio Nx](https://nx.dev/concepts/nx-daemon), que está habilitado por defecto pero se desactiva dentro de CI, contenedores Docker y entornos aislados. Donde está desactivado, el comando sale con `Daemon is not running` — establece `NX_DAEMON=true` para habilitarlo.
:::
</Fragment>
<Fragment slot="terraform">
...
Los nombres de proyectos Python usan guiones bajos, por lo que una API generada con `--name=MyApi` se direcciona como `my_api` (o por su nombre completo, `<scope>.my_api`). El puerto en el que escucha el servidor se asigna por proyecto — léelo del objetivo `serve` en el `project.json` de tu proyecto.
:::
