...
Excepciones no capturadas son manejadas por el middleware y:
...
Como API Gateway de AWS no soporta streaming, necesitarás desplegar en una plataforma que lo soporte. La opción más simple es usar Function URL de AWS Lambda. Para esto, reemplaza el construct generado `common/constructs/src/app/apis/<name>-api.ts` con uno que use function URLs.
...
Para consumir streams, usa el <Link path="guides/api-connection/react-fastapi#consuming-a-stream">Generador API Connection</Link> que provee métodos type-safe para iterar chunks.
...
1. Función Lambda por cada operación de la API
...
El construct `UserIdentity` se genera con el <Link path="/guides/react-website-auth">generador `ts#react-website-auth`</Link>
:::
...
Como las operaciones se definen en Python y la infraestructura en TypeScript, usamos generación de código para proveer metadata type-safe al construct CDK.

Se añade un target `generate:<ApiName>-metadata` en `project.json` para generar archivos como `packages/common/constructs/src/generated/my-api/metadata.gen.ts`. Este archivo se ignora en control de versiones.
...
:::tip
Para regenerar tipos automáticamente durante el desarrollo, usa [`nx watch`](https://nx.dev/nx-api/nx/documents/watch):
...
Para invocar la API desde un sitio React, usa el <Link path="guides/api-connection/react-fastapi">generador `api-connection`</Link>.