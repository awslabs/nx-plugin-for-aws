...
Para invocar tu Strands Agent desde un sitio web React, puedes hacer uso del <Link path="/guides/connection/react-ts-agent">generador `connection`</Link>, que configura automáticamente un cliente tRPC WebSocket con la autenticación correcta (IAM o Cognito).
