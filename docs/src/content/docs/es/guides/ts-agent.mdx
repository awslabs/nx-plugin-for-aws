...
El generador configura un target Nx `<your-agent-name>-chat` que te lleva a un chat interactivo de terminal con tu agente, impulsado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli).

El target de chat se ejecuta de forma independiente — **no** inicia el agente por ti. Por defecto se conecta a tu agente ejecutándose localmente, así que primero inicia `<your-agent-name>-serve-local` (en una terminal separada):
