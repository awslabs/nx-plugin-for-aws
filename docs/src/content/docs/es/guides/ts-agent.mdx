...
También se genera un target `<your-agent-name>-serve`, que ejecuta el agente contra tu infraestructura desplegada y por lo tanto requiere que se establezca `RUNTIME_CONFIG_APP_ID`. Consulta la guía de <Link path="guides/local-development">Desarrollo local</Link> para conocer la diferencia entre `dev` y `serve`.
