...
Por defecto, los agentes Strands usan Claude Sonnet 4.6 en Amazon Bedrock, pero puedes cambiar fácilmente entre proveedores de modelos:
...
La mayoría de los usuarios no necesitarán modificar este archivo — edita `agent.ts` para cambiar herramientas o el prompt del sistema. Los agentes A2A escuchan en el puerto `9000` (vs `8080` para HTTP), para lo cual la infraestructura generada ya está configurada.
</OptionFilter>
...
La mayoría de los usuarios no necesitarán modificar `index.ts` — edita `agent.ts` para cambiar herramientas o el prompt del sistema. Los agentes AG-UI escuchan en el puerto `8080` (igual que HTTP), para lo cual la infraestructura generada ya está configurada.
</OptionFilter>
...
Esto usa `tsx --watch` para reiniciar automáticamente el servidor cuando los archivos cambian. El agente estará disponible en `http://localhost:8081` (o el puerto asignado si tienes múltiples agentes — léelo de `metadata.ports` en el `project.json` del proyecto).
...
Luego invócalo usando el método de fábrica `.local` de la fábrica de cliente.

Puedes, por ejemplo, crear un archivo llamado `scripts/test.ts` en tu workspace que importe el cliente:

La clase del cliente se nombra según tu agente, por lo que un agente llamado `my-agent` exporta `MyAgentClient`.
...
Sustituye el puerto asignado a tu agente — léelo de `metadata.ports` en el `project.json` del proyecto.
...
Usa `<your-agent-name>-dev` para esto, no `<your-agent-name>-serve`. `-serve` ejecuta la ruta de código desplegado, por lo que necesita `RUNTIME_CONFIG_APP_ID` establecido — consulta <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Sin él, cada invocación falla con `RUNTIME_CONFIG_APP_ID is not set`.
:::
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'es'}/guides/connection/ts-agent-mcp`}
    source="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'es'}/guides/connection/ts-agent-rdb`}
    source="strands"
    sourceBadge="typescript"
    target="aurora"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'es'}/guides/connection/ts-agent-dynamodb`}
    source="strands"
    sourceBadge="typescript"
    target="dynamodb"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'es'}/guides/connection/ts-agent-gateway`}
    source="strands"
    sourceBadge="typescript"
    target="agentcore"
  />
