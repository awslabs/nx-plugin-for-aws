...
El modelo `InvokeInput` por defecto acepta un prompt.
...
Dado que el generador provee infraestructura CDK o Terraform que gestiona el despliegue de tu agente, no necesitas utilizar el `bedrock-agentcore-starter-toolkit` que mencionan los documentos para desplegar tu agente.
:::
...
- **Strands**: envuelve tu `Agent` en un `ag_ui_strands.StrandsAgent`, construido dentro de un manejador `lifespan` de FastAPI (para que la construcción ocurra al inicio del contenedor/sesión en lugar de en el momento de importación), y servido desde un bucle FastAPI `/invocations` hecho a mano.
- **LangChain**: envuelve el grafo compilado en un `ag_ui_langgraph.LangGraphAgent`, construido de la misma manera dentro de `lifespan`, y servido desde un bucle FastAPI `/invocations` hecho a mano.
...
Para construir tu Agente para Bedrock AgentCore Runtime, se añade un target `bundle` a tu proyecto, que:
...
Tu agente se configura automáticamente con observabilidad usando el [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT), configurando auto-instrumentación en tu `Dockerfile`.
...
## Asegurando tu Agente

<Snippet name="agent/securing-your-agent" parentHeading="Asegurando tu Agente" />
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Coloca un agent detrás de un AgentCore Gateway como destino de ejecución"
