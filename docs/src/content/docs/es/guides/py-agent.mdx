...
Si seleccionaste `None` para `computeType`, no se generan constructos CDK ni módulos Terraform — el Agente solo puede ejecutarse localmente. La opción `auth` se ignora en este modo ya que no hay un endpoint alojado para autenticar.
</OptionFilter>
