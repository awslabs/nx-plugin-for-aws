...
Primero usa el generador <Link path="/guides/python-project">`py#project`</Link> para crear un proyecto al cual agregar tu Agent.
:::
...
- **HTTP** (predeterminado): Un servidor FastAPI estándar con un endpoint personalizado `/invocations`, CORS y streaming. Mejor para integraciones de cliente personalizadas.
...
- **AG-UI**: El [protocolo AG-UI](https://docs.ag-ui.com/) sobre SSE (Strands usa `ag-ui-strands`; LangChain usa `ag-ui-langgraph`). Mejor para integración directa con frontend usando [CopilotKit](https://docs.copilotkit.ai/) en un sitio web React.

El punto de entrada del servidor difiere según el framework (Strands produce un `Agent` gestionado por contexto, mientras que LangChain maneja un grafo `create_agent` compilado), pero el contrato externo para cada protocolo es el mismo.
...
El `main.py` generado expone un único endpoint POST que transmite eventos [AG-UI](https://docs.ag-ui.com/) sobre Server-Sent Events (SSE), así como `/ping` para la verificación de salud del runtime AgentCore. La conexión depende del framework:

- **Strands**: envuelve tu `Agent` en un `ag_ui_strands.StrandsAgent`, construido dentro de un manejador `lifespan` de FastAPI (para que la construcción ocurra al inicio del contenedor/sesión en lugar del tiempo de importación), y servido desde un bucle `/invocations` de FastAPI hecho a mano.
...
Esto usa `uv run` para ejecutar tu Agent usando el [Bedrock AgentCore Python SDK](https://github.com/aws/bedrock-agentcore-sdk-python).
...
Para construir tu Agent para Bedrock AgentCore Runtime, se agrega un target `bundle` a tu proyecto, que:
...
También se agrega un target `docker` específico para tu Agent, que copia el `Dockerfile` y los artefactos empaquetados en un directorio de contexto docker. Esto coloca el `Dockerfile` junto con la salida construida, permitiendo que CDK construya la imagen Docker directamente usando `AgentRuntimeArtifact.fromAsset`.
...
Tu agente se configura automáticamente con observabilidad usando el [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT), configurando la auto-instrumentación en tu `Dockerfile`.
...
Para invocar un Agent ejecutándose localmente a través del target `<your-agent-name>-serve`, puedes enviar una solicitud POST simple a `/invocations` en el puerto en el que se está ejecutando tu agente local. Por ejemplo, con `curl`:
...
Para invocar tu Agent desde un sitio web React, puedes hacer uso del <Link path="/guides/connection/react-py-agent">generador `connection`</Link>, que configura automáticamente un cliente con la autenticación correcta (IAM o Cognito).
