...
Para agentes Strands, el generador usa [FastAPI](https://fastapi.tiangolo.com/) para exponer un servidor HTTP por defecto. Alternativamente, puedes elegir el protocolo [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) para interoperabilidad con otros agentes compatibles con A2A, o el protocolo [AG-UI](https://docs.ag-ui.com/) para integración directa con el frontend mediante [CopilotKit](https://docs.copilotkit.ai/).

:::note[LangChain solo soporta AG-UI]
El framework `langchain` actualmente solo soporta el protocolo `ag-ui`. Elige el framework predeterminado `strands` si necesitas el protocolo `http` o `a2a`. Los agentes LangChain aún pueden conectarse a servidores MCP, AgentCore Gateways y agentes A2A mediante el <Link path="/guides/connection/py-agent-mcp">generador `connection`</Link>.
:::
...
Los ejemplos a continuación usan Strands. Un agente LangChain funciona de la misma manera, usando `create_agent` con el decorador `@tool` de `langchain_core.tools` y un modelo `ChatBedrockConverse` (las variables de entorno `MODEL_ID` y `AWS_REGION` establecen el modelo y la región). Consulta la [documentación de LangChain](https://docs.langchain.com/oss/python/langchain/overview) para más detalles.
...
Para consumir servidores MCP creados con los generadores <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> o <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> puedes hacer uso del <Link path="/guides/connection/py-agent-mcp">generador `connection`</Link>.
...
Para una guía detallada sobre cómo escribir agentes Strands, consulta la [documentación de Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Para agentes LangChain, el <Link path="/guides/connection/py-agent-mcp">generador `connection`</Link> también conecta herramientas de servidores MCP, AgentCore Gateway y agentes A2A en `create_agent`.
