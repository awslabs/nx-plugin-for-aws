...
El generador configura un target Nx `<your-agent-name>-chat` que te lleva a un chat interactivo en la terminal con tu agente, impulsado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli).
...
Para invocar tu Agente Strands desde un sitio web React, puedes hacer uso del <Link path="/guides/connection/react-py-agent">generador `connection`</Link>, que configura automáticamente un cliente con la autenticación correcta (IAM o Cognito).
