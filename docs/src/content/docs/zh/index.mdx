---
title: "@aws/nx-plugin"
description: 在 AWS 上快速搭建和构建应用程序，让您更接近生产环境。
template: splash
hero:
  tagline: 在几分钟内构建全栈 AWS 应用，而不是几天。
  image:
    light: ../assets/bulb-black.svg
    dark: ../assets/bulb-white.svg
  actions:
    - text: 开始使用
      link: get_started/quick-start
      icon: right-arrow
      variant: primary
    - text: 使用 AI 构建
      link: get_started/building-with-ai
      icon: rocket
      variant: secondary
---

import {
  CardGrid,
  Card,
  Icon,
  LinkButton,
} from "@astrojs/starlight/components";
import { ContributorList } from "astro-contributors";
import CopyCommand from "@components/copy-command.astro";
import GeneratorCarousel from "@components/generator-carousel.astro";

<div class="landing-section landing-hero-extras">
  <p class="landing-subline">
    一套用于在 AWS 上快速构建云原生应用程序的 Nx 生成器集合——类型安全、可本地运行且随时可部署。
  </p>

  <div class="landing-install">
    <span class="landing-install-label">创建新工作区</span>
    <CopyCommand command="pnpm create @aws/nx-workspace" />
  </div>
</div>

<div class="landing-section landing-carousel-section">
  <h2 class="landing-heading">搭建任何内容。将它们连接在一起。</h2>
  <p class="landing-lede">
    每个生成器都会生成最佳实践的应用程序代码<em>以及</em>用于部署它的基础设施，让您更接近生产环境。以交互方式运行它们——系统会提示您输入所需的值。
  </p>
  <GeneratorCarousel />
</div>

<div class="landing-section">
  <h2 class="landing-heading">为什么选择 @aws/nx-plugin？</h2>
  <CardGrid>
    <Card title="几分钟内完成全栈开发" icon="puzzle">
      网站、API、身份验证、代理和基础设施——全部搭建完成并连接在一起。
    </Card>
    <Card title="使用 AI 构建" icon="rocket">
      插件附带 MCP 服务器，因此您的 AI 助手可以为您搭建和连接项目。
    </Card>
    <Card title="端到端类型安全" icon="seti:json">
      共享类型从您的 API 流向前端。放心重构。
    </Card>
    <Card title="您的代码，您的方式" icon="setting">
      生成器为您提供一个良好的开端，而不是一个框架。代码属于您，可以阅读、修改和扩展。
    </Card>
  </CardGrid>
</div>

<div class="landing-section landing-community">
  <h2 class="landing-heading">加入社区</h2>
  <p>有问题或想分享反馈？</p>
  <p>
    加入我们的{" "}
    <a href="https://cdk-dev.slack.com/archives/C0AG11EUHM4">
      <strong>#nx-plugin-for-aws</strong>
    </a>{" "}
    Slack 频道，与其他用户和贡献者交流。
  </p>
  <LinkButton
    href="https://cdk-dev.slack.com/archives/C0AG11EUHM4"
    icon="slack"
    variant="secondary"
  >
    在 Slack 上加入我们
  </LinkButton>
</div>

<div class="landing-section">
  <h2 class="landing-heading">感谢我们出色的贡献者！</h2>
  <ContributorList githubRepo="awslabs/nx-plugin-for-aws" />
</div>
