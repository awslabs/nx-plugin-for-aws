...
Nx AWS 插件包含一个 [Skill](https://code.claude.com/docs/en/skills)/[Power](https://kiro.dev/docs/powers/) 和一个 [MCP Server](https://modelcontextprotocol.io/introduction)，它们共同使 AI 助手能够有效地使用该插件。通过使用这些工具，您可以通过 AI 加速开发工作流程，同时受益于以确定性方式搭建项目——在设置主要组件时花费更少的时间和更少的上下文窗口。
