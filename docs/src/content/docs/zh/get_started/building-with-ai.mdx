...
<Tabs>
  <TabItem label="Kiro">
    如果您尚未安装上面的 [Kiro Power](#kiro-power)，可以在 `.kiro/mcp.json` 中手动添加 MCP 服务器：
  ...
  <TabItem label="Codex">
    ...
    详情请参阅 [Codex 文档](https://github.com/openai/codex)。
  </TabItem>
  <TabItem label="Other">
    ...
    如果遇到 `ENOENT npx` 等问题，请将命令替换为 `/full/path/to/npx`（使用 `which npx` 查找路径）。
