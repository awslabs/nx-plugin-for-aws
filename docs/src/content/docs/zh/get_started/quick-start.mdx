...
:::note[保留的资源]
Cognito 用户池在创建时启用了删除保护,因此它不会被自动删除(destroy 命令将报告失败)。如果您不再需要它(以及 Cognito 用于发送短信的 IAM 角色),请手动删除它们。
:::
</Fragment>
</Infrastructure>
...
这会清空并删除保存您的 Terraform 状态的 S3 存储桶,因此仅在您拆除所有已部署的基础设施后才运行它。
:::
</Fragment>
</Infrastructure>
