...
1. 在您的 AI 编码助手中打开工作空间。
    ...
    新工作空间已经为 Kiro、Claude Code、Cursor、Gemini CLI、GitHub Copilot 和 OpenAI Codex 配置了插件的 <Link path="get_started/building-with-ai">MCP server</Link>,因此您的助手可以查找插件的生成器并运行它们,无需任何设置。
...
根据您正在构建的项目类型,您可以选择任意生成器组合来快速启动您的项目。查看左侧导航栏中的 __生成器__ 以查看完整的选项列表,或尝试使用 <Link path="get_started/graph-builder">图形构建器</Link> 以可视化方式构建您的工作空间。
...
<Drawer title="从您的网站调用 API" trigger="试试看:从您的网站调用 API">

将网站连接到 API 后会添加一个 `useDemoApi` 钩子,它返回一个 [tRPC options proxy](https://trpc.io/docs/client/tanstack-react-query):您的 API 上的每个过程都提供 `queryOptions` 和 `mutationOptions` 以传递给 [TanStack Query](https://tanstack.com/query/latest)。
...
请参阅 <Link path="guides/connection/react-trpc">React 到 tRPC 连接指南</Link>了解变更、订阅、错误处理和乐观更新。
...
:::note[根模块输出]
Terraform 仅打印在 `packages/infra/src` 中声明的输出,因此您希望在部署后读取的任何内容都必须从生成它的模块重新导出——因此上面有两个 `output` 块。
:::
</Fragment>
</Infrastructure>
...
:::tip[引导区域]
CDK 引导对于您的目标部署 AWS 区域和 `us-east-1` 都是必需的,以允许为网站部署 AWS WAF WebACL。如果您要部署到其他地方,也请引导 `us-east-1`:
...
要登录,您需要[在 Cognito 用户池中创建用户](https://docs.aws.amazon.com/cognito/latest/developerguide/how-to-create-user-accounts.html)。

<Drawer title="使用 AWS CLI 创建用户" trigger="点击此处查看使用 AWS CLI 的说明">

以下命令在用户池中创建用户。您可以在部署输出中找到用户池 ID,或在 [Amazon Cognito 控制台](https://console.aws.amazon.com/cognito/)中找到。替换用户名、电子邮件、名字和姓氏以及临时密码的值。
...
您的网站也通过 CloudFront 提供服务。打开部署输出中打印的分发域名——对于 CDK 是以 `DistributionDomainName` 结尾的输出,对于 Terraform 是 `website_url`——以使用完全在 AWS 上运行的版本。
...
Cognito 用户池在创建时启用了删除保护,因此它在上述命令后仍然存在。在删除它之前,请在 [Amazon Cognito 控制台](https://console.aws.amazon.com/cognito/)中关闭删除保护。

<Infrastructure>
<Fragment slot="cdk">
:::note[保留的资源]
删除堆栈会留下用户池,以及 Cognito 用于发送短信的 IAM 角色、API Gateway 用于写入 CloudWatch Logs 的 IAM 角色、加密网站存储桶和 API 及 WAF 日志组的 KMS 密钥,以及日志组本身。一旦您不再需要它们,请删除池、角色和日志组,并安排密钥删除。
:::
</Fragment>
<Fragment slot="terraform">
:::caution[删除保护关闭前销毁会失败]
Terraform 会尝试删除用户池,因此 `destroy` 会因 `The user pool cannot be deleted because deletion protection is activated` 而停止,并将尚未到达的资源留在原处。关闭删除保护,然后再次运行销毁命令以删除所有内容——包括用户池和 Cognito 用于发送短信的 IAM 角色。
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/tutorials/dungeon-game/overview">Dungeon Adventure 教程</Link> — 构建一个全栈 AI 地牢冒险游戏,获得比本指南更深入的演练。
- <Link path="guides/workspace">工作空间</Link> — 了解工作空间的布局以及生成器共享的配置。
- <Link path="get_started/existing-project">添加到现有项目</Link> — 在您已有的 Nx 工作空间或 monorepo 中采用该插件。
