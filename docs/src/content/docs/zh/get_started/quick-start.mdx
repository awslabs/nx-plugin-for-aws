...
:::note[保留的资源]
Cognito 用户池在创建时启用了删除保护,因此它不会被自动删除。如果您不再需要它(以及 Cognito 用于发送短信的 IAM 角色),请手动删除它们。
:::

### 删除引导资源

您引导的资源由部署到该 AWS 账户和区域的每个项目共享,保留它们的成本几乎为零。仅在没有其他项目部署到那里时才删除它们。
...
资产存储桶是有意保留的,因此需要单独删除。它启用了版本控制,`aws s3 rb --force` 无法处理,因此请先删除每个版本和删除标记:
...
当没有该类型的内容可删除时,每个 `delete-objects` 会报告错误而不是什么都不做,这可以安全地忽略。
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[您的 Terraform 状态也会被删除]
这会清空并删除保存您的 Terraform 状态的 S3 存储桶,因此仅在您关心的内容都不再从该状态部署后才运行它。
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">添加到现有项目</Link> — 在您现有的代码库中采用该插件。
