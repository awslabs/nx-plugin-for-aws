...
:::tip
系统会提示选择基础设施即代码（IaC）提供商，可选 [CDK](https://docs.aws.amazon.com/cdk/) 或 [Terraform](https://developer.hashicorp.com/terraform)。您可以通过添加 `--iacProvider` 参数跳过提示：
...
本快速入门指南将添加 tRPC API、React 网站、Cognito 身份验证和 CDK/Terraform 基础设施。根据项目类型，您可以选择任意组合的生成器快速启动项目。查看左侧导航栏的 __指南__ 查看完整选项列表。
...
这将配置一个 Terraform 项目，用于在 AWS 上部署基础设施。
</Fragment>
</Infrastructure>
...
对网站和 API 的修改将实时更新，因为本地网站和 API 服务器都会热重载。
...
:::tip
CDK 初始化需要在目标部署 AWS 区域和 `us-east-1` 区域执行，以允许部署网站使用的 AWS WAF WebACL。
:::
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['run infra:bootstrap']} />
...
:::tip
Terraform 初始化会创建用于存储 Terraform 状态文件的 S3 存储桶。每个 AWS 账户/区域组合只需执行一次。
:::
</Fragment>
</Infrastructure>
...
:::tip
该命令会先运行 `terraform plan` 显示变更计划，然后应用变更部署基础设施。
:::
</Fragment>
</Infrastructure>
...
网站将在 `http://localhost:4200` 可用，并指向已部署的 API 和身份验证资源。
