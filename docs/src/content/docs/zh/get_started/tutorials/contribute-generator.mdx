...
我们有一套"冒烟测试"，它们在全新的工作区中运行生成器并确保一切都能构建。至少，您的新生成器应该被添加到 `packages/nx-plugin/src/internal/test-matrix/generator.ts` 中的生成器矩阵中，以便冒烟测试能够执行它。

矩阵本身是一个（隐藏的）生成器，它组合了所有其他生成器，因此它随插件一起发布。这意味着每个发布的版本都携带着_它_拥有的生成器矩阵，这使得升级旧工作区的测试可以使用该版本自己的矩阵来搭建脚手架。由于它直接调用生成器而不是通过 CLI，选项会根据每个生成器的模式进行类型检查——因此新的必需选项或更改的枚举将在此处导致构建失败，而不是悄悄地降低覆盖率。
