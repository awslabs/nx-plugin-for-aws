...
<Tabs>
  <TabItem label="本地">
  代理生成器提供了一个交互式终端聊天 CLI（`agent-chat`），它使用从代理的 OpenAPI 规范生成的类型安全 TypeScript 客户端。由于我们更改了代理的输入形状以接受 `playerName`、`genre` 和 `actions`，我们需要更新生成的聊天脚本以提供这些字段。

  更新 `packages/story/scripts/agent/chat.ts`：
  ...
  然后启动与本地运行的 Story 代理的交互式聊天：
    ...
    `agent-chat` 目标会自动启动 `agent-serve-local`（它会使用 `SERVE_LOCAL=true` 启动代理和任何 MCP 服务器依赖项），并在将您带入聊天之前运行 OpenAPI 规范和客户端生成。

    :::note[`agent-serve` 与 `agent-serve-local`]
    `agent-chat` 依赖于 `agent-serve-local`，后者使用 `SERVE_LOCAL=true` 启动代理，并在本地启动任何 MCP 服务器依赖项。如果您希望代理连接到**已部署**的 MCP 服务器（通过 `RUNTIME_CONFIG_APP_ID`），请在单独的终端中运行 `agent-serve` 目标，然后通过 `curl` 针对其 `/invocations` 端点手动调用代理。
    :::
  </TabItem>
  <TabItem label="已部署">
