...
故事代理是一个通过 [AG-UI](https://ag-ui.com/) 协议公开的 [Strands](https://strandsagents.com/) 代理。AgentCore 将每个 `runtimeSessionId` 路由到专用的 microVM，因此每个容器中的单个长期存在的 `Agent` 为该会话的每个回合提供服务。当附加 AgentCore 管理的会话存储时，生成器生成的 `agent.py` 通过 `FileSessionManager` 将对话历史持久化到挂载的 `SESSION_STORAGE_PATH`，因此冒险可以在停止/恢复后继续进行。
...
在测试已部署的代理之前（以及在下一个模块中通过 UI 玩游戏之前），请创建一个 Cognito 测试用户。我们之前禁用了自助注册，因此无论您是在本地测试还是针对已部署的运行时进行测试，此步骤都是必需的：
