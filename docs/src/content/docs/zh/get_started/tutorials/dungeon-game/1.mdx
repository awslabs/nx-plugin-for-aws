...
- packages/
  - story/
    - dungeon_adventure_story/ Python 模块
      - agent/
        - main.py AG-UI Starlette 应用入口点,用于 Bedrock AgentCore Runtime
        - agent.py 定义 Strands 代理及其工具
        - session.py 捕获入站 AgentCore 运行时会话 ID
...
@contextmanager
def get_agent():
    # 当 AgentCore 管理的会话存储附加到运行时时,挂载路径通过
    # SESSION_STORAGE_PATH 暴露。通过 FileSessionManager 持久化可让代理
    # 在停止/恢复周期后恢复完整的对话历史。如果缺少环境变量(例如本地
    # serve),代理仅在内存中运行。
...
`agent.py` 定义了一个示例 Strands 代理。我们将在<Link path="get_started/tutorials/dungeon-game/3">模块 3</Link> 中用地牢冒险逻辑替换系统提示和工具。
...
class _LazyStrandsApp:
    """在首次 ``/invocations`` 请求时构建内部 Strands 应用,以便
    ``/ping`` 健康检查保持轻量级,并在任何下游 MCP / A2A 客户端读取之前
    捕获入站 AgentCore 会话 ID。"""
...
因为我们选择了 `protocol=AG-UI`,生成器将我们的 Strands 代理包装在通过 Starlette 应用暴露的 [AG-UI](https://ag-ui.com/) 适配器中。AgentCore 将每个 `runtimeSessionId` 路由到专用的 microVM,因此每个容器一个长期存活的 `Agent` 是正确的生命周期 - `main.py` 在捕获入站会话 ID 后在首次 `/invocations` 请求时延迟构建它。当附加 AgentCore 管理的会话存储时,`agent.py` 针对 `SESSION_STORAGE_PATH` 连接 `FileSessionManager`,以便代理可以在停止/恢复后恢复完整的对话历史。在本教程中我们无需修改 `main.py`。
...
这是挂载 React 的入口点。它最初只是配置了 [`@tanstack/react-router` 的文件路由](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) - 只要开发服务器运行,在 `src/routes/` 中创建文件会自动添加路由并重新生成 `routeTree.gen.ts`。这使得 `<Link to="...">` 完全类型安全。
...
导航到 `/` 路由时将渲染该组件。因为我们选择了 `uxProvider=Shadcn`,生成器还使用 [shadcn/ui](https://ui.shadcn.com/) 原语在 `components/AppLayout` 中搭建了侧边栏 + 应用外壳,这些原语共享在 `packages/common/shadcn` 下。
...
`useGameApi` 钩子无需构建即可反映变更,这得益于 tRPC 使用 [TypeScript 类型推断](https://trpc.io/docs/concepts)。这使得开发者对后端的修改能即时反映到前端。
</Aside>
...
更多详情请参考 <Link path="guides/connection/py-strands-agent-mcp">Python Strands 代理到 MCP 连接指南</Link>。
</Drawer>

### 游戏 UI:连接故事代理

连接游戏 UI 到故事代理,以便 React 网站可以通过 AG-UI 协议与代理聊天。
...
因为故事代理是使用 `protocol=AG-UI` 生成的,`connection` 生成器使用 [CopilotKit](https://copilotkit.ai/)(支持 [AG-UI](https://ag-ui.com/) 协议)将网站连接起来。这些文件被生成/更新:
...
- packages/
  - game-ui/
    - src/
      - components/
        - **AguiProvider.tsx** 注册每个 AG-UI 代理的 CopilotKitProvider
        - **copilot/** 主题化的 CopilotKit 聊天组件(助手/用户气泡、输入、光标) - 根据网站的 UX 提供者(此处为 Shadcn)自动选择
      - hooks/
        - **useAguiStoryAgent.tsx** 每个代理的钩子,构建针对 AgentCore 运行时的 Cognito 认证 AG-UI `HttpAgent`
      - **main.tsx** 更新以在应用周围添加 `<AguiProvider>`

</FileTree>

生成器会:
- 添加共享的 `AguiProvider`(如果尚不存在),将应用包装在 CopilotKit 的 `CopilotKitProvider` 中,并注册每个 AG-UI 代理的钩子
- 生成每个代理的钩子(`useAguiStoryAgent`),从 `runtime-config.json` 解析 AgentCore 运行时 ARN,构建认证的 `HttpAgent`,并在每个请求上附加 `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` 头
- 在 `src/components/copilot/` 下提供主题化的聊天原语,使聊天看起来与网站的 UX 提供者原生一致

更多详情请参考 <Link path="guides/connection/react-py-strands-agent">React 到 Python Strands 代理连接指南</Link>。
</Drawer>
...
该消息表示 NX 检测到可自动更新的文件。此处指未在引用项目上设置 TypeScript 引用的 `tsconfig.json` 文件。

选择 **Yes, sync the changes and run the tasks** 选项继续。您会注意到所有 IDE 相关导入错误自动解决,因为同步生成器会自动添加缺失的 TypeScript 引用!

<Aside type="tip" title="非交互式 shell">
如果您在非交互式 shell(CI 或无头环境)中运行,您将不会看到提示 - `build` 会立即失败并显示"The workspace is out of sync"。在这种情况下,请先手动运行 <NxCommands commands={['sync']} />,然后重新运行构建。
</Aside>
