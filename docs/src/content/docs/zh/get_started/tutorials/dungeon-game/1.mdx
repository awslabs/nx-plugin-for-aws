...
连接游戏 UI 到故事代理,以便通过类型安全的自动生成客户端从 React 网站调用代理的 API。
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** 从代理的 FastAPI 应用生成 OpenAPI 规范的脚本
    - project.json 更新添加 `agent-openapi` 目标
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** 类型安全 OpenAPI 客户端的提供者
        - **QueryClientProvider.tsx** TanStack React Query 客户端提供者(如果尚不存在)
      - hooks/
        - **useStoryAgent.tsx** TanStack Query 选项代理的钩子
        - **useStoryAgentClient.tsx** 原生 API 客户端的钩子
      - generated/
        - story-agent/ 在构建时从 OpenAPI 规范生成
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json 更新添加客户端生成目标
    - **main.tsx** 更新添加 StoryAgentProvider

</FileTree>

生成器会:
- 在 story 项目中创建代理范围的 OpenAPI 规范生成脚本
- 在构建时从代理的 OpenAPI 规范生成类型安全的 TypeScript 客户端
- 为网站添加提供者、钩子和客户端以调用代理的 API
- 配置 `serve-local` 以在本地运行时自动启动代理的本地服务器并在变更时重新生成客户端
