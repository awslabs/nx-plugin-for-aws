...
:::tip[本地优先开发]
`ts#dynamodb` 生成器提供 `dev` 目标,在容器中运行 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)。结合 `connection` 生成器(我们将在下面运行),这意味着**整个游戏在您的机器上运行,无需部署**,直到教程结束。
:::
...
生成的 `src/client.ts` 导出 `getDynamoDBClient()` 和 `resolveTableName()`。当 `LOCAL_DEV=true`(由 `dev` 目标自动设置)时,它们连接到 DynamoDB Local;否则它们连接到 AWS 并从<Link path="guides/runtime-config">运行时配置</Link>解析已部署的表名。我们将在<Link path="get_started/tutorials/dungeon-game/2">模块 2</Link> 中在此项目中建模 `Game` 和 `Inventory` 实体。
...
- packages/
  ...
  - game-ui/
    ...
    - ...
  - common/
    - shadcn/ 共享 shadcn/ui 库(主题令牌、`Button`、`Card`、`Input`、`Sidebar` 等),由每个 `ux=shadcn` 网站导入
      - src/components/ui/*
...
游戏 API 和库存 MCP 服务器都会读写我们的 DynamoDB 表,因此让我们将它们连接到 `DungeonDb` 项目。`connection` 生成器检测到目标是 `ts#dynamodb` 项目,并将每个源项目的 `dev` 目标配置为自动启动 DynamoDB Local。
...
因为故事代理的 `agent-dev` 已经依赖于库存 MCP 服务器的 `mcp-server-dev`(通过上面的 `story → inventory` 连接),而游戏 API 和 MCP 服务器现在都依赖于 `dungeon-db:dev`,运行任何一个项目的 `dev` 目标都会按正确顺序启动它所需的每个依赖项。
</Aside>
