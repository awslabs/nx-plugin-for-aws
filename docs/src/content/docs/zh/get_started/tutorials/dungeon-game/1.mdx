...
:::tip[本地优先开发]
`ts#dynamodb` 生成器提供 `serve-local` 目标,在容器中运行 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)。结合 `connection` 生成器(我们将在下面运行),这意味着**整个游戏在您的机器上运行,无需调用 AWS**,直到您在教程结束时选择部署。
:::
