...
<Aside>
根目录 `package.json` 现在配置了 `type` 为 `module`，这意味着由 `@aws/nx-plugin` 提供的所有基于 node 的子项目默认使用 ESM 模块类型。有关 TypeScript 项目的更多细节，请参考 <Link path="guides/typescript-project">ts#project 指南</Link>。
</Aside>
...
以下是 `py#strands-agent` 生成器生成的文件列表：
...
您可能注意到额外的 `Dockerfile` - 这引用了 `story` 项目的 Docker 镜像，允许我们将 Dockerfile 与代理源代码放在一起。
...
以下是 `ts#react-website` 生成器生成的文件列表。我们将重点查看文件树中高亮的关键文件：
<FileTree>
- packages/
  - common/
    - constructs/
...
这是挂载 React 的入口点。配置了 [`@tanstack/react-router` 的文件路由](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing)，只要开发服务器运行，在 `routes` 文件夹中创建文件时，`@tanstack/react-router` 会自动生成样板文件并更新 `routeTree.gen.ts` 文件。该文件以类型安全的方式维护所有路由，使用 `<Link>` 时 `to` 选项仅显示有效路由。更多信息请参考 [`@tanstack/react-router` 文档](https://tanstack.com/router/v1/docs/framework/react/quick-start)。
...
导航到 `/welcome` 路由时渲染的组件。只要开发服务器运行，`@tanstack/react-router` 会在创建/移动此文件时管理 `Route`。这将在本教程后续部分展示。
...
以下是 `ts#react-website#auth` 生成器生成/更新的文件列表。我们将重点查看文件树中高亮的关键文件：
<FileTree>
- packages/
  - common/
    - constructs/
...
通过 AST 转换在 `main.tsx` 文件中添加了 `RuntimeConfigProvider` 和 `CognitoAuth` 组件。这使得 `CognitoAuth` 组件可以通过获取包含 Cognito 连接配置的 `runtime-config.json` 来与 Amazon Cognito 进行身份验证。
...
以下是 `api-connection` 生成器生成/更新的文件列表。我们将重点查看文件树中高亮的关键文件：
<FileTree>
- packages/
  - game-ui/
    - src/
...
以下是 `ts#infra` 生成器生成/更新的文件列表。我们将重点查看文件树中高亮的关键文件：
<FileTree>
- packages/
  - common/
    - constructs/
...
<Aside type="tip">如果在 IDE 中看到导入错误，这是因为基础设施项目尚未在 tsconfig.json 中设置 TypeScript 引用。Nx 已配置为在运行构建/编译时[动态创建](https://nx.dev/nx-api/js/generators/typescript-sync)这些引用，或手动运行 `nx sync` 命令。更多信息请参考 <Link path="guides/typescript-project#importing-your-library-code-in-other-projects">TypeScript 指南</Link>。</Aside>
