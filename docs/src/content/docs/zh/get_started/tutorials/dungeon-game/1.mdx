...
- packages/
  - story/
    - dungeon_adventure_story/ Python 模块
      - agent/
        - main.py AG-UI FastAPI 应用入口点,用于 Bedrock AgentCore Runtime
...
因为我们选择了 `protocol=AG-UI`,生成器将我们的 Strands 代理包装在 [AG-UI](https://ag-ui.com/) 适配器中。`create_strands_app` 构建一个 FastAPI 应用,暴露单个 `/invocations` 端点 - 每个会话的代理实例会自动创建,对话历史在服务器生命周期内按 `threadId` 保存在内存中。在本教程中我们无需修改 `main.py`。
