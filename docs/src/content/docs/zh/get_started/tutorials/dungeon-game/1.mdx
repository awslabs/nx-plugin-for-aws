...
:::note[Shadcn UI]
我们选择 `--uxProvider=Shadcn` 使生成的网站使用 [shadcn/ui](https://ui.shadcn.com/) 组件并使用 Tailwind 样式 — 我们所有的路由代码都使用 shadcn 原语如 `Card`、`Button` 和 `Input`,稍后的 `connection` 生成器会连接匹配的 shadcn 主题 [CopilotKit](https://docs.copilotkit.ai/) 聊天界面。
:::
...
这是挂载 React 的入口点。因为我们选择了 `--uxProvider=Shadcn`,没有 Cloudscape 的 `I18nProvider` — 样式来自通过 `styles.css` 导入的 Tailwind v4 令牌。`@tanstack/react-router` 配置为[基于文件的路由](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing)模式:只要开发服务器运行,在 `routes/` 下创建的任何文件都会自动被识别并重新生成路由树。稍后的生成器(auth、connection)将通过 AST 修补此文件,用额外的提供者包装 `<App />`。
...
连接游戏 UI 到故事代理。因为代理使用 AG-UI,`connection` 生成器会连接 [CopilotKit](https://docs.copilotkit.ai/) 而不是生成 OpenAPI 客户端 — 您将获得一个主题化的聊天组件和一个准备好渲染的 `@ag-ui/client` `HttpAgent`。
...
- 将 `threadId` 填充到 AgentCore 的 33 字符最小值 — 这就是为什么我们的模块 4 路由可以安全地构建 `{player}-{genre}` 作为稳定的线程 id。
