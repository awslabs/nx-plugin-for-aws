...
我们将从创建一个新的单仓库开始。在您选择的目录下运行以下命令：

<CreateNxWorkspaceCommand workspace="dungeon-adventure" iacProvider="CDK" />

:::note
我们传递 `--iacProvider=CDK` 因为本教程中将使用 CDK 作为基础设施即代码工具。Nx AWS 插件也支持 `Terraform`。
:::

这将在 `dungeon-adventure` 目录中设置一个 NX 单仓库，您可以在 vscode 中打开它。其结构应如下所示：
...
- tsconfig.json
- aws-nx-plugin.config.mts 配置 Nx AWS 插件
</FileTree>

现在，我们已准备好使用 `@aws/nx-plugin` 开始创建不同的子项目。

<Aside type="tip">最佳实践是在运行任何生成器之前确保所有未暂存的文件已提交到 Git。这允许您通过 `git diff` 查看生成器运行后的更改。</Aside>
...
<Drawer title="ts#trpc-api 更新文件" trigger="点击此处查看这些文件的更多细节。">
  ...
  constructor(
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          // 此处授予部署账户中的任何 AWS 凭证调用 API 的权限
          // 如果需要，可以在此定义更细粒度的访问控制（如角色或用户）和资源（如哪些 API 路径可被哪些主体调用）
          new PolicyStatement({
            effect: Effect.ALLOW,
...
这是定义 GameApi 的 CDK 构造。如您所见，它提供了 `defaultIntegrations` 方法，自动为 tRPC API 中的每个过程创建 Lambda 函数，指向已打包的 API 实现。这意味着在 `cdk synth` 时不会进行打包（与使用 [NodeJsFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_lambda_nodejs.NodejsFunction.html) 不同），因为我们已经将其作为后端项目构建目标的一部分进行了打包。
...
您应该看到文件树中出现了一些新文件。
<Drawer title="py#fast-api 更新文件" trigger="点击此处查看这些文件的更多细节。">
...
这是定义 StoryApi 的 CDK 构造。如您所见，它提供了 `defaultIntegrations` 方法，自动为 FastAPI 中的每个操作创建 Lambda 函数，指向已打包的 API 实现。这意味着在 `cdk synth` 时不会进行打包（与 [PythonFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/@aws-cdk_aws-lambda-python-alpha.PythonFunction.html) 不同），因为我们已经将其作为后端项目构建目标的一部分进行了打包。
...
这是定义所有 API 方法的地方。如您所见，我们有一个映射到 `GET /` 路由的 `read_root` 方法。您可以使用 [Pydantic](https://docs.pydantic.dev/latest/) 声明方法的输入和输出以确保类型安全。
...
<Drawer title="ts#react-website 更新文件" trigger="点击此处查看这些文件的更多细节。">
...
这是定义 GameUI 的 CDK 构造。如您所见，它已配置了指向生成的 Vite UI 打包文件的路径。这意味着在 `build` 时，打包发生在 game-ui 项目的构建目标中，其输出在此处使用。
...
这是挂载 React 的入口点。如所示，它最初仅配置了基于[文件路由](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing)的 `@tanstack/react-router`。这意味着，只要开发服务器在运行，您就可以在 `routes` 文件夹中创建文件，`@tanstack/react-router` 将为您创建样板文件设置并更新 `routeTree.gen.ts` 文件。该文件以类型安全的方式维护所有路由，这意味着当您使用 `<Link>` 时，`to` 选项仅显示有效路由。更多信息请参考 [`@tanstack/react-router` 文档](https://tanstack.com/router/v1/docs/framework/react/quick-start)。
...
当导航到 `/welcome` 路由时，此组件将被渲染。只要开发服务器在运行，`@tanstack/react-router` 将在您创建/移动此文件时管理 `Route`。这将在本教程的后续部分中展示。

</Drawer>

### 游戏 UI：身份验证

现在配置我们的游戏 UI，要求通过 Amazon Cognito 进行身份验证。按照以下步骤操作：
...
您应该看到文件树中出现了一些新文件或更改。

<Drawer title="ts#react-website#auth 更新文件" trigger="点击此处查看这些文件的更多细节。">
...
通过 AST 转换，`RuntimeConfigProvider` 和 `CognitoAuth` 组件已添加到 `main.tsx` 文件中。这使得 `CognitoAuth` 组件能够通过获取包含所需 Cognito 连接配置的 `runtime-config.json` 来与 Amazon Cognito 进行身份验证，以便向后端正确目标发起调用。
...
<Drawer title="UI -> FastAPI api-connection 更新文件" trigger="点击此处查看这些文件的更多细节。">
...
此钩子可用于向 `StoryApi` 发起经过身份验证的 API 请求。如实现所示，它使用在构建时生成的 `StoryApi`，因此在构建代码之前，IDE 中会显示错误。有关客户端生成或如何消费 API 的更多细节，请参考 <Link path="guides/api-connection/react-fastapi">React 到 FastAPI 指南</Link>。
...
上述提供者组件使用 `useStoryApiClient` 钩子，并实例化 `StoryApiOptionsProxy`，用于为 TanStack Query 钩子构建选项。您可以使用相应的钩子 `useStoryApi` 访问此选项代理，它提供了一种与您的 FastAPI 交互的方式，与 tRPC API 保持一致。

由于 `useStoryApiClient` 为我们的流式 API 提供了异步迭代器，我们将在本教程中直接使用原生客户端。
...
<Drawer title="UI -> tRPC api-connection 更新文件" trigger="点击此处查看这些文件的更多细节。">
以下是 `api-connection` 生成器生成/更新的所有文件列表。我们将检查文件树中突出显示的一些关键文件：
<FileTree>
- packages/
  - game-ui/
...
<Aside>
`useGameApi` 钩子与 `useStoryApi` 钩子不同，因为它不需要构建即可反映更改，这得益于 tRPC 的 [TypeScript 推断](https://trpc.io/docs/concepts) 使用。这使得开发者对后端的更改能即时反映到前端！
</Aside>
...
<Drawer title="ts#infra 更新文件" trigger="点击此处查看这些文件的更多细节。">
...
- packages/
  ...
  - infra
    - src/
      - stages/
        - **application-stage.ts** 定义的 CDK 堆栈
      - stacks/
        - **application-stack.ts** 定义的 CDK 资源
      - index.ts
      - **main.ts** 定义所有阶段的入口点
    - cdk.json
    - project.json
...
<Aside type="tip">如果 IDE 中出现导入错误，这是因为我们的基础设施项目尚未在其 tsconfig.json 中设置 TypeScript 引用。Nx 已[配置](https://nx.dev/nx-api/js/generators/typescript-sync)为在运行构建/编译时或手动运行 `nx sync` 命令时动态创建这些引用。更多信息请参考 <Link path="guides/typescript-project#importing-your-library-code-in-other-projects">TypeScript 指南</Link>。</Aside>

这是 CDK 应用程序的入口点。

它配置为使用 [`cfn-guard`](https://github.com/cdklabs/cdk-validator-cfnguard) 根据配置的规则集运行基础设施验证。这在合成后执行。

<Aside type="tip">
在某些情况下，您可能希望抑制某些资源上的规则。您可以通过两种方式实现：
...
...
// 如果构造是 Bucket 的实例，则抑制构造或其任何子构造的 RULE_NAME
...
注意，我们为两个 API 提供了默认集成。默认情况下，API 中的每个操作都映射到一个独立的 Lambda 函数来处理该操作。
...
Nx 依赖[缓存](https://nx.dev/concepts/how-caching-works)以便重用先前构建的产物，从而加速开发。需要一些配置才能正确工作，有时您可能希望执行不使用缓存的构建。为此，只需在命令后附加 `--skip-nx-cache` 参数。例如：
...
此消息表示 NX 检测到一些可以自动更新的文件。在本例中，它指的是 `tsconfig.json` 文件，这些文件未在引用的项目上设置 TypeScript 引用。选择 **Yes, sync the changes and run the tasks** 选项继续。您应该注意到所有与 IDE 相关的导入错误都会自动解决，因为同步生成器将自动添加缺失的 TypeScript 引用！
...
如果遇到 `@dungeon-adventure/infra` 项目的构建/合成错误，这是预期的，因为当前支持 `cfn-guard` 的库不支持 Windows。有一个功能请求正在跟踪此问题，但在此期间，我们可以通过修改 `packages/infra/src/main.ts` 文件来禁用 `cfn-guard`，如下所示：
...
所有构建产物现在都位于单仓库根目录的 `dist/` 文件夹中。这是使用 `@aws/nx-plugin` 生成的项目时的标准做法，因为它不会用生成的文件污染文件树。如果您想清理文件，只需删除 `dist/` 文件夹，而无需担心生成的文件散落在文件树中。
