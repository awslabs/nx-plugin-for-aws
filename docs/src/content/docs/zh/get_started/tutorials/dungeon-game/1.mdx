...
<Aside type="tip" title="经常提交">最佳实践是在运行任何生成器前将所有未暂存文件提交到 Git。这样您可以通过 `git diff` 查看生成器运行后的变更。</Aside>
...
```
路由文件定义了 API 的 tRPC 路由,您需要在此声明所有 API 方法。如上所示,我们有一个名为 `echo` 的方法,其实现位于 `./procedures/echo.ts` 文件中。Lambda 处理程序入口点位于 `handler.ts`,由生成器自动配置。
...
这配置了一个 Python 项目和 [UV Workspace](https://docs.astral.sh/uv/concepts/projects/workspaces/),使用共享虚拟环境。
...
:::note[AG-UI 协议]
我们选择 `--protocol=AG-UI` 使代理使用 [Agent-User Interaction 协议](https://docs.copilotkit.ai/aws-strands/protocol) — 这让我们的 React 网站可以通过 [CopilotKit](https://docs.copilotkit.ai/) 直接与其通信,流式传输、工具调用和对话历史由协议处理,而不是手工编写 HTTP 客户端。
:::
...
:::tip[本地优先开发]
`ts#dynamodb` 生成器提供 `serve-local` 目标,在容器中运行 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)。结合 `connection` 生成器(我们将在下面运行),这意味着**整个游戏在您的机器上运行,无需部署**,直到教程结束。
:::
...
生成器会:
- 创建共享的 `agent_connection` Python 项目(如果尚不存在),包含核心 `AgentCoreMCPClientStrands`
- 生成 `InventoryMcpServerClientStrands` 类,处理本地(直接 HTTP)和部署时(通过 AgentCore 使用 IAM 身份验证)连接 MCP 服务器
- 转换 `agent.py` 以导入客户端、创建实例并将 MCP 服务器的工具连接到代理
- 将 `agent_connection` 项目作为工作区依赖添加到 story 项目
- 更新 `serve-local` 目标,在本地运行时自动启动 MCP 服务器

更多详情请参考 <Link path="guides/connection/py-agent-mcp">Python Agent 到 MCP 连接指南</Link>。
</Drawer>
...
因为故事代理的 `agent-serve-local` 已经依赖于库存 MCP 服务器的 `mcp-server-serve-local`(通过上面的 `story → inventory` 连接),而游戏 API 和 MCP 服务器现在都依赖于 `dungeon-db:serve-local`,运行任何一个项目的 `serve-local` 目标都会按正确顺序启动它所需的每个依赖项。
</Aside>
