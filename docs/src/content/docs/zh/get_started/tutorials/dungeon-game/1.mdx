...
有了工作区后,我们搭建游戏的子项目 — 游戏 API、故事代理、库存 MCP 服务器、游戏数据库和网站 — 以及连接它们的连接。有两种方式:

- **快速** — 直接从下面的图表复制命令并运行。这是达到相同起点的最快方式。
- **逐步** — 展开下面的部分,自己运行每个生成器并准确查看每个生成器产生的内容。

下图*就是*地牢冒险工作区:您将在本模块中构建的每个项目、组件和连接。点击 **Copy commands** 获取整个系列命令,然后从您在任务 1 中创建的 `dungeon-adventure` 目录中运行它们。

<EmbeddedGraph preset="dungeon-adventure" workspace="dungeon-adventure" iac="cdk" skipWorkspace />

<Drawer title="Step By Step" trigger="逐步:自己运行每个生成器并查看其产生的内容">

与其一次性复制所有命令,您可以单独运行每个生成器。这是了解每个生成器向工作区添加什么的最佳方式。从您在任务 1 中创建的 `dungeon-adventure` 目录中依次运行每个生成器。
...
<details>
<summary>查看故事代理 → 库存 MCP 连接文件</summary>
...
<details>
<summary>查看 UI → 故事代理连接文件</summary>
