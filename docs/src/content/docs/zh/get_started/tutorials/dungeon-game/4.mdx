...
在 <Link path="get_started/tutorials/dungeon-game/1">模块一</Link> 中，我们将 `GameUI` 连接到故事代理。由于代理是使用 `--protocol=AG-UI` 生成的，connection 生成器配置了 CopilotKit `AguiProvider` 和每个代理的钩子（`useAguiStoryAgent`），而不是通常的 OpenAPI 客户端。我们可以使用单个 `<CopilotChat />` 组件渲染聊天，并通过 `threadId`（我们将其用作 AgentCore 会话 id）驱动它。

在 `packages/game-ui/src/routes/game/` 下创建两个新文件：
