...
`game-ui` 上的 `dev` 目标依赖于 `game-api:dev` 和 `dungeon_adventure.story:agent-dev`，因此 Nx 会并行启动每个项目的本地服务器。通过我们在模块一中设置的连接，这些服务器又会启动 Inventory MCP 服务器和 DynamoDB Local。确保您的容器引擎正在运行，然后在浏览器中打开开发服务器。
...
您已经看到了两个极端：完全本地（`dev`）和完全部署。在日常开发中，混合使用两者通常很有用——例如，针对*真实*部署的 API 和代理迭代网站代码，或者针对*真实* DynamoDB 表在本地运行 API。
