...
`game-ui:serve-local` 依赖于 `game-api:serve-local` 和 `dungeon_adventure.story:agent-serve-local`，因此 Nx 会并行启动每个项目的本地服务器。通过我们在模块一中设置的连接，这些服务器又会启动 Inventory MCP 服务器和 DynamoDB Local——**整个游戏在您的机器上运行，无需任何 AWS 调用**。确保您的容器引擎正在运行，然后在浏览器中打开开发服务器。
...
您的第一次部署将需要大约 8 分钟才能完成（它会构建并将代理和 MCP 服务器 Docker 镜像推送到 AgentCore）。后续部署会更快。

部署完成后，您将看到类似以下的输出 _（某些值已被隐藏）_：
...
关键是 `RUNTIME_CONFIG_APP_ID` 环境变量。当项目运行时**没有** `SERVE_LOCAL=true`，生成的 `resolveTableName()`（以及代理 / API 运行时配置查找）会使用此应用程序 ID 从 AWS AppConfig 获取其配置——即 CDK 输出中的 `RuntimeConfigApplicationId` 值。

每个网站都有一个 `load:runtime-config` 目标，可将已部署的 `runtime-config.json`（Cognito 池、API 端点、代理 ARN）下载到本地开发服务器中。一些有用的组合：

- **本地网站 → 已部署后端。** 拉取一次已部署的配置，然后运行普通的 `serve` 目标（而不是 `serve-local`），以便 UI 与已部署的 API 和代理通信：

  <NxCommands commands={["run game-ui:load:runtime-config"]} />
  <NxCommands commands={["serve game-ui"]} />

- **本地 API → 真实 DynamoDB 表。** 运行普通的 `serve` 目标，并将 `RUNTIME_CONFIG_APP_ID` 设置为已部署的应用程序 ID，以便 `resolveTableName()` 解析已部署的表而不是 DynamoDB Local：
