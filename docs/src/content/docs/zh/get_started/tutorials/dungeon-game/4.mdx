...
回想一下，在 <Link path="get_started/tutorials/dungeon-game/1#task-7-connect-your-components">模块一</Link> 中，我们使用 `connection` 生成器将游戏 UI 连接到故事代理。这为我们设置了用于与代理交互的类型安全 OpenAPI 客户端，以及钩子和提供程序。

connection 生成器为我们创建了以下内容：
- 在 `main.tsx` 中包装应用程序的 `StoryAgentProvider` 组件
- 用于 TanStack Query 集成的 `useStoryAgent` 钩子
- 用于直接客户端访问的 `useStoryAgentClient` 钩子
- 从代理的 OpenAPI 规范生成的 TypeScript 类型

我们将在游戏组件中使用 `useStoryAgentClient` 钩子来从代理流式传输故事响应。
