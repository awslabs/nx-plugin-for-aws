...
:::tip[运行时配置]
DynamoDB 表名在<Link path="guides/runtime-config">运行时配置</Link>中的 `tables` 命名空间下注册，存储于 AWS AppConfig。Lambda 函数和 MCP 服务在运行时使用 Lambda Powertools 检索该配置，而无需通过环境变量传递。这种方法的优势在于，随着表或其他基础设施标识符数量增加，本地运行时只需要一个 `RUNTIME_CONFIG_APP_ID` 环境变量。
:::
  </TabItem>
</Tabs>
