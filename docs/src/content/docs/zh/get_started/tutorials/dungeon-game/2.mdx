...
我们将使用 DynamoDB 实现数据库，并通过 [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDB 客户端库简化操作。游戏 API 的 `queryActions` 还需要 S3 客户端来读取智能体写入的对话轮次。使用以下命令安装所有依赖：
