...
1. `saveGame` - 创建或更新游戏。
2. `queryGames` - 返回已登录玩家保存的游戏的分页列表。
...
:::tip[所有内容都限定在已登录玩家的范围内]
两个存储都以调用者为键，因此一个玩家永远无法看到另一个玩家的冒险：

- **游戏**在 DynamoDB 中按用户的 Cognito `sub` 进行分区，因此 `queryGames` 查询该分区而不是扫描整个表。两个玩家也可以选择相同的英雄名称而不会冲突。
- **对话**由代理进行命名空间划分，因此键看起来像 `session_u-<sub>-<threadId>/…`。
- **库存**以调用代理的会话加上玩家名称为键。库存 MCP 服务器从请求上下文中读取该会话，而不是信任模型传入的 `playerName` — 模型参数不是信任边界。

`userId` 和命名空间都不是从请求正文中获取的 — 网站可能会对此撒谎。`identity.ts` 中的 `requireUserId` 从 Cognito Identity Pool 填充的 IAM 授权器上下文中读取调用者（`requestContext.authorizer.iam.cognitoIdentity.amr`，其中包含 `…:CognitoSignIn:<sub>` 条目）。该值由 Identity Pool 令牌交换生成，因此它是可信的而不是由浏览器提供的，没有它的请求将被拒绝。有关作为 tRPC 插件的可重用版本，请参阅<Link path="guides/trpc">实现自定义中间件</Link>。
:::
...
每个过程都将其数据限定在已登录玩家的范围内，因此它们都需要调用者的身份。添加 `packages/game-api/src/identity.ts`：
