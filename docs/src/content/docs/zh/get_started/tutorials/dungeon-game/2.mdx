...
`ts#dynamodb` 生成器设置了 [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/)，我们将使用它来建模数据。我们将对话历史持久化到 S3，因此添加 S3 客户端依赖：
...
:::tip[运行时配置]
`DungeonDb` 构造在<Link path="guides/runtime-config">运行时配置</Link>中的 `dynamodb` 命名空间下注册已部署的表名，存储在 AWS AppConfig 中。游戏 API 和库存 MCP 服务器在运行时使用 Lambda Powertools 检索表名，而无需通过环境变量传递。这种方法的好处是，随着表或其他基础设施标识符数量的增加，在针对已部署基础设施运行时只需要一个 `RUNTIME_CONFIG_APP_ID` 环境变量。
:::
...
使用 `serve-local` 目标在本地启动游戏 API，这也会启动 DynamoDB Local：
...
我们可以使用生成的 `mcp-server-inspect` 目标通过 [MCP Inspector](https://github.com/modelcontextprotocol/inspector) 试用 MCP 服务器的工具：

<NxCommands commands={["mcp-server-inspect inventory"]} />

这会在本地提供 MCP 服务器（同时启动 DynamoDB Local），并在 `http://localhost:6274` 启动预配置好的 MCP Inspector。点击 **Connect**，切换到 **Tools** 选项卡，点击 **List Tools**，然后尝试 `add-to-inventory`（例如 `playerName: Alice`、`itemName: Rusty Sword`、`emoji: ⚔️`），然后使用 `list-inventory-items` 查看它已持久化到 DynamoDB Local。完成后停止服务器（`Ctrl+C`）。
