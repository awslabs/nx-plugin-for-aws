...
项目中将不再使用`packages/game-api/src/schema/echo.ts`文件，可将其删除。

<Aside type="tip">
如上所示，我们为每个Zod模式定义导出了使用`z.TypeOf`语法生成的接口。这让我们无需重复工作即可将Zod定义转换为TypeScript接口！
</Aside>
...
<Aside>
所有依赖项都添加到根目录`package.json`中，因为`@aws/nx-plugin`遵循[单一版本策略](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)原则。更多信息请参考<Link path="guides/typescript-project#dependencies">TypeScript项目指南</Link>。
</Aside>

现在根据上述ER图，在`packages/game-api/src/entities`目录下创建以下文件定义ElectroDB实体：
...
ElectroDB功能强大，不仅可以定义类型，还能为时间戳等字段提供默认值。此外，ElectroDB遵循[单表设计](https://electrodb.dev/en/core-concepts/single-table-relationships/)，这是使用DynamoDB的最佳实践。

<Aside>
虽然ElectroDB支持[集合](https://electrodb.dev/en/modeling/collections/)，但本教程为简化说明暂不使用。
</Aside>

为了让MCP服务能与库存交互，确保在`packages/game-api/src/index.ts`中导出库存实体：
...
:::note
若将实体重构到独立的共享项目中，MCP服务就不需要依赖API，这样的包结构会更合理。
:::

### 将DynamoDB客户端注入tRPC上下文

由于每个操作流程都需要访问DynamoDB客户端，我们需要创建单例客户端并通过上下文传递。在`packages/game-api/src`中进行以下修改：
...
<Aside>
`concat` API将中间件绑定到定义的操作流程。更多细节请参考[concat指南](https://trpc.io/docs/server/middlewares#concat)。
</Aside>
  </TabItem>
</Tabs>
...
现在开始实现API方法。在`packages/game-api/src/procedures`中进行以下修改：
...
项目中不再使用`echo.ts`文件（位于`packages/game-api/src/procedures`），可将其删除。

### 路由配置

完成操作流程定义后，将其接入API。更新以下文件：
...
随着工具数量增加，可按需将其重构到独立文件中。

现在可以删除`packages/inventory/src/mcp-server`中未使用的`tools`和`resources`目录。

### 基础设施

最后更新基础设施以创建DynamoDB表并授予GameAPI操作权限。按以下方式更新`packages/infra/src`：
...
:::note
注意此处每个操作流程由独立lambda函数处理，因此可以遵循最小权限原则，根据实现仅分配必要的读写权限。
:::
  </TabItem>
</Tabs>
...
:::tip
若仅修改lambda函数代码，可在构建后使用`--hotswap`标志进行热部署（约2-3秒）：
...
可通过以下方式测试API：
<ul>
<li>启动本地tRPC后端实例并使用`curl`调用API</li>
<li>
<Drawer title="启用Sigv4签名的curl" trigger="调用已部署API（使用Sigv4签名curl）">
...
<Tabs>
  <TabItem label="本地测试">
    ...
    <Aside type="caution">
    请使用CDK部署输出中的`dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX`值替换高亮占位符。
    </Aside>
...
```
    <Aside type="caution">
    使用CDK部署输出中的`dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX`值替换高亮占位符，并设置对应区域。
    </Aside>
  </TabItem>
</Tabs>
