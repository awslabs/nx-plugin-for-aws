...
4. `queryActions` - 返回分页的指定游戏相关操作列表
...
<Aside type="tip">
如上所示，每个Zod模式定义都通过`z.TypeOf`语法导出对应TypeScript接口，避免重复定义。
</Aside>
...
使用DynamoDB作为数据库，并通过[ElectroDB](https://electrodb.dev/en/core-concepts/introduction/)客户端库简化操作。首先安装依赖：

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
所有依赖均添加至根目录`package.json`，遵循[@aws/nx-plugin](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)的单版本策略。详见<Link path="guides/typescript-project#dependencies">TypeScript项目指南</Link>。
</Aside>
...
ElectroDB不仅支持类型定义，还能为时间戳等字段提供默认值。其遵循DynamoDB最佳实践——[单表设计](https://electrodb.dev/en/core-concepts/single-table-relationships/)。

<Aside>
虽然ElectroDB支持[集合](https://electrodb.dev/en/modeling/collections/)，但本教程为简化流程暂不使用。
</Aside>

在`packages/game-api/src/index.ts`中导出库存实体以便MCP服务调用：
...
:::note
将实体重构至独立共享项目可使MCP服务不依赖API，优化包结构。
:::

### 将DynamoDB客户端注入tRPC上下文

为在各流程中访问DynamoDB客户端，需创建单例并通过上下文传递。修改`packages/game-api/src`目录下文件：
...
<Aside>
`concat`方法将中间件绑定到流程定义。详见[concat指南](https://trpc.io/docs/server/middlewares#concat)。
</Aside>
  </TabItem>
</Tabs>
...
在`packages/game-api/src/procedures`目录下实现API方法：
...
定义以下工具方法：
- `list-inventory-items` 获取当前库存
- `add-to-inventory` 添加物品
- `remove-from-inventory` 移除物品
...
工具数量增加时可拆分至独立文件。删除`packages/inventory/src/mcp-server`下未使用的`tools`和`resources`目录。

### 基础设施

更新基础设施代码创建DynamoDB表并配置权限：
...
:::note
每个Lambda函数对应独立流程，遵循最小权限原则按需分配读写权限。
:::
  </TabItem>
</Tabs>
...
:::tip
Lambda代码更新时使用`--hotswap`参数可加速部署（2-3秒）：
...
测试方式：
<ul>
<li>本地启动tRPC后端，使用`curl`调用API</li>
<li>
<Drawer title="Sigv4签名curl" trigger="调用已部署API（需Sigv4签名）">

<Tabs>
  <TabItem label="Bash/Linux/macOS">
将以下脚本添加至`.bashrc`或当前终端：
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
<Tabs>
  <TabItem label="本地测试">
    ...
    <Aside type="caution">
    使用CDK部署输出的`dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX`值替换占位符。
    </Aside>
...
```
    <Aside type="caution">
    使用CDK输出的`dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX`值替换URL，并设置正确区域。
    </Aside>
  </TabItem>
</Tabs>
