...
使用 [Zod](https://zod.dev/) 在 `packages/game-api/src/schema` 目录下定义 API 输入输出模式：
...
可以删除 `packages/game-api/src/schema/echo.ts` 文件，因为本项目不会使用它。
...
我们将使用 DynamoDB 实现数据库，并通过 [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) 客户端库简化操作。要安装 `electrodb` 和 DynamoDB 客户端，运行以下命令：
...
根据 ER 图定义 ElectroDB 实体，在 `packages/game-api/src/entities` 目录下创建以下文件：
...
<Aside>
`concat` API 将中间件绑定到定义的操作流程。更多细节请参考 [concat 指南](https://trpc.io/docs/server/middlewares#concat)。
</Aside>
  </TabItem>
</Tabs>
...
可以删除 `packages/game-api/src/procedures` 中的 `echo.ts` 文件，因为本项目不会使用。
