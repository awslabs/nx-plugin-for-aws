...
<Aside title="项目依赖">
S3 客户端被添加到 `game-api` 项目自己的 `package.json` 中，因为该项目的源代码导入了它。版本仍然通过包管理器的依赖目录（如果支持）在工作区中保持一致（[单一版本策略](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)）。更多信息请参考<Link path="guides/typescript-project#dependencies">TypeScript 项目指南</Link>。
</Aside>
...
无需部署到 AWS 即可试用我们的 API——`dev` 目标针对 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) 运行游戏 API。因为我们在模块 1 中将游戏 API 连接到了 `DungeonDb` 项目，此目标也会自动启动 DynamoDB Local。
