...
本地网关公开单个 MCP 端点，聚合每个附加的 MCP 服务器（通过 <Link path="guides/connection/agentcore-gateway-mcp">`agentcore-gateway#mcp-connection` 生成器</Link>连接），工具前缀为 `<target>___<tool>` 以匹配已部署的 Gateway。
</OptionFilter>
