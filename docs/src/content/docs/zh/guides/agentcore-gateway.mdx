...
生成一个 [Amazon Bedrock AgentCore Gateway](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html) 项目。AgentCore Gateway 是一个位于您的 MCP 服务器或代理前面的托管入口点，对入站请求进行身份验证（IAM 或 Cognito），并使用 IAM SigV4 对发往其目标的出站流量进行签名。

`protocol` 选项选择 Gateway 前置的内容：

- **`mcp`**（默认）— 将一个或多个 MCP 服务器目标聚合在单个 MCP 端点后面，并针对 [Cedar 策略引擎](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/policy.html)评估每个工具调用。
- **`http`** — 通过基于路径的路由（`/<targetName>/invocations`）直接代理请求到 [AgentCore Runtime 目标](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-target-http-runtime.html)（您的代理），无需聚合或协议转换。使用此选项可以使用单个受治理的端点来前置代理——例如，使网站可以通过 Gateway 访问部署在 VPC 内的代理。
...
<FileTree>
  - packages/\<name>/
    - policies/ Cedar 策略源文件（仅限 `mcp` 协议；当 `cedarPolicy: false` 时省略）
      - permit-all.cedar 默认的 Cedar 策略，允许已认证的调用者
...
- 一个使用入站 IAM 身份验证（默认）或 Cognito JWT 身份验证（参见 <a href="#authentication">Authentication</a>）的 `AgentCore::Gateway`。`mcp` 网关配置为 MCP 协议；`http` 网关没有协议类型，这是 AgentCore 对其运行时目标的要求
...
生成器向 Gateway 项目添加一个 `dev` 目标，该目标运行 `local-dev.ts`。运行它会一起启动本地网关和每个附加的目标：

<NxCommands commands={["dev <name>"]} />

对于 `mcp` 网关，本地网关公开单个 MCP 端点，聚合每个附加的 MCP 服务器（通过 <Link path="guides/connection/agentcore-gateway-mcp">`agentcore-gateway#mcp-connection` 生成器</Link>连接），工具前缀为 `<target>___<tool>` 以匹配已部署的 Gateway。

对于 `http` 网关，本地网关将 `/<targetName>/...` 路径代理到每个附加代理的本地服务器（通过 <Link path="guides/connection/agentcore-gateway-agent">`agentcore-gateway#agent-connection` 生成器</Link>连接），匹配已部署 Gateway 的基于路径的路由。
