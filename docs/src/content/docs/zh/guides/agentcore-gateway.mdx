...
生成器向 Gateway 项目添加一个 `<name>-dev` 目标，该目标运行 `local-dev.ts`：一个本地网关，公开单个 MCP 端点，聚合每个附加的 MCP 服务器（通过 <Link path="guides/connection/agentcore-gateway-mcp">`agentcore-gateway#mcp-connection` 生成器</Link>连接），工具前缀为 `<target>___<tool>` 以匹配已部署的 Gateway。运行它会一起启动本地网关和所有附加的 MCP 服务器。使用项目的 `dev` 目标：
