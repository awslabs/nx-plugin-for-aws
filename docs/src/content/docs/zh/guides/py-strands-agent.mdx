...
:::tip
您的`Dockerfile`与Strands智能体项目位于同一位置。对于CDK，构造使用`buildContexts`直接从此Dockerfile构建镜像。对于Terraform，Docker镜像使用标签（如`my-scope-my-project-agent:latest`）预先构建，并由Terraform基础设施引用。
:::
