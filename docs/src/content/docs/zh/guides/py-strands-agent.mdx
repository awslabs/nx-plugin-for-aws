...
生成器还在`scripts/<your-agent-name>/chat.ts`下提供了一个最小的交互式聊天CLI，以及相应的`<your-agent-name>-chat`目标。该目标依赖于`<your-agent-name>-serve-local`，因此运行它会在本地启动智能体并将您带入交互式终端聊天：

<NxCommands commands={['run your-project:agent-chat']} />

该CLI是一个TypeScript脚本（通过`tsx`运行），使用适合协议的客户端通过[`@clack/prompts`](https://github.com/bombshell-dev/clack)逐令牌流式传输响应：

- **HTTP**：`POST /invocations`，使用JSONL流式传输
- **A2A**：`@a2a-js/sdk` `ClientFactory`，使用`sendMessageStream`
