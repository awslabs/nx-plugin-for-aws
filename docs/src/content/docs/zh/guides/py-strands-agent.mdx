...
对于流式响应，生成器提供`JsonStreamingResponse`，它会自动将Pydantic模型序列化为JSON Lines格式（`application/jsonl`）。此格式兼容OpenAPI 3.2的流式规范，并可与生成的TypeScript客户端无缝协作。

默认情况下，智能体yield `StreamChunk`对象，包含智能体的响应文本：
...
:::note
请注意，由于FastAPI目前不支持为流式响应生成OpenAPI规范，我们必须显式设置每个流式操作的响应模型，例如：
...
FastAPI有一个[原生支持的开放功能请求](https://github.com/fastapi/fastapi/discussions/14362)。
