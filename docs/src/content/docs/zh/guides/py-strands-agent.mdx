...
生成器提供`auth`选项来配置Strands智能体的认证方式。生成智能体时可选择`IAM`（默认）或`Cognito`认证。
...
可使用`grantInvokeAccess`方法授权调用Bedrock AgentCore Runtime上的智能体，例如：
...
选择`Cognito`认证时，生成器会自动创建`UserIdentity`构造，并配置智能体使用Cognito进行认证。

<Infrastructure>
<Fragment slot="cdk">
生成的构造接受`identity`属性来配置Cognito认证：
...
选择`Cognito`认证时会自动生成`UserIdentity`构造。
</Fragment>
<Fragment slot="terraform">
生成的模块接受`user_pool_id`和`user_pool_client_ids`变量进行Cognito认证配置：
