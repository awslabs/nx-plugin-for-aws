...
当`protocol`设为`A2A`时，入口点使用[Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)而非FastAPI：
...
智能体的服务器协议决定其通信方式。两种选项都由[FastAPI](https://fastapi.tiangolo.com/)提供服务——区别在于入口点：
...
两种协议都暴露`/ping`端点以满足AgentCore运行时健康检查契约。A2A智能体监听端口`9000`（HTTP为`8080`）；生成的Dockerfile和基础设施已为您配置。
...
当`protocol=A2A`时，生成的`main.py`将`A2AServer.to_fastapi_app()`挂载到父FastAPI应用上，该应用同时暴露`/ping`端点。部署到AgentCore时，入口点会从AppConfig解析运行时的公共ARN，并在智能体卡片中发布。
...
Strands SDK的`A2AAgent`封装了远程A2A端点，使您可以像调用本地`Agent`一样调用它。
...
#### 在AgentCore上使用IAM认证调用

对已部署到AgentCore的A2A智能体的请求（包括智能体卡片获取）必须使用SigV4签名。向`A2AAgent`传递一个`client_config`，其中包含使用SigV4认证流的`httpx.AsyncClient`：
...
:::note[IAM权限]
调用者的IAM角色需要对目标运行时ARN具有`bedrock-agentcore:InvokeAgentRuntime`和`bedrock-agentcore:GetAgentCard`权限。生成的构造的`grantInvokeAccess`辅助方法会同时授予这两个权限。
:::

:::note[额外依赖]
A2A生成器会为您添加`strands-agents[a2a]`。如果您要将A2A客户端代码添加到HTTP智能体，请在`pyproject.toml`中添加`strands-agents[a2a]`和`httpx`。
:::

#### 将A2A智能体用作工具

`A2AAgent`像本地`Agent`一样继承自`AgentBase`，因此您可以将其包装在`@tool`中并交给另一个智能体：

```python
from strands import Agent, tool

remote = # 使用上述client_config构建的A2AAgent

async def ask_remote_agent(prompt: str) -> str:
    """将问题委托给远程A2A智能体并返回其回复。"""
    result = await remote.invoke_async(prompt)
    return str(result)

ask_remote = tool(
    name="ask_remote_agent",
    description="将问题委托给远程A2A智能体并返回其回复。",
)(ask_remote_agent)

agent = Agent(
    system_prompt="在适当时委托给ask_remote_agent。",
    tools=[ask_remote],
)
```

在CDK堆栈中，授予宿主智能体对远程智能体的调用访问权——这会配置A2A客户端所需的两个IAM操作：
