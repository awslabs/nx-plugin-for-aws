...
生成器配置了一个`<your-agent-name>-chat` Nx目标，该目标依赖于`<your-agent-name>-serve-local`。运行它会在本地启动智能体并将您带入由[`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)驱动的交互式终端聊天：

<NxCommands commands={['run your-project:agent-chat']} />

对于**A2A**和**AG-UI**智能体，聊天目标只是将`agent-chat-cli`作为二进制文件调用本地运行的智能体——不会在您的工作区中生成任何代码。

对于**HTTP**智能体，生成器会生成：

- `scripts/<your-agent-name>_openapi.py` — 一个导出智能体OpenAPI规范的小脚本
- 一个运行该脚本的`<your-agent-name>-openapi` Nx目标
- 一个运行`open-api#ts-client`的`<your-agent-name>-generate-client` Nx目标，在`scripts/<your-agent-name>/generated/`下生成类型安全的TypeScript客户端
- `scripts/<your-agent-name>/chat.ts` — 一个轻量级适配器，导入生成的客户端并将其插入`agent-chat-cli`的`chatLoop`

聊天脚本是生成的客户端的一个小包装器。当您自定义智能体的输入形状（例如向`InvokeInput`添加新字段）时，更新适配器的`sendMessage`以传递新字段，其余部分将自动工作。
