...
这意味着您无需显式运行检查，尽管您仍然可以使用 NxCommands `license-check` 目标来执行：
...
要永久禁用，请从 `aws-nx-plugin.config.mts` 配置中移除 `license.dependencies` 键。您也可以使用 `--dependencyCheck=false` 重新运行 `license` 生成器以在不包含它的情况下进行脚手架 — 生成器标志仍命名为 `--dependencyCheck`，并控制生成器是否写入 `dependencies` 块。