...
对于检查失败的包使用 `exceptions`——要么是因为它们的许可证不在允许列表中，要么是因为它们没有可检测的许可证元数据。

每个条目接受：

- `package`（必需）：例外适用的包名称
- `reason`（必需）：授予例外的原因，以便审查者可以看到
- `version`（可选）：将例外限制为单个版本，以便再次检查更高版本
- `spdx`（可选）：将包视为声明的许可证，用于在其元数据中未声明许可证文本的包。这是下面的 `onDependency` 钩子报告的内容
...
收集器发现依赖项并提取许可证元数据。内置收集器是 `npmCollector()`（扫描 `node_modules`）和 `pythonCollector()`（扫描 Python 虚拟环境），两者都从 `@aws/nx-plugin/sdk/license` 导出。许可证生成器默认配置 `npmCollector()`，并在存在 Python 项目时添加 `pythonCollector()`。
