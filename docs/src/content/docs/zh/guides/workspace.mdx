...
从 Node 的角度来看，这意味着在根目录有一个单一的锁文件，依赖项只安装一次并链接到每个项目中。每个 Node 项目在其自己的 `package.json` 中声明其源代码导入的运行时依赖项，而共享的构建/测试工具则位于根目录的 `package.json` 的 `devDependencies` 中。通过将依赖项安装到该项目来添加项目的运行时依赖项：
