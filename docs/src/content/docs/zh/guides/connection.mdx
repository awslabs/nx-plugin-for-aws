...
:::tip[可视化草图]
更喜欢先设计您的工作空间？<Link path="get_started/graph-builder">Graph Builder</Link> 让您可以拖出项目并绘制它们之间的连接，然后为您提供命令来搭建整个结构。
:::
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="将 FastAPI 连接到 Python Aurora 关系数据库"
  ...
  <ConnectionCard
    ...
    description="将 Python Agent 连接到 Python Aurora 关系数据库"
  ...
  <ConnectionCard
    ...
    description="将 Python MCP Server 连接到 Python Aurora 关系数据库"
  ...
  <ConnectionCard
    ...
    description="使用 AgentCore Gateway 作为运行时目标为 agent 提供前端"
  ...
  <ConnectionCard
    ...
    description="通过 AgentCore Gateway 将 React 网站连接到 agents"
...
:::note[运行时配置]
连接生成器利用<Link path="guides/runtime-config">运行时配置</Link>在运行时在生成的项目和组件之间传递部署时值（例如 API URL、Cognito 设置和 agent 运行时 ARN），以便它们可以相互发现和连接。
:::

:::tip[本地开发]
连接的项目可以使用 `serve` 和 `dev` 目标在您的机器上运行。有关详细信息，请参阅<Link path="guides/local-development">本地开发</Link>指南。
:::
