...
- 声明每个响应块的返回类型
- 如果计划使用 <Link path="guides/connection/react-fastapi">API 连接</Link>,需添加 OpenAPI 供应商扩展 `x-streaming: true`
...
要消费流式响应,可以使用 <Link path="guides/connection/react-fastapi#consuming-a-stream">API 连接生成器</Link>,该生成器将提供类型安全的方法来迭代流式数据块。
...
要从 React 网站调用 API,可以使用 <Link path="guides/connection/react-fastapi">`connection`</Link> 生成器。