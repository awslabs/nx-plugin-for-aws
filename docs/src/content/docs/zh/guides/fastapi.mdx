...
:::caution[将 `boto3` 固定到上述版本]
其他生成器添加的 `boto3` 固定到此版本。由于您的工作区共享单个 UV 虚拟环境和锁文件，未固定添加（`add boto3`）会解析最新版本并写入 `boto3>=<latest>` 约束，`uv` 无法与固定版本一起满足。工作区中的所有 Python 目标随后将无法解析依赖项。
:::
