...
:::tip[集成模式]
`integrationPattern` 选项默认为 `isolated`,这会为每个 FastAPI 操作创建一个 Lambda。如果您希望整个 API 使用单个共享的 Lambda 处理程序,并可选择性地覆盖特定操作,请选择 `shared`。
:::
...
FastAPI 生成器根据您选择的 `iac` 创建 CDK 或 Terraform 基础设施即代码。您可以使用此代码部署 FastAPI。
...
API 模块将其 Lambda 部署 zip 暂存在共享的 S3 资产存储桶中 — 详情请参阅 <Link path="/guides/terraform-project">Terraform 基础设施指南</Link>。每个部署实例化一次 `core/asset-bucket` 模块,并通过 `asset_bucket_name` 输入将其 `bucket_name` 输出传递给每个 API / Lambda 模块:
