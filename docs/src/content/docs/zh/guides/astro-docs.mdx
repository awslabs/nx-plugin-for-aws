...
除非您传递了 `--noTranslation`，否则生成器会向 `project.json` 添加一个 `translate` 目标。它翻译到哪些语言环境来自 `scripts/translate.config.json` 中的 `targetLanguages`，该配置默认为空——因此首先在命令行上指定您想要的语言环境：
...
一旦您填充了 `targetLanguages`（参见[配置翻译](#配置翻译)），`--languages` 就变成可选的，并使用配置的语言环境：
...
:::caution[首先配置您的目标语言]
如果没有 `--languages` 且 `targetLanguages` 仍为空，目标将以非零状态退出，并显示 `ERROR No target languages configured`。
:::
...
| `translationCommitMessage` | 翻译提交的提交消息标记。不在生成的配置中——添加此键以覆盖其 `docs: update translations` 默认值。 |
...
3. 将生成的翻译提交回 PR 分支，使用 `docs: update translations` 作为提交消息，以便后续的增量运行可以检测基线提交，并仅重新翻译自那时以来更改的文件。要使用不同的消息，请向 `scripts/translate.config.json` 添加 `translationCommitMessage` 键，并改为匹配该值。
