...
3. 将生成的翻译提交回 PR 分支，使用 `docs: update translations` 作为提交消息，以便后续的增量运行可以检测基线提交，并仅重新翻译自那时以来更改的文件。要使用不同的消息，请在 `scripts/translate.config.json` 中添加 `translationCommitMessage` 键并匹配该值。
