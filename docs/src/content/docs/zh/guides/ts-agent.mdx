...
生成器配置了一个 `<your-agent-name>-chat` Nx 目标,让您进入与代理的交互式终端聊天界面,由 [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) 提供支持。

聊天目标独立运行——它**不会**为您启动代理。默认情况下,它连接到您本地运行的代理,因此请先启动 `<your-agent-name>-serve-local`(在单独的终端中):
