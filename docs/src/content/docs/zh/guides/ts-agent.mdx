...
会话 ID 来自调用者，因此它本身只能识别对话，而不能识别对话属于谁。AgentCore Runtime 针对代理运行时资源 ARN 而不是针对单个会话授权调用，这使代理可以自由决定会话对您的应用程序意味着什么。

如果您希望每个用户只能访问自己的对话，请在您自己的应用程序中记录谁拥有每个会话，并在进入时检查该所有权。然后，会话 ID 是您的应用程序发出的内容，而不是调用者选择的内容。

**1. 通过您的 API 创建会话**

添加一个启动对话的过程。它自己生成会话 ID，从自己的请求上下文解析调用者，并存储该对 — 例如在使用 <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link> 生成器创建的表中：
...
从已验证的请求上下文而不是从输入字段解析调用者，这样它就不能由浏览器提供。<Link path="guides/trpc">实现自定义中间件</Link>展示了如何为 IAM 和 Cognito 身份验证的 API 执行此操作。

查询该索引的第二个过程让 UI 为用户提供他们自己的对话，而不会看到其他人的对话。

**2. 在代理中验证所有权**

您的代理已经接收会话 ID，并且使用 `auth=cognito` 时，它还接收调用者的 JWT — AgentCore Runtime 授权器验证它，生成的基础设施将 `Authorization` 标头列入白名单并传递给您的代码。查找会话，将其所有者与调用者进行比较，如果它们不同则拒绝请求：
...
在这里，无需验证签名即可解码声明就足够了，因为 AgentCore Runtime 在入站授权期间验证了令牌。

:::tip[为什么要在代理和 API 中都进行验证]
API 决定谁拥有会话；代理强制执行它。在两个地方检查意味着泄漏的会话 ID — 从浏览器复制或从旧对话重用 — 在下次调用时仍会被拒绝，并且代理的任何未来客户端都会继承相同的规则。
:::
...
在您的 API 中生成不透明的会话 ID 可以避免这种情况，因为调用者无法访问未给予它的会话。从用户提供的值派生一个 — 对话名称、电子邮件、游戏名称 — 是可预测的，并且让两个调用者发生冲突。
:::
