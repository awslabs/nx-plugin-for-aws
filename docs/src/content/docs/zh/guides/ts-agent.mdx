...
还会生成一个 `<your-agent-name>-serve` 目标，它针对您已部署的基础设施运行代理，因此需要设置 `RUNTIME_CONFIG_APP_ID`。有关 `dev` 和 `serve` 之间的区别，请参阅<Link path="guides/local-development">本地开发</Link>指南。
