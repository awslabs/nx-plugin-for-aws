...
这使用 `tsx --watch` 在文件更改时自动重启服务器。代理将在 `http://localhost:8081`（或如果您有多个代理，则为分配的端口 — 从项目的 `project.json` 中的 `metadata.ports` 读取）上可用。
...
替换分配给您的代理的端口 — 从项目的 `project.json` 中的 `metadata.ports` 读取。
...
为此使用 `<your-agent-name>-dev`，而不是 `<your-agent-name>-serve`。`-serve` 运行已部署的代码路径，因此需要设置 `RUNTIME_CONFIG_APP_ID` — 请参阅 <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>。如果没有它，每次调用都会失败，并显示 `RUNTIME_CONFIG_APP_ID is not set`。
:::
