...
description: 生成用于构建具有工具的 AI 代理的 TypeScript Agent，并部署到 Amazon Bedrock AgentCore Runtime
generator: ts#agent
---
...
:::tip[项目前提条件]
首先使用 <Link path="/guides/typescript-project">`ts#project`</Link> 生成器创建一个项目，以便将您的 Agent 添加到其中。
:::
...
TypeScript Agent 使用基于 WebSocket 的 [tRPC](https://trpc.io/)，利用 [AgentCore 的双向流支持](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-get-started-websocket.html)来实现客户端和代理之间的实时、类型安全的通信。
...
工具是 AI 代理可以调用来执行操作的函数。您可以在 `agent.ts` 文件中添加新工具：
...
默认情况下，Strands 代理使用 Claude 4 Sonnet，但您可以轻松在模型提供商之间切换：
...
生成的 `index.ts` 将 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) 挂载到 Express 应用程序上，因此生成的代理会公开 A2A 协议端点以及 `/ping` 健康检查。当部署到 AgentCore 时，入口点会从 AppConfig 解析运行时的公共 ARN 并在代理卡中公布它。

大多数用户不需要修改此文件 — 编辑 `agent.ts` 以更改工具或系统提示。A2A 代理监听端口 `9000`（而 HTTP 为 `8080`），生成的 Dockerfile 和基础设施已为此配置。
</OptionFilter>

<OptionFilter when={{ protocol: 'ag-ui' }} description="AG-UI server details">
## AG-UI 服务器（AG-UI 协议）

生成的 `index.ts` 将您的 Strands `Agent` 包装在 [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands) `StrandsAgent` 中，并通过 `createStrandsApp()` 创建 Express 应用程序。生成的应用程序公开一个通过服务器发送事件（SSE）流式传输 [AG-UI](https://docs.ag-ui.com/) 事件的单个 POST 端点，以及用于 AgentCore 运行时健康检查的 `/ping`。

AG-UI 代理设计为直接由前端使用。使用 <Link path="/guides/connection/react-agui">`connection` 生成器</Link>将您的 React 网站与代理连接，使用 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 提供程序和 [AG-UI HttpAgent](https://docs.ag-ui.com/) 客户端。

大多数用户不需要修改 `index.ts` — 编辑 `agent.ts` 以更改工具或系统提示。AG-UI 代理监听端口 `8080`（与 HTTP 相同），生成的 Dockerfile 和基础设施已为此配置。
</OptionFilter>
...
生成器为每个协议生成一个 `scripts/<your-agent-name>/chat.ts`。您可以在演进代理的输入形状时对其进行自定义。默认情况下，它连接到本地代理，或者在设置 `RUNTIME_CONFIG_APP_ID` 时连接到已部署的代理（请参阅下面的[与已部署的代理聊天](#chat-with-your-deployed-agent)）。
...
生成器配置了一个 `<your-agent-name>-docker` 目标，该目标将 `Dockerfile` 从您的代理源目录复制到 bundle 输出目录。这将 `Dockerfile` 与打包的工件放在一起，允许 CDK 直接使用 `AgentRuntimeArtifact.fromAsset` 构建 Docker 镜像。
...
您可以在 CloudWatch AWS 控制台中找到追踪，方法是在菜单中选择"GenAI Observability"。请注意，要填充追踪，您需要启用[事务搜索](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Transaction-Search.html)。
...
- **`s3`**（默认）：CDK/Terraform 基础设施为会话数据配置专用的 S3 存储桶，使用专用的 KMS 密钥加密并阻止所有公共访问；服务器访问日志通过同一密钥传送到 CloudWatch Logs 日志组。代理的 IAM 角色被授予对存储桶的读/写/列表/删除访问权限以及对密钥的解密/生成数据密钥访问权限，存储桶名称与代理的 ARN 一起注册在 AppConfig 运行时配置中。
- **`in-memory`**：不配置存储桶。对话状态仅在运行进程的生命周期内保存在内存中，不会在重启或缩容后保留。

这在生成的 `session.ts` 中实现，该文件导出一个 `getSessionManager()` 函数，为当前会话解析 `SessionManager`。

<OptionFilter when={{ protocol: 'ag-ui' }} description="AG-UI session wiring">
由于 AG-UI 为每个对话线程克隆一个模板代理，`getSessionManager` 在 `index.ts` 中作为 `StrandsAgent` 配置上的 `sessionManagerProvider` 连接，因此为每个线程解析一个新的 `SessionManager`，而不是烘焙到共享的模板代理中。
</OptionFilter>
...
:::caution[HTTP 标头身份验证]
客户端工厂在 WebSocket 握手中使用 HTTP 标头来对 Bedrock AgentCore Runtime 的请求进行身份验证。这在浏览器中是不可能的，因此此客户端不适合浏览器环境。有关从浏览器调用的详细信息，请参阅下面的"浏览器"部分。
:::
...
:::note[AWS 凭证]
这将使用默认的 AWS 凭证提供程序链来对请求进行身份验证，因此运行上述代码的环境必须配置 AWS 凭证，并具有调用运行时的访问权限。
:::
</TabItem>
...
`accessTokenProvider` 必须返回用于对请求进行身份验证的令牌。例如，您可以在此方法中获取令牌，以确保在 tRPC 重启 WebSocket 连接时重用新凭证。以下演示了使用 AWS SDK 从 Cognito 获取令牌：
...
要将工作从此代理委托给远程 A2A 代理（<Link path="/guides/ts-agent">TypeScript</Link> 或 <Link path="/guides/py-agent">Python</Link>），请使用 <Link path="/guides/connection/ts-agent-a2a">`connection` 生成器</Link>。它为目标代理提供 SigV4 身份验证的客户端，并对此代理的 `agent.ts` 进行 AST 转换，将远程 A2A 代理注册为 Strands `tool`。
...
要从 React 网站调用您的 AG-UI 代理，请使用 <Link path="/guides/connection/react-agui">`connection` 生成器</Link>，它会连接一个为已部署的代理配置的 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 客户端，并具有正确的身份验证（IAM 或 Cognito）。
