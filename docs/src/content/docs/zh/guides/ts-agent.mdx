...
生成器配置了一个名为 `<your-agent-name>-serve` 的目标,用于在本地启动您的 Agent 进行开发和测试。
...
聊天目标独立运行。默认情况下,它连接到您本地运行的代理,因此请先启动 `<your-agent-name>-serve-local`(在单独的终端中):
...
<CardGrid>
  ...
  />
  <ConnectionCard
    title="TypeScript Agent 到 TypeScript DynamoDB"
  ...
  />
  <ConnectionCard
    title="TypeScript Agent 到 AgentCore Gateway"
