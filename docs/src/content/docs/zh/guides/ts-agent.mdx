...
生成的 `index.ts` 将您的 Strands `Agent` 包装在 [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands) `StrandsAgent` 中，并构建 Express 应用程序。生成的应用程序公开一个单一的 POST 端点，通过服务器发送事件（SSE）流式传输 [AG-UI](https://docs.ag-ui.com/) 事件，以及用于 AgentCore 运行时健康检查的 `/ping`。
