...
:::note
使用 CDK 时，网站构造可以在堆栈中的任意位置声明。运行时配置在合成时延迟解析，因此无论声明顺序如何，所有值都会被包含。
:::
</Fragment>
<Fragment slot="terraform">
...
:::note
关于运行时配置如何存储在 AWS AppConfig 中，以及服务端消费者（Lambda 函数、代理）如何检索配置的详细信息，请参阅 <Link path="guides/runtime-config">运行时配置</Link> 指南。
:::
