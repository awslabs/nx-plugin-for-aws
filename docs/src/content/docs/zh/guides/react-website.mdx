...
<FileTree>
  ...
  - src
    ...
    - components
      ...
      - RuntimeConfig 在启动时加载 `runtime-config.json` 的提供者
      - app-sidebar.tsx 侧边栏导航（仅限 shadcn）
      - alert.tsx 包装您的 UX 提供者自己的 Alert 组件
      - spinner.tsx 包装您的 UX 提供者自己的 Spinner 组件
    - hooks
      - useRuntimeConfig.tsx 用于读取[运行时配置](#runtime-configuration)的钩子
...
选择 `S3_MANAGED` 会导致您项目的 `checkov` 目标在网站和分配日志存储桶上失败，错误为 `CKV_AWS_145`。该目标针对您项目自己的 `checkov.yml` 运行 `checkov`，因此将检查添加到其 `skip-check` 列表中：
...
`checkov` 目标传递 `--config-file`，这会关闭 `checkov` 自己对 `.checkov.yaml` 的发现 — 在其他任何地方放置的配置文件都会被忽略。
:::
...
在自动创建的密钥上禁用密钥轮换会导致您项目的 `checkov` 目标失败，错误为 `CKV_AWS_7`。将其添加到 `packages/infra/checkov.yml` 中的同一个 `skip-check` 列表中：
...
来自基础设施的配置通过<Link path="guides/runtime-config">运行时配置</Link>提供给您的网站。这允许您的网站访问诸如 API URL 之类的详细信息，这些信息在应用程序部署之前是未知的。
...
您的网站生成时带有一个 `RuntimeConfigProvider`（在 `src/components/RuntimeConfig` 中），它在渲染应用程序之前加载 `runtime-config.json`，以及一个用于从中读取值的 `useRuntimeConfig` 钩子：
...
当没有提供 `runtime-config.json` 时，提供者会回退到空配置，因此尚未部署任何内容的网站仍然可以渲染。`IRuntimeConfig` 默认类型为 `any` — 如果您想要类型安全的访问，请在 `src/components/RuntimeConfig/index.tsx` 中缩小它。
