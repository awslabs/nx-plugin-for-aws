...
`dev` 是下面描述的 `serve-local` 目标的别名。当您需要控制应用程序在本地运行的部分与指向已部署 AWS 基础设施的部分时，`serve` 和 `serve-local` 仍然可用。有关跨连接项目的本地开发的更广泛概述，包括 `dev` 在具有多个组件的项目中的行为方式，请参阅<Link path="guides/local-development">本地开发</Link>指南。
...
`serve-local` 目标（由 `dev` 别名）启动网站的本地开发服务器（设置 [Vite `MODE`](https://vite.dev/guide/env-and-mode) 为 `serve-local`），并同时启动通过 <Link path="/guides/connection">连接生成器</Link> 关联的本地 API 服务器。
