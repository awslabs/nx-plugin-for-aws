...
<FileTree>
  ...
  - package.json 项目清单，定义项目的包名和依赖项
</FileTree>

:::note[不使用TanStack Router]
...
默认情况下，分发使用默认的 CloudFront 域名（`*.cloudfront.net`）及其默认证书，该证书不支持强制执行最低 TLS 版本 1.2。要从您自己的域名提供网站服务，请提供一个 [ACM 证书](https://docs.aws.amazon.com/acm/latest/userguide/acm-overview.html)（必须位于 `us-east-1` 区域才能与 CloudFront 一起使用）和您的域名 — 然后将为查看者强制执行最低 TLS 版本 1.2：
...
[React 文档](https://react.dev/learn) 是学习 React 基础的良好起点。

<OptionFilter when={{ ux: 'cloudscape' }} description="Cloudscape component docs pointer">
您可以参考 [Cloudscape 文档](https://cloudscape.design/components/)，了解可用组件及其使用方法的详细信息。
</OptionFilter>

<OptionFilter when={{ ux: 'shadcn' }} description="Shadcn component docs pointer">
您可以参考 [shadcn/ui 文档](https://ui.shadcn.com/docs)，了解可用组件及其使用方法的详细信息。
</OptionFilter>
...
您的网站已默认配置了 [TanStack Router](https://tanstack.com/router/v1)，添加新路由非常简单：
...
基础设施配置通过 <Link href="guides/runtime-config">运行时配置</Link> 提供给网站，这使得网站可以访问部署时才能确定的参数（如 API 地址）。

### 基础设施配置

<Infrastructure>
<Fragment slot="cdk">
使用 `RuntimeConfig` CDK 构造管理配置。通过 `@aws/nx-plugin` 生成器（如 <Link path="guides/trpc">`ts#api`</Link> 和 <Link path="guides/fastapi">`py#api`</Link>）创建的 CDK 构造会自动向 `RuntimeConfig` 添加值。

网站 CDK 构造会将运行时配置的 `connection` 命名空间部署为 S3 根目录下的 `runtime-config.json` 文件。
...
:::note[CDK网站构造]
在 CDK 中，网站构造可以在堆栈的任意位置声明。运行时配置会在合成时延迟解析，因此无论声明顺序如何，所有值都会被包含。
:::
</Fragment>
<Fragment slot="terraform">
Terraform 通过 runtime-config 模块管理配置。通过 `@aws/nx-plugin` 生成器（如 <Link path="guides/trpc">`ts#api`</Link> 和 <Link path="guides/fastapi">`py#api`</Link>）创建的 Terraform 模块会自动更新运行时配置。

网站 Terraform 模块会将运行时配置的 `connection` 命名空间部署为 S3 根目录下的 `runtime-config.json` 文件。
...
:::note[运行时配置]
有关运行时配置如何存储在 AWS AppConfig 中，以及服务器端消费者（Lambda 函数、代理）如何检索配置的详细信息，请参阅 <Link path="guides/runtime-config">运行时配置</Link> 指南。
:::
...
本地开发的标准命令是 `dev`，它通过一条命令启动您的网站（以及您已连接的任何 API 的本地服务器）：

<NxCommands commands={['dev <my-website>']} />

`serve` 目标也可用于需要控制应用程序在本地运行的部分与指向已部署 AWS 基础设施的部分时使用。有关跨连接项目的本地开发的更广泛概述，包括 `dev` 在具有多个组件的项目中的行为方式，请参阅<Link path="guides/local-development">本地开发</Link>指南。
...
`dev` 目标启动网站的本地开发服务器（设置 [Vite `MODE`](https://vite.dev/guide/env-and-mode) 为 `local-dev`），并同时启动通过 <Link path="/guides/connection">连接生成器</Link> 关联的本地 API 服务器。
...
:::note[工作区 `dev` 脚本]
您的工作区根目录 `package.json` 包含一个 `dev` 脚本，它会为每个拥有 `dev` 目标的项目运行该目标：

<PackageManagerShortCommand commands={["dev"]} />

这会在一个命令中启动您的网站（以及任何其他具有 `dev` 目标的项目）的本地开发服务器。如果只想运行此网站，请使用其自己的 `dev` 目标（例如 `nx dev <my-website>`）。
:::

:::warning[模拟认证]
在此模式下若未配置 `runtime-config.json`，如果配置了 Cognito 认证（通过 <Link path="/guides/react-website-auth">`ts#website#auth` 生成器</Link>），登录流程将被跳过，请求不会携带认证头。

如需启用 `dev` 的登录认证功能，请部署基础设施后加载运行时配置。
:::
...
使用 `build` 目标构建您的网站。该目标会运行 `bundle`、`compile`、`test` 和 `lint` 目标，执行类型检查、打包、测试和代码检查。

<NxCommands commands={['build <my-website>']} />

`bundle` 目标使用 Vite 在根目录 `dist/packages/<my-website>/bundle` 下创建生产包。这是您的网站基础设施所使用的可部署构件。您可以单独运行它：
...
根据选择的 `iac`，React 网站生成器会创建 CDK 或 Terraform 基础设施代码用于部署。
...
<Fragment slot="terraform">
推荐使用 <Link path="/guides/terraform-project">`terraform#project` 生成器</Link> 创建 Terraform 项目。
