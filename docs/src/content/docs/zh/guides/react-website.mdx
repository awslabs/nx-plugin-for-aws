...
:::note[在您需要时添加]
`src/hooks/useRuntimeConfig.tsx` 和 `src/components/RuntimeConfig` 由 <Link path="/guides/react-website-auth">`ts#website#auth`</Link> 生成器和 <Link path="guides/connection">`connection`</Link> 生成器添加到您的网站，因为它们会将值放入运行时配置中。没有这两者的网站没有要读取的运行时配置，因此在其中一个运行之前不会提供该钩子。
:::
