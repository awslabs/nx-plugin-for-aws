...
:::note[`dev` 脚本]
如果在生成器运行时根目录 `package.json` 中不存在 `dev` 脚本，系统会自动添加一个 `dev` 脚本来调用生成网站的 `serve-local` 目标，这意味着您也可以使用以下命令启动本地开发服务器：

<PackageManagerShortCommand commands={["dev"]} />

这使得您生成的第一个 React 网站成为工作区的默认 `dev` 目标。后续生成的网站不会覆盖现有的 `dev` 脚本 — 您可以直接调用它们的 `serve-local` 目标，或手动更新脚本。
:::
