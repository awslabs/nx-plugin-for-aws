...
<FileTree>
  ...
  - src
    ...
    - providers.tf 带 S3 后端配置的 Provider 文件
    - variables.tf 输入变量定义
    - outputs.tf 输出值定义
    - env 环境特定变量文件
      - dev.tfvars 开发环境变量
  - bootstrap 远程状态引导配置
    - main.tf 状态存储的 S3 存储桶及策略
...
:::note
生成器还会创建 `packages/common/terraform` 库，该库初始仅添加了一个指标模块（创建包含元数据的空 Cloudformation 堆栈），该模块已自动集成以便 `nx-plugin-for-aws` 团队跟踪使用指标。
:::
...
您可以在 `src/main.tf` 中开始编写基础设施代码，例如：
...
在 `src/env/*.tfvars` 文件中配置环境特定变量。要新增环境，需：

1. 创建 `src/env/<环境名>.tfvars` 文件并定义环境变量
2. 在 `project.json` 中为 `apply, destroy, init, plan` 目标添加新环境配置
...
不同环境的 tfstate 文件会自动隔离，支持随时切换环境。
:::

### 远程状态引导（仅限应用项目）

对于应用项目，在部署基础设施前需要引导远程状态后端（创建存储 Terraform 状态文件的 S3 存储桶）：
...
该目标仅适用于应用类型项目。库项目作为可复用模块无需远程状态管理。
:::
...
安全测试结果位于根目录 `dist` 文件夹下的 `dist/packages/<terraform项目名>/checkov`。
...
计划文件将生成于 `dist/packages/<terraform项目名>/terraform/dev.tfplan`。
