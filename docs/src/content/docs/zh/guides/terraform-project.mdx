...
Checkov 作为 `build` 和 `assemble` 的一部分运行，因此交互式 `plan` 会在您应用之前扫描您的基础设施。
...
`plan` 依赖于 `assemble` 而不是 `build`，因此它会生成模块引用的工件（Lambda 包和生成的操作元数据）并运行 Checkov，而不会同时运行每个项目的 lint、test 和 type-check 门控。

:::caution
由于 `plan` 不再运行 Terraform 模块测试或项目的单元测试，因此在应用任何您关心的内容之前运行 `build` 来检查您的代码，并在其上设置管道门控：
