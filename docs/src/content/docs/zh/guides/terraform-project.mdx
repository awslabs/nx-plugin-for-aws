...
<FileTree>
  ...
  - scripts 由 nx `bootstrap` 和 `init` 目标运行的 Node 辅助脚本
    - aws-config.ts 通过 AWS SDK 凭证链解析账户和区域
    - bootstrap.ts 拉取/推送 bootstrap tfstate 并运行 `terraform apply`
    - init.ts 使用 S3 后端配置运行 `terraform init`
    - paths.ts 在运行时定位工作区和项目根目录
...
:::tip[默认环境]
默认所有目标使用 `dev` 环境，但可通过以下方式指定环境：<NxCommands commands={['apply tf-infra --configuration=prod']} />
...
在部署基础设施前需要初始化远程状态后端（创建存储 Terraform 状态文件的 S3 桶）：
...
:::note[部署别名]
还提供了 `deploy` 目标作为 `apply` 的别名。您可以交替使用 <NxCommands commands={['deploy tf-infra']} /> 或 <NxCommands commands={['apply tf-infra']} />。
:::
