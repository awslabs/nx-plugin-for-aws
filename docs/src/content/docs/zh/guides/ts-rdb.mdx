...
此生成器创建一个由 [Amazon Aurora](https://aws.amazon.com/rds/aurora/)（PostgreSQL 或 MySQL）和 [Prisma ORM](https://www.prisma.io/docs/orm) 支持的新关系数据库项目。它生成使用 AWS CDK 或 Terraform 配置和管理数据库所需的应用程序代码和基础设施，具有声明式架构定义、自动迁移部署和类型安全的 ORM 客户端。
...
- <Link path="guides/connection/ts-strands-agent-rdb">TypeScript Strands Agent → 关系型数据库</Link>
