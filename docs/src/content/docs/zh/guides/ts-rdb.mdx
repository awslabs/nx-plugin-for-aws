...
Aurora 集群将数据库日志导出到 CloudWatch Logs（PostgreSQL 为 `postgresql`，MySQL 为 `error` 和 `slowquery`），并在写入实例上启用 Performance Insights（使用集群的 KMS 密钥加密），以便查询可观察以进行故障排除和审计。请注意，CloudWatch Logs 和 Performance Insights 会产生额外费用。
