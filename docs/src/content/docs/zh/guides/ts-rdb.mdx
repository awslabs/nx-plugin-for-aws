...
:::note[从包目录运行 Prisma]
使用 Prisma CLI 时，从生成的数据库包目录（例如 `packages/mydb`）运行命令，以便 Prisma 可以找到 `prisma/schema.prisma` 和 `prisma.config.ts`。
:::

您也可以使用 `prisma` 目标从工作区根目录运行：
...
使用生成的 `prisma` 目标，它会在运行前通过 Docker 自动启动本地数据库：
...
:::note
有关中间件模式的更多详细信息，请参阅 <Link path="/zh/guides/trpc">`ts#trpc-api` 生成器指南</Link>。
...
使用 `prisma` 目标对本地数据库运行任何 Prisma CLI 命令。它会在运行前通过 `serve-local` 自动启动数据库，因此无需单独设置本地数据库。

<NxCommands commands={['run <project>:prisma migrate dev']} />

<NxCommands commands={['run <project>:prisma migrate deploy']} />

`prisma` 目标会自动设置 `SERVE_LOCAL=true`，`prisma.config.ts` 使用它从生成时配置的本地凭证构造数据库连接 URL。

### Prisma Studio

[Prisma Studio](https://www.prisma.io/studio) 是本地数据库的可视化编辑器。使用它浏览表、检查和编辑记录、过滤数据、跟踪关系，并通过内置 SQL 控制台运行原始 SQL——对于在开发期间验证迁移和填充测试数据很有用。使用以下命令启动它：
