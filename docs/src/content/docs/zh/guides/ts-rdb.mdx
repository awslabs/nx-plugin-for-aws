...
本地数据库镜像在数据库项目根目录中生成的 `config.json` 文件的 `serveLocal.image` 字段中配置。当您更改引擎版本时，请更新该值。
...
<CardGrid>
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/trpc-rdb`}
    source="trpc"
    target="aurora"
  />
  <ConnectionCard
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/ts-agent-rdb`}
    source="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/ts-mcp-server-rdb`}
    source="mcp"
    target="aurora"
  />
</CardGrid>
