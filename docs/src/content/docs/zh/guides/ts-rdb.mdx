...
<FileTree>
  ...
  - scripts
    - pull-image.ts 拉取用于本地开发的数据库容器镜像
...
停止 `serve-local`（例如使用 `Ctrl+C`）会自动删除本地数据库容器，但保留命名卷，因此您的数据在重启后仍然保留。

:::caution[Windows]
由于 Windows 上信号处理的限制，停止 `serve-local` 时容器不会自动删除。您需要手动删除它：
...
默认情况下，Aurora 写入实例上启用了 Performance Insights（使用集群的 KMS 密钥加密）。您还可以将 Aurora 引擎日志导出到 [CloudWatch Logs](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/USER_LogAccess.html)（[Aurora PostgreSQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraPostgreSQL.CloudWatch.html) 为 `postgresql`；[Aurora MySQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraMySQL.Integrating.CloudWatch.html) 为 `audit`、`error`、`general` 和 `slowquery`）。两者都会产生额外费用，因此日志导出默认关闭，可以按数据库启用：
