...
生成器会根据所选 `iac` 创建基础设施即代码（IaC）用于部署函数：
...
若生成 Lambda 函数时指定了 `event`，函数将使用 [AWS Lambda Powertools 的 `@event_parser`](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/) 进行类型校验。例如：
...
如果您选择了 `Any` 作为 `event`，则不会应用 `@event_parser` 装饰器，处理程序将接收未类型化的事件字典。如果您需要 Pydantic 验证和类型化字段访问，请使用特定的 `event` 重新生成。
</OptionFilter>
...
2. 使用 [`uv pip install`](https://docs.astral.sh/uv/reference/cli/#uv-pip-install) 为 Lambda 目标平台 (`x86_64-manylinux_2_28`) 安装依赖

:::note[架构变更]
若需切换至 [ARM 架构](https://docs.aws.amazon.com/lambda/latest/dg/foundation-arch.html)，需修改打包目标为 `aarch64-manylinux_2_28`。
:::
