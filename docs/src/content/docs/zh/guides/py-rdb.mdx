...
<FileTree>
  - \<name>
    - \_\_init\_\_.py 包导出（`get_engine`、`session_context`、`get_session`）
...
从数据库包中导入 `session_context` 并将其用作上下文管理器以获取 SQLModel `Session`：
...
当直接连接到 Aurora 集群（不使用 RDS Proxy）时，使用可选的 `rds_ca` 参数传递 Amazon RDS CA 捆绑包路径：
...
`rds_ca` 值必须指向运行时文件系统中可用的 CA 证书捆绑包。对于 Amazon RDS，请使用以下全局 CA 捆绑包：

```text
https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
```

如何使该文件可用取决于连接到数据库的运行时。

#### 运行时容器镜像

如果您为运行时准备自己的容器镜像，请在 Dockerfile 中添加 RDS CA 捆绑包，并将该路径传递给 `session_context`：
...
如果您部署压缩的 Lambda 函数，请附加 `RdsCaLayer` Lambda 层（CDK）或 `ca-layer` Terraform 模块。CDK 层使用 Docker 捆绑来添加 CA 捆绑包，部署的层在 `/opt/global-bundle.pem` 处提供它，您可以将其传递给 `session_context`：
...
<CardGrid>
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/py-agent-rdb`}
    source="strands"
    sourceBadge="python"
    target="aurora"
    targetBadge="python"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/py-fast-api-rdb`}
    source="fastapi"
    target="aurora"
    targetBadge="python"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/py-mcp-server-rdb`}
    source="mcp"
    sourceBadge="python"
    target="aurora"
    targetBadge="python"
