...
此生成器创建一个新的 Python 关系数据库项目，由 [Amazon Aurora](https://aws.amazon.com/rds/aurora/)（PostgreSQL 或 MySQL）、[SQLModel](https://sqlmodel.tiangolo.com/) 用于数据建模，以及 [Alembic](https://alembic.sqlalchemy.org/) 用于模式迁移提供支持。它生成应用程序代码和基础设施，使用 AWS CDK 或 Terraform 来配置和管理数据库，具有声明式模式定义、自动迁移部署和数据库客户端。
...
生成的项目使用 [SQLModel](https://sqlmodel.tiangolo.com/) 来定义您的数据库模式。工作流程是模型优先的：在数据库项目的 `<name>/models/` 目录下添加或更新 SQLModel 表类，然后从这些模型更改生成迁移。
...
将 `<engine>` 替换为您的容器引擎（`docker` 或 `finch`），将 `<scope>` 替换为您的 Nx 工作空间作用域（例如 `proj`），将 `<db-name>` 替换为您的数据库项目名称（例如 `my-db`）。
:::
...
`my_scope_my_db` 是您的数据库项目的模块名称——一个名为 `<scope>_<name>` 的单一扁平模块，在项目内部显示为包目录。
...
对于 zip 部署的 Lambda 函数（例如 <Link path="guides/fastapi">`py#api` FastAPI</Link>），Amazon Linux 2023 Lambda 执行环境的内置 CA 信任存储包含 RDS 使用的 Amazon 根 CA。
...
使用 <Link path="guides/connection">`connection`</Link> 生成器将此项目与工作空间中的其他项目集成。以下连接涉及此项目：
