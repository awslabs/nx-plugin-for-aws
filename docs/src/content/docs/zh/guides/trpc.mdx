...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayRestApi' }} description="流式订阅 — 仅限 REST API,使用 SSE">
### 订阅(流式传输)
...
<OptionFilter when={{ auth: ['IAM', 'Custom'] }} description="IAM 或无身份验证的 CDK 用法">
```ts {6-8}
...
<OptionFilter when={{ auth: 'Cognito' }} description="Cognito 身份验证的 CDK 用法 — 传递身份构造">
```ts {6,9}
...
<OptionFilter when={{ auth: ['IAM', 'Custom'] }} description="IAM 或无身份验证的 Terraform 用法">
```hcl {1-3, 8}
module "asset_bucket" {
...
<OptionFilter when={{ auth: 'Cognito' }} description="Cognito 身份验证的 Terraform 用法 — 提供用户池和客户端">
```hcl {1-3, 8-9}
module "asset_bucket" {
  source = "../../common/terraform/src/core/asset-bucket"
}
...
<OptionFilter when={{ auth: 'Custom' }} description="当身份验证为 None 时可能看到的 Checkov 失败">
:::caution[无认证]
