...
:::tip[API类型]
如果您希望使用 [tRPC 订阅](https://trpc.io/docs/server/subscriptions)来流式传输响应,请选择 `rest-lambda`(默认)作为您的 `infra`。
:::
...
tRPC API 生成器根据您选择的 `iac` 创建 CDK 或 Terraform 基础设施即代码。您可以使用此代码部署 tRPC API。
...
可以使用 <Link path="/guides/react-website-auth">`ts#website#auth` 生成器</Link>生成 `UserIdentity` 构造。
</OptionFilter>
