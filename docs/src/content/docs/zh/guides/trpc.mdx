...
当您使用 `auth: 'cognito'` 部署时，API Gateway Cognito User Pools 授权器会验证调用者在 `Authorization` 标头中提供的 JWT，并将验证的声明放在 Lambda 事件的 `event.requestContext.authorizer.claims` 上。我们的中间件只是读取这些声明 — 无需额外的 AWS SDK 调用，无需手动 JWT 验证。
