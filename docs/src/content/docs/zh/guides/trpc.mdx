...
:::tip
`integrationStyle` 选项默认为 `Individual Functions`,它会为每个 tRPC 过程创建一个 Lambda。如果您希望整个 API 使用单个共享的 Lambda 处理程序,请选择 `Router`。
:::
...
tRPC 生成器通过 `integrationStyle` 选项支持两种基础设施模式:

- `Individual Functions` 是默认选项。它为每个过程生成一个 Lambda 函数。
- `Router` 为整个 API 生成一个共享的 Lambda 函数,并在每个集成中重用它。

`Individual Functions` 为您提供更细粒度的基础设施,使得为每个操作附加不同的权限或配置更容易。`Router` 减少了 Lambda 和集成的数量,这对于较小的 API 或当您希望使用单个中央处理程序时很有用。

两种集成样式目前都支持 `ServerlessApiGatewayRestApi` 和 `ServerlessApiGatewayHttpApi`。
