...
:::tip[API类型]
如果您希望使用 [tRPC 订阅](https://trpc.io/docs/server/subscriptions)来流式传输响应,请选择 `ServerlessApiGatewayRestApi`(默认)作为您的 `computeType`。选择 `BedrockAgentCoreRuntimeWebSocket` 可将 tRPC 服务器部署为 Bedrock AgentCore 上的容器,并支持流式 WebSocket 订阅 — 请参阅下面的[部署到 Bedrock AgentCore](#deploying-to-bedrock-agentcore)。
:::

:::tip[集成模式]
`integrationPattern` 选项默认为 `isolated`,它会为每个 tRPC 过程创建一个 Lambda。如果您希望整个 API 使用单个共享的 Lambda 处理程序,并可选择按过程覆盖,请选择 `shared`。当 `computeType` 为 `BedrockAgentCoreRuntimeWebSocket` 时,`integrationPattern` 选项不适用。
:::
...
:::caution[订阅计算类型]
订阅仅在 `computeType` 为 `ServerlessApiGatewayRestApi`(默认)或 `BedrockAgentCoreRuntimeWebSocket` 时受支持,因为 API Gateway HTTP API 不支持响应流式传输。
:::
...
## 部署到 Bedrock AgentCore

将 `computeType` 设置为 `BedrockAgentCoreRuntimeWebSocket` 会将 tRPC 服务器部署为 [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) 上的容器化 WebSocket 服务器,而不是 API Gateway + Lambda。AgentCore 为您提供:

- **通过 WebSocket 的流式订阅** — 为 tRPC 订阅提供全双工消息传递,没有 30 秒的 Lambda 或 API Gateway 超时限制。
- **会话亲和性** — `x-amzn-bedrock-agentcore-runtime-session-id` 标头会传播到您的 tRPC 上下文中作为 `ctx.sessionId`,因此您可以键入每个会话的状态。
- **托管运行时** — AgentCore 处理扩展、健康检查(`/ping`)和授权。

:::caution[认证选项]
AgentCore 仅支持 `IAM` 或 `Cognito` 认证。选择 `auth=None` 并使用 `computeType=BedrockAgentCoreRuntimeWebSocket` 将导致生成失败。
:::

### 工作原理

当您选择 `BedrockAgentCoreRuntimeWebSocket` 时,生成器会生成:

- `src/handler-agentcore.ts` — 一个 Node WebSocket + HTTP 服务器,为 AgentCore 健康检查暴露 `/ping`,并通过 `@trpc/server/adapters/ws` 为 tRPC 订阅暴露 `/ws`。
- `Dockerfile` — 容器化打包的服务器,并包含 [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) 以实现托管可观测性。
- 一个 CDK 构造(或 Terraform 模块),用于配置 Bedrock AgentCore `Runtime`,授予其调用 Bedrock 模型和读取运行时配置的权限,并将运行时 ARN 发布到 `runtimeConfig.apis.<YourApiName>` 供前端使用。
- 一个 `docker` 构建目标,连接到项目的 `build` 目标,以便 `nx build` 生成容器镜像。

### 访问会话 ID

当 API 部署到 AgentCore 时,每个过程都会接收 `ctx.sessionId`:
