...
tRPC API 生成器会创建一个配置了 AWS CDK 基础设施的新 tRPC API。生成的后端使用 AWS Lambda 进行无服务器部署，并通过 [Zod](https://zod.dev/) 实现模式验证。同时集成了 [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) 用于可观测性，包括日志记录、AWS X-Ray 追踪和 CloudWatch 指标。
...
:::note
在 monorepo 根目录的 `tsconfig.base.json` 中添加了别名，将 Zod 解析到 [4.x 版本](https://zod.dev/v4)。这是为 Zod v4 正式发布到 NPM 时预留升级路径，无需修改导入路径。
:::
...
模式在服务端和客户端代码中共享，为 API 使用的数据结构变更提供单一更新点。运行时 tRPC API 会自动验证模式，省去手动编写后端验证逻辑的麻烦。
...
- `input` 接受定义操作预期输入的 Zod 模式，请求会自动验证该模式
- `output` 接受定义操作预期输出的 Zod 模式，若实现返回不符合模式的输出将产生类型错误
- `query` 接收定义操作实现的函数，通过 `opts` 获取输入及中间件设置的上下文 (`opts.ctx`)，返回值需符合 `output` 模式

使用 `query` 表示非变更操作，用于数据检索。变更操作应使用 `mutation` 方法。新增过程后需在 `src/router.ts` 中注册。
...
AWS Lambda Powertools 日志记录器在 `src/middleware/logger.ts` 配置，可通过 `opts.ctx.logger` 访问。例如：
...
AWS Lambda Powertools 指标在 `src/middleware/metrics.ts` 配置，通过 `opts.ctx.metrics` 访问。例如：
...
AWS Lambda Powertools 追踪器在 `src/middleware/tracer.ts` 配置，通过 `opts.ctx.tracer` 访问。例如：
...
生成器在 `common/constructs` 目录创建 CDK 构造。示例 CDK 应用：
...
此配置包括 AWS API Gateway REST/HTTP API、业务逻辑的 Lambda 函数及认证方式。
...
:::tip
tRPC API 的过程增删会立即反映在 CDK 构造中，无需重新构建。
:::
...
入口文件为 `src/local-server.ts`，修改代码后会自动重载。
