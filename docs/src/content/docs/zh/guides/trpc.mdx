...
<OptionFilter when={{ auth: ['IAM', 'None'] }} description="IAM 或无身份验证的 CDK 用法">
```ts {6-8}
...
<OptionFilter when={{ auth: ['IAM', 'None'] }} description="IAM 或无身份验证的 Terraform 用法">
```hcl {1-3, 8}
module "asset_bucket" {
...
<OptionFilter when={{ auth: 'None' }} description="当身份验证为 None 时可能看到的 Checkov 失败">
:::caution[无认证]
