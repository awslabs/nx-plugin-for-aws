...
tRPC 生成器通过 `integrationPattern` 选项支持两种基础设施模式:

- `isolated` 是默认选项。它为每个过程生成一个 Lambda 函数。
- `shared` 为整个 API 生成一个默认的 `$router` Lambda 集成,并为每个过程重用它,除非您提供按过程覆盖。

`isolated` 为您提供更细粒度的基础设施,使得为每个操作附加不同的权限或配置更容易。`shared` 减少了 Lambda 和集成的数量,同时仍允许您在需要时使用专用集成覆盖特定过程。

两种集成模式目前都支持 `ServerlessApiGatewayRestApi` 和 `ServerlessApiGatewayHttpApi`。
