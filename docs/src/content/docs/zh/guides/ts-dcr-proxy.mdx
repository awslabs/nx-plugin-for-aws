...
:::tip[重用现有的 User Pool]
如果您已经从 <Link path="guides/react-website-auth">`ts#website#auth`</Link> 生成器获得了一个 User Pool，您可以为相同的网站用户提供 MCP 服务器前端。请参阅下面的<Link path="guides/ts-dcr-proxy#reusing-a-useridentity-user-pool">重用 UserIdentity User Pool</Link>。
:::
...
`UserIdentity` 使用 [Managed Login (version 2)](https://docs.aws.amazon.com/cognito/latest/developerguide/managed-login.html) 配置 User Pool 域。Managed Login 要求**每个 App Client 都有一个品牌样式**，因此您必须为新的代理客户端创建一个 — 否则其托管登录页面会返回 `403`。
...
// ts#website#auth 为您的网站用户创建的 user pool
const identity = new UserIdentity(this, 'Identity');

// DCR 代理在相同 user pool 上的机密 App Client
const proxyClient = identity.userPool.addClient('DcrProxyClient', {
  generateSecret: true,
  oAuth: {
    flows: { authorizationCodeGrant: true },
...
<Fragment slot="terraform">
```hcl
# ts#website#auth 为您的网站用户创建的 user pool 模块
