...
在 DynamoDB 中，模式设计从查询开始，而不是数据形状。在编写任何模型之前，列出应用程序需要的每个访问模式，然后设计 `pk`、`sk` 和 GSI 键值，使每个模式都由单个表请求回答 —— 没有 JOIN，没有顺序读取。
...
:::caution[PynamoDB 不是专门为单表设计而构建的]
PynamoDB 的 `DiscriminatorAttribute` 是为单个同构表中的类层次结构多态性设计的 —— 而不是在一个表中存储结构不同的实体类型。这里使用的模式有效，但请记住以下注意事项：

- **键冲突是您的责任。** PynamoDB 不会阻止两种实体类型写入相同的 `pk`/`sk` 对。始终使用唯一的类型前缀（例如 `ORDER#`、`USER#`）。
- **跨实体查询需要 `BaseModel`。** PynamoDB 在查询特定子类时会自动添加鉴别器过滤器，因此同一键上其他实体类型的项目永远不会被返回。要从共享的 GSI 分区键检索混合类型，始终通过 `BaseModel` 查询。
- **共享表设置。** 所有实体共享在 `BaseModel` 中定义的 `Meta` 配置（表名、区域、凭证）。
:::
...
`BaseModel` 提供两个共享的 GSI（`gsi1_index`、`gsi2_index`）。上面的 `UserModel` 和 `OrderModel` 都写入 `gsi2` —— 但使用不同的 `gsi2pk` 值（`USER` 与 `ORDER`）。这就是 **GSI 重载**：重用单个物理索引来服务多个独立的访问模式，而不消耗额外的 GSI 容量。

- **`UserModel`** —— `gsi2pk=USER`，`gsi2sk=<created_at>` → 按日期列出所有用户
- **`OrderModel`** —— `gsi2pk=ORDER`，`gsi2sk=<created_at>` → 按日期列出所有订单

当多个实体类型共享同一父级时，`gsi1` 也可以重载。如果您稍后添加一个也属于用户的 `ReviewModel`，您可以为其分配 `gsi1pk=USER#<user_id>` 和 `REVIEW#<id>` 排序键 —— 不需要额外的 GSI。然后通过 `BaseModel` 查询 `gsi1` 会在一个请求中返回该用户的订单和评论，PynamoDB 将每个项目实例化为其正确的子类：
...
GSI 查找的替代方案是**项目集合**模式：给子项目与其父级相同的 `pk`，并使用排序键来区分它们。这使您可以在单个主表查询中检索父级及其所有子级，而无需 GSI：
...
权衡：项目集合将所有子项放在单个分区键下，这对于大多数工作负载是最佳的，但在极端写入吞吐量下可能会创建热分区。GSI 方法（在上面的示例中使用）将每个实体保留在自己的分区中，通常更安全。

#### 多对多关系

多对多关系需要使用[邻接列表模式](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-adjacency-graphs.html)的**连接实体**：一个专用项目记录每个链接，其 GSI 键反转方向，以便可以双向遍历关系。
...
生成的 `client.py` 导出两个关键实用程序：

- `is_local()` —— 当 `LOCAL_DEV=true` 时返回 `True`，用于在本地和 AWS 行为之间切换。
- `get_table_name()` —— 返回 DynamoDB 表名。当 `LOCAL_DEV=true` 时，从 `config.json` 中的 `localDev.tableName` 读取表名；否则使用 `RUNTIME_CONFIG_APP_ID` 环境变量从 AWS AppConfig 获取名称并缓存以供后续调用。

`entities/base.py` 中的 `BaseModel` 使用两者来自动配置 PynamoDB：

- **连接** —— `BaseModel.Meta` 从 `config.json` 设置 `host`，并在 `is_local()` 为 `True` 时硬编码 `region`、`aws_access_key_id` 和 `aws_secret_access_key`，将 PynamoDB 指向本地 DynamoDB 实例。在 AWS 中，这些保持未设置状态，因此 PynamoDB 使用默认凭证链。
...
在幕后，`ExampleModel` 调用 `get_table_name()` 在运行时从 AWS AppConfig 获取表名。

<Snippet name="runtime-config-app-id-note" parentHeading="连接到表" />
