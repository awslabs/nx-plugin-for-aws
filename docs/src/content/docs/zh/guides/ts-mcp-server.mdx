---
title: "ts#mcp-server"
description: "为大型语言模型提供上下文的生成 TypeScript 模型上下文协议（MCP）服务器"
---



import { FileTree } from '@astrojs/starlight/components';
import RunGenerator from '@components/run-generator.astro';
import NxCommands from '@components/nx-commands.astro';
import Link from '@components/link.astro';
import GeneratorParameters from '@components/generator-parameters.astro';
import schema from '../../../../../../packages/nx-plugin/src/ts/mcp-server/schema.json';

# TypeScript MCP 服务器生成器

生成用于为大型语言模型（LLMs）提供上下文的 TypeScript [模型上下文协议（MCP）](https://modelcontextprotocol.io/) 服务器。

## 什么是 MCP？

[模型上下文协议（MCP）](https://modelcontextprotocol.io/) 是一个开放标准，允许 AI 助手与外部工具和资源交互。它为 LLMs 提供了一致的方式来：

- 执行执行操作或检索信息的工具（函数）
- 访问提供上下文或数据的资源

## 使用方式

### 生成 MCP 服务器

您可以通过两种方式生成 TypeScript MCP 服务器：

<RunGenerator generator="ts#mcp-server" />

### 选项参数

<GeneratorParameters schema={schema} />

## 生成器输出

生成器将创建以下项目文件：

<FileTree>
  - packages/\<name>/
    - README.md MCP 服务器文档及使用说明
    - project.json Nx 项目配置，包含 build、bundle 和 dev 目标
    - src/
      - index.ts MCP 服务器入口文件
      - server.ts 主服务器定义，声明工具和资源
      - global.d.ts 用于导入 Markdown 文件的 TypeScript 类型声明
      - resources/
        - example-context.md 作为 MCP 服务器资源使用的示例 Markdown 文件
</FileTree>

:::note
更多关于生成器输出的详细信息，请参考 <Link path="/guides/typescript-project">TypeScript 项目生成器文档</Link>。
:::

## 开发 MCP 服务器

### 添加工具

工具是 AI 助手可调用的执行操作的函数。您可以在 `server.ts` 文件中添加新工具：

```typescript
server.tool("工具名称",
  { param1: z.string(), param2: z.number() }, // 使用 Zod 的输入模式
  async ({ param1, param2 }) => {
    // 工具实现
    return {
      content: [{ type: "text", text: "执行结果" }]
    };
  }
);
```

### 添加资源

资源为 AI 助手提供上下文。您可以添加静态文件资源或动态资源：

```typescript
// 从文件导入静态资源
import resourceContent from './resources/my-resource.md';

server.resource('资源名称', 'example://resource', async (uri) => ({
  contents: [{ uri: uri.href, text: resourceContent }],
}));

// 动态资源
server.resource('动态资源', 'dynamic://resource', async (uri) => {
  const data = await fetchSomeData();
  return {
    contents: [{ uri: uri.href, text: data }],
  };
});
```

## 配置 AI 助手

要使用 MCP 服务器，需要先进行打包：

<NxCommands commands={['run your-mcp-server:bundle']} />

打包文件将生成于 `dist/packages/your-mcp-server/bundle/index.js`（具体路径可能因目录设置而异）。

### 配置文件

大多数支持 MCP 的 AI 助手使用类似的配置方式。需要创建/更新包含 MCP 服务器信息的配置文件：

```json
{
  "mcpServers": {
    "your-mcp-server": {
      "command": "node",
      "args": [
        "/workspace/dist/packages/your-mcp-server/bundle/index.js"
      ],
      "transportType": "stdio"
    }
  }
}
```

请将 `/workspace/dist/packages/your-mcp-server/bundle/index.js` 替换为实际的打包文件路径。

:::caution
若出现类似 `ENOENT node` 的错误，可能需要通过终端运行 `which node` 获取 node 的完整路径。
:::

### 特定助手配置

各 AI 助手的 MCP 配置文档参考：

- [Amazon Q Developer](https://docs.aws.amazon.com/amazonq/latest/qdeveloper-ug/command-line-mcp-configuration.html)
- [Cline](https://docs.cline.bot/mcp-servers/configuring-mcp-servers)
- [Cursor](https://docs.cursor.com/context/model-context-protocol)
- [Claude Code](https://docs.anthropic.com/en/docs/agents-and-tools/claude-code/tutorials#set-up-model-context-protocol-mcp)

:::tip
部分 AI 助手（如 Amazon Q Developer）支持在项目级别配置 MCP 服务器，这在定义项目相关 MCP 服务器时非常实用。
:::

## 开发工作流

### 构建目标

本生成器基于 <Link path="/guides/typescript-project">TypeScript 项目生成器</Link>，继承了其构建目标，并新增了以下目标：

#### 打包

`bundle` 任务使用 [esbuild](https://esbuild.github.io/) 生成适用于 AI 助手的单一 JavaScript 打包文件：

<NxCommands commands={['run your-mcp-server:bundle']} />

打包文件位于 `dist/packages/your-mcp-server/bundle/index.js`（具体路径可能因目录设置而异）。

#### 开发模式

`dev` 任务监控项目变更并自动重新打包：

<NxCommands commands={['run your-mcp-server:dev']} />

该模式在开发过程中非常有用，可确保 AI 助手始终使用最新的 MCP 服务器版本。

:::note
部分 AI 助手需要重启 MCP 服务器才能使变更生效。
:::