...
生成器配置了名为 `<your-server-name>-inspect` 的目标，该目标在本地启动您的 MCP 服务器（通过 `<your-server-name>-serve-local` 目标，包括任何连接的依赖项，如本地数据库），并启动预配置的 [MCP 检查器](https://github.com/modelcontextprotocol/inspector)，通过流式 HTTP 传输连接到服务器。
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="将 TypeScript MCP 服务器连接到 Aurora 关系数据库"
  ...
  <ConnectionCard
    ...
    description="将 TypeScript MCP 服务器连接到 DynamoDB 表"
