...
`createServer` 接收一个描述当前请求的 `McpRequestContext`。通过 Streamable HTTP，这包括调用者的 AgentCore Runtime 会话 ID，<Link path="guides/ts-agent">代理</Link>会在每次出站 MCP 调用时自动转发该 ID。
...
:::caution
将存储的数据范围限定在请求上下文上，而不是工具参数上。模型可以被说服传递任何它喜欢的值，因此诸如用户或玩家名称之类的参数不是信任边界。在 STDIO 上下文中不存在上下文，因为没有请求。
:::
