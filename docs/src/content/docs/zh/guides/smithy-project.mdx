...
在 macOS 和 Linux 上，CLI 由 [mise](https://mise.jdx.dev/) 解析，构建会按需获取它，因此无需安装 — 它会在您第一次构建时下载并缓存固定版本。在 Windows 上，它是您需要自行安装的先决条件 — 请参阅<Link path="guides/ts-smithy-api#building-on-windows">在 Windows 上构建</Link>。
...
2. 将库的 `build` 目标添加为消费项目的 `compile` 目标的依赖项（在其 `project.json` 中），以便在消费者构建之前模型已存在：
