...
代理监听在生成时从工作区池分配的端口。从项目的 `project.json` 中的 `metadata.ports` 读取它，或从 `<your-agent-name>-dev` 目标的命令中的 `--port` 标志读取它。下面的示例使用 `8081`，这是在全新工作区中为第一个 HTTP 代理分配的端口。

<Snippet name="agent/dev-vs-serve" parentHeading="本地开发" />
...
您的代理使用 [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/)（ADOT）自动配置可观测性。
...
然后向您的本地代理运行的端口上的 `/invocations` 发送 POST 请求。替换分配给您的代理的端口 — 从项目的 `project.json` 中的 `metadata.ports` 读取它，或从 `<your-agent-name>-dev` 目标的命令中的 `--port` 标志读取它。在全新工作区中，第一个 HTTP 代理被分配 `8081`：
...
:::caution[使用 `-dev`，而不是 `-serve`]
为此使用 `<your-agent-name>-dev`，而不是 `<your-agent-name>-serve`。`-serve` 运行部署的代码路径，因此它需要设置 `RUNTIME_CONFIG_APP_ID` — 请参阅 <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>。如果没有它，在代理尝试解析其会话存储时流式传输已经开始，因此请求返回一个空的 `200`，没有错误，失败仅出现在服务器日志中。
:::
