...
`langchain`框架目前仅支持`ag-ui`协议。如果您需要`http`或`a2a`协议，请选择默认的`strands`框架。LangChain代理仍然可以通过<Link path="/guides/connection/py-agent-mcp">`connection`生成器</Link>连接到MCP服务器、AgentCore网关和A2A代理。
:::
...
深入指南请参考[Strands官方文档](https://strandsagents.com/docs/user-guide/quickstart/overview/)。对于LangChain代理，<Link path="/guides/connection/py-agent-mcp">`connection`生成器</Link>也会将MCP服务器、AgentCore网关和A2A代理工具连接到`create_agent`。
