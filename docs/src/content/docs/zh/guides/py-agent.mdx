...
如果您选择了`None`作为`infra`，则不会生成CDK构造或Terraform模块——Strands智能体只能在本地运行。在此模式下，`auth`选项会被忽略，因为没有需要进行身份验证的托管端点。
</OptionFilter>
...
生成器配置了一个`<your-agent-name>-chat` Nx目标，让您进入与智能体的交互式终端聊天，由[`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)提供支持。

聊天目标独立运行——它**不会**为您启动智能体。默认情况下，它连接到本地运行的智能体，因此请先启动`<your-agent-name>-serve-local`（在单独的终端中）：
...
要从React网站调用Strands智能体，可以使用<Link path="/guides/connection/react-py-agent">`connection`生成器</Link>，它会自动设置具有正确认证方式（IAM或Cognito）的客户端。
