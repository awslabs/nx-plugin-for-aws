...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain 提供了一个庞大的[工具和集成](https://docs.langchain.com/oss/python/integrations/tools)生态系统，每个都分布在自己的包中。生成器不会添加这些，因此请先安装您需要的那个 — 例如 `langchain-community`，下面的搜索工具来自该包：

<NxCommands commands={['run <project-name>:add langchain-community']} />

然后导入工具并将它们添加到 `get_agent()`：
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
生成的代理使用 Amazon Bedrock 上的默认 Strands 模型。要配置它，请将 `model` 传递给 `Agent`。有关可用提供商及其选项，请参阅 [Strands 关于模型提供商的文档](https://strandsagents.com/docs/user-guide/concepts/model-providers/)：
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain 代理使用 [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse) 模型。生成的代理从 `MODEL_ID` 和 `AWS_REGION` 环境变量读取模型 ID 和区域，基础设施为部署的代理设置这些变量。要进一步配置模型，请在 `get_agent()` 中构造它的位置添加参数：
...
还会生成一个 `<your-agent-name>-serve` 目标，它针对您部署的基础设施运行代理，因此需要设置 `RUNTIME_CONFIG_APP_ID`。有关 `dev` 和 `serve` 之间的区别，请参阅<Link path="guides/local-development">本地开发</Link>指南。
