...
- **`s3`**：部署的代理使用带有提供的会话存储桶的 `S3CheckpointSaver`，将检查点和待处理写入存储在 `checkpoints/` 前缀下。此类（改编自 [`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3)，MIT 许可）位于共享代理连接项目中的 `s3_checkpoint_saver_langchain.py` 中。
...
- **`in-memory`**（默认）：不提供表或存储桶。对话状态仅在运行进程的生命周期内保留在内存中，不会在重启或缩容后保留。
