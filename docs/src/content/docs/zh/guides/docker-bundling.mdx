...
在项目源目录中创建一个 `Dockerfile`。该文件只是将包 `COPY` 到 Node 基础镜像中,加上 `npm install` 任何无法打包的 `external` 包。将 `RUN npm install` 步骤放在 `COPY` **之前**,这样 Docker 可以缓存已安装的 `node_modules` 层,并且只在依赖项列表实际更改时重新运行它。在 npm 的缓存目录挂载缓存可以让重复构建重用已下载的包,而不是再次获取它们:
