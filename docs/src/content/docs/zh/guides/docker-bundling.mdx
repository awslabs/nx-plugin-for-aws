...
扫描故意**不**连接到 `build`。Trivy 的结果取决于其漏洞数据库,该数据库会不断更新:今天通过的扫描可能在明天新 CVE 针对您未更改的基础镜像或依赖项发布时失败。将其耦合到 `build` 会使本地和 CI 构建不确定,并阻塞无关的工作。

相反,将其作为 CI 流水线中的专用步骤运行(并在发布前在本地运行):

<PackageManagerShortCommand commands={['trivy']} />

当扫描因新发布的 CVE 而开始失败时,请升级 `Dockerfile` 中受影响的包或基础镜像,或者如果尚无修复,则在 `.trivyignore` 中抑制它(见下文)。
:::
