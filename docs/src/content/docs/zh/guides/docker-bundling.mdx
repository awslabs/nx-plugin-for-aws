...
多个生成器(例如 <Link path="/guides/ts-agent">`ts#agent`</Link> 和 <Link path="/guides/py-agent">`py#agent`</Link>)会生成一个 Docker 镜像,该镜像被推送到 Amazon ECR 并由 AWS 基础设施使用。本指南描述了它们遵循的模式,以便您可以将其应用于其他用例——例如,在 Amazon ECS 上运行 <Link path="/guides/fastapi">FastAPI</Link> 项目,或部署容器化的 Express 服务器。

:::tip[Docker 或 Finch]
用于构建镜像的容器引擎在工作空间创建时通过 `--containers` 标志选择(`docker`、`finch` 或 `infer`——默认值——会自动检测已安装的引擎)。[Finch](https://runfinch.com/) 是 Docker 的开源即插即用替代方案。该选择记录在 `aws-nx-plugin.config.mts` 中,并应用于每个生成容器构建命令的生成器。CDK 镜像资产构建通过 `CDK_DOCKER` 环境变量遵循该选择。
:::
...
扫描镜像以查找已知漏洞是一个好习惯。遵循此模式的生成器会添加一个 `trivy` 目标,该目标使用 [Trivy](https://trivy.dev/) 扫描构建的镜像,从 [ECR 托管的 Trivy 镜像](https://gallery.ecr.aws/aquasecurity/trivy)运行,并在发现 `HIGH` 或 `CRITICAL` 级别的问题时以非零状态退出。

添加一个 `dependsOn` 您的 `docker` 目标的 `trivy` 目标。它将构建的镜像保存为 tarball 并通过工作区相对绑定挂载进行扫描,因此相同的命令在 `docker` 和 `finch` 下都可以工作:

<Code lang="json" code={trivyTarget} />

每个项目的单镜像扫描目标都聚合在工作区范围的 `trivy` 目标下,由提供的 `trivy` 根脚本运行(`nx run-many --target trivy`)。
...
:::tip[缓存跳过未更改的镜像]
因为镜像完全由您的项目源代码决定,声明 `inputs`(这里是 `default` 和 `^production`,与 `bundle` 目标匹配)让 Nx 缓存扫描并跳过重新扫描未更改的镜像。固定 Trivy 镜像版本(例如 <code>trivy:<TrivyVersion /></code>),以便扫描是可重现的。
:::

:::note[抑制发现]
Trivy 从项目根目录读取 `.trivyignore` 文件(漏洞 ID 列表,每行一个)。`--ignore-unfixed` 跳过没有可用修复的漏洞,因此扫描仅在您可以通过升级 `Dockerfile` 中的工具来采取行动的问题上失败。有关详细信息,请参阅 [Trivy 过滤文档](https://trivy.dev/latest/docs/configuration/filtering/#by-finding-ids)。
:::
...
3. 一个带有 `local-exec` provisioner 的 `null_resource`,它向 ECR 进行身份验证,使用内容摘要重新标记本地构建的镜像,然后推送它。
4. 下游资源(例如 `aws_ecs_task_definition`)通过不可变的、基于摘要的标签引用镜像。
...
`data.external.docker_digest` 块确保每当本地镜像哈希更改时 `null_resource` 重新运行,在每次有意义的代码更改时触发新的推送。镜像在从其摘要派生的不可变的、基于内容的标签下推送,因此 ECR 仓库可以使用 `IMMUTABLE` 标签可变性并拒绝任何覆盖现有标签的尝试。
...
- <Link path="/guides/ts-agent">`ts#agent` 生成器</Link> —— 部署到 Bedrock AgentCore Runtime 的 TypeScript agent 的完整模式示例。
