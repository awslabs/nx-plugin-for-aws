...
扫描镜像以查找已知漏洞是一个好习惯。遵循此模式的生成器会添加一个 `trivy` 目标,该目标使用 [Trivy](https://trivy.dev/) 扫描构建的镜像,从 [ECR 托管的 Trivy 镜像](https://gallery.ecr.aws/aquasecurity/trivy)运行,并在发现 `HIGH` 或 `CRITICAL` 级别的问题时使构建失败。
...
然后让 `build` 依赖于 `trivy`,以便扫描作为构建的一部分运行:
