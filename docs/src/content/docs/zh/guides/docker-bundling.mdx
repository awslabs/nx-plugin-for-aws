...
:::tip[固定您的依赖项]
将 `Dockerfile` 内部安装的每个依赖项固定到确切的版本(如上所示)——否则 `npm install` 会在下次构建时静默地获取较新的版本,导致镜像的行为因构建时间而异。同样的建议也适用于基础镜像:优先使用特定的摘要或标签(例如 `node:22.14.0`)而不是像 `lts` 这样的浮动标签。
:::
...
1. 一个 `aws_ecr_repository` 来保存镜像。
2. 一个带有 `local-exec` provisioner 的 `null_resource`,它向 ECR 进行身份验证,重新标记本地构建的镜像,然后推送它。
3. 下游资源(例如 `aws_ecs_task_definition`)引用 `"${aws_ecr_repository.repo.repository_url}:latest"`。
