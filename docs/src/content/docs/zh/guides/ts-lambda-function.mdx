...
生成器会根据选定的 `iac` 创建用于部署函数的基础设施即代码：
...
如果在生成 Lambda 函数时选择了 `event`，您的函数将通过 [AWS Lambda Powertools 的解析器中间件](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/) 实现类型安全。例如：
...
如果您选择了 `event` 为 `Any`，则不会配置 `parser` 中间件，`event` 参数将保持 `any` 类型。如果您需要编译时类型安全和运行时验证，请使用特定的 `event` 重新生成函数。
</OptionFilter>
