...
某些选项仅适用于另一个选项的特定值。使用 `x-when` 来指定，这样文档站点会在不适用的地方省略该选项，并在适用的地方说明条件：
...
多个键使用 AND 逻辑，键内的值列表使用 OR 逻辑，与 `<OptionFilter when={{…}}>` 匹配：
...
`x-when` 记录选项；它不强制执行任何内容，因此 generator 仍然会验证或忽略不适用于给定运行的选项。
...
- 使用 `@aws/nx-plugin/sdk/utils/test` 中的 `createTreeUsingTsSolutionSetup()` 创建虚拟文件系统。它会为工作区添加我们的预设所写入的标记（`tsconfig.base.json`、`pnpm-workspace.yaml`、`biome.json`、一个 `type: module` 的根 `package.json`），这是任何组合 `@aws/nx-plugin` generator 的 generator 所需要的
