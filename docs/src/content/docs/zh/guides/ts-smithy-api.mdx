...
Smithy TypeScript API 生成器使用 Smithy 进行服务定义,并使用 [Smithy TypeScript Server SDK](https://github.com/awslabs/smithy-typescript) 进行实现,创建一个新的 API。该生成器提供 CDK 或 Terraform 基础设施即代码,将您的服务部署到 AWS Lambda,并通过 AWS API Gateway REST API 公开。它提供类型安全的 API 开发,并从 Smithy 模型自动生成代码。生成的处理程序使用 [AWS Lambda Powertools for TypeScript](https://docs.powertools.aws.dev/lambda/typescript/latest/) 进行可观测性,包括日志记录、AWS X-Ray 追踪和 CloudWatch 指标。
...
:::tip[集成模式]
`integrationPattern` 选项默认为 `isolated`,为每个 Smithy 操作创建一个 Lambda。如果您希望整个 API 使用单个共享的 Lambda 处理程序,并可选择性地覆盖每个操作,请选择 `shared`。
:::

## 生成器输出

生成器在 `<directory>/<api-name>` 目录中创建两个相关的项目:
...
- **backend/** TypeScript 后端实现
  ...
  - tsconfig.lib.json 库源代码的 TypeScript 配置
  ...
  - packages/common/constructs
    - src
      - app/ 特定于项目/生成器的基础设施构造
        - apis/
          - \<project-name>.ts 用于部署您的 API 的 CDK 构造
  ...
  - packages/common/terraform
    - src
      - app/ 特定于项目/生成器的基础设施 Terraform 模块
        - apis/
          - \<project-name>/
            - \<project-name>.tf 用于部署您的 API 的模块
...
操作在模型项目的 Smithy 文件中定义。主服务定义在 `main.smithy` 中:
...
有关如何创建形状库并将其作为 API 模型的依赖项连接起来,请参阅 <Link path="guides/smithy-project">Smithy 项目指南</Link>。

### 在 TypeScript 中实现操作

操作实现位于后端项目的 `src/operations/` 目录中。每个操作都使用从 TypeScript Server SDK 生成的类型(在构建时从您的 Smithy 模型生成)来实现。
...
AWS X-Ray 追踪通过 `captureLambdaHandler` 中间件自动配置。
...
首先,在 `src/context.ts` 中的服务上下文上公开已解析的身份。我们将其作为函数提供,以便 `UnauthorizedError` 从操作内部抛出(Server SDK 将其序列化为 `403`),而不是从处理程序抛出:
...
使用 `auth: 'cognito'` 时,API Gateway Cognito User Pools 授权器会验证调用者在 `Authorization` 标头中提供的 JWT,并将验证的声明放在事件的 `event.requestContext.authorizer.claims` 上:
...
:::tip[无需令牌验证]
您不需要 `aws-jwt-verify` 或任何其他 JWT 验证库 - API Gateway Cognito User Pools 授权器在您的 Lambda 运行之前已经验证了签名、发行者、范围和过期时间。如果这些检查中的任何一个失败,API Gateway 会返回 `401 Unauthorized`,并且您的处理程序永远不会被调用。
:::
</OptionFilter>
...
`getIdentity` 是 `ServiceContext` 上的必需字段,如[上面的注意事项](#服务上下文)所述,上下文在**两个**入口点中构造 - 因此 `src/local-server.ts` 也需要它。本地服务器前面没有 API Gateway 授权器,因此为本地开发提供一个存根身份:
...
在 macOS 和 Linux 上,CLI 由 [mise](https://mise.jdx.dev/) 解析,构建会按需获取它,因此无需安装 - 它会在您第一次构建时下载并缓存固定版本。
...
`mise` 没有向 npm 发布 Windows 包,因此在 Windows 上,Smithy CLI 是您自己安装的先决条件。按照 [Smithy CLI 安装指南](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html)安装一次(例如 `winget install smithy` 或 `scoop install smithy`),并确保 `smithy` 在您的 `PATH` 上。在 Windows 上生成的 Smithy 项目直接运行 `smithy`,而不是通过 `mise`。

:::caution[升级 CLI]
因为 CLI 是全局安装的,而不是固定在您的工作区中,所以使用 `nx migrate` 升级工作区**不会**升级它。当迁移将 Smithy 依赖项向前移动时,请自己将全局 Smithy CLI 升级到匹配的版本。
:::

或者,在 [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) 内开发,构建运行 Linux 路径,`mise` 为您解析 CLI - 无需安装。
...
:::tip[热重载]
本地服务器不仅会在您对后端代码进行 TypeScript 更改时热重载,还会在您对 Smithy 模型项目进行更改时重新加载,允许您持续迭代 Smithy 模型和服务器。
:::
...
<Infrastructure>
<Fragment slot="cdk">
用于部署您的 API 的 CDK 构造位于 `common/constructs` 文件夹中:
...
<OptionFilter when={{ auth: 'custom' }} description="自定义 Lambda 授权器 CDK 使用">
:::caution[自定义 Lambda 授权器]
使用 `Custom` 身份验证时,构造会从生成的 `src/authorizer.ts` 文件内部创建一个 Lambda 授权器,**默认拒绝所有请求**。您必须在该文件中实现您的授权逻辑,然后您的 API 才会接受任何流量。

授权器结果缓存 5 分钟,以 `Authorization` 标头为键,因此被撤销的令牌可能会被接受,直到其缓存结果过期。如果您需要更快的撤销,请在生成的基础设施中减少缓存持续时间。
:::
</OptionFilter>
</Fragment>
<Fragment slot="terraform">
用于部署您的 API 的 Terraform 模块位于 `common/terraform` 文件夹中。
...
1. 为 Smithy API 提供服务的 AWS Lambda 函数
2. 作为函数触发器的 API Gateway REST API
3. IAM 角色和权限
4. CloudWatch 日志组
5. X-Ray 追踪配置
...
<OptionFilter when={{ auth: 'custom' }} description="使用 Terraform 的自定义 Lambda 授权器">
:::caution[自定义 Lambda 授权器]
使用 `Custom` 身份验证时,您的 API 受 Lambda 授权器保护,**默认拒绝所有请求**。您必须在生成的 `src/authorizer.ts` 文件中实现您的授权逻辑,然后您的 API 才会接受任何流量。
...
<Snippet name="api/custom-domain" parentHeading="自定义域" />

### 集成

<Infrastructure>
<Fragment slot="cdk">
:::caution[操作名称在 CDK 中是 camelCase]
Smithy 操作以 PascalCase 声明(`Echo`),您的后端以该名称导出它们。CDK 集成键是相同名称的 **camelCase** 形式,因此操作 `Echo` 在您的堆栈中的任何地方都是 `echo`:
...
<Fragment slot="terraform">
:::note[操作名称在 Terraform 中是 camelCase]
生成的模块的操作键控输出及其 `operations.json` 使用 Smithy 操作名称的 **camelCase** 形式,因此 Smithy 操作 `Echo` 的键是 `echo`:
...
将 `generate:<ApiName>-metadata` 目标添加到通用构造 `project.json` 以促进此代码生成,它会生成一个文件,例如 `packages/common/constructs/src/generated/my-api/metadata.gen.ts`。由于这是在构建时生成的,因此在版本控制中被忽略。

:::note[部署前构建]
每当您更改 Smithy 模型时,您都需要运行构建,以确保 CDK 构造使用的类型是最新的。

<PackageManagerShortCommand commands={["build"]} />
:::

:::tip[热重载]
如果您正在同时积极开发 CDK 基础设施和 Smithy API,您可以使用 [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) 在每次进行模型更改时重新生成这些类型:
...
如果您选择了 `IAM` 身份验证,您可以使用 `grantInvokeAccess` 方法授予对您的 API 的访问权限:
