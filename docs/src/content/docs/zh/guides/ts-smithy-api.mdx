...
在 macOS 和 Linux 上，CLI 由 [mise](https://mise.jdx.dev/) 解析，构建会按需获取，因此无需安装——它会在您第一次构建时下载并缓存固定版本。在 Windows 上，CLI 是您需要自行安装的先决条件，并且升级工作区不会升级它——请参阅 Smithy 项目指南中的 <Link path="guides/smithy-project#building-on-windows">在 Windows 上构建</Link> 和 <Link path="guides/smithy-project#using-the-smithy-cli-directly">直接使用 Smithy CLI</Link>。
