...
:::note
除非选择`None`作为`computeType`，否则生成器还会创建用于部署MCP服务器的CDK或Terraform基础设施。
:::
...
测试和使用MCP服务器最简单的方式是使用检查器或配置AI助手（如上所述）。

也可以通过`<your-server-name>-serve-stdio`目标直接使用[STDIO传输](https://modelcontextprotocol.io/docs/concepts/transports#standard-input%2Foutput-stdio)运行服务器。
...
如需使用[流式HTTP传输](https://modelcontextprotocol.io/docs/concepts/transports#streamable-http)本地运行MCP服务器，可使用`<your-server-name>-serve-http`目标。
...
- 使用`uv export`将Python依赖导出到`requirements.txt`
- 使用`uv pip install`为目标平台（`aarch64-manylinux2014`）安装依赖

同时添加了特定于MCP服务器的`docker`目标，该目标：

- 从`Dockerfile`构建docker镜像，根据[MCP协议规范](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html#mcp-protocol-contract)在8000端口运行MCP服务器

:::tip
docker镜像使用特定标签（如`my-scope-my-project-mcp-server:latest`）构建，该标签被CDK或Terraform基础设施引用，使得`Dockerfile`可与MCP服务器项目共存。
:::
