...
<FileTree>
  - your-project/
    - your_module/
      - mcp_server/ (如果指定了自定义名称则使用该名称)
        - \_\_init\_\_.py Python包初始化文件
        - server.py 主服务器定义文件，包含示例工具和资源
        - stdio.py STDIO传输入口点，适用于简单的本地MCP服务器
        - http.py Streamable HTTP传输入口点，适用于托管MCP服务器
        - Dockerfile MCP服务器托管入口点（当`computeType`设为`BedrockAgentCoreRuntime`时）
    - pyproject.toml 更新了MCP依赖项
    - project.json 更新了MCP服务器服务目标
</FileTree>

:::note
如果为`computeType`选择了`BedrockAgentCoreRuntime`，生成器还将创建CDK构造用于部署MCP服务器。
:::
...
测试和使用MCP服务器最简单的方式是通过AI助手配置（如上所述）。

您也可以直接使用`<your-server-name>-serve-stdio`目标运行[STDIO传输](https://modelcontextprotocol.io/docs/concepts/transports#standard-input%2Foutput-stdio)：
...
如需使用[Streamable HTTP传输](https://modelcontextprotocol.io/docs/concepts/transports#streamable-http)在本地运行MCP服务器，可使用`<your-server-name>-serve-http`目标：
...
:::tip
docker镜像使用特定标签（例如`my-scope-my-project-mcp-server:latest`）构建，CDK构造的`Dockerfile`继承该标签，允许将您的`Dockerfile`与MCP服务器项目共存。
:::
