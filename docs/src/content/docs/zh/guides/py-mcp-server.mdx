...
生成器配置了名为`<your-server-name>-inspect`的目标，该目标会在本地启动您的MCP服务器（通过`<your-server-name>-serve-local`目标，包括任何连接的依赖项，如本地数据库），并启动预配置的[MCP检查器](https://github.com/modelcontextprotocol/inspector)，通过可流式HTTP传输连接到服务器。
...
<CardGrid>
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/ts-agent-mcp`}
    source="strands"
  ...
  <ConnectionCard
    ...
    description="将 Python MCP 服务器连接到 DynamoDB 表"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/py-mcp-server-dynamodb`}
    source="mcp"
    sourceBadge="python"
    target="dynamodb"
    targetBadge="python"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'zh'}/guides/connection/agentcore-gateway-mcp`}
    source="agentcore"
    target="mcp"
  />
</CardGrid>
