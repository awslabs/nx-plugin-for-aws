...
您的项目还有一个 `assemble` 目标，它是 `build` 的仅产物版本：它生成项目对部署的贡献（其编译输出，以及它声明的任何捆绑包、容器镜像或生成的规范），而不运行代码检查、测试或类型检查门控。

<NxCommands commands={['assemble <project-name>']} />

<Link path="guides/typescript-infrastructure#deploying-to-aws">部署目标</Link>依赖于 `assemble` 而不是 `build`，因此部署只构建即将部署的内容。`build` 保持不变并仍然执行所有操作，因此它仍然是您运行以检查项目的内容——也是您应该在 CI 中运行的内容。

:::caution
因为 `assemble` 跳过了质量门控，部署不再会因代码检查错误、失败的单元测试或类型错误而失败。运行 `build`（或您的 CI 流水线）来检查您的代码。
:::
