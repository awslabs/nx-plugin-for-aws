...
当您将 Python 项目用作运行时代码（例如作为 AWS lambda 函数的处理程序）时，您需要创建源代码及其所有依赖项的捆绑包。<Link path="guides/python-lambda-function">`py#lambda-function`</Link>、<Link path="guides/fastapi">`py#api`</Link> 和 <Link path="guides/py-mcp-server">`py#mcp-server`</Link> 生成器会为您添加此功能。要手动添加，请向您的 `project.json` 文件添加如下目标，匹配这些生成器提供的形式：
...
`assemble` 用于在开发过程中更快地迭代。在 CI 管道中运行 `build` 以确保您的项目在生产部署之前通过质量关卡。
:::
...
`test`、`lint`、`format` 和 `typecheck` 目标读取 `default`，因此在您编辑测试时仍会重新运行。请注意，`typecheck` 也会对您的测试进行类型检查，因此测试中的类型错误仍会被报告。
