...
4. 如果连接使用 Cognito 或 IAM 认证的 API，需通过 <Link path="/guides/react-website-auth">`ts#website#auth` 生成器</Link> 添加 Cognito 认证
...
有关如何在后端定义订阅过程的信息，请参阅 <Link path="guides/trpc">tRPC API 生成器指南</Link>。
...
如果您的 tRPC API 使用 `Custom` 认证（Lambda 授权器），生成的客户端提供程序包含占位符 `headers`，您必须在其中添加授权器所需的授权标头。在生成的 `<ApiName>ClientProvider.tsx` 中查找 `// TODO: Add headers required by your custom authorizer` 注释，并将其替换为您的令牌或 API 密钥逻辑。
