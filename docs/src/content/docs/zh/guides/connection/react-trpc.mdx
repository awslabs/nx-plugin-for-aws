...
- **`ServerlessApiGatewayRestApi`** — HTTP 链接，通过服务器发送事件（Server-Sent Events）使用 `httpSubscriptionLink` 支持 tRPC 订阅。
- **`ServerlessApiGatewayHttpApi`** — 纯 HTTP 链接（不支持订阅）。
- **`BedrockAgentCoreRuntimeWebSocket`** — 针对 AgentCore 运行时 URL 的 WebSocket 链接，使用 SigV4 签名握手（IAM）或 base64url 编码的 JWT 子协议（Cognito）。运行时 ARN 从 `runtimeConfig.apis.<ApiName>` 读取，并动态转换为 `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` URL。
...
:::caution[订阅计算类型]
仅当 tRPC API 使用 `ServerlessApiGatewayRestApi`（REST API）作为计算类型时才支持订阅。API Gateway HTTP API 不支持响应流式传输。
:::
