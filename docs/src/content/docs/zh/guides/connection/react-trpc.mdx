...
生成器会检查后端的 `computeType` 并生成匹配的客户端：

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
一个 HTTP 链接，通过 Server-Sent Events 使用 `httpSubscriptionLink` 实现 tRPC 订阅。
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
一个普通的 HTTP 链接。不支持订阅，因为 API Gateway HTTP API 不支持响应流式传输。
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
一个针对 AgentCore 运行时 URL 的 WebSocket 链接，使用 SigV4 签名握手（IAM）或 base64url 编码的 JWT 子协议（Cognito）。运行时 ARN 从 `runtimeConfig.apis.<ApiName>` 读取，并动态转换为 `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` URL。
</TabItem>
</Tabs>
...
同时会安装以下依赖：
  ...
  - `event-source-polyfill`（如果使用 REST API 后端，用于订阅支持）
  - `react-oidc-context`（如果使用 Cognito 认证）

## 使用生成代码

### 使用 tRPC 选项代理钩子

生成器提供 `use<ApiName>` 钩子，返回 [tRPC 选项代理](https://trpc.io/docs/client/tanstack-react-query)，用于 TanStack Query 钩子（如 `useQuery` 和 `useMutation`）：
...
`use<ApiName>Client` 钩子提供对 [原生 tRPC 客户端](https://trpc.io/docs/client/vanilla) 的访问，适用于命令式 API 调用和订阅：
...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayHttpApi' }} description="HTTP API 后端不支持订阅">
:::caution[HTTP API 不支持订阅]
API Gateway HTTP API 不支持响应流式传输。如果需要订阅功能，请选择 `ServerlessApiGatewayRestApi` 或 `BedrockAgentCoreRuntimeWebSocket` 作为后端的 `computeType`。
:::
</OptionFilter>

<OptionFilter when={{ computeType: ['ServerlessApiGatewayRestApi', 'BedrockAgentCoreRuntimeWebSocket'] }} description="订阅如何到达客户端 — 传输方式因后端计算类型而异">
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
生成的客户端使用 `splitLink`，将订阅操作通过 `httpSubscriptionLink`（Server-Sent Events）路由，常规查询/变更通过 `httpLink` 路由。订阅开箱即用，无需额外配置。
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
生成的客户端使用针对 AgentCore 运行时 URL 的 `wsLink` — 所有 tRPC 操作（查询、变更和订阅）都通过单个 WebSocket 连接传输。握手会自动进行身份验证：`IAM` 认证使用 SigV4 签名，`Cognito` 认证则将 JWT 作为 base64url 编码的子协议携带。
</TabItem>
</Tabs>

有关如何在后端定义订阅过程的信息，请参阅 <Link path="guides/trpc">`ts#trpc-api` 生成器指南</Link>。
...
- [tRPC TanStack React Query 集成](https://trpc.io/docs/client/tanstack-react-query)
- [tRPC 原生客户端](https://trpc.io/docs/client/vanilla)
