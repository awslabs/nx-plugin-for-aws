...
:::caution[Cognito 代理需要 Cognito Gateway]
`auth: cognito` 代理通过转发 Gateway 在入站时验证的令牌来调用，因此它也需要使用 `auth: cognito` 生成的 Gateway——`auth: iam` Gateway 使用 SigV4 对其调用者进行身份验证，没有可转发的持有者令牌。生成器会拒绝该配对。Cognito Gateway 可以前置 `auth: iam` 代理，它使用自己的角色调用该代理。
:::
