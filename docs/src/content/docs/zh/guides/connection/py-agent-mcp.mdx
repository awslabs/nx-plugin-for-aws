...
1. 一个包含 <Link path="guides/py-agent">Agent</Link> 组件的 Python 项目
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_strands.py 封装传输的 Strands MCP 客户端
      - auth/ 框架无关的 SigV4 / 会话转发 `httpx.Auth`
    - app
      - \<mcp\_server\_name>\_client\_strands.py 每个 MCP 服务器的专用 Strands 客户端
