...
`connection` 生成器可以将您的 <Link path="guides/py-agent">Python Strands Agent</Link> 连接到 MCP 服务器(可以是 <Link path="guides/ts-mcp-server">TypeScript</Link> 或 <Link path="guides/py-mcp-server">Python</Link>)。
...
1. 一个包含 <Link path="guides/py-agent">Strands Agent</Link> 组件的 Python 项目
