...
`connection` 生成器可以将您的 <Link path="guides/py-agent">Python Agent</Link> 连接到 MCP 服务器（<Link path="guides/ts-mcp-server">TypeScript</Link> 或 <Link path="guides/py-mcp-server">Python</Link>）。
...
1. 一个包含 <Link path="guides/py-agent">Python Agent</Link> 组件（Strands 或 LangChain）的 Python 项目
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_\<framework>.py 为您的代理框架包装传输的 MCP 客户端
      - auth/ 框架无关的 SigV4 / 会话转发 `httpx.Auth`
    - app
      - \_\_init\_\_.py Python package initialization
      - \<mcp\_server\_name>\_client\_\<framework>.py 每个 MCP 服务器的连接客户端

</FileTree>

客户端后缀与您的代理框架匹配（`_strands` 或 `_langchain`）。
...
LangChain 客户端返回通过 [`langchain-mcp-adapters`](https://docs.langchain.com/oss/python/langchain/mcp) 加载的工具列表。每个工具在每次调用时都会打开一个新的 MCP 会话，因此这些工具在代理的生命周期内保持可用——不需要 `with` 块。
</TabItem>
</Tabs>

AgentCore 会话 ID 会通过 `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` 标头自动传播到 MCP 服务器，适用于两种框架，确保 [Bedrock AgentCore 可观测性](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html)的一致性。
...
`grantInvokeAccess` 会在 MCP 服务器的运行时 ARN 上连接 AgentCore 调用操作（`InvokeAgentRuntime`、`InvokeAgentRuntimeForUser` 和 `InvokeAgentRuntimeWithWebSocketStream`）。

MCP 服务器的 AgentCore 运行时 ARN 会由生成的 CDK 构造自动注册到 <Link path="guides/runtime-config">运行时配置</Link>的 `agentcore` 命名空间中，以便代理可以在运行时发现它。
</Fragment>
...
MCP 服务器的 AgentCore 运行时 ARN 会由生成的 Terraform 模块自动注册到 <Link path="guides/runtime-config">运行时配置</Link>的 `agentcore` 命名空间中，以便代理可以在运行时发现它。
</Fragment>
</Infrastructure>
