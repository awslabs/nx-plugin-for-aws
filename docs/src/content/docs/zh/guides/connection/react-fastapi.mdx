...
`connection` 生成器提供了一种快速将 React 网站与 FastAPI 后端集成的方式。它设置了所有必要的配置，以类型安全的方式连接到 FastAPI 后端，包括客户端和 [TanStack Query](https://tanstack.com/query/v5) 钩子生成、AWS IAM 和 Cognito 身份验证支持以及正确的错误处理。
...
:::tip[Git中的生成客户端]
默认情况下，生成的客户端会被版本控制系统忽略。如需提交，可从 React 应用的 `.gitignore` 文件中移除相关条目，但请注意构建项目时会覆盖对 `.gen.ts` 文件的手动修改。
:::
...
:::note[重新生成客户端代码]
修改 FastAPI 后，需重新构建项目以使更改反映到生成的客户端。例如：

<PackageManagerShortCommand commands={["build"]} />
:::

:::tip[自动重新生成]
若需同时开发 React 应用和 FastAPI，可使用 React 应用的 `dev` 目标（`serve-local` 的别名），它会在 API 变更时自动重新生成客户端，并热重载网站和本地 FastAPI 服务器：
...
:::warning[文件监听依赖]
`watch-generate:<ApiName>-client` 依赖 [`nx watch`](https://nx.dev/docs/guides/tasks--caching/workspace-watching) 命令，需要 [Nx Daemon](https://nx.dev/docs/concepts/nx-daemon) 运行。因此如果禁用了守护进程，修改 FastAPI 时客户端不会自动重新生成。
:::
...
<Drawer title="直接使用客户端处理流" trigger="点击查看直接使用普通客户端的示例。">
...
默认情况下，FastAPI 中使用 `PUT`、`POST`、`PATCH` 和 `DELETE` HTTP 方法的操作视为变更，其他视为查询。
