...
如果您已<Link path="zh/guides/fastapi#streaming">配置 FastAPI 以流式传输响应</Link>，您的 `useQuery` 钩子将在流的新块到达时自动更新其数据。
...
如果您已<Link path="zh/guides/fastapi#streaming">配置 FastAPI 以流式传输响应</Link>，生成的客户端将包括使用 `for await` 语法异步迭代流中块的类型安全方法。
