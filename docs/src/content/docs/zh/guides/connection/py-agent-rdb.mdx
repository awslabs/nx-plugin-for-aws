...
:::caution
当省略 `securityGroups` 时，`RuntimeNetworkConfiguration.usingVpc` 会在给定作用域上创建一个具有固定构造 ID（`SecurityGroup`）的安全组。如示例所示传递显式安全组可以避免在同一堆栈中定义多个运行时（例如一个 agent 和一个 MCP 服务器）时的构造 ID 冲突。
:::
