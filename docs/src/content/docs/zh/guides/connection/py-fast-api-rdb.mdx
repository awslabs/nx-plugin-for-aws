---
title: FastAPI 到关系数据库
description: 将 FastAPI 连接到 Python 关系数据库
when:
...
- packages/my_api
  - project.json 添加从 `dev` 到数据库的 `dev` 目标的依赖
...
此生成器配置了一个可注入的 [FastAPI Dependency](https://fastapi.tiangolo.com/tutorial/dependencies/)，您可以在路由处理程序中使用它：
...
`my_scope_my_db` 是您的数据库项目的模块名称——一个命名为 `<scope>_<name>` 的单一扁平模块，在项目内部显示为包目录。

`.scalars()` 将结果行解包为 `ExampleModel` 实例，FastAPI 可以对其进行序列化。
...
将 API Lambda 函数部署到**具有出口的私有子网**，而不是私有隔离子网。在运行时，`session_context()` 从 AWS AppConfig 检索数据库配置，这是一个需要出站互联网访问的公共 AWS 服务端点。
...
下面的示例引用了 <Link path="guides/py-rdb">Python 关系数据库</Link>部署指南中的 VPC 资源：`aws_subnet.database` 子网没有互联网路由，而 `aws_subnet.private` 子网具有 NAT 出口。
...
这将启动 FastAPI 和所有连接的数据库。`LOCAL_DEV=true` 环境变量会使数据库客户端连接到其本地 Docker 数据库而不是 Aurora。
