...
将 API Lambda 函数部署到**具有出口的私有子网**中，而不是私有隔离子网。`appconfig_application_id`/`appconfig_application_arn` 来自共享的 <Link path="guides/runtime-config">运行时配置</Link> AppConfig 应用程序，该应用程序在根模块中声明一次，而不是来自数据库模块——传递它们会在 Lambda 函数上设置 `RUNTIME_CONFIG_APP_ID` 并授予它们对应用程序的读取访问权限。
