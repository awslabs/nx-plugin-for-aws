...
该生成器会配置 agent，使其在部署时通过 IAM SigV4（通过 `httpx` 请求签名）向 Gateway 进行身份验证，并在本地运行时连接到 Gateway 项目启动的本地 gateway。
...
生成器会将共享的 core-gateway 模块发送到您的 `agent_connection` Python 项目中，以及一个针对每个 Gateway 的包装器，并修改您的 agent：
...
- 修改您的 agent 的 `agent.py`，导入 Gateway 客户端并在 `tools` 中注册其工具
...
`MyGatewayClientStrands.create()` 返回一个可作为上下文管理器的 `MCPClient`，其 `list_tools_sync()` 会生成通过 Gateway 可用的所有工具。
</TabItem>
...
在这两种情况下，客户端在每种模式下的行为相同：

- **部署模式**（未设置 `LOCAL_DEV`）：工具指向 Gateway 的 MCP 端点，使用 SigV4 签名。
- **本地模式**（`LOCAL_DEV=true`）：纯 HTTP 工具指向 Gateway 项目的 `dev` 目标启动的本地 gateway。
...
Gateway URL 会由生成的 CDK 构造自动注册到 <Link path="guides/runtime-config">运行时配置</Link> 的 `agentcore.gateways.<ClassName>` 命名空间中，因此 agent 可以在运行时发现它。
</Fragment>
...
Gateway URL 会由生成的 Terraform 模块自动注册到 <Link path="guides/runtime-config">运行时配置</Link> 的 `agentcore.gateways.<ClassName>` 命名空间中，因此 agent 可以在运行时发现它。
</Fragment>
</Infrastructure>
...
1. 启动已连接的 Gateway 的本地 gateway 以及所有附加的 MCP 服务器
2. 设置 `LOCAL_DEV=true`，使生成的客户端指向本地 gateway 而不是已部署的 Gateway
...
- **没有 Cedar 策略评估。** 无论策略如何，agent 都可以看到所有工具。使用 `serve` 目标针对已部署的 Gateway 测试策略。
- **保留工具名称前缀。** 每个本地 MCP 服务器的工具都以 `<target-name>___<tool-name>` 的形式公开，与已部署的 Gateway 发出的内容相匹配。这使得 agent 的系统提示和您引用的 Cedar 操作名称在本地和已部署运行中保持一致。
