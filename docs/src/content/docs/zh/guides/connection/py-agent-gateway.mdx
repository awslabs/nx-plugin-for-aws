...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py 用于已部署 Gateway 的 Strands MCP 客户端
      - auth/ 框架无关的 SigV4 / 会话转发 `httpx.Auth`
    - app/
      - \<gateway\_snake>\_client\_strands.py 针对每个 Gateway 的 Strands 客户端包装器
...
`MyGatewayClientStrands.create()` 返回一个可以作为上下文管理器的单一客户端，其 `list_tools_sync()` 会产生通过 Gateway 可用的每个工具：

- **部署模式**（未设置 `SERVE_LOCAL`）：指向 Gateway 的 MCP 端点的单个 `MCPClient`，使用 SigV4 签名。
- **本地模式**（`SERVE_LOCAL=true`）：指向 Gateway 项目的 `serve-local` 目标启动的本地网关的纯 HTTP `MCPClient`。
