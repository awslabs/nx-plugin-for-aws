...
生成器会更新 agent 的 `<agent-name>-dev` 目标（在 `project.json` 中），使其依赖于 DynamoDB 项目的 `dev` 目标，并将 DynamoDB 包添加为工作区依赖项。不会修改任何源文件。
...
`grantReadWriteData` 将 DynamoDB 和 KMS 权限授予 agent 的执行角色。
</Fragment>
