...
生成器**无法**自动将 MCP 服务器目标连接到您的基础设施中，因为它不知道哪个堆栈或模块实例化了 Gateway。您需要自己添加一个对 `gateway.addMcpServer(server)` 的调用。
...
启动本地 gateway 以及每个附加的 MCP 服务器在其分配的本地端口上。本地 gateway 公开一个单一的 MCP 端点，该端点聚合附加服务器的工具。通过 <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> 或 <Link path="guides/connection/py-agent-gateway">Python</Link> gateway-connection 生成器连接到 Gateway 的 Agent 在使用 `LOCAL_DEV=true` 运行时会指向它。

:::caution[本地保真度]
本地开发使用**本地替代 gateway** — 由 Gateway 项目启动的轻量级 MCP 聚合器，而不是 AgentCore Gateway 服务。因此，**Cedar 策略不会在本地评估**。每个 agent 都能看到每个附加 MCP 服务器上的每个工具。要执行 Cedar 策略，请改为运行 agent 的 `serve` 目标（请参阅 <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> / <Link path="guides/connection/py-agent-gateway">Python</Link> agent-connection 指南），以便本地运行的 agent 调用已部署的 Gateway。

工具名称在本地仍以 `<target-name>___<tool-name>` 为前缀，以匹配已部署 Gateway 的命名空间，因此 agent 的系统提示和您引用的 Cedar 操作名称在本地和已部署运行中保持一致。
:::
