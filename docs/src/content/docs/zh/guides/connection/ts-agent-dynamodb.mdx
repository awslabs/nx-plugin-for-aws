...
`connection` 生成器将 <Link path="guides/ts-agent">TypeScript Agent</Link> 连接到 <Link path="guides/ts-dynamodb">DynamoDB</Link> 项目，配置本地开发环境使两者自动一起启动。
...
确保 agent 设置了 `RUNTIME_CONFIG_APP_ID` 环境变量，以便 `resolveTableName()` 可以在运行时查找表名。
