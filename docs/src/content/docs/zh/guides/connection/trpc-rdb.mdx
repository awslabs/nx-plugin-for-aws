...
要允许您的 tRPC API 在运行时连接到数据库，API Lambda 函数必须部署到与数据库相同的 VPC 中，并被授予网络和 IAM 访问权限。

<Infrastructure>
<Fragment slot="cdk">

在您的应用程序堆栈中，将 API 部署到与数据库相同的 VPC 中，然后调用 `allowDefaultPortFrom` 和 `grantConnect` 以打开网络路径并向每个 Lambda 处理程序授予 IAM `rds-db:connect` 权限：
...
将数据库模块输出传递到您的 API 模块，以便它可以访问数据库并读取其运行时配置：
