...
当部署时（未设置 `SERVE_LOCAL`），客户端指向 Gateway 的 MCP 端点并使用 SigV4 进行身份验证。当 `SERVE_LOCAL=true` 时，它指向由 Gateway 项目的 `serve-local` 目标启动的本地 gateway，因此相同的 `agent.ts` 在两种模式下都能统一工作。
...
�的 Gateway 发出的名称相匹配。这使得 agent 的系统提示和您引用的 Cedar 操作名称在本地和已部署运行中保持一致。
