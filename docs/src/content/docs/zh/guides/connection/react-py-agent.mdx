...
Nx Plugin for AWS 提供了一个生成器，可以快速将您的 <Link path="guides/py-agent">Python Strands Agent</Link> 与 React 网站集成。它会设置所有必要的配置，通过类型安全的 OpenAPI 生成客户端连接到您的代理，包括 AWS IAM 和 Cognito 身份验证支持。
...
2. 一个使用 `protocol=HTTP` 的 Python Strands Agent（使用 <Link path="guides/py-agent">`py#agent` 生成器</Link>生成）
