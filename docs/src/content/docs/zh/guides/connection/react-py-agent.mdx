...
Nx Plugin for AWS 提供了一个生成器，可以快速将您的 <Link path="guides/py-agent">Python Agent</Link> 与 React 网站集成。它会设置所有必要的配置，通过类型安全的 OpenAPI 生成客户端连接到您的代理，包括 AWS IAM 和 Cognito 身份验证支持。

:::tip[AG-UI 代理]
如果您的 Python Agent 是使用 `protocol=AG-UI` 生成的，连接生成器将在 React 网站上配置 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 和 [`@ag-ui/client`](https://docs.ag-ui.com/) `HttpAgent`。有关详细信息，请参阅 <Link path="guides/connection/react-agui">React 到 AG-UI Agent</Link> 指南。
:::
...
系统将提示您选择 React 网站作为源项目，选择包含 Python Agent 的项目作为目标项目。如果您的目标项目包含多个组件（例如多个代理或其他组件类型），系统将提示您指定 `targetComponent` 以消除歧义。
...
在构建时，会内省 Python Agent 的 FastAPI 应用以生成 OpenAPI 规范。然后使用此规范生成带有 TanStack Query hooks 的类型安全 TypeScript 客户端，遵循与 <Link path="guides/connection/react-fastapi">React 到 FastAPI 连接</Link>相同的模式。
...
1. 运行 `nx dev <website>` 也会启动代理的本地 FastAPI 服务器
