...
保持 `threadId` 未设置是最简单的——CopilotKit 生成的 UUID 已经是 36 个字符。如果您显式传递一个，请确保它至少有 33 个字符，因为填充会将仅在尾随字符上不同的线程 ID 映射到同一个会话。

这两个值都来自浏览器，因此它们命名了一个对话，但没有说明它属于谁。要将每个用户限制为他们自己的对话，请参阅<Link path="guides/ts-agent">将会话限制为其所有者</Link>。
