...
<Snippet name="connection/ts-lambda-rdb-ssl-requirements" parentHeading="在不使用 RDS Proxy 连接时的 SSL 要求" />
...
这将同时启动 API 和本地数据库。`LOCAL_DEV=true` 环境变量会自动设置，因此 Prisma 客户端会连接到本地 Docker 数据库而不是 Aurora。
