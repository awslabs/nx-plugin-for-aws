...
`connection` 生成器可以将您的 <Link path="guides/py-agent">Python Strands Agent</Link> 连接到远程 [A2A](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) 代理 — 可以是 <Link path="guides/ts-agent">TypeScript</Link> 或 <Link path="guides/py-agent">Python</Link> — 这样您的代理就可以将任务委托给另一个代理作为工具。
...
1. 一个包含 <Link path="guides/py-agent">Strands Agent</Link> 组件的 Python 项目(任何协议)
2. 一个使用 `--protocol=A2A` 和 `--auth=IAM` 生成的 Strands Agent 组件项目(可以是 <Link path="guides/ts-agent">`ts#agent`</Link> 或 <Link path="guides/py-agent">`py#agent`</Link>)
