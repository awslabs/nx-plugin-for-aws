...
2. 正常运行的 Smithy TypeScript API 后端（使用 <Link path="/guides/ts-smithy-api">`ts#api` 生成器</Link>并指定 `--framework=smithy` 生成）
3. 如果连接的 API 使用 Cognito 或 IAM 身份验证，需通过 <Link path="/guides/react-website-auth">`ts#website#auth` 生成器</Link>添加 Cognito 身份验证
...
:::tip[自动重新生成]
如果同时开发 React 应用程序和 Smithy API，可使用 React 应用程序的 `dev` 目标（`serve-local` 的别名），该目标会在 API 变更时自动重新生成客户端，并热重载网站和本地 Smithy API 服务器：
...
如果 Smithy API 使用 `Custom` 身份验证（Lambda 授权器），需要编辑生成的客户端提供者以添加授权器所需的授权标头。在生成的 `<ApiName>Provider.tsx` 中查找 `fetch` 配置，并将令牌或 API 密钥添加到请求标头中。
