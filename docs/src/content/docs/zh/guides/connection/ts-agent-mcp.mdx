...
`connection` 生成器可以将您的 <Link path="guides/ts-agent">TypeScript Strands Agent</Link> 连接到 MCP 服务器（<Link path="guides/ts-mcp-server">TypeScript</Link> 或 <Link path="guides/py-mcp-server">Python</Link>）。
