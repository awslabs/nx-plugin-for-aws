...
1. 一个包含 <Link path="guides/ts-agent">Agent</Link> 组件的 TypeScript 项目
...
- packages/common/agent-connection
  - src
    - app
      - \<mcp-server-name>-client-strands.ts 连接的 MCP 服务器的高级 Strands 客户端
    - core
      - agentcore-endpoints.ts 框架无关的 ARN/URL 解析
      - agentcore-fetch.ts 框架无关的 SigV4 / JWT / 会话转发 fetch
      - agentcore-mcp-transport.ts 框架无关的 MCP 传输
      - agentcore-mcp-client-strands.ts 包装传输的 Strands MCP 客户端
    - index.ts 导出所有客户端
  - project.json
  - tsconfig.json
