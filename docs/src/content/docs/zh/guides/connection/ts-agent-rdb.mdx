...
`appconfig_application_id`/`appconfig_application_arn` 来自共享的 <Link path="guides/runtime-config">运行时配置</Link> AppConfig 应用程序，该应用程序在您的根模块中声明一次，而不是来自数据库模块。实例化时包含 `database` 命名空间，以便部署数据库模块的运行时配置条目：
