...
`connection` 生成器将 <Link path="guides/ts-agent">TypeScript Agent</Link> 连接到 <Link path="guides/ts-rdb">关系数据库</Link> 项目，使 Prisma 客户端在您的 agent 工厂中可用。
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="不使用 RDS Proxy 连接时的 SSL 要求" />
