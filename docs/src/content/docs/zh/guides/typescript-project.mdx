...
您的项目还有一个 `assemble` 目标，它是 `build` 的仅产物版本：它生成项目对部署的贡献（其编译输出，以及它声明的任何打包、容器镜像或生成的规范），而不运行代码检查、测试或类型检查。

<NxCommands commands={['assemble <project-name>']} />

<Link path="zh/guides/typescript-infrastructure#deploying-to-aws">部署目标</Link>依赖于 `assemble` 而不是 `build`，因此部署只构建即将部署的内容。`build` 保持不变，仍然执行所有操作，因此它仍然是您检查项目时运行的命令——也是您应该在 CI 中运行的命令。

:::caution
因为 `assemble` 跳过了质量检查，所以部署不再会因代码检查错误或失败的单元测试而失败。运行 `build`（或您的 CI 流水线）来检查您的代码。
:::
