...
当将TypeScript项目作为运行时代码（例如AWS Lambda函数处理程序）时，建议使用[Rolldown](https://rolldown.rs/)等工具进行打包，因其支持[tree-shaking](https://rolldown.rs/in-depth/why-bundlers)确保仅包含实际使用的依赖。
