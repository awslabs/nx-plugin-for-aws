...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    使用 [`--save-catalog`](https://pnpm.io/settings#catalogmode) 添加依赖项,以便 pnpm 在目录中记录版本并引用它:
    ...
    :::note
    在 `pnpm-workspace.yaml` 中设置 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) 会使其成为每个 `pnpm add` 的默认设置。它还会使 `pnpm add some-npm-package@<version>` 在目录固定不同版本时失败,这会阻止 `nx migrate` 升级 Nx——因此生成的工作区将其保持未设置状态。
    :::
  </TabItem>
  <TabItem label="yarn">
