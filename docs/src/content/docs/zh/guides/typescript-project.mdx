...
<FileTree>
  ...
  - vitest.config.mts 您的项目自己的 vitest.config.mts 扩展的共享 Vitest 配置（由第一个 TypeScript 项目创建）
  - package.json 添加了共享的构建和测试工具
  - pnpm-workspace.yaml 项目被添加到工作区，依赖项版本被添加到[目录](#catalogs)（您的包管理器的等效文件）
  - .gitignore 忽略构建和测试输出路径
...
`cwd` 是让 `rolldown` 找到其配置的关键：没有它，命令会从工作区根目录运行，而那里不存在 `rolldown.config.ts`。
