...
**构建和测试工具**（例如 `vitest`、`tsx`、`rolldown`、`@nx/*`、`typescript`、`@biomejs/biome`）在工作区中共享，因此它们位于**根目录** `package.json` 的 `devDependencies` 中，而不是在每个项目中。这些工具仅从配置文件、构建脚本和测试中引用，lint 规则会豁免这些文件。
...
:::tip[在 npm 上对齐版本]
npm 没有目录功能，因此版本直接在每个 `package.json` 中声明。要在 npm 工作区的项目之间保持一致，我们推荐使用 [syncpack](https://github.com/JamieMason/syncpack)。
:::
...
如果您要将 TypeScript 项目发布到 NPM，其 `package.json` 必须声明项目引用的依赖，以及入口点（`main`/`exports`）。由于在构建时您的项目会解析通过工作区根目录 `package.json` 安装的依赖，如果您的项目导入了未在其 `package.json` 中列出的包，Biome 的 `noUndeclaredDependencies` 规则会发出警告。
