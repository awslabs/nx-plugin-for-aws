...
禁用目录后，生成器会将依赖版本直接写入每个项目的 `package.json`，保持项目间版本一致是您的责任（例如使用 [syncpack](https://github.com/JamieMason/syncpack)）。在 pnpm 上启用目录时，工作区还会设置 [`catalogMode: prefer`](https://pnpm.io/settings#catalogmode)，以便 `pnpm add` 继续将新依赖添加到目录中。
