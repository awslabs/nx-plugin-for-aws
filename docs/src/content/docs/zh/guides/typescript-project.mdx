...
该文件需声明项目依赖。由于构建时项目会解析工作区根目录`package.json`中的依赖，如果项目导入了未在其`package.json`中列出的包，Biome的`noUndeclaredDependencies`规则会发出警告。
...
TypeScript项目使用[Biome](https://biomejs.dev/)进行代码检查和格式化。Biome在工作区根目录`biome.json`文件中配置——此处的修改将应用于工作区所有TypeScript项目以确保一致性。
...
大多数检查或格式化问题可自动修复。通过`--configuration=fix`参数运行即可自动修复。
