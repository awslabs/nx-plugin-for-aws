...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    工作区设置了 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)，因此 `pnpm add` 会在目录中记录版本并自动引用它——无需额外步骤。
  ...
  <TabItem label="yarn">
    yarn 的 `catalog:` 协议只能*引用*现有的目录条目——无法创建新条目——因此请先记录版本。
  ...
  <TabItem label="bun">
    bun 的 `catalog:` 协议只能*引用*现有的目录条目——无法创建新条目——因此请先记录版本。

    <Steps>
    1. 在根目录 `package.json` 的 `catalog` 下添加版本：
