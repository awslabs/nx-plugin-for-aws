...
这是因为尚未设置[项目引用](https://www.typescriptlang.org/docs/handbook/project-references.html)，并且导入的项目尚未在您项目的 `package.json` 中声明为工作区依赖。
...
:::tip[构建快捷方式]
您也可以直接构建项目——同步生成器会先运行，并提示您应用任何未完成的更改：
...
:::note
Biome 的 [`noUndeclaredDependencies`](https://biomejs.dev/linter/rules/no-undeclared-dependencies/) lint 规则强制项目的源代码只能导入在其自己的 `package.json` 中声明的包。配置文件、构建脚本和测试被豁免，因此共享工具可以保留在根目录。
:::

#### 目录

对于支持目录的包管理器（[pnpm](https://pnpm.io/catalogs)、[yarn](https://yarnpkg.com/features/catalogs) 和 [bun](https://bun.com/docs/install/catalogs)），生成器会在目录中记录每个版本，并使用 `catalog:` 协议引用它（例如 `"zod": "catalog:"`），因此无论哪个项目声明依赖，目录都是版本的唯一真实来源。这有助于您遵循[单一版本策略](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)，避免类型冲突和部署包中的多个包副本。

在 pnpm 上，工作区还设置了 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)，因此 `pnpm add` 会在目录中记录版本并自动引用它。

yarn 和 bun 没有等效功能，它们的 `catalog:` 协议只能*引用*现有的目录条目——无法创建新条目。要在 yarn 或 bun 上添加新依赖，首先在目录中记录其版本，然后从项目中引用它：

- **yarn** — 在 `.yarnrc.yml` 的 `catalog` 下添加版本，然后运行 `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — 在根目录 `package.json` 的 `catalog` 下添加版本，然后运行 `bun add some-npm-package@catalog: --cwd packages/my-library`

:::tip[在 npm 上对齐版本]
npm 没有目录功能，因此版本直接在每个 `package.json` 中声明。要在项目之间保持一致，我们推荐使用 [syncpack](https://github.com/JamieMason/syncpack)（`npx syncpack lint`）。
:::
