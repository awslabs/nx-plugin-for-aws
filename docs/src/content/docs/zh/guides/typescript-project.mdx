...
在 pnpm 上，工作区还设置了 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)，以便 `pnpm add` 自动在目录中记录新依赖。yarn 和 bun 没有等效设置，因此使用 `catalog:` 协议添加新依赖以将它们记录在目录中，例如 `yarn workspace @my-scope/my-library add some-npm-package@catalog:` 或 `bun add some-npm-package@catalog: --cwd packages/my-library`。
