...
该文件需声明项目引用的依赖。由于构建时项目会解析工作区根目录`package.json`中安装的依赖，Biome的`noUndeclaredDependencies`规则会在项目导入未在其`package.json`中列出的包时发出警告。
...
TypeScript项目使用[Biome](https://biomejs.dev/)进行代码检查和格式化。Biome在工作区根目录的`biome.json`文件中配置——对此文件的修改将应用于工作区中的所有TypeScript项目并确保一致性。
...
大多数代码检查或格式化问题可通过使用`--configuration=fix`参数运行来自动修复。
