...
`api-connection` 生成器提供了一种快速将 React 网站与 FastAPI 后端集成的方式。它设置了所有必要的配置，以类型安全的方式连接到 FastAPI 后端，包括客户端和 [TanStack Query](https://tanstack.com/query/v5) 钩子生成、AWS IAM 和 Cognito 身份验证支持以及正确的错误处理。
...
构建时，根据 FastAPI 的 OpenAPI 规范生成类型安全客户端。这将为 React 应用添加三个新文件：
...
:::tip
默认情况下，生成的客户端会被版本控制忽略。如需提交，可从 React 应用的 `.gitignore` 中移除相关条目，但请注意构建时 `.gen.ts` 文件的任何手动修改将被覆盖。
:::

## 使用生成代码

生成的类型安全客户端可用于从 React 应用调用 FastAPI。建议通过 TanStack Query 钩子使用客户端，但也可直接使用原生客户端。
...
:::tip
如需同时开发 React 应用和 FastAPI，可使用 [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) 在 API 变更时自动重新生成客户端：
...
使用 `queryOptions` 方法获取调用 API 所需的选项，配合 TanStack Query 的 `useQuery` 钩子：
...
对于接受 `cursor` 参数的端点，生成的钩子支持使用 TanStack Query 的 `useInfiniteQuery` 实现无限滚动或"加载更多"功能。
...
生成的钩子自动处理基于游标的分页。`nextCursor` 值从响应中提取并用于获取下一页。

:::tip
如果分页 API 的参数名不是 `cursor`，可通过 [OpenAPI 的 `x-cursor` 扩展自定义](#custom-pagination-cursor)。
:::
...
集成包含内置的错误处理，生成类型化的错误响应。`<operation-name>Error` 类型封装了 OpenAPI 规范中定义的可能错误响应。每个错误都有 `status` 和 `error` 属性，通过检查 `status` 可确定具体错误类型。
...
如果已为 FastAPI <Link path="guides/fastapi#streaming">配置流式响应</Link>，`useQuery` 钩子会在新数据块到达时自动更新。
...
可使用 `isLoading` 和 `fetchStatus` 属性确定流状态。流生命周期如下：
...
如果已为 FastAPI <Link path="guides/fastapi#streaming">配置流式响应</Link>，生成的客户端将包含使用 `for await` 语法异步迭代流的方法。
...
:::note
如果流式 API 接受 `cursor` 参数，使用 `useInfiniteQuery` 钩子时，每个页面将在流完成后加载。
:::
...
如需禁用操作的 `infiniteQueryOptions`，可将 `x-cursor` 设为 `False`：
...
通过定义自定义异常类、异常处理程序并为不同状态码指定响应模型，可自定义错误响应。生成的客户端将自动处理这些自定义错误类型。
...
生成的客户端自动处理这些自定义错误类型，允许类型检查和错误响应处理：
...
:::tip
在 FastAPI 中定义错误响应时，始终使用 `responses` 参数为每个状态码指定模型，确保生成的客户端具有正确的错误处理类型信息。
:::
...
集成提供端到端类型安全。IDE 将为所有 API 调用提供完整的自动补全和类型检查：
...
类型根据 FastAPI 的 OpenAPI 模式自动生成，确保 API 变更在构建后能反映到前端代码。