...
3. 正常运行的 tRPC API（使用 tRPC API 生成器生成）
4. 若连接使用 Cognito 或 IAM 认证的 API，需通过 <Link path="/guides/react-website-auth">`ts#react-website-auth` 生成器</Link> 添加 Cognito 认证
...
生成器将在 React 应用中创建以下结构：

<FileTree>

- src
  - components
    - \<ApiName>ClientProvider.tsx 设置 tRPC 客户端并绑定到后端模式。ApiName 将解析为 API 名称
...
类型会自动从后端路由器和模式定义中推断，确保 API 的任何变更都能立即反映在前端代码中，无需重新构建。
