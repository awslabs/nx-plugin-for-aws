...
Nx 的 AWS 插件提供了一个生成器，可快速将您的 <Link path="guides/trpc">tRPC API</Link> 与 React 网站集成。它会设置所有必要的配置以连接到 tRPC 后端，包括 AWS IAM 和 Cognito 身份验证支持以及适当的错误处理。该集成在您的前端和 tRPC 后端之间提供完整的端到端类型安全。

## 前提条件

在使用此生成器之前，请确保您的 React 应用具备：

1. 渲染应用的 `main.tsx` 文件
2. 自动注入 tRPC provider 的 `<App/>` JSX 元素
3. 使用 tRPC 后端生成器创建的可运行 tRPC 后端
4. 如果连接使用 Cognito 或 IAM 认证的 API，需通过 <Link path="/guides/cloudscape-website-auth">`ts#cloudscape-website-auth` 生成器</Link> 添加 Cognito 认证
...
生成器提供 `use<ApiName>` 钩子，让您可以访问类型安全的 tRPC 客户端：
...
需要注意的是，无限查询仅适用于输入属性包含 `cursor` 的 procedures。
...
类型会自动从后端路由器和 schema 定义中推断，确保 API 的任何变更都能立即反映到前端代码中，无需重新构建。

## 更多信息

更多信息请参考 [tRPC TanStack React Query 文档](https://trpc.io/docs/client/tanstack-react-query/usage)。

您也可以直接查阅 [TanStack Query 文档](https://tanstack.com/query/v5)。