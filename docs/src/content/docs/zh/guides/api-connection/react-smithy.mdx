...
`api-connection` 生成器提供了一种快速将 React 网站与 Smithy TypeScript API 后端集成的方式。它设置了所有必要的配置，以类型安全的方式连接到您的 Smithy API，包括客户端和 [TanStack Query](https://tanstack.com/query/v5) 钩子生成、AWS IAM 和 Cognito 身份验证支持以及正确的错误处理。
