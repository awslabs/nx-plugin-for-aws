...
:::caution[TypeScript 尚不支持 AG-UI]
[AG-UI 协议](https://docs.ag-ui.com/)尚不支持 TypeScript Strands Agents。如果您需要 AG-UI 支持,请使用 <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> 生成器并设置 `protocol=AG-UI`。有关 TypeScript AG-UI 支持状态,请参阅 [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347)。
:::
...
生成器配置了一个名为 `<your-agent-name>-invoke` 的目标,可从终端启动与您的代理的交互式聊天。默认情况下,它连接到本地服务器:

<NxCommands commands={['agent-invoke your-project']} />

要改为与已部署的代理聊天,请将 `RUNTIME_CONFIG_APP_ID` 设置为您工作区的 AppConfig 应用程序 ID — CLI 会解析运行时 ARN 并调用已部署的端点:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

对于 Cognito 认证的代理,使用 `--accessToken` 传递 JWT:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

CLI 脚本位于 `scripts/<agent>/cli.ts`,会自动为您的代理选择正确的协议(HTTP/A2A)。
