...
生成器提供了一个 `auth` 选项来配置您的 Strands Agent 的身份验证。在生成代理时,您可以在 `IAM`(默认)或 `Cognito` 身份验证之间进行选择。
...
您可以使用 `grantInvokeAccess` 方法授予在 Bedrock AgentCore Runtime 上调用代理的访问权限,例如:
...
当您选择 `Cognito` 身份验证时,会自动生成 `UserIdentity` 构造。
</Fragment>
<Fragment slot="terraform">
