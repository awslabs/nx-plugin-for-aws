...
生成器配置了一个 `<your-agent-name>-chat` Nx 目标,该目标依赖于 `<your-agent-name>-serve-local`。运行它会在本地启动代理,并让您进入由 [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) 驱动的交互式终端聊天界面:

<NxCommands commands={['run your-project:agent-chat']} />

对于 **A2A** 代理,该目标只是将 `agent-chat-cli` 作为二进制文件运行 — 不会向您的工作区生成任何代码。

对于 **HTTP**(基于 WebSocket 的 tRPC)代理,生成器会生成一个小型的 `scripts/<your-agent-name>/chat.ts` 文件,该文件将生成的 `<Agent>Client.local({ url })` 包装为 `ChatAdapter`,并将其传递给 `agent-chat-cli` 的 `chatLoop`。
