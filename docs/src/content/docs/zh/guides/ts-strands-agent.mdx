...
当 `protocol` 设置为 `A2A` 时,入口点使用 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) 而不是 tRPC:
...
当 `protocol=A2A` 时,生成的 `index.ts` 将 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) 挂载到 Express 应用上,因此生成的代理会公开 A2A 协议端点以及 `/ping` 健康检查。当部署到 AgentCore 时,入口点会从 AppConfig 解析运行时的公共 ARN,并在代理卡中进行公告。
...
Strands SDK 的 `A2AAgent` 包装了远程 A2A 端点,因此您可以像调用本地 `Agent` 一样调用它。`invoke` 解析为 `AgentResult`;对其调用 `.toString()` 会连接远程代理回复中的文本块。
...
#### 使用 IAM 身份验证在 AgentCore 上调用

对 AgentCore 上已部署的 A2A 代理的请求(包括代理卡获取)必须进行 SigV4 签名。向 `A2AAgent` 传递一个 `clientFactory`,该工厂对卡解析器和 JSON-RPC 传输都使用 SigV4 签名的 fetch:
...
:::note[IAM 权限]
调用者的 IAM 角色需要对目标运行时 ARN 拥有 `bedrock-agentcore:InvokeAgentRuntime` 和 `bedrock-agentcore:GetAgentCard` 两个权限。生成的构造的 `grantInvokeAccess` 辅助方法会授予这两个权限。
:::

:::note[额外依赖项]
上面的 SigV4 客户端示例需要 `@a2a-js/sdk`、`aws4fetch` 和 `@aws-sdk/credential-providers`。在运行示例之前将它们添加到您的 `package.json` 中。
:::

#### 将 A2A 代理用作工具

`A2AAgent` 实现了与本地 `Agent` 相同的 `InvokableAgent` 接口,因此您可以将其包装在 Strands `tool` 中并交给另一个代理。这就是生成的 HTTP/tRPC 代理模板如何委托给远程 A2A 代理:
...
在您的 CDK 堆栈中,授予宿主代理对远程代理的调用访问权限 — 这会连接 A2A 客户端需要的两个 IAM 操作:
