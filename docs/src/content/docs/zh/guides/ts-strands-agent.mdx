...
生成器还在 `scripts/<your-agent-name>/chat.ts` 下提供了一个最小的交互式聊天 CLI,以及相应的 `<your-agent-name>-chat` 目标。此目标依赖于 `<your-agent-name>-serve-local`,因此运行它会在本地启动代理,并让您进入与其交互的终端聊天界面:

<NxCommands commands={['run your-project:agent-chat']} />

该 CLI 使用适合协议的客户端(HTTP/tRPC 使用 `AgentClient.local`,A2A 使用 `@a2a-js/sdk`),并通过 [`@clack/prompts`](https://github.com/bombshell-dev/clack) 逐令牌流式传输代理响应。
