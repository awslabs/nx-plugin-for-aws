...
用户池在 Cognito [Plus 功能计划](https://docs.aws.amazon.com/cognito/latest/developerguide/feature-plans-features-plus.html)上创建，并将标准身份验证的[威胁防护](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-threat-protection.html)设置为**仅审计**强制执行模式。在仅审计模式下，Cognito 为每次登录分配风险级别并记录评估，而不会阻止用户。
