...
:::note[回调 URL]
生成的用户池客户端允许使用 `http://localhost:4200` 和 `http://localhost:4300` 作为 OAuth 回调/登出 URL（用于针对已部署池的本地开发），以及您的 CloudFront 分发 URL。请保持此允许列表最小化：对于加固的生产部署，一旦您不再针对已部署的用户池进行本地运行，请考虑删除 localhost URL（或为本地开发使用单独的客户端）。
:::
