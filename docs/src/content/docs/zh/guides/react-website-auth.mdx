...
用户池在 Cognito [Plus 功能计划](https://docs.aws.amazon.com/cognito/latest/developerguide/feature-plans-features-plus.html)上创建，并将标准身份验证的[威胁防护](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-threat-protection.html)设置为**仅审计**强制执行。在仅审计模式下，Cognito 为每次登录分配风险级别并记录评估，而不会阻止用户。

一旦您观察了用户的风险评估，您可以切换到全功能强制执行，以自动响应风险活动（例如要求 MFA 或阻止登录），然后通过[自适应身份验证](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-adaptive-authentication.html)按风险级别配置响应：
...
与下面的 `enableWaf` 和 MFA 选项不同，强制执行模式不会作为属性或变量公开，因此此编辑适用于工作区中的每个 `UserIdentity`。
