...
运行时配置是 Nx Plugin for AWS 用于在生成的项目和组件之间传递部署时值的机制,使它们能够相互连接。例如,当您生成一个 API 时,其 URL 会自动注册到运行时配置中,以便连接的网站可以发现它。
...
`connection` 命名空间还会作为 `runtime-config.json` 文件部署到您网站的 S3 存储桶,使客户端能够发现后端资源。

您可以根据需要定义任意数量的附加命名空间,为传递部署时值(如 DynamoDB 表名)到 Lambda 函数或其他计算资源提供了一种便捷的环境变量替代方案。
...
在合成/部署时,`RuntimeConfig` 会创建一个 **AWS AppConfig** 应用程序,其中包含:
...
然后您必须声明 `runtime-config/appconfig` 模块以从所有命名空间文件创建 AppConfig 资源:
...
服务器端使用者需要 **AppConfig Application ID** 和 IAM 权限才能在运行时读取配置。生成的构造会自动处理这一点。
...
:::note
生成的 API 和 agent 模块会自动处理这一点——每个都会创建自己的 `appconfig` 模块实例并配置环境变量和 IAM 权限。只有对于自定义 Lambda 函数或计算资源,才需要手动执行此操作。
:::
</Fragment>
</Infrastructure>

:::tip[本地开发]
如果您使用具有自定义命名空间(例如 `tables`、`features`)的运行时配置,您只需在本地定义一个 `RUNTIME_CONFIG_APP_ID` 环境变量来指向已部署的 AppConfig 应用程序。这允许您的本地 API 或 agent 检索与已部署环境相同的配置,而无需复制所有单独的环境变量。
:::

## 通过 AppConfig 进行服务器端访问

服务器端使用者(如 Lambda 函数和 agent)可以使用 [AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/) 从 **AWS AppConfig** 检索运行时配置。
