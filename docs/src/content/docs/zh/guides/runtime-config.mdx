...
1. **`core/runtime-config/appconfig`** — 声明一次,位于根模块顶部附近。创建 AppConfig 应用程序、环境、部署策略以及每个命名空间的一个配置文件。其 `application_id` 和 `application_arn` 输出会传递到每个在运行时读取运行时配置的模块(agent、MCP 服务器、Lambda 函数等)。
...
3. **`core/runtime-config/appconfig-deployment`** — 声明一次,位于根模块末尾,使用 `depends_on` 覆盖每个贡献条目的模块。将贡献的条目聚合为每个命名空间一个 JSON,并针对共享应用程序创建托管配置版本 + 部署。
...
:::note[自动注册]
生成的 agent 和 MCP 模块接受 `appconfig_application_id` + `appconfig_application_arn` 输入——传递共享的 `runtime_config_appconfig` 输出,它们会为您连接环境变量和 IAM 权限。仅当您需要为自定义 Lambda 函数或计算资源手动执行此操作时才需要。
:::
</Fragment>
</Infrastructure>
