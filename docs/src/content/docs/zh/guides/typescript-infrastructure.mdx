...
```ts title="src/main.ts (使用 stageConfig)"
import { resolveStage } from ':my-scope/common-infra-config';

// 在此项目（packages/infra）下查找阶段，回退到共享阶段。
// 当阶段不存在配置时返回 undefined。
...
:::note[阶段配置]
本节适用于使用 `stageConfig` 生成的情况。如果不使用该选项，生成器会生成一个更简单的设置，您需要自己管理 AWS 凭证（例如，在部署前导出 `AWS_PROFILE`）。
:::
...
:::note[CDK网站构造]
使用 CDK 时，网站构造可以在堆栈中的任意位置声明 — 运行时配置在合成时延迟解析。对于 Terraform，请确保网站模块对添加到 <Link path="guides/react-website#runtime-configuration">运行时配置</Link> 的模块具有适当的 `depends_on`。
:::
...
有关 CDK 的更多信息，请参考 [CDK 开发者指南](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html) 和 [API 参考](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)。ws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)。ws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)。