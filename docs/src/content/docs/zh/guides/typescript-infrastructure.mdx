...
:::tip[快速模式以加快部署速度]
`deploy` 目标默认使用 [CloudFormation 快速模式](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)（通过 CDK 的 `--express` 标志）来加快开发期间的部署速度。快速模式在确认资源配置已应用后即完成，而不是等待完全稳定，传播会在后台继续进行，从而显著加快迭代周期。

快速模式适用于开发和迭代。要在不使用快速模式的情况下部署并等待资源完全稳定，请从基础设施项目的 `project.json` 中的 `deploy` 目标命令中删除 `--express` 标志。

对于生产阶段的实际部署，请使用 `deploy-ci` 目标（参见[下文](#deploying-to-aws-in-a-cicd-pipeline)）作为 CI/CD 流水线的一部分。此目标不使用快速模式，因此始终等待完全稳定。
:::
