...
:::note
使用 CDK 时，网站构造可以在堆栈中的任何位置声明 — 运行时配置会在合成时延迟解析。对于 Terraform，请确保网站模块对添加到 <Link path="guides/react-website#runtime-configuration">运行时配置</Link> 的模块具有适当的 `depends_on`。
:::
