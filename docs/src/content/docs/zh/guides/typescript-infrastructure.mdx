...
`synth` 依赖于 `^assemble` 而不是 `^build`，因此合成会构建您的堆栈部署的工件 — Lambda 包、容器镜像和生成的规范 — 而不会同时运行每个上游项目的检查、测试和类型检查门控。有关 `assemble` 与 `build` 的关系，请参阅 <Link path="guides/typescript-project#assembling">组装</Link>。
...
:::caution
因为部署目标跳过了检查、测试和类型检查门控，所以部署不会因检查错误或失败的单元测试而失败。在部署任何您关心的内容之前运行 `build`，并在管道中对其进行门控：
