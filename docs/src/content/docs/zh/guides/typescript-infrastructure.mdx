...
如果您使用 `--stageConfig=true` 生成，生成器还会创建两个用于集中凭证管理的共享包（如果它们尚不存在）：
...
:::note[阶段配置]
本节适用于使用 `stageConfig` 生成的情况：

<NxCommands commands={['g @aws/nx-plugin:ts#infra --name=infra --stageConfig=true']} />

如果没有它，生成器会生成一个更简单的设置，您需要自己管理 AWS 凭证（例如，在部署前导出 `AWS_PROFILE`）。
:::
...
| `deploy`         | 通过命名您想要的阶段或堆栈来部署任何阶段。除非您使用 `stageConfig` 生成，否则使用快速模式。  |
...
:::tip[快速模式以实现更快的部署]
`deploy-sandbox` 目标默认使用 [CloudFormation 快速模式](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)（通过 CDK 的 `--express` 标志）。快速模式在确认应用资源配置后立即完成，而不是等待完全稳定，传播在后台继续进行，从而显著加快迭代周期。

由于它在资源完成稳定之前返回，快速模式适合您迭代的沙箱阶段，而不适合 CI/CD 或生产部署 — 对于这些情况，请使用 [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline)。要在这里也等待完全的资源稳定，请传递 `--no-express`：
...
此目标是否使用快速模式取决于您如何生成项目：

- **使用 `stageConfig`** 时，它会等待完全的资源稳定，因为该目标可以命名任何阶段，包括您的生产阶段。对于您正在迭代的阶段，传递 `--express` 以选择使用快速模式。
- **不使用 `stageConfig`** 时，它使用快速模式，就像 `deploy-sandbox` 一样。由于您命名的阶段可能是生产阶段，请传递 `--no-express` 以等待完全的资源稳定：

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  从管道中使用 [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) — 它从不使用快速模式。
...
此目标与 `deploy` 和 `deploy-sandbox` 目标在两个方面有所不同。首先，它部署预合成的云程序集，而不是即时合成，避免了包版本更改带来的潜在非确定性，并确保每个管道阶段使用相同的云程序集进行部署。其次，它从不使用快速模式，因此在报告成功之前始终等待每个资源完全稳定。
