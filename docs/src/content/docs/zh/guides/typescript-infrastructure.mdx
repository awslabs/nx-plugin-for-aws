...
| `deploy`         | 通过命名您想要的阶段或堆栈来部署任何阶段。等待完全稳定。               |
...
由于此目标可以命名任何阶段，包括您的生产阶段，它会等待完全的资源稳定。对于您正在迭代的阶段，传递 `--express` 以选择使用快速模式。
...
此目标部署预合成的云程序集，而不是即时合成，避免了包版本更改带来的潜在非确定性，并确保每个管道阶段使用相同的云程序集进行部署。与 `deploy` 一样，它在报告成功之前等待每个资源完全稳定，并且与 `deploy` 不同，它不接受 `--express` 选择退出。
