...
如果您使用过 <Link path="guides/react-website">CloudScape 网站</Link> 生成器，会注意到 `packages/common/constructs` 中已有现成的构造用于部署。例如：
...
:::warning
为确保网站 <Link path="guides/react-website#runtime-configuration">运行时配置</Link> 包含所有 API 配置，必须确保在声明任何 API 构造之后声明网站。
:::
