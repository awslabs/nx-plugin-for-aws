...
- 编译后的 JavaScript 文件
- TypeScript 声明文件
- Nx 从已发布插件中读取的 `generators.json`、`executors.json` 和 `migrations.json` 清单
- 文档和许可证文件
...
1. **更新包信息**：编辑您的插件的 [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json)，填写正确的名称、版本、描述和仓库信息。同时删除 `"private": true`。

2. **构建包**：
   <NxCommands commands={['package your-plugin']} />

   包会放在 `dist/<your-plugin's project root>/package` 中 — 对于使用 `--name=nx-plugin --directory=tools` 生成的插件，那就是 `dist/tools/nx-plugin/package`。
