...
Strands Agents 从共享的 AWS AppConfig 应用程序读取其运行时配置 — 详情请参阅 <Link path="/guides/terraform-project#shared-runtime-config-appconfig">Terraform 基础设施指南</Link>。每个部署实例化一次 `core/runtime-config/appconfig` 模块，并将其输出传递给每个代理 / MCP 模块：
...
在根模块的末尾声明一次 `module "runtime_config_appconfig_deployment"`（使用 `depends_on` 覆盖每个贡献运行时配置条目的模块）— 它聚合贡献的条目并创建 AppConfig 托管配置版本 + 部署。请参阅<Link path="/guides/terraform-project#shared-runtime-config-appconfig">完整模式</Link>。
</Fragment>
</Infrastructure>
