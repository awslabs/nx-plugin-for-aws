...
将 API Lambda 函数部署到**具有出口的私有子网**中,而不是私有隔离子网。`RUNTIME_CONFIG_APP_ID` 应指向共享的 <Link path="guides/runtime-config">运行时配置</Link> AppConfig 应用程序,而不是数据库模块的输出。在实例化时包含 `database` 命名空间,以便部署数据库模块的运行时配置条目:
