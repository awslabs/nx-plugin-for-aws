...
将 API Lambda 函数部署到**具有出口的私有子网**中,而不是私有隔离子网。`appconfig_application_id`/`appconfig_application_arn` 来自在根模块中声明一次的共享 <Link path="guides/runtime-config">运行时配置</Link> AppConfig 应用程序,而不是来自数据库模块——传递它们会在 Lambda 函数上设置 `RUNTIME_CONFIG_APP_ID` 并授予它们对应用程序的读取访问权限。在实例化时包含 `database` 命名空间,以便部署数据库模块的运行时配置条目:
