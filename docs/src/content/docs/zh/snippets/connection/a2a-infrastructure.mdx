...
远程代理的 AgentCore 运行时 ARN 会由生成的 CDK 构造自动注册到<Link path="guides/runtime-config">运行时配置</Link>的 `agentcore` 命名空间中,以便宿主代理可以在运行时发现它。
</Fragment>
...
远程代理的 AgentCore 运行时 ARN 会由生成的 Terraform 模块自动注册到<Link path="guides/runtime-config">运行时配置</Link>的 `agentcore` 命名空间中,以便宿主代理可以在运行时发现它。
</Fragment>
</Infrastructure>