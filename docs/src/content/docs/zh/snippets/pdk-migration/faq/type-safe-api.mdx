...
对于 TypeScript 客户端，可以使用 <Link path="/guides/react-website">`ts#website` 生成器</Link>和 <Link path="/guides/connection">`connection` 生成器</Link>，结合示例 `ts#api`（将 `framework` 设置为 `smithy`）来了解客户端的生成与网站集成方式。这会配置通过调用我们的 `open-api#ts-client` 或 `open-api#ts-hooks` 生成器来生成客户端的构建目标。您可以通过指向 OpenAPI 规范来自行使用这些生成器。
...
等效的方式是使用 <Link path="/guides/smithy-project">`smithy#project` 生成器</Link>并将 `type` 设置为 `shapes`：

<RunGenerator generator="smithy#project" requiredParameters={{ name: 'my-shapes', type: 'shapes' }} />

将 `SmithyShapeLibraryProject` 中的模型移动到生成项目的 `src` 文件夹中，然后参考 <Link path="/guides/smithy-project#depending-on-a-shape-library">Smithy 项目指南</Link>了解如何将该库连接为 API 模型的依赖项。
