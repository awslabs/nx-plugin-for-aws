...
此项目类型利用了 [`create-react-app`](https://github.com/facebook/create-react-app)，该工具现已弃用。在本指南中迁移网站时，我们将使用 <Link path="/zh/guides/react-website">`ts#website` 生成器</Link>，它使用更现代且受支持的技术，即 [Vite](https://vite.dev/)。

作为迁移的一部分，我们还将从 PDK 配置的 React Router 迁移到 [TanStack Router](https://tanstack.com/router)，它为网站路由增加了额外的类型安全性。

#### 生成 React 网站

运行 <Link path="/zh/guides/react-website">`ts#website` 生成器</Link>，将 `framework` 设置为 `react`，以在 `packages/website` 中设置您的网站项目。由于购物清单应用程序是使用 CloudScape 组件构建的，我们还将 `ux` 设置为 `cloudscape`（默认值为 `shadcn`）：

<RunGenerator generator="ts#website" noInteractive requiredParameters={{ name: 'website', framework: 'react', ux: 'cloudscape' }} />

#### 添加 Cognito 身份验证

上面的 React 网站生成器默认不像 `CloudscapeReactTsWebsiteProject` 那样捆绑 cognito 身份验证，而是通过 <Link path="/zh/guides/react-website-auth">`ts#website#auth` 生成器</Link>显式添加。
...
使用 Nx Plugin for AWS，API 集成通过 <Link path="/zh/guides/connection">`connection` 生成器</Link>支持。接下来，我们使用此生成器，以便我们的网站可以调用我们的 Smithy API：
