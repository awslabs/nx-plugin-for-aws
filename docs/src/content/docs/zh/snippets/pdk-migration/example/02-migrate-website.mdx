...
在PDK中，通过传递项目实例来触发集成代码生成。在Nx Plugin for AWS中，API集成通过<Link path="/guides/api-connection">`api-connection`生成器</Link>实现。运行以下命令使网站能够调用Smithy API：
...
:::tip
`serve-local`目标会同时启动通过`api-connection`连接的API本地服务，并在网站、模型或后端变更时热重载！这使得我们无需编写CDK代码即可本地测试API与网站。
:::
