...
运行<Link path="/guides/react-website">`ts#website`生成器</Link>并将`framework`设置为`react`，在`packages/website`目录下创建网站项目：
...
Smithy API的本地服务器使用[`tsx`](https://github.com/privatenumber/tsx)运行，因此在启动本地服务器之前，让我们先将其添加为开发依赖：
