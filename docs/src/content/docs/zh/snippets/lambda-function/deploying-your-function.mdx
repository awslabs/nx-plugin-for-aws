...
该生成器会根据您选择的 `iac` 创建 CDK 或 Terraform 基础设施即代码。您可以用它来部署函数。
...
:::note[事件源匹配]
请确保事件源与选定的 `event` 选项匹配，以保证事件在处理器函数中正确处理。
:::
...
该生成器会在 `common/terraform` 目录下创建用于部署函数的 Terraform 模块。每次部署实例化一次共享的 `core/asset-bucket` 模块，并将其 `bucket_name` 传递给 `asset_bucket_name` 输入：
