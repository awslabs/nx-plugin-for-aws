...
- 对于 REST API，添加一个 `ALLOWED_ORIGINS` 环境变量，设置为 `"https://<domain_name>"`。这会将 CloudFront 分发设置为 AWS Lambda 集成中唯一允许的 CORS 源（除了本地主机）。请注意，此限制不适用于预检 OPTIONS - 请为[此 GitHub issue](https://github.com/awslabs/nx-plugin-for-aws/issues/377) 点赞以帮助优先解决此问题。
...
```
`MyWebsite` 构造可以使用 <Link path="/zh/guides/react-website">`ts#website` 生成器</Link>生成。此示例假设网站的本地端口为 `4200`/`4300` - 请检查其 `project.json`（或 `vite.config.mts`）以获取实际分配给它的端口，因为以这种方式生成的每个网站都有自己的端口。
:::