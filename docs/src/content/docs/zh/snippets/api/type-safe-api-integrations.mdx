...
- `ts#trpc-api` 通过 `integrationPattern` 选项为 `ServerlessApiGatewayRestApi` 和 `ServerlessApiGatewayHttpApi` 同时支持 `isolated` 和 `shared`。
- `py#fast-api` 和 `ts#smithy-api` 目前生成 `isolated`。

您也可以通过编辑现有生成的 CDK API 构造的 `defaultIntegrations` 方法中的 `pattern` 字段在两种模式间切换：

```ts {3}
return IntegrationBuilder.rest({
  // 将此更改为 'isolated' 或 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Terraform 模块默认已使用路由模式，单个 Lambda 函数处理所有 API 操作。
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
如果生成器未直接暴露路由模式，或您想进一步自定义，可以随意编辑 API 的 `defaultIntegrations` 方法以按您的偏好配置集成：
...
您也可以用其他方式修改代码，例如您可能更喜欢将 `router` 函数定义为 `defaultIntegrations` 的参数，而不是在方法内构造它。
</Fragment>
<Fragment slot="terraform">
