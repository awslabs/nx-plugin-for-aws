...
:::tip
To run your MCP server (and any connected dependencies) without the inspector, use the project's `dev` target. If you have generated multiple components into the same project, run a single MCP server on its own with its `<your-server-name>-dev` target.
:::
