...
You can use [GritQL](https://docs.grit.io/) to declaratively search and transform source code in your generators. The Nx Plugin for AWS exposes two helpers:
...
- `matchesGritQL(tree, filePath, pattern)` — checks whether a GritQL pattern matches anywhere in a file and returns `Promise<boolean>`

```typescript
import { applyGritQL, matchesGritQL } from '@aws/nx-plugin/sdk/utils/ast';
...
if (!(await matchesGritQL(tree, filePath, '`import { Auth } from "./auth"`'))) {
  // Add the import
}
```

GritQL patterns use backtick-delimited code snippets with `$metavariables` as wildcards. Use `=>` for rewrites and `where` clauses for conditions. Patterns work across TypeScript, JavaScript, Python, HCL, and other languages.
