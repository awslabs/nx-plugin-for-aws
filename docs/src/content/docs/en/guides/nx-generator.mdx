...
Some options only apply for certain values of another. Use `x-when` to say which,
so the documentation site leaves the option out where it doesn't apply and states
the condition where it does:
...
Multiple keys are ANDed and a list of values is ORed within a key, matching
`<OptionFilter when={{…}}>`:
...
`x-when` documents the option; it does not enforce anything, so a generator still
validates or ignores an option that doesn't apply to the run it was given.
