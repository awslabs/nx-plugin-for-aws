...
:::caution[Pin `boto3` to the version above]
Other generators add `boto3` pinned to exactly this version. Since your workspace shares a single UV virtual environment and lockfile, adding it unpinned (`add boto3`) resolves the latest release and writes a `boto3>=<latest>` constraint, which `uv` cannot satisfy alongside the pin. All Python targets in the workspace then fail to resolve dependencies.
:::
