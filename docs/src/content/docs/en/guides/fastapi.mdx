...
When using `Custom` auth, the construct creates a Lambda Authorizer internally from the generated `authorizer.py` file, which denies all requests by default. Implement your custom authorization logic in that file.
:::
</OptionFilter>
</Fragment>
