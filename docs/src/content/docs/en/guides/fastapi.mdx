...
:::note[Custom Lambda Authorizer]
If you selected `Custom` authentication, you will need to supply the `authorizer` property to the API construct:
...
The `defaultAuthorizer` creates a Lambda function from the generated `authorizer.py` file, which denies all requests by default. Implement your custom authorization logic in that file.
:::
</OptionFilter>
</Fragment>
...
When using `Custom` auth, your API is protected by a Lambda Authorizer that denies all requests by default. Implement your authorization logic in the generated `authorizer.py` file within your project module. The authorizer receives the request and must return an IAM policy document allowing or denying access.
:::
</OptionFilter>
</Fragment>
</Infrastructure>
