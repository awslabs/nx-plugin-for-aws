...
If you want each user to reach only their own conversations, record who owns each session in your own application and check that ownership on the way in. A session ID is then something your application issues rather than something the caller chooses.

**1. Create sessions through your API**

Add a procedure that starts a conversation. It generates the session ID itself, resolves the caller from its own request context, and stores the pair — for example in a table created with the <Link path="guides/py-dynamodb">`py#dynamodb`</Link> generator:
...
The API can be in any language; the example above uses the <Link path="guides/trpc">`ts#trpc-api`</Link> generator.

Resolve the caller from the verified request context rather than from an input field, so it cannot be supplied by the browser. <Link path="guides/trpc">Implementing Custom Middleware</Link> shows how to do this for both IAM and Cognito authenticated APIs.

A second procedure querying that index lets the UI offer a user their own conversations without ever seeing anyone else's.

**2. Validate ownership in the agent**

Your agent already receives the session ID, and with `auth=cognito` it also receives the caller's JWT — the AgentCore Runtime authorizer validates it and the generated infrastructure allowlists the `Authorization` header through to your code. Look the session up, compare its owner against the caller, and refuse the request if they differ:
...
Decoding the claim without verifying its signature is sufficient here, since AgentCore Runtime validated the token during inbound authorization.

:::tip[Why validate in the agent as well as the API]
The API decides who owns a session; the agent enforces it. Checking in both places means a session ID that leaks — copied from a browser, or reused from an old conversation — is still refused on the next invocation, and any future client of the agent inherits the same rule.
:::

:::caution[The session also selects the microVM]
A session ID selects the [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) that serves the request, so its filesystem, `/tmp` and in-process caches are shared by every request resolving to that session. This is worth attention if your agent or its tools write to local disk: two callers who send the same session ID share those resources even when their stored conversations are separate.

Generating opaque session IDs in your API avoids this, because a caller cannot land on a session it was not given. Deriving one from user-supplied values — a conversation name, an email, a game name — is predictable and lets two callers collide.
:::
