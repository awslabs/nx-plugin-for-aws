...
For a more in-depth guide to writing Strands agents, refer to the [Strands documentation](https://strandsagents.com/docs/user-guide/quickstart/overview/). For LangChain agents, the <Link path="/guides/connection/py-agent-mcp">`connection` generator</Link> also wires MCP server and A2A agent tools into `create_agent`.
