...
<Snippet name="agent/dev-vs-serve" parentHeading="Local Development" />
...
:::caution[Use `-dev`, not `-serve`]
Use `<your-agent-name>-dev` for this, not `<your-agent-name>-serve`. `-serve` runs the deployed code path, so it needs `RUNTIME_CONFIG_APP_ID` set — see <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Without it, streaming has already begun by the time the agent tries to resolve its session storage, so the request returns an empty `200` with no error and the failure appears only in the server log.
:::
