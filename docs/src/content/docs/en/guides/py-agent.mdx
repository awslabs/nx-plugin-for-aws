...
The generated `main.py` passes `lambda _input_data: get_session_manager()`, which resolves the session ID from the header-derived context rather than from `thread_id`. The two stay aligned because the <Link path="/guides/connection/react-agui">`connection` generator</Link>'s React hook derives the session header from the thread ID. If you invoke the agent from your own client, keep that relationship: send the same value as both, and note that because the provider is only called once per thread, changing the session header on a later request for the same thread will not switch the stored conversation.
...
Both the session ID and `thread_id` are supplied by the caller, and the session ID is the key used to read and write stored conversation state. The generated code does not namespace sessions per user, so any caller able to invoke the agent can read another user's conversation by supplying their session ID.

AgentCore does not prevent this: invocations are authorized ["against the agent runtime resource ARN, not against individual sessions"](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-instances-security.html), and AgentCore "validates the format of that session ID but does not verify that it belongs to the calling identity". The same page advises treating the session ID as "a server-side value derived from the authenticated end user".

With `auth=cognito`, the generated infrastructure sets `requestHeaderConfiguration.allowlistedHeaders: ['Authorization']`, so the caller's JWT — already validated by the AgentCore authorizer — reaches your agent code as an ordinary request header. You can namespace the session on its `sub` claim:

```python
# The AgentCore authorizer has already validated this token, so decoding
# without signature verification is sufficient here.
claims = jwt.decode(token, options={"verify_signature": False})
return S3SessionManager(session_id=f"{claims['sub']}/{session_id}", bucket=bucket_name)
```

Skipping signature verification is [the approach AWS documents](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-oauth.html) for this case, since the runtime validated the token during inbound authorization. Fail closed if the claim is missing, and bind the token into the request context (as the session ID already is) so it is reachable from `get_session_manager()`.

With `auth=iam`, the calling principal is **not** available to your agent code — AgentCore does not forward the `x-amz-*` signing headers and they cannot be allowlisted, and the instance metadata service exposes only the agent's own execution role. Treat the `bedrock-agentcore:InvokeAgentRuntime` permission as the trust boundary and grant it only to trusted backends, or use per-user IAM principals so IAM enforces the scoping.
:::
...
:::caution[Session isolation]
The identifier that selects the stored conversation is supplied by the caller — the AgentCore session ID for HTTP, or the AG-UI `thread_id` for AG-UI. The generated code does not namespace conversations per user, so any caller able to invoke the agent can read another user's conversation by supplying their identifier.

AgentCore does not prevent this: invocations are authorized ["against the agent runtime resource ARN, not against individual sessions"](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-instances-security.html), and AgentCore "validates the format of that session ID but does not verify that it belongs to the calling identity".

To scope conversations to a user, build the config's `thread_id` from a verified identity rather than passing the caller's value through:

```python
# The AgentCore authorizer has already validated this token, so decoding
# without signature verification is sufficient here.
claims = jwt.decode(token, options={"verify_signature": False})
config = {"configurable": {"thread_id": f"{claims['sub']}/{thread_id}"}}
```

With `auth=cognito`, the generated infrastructure allowlists the `Authorization` header, so the validated JWT reaches your agent code and [`sub` is the claim AWS documents](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory-gateway-fgac.html) for per-user isolation. With `auth=iam`, the calling principal is not available to your agent code (AgentCore does not forward the `x-amz-*` signing headers and they cannot be allowlisted), so treat the `bedrock-agentcore:InvokeAgentRuntime` permission as the trust boundary and grant it only to trusted backends.
:::
