...
For a more in-depth guide to writing Strands agents, refer to the [Strands documentation](https://strandsagents.com/docs/user-guide/quickstart/overview/). For LangChain agents, the [`connection` generator](/guides/connection/py-agent-mcp) also wires MCP server and A2A agent tools into `create_agent`.
