...
You selected `Any` for your `eventSource`, so the `@event_parser` decorator is not applied and the handler receives an untyped event dict. Regenerate with a specific `eventSource` if you want Pydantic validation and typed field access.
</OptionFilter>
