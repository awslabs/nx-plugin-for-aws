...
Node's own `fs.cpSync` copies files and directories on every platform, avoiding `cp` (unavailable on Windows) and needing nothing installed. `verbatimSymlinks` keeps relative symlink targets relative, which a bundled `node_modules` tree depends on.
:::
