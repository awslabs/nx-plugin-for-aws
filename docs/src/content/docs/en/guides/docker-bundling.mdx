...
The `shx` package provides cross-platform `cp`, `rm` and `mkdir` commands, avoiding the shell builtins (unavailable on Windows). Install it at the root of your workspace with `pnpm add -D -w shx`.

Copying a directory needs `-R` and a trailing `/.` on the source — `shx cp -R src/. dst` — which merges the contents into `dst`. Without the `/.` the source is nested inside `dst` instead, and unlike a `src/*` glob it still includes dotfiles.
:::
