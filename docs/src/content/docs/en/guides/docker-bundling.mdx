...
The scan is intentionally **not** wired into `build`. Trivy's result depends on its vulnerability database, which updates constantly: a scan that passes today can fail tomorrow when a new CVE is published against a base image or dependency you haven't changed. Coupling it to `build` would make local and CI builds non-deterministic and block unrelated work.

Instead, run it as a dedicated step in your CI pipeline (and locally before a release):

<PackageManagerShortCommand commands={['trivy']} />

When a scan starts failing on a newly-published CVE, upgrade the affected package or base image in your `Dockerfile`, or suppress it in `.trivyignore` if there is no fix yet (see below).
:::
