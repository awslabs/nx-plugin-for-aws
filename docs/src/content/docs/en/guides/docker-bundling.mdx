...
:::caution[Do not let Nx cache an image build]
A built image lives in the container engine's store, not under any `outputs` path, so Nx has nothing to restore on a cache hit — it would report the build complete while the image is missing (say after a `docker rmi` or a `docker system prune`), and a `trivy` target depending on it would report a cached pass against an image that isn't there.

Setting `"cache": false` keeps this sound at almost no cost: the container engine's own layer cache is keyed on the build context and lives alongside the image, so an unchanged context rebuilds in well under a second and always leaves the image present.
:::
...
:::tip[Why the scan is deliberately not cached]
The image the scan reads is only reachable through the container engine, so a restored cache entry would report a pass for an image that may no longer be there. With `"cache": false` the scan always runs against the real image, and fails loudly if it is missing. Pin the Trivy image version (e.g. <code>trivy:<TrivyVersion /></code>) so scans are reproducible.

This means every `trivy` run does a full `docker save` plus scan per image — expect tens of seconds per image rather than a free cache hit, which is why the scan belongs in its own CI job rather than in `build`. It also refreshes Trivy's vulnerability database on each run, so it needs network access; that refresh is the point, since a scan is only as good as the CVE data behind it. On a network-isolated runner, mount a persistent cache directory at `/root/.cache/trivy` and add `--skip-db-update`, having seeded the database in an earlier step.
:::
