...
:::tip[Declare `inputs` on a cacheable target]
A cacheable target that declares no `inputs` falls back to Nx's implicit `["default", "^default"]`. That `^default` reads each dependency's **whole project directory** — its README, its dev scripts, everything — so editing a dependency's README re-runs the bundle even though its output cannot change.

`["default"]` is enough **here** because this target declares `dependsOn: ["compile"]`. `default` carries a transitive `dependentTasksOutputFiles` entry, which Nx resolves against the task's upstream tasks — so with `compile` upstream, the dependency's build output is still hashed.

A cacheable target with **no** `dependsOn` has no upstream task for that entry to resolve against, so `default` collapses to the project's own files. There, dropping `^default` removes the last edge to the dependency and the target will serve stale output — use `["default", "^production"]` instead.
:::
