...
Unlike the image build above — where the engine's layer cache absorbs the cost — this one is not free: every `trivy` run does a full `docker save` plus scan per image, so expect tens of seconds per image rather than a cache hit. That is why the scan belongs in its own CI job rather than in `build`. It also refreshes Trivy's vulnerability database on each run, so it needs network access; that refresh is the point, since a scan is only as good as the CVE data behind it. On a network-isolated runner, mount a persistent cache directory at `/root/.cache/trivy` and add `--skip-db-update`, having seeded the database in an earlier step.
:::
