...
2. The shared `core/asset-ecr` repository holds the image. Every container in the workspace is published here, so nothing provisions a repository of its own — instantiate the module once per deployment and thread its `repository_url` into each module that publishes an image.
