...
:::tip[Always declare `inputs` on a cacheable target]
A cacheable target that declares no `inputs` falls back to Nx's implicit `["default", "^default"]`. That `^default` reads each dependency's **whole project directory** — its README, its dev scripts, everything — so editing a dependency's README re-runs the bundle even though its output cannot change. Rolldown resolves dependencies through the build artifacts `default` already tracks, so declaring `["default"]` keeps the bundle correct while dropping those false invalidations.
:::
