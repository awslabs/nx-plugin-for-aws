...
Pin every dependency installed inside the `Dockerfile` to an exact version (as above) — otherwise `npm install` will silently pick up a newer release the next time you build, producing images that behave differently depending on when they were built. The same advice applies to the base image: prefer a specific digest or tag (for example `node:22.14.0`) over a floating tag like `lts`.
:::
