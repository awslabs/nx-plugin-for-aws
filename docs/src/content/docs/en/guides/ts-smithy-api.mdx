...
On macOS and Linux the CLI is resolved by [mise](https://mise.jdx.dev/), which the build fetches on demand, so there is nothing to install — it downloads and caches the pinned version the first time you build. On Windows the CLI is a prerequisite you install yourself, and upgrading the workspace does not upgrade it — see <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> and <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> in the Smithy project guide.
