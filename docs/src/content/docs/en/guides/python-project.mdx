...
Your project also has a `package` target, which is the artifact-only sibling of `build`: it produces whatever your project contributes to a deployment (its compiled output, and any bundle, container image or generated specification it declares) without running the lint, test or type-check gates.

<NxCommands commands={['package <project-name>']} />

The <Link path="guides/typescript-infrastructure#deploying-to-aws">deploy targets</Link> depend on `package` rather than `build`, so deploying builds only what it is about to deploy. `build` is unchanged and still does everything, so it remains what you run to check a project — and what you should run in CI.

:::caution
Because `package` skips the quality gates, a deploy no longer fails on a lint error, a failing unit test or a type error. Run `build` (or your CI pipeline) to check your code.
:::
