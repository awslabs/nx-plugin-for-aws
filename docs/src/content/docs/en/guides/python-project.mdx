...
Test files are excluded from the `production` named input in `nx.json`, which covers the `tests` directory as well as `test_*.py`, `*_test.py` and `conftest.py` files anywhere in your project.
