...
`bundle-x86` does the work and `bundle` aggregates it, so a project bundling for more than one architecture (`bundle-arm` for `aarch64-manylinux_2_28`) builds every bundle from the one target. `--python-version` pins wheel resolution to the Lambda Python runtime, so the bundle cannot be built against whichever interpreter happens to be on the build machine.
