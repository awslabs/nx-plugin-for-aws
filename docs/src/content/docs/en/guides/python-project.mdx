...
The <Link path="guides/typescript-infrastructure#deploying-to-aws">deploy targets</Link> depend on `assemble` rather than `build`, so deploying builds only what it is about to deploy. `build` is unchanged and still does everything, so it remains what you run to check a project — and what you should run in CI.
