...
The `type` option selects whether you get an application (a deployable project, for example a lambda function or container image) or a library (reusable code, imported by other projects). It defaults to `application`, so pass `--type=library` for a library:
