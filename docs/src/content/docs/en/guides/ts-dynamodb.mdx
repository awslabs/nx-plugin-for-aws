...
### Keeping Local GSIs in Sync

When you add or remove a Global Secondary Index in your deployment construct, update `GLOBAL_SECONDARY_INDEXES` in `scripts/create-local-table.ts` to match. On the next `serve-local` run, the script will automatically add or remove indexes on the local table to reflect the new list.
