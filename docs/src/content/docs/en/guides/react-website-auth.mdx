...
Unlike the `enableWaf` and MFA options below, the enforcement mode is not exposed as a prop or variable, so this edit applies to every `UserIdentity` in your workspace.
