...
`mfa` accepts `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` in CDK, and `"OFF"` / `"OPTIONAL"` / `"ON"` in Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` and the `otp` equivalent enable or disable each second-factor method independently; they have no effect when `mfa` is off.

:::caution[Terraform ties SMS MFA to phone number verification]
Unlike the CDK construct, the Terraform AWS provider has no way to offer SMS as an MFA method independently of using it for phone number verification. Setting `mfa_second_factor_sms = false` therefore also disables SMS-based phone number verification (`auto_verified_attributes` drops `phone_number`).
:::
