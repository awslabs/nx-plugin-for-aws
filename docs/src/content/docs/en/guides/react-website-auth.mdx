...
:::note[Callback URLs]
The generated User Pool client allows `http://localhost:4200` and `http://localhost:4300` as OAuth callback/logout URLs (for local development against the deployed pool), plus your CloudFront distribution URL. Keep this allowlist minimal: for hardened production deployments, consider removing the localhost URLs (or using a separate client for local development) once you no longer run locally against the deployed User Pool.
:::
