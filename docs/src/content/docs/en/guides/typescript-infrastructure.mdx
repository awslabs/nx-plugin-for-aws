...
`synth` depends on `^assemble` rather than `^build`, so synthesizing builds the artifacts your stacks deploy — the Lambda bundles, container images and generated specifications — without also running every upstream project's lint, test and type-check gates. See <Link path="guides/typescript-project#assembling">Assembling</Link> for how `assemble` relates to `build`.
