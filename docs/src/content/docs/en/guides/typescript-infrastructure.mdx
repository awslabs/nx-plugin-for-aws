...
:::tip
Your infrastructure is a TypeScript project, so you can refer to the [TypeScript project documentation](/nx-plugin-for-aws/guides/typescript-project) for more details about their general usage.
:::
...
If you have used the [tRPC API](/nx-plugin-for-aws/guides/trpc) or [FastAPI](/nx-plugin-for-aws/guides/fastapi) generators to create APIs, you will notice you already have some constructs available in `packages/common/constructs` to deploy them.
...
If you have used the [CloudScape website](/nx-plugin-for-aws/guides/cloudscape-website) generator, you will notice you already have a construct in `packages/common/constructs` to deploy it. For example:
...
:::warning
It is important to ensure that the website is declared _after_ any API constructs in order for the website [Runtime Config](/nx-plugin-for-aws/guides/cloudscape-website#runtime-configuration) to include all API config.
:::

## Synthesizing your Infrastructure

As part of your `build` target, as well as running the [default compile, lint and test targets](/nx-plugin-for-aws/guides/typescript-project#building), your infrastructure project is _synthesized_ to CloudFormation. This can also be executed in a standalone fashion, by running the `synth` target:
