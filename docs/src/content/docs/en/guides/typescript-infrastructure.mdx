...
If you need a standard deployment that waits for full resource stabilization, opt out with `--no-express`:

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

The `deploy-ci` target does not use express mode, so CI/CD pipeline deployments always wait for full stabilization.
:::
