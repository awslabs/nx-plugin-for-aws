...
| `deploy`         | Deploying any stage, by naming the stage or stacks you want. Uses express mode unless you generate with `stageConfig`.  |
...
Whether this target uses express mode depends on how you generated the project:

- **With `stageConfig`** it waits for full resource stabilization, since the target can name any stage including your production one. Pass `--express` to opt into express mode for a stage you are iterating on.
- **Without `stageConfig`** it uses express mode, as `deploy-sandbox` does. Since the stage you name may be a production one, pass `--no-express` to wait for full resource stabilization:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Use [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) from a pipeline instead — it never uses express mode.
