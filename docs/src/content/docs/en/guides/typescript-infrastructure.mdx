...
`synth` depends on `^package` rather than `^build`, so synthesizing builds the artifacts your stacks deploy — the Lambda bundles, container images and generated specifications — without also running every upstream project's lint, test and type-check gates. See <Link path="guides/typescript-project#packaging">Packaging</Link> for how `package` relates to `build`.
...
`deploy-sandbox` and `deploy` depend on `^package`, so they build the artifacts they are about to deploy and nothing else. `deploy-ci` deploys an assembly your pipeline already built, so it has no build dependencies at all.

:::caution
Because the deploy targets skip the lint, test and type-check gates, deploying will not fail on a lint error or a failing unit test. Run `build` before you deploy anything you care about, and gate your pipeline on it:
