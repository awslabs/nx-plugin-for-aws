...
| `deploy`         | Deploying any stage during development, by naming the stage or stacks you want. Uses express mode. |
...
The `deploy-sandbox` and `deploy` targets use [CloudFormation express mode](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) by default (via CDK's `--express` flag). Express mode finishes once resource configuration is confirmed applied rather than waiting for full stabilization, with propagation continuing in the background, for significantly faster iteration cycles.

To wait for full resource stabilization instead, pass `--no-express`:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox` and `deploy` are for development only]
Because express mode returns before resources have finished stabilizing, `deploy-sandbox` and `deploy` are intended for development and iteration, not for CI/CD or production deployments. Use [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline) in your pipeline — it never uses express mode, so it always waits for full stabilization.
:::
...
This target differs from the `deploy` and `deploy-sandbox` targets in two ways. First, it deploys a pre-synthesized cloud assembly rather than synthesizing on the fly, avoiding potential non-determinism from package version changes and ensuring that every pipeline stage deploys using the same cloud assembly. Second, it does not use express mode, so it waits for every resource to fully stabilize before reporting success.

:::caution[Always use `deploy-ci` for production]
`deploy` and `deploy-sandbox` use express mode and return before resources have finished stabilizing, which makes them unsuitable for CI/CD and production deployments. Use `deploy-ci` for those.
:::
