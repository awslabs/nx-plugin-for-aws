...
The CLI uses the protocol-appropriate client (`AgentClient.local` for HTTP/tRPC or `@a2a-js/sdk` for A2A) and streams agent responses token-by-token via [`@clack/prompts`](https://github.com/bombshell-dev/clack).
