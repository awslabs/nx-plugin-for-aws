...
When `protocol` is set to `A2A`, the entry point uses the [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) instead of tRPC:
...
The Strands SDK's `A2AAgent` wraps a remote A2A endpoint so you can call it like a local `Agent`. `invoke` resolves to an `AgentResult`; calling `.toString()` on it joins the text blocks from the remote agent's reply.
...
#### Invoking on AgentCore with IAM Authentication

Requests to a deployed A2A agent on AgentCore (including the agent card fetch) must be SigV4-signed. Pass a `clientFactory` to `A2AAgent` that uses a SigV4-signing fetch for both the card resolver and the JSON-RPC transport:
...
The caller's IAM role needs both `bedrock-agentcore:InvokeAgentRuntime` and `bedrock-agentcore:GetAgentCard` on the target runtime ARN. The generated construct's `grantInvokeAccess` helper grants both.
:::

:::note[Extra dependencies]
The SigV4 client example above requires `@a2a-js/sdk`, `aws4fetch`, and `@aws-sdk/credential-providers`. Add them to your `package.json` before running the example.
:::

#### Using an A2A Agent as a Tool

`A2AAgent` implements the same `InvokableAgent` interface as a local `Agent`, so you can wrap it in a Strands `tool` and hand it to another agent. This is how the generated HTTP/tRPC agent template delegates to a remote A2A agent:
...
In your CDK stack, grant the host agent invoke access to the remote agent — this wires up both IAM actions the A2A client needs:
