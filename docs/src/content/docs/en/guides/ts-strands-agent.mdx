...
The generator configures a target named `<your-agent-name>-invoke` that starts an interactive chat with your agent from the terminal. By default it connects to the local server:

<NxCommands commands={['agent-invoke your-project']} />

To chat with a deployed agent instead, set `RUNTIME_CONFIG_APP_ID` to your workspace's AppConfig application ID — the CLI resolves the runtime ARN and invokes the deployed endpoint:
...
The CLI script lives at `scripts/<agent>/cli.ts` and selects the right protocol (HTTP/A2A) for your agent automatically.
