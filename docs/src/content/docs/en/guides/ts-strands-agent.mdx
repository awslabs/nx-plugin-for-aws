...
For **A2A** agents, the target simply runs `agent-chat-cli` as a binary — no code is vended into your workspace.

For **HTTP** (tRPC over WebSocket) agents, the generator emits a tiny `scripts/<your-agent-name>/chat.ts` that wraps the generated `<Agent>Client.local({ url })` as a `ChatAdapter` and passes it to `agent-chat-cli`'s `chatLoop`.
