...
On pnpm, the workspace also sets [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) so `pnpm add` records new dependencies in the catalog automatically. yarn and bun have no equivalent setting, so add new dependencies with the `catalog:` protocol to record them in the catalog, for example `yarn workspace @my-scope/my-library add some-npm-package@catalog:` or `bun add some-npm-package@catalog: --cwd packages/my-library`.
