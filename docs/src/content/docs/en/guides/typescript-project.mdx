...
On pnpm, the workspace also sets [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), so `pnpm add` records the version in the catalog and references it automatically.

yarn and bun have no equivalent, and their `catalog:` protocol only *references* an existing catalog entry — it cannot create one. To add a new dependency on yarn or bun, first record its version in the catalog, then reference it from the project:

- **yarn** — add the version under `catalog` in `.yarnrc.yml`, then `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — add the version under `catalog` in the root `package.json`, then `bun add some-npm-package@catalog: --cwd packages/my-library`
