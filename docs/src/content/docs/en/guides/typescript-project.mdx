...
**Build and test tooling** (for example `vitest`, `tsx`, `rolldown`, `@nx/*`, `typescript`, `@biomejs/biome`) is shared across the workspace, so it lives in the **root** `package.json` `devDependencies` rather than in each project. These tools are only referenced from config files, build scripts and tests, which the lint rule exempts.
...
npm has no catalog feature, so versions are declared directly in each `package.json`. To keep them consistent across projects in an npm workspace, we recommend [syncpack](https://github.com/JamieMason/syncpack).
:::
...
If you are publishing your TypeScript project to NPM, its `package.json` must declare the dependencies your project references, along with entry points (`main`/`exports`). Since at build time your project resolves dependencies installed via the workspace root `package.json`, Biome's `noUndeclaredDependencies` rule will warn you if your project imports a package that isn't listed in its `package.json`.
