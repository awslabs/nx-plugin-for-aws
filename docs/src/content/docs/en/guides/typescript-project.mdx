...
Aliases match your projects' package names, so any project in your workspace can be imported by the name declared in its `package.json` — no need to declare local projects as dependencies.
:::
...
Each TypeScript project declares the third-party **runtime dependencies its source code imports** in its own `package.json`. This keeps every project self-describing — its manifest lists exactly what it needs — and lets Biome's [`noUndeclaredDependencies`](https://biomejs.dev/linter/rules/no-undeclared-dependencies/) lint rule enforce that a project never imports a package it hasn't declared.

**Build and test tooling** (for example `vitest`, `tsx`, `rolldown`, the Nx toolchain (`nx`, `@nx/js`, ...), `typescript`, `@biomejs/biome`) is shared across the workspace, so it lives in the **root** `package.json` `devDependencies` rather than in each project. These tools are only referenced from config files, build scripts and tests, which the lint rule exempts.

To add a runtime dependency to a project, add it to that project's `package.json`. Using your package manager's filter/workspace flag targets the right project, for example with pnpm:

```bash
pnpm add some-npm-package --filter @my-scope/my-library
```

To add a shared dev tool, add it to the workspace root:

<InstallCommand pkg="-D some-dev-tool" />

This follows a [single version policy](https://nx.dev/concepts/decisions/dependency-management#single-version-policy), meaning all projects use the same version of each dependency — avoiding type conflicts and multiple copies of packages in deployment bundles. For package managers which support catalogs ([pnpm](https://pnpm.io/catalogs), [yarn](https://yarnpkg.com/features/catalogs) and [bun](https://bun.com/docs/install/catalogs)), generators record each version in the catalog and reference it with the `catalog:` protocol (eg `"zod": "catalog:"`), so the catalog is the single source of truth for versions regardless of which project declares the dependency. The vended `biome.json` enables Biome's [`experimentalPnpmCatalogs`](https://biomejs.dev/reference/configuration/#javascriptresolverexperimentalpnpmcatalogs) resolver so the lint rule understands `catalog:` references.

:::tip[Aligning versions on npm]
npm (and yarn classic) has no catalog feature, so versions are declared directly in each `package.json`. To keep them consistent across projects, we recommend [syncpack](https://github.com/JamieMason/syncpack) (`npx syncpack lint`).
:::

:::note[bun]
On bun, a few packages whose versions Nx generators read from the root manifest (`vite`, `react`, `react-dom`) keep direct version ranges instead of `catalog:` references — Nx does not yet resolve bun catalogs, and a `catalog:` reference would break those generators.
:::
...
With catalogs disabled, generators write dependency versions directly into each project's `package.json`, and keeping versions aligned across projects is your responsibility (for example using [syncpack](https://github.com/JamieMason/syncpack)). When catalogs are enabled on pnpm, the workspace also sets [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) so `pnpm add` records new dependencies in the catalog, keeping the single version policy without manual editing.

When one project imports another project in your workspace, you don't need to declare it by hand: the TypeScript path aliases resolve the import for building, and the `ts#sync` generator (which runs before `compile`, or on demand via `nx sync`) declares the imported project as a workspace dependency in the importing project's `package.json` (`workspace:*` where the package manager supports the workspace protocol, `*` on npm and yarn classic). This keeps each project's manifest an accurate record of what it uses — satisfying the `noUndeclaredDependencies` lint rule — without manual bookkeeping.
...
If you are publishing your TypeScript project to NPM, its `package.json` must declare entry points (`main`/`exports`) in addition to its dependencies, which the generators already keep accurate (see [Dependencies](#dependencies)).
