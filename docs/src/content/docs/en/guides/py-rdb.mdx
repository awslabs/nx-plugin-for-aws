...
Import `session_context` from your database package and use it as a context manager to obtain a SQLModel `Session`:
...
The `rds_ca` value must point to a CA certificate bundle available in your runtime filesystem. For Amazon RDS, use the global CA bundle from:

```text
https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
```

How you make that file available depends on the runtime that connects to the database.

#### Runtime Container Images

If you prepare your own container image for the runtime, add the RDS CA bundle in your Dockerfile and pass that path to `session_context`:
...
If you deploy a zipped Lambda function, attach the `RdsCaLayer` Lambda layer (CDK) or `ca-layer` Terraform module. The CDK layer uses Docker bundling to add the CA bundle, and the deployed layer provides it at `/opt/global-bundle.pem`, which you can pass to `session_context`:
