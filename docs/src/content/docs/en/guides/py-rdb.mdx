...
<FileTree>
  - \<name>
    - \_\_init\_\_.py Package exports (`get_engine`, `is_local_dev`, `session_context`)
  ...
  - migrations
    - versions
      - .gitkeep Keeps the (initially empty) versions directory in version control
...
Alembic writes each generated migration script into `migrations/versions/`. `is_local_dev()` returns whether the process is running against the local development database (`LOCAL_DEV=true`), which is useful for branching on local versus deployed behaviour in your own code.
