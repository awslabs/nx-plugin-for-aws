...
<FileTree>
  - \<name>
    - \_\_init\_\_.py Package exports (`get_engine`, `get_session`)
...
Import `get_session` from your database package and use it as a generator to obtain a SQLModel `Session`:

```python
from my_scope.my_db import get_session

for session in get_session():
    results = session.exec(select(ExampleModel)).all()
```

Or use `get_engine` for direct engine access:
...
`get_engine()` returns a lazily-initialised, cached engine. Subsequent calls within the same Lambda execution context reuse the existing engine rather than creating a new one.

In FastAPI, use `get_session` as a dependency via `Annotated`:
...
- Generates temporary authentication tokens via `boto3` RDS Signer for IAM authentication (PostgreSQL)
- Manages SSL/TLS connections with certificate validation

## Deploying your Database

<Snippet name="rdb/deploying" />

### RDS Proxy Configuration

<Snippet name="rdb/rds-proxy" />
...
The Python MySQL driver (PyMySQL) does not support the `mysql_clear_password` plugin required for RDS IAM authentication. As a result, the migration handler and create-db-user handler use the admin credentials from AWS Secrets Manager to authenticate, rather than IAM tokens.

:::note
This limitation does not affect the application-level connection (`get_session` / `get_engine`) for IAM-authenticated database users — that path uses the generated application user created during deployment. The Secrets Manager credentials are only used during the Lambda-based setup phase (migrations and user creation).
:::
