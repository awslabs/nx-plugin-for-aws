...
Checkov runs as part of both `build` and `package`, so an interactive `plan` scans your infrastructure before you apply it.
...
`plan` depends on `package` rather than `build`, so it produces the artifacts your modules reference — the Lambda bundles and generated operations metadata — and runs Checkov, without also running every project's lint, test and type-check gates.
