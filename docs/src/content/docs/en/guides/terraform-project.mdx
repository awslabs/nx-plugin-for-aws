...
Every target that runs `terraform init` shares a provider cache at `.terraform/plugin-cache` in your workspace root. See the [Terraform documentation](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) for more information.
