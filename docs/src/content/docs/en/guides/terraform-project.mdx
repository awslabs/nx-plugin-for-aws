...
The `fmt` target checks that your Terraform code is formatted, and fails if it is not. It runs as part of `build`:

<NxCommands commands={['fmt tf-infra']} />

To reformat your code, run with the `fix` configuration:

<NxCommands commands={['fmt tf-infra --configuration=fix']} />

A `lint` target orchestrates the format check, so `run-many` reaches your Terraform projects alongside your TypeScript and Python ones:

<NxCommands commands={['run-many --target lint --all --configuration=fix']} />

:::tip[Why the check is the default]
`terraform fmt` rewrites the files it reads — the very files the `fmt` target declares as its cache inputs. Writing from the base target would change its own cache key on every run, so it could never register a cache hit. Checking instead keeps the target cacheable, mirroring how the TypeScript and Python `format` targets work.
:::

To skip formatting and linting during development, use the `skip-lint` configuration:
