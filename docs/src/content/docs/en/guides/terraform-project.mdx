...
Every target that runs `terraform init` shares one provider cache at `.terraform/plugin-cache` in your workspace root, via Terraform's [`TF_PLUGIN_CACHE_DIR`](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache). Providers are downloaded once and linked from there afterwards, which matters most for the `test` target: it keeps its working directory under `dist`, so a cleaned `dist` or a cache miss would otherwise re-download the AWS provider — several hundred megabytes — on every run.

The directory is created automatically before each `terraform init`, and `.terraform` is gitignored. Unlike `.nx/cache`, it is not removed by `nx reset`, so a reset does not throw the providers away. In CI, cache `.terraform/plugin-cache` between jobs to get the same saving there.

Nx's `env` takes precedence over your shell, so setting `TF_PLUGIN_CACHE_DIR` in your environment does not change where these targets look. To point them somewhere else — a location shared between several workspaces, say — change `TF_PLUGIN_CACHE_DIR` in your project's `project.json`, and the path in `scripts/plugin-cache.ts` for the `bootstrap`, `bootstrap-destroy` and `init` targets. Paths in `project.json` are relative to the target's `cwd`, since Nx does not expand `{workspaceRoot}` inside `env`.

:::note[Lock Files]
The cache does not change which provider versions you get: `.terraform.lock.hcl` still pins them, and Terraform records the same hashes whether a provider came from the cache or the registry. Commit the lock file as usual.
:::
