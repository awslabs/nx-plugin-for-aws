...
The `install-providers` target is what fills that cache, and every target that runs `terraform init` depends on it:

<NxCommands commands={['install-providers tf-infra']} />

You will not normally run it yourself. It exists because two `terraform init` runs filling one cache at the same time can each compute a different hash for the same provider, which terraform then rejects against your `.terraform.lock.hcl` with "the cached package ... does not match any of the checksums recorded in the dependency lock file". Downloading the providers once, ahead of the targets that need them, leaves those targets only reading from a cache that is already warm.

Set `TF_PLUGIN_CACHE_DIR` in your environment to point the vended `init` script at a cache you manage yourself — a volume shared between workspaces, say. Note that the `install-providers`, `test` and `validate` targets read their path from `project.json`, so change it there too.
