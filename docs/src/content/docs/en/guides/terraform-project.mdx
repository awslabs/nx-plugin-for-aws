...
Checkov runs as part of both `build` and `assemble`, so an interactive `plan` scans your infrastructure before you apply it.
...
`plan` depends on `assemble` rather than `build`, so it produces the artifacts your modules reference — the Lambda bundles and generated operations metadata — and runs Checkov, without also running every project's lint, test and type-check gates.

:::caution
Because `plan` no longer runs the Terraform module tests or your projects' unit tests, run `build` to check your code before applying anything you care about, and gate your pipeline on it:
