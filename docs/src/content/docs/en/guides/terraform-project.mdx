...
A project with no test files is a no-op success, so you can add tests when you need them. This target is not part of `build`, since running tests downloads the providers — invoke it directly, or from CI where Terraform is available.
