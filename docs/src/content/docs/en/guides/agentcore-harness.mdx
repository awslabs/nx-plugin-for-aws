...
Narrow `@builtin` to specific patterns such as `@builtin/file_operations` to restrict what the agent loop can do. Tools that reach customer-owned resources also need execution role permissions — see <a href="#execution-role-permissions">Execution Role Permissions</a>.
