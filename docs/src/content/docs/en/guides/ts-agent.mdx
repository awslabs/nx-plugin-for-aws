...
Add a procedure that starts a conversation. It generates the session ID itself, resolves the caller from its own request context, and stores the pair — for example in a table created with the <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link> generator:
