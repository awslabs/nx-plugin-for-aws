...
The AG-UI protocol has its own conversation identifier, `threadId`, in the request body. `@ag-ui/aws-strands` caches one Strands agent per `threadId` and calls `sessionManagerProvider` **once per thread** — the first time that thread is seen.

The generated `index.ts` therefore passes `sessionManagerProvider: (input) => getSessionManager(input.threadId)`, keying conversation state on that same thread id so the provider and the adapter's cache always agree.

`getSessionManager()` falls back to the current AgentCore session ID when called without an argument, which is how the HTTP and A2A protocols use it.
:::

:::caution[Session isolation]
The session ID and `threadId` are both supplied by the caller, so neither can by itself identify whose conversation it is. AgentCore Runtime authorizes invocations against the agent runtime resource ARN rather than against individual sessions, and validates the format of a session ID without tying it to the calling identity — [scoping a session to a user is the agent's responsibility](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-instances-security.html).

With `auth=cognito`, the generated infrastructure sets `requestHeaderConfiguration.allowlistedHeaders: ['Authorization']`, so the caller's JWT — already validated by the AgentCore authorizer — reaches your agent code as an ordinary request header. The server entry point reads its `sub` claim and binds it for the request, and `session.ts` builds the storage key from it via `getSessionStorageKey()`, producing `u-<sub>-<conversationId>`.
...
On the AG-UI path the middleware also deletes the protocol's `thread_id` alias after folding it into `threadId`. The adapter maps `thread_id` onto `threadId` first-key-wins when it parses the body, so leaving the alias in place would let a client smuggle an unscoped thread id past the rewrite — keep both steps together if you customise this.
...
`getSessionStorageKey()` also flattens path separators, so a conversation ID cannot traverse out of its namespace.
:::
...
Add a procedure that starts a conversation. It generates the session ID, resolves the caller from its own request context, and stores the pair — for example in a table created with the <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link> generator:
...
Resolve the caller from the verified request context rather than from an input field, so it cannot be supplied by the browser. <Link path="guides/trpc">Implementing Custom Middleware</Link> shows how for both IAM and Cognito authenticated APIs. A second procedure querying the index lets the UI offer a user their own conversations without seeing anyone else's.

**2. Validate ownership in the agent**

The agent receives the session ID, and with `auth=cognito` the caller's JWT. Look the session up, compare its owner against the caller, and refuse the request if they differ:
