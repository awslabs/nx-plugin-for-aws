...
The generator configures a `<your-agent-name>-package` target which assembles the deployable code package: the bundled `index.js` plus a vendored install of the AWS Distro for OpenTelemetry, which AgentCore requires to be present in the package. The infrastructure uploads this directory as a `.zip` using `AgentRuntimeArtifact.fromCodeAsset`.
</OptionFilter>
