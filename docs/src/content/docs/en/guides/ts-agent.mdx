...
The generated `index.ts` passes `getSessionManager`, which resolves the session ID from the header-derived context rather than from `threadId`. The two stay aligned because the <Link path="/guides/connection/react-agui">`connection` generator</Link>'s React hook derives the session header from the thread ID. If you invoke the agent from your own client, keep that relationship: send the same value as both, and note that because the provider is only called once per thread, changing the session header on a later request for the same thread will not switch the stored conversation.
:::

:::caution[Session isolation]
Both the session ID and `threadId` are supplied by the caller, and the session ID is the key used to read and write stored conversation state. The generated code does not namespace sessions per user, so any caller able to invoke the agent can read another user's conversation by supplying their session ID.
