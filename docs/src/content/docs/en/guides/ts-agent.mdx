...
Use `<your-agent-name>-dev` for this, not `<your-agent-name>-serve`. `-serve` runs the deployed code path, so it needs `RUNTIME_CONFIG_APP_ID` set — see <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Without it, every invocation fails with `RUNTIME_CONFIG_APP_ID is not set`.
:::
