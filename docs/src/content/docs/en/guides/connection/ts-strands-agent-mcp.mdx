...
```ts title="packages/infra/src/stacks/application-stack.ts"
import { UserIdentity, AgentCoreM2MIdentity } from ':my-scope/common-constructs';

// 1. Shared Cognito User Pool
const identity = new UserIdentity(this, 'Identity');

// 2. Register M2M credentials with AgentCore Identity
new AgentCoreM2MIdentity(this, 'McpM2MIdentity', {
  providerName: 'mcp-my-mcp-server',
  serverClassName: 'MyMcpServer',
  userPool: identity.userPool,
  m2mClient: identity.m2mClient,
  scope: identity.scope,
  cognitoDomainPrefix: identity.domainPrefix,
});

// 3. MCP server with Cognito JWT authorizer
const mcpServer = new MyMcpServer(this, 'MyMcpServer', {
  identity: {
    userPool: identity.userPool,
    userPoolClient: identity.m2mClient,
  },
});

// 4. Agent with Cognito JWT authorizer (enables workload access token delivery)
const myAgent = new MyAgent(this, 'MyAgent', {
  identity: {
    userPool: identity.userPool,
    userPoolClient: identity.userClient,
    m2mClients: [identity.m2mClient],
  },
});
...
- **UserIdentity**: Creates a Cognito User Pool with M2M app client and resource server
- **AgentCoreM2MIdentity**: Registers the M2M client credentials in AgentCore Identity's Token Vault via `CreateOauth2CredentialProvider`
