...
The generator inspects the backend's `computeType` and produces the matching client:

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
An HTTP link with `httpSubscriptionLink` over Server-Sent Events for tRPC subscriptions.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
A plain HTTP link. Subscriptions are not supported because API Gateway HTTP APIs don't support response streaming.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
A WebSocket link against the AgentCore runtime URL, with SigV4-signed handshake (IAM) or base64url-encoded JWT subprotocol (Cognito). The runtime ARN is read from `runtimeConfig.apis.<ApiName>` and converted to a `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` URL on the fly.
</TabItem>
</Tabs>
...
Additionally, it installs the required dependencies:
  ...
  - `event-source-polyfill` (if using a REST API backend, for subscription support)
  - `react-oidc-context` (if using Cognito auth)
...
:::caution[No subscriptions on HTTP API]
API Gateway HTTP APIs don't support response streaming. Select `ServerlessApiGatewayRestApi` or `BedrockAgentCoreRuntimeWebSocket` as your backend `computeType` if you need subscriptions.
:::
</OptionFilter>
...
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
The generated client uses a `splitLink` that routes subscription operations through `httpSubscriptionLink` (Server-Sent Events) and regular queries/mutations through `httpLink`. Subscriptions work out of the box with no additional configuration.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
The generated client uses `wsLink` against the AgentCore runtime URL — all tRPC operations (queries, mutations, and subscriptions) travel over a single WebSocket connection. The handshake is authenticated automatically: SigV4-signed for `IAM` auth, or carrying the JWT as a base64url-encoded subprotocol for `Cognito`.
</TabItem>
</Tabs>
