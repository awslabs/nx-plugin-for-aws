...
- **`ServerlessApiGatewayRestApi`** — HTTP link with `httpSubscriptionLink` over Server-Sent Events for tRPC subscriptions.
- **`ServerlessApiGatewayHttpApi`** — plain HTTP link (no subscriptions).
- **`BedrockAgentCoreRuntimeWebSocket`** — WebSocket link against the AgentCore runtime URL, with SigV4-signed handshake (IAM) or base64url-encoded JWT subprotocol (Cognito). The runtime ARN is read from `runtimeConfig.apis.<ApiName>` and converted to a `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` URL on the fly.
