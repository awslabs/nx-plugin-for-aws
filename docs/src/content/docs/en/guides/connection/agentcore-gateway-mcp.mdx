...
1. A <Link path="guides/agentcore-gateway">`cedar#agentcore-gateway`</Link> project
...
- packages/common/agent-connection (TypeScript, if a TS gateway client exists)
  - src/app
    - \<gateway-kebab>-client.ts `ATTACHED_MCP_SERVERS` updated to include the MCP server
- packages/common/agent\_connection (Python, if a Py gateway client exists)
  - \<scope>\_agent\_connection/app
    - \<gateway\_snake>\_client.py `ATTACHED_MCP_SERVERS` updated to include the MCP server

</FileTree>

The Gateway project's `<gateway>-serve-local` aggregator target gains a dependency on the MCP server's `<mcp>-serve-local` target, so running the Gateway locally also starts the MCP server. Any existing `<Gateway>Client.ts` / `_client.py` files (emitted by the <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> or <Link path="guides/connection/py-agent-gateway">Python</Link> gateway-connection generators) have their `ATTACHED_MCP_SERVERS` list updated to include this MCP server so local multiplexing picks it up.
...
starts every attached MCP server on its assigned local port. Agents connected to the Gateway via the <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> or <Link path="guides/connection/py-agent-gateway">Python</Link> gateway-connection generators multiplex directly to the MCP servers using their client's local fallback path.

:::caution[Local fidelity]
Local development **bypasses the Gateway entirely** — the generated gateway client detects `SERVE_LOCAL=true` and connects directly to each attached MCP server over plain HTTP. As a consequence, **Cedar policies are not evaluated locally**. Every agent sees every tool on every attached MCP server. To exercise Cedar policies, set `GATEWAY_URL_OVERRIDE` (see the <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> / <Link path="guides/connection/py-agent-gateway">Python</Link> agent-connection guides) so the locally-running agent calls the deployed Gateway instead.
