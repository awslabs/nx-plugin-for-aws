...
| ------------ | -------------------------------------------------------------------- |
| `Cloudscape` | Assistant and user messages render inside Cloudscape `ChatBubble` components with gen-AI `Avatar`s, matching the [Cloudscape generative AI chat pattern](https://cloudscape.design/patterns/genai/generative-AI-chat/). The message toolbar becomes a Cloudscape `ButtonGroup` on the bubble's `actions` slot (copy, thumbs up/down, regenerate), Markdown is rendered via CopilotKit's default renderer inside the bubble, the typing indicator becomes a gen-AI `LoadingBar`, and the input is replaced with Cloudscape `PromptInput`. Built from `@cloudscape-design/components` and `@cloudscape-design/chat-components`. |
| `Shadcn` | Assistant messages render with a `Sparkles` avatar and a subdued `bg-muted` card-style bubble using Tailwind `prose` styling; a hover-revealed `Button` toolbar provides copy/thumbs/regenerate. User messages render right-aligned in a `bg-primary` bubble with a `User` avatar. The input is replaced with a rounded `Textarea` + pill-shaped icon `Button` (Enter submits, Shift+Enter inserts a newline; auto-grows up to 8 rows), and the typing indicator is a three-dot pulse animation. Uses shadcn primitives (`Avatar`, `Button`, `Textarea`) from the shared `common-shadcn` package. |
| `None` (or anything else) | No theme applied — the wrapper simply re-exports the default CopilotKit components. |
