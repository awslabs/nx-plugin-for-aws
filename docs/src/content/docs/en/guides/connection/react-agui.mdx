...
Both values come from the browser, so they name a conversation without saying whose it is. To restrict each user to their own conversations, see <Link path="guides/ts-agent">Restricting sessions to their owner</Link>.
