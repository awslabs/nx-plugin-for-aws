...
AG-UI is not yet supported for TypeScript Agents. See [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) for the latest status. For now, use the <Link path="guides/py-agent">`py#agent` generator</Link> with `protocol=AG-UI`.
:::
...
2. A Python Agent with `protocol=AG-UI` (generated using the <Link path="guides/py-agent">`py#agent` generator</Link>)
...
Instantiate CopilotKit components with `agentId` to select which agent to use. The id is the agent's name — the same one you chose when you ran the <Link path="guides/py-agent">`py#agent` generator</Link> — and you can also find it in the generated hook file (e.g. the key returned from `packages/web/src/hooks/useAgui<AgentName>.tsx`).
...
- <Link path="guides/connection/react-py-agent">React to Python Agent (HTTP) Guide</Link>
