...
:::note
AG-UI is supported for both TypeScript and Python Agents. Use the <Link path="guides/ts-agent">`ts#agent`</Link> or <Link path="guides/py-agent">`py#agent`</Link> generator with `protocol=AG-UI`.
:::
...
- <Link path="guides/connection/react-ts-agent">React to TypeScript Agent (HTTP) Guide</Link>
