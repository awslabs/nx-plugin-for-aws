...
If your agent uses IAM auth, the Cognito Identity Pool's authenticated role must be granted permission to invoke the agent. You can use the `grantInvokeAccess` method which grants both `InvokeAgentRuntime` and `InvokeAgentRuntimeWithWebSocketStream` permissions:
