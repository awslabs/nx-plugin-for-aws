...
:::note
For IAM auth, the Cognito Identity Pool's authenticated role must have the `bedrock-agentcore:InvokeAgentRuntimeWithWebSocketStream` permission to invoke agents via WebSocket. The CDK `grantInvoke` method does not yet include this permission automatically (see [aws-cdk#37495](https://github.com/aws/aws-cdk/issues/37495)), so you will need to add it explicitly in your infrastructure stack:
...
3. For IAM auth, request signing is automatically skipped in `serve-local` mode when <Link path="guides/react-website#runtime-configuration">`runtime-config.json`</Link> is not present
