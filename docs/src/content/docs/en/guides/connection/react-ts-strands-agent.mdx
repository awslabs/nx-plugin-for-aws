...
- **IAM** (default): Uses AWS SigV4 presigned URLs to authenticate the WebSocket connection. Credentials are obtained from the Cognito Identity Pool configured with your website's auth. In `serve-local` mode, signing is automatically skipped when Cognito is not configured
...
3. For IAM auth, request signing is automatically skipped in `serve-local` mode when Cognito is not configured
