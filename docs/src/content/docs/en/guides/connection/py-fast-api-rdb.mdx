...
- packages/my_api/my_api
  - dependencies
    - my_db.py FastAPI `SessionDep` dependency for the database session
...
The generated dependency file exports a `SessionDep` type alias that injects an `AsyncSession` via FastAPI's `Depends`:
