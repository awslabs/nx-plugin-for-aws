...
When invoking the API directly, note that the `api_endpoint` output is the API Gateway base URL without the stage path — append `/prod` (or use the `stage_invoke_url` output) when calling the deployed API.
