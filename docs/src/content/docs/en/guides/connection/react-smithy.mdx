...
If you're actively working on both your React application and Smithy API together, use the React application's `dev` target (an alias for `serve-local`) which will automatically regenerate the client whenever your API changes, as well as hot-reloading your website and local Smithy API server:
