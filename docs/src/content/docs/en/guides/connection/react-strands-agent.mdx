...
3. Cognito Auth added via the <Link path="/guides/react-website-auth">`ts#react-website-auth` generator</Link> if connecting an agent which uses Cognito or IAM auth
...
- src
  ...
  - hooks
    ...
    - use\<AgentName>Agent.tsx A hook returning the tRPC options proxy for TanStack Query integration
    - use\<AgentName>AgentClient.tsx A hook returning the vanilla tRPC client for direct agent calls
...
- **Deployed**: The agent runtime ARN is loaded from <Link path="guides/runtime-config">Runtime Configuration</Link>. The ARN is converted to a WebSocket URL following the Bedrock AgentCore Runtime WebSocket protocol: `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/<encoded-arn>/ws`
...
- **IAM** (default): Uses AWS SigV4 presigned URLs to authenticate the WebSocket connection. Credentials are obtained from the Cognito Identity Pool configured with your website's auth
...
- **No auth**: Connects directly without authentication (suitable for local development)

## Using the Generated Code

### Streaming Agent Responses with useSubscription

The most common use case is streaming the agent's response using the `invoke` subscription:
...
Here is an example of building a simple chat interface with the generated client:
...
3. The client detects the local URL and connects directly without authentication overhead

This means you can develop and test your agent integration locally with a single command.
