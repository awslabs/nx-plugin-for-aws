...
`create()` always returns a single `McpClient`. When deployed (`SERVE_LOCAL` unset), it points at the Gateway's MCP endpoint and authenticates with SigV4. When `SERVE_LOCAL=true`, it points at the local gateway started by the Gateway project's `serve-local` target, so the same `agent.ts` works uniformly in both modes.
