...
When deployed (`SERVE_LOCAL` unset), `create()` returns a single `McpClient` pointed at the Gateway's MCP endpoint and authenticated with SigV4. When `SERVE_LOCAL=true`, it returns an array of `McpClient`s — one per MCP server in the client's `ATTACHED_MCP_SERVERS` list. Strands' `Agent.tools` accepts either shape (it transparently flattens nested tool arrays), so the same `agent.ts` works uniformly in both modes.
