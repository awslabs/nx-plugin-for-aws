...
Import `getDynamoDBClient`, `resolveTableName`, and your entity factories from the DynamoDB package and use them inside `createServer`:
...
`getDynamoDBClient()` returns a cached singleton and `resolveTableName()` caches the table name after the first call, so both are efficient across invocations.

## Multiple Tables

Running the generator again with a different target adds the second table alongside the first. Import from both packages inside `createServer`:

```ts title="packages/my-service/src/my-mcp/server.ts"
import { createExampleEntity } from ':my-scope/my-table';
import { createOtherEntity } from ':my-scope/other-table';

export const createServer = async () => {
  const server = new McpServer({ ... });
  // register tools using both entity factories
  return server;
};
```
...
table.table.grantReadWriteData(myMcpServer);
```
...
This starts the MCP server and the local DynamoDB instance together. The `SERVE_LOCAL=true` environment variable is set automatically, so `getDynamoDBClient()` and `resolveTableName()` connect to the local DynamoDB Local instance instead of AWS.
