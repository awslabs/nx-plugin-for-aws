...
When the agent connects directly to the Aurora cluster (without RDS Proxy), the connection generator updates the generated agent Dockerfile to add the Amazon RDS CA bundle at `/opt/global-bundle.pem`:
