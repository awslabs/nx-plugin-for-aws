...
The generator adds the database package as a workspace dependency and updates the agent's `<agent-name>-dev` target to depend on the database's `dev` target so both start together during local development. No source files are modified.

## Using the Database in Agent Tools

Import `get_session` or `get_engine` from your database package and use them inside your agent tools:
