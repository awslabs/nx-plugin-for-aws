...
`my_scope_my_db` is the module name of your database project — a single flat module named `<scope>_<name>`, visible as the package directory inside the project.

`.scalars()` unwraps the result rows into `ExampleModel` instances, so `model_dump()` is available on each item.
