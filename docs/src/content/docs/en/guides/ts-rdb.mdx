...
- `DB_PACKAGE_NAME`, the key used under `connection.rdbs` in runtime config
...
This provisions an Aurora cluster, the generated admin credentials secret, the create-db-user Lambda function, runtime config registration in AWS AppConfig, the migration Lambda function, and the supporting resources needed to publish and run the migration container image.
...
To allow Lambda-based application code to use the generated Prisma client in AWS, deploy it into the same VPC as the database, pass `RUNTIME_CONFIG_APP_ID`, grant permission to read runtime config from AWS AppConfig, allow network access to the database port, and grant IAM database connect permissions for the generated runtime user.

:::note[Local Prisma CLI vs Deployed Runtime]
Prisma CLI commands such as `prisma migrate dev` and `prisma migrate deploy` still use `DATABASE_URL` when you run them locally.

The generated runtime client in `src/prisma.ts` does not read `DATABASE_URL`. It loads `connection.rdbs[DB_PACKAGE_NAME]` from AWS AppConfig using `RUNTIME_CONFIG_APP_ID` and connects to Aurora using IAM authentication.
:::
...
const api = new Api(this, 'Api', {
  integrations: Api.defaultIntegrations(this)
    .withDefaultOptions({
      ...
      environment: {
        RUNTIME_CONFIG_APP_ID: runtimeConfig.appConfigApplicationId,
      },
    })
    .build(),
});
...
Object.entries(api.integrations).forEach(([operation, integration]) => {
  runtimeConfig.grantReadAppConfig(integration.handler);
...
- pass `RUNTIME_CONFIG_APP_ID = module.postgres.appconfig_application_id`
