...
The Aurora cluster exports database logs to CloudWatch Logs (`postgresql` for PostgreSQL, `error` and `slowquery` for MySQL) and enables Performance Insights on the writer instance (encrypted with the cluster's KMS key) so that queries are observable for troubleshooting and auditing. Note that CloudWatch Logs and Performance Insights incur additional cost.
