...
Use the `prisma` target to run any Prisma CLI command against the local database. It automatically starts the database via `serve-local` before running, so there is no need to set up a local database separately.

<NxCommands commands={['run <project>:prisma migrate dev --name <migration-name>']} />

<NxCommands commands={['run <project>:prisma migrate deploy']} />

<NxCommands commands={['run <project>:prisma studio']} />

The `prisma` target sets `SERVE_LOCAL=true` automatically, which `prisma.config.ts` uses to construct the database connection URL from the local credentials configured at generation time.
