...
When using the Prisma CLI, run commands from your generated database package directory (e.g., `packages/mydb`) so Prisma can find `prisma/schema.prisma` and `prisma.config.ts`.
:::

You can also run from the workspace root with the `prisma` target:
...
<PackageManagerExecCommand commands={['prisma migrate dev --create-only']} />
...
<PackageManagerExecCommand commands={['prisma migrate deploy']} />
...
:::note
See the <Link path="/guides/trpc">`ts#trpc-api` generator guide</Link> for more details on the middleware pattern.
