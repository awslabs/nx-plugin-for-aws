...
The MySQL engine uses the [`mariadb`](https://www.npmjs.com/package/mariadb) npm package as its database driver. This package is licensed under `LGPL-2.1-or-later`, which is not included in the default license allowlist written by the <Link path="/guides/license/#dependency-license-checks">license generator</Link>. If you enable the dependency license check and have determined that the LGPL-2.1 terms are acceptable for your use case, add an exception for `mariadb` in your `aws-nx-plugin.config.mts`:
