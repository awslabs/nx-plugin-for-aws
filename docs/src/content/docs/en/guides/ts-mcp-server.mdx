...
:::caution
The MCP SDK only supports Zod v3 at present. To allow for Zod v4 to be used in other generators, we still install v4 but import v3. For MCP tool schemas, you will need to import Zod as follows:

```ts
import { z } from 'zod/v3';
```

Once [this issue with the MCP SDK](https://github.com/modelcontextprotocol/typescript-sdk/issues/164) is resolved, you can drop the `/v3` and just import directly from `zod`.
:::
