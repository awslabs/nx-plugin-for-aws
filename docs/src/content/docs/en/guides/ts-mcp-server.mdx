...
### Scoping Tool Data per Caller

`createServer` receives an `McpRequestContext` describing the current request. Over Streamable HTTP this includes the caller's AgentCore Runtime session id, which an <Link path="guides/ts-agent">agent</Link> forwards automatically on every outbound MCP call.

Pass it to a tool that stores data per caller:
...
:::caution
Scope stored data on the request context rather than on a tool argument. A model can be persuaded to pass any value it likes, so arguments such as a user or player name are not a trust boundary. The context is absent over STDIO, where there is no request.
:::
