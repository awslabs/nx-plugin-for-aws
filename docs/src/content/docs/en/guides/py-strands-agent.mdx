...
The generator configures a `<your-agent-name>-chat` Nx target that depends on `<your-agent-name>-serve-local`. Running it starts the agent locally and drops you into an interactive terminal chat powered by [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

For **A2A** and **AG-UI** agents, the chat target simply invokes `agent-chat-cli` as a binary against the locally-running agent — no code is vended into your workspace.

For **HTTP** agents, the generator emits:
...
- An `<your-agent-name>-generate-client` Nx target that runs `open-api#ts-client` to produce a type-safe TypeScript client under `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — a thin adapter that imports the generated client and plugs it into `agent-chat-cli`'s `chatLoop`

The chat script is a small wrapper around the generated client. When you customize the agent's input shape (e.g. add new fields to `InvokeInput`), update the adapter's `sendMessage` to pass the new fields and the rest works automatically.
