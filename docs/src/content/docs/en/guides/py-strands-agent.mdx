...
The generator also vends a minimal interactive chat CLI under `scripts/<your-agent-name>/chat.ts` and a corresponding `<your-agent-name>-chat` target. This target depends on `<your-agent-name>-serve-local`, so running it starts the agent locally and drops you into an interactive terminal chat with it:

<NxCommands commands={['run your-project:agent-chat']} />

The CLI is a TypeScript script (run via `tsx`) that uses the protocol-appropriate client to stream responses token-by-token via [`@clack/prompts`](https://github.com/bombshell-dev/clack):

- **HTTP**: `POST /invocations` with JSONL streaming
- **A2A**: `@a2a-js/sdk` `ClientFactory` with `sendMessageStream`
