...
When `protocol` is set to `AG-UI`, the entry point uses the [ag-ui-strands](https://docs.ag-ui.com/) integration, which exposes your agent via the AG-UI protocol for direct frontend integration with [CopilotKit](https://docs.copilotkit.ai/aws-strands):
...
When `protocol=AG-UI`, the generated `main.py` wraps your Strands `Agent` in an `ag_ui_strands.StrandsAgent` and creates a FastAPI app via `create_strands_app()`. The resulting app exposes a single POST endpoint that streams [AG-UI](https://docs.ag-ui.com/) events over Server-Sent Events (SSE), as well as `/ping` for the AgentCore runtime health check.
...
The generator configures a target named `<your-agent-name>-invoke` that starts an interactive chat with your agent from the terminal. It ships as a TypeScript script (via `tsx`) and automatically picks the right protocol (HTTP, A2A, AG-UI) for your agent. By default it connects to the local server:

<NxCommands commands={['agent-invoke your-project']} />

To chat with a deployed agent instead, set `RUNTIME_CONFIG_APP_ID` to your workspace's AppConfig application ID — the CLI resolves the runtime ARN and invokes the deployed endpoint with SigV4-signed requests:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

For Cognito-authenticated agents, pass a JWT with `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

The CLI script lives at `scripts/<agent>/cli.ts` in your project.
