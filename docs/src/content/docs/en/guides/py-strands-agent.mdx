...
The Strands SDK's `A2AAgent` wraps a remote A2A endpoint so you can call it like a local `Agent`.
...
Requests to a deployed A2A agent on AgentCore (including the agent card fetch) must be SigV4-signed. Pass a `client_config` to `A2AAgent` with an `httpx.AsyncClient` that uses a SigV4 auth flow:
...
The A2A generator adds `strands-agents[a2a]` for you. If you're adding the A2A client code to an HTTP agent, add `strands-agents[a2a]` and `httpx` to your `pyproject.toml`.
:::

#### Using an A2A Agent as a Tool

`A2AAgent` inherits from `AgentBase` like a local `Agent`, so you can wrap it in a `@tool` and hand it to another agent:
