...
:::tip
AG-UI agents can be connected to a React frontend using the <Link path="/guides/connection/react-py-strands-agent">`react` → `py#strands-agent` connection generator</Link>, which will set up [CopilotKit](https://copilotkit.ai/) components for a rich chat experience.
:::
...
- **AG-UI**: The [ag-ui-strands](https://docs.ag-ui.com/) integration, which exposes the [AG-UI protocol](https://docs.ag-ui.com/) over SSE. Best for direct frontend integration with [CopilotKit](https://copilotkit.ai/) in a React website.
...
AG-UI agents are designed to be consumed directly by a frontend. Use the <Link path="/guides/connection/react-py-strands-agent">`react` → `py#strands-agent` connection generator</Link> to wire your React website up to the agent with a [CopilotKit](https://copilotkit.ai/) provider and [AG-UI HttpAgent](https://docs.ag-ui.com/) client.
:::
