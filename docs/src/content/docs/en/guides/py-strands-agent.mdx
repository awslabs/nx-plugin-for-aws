...
For streaming responses, the generator uses `JsonStreamingResponse` which automatically serializes Pydantic models to JSON Lines format (`application/jsonl`). This format is compatible with OpenAPI 3.2's streaming specification and works seamlessly with the generated TypeScript client.

By default, the agent yields `StreamChunk` objects containing the agent's response text:

```python
from .streaming import JsonStreamingResponse
...
You can define your own Pydantic model to yield structured data. Simply update the `StreamChunk` model and the response configuration:
...
@app.post(
    "/invocations",
    response_class=JsonStreamingResponse,
    responses={200: JsonStreamingResponse.openapi_response(StreamChunk, "Stream of response chunks")},
)
async def invoke(input: InvokeInput) -> JsonStreamingResponse:
    return JsonStreamingResponse(handle_invoke(input))
```

The `JsonStreamingResponse` class handles serialization automatically, converting each yielded Pydantic model to a JSON line.
