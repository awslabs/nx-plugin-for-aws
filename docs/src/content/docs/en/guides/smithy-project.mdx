...
The CLI is resolved by [mise](https://mise.jdx.dev/), added to your workspace as a dev dependency, so there is nothing to install — it downloads and caches the pinned version the first time you build.

:::caution[Windows]
`mise` publishes no Windows package to npm, so follow the [Smithy CLI installation guide](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) to install it yourself before building.
:::

:::tip[Building in CI]
`mise` reads the CLI's release metadata from the GitHub API, whose unauthenticated rate limit is shared per IP address and easily exhausted on a CI runner — the download then fails with `403 Forbidden`. Set `GITHUB_TOKEN` in your build environment to lift it. Subsequent builds hit the local cache and make no request at all.
:::
