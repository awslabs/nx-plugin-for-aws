...
### 1. Copy the built model into the Docker build

Smithy projects build inside a container, and the build is given the workspace root as a named build context. Add a `COPY` to the consuming project's `build.Dockerfile`, alongside where its own sources are copied in:
...
Add the copied file to `imports` in the consuming project's `smithy-build.json`:
...
:::note
`imports` rather than `sources` keeps the library's shapes out of your project's own source projection, while still resolving them and carrying them through to your build output.
:::

### 3. Build the library first

Add the library's `build` target as a dependency of the consuming project's `compile` target in its `project.json`, so the model exists before the consumer builds:
