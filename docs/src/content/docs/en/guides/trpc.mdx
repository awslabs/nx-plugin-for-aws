...
:::tip
`schema` and `backend` are both TypeScript projects, so you can refer to the [TypeScript project documentation](/nx-plugin-for-aws/guides/typescript-project) for more details about their general usage.
:::
...
If you are calling your API from a React website, consider using the [API Connection](/nx-plugin-for-aws/guides/api-connection/react-trpc) generator to configure the client.
