...
This example walks through identity middleware for `IAM` authentication, where we look up the caller in Cognito using the sub extracted from the API Gateway event. For `Cognito` authentication, identity middleware is more straightforward — extract claims directly from the `event` instead.
