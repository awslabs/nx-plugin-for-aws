...
The tRPC generator supports two infrastructure patterns via the `integrationPattern` option:

- `isolated` is the default. It generates one Lambda function per procedure.
- `shared` generates a single default `$router` Lambda integration for the whole API and reuses it for every procedure unless you provide per-procedure overrides.

`isolated` gives you more granular infrastructure and makes it easier to attach different permissions or configuration per operation. `shared` reduces Lambda and integration sprawl, while still allowing you to override specific procedures with dedicated integrations when needed.

Both integration patterns are currently supported for `ServerlessApiGatewayRestApi` and `ServerlessApiGatewayHttpApi`.
