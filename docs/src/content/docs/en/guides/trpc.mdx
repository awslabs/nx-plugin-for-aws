...
:::tip[API Type]
Select `ServerlessApiGatewayRestApi` (default) as your `computeType` if you would like to use [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) to stream responses. Select `BedrockAgentCoreRuntimeWebSocket` to deploy the tRPC server as a container on Bedrock AgentCore with streaming WebSocket subscriptions — see [Deploying to Bedrock AgentCore](#deploying-to-bedrock-agentcore) below.
:::

:::tip[Integration Pattern]
The `integrationPattern` option defaults to `isolated`, which creates one Lambda per tRPC procedure. Select `shared` if you would prefer a single shared Lambda handler for the whole API, with optional per-procedure overrides. The `integrationPattern` option does not apply when `computeType` is `BedrockAgentCoreRuntimeWebSocket`.
:::
...
Subscriptions are only supported when `computeType` is `ServerlessApiGatewayRestApi` (default), since API Gateway HTTP APIs do not support response streaming.
:::
...
Setting `computeType` to `BedrockAgentCoreRuntimeWebSocket` deploys the tRPC server as a containerised WebSocket server on [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) instead of API Gateway + Lambda. AgentCore gives you:

- **Streaming subscriptions over WebSocket** — full duplex messaging for tRPC subscriptions with no 30-second Lambda or API Gateway timeouts.
- **Session affinity** — the `x-amzn-bedrock-agentcore-runtime-session-id` header is propagated into your tRPC context as `ctx.sessionId`, so you can key per-session state.
- **Managed runtime** — AgentCore handles scaling, health checks (`/ping`), and authorization.

:::caution[Auth options]
AgentCore only supports `IAM` or `Cognito` auth. Selecting `auth=None` with `computeType=BedrockAgentCoreRuntimeWebSocket` will fail generation.
:::

### How it works

When you choose `BedrockAgentCoreRuntimeWebSocket`, the generator emits:

- `src/handler-agentcore.ts` — a Node WebSocket + HTTP server that exposes `/ping` for AgentCore health checks and `/ws` for tRPC subscriptions via `@trpc/server/adapters/ws`.
- `Dockerfile` — containerises the bundled server and includes the [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) for managed observability.
- A CDK construct (or Terraform module) that provisions a Bedrock AgentCore `Runtime`, grants it permission to invoke Bedrock models and read runtime configuration, and publishes the runtime ARN to `runtimeConfig.apis.<YourApiName>` for the frontend to pick up.
- A `docker` build target wired into the project's `build` target so `nx build` produces the container image.

### Accessing the session id

Every procedure receives `ctx.sessionId` when the API is deployed to AgentCore:
