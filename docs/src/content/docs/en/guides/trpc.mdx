...
:::tip
The `integrationStyle` option defaults to `Individual Functions`, which creates one Lambda per tRPC procedure. Select `Router` if you would prefer a single shared Lambda handler for the whole API.
:::
...
The tRPC generator supports two infrastructure patterns via the `integrationStyle` option:

- `Individual Functions` is the default. It generates one Lambda function per procedure.
- `Router` generates a single shared Lambda function for the whole API and reuses it for every integration.

`Individual Functions` gives you more granular infrastructure and makes it easier to attach different permissions or configuration per operation. `Router` reduces Lambda and integration sprawl, which can be useful for smaller APIs or when you prefer a single central handler.

Both integration styles are currently supported for `ServerlessApiGatewayRestApi` and `ServerlessApiGatewayHttpApi`.
