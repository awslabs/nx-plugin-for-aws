...
The tRPC API generator creates a new tRPC API with AWS CDK or Terraform infrastructure setup. The generated backend can be deployed as AWS Lambda functions exposed via an AWS API Gateway API, or as a containerised WebSocket server running on [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html). It includes schema validation using [Zod](https://zod.dev/) and sets up [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) for observability, including logging, AWS X-Ray tracing and CloudWatch Metrics.
...
:::tip[When to use each `computeType`]
- **`ServerlessApiGatewayHttpApi`** — the most cost-effective option. Pick this when you only need queries and mutations and don't need [WAF](https://aws.amazon.com/waf/) at the API tier. [HTTP APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html) are cheaper and lower-latency than REST APIs, but can't stream responses (so no [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions)).
- **`ServerlessApiGatewayRestApi`** (default) — pick this when you need [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) over [Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events) and/or a [WAF Web ACL](https://aws.amazon.com/waf/) on the API. [REST APIs](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-rest-api.html) cost more than HTTP APIs but support response streaming and WAF integration.
- **`BedrockAgentCoreRuntimeWebSocket`** — pick this when you need [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) over a WebSocket for real-time streaming with no 30-second Lambda or API Gateway timeout. [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) hosts your tRPC server as a containerised WebSocket server with managed auth, session affinity, and scaling.

For a deeper comparison of REST vs HTTP API Gateway, refer to the [API Gateway Developer Guide](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html).
:::
...
<FileTree>
  - src
    ...
    - handler.ts Lambda handler entrypoint (Lambda compute types only)
    - handler-agentcore.ts WebSocket server entrypoint (AgentCore compute type only)
    - router.ts tRPC router definition
    - schema Schema definitions using Zod
      - echo.ts Example definitions for the input and output of the "echo" procedure
      - z-async-iterable.ts Zod helper for subscriptions (REST API and AgentCore only)
  ...
  - Dockerfile Container image for AgentCore deployments (AgentCore compute type only)
...
tRPC subscriptions stream data from the server to the client. The generator automatically configures the `ZodAsyncIterable` schema helper and the handler for streaming responses.

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Subscriptions use [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). The generated infrastructure uses a streaming Lambda handler with `ResponseTransferMode.STREAM` in API Gateway for all REST API operations, which enables subscriptions to work alongside regular queries and mutations.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Subscriptions use [WebSockets](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) against the AgentCore runtime endpoint. The generated handler exposes `/ws` via `@trpc/server/adapters/ws`, which carries queries, mutations, and subscriptions over a single full-duplex connection with no 30-second Lambda or API Gateway timeout.
</TabItem>
</Tabs>
...
When deployed to AgentCore, every procedure receives `ctx.sessionId` — the value of the `x-amzn-bedrock-agentcore-runtime-session-id` header AgentCore sets on the WebSocket upgrade. Use it to key per-session state:
...
export const chat = publicProcedure
  ...
  .subscription(async function* (opts) {
    ...
    // ...stream chunks that build on `history`
  });
```

Locally, `nx serve` runs a standalone HTTP + WebSocket server so clients can connect the same way they do in production. The session id defaults to a random UUID when the header is not present.
</OptionFilter>
...
On AgentCore, the runtime has already enforced `IAM` or `Cognito` auth on the WebSocket upgrade before your handler sees any traffic. The generated context exposes:

- `ctx.sessionId` — the value of the `x-amzn-bedrock-agentcore-runtime-session-id` header set by AgentCore on the WebSocket upgrade.

You can read `ctx.sessionId` directly in any procedure — no extra middleware is required:
...
If you want to expose additional fields (e.g. custom JWT claims forwarded via headers) to procedures, add middleware that augments `ctx` in the same pattern as the Lambda examples above — the only difference is that the source of truth is the WebSocket upgrade headers, not an API Gateway event.
</OptionFilter>
...
You may see Checkov check failures such as:
...
The generator emits a CDK construct (or Terraform module) that provisions a Bedrock AgentCore `Runtime`, grants it permission to invoke Bedrock models and read runtime configuration, and publishes the runtime ARN to `runtimeConfig.apis.<YourApiName>` for the frontend to pick up. A `docker` build target is wired into the project's `build` target so `nx build` produces the container image.
...
The construct provisions the AgentCore runtime from the container image built by the `docker` target and surfaces the runtime ARN via `api.agentCoreRuntime.agentRuntimeArn`.
</Fragment>
<Fragment slot="terraform">
```hcl
module "my_api" {
...
The module provisions the AgentCore runtime from the container image built by the `docker` target and publishes the runtime ARN via the `agent_runtime_arn` output.
</Fragment>
</Infrastructure>
</OptionFilter>
