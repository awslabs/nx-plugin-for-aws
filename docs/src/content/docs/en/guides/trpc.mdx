...
<OptionFilter when={{ auth: 'IAM' }} description="CDK usage for IAM authentication">
```ts {6-8}
...
<OptionFilter when={{ auth: 'Custom' }} description="CDK usage with Custom Lambda Authorizer">
```ts {6-9}
import { MyApi } from ':my-scope/common-constructs';
...
export class ExampleStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    const api = new MyApi(this, 'MyApi', {
      integrations: MyApi.defaultIntegrations(this).build(),
      authorizer: MyApi.defaultAuthorizer(this),
    });
  }
}
```

The `defaultAuthorizer` creates a Lambda function from the generated `src/authorizer.ts` file, which denies all requests by default. Implement your custom authorization logic in that file.
</OptionFilter>
...
:::tip[Custom Lambda Authorizer]
When using `Custom` auth, your API is protected by a Lambda Authorizer that denies all requests by default. Implement your authorization logic in the generated `src/authorizer.ts` file. The authorizer receives the request and must return an IAM policy document allowing or denying access.
:::
</OptionFilter>
</Fragment>
</Infrastructure>
