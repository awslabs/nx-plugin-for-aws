...
:::caution[Claims location differs by API type]
The example below is for a **REST API** (`infra: 'rest-lambda'`), where the claims are at `event.requestContext.authorizer.claims` and the event type is `APIGatewayProxyEvent`.

For an **HTTP API** (`infra: 'http-lambda'`), the claims are nested one level deeper at `event.requestContext.authorizer.jwt.claims`, and the event type is `APIGatewayProxyEventV2WithJWTAuthorizer`. Adjust both the import and the claims path accordingly.
:::
...
With `auth: 'cognito'`, the client config additionally requires a `token` — the caller's Cognito JWT, which is sent in the `Authorization` header:
