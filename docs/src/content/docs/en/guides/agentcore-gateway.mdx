...
Pass the role ARN in as a template variable (see <a href="#template-variables">Template variables</a> below) rather than hard-coding or pattern-matching it. Re-synth or re-plan to deploy the new policy.
...
Add new variables where the policies are rendered, for example to pass an agent's execution role ARN to the `ts-agent-divide.cedar` example above:

<Infrastructure>
<Fragment slot="cdk">
In `packages/common/constructs/src/app/gateways/<name>/<name>.ts`, add to the object passed to `ejs.render`:
...
The principal's `id` attribute holds the caller's IAM ARN. Callers typically arrive as STS assumed-role ARNs with a generated session suffix, so pass the exact role ARN in as a template variable and match the session suffix with `like`:
