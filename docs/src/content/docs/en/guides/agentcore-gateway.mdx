...
<FileTree>
  - packages/\<name>/
    - policies/ Cedar policy source files (omitted when `cedarPolicy: false`)
      - permit-all.cedar Default Cedar policy that permits authenticated callers (IAM callers from the same AWS account, or any Cognito user — see [Authentication](#authentication))
