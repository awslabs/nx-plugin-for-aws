...
- A `license-check` target is registered on the workspace root project via a `createNodesV2` plugin.
- Run it with `nx run-many -t license-check` or `nx license-check`.
- Results are cached against lockfiles and the workspace config — re-runs are instant when nothing has changed.
- Collectors determine what gets scanned. The built-in `npmCollector` scans `node_modules` via `license-checker-rseidelsohn`. The `pythonCollector` reads `*.dist-info/METADATA` files from virtual environments.
- If no installed dependencies are found, the check passes with nothing to inspect.
...
The default config uses `npmCollector()` which automatically detects your package manager from the lockfile for dependency chain tracing. Add `pythonCollector()` if you have Python projects:
...
const myCollector = (): LicenseCollector => ({
  ...
  async collect({ workspaceRoot }) {
    // Scan workspaceRoot for dependencies and return their license info
    return [
...
The `traceCommand` is shown in violation reports to help users investigate dependency chains. Each collector is responsible for its own discovery strategy (e.g. walking `node_modules`, reading lockfiles, or inspecting virtual environments).
...
```bash
LICENSE_DEPENDENCY_CHECK=skip pnpm nx run-many -t lint
```

To disable permanently, remove the `dependencyCheck` block from your `aws-nx-plugin.config.mts` and remove the `license-check` entry from `targetDefaults.lint.dependsOn` in `nx.json`. Or re-run the generator with `--dependencyCheck=false`.
