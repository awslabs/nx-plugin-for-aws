...
This means you don't need to run the check explicitly, though you can still do so with the NxCommands `license-check` target:
...
To disable permanently, remove the `license.dependencies` key from your configuration in `aws-nx-plugin.config.mts`. You can also re-run the `license` generator with `--dependencyCheck=false` to scaffold without it — the generator flag is still named `--dependencyCheck` and controls whether the generator writes the `dependencies` block.
