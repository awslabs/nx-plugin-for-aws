...
You selected `Any` for your `eventSource`, which means the `parser` middleware is not wired in and the `event` parameter is typed as `any`. If you later want compile-time type safety and runtime validation, regenerate the function with a specific `eventSource`.
</OptionFilter>
