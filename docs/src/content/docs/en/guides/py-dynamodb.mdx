...
- **Connection** — `BaseModel.Meta` sets `host`, `region`, `aws_access_key_id`, and `aws_secret_access_key` from `config.json` when `is_local()` is `True`, pointing PynamoDB at the local DynamoDB instance. In AWS, these are left unset so PynamoDB uses the default credential chain.
