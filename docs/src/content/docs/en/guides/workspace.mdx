...
From a Node perspective this means a single lockfile at the root with a single `node_modules` containing all dependencies. Each Node project declares the runtime dependencies its source imports in its own `package.json`, while shared build/test tooling lives in the root `package.json` `devDependencies`. Add a project's runtime dependency with your package manager's filter/workspace flag (eg `pnpm add <pkg> --filter <project>`), and shared dev tooling at the root.
