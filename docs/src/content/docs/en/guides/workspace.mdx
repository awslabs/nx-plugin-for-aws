...
This will additionally run any targets that it depends on.
...
<Steps>
1. When `git commit` runs, the [husky](https://typicode.github.io/husky/) pre-commit hook invokes git-secrets.

2. On the first commit, git-secrets registers AWS credential patterns in your local git config.

3. Staged files are scanned against the patterns. If a match is found, the commit is blocked.
</Steps>
...
You can also create a `.gitallowed` file at the repository root with one regex per line (shared with your team via version control):
...
<FileTree>
  ...
  - nx.json Nx configuration (targets, sync generators, caching)
