...
Nx's own default is 3, which leaves cores idle in these workspaces — each project contributes several targets (`lint`, `compile`, `test`, and often `bundle` and `docker`), producing a wide task graph with plenty of independent work. Raising this beyond 8 tends not to help, since the graph's critical path becomes the limit rather than available cores.
