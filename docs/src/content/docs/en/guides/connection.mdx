---
title: Connection
description: Reference documentation for connections
---
import Astro from '@astrojs/react';
import { LinkCard, CardGrid, LinkButton } from '@astrojs/starlight/components';
import Link from '@components/link.astro';

This generator is used to connect projects together, such as websites calling APIs. Simply select the source project (for example the project that will call your API) and target project (for example your API project), and this generator will handle integrating the two.

### Supported Connections

The Connection generator supports the following connections:

<CardGrid>
  <LinkCard
    title="React to tRPC"
    description="Call a tRPC API from a React website"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/react-trpc`}
  />
  <LinkCard
    title="React to FastAPI"
    description="Call a Python FastAPI from a React website"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/react-fastapi`}
  />
  <LinkCard
    title="React to Smithy API"
    description="Call a Smithy API from a React website"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/react-smithy`}
  />
  <LinkCard
    title="React to Python Strands Agent"
    description="Call a Python Strands Agent from a React website"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/react-py-strands-agent`}
  />
  <LinkCard
    title="React to TypeScript Strands Agent"
    description="Call a TypeScript Strands Agent from a React website"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/react-ts-strands-agent`}
  />
  <LinkCard
    title="Python Strands Agent to MCP"
    description="Connect a Python Strands Agent to an MCP server"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/py-strands-agent-mcp`}
  />
  <LinkCard
    title="TypeScript Strands Agent to MCP"
    description="Connect a TypeScript Strands Agent to an MCP server"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/ts-strands-agent-mcp`}
  />
  <LinkCard
    title="Python Strands Agent to A2A Agent"
    description="Connect a Python Strands Agent to a remote A2A agent"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/py-strands-agent-a2a`}
  />
  <LinkCard
    title="TypeScript Strands Agent to A2A Agent"
    description="Connect a TypeScript Strands Agent to a remote A2A agent"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'en'}/guides/connection/ts-strands-agent-a2a`}
  />
</CardGrid>

:::note[Runtime Configuration]
The connection generator makes use of <Link path="guides/runtime-config">Runtime Configuration</Link> to pass deploy-time values (such as API URLs, Cognito settings, and agent runtime ARNs) between generated projects and components at runtime so they can discover and connect to one another.

For agent runtime ARNs, the connection generator also patches the generated agent construct to publish the ARN to the website's `runtime-config.json`. Unconnected agents register their ARN under the `agentcore` namespace only (server-side discovery via AppConfig) and are not exposed to any frontend.
:::
