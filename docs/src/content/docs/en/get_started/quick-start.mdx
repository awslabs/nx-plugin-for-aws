...
export class ApplicationStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const { identityPool } = new UserIdentity(this, 'identity');
    const integrations = DemoApi.defaultIntegrations(this).build();
    const api = new DemoApi(this, 'api', { integrations });
    const { cloudFrontDistribution } = new DemoWebsite(this, 'website');

    // Update API lambda environment variables with website domain name for CORS
    // NOTE: If you add a custom domain to the website, update this to use that domain instead
    const allowedOrigin = `https://${cloudFrontDistribution.domainName}`;
    Object.values(integrations).forEach(({ handler }) =>
      handler.addEnvironment('ALLOWED_ORIGIN', allowedOrigin),
    );

    api.grantInvokeAccess(identityPool.authenticatedRole);
  }
}
```
...
module "demo_api" {
  ...
  # Update API lambda environment variable with website domain name for CORS
  # NOTE: If you add a custom domain to the website, update this to use that domain instead
  # env = {
  #   ALLOWED_ORIGIN = "https://CLOUDFRONT_DOMAIN_NAME"
  # }
}
...
Uncomment the following within `packages/infra/src/main.tf` and replace `CLOUDFRONT_DOMAIN_NAME` with the CloudFront domain name of the deployed project. 
