...
The resources you bootstrapped are shared by every project deploying to that AWS account and region, and cost next to nothing to leave in place. Remove them only if nothing else deploys there.
...
The asset bucket is retained on purpose, so delete it separately. It is versioned, which `aws s3 rb --force` cannot handle, so remove every version and delete marker first:
...
Each `delete-objects` reports an error rather than doing nothing when there is nothing of that kind left to remove, which is safe to ignore.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Your Terraform State Is Deleted Too]
This empties and deletes the S3 bucket holding your Terraform state, so run it only once nothing you care about is still deployed from that state.
:::
</Fragment>
</Infrastructure>
