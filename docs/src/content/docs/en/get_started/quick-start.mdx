...
Your website is also served from CloudFront. Open the distribution domain name printed in your deployment outputs — the output ending in `DistributionDomainName` for CDK, or `website_url` for Terraform — to use the version running entirely on AWS.
...
The Cognito user pool is created with deletion protection enabled, so it survives the commands above. Turn deletion protection off in the [Amazon Cognito console](https://console.aws.amazon.com/cognito/) before removing it.

<Infrastructure>
<Fragment slot="cdk">
:::note[Retained Resources]
Deleting the stacks leaves the user pool behind, along with the IAM role Cognito uses to send SMS messages, the IAM role API Gateway uses to write to CloudWatch Logs, the KMS keys encrypting the website bucket and the API and WAF log groups, and the log groups themselves. Delete the pool, roles and log groups, and schedule the keys for deletion, once you no longer need them.
:::
</Fragment>
<Fragment slot="terraform">
:::caution[Destroy Fails Until Deletion Protection Is Off]
