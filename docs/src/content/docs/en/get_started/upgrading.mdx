...
<Aside type="tip">
Upgrading `@aws/nx-plugin` does not upgrade Nx (or vice versa) — `nx migrate` only ever updates the package you name. When jumping across multiple Nx major versions, upgrade one major version at a time — see [Automate Updating Dependencies](https://nx.dev/docs/features/automate-updating-dependencies) for details.
</Aside>
