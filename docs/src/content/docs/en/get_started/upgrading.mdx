...
`@aws/nx-plugin` ships migrations that update your workspace automatically when you upgrade it, using [`nx migrate`](https://nx.dev/docs/features/automate-updating-dependencies). The <Link path="get_started/migrations">migrations reference</Link> lists every migration in the order they run.
