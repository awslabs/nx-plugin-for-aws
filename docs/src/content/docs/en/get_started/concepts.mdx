...
So you get both halves of the deal:

- **You're never blocked.** The plugin isn't in your runtime path, so you can change any generated file whenever you need to and ship without waiting on us.
- **You're never stranded.** Keeping up with upstream improvements is an automated path rather than a manual re-adoption exercise, so a workspace generated months ago can move forward with the plugin rather than drifting further from it.
- **Your changes are respected.** Migrations pattern-match against the shape the generator produced. Where a file has diverged beyond what they can safely update, they leave your code alone and report the manual follow-up instead of overwriting your work. Where the right edit depends on what you've built, the migration ships as a prompt your AI coding agent applies for you.

Migrations narrow the gap rather than eliminate it — how far your code has moved decides how much they can do for you, and some steps will still need your hands. Upgrades also stay opt-in: nothing in your workspace changes until you choose to run `nx migrate`.
