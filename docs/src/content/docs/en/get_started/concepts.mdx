...
[Nx](https://nx.dev) is a smart build framework for managing monorepos. It is language agnostic, letting TypeScript, Python and infrastructure projects live and build together in a single workspace. Its build system uses caching and dependency graph analysis to only rebuild what changed and run tasks in parallel, keeping testing, linting and building fast as your workspace grows.
