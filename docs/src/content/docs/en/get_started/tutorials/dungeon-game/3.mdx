...
- `main.py` adds a `session_manager_provider` that creates an `S3SessionManager` per `thread_id` — this is what makes conversation history stick across turns for the same `(player, genre)`, and puts it in the bucket the Game API's `queryActions` reads from.
...
<Tabs>
  ...
  Start your local Agent server:
    ...
    The `agent-serve` target starts the agent locally but connects to your **deployed** MCP server (via the `RUNTIME_CONFIG_APP_ID`). If you'd prefer to run the MCP server locally too, use `agent-serve-local` instead, which starts all dependent services and sets `SERVE_LOCAL=true` for direct HTTP connections.
    :::

    Then, in a second terminal, open an interactive REPL against the local AG-UI agent via the generated `agent-chat` target:

    <NxCommands commands={["run dungeon_adventure.story:agent-chat"]} />

    This uses [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) to speak the AG-UI protocol against `http://localhost:8081/invocations`. Your first message should tell the agent your hero's name and the genre (for example: `My name is Alice. Start my zombie adventure.`) and the story will stream back.

    <Aside type="tip" title="Raw curl">
    If you'd prefer a raw HTTP call, the AG-UI `RunAgentInput` shape is:
    ```bash
