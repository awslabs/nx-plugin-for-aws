...
<Tabs>
  <TabItem label="Local">
  The agent generator vends an interactive terminal chat CLI (`agent-chat`) that uses a type-safe TypeScript client generated from the agent's OpenAPI spec. Because we changed the agent's input shape to accept `playerName`, `genre`, and `actions`, we need to update the generated chat script to supply those fields.

  Update `packages/story/scripts/agent/chat.ts`:
  ...
  Then start an interactive chat with your locally-running Story agent:
    ...
    The `agent-chat` target automatically starts `agent-serve-local` (which starts the agent and any MCP server dependencies with `SERVE_LOCAL=true`) and runs the OpenAPI spec + client generation before dropping you into the chat.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat` depends on `agent-serve-local`, which starts the agent with `SERVE_LOCAL=true` and also starts any MCP server dependencies locally. If you'd prefer the agent to connect to your **deployed** MCP server instead (via `RUNTIME_CONFIG_APP_ID`), run the `agent-serve` target in a separate terminal and invoke the agent manually via `curl` against its `/invocations` endpoint.
    :::
  </TabItem>
