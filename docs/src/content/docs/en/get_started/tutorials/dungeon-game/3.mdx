...
- `main.py` adds a `session_manager_provider` that creates a `FileSessionManager` per `thread_id` — this is what makes conversation history stick across turns for the same `(player, genre)`.
...
<Tabs>
  ...
  Start your local Agent server:
    ...
    ```
    Reusing the same `threadId` on subsequent calls will continue the same story — the `FileSessionManager` replays the earlier messages.
    </Aside>
  </TabItem>
