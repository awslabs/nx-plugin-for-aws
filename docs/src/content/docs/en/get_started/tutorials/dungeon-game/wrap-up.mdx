...
3. Swap Strands' `FileSessionManager` for [Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html) to give the Story Agent a richer long-term memory that can be shared across runtimes and consoles - see the [using AgentCore Memory with Strands](https://github.com/aws/bedrock-agentcore-sdk-python/tree/main/src/bedrock_agentcore/memory/integrations/strands) docs. The `session_manager_provider` hook on `StrandsAgentConfig` is the same extension point the generator already uses for file-based persistence.
