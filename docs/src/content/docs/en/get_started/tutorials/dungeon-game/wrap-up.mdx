...
:::caution[CloudFront WAF stack in us-east-1]
The Game UI's CloudFront WAF WebACL is deployed to `us-east-1` (a CloudFront requirement), in a separate stack such as `dungeon-adventure-infra-sandbox-ApplicationGameUIwaf...`. If it isn't removed by the command above, delete it from `us-east-1` to avoid leaving billable resources behind:
