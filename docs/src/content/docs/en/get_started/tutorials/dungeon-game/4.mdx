...
`game-ui:serve-local` has `dependsOn` on `game-api:serve-local` and `dungeon_adventure.story:agent-serve-local`, so Nx spins up every project's local server in parallel. Open the dev server in a browser, log in, and follow the prompts to create a new user. You should see the baseline Shadcn website:
...
<Aside type="tip" title="Pointing at the deployed backend instead">
If you'd prefer your local dev server to talk to the already-deployed APIs + agent, pull the deployed runtime config once and then use the plain `serve` target:

<NxCommands commands={["run game-ui:load:runtime-config"]} />
<NxCommands commands={["serve game-ui"]} />

This is handy when you're only iterating on UI code and don't want to re-boot the backend.
</Aside>
