...
`game-ui:serve-local` has `dependsOn` on `game-api:serve-local` and `dungeon_adventure.story:agent-serve-local`, so Nx spins up every project's local server in parallel. Open the dev server in a browser and follow the prompts to create a new user.
