...
`dev` is an alias for `game-ui:serve-local`, which has `dependsOn` on `game-api:serve-local` and `dungeon_adventure.story:agent-serve-local`, so Nx spins up every project's local server in parallel. Through the connections we set up in Module 1, those in turn boot the Inventory MCP server and DynamoDB Local. Make sure your container engine is running, then open the dev server in a browser.
