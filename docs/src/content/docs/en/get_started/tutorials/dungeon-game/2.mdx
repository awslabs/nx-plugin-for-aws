...
2. `queryGames` - return a paginated list of the signed-in player's saved games.
...
:::tip[Everything is scoped to the signed-in player]
Both stores are keyed on the caller, so one player can never see another's adventures:

- **Games** are partitioned in DynamoDB on the user's Cognito `sub`, so `queryGames` queries that partition instead of scanning the table. Two players can also pick the same hero name without colliding.
- **Conversations** are namespaced by the agent, so keys look like `session_u-<sub>-<threadId>/…`.
- **Inventory** is keyed on the calling agent's session plus the player name. The Inventory MCP server reads that session from the request context rather than trusting the `playerName` the model passes in — a model argument is not a trust boundary.

Neither the `userId` nor the namespace is taken from the request body — the website could lie about it. `requireUserId` in `identity.ts` reads the caller from the IAM authorizer context that the Cognito Identity Pool populates (`requestContext.authorizer.iam.cognitoIdentity.amr`, which contains a `…:CognitoSignIn:<sub>` entry). That value is produced by the Identity Pool token exchange, so it is trustworthy rather than supplied by the browser, and requests without it are rejected. See <Link path="guides/trpc">Implementing Custom Middleware</Link> for the reusable version of this as a tRPC plugin.
:::
...
Every procedure scopes its data to the signed-in player, so they all need the caller's identity. Add `packages/game-api/src/identity.ts`:
