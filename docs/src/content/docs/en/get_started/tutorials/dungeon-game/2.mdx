...
:::note[No chat history procedure]
You might have expected a procedure to save each user/assistant turn of the chat. We don't need one: the AG-UI agent is hosted on Bedrock AgentCore Runtime, which persists the conversation history per session automatically as long as the same thread id is used.
:::
...
<Tabs>
  <TabItem label="Local">
    ...
    Use the CDK deploy output value of `dungeon-adventure-infra-sandbox-Application.RuntimeConfigApplicationId` to replace the highlighted placeholder.
    </Aside>
