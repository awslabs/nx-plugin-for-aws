...
:::note[Conversation history lives with the agent]
We do not persist chat messages through the Game API. In <Link path="get_started/tutorials/dungeon-game/3">Module 3</Link> the Story Agent plugs in a `FileSessionManager` that writes each thread's history to disk, keyed by the thread id the UI sends on every turn — so each `(player, genre)` pair resumes its own story without the UI having to replay messages.
:::
