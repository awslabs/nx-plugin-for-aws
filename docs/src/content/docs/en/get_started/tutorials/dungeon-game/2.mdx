...
The `@aws-sdk/client-s3` dependency is already in the workspace, so there's nothing new to install. The bucket name comes from the same Runtime Configuration mechanism used for `tables` — we register it under a new `buckets` namespace in the infrastructure stack below.
  </TabItem>
</Tabs>
