...
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — a hidden generator which composes all the others in-process on a single tree.

Both are worth keeping. The CLI matrix tests the path users actually take. The composed matrix ships with the plugin, so each released version carries the matrix of the generators _it_ had — which lets tests that upgrade an older workspace scaffold it with that version's own matrix. Because it calls generators directly rather than through the CLI, its options are type-checked against each generator's schema, so a new required option or a changed enum fails the build there rather than silently reducing coverage.
