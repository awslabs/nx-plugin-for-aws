...
We have a suite of "smoke tests" which run generators in a fresh workspace and make sure that everything builds. At a minimum, your new generator should be added to the generator matrix in `packages/nx-plugin/src/internal/test-matrix/generator.ts` so that it's exercised by the smoke tests.

The matrix is itself a (hidden) generator which composes all the others, so it ships with the plugin. That means each released version carries the matrix of the generators _it_ had, which lets tests that upgrade an older workspace scaffold it with that version's own matrix. Since it calls generators directly rather than through the CLI, options are type-checked against each generator's schema — so a new required option or a changed enum will fail the build here rather than silently reducing coverage.
