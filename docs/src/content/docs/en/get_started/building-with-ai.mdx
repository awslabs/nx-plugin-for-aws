...
The Nx Plugin for AWS is available as a [Claude Code plugin](https://code.claude.com/docs/en/plugins) that bundles skills and the MCP server. This is the recommended way to get started with Claude Code.
...
```
/plugins marketplace add awslabs/nx-plugin-for-aws
```

Once installed, Claude Code automatically has access to the `nx-plugin-for-aws` MCP server and its skills. No additional configuration is needed.
