...
Generator support currently varies by backend:

- `trpc#backend` supports both `isolated` and `shared` for `ServerlessApiGatewayRestApi` and `ServerlessApiGatewayHttpApi` via the `integrationPattern` option.
- `py#fast-api` and `smithy#ts-api` currently generate `isolated`.

You can also switch an existing generated CDK API construct between the two patterns by editing the `pattern` field in its `defaultIntegrations` method:

```ts {3}
return IntegrationBuilder.rest({
  // Change this to 'isolated' or 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Terraform modules already use the router pattern by default, with a single Lambda function handling all API operations.
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
If your generator does not expose the router pattern directly, or you want to customise it further, you can edit the `defaultIntegrations` method for your API to create a single function instead of one per integration:
