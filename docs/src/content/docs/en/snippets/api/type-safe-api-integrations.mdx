...
- `ts#trpc-api` supports both `isolated` and `shared` for `ServerlessApiGatewayRestApi` and `ServerlessApiGatewayHttpApi` via the `integrationPattern` option.
- `py#fast-api` and `ts#smithy-api` currently generate `isolated`.
...
<Infrastructure>
<Fragment slot="cdk">
If you need to customise the router pattern beyond the generated defaults, you can freely edit the `defaultIntegrations` method for your API to wire integrations however you prefer:
