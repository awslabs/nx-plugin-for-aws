...
The `SizeRestrictions_BODY` rule from `AWSManagedRulesCommonRuleSet` is set to `Count` rather than `Block`, since the rule's 8 KB limit is often stricter than what the API needs. Oversized requests will still be recorded as metrics so you can monitor them. See the [AWS WAF body size limits](https://docs.aws.amazon.com/waf/latest/developerguide/waf-rule-statement-oversize-handling.html) guide for more details.
