...
Pass the shared <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> module's outputs into the MCP server module. Under the default `agentcore` packaging the server's code is staged in the shared asset bucket, so pass that too — instantiate the `core/asset-bucket` module once per deployment, as the Lambda and API modules already do:
