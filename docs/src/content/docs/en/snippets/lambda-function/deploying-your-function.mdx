...
The function module stages its deployment zip in a shared S3 asset bucket rather than uploading inline via the Lambda API (which has a 70 MB request limit). Instantiate the `core/asset-bucket` module once per deployment and pass its `bucket_name` output into every lambda / API module via the `asset_bucket_name` input:
...
The `core/asset-bucket` module provisions an S3 bucket with S3-managed (AES256) encryption, versioning, a TLS-only bucket policy, public-access block, and S3 access logging enabled (the logs are delivered to a sibling access-logs bucket). Reuse the same module instance across every Lambda / API module in your deployment — each caller writes under a module-scoped key prefix so there are no collisions.
