...
Strands Agents read their runtime configuration from a shared AWS AppConfig application — see the <Link path="/guides/terraform-project#shared-runtime-config-appconfig">Terraform infrastructure guide</Link> for details. Instantiate the `core/runtime-config/appconfig` module once per deployment and thread its outputs into every agent / MCP module:
...
Declare `module "runtime_config_appconfig_deployment"` once at the end of your root module (with `depends_on` covering every module that contributes a runtime-config entry) — it aggregates the contributed entries and creates the AppConfig hosted configuration versions + deployments. See the <Link path="/guides/terraform-project#shared-runtime-config-appconfig">full pattern</Link>.
</Fragment>
</Infrastructure>
