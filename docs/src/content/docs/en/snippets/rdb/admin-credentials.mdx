...
:::caution[Rotating manually]
Rotating the admin password outside of Aurora's schedule is done through the RDS API (`modify-db-cluster --rotate-master-user-password --apply-immediately`), not through Secrets Manager. Attaching your own rotation function to an RDS-managed secret is rejected.
:::
