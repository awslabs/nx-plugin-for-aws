...
The admin credentials are generated and rotated by Aurora itself in Secrets Manager, so the password is never handled by your infrastructure code — it does not appear in a CloudFormation template or in Terraform state. Your application does not use these credentials: it connects as a least-privilege database user via IAM authentication, and only the migration and create-db-user handlers read the secret.
