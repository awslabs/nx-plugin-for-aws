...
A `trivy` target is added to your project which scans the built image and **fails the build** if any fixable `HIGH` or `CRITICAL` severity vulnerability is found. Vulnerabilities without an available fix are ignored, since they cannot be actioned by upgrading tooling in your `Dockerfile`.
