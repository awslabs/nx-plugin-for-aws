...
:::tip
We're using the `dev` target here (an alias for `serve-local`), which also starts local servers for any APIs which have been connected with `connection`, and hot-reloads if your website, model, or backend changes! This allows us to test our API and website locally before we've even written any CDK code.
:::
