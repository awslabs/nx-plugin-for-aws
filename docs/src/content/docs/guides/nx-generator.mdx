...
The `schema.json` file defines the options that your generator accepts. It follows the [JSON Schema](https://json-schema.org/) format with Nx-specific extensions.
...
Options can have priorities of `"important"` or `"internal"`. This helps Nx to order properties in the Nx VSCode extension.
...
A generator is a function which mutates a virtual filesystem (the `Tree`), reading and writing files to make the desired changes.
...
:::tip
For a more in-depth guide regarding contributing to the Nx Plugin for AWS, please refer to the [tutorial here](/nx-plugin-for-aws/get_started/tutorials/contribute-generator).
:::
