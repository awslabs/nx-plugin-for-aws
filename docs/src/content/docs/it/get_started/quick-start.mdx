...
:::note[Output del Modulo Root]
Terraform stampa solo gli output dichiarati in `packages/infra/src`, quindi qualsiasi cosa tu voglia leggere dopo una distribuzione deve essere ri-esportata dal modulo che la produce — da qui i due blocchi `output` sopra.
:::
</Fragment>
</Infrastructure>
...
Il tuo sito web è anche servito da CloudFront. Apri il nome di dominio della distribuzione stampato negli output della distribuzione — l'output che termina con `DistributionDomainName` per CDK, o `website_url` per Terraform — per utilizzare la versione in esecuzione interamente su AWS.
...
Il pool di utenti Cognito viene creato con la protezione dall'eliminazione abilitata, quindi sopravvive ai comandi sopra. Disattiva la protezione dall'eliminazione nella [console Amazon Cognito](https://console.aws.amazon.com/cognito/) prima di rimuoverlo.

<Infrastructure>
<Fragment slot="cdk">
:::note[Risorse Conservate]
L'eliminazione degli stack lascia il pool di utenti, insieme al ruolo IAM che Cognito utilizza per inviare messaggi SMS, al ruolo IAM che API Gateway utilizza per scrivere su CloudWatch Logs, alle chiavi KMS che crittografano il bucket del sito web e i gruppi di log dell'API e WAF, e i gruppi di log stessi. Elimina il pool, i ruoli e i gruppi di log, e pianifica l'eliminazione delle chiavi, una volta che non ne hai più bisogno.
:::
</Fragment>
<Fragment slot="terraform">
:::caution[Destroy Fallisce Finché la Protezione dall'Eliminazione È Attiva]
Terraform tenta di eliminare il pool di utenti, quindi `destroy` si ferma con `The user pool cannot be deleted because deletion protection is activated` e lascia in posizione le risorse che non aveva ancora raggiunto. Disattiva la protezione dall'eliminazione, quindi esegui nuovamente il comando destroy per rimuovere tutto — inclusi il pool di utenti e il ruolo IAM che Cognito utilizza per inviare messaggi SMS.
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">Aggiungere a un Progetto Esistente</Link> — adotta il plugin in un workspace Nx o monorepo che hai già.
