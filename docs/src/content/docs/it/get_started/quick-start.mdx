...
### Rimuovere le Risorse di Bootstrap

Le risorse di cui hai eseguito il bootstrap sono condivise da ogni progetto che distribuisce su quell'account AWS e regione, e costano quasi nulla da lasciare in posizione. Rimuovile solo se nient'altro distribuisce lì.
...
Il bucket degli asset viene conservato di proposito, quindi eliminalo separatamente. È versionato, cosa che `aws s3 rb --force` non può gestire, quindi rimuovi prima ogni versione e marcatore di eliminazione:
...
Ogni `delete-objects` riporta un errore invece di non fare nulla quando non c'è più nulla di quel tipo da rimuovere, il che è sicuro da ignorare.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Anche il Tuo Stato Terraform Viene Eliminato]
Questo svuota ed elimina il bucket S3 che contiene il tuo stato Terraform, quindi eseguilo solo una volta che nulla a cui tieni è ancora distribuito da quello stato.
:::
</Fragment>
</Infrastructure>
