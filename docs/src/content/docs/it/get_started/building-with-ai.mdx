...
Utilizza il MCP Server con l'assistente AI di tua scelta come Kiro, Kiro CLI, Cline, Claude Code o Cursor.

Per altri assistenti AI, puoi configurare il MCP server manualmente. La maggior parte degli assistenti AI utilizza un file JSON per la configurazione del MCP Server. In Kiro, questo si trova in `~/.kiro/settings/mcp.json`. Aggiungi una voce per il MCP Server del Plugin Nx per AWS:
