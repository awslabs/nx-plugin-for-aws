...
:::note
Per semplicità in questo tutorial implementiamo l'Agente in modo "stateless" dove ogni invocazione è una sessione indipendente che include l'intera cronologia dei messaggi. Per ambienti di produzione, considera l'integrazione con [Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html). Ulteriore documentazione sull'uso di AgentCore Memory con Strands è [disponibile qui](https://github.com/aws/bedrock-agentcore-sdk-python/tree/main/src/bedrock_agentcore/memory/integrations/strands).
:::
...
- Costruzione di un client che l'Agente può usare per invocare il nostro server MCP con autenticazione SigV4
...
<Tabs>
  ...
  <TabItem label="Distribuito">
    ...
    # Imposta password permanente (usa qualcosa di più sicuro!)
    ...
    # Autentica utente e cattura token
    ...
    :::caution
    Usa l'output `UserIdentityUserIdentityUserPoolIdXXX` per `POOL_ID` e assicurati di avere il corrispondente Client ID dagli output del deployment CDK.
    :::
...
Se il comando ha successo, dovresti vedere eventi in streaming simili a:
...
Complimenti. Hai costruito e deployato il tuo primo Strands Agent su Bedrock AgentCore Runtime!  🎉🎉🎉