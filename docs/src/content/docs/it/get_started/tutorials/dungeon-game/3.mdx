...
<Tabs>
  <TabItem label="Local">
  Avvia il tuo server Agent locale eseguendo il seguente comando:
    ...
    Il target `agent-serve` avvia l'agente localmente ma si connette al tuo server MCP **deployato** (tramite `RUNTIME_CONFIG_APP_ID`). Se preferisci eseguire anche il server MCP localmente, usa invece `agent-serve-local`, che avvia tutti i servizi dipendenti e imposta `SERVE_LOCAL=true` per connessioni HTTP dirette.
    :::

    Una volta che il server Agent è attivo e in esecuzione (non vedrai alcun output), chiamalo eseguendo il seguente comando:
...
Se il comando viene eseguito con successo, dovresti iniziare a vedere il testo iniziale della storia trasmesso in streaming come JSON Lines:
