...
<Tabs>
  <TabItem label="Local">
  Il generatore di agenti fornisce una CLI di chat interattiva da terminale (`agent-chat`) che utilizza un client TypeScript type-safe generato dalla specifica OpenAPI dell'agente. Poiché abbiamo modificato la forma di input dell'agente per accettare `playerName`, `genre` e `actions`, dobbiamo aggiornare lo script di chat generato per fornire questi campi.

  Aggiorna `packages/story/scripts/agent/chat.ts`:
  ...
  Quindi avvia una chat interattiva con il tuo agente Story in esecuzione locale:
    ...
    Il target `agent-chat` avvia automaticamente `agent-serve-local` (che avvia l'agente e tutte le dipendenze del server MCP con `SERVE_LOCAL=true`) ed esegue la generazione della specifica OpenAPI + client prima di portarti nella chat.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat` dipende da `agent-serve-local`, che avvia l'agente con `SERVE_LOCAL=true` e avvia anche tutte le dipendenze del server MCP localmente. Se preferisci che l'agente si connetta al tuo server MCP **deployato** invece (tramite `RUNTIME_CONFIG_APP_ID`), esegui il target `agent-serve` in un terminale separato e invoca l'agente manualmente tramite `curl` contro il suo endpoint `/invocations`.
    :::
  </TabItem>
