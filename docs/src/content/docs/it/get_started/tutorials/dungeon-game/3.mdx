...
- `main.py` aggiunge un `session_manager_provider` che crea un `FileSessionManager` per ogni `thread_id` — questo è ciò che fa persistere la cronologia delle conversazioni tra i turni per la stessa coppia `(player, genre)`.
...
<Tabs>
  ...
  Avvia il server Agent locale:
    ...
    ```
    Riutilizzare lo stesso `threadId` nelle chiamate successive continuerà la stessa storia — il `FileSessionManager` riproduce i messaggi precedenti.
    </Aside>
  </TabItem>
