...
`game-ui:serve-local` ha `dependsOn` su `game-api:serve-local` e `dungeon_adventure.story:agent-serve-local`, quindi Nx avvia il server locale di ogni progetto in parallelo. Attraverso le connessioni che abbiamo configurato nel Modulo 1, questi a loro volta avviano il server MCP Inventory e DynamoDB Local — l'**intero gioco viene eseguito sulla tua macchina senza chiamate AWS**. Assicurati che il tuo motore di container sia in esecuzione, quindi apri il server di sviluppo in un browser.
...
La tua prima distribuzione richiederà circa 8 minuti per completarsi (compila e carica le immagini Docker dell'agente e del server MCP su AgentCore). Le distribuzioni successive sono più veloci.

Una volta completata la distribuzione, vedrai output simili ai seguenti _(alcuni valori sono stati oscurati)_:
...
La chiave è la variabile d'ambiente `RUNTIME_CONFIG_APP_ID`. Quando un progetto viene eseguito **senza** `SERVE_LOCAL=true`, il `resolveTableName()` generato (e le ricerche della configurazione runtime dell'agente / API) recuperano la loro configurazione da AWS AppConfig utilizzando questo id applicazione — il valore `RuntimeConfigApplicationId` dagli output CDK.
...
- **Sito web locale → backend distribuito.** Scarica la configurazione distribuita una volta, quindi esegui il target `serve` semplice (non `serve-local`) in modo che l'UI comunichi con l'API e l'agente distribuiti:

  <NxCommands commands={["run game-ui:load:runtime-config"]} />
  <NxCommands commands={["serve game-ui"]} />

- **API locale → tabella DynamoDB reale.** Esegui il target `serve` semplice con `RUNTIME_CONFIG_APP_ID` impostato sull'id applicazione distribuito, in modo che `resolveTableName()` risolva la tabella distribuita invece di DynamoDB Local:
