...
:::tip
Puoi eseguire comandi Nx per avviare anche eventuali server API locali collegati, permettendoti di lavorare simultaneamente sulla tua API tRPC e sul sito web. <NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} env={{RUNTIME_CONFIG_APP_ID:"xxxx"}} />

Utilizza il valore di output del deploy CDK di `RuntimeConfigApplicationId` per la variabile d'ambiente `RUNTIME_CONFIG_APP_ID`.
:::
