...
<Drawer title="File aggiornati da ts#api" trigger="Clicca qui per esaminare questi file in dettaglio.">
...
## Task 5: Creare il database del gioco
...
Il generatore `ts#dynamodb` fornisce un target `serve-local` che esegue [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) in un container. Combinato con il generatore `connection` (che eseguiremo di seguito), questo significa che **l'intero gioco viene eseguito sulla tua macchina senza chiamate AWS** fino a quando non scegli di deployare alla fine del tutorial. Avrai bisogno di [un motore container](https://docs.docker.com/get-started/get-docker/) come Docker o Finch installato e in esecuzione.
:::

Vedrai alcuni nuovi file apparire nella struttura delle cartelle.
<Drawer title="File aggiornati da ts#dynamodb" trigger="Clicca qui per esaminare questi file in dettaglio.">
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - constants.ts costanti per sviluppo locale e chiave config runtime
      - client.ts singleton client DynamoDB e risoluzione nome tabella
      - gsi.ts fonte di verità per Global Secondary Indexes
      - entities/
        - example.ts entità ElectroDB di esempio (la sostituiremo)
        - index.ts export entità
    - project.json aggiunge i target `serve-local` e `pull-image`
  - common/constructs/
    - src/
      - app/dynamodb/
...
Il `src/client.ts` generato esporta `getDynamoDBClient()` e `resolveTableName()`. Quando `SERVE_LOCAL=true` (impostato automaticamente dai target `serve-local`) questi si connettono a DynamoDB Local; altrimenti si connettono ad AWS e risolvono il nome della tabella deployata dalla <Link path="guides/runtime-config">Runtime Configuration</Link>. Modelleremo le nostre entità `Game` e `Inventory` in questo progetto nel <Link path="get_started/tutorials/dungeon-game/2">Modulo 2</Link>.

Per maggiori dettagli, consulta la <Link path="guides/ts-dynamodb">guida del generatore ts#dynamodb</Link>.

</Drawer>

## Task 6: Creare l'interfaccia utente (UI)
...
Sia la Game API che il server MCP Inventory leggono e scrivono nella nostra tabella DynamoDB, quindi connettiamoli al progetto `DungeonDb`. Il generatore `connection` rileva che il target è un progetto `ts#dynamodb` e collega il target `serve-local` di ogni progetto sorgente per avviare automaticamente DynamoDB Local.
...
Poiché l'`agent-serve-local` dello Story Agent dipende già dal `mcp-server-serve-local` del server MCP Inventory (tramite la connessione `story → inventory` sopra), e sia la Game API che il server MCP ora dipendono da `dungeon-db:serve-local`, eseguire il target `serve-local` di qualsiasi progetto avvia tutte le dipendenze necessarie — incluso DynamoDB Local — nell'ordine corretto. Ci affideremo a questo per tutto il resto del tutorial.
</Aside>
...
## Task 7: Aggiornare l'infrastruttura
...
<Aside type="note" title="Ambienti non interattivi">
In una shell non interattiva (CI, ad esempio) non c'è prompt — la build invece fallisce con un errore "workspace is out of sync". Esegui <NxCommands commands={['sync']} /> per applicare i generatori sync in sospeso, quindi ricompila, oppure imposta `sync.applyChanges` a `true` in `nx.json` per applicare automaticamente le modifiche ad ogni build. Incontrerai lo stesso controllo out-of-sync ogni volta che cambiano i riferimenti dei progetti (ad esempio dopo aver aggiunto procedure nel prossimo modulo).
</Aside>
