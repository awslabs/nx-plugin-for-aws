...
- packages/
  - story/
    - dungeon\_adventure\_story/ modulo Python
      - agent/
        - main.py entry point app FastAPI AG-UI per Bedrock AgentCore Runtime
...
Poiché abbiamo scelto `protocol=AG-UI`, il generatore avvolge il nostro agente Strands in un adapter [AG-UI](https://ag-ui.com/). `create_strands_app` costruisce un'app FastAPI che espone un singolo endpoint `/invocations` - le istanze dell'agente per sessione vengono create automaticamente e la cronologia delle conversazioni viene mantenuta in memoria per `threadId` per la durata del server. Non avremo bisogno di modificare `main.py` in nessun punto di questo tutorial.
