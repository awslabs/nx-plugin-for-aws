...
:::note
Passiamo `--iacProvider=CDK` poiché utilizzeremo CDK per l'infrastruttura come codice in questo tutorial. Il Plugin Nx per AWS supporta anche `Terraform`.
:::
...
<Aside type="tip">È una best practice assicurarsi che tutti i file non inseriti nello staging siano commitati in Git prima di eseguire qualsiasi generatore. Questo ti permette di vedere cosa è cambiato dopo aver eseguito il generatore tramite `git diff`</Aside>
...
<Drawer title="File aggiornati da ts#trpc-api" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati dal generatore `ts#trpc-api`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - common/
    - constructs/
      - src/
        - app/ costrutti CDK specifici dell'applicazione
          - apis/
            - **game-api.ts** costrutto CDK per creare la tua tRPC API
            - index.ts
            - ...
          - index.ts
        - core/ costrutti CDK generici
          - api/
            - rest-api.ts costrutto CDK di base per un'API Gateway Rest API
...
```
Il router definisce il punto di ingresso per la tua tRPC API ed è il luogo dove dichiarerai tutti i metodi della tua API. Come puoi vedere sopra, abbiamo un metodo chiamato `echo` con la sua implementazione nel file `./procedures/echo.ts`.
...
Questo file è l'implementazione del metodo `echo` e come puoi vedere è fortemente tipizzato dichiarando le sue strutture di dati di input e output.
...
> extends RestApi<Operations, TIntegrations> {
  /**
   * Crea integrazioni predefinite per tutte le operazioni, che implementano ogni operazione come
  ...
  constructor(
    ...
    super(scope, id, {
      ...
      policy: new PolicyDocument({
        statements: [
          // Qui concediamo a qualsiasi credenziale AWS dell'account in cui il progetto è distribuito di chiamare l'API.
...
Questo è il costrutto CDK che definisce la nostra GameApi. Come puoi vedere, fornisce un metodo `defaultIntegrations` che crea automaticamente una funzione lambda per ogni procedura nella nostra tRPC API, puntando all'implementazione dell'API già bundleizzata. Ciò significa che al momento di `cdk synth` non avviene il bundling (contrariamente all'uso di [NodeJsFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_lambda_nodejs.NodejsFunction.html)) poiché lo abbiamo già bundleizzato come parte del target di build del progetto backend.
...
Dovresti vedere alcuni nuovi file apparire nella tua struttura di cartelle.
<Drawer title="File aggiornati da py#fast-api" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati dal generatore `py#fast-api`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
...
Questo è il costrutto CDK che definisce la nostra StoryApi. Come puoi vedere, fornisce un metodo `defaultIntegrations` che crea automaticamente una funzione lambda per ogni operazione definita nella nostra FastAPI, puntando all'implementazione dell'API già bundleizzata. Ciò significa che al momento di `cdk synth` non avviene il bundling (contrariamente a [PythonFunction](https://docs.aws.amazon.com/cdk/api/v2/docs/@aws-cdk_aws-lambda-python-alpha.PythonFunction.html)) poiché lo abbiamo già bundleizzato come parte del target di build del progetto backend.
...
<Drawer title="File aggiornati da ts#react-website" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati dal generatore `ts#react-website`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - common/
    - constructs/
...
Questo è il punto di ingresso dove viene montato React. Come mostrato, inizialmente configura solo un `@tanstack/react-router` in una configurazione [`file-based-routing`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing). Ciò significa che, finché il server di sviluppo è in esecuzione, puoi semplicemente creare file nella cartella `routes` e `@tanstack/react-router` creerà automaticamente la configurazione boilerplate per te, aggiornando il file `routeTree.gen.ts`. Questo file mantiene tutte le route in modo type-safe, il che significa che quando usi `<Link>`, l'opzione `to` mostrerà solo route valide. Per maggiori informazioni, consulta la [documentazione di `@tanstack/react-router`](https://tanstack.com/router/v1/docs/framework/react/quick-start).
...
Un componente che verrà renderizzato quando si naviga alla route `/welcome`. `@tanstack/react-router` gestirà la `Route` per te ogni volta che crei/sposti questo file (a patto che il server di sviluppo sia in esecuzione). Questo verrà mostrato in una sezione successiva di questo tutorial.
...
<Drawer title="File aggiornati da ts#react-website#auth" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati/aggiornati dal generatore `ts#react-website#auth`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - common/
    - constructs/
...
<Drawer title="File aggiornati da UI -> FastAPI api-connection" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati/aggiornati dal generatore `api-connection`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - game-ui/
    - src/
...
Il componente provider sopra utilizza l'hook `useStoryApiClient` e istanzia `StoryApiOptionsProxy`, che viene utilizzato per costruire le opzioni per gli hook TanStack Query. Puoi utilizzare l'hook corrispondente `useStoryApi` per accedere a questo proxy di opzioni, che fornisce un modo per interagire con la tua FastAPI in modo coerente con la tua tRPC API.
...
<Drawer title="File aggiornati da UI -> tRPC api-connection" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati/aggiornati dal generatore `api-connection`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - game-ui/
...
<Aside>
L'hook `useGameApi` è diverso dall'hook `useStoryApi` poiché non richiede una build affinché le modifiche vengano riflesse, grazie all'uso da parte di tRPC dell'[inferenza TypeScript](https://trpc.io/docs/concepts). Questo permette agli sviluppatori di apportare modifiche al backend che vengono immediatamente riflesse nel frontend!
</Aside>
...
<Drawer title="File aggiornati da ts#infra" trigger="Fai clic qui per esaminare questi file in dettaglio.">
Di seguito è riportato un elenco di tutti i file generati/aggiornati dal generatore `ts#infra`. Esamineremo alcuni dei file chiave evidenziati nella struttura:
<FileTree>
- packages/
  - common/
    - constructs/
...
Puoi anche attivare una build (o qualsiasi altro task) per un singolo progetto eseguendo il target sul progetto direttamente. Ad esempio, se vogliamo buildare il progetto `@dungeon-adventure/infra`, puoi eseguire:
