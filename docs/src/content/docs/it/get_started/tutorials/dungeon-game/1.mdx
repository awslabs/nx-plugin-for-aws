...
Questo è l'entrypoint dell'agente, configurato come applicazione FastAPI compatibile con [Amazon Bedrock AgentCore Runtime](https://github.com/aws/bedrock-agentcore-sdk-python). Utilizza il supporto streaming di Strands per inviare eventi al client in tempo reale tramite JSON Lines.
...
Connettiamo la nostra Game UI allo Story Agent in modo da poter chiamare l'API dell'agente dal nostro sito web React con un client type-safe auto-generato.
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** Script per generare la specifica OpenAPI dall'app FastAPI dell'agente
    - project.json Aggiornato con target `agent-openapi`
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** Provider per il client OpenAPI type-safe
        - **QueryClientProvider.tsx** Provider client TanStack React Query (se non già presente)
      - hooks/
        - **useStoryAgent.tsx** Hook per proxy opzioni TanStack Query
        - **useStoryAgentClient.tsx** Hook per il client API vanilla
      - generated/
        - story-agent/ Generato al momento della build dalla specifica OpenAPI
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json Aggiornato con target di generazione client
    - **main.tsx** Aggiornato per aggiungere StoryAgentProvider

</FileTree>

Il generatore:
- Crea uno script di generazione della specifica OpenAPI agent-scoped nel progetto story
- Genera un client TypeScript type-safe dalla specifica OpenAPI dell'agente al momento della build
- Aggiunge un provider, hook e client al sito web per chiamare l'API dell'agente
- Configura `serve-local` per avviare automaticamente il server locale dell'agente e rigenerare il client alle modifiche
