...
- packages/
  - story/
    - dungeon\_adventure\_story/ modulo Python
      - agent/
        - main.py entry point app Starlette AG-UI per Bedrock AgentCore Runtime
        - agent.py definisce l'agente Strands e i suoi strumenti
        - session.py cattura l'id di sessione AgentCore Runtime in ingresso
...
Esaminiamo i file chiave in dettaglio:
...
@contextmanager
def get_agent():
    # Quando lo storage di sessione gestito da AgentCore è collegato al runtime, il
    # percorso di mount è esposto tramite SESSION_STORAGE_PATH. Persistere attraverso
    # FileSessionManager permette all'agente di riprendere la cronologia completa della conversazione
    # dopo un ciclo di stop/resume. In assenza della variabile d'ambiente (es. serve locale), l'
    # agente funziona solo in memoria.
    ...
    yield Agent(
        ...
        system_prompt="""
Sei un mago matematico.
Usa i tuoi strumenti per compiti matematici.
...
`agent.py` definisce un agente Strands di esempio. Sostituiremo il system prompt e gli strumenti con la logica dungeon-adventure nel <Link path="get_started/tutorials/dungeon-game/3">Modulo 3</Link>.
...
class _LazyStrandsApp:
    """Costruisce l'app Strands interna alla prima richiesta ``/invocations`` in modo che
    i controlli di salute ``/ping`` rimangano leggeri e l'id di sessione AgentCore in ingresso
    venga catturato prima che qualsiasi client MCP / A2A downstream lo legga."""
...
Poiché abbiamo scelto `protocol=AG-UI`, il generatore avvolge il nostro agente Strands in un adapter [AG-UI](https://ag-ui.com/) esposto tramite un'app Starlette. AgentCore instrada ogni `runtimeSessionId` a una microVM dedicata, quindi un `Agent` di lunga durata per container è il ciclo di vita corretto - `main.py` lo costruisce in modo lazy alla prima richiesta `/invocations` dopo aver catturato l'id di sessione in ingresso. Quando lo storage di sessione gestito da AgentCore è collegato, `agent.py` collega un `FileSessionManager` a `SESSION_STORAGE_PATH` in modo che l'agente possa riprendere la cronologia completa della conversazione dopo uno stop/resume. Non avremo bisogno di modificare `main.py` in nessun punto di questo tutorial.
...
Questo è l'entry point dove React viene montato. Inizialmente configura solo un `@tanstack/react-router` in una configurazione [`file-based-routing`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) - finché il server dev è in esecuzione, creare un file in `src/routes/` aggiunge automaticamente una route e rigenera `routeTree.gen.ts`. Questo mantiene `<Link to="...">` completamente type-safe.
...
function RouteComponent() {
  return (
    <div className="text-center">
      <header>
        <h1>Welcome</h1>
        <p>Benvenuto nel tuo nuovo sito React!</p>
      </header>
    </div>
  );
}
...
Un componente verrà renderizzato quando si naviga verso la route `/`. Poiché abbiamo scelto `uxProvider=Shadcn`, il generatore crea anche una sidebar + app shell in `components/AppLayout` usando primitive [shadcn/ui](https://ui.shadcn.com/) condivise sotto `packages/common/shadcn`.
...
L'hook `useGameApi` non richiede una build per riflettere le modifiche grazie all'utilizzo dell'[inferenza TypeScript](https://trpc.io/docs/concepts) di tRPC. Questo permette agli sviluppatori di apportare modifiche al backend e vederle riflesse istantaneamente nel frontend.
</Aside>
...
Per maggiori dettagli, consulta la <Link path="guides/connection/py-strands-agent-mcp">guida alla connessione Python Strands Agent a MCP</Link>.
</Drawer>

### Game UI: Connessione allo Story Agent

Connettiamo la nostra Game UI allo Story Agent in modo che il sito web React possa chattare con l'agente tramite il protocollo AG-UI.
...
Poiché lo Story Agent è stato generato con `protocol=AG-UI`, il generatore `connection` collega il sito web usando [CopilotKit](https://copilotkit.ai/) (che parla il protocollo [AG-UI](https://ag-ui.com/)). Questi file vengono generati/aggiornati:
...
- packages/
  - game-ui/
    - src/
      - components/
        - **AguiProvider.tsx** CopilotKitProvider che registra ogni agente AG-UI
        - **copilot/** Componenti chat CopilotKit tematizzati (bolle assistente/utente, input, cursore) - scelti automaticamente in base al provider UX del sito web (Shadcn qui)
      - hooks/
        - **useAguiStoryAgent.tsx** Hook per agente che costruisce un `HttpAgent` AG-UI autenticato con Cognito che punta al runtime AgentCore
      - **main.tsx** Aggiornato per aggiungere `<AguiProvider>` attorno all'app

</FileTree>

Il generatore:
- Aggiunge un `AguiProvider` condiviso (se non esiste già) che avvolge l'app nel `CopilotKitProvider` di CopilotKit e registra l'hook di ogni agente AG-UI
- Genera un hook per agente (`useAguiStoryAgent`) che risolve l'ARN del runtime AgentCore da `runtime-config.json`, costruisce un `HttpAgent` autenticato e allega l'header `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` su ogni richiesta
- Fornisce primitive chat tematizzate sotto `src/components/copilot/` in modo che la chat appaia nativa al provider UX del sito web

Per maggiori dettagli, consulta la <Link path="guides/connection/react-py-strands-agent">guida alla connessione React a Python Strands Agent</Link>.
</Drawer>
...
<Aside type="tip" title="Shell non interattive">
Se stai eseguendo in una shell non interattiva (CI o un ambiente headless) non vedrai il prompt - `build` fallirà immediatamente con "The workspace is out of sync". In tal caso, esegui <NxCommands commands={['sync']} /> manualmente una volta, quindi riesegui la build.
</Aside>
