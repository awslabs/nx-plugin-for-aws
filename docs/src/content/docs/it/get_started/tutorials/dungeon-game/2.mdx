...
Inizieremo implementando la nostra Game API. Per fare questo, dobbiamo creare 4 API in totale:

1. `createGame` - creerà una nuova istanza di gioco.
2. `queryGames` - restituirà un elenco paginato di partite salvate precedentemente.
...
Per definire input e output delle nostre API, creiamo lo schema utilizzando [Zod](https://zod.dev/) nel progetto `packages/game-api/schema/src` come segue:
...
ElectroDB è molto potente e ci permette non solo di definire i tipi, ma anche di fornire valori predefiniti per alcuni campi come i timestamp sopra. Inoltre, ElectroDB segue il [single-table design](https://electrodb.dev/en/core-concepts/single-table-relationships/), considerata la best practice per DynamoDB.
...
Dato che abbiamo bisogno del client DynamoDB in ogni nostra procedura, vogliamo creare un'istanza unica del client da passare tramite contesto. Apporta queste modifiche in `packages/game-api/backend/src`:
...
<Aside>
L'API `concat` associa il nostro middleware alle procedure definite. Per dettagli, vedi la [guida concat](https://trpc.io/docs/server/middlewares#concat).
</Aside>
  </TabItem>
</Tabs>
...
Puoi anche eliminare il file `echo.ts` (da `packages/game-api/backend/src/procedures`) poiché non lo useremo.

### Configurazione del router

Ora colleghiamo le procedure alla nostra API. Aggiorniamo il file:

<E2EDiff lang="typescript" before="dungeon-adventure/2/router.ts.old.template" after="dungeon-adventure/2/router.ts.template" />

### Infrastruttura

L'ultimo passo è aggiornare l'infrastruttura per creare la tabella DynamoDB e concedere le autorizzazioni necessarie. Modifica `packages/infra/src` come segue:
...
:::note
Nota che ogni procedura è gestita da una singola lambda function, permettendoci di applicare il principio del minimo privilegio concedendo solo le autorizzazioni necessarie.
:::
  </TabItem>
</Tabs>
...
:::tip
Per modifiche al codice lambda, puoi usare il flag `--hotswap` dopo la build per deployment più rapidi (2-3 secondi):
...
**Sconsigliato** se hai stack separati per ambienti diversi (es. infra-prod). Il flag `--all` tenterà di deployare tutto rischiando deployment indesiderati!
...
<Tabs>
  <TabItem label="Locale">
    ...
    <Aside type="caution">
    Usa il valore `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX` dall'output CDK per sostituire il placeholder evidenziato.
    </Aside>
...
```
    <Aside type="caution">
    Sostituisci il placeholder con il valore `dungeon-adventure-infra-sandbox.GameApiGameApiEndpointXXX` dall'output CDK e imposta la regione corretta.
    </Aside>
  </TabItem>
</Tabs>
...
Se il comando ha successo, vedrai una risposta tipo:
```json
