...
5. `queryInventory` - restituirà una lista paginata degli oggetti nell'inventario di un giocatore.

### Schema API

Per definire input e output delle API, creiamo il nostro schema utilizzando [Zod](https://zod.dev/) nella directory `packages/game-api/src/schema` come segue:
...
<Aside type="tip">
Come puoi vedere sopra, per ogni schema definito con Zod, esportiamo anche un'interfaccia usando la sintassi `z.TypeOf`. Questo converte la nostra definizione Zod in un'interfaccia TypeScript senza duplicare gli sforzi!
</Aside>
...
Ora creiamo i seguenti file nella cartella `packages/game-api/src/entities` per definire le nostre entità ElectroDB secondo il diagramma ER:
...
ElectroDB è molto potente e ci permette non solo di definire i tipi, ma anche di fornire valori predefiniti per alcuni campi come i timestamp. Inoltre, ElectroDB segue il [single-table design](https://electrodb.dev/en/core-concepts/single-table-relationships/), best practice per DynamoDB.
...
Per preparare l'interazione del server MCP con l'inventario, esportiamo l'entità inventory in `packages/game-api/src/index.ts`:
...
:::note
La struttura del package migliorerebbe se rifattorizzassimo le entità in un progetto condiviso, così che il server MCP non dipenda dall'API.
:::

### Aggiunta del client DynamoDB al contesto tRPC

Dato che abbiamo bisogno del client DynamoDB in ogni procedura, creiamo un'istanza singola da passare via contesto. Apportiamo queste modifiche in `packages/game-api/src`:

<Tabs>
  <TabItem label="middleware/dynamodb.ts">
<E2ECode lang="typescript" path="dungeon-adventure/2/middleware/dynamodb.ts.template" />

Questo plugin crea il `DynamoDBClient` e lo inietta nel contesto.
  </TabItem>
...
<Aside>
L'API `concat` associa il middleware alle procedure. Per dettagli, consulta la [guida concat](https://trpc.io/docs/server/middlewares#concat).
</Aside>
  </TabItem>
</Tabs>
...
Implementiamo ora i metodi API apportando queste modifiche in `packages/game-api/src/procedures`:
...
Puoi eliminare il file `echo.ts` da `packages/game-api/src/procedures` poiché non verrà utilizzato.

### Configurazione del router

Colleghiamo le procedure all'API aggiornando:
...
Creiamo un server MCP che permetta al nostro agent di gestire l'inventario dei giocatori.

Definiamo questi strumenti per l'Agent:
- `list-inventory-items` per recuperare gli oggetti nell'inventario
- `add-to-inventory` per aggiungere oggetti
- `remove-from-inventory` per rimuoverli

Definiamo tutti gli strumenti inline per risparmiare tempo:
...
Man mano che gli strumenti aumentano, puoi organizzarli in file separati.

Ora puoi eliminare le directory `tools` e `resources` in `packages/inventory/src/mcp-server` poiché non sono utilizzate.

### Infrastruttura

Aggiorniamo l'infrastruttura per creare la tabella DynamoDB e concedere i permessi:
...
:::note
Ogni procedura è servita da una lambda function separata, permettendo di applicare il principio del minimo privilegio nei permessi.
:::
  </TabItem>
</Tabs>
...
:::tip
Per modifiche al codice lambda, usa il flag `--hotswap` dopo la build per deployment rapidi (2-3 secondi):

<NxCommands commands={['run @dungeon-adventure/infra:deploy dungeon-adventure-infra-sandbox/* --hotswap']} />
:::

Al completamento, vedrai output simili a:
...
Per testare l'API:
<ul>
<li>Avvia un'istanza locale del backend tRPC e invoca le API con `curl`</li>
<li>
<Drawer title="Curl con Sigv4 abilitato" trigger="Chiama l'API deployata usando curl con Sigv4">

<Tabs>
  <TabItem label="Bash/Linux/macOS">
Aggiungi questo script al tuo `.bashrc` (e esegui `source`) o incollalo direttamente:
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
Aggiungi questa funzione al tuo profilo PowerShell o incollala:
```powershell
function acurl {
    param(
...
<Tabs>
  <TabItem label="Locale">
    Avvia il server `game-api` locale con:
    ...
    <Aside type="caution">
    Sostituisci il placeholder con il valore `dungeon-adventure-infra-sandbox-Application.ElectroDbTableTableNameXXX` dall'output CDK.
    </Aside>
...
```
    <Aside type="caution">
    Utilizza il valore `dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX` dall'output CDK e imposta la regione corretta.
    </Aside>
  </TabItem>
</Tabs>
