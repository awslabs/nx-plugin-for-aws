...
Implementeremo il nostro database in DynamoDB e utilizzeremo la libreria client [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) per semplificare le operazioni. Il `queryActions` della Game API necessita anche del client S3 per leggere i turni di conversazione che l'agente ha scritto. Installali tutti con:
