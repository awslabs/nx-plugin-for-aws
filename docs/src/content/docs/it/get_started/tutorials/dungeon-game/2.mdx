...
2. `queryGames` - restituirà un elenco paginato di partite precedentemente salvate.
3. `saveAction` - salverà un'azione per una partita specifica.
4. `queryActions` - restituirà un elenco paginato di tutte le azioni relative a una partita.

### Schema delle API

Per definire input e output delle nostre API, creiamo lo schema utilizzando [Zod](https://zod.dev/) all'interno del progetto `packages/game-api/schema/src` come segue:
...
<Aside type="tip">
Come vedi sopra, per ogni schema definito in Zod, esportiamo anche un'interfaccia utilizzando la sintassi `z.TypeOf`. Questo converte la nostra definizione Zod in un'interfaccia TypeScript senza duplicare gli sforzi!
</Aside>
...
ElectroDB è molto potente e ci permette non solo di definire i tipi, ma anche di fornire valori predefiniti per certi campi come i timestamp. Inoltre, ElectroDB segue il [single-table design](https://electrodb.dev/en/core-concepts/single-table-relationships/), considerata la best practice con DynamoDB.
...
Dato che necessitiamo del client DynamoDB in ogni nostra procedura, creiamo un'istanza unica del client da passare tramite contesto. Apportiamo queste modifiche in `packages/game-api/backend/src`:
...
Questo plugin viene utilizzato per creare il `DynamoDBClient` e iniettarlo nel contesto.
  </TabItem>
...
Estendiamo `IMiddlewareContext` per includere `IDynamoDBContext`.
  </TabItem>
...
<Aside>
Utilizziamo l'API `unstable_concat` che è sicura da usare. Il prefisso `unstable_` è presente solo perché è una nuova API. Per maggiori dettagli, consulta la [guida concat](https://trpc.io/docs/server/middlewares#concat).
</Aside>
  </TabItem>
</Tabs>
...
Ora implementiamo i metodi delle API. Apportiamo queste modifiche in `packages/game-api/backend/src/procedures`:
...
È possibile eliminare il file `echo.ts` (da `packages/game-api/backend/src/procedures`) dato che non verrà utilizzato.

### Configurazione del router

Ora colleghiamo le procedure alla nostra API modificando:
...
Aggiorniamo l'infrastruttura per creare la tabella DynamoDB e concedere le autorizzazioni:
...
<Aside type="tip">
In caso di errori di linting, esegui:
...
Il primo deployment richiede circa 8 minuti. I successivi circa 2 minuti.

:::tip
Per aggiornamenti rapidi al codice lambda, usa il flag `--hotswap`:

<NxCommands commands={['run @dungeon-adventure/infra:deploy dungeon-adventure-infra-sandbox --hotswap']} />
:::

<Drawer title="Comando di deployment" trigger="Dettagli sul deployment completo">
Per deployare tutti gli stack:

<NxCommands commands={['run @dungeon-adventure/infra:deploy --all']} />

**Non raccomandato** per ambienti separati (es. `infra-prod`).
</Drawer>

Alla fine del deployment, vedrai output simili a:
...
Per testare l'API:
<ul>
<li>Avvia un'istanza locale del backend tRPC e usa `curl`</li>
<li>
<Drawer title="Curl con Sigv4" trigger="Chiama l'API deployata con curl abilitato a Sigv4">
...
<Tabs>
  <TabItem label="Locale">
    ...
    <Aside type="caution">
    Sostituisci il placeholder con il valore `dungeon-adventure-infra-sandbox.ElectroDbTableTableNameXXX` dall'output del deployment.
    </Aside>
...
```
    <Aside type="caution">
    Utilizza il valore `dungeon-adventure-infra-sandbox.GameApiGameApiUrlXXX` dall'output e imposta la regione corretta.
    </Aside>
  </TabItem>
</Tabs>
