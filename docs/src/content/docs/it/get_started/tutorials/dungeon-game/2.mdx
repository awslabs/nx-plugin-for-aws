...
La dipendenza `@aws-sdk/client-s3` è già presente nel workspace, quindi non c'è nulla di nuovo da installare. Il nome del bucket proviene dallo stesso meccanismo di Configurazione Runtime utilizzato per `tables` — lo registriamo sotto un nuovo namespace `buckets` nello stack dell'infrastruttura qui sotto.
  </TabItem>
</Tabs>
