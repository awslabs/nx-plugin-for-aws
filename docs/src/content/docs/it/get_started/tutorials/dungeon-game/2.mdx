...
<Aside type="tip" title="Perché questo passaggio è necessario">
`nx sync` dichiara `@dungeon-adventure/dungeon-db` nei file `package.json` di `game-api` e `inventory` e aggiunge i riferimenti al progetto TypeScript corrispondenti per te, risparmiandoti la necessità di aggiungerli manualmente quando dichiari dipendenze tra progetti.
</Aside>
