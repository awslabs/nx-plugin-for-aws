...
:::note[La cronologia delle conversazioni risiede nell'agente]
Non persistiamo i messaggi della chat attraverso la Game API. Nel <Link path="get_started/tutorials/dungeon-game/3">Modulo 3</Link> lo Story Agent utilizza un `FileSessionManager` che scrive la cronologia di ogni thread su disco, indicizzata dall'id del thread che l'UI invia ad ogni turno — così ogni coppia `(player, genre)` riprende la propria storia senza che l'UI debba riprodurre i messaggi.
:::
