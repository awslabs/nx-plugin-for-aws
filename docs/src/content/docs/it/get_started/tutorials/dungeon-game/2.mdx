...
Prima di implementare la nostra Game API, dobbiamo creare queste 5 API:
...
Puoi eliminare il file `packages/game-api/src/schema/echo.ts` poiché non lo utilizzeremo in questo progetto.
...
Per definire le entità ElectroDB dal diagramma ER, creiamo i seguenti file nella nostra cartella `packages/game-api/src/entities`:
...
Dato che abbiamo bisogno di accedere al client DynamoDB in ognuna delle nostre procedure, creeremo un'unica istanza del client che possiamo passare tramite il contesto. Per farlo, apporta le seguenti modifiche in `packages/game-api/src`:
...
Questo strumenterà il plugin DynamoDB.
...
Puoi eliminare il file `echo.ts` (da `packages/game-api/src/procedures`) poiché non lo utilizzeremo in questo progetto.
