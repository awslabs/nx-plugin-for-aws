...
:::note[Nessuna procedura per la cronologia della chat]
Potresti aver previsto una procedura per salvare ogni turno utente/assistente della chat. Non ne abbiamo bisogno: l'agente AG-UI è ospitato su Bedrock AgentCore Runtime, che persiste automaticamente la cronologia della conversazione per sessione finché viene utilizzato lo stesso thread id.
:::
