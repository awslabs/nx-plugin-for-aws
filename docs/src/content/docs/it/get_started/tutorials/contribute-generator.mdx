...
:::tip[Formattazione file]
Abbiamo anche chiamato `formatFilesInSubtree` alla fine del generatore, che assicura che tutti i file creati o modificati siano formattati secondo la configurazione [Biome](https://biomejs.dev/) del workspace.
:::
...
Usiamo [GritQL](https://docs.grit.io/) per cercare e trasformare il codice sorgente in modo dichiarativo. L'helper `addDestructuredImport` aggiunge import nominati, e `applyGritQL` applica un pattern GritQL per aggiungere la procedura all'object literal del router.
...
:::tip[Pattern GritQL]
Nel frammento di codice sopra, `applyGritQL` usa un pattern [GritQL](https://docs.grit.io/) per trovare la chiamata `router({ ... })` e aggiungere la nuova procedura al suo object literal. La clausola `where` garantisce l'idempotenza — se la procedura è già presente, non viene apportata alcuna modifica.

Puoi provare i pattern GritQL nel [Playground GritQL](https://docs.grit.io/playground), o installare la CLI GritQL per testare i pattern localmente:
...
Se hai completato il <Link path="get_started/tutorials/dungeon-game/overview">tutorial dell'avventura nel dungeon</Link>, o hai già un altro workspace Nx esistente che utilizza un'API tRPC, puoi saltare questo passaggio.
:::
...
Abbiamo una suite di "test di smoke" che eseguono i generatori in un workspace nuovo e verificano che tutto venga compilato correttamente. Come minimo, il tuo nuovo generatore dovrebbe essere aggiunto a **entrambe** le matrici dei generatori in modo che venga esercitato dai test di smoke:

- `e2e/src/smoke-tests/generator-matrix.ts` — esegue ogni generatore tramite la CLI, un'invocazione alla volta, esattamente come farebbe un utente.
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — un generatore nascosto che compone tutti gli altri in-process su un singolo tree.

Entrambe sono utili da mantenere. La matrice CLI testa il percorso che gli utenti effettivamente seguono. La matrice composta viene distribuita con il plugin, quindi ogni versione rilasciata porta con sé la matrice dei generatori che _essa_ aveva — il che consente ai test che aggiornano un workspace più vecchio di scaffoldarlo con la matrice della propria versione. Poiché chiama i generatori direttamente anziché tramite la CLI, le sue opzioni sono verificate per tipo rispetto allo schema di ciascun generatore, quindi una nuova opzione obbligatoria o un enum modificato causerà il fallimento della build lì anziché ridurre silenziosamente la copertura.

Nota che la matrice dei generatori esegue solo i generatori e compila il workspace — non istanzia alcuna infrastruttura. Per i generatori che distribuiscono infrastruttura, considera di estendere i test e2e di deployment (`e2e/src/smoke-tests/cdk-deploy.spec.ts` e `terraform-deploy.spec.ts`) per distribuire effettivamente le tue risorse (tramite `cdk deploy` / `terraform apply`), quindi aggiungi un'asserzione a `deploy-invocations.ts` che invoca la risorsa distribuita e verifica che si comporti come previsto.

Se il tuo generatore fornisce un server di sviluppo locale, considera di aggiungerlo al test e2e di sviluppo locale (`e2e/src/smoke-tests/local-dev.spec.ts`), che avvia il target `dev` ed esercita il server in esecuzione.
