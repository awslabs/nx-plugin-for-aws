...
Abbiamo una suite di "test di smoke" che eseguono i generatori in un workspace nuovo e verificano che tutto venga compilato correttamente. Come minimo, il tuo nuovo generatore dovrebbe essere aggiunto alla matrice dei generatori in `packages/nx-plugin/src/internal/test-matrix/generator.ts` in modo che venga esercitato dai test di smoke.

La matrice stessa è un generatore (nascosto) che compone tutti gli altri, quindi viene distribuita con il plugin. Ciò significa che ogni versione rilasciata porta con sé la matrice dei generatori che _essa_ aveva, il che consente ai test che aggiornano un workspace più vecchio di scaffoldarlo con la matrice della propria versione. Poiché chiama i generatori direttamente anziché tramite la CLI, le opzioni sono verificate per tipo rispetto allo schema di ciascun generatore — quindi una nuova opzione obbligatoria o un enum modificato causerà il fallimento della build qui anziché ridurre silenziosamente la copertura.
