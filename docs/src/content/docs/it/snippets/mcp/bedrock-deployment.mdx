...
Puoi concedere l'accesso per invocare il tuo server MCP su Bedrock AgentCore Runtime utilizzando il metodo `grantInvokeAccess`. Ad esempio, potresti voler consentire a un agent generato con il generatore <Link path="/guides/py-agent">`py#agent`</Link> di chiamare il tuo server MCP:
