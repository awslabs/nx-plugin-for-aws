...
Se hai selezionato `agentcore` per `infra`, viene generata l'infrastruttura CDK o Terraform rilevante che puoi utilizzare per distribuire il tuo server MCP su [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Passa gli output del modulo <Link path="/it/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> condiviso nel modulo del server MCP:
...
[Docker](https://www.docker.com/) è necessario per compilare e distribuire il tuo server MCP. Assicurati di aver installato e avviato Docker per evitare errori come:
