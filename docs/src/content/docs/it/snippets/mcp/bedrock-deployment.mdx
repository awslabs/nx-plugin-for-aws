...
<Infrastructure>
<Fragment slot="cdk">
Per configurare l'autenticazione JWT utilizzando Cognito, usa il metodo factory `RuntimeAuthorizerConfiguration.usingCognito()`:
...
<Fragment slot="terraform">
Per configurare l'autenticazione JWT, puoi modificare il modulo del server MCP per configurare la variabile `authorizer_configuration` come segue:
