...
:::tip
Utilizziamo il target `dev` (un alias per `serve-local`) che avvia anche server locali per le API connesse tramite `connection`, con hot-reload in caso di modifiche! Questo ci permette di testare API e sito web localmente prima ancora di scrivere codice CDK.
:::
