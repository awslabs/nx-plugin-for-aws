...
Esegui il <Link path="/it/guides/react-website">generatore `ts#website`</Link> con `framework` impostato su `react` per configurare il tuo progetto website in `packages/website`. Poiché l'applicazione shopping list è costruita con componenti CloudScape, impostiamo anche `ux` su `cloudscape` (il valore predefinito è `shadcn`):
...
Il generatore React website sopra non include l'autenticazione cognito per impostazione predefinita come `CloudscapeReactTsWebsiteProject`, invece viene aggiunta esplicitamente tramite il <Link path="/it/guides/react-website-auth">generatore `ts#website#auth`</Link>.
...
Con Nx Plugin for AWS, l'integrazione API è supportata tramite il <Link path="/it/guides/connection">generatore `connection`</Link>. Successivamente, utilizziamo questo generatore in modo che il nostro sito web possa invocare la nostra API Smithy:
