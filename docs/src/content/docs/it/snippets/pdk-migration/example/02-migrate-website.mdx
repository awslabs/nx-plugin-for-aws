...
Esegui il <Link path="/guides/react-website">generatore `ts#website`</Link> con `framework` impostato su `react` per configurare il progetto del sito web in `packages/website`:
...
Il server locale dell'API Smithy viene eseguito con [`tsx`](https://github.com/privatenumber/tsx), quindi aggiungiamolo come dipendenza di sviluppo prima di avviare i server locali:

<InstallCommand dev pkg="tsx" />

Ora avviamo il server locale del sito web:
