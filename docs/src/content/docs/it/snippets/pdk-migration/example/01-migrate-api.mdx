...
1. Aggiungi il nuovo tratto `@cursor` all'operazione `GetShoppingLists` in `get-shopping-lists.smithy`:
    ...
    Qualsiasi operazione `@paginated` dovrebbe utilizzare anche `@cursor` se stai usando il generatore di client fornito dal Nx Plugin for AWS (tramite il generatore <Link path="guides/connection/react-smithy">`api-connection`</Link>).
...
<NxCommands commands={["run-many --target lint --fix"]} />
::: la build del progetto per verificare che la migrazione abbia funzionato finora:
