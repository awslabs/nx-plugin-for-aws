...
1. Aggiungi il nuovo tratto `@cursor` all'operazione `GetShoppingLists` in `get-shopping-lists.smithy`:
    ...
    Qualsiasi operazione `@paginated` dovrebbe utilizzare anche `@cursor` se stai usando il generatore di client fornito dal Nx Plugin for AWS (tramite il generatore <Link path="guides/api-connection/react-smithy">`api-connection`</Link>).
