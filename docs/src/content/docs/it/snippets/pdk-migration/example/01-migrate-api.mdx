...
1. Aggiungi il nuovo trait `@cursor` all'operazione `GetShoppingLists` in `get-shopping-lists.smithy`:
    ...
    Qualsiasi operazione `@paginated` dovrebbe utilizzare anche `@cursor` se stai usando il generatore di client fornito da Nx Plugin for AWS (tramite il generatore <Link path="/guides/connection/react-smithy">`api-connection`</Link>).
