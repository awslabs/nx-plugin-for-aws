...
Per client TypeScript, puoi utilizzare i generatori <Link path="/guides/react-website">`ts#react-website`</Link> e <Link path="/guides/api-connection">`api-connection`</Link> con un esempio `ts#smithy-api` per vedere come i client vengono generati e integrati con un sito web. Questo configura target di build che generano client invocando i nostri generatori `open-api#ts-client` o `open-api#ts-hooks`. Puoi usare questi generatori puntandoli alla tua specifica OpenAPI.
