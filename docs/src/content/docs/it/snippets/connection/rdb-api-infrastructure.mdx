...
Distribuisci l'API nello stesso VPC del database, concedigli `rds-db:connect` tramite `additional_iam_policy_statements` e apri il percorso di rete con una coppia di regole del security group. Le risorse `aws_vpc.main` e `aws_subnet` sono definite nella guida alla distribuzione del database:
