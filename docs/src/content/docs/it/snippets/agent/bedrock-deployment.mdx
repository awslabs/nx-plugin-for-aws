...
Passa gli output del modulo condiviso <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> nel modulo agent. Con il packaging predefinito `agentcore`, il codice dell'agent viene messo in staging nel bucket di asset condiviso, quindi passa anche quello — istanzia il modulo `core/asset-bucket` una volta per deployment, come fanno già i moduli Lambda e API:
