...
Se hai selezionato `agentcore` o `agentcore-ecr` per `infra`, viene generata l'infrastruttura CDK o Terraform rilevante che puoi utilizzare per distribuire il tuo Agent su [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
:::note[Costrutto CDK]
Questo costrutto utilizza il [modulo `aws-cdk-lib/aws-bedrockagentcore`](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-cdk-lib.aws_bedrockagentcore-readme.html).
:::
</Fragment>
<Fragment slot="terraform">
Viene generato un modulo Terraform per te, denominato in base al `name` che hai scelto quando hai eseguito il generatore, o `<ProjectName>-agent` per impostazione predefinita.

Passa gli output del modulo condiviso <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> nel modulo agent, insieme all'archivio di artefatti condiviso utilizzato dal suo packaging. Con il packaging predefinito `agentcore`, il codice dell'agent viene messo in staging nel bucket di asset condiviso, quindi istanzia il modulo `core/asset-bucket` una volta per deployment, come fanno già i moduli Lambda e API:
...
Con `agentcore-ecr` l'immagine dell'agent viene invece pubblicata nel registro di asset condiviso, quindi passa gli output di `core/asset-ecr` invece di quelli del bucket. Un registro serve tutti i container nel workspace, quindi nessun agent ha bisogno di un proprio repository:
...
:::note[Permessi Modello Bedrock]
Il costrutto agent generato concede il permesso di invocare tutti i modelli di base e i profili di inferenza Bedrock per impostazione predefinita (`bedrock:InvokeModel` e `bedrock:InvokeModelWithResponseStream` su `arn:aws:bedrock:*:*:foundation-model/*` e `arn:aws:bedrock:*:*:inference-profile/*`).
...
Il costrutto `UserIdentity` può essere generato utilizzando il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link>, oppure puoi creare il tuo `UserPool` e `UserPoolClient` CDK.
</Fragment>
<Fragment slot="terraform">
