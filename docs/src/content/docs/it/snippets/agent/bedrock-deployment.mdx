...
Se hai selezionato `agentcore` per `infra`, viene generata l'infrastruttura CDK o Terraform rilevante che puoi utilizzare per distribuire il tuo Agent su [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Passa gli output del modulo condiviso <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> nel modulo agent:
...
[Docker](https://www.docker.com/) è richiesto per compilare e distribuire il tuo Agent. Assicurati di aver installato e avviato Docker per evitare errori come:
