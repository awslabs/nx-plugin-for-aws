...
<Fragment slot="terraform">
La configurazione VPC è già supportata dal modulo generato — imposta `enable_vpc` insieme a `vpc_id` e `subnet_ids`, e il modulo distribuisce la funzione Lambda nel tuo VPC dietro un security group che crea per te:
...
Per opzioni che il modulo non espone, modifica direttamente la risorsa `aws_lambda_function` nel modulo Terraform generato.
</Fragment>
</Infrastructure>
