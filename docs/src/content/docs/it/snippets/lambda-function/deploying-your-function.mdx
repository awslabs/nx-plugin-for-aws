...
:::note[Corrispondenza della sorgente eventi]
...
Il modulo della funzione prepara il suo zip di distribuzione in un bucket S3 condiviso per gli asset anziché caricarlo inline tramite l'API Lambda (che ha un limite di richiesta di 70 MB). Istanzia il modulo `core/asset-bucket` una volta per distribuzione e passa il suo output `bucket_name` in ogni modulo lambda / API tramite l'input `asset_bucket_name`:
...
Il modulo `core/asset-bucket` fornisce un bucket S3 con crittografia gestita da S3 (AES256), versionamento, una policy del bucket solo TLS, blocco dell'accesso pubblico e logging degli accessi S3 abilitato (i log vengono consegnati a un bucket sibling access-logs). Riutilizza la stessa istanza del modulo in ogni modulo Lambda / API nella tua distribuzione — ogni chiamante scrive sotto un prefisso di chiave con ambito modulo, quindi non ci sono collisioni.
