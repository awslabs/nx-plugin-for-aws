...
Questo generatore crea infrastrutture come codice CDK o Terraform in base al `iac` selezionato. Puoi utilizzarlo per distribuire la tua funzione.
...
Assicurati che la sorgente eventi corrisponda all'opzione `event` selezionata per garantire una corretta gestione degli eventi nella tua funzione handler.
:::
