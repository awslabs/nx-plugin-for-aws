...
3. Effettui il commit delle traduzioni risultanti nel branch della PR. Il messaggio di commit
   deve corrispondere al valore `translationCommitMessage` in
   `scripts/translate.config.json` (predefinito `docs: update translations`) in modo che le
