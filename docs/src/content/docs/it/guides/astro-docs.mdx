...
lingue che desideri sulla riga di comando:
...
traduzione](#configurare-la-traduzione)), `--languages` diventa opzionale e vengono
...
:::caution[Configura prima le tue lingue di destinazione]
Senza `--languages` e con `targetLanguages` ancora vuoto, il target esce
...
3. Effettui il commit delle traduzioni risultanti nel branch della PR, utilizzando
   ...
   chiave `translationCommitMessage` a `scripts/translate.config.json` e fai corrispondere
   quel valore invece.
