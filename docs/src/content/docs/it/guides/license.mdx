...
:::note[Terminali non interattivi]
Il prompt appare solo in un terminale interattivo. In CI — o in qualsiasi ambiente non-TTY — il task fallisce invece, e Nx stampa le istruzioni per eseguire `nx sync` (o `nx sync:check` per visualizzare l'anteprima). Imposta `sync.applyChanges` a `true` nel tuo `nx.json` per saltare il prompt e sincronizzare sempre automaticamente.
:::
