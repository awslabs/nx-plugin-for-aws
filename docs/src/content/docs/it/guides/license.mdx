...
Questo significa che non è necessario eseguire il controllo esplicitamente, anche se puoi ancora farlo con il target NxCommands `license-check`:
...
Per disabilitare permanentemente, rimuovi la chiave `license.dependencies` dalla tua configurazione in `aws-nx-plugin.config.mts`. Puoi anche rieseguire il generatore `license` con `--dependencyCheck=false` per generare lo scaffold senza di essa — il flag del generatore è ancora chiamato `--dependencyCheck` e controlla se il generatore scrive il blocco `dependencies`.