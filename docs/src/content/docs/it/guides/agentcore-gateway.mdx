...
<FileTree>
  - packages/\<name>/
    - policies/ File sorgente delle policy Cedar (omessi quando `cedarPolicy: false`)
      - permit-all.cedar Policy Cedar predefinita che permette ai chiamanti autenticati (chiamanti IAM dallo stesso account AWS, o qualsiasi utente Cognito — vedi [Autenticazione](#authentication))
...
- Un `AgentCore::Gateway` configurato per il protocollo MCP con autenticazione IAM in ingresso (predefinita) o autenticazione JWT Cognito (vedi <a href="#authentication">Autenticazione</a>)
