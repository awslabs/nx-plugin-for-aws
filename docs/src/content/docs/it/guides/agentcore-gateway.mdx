...
Per un gateway `mcp` il gateway locale espone un singolo endpoint MCP che aggrega ogni server MCP collegato (connesso tramite il <Link path="guides/connection/agentcore-gateway-mcp">generatore `agentcore-gateway#mcp-connection`</Link>), con strumenti prefissati `<target>___<tool>` per corrispondere al Gateway distribuito.

Per un gateway `http` il gateway locale inoltra i percorsi `/<targetName>/...` al server locale di ciascun agente collegato (connesso tramite il <Link path="guides/connection/agentcore-gateway-agent">generatore `agentcore-gateway#agent-connection`</Link>), corrispondendo al routing basato sul percorso del Gateway distribuito.
