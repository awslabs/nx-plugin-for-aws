...
<FileTree>
  - packages/\<name>/
    ...
    - package.json Dichiara le dipendenze importate da `local-dev.ts`
    - tsconfig.json Configurazione TypeScript utilizzata dal target `typecheck`
    - project.json Aggiunge i target `serve`, `dev`, `build`, `lint`, `format` e `typecheck`
</FileTree>
...
Seleziona `cognito` quando il Gateway fronteggia un agente con `auth: cognito`: il Gateway inoltra il token del chiamante al runtime dell'agente, quindi deve averne ricevuto uno. Vedi <Link path="guides/connection/agentcore-gateway-agent">Gateway a Agent</Link>.
