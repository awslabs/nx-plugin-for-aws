...
Genera un completo [Plugin Nx](https://nx.dev/extending-nx/intro/getting-started) in TypeScript con un server integrato [Model Context Protocol (MCP)](https://modelcontextprotocol.io/). Questo generatore crea una base per sviluppare generatori personalizzati che possono essere utilizzati in modo fluido da assistenti AI per la generazione automatica di codice e l'impalcatura di progetti.
...
- your-plugin/
  - src/
    ...
    - mcp-server/
      ...
      - stdio.ts Punto di ingresso per il server MCP con trasporto STDIO
      - http.ts Punto di ingresso per il server MCP con trasporto HTTP streamable
      - tools/
  ...
  - tsconfig.json Configurazione TypeScript (CommonJS per compatibilità con Nx)
...
Una volta creato il progetto del plugin, puoi aggiungere generatori usando il generatore <Link path="/guides/ts-nx-generator">`ts#nx-generator`</Link>:
...
Assicurati di scrivere un `README.md` dettagliato per il tuo generatore, poiché viene utilizzato dallo strumento `generator-guide` del Server MCP.
...
1. **Aggiorna le Informazioni del Package**: Modifica il [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) del tuo plugin con nome, versione, descrizione e informazioni del repository corrette.

2. **Compila il Package**:
   <NxCommands commands={['package your-plugin']} />

3. **Test Locale**: Puoi testare il tuo plugin localmente installandolo in un altro workspace:
   ```bash
...
#### Configurazione Assistanti AI con il Tuo Plugin Pubblicato
