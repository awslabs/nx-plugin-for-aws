...
<FileTree>
  - \<name>
    - \_\_init\_\_.py Esportazioni del package (`get_engine`, `session_context`, `get_session`)
...
Importa `session_context` dal tuo package di database e usalo come context manager per ottenere una `Session` SQLModel:
...
Quando ti connetti direttamente al cluster Aurora (senza RDS Proxy), passa il percorso del bundle CA di Amazon RDS utilizzando il parametro opzionale `rds_ca`:
...
Il valore `rds_ca` deve puntare a un bundle di certificati CA disponibile nel filesystem del tuo runtime. Per Amazon RDS, utilizza il bundle CA globale da:

```text
https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
```

Come rendere disponibile quel file dipende dal runtime che si connette al database.

#### Immagini Container del Runtime

Se prepari la tua immagine container per il runtime, aggiungi il bundle CA RDS nel tuo Dockerfile e passa quel percorso a `session_context`:
...
Se distribuisci una funzione Lambda zippata, allega il layer Lambda `RdsCaLayer` (CDK) o il modulo Terraform `ca-layer`. Il layer CDK utilizza il bundling Docker per aggiungere il bundle CA, e il layer distribuito lo fornisce in `/opt/global-bundle.pem`, che puoi passare a `session_context`:
