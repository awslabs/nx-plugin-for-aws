...
Se hai selezionato `Any` come `eventSource`, il middleware `parser` non è configurato e il parametro `event` è tipizzato come `any`. Rigenera la funzione con un `eventSource` specifico se desideri type safety a compile-time e validazione runtime.
</OptionFilter>
