...
Il generatore aggiunge anche un target `bundle` al `project.json` del progetto e le dipendenze runtime Powertools, Middy e Zod che l'handler importa al suo `package.json`.
...
1. **Stack di middleware Middy** per funzionalità Lambda avanzate
2. **Integrazione AWS Lambda Powertools** per l'osservabilità
3. **Raccolta di metriche** con CloudWatch
4. **Sicurezza dei tipi** utilizzando il middleware parser
...
Il tracciamento AWS X-Ray è configurato automaticamente tramite il middleware `captureLambdaHandler`. Puoi aggiungere sottosegmenti personalizzati alle tue tracce:
...
Se hai scelto un `event` durante la generazione della tua funzione lambda, la tua funzione è strumentata con il [`parser` middleware from AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Ad esempio:
...
Questo fornisce sicurezza dei tipi in fase di compilazione e validazione in fase di esecuzione per i tuoi eventi Lambda.

:::caution[Gestione degli errori]
Se non vuoi che il tuo handler generi un errore quando l'evento non è conforme allo schema, puoi utilizzare l'opzione [`safeParse`](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#safe-parsing).
:::

:::tip[Dati evento personalizzati]
Se hai dati personalizzati annidati all'interno di un evento, ad esempio uno stream DynamoDB o un evento EventBridge, potresti trarre vantaggio dall'utilizzo di [Envelopes](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#envelopes) per fornire sicurezza dei tipi per quei dati personalizzati.
:::
</OptionFilter>
...
<Snippet name="lambda-function/deploying-your-function" parentHeading="Distribuzione della tua funzione" />
