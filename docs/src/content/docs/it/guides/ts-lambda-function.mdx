...
Hai selezionato `Any` come `eventSource`, il che significa che il middleware `parser` non è configurato e il parametro `event` è tipizzato come `any`. Se in seguito desideri type safety a compile-time e validazione runtime, rigenera la funzione con un `eventSource` specifico.
</OptionFilter>
