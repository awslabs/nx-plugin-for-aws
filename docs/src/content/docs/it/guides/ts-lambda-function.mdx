...
Il generatore crea infrastruttura as code per distribuire la tua funzione in base al `iac` selezionato:
...
Se hai selezionato un `event` durante la generazione della tua funzione lambda, questa viene strumentata con il middleware [`parser` di AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Esempio:
...
Se hai selezionato `Any` come `event`, il middleware `parser` non è configurato e il parametro `event` è tipizzato come `any`. Rigenera la funzione con un `event` specifico se desideri type safety a compile-time e validazione runtime.
</OptionFilter>
