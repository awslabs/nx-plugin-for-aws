...
Il generatore `api-connection` fornisce un modo rapido per integrare il tuo sito React con il backend FastAPI. Configura tutte le impostazioni necessarie per connettersi ai backend FastAPI in modo type-safe, inclusi la generazione del client e degli hook [TanStack Query](https://tanstack.com/query/v5), il supporto per l'autenticazione AWS IAM e Cognito, e una corretta gestione degli errori.
...
3. L'autenticazione Cognito aggiunta tramite il generatore <Link path="/guides/react-website-auth">`ts#react-website-auth`</Link> se ci si connette a un'API che utilizza l'autenticazione Cognito o IAM
...
- project.json Aggiunge un nuovo target al build che invoca lo script di generazione sopra indicato
...
- .gitignore I file del client generati vengono ignorati per default

</FileTree>

Il generatore aggiungerà anche la Runtime Config all'infrastruttura del tuo sito web se non è già presente, assicurando che l'URL dell'API per il tuo FastAPI sia disponibile nel sito e configurato automaticamente dall'hook `use<ApiName>.tsx`.

### Generazione del codice

Durante il build, viene generato un client type-safe dalla specifica OpenAPI del tuo FastAPI. Questo aggiungerà tre nuovi file all'applicazione React:
...
:::tip
Per default, il client generato viene ignorato dal controllo versione. Se preferisci includerlo, puoi rimuovere la voce dal file `.gitignore` della tua applicazione React, ma tieni presente che eventuali modifiche manuali ai file `.gen.ts` verranno sovrascritte durante il build del progetto.
:::
...
:::note
Ogni volta che apporti modifiche al tuo FastAPI, devi ricostruire il progetto affinché queste modifiche si riflettano nel client generato. Ad esempio:

<NxCommands commands={['run-many --target build --all']} />
:::

:::tip
Se stai lavorando contemporaneamente alla tua applicazione React e al FastAPI, utilizza il target `serve-local` dell'applicazione React che rigenererà automaticamente il client ogni volta che l'API cambia, oltre a effettuare l'hot-reload del sito e del server FastAPI locale:
...
Puoi utilizzare il metodo `queryOptions` per recuperare le opzioni richieste per chiamare la tua API utilizzando l'hook `useQuery` di TanStack Query:
...
function MyComponent() {
  ...
  if (item.isLoading) return <div>Caricamento...</div>;
  if (item.isError) return <div>Errore: {item.error.message}</div>;

  return <div>Elemento: {item.data.name}</div>;
}
```
...
function MyComponent() {
  ...
  return <div>Elemento: {item.name}</div>;
}
```
</Drawer>
...
function CreateItemForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    createItem.mutate({ name: 'Nuovo elemento', description: 'Un nuovo elemento' });
  };
...
Per endpoint che accettano un parametro `cursor` come input, gli hook generati forniscono il supporto per infinite queries utilizzando l'hook `useInfiniteQuery` di TanStack Query. Questo semplifica l'implementazione della funzionalità "carica più elementi" o scroll infinito.
...
function MyComponent() {
  ...
  const handleClick = () => {
    createItem.mutate({ name: 'Nuovo elemento' });
  };
...
function MyComponent() {
  ...
  const handleClick = async () => {
    try {
      await api.createItem({ name: 'Nuovo elemento' });
    } catch (e) {
...
Se hai <Link path="guides/fastapi#streaming">configurato il tuo FastAPI per streammare le risposte</Link>, il tuo hook `useQuery` aggiornerà automaticamente i suoi dati man mano che arrivano nuovi chunk dello stream.
...
Se hai <Link path="guides/fastapi#streaming">configurato il tuo FastAPI per streammare le risposte</Link>, il client generato includerà metodi type-safe per iterare in modo asincrono sui chunk del tuo stream utilizzando la sintassi `for await`.
...
Per default, le operazioni nel tuo FastAPI che utilizzano i metodi HTTP `PUT`, `POST`, `PATCH` e `DELETE` sono considerate mutazioni, mentre tutte le altre sono considerate query.
...
Per default, gli hook generati assumono una paginazione basata su cursor con un parametro chiamato `cursor`. Puoi personalizzare questo comportamento utilizzando l'estensione `x-cursor`:
...
function ItemsAndUsers() {
  ...
  const handleCreateItem = async () => {
    try {
      ...
      const newItem = await api.items.create({ name: 'Nuovo elemento' });
...
:::tip
Quando definisci risposte di errore in FastAPI, utilizza sempre il parametro `responses` per specificare il modello per ogni codice di stato. Questo assicura che il client generato avrà le corrette informazioni sui tipi per la gestione degli errori.
:::
...
function ItemList() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err, itemId, context) => {
      // Se la mutazione fallisce, utilizza il contesto restituito da onMutate per ripristinare
...
L'integrazione fornisce una completa type safety end-to-end. Il tuo IDE fornirà autocompletamento e type checking per tutte le chiamate API:
...
function ItemForm() {
  ...
  // L'UI degli errori può utilizzare il type narrowing per gestire diversi tipi di errore
  if (createItem.error) {
  ...
  return (
    <form onSubmit={(e) => {
      e.preventDefault();
      handleSubmit({ name: 'Nuovo elemento' });
    }}>
      {/* Campi del form */}
      <button
        type="submit"
...
I tipi vengono generati automaticamente dallo schema OpenAPI del tuo FastAPI, assicurando che eventuali modifiche alla tua API si riflettano nel codice frontend dopo un build.