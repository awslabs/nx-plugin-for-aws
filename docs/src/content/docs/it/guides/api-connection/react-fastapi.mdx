...
Il generatore `api-connection` fornisce un modo rapido per integrare il tuo sito React con il backend FastAPI. Configura automaticamente tutto il necessario per connettersi in modo type-safe ai backend FastAPI, includendo la generazione di client e hook [TanStack Query](https://tanstack.com/query/v5), il supporto per autenticazione AWS IAM e Cognito, e una corretta gestione degli errori.
