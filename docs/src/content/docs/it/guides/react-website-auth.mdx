---
title: Autenticazione per siti web React
description: Documentazione di riferimento per l'autenticazione per siti web React
...
Il generatore di autenticazione per siti web React aggiunge l'autenticazione al tuo sito web React utilizzando [Amazon Cognito](https://aws.amazon.com/cognito/).
...
`mfa` accetta `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` in CDK, e `"OFF"` / `"OPTIONAL"` / `"ON"` in Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` e l'equivalente `otp` abilitano o disabilitano ciascun metodo di secondo fattore in modo indipendente; non hanno effetto quando `mfa` è disattivato.

:::caution[Terraform collega l'MFA SMS alla verifica del numero di telefono]
A differenza del costrutto CDK, il provider AWS di Terraform non ha modo di offrire SMS come metodo MFA indipendentemente dal suo utilizzo per la verifica del numero di telefono. Impostare `mfa_second_factor_sms = false` disabilita quindi anche la verifica del numero di telefono basata su SMS (`auto_verified_attributes` elimina `phone_number`).
:::
...
Il costrutto `UserIdentity` aggiunge automaticamente la <Link path="guides/react-website#runtime-configuration">configurazione di runtime</Link> necessaria per garantire che il tuo sito web possa puntare al Cognito User Pool corretto per l'autenticazione.

:::note[Conservazione del ruolo SMS]
Il User Pool è supportato da un ruolo IAM che Cognito assume per inviare messaggi SMS. La politica di rimozione del ruolo rispecchia quella del User Pool — per impostazione predefinita il `UserPool` di CDK utilizza `RemovalPolicy.RETAIN`, quindi con `cdk destroy` sia il User Pool che il suo ruolo SMS vengono conservati e dovranno essere puliti manualmente se non più necessari.
:::
</Fragment>
<Fragment slot="terraform">
...
Il client User Pool generato consente l'URL di distribuzione CloudFront (e qualsiasi nome di dominio personalizzato configurato su di esso) come URL di callback/logout OAuth, più `http://localhost:4200` e `http://localhost:4300` per lo sviluppo locale contro il pool distribuito.
