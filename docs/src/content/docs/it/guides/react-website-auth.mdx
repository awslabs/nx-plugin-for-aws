...
:::note[URL di Callback]
Il client User Pool generato consente `http://localhost:4200` e `http://localhost:4300` come URL di callback/logout OAuth (per lo sviluppo locale contro il pool distribuito), oltre all'URL della distribuzione CloudFront. Mantieni questa allowlist al minimo: per distribuzioni di produzione rafforzate, considera di rimuovere gli URL localhost (o di utilizzare un client separato per lo sviluppo locale) una volta che non esegui più localmente contro il User Pool distribuito.
:::
