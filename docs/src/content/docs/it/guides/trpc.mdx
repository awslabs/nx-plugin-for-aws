...
:::tip[Tipo di API]
Seleziona `ServerlessApiGatewayRestApi` (predefinito) come `computeType` se desideri utilizzare le [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) per lo streaming delle risposte. Seleziona `BedrockAgentCoreRuntimeWebSocket` per deployare il server tRPC come container su Bedrock AgentCore con subscriptions WebSocket in streaming — vedi [Deploying to Bedrock AgentCore](#deploying-to-bedrock-agentcore) di seguito.
:::

:::tip[Pattern di integrazione]
L'opzione `integrationPattern` ha come valore predefinito `isolated`, che crea un Lambda per ogni procedura tRPC. Seleziona `shared` se preferisci un singolo Lambda handler condiviso per l'intera API, con override opzionali per singole procedure. L'opzione `integrationPattern` non si applica quando `computeType` è `BedrockAgentCoreRuntimeWebSocket`.
:::
...
:::caution[Tipo di calcolo sottoscrizioni]
Le subscriptions sono supportate solo quando `computeType` è `ServerlessApiGatewayRestApi` (predefinito), poiché le API HTTP di API Gateway non supportano lo streaming delle risposte.
:::

Le subscriptions tRPC ti permettono di effettuare lo streaming di dati dal server al client utilizzando [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Quando selezioni `ServerlessApiGatewayRestApi` come tipo di compute, il generatore configura automaticamente l'infrastruttura necessaria per lo streaming, oltre a un Lambda handler per lo streaming e l'helper schema `ZodAsyncIterable`.
...
Impostare `computeType` su `BedrockAgentCoreRuntimeWebSocket` deploya il server tRPC come server WebSocket containerizzato su [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) invece di API Gateway + Lambda. AgentCore ti offre:

- **Subscriptions in streaming su WebSocket** — messaggistica full duplex per le subscriptions tRPC senza timeout di 30 secondi di Lambda o API Gateway.
- **Affinità di sessione** — l'header `x-amzn-bedrock-agentcore-runtime-session-id` viene propagato nel contesto tRPC come `ctx.sessionId`, così puoi gestire lo stato per sessione.
- **Runtime gestito** — AgentCore gestisce lo scaling, i controlli di salute (`/ping`) e l'autorizzazione.

:::caution[Opzioni di autenticazione]
AgentCore supporta solo autenticazione `IAM` o `Cognito`. Selezionare `auth=None` con `computeType=BedrockAgentCoreRuntimeWebSocket` causerà un errore nella generazione.
:::

### Come funziona

Quando scegli `BedrockAgentCoreRuntimeWebSocket`, il generatore emette:

- `src/handler-agentcore.ts` — un server Node WebSocket + HTTP che espone `/ping` per i controlli di salute di AgentCore e `/ws` per le subscriptions tRPC tramite `@trpc/server/adapters/ws`.
- `Dockerfile` — containerizza il server bundled e include [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) per l'osservabilità gestita.
- Un costrutto CDK (o modulo Terraform) che provisiona un `Runtime` Bedrock AgentCore, gli concede il permesso di invocare modelli Bedrock e leggere la configurazione runtime, e pubblica l'ARN del runtime su `runtimeConfig.apis.<YourApiName>` per il frontend.
- Un target di build `docker` collegato al target `build` del progetto in modo che `nx build` produca l'immagine container.

### Accesso al session id

Ogni procedura riceve `ctx.sessionId` quando l'API è deployata su AgentCore:
...
Localmente, `nx serve` esegue un server HTTP + WebSocket standalone in modo che i client possano connettersi allo stesso modo in cui lo fanno in produzione. Il session id è predefinito su un UUID casuale quando l'header non è presente.
...
:::warning[Presupposto di autenticazione]
...
Nel nostro caso, vogliamo estrarre dettagli sull'utente Cognito chiamante. Lo faremo estraendo l'ID subject (o "sub") dell'utente dall'evento API Gateway e recuperando i dettagli utente da Cognito. L'implementazione varia leggermente a seconda che l'evento sia fornito alla nostra funzione da un'API REST o HTTP:
