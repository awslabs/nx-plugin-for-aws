...
Il generatore tRPC supporta due pattern infrastrutturali tramite l'opzione `integrationPattern`:

- `isolated` è il valore predefinito. Genera una funzione Lambda per procedura.
- `shared` genera una singola integrazione Lambda predefinita `$router` per l'intera API e la riutilizza per ogni procedura, a meno che tu non fornisca override per singole procedure.

`isolated` ti fornisce un'infrastruttura più granulare e rende più facile associare permessi o configurazioni diverse per operazione. `shared` riduce la proliferazione di Lambda e integrazioni, consentendoti comunque di sovrascrivere procedure specifiche con integrazioni dedicate quando necessario.

Entrambi i pattern di integrazione sono attualmente supportati per `ServerlessApiGatewayRestApi` e `ServerlessApiGatewayHttpApi`.
