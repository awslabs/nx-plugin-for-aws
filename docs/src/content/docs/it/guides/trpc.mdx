...
Questo esempio illustra il middleware per l'identità per l'autenticazione `IAM`, dove cerchiamo il chiamante in Cognito utilizzando il sub estratto dall'evento API Gateway. Per l'autenticazione `Cognito`, il middleware per l'identità è più diretto — estrai i claim direttamente dall'`event`.
...
Per collegare un Cognito User Pool e client, fornisci le variabili rilevanti:
