...
Il generatore di API tRPC crea una nuova API tRPC con configurazione dell'infrastruttura AWS CDK o Terraform. Il backend generato può essere deployato come funzioni AWS Lambda esposte tramite un'API AWS API Gateway, o come server WebSocket containerizzato in esecuzione su [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html). Include la validazione degli schemi tramite [Zod](https://zod.dev/) e configura [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/) per l'osservabilità, inclusi logging, tracciamento AWS X-Ray e metriche CloudWatch.
...
:::tip[Quando usare ogni `computeType`]
- **`ServerlessApiGatewayHttpApi`** — l'opzione più conveniente. Scegli questa quando hai bisogno solo di query e mutation e non necessiti di [WAF](https://aws.amazon.com/waf/) a livello API. Le [HTTP API](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api.html) sono più economiche e a latenza inferiore rispetto alle REST API, ma non possono effettuare lo streaming delle risposte (quindi niente [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions)).
- **`ServerlessApiGatewayRestApi`** (predefinito) — scegli questa quando hai bisogno di [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) tramite [Server-Sent Events](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events) e/o un [WAF Web ACL](https://aws.amazon.com/waf/) sull'API. Le [REST API](https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-rest-api.html) costano di più rispetto alle HTTP API ma supportano lo streaming delle risposte e l'integrazione con WAF.
- **`BedrockAgentCoreRuntimeWebSocket`** — scegli questa quando hai bisogno di [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) tramite WebSocket per lo streaming in tempo reale senza il timeout di 30 secondi di Lambda o API Gateway. [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) ospita il tuo server tRPC come server WebSocket containerizzato con autenticazione gestita, affinità di sessione e scaling.

Per un confronto più approfondito tra REST e HTTP API Gateway, consulta la [Guida per sviluppatori di API Gateway](https://docs.aws.amazon.com/apigateway/latest/developerguide/http-api-vs-rest.html).
:::
...
<FileTree>
  - src
    ...
    - handler.ts Punto di ingresso del Lambda handler (solo tipi di compute Lambda)
    - handler-agentcore.ts Punto di ingresso del server WebSocket (solo tipo di compute AgentCore)
    ...
    - schema Definizioni schema con Zod
      ...
      - z-async-iterable.ts Helper Zod per le subscriptions (solo REST API e AgentCore)
  ...
  - Dockerfile Immagine container per deployment AgentCore (solo tipo di compute AgentCore)
...
Le subscriptions tRPC effettuano lo streaming di dati dal server al client. Il generatore configura automaticamente l'helper schema `ZodAsyncIterable` e l'handler per le risposte in streaming.

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Le subscriptions utilizzano [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). L'infrastruttura generata utilizza un Lambda handler per lo streaming con `ResponseTransferMode.STREAM` in API Gateway per tutte le operazioni REST API, il che consente alle subscriptions di funzionare insieme a query e mutation regolari.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Le subscriptions utilizzano [WebSockets](https://developer.mozilla.org/en-US/docs/Web/API/WebSocket) contro l'endpoint runtime AgentCore. L'handler generato espone `/ws` tramite `@trpc/server/adapters/ws`, che trasporta query, mutation e subscriptions su una singola connessione full-duplex senza il timeout di 30 secondi di Lambda o API Gateway.
</TabItem>
</Tabs>
...
Quando deployato su AgentCore, ogni procedura riceve `ctx.sessionId` — il valore dell'header `x-amzn-bedrock-agentcore-runtime-session-id` che AgentCore imposta sull'upgrade WebSocket. Usalo per gestire lo stato per sessione:
...
export const chat = publicProcedure
  ...
  .subscription(async function* (opts) {
    ...
    // ...stream chunks che si basano su `history`
  });
```

Localmente, `nx serve` esegue un server HTTP + WebSocket standalone in modo che i client possano connettersi nello stesso modo in cui lo fanno in produzione. L'ID di sessione ha come valore predefinito un UUID casuale quando l'header non è presente.
</OptionFilter>
...
Su AgentCore, il runtime ha già applicato l'autenticazione `IAM` o `Cognito` sull'upgrade WebSocket prima che il tuo handler veda qualsiasi traffico. Il contesto generato espone:

- `ctx.sessionId` — il valore dell'header `x-amzn-bedrock-agentcore-runtime-session-id` impostato da AgentCore sull'upgrade WebSocket.

Puoi leggere `ctx.sessionId` direttamente in qualsiasi procedura — non è richiesto middleware aggiuntivo:
...
Se vuoi esporre campi aggiuntivi (ad es. claim JWT personalizzati inoltrati tramite header) alle procedure, aggiungi middleware che aumenta `ctx` nello stesso pattern degli esempi Lambda sopra — l'unica differenza è che la fonte di verità sono gli header di upgrade WebSocket, non un evento API Gateway.
</OptionFilter>
...
:::caution[Nessuna autenticazione]
Potresti vedere errori nei check Checkov come:
...
Il generatore emette un costrutto CDK (o modulo Terraform) che fornisce un `Runtime` Bedrock AgentCore, gli concede il permesso di invocare modelli Bedrock e leggere la configurazione runtime, e pubblica l'ARN del runtime in `runtimeConfig.apis.<YourApiName>` per il frontend. Un target di build `docker` è collegato al target `build` del progetto in modo che `nx build` produca l'immagine container.
...
Il costrutto fornisce il runtime AgentCore dall'immagine container costruita dal target `docker` e espone l'ARN del runtime tramite `api.agentCoreRuntime.agentRuntimeArn`.
</Fragment>
<Fragment slot="terraform">
```hcl
module "my_api" {
...
Il modulo fornisce il runtime AgentCore dall'immagine container costruita dal target `docker` e pubblica l'ARN del runtime tramite l'output `agent_runtime_arn`.
</Fragment>
</Infrastructure>
</OptionFilter>
...
Se hai selezionato CDK per il tuo `iacProvider`, quando aggiungi o rimuovi una procedura nella tua API tRPC, queste modifiche si rifletteranno immediatamente nel costrutto CDK senza necessità di ricompilare.
:::
</OptionFilter>
