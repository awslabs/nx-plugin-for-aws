...
<FileTree>
  - src
    ...
    - schema Definizioni degli schemi tramite Zod
      - echo.ts Esempi di definizioni per l'input e l'output della procedura "echo"
    ...
    - middleware
      ...
      - logger.ts Middleware per la configurazione di AWS Powertools per il logging Lambda
      - tracer.ts Middleware per la configurazione di AWS Powertools per il tracciamento Lambda
      - metrics.ts Middleware per la configurazione di AWS Powertools per le metriche Lambda
...
Il generatore creerà anche infrastruttura come codice CDK o Terraform utilizzabile per il deployment dell'API, residente nella directory `packages/common`.

## Implementazione dell'API tRPC

Ad alto livello, le API tRPC consistono in un router che delega le richieste a procedure specifiche. Ogni procedura ha un input e un output definiti come schemi Zod.
...
Gli schemi sono condivisi sia dal codice server che client, fornendo un unico punto di aggiornamento per le modifiche alle strutture utilizzate nella tua API.

Gli schemi sono automaticamente validati dalla tua API tRPC a runtime, evitando la necessità di scrivere logiche di validazione personalizzate nel backend.
...
## Personalizzazione dell'API tRPC
...
Il logger AWS Lambda Powertools è configurato in `src/middleware/logger.ts` e accessibile in un'implementazione API tramite `opts.ctx.logger`. Puoi usarlo per loggare su CloudWatch Logs e/o controllare valori aggiuntivi da includere in ogni messaggio di log strutturato. Ad esempio:
...
Le metriche AWS Lambda Powertools sono configurate in `src/middleware/metrics.ts` e accessibili tramite `opts.ctx.metrics`. Puoi usarle per registrare metriche in CloudWatch senza bisogno di importare e usare l'AWS SDK, ad esempio:
...
Il tracer AWS Lambda Powertools è configurato in `src/middleware/tracer.ts` e accessibile tramite `opts.ctx.tracer`. Puoi usarlo per aggiungere tracce con AWS X-Ray e ottenere dettagliate informazioni sulle prestazioni e il flusso delle richieste API. Ad esempio:
...
:::warning
Questo esempio presuppone che `auth` sia impostato su `IAM`. Per l'autenticazione Cognito, il middleware per l'identità è più diretto, estraendo i claim rilevanti dall'`event`.
:::
...
export const createIdentityPlugin = () => {
  ...
  return t.procedure.use(async (opts) => {
    ...
    const { Users } = await cognito.listUsers({
      // Presuppone che l'ID del user pool sia configurato nell'ambiente lambda
...
## Deployment dell'API tRPC

Il generatore di API tRPC crea infrastruttura come codice CDK o Terraform in base al `iacProvider` selezionato. Puoi usarla per deployare la tua API tRPC.

<Infrastructure>
<Fragment slot="cdk">
Il costrutto CDK per il deployment dell'API si trova nella cartella `common/constructs`. Puoi utilizzarlo in un'applicazione CDK, ad esempio:
...
Questo configura l'infrastruttura dell'API, inclusa un'API AWS API Gateway REST o HTTP, funzioni Lambda per la business logic e autenticazione in base al metodo `auth` scelto.
...
I moduli Terraform per il deployment dell'API si trovano nella cartella `common/terraform`. Puoi usarli in una configurazione Terraform:
...
module "my_api" {
  ...
  additional_iam_policy_statements = [
    # Aggiungi eventuali permessi aggiuntivi richiesti dall'API
  ]
...
1. Una funzione AWS Lambda che gestisce tutte le procedure tRPC
2. Un'API Gateway HTTP/REST API come trigger della funzione
3. Ruoli e permessi IAM
4. Un log group CloudWatch
...
Puoi [aggiungere un commento di soppressione](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html) se desideri che l'API rimanga pubblica.
:::
</Fragment>
</Infrastructure>
...
Se hai selezionato l'autenticazione `IAM`, puoi concedere accesso all'API:
...
Gli output chiave del modulo API utilizzabili per le politiche IAM sono:
...
Questo ricaricherà automaticamente le modifiche all'API.

## Invocazione dell'API tRPC

Puoi creare un client tRPC per invocare l'API in modo type-safe. Se stai chiamando l'API tRPC da un altro backend, puoi usare il client in `src/client/index.ts`, ad esempio:
...
Se stai chiamando l'API da un sito React, considera l'uso del generatore <Link path="guides/api-connection/react-trpc">Connessione API</Link> per configurare il client.
