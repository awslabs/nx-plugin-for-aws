...
:::tip
L'opzione `integrationStyle` ha come valore predefinito `Individual Functions`, che crea un Lambda per ogni procedura tRPC. Seleziona `Router` se preferisci un singolo Lambda handler condiviso per l'intera API.
:::
...
### Scelta di uno Stile di Integrazione

Il generatore tRPC supporta due pattern infrastrutturali tramite l'opzione `integrationStyle`:

- `Individual Functions` è il valore predefinito. Genera una funzione Lambda per procedura.
- `Router` genera una singola funzione Lambda condivisa per l'intera API e la riutilizza per ogni integrazione.

`Individual Functions` ti fornisce un'infrastruttura più granulare e rende più facile associare permessi o configurazioni diverse per operazione. `Router` riduce la proliferazione di Lambda e integrazioni, il che può essere utile per API più piccole o quando preferisci un singolo handler centrale.

Entrambi gli stili di integrazione sono attualmente supportati per `ServerlessApiGatewayRestApi` e `ServerlessApiGatewayHttpApi`.
