...
I target di deploy dipendono da `assemble` piuttosto che da `build`, quindi il deploy costruisce solo ciò che sta per deployare. `build` rimane invariato e fa ancora tutto, quindi rimane ciò che esegui per controllare un progetto — e ciò che dovresti eseguire in CI.
