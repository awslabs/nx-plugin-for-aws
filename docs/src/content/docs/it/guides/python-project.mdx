...
L'opzione `type` seleziona se ottenere un'applicazione (un progetto distribuibile, ad esempio una funzione lambda o un'immagine container) o una libreria (codice riutilizzabile, importato da altri progetti). Il valore predefinito è `application`, quindi passa `--type=library` per una libreria:
