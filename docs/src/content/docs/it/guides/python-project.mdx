...
Il tuo progetto ha anche un target `package`, che è il fratello solo-artefatti di `build`: produce tutto ciò che il tuo progetto contribuisce a un deployment (il suo output compilato e qualsiasi bundle, immagine container o specifica generata che dichiara) senza eseguire i controlli di lint, test o type-check.

<NxCommands commands={['package <project-name>']} />

I <Link path="guides/typescript-infrastructure#deploying-to-aws">target di deploy</Link> dipendono da `package` piuttosto che da `build`, quindi il deploy costruisce solo ciò che sta per distribuire. `build` rimane invariato e fa ancora tutto, quindi rimane ciò che esegui per controllare un progetto — e ciò che dovresti eseguire in CI.

:::caution
Poiché `package` salta i controlli di qualità, un deploy non fallisce più per un errore di lint, un test unitario fallito o un errore di tipo. Esegui `build` (o la tua pipeline CI) per controllare il tuo codice.
:::
