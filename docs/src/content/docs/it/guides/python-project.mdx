...
I <Link path="guides/typescript-infrastructure#deploying-to-aws">target di deploy</Link> dipendono da `assemble` piuttosto che da `build`, quindi il deploy costruisce solo ciò che sta per distribuire. `build` rimane invariato e fa ancora tutto, quindi rimane ciò che esegui per controllare un progetto — e ciò che dovresti eseguire in CI.

:::caution
Poiché `assemble` salta i controlli di qualità, un deploy non fallisce più per un errore di lint, un test unitario fallito o un errore di tipo. Esegui `build` (o la tua pipeline CI) per controllare il tuo codice.
:::
