...
- **Formatta ciò che scrivi.** `updateJson` e `writeJson` ri-serializzano un intero file, espandendo gli array inline in una forma che il target `format` fornito rifiuta — lasciando l'utente con una build fallita dopo un aggiornamento altrimenti pulito. Termina con `formatFilesInSubtree(tree)` in modo che il diff rimanga limitato alle righe che intendevi modificare.
