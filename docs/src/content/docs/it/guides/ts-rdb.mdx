...
<FileTree>
  ...
  - scripts
    - pull-image.ts Scarica l'immagine container del database per lo sviluppo locale
...
Fermare `serve-local` (ad esempio con `Ctrl+C`) rimuove automaticamente il container del database locale, ma preserva il volume nominato in modo che i tuoi dati persistano tra un riavvio e l'altro.

:::caution[Windows]
A causa delle limitazioni nella gestione dei segnali su Windows, il container non viene rimosso automaticamente quando `serve-local` viene fermato. Dovrai rimuoverlo manualmente:
...
Per tipi di progetto specifici, utilizza il generatore `connection` per collegare automaticamente il database — gestisce import, iniezione del contesto e dipendenze di sviluppo locale:

- <Link path="guides/connection/trpc-rdb">API tRPC → Database Relazionale</Link>
- <Link path="guides/connection/smithy-rdb">API Smithy → Database Relazionale</Link>
- <Link path="guides/connection/ts-agent-rdb">Agente TypeScript Strands → Database Relazionale</Link>
- <Link path="guides/connection/ts-mcp-server-rdb">Server MCP → Database Relazionale</Link>
...
L'immagine del database locale è configurata nel target `serve-local` del progetto database generato in `project.json`. Aggiorna l'argomento dell'immagine passato a `scripts/start-container.ts` quando modifichi le versioni del motore.
