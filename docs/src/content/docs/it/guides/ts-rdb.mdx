...
Il cluster Aurora esporta i log del database in CloudWatch Logs (`postgresql` per PostgreSQL, `error` e `slowquery` per MySQL) e abilita Performance Insights sull'istanza writer (crittografata con la chiave KMS del cluster) in modo che le query siano osservabili per la risoluzione dei problemi e l'auditing. Si noti che CloudWatch Logs e Performance Insights comportano costi aggiuntivi.
