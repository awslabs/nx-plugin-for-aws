...
Genera un server [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) in TypeScript per fornire contesto ai Modelli Linguistici di Grandi Dimensioni (LLMs), con opzione di deploy su [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
  ...
  - your-project/
    - src/
      - mcp-server/ (o nome personalizzato se specificato)
        ...
        - Dockerfile Punto d'ingresso per l'hosting del server MCP (escluso quando `computeType` è impostato su `None`)
...
:::caution
L'SDK MCP supporta solo Zod v3 al momento. Per consentire l'uso di Zod v4 in altri generatori, la dipendenza su Zod v3 è installata come `zod-v3`. Per gli schemi degli strumenti MCP, dovrai importare Zod come segue:
...
- Costruisce un'immagine docker dal `Dockerfile` che esegue questo server in bundle sulla porta `8000`, come da [contratto del protocollo MCP](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html#mcp-protocol-contract)

:::tip
L'immagine docker è costruita usando un tag (ad esempio `my-scope-my-project-mcp-server:latest`), che il `Dockerfile` del costrutto CDK eredita, permettendo al tuo `Dockerfile` di essere co-locato con il progetto del server MCP.
:::
