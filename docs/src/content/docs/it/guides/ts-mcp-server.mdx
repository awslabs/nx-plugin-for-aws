...
<FileTree>
  - packages/\<name>/
    - README.md Documentazione per il server MCP con istruzioni per l'uso
...
Per usare il tuo server MCP con assistenti AI, devi prima crearne il bundle:
...
La maggior parte degli assistenti AI che supportano MCP utilizza un approccio simile alla configurazione. Dovrai creare o aggiornare un file di configurazione con i dettagli del tuo server MCP:
...
:::caution
Se ricevi un errore come `ENOENT node` durante la connessione al server, potresti dover specificare il percorso completo di `node`, ottenibile eseguendo `which node` nel terminale.
:::
