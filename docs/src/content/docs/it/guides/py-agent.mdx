...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
Per impostazione predefinita, gli agenti Strands utilizzano Claude Sonnet 4.6 su Amazon Bedrock, ma puoi personalizzare il provider del modello. Consulta la [documentazione Strands sui provider di modelli](https://strandsagents.com/docs/user-guide/concepts/model-providers/) per le opzioni di configurazione:
...
<Snippet name="agent/dev-vs-serve" parentHeading="Sviluppo Locale" />
...
- `scripts/<your_agent_name>_openapi.py` — un piccolo script che esporta la specifica OpenAPI dell'agente (denominato con il nome del tuo agente in `snake_case`)
...
:::caution[Usa `-dev`, non `-serve`]
Usa `<your-agent-name>-dev` per questo, non `<your-agent-name>-serve`. `-serve` esegue il percorso del codice distribuito, quindi necessita di `RUNTIME_CONFIG_APP_ID` impostato — vedi <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Senza di esso, lo streaming è già iniziato quando l'agente tenta di risolvere il suo storage di sessione, quindi la richiesta restituisce un `200` vuoto senza errore e il fallimento appare solo nel log del server.
:::
