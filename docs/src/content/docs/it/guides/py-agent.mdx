...
- **Strands**: avvolge il tuo `Agent` in un `ag_ui_strands.StrandsAgent`, costruito all'interno di un gestore `lifespan` di FastAPI (così la costruzione avviene all'avvio del container/sessione piuttosto che al momento dell'importazione), e servito da un ciclo FastAPI `/invocations` scritto manualmente.
- **LangChain**: avvolge il grafo compilato in un `ag_ui_langgraph.LangGraphAgent`, costruito allo stesso modo all'interno di `lifespan`, e servito da un ciclo FastAPI `/invocations` scritto manualmente.
...
Questo usa `uv run` per eseguire il tuo Agent utilizzando il [Bedrock AgentCore Python SDK](https://github.com/aws/bedrock-agentcore-sdk-python).
...
<Snippet name="agent/securing-your-agent" parentHeading="Proteggere il tuo Agente" />
...
Consulta la guida [Guardrails](https://strandsagents.com/docs/user-guide/safety-security/guardrails/) di Strands per maggiori dettagli.
</TabItem>
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
```python
# agent.py
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Posiziona un agent dietro un AgentCore Gateway come target di runtime"
