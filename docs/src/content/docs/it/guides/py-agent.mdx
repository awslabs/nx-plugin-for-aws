...
Poiché l'adattatore `ag-ui-strands` di AG-UI clona un `Agent` template per thread di conversazione, `get_session_manager` viene collegato come `session_manager_provider` sul `StrandsAgentConfig` passato a `StrandsAgent` in `main.py`, quindi un nuovo `SessionManager` viene risolto per ogni thread piuttosto che essere incorporato nell'agente template condiviso.
</OptionFilter>

<OptionFilter when={{ protocol: ['http', 'a2a'] }} description="HTTP/A2A session wiring">
Poiché `with_session_id` già memorizza nella cache un'istanza `Agent` per sessione, `get_session_manager` viene chiamato direttamente all'interno della chiamata `Agent(session_manager=get_session_manager())` di `get_agent` in `agent.py`.
</OptionFilter>
...
- **`s3`**: L'agente distribuito utilizza un `S3CheckpointSaver` con il bucket di sessione fornito, memorizzando checkpoint e scritture in sospeso sotto il prefisso `checkpoints/`. Questa classe (adattata da [`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3), con licenza MIT) si trova in `s3_checkpoint_saver_langchain.py` nel progetto di connessione agente condiviso.
...
- **`in-memory`** (predefinito): Nessuna tabella o bucket viene fornito. Lo stato della conversazione viene mantenuto in memoria solo per la durata del processo in esecuzione e non sopravvive ai riavvii o allo scale-in.
