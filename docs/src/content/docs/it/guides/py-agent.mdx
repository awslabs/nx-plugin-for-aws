...
- **A2A**: Un server [Agent-to-Agent](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) montato su un'app FastAPI (Strands utilizza lo [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent); LangChain utilizza l'[`a2a-sdk`](https://a2a-protocol.org/) agnostico al framework). Ideale quando il tuo agente deve essere individuabile e invocabile da altri agenti compatibili A2A.
...
Il punto d'ingresso del server differisce per framework (Strands produce un `Agent` gestito tramite contesto, mentre LangChain guida un grafo `create_agent` compilato), ma il contratto esterno per ogni protocollo è lo stesso.
...
Il `main.py` generato monta un server A2A su un'app FastAPI padre che espone anche `/ping`. Gli agenti Strands utilizzano lo Strands `A2AServer`; gli agenti LangChain avvolgono il grafo compilato in un `AgentExecutor` [`a2a-sdk`](https://a2a-protocol.org/). Quando distribuito su AgentCore, il punto d'ingresso risolve l'ARN pubblico del runtime da AppConfig e lo pubblicizza nella scheda dell'agente.
