...
- **`s3`** (predefinito): L'agente distribuito utilizza un `S3CheckpointSaver` con il bucket di sessione fornito, memorizzando checkpoint e scritture in sospeso sotto il prefisso `checkpoints/`. Questa classe (adattata da [`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3), con licenza MIT) si trova in `s3_checkpoint_saver_langchain.py` nel progetto di connessione agente condiviso.
