...
L'entry point espone il tuo agente tramite il protocollo A2A (Strands utilizza il [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent); LangChain avvolge il grafo in un executor [`a2a-sdk`](https://a2a-protocol.org/)), montato su un'app FastAPI:
...
Il protocollo server del tuo agente determina come comunica. Tutte le opzioni sono servite da [FastAPI](https://fastapi.tiangolo.com/) — l'entry point differisce:
...
- **AG-UI**: Il [protocollo AG-UI](https://docs.ag-ui.com/) su SSE (Strands utilizza `ag-ui-strands`; LangChain utilizza `ag-ui-langgraph`). Ideale per l'integrazione diretta con il frontend tramite [CopilotKit](https://docs.copilotkit.ai/) in un sito web React.

L'entry point del server differisce per framework (Strands produce un `Agent` gestito dal contesto, mentre LangChain guida un grafo `create_agent` compilato), ma il contratto esterno per ogni protocollo è lo stesso.
...
Poiché il generatore fornisce infrastruttura CDK o Terraform che gestisce la distribuzione del tuo agente, non è necessario utilizzare il `bedrock-agentcore-starter-toolkit` menzionato nella documentazione per distribuire il tuo agente.
:::

<OptionFilter when={{ protocol: 'a2a' }} description="A2A server details">
## Server A2A (protocollo A2A)

Il `main.py` generato monta un server A2A su un'app FastAPI padre che espone anche `/ping`. Gli agenti Strands utilizzano lo Strands `A2AServer`; gli agenti LangChain avvolgono il grafo compilato in un `AgentExecutor` [`a2a-sdk`](https://a2a-protocol.org/). Quando distribuito su AgentCore, l'entry point risolve l'ARN pubblico del runtime da AppConfig e lo pubblicizza nella scheda dell'agente.

La maggior parte degli utenti non avrà bisogno di modificare questo file; modifica `agent.py` per cambiare gli strumenti o il prompt di sistema. Il server A2A popola la scheda dell'agente (`/.well-known/agent-card.json`) dal `name` e dalla `description` dell'agente.
</OptionFilter>
...
- **Strands**: avvolge il tuo `Agent` in un `ag_ui_strands.StrandsAgent`, costruito all'interno di un handler `lifespan` di FastAPI (in modo che la costruzione avvenga all'avvio del container/sessione piuttosto che al momento dell'importazione), e servito da un ciclo `/invocations` FastAPI scritto a mano.
...
La maggior parte degli utenti non avrà bisogno di modificare questo file — modifica `agent.py` per cambiare gli strumenti o il prompt di sistema.
...
Viene anche aggiunto un target `docker` specifico per il tuo Agent, che copia il `Dockerfile` e gli artefatti del bundle in una directory di contesto docker. Questo co-localizza il `Dockerfile` con l'output costruito, consentendo a CDK di costruire l'immagine Docker direttamente utilizzando `AgentRuntimeArtifact.fromAsset`.
...
L'argomento `-N` dato a `curl` disabilita il buffering del flusso di output, così puoi vedere la risposta in streaming in tempo reale.
:::
...
Consulta la documentazione di [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse) per i campi di `guardrail_config`.
</TabItem>
</Tabs>
