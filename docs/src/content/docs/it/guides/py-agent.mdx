...
Gli esempi seguenti utilizzano Strands. Un agente LangChain funziona allo stesso modo, utilizzando `create_agent` con il decoratore `@tool` da `langchain_core.tools` e un modello `ChatBedrockConverse` (le variabili d'ambiente `MODEL_ID` e `AWS_REGION` impostano il modello e la regione). Consulta la [documentazione LangChain](https://docs.langchain.com/oss/python/langchain/overview) per i dettagli.
