...
Definisci lo strumento, quindi aggiungilo alla lista `tools` all'interno di `get_agent()`:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
Strands fornisce una collezione di strumenti pre-costruiti tramite il pacchetto `strands-agents-tools`, che il generatore aggiunge già al `pyproject.toml` del tuo progetto. Importa gli strumenti che desideri e aggiungili a `get_agent()`:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain fornisce un ampio ecosistema di [strumenti e integrazioni](https://docs.langchain.com/oss/python/integrations/tools), ciascuno distribuito nel proprio pacchetto. Il generatore non li aggiunge, quindi installa prima quello di cui hai bisogno — ad esempio `langchain-community`, da cui proviene lo strumento di ricerca qui sotto:

<NxCommands commands={['run <project-name>:add langchain-community']} />

Quindi importa gli strumenti e aggiungili a `get_agent()`:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
L'agente generato utilizza il modello Strands predefinito su Amazon Bedrock. Per configurarlo, passa un `model` all'`Agent`. Consulta la [documentazione Strands sui provider di modelli](https://strandsagents.com/docs/user-guide/concepts/model-providers/) per i provider disponibili e le loro opzioni:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
Gli agenti LangChain utilizzano un modello [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse). L'agente generato legge l'id del modello e la regione dalle variabili d'ambiente `MODEL_ID` e `AWS_REGION`, che l'infrastruttura imposta per l'agente distribuito. Per configurare ulteriormente il modello, aggiungi argomenti dove viene costruito in `get_agent()`:
...
Tutti i protocolli espongono `/ping` per il contratto di controllo dello stato del runtime AgentCore. Gli agenti A2A ascoltano sulla porta `9000`; gli agenti HTTP e AG-UI ascoltano sulla porta `8080`. L'infrastruttura generata è configurata per te.
...
L'agente ascolta su una porta assegnata dal pool del workspace quando è stato generato. Leggila da `metadata.ports` nel `project.json` del progetto, o dal flag `--port` nel comando del target `<your-agent-name>-dev`. Gli esempi seguenti utilizzano `8081`, la porta assegnata a un primo agente HTTP in un workspace nuovo.

Viene anche generato un target `<your-agent-name>-serve`, che esegue l'agente contro la tua infrastruttura distribuita e quindi richiede che `RUNTIME_CONFIG_APP_ID` sia impostato. Consulta la guida <Link path="guides/local-development">Sviluppo Locale</Link> per la differenza tra `dev` e `serve`.
...
- `scripts/<your_agent_name>_openapi.py` — un piccolo script che esporta la specifica OpenAPI dell'agente (nominato con il nome del tuo agente in `snake_case`)
...
Viene anche aggiunto un target `<your-agent-name>-package`, che assembla il pacchetto di codice distribuibile: il bundle di dipendenze `aarch64`, l'albero dei moduli Python e un punto di ingresso `main.py` radice. L'infrastruttura generata carica questa directory come `.zip` — tramite `AgentRuntimeArtifact.fromCodeAsset` sotto CDK, o archiviata nel bucket di asset condiviso sotto Terraform.
</OptionFilter>
...
Il tuo agente è automaticamente configurato con l'osservabilità utilizzando l'[AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT).
...
Avvia il tuo agente con il target `<your-agent-name>-dev`:

<NxCommands commands={['agent-dev your-project']} />

Quindi invia una richiesta POST a `/invocations` sulla porta su cui è in esecuzione il tuo agente locale. Sostituisci la porta assegnata al tuo agente — leggila da `metadata.ports` nel `project.json` del progetto, o dal flag `--port` nel comando del target `<your-agent-name>-dev`. Un primo agente HTTP in un workspace nuovo è assegnato a `8081`:
