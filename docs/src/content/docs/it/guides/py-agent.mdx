...
Per gli agenti Strands, il generatore utilizza [FastAPI](https://fastapi.tiangolo.com/) per esporre un server HTTP per impostazione predefinita. In alternativa, puoi scegliere il protocollo [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) per l'interoperabilità con altri agenti compatibili A2A, o il protocollo [AG-UI](https://docs.ag-ui.com/) per l'integrazione diretta con il frontend tramite [CopilotKit](https://docs.copilotkit.ai/).

:::note[LangChain supporta solo AG-UI]
Il framework `langchain` attualmente supporta solo il protocollo `ag-ui`. Scegli il framework predefinito `strands` se hai bisogno del protocollo `http` o `a2a`. Gli agenti LangChain possono comunque connettersi a server MCP, Gateway AgentCore e agenti A2A tramite il <Link path="/guides/connection/py-agent-mcp">generatore `connection`</Link>.
:::
...
Gli esempi seguenti utilizzano Strands. Un agente LangChain funziona allo stesso modo, utilizzando `create_agent` con il decoratore `@tool` da `langchain_core.tools` e un modello `ChatBedrockConverse` (le variabili d'ambiente `MODEL_ID` e `AWS_REGION` impostano il modello e la regione). Vedi la [documentazione LangChain](https://docs.langchain.com/oss/python/langchain/overview) per i dettagli.
...
Per utilizzare server MCP creati con i generatori <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> o <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> puoi utilizzare il <Link path="/guides/connection/py-agent-mcp">generatore `connection`</Link>.
...
Per una guida più dettagliata sulla scrittura di agenti Strands, consulta la [documentazione Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Per gli agenti LangChain, il <Link path="/guides/connection/py-agent-mcp">generatore `connection`</Link> collega anche strumenti di server MCP, Gateway AgentCore e agenti A2A in `create_agent`.
