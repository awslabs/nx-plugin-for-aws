...
Se vuoi che ogni utente raggiunga solo le proprie conversazioni, registra chi possiede ogni sessione nella tua applicazione e verifica tale proprietà all'ingresso. Un ID di sessione è quindi qualcosa che la tua applicazione emette piuttosto che qualcosa che il chiamante sceglie.

**1. Creare sessioni tramite la tua API**

Aggiungi una procedura che avvia una conversazione. Genera l'ID di sessione stesso, risolve il chiamante dal proprio contesto di richiesta e memorizza la coppia — ad esempio in una tabella creata con il generatore <Link path="guides/py-dynamodb">`py#dynamodb`</Link>:

| Attribute | Example | Notes |
| --- | --- | --- |
| `sessionId` | `a5f1…` | Chiave di partizione — l'agente cerca un proprietario dall'ID di sessione che gli viene fornito |
...
L'API può essere in qualsiasi linguaggio; l'esempio sopra utilizza il generatore <Link path="guides/trpc">`ts#trpc-api`</Link>.

Risolvi il chiamante dal contesto di richiesta verificato piuttosto che da un campo di input, in modo che non possa essere fornito dal browser. <Link path="guides/trpc">Implementing Custom Middleware</Link> mostra come farlo sia per le API autenticate con IAM che con Cognito.

Una seconda procedura che interroga quell'indice consente all'UI di offrire a un utente le proprie conversazioni senza mai vedere quelle di nessun altro.

**2. Validare la proprietà nell'agente**

Il tuo agente riceve già l'ID di sessione e con `auth=cognito` riceve anche il JWT del chiamante — l'autorizzatore AgentCore Runtime lo valida e l'infrastruttura generata consente l'header `Authorization` fino al tuo codice. Cerca la sessione, confronta il suo proprietario con il chiamante e rifiuta la richiesta se differiscono:
...
Decodificare il claim senza verificarne la firma è sufficiente qui, poiché AgentCore Runtime ha validato il token durante l'autorizzazione in ingresso.

:::tip[Perché validare nell'agente oltre che nell'API]
L'API decide chi possiede una sessione; l'agente la applica. Controllare in entrambi i posti significa che un ID di sessione che trapela — copiato da un browser o riutilizzato da una vecchia conversazione — viene comunque rifiutato alla prossima invocazione, e qualsiasi futuro client dell'agente eredita la stessa regola.
:::

:::caution[La sessione seleziona anche la microVM]
Un ID di sessione seleziona la [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) che serve la richiesta, quindi il suo filesystem, `/tmp` e le cache in-process sono condivise da ogni richiesta che si risolve in quella sessione. Questo merita attenzione se il tuo agente o i suoi strumenti scrivono sul disco locale: due chiamanti che inviano lo stesso ID di sessione condividono quelle risorse anche quando le loro conversazioni memorizzate sono separate.

Generare ID di sessione opachi nella tua API evita questo, perché un chiamante non può atterrare su una sessione che non gli è stata data. Derivarne uno da valori forniti dall'utente — un nome di conversazione, un'email, un nome di gioco — è prevedibile e consente a due chiamanti di collidere.
:::
