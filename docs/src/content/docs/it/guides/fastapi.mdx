...
Il generatore configura il logging strutturato usando AWS Lambda Powertools. Puoi accedere al logger nei gestori delle route:
...
def read_item(item_id: int):
    if item_id < 0:
        raise HTTPException(status_code=400, detail="Item ID deve essere positivo")
    return {"item_id": item_id}
```
...
Dopo aver aggiornato l'infrastruttura per supportare lo streaming, puoi implementare un'API in streaming in FastAPI. L'API dovrebbe:
...
:::note
Se è stata selezionata l'autenticazione `Cognito`, bisogna fornire la proprietà `identity` al costrutto API:
...
:::note
È necessario eseguire un build dopo ogni modifica all'API per mantenere aggiornati i tipi usati dal costrutto CDK.
...
Se è stata selezionata l'autenticazione `IAM`, puoi usare il metodo `grantInvokeAccess` per concedere l'accesso:
