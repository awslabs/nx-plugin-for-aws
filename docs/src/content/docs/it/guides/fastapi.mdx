...
I nomi dei progetti Python utilizzano underscore, quindi un'API generata con `--name=MyApi` viene indirizzata come `my_api` (o con il suo nome completo, `<scope>.my_api`). La porta su cui il server è in ascolto viene assegnata per progetto — leggila dal target `serve` nel `project.json` del tuo progetto.
:::
