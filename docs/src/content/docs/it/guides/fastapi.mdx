...
:::caution[Fissa `boto3` alla versione sopra indicata]
Altri generatori aggiungono `boto3` fissato esattamente a questa versione. Poiché il tuo workspace condivide un singolo ambiente virtuale UV e lockfile, aggiungerlo senza fissarlo (`add boto3`) risolve l'ultima release e scrive un vincolo `boto3>=<latest>`, che `uv` non può soddisfare insieme al pin. Tutti i target Python nel workspace quindi falliscono nel risolvere le dipendenze.
:::
