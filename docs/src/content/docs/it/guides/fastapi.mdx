...
:::tip
Si raccomanda di specificare i modelli di risposta per le operazioni API per una migliore generazione del codice se si utilizza il generatore `api-connection`. <Link path="guides/api-connection/react-fastapi#errors">Vedi qui per dettagli</Link>.
:::
...
resource "aws_lambda_function" "my_api_handler" {
  ...
  # Aggiunge il layer Lambda Web Adapter
  layers = [
...
# CloudWatch Log Group per la funzione Lambda
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/my-api-handler"
  retention_in_days = 14
}

# IAM role per l'esecuzione Lambda
...
# Allega la policy di esecuzione base
resource "aws_iam_role_policy_attachment" "lambda_logs" {
  role       = aws_iam_role.lambda_execution_role.name
  policy_arn = "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"
}

# Allega la policy per X-Ray
...
# Output dell'URL della Function
output "my_api_url" {
...
# IAM policy per concedere l'accesso di invocazione alla Function URL
resource "aws_iam_policy" "my_api_invoke_policy" {
  name        = "my-api-invoke-policy"
  description = "Policy per consentire l'invocazione della Lambda Function URL in streaming"
...
# Esempio: Allega la policy a un ruolo (scommenta e modifica se necessario)
...
<Fragment slot="terraform">
I moduli Terraform per distribuire l'API si trovano nella cartella `common/terraform`. Puoi usarli in una configurazione Terraform:
...
:::tip
Se stai lavorando attivamente sia sull'infrastruttura CDK che su FastAPI, puoi usare [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) per rigenerare questi tipi ad ogni modifica all'API:
...
Per invocare la tua API da un sito React, puoi usare il generatore <Link path="guides/api-connection/react-fastapi">`api-connection`</Link>.