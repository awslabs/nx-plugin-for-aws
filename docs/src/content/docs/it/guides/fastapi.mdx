...
Il generatore creerà anche infrastruttura come codice CDK o Terraform utilizzabile per deployare l'API, residente nella directory `packages/common`.

## Implementazione della tua FastAPI

L'implementazione principale dell'API è in `main.py`. Qui definisci le route API e le relative implementazioni. Ecco un esempio:
...
Le metriche CloudWatch sono raccolte automaticamente per ogni richiesta. Puoi aggiungere metriche personalizzate:
...
Poiché AWS API Gateway non supporta risposte in streaming, dovrai deployare la tua FastAPI su una piattaforma che lo supporti. L'opzione più semplice è usare un AWS Lambda Function URL.

<Infrastructure>
<Fragment slot="cdk">
Per ottenere questo, puoi sostituire il costrutto generato `common/constructs/src/app/apis/<name>-api.ts` con uno che deploya un Function URL.
...
export class MyApi extends Construct {
  ...
  constructor(scope: Construct, id: string) {
    ...
    // Registra l'URL API nella configurazione runtime per il client
...
Per ottenere questo con Terraform, puoi sostituire l'infrastruttura API Gateway generata con un Lambda Function URL che supporti lo streaming.
...
resource "aws_lambda_function" "my_api_handler" {
  ...
  # Aggiunge layer Lambda Web Adapter
  layers = [
...
# Gruppo di log CloudWatch per Lambda
resource "aws_cloudwatch_log_group" "lambda_logs" {
  name              = "/aws/lambda/my-api-handler"
  retention_in_days = 14
}

# Ruolo IAM per esecuzione Lambda
...
# Lambda Function URL con supporto streaming
...
# Output dell'URL Function
output "my_api_url" {
  description = "URL per la FastAPI in streaming su Lambda"
  value       = aws_lambda_function_url.my_api_url.function_url
}

# Opzionale: Crea parametro SSM per configurazione runtime
...
# Policy IAM per concedere accesso invoke al Function URL
resource "aws_iam_policy" "my_api_invoke_policy" {
  name        = "my-api-invoke-policy"
  description = "Policy per consentire l'invoke del Lambda Function URL in streaming"
...
# Esempio: Allega la policy invoke a un ruolo (scommenta e modifica se necessario)
...
- Aggiungere l'estensione vendor OpenAPI `x-streaming: true` se intendi usare <Link path="guides/api-connection/react-fastapi">API Connection</Link>.

Ad esempio, per inviare una serie di oggetti JSON in streaming:
...
Per consumare uno stream di risposte, puoi usare il <Link path="guides/api-connection/react-fastapi#consuming-a-stream">Generatore API Connection</Link> che fornisce un metodo type-safe per iterare sui chunk.

## Deploy della tua FastAPI

Il generatore FastAPI crea infrastruttura come codice CDK o Terraform in base al `iacProvider` selezionato. Puoi usarlo per deployare la tua API.

<Infrastructure>
<Fragment slot="cdk">
Il costrutto CDK per deployare l'API si trova in `common/constructs`. Puoi usarlo in un'applicazione CDK:
...
<Fragment slot="terraform">
I moduli Terraform per deployare l'API sono in `common/terraform`. Puoi usarli in una configurazione Terraform:

```hcl {2}
module "my_api" {
  source = "../../common/terraform/src/app/apis/my-api"

  # Variabili d'ambiente per Lambda
  env = {
  ...
  additional_iam_policy_statements = [
    # Aggiungi permessi aggiuntivi per l'API
  ]
...
1. Funzione AWS Lambda che serve tutte le route FastAPI
2. API Gateway HTTP/REST API come trigger
3. Ruoli e permessi IAM
4. Gruppo di log CloudWatch
5. Configurazione tracciamento X-Ray
...
:::note
Se hai selezionato autenticazione `Cognito`, devi fornire la configurazione:
...
Puoi configurare User Pool e Client Cognito usando le risorse/moduli Terraform appropriati.
:::

Il modulo Terraform fornisce diversi output utili:
...
# Dettagli funzione Lambda
output "lambda_function_name" {
  value = module.my_api.lambda_function_name
}

# Ruolo IAM per permessi aggiuntivi
...
:::caution
Se hai selezionato `None` per `auth`, potresti vedere errori Checkov come:
...
Puoi [aggiungere un commento di soppressione](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html) se vuoi che l'API sia pubblica.
:::
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
Poiché le operazioni in FastAPI sono definite in Python e l'infrastruttura CDK in TypeScript, utilizziamo code-generation per fornire metadati al costrutto CDK e garantire un'interfaccia type-safe.

Un target `generate:<ApiName>-metadata` è aggiunto al `project.json` dei costrutti comuni per generare file come `packages/common/constructs/src/generated/my-api/metadata.gen.ts`. Essendo generato al build time, è ignorato dal version control.

:::note
Devi eseguire un build dopo modifiche all'API per aggiornare i tipi nel costrutto CDK.

<NxCommands commands={["run-many --target build --all"]} />
:::

:::tip
Se lavori sia su infrastruttura CDK che FastAPI, puoi usare [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) per rigenerare i tipi automaticamente:
...
<Fragment slot="terraform">
:::note
Non supportiamo integrazioni type-safe per Terraform, quindi nessun target di code-generation è configurato se hai selezionato Terraform come `iacProvider`.
:::
</Fragment>
</Infrastructure>
...
Se hai selezionato autenticazione `IAM`, puoi usare il metodo `grantInvokeAccess` per concedere accesso:
...
<Fragment slot="terraform">
```hcl
# Crea policy IAM per consentire invoke API
resource "aws_iam_policy" "api_invoke_policy" {
  name        = "MyApiInvokePolicy"
  description = "Policy per consentire l'invoke della FastAPI"
...
# Allega policy a un ruolo IAM (es. utenti autenticati)
resource "aws_iam_role_policy_attachment" "api_invoke_access" {
  role       = aws_iam_role.authenticated_user_role.name
  policy_arn = aws_iam_policy.api_invoke_policy.arn
}

# O allega a un ruolo esistente
...
Gli output principali del modulo API per policy IAM sono:

- `module.my_api.api_execution_arn` - Per permessi execute-api:Invoke
- `module.my_api.api_arn` - ARN API Gateway
- `module.my_api.lambda_function_arn` - ARN funzione Lambda
</Fragment>
</Infrastructure>
...
Avvia un server di sviluppo FastAPI con:
...
Per invocare l'API da un sito React, puoi usare il <Link path="guides/api-connection/react-fastapi">generatore `api-connection`</Link>.