...
:::tip[Tipo di API]
Seleziona `rest-lambda` (predefinito) come `infra` se intendi implementare operazioni di streaming.
:::

:::tip[Pattern di integrazione]
L'opzione `integrationPattern` è impostata su `isolated` per impostazione predefinita, che crea una Lambda per ogni operazione FastAPI. Seleziona `shared` se preferisci un singolo handler Lambda condiviso per l'intera API, con override opzionali per operazione.
:::
...
Il generatore FastAPI crea infrastruttura come codice CDK o Terraform in base al `iac` selezionato. Puoi utilizzarlo per distribuire la tua FastAPI.
...
Non supportiamo integrazioni type-safe per Terraform, quindi nessun target di generazione codice viene configurato se hai selezionato Terraform come `iac`.
:::
</Fragment>
</Infrastructure>
