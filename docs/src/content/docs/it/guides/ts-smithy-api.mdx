...
Su macOS e Linux la CLI è risolta da [mise](https://mise.jdx.dev/), che la build recupera su richiesta, quindi non c'è nulla da installare — scarica e memorizza nella cache la versione fissata la prima volta che esegui la build. Su Windows la CLI è un prerequisito che installi tu stesso, e l'aggiornamento del workspace non la aggiorna — vedi <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> e <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> nella guida al progetto Smithy.
