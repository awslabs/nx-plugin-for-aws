...
Per invocare la tua API da un sito React, puoi usare il generatore <Link path="guides/api-connection/react-smithy">`api-connection`</Link>, che fornisce generazione type-safe del client dal tuo modello Smithy.