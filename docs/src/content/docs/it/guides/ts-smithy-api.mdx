...
Modelleremo il caso non autorizzato come un errore Smithy in modo che venga serializzato in una risposta `403` corretta. Aggiungilo al tuo modello, ad esempio in `model/src/operations/errors.smithy`, e referenzialo su qualsiasi operazione che richiede l'identità:
...
Prima, esponi l'identità risolta sul contesto del servizio in `src/context.ts`. La forniamo come funzione in modo che l'`UnauthorizedError` venga lanciato dall'interno di un'operazione (dove l'SDK Server lo serializza in un `403`), piuttosto che dall'handler:
...
Non supportiamo integrazioni type-safe per Terraform, quindi nessun target di generazione codice è configurato se hai selezionato Terraform come `iacProvider`.
:::
</Fragment>
</Infrastructure>
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Connetti un'API Smithy a un database relazionale Aurora"
  ...
  <ConnectionCard
    ...
    description="Connetti un'API Smithy a una tabella DynamoDB"
