...
La CLI è risolta da [mise](https://mise.jdx.dev/), aggiunto al tuo workspace come dipendenza di sviluppo, quindi non c'è nulla da installare — scarica e memorizza nella cache la versione fissata la prima volta che esegui la build.

:::caution[Windows]
`mise` non pubblica alcun pacchetto Windows su npm, quindi segui la [guida all'installazione della Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) per installarla manualmente prima di eseguire la build.
:::

:::tip[Building in CI]
`mise` legge i metadati di rilascio della CLI dall'API di GitHub, il cui limite di velocità non autenticato è condiviso per indirizzo IP e si esaurisce facilmente su un runner CI — il download fallisce quindi con `403 Forbidden`. Imposta `GITHUB_TOKEN` nel tuo ambiente di build per rimuovere questo limite. Le build successive utilizzano la cache locale e non effettuano alcuna richiesta.
:::
