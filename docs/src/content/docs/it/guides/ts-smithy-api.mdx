...
L'opzione `integrationPattern` è impostata di default su `isolated`, che crea un Lambda per ogni operazione Smithy. Seleziona `shared` se preferisci un singolo handler Lambda condiviso per l'intera API, con possibili override per singole operazioni.
:::
...
- **model/** Progetto del modello Smithy
  - package.json Manifesto di progetto che definisce il nome del pacchetto e le dipendenze del progetto
...
- **backend/** Implementazione TypeScript del backend
  - project.json Configurazione del progetto e target di build
...
Poiché questo generatore crea infrastruttura come codice in base al `iac` scelto, genererà un progetto in `packages/common` che include i costrutti CDK o i moduli Terraform rilevanti.
...
<FileTree>
  - packages/common/constructs
    - src
      - app/ Costrutti per infrastruttura specifica di un progetto/generatore
        - apis/
...
<FileTree>
  - packages/common/terraform
    - src
      - app/ Moduli Terraform per infrastruttura specifica di un progetto/generatore
        - apis/
          - \<project-name>/
...
:::note[Struttura cartelle personalizzata]
...
Se hai diverse API Smithy che condividono gli stessi tipi di dati, puoi definire quei tipi una sola volta in una libreria di forme anziché duplicarli in ogni modello. Una libreria di forme è un progetto Smithy senza servizio — solo forme riutilizzabili — su cui qualsiasi numero di progetti Smithy può dipendere.
...
Quando la tua API è protetta da autenticazione, le tue operazioni spesso devono sapere chi sta chiamando. L'approccio consigliato è risolvere l'identità del chiamante una volta nell'handler e passarla attraverso il [contesto del servizio](#service-context) per il consumo da parte di operazioni specifiche.

Modelleremo il caso non autorizzato come un errore Smithy in modo che venga serializzato in una risposta `403` appropriata. Aggiungilo al tuo modello, ad esempio in `model/src/operations/errors.smithy`, e fai riferimento ad esso su qualsiasi operazione che richiede l'identità:
...
Prima, esponi l'identità risolta sul contesto del servizio in `src/context.ts`. La forniamo come funzione in modo che `UnauthorizedError` venga lanciato dall'interno di un'operazione (dove l'SDK Server lo serializza in un `403`), piuttosto che dall'handler:
...
Con `auth: 'cognito'`, l'autorizzatore Cognito User Pools di API Gateway verifica il JWT che il chiamante fornisce nell'header `Authorization` e posiziona i claim verificati sull'evento in `event.requestContext.authorizer.claims`:
...
Ora possiamo usare l'identità risolta in un'operazione, ad esempio in `src/operations/echo.ts`:
...
Il progetto del modello Smithy utilizza la [Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/index.html) per costruire gli artefatti Smithy e generare l'SDK Server TypeScript:

<NxCommands commands={['build <model-project>']} />

Su macOS e Linux la CLI è risolta da [mise](https://mise.jdx.dev/), che la build recupera su richiesta, quindi non c'è nulla da installare — scarica e memorizza nella cache la versione fissata la prima volta che esegui la build.
...
Poiché la CLI è installata globalmente anziché fissata nel tuo workspace, l'aggiornamento del workspace con `nx migrate` **non** la aggiorna. Quando una migrazione sposta avanti le dipendenze Smithy, aggiorna tu stesso la tua Smithy CLI globale a una versione corrispondente.
:::
...
Un progetto generato su Windows effettua il commit di un target `compile` che invoca `smithy` direttamente, quindi chiunque altro ci lavori — inclusi utenti su macOS o Linux — ha bisogno della Smithy CLI nel proprio `PATH`. Per fare in modo che quelle macchine risolvano la CLI tramite `mise`, passa il target al comando `mise` come [descritto di seguito](#choosing-how-the-cli-is-resolved).

### Scegliere come viene risolta la CLI

macOS e Linux risolvono la CLI tramite `mise` e Windows usa una CLI installata globalmente, ma puoi scegliere una delle due opzioni su qualsiasi piattaforma modificando il comando del target `compile` nel `project.json` del progetto del modello.
...
:::tip[Ricaricamento a caldo]
Il server locale non solo ricarica automaticamente le modifiche TypeScript al codice backend, ma si riavvia anche quando modifichi il progetto del modello Smithy, permettendoti di iterare continuamente su modello e server insieme.
:::

## Distribuzione della tua API Smithy

Il generatore crea infrastruttura CDK o Terraform in base al `iac` selezionato.
...
Il costrutto `UserIdentity` può essere generato usando il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link>
:::
</OptionFilter>
</Fragment>
...
Il modulo API prepara il suo zip di distribuzione Lambda in un bucket S3 di asset condiviso — vedi la <Link path="/guides/terraform-project">guida all'infrastruttura Terraform</Link> per i dettagli. Istanzia il modulo `core/asset-bucket` una volta per distribuzione e passa il suo output `bucket_name` in ogni modulo API / Lambda tramite l'input `asset_bucket_name`:
...
:::note[Compilare prima del deploy]
...
Non supportiamo integrazioni type-safe per Terraform, quindi nessun target di generazione codice è configurato se hai selezionato Terraform come `iac`.
:::
</Fragment>
</Infrastructure>
