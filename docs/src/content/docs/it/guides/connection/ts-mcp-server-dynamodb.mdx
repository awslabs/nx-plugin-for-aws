...
description: Connetti un MCP Server TypeScript a una tabella DynamoDB
when:
...
Il generatore `connection` collega un <Link path="guides/ts-mcp-server">TypeScript MCP Server</Link> a un progetto <Link path="guides/ts-dynamodb">DynamoDB</Link>, configurando lo sviluppo locale in modo che entrambi si avviino insieme automaticamente.
...
Assicurati che il MCP server abbia la variabile d'ambiente `RUNTIME_CONFIG_APP_ID` impostata in modo che `resolveTableName()` possa cercare il nome della tabella a runtime.
