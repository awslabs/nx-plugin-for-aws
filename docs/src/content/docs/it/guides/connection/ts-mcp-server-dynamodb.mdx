...
description: Connetti un MCP Server TypeScript a un progetto DynamoDB TypeScript
when:
...
Il generatore `connection` collega un <Link path="guides/ts-mcp-server">MCP Server TypeScript</Link> a un progetto <Link path="guides/ts-dynamodb">DynamoDB TypeScript</Link>, configurando lo sviluppo locale in modo che entrambi si avviino automaticamente insieme.
...
Per consentire al MCP server di accedere alla tabella DynamoDB, concedi i permessi necessari nella tua infrastruttura.
...
`grantReadWriteData` concede sia i permessi DynamoDB che KMS al ruolo di esecuzione del MCP server.
</Fragment>
