...
:::caution
Quando `securityGroups` viene omesso, `RuntimeNetworkConfiguration.usingVpc` crea un security group con un id di costrutto fisso (`SecurityGroup`) sullo scope specificato. Passare un security group esplicito come mostrato evita collisioni di id di costrutto quando più runtime (ad esempio un agente e un server MCP) sono definiti nello stesso stack.
:::
