...
3. L'autenticazione Cognito aggiunta tramite il generatore <Link path="/guides/react-website-auth">`ts#website#auth`</Link> se ci si connette a un'API che utilizza autenticazione Cognito o IAM
...
Se stai lavorando contemporaneamente all'applicazione React e al FastAPI, utilizza il target `dev` dell'applicazione React (un alias per `serve-local`) che rigenererà automaticamente il client quando l'API cambia, oltre a effettuare l'hot-reload del sito e del server FastAPI locale:
...
:::warning[Dipendenza dal file watcher]
