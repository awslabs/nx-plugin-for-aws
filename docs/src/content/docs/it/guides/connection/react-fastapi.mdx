...
3. Cognito Auth aggiunto tramite il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link> se si connette a un'API che utilizza l'autenticazione Cognito o IAM
...
Se hai <Link path="/guides/fastapi#streaming">configurato il tuo FastAPI per lo streaming delle risposte</Link>, il tuo hook `useQuery` aggiornerà automaticamente i suoi dati man mano che arrivano nuovi chunk dello stream.
...
Se hai <Link path="/guides/fastapi#streaming">configurato il tuo FastAPI per lo streaming delle risposte</Link>>, il client generato includerà metodi type-safe per iterare in modo asincrono sui chunk nel tuo stream utilizzando la sintassi `for await`.
