...
- packages/api/src
  ...
  - local-server.ts client Prisma creato all'interno del request handler, passato a `serviceHandler.handle`
...
Il generatore applica la stessa iniezione del client Prisma all'interno del request handler in `local-server.ts`:
