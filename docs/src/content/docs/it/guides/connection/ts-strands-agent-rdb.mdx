...
Il client Prisma viene istanziato all'interno di `getAgent()`. Poiché il generatore `ts#strands-agent` configura un singolo Agent per sessione, il client viene anche riutilizzato per la durata della sessione.
