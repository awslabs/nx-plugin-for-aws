...
4. Autenticazione Cognito aggiunta tramite il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link> se si connette un'API che utilizza autenticazione Cognito o IAM
...
Le subscription sono supportate solo quando l'API tRPC utilizza `rest-lambda` (REST API) come tipo di compute. Le API Gateway HTTP APIs non supportano lo streaming delle risposte.
:::
...
Per informazioni su come definire procedure di subscription nel backend, consulta la <Link path="guides/trpc">guida al generatore di API tRPC</Link>.
...
Se la tua API tRPC utilizza autenticazione `Custom` (Lambda Authorizer), il provider del client generato include `headers` segnaposto dove devi aggiungere gli header di autorizzazione che il tuo authorizer si aspetta. Cerca i commenti `// TODO: Add headers required by your custom authorizer` nel file `<ApiName>ClientProvider.tsx` generato e sostituiscili con la logica del tuo token o chiave API.
