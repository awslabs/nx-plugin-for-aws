...
Se la tua API tRPC utilizza autenticazione `Custom` (Lambda Authorizer), il provider del client generato include `headers` segnaposto dove devi aggiungere gli header di autorizzazione richiesti dal tuo authorizer. Cerca i commenti `// TODO: Add headers required by your custom authorizer` nel file generato `<ApiName>ClientProvider.tsx` e sostituiscili con la logica del tuo token o chiave API.
