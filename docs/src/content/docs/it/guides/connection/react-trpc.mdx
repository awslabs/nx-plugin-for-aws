...
- **`ServerlessApiGatewayRestApi`** — link HTTP con `httpSubscriptionLink` su Server-Sent Events per le subscription tRPC.
- **`ServerlessApiGatewayHttpApi`** — link HTTP semplice (senza subscription).
- **`BedrockAgentCoreRuntimeWebSocket`** — link WebSocket verso l'URL runtime di AgentCore, con handshake firmato SigV4 (IAM) o sottoprotocollo JWT codificato in base64url (Cognito). L'ARN del runtime viene letto da `runtimeConfig.apis.<ApiName>` e convertito al volo in un URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws`.
