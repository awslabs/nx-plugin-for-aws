...
Il generatore ispeziona il `computeType` del backend e produce il client corrispondente:

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Un link HTTP con `httpSubscriptionLink` su Server-Sent Events per le subscription tRPC.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
Un semplice link HTTP. Le subscription non sono supportate perché le API Gateway HTTP APIs non supportano lo streaming delle risposte.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Un link WebSocket verso l'URL runtime di AgentCore, con handshake firmato SigV4 (IAM) o subprotocollo JWT codificato in base64url (Cognito). L'ARN runtime viene letto da `runtimeConfig.apis.<ApiName>` e convertito al volo in un URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws`.
</TabItem>
</Tabs>
...
Inoltre, installa le dipendenze necessarie:
  ...
  - `event-source-polyfill` (se si utilizza un backend REST API, per il supporto alle subscription)
  - `react-oidc-context` (se si utilizza autenticazione Cognito)
...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayHttpApi' }} description="I backend HTTP API non supportano le subscription">
:::caution[Nessuna subscription su HTTP API]
Le API Gateway HTTP APIs non supportano lo streaming delle risposte. Seleziona `ServerlessApiGatewayRestApi` o `BedrockAgentCoreRuntimeWebSocket` come `computeType` del backend se hai bisogno di subscription.
:::
</OptionFilter>

<OptionFilter when={{ computeType: ['ServerlessApiGatewayRestApi', 'BedrockAgentCoreRuntimeWebSocket'] }} description="Come le subscription raggiungono il client — il trasporto differisce in base al tipo di compute del backend">
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Il client generato utilizza un `splitLink` che instrada le operazioni di subscription attraverso `httpSubscriptionLink` (Server-Sent Events) e le query/mutation regolari attraverso `httpLink`. Le subscription funzionano immediatamente senza configurazione aggiuntiva.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Il client generato utilizza `wsLink` verso l'URL runtime di AgentCore — tutte le operazioni tRPC (query, mutation e subscription) viaggiano su una singola connessione WebSocket. L'handshake è autenticato automaticamente: firmato SigV4 per l'autenticazione `IAM`, o trasportando il JWT come subprotocollo codificato in base64url per `Cognito`.
</TabItem>
</Tabs>
