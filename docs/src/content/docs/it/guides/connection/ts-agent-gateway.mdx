...
2. Un progetto <Link path="guides/agentcore-gateway">`cedar#agentcore-gateway`</Link>
...
Per eseguire l'agent localmente **contro un Gateway distribuito** (ad esempio, per esercitare le policy Cedar), imposta:

```bash
export GATEWAY_URL_OVERRIDE=https://<gateway-id>.gateway.bedrock-agentcore.<region>.amazonaws.com/mcp
```

Quando impostato, il client utilizza l'URL del Gateway distribuito e firma le richieste con SigV4 utilizzando le tue credenziali AWS locali invece di ricorrere al multiplexing locale.

### Fedeltà locale

La modalità multiplex locale bypassa il Gateway stesso, quindi:

- **Nessuna valutazione delle policy Cedar.** Ogni strumento è visibile all'agent indipendentemente dalle policy. Usa `GATEWAY_URL_OVERRIDE` per esercitare le policy contro un Gateway distribuito.
...
rompt di sistema dell'agent e i nomi delle azioni Cedar a cui fai riferimento coerenti tra esecuzioni locali e distribuite.
