...
Quando distribuito (`SERVE_LOCAL` non impostato), `create()` restituisce un singolo `McpClient` puntato all'endpoint MCP del Gateway e autenticato con SigV4. Quando `SERVE_LOCAL=true`, restituisce un array di `McpClient` — uno per server MCP nella lista `ATTACHED_MCP_SERVERS` del client. `Agent.tools` di Strands accetta entrambe le forme (appiattisce in modo trasparente gli array di strumenti annidati), quindi lo stesso `agent.ts` funziona uniformemente in entrambe le modalità.
