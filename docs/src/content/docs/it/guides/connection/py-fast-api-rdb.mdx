...
`my_scope_my_db` è il nome del modulo del tuo progetto database — un singolo modulo flat denominato `<scope>_<name>`, visibile come directory del pacchetto all'interno del progetto.

`.scalars()` estrae le righe del risultato in istanze di `ExampleModel`, che FastAPI può serializzare.
