...
:::caution
Quando `securityGroups` viene omesso, `RuntimeNetworkConfiguration.usingVpc` crea un security group con un id di costrutto fisso (`SecurityGroup`) sullo scope fornito. Passare un security group esplicito come mostrato evita collisioni di id di costrutto quando più runtime (ad esempio un agent e un MCP server) sono definiti nello stesso stack.
:::
