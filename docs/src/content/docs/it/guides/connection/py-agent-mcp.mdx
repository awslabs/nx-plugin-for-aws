...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      - agentcore\_endpoints.py Risoluzione ARN/URL framework-agnostic
      - agentcore\_mcp\_transport.py Trasporto MCP framework-agnostic
      - agentcore\_mcp\_client\_\<framework>.py Client MCP che avvolge il trasporto per il framework del tuo agente
      - auth/ `httpx.Auth` framework-agnostic per SigV4 / session-forwarding
    - app
      - \<mcp\_server\_name>\_client\_\<framework>.py Client per-connessione per ogni server MCP

</FileTree>

Il suffisso del client corrisponde al framework del tuo agente (`_strands` o `_langchain`).
...
- Aggiorna il target `dev` dell'agente per dipendere dal target serve del server MCP
...
Il client Strands è un context manager, inserito in un blocco `with` attorno all'agente.
</TabItem>
<TabItem label="LangChain">
```python title="packages/my-project/my_module/agent/agent.py" {4,9,14}
...
Il client LangChain restituisce una lista di strumenti caricati tramite [`langchain-mcp-adapters`](https://docs.langchain.com/oss/python/langchain/mcp). Ogni strumento apre una nuova sessione MCP per chiamata, quindi gli strumenti rimangono utilizzabili per tutta la durata dell'agente — non è necessario un blocco `with`.
</TabItem>
</Tabs>

L'ID di sessione AgentCore viene propagato automaticamente al server MCP tramite l'header `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` per entrambi i framework, garantendo la coerenza per [Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
...
L'ARN del runtime AgentCore del server MCP viene registrato automaticamente nel namespace `agentcore` della <Link path="guides/runtime-config">Runtime Configuration</Link> dal costrutto CDK generato, in modo che l'agente possa scoprirlo a runtime.
</Fragment>
...
L'ARN del runtime AgentCore del server MCP viene registrato automaticamente nel namespace `agentcore` della <Link path="guides/runtime-config">Runtime Configuration</Link> dal modulo Terraform generato, in modo che l'agente possa scoprirlo a runtime.
</Fragment>
</Infrastructure>
