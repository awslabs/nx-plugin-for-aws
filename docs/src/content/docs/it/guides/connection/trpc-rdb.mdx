...
Il plugin unisce `IMyDbContext` nel contesto della tua procedura, rendendo `myDb` disponibile come proprietà opzionale:
...
Questo risolve il problema dell'adapter MySQL che mantiene aperto il loop degli eventi di Node.js dopo una query, il che altrimenti impedirebbe a Lambda di svuotare le risposte in streaming. Disconnettersi in `finally` rilascia il loop degli eventi in modo che la risposta possa completarsi. Vedi <Link path="guides/ts-rdb#mysql-api-gateway-streaming-mode">MySQL: Modalità Streaming API Gateway</Link> per i dettagli.
...
Distribuisci le funzioni Lambda dell'API in una **subnet privata con egress**, non in una subnet privata isolata. A runtime, `getPrisma()` recupera i dettagli di connessione al database da AWS AppConfig, che è un endpoint di servizio AWS pubblico che richiede accesso a internet in uscita.
