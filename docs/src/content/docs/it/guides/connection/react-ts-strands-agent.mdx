...
Se il tuo agente utilizza l'autenticazione IAM, il ruolo autenticato del Cognito Identity Pool deve essere autorizzato a invocare l'agente. Puoi utilizzare il metodo `grantInvokeAccess` che concede sia i permessi `InvokeAgentRuntime` che `InvokeAgentRuntimeWithWebSocketStream`:
