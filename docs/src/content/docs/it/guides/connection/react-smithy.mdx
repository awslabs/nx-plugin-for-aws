...
2. Un backend API Smithy TypeScript funzionante (generato usando il <Link path="/guides/ts-smithy-api">generatore `ts#api`</Link> con `--framework=smithy`)
3. Autenticazione Cognito aggiunta tramite il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link> se si connette un'API che utilizza autenticazione Cognito o IAM
...
Se stai lavorando attivamente sia sulla tua applicazione React che sull'API Smithy insieme, usa il target `dev` dell'applicazione React (un alias per `serve-local`) che rigenererà automaticamente il client ogni volta che l'API cambia, oltre a ricaricare a caldo il tuo sito web e il server API Smithy locale:
