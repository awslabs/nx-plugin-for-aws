...
Il generatore `connection` fornisce un modo rapido per integrare la tua applicazione React con il backend API Smithy TypeScript. Configura tutto il necessario per connettersi all'API Smithy in modo type-safe, inclusi la generazione di client e hook [TanStack Query](https://tanstack.com/query/v5), il supporto per autenticazione AWS IAM e Cognito, e una corretta gestione degli errori.
