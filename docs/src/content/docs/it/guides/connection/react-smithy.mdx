...
2. Un backend Smithy TypeScript API funzionante (generato utilizzando il <Link path="/guides/ts-smithy-api">generatore `ts#api`</Link> con `--framework=smithy`)
3. Cognito Auth aggiunto tramite il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link> se si connette un'API che utilizza l'autenticazione Cognito o IAM
