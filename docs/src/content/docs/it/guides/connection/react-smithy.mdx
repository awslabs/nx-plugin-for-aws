...
Se stai lavorando attivamente sia sulla tua applicazione React che sull'API Smithy insieme, usa il target `dev` dell'applicazione React che rigenererà automaticamente il client ogni volta che l'API cambia, oltre a ricaricare a caldo il tuo sito web e il server API Smithy locale:
