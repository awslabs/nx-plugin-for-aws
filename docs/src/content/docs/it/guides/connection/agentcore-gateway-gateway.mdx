...
Il target `<source-gateway>-dev` del progetto Gateway sorgente acquisisce una dipendenza dal target `<target-gateway>-dev` del Gateway target, quindi l'esecuzione del Gateway sorgente localmente avvia anche il gateway target (e, transitivamente, ogni server MCP collegato ad esso). Il gateway target viene anche registrato nel `local-dev.ts` del progetto Gateway sorgente in modo che il gateway locale aggreghi i suoi strumenti.
...
:::
cuzione locale chiami il Gateway distribuito.
:::
uito.
:::
