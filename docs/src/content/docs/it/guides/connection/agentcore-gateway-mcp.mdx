...
Il target `dev` del progetto Gateway acquisisce una dipendenza dal target `<mcp>-dev` del server MCP, quindi eseguire il Gateway localmente avvia anche il server MCP. Il server MCP viene anche registrato nel `local-dev.ts` del progetto Gateway in modo che il gateway locale aggreghi i suoi strumenti.
...
Il `name` del target (`my-mcp-server` sopra) viene utilizzato come prefisso per i nomi delle azioni Cedar — vedi la <Link path="guides/agentcore-gateway">sezione Scrittura delle Policy</Link>. `policy_dependencies` assicura che le policy Cedar che fanno riferimento alle azioni di questo target vengano create dopo che il target le ha registrate.
</Fragment>
</Infrastructure>
...
avvia un gateway locale più ogni server MCP collegato sulla sua porta locale assegnata. Il gateway locale espone un singolo endpoint MCP che aggrega gli strumenti dei server collegati. Gli agenti connessi al Gateway tramite i generatori di connessione gateway <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> o <Link path="guides/connection/py-agent-gateway">Python</Link> puntano ad esso quando vengono eseguiti con `LOCAL_DEV=true`.

:::caution[Fedeltà locale]
Lo sviluppo locale utilizza un **gateway sostitutivo locale** — un aggregatore MCP leggero avviato dal progetto Gateway, non il servizio AgentCore Gateway. Di conseguenza, **le policy Cedar non vengono valutate localmente**. Ogni agente vede ogni strumento su ogni server MCP collegato. Per esercitare le policy Cedar, esegui invece il target `serve` dell'agente (vedi le guide di connessione agente <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> / <Link path="guides/connection/py-agent-gateway">Python</Link>) in modo che l'agente in esecuzione locale chiami il Gateway distribuito.

I nomi degli strumenti sono ancora prefissati localmente come `<target-name>___<tool-name>` per corrispondere alla denominazione del Gateway distribuito, quindi il prompt di sistema di un agente e i nomi delle azioni Cedar a cui fai riferimento rimangono coerenti tra le esecuzioni locali e distribuite.
:::
