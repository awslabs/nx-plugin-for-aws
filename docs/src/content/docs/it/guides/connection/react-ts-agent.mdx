...
Nx Plugin for AWS fornisce un generatore per integrare rapidamente il tuo <Link path="guides/ts-agent">TypeScript Agent</Link> con un sito web React. Configura tutte le impostazioni necessarie per connettersi al tuo agent tramite tRPC su WebSocket, incluso il supporto per l'autenticazione AWS IAM e Cognito. L'integrazione fornisce una completa type safety end-to-end tra il tuo frontend e il router tRPC dell'agent.
...
### Utilizzo di TanStack Query

Il caso d'uso più comune è lo streaming della risposta dell'agent utilizzando la subscription `invoke` con l'hook `use<AgentName>Agent`, che restituisce un [proxy delle opzioni tRPC](https://trpc.io/docs/client/tanstack-react-query) da utilizzare con TanStack Query:
...
:::note[Proxy delle Opzioni tRPC]
Per maggiori dettagli sul proxy delle opzioni tRPC e l'integrazione con TanStack Query, consulta la [documentazione di tRPC TanStack React Query](https://trpc.io/docs/client/tanstack-react-query).
:::
