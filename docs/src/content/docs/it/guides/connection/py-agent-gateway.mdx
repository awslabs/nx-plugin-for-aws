...
`MyGatewayClientStrands.create()` restituisce un singolo `MCPClient` gestibile tramite context manager il cui `list_tools_sync()` produce ogni strumento disponibile attraverso il Gateway.
</TabItem>
...
L'URL del Gateway viene automaticamente registrato nel namespace `agentcore.gateways.<ClassName>` della <Link path="guides/runtime-config">Runtime Configuration</Link> dal costrutto CDK generato, così l'agent può scoprirlo a runtime.
</Fragment>
...
L'URL del Gateway viene automaticamente registrato nel namespace `agentcore.gateways.<ClassName>` della <Link path="guides/runtime-config">Runtime Configuration</Link> dal modulo Terraform generato, così l'agent può scoprirlo a runtime.
</Fragment>
</Infrastructure>
...
- **Il prefisso dei nomi degli strumenti è preservato.** Gli strumenti di ogni server MCP locale sono esposti come `<target-name>___<tool-name>`, corrispondendo a ciò che emette il Gateway distribuito. Questo mantiene il prompt di sistema dell'agent e i nomi delle azioni Cedar a cui fai riferimento coerenti tra esecuzioni locali e distribuite.
