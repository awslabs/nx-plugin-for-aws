...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py Client MCP Strands per il Gateway distribuito
      - auth/ `httpx.Auth` SigV4 / session-forwarding indipendente dal framework
    - app/
      - \<gateway\_snake>\_client\_strands.py Wrapper client Strands per-Gateway
...
`MyGatewayClientStrands.create()` restituisce un singolo client gestibile tramite context manager il cui `list_tools_sync()` fornisce ogni strumento disponibile attraverso il Gateway:
