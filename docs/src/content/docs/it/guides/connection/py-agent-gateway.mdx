...
- **Modalità distribuita** (`SERVE_LOCAL` non impostato): strumenti puntati all'endpoint MCP del Gateway, firmati con SigV4.
- **Modalità locale** (`SERVE_LOCAL=true`): strumenti HTTP semplici puntati al gateway locale avviato dal target `serve-local` del progetto Gateway.
...
l'agent. Senza `SERVE_LOCAL` impostato, il client risolve l'URL del Gateway distribuito dalla configurazione runtime e firma le richieste con SigV4 utilizzando le tue credenziali AWS locali:
