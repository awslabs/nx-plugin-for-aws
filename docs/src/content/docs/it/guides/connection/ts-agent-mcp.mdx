...
Il generatore `connection` può connettere il tuo <Link path="guides/ts-agent">TypeScript Strands Agent</Link> a un server MCP (sia <Link path="guides/ts-mcp-server">TypeScript</Link> che <Link path="guides/py-mcp-server">Python</Link>).
