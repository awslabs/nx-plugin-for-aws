...
2. Un TypeScript o Python Agent con `protocol=ag-ui` (generato utilizzando il generatore <Link path="guides/ts-agent">`ts#agent`</Link> o <Link path="guides/py-agent">`py#agent`</Link>)
3. Per gli agenti distribuiti, Cognito Auth aggiunto tramite il <Link path="/guides/react-website-auth">generatore `ts#website#auth`</Link>
...
- src
  - components
    ...
    - copilot
      - index.tsx Ri-esporta `CopilotChat`, `CopilotSidebar` e `CopilotPopup` con valori predefiniti di slot che corrispondono al `ux` del tuo sito web (Cloudscape, Shadcn, o nessun tema).
      - *ThemeComponents*.tsx Componenti tema per-slot (es. `CloudscapeAssistantMessage.tsx`, `ShadcnChatInput.tsx`). Forniti solo quando `ux` è `cloudscape` o `shadcn`.
  - hooks
...
Istanzia i componenti CopilotKit con `agentId` per selezionare quale agente utilizzare. L'id è il nome dell'agente — lo stesso che hai scelto quando hai eseguito il generatore <Link path="guides/ts-agent">`ts#agent`</Link> o <Link path="guides/py-agent">`py#agent`</Link> — e puoi anche trovarlo nel file hook generato (es. la chiave restituita da `packages/web/src/hooks/useAgui<AgentName>.tsx`).
...
| `ux` | Stilizzazione applicata a `CopilotChat` / `CopilotSidebar` / `CopilotPopup` |
| ------------ | -------------------------------------------------------------------- |
...
1. Eseguire `nx dev <website>` avvierà anche il server locale dell'agente
...
- [Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/)vguide/)