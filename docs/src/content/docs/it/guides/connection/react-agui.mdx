...
AG-UI non è ancora supportato per TypeScript Agents. Consulta [agents/sdk-typescript#347](https://github.com/agents/sdk-typescript/issues/347) per lo stato più recente. Per ora, usa il <Link path="guides/py-agent">generatore `py#agent`</Link> con `protocol=AG-UI`.
:::
...
2. Un Python Agent con `protocol=AG-UI` (generato utilizzando il <Link path="guides/py-agent">generatore `py#agent`</Link>)
