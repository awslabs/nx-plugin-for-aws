...
- **Session ID** — la sessione AgentCore Runtime, inviata nell'header `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id`. Seleziona la [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) che serve la richiesta, ed è ciò su cui il `session.ts` / `session.py` del tuo agent basa lo stato della conversazione.
...
Lasciare `threadId` non impostato è la soluzione più semplice — l'UUID generato da CopilotKit è già di 36 caratteri. Se ne passi uno esplicitamente, assicurati che sia di almeno 33 caratteri, poiché il riempimento mappa thread ID che differiscono solo nei caratteri finali sulla stessa sessione.

Entrambi i valori provengono dal browser, quindi nominano una conversazione senza dire di chi sia. Per limitare ogni utente alle proprie conversazioni, consulta <Link path="guides/ts-agent">Limitare le sessioni al loro proprietario</Link>.
