...
AG-UI non è ancora supportato per TypeScript Strands Agents. Consulta [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) per lo stato più recente. Per ora, usa il <Link path="guides/py-agent">generatore `py#agent`</Link> con `protocol=AG-UI`.
:::
...
2. Un Python Strands Agent con `protocol=AG-UI` (generato utilizzando il <Link path="guides/py-agent">generatore `py#agent`</Link>)
...
Istanzia i componenti CopilotKit con `agentId` per selezionare quale agente utilizzare. L'id è il nome dell'agente — lo stesso che hai scelto quando hai eseguito il <Link path="guides/py-agent">generatore `py#agent`</Link> — e puoi anche trovarlo nel file hook generato (es. la chiave restituita da `packages/web/src/hooks/useAgui<AgentName>.tsx`).
...
- <Link path="guides/py-agent">Guida Python Strands Agent</Link>
- <Link path="guides/connection/react-py-agent">Guida React to Python Strands Agent (HTTP)</Link>
