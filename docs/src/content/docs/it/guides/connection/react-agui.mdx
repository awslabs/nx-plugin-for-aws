...
| ------------ | -------------------------------------------------------------------- |
| `Cloudscape` | I messaggi dell'assistente e dell'utente vengono renderizzati all'interno dei componenti Cloudscape `ChatBubble` con `Avatar` gen-AI, corrispondenti al [pattern di chat AI generativa Cloudscape](https://cloudscape.design/patterns/genai/generative-AI-chat/). La barra degli strumenti dei messaggi diventa un Cloudscape `ButtonGroup` nello slot `actions` della bolla (copia, pollice su/giù, rigenera), il Markdown viene renderizzato tramite il renderer predefinito di CopilotKit all'interno della bolla, l'indicatore di digitazione diventa una `LoadingBar` gen-AI, e l'input viene sostituito con Cloudscape `PromptInput`. Costruito da `@cloudscape-design/components` e `@cloudscape-design/chat-components`. |
| `Shadcn` | I messaggi dell'assistente vengono renderizzati con un avatar `Sparkles` e una bolla in stile card `bg-muted` attenuata utilizzando lo stile Tailwind `prose`; una barra degli strumenti `Button` rivelata al passaggio del mouse fornisce copia/pollice/rigenera. I messaggi dell'utente vengono renderizzati allineati a destra in una bolla `bg-primary` con un avatar `User`. L'input viene sostituito con una `Textarea` arrotondata + `Button` con icona a forma di pillola (Enter invia, Shift+Enter inserisce una nuova riga; si espande automaticamente fino a 8 righe), e l'indicatore di digitazione è un'animazione di pulsazione a tre punti. Utilizza primitive shadcn (`Avatar`, `Button`, `Textarea`) dal pacchetto condiviso `common-shadcn`. |
| `None` (o qualsiasi altro valore) | Nessun tema applicato — il wrapper semplicemente ri-esporta i componenti CopilotKit predefiniti. |
