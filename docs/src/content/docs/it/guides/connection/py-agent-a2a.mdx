...
1. Un progetto Python con un componente <Link path="guides/py-agent">Agent</Link> (qualsiasi protocollo)
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_a2a\_client\_strands.py Client A2A Strands che avvolge la configurazione
      - auth/ `httpx.Auth` SigV4 / session-forwarding indipendente dal framework
    - app
      - \<target\_agent\_name>\_client\_strands.py Client Strands per-connessione per ogni agente A2A
...
Internamente, `RemoteAgentClientStrands.create(session_id=...)` restituisce uno Strands `A2AAgent` configurato con un `httpx.AsyncClient` che firma le richieste con SigV4 quando distribuito su AWS, e un endpoint semplice `http://localhost:<port>/` quando `SERVE_LOCAL=true`.
