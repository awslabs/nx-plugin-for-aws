...
1. Un progetto Python con un componente <Link path="guides/py-agent">Strands Agent</Link> (qualsiasi protocollo)
2. Un progetto con un componente Strands Agent generato con `--protocol=A2A` e `--auth=IAM` (sia <Link path="guides/ts-agent">`ts#agent`</Link> che <Link path="guides/py-agent">`py#agent`</Link>)
