...
- **Connessione** — `BaseModel.Meta` imposta `host`, `region`, `aws_access_key_id` e `aws_secret_access_key` da `config.json` quando `is_local()` è `True`, puntando PynamoDB all'istanza locale di DynamoDB. In AWS, questi vengono lasciati non impostati in modo che PynamoDB utilizzi la catena di credenziali predefinita.
