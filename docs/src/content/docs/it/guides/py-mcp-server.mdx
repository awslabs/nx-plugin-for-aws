  ...
  - your-project/
    - your_module/
      - mcp_server/ (o nome personalizzato se specificato)
        ...
        - Dockerfile Punto d'ingresso per hostare il server MCP (quando `computeType` è impostato a `BedrockAgentCoreRuntime`)
...
:::tip
L'immagine docker è costruita usando un tag (ad esempio `my-scope-my-project-mcp-server:latest`), da cui il costrutto CDK eredita il `Dockerfile`, permettendo di collocare il tuo `Dockerfile` insieme al progetto del server MCP.
:::
