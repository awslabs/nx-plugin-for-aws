...
:::tip
Il tuo `Dockerfile` è co-locato con il progetto del server MCP. Per CDK, il costrutto usa `buildContexts` per costruire l'immagine direttamente da questo Dockerfile. Per Terraform, l'immagine docker è pre-costruita usando un tag (esempio `my-scope-my-project-mcp-server:latest`) che viene referenziato dalla tua infrastruttura Terraform.
:::
