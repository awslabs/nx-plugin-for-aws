...
Passa `identity.userPool.userPoolId`, `proxyClient.userPoolClientId` e `identity.userPoolDomain.baseUrl()` al proxy. Il costrutto `UserIdentity` crea sempre un dominio, quindi non è necessario aggiungerne uno.
</Fragment>
<Fragment slot="terraform">
```hcl
# Un App Client confidenziale sullo STESSO user pool creato da ts#website#auth
...
Passa l'id del user pool, l'id di questo client e il dominio user pool esistente al modulo proxy.
</Fragment>
</Infrastructure>
:::
