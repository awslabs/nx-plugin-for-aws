...
Se hai già un User Pool dal generatore <Link path="/guides/react-website-auth">`ts#website#auth`</Link>, puoi mettere davanti un server MCP per gli stessi utenti del sito web. Vedi <Link path="/guides/ts-dcr-proxy#riutilizzare-un-useridentity-user-pool">Riutilizzare un UserIdentity User Pool</Link> di seguito.
:::
