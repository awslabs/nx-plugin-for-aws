...
Se hai selezionato `Any` come `eventSource`, il decorator `@event_parser` non viene applicato e l'handler riceve un dict di eventi non tipizzato. Rigenera con un `eventSource` specifico se desideri la validazione Pydantic e l'accesso tipizzato ai campi.
</OptionFilter>
