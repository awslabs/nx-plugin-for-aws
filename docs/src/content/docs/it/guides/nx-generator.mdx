...
<FileTree>
  - src/\<name>/
    ...
    - files/
      - hello.ts.template Template di esempio renderizzato dal generator stub
  - src/index.ts Aggiornato per esportare il tuo generator
  - generators.json Creato o aggiornato per definire i tuoi generator
  - project.json Aggiornato per registrare il generator nel progetto
  - package.json Creato se assente e puntato a `generators.json`
</FileTree>
...
Alcune opzioni si applicano solo per determinati valori di un'altra. Usa `x-when` per specificare quali,
in modo che il sito di documentazione ometta l'opzione dove non si applica e dichiari
la condizione dove si applica:
...
Più chiavi sono in AND e un elenco di valori è in OR all'interno di una chiave, corrispondendo
a `<OptionFilter when={{…}}>`:
...
`x-when` documenta l'opzione; non impone nulla, quindi un generator deve comunque
validare o ignorare un'opzione che non si applica all'esecuzione che gli è stata fornita.
...
- Usa `createTreeUsingTsSolutionSetup()` da `@aws/nx-plugin/sdk/utils/test` per creare un filesystem virtuale. Popola i marker del workspace che il nostro preset scrive (`tsconfig.base.json`, `pnpm-workspace.yaml`, `biome.json`, un `package.json` root con `type: module`), di cui qualsiasi generator che compone un generator `@aws/nx-plugin` ha bisogno
