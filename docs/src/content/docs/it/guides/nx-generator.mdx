...
Puoi utilizzare [GritQL](https://docs.grit.io/) per cercare e trasformare in modo dichiarativo il codice sorgente nei tuoi generatori. GritQL supporta più linguaggi tra cui TypeScript, JavaScript, Python, HCL (Terraform) e altri — così puoi utilizzare la stessa sintassi di pattern su tutto il tuo stack.
...
Puoi provare i pattern GritQL nel [Playground GritQL](https://docs.grit.io/playground), oppure installare la CLI GritQL localmente per testare i pattern sui tuoi file:
...
:::tip[Agenti di codifica IA]
Se utilizzi un agente di codifica AI per aiutarti a scrivere generatori, aggiungi il [server MCP Context7](https://context7.com/) per fornirgli documentazione aggiornata. Chiedigli di cercare `gritql` e `docs.grit.io` per il riferimento alla sintassi dei pattern GritQL, e fornisci accesso alla CLI GritQL in modo che possa testare i pattern direttamente.
:::
...
export const myGenerator = async (tree: Tree, schema: MyGeneratorSchema) => {
  ...
  generateFiles(
    tree,
    joinPathFragments(import.meta.dirname, 'files'), // Directory template
    'path/to/output', // Directory di output
    data,
  );
};
