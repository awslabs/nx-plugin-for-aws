...
Consigliamo di utilizzare il generator <Link path="/it/guides/ts-nx-plugin">`ts#nx-plugin`</Link> come progetto base per qualsiasi generator che crei, poiché fornisce anche un MCP Server che aiuta l'AI a utilizzare i tuoi generator.
:::
...
Per una guida più approfondita riguardo al contribuire al Nx Plugin for AWS, fare riferimento al <Link path="/it/get_started/tutorials/contribute-generator">tutorial qui</Link>.
:::