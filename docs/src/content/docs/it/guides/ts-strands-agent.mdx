...
Il generatore configura un target denominato `<your-agent-name>-invoke` che avvia una chat interattiva con il tuo agente dal terminale. Per impostazione predefinita si connette al server locale:

<NxCommands commands={['agent-invoke your-project']} />

Per chattare invece con un agente distribuito, imposta `RUNTIME_CONFIG_APP_ID` sull'ID dell'applicazione AppConfig del tuo workspace — la CLI risolve l'ARN del runtime e invoca l'endpoint distribuito:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Per gli agenti autenticati con Cognito, passa un JWT con `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

Lo script CLI si trova in `scripts/<agent>/cli.ts` e seleziona automaticamente il protocollo corretto (HTTP/A2A) per il tuo agente.
