...
Il generatore fornisce anche una CLI di chat interattiva minimale in `scripts/<your-agent-name>/chat.ts` e un corrispondente target `<your-agent-name>-chat`. Questo target dipende da `<your-agent-name>-serve-local`, quindi eseguendolo avvia l'agente localmente e ti porta in una chat interattiva da terminale con esso:

<NxCommands commands={['run your-project:agent-chat']} />

La CLI utilizza il client appropriato per il protocollo (`AgentClient.local` per HTTP/tRPC o `@a2a-js/sdk` per A2A) e trasmette le risposte dell'agente token per token tramite [`@clack/prompts`](https://github.com/bombshell-dev/clack).
