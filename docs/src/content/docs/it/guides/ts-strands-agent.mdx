...
Il generatore configura un target Nx `<your-agent-name>-chat` che dipende da `<your-agent-name>-serve-local`. Eseguendolo avvia l'agente localmente e ti porta in una chat interattiva da terminale alimentata da [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Per gli agenti **A2A**, il target esegue semplicemente `agent-chat-cli` come binario — nessun codice viene generato nel tuo workspace.

Per gli agenti **HTTP** (tRPC su WebSocket), il generatore emette un piccolo `scripts/<your-agent-name>/chat.ts` che avvolge il `<Agent>Client.local({ url })` generato come `ChatAdapter` e lo passa al `chatLoop` di `agent-chat-cli`.
