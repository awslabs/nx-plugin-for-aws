...
:::tip
Il tuo `Dockerfile` è co-localizzato con il tuo progetto Strands Agent. Per CDK, il costrutto utilizza `buildContexts` per creare l'immagine direttamente da questo Dockerfile. Per Terraform, l'immagine docker viene pre-creata utilizzando un tag (ad esempio `my-scope-my-project-agent:latest`) a cui fa riferimento la tua infrastruttura Terraform.
:::
