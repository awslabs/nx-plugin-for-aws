...
Il generatore fornisce un'opzione `auth` per configurare l'autenticazione per il tuo Strands Agent. Puoi scegliere tra l'autenticazione `IAM` (predefinita) o `Cognito` quando generi il tuo agente.

#### IAM

Per impostazione predefinita, il tuo Strands Agent sarà protetto utilizzando l'autenticazione IAM, distribuiscilo semplicemente senza argomenti:
...
Puoi concedere l'accesso per invocare il tuo agente su Bedrock AgentCore Runtime utilizzando il metodo `grantInvokeAccess`, ad esempio:
...
Per concedere l'accesso per invocare il tuo agente, dovrai aggiungere una policy come la seguente, facendo riferimento all'output `module.my_project_agent.agent_core_runtime_arn`:
...
Quando selezioni l'autenticazione `Cognito`, il generatore crea automaticamente il costrutto `UserIdentity` e configura l'agente per utilizzare Cognito per l'autenticazione.
...
Il costrutto `UserIdentity` viene generato automaticamente quando selezioni l'autenticazione `Cognito`.
</Fragment>
<Fragment slot="terraform">
