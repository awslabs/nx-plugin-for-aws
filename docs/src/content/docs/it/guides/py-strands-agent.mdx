...
Gli agenti AG-UI possono essere connessi a un frontend React utilizzando il <Link path="/guides/connection/react-agui">generatore di connessione `react` → agente AG-UI</Link>, che configurerà i componenti [CopilotKit](https://docs.copilotkit.ai/aws-strands) per un'esperienza di chat ricca.
:::
...
Gli agenti AG-UI sono progettati per essere consumati direttamente da un frontend. Utilizza il <Link path="/guides/connection/react-agui">generatore di connessione `react` → agente AG-UI</Link> per collegare il tuo sito web React all'agente con un provider [CopilotKit](https://docs.copilotkit.ai/aws-strands) e un client [AG-UI HttpAgent](https://docs.ag-ui.com/).
:::
