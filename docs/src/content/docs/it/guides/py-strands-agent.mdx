...
La CLI è uno script TypeScript (eseguito tramite `tsx`) che utilizza il client appropriato per il protocollo per trasmettere le risposte token per token tramite [`@clack/prompts`](https://github.com/bombshell-dev/clack):

- **HTTP**: `POST /invocations` con streaming JSONL
- **A2A**: `@a2a-js/sdk` `ClientFactory` con `sendMessageStream`
