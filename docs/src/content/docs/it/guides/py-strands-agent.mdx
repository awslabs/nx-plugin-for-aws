...
Per gli agenti **A2A** e **AG-UI**, il target chat invoca semplicemente `agent-chat-cli` come binario contro l'agente in esecuzione localmente — nessun codice viene generato nel tuo workspace.

Per gli agenti **HTTP**, il generatore emette:
...
- Un target Nx `<your-agent-name>-generate-client` che esegue `open-api#ts-client` per produrre un client TypeScript type-safe sotto `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — un adattatore sottile che importa il client generato e lo collega al `chatLoop` di `agent-chat-cli`

Lo script di chat è un piccolo wrapper attorno al client generato. Quando personalizzi la forma di input dell'agente (ad esempio aggiungi nuovi campi a `InvokeInput`), aggiorna il `sendMessage` dell'adattatore per passare i nuovi campi e il resto funziona automaticamente.
