...
:::tip
Il tuo `Dockerfile` è co-locato con il progetto Strands Agent. Per CDK, il costrutto utilizza `buildContexts` per costruire l'immagine direttamente da questo Dockerfile. Per Terraform, l'immagine docker è pre-costruita usando un tag (ad esempio `my-scope-my-project-agent:latest`) che viene referenziato dalla tua infrastruttura Terraform.
:::
