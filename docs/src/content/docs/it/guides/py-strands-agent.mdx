...
Quando `protocol` è impostato su `AG-UI`, il punto d'ingresso utilizza l'integrazione [ag-ui-strands](https://docs.ag-ui.com/), che espone il tuo agente tramite il protocollo AG-UI per l'integrazione diretta con il frontend tramite [CopilotKit](https://docs.copilotkit.ai/aws-strands):
...
Quando `protocol=AG-UI`, il `main.py` generato avvolge il tuo Strands `Agent` in un `ag_ui_strands.StrandsAgent` e crea un'app FastAPI tramite `create_strands_app()`. L'app risultante espone un singolo endpoint POST che trasmette eventi [AG-UI](https://docs.ag-ui.com/) tramite Server-Sent Events (SSE), oltre a `/ping` per il controllo dello stato del runtime AgentCore.
...
Il generatore configura un target chiamato `<your-agent-name>-invoke` che avvia una chat interattiva con il tuo agente dal terminale. Viene fornito come script TypeScript (tramite `tsx`) e seleziona automaticamente il protocollo corretto (HTTP, A2A, AG-UI) per il tuo agente. Per impostazione predefinita si connette al server locale:

<NxCommands commands={['agent-invoke your-project']} />

Per chattare invece con un agente distribuito, imposta `RUNTIME_CONFIG_APP_ID` sull'ID applicazione AppConfig del tuo workspace — la CLI risolve l'ARN del runtime e invoca l'endpoint distribuito con richieste firmate SigV4:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Per agenti autenticati con Cognito, passa un JWT con `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

Lo script CLI si trova in `scripts/<agent>/cli.ts` nel tuo progetto.
