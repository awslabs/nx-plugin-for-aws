...
Il generatore fornisce un'opzione `auth` per configurare l'autenticazione per il tuo Strands Agent. Puoi scegliere tra autenticazione `IAM` (predefinita) o `Cognito` quando generi il tuo agente.
...
Puoi concedere l'accesso per invocare il tuo agente su Bedrock AgentCore Runtime usando il metodo `grantInvokeAccess`, ad esempio:
