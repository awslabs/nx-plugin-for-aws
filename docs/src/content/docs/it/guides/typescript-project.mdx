...
**Gli strumenti di build e test** (ad esempio `vitest`, `tsx`, `rolldown`, la toolchain Nx (`nx`, `@nx/js`, ...), `typescript`, `@biomejs/biome`) sono condivisi nell'intero workspace, quindi risiedono nelle `devDependencies` del `package.json` **root** anziché in ogni progetto. Questi strumenti sono referenziati solo da file di configurazione, script di build e test, che la regola di lint esclude.
...
npm (e yarn classic) non ha una funzionalità di catalog, quindi le versioni sono dichiarate direttamente in ogni `package.json`. Per mantenerle coerenti tra i progetti, raccomandiamo [syncpack](https://github.com/JamieMason/syncpack) (`npx syncpack lint`).
:::

:::note[bun]
Su bun, alcuni pacchetti le cui versioni i generatori Nx leggono dal manifesto root (`vite`, `react`, `react-dom`) mantengono intervalli di versione diretti invece di riferimenti `catalog:` — Nx non risolve ancora i catalog di bun, e un riferimento `catalog:` romperebbe quei generatori.
:::
...
Con i catalog disabilitati, i generatori scrivono le versioni delle dipendenze direttamente nel `package.json` di ogni progetto, e mantenere le versioni allineate tra i progetti è tua responsabilità (ad esempio utilizzando [syncpack](https://github.com/JamieMason/syncpack)). Quando i catalog sono abilitati su pnpm, il workspace imposta anche [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) in modo che `pnpm add` registri le nuove dipendenze nel catalog, mantenendo la single version policy senza modifiche manuali.
