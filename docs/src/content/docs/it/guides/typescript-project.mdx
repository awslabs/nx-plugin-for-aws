...
<FileTree>
  ...
  - vitest.config.mts la configurazione Vitest condivisa che il vitest.config.mts del tuo progetto estende (creato dal primo progetto TypeScript)
  - package.json vengono aggiunti gli strumenti di build e test condivisi
  - pnpm-workspace.yaml il progetto viene aggiunto al workspace e le versioni delle dipendenze al [catalogo](#catalogs) (il file equivalente per il tuo package manager)
  - .gitignore i percorsi di output di build e test vengono ignorati
...
Il `cwd` è ciò che consente a `rolldown` di trovare la sua configurazione: senza di esso il comando viene eseguito dalla radice del workspace, dove `rolldown.config.ts` non esiste.
...
Il tuo progetto ha anche un target `assemble`, che produce tutto ciò che il tuo progetto contribuisce a una distribuzione (ad esempio il suo output compilato o raggruppato), senza eseguire i controlli di lint, test o type-check.

<NxCommands commands={['assemble <project-name>']} />

I <Link path="/it/guides/typescript-infrastructure#deploying-to-aws">target di deploy</Link> dipendono da `assemble`, quindi il deploy costruisce solo ciò che sta per distribuire.

:::caution[Usa `build` nella tua pipeline]
`assemble` viene utilizzato per un'iterazione più rapida durante lo sviluppo. Esegui `build` nella tua pipeline CI per garantire che il tuo progetto superi i controlli di qualità prima di una distribuzione in produzione.
:::
