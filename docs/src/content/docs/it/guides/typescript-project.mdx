...
Su pnpm, il workspace imposta anche [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), quindi `pnpm add` registra la versione nel catalog e la referenzia automaticamente.

yarn e bun non hanno un equivalente, e il loro protocollo `catalog:` *referenzia* solo una voce di catalog esistente — non può crearne una. Per aggiungere una nuova dipendenza su yarn o bun, prima registra la sua versione nel catalog, poi referenziala dal progetto:

- **yarn** — aggiungi la versione sotto `catalog` in `.yarnrc.yml`, poi `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — aggiungi la versione sotto `catalog` nel `package.json` root, poi `bun add some-npm-package@catalog: --cwd packages/my-library`
