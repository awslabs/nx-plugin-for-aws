...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Aggiungi la dipendenza con [`--save-catalog`](https://pnpm.io/settings#catalogmode), in modo che pnpm registri la versione nel catalogo e vi faccia riferimento:
    ...
    :::note
    Impostare [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) in `pnpm-workspace.yaml` rende questo il comportamento predefinito per ogni `pnpm add`. Inoltre fa fallire `pnpm add some-npm-package@<version>` ogni volta che il catalogo fissa una versione diversa, il che impedisce a `nx migrate` di aggiornare Nx — quindi il workspace generato lo lascia non impostato.
    :::
  </TabItem>
  <TabItem label="yarn">
