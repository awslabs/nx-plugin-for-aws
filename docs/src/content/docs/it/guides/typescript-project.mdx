...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Il workspace imposta [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), quindi `pnpm add` registra la versione nel catalog e la referenzia automaticamente — nessun passaggio aggiuntivo.
  ...
  <TabItem label="yarn">
    Il protocollo `catalog:` di yarn *referenzia* solo una voce di catalog esistente — non può crearne una — quindi registra prima la versione.
  ...
  <TabItem label="bun">
    ...
    <Steps>
    1. Aggiungi la versione sotto `catalog` nel `package.json` root:
