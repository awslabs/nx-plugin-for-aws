...
Il tuo progetto ha anche un target `assemble`, che è il fratello solo-artefatti di `build`: produce tutto ciò che il tuo progetto contribuisce a un deployment (il suo output compilato, e qualsiasi bundle, immagine container o specifica generata che dichiara) senza eseguire i controlli di lint, test o type-check.

<NxCommands commands={['assemble <project-name>']} />

I <Link path="guides/typescript-infrastructure#deploying-to-aws">target di deploy</Link> dipendono da `assemble` invece che da `build`, quindi il deploy costruisce solo ciò che sta per distribuire. `build` rimane invariato e fa ancora tutto, quindi rimane ciò che esegui per controllare un progetto — e ciò che dovresti eseguire in CI.

:::caution
Poiché `assemble` salta i controlli di qualità, un deploy non fallisce più su un errore di lint o un test unitario fallito. Esegui `build` (o la tua pipeline CI) per controllare il tuo codice.
:::
