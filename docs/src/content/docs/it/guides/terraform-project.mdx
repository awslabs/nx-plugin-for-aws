...
[Terraform](https://www.terraform.io/) è uno strumento open-source di infrastruttura come codice che ti permette di creare, modificare e migliorare l'infrastruttura in modo sicuro e prevedibile.

Il generatore di infrastruttura Terraform crea un progetto infrastrutturale Terraform. L'applicazione generata include best practice di sicurezza attraverso i controlli di [Checkov](https://www.checkov.io/).
...
<FileTree>
  ...
  - src
    - main.tf File di configurazione principale di Terraform
...
Per progetti libreria (`--type=library`), il generatore crea una struttura semplificata per moduli Terraform riutilizzabili:
...
:::tip
I progetti di applicazione includono capacità di deployment complete con gestione dello stato remoto, mentre i progetti libreria sono pensati per creare moduli Terraform riutilizzabili da consumare in altri progetti.
:::

## Implementa la tua Infrastruttura Terraform
...
-}
+
+# Dichiarazione della tua infrastruttura qui
...
Per aggiungere nuovi ambienti, crea un nuovo file `src/env/<environment>.tfvars` con le variabili specifiche e aggiungi nuove voci per `apply, destroy, init, plan` nel `project.json`. Esempio per aggiungere un ambiente `prod`:
...
:::tip
Per default tutti i target usano l'ambiente `dev`, ma puoi specificarne uno con: <NxCommands commands={['run tf-infra:apply --configuration=prod']} />

Puoi cambiare ambiente in qualsiasi momento poiché lo stato viene segmentato automaticamente tra ambienti.
:::

### Bootstrap Stato Remoto (Solo Progetti Applicazione)

Per i progetti applicazione, prima del deploy devi eseguire il bootstrap del backend per lo stato remoto. Questo crea un bucket S3 per archiviare gli stati Terraform:
...
Esegui controlli di sicurezza con Checkov via target `test`:
...
Genererà un file di piano in `dist/packages/<my-terraform-project>/terraform/dev.tfplan`.
...
:::tip
Questo comando applica il piano generato da `plan`. Assicurati di eseguire prima `plan` per verificare le modifiche.
:::

#### Recupero Outputs

Ottieni i valori di output dalla configurazione con:

<NxCommands commands={['run tf-infra:output']} />

#### Distruzione Infrastruttura 

Elimina tutte le risorse gestite con il target `destroy`:

<NxCommands commands={['run tf-infra:destroy']} />

:::caution
Questo eliminerà permanentemente tutte le risorse gestite. Usa con cautela!
:::
...
Per maggiori dettagli su Terraform, consulta la [Documentazione Terraform](https://www.terraform.io/docs) e la [Documentazione AWS Provider](https://registry.terraform.io/providers/hashicorp/aws/latest/docs).