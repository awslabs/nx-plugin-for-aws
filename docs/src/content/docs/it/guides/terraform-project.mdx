...
Ogni target che esegue `terraform init` condivide una cache dei provider in `.terraform/plugin-cache` nella radice del tuo workspace. Consulta la [documentazione Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) per ulteriori informazioni.
