...
Un semplice `aws_s3_bucket` fallisce la scansione [Checkov](#test-di-sicurezza) eseguita da `build`, quindi l'esempio dichiara le risorse di versioning, crittografia e blocco dell'accesso pubblico che la scansione si aspetta, e sopprime i controlli rimanenti per risorsa con commenti `#checkov:skip` che ne indicano il motivo. Rivedi ogni soppressione in base ai tuoi requisiti — vedi [Test di sicurezza](#test-di-sicurezza) per entrambi i meccanismi di soppressione.
...
:::note[Nomi delle variabili]
Ogni nome che imposti qui deve essere dichiarato in `src/variables.tf` — lo scaffold dichiara `aws_region` e `environment`. Terraform avvisa solo di un valore per una variabile non dichiarata, quindi un nome scritto in modo errato viene silenziosamente ignorato anziché applicato.
:::
</TabItem>
...
Come `test`, questo esegue prima `terraform init -backend=false`, quindi installa i moduli e i provider necessari per la validazione senza configurare il backend S3. Pertanto viene eseguito su un progetto nuovo, prima di <Link path="guides/terraform-project#bootstrap-dello-stato-remoto-solo-progetti-application">`bootstrap`</Link> e senza credenziali AWS, come fanno `lint`, `test` e `checkov`.

#### Linting

I progetti Terraform utilizzano [`terraform fmt`](https://developer.hashicorp.com/terraform/cli/commands/fmt) per verificare la formattazione.
...
Imposta `TF_PLUGIN_CACHE_DIR` nel tuo ambiente per puntare lo script `init` fornito a una cache che gestisci tu stesso — ad esempio un volume condiviso tra workspace. Questo raggiunge solo i target che passano attraverso lo script, che è l'`init` di un progetto application. Ogni altro target dichiara il percorso come voce `env` in `project.json`, e il blocco `env` di Nx prevale sull'ambiente circostante, quindi modificalo anche lì — su `test` e `validate` per entrambi i tipi di progetto, e anche su `init` per un progetto library.
