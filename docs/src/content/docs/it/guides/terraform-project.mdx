...
Ogni nome che imposti qui deve essere dichiarato in `src/variables.tf` — il generatore dichiara `aws_region` e `environment`.
:::
</TabItem>
...
Ogni target che esegue `terraform init` riutilizza una cache dei provider in `.terraform/plugin-cache` nella radice del tuo workspace, quindi i provider vengono scaricati una sola volta anziché ad ogni esecuzione. Ogni progetto ottiene la propria directory lì. Consulta la [documentazione Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) per ulteriori informazioni.

Il target `install-providers` è ciò che riempie quella cache, e ogni target che esegue `terraform init` dipende da esso:

<NxCommands commands={['install-providers tf-infra']} />

Normalmente non lo eseguirai tu stesso. Esiste perché due esecuzioni di `terraform init` che riempiono una cache contemporaneamente possono ciascuna calcolare un hash diverso per lo stesso provider, che terraform poi rifiuta confrontandolo con il tuo `.terraform.lock.hcl` con "the cached package ... does not match any of the checksums recorded in the dependency lock file". Scaricare i provider una volta, prima dei target che ne hanno bisogno, lascia quei target a leggere solo da una cache che è già pronta.

Imposta `TF_PLUGIN_CACHE_DIR` nel tuo ambiente per puntare lo script `init` fornito a una cache che gestisci tu stesso — ad esempio un volume condiviso tra workspace. Nota che i target `install-providers`, `test` e `validate` leggono il loro percorso da `project.json`, quindi modificalo anche lì.
