...
Checkov viene eseguito come parte sia di `build` che di `package`, quindi un `plan` interattivo scansiona la tua infrastruttura prima di applicarla.
...
`plan` dipende da `package` piuttosto che da `build`, quindi produce gli artefatti a cui fanno riferimento i tuoi moduli — i bundle Lambda e i metadati delle operazioni generate — ed esegue Checkov, senza eseguire anche i gate di lint, test e type-check di ogni progetto.

:::caution
Poiché `plan` non esegue più i test dei moduli Terraform o i test unitari dei tuoi progetti, esegui `build` per controllare il tuo codice prima di applicare qualsiasi cosa a cui tieni, e vincola la tua pipeline ad esso:
