...
<FileTree>
  ...
  - .husky/ Hook Git
  - .mcp.json Configurazione server MCP Nx Plugin for AWS per Claude Code
  - .cursor/mcp.json ...e per Cursor
  - .kiro/settings/mcp.json ...e per Kiro
  - .gemini/settings.json ...e per Gemini CLI
  - .vscode/mcp.json ...e per GitHub Copilot
  - .codex/config.toml ...e per OpenAI Codex
</FileTree>
...
Dal punto di vista di Node, questo significa un singolo lockfile alla radice con un singolo `node_modules` contenente tutte le dipendenze. Ogni progetto Node dichiara le dipendenze runtime che il suo codice sorgente importa nel proprio `package.json`, mentre gli strumenti condivisi di build/test risiedono nelle `devDependencies` del `package.json` radice. Aggiungi una dipendenza runtime di un progetto con il flag filter/workspace del tuo package manager (ad es. `pnpm add <pkg> --filter <project>`), e gli strumenti di sviluppo condivisi alla radice.

Per i package manager con supporto per i catalog ([pnpm](https://pnpm.io/catalogs), [yarn](https://yarnpkg.com/features/catalogs) e [bun](https://bun.com/docs/install/catalogs)), le versioni delle dipendenze sono registrate nel catalog e referenziate con il protocollo `catalog:`, mantenendo un'unica fonte di verità per le versioni in ogni `package.json` del progetto. Per npm workspaces, raccomandiamo [syncpack](https://github.com/JamieMason/syncpack) per allineare le versioni dichiarate in più file `package.json`.
