...
Il valore predefinito di Nx è 3, che lascia i core inattivi in questi workspace — ogni progetto contribuisce con diversi target (`lint`, `compile`, `test`, e spesso `bundle` e `docker`), producendo un ampio grafo di attività con molto lavoro indipendente. Aumentare questo valore oltre 8 tende a non aiutare, poiché il percorso critico del grafo diventa il limite piuttosto che i core disponibili.
...
Il workspace viene fornito con un file `aws-nx-plugin.config.mts` alla radice. I generatori leggono questo file per scegliere impostazioni predefinite sensate in modo da non dover passare gli stessi flag ogni volta. Due impostazioni sono particolarmente utili:
...
Puoi modificare entrambe le impostazioni in qualsiasi momento — le successive esecuzioni del generatore rileveranno il nuovo valore.