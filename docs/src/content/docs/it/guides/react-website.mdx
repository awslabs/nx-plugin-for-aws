...
:::note[Script `dev`]
Se non esiste uno script `dev` nel tuo `package.json` root quando questo generatore viene eseguito, viene aggiunto uno script `dev` che invoca `serve-local` per il sito web generato, il che significa che puoi anche avviare il server di sviluppo locale con:

<PackageManagerShortCommand commands={["dev"]} />

Questo rende il primo sito web React che generi il target `dev` predefinito per il workspace. I siti web successivi non sovrascrivono lo script `dev` esistente — puoi invocare i loro target `serve-local` direttamente, o aggiornare lo script manualmente.
:::
