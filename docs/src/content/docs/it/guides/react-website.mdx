...
Scegliere `S3_MANAGED` fa fallire il target `checkov` del tuo progetto con `CKV_AWS_145` sui bucket del sito web e dei log di distribuzione. Il target esegue `checkov` contro il `checkov.yml` del tuo progetto, quindi aggiungi il controllo alla sua lista `skip-check`:
...
Il target `checkov` passa `--config-file`, che disattiva la scoperta automatica di un `.checkov.yaml` da parte di `checkov` — un file di configurazione inserito altrove viene ignorato.
:::
...
Disabilitare la rotazione delle chiavi sulla chiave creata automaticamente fa fallire il target `checkov` del tuo progetto con `CKV_AWS_7`. Aggiungilo alla stessa lista `skip-check` in `packages/infra/checkov.yml`:
...
La configurazione dalla tua infrastruttura viene fornita al tuo sito web tramite <Link path="guides/runtime-config">Configurazione runtime</Link>. Questo consente al tuo sito web di accedere a dettagli come gli URL delle API che non sono noti fino a quando la tua applicazione non viene distribuita.
...
Il tuo sito web viene generato con un `RuntimeConfigProvider` (in `src/components/RuntimeConfig`) che carica `runtime-config.json` prima di renderizzare la tua applicazione, e un hook `useRuntimeConfig` per leggere i valori da esso:
...
Il provider ricade su una configurazione vuota quando non viene servito alcun `runtime-config.json`, quindi un sito web senza nulla ancora distribuito viene comunque renderizzato. `IRuntimeConfig` è tipizzato `any` di default — restringilo in `src/components/RuntimeConfig/index.tsx` se desideri un accesso type-safe.
