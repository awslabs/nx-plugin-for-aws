...
`src/hooks/useRuntimeConfig.tsx` e `src/components/RuntimeConfig` vengono aggiunti al tuo sito web dal generatore <Link path="/guides/react-website-auth">`ts#website#auth`</Link> e dai generatori <Link path="guides/connection">`connection`</Link>, poiché sono quelli che inseriscono valori nella configurazione runtime. Un sito web senza nessuno dei due non ha configurazione runtime da leggere, quindi l'hook non viene fornito finché uno non è stato eseguito.
:::
