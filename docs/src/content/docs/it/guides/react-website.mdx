...
Il target `serve-local` (alias di `dev`) avvia un server di sviluppo locale per il tuo sito web (con [Vite `MODE`](https://vite.dev/guide/env-and-mode) impostato a `serve-local`), avviando anche server locali per API che hai connesso al sito tramite il <Link path="/guides/connection">generatore Connection</Link>.
...
:::tip[Comportamento di Serve-Local]
