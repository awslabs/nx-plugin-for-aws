...
:::note
Con CDK, il construct del sito web può essere dichiarato in qualsiasi punto dello stack. La configurazione runtime è risolta in modo lazy al momento del synth, quindi tutti i valori saranno inclusi indipendentemente dall'ordine di dichiarazione.
:::
</Fragment>
<Fragment slot="terraform">
...
:::note
Per dettagli su come la configurazione runtime viene memorizzata in AWS AppConfig e come i consumer lato server (funzioni Lambda, agenti) possono recuperarla, consulta la guida <Link path="guides/runtime-config">Configurazione Runtime</Link>.
:::
