...
:::note[Aggiunto quando ne hai bisogno]
`src/hooks/useRuntimeConfig.tsx` e `src/components/RuntimeConfig` vengono aggiunti al tuo sito web dal generatore <Link path="/guides/react-website-auth">`ts#website#auth`</Link> e dai generatori <Link path="guides/connection">`connection`</Link>, poiché sono questi che inseriscono valori nella configurazione runtime. Un sito web senza nessuno dei due non ha configurazione runtime da leggere, quindi l'hook non viene fornito fino a quando uno non è stato eseguito.
:::
