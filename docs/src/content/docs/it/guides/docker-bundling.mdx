...
Il pacchetto `shx` fornisce comandi `cp`, `rm` e `mkdir` cross-platform, evitando i builtin della shell (non disponibili su Windows). Installalo alla radice del tuo workspace con `pnpm add -D -w shx`.

Copiare una directory richiede `-R` e un `/.` finale sulla sorgente — `shx cp -R src/. dst` — che unisce i contenuti in `dst`. Senza il `/.` la sorgente viene nidificata all'interno di `dst` invece, e a differenza di un glob `src/*` include ancora i dotfile.
:::
