...
Poiché l'immagine è completamente determinata dal sorgente del tuo progetto, dichiarare `inputs` (qui `default` e `^production`, corrispondenti al target `bundle`) permette a Nx di cachare la scansione e saltare la ri-scansione di un'immagine che non è cambiata. Fissa la versione dell'immagine Trivy (es. <code>trivy:<TrivyVersion /></code>) in modo che le scansioni siano riproducibili.
:::
