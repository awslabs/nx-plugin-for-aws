...
Fissa ogni dipendenza installata all'interno del `Dockerfile` a una versione esatta (come sopra) — altrimenti `npm install` scaricherà silenziosamente una release più recente la prossima volta che esegui la build, producendo immagini che si comportano diversamente a seconda di quando sono state costruite. Lo stesso consiglio si applica all'immagine base: preferisci un digest o tag specifico (ad esempio `node:22.14.0`) rispetto a un tag fluttuante come `lts`.
:::
...
- <Link path="/guides/ts-strands-agent">generatore `ts#strands-agent`</Link> — un esempio completo di questo pattern per un agente TypeScript distribuito su Bedrock AgentCore Runtime.
- <Link path="/guides/py-strands-agent">generatore `py#strands-agent`</Link> — l'equivalente per Python.
