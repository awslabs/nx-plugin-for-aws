...
2. Il repository condiviso `core/asset-ecr` contiene l'immagine. Ogni container nel workspace viene pubblicato qui, quindi nulla fornisce un repository proprio — istanzia il modulo una volta per deployment e passa il suo `repository_url` in ogni modulo che pubblica un'immagine.
