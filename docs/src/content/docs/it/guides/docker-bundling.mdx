...
:::tip[Docker o Finch]
Il motore di container utilizzato per costruire le immagini viene scelto al momento della creazione del workspace tramite il flag `--containers` (`docker`, `finch`, o `infer` — il valore predefinito — che rileva automaticamente ciò che è installato). [Finch](https://runfinch.com/) è un'alternativa open-source e drop-in a Docker. La selezione viene registrata in `aws-nx-plugin.config.mts` e applicata a ogni generatore che emette comandi di build dei container. Le build degli asset immagine CDK rispettano la scelta tramite la variabile d'ambiente `CDK_DOCKER`.
:::
...
La scansione è intenzionalmente non collegata a `build` poiché questo può introdurre attriti non necessari durante l'iterazione in fase di sviluppo, dato che il database di vulnerabilità di Trivy viene continuamente aggiornato con nuove CVE.

Invece raccomandiamo di eseguire il comando sopra come step dedicato nella tua pipeline CI prima del deployment agli stage di produzione.
:::
