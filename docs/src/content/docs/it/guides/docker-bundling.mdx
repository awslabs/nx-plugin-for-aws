...
Diversi generatori (come <Link path="/guides/ts-agent">`ts#agent`</Link> e <Link path="/guides/py-agent">`py#agent`</Link>) producono un'immagine Docker che viene inviata ad Amazon ECR e consumata dall'infrastruttura AWS. Questa guida descrive il pattern che seguono in modo che tu possa applicarlo ad altri casi d'uso — ad esempio, eseguire un progetto <Link path="/guides/fastapi">FastAPI</Link> su Amazon ECS, o distribuire un server Express containerizzato.
...
- <Link path="/guides/ts-agent">Generatore `ts#agent`</Link> — un esempio completo di questo pattern per un agent TypeScript distribuito su Bedrock AgentCore Runtime.
