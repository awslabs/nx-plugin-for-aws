...
3. Una `null_resource` con un provisioner `local-exec` che si autentica su ECR, ri-tagga l'immagine costruita localmente e la carica.
4. La risorsa downstream (ad es. `aws_ecs_task_definition`) fa riferimento a `"${aws_ecr_repository.repo.repository_url}:latest"`.
