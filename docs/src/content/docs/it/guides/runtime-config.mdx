...
- **ARN runtime degli agenti** — registrati automaticamente dai costrutti agent

Il namespace `connection` viene anche distribuito come file `runtime-config.json` nel bucket S3 del tuo sito web, consentendo la scoperta lato client delle risorse backend.

Puoi definire tutti i namespace aggiuntivi che desideri, fornendo una comoda alternativa alle variabili d'ambiente per passare valori al momento del deploy come i nomi delle tabelle DynamoDB alle tue funzioni Lambda o ad altre risorse di calcolo.
...
Al momento della sintesi/deploy, `RuntimeConfig` crea un'applicazione **AWS AppConfig** contenente:
...
Devi quindi dichiarare il modulo `runtime-config/appconfig` per creare le risorse AppConfig da tutti i file namespace:
...
:::note
Ogni costrutto generato (API, agenti) crea la propria istanza del modulo `appconfig` autonoma. Queste sono tutte identiche — ognuna legge lo stesso insieme di file namespace e produce la stessa applicazione AppConfig. Questo garantisce che ogni modulo sia distribuibile in modo indipendente.
:::
</Fragment>
</Infrastructure>
...
:::note
I costrutti API e agent generati fanno questo automaticamente — devi farlo manualmente solo se crei le tue funzioni Lambda o risorse di calcolo che necessitano di leggere la configurazione runtime.
:::
</Fragment>
<Fragment slot="terraform">
Fai riferimento all'output `application_id` del modulo `appconfig` per ottenere l'AppConfig Application ID e aggiungi le dichiarazioni di policy IAM appropriate:
...
I consumatori lato server come le funzioni Lambda e gli agenti possono recuperare la configurazione runtime da **AWS AppConfig** utilizzando [AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/).
