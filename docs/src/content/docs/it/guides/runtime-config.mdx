...
`getAppConfig` restituisce qualsiasi valore JSON valido, quindi dichiara la forma che ti aspetti e fai il cast del risultato — altrimenti gli accessi alle proprietà non compilano sotto lo strict `tsconfig` del workspace:
...
:::tip[Valida al confine]
Un cast dice al compilatore la forma ma non la verifica a runtime. Per la configurazione che non controlli, analizzala invece con uno schema [Zod](https://zod.dev/), in modo che un valore malformato fallisca in modo evidente piuttosto che emergere come `undefined` più tardi.
:::
