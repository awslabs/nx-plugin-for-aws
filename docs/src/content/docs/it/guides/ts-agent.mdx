...
Se hai selezionato `None` per `computeType`, non vengono generati costrutti CDK o moduli Terraform — lo Agent può essere eseguito solo localmente. L'opzione `auth` viene ignorata in questa modalità poiché non esiste un endpoint ospitato da autenticare.
</OptionFilter>
