...
Per invocare il tuo Strands Agent da un sito web React, puoi utilizzare il <Link path="/guides/connection/react-ts-agent">generatore `connection`</Link>, che configura automaticamente un client tRPC WebSocket con l'autenticazione corretta (IAM o Cognito).
