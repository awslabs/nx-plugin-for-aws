...
Il generatore configura un target Nx `<your-agent-name>-chat` che ti porta in una chat interattiva da terminale con il tuo agente, alimentata da [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli).

Il target chat funziona in modo autonomo — **non** avvia l'agente per te. Per impostazione predefinita si connette al tuo agente in esecuzione locale, quindi avvia prima `<your-agent-name>-serve-local` (in un terminale separato):
