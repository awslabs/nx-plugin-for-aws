...
Aggiungi una procedura che avvia una conversazione. Genera l'ID di sessione stesso, risolve il chiamante dal proprio contesto di richiesta e memorizza la coppia — ad esempio in una tabella creata con il generatore <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link>:

| Attribute | Example | Notes |
| --- | --- | --- |
| `sessionId` | `a5f1…` | Partition key — the agent looks up an owner from the session ID it is given |
| `userId` | `<sub>` | The authenticated caller who owns the session |
| `createdAt`, `title` | … | Anything your UI needs to list conversations |

Aggiungi un indice secondario su `userId` se vuoi anche elencare le sessioni di un utente.
...
Una seconda procedura che interroga quell'indice consente all'interfaccia utente di offrire a un utente le proprie conversazioni senza mai vedere quelle di qualcun altro.
