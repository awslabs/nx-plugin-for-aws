...
- **AG-UI**: Utilizza il [protocollo AG-UI](https://docs.ag-ui.com/) (SSE su POST) tramite [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands) per l'integrazione diretta con il frontend tramite [CopilotKit](https://docs.copilotkit.ai/aws-strands). Ideale quando desideri un'interfaccia utente di chat ricca con streaming, visualizzazione delle chiamate agli strumenti e gestione dello stato.
...
Per consumare i Server MCP che hai creato utilizzando i generatori <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> o <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> puoi utilizzare il <Link path="/guides/connection/ts-agent-mcp">generatore `connection`</Link>.
...
L'`index.ts` generato avvolge il tuo Strands `Agent` in uno [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands) `StrandsAgent` e crea un'app Express tramite `createStrandsApp()`. L'app risultante espone un singolo endpoint POST che trasmette eventi [AG-UI](https://docs.ag-ui.com/) tramite Server-Sent Events (SSE), oltre a `/ping` per il controllo dello stato del runtime AgentCore.
...
Il generatore configura un target denominato `<your-agent-name>-serve`, che avvia il tuo Agent localmente per lo sviluppo e il testing.
...
Il target chat funziona in modo autonomo. Per impostazione predefinita si connette al tuo agente in esecuzione locale, quindi avvia prima `<your-agent-name>-serve-local` (in un terminale separato):
...
Per delegare lavoro da questo agente a un agente A2A remoto (sia <Link path="/guides/ts-agent">TypeScript</Link> che <Link path="/guides/py-agent">Python</Link>), utilizza il <Link path="/guides/connection/ts-agent-a2a">generatore `connection`</Link>. Fornisce un client autenticato con SigV4 per l'agente di destinazione e trasforma tramite AST l'`agent.ts` di questo agente per registrare l'agente A2A remoto come `tool` di Strands.
