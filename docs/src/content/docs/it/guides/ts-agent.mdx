...
Viene generato anche un target `<your-agent-name>-serve`, che esegue l'agente contro la tua infrastruttura distribuita e quindi richiede che sia impostato `RUNTIME_CONFIG_APP_ID`. Consulta la guida <Link path="guides/local-development">Sviluppo Locale</Link> per la differenza tra `dev` e `serve`.
