...
`deploy-sandbox` e `deploy` dipendono da `^package`, quindi costruiscono gli artefatti che stanno per distribuire e nient'altro. `deploy-ci` distribuisce un assembly che la tua pipeline ha già costruito, quindi non ha alcuna dipendenza di build.

:::caution
Poiché i target di distribuzione saltano i gate di lint, test e type-check, la distribuzione non fallirà in caso di errore di lint o di un test unitario fallito. Esegui `build` prima di distribuire qualsiasi cosa a cui tieni, e vincola la tua pipeline su di esso:
