...
Se hai bisogno di un deploy standard che attende la completa stabilizzazione delle risorse, disattivala con `--no-express`:

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

Il target `deploy-ci` non utilizza la modalità express, quindi i deploy delle pipeline CI/CD attendono sempre la completa stabilizzazione.
:::
