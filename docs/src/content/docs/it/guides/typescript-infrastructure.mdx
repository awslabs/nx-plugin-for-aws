...
| `deploy`         | Distribuire qualsiasi stage, nominando lo stage o gli stack che desideri. Utilizza la modalità express a meno che non generi con `stageConfig`.  |
...
Se questo target utilizza la modalità express dipende da come hai generato il progetto:

- **Con `stageConfig`** attende la stabilizzazione completa delle risorse, poiché il target può nominare qualsiasi stage incluso quello di produzione. Passa `--express` per optare per la modalità express per uno stage su cui stai iterando.
- **Senza `stageConfig`** utilizza la modalità express, come fa `deploy-sandbox`. Poiché lo stage che nomini potrebbe essere uno di produzione, passa `--no-express` per attendere la stabilizzazione completa delle risorse:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Usa [`deploy-ci`](#distribuire-su-aws-in-una-pipeline-cicd) da una pipeline invece — non utilizza mai la modalità express.
...
Questo target differisce dai target `deploy` e `deploy-sandbox` in due modi. Primo, distribuisce un cloud assembly pre-sintetizzato piuttosto che sintetizzare al volo, evitando potenziali non-determinismi da cambiamenti di versione dei pacchetti e assicurando che ogni fase della pipeline distribuisca utilizzando lo stesso cloud assembly. Secondo, non utilizza mai la modalità express, quindi attende sempre che ogni risorsa si stabilizzi completamente prima di segnalare il successo.
