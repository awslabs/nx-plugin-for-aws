...
Questa sezione si applica quando generi con `stageConfig`. Senza questa opzione, il generatore produce una configurazione più semplice dove gestisci tu stesso le credenziali AWS (ad esempio, esportando `AWS_PROFILE` prima del deploy).
:::
...
Per maggiori informazioni su CDK, consulta la [CDK Developer Guide](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html) e l'[API Reference](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html).ml) e l'[API Reference](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html).