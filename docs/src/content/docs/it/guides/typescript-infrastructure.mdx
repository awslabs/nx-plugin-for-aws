...
`synth` dipende da `^assemble` piuttosto che da `^build`, quindi la sintesi costruisce gli artefatti che i tuoi stack distribuiscono — i bundle Lambda, le immagini container e le specifiche generate — senza eseguire anche i gate di lint, test e type-check di ogni progetto upstream. Consulta <Link path="guides/typescript-project#assemblaggio">Assemblaggio</Link> per come `assemble` si relaziona a `build`.
...
:::caution
Poiché i target di distribuzione saltano i gate di lint, test e type-check, la distribuzione non fallirà in caso di errore di lint o di un test unitario fallito. Esegui `build` prima di distribuire qualsiasi cosa a cui tieni, e blocca la tua pipeline su di esso:
...
I target di distribuzione attendono per impostazione predefinita la stabilizzazione completa delle risorse, favorendo la coerenza rispetto alla velocità. Per accelerare l'iterazione durante lo sviluppo, opta per la [modalità express di CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) passando il flag `--express` di CDK:

<NxCommands commands={['deploy-sandbox <my-infra> --express']} />

La modalità Express termina una volta che la configurazione della risorsa è confermata applicata piuttosto che attendere la stabilizzazione completa, con la propagazione che continua in background, per cicli di iterazione significativamente più veloci. È destinata allo sviluppo e all'iterazione.
:::
