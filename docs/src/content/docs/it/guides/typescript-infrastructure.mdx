...
Se hai utilizzato il generatore <Link path="guides/react-website">CloudScape website</Link>, noterai che è già disponibile un construct in `packages/common/constructs` per deployarlo. Ad esempio:
...
:::warning
È importante assicurarsi che il sito web venga dichiarato _dopo_ eventuali construct di API per permettere alla <Link path="guides/react-website#runtime-configuration">Runtime Config</Link> del sito di includere tutte le configurazioni delle API.
:::
