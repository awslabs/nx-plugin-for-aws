...
Questa sezione si applica quando generi con `stageConfig`. Senza di essa, il generatore produce una configurazione più semplice in cui gestisci le credenziali AWS da solo (ad esempio, esportando `AWS_PROFILE` prima della distribuzione).
:::
...
| `deploy`         | Distribuire qualsiasi stage durante lo sviluppo, nominando lo stage o gli stack che desideri. Utilizza la modalità express. |
...
I target `deploy-sandbox` e `deploy` utilizzano la [modalità express di CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) per impostazione predefinita (tramite il flag `--express` di CDK). La modalità Express termina una volta che la configurazione della risorsa è confermata applicata piuttosto che attendere la stabilizzazione completa, con la propagazione che continua in background, per cicli di iterazione significativamente più veloci.

Per attendere invece la stabilizzazione completa delle risorse, passa `--no-express`:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox` e `deploy` sono solo per lo sviluppo]
Poiché la modalità express ritorna prima che le risorse abbiano terminato la stabilizzazione, `deploy-sandbox` e `deploy` sono destinati allo sviluppo e all'iterazione, non per distribuzioni CI/CD o in produzione. Usa [`deploy-ci`](#distribuire-su-aws-in-una-pipeline-cicd) nella tua pipeline — non utilizza mai la modalità express, quindi attende sempre la stabilizzazione completa.
:::
...
Questo target differisce dai target `deploy` e `deploy-sandbox` in due modi. Primo, distribuisce un cloud assembly pre-sintetizzato piuttosto che sintetizzare al volo, evitando potenziali non-determinismi da cambiamenti di versione dei pacchetti e assicurando che ogni fase della pipeline distribuisca utilizzando lo stesso cloud assembly. Secondo, non utilizza la modalità express, quindi attende che ogni risorsa si stabilizzi completamente prima di segnalare il successo.

:::caution[Usa sempre `deploy-ci` per la produzione]
`deploy` e `deploy-sandbox` utilizzano la modalità express e ritornano prima che le risorse abbiano terminato la stabilizzazione, il che li rende inadatti per distribuzioni CI/CD e in produzione. Usa `deploy-ci` per quelle.
:::
