...
:::tip[Funzionalità del progetto TypeScript]
...
:::tip[Committare la config dello stage]
...
Con CDK, il costrutto del website può essere dichiarato in qualsiasi punto dello stack — la configurazione runtime viene risolta in modo lazy al momento della sintesi. Per Terraform, assicurati che il modulo del website abbia un `depends_on` appropriato per i moduli che aggiungono al <Link path="guides/react-website#runtime-configuration">Runtime Config</Link>.
:::
...
Il target `deploy` utilizza la [modalità express di CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) per impostazione predefinita (tramite il flag `--express` di CDK) per velocizzare i deploy durante lo sviluppo. La modalità express termina una volta che la configurazione delle risorse è confermata applicata anziché attendere la completa stabilizzazione, con la propagazione che continua in background, per cicli di iterazione significativamente più veloci.

La modalità express è pensata per lo sviluppo e l'iterazione. Per eseguire il deploy senza di essa e attendere la completa stabilizzazione delle risorse, rimuovi il flag `--express` dal comando del target `deploy` nel file `project.json` del tuo progetto di infrastruttura.

Per deploy reali verso stage di produzione, utilizza il target `deploy-ci` (vedi [sotto](#deploying-to-aws-in-a-cicd-pipeline)) come parte di una pipeline CI/CD. Questo target non utilizza la modalità express, quindi attende sempre la completa stabilizzazione.
:::
