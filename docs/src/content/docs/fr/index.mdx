---
title: "@aws/nx-plugin"
description: Créez rapidement des applications sur AWS, en vous rapprochant de la production.
template: splash
hero:
  tagline: Créez des applications AWS full-stack en quelques minutes, pas en plusieurs jours.
  image:
    light: ../assets/bulb-black.svg
    dark: ../assets/bulb-white.svg
  actions:
    - text: Commencer
      link: get_started/quick-start
      icon: right-arrow
      variant: primary
    - text: Créer avec l'IA
      link: get_started/building-with-ai
      icon: rocket
      variant: secondary
---

import {
  CardGrid,
  Card,
  Icon,
  LinkButton,
} from "@astrojs/starlight/components";
import { ContributorList } from "astro-contributors";
import CopyCommand from "@components/copy-command.astro";
import GeneratorCarousel from "@components/generator-carousel.astro";

<div class="landing-section landing-hero-extras">
  <p class="landing-subline">
    Une collection de générateurs Nx pour créer rapidement des applications cloud-natives sur AWS — type-safe, exécutables localement et prêtes à déployer.
  </p>

  <div class="landing-install">
    <span class="landing-install-label">Créer un nouvel espace de travail</span>
    <CopyCommand command="pnpm create @aws/nx-workspace" />
  </div>
</div>

<div class="landing-section landing-carousel-section">
  <h2 class="landing-heading">Générez n'importe quoi. Connectez le tout.</h2>
  <p class="landing-lede">
    Chaque générateur produit du code d'application suivant les meilleures pratiques <em>et</em> l'infrastructure pour le déployer, en vous rapprochant de la production. Exécutez-les de manière interactive — vous serez invité à fournir les valeurs dont vous avez besoin.
  </p>
  <GeneratorCarousel />
</div>

<div class="landing-section">
  <h2 class="landing-heading">Pourquoi @aws/nx-plugin ?</h2>
  <CardGrid>
    <Card title="Full-stack en quelques minutes" icon="puzzle">
      Site web, API, authentification, agents et infrastructure — tout est généré et connecté ensemble.
    </Card>
    <Card title="Créer avec l'IA" icon="rocket">
      Un serveur MCP est fourni avec le plugin pour que votre assistant IA puisse générer et connecter des projets pour vous.
    </Card>
    <Card title="Type-safe, de bout en bout" icon="seti:json">
      Les types partagés circulent de votre API jusqu'à votre frontend. Refactorisez en toute confiance.
    </Card>
    <Card title="Votre code, à votre façon" icon="setting">
      Les générateurs vous donnent une longueur d'avance, pas un framework. Le code vous appartient pour le lire, le modifier et l'étendre.
    </Card>
  </CardGrid>
</div>

<div class="landing-section landing-community">
  <h2 class="landing-heading">Rejoignez la communauté</h2>
  <p>Vous avez des questions ou souhaitez partager vos commentaires ?</p>
  <p>
    Rejoignez-nous sur le canal Slack{" "}
    <a href="https://cdk-dev.slack.com/archives/C0AG11EUHM4">
      <strong>#nx-plugin-for-aws</strong>
    </a>{" "}
    pour échanger avec d'autres utilisateurs et contributeurs.
  </p>
  <LinkButton
    href="https://cdk-dev.slack.com/archives/C0AG11EUHM4"
    icon="slack"
    variant="secondary"
  >
    Rejoignez-nous sur Slack
  </LinkButton>
</div>

<div class="landing-section">
  <h2 class="landing-heading">Merci à nos incroyables contributeurs !</h2>
  <ContributorList githubRepo="awslabs/nx-plugin-for-aws" />
</div>
