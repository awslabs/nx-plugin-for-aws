...
Utilisez le serveur MCP avec l'assistant IA de votre choix comme Kiro, Kiro CLI, Cline, Claude Code ou Cursor.

Pour les autres assistants IA, vous pouvez configurer le serveur MCP manuellement. La plupart des assistants IA utilisent un fichier JSON pour la configuration du serveur MCP. Dans Kiro, ce fichier se trouve dans `~/.kiro/settings/mcp.json`. Ajoutez une entrée pour le serveur MCP du Plugin Nx pour AWS :
