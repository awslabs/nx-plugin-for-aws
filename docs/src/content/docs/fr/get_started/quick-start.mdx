...
### Supprimer les ressources de bootstrap

Les ressources que vous avez initialisées sont partagées par tous les projets déployés sur ce compte AWS et cette région, et ne coûtent presque rien à laisser en place. Supprimez-les uniquement si rien d'autre ne se déploie là-bas.
...
Le bucket d'assets est conservé intentionnellement, donc supprimez-le séparément. Il est versionné, ce que `aws s3 rb --force` ne peut pas gérer, donc supprimez d'abord chaque version et marqueur de suppression :
...
Chaque `delete-objects` signale une erreur plutôt que de ne rien faire lorsqu'il ne reste rien de ce type à supprimer, ce qui peut être ignoré en toute sécurité.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Votre état Terraform est également supprimé]
Cela vide et supprime le bucket S3 contenant votre état Terraform, donc exécutez-le uniquement une fois que rien de ce qui vous importe n'est encore déployé à partir de cet état.
:::
</Fragment>
</Infrastructure>
