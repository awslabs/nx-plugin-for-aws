...
:::tip
Pour un tutoriel plus approfondi sur la création d'une application full-stack, consultez le <Link path="get_started/tutorials/dungeon-game/overview">Dungeon Adventure Tutorial</Link>.
:::
...
Ceci génère automatiquement un nouveau site React dans `packages/demo-website`.
...
Ceci configure les fournisseurs nécessaires pour que votre site puisse appeler votre API tRPC.
...
Ceci est tout le CDK nécessaire pour déployer notre application full-stack.
...
## Étape 5 : Exécuter le frontend en local
