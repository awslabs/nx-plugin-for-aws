...
Ce guide vous explique les bases de l'installation et de l'utilisation de `@aws/nx-plugin` pour développer rapidement des projets sur AWS. Test.
...
:::tip
Vous serez invité à choisir votre fournisseur d'infrastructure as code (IaC), soit [CDK](https://docs.aws.amazon.com/cdk/) soit [Terraform](https://developer.hashicorp.com/terraform). Vous pouvez ignorer l'invite en exécutant la commande avec `--iacProvider`, par exemple :
...
Ceci générera un nouveau site React dans `packages/demo-website`.
...
## Étape 3 : Exécuter votre site web et votre API en local
...
Les modifications apportées à votre site et à votre API seront reflétées en temps réel grâce au rechargement automatique des serveurs locaux.
...
:::tip
L'initialisation CDK est requise à la fois pour votre région AWS de déploiement cible et pour `us-east-1` afin de permettre le déploiement du WebACL AWS WAF pour le site.
:::
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['run infra:bootstrap']} />
...
:::tip
L'initialisation Terraform crée un bucket S3 pour stocker vos fichiers d'état Terraform. Cela ne doit être fait qu'une fois par combinaison compte AWS/région.
:::
</Fragment>
</Infrastructure>
...
:::tip
Cette commande exécutera d'abord `terraform plan` pour afficher les modifications prévues, puis appliquera ces modifications pour déployer votre infrastructure.
:::
</Fragment>
</Infrastructure>
