...
:::note[Dépôts Git existants]
Si le répertoire de l'espace de travail se trouve déjà dans un dépôt Git, l'initialisation Git est ignorée afin de laisser votre dépôt existant intact.
:::

:::tip[Choix d'un fournisseur IaC]
Vous serez invité à choisir votre fournisseur d'infrastructure as code (IaC), soit [CDK](https://docs.aws.amazon.com/cdk/) soit [Terraform](https://developer.hashicorp.com/terraform). Vous pouvez ignorer cette étape en ajoutant `--iac` à la commande, par exemple :
...
:::tip[Correction des erreurs de lint]
