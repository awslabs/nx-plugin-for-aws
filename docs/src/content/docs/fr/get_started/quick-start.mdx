...
:::note[Sorties du module racine]
Terraform n'affiche que les sorties déclarées dans `packages/infra/src`, donc tout ce que vous souhaitez lire après un déploiement doit être ré-exporté depuis le module qui le produit — d'où les deux blocs `output` ci-dessus.
:::
</Fragment>
</Infrastructure>
...
Votre site web est également servi depuis CloudFront. Ouvrez le nom de domaine de distribution affiché dans vos sorties de déploiement — la sortie se terminant par `DistributionDomainName` pour CDK, ou `website_url` pour Terraform — pour utiliser la version fonctionnant entièrement sur AWS.
...
Le pool d'utilisateurs Cognito est créé avec la protection contre la suppression activée, il survit donc aux commandes ci-dessus. Désactivez la protection contre la suppression dans la [console Amazon Cognito](https://console.aws.amazon.com/cognito/) avant de le supprimer.

<Infrastructure>
<Fragment slot="cdk">
:::note[Ressources conservées]
La suppression des stacks laisse le pool d'utilisateurs derrière, ainsi que le rôle IAM que Cognito utilise pour envoyer des messages SMS, le rôle IAM qu'API Gateway utilise pour écrire dans CloudWatch Logs, les clés KMS chiffrant le bucket du site web et les groupes de journaux de l'API et du WAF, et les groupes de journaux eux-mêmes. Supprimez le pool, les rôles et les groupes de journaux, et planifiez la suppression des clés, une fois que vous n'en avez plus besoin.
:::
</Fragment>
<Fragment slot="terraform">
:::caution[La destruction échoue tant que la protection contre la suppression est activée]
Terraform tente de supprimer le pool d'utilisateurs, donc `destroy` s'arrête avec `The user pool cannot be deleted because deletion protection is activated` et laisse en place les ressources qu'il n'avait pas encore atteintes. Désactivez la protection contre la suppression, puis exécutez à nouveau la commande de destruction pour tout supprimer — le pool d'utilisateurs et le rôle IAM que Cognito utilise pour envoyer des messages SMS inclus.
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">Add to an Existing Project</Link> — adoptez le plugin dans un espace de travail Nx ou un monorepo que vous avez déjà.
