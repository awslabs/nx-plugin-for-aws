...
Votre site web est protégé par Amazon Cognito, et l'auto-inscription est désactivée par défaut, alors créez votre premier utilisateur dans le pool d'utilisateurs que vous avez déployé. Vous pouvez trouver l'identifiant du pool d'utilisateurs dans vos sorties de déploiement, ou dans la [console Amazon Cognito](https://console.aws.amazon.com/cognito/) :
...
Supprimez `--message-action SUPPRESS` pour que Cognito envoie le mot de passe temporaire par e-mail à l'adresse que vous avez fournie, plutôt que de le partager vous-même avec l'utilisateur. Vous pouvez également créer des utilisateurs à partir de l'onglet **Users** de votre pool d'utilisateurs dans la console Cognito.
...
:::tip[Auto-inscription]
Exécutez le générateur <Link path="guides/react-website-auth">`ts#website#auth`</Link> avec `--allowSignup=true` si vous préférez que les utilisateurs s'inscrivent eux-mêmes depuis la page de connexion plutôt que de créer chaque utilisateur vous-même.
:::

## Étape 6 : Tester le site web avec les ressources cloud déployées
...
Votre site web sera disponible à l'adresse `http://localhost:4200` et pointera vers les ressources que vous avez déployées pour l'API et l'authentification. Connectez-vous avec l'utilisateur que vous avez créé pour le voir en action.

Votre site web est également servi depuis CloudFront — ouvrez son nom de domaine de distribution pour utiliser la version fonctionnant entièrement sur AWS.
