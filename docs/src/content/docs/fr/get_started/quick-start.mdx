...
:::tip
Vous devez vous assurer que votre environnement AWS a été correctement initialisé pour CDK.

L'initialisation CDK est requise pour votre région AWS de déploiement cible, ainsi que `us-east-1` pour permettre le déploiement du WebACL AWS WAF pour le site.
