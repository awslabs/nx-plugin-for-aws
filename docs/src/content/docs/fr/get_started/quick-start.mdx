...
Le <Link path="get_started/building-with-ai">serveur MCP Nx Plugin for AWS</Link> est déjà configuré dans votre espace de travail, vous pouvez donc demander à votre agent de codage IA préféré comme Kiro ou Claude de créer ces projets pour vous au lieu de taper vous-même les commandes CLI.
:::
...
Félicitations ! 🎉 Vous avez créé et déployé avec succès une application full-stack en utilisant `@aws/nx-plugin` !
