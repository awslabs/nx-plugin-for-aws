...
- .mcp.json configure le serveur MCP pour les agents de codage pris en charge sauf si désactivé (également les équivalents .cursor/, .kiro/, .gemini/, .vscode/ et .codex/). Il exécute le binaire `aws-nx-mcp` depuis le propre `@aws/nx-plugin` de l'espace de travail, de sorte que le serveur correspond toujours à la version du plugin installée
</FileTree>
