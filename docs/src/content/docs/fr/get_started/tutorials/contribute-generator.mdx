...
Nous disposons d'une suite de "smoke tests" qui exécutent les générateurs dans un espace de travail vierge et s'assurent que tout se compile. Au minimum, votre nouveau générateur devrait être ajouté aux **deux** matrices de générateurs afin qu'il soit testé par les smoke tests :
...
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — un générateur caché qui compose tous les autres en processus sur un seul arbre.

Les deux valent la peine d'être conservées. La matrice CLI teste le chemin que les utilisateurs empruntent réellement. La matrice composée est livrée avec le plugin, donc chaque version publiée contient la matrice des générateurs qu'_elle_ avait — ce qui permet aux tests qui mettent à niveau un ancien espace de travail de le structurer avec la matrice de cette version. Comme elle appelle les générateurs directement plutôt que via la CLI, ses options sont vérifiées par type par rapport au schéma de chaque générateur, donc une nouvelle option requise ou un enum modifié fait échouer la compilation là plutôt que de réduire silencieusement la couverture.
