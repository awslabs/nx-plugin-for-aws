...
Nous disposons d'une suite de "smoke tests" qui exécutent les générateurs dans un espace de travail vierge et s'assurent que tout se compile. Au minimum, votre nouveau générateur devrait être ajouté à la matrice de générateurs dans `packages/nx-plugin/src/internal/test-matrix/generator.ts` afin qu'il soit testé par les smoke tests.

La matrice est elle-même un générateur (caché) qui compose tous les autres, elle est donc livrée avec le plugin. Cela signifie que chaque version publiée contient la matrice des générateurs qu'_elle_ avait, ce qui permet aux tests qui mettent à niveau un ancien espace de travail de le structurer avec la matrice de cette version. Comme elle appelle les générateurs directement plutôt que via la CLI, les options sont vérifiées par type par rapport au schéma de chaque générateur — ainsi, une nouvelle option requise ou un enum modifié fera échouer la compilation ici plutôt que de réduire silencieusement la couverture.
