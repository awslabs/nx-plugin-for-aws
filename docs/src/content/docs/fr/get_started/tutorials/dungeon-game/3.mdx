...
<Tabs>
  ...
  Démarrez votre serveur Agent local en exécutant la commande suivante :
    ...
    La cible `agent-serve` démarre l'agent localement mais se connecte à votre serveur MCP **déployé** (via le `RUNTIME_CONFIG_APP_ID`). Si vous préférez exécuter le serveur MCP localement également, utilisez `agent-serve-local` à la place, qui démarre tous les services dépendants et définit `SERVE_LOCAL=true` pour les connexions HTTP directes.
    :::

    Une fois que le serveur Agent est opérationnel (vous ne verrez aucune sortie), appelez-le en exécutant la commande suivante :
...
Si la commande s'exécute avec succès, vous devriez commencer à voir le texte initial de l'histoire diffusé sous forme de JSON Lines :
