...
La StoryApi comprend une unique API `generate_story` qui, étant donné un `Game` et une liste d'`Action` comme contexte, fait progresser une histoire. Cette API sera implémentée comme une API de streaming en Python/FastAPI et démontrera comment adapter le code généré pour répondre aux besoins spécifiques.
...
- Le paramètre `x-query` indique que bien qu'il s'agisse d'une requête POST, nous la traitons comme une `query` plutôt qu'une `mutation`, permettant à TanStack Query de gérer efficacement l'état du streaming.
...
Mettons maintenant à jour la `story_api` pour supporter le [Lambda Web Adapter](https://github.com/awslabs/aws-lambda-web-adapter).
...
<Drawer title="Commande de déploiement" trigger="Vous pouvez également déployer toutes les piles en une fois. Cliquez ici pour plus de détails.">

Vous pouvez déployer toutes les piles de l'application CDK avec :

<NxCommands commands={['run @dungeon-adventure/infra:deploy --all']} />

Cette approche est **déconseillée** car vous pourriez souhaiter déployer des étapes séparées (comme `infra-prod`). Le flag `--all` tenterait alors de tout déployer, ce qui peut entraîner des déploiements indésirables !

</Drawer>

Une fois le déploiement terminé, vous devriez voir des sorties similaires à celles-ci _(certaines valeurs ont été masquées)_ :
...
Pour tester notre API, nous pouvons :
<ul>
<li>Démarrer une instance locale du serveur FastAPI et appeler les APIs avec `curl`</li>
<li>
<Drawer title="curl avec Sigv4" trigger="Appeler l'API déployée directement avec curl signé Sigv4">
Ajoutez ce script à votre fichier `.bashrc` (puis `source`-le) ou collez-le directement dans le terminal :
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
<Tabs>
  <TabItem label="Local">
  Démarrez le serveur FastAPI localement avec :
    <NxCommands commands={["run dungeon_adventure.story_api:serve"]} />

    Puis appelez l'API avec :
...
```
    <Aside type="caution">
    Utilisez la valeur de sortie `dungeon-adventure-infra-sandbox.StoryApiStoryApiUrlXXX` du déploiement CDK pour remplacer l'url et ajustez la région.
    </Aside>
  </TabItem>
</Tabs>
