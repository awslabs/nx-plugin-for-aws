...
<Tabs>
  <TabItem label="Local">
  Démarrez votre serveur Agent local :
    ...
    La cible `agent-serve` démarre l'agent localement mais se connecte à votre serveur MCP **déployé** (via le `RUNTIME_CONFIG_APP_ID`). Si vous préférez exécuter le serveur MCP localement également, utilisez plutôt `agent-serve-local`, qui démarre tous les services dépendants et définit `SERVE_LOCAL=true` pour des connexions HTTP directes.
    :::

    Ensuite, dans un second terminal, ouvrez un REPL interactif contre l'agent AG-UI local via la cible générée `agent-chat` :

    <NxCommands commands={["run dungeon_adventure.story:agent-chat"]} />

    Cela utilise [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) pour communiquer avec le protocole AG-UI contre `http://localhost:8081/invocations`. Votre premier message devrait indiquer à l'agent le nom de votre héros et le genre (par exemple : `My name is Alice. Start my zombie adventure.`) et l'histoire sera diffusée en retour.

    <Aside type="tip" title="curl brut">
    Si vous préférez un appel HTTP brut, la forme `RunAgentInput` AG-UI est :
    ```bash
