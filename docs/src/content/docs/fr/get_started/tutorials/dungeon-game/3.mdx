...
:::note
À chaque modification de votre FastAPI, vous devez reconstruire votre projet pour voir les changements reflétés dans le client généré de votre site.

Nous allons effectuer quelques modifications supplémentaires ci-dessous avant de reconstruire.
:::
...
Une fois terminé, vous devriez voir des sorties similaires à ceci _(certaines valeurs ont été masquées)_ :
...
Vous pouvez ajouter ce script à votre `.bashrc` (puis `source`) ou le coller directement dans le terminal :
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
###### URL de fonction Lambda streaming
```bash
...
```
    <Aside type="caution">
    Utilisez la valeur de sortie `dungeon-adventure-infra-sandbox-Application.StoryApiStoryApiUrlXXX` du déploiement CDK pour remplacer l'URL et ajustez la région.
    </Aside>
  </TabItem>
</Tabs>
