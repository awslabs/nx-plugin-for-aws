...
- `main.py` ajoute un `session_manager_provider` qui crée un `FileSessionManager` par `thread_id` — c'est ce qui permet à l'historique des conversations de persister entre les tours pour la même paire `(player, genre)`.
...
<Tabs>
  ...
  Démarrez votre serveur Agent local :
    ...
    ```
    Réutiliser le même `threadId` lors des appels suivants continuera la même histoire — le `FileSessionManager` rejoue les messages précédents.
    </Aside>
  </TabItem>
