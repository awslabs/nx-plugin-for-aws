...
<Tabs>
  <TabItem label="Local">
  Le générateur d'agent fournit une CLI de chat terminal interactive (`agent-chat`) qui utilise un client TypeScript type-safe généré à partir de la spécification OpenAPI de l'agent. Comme nous avons modifié la forme d'entrée de l'agent pour accepter `playerName`, `genre` et `actions`, nous devons mettre à jour le script de chat généré pour fournir ces champs.

  Mettez à jour `packages/story/scripts/agent/chat.ts` :
  ...
  Ensuite, démarrez un chat interactif avec votre agent Story exécuté localement :
    ...
    La cible `agent-chat` démarre automatiquement `agent-serve-local` (qui démarre l'agent et toutes les dépendances du serveur MCP avec `SERVE_LOCAL=true`) et exécute la génération de la spécification OpenAPI + client avant de vous placer dans le chat.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat` dépend de `agent-serve-local`, qui démarre l'agent avec `SERVE_LOCAL=true` et démarre également toutes les dépendances du serveur MCP localement. Si vous préférez que l'agent se connecte à votre serveur MCP **déployé** à la place (via `RUNTIME_CONFIG_APP_ID`), exécutez la cible `agent-serve` dans un terminal séparé et invoquez l'agent manuellement via `curl` contre son point de terminaison `/invocations`.
    :::
  </TabItem>
