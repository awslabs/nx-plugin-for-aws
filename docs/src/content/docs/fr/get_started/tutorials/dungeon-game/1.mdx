...
<Aside type="tip">Il est recommandé de s'assurer que tous vos fichiers non commités sont validés dans Git avant d'exécuter des générateurs. Cela permet de visualiser les modifications via `git diff` après exécution.</Aside>
...
- packages/
  ...
  - game-api/ API tRPC
    - src/
      ...
      - local-server.ts pour exécution locale
...
Les schémas tRPC utilisent [Zod](https://zod.dev/) pour générer des types TypeScript via `z.TypeOf`.
...
Ce construct CDK pour `GameApi` génère automatiquement une fonction Lambda par procédure tRPC lors de la synthèse CDK.
...
Ce construct CDK crée une fonction Lambda par opération FastAPI.
...
Ce construct pointe vers le bundle Vite généré.
...
Entrée React avec routage basé fichiers via `@tanstack/react-router`.
...
- packages/
  ...
  - game-ui/
    - src/
      - components/
        - AppLayout/
          - index.tsx ajoute gestion utilisateur
        - CognitoAuth/
          - index.ts gestion de l'authentification
        - RuntimeConfig/
          - index.tsx récupère la configuration
      - hooks/
...
Ajout des fournisseurs Cognito et configuration runtime.
...
Entrée CDK avec validation via cfn-guard.
...
Structure de base pour les ressources CDK.
...
Les intégrations par défaut créent une fonction Lambda par opération.
