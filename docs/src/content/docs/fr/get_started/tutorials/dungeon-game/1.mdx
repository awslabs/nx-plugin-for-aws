...
- packages/
  - story/
    - dungeon_adventure_story/ module Python
      - agent/
        - main.py point d'entrée de l'application FastAPI AG-UI pour Bedrock AgentCore Runtime
...
Parce que nous avons choisi `protocol=AG-UI`, le générateur enveloppe notre agent Strands dans un adaptateur [AG-UI](https://ag-ui.com/). `create_strands_app` construit une application FastAPI qui expose un seul endpoint `/invocations` - les instances d'agent par session sont créées automatiquement, et l'historique des conversations est conservé en mémoire par `threadId` pendant la durée de vie du serveur. Nous n'aurons pas besoin de modifier `main.py` dans ce tutoriel.
