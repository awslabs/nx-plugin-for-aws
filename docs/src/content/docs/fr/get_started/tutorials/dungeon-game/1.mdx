...
Nous utilisons `--iac=cdk` car nous utiliserons CDK pour l'infrastructure as code dans ce tutoriel. Le plugin Nx pour AWS supporte également `Terraform`.
:::
...
Vous verrez de nouveaux fichiers apparaître dans votre arborescence de fichiers.
<Drawer title="Fichiers modifiés par py#agent" trigger="Cliquez ici pour examiner ces fichiers en détail.">
...
def get_agent():
    yield Agent(
        ...
        system_prompt="""
Vous êtes un sorcier mathématique.
Utilisez vos outils pour les tâches mathématiques.
...
Cela crée un exemple d'agent Strands et définit un outil de soustraction.
...
## Tâche 5 : Créer la base de données du jeu
...
Le générateur `ts#dynamodb` fournit une cible `serve-local` qui exécute [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) dans un conteneur. Combiné avec le générateur `connection` (que nous exécutons ci-dessous), cela signifie que **le jeu entier s'exécute sur votre machine sans appels AWS** jusqu'à ce que vous choisissiez de déployer à la fin du tutoriel. Vous aurez besoin d'[un moteur de conteneur](https://docs.docker.com/get-started/get-docker/) tel que Docker ou Finch installé et en cours d'exécution.
:::

Vous verrez de nouveaux fichiers apparaître dans votre arborescence de fichiers.
<Drawer title="Fichiers modifiés par ts#dynamodb" trigger="Cliquez ici pour examiner ces fichiers en détail.">
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - constants.ts constantes de développement local et clé de configuration runtime
      - client.ts singleton du client DynamoDB et résolution du nom de table
      - gsi.ts source de vérité pour les index secondaires globaux
      - entities/
        - example.ts entité ElectroDB d'exemple (nous la remplacerons)
        - index.ts exports d'entités
    - project.json ajoute les cibles `serve-local` et `pull-image`
  - common/constructs/
    - src/
      - app/dynamodb/
...
## Tâche 6 : Créer l'interface utilisateur (UI)
...
<Drawer title="Fichiers modifiés par ts#website" trigger="Cliquez ici pour examiner ces fichiers en détail.">
...
- packages/
  ...
  - game-ui/
    ...
    - ...
  - common/
    - shadcn/ bibliothèque shadcn/ui partagée (tokens de thème, `Button`, `Card`, `Input`, `Sidebar`, …) importée par chaque site web `uxProvider=Shadcn`
      - src/components/ui/*
...
<Drawer title="Fichiers modifiés par ts#website#auth" trigger="Cliquez ici pour examiner ces fichiers en détail.">
...
Le générateur :
- Détecte le `uxProvider` du site web React (Shadcn ici) et fournit les composants de chat correspondants.
...
<Aside type="tip" title="Une seule commande démarre toute la pile localement">
Comme la cible `agent-serve-local` de l'agent Story dépend déjà de la cible `mcp-server-serve-local` du serveur MCP d'inventaire (via la connexion `story → inventory` ci-dessus), et que l'API de jeu et le serveur MCP dépendent maintenant de `dungeon-db:serve-local`, l'exécution de la cible `serve-local` de n'importe quel projet démarre toutes les dépendances dont il a besoin — y compris DynamoDB Local — dans le bon ordre. Nous nous appuierons sur cela tout au long du reste du tutoriel.
</Aside>
...
## Tâche 7 : Mettre à jour notre infrastructure
...
<Aside type="note" title="Environnements non interactifs">
Dans un shell non interactif (CI, par exemple), il n'y a pas d'invite — le build échoue à la place avec une erreur "workspace is out of sync". Exécutez <NxCommands commands={['sync']} /> pour appliquer les générateurs de synchronisation en attente, puis relancez le build, ou définissez `sync.applyChanges` à `true` dans `nx.json` pour appliquer les changements automatiquement à chaque build. Vous rencontrerez cette même vérification de désynchronisation chaque fois que les références de projet changent (par exemple après l'ajout de procédures dans le module suivant).
</Aside>
