...
Connectons notre interface utilisateur du jeu à l'agent Story afin que nous puissions appeler l'API de l'agent depuis notre site web React avec un client type-safe, auto-généré.
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** Script pour générer la spécification OpenAPI depuis l'application FastAPI de l'agent
    - project.json Mis à jour avec la cible `agent-openapi`
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** Provider pour le client OpenAPI type-safe
        - **QueryClientProvider.tsx** Provider de client TanStack React Query (s'il n'est pas déjà présent)
      - hooks/
        - **useStoryAgent.tsx** Hook pour le proxy d'options TanStack Query
        - **useStoryAgentClient.tsx** Hook pour le client API vanilla
      - generated/
        - story-agent/ Généré au moment du build depuis la spécification OpenAPI
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json Mis à jour avec les cibles de génération de client
    - **main.tsx** Mis à jour pour ajouter StoryAgentProvider

</FileTree>

Le générateur :
- Crée un script de génération de spécification OpenAPI scopé à l'agent dans le projet story
- Génère un client TypeScript type-safe depuis la spécification OpenAPI de l'agent au moment du build
- Ajoute un provider, des hooks et un client au site web pour appeler l'API de l'agent
- Configure `serve-local` pour démarrer automatiquement le serveur local de l'agent et régénérer le client lors des changements
