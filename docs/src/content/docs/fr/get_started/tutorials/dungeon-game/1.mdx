...
Les schémas tRPC utilisent [Zod](https://zod.dev/) avec des types TypeScript générés via `z.TypeOf`.
...
Construct CDK pour l'API de jeu. La méthode `defaultIntegrations` crée automatiquement une fonction Lambda par procédure tRPC.
...
// ... (code CDK similaire à GameApi pour FastAPI)
```

Construct CDK pour StoryApi avec intégrations Lambda par défaut.
...
Point d'entrée FastAPI avec route GET /.

</Drawer>

### Interface de jeu : Site web

Créons l'interface utilisateur :
  ...
  - game-ui/
    - src/
      - routes/ routage basé sur les fichiers
        - **welcome/index.tsx** page d'accueil
    - index.html
    - vite.config.ts
</FileTree>
...
- packages/game-ui/
  - src/
    ...
    - main.tsx mis à jour avec les fournisseurs
</FileTree>
...
<Drawer title="Connexion UI -> FastAPI" trigger="Cliquez ici pour examiner les modifications.">
<FileTree>
- packages/game-ui/
  - src/hooks/useStoryApiClient.tsx
  - components/StoryApiProvider.tsx
</FileTree>
...
// ... (fournisseur TanStack Query pour StoryApi)
```
...
<Drawer title="Connexion UI -> tRPC" trigger="Cliquez ici pour examiner les modifications.">
<FileTree>
- packages/game-ui/
...
<Drawer title="Fichiers modifiés par ts#infra" trigger="Cliquez ici pour examiner les modifications.">
<FileTree>
- packages/infra/
...
// ... (configuration CDK avec validation cfn-guard)
```
...
Si des erreurs TypeScript apparaissent, exécutez la synchronisation proposée par Nx.

<Aside type="caution" title="Échec de construction sous Windows">
<Drawer trigger="Solution temporaire pour Windows" title="Problème cfn-guard">
Modifiez `packages/infra/src/main.ts` pour désactiver cfn-guard.
</Drawer>
</Aside>

Les artefacts sont générés dans `dist/`. Félicitations, vous avez configuré tous les sous-projets ! 🎉🎉🎉