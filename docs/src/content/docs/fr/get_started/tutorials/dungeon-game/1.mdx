...
:::note[Shadcn UI]
Nous sélectionnons `--uxProvider=Shadcn` pour que le site web généré utilise les composants [shadcn/ui](https://ui.shadcn.com/) stylisés avec Tailwind — tout notre code de route utilise des primitives shadcn comme `Card`, `Button` et `Input`, et le générateur `connection` câblera plus tard une surface de chat [CopilotKit](https://docs.copilotkit.ai/) thématisée shadcn correspondante.
:::
...
C'est le point d'entrée où React est monté. Parce que nous avons choisi `--uxProvider=Shadcn`, il n'y a pas de `I18nProvider` Cloudscape — le style provient des tokens Tailwind v4 importés via `styles.css` à la place. `@tanstack/react-router` est configuré en mode [file-based routing](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) : tant que le serveur de développement est en cours d'exécution, tout fichier que vous créez sous `routes/` est automatiquement détecté et l'arbre de routes est régénéré. Les générateurs ultérieurs (auth, connection) patcheront ce fichier via AST pour envelopper `<App />` dans des fournisseurs supplémentaires.
...
Connectons notre interface utilisateur du jeu à l'agent Story. Parce que l'agent parle AG-UI, le générateur `connection` câble [CopilotKit](https://docs.copilotkit.ai/) au lieu de générer un client OpenAPI — vous obtenez un composant de chat thématisé et un `HttpAgent` `@ag-ui/client` prêt à rendre.
...
- Remplit le `threadId` au minimum de 33 caractères d'AgentCore — c'est pourquoi notre route du Module 4 peut construire en toute sécurité ``{player}-{genre}`` comme un thread id stable.
