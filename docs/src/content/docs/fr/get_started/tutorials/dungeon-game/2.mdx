...
La dépendance `@aws-sdk/client-s3` est déjà dans l'espace de travail, il n'y a donc rien de nouveau à installer. Le nom du bucket provient du même mécanisme de Configuration d'Exécution utilisé pour `tables` — nous l'enregistrons sous un nouvel espace de noms `buckets` dans la pile d'infrastructure ci-dessous.
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Local">
    ...
    <Aside type="caution" title="Utiliser les valeurs de sortie CDK">
