...
Le client S3 est ajouté au `package.json` propre au projet `game-api`, puisque le code source de ce projet l'importe. Les versions restent alignées dans l'espace de travail via le catalogue de dépendances du gestionnaire de paquets ([single version policy](https://nx.dev/concepts/decisions/dependency-management#single-version-policy)). Pour plus d'informations, consultez le <Link path="guides/typescript-project#dependencies">guide ts#project</Link>.
</Aside>
