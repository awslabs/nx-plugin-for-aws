...
2. `queryGames` - retournera une liste paginée des jeux précédemment sauvegardés.
3. `saveAction` - sauvegardera une action pour un jeu donné.
4. `queryActions` - retournera une liste paginée de toutes les actions liées à un jeu.
5. `queryInventory` - retournera une liste paginée des objets dans l'inventaire d'un joueur.
...
<Aside type="tip">
Comme visible ci-dessus, pour chaque schéma Zod défini, nous exportons également une interface en utilisant la syntaxe `z.TypeOf`. Cela convertit notre définition Zod en interface TypeScript sans duplication d'effort !
</Aside>
...
<Image class="centered-image white-bg" src={dungeonAdventureErPng} alt="Diagramme ER dungeon-adventure" width="400" height="300" />
...
Nous allons implémenter notre base de données dans DynamoDB en utilisant la bibliothèque cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/). Pour commencer, installons d'abord `electrodb` en exécutant la commande suivante :

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
Toutes les dépendances sont ajoutées au `package.json` racine car le `@aws/nx-plugin` suit le principe de [single version policy](https://nx.dev/concepts/decisions/dependency-management#single-version-policy). Pour plus d'informations, consultez le <Link path="guides/typescript-project#dependencies">guide TypeScript</Link>.
</Aside>
...
ElectroDB est très puissant et nous permet non seulement de définir nos types, mais aussi de fournir des valeurs par défaut pour certains champs comme les horodatages ci-dessus. De plus, ElectroDB suit le [single-table design](https://electrodb.dev/en/core-concepts/single-table-relationships/), une meilleure pratique avec DynamoDB.
...
Pour préparer l'interaction du serveur MCP avec l'inventaire, exportons l'entité d'inventaire dans `packages/game-api/src/index.ts` :
...
:::note
La structure des packages serait améliorée en refactorisant les entités dans leur propre projet partagé, pour éviter que le serveur MCP dépende de l'API.
:::

### Intégration du client DynamoDB dans le contexte tRPC

Ayant besoin d'accéder au client DynamoDB dans chaque procédure, nous devons créer une instance unique du client à injecter via le contexte. Effectuons les modifications suivantes dans `packages/game-api/src` :
...
Implémentons maintenant les méthodes de l'API. Effectuons les modifications suivantes dans `packages/game-api/src/procedures` :
...
Maintenant que nos procédures sont définies, connectons-les à notre API. Mettons à jour le fichier suivant :
...
La dernière étape consiste à mettre à jour notre infrastructure pour créer la table DynamoDB et accorder les permissions nécessaires à l'API de jeu. Modifions `packages/infra/src` comme suit :
...
:::note
Notez que chaque procédure étant servie par une fonction lambda distincte, nous pouvons appliquer le principe de moindre privilège en accordant uniquement les permissions nécessaires.
:::
  </TabItem>
</Tabs>
...
:::tip
Pour les modifications de code lambda itératives, utilisez le flag `--hotswap` après compilation pour un déploiement ultra-rapide (2-3 secondes) :
...
Ajoutez cette fonction à votre profil PowerShell ou collez-la dans la session :
```powershell
function acurl {
    param(
