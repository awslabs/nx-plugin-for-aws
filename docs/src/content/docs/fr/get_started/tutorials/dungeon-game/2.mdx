...
Nous allons implémenter notre base de données dans DynamoDB en utilisant la bibliothèque cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) pour simplifier les choses. Le `queryActions` de l'API de jeu a également besoin du client S3 pour lire les tours de conversation que l'agent a écrits. Installez-les tous avec :
