...
Avant d'implémenter notre Game API, nous devons créer ces 5 API :
...
Vous pouvez supprimer le fichier `packages/game-api/src/schema/echo.ts` car nous ne l'utiliserons pas dans ce projet.
...
Nous allons implémenter notre base de données dans DynamoDB en utilisant la bibliothèque cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/). Pour installer `electrodb` et le client DynamoDB, exécutez cette commande :
...
Pour définir nos entités ElectroDB à partir du diagramme ER, créons les fichiers suivants dans notre dossier `packages/game-api/src/entities` :
...
Cela instrumentera le plugin DynamoDB.
...
Vous pouvez supprimer le fichier `echo.ts` (dans `packages/game-api/src/procedures`) car nous ne l'utiliserons pas dans ce projet.
...
Pour gagner du temps, nous définirons tous les outils dans un seul fichier :
...
Vous pouvez soit ajouter ce script à votre fichier `.bashrc` (et l'activer avec `source`), soit le coller dans le même terminal où vous souhaitez exécuter la commande.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
Pour effectuer une requête curl authentifiée avec `sigv4`, invoquez `acurl` en utilisant ces exemples :
...
Vous pouvez soit ajouter cette fonction à votre profil PowerShell, soit la coller dans la même session PowerShell où vous souhaitez exécuter la commande.
```powershell
