...
:::note[Pas de procédure d'historique de chat]
Vous auriez pu vous attendre à une procédure pour sauvegarder chaque tour utilisateur/assistant du chat. Nous n'en avons pas besoin : l'agent AG-UI est hébergé sur Bedrock AgentCore Runtime, qui persiste automatiquement l'historique de conversation par session tant que le même identifiant de thread est utilisé.
:::
...
dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX = https://xxx.execute-api.region.amazonaws.com/prod/
