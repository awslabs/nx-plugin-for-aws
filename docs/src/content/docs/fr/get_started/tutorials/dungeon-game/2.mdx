...
:::note[L'historique des conversations réside avec l'agent]
Nous ne persistons pas les messages de chat via l'API de jeu. Dans le <Link path="get_started/tutorials/dungeon-game/3">Module 3</Link>, l'Agent d'Histoire intègre un `FileSessionManager` qui écrit l'historique de chaque fil de discussion sur le disque, indexé par l'identifiant de fil que l'interface utilisateur envoie à chaque tour — ainsi chaque paire `(joueur, genre)` reprend sa propre histoire sans que l'interface utilisateur ait besoin de rejouer les messages.
:::
