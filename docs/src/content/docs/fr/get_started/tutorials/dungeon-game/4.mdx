...
Cette commande récupèrera le fichier `runtime-config.json` déployé et le stockera localement dans le dossier `packages/game-ui/public`.

Démarrez maintenant le serveur de développement avec :

<NxCommands commands={["run @dungeon-adventure/game-ui:serve"]} />

Ouvrez ensuite le site local dans votre navigateur. Vous serez invité à vous connecter et à créer un nouvel utilisateur. Après inscription, vous verrez le site de base :

<Image src={baselineWebsitePng} alt="baseline-website.png" width="800" height="600" />

<Aside type="caution">
_Nous garderons le serveur de développement actif pendant tout ce module car il rechargera automatiquement les modifications._
</Aside>

### Création d'une nouvelle route '/game'

Montrons les capacités de `@tanstack/react-router` en créant une route typée. Créez un fichier vide à l'emplacement : `packages/game-ui/src/routes/game/index.tsx`. Surveillez les logs du serveur :
...
Le routeur a automatiquement configuré la nouvelle route. Le fichier contient déjà le chemin :
...
Accédez à `http://localhost:4200/game` pour voir la nouvelle page !

<Image src={baselineGamePng} alt="baseline-game.png" width="800" height="600" />

Mettons à jour `index.tsx` pour charger `/game` par défaut. Notez la sélection typée des routes :
...
Supprimez maintenant le dossier `packages/game-ui/src/routes/welcome/`.

### Mise à jour du layout

Le layout par défaut est plus adapté à une application SaaS. Reconfigurons-le pour un style jeu de donjon.
...
/**
 * Définit le layout de l'application et contient la logique de routage
 */
        ...
        utilities={[
          {
            ...
            items: [{ id: 'signout', text: 'Déconnexion' }],
          },
        ]}
      />
      <Outlet />
...
Supprimez `packages/game-ui/src/components/AppLayout/navitems.ts` et `packages/game-ui/src/hooks/useAppLayout.tsx`.
...
// Hook pour vérifier la visibilité d'une référence
    ...
    <div className="game-interface">
      ...
      <div className="input-area">
        <PromptInput
          ...
          value={currentInput}
          actionButtonAriaLabel="Envoyer le message"
          actionButtonIconName="send"
          ariaLabel="Invite de saisie par défaut"
          placeholder="Quelle action effectuez-vous ?"
...
<Aside type="tip">
La syntaxe `$playerName` indique à `@tanstack/react-router` de traiter `playerName` comme un [paramètre de chemin](https://tanstack.com/router/v1/docs/framework/react/guide/path-params). La méthode `validateSearch` garantit que le paramètre `genre` est fortement typé selon notre enum.
</Aside>
</TabItem>
</Tabs>

Votre serveur local (http://localhost:4200/) contient maintenant le jeu prêt à être joué !

<Drawer title="Build et Déploiement" trigger="Vous pouvez aussi builder et déployer sur Cloudfront">

Pour builder le code :
...
Une fois déployé, accédez à l'URL Cloudfront dans les sorties du déploiement CDK.
