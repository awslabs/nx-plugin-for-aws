...
`game-ui:serve-local` a un `dependsOn` sur `game-api:serve-local` et `dungeon_adventure.story:agent-serve-local`, donc Nx démarre le serveur local de chaque projet en parallèle. Ouvrez le serveur de développement dans un navigateur, connectez-vous et suivez les étapes pour créer un nouvel utilisateur. Vous devriez voir le site de base Shadcn :
...
C'est pratique lorsque vous itérez uniquement sur le code de l'interface utilisateur et que vous ne voulez pas redémarrer le backend.
</Aside>
