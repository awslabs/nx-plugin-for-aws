...
:::tip
Vous pouvez exécuter des commandes Nx à la place pour démarrer les serveurs API locaux connectés, vous permettant d'itérer simultanément sur votre API tRPC et votre site web. <NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} env={{TABLE_NAME:"dungeon-adventure-infra-sandbox-Application-ElectroDbTableXXX-YYY"}} />
:::
...
:::tip
L'intégration avec notre Game API est déjà configurée, car nous avons utilisé le générateur `api-connection`. Si vous souhaitez une expérience similaire pour les agents, +1 [cette issue GitHub](https://github.com/awslabs/nx-plugin-for-aws/issues/326).
:::
