...
## Tâche 4 : Intégrer l'Agent Narratif

Créons un hook pour initialiser un client permettant d'interagir avec notre Agent Narratif.
...
- Invoque l'Agent avec le token JWT de l'utilisateur connecté et un ID de session aléatoire, et
- Retourne un itérateur asynchrone pour consommer les messages streamés de l'agent.

:::tip[Intégration API préconfigurée]
L'intégration avec notre Game API est déjà configurée, car nous avons utilisé le générateur `connection`. Si vous souhaitez une expérience similaire pour les agents, +1 [cette issue GitHub](https://github.com/awslabs/nx-plugin-for-aws/issues/326).
:::
