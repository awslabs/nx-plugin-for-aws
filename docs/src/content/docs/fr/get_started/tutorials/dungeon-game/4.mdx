...
Votre premier déploiement prendra environ 8 minutes. Les déploiements suivants sont plus rapides.
...
La clé est la variable d'environnement `RUNTIME_CONFIG_APP_ID`. Lorsqu'un projet s'exécute **sans** `SERVE_LOCAL=true`, les recherches de configuration d'exécution récupèrent leur configuration depuis AWS AppConfig en utilisant cet identifiant d'application — la valeur `RuntimeConfigApplicationId` de vos sorties CDK.
...
Félicitations. Vous avez construit, testé et déployé votre Jeu d'Aventure en Donjon Agentique ! 🎉🎉🎉onjon Agentique ! 🎉🎉🎉e en Donjon Agentique ! 🎉🎉🎉