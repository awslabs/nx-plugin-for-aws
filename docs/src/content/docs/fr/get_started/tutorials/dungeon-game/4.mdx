...
`dev` est un alias pour `game-ui:serve-local`, qui a un `dependsOn` sur `game-api:serve-local` et `dungeon_adventure.story:agent-serve-local`, donc Nx démarre le serveur local de chaque projet en parallèle. Grâce aux connexions que nous avons configurées dans le Module 1, celles-ci démarrent à leur tour le serveur MCP Inventory et DynamoDB Local. Assurez-vous que votre moteur de conteneurs est en cours d'exécution, puis ouvrez le serveur de développement dans un navigateur.
