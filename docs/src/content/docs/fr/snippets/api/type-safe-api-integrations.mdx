...
Le support du générateur varie actuellement selon le backend :

- `ts#trpc-api` supporte à la fois `isolated` et `shared` pour `ServerlessApiGatewayRestApi` et `ServerlessApiGatewayHttpApi` via l'option `integrationPattern`.
- `py#fast-api` et `ts#smithy-api` génèrent actuellement `isolated`.

Vous pouvez également basculer un construct d'API CDK généré existant entre les deux modèles en modifiant le champ `pattern` dans sa méthode `defaultIntegrations` :

```ts {3}
return IntegrationBuilder.rest({
  // Changez ceci en 'isolated' ou 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Les modules Terraform utilisent déjà le modèle router par défaut, avec une seule fonction Lambda gérant toutes les opérations de l'API.
</Fragment>
</Infrastructure>
...
#### Modèle Router personnalisé

<Infrastructure>
<Fragment slot="cdk">
Si vous devez personnaliser le modèle router au-delà des valeurs par défaut générées, vous pouvez librement modifier la méthode `defaultIntegrations` de votre API pour configurer les intégrations comme vous le souhaitez :
  ...
  public static defaultIntegrations = (scope: Construct) => {
    ...
    return IntegrationBuilder.rest({
      ...
      buildDefaultIntegration: (op) => {
        return {
          // Référencer le même handler router Lambda dans chaque intégration
...
Vous pouvez modifier le code d'autres manières si vous préférez, par exemple vous pourriez préférer définir la fonction `router` comme paramètre de `defaultIntegrations` au lieu de la construire dans la méthode.
</Fragment>
<Fragment slot="terraform">
