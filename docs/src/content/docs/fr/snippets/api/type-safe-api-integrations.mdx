...
<Fragment slot="terraform">
:::note[Surcharges d'autorisateur]
Les surcharges d'autorisations par opération ne sont pas supportées avec les modules Terraform. L'API entière utilise la méthode d'authentification spécifiée lors de la génération de l'API (IAM, Cognito, ou Custom).
