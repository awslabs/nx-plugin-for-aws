...
Ce générateur crée une infrastructure CDK ou Terraform sous forme de code en fonction du `iac` sélectionné. Vous pouvez l'utiliser pour déployer votre fonction.
...
Vérifiez que la source d'événement correspond à l'option `event` sélectionnée pour garantir un traitement correct des événements dans votre fonction de gestion.
:::
...
Ce générateur crée un module Terraform pour déployer votre fonction dans le dossier `common/terraform`. Instanciez le module partagé `core/asset-bucket` une fois par déploiement et transmettez son `bucket_name` à l'entrée `asset_bucket_name` :
