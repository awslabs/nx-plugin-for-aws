...
L'ARN du runtime AgentCore de l'agent distant est automatiquement enregistré dans l'espace de noms `agentcore` de la <Link path="guides/runtime-config">Configuration d'Exécution</Link> par la construction CDK générée, afin que l'agent hôte puisse le découvrir au moment de l'exécution.
</Fragment>
...
L'ARN du runtime AgentCore de l'agent distant est automatiquement enregistré dans l'espace de noms `agentcore` de la <Link path="guides/runtime-config">Configuration d'Exécution</Link> par le module Terraform généré, afin que l'agent hôte puisse le découvrir au moment de l'exécution.
</Fragment>
</Infrastructure>