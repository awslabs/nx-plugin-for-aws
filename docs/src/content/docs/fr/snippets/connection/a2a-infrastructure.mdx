...
`grantInvokeAccess` sur un agent A2A configure à la fois `bedrock-agentcore:InvokeAgentRuntime` et `bedrock-agentcore:GetAgentCard` — le client A2A a besoin des deux pour récupérer la carte d'agent et envoyer des messages.

L'ARN du runtime AgentCore de l'agent distant est automatiquement enregistré dans l'espace de noms `agentcore` de la <Link path="guides/runtime-config">Configuration d'exécution</Link> par la construction CDK générée, afin que l'agent hôte puisse le découvrir à l'exécution.
</Fragment>
...
L'ARN du runtime AgentCore de l'agent distant est automatiquement enregistré dans l'espace de noms `agentcore` de la <Link path="guides/runtime-config">Configuration d'exécution</Link> par le module Terraform généré, afin que l'agent hôte puisse le découvrir à l'exécution.
</Fragment>
</Infrastructure>
