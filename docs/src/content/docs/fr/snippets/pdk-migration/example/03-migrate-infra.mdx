...
Le dernier projet que nous devons migrer pour notre application de liste de courses est le `InfrastructureTsProject`. Il s'agit d'un projet TypeScript CDK, pour lequel l'équivalent dans le Nx Plugin for AWS est le <Link path="/fr/guides/typescript-infrastructure">générateur `ts#infra`</Link>.
...
Exécutez le <Link path="/fr/guides/typescript-infrastructure">générateur `ts#infra`</Link> pour configurer votre projet d'infrastructure dans `packages/infra` :
