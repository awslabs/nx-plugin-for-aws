...
Le générateur de site web React ci-dessus n'inclut pas l'authentification cognito par défaut comme `CloudscapeReactTsWebsiteProject`, elle est ajoutée explicitement via le <Link path="/fr/guides/react-website-auth">générateur `ts#website#auth`</Link>.
...
Avec le Nx Plugin for AWS, l'intégration d'API est prise en charge via le <Link path="/fr/guides/connection">générateur `connection`</Link>. Ensuite, nous utilisons ce générateur pour que notre site web puisse invoquer notre API Smithy :
