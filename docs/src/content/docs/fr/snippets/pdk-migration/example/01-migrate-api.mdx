...
Nous pouvons donc utiliser le <Link path="/guides/ts-smithy-api">générateur `ts#smithy-api`</Link> pour obtenir une fonctionnalité équivalente.

:::note
Si vous avez utilisé OpenAPI ou TypeSpec comme langage de modélisation, ou des gestionnaires implémentés en Python ou Java, consultez la [FAQ](#frequently-asked-questions) pour les options possibles.
:::

#### Générer une API Smithy en TypeScript
...
1. Supprimez les fichiers Smithy générés en exemple dans `packages/api/model/src`
...
1. Ajoutez le nouveau trait `@cursor` à l'opération `GetShoppingLists` dans `get-shopping-lists.smithy` :
    ...
    Toutes les opérations `@paginated` doivent également utiliser `@cursor` si vous utilisez le générateur de client fourni par le Nx Plugin pour AWS (via le générateur <Link path="guides/api-connection/react-smithy">`api-connection`</Link>).

1. Enfin, supprimez le trait `@handler` de toutes les opérations car il n'est pas supporté par le Nx Plugin pour AWS. Avec `ts#smithy-api`, nous n'avons pas besoin des constructs CDK de fonctions lambda générés automatiquement ni des cibles de bundling générées par ce trait, car nous utilisons un seul bundle pour toutes les fonctions lambda.

</Steps>

À ce stade, exécutons une build pour vérifier nos modifications de modèle et nous assurer d'avoir du code serveur généré à utiliser. Il y aura quelques erreurs dans le projet backend (`@shopping-list/api`), mais nous les traiterons ensuite.
...
Nos gestionnaires lambda dépendent du package `@aws-sdk/client-dynamodb`, installons-le d'abord :
...
1. Ajoutez l'opération à `service.ts`
    ...
    // Enregistrez les opérations au service ici
...
<Drawer title="Migration des gestionnaires de liste de courses" trigger="Cliquez ici pour voir des exemples complets avant/après pour les trois opérations de liste de courses du tutoriel">

<h4>Supprimer la liste de courses</h4>
 ...
 * @param token un token passé à la requête paginée
 */
...
Nous pouvons maintenant builder le projet pour vérifier que la migration a fonctionné jusqu'ici :
