...
1. Ajoutez le nouveau trait `@cursor` à l'opération `GetShoppingLists` dans `get-shopping-lists.smithy` :
    ...
    Toutes les opérations `@paginated` devraient également utiliser `@cursor` si vous utilisez le générateur de client fourni par le Nx Plugin pour AWS (via le générateur <Link path="guides/connection/react-smithy">`connection`</Link>).
