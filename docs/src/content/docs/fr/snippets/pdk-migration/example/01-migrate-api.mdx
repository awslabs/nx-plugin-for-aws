...
1. Ajoutez le nouveau trait `@cursor` à l'opération `GetShoppingLists` dans `get-shopping-lists.smithy` :
    ...
    Toutes les opérations `@paginated` devraient également utiliser `@cursor` si vous utilisez le générateur de client fourni par le Nx Plugin pour AWS (via le générateur <Link path="guides/connection/react-smithy">`api-connection`</Link>).
...
Les gestionnaires lambda de l'application de liste de courses dépendent du package `@aws-sdk/client-dynamodb`, installons-le dans le projet `@shopping-list/api` :
...
Le générateur `ts#smithy-api` crée un exemple d'opération `Echo`. Puisque nous l'avons supprimée de notre modèle, supprimez le gestionnaire correspondant dans `backend/src/operations/echo.ts`. Nous enregistrerons nos opérations migrées dans `service.ts` plus bas.
