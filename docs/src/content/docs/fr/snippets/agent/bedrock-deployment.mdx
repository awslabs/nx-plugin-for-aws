...
Si vous avez sélectionné `agentcore` pour `infra`, l'infrastructure CDK ou Terraform pertinente est générée et vous pouvez l'utiliser pour déployer votre Agent sur [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Passez les sorties du module partagé <Link path="/fr/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> dans le module agent :
