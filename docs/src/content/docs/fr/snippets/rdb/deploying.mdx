...
La Lambda de migration est déployée en tant qu'image de conteneur, publiée dans le registre d'actifs partagé. Instanciez le module `core/asset-ecr` une fois par déploiement et transmettez son `repository_url` à chaque module de base de données :
...
Cela provisionne un cluster Aurora avec RDS Proxy, des identifiants administrateur, une Lambda create-db-user, l'enregistrement de configuration d'exécution et une Lambda de migration.
...
<Drawer title="Exemple de configuration VPC" trigger="Cliquez ici pour un exemple de configuration VPC.">
...
Utilisez le générateur <Link path="guides/connection">`connection`</Link> pour connecter un projet à cette base de données — consultez le guide de connexion pour le type de calcul pertinent (par exemple FastAPI, serveur MCP, agent) pour le câblage d'infrastructure requis pour l'atteindre.
