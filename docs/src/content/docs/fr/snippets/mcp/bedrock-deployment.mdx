...
Si vous avez sélectionné `agentcore` pour `infra`, l'infrastructure CDK ou Terraform pertinente est générée et vous pouvez l'utiliser pour déployer votre serveur MCP sur [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Passez les sorties du module partagé <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> dans le module du serveur MCP :
...
[Docker](https://www.docker.com/) est requis pour construire et déployer votre serveur MCP. Assurez-vous d'avoir installé et lancé Docker pour éviter des erreurs telles que :
