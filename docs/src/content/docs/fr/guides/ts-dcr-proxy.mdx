...
Le générateur crée un <Link path="/fr/guides/typescript-project">projet TypeScript</Link> autonome contenant les gestionnaires Lambda, et l'infrastructure pour les déployer en fonction de votre `iac` sélectionné.
...
Si vous avez déjà un User Pool du générateur <Link path="fr/guides/react-website-auth">`ts#website#auth`</Link>, vous pouvez placer un serveur MCP devant les mêmes utilisateurs du site web. Voir <Link path="fr/guides/ts-dcr-proxy#reusing-a-useridentity-user-pool">Réutiliser un UserIdentity User Pool</Link> ci-dessous.
:::

### Placer un proxy devant un serveur MCP

Pour placer un proxy devant un serveur MCP généré avec le générateur <Link path="fr/guides/ts-mcp-server">`ts#mcp-server`</Link> (ou <Link path="fr/guides/py-mcp-server">`py#mcp-server`</Link>) en utilisant `--auth cognito`, passez le **même** User Pool et App Client au serveur MCP et au proxy, et utilisez l'`invocationUrl` du construct du serveur MCP comme `upstreamUrl` du proxy.
