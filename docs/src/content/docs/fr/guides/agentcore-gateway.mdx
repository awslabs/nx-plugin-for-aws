...
<FileTree>
  - packages/\<name>/
    ...
    - package.json Déclare les dépendances importées par `local-dev.ts`
    - tsconfig.json Configuration TypeScript utilisée par la cible `typecheck`
    - project.json Ajoute les cibles `serve`, `dev`, `build`, `lint`, `format` et `typecheck`
</FileTree>
...
Sélectionnez `cognito` lorsque le Gateway expose un agent `auth: cognito` : le Gateway transmet le jeton de l'appelant au runtime de l'agent, il doit donc en avoir reçu un. Voir <Link path="guides/connection/agentcore-gateway-agent">Gateway vers Agent</Link>.
