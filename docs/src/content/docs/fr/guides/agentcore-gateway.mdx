...
:::note
Cette section s'applique lorsque la gateway est générée avec `cedarPolicy: true` (la valeur par défaut). Avec `cedarPolicy: false`, la gateway n'effectue aucune évaluation de politique — tout appelant avec un accès IAM peut invoquer n'importe quel outil.
:::
