...
Le générateur ajoute une cible `<name>-serve-local` au projet Gateway. Cette cible est un agrégateur continu : lorsqu'elle est connectée à des serveurs MCP via le <Link path="guides/connection/agentcore-gateway-mcp">générateur `agentcore-gateway#mcp-connection`</Link>, son exécution démarre tous les serveurs MCP attachés ensemble pour le multiplexage local.
