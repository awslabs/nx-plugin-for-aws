...
Générez un projet [Amazon Bedrock AgentCore Gateway](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html). Une AgentCore Gateway est un point d'entrée géré devant vos serveurs MCP ou agents, authentifiant les requêtes entrantes (IAM ou Cognito) et signant le trafic sortant vers ses cibles avec IAM SigV4.

L'option `protocol` sélectionne ce que la Gateway expose :

- **`mcp`** (par défaut) — agrège un ou plusieurs serveurs MCP cibles derrière un seul point de terminaison MCP, et évalue chaque appel d'outil par rapport à un [moteur de politiques Cedar](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/policy.html).
- **`http`** — proxie les requêtes directement vers les [cibles AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway-target-http-runtime.html) (vos agents) via un routage basé sur le chemin (`/<targetName>/invocations`), sans agrégation ni traduction de protocole. Utilisez ceci pour exposer des agents avec un seul point de terminaison gouverné — par exemple pour qu'un site web puisse atteindre des agents déployés dans un VPC via la Gateway.
...
<FileTree>
  - packages/\<name>/
    - policies/ Fichiers source de politiques Cedar (protocole `mcp` uniquement ; omis lorsque `cedarPolicy: false`)
    ...
    - local-dev.ts Gateway locale pour le développement local — agrège les serveurs MCP attachés (`mcp`) ou proxie les agents attachés (`http`)
...
- Une `AgentCore::Gateway` avec authentification IAM entrante (par défaut) ou authentification JWT Cognito (voir <a href="#authentication">Authentification</a>). Une gateway `mcp` est configurée pour le protocole MCP ; une gateway `http` n'a pas de type de protocole, ce qu'AgentCore requiert pour ses cibles runtime
- Un `AgentCore::PolicyEngine` fonctionnant en mode `ENFORCE`, attaché à la Gateway (gateways `mcp` uniquement ; omis lorsque `cedarPolicy: false`)
...
Le générateur ajoute une cible `dev` au projet Gateway, qui exécute `local-dev.ts`. Son exécution démarre la gateway locale et chaque cible attachée ensemble :

<NxCommands commands={["dev <name>"]} />

Pour une gateway `mcp`, la gateway locale expose un seul point de terminaison MCP qui agrège chaque serveur MCP attaché (connecté via le <Link path="guides/connection/agentcore-gateway-mcp">générateur `agentcore-gateway#mcp-connection`</Link>), avec des outils préfixés `<target>___<tool>` pour correspondre à la Gateway déployée.

Pour une gateway `http`, la gateway locale proxie les chemins `/<targetName>/...` vers le serveur local de chaque agent attaché (connecté via le <Link path="guides/connection/agentcore-gateway-agent">générateur `agentcore-gateway#agent-connection`</Link>), correspondant au routage basé sur le chemin de la Gateway déployée.
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Exposer un agent avec une AgentCore Gateway en tant que cible runtime"
