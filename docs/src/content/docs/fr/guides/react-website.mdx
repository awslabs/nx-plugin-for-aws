...
:::warning
Dans ce mode sans `runtime-config.json`, si vous avez configuré l'authentification Cognito (via le <Link path="/guides/react-website-auth">générateur CloudScape Website Auth</Link>), le login sera désactivé et les requêtes locales n'incluront pas d'en-têtes d'authentification.
