...
La cible `checkov` passe `--config-file`, ce qui désactive la découverte propre de `checkov` d'un `.checkov.yaml` — un fichier de configuration déposé ailleurs est ignoré.
:::
...
La configuration de votre infrastructure est fournie à votre site Web via la <Link path="guides/runtime-config">Configuration d'exécution</Link>. Cela permet à votre site Web d'accéder à des détails tels que les URL d'API qui ne sont pas connus avant le déploiement de votre application.
...
Votre site Web est généré avec un `RuntimeConfigProvider` (dans `src/components/RuntimeConfig`) qui charge `runtime-config.json` avant de rendre votre application, et un hook `useRuntimeConfig` pour lire les valeurs :
...
Le provider se replie sur une configuration vide lorsqu'aucun `runtime-config.json` n'est servi, donc un site Web sans rien de déployé se rend toujours. `IRuntimeConfig` est typé `any` par défaut — affinez-le dans `src/components/RuntimeConfig/index.tsx` si vous souhaitez un accès type-safe.
