...
:::note[Script `dev`]
Si aucun script `dev` n'existe dans votre `package.json` racine lorsque ce générateur s'exécute, un script `dev` est ajouté qui invoque `serve-local` pour le site web généré, ce qui signifie que vous pouvez également démarrer le serveur de développement local avec :

<PackageManagerShortCommand commands={["dev"]} />

Cela fait du premier site Web React que vous générez la cible `dev` par défaut pour l'espace de travail. Les sites web suivants n'écrasent pas le script `dev` existant — vous pouvez invoquer leurs cibles `serve-local` directement, ou mettre à jour le script manuellement.
:::
