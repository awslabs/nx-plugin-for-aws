...
C'est une bonne pratique d'analyser vos images pour détecter les vulnérabilités connues. Les générateurs qui suivent ce modèle ajoutent une cible `trivy` qui analyse l'image construite avec [Trivy](https://trivy.dev/), s'exécutant depuis l'[image Trivy hébergée sur ECR](https://gallery.ecr.aws/aquasecurity/trivy), et fait échouer le build sur les résultats `HIGH` ou `CRITICAL`.
...
Ensuite, faites dépendre `build` de `trivy` pour que l'analyse s'exécute dans le cadre d'un build :
