...
`mfa` accepte `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` dans CDK, et `"OFF"` / `"OPTIONAL"` / `"ON"` dans Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` et l'équivalent `otp` activent ou désactivent chaque méthode de second facteur indépendamment ; ils n'ont aucun effet lorsque `mfa` est désactivé.

:::caution[Terraform lie l'authentification multifacteur par SMS à la vérification du numéro de téléphone]
Contrairement au construct CDK, le fournisseur AWS Terraform n'a aucun moyen d'offrir le SMS comme méthode d'authentification multifacteur indépendamment de son utilisation pour la vérification du numéro de téléphone. Définir `mfa_second_factor_sms = false` désactive donc également la vérification du numéro de téléphone par SMS (`auto_verified_attributes` supprime `phone_number`).
:::

#### Pare-feu d'application Web (WAF)

Par défaut, le User Pool est associé à un Web ACL [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) utilisant les groupes de règles gérées `AWSManagedRulesCommonRuleSet` et `AWSManagedRulesKnownBadInputsRuleSet`. Vous pouvez désactiver cela si vous souhaitez gérer votre propre Web ACL ou si vous n'en avez pas besoin :
