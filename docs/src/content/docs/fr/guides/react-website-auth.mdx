...
<FileTree>
  - src
    - components
      - CognitoAuth
        - index.tsx Composant d'authentification principal
    - main.tsx Mis à jour pour instrumenter le composant CognitoAuth
</FileTree>
...
<FileTree>
  - packages/common/terraform/src
    - core
      - user-identity
        - main.tf Wrapper de module pour la configuration d'identité
        - identity
          - identity.tf Infrastructure d'identité de base incluant Cognito User Pool et Identity Pool
        - add-callback-url
          - add-callback-url.tf Module pour ajouter des URL de rappel aux clients de user pool existants
</FileTree>
</Fragment>
</Infrastructure>
...
Ce générateur ajoute un user pool Amazon Cognito (pour la connexion) et un identity pool (pour fédérer les utilisateurs connectés aux informations d'identification IAM délimitées) à l'architecture de site Web statique existante :
...
Une fois que vous avez observé les évaluations de risque pour vos utilisateurs, vous pouvez passer à l'application complète pour répondre automatiquement aux activités à risque (par exemple, exiger MFA ou bloquer la connexion) :
...
:::caution[Supprimer les URL de rappel localhost pour la production]
Le client User Pool généré autorise l'URL de votre distribution CloudFront (et tous les noms de domaine personnalisés configurés dessus) comme URL de rappel/déconnexion OAuth, plus `http://localhost:4200` et `http://localhost:4300` pour le développement local contre le pool déployé.

Il est recommandé de **supprimer les URL de rappel/déconnexion `http://localhost` pour les environnements de production**, en gardant la liste d'autorisation limitée aux origines réelles de votre application.
...
Afin d'accorder aux utilisateurs authentifiés l'accès pour effectuer certaines actions, comme accorder des autorisations pour invoquer une API, vous pouvez ajouter des instructions de politique IAM au rôle authentifié de l'identity pool :
