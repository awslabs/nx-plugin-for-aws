...
:::caution[L'authentification multifacteur par SMS est liée à la vérification du numéro de téléphone]
Cognito rejette la vérification du numéro de téléphone par SMS sauf si le SMS est également une méthode d'authentification multifacteur activée lorsque l'authentification multifacteur n'est pas désactivée, à la fois dans CDK et Terraform. Désactiver `mfaSecondFactor.sms` / `mfa_second_factor_sms` lorsque `mfa` est activé désactive donc également la vérification du numéro de téléphone par SMS pour le pool d'utilisateurs.
:::
