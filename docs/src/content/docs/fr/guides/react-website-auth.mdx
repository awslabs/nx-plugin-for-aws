...
Le client User Pool généré autorise `http://localhost:4200` et `http://localhost:4300` comme URLs de callback/déconnexion OAuth (pour le développement local contre le pool déployé), ainsi que l'URL de votre distribution CloudFront. Gardez cette liste d'autorisation minimale : pour les déploiements de production renforcés, envisagez de supprimer les URLs localhost (ou d'utiliser un client séparé pour le développement local) une fois que vous n'exécutez plus localement contre le User Pool déployé.
:::
