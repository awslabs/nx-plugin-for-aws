...
L'option `integrationPattern` est définie par défaut sur `isolated`, ce qui crée un Lambda par opération Smithy. Sélectionnez `shared` si vous préférez un seul gestionnaire Lambda partagé pour l'ensemble de l'API, avec des remplacements optionnels par opération.
:::
...
- **model/** Projet de modèle Smithy
  ...
  - ssdk.rolldown.config.mjs Bundle le SDK serveur TypeScript généré
  - src/
...
Comme ce générateur crée une infrastructure as code selon votre choix de `iac`, il génère un projet dans `packages/common` incluant les constructs CDK ou modules Terraform pertinents.
...
Le projet de modèle Smithy utilise la [CLI Smithy](https://smithy.io/2.0/guides/smithy-cli/index.html) pour construire les artefacts Smithy et générer le SDK serveur TypeScript :
...
Un projet généré sur Windows valide une cible `compile` qui invoque `smithy` directement, donc toute autre personne travaillant dessus — y compris sur macOS ou Linux — a également besoin de la CLI Smithy dans son `PATH`. Pour que ces machines résolvent la CLI via `mise` à la place, basculez la cible vers la commande `mise` comme [décrit ci-dessous](#choosing-how-the-cli-is-resolved).
...
Le générateur crée une infrastructure CDK ou Terraform selon votre choix de `iac`.
...
Nous ne prenons pas en charge les intégrations fortement typées pour Terraform, donc aucune cible de génération de code n'est configurée si vous avez choisi Terraform comme `iac`.
:::
</Fragment>
</Infrastructure>
