...
Pour invoquer votre API depuis un site React, vous pouvez utiliser le générateur <Link path="guides/connection/react-smithy">`connection`</Link>, qui fournit un client fortement typé généré à partir de votre modèle Smithy. projet avec d'autres dans votre espace de travail. Les connexions suivantes impliquent ce projet :
