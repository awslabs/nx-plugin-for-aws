...
La CLI est résolue par [mise](https://mise.jdx.dev/), ajouté à votre espace de travail comme dépendance de développement, donc il n'y a rien à installer — elle télécharge et met en cache la version épinglée la première fois que vous construisez.

:::caution[Windows]
`mise` ne publie aucun package Windows sur npm, donc suivez le [guide d'installation de la CLI Smithy](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) pour l'installer vous-même avant de construire.
:::

:::tip[Construction en CI]
`mise` lit les métadonnées de version de la CLI depuis l'API GitHub, dont la limite de taux non authentifiée est partagée par adresse IP et facilement épuisée sur un runner CI — le téléchargement échoue alors avec `403 Forbidden`. Définissez `GITHUB_TOKEN` dans votre environnement de build pour la lever. Les builds ultérieurs utilisent le cache local et ne font aucune requête.
:::
