...
Sur macOS et Linux, la CLI est résolue par [mise](https://mise.jdx.dev/), que le build récupère à la demande, donc il n'y a rien à installer — elle télécharge et met en cache la version épinglée la première fois que vous construisez. Sur Windows, la CLI est un prérequis que vous installez vous-même, et la mise à niveau de l'espace de travail ne la met pas à niveau — voir <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> et <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> dans le guide du projet Smithy.
