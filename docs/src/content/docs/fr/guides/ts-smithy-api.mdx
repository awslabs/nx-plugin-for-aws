...
L'option `integrationPattern` est définie par défaut sur `isolated`, ce qui crée un Lambda par opération Smithy. Sélectionnez `shared` si vous préférez un seul gestionnaire Lambda partagé pour toute l'API, avec des remplacements optionnels par opération.
:::
...
### Ajouter une bibliothèque de formes
...
Consultez le <Link path="guides/smithy-project">guide du projet Smithy</Link> pour savoir comment créer une bibliothèque de formes et la connecter en tant que dépendance du modèle de votre API.
...
Ce contexte est transmis à toutes les implémentations d'opérations et peut être utilisé pour partager des ressources comme des connexions de base de données, de la configuration ou des utilitaires de journalisation.

:::caution[Contexte manuel requis]
Vous devez construire le contexte vous-même dans `handler.ts` (le point d'entrée de la fonction Lambda) et `local-server.ts` (le point d'entrée pour l'exécution locale via la cible `serve`).
:::
...
Tout d'abord, exposez l'identité résolue sur le contexte du service dans `src/context.ts`. Nous la fournissons sous forme de fonction afin que l'`UnauthorizedError` soit levée depuis une opération (où le Server SDK la sérialise en un `403`), plutôt que depuis le gestionnaire :
...
<OptionFilter when={{ auth: 'iam' }} description="Résolution d'identité pour les API authentifiées IAM">
Pour l'authentification `IAM`, nous recherchons l'appelant dans Cognito en utilisant le sub extrait de l'événement API Gateway. La recherche utilise le client Cognito Identity Provider, qui n'est pas une dépendance d'un backend Smithy généré, alors installez-le d'abord dans le projet backend :
...
<OptionFilter when={{ auth: 'cognito' }} description="Résolution d'identité pour les API authentifiées Cognito">
...
Si vous avez sélectionné l'authentification `Cognito`, vous devrez fournir la propriété `identity` au construct de l'API :
...
Lors de l'utilisation de l'authentification `Custom`, le construct crée un autorisateur Lambda en interne à partir du fichier `src/authorizer.ts` généré, qui **refuse toutes les requêtes par défaut**. Vous devez implémenter votre logique d'autorisation dans ce fichier avant que votre API n'accepte du trafic.
...
:::caution[Autorisateur Lambda personnalisé]
Lors de l'utilisation de l'authentification `Custom`, votre API est protégée par un autorisateur Lambda qui **refuse toutes les requêtes par défaut**. Vous devez implémenter votre logique d'autorisation dans le fichier `src/authorizer.ts` généré avant que votre API n'accepte du trafic.
...
<Snippet name="api/custom-domain" parentHeading="Domaine personnalisé" />
...
:::tip[Rechargement à chaud]
Si vous travaillez activement à la fois sur votre infrastructure CDK et votre API Smithy ensemble, vous pouvez utiliser [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) pour régénérer ces types à chaque fois que vous apportez des modifications au modèle :
...
<OptionFilter when={{ auth: 'iam' }} description="API authentifiées IAM uniquement">
