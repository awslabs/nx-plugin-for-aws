...
Pour invoquer votre API depuis un site React, vous pouvez utiliser le générateur <Link path="guides/connection/react-smithy">`connection`</Link>, qui fournit un client fortement typé généré à partir de votre modèle Smithy. généré à partir de votre modèle Smithy.