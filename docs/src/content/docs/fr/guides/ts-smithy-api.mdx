...
Pour invoquer votre API depuis un site React, vous pouvez utiliser le générateur <Link path="guides/api-connection/react-smithy">`api-connection`</Link>, qui fournit un client fortement typé généré à partir de votre modèle Smithy.