...
Le générateur fournit également une interface de chat interactive minimale sous `scripts/<your-agent-name>/chat.ts` et une cible correspondante `<your-agent-name>-chat`. Cette cible dépend de `<your-agent-name>-serve-local`, donc son exécution démarre l'agent localement et vous place dans un chat terminal interactif avec celui-ci :

<NxCommands commands={['run your-project:agent-chat']} />

L'interface CLI est un script TypeScript (exécuté via `tsx`) qui utilise le client approprié au protocole pour diffuser les réponses token par token via [`@clack/prompts`](https://github.com/bombshell-dev/clack) :

- **HTTP** : `POST /invocations` avec streaming JSONL
- **A2A** : `@a2a-js/sdk` `ClientFactory` avec `sendMessageStream`
