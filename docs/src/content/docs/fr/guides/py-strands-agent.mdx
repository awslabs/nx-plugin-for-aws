...
Le générateur configure une cible Nx `<your-agent-name>-chat` qui dépend de `<your-agent-name>-serve-local`. Son exécution démarre l'agent localement et vous place dans un chat terminal interactif alimenté par [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli) :

<NxCommands commands={['run your-project:agent-chat']} />

Pour les agents **A2A** et **AG-UI**, la cible de chat invoque simplement `agent-chat-cli` en tant que binaire contre l'agent exécuté localement — aucun code n'est généré dans votre espace de travail.

Pour les agents **HTTP**, le générateur émet :
...
- Une cible Nx `<your-agent-name>-generate-client` qui exécute `open-api#ts-client` pour produire un client TypeScript type-safe sous `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — un adaptateur léger qui importe le client généré et le connecte au `chatLoop` de `agent-chat-cli`

Le script de chat est un petit wrapper autour du client généré. Lorsque vous personnalisez la forme d'entrée de l'agent (par exemple, ajoutez de nouveaux champs à `InvokeInput`), mettez à jour le `sendMessage` de l'adaptateur pour passer les nouveaux champs et le reste fonctionne automatiquement.
