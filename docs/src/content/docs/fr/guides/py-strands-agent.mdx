...
Générez un agent [Strands Agent](https://strandsagents.com/) en Python pour créer des agents IA avec des outils, et déployez-le optionnellement sur [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
  ...
  - your-project/
    - your_module/
      - agent/ (ou nom personnalisé si spécifié)
        ...
        - agent.py Définition principale de l'agent avec des outils exemple
        - main.py Point d'entrée pour Bedrock AgentCore Runtime
        - agentcore_mcp_client.py Factory client utile pour invoquer des serveurs MCP hébergés sur Bedrock AgentCore
...
:::note
Sauf si vous avez sélectionné `None` pour `computeType`, le générateur créera également l'infrastructure CDK ou Terraform pour déployer votre agent Strands.
:::
...
- La validation de types basée sur les indications de type
- La génération de schémas JSON pour l'appel d'outils
- La gestion d'erreurs et le formatage des réponses

### Utilisation d'outils prédéfinis

Strands fournit une collection d'outils prédéfinis via le package `strands-tools` :
...
Avec l'exemple d'authentification IAM ci-dessus, nous devons configurer deux choses dans notre infrastructure. Premièrement, ajouter la variable d'environnement consommée par notre agent pour l'ARN du runtime AgentCore du serveur MCP, et deuxièmement accorder les permissions d'invocation du serveur MCP à notre agent. Cela peut être réalisé ainsi :
...
Pour un guide approfondi sur l'écriture d'agents Strands, consultez la [documentation Strands](https://strandsagents.com/latest/documentation/docs/).

## SDK Python Bedrock AgentCore

Le générateur configure le [SDK Python Bedrock AgentCore](https://github.com/aws/bedrock-agentcore-sdk-python) pour gérer le contrat HTTP sous-jacent requis pour les agents sur AgentCore.
...
:::note
Comme le générateur fournit une infrastructure CDK ou Terraform qui gère le déploiement de votre agent, vous n'avez pas besoin d'utiliser le `bedrock-agentcore-starter-toolkit` mentionné dans la documentation.
:::
...
export class ExampleStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    // Accordez les permissions d'invocation des modèles Bedrock
    agent.agentCoreRuntime.role.addToPolicy(
      new PolicyStatement({
        ...
        // Vous pouvez restreindre cette ressource aux modèles spécifiques utilisés
...
:::note
Lors du déploiement sur Bedrock AgentCore Runtime, assurez-vous que les modèles de base appropriés sont activés dans votre compte AWS. Par défaut, les agents Strands utilisent Claude 4 Sonnet, mais cela peut être personnalisé comme décrit ci-dessus.
...
:::tip
L'image Docker est construite avec un tag (ex: `my-scope-my-project-agent:latest`), référencé par votre infrastructure CDK ou Terraform, permettant à votre `Dockerfile` d'être colocalisé avec votre projet d'agent Strands.
:::
...
Vous pouvez accorder l'accès d'invocation à votre agent via la méthode `grantInvoke`, par exemple :
...
Pour accorder l'accès d'invocation, ajoutez une politique faisant référence à la sortie `module.my_project_agent.agent_core_runtime_arn` :
