...
[Strands](https://strandsagents.com/latest/documentation/docs/) est un framework Python léger et prêt pour la production destiné à créer des agents IA. Principales fonctionnalités :
...
# Récupération de la région et des credentials
...
:::tip
Si votre serveur MCP cible utilise une authentification JWT, vous pouvez utiliser la méthode `AgentCoreMCPClient.with_jwt_auth` à la place.
:::

Avec l'exemple d'authentification IAM ci-dessus, nous devons configurer deux choses dans notre infrastructure CDK. Premièrement, ajouter la variable d'environnement consommée par notre agent pour l'ARN du runtime AgentCore du serveur MCP, et deuxièmement accorder les permissions d'invocation à notre agent. Cela peut être réalisé comme suit :
...
Pour un guide détaillé sur l'écriture d'agents Strands, consultez la [documentation Strands](https://strandsagents.com/latest/documentation/docs/).
...
:::note
Comme le générateur fournit un construct CDK qui gère le déploiement de votre agent, vous n'avez pas besoin d'utiliser le `bedrock-agentcore-starter-toolkit` mentionné dans la documentation.
:::
...
Si vous avez sélectionné `BedrockAgentCoreRuntime` pour `computeType`, un construct CDK est généré pour déployer votre agent Strands sur [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Une cible `docker` spécifique à votre agent Strands est aussi ajoutée, qui :
...
:::tip
L'image Docker est construite avec un tag (exemple `my-scope-my-project-agent:latest`), que le construct CDK hérite dans son `Dockerfile`, permettant à votre `Dockerfile` d'être colocalisé avec votre projet Strands Agent.
:::
...
Pour configurer l'authentification JWT, passez la propriété `authorizerConfiguration` à votre construct d'agent. Voici un exemple configurant un user pool Cognito pour sécuriser l'agent :
