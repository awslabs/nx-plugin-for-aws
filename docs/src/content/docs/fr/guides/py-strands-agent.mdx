...
Lorsque `protocol` est défini sur `AG-UI`, le point d'entrée utilise l'intégration [ag-ui-strands](https://docs.ag-ui.com/), qui expose votre agent via le protocole AG-UI pour une intégration frontend directe avec [CopilotKit](https://docs.copilotkit.ai/aws-strands) :
...
Le générateur configure une cible nommée `<your-agent-name>-invoke` qui démarre un chat interactif avec votre agent depuis le terminal. Il est fourni sous forme de script TypeScript (via `tsx`) et sélectionne automatiquement le bon protocole (HTTP, A2A, AG-UI) pour votre agent. Par défaut, il se connecte au serveur local :

<NxCommands commands={['agent-invoke your-project']} />

Pour discuter avec un agent déployé à la place, définissez `RUNTIME_CONFIG_APP_ID` sur l'ID d'application AppConfig de votre espace de travail — le CLI résout l'ARN du runtime et invoque le point de terminaison déployé avec des requêtes signées SigV4 :

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Pour les agents authentifiés par Cognito, passez un JWT avec `--accessToken` :

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

Le script CLI se trouve dans `scripts/<agent>/cli.ts` dans votre projet.
