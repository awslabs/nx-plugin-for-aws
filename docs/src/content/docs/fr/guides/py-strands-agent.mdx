...
Vous pouvez définir un modèle Pydantic pour générer des données structurées à la place :
...
<Infrastructure>
<Fragment slot="cdk">
Un construct CDK est généré pour votre agent, nommé selon le `name` choisi lors de l'exécution du générateur, ou `<ProjectName>Agent` par défaut.
