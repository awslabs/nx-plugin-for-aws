...
Lorsque `protocol` est défini sur `A2A`, le point d'entrée utilise le [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) au lieu de FastAPI :
...
Le protocole serveur de votre agent détermine comment il communique. Les deux options sont servies par [FastAPI](https://fastapi.tiangolo.com/) — le point d'entrée diffère :
...
Les deux protocoles exposent `/ping` pour le contrat de vérification de santé du runtime AgentCore. Les agents A2A écoutent sur le port `9000` (vs `8080` pour HTTP) ; le Dockerfile et l'infrastructure générés sont configurés pour vous.
...
Le `A2AAgent` du SDK Strands encapsule un point de terminaison A2A distant afin que vous puissiez l'appeler comme un `Agent` local.
...
Les requêtes vers un agent A2A déployé sur AgentCore (y compris la récupération de la carte d'agent) doivent être signées SigV4. Passez un `client_config` à `A2AAgent` avec un `httpx.AsyncClient` qui utilise un flux d'authentification SigV4 :
...
Le rôle IAM de l'appelant nécessite à la fois `bedrock-agentcore:InvokeAgentRuntime` et `bedrock-agentcore:GetAgentCard` sur l'ARN du runtime cible. L'assistant `grantInvokeAccess` du construct généré accorde les deux.
:::

:::note[Dépendance supplémentaire]
Le générateur A2A ajoute `strands-agents[a2a]` pour vous. Si vous ajoutez le code client A2A à un agent HTTP, ajoutez `strands-agents[a2a]` et `httpx` à votre `pyproject.toml`.
:::

#### Utilisation d'un agent A2A comme outil

`A2AAgent` hérite de `AgentBase` comme un `Agent` local, vous pouvez donc l'encapsuler dans un `@tool` et le donner à un autre agent :

```python
from strands import Agent, tool

remote = # A2AAgent construit avec client_config comme ci-dessus

async def ask_remote_agent(prompt: str) -> str:
    """Délègue une question à l'agent A2A distant et retourne sa réponse."""
    result = await remote.invoke_async(prompt)
    return str(result)

ask_remote = tool(
    name="ask_remote_agent",
    description="Délègue une question à l'agent A2A distant et retourne sa réponse.",
)(ask_remote_agent)

agent = Agent(
    system_prompt="Délègue à ask_remote_agent lorsque c'est approprié.",
    tools=[ask_remote],
)
```
