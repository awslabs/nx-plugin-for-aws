...
Les agents AG-UI peuvent être connectés à un frontend React en utilisant le <Link path="/guides/connection/react-agui">générateur de connexion `react` → agent AG-UI</Link>, qui configurera les composants [CopilotKit](https://docs.copilotkit.ai/aws-strands) pour une expérience de chat enrichie.
:::
