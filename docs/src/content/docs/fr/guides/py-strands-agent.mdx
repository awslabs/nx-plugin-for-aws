...
Le générateur fournit une option `auth` pour configurer l'authentification de votre agent Strands. Vous pouvez choisir entre l'authentification `IAM` (par défaut) ou `Cognito` lors de la génération de votre agent.
...
Vous pouvez accorder l'accès à votre agent sur Bedrock AgentCore Runtime via la méthode `grantInvokeAccess`, par exemple :
...
Le construct `UserIdentity` est automatiquement généré lorsque vous sélectionnez l'authentification `Cognito`.
</Fragment>
<Fragment slot="terraform">
