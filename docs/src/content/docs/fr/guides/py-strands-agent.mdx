...
Vous avez sélectionné `None` pour `computeType`, donc aucun construct CDK ou module Terraform n'est généré — l'agent Strands ne peut être exécuté que localement.
</OptionFilter>
...
<OptionFilter when={{ auth: 'IAM' }} description="IAM SigV4-signed invocation example">
...
<OptionFilter when={{ auth: 'Cognito' }} description="JWT / Cognito invocation example">
