...
Les **outils de build et de test** (par exemple `vitest`, `tsx`, `rolldown`, la chaîne d'outils Nx (`nx`, `@nx/js`, ...), `typescript`, `@biomejs/biome`) sont partagés dans le workspace, ils se trouvent donc dans les `devDependencies` du `package.json` **racine** plutôt que dans chaque projet. Ces outils ne sont référencés que depuis les fichiers de configuration, les scripts de build et les tests, que la règle de lint exempte.
...
npm (et yarn classic) n'a pas de fonctionnalité de catalogue, donc les versions sont déclarées directement dans chaque `package.json`. Pour les maintenir cohérentes entre les projets, nous recommandons [syncpack](https://github.com/JamieMason/syncpack) (`npx syncpack lint`).
:::

:::note[bun]
Sur bun, quelques packages dont les versions sont lues par les générateurs Nx depuis le manifeste racine (`vite`, `react`, `react-dom`) conservent des plages de versions directes au lieu de références `catalog:` — Nx ne résout pas encore les catalogues bun, et une référence `catalog:` casserait ces générateurs.
:::
