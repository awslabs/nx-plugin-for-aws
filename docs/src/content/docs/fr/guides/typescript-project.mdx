...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Ajoutez la dépendance avec [`--save-catalog`](https://pnpm.io/settings#catalogmode), afin que pnpm enregistre la version dans le catalogue et la référence :

    ```bash
    pnpm add some-npm-package --save-catalog --filter my-library
    ```

    :::note
    Définir [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) dans `pnpm-workspace.yaml` en fait le comportement par défaut pour chaque `pnpm add`. Cela fait également échouer `pnpm add some-npm-package@<version>` chaque fois que le catalogue épingle une version différente, ce qui empêche `nx migrate` de mettre à niveau Nx — l'espace de travail généré le laisse donc non défini.
    :::
  </TabItem>
  <TabItem label="yarn">
