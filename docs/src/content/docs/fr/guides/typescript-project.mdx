...
Sur pnpm, le workspace définit également [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), de sorte que `pnpm add` enregistre la version dans le catalogue et la référence automatiquement.

yarn et bun n'ont pas d'équivalent, et leur protocole `catalog:` ne fait que *référencer* une entrée de catalogue existante — il ne peut pas en créer une. Pour ajouter une nouvelle dépendance sur yarn ou bun, enregistrez d'abord sa version dans le catalogue, puis référencez-la depuis le projet :

- **yarn** — ajoutez la version sous `catalog` dans `.yarnrc.yml`, puis `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — ajoutez la version sous `catalog` dans le `package.json` racine, puis `bun add some-npm-package@catalog: --cwd packages/my-library`
