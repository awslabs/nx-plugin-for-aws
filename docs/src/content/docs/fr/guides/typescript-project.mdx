...
Si vous construisez une fonction AWS Lambda, consultez le générateur <Link path="/guides/ts-lambda-function">`ts#lambda-function`</Link> car il configure le bundling pour vous, ainsi que la génération d'infrastructure et l'ajout d'observabilité et de sécurité des types.
:::
...
Les <Link path="/guides/typescript-infrastructure#deploying-to-aws">cibles de déploiement</Link> dépendent de `assemble`, de sorte que le déploiement ne construit que ce qu'il est sur le point de déployer.
