...
Sur pnpm, le workspace définit également [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) afin que `pnpm add` enregistre automatiquement les nouvelles dépendances dans le catalogue. yarn et bun n'ont pas de paramètre équivalent, donc ajoutez les nouvelles dépendances avec le protocole `catalog:` pour les enregistrer dans le catalogue, par exemple `yarn workspace @my-scope/my-library add some-npm-package@catalog:` ou `bun add some-npm-package@catalog: --cwd packages/my-library`.
