...
Le générateur de projet TypeScript peut être utilisé pour créer une bibliothèque ou une application [TypeScript](https://www.typescriptlang.org/) moderne configurée avec les meilleures pratiques telles que les [modules ECMAScript (ESM)](https://www.typescriptlang.org/docs/handbook/modules/reference.html), les [références de projet](https://www.typescriptlang.org/docs/handbook/project-references.html) TypeScript, [Vitest](https://vitest.dev/) pour exécuter les tests et [Biome](https://biomejs.dev/) pour le linting et le formatage.
...
Le `package.json` du projet commence de manière minimale. Au fur et à mesure que vous ajoutez des dépendances d'exécution, elles sont déclarées ici (les outils de build/test partagés restent à la racine de l'espace de travail) — vous pouvez en savoir plus [ci-dessous](#dependencies).
:::
...
<FileTree>
  ...
  - vitest.config.mts la configuration Vitest partagée que le vitest.config.mts de votre projet étend (créée par le premier projet TypeScript)
  - package.json les outils de build et de test partagés sont ajoutés
  - pnpm-workspace.yaml le projet est ajouté à l'espace de travail, et les versions de dépendances au [catalogue](#catalogs) (le fichier équivalent pour votre gestionnaire de packages)
  - .gitignore les chemins de sortie de build et de test sont ignorés
...
Même si nous utilisons TypeScript, et que `sayHello` est défini dans `hello.ts`, nous utilisons l'extension de fichier `.js` dans notre importation. Vous pouvez en savoir plus à ce sujet [ici](https://www.typescriptlang.org/docs/handbook/modules/reference.html).
:::
...
Les [alias TypeScript](https://www.typescriptlang.org/docs/handbook/modules/reference.html#paths) pour votre projet sont configurés dans le `tsconfig.base.json` de votre espace de travail, ce qui vous permet de référencer votre projet TypeScript depuis d'autres projets TypeScript :
...
C'est parce qu'une [référence de projet](https://www.typescriptlang.org/docs/handbook/project-references.html) n'a pas encore été configurée, et que le projet importé n'a pas encore été déclaré comme dépendance d'espace de travail dans le `package.json` de votre projet.
...
:::tip[Raccourci de build]
Vous pouvez également simplement construire votre projet — les générateurs de synchronisation s'exécutent en premier et vous serez invité à appliquer les modifications en attente :
...
Sélectionnez `Yes` pour permettre à Nx de synchroniser votre espace de travail.
:::
...
:::note
La règle de lint [`noUndeclaredDependencies`](https://biomejs.dev/linter/rules/no-undeclared-dependencies/) de Biome impose que la source d'un projet n'importe que des packages déclarés dans son propre `package.json`. Les fichiers de configuration, les scripts de build et les tests sont exemptés, de sorte que les outils partagés peuvent rester à la racine.
:::

#### Catalogues

Pour les gestionnaires de packages qui prennent en charge les catalogues ([pnpm](https://pnpm.io/catalogs), [yarn](https://yarnpkg.com/features/catalogs) et [bun](https://bun.com/docs/install/catalogs)), les générateurs enregistrent chaque version dans le catalogue et la référencent avec le protocole `catalog:` (par exemple `"zod": "catalog:"`), de sorte que le catalogue est la source unique de vérité pour les versions, quel que soit le projet qui déclare la dépendance. Cela vous aide à suivre une [politique de version unique](https://nx.dev/concepts/decisions/dependency-management#single-version-policy), en évitant les conflits de types et les copies multiples de packages dans les bundles de déploiement.

Lorsque vous ajoutez vous-même une nouvelle dépendance, conservez-la également dans le catalogue :

<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    L'espace de travail définit [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), donc `pnpm add` enregistre la version dans le catalogue et la référence automatiquement — aucune étape supplémentaire.
...
Lorsque vous utilisez votre projet TypeScript comme code d'exécution (par exemple comme gestionnaire pour une fonction AWS Lambda), il est recommandé d'utiliser un outil tel que [Rolldown](https://rolldown.rs/) pour regrouper votre projet, car cela peut effectuer un [tree-shaking](https://rolldown.rs/in-depth/why-bundlers) pour garantir que seules les dépendances que votre projet référence réellement sont incluses.
...
Le `cwd` est ce qui permet à `rolldown` de trouver sa configuration : sans lui, la commande s'exécute depuis la racine de l'espace de travail, où `rolldown.config.ts` n'existe pas.
...
Notez que dans la cible ci-dessus, nous avons choisi `src/index.ts` comme point d'entrée pour le bundle, ce qui signifie que le code exporté depuis ce fichier sera inclus dans le bundle, ainsi que toutes ses dépendances.
:::

:::tip[Fonctions Lambda]
Si vous construisez une fonction AWS Lambda, consultez le générateur <Link path="/fr/guides/ts-lambda-function">`ts#lambda-function`</Link> car il configure le bundling pour vous, ainsi que la génération d'infrastructure et l'ajout d'observabilité et de sécurité des types.
:::

### Installation

Installez les dépendances TypeScript avec la commande suivante :
...
Votre projet dispose également d'une cible `assemble`, qui produit tout ce que votre projet contribue à un déploiement (par exemple sa sortie compilée ou regroupée), sans exécuter les contrôles de lint, de test ou de vérification de type.

<NxCommands commands={['assemble <project-name>']} />

Les <Link path="/fr/guides/typescript-infrastructure#deploying-to-aws">cibles de déploiement</Link> dépendent de `assemble`, de sorte que le déploiement ne construit que ce qu'il est sur le point de déployer.
...
Pour plus de détails sur la façon d'écrire des tests et des fonctionnalités telles que le mocking de dépendances, consultez la [documentation Vitest](https://vitest.dev/guide/#writing-tests)
...
:::tip[Commande raccourcie]
Cela a une commande raccourcie depuis la racine de votre espace de travail :
