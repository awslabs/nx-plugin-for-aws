...
Pour les agents **A2A**, la cible exécute simplement `agent-chat-cli` en tant que binaire — aucun code n'est fourni dans votre espace de travail.

Pour les agents **HTTP** (tRPC sur WebSocket), le générateur émet un petit fichier `scripts/<your-agent-name>/chat.ts` qui encapsule le `<Agent>Client.local({ url })` généré en tant que `ChatAdapter` et le transmet à la fonction `chatLoop` de `agent-chat-cli`.
