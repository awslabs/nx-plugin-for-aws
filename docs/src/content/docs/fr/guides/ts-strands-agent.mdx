...
Le générateur fournit également une interface CLI de chat interactive minimale sous `scripts/<your-agent-name>/chat.ts` et une cible `<your-agent-name>-chat` correspondante. Cette cible dépend de `<your-agent-name>-serve-local`, donc son exécution démarre l'agent localement et vous place dans un chat terminal interactif avec celui-ci :

<NxCommands commands={['run your-project:agent-chat']} />

La CLI utilise le client approprié au protocole (`AgentClient.local` pour HTTP/tRPC ou `@a2a-js/sdk` pour A2A) et diffuse les réponses de l'agent token par token via [`@clack/prompts`](https://github.com/bombshell-dev/clack).
