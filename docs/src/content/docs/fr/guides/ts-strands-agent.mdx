...
:::caution[AG-UI pas encore pris en charge pour TypeScript]
Le [protocole AG-UI](https://docs.ag-ui.com/) n'est pas encore pris en charge pour les Strands Agents TypeScript. Si vous avez besoin de la prise en charge AG-UI, veuillez utiliser le générateur <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> avec `protocol=AG-UI`. Consultez [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) pour connaître l'état de la prise en charge AG-UI pour TypeScript.
:::
...
Le générateur configure une cible nommée `<your-agent-name>-invoke` qui démarre un chat interactif avec votre agent depuis le terminal. Par défaut, il se connecte au serveur local :

<NxCommands commands={['agent-invoke your-project']} />

Pour discuter avec un agent déployé à la place, définissez `RUNTIME_CONFIG_APP_ID` sur l'ID d'application AppConfig de votre espace de travail — le CLI résout l'ARN du runtime et invoque le point de terminaison déployé :
...
Le script CLI se trouve dans `scripts/<agent>/cli.ts` et sélectionne automatiquement le bon protocole (HTTP/A2A) pour votre agent.
