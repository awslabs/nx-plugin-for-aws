...
Vous avez sélectionné `None` pour `computeType`, donc aucun construct CDK ou module Terraform n'est généré — le Strands Agent ne peut être exécuté que localement.
</OptionFilter>
...
<OptionFilter when={{ auth: 'IAM' }} description="IAM-authenticated client factory example">
...
<OptionFilter when={{ auth: 'Cognito' }} description="JWT / Cognito-authenticated client factory example">
