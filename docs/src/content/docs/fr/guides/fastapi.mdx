...
Puisqu'AWS API Gateway ne supporte pas les réponses streamées, vous devrez déployer votre FastAPI sur une plateforme qui le supporte. L'option la plus simple est d'utiliser une URL de fonction AWS Lambda.
...
Pour cela avec Terraform, vous pouvez remplacer l'infrastructure API Gateway générée par une URL de fonction Lambda supportant le streaming de réponses.
...
# Exemple : Attachement de la politique à un rôle (décommenter et modifier si besoin)
...
Par exemple, si vous souhaitez streamer une série d'objets JSON depuis votre API, vous pouvez l'implémenter comme suit :
...
Le construct `UserIdentity` peut être généré avec le <Link path="/guides/react-website-auth">générateur `ts#react-website-auth`</Link>
:::
</Fragment>
<Fragment slot="terraform">
...
# Accéder au rôle IAM pour les permissions supplémentaires
...
<Fragment slot="terraform">
```hcl
# Créer une politique IAM pour autoriser l'invocation de l'API
...
# Attacher la politique à un rôle IAM (ex: utilisateurs authentifiés)
...
- `module.my_api.api_execution_arn` - Pour les permissions execute-api:Invoke
