...
- Ajouter l'extension OpenAPI `x-streaming: true` si vous prévoyez d'utiliser le générateur <Link path="guides/connection/react-fastapi">Connection</Link>.
...
Pour consommer un flux de réponses, vous pouvez utiliser le <Link path="guides/connection/react-fastapi#consuming-a-stream">Générateur Connection</Link> qui fournira une méthode typée pour itérer sur les chunks streamés.
...
Pour invoquer votre API depuis un site React, vous pouvez utiliser le générateur <Link path="guides/connection/react-fastapi">`connection`</Link>.