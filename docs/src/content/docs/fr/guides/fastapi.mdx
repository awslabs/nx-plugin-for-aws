...
- Ajouter l'extension OpenAPI `x-streaming: true` si vous prévoyez d'utiliser <Link path="guides/connection/react-fastapi">API Connection</Link>.
...
Pour consommer un flux de réponses, vous pouvez utiliser le <Link path="guides/connection/react-fastapi#consuming-a-stream">Générateur API Connection</Link> qui fournira une méthode typée pour itérer sur les chunks streamés.
