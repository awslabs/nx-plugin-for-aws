...
`nx watch` nécessite le [démon Nx](https://nx.dev/concepts/nx-daemon), qui est activé par défaut mais désactivé dans les environnements CI, les conteneurs Docker et les environnements sandboxés. Lorsqu'il est désactivé, la commande se termine avec `Daemon is not running` — définissez `NX_DAEMON=true` pour l'activer.
:::
</Fragment>
<Fragment slot="terraform">
