...
:::caution[Épingler `boto3` à la version ci-dessus]
D'autres générateurs ajoutent `boto3` épinglé exactement à cette version. Étant donné que votre espace de travail partage un seul environnement virtuel UV et un fichier de verrouillage, l'ajouter sans épinglage (`add boto3`) résout la dernière version et écrit une contrainte `boto3>=<latest>`, que `uv` ne peut pas satisfaire aux côtés de l'épinglage. Toutes les cibles Python dans l'espace de travail échouent alors à résoudre les dépendances.
:::
