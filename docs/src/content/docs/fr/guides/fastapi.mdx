...
Le générateur FastAPI crée une nouvelle FastAPI avec une infrastructure AWS CDK ou Terraform configurée. Le backend généré utilise AWS Lambda pour un déploiement serverless, exposé via une API AWS API Gateway. Il configure [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) pour l'observabilité, incluant la journalisation, le traçage AWS X-Ray et les métriques Cloudwatch.
...
L'implémentation principale de l'API se trouve dans `main.py`. C'est là que vous définissez vos routes API et leurs implémentations. Voici un exemple :
...
Le générateur configure la journalisation structurée en utilisant AWS Lambda Powertools. Vous pouvez accéder au logger dans vos gestionnaires de route :
...
Il est recommandé de spécifier des modèles de réponse pour vos opérations API pour une meilleure génération de code si vous utilisez le générateur `connection`. <Link path="guides/connection/react-fastapi#errors">Voir ici pour plus de détails</Link>.
:::
...
La FastAPI générée prend en charge les réponses en streaming prêtes à l'emploi lors de l'utilisation d'une API REST. L'infrastructure est configurée pour utiliser le [AWS Lambda Web Adapter](https://github.com/awslabs/aws-lambda-web-adapter) pour exécuter votre FastAPI via uvicorn dans Lambda, avec `ResponseTransferMode.STREAM` dans API Gateway pour toutes les opérations d'API REST, ce qui permet au streaming de fonctionner aux côtés d'opérations non-streaming.

#### Utilisation de `JsonStreamingResponse`

Le `init.py` généré exporte une classe `JsonStreamingResponse` qui fournit un streaming type-safe avec une génération de schéma OpenAPI appropriée. Cela garantit que le <Link path="guides/connection/react-fastapi">générateur `connection`</Link> peut produire des méthodes client de streaming correctement typées.
...
Une cible `generate:<ApiName>-metadata` est ajoutée au `project.json` des constructs communs pour faciliter cette génération de code, qui émet un fichier tel que `packages/common/constructs/src/generated/my-api/metadata.gen.ts`. Comme cela est généré au moment du build, il est ignoré dans le contrôle de version.
...
Si vous travaillez activement à la fois sur votre infrastructure CDK et FastAPI, vous pouvez utiliser [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) pour régénérer ces types à chaque fois que vous apportez des modifications à l'API :
...
`nx watch` nécessite le [démon Nx](https://nx.dev/concepts/nx-daemon), qui est activé par défaut mais désactivé dans CI, les conteneurs Docker et les environnements sandboxés. Lorsqu'il est désactivé, la commande se termine avec `Daemon is not running` — définissez `NX_DAEMON=true` pour l'activer.
:::
</Fragment>
<Fragment slot="terraform">
...
- Documentation API interactive à `/docs` ou `/redoc`
- Schéma OpenAPI à `/openapi.json`

:::note[Noms de projet]
Les noms de projet Python utilisent des underscores, donc une API générée avec `--name=MyApi` est adressée comme `my_api` (ou par son nom complet, `<scope>.my_api`). Le port sur lequel le serveur écoute est attribué par projet — lisez-le depuis la cible `serve` dans le `project.json` de votre projet.
:::
