...
Le générateur FastAPI crée une nouvelle API FastAPI avec une infrastructure AWS CDK ou Terraform. Le backend généré utilise AWS Lambda pour un déploiement serverless, exposé via une API AWS API Gateway. Il configure [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) pour l'observabilité, incluant la journalisation, le traçage AWS X-Ray et les métriques CloudWatch.
...
Le générateur créera également une infrastructure CDK ou Terraform en tant que code qui peut être utilisée pour déployer votre API, située dans le répertoire `packages/common`.
...
Puisqu'AWS API Gateway ne supporte pas les réponses en streaming, vous devrez déployer votre FastAPI sur une plateforme qui le supporte. L'option la plus simple est d'utiliser une URL de fonction AWS Lambda.
...
<details>
<summary>Exemple de configuration Terraform pour une URL de fonction Lambda en streaming</summary>
...
# Politique IAM pour accorder l'accès d'invocation à l'URL de fonction
...
Après avoir mis à jour l'infrastructure pour supporter le streaming, vous pouvez implémenter une API en streaming avec FastAPI. L'API doit :
...
- Ajouter l'extension vendor OpenAPI `x-streaming: true` si vous prévoyez d'utiliser <Link path="guides/api-connection/react-fastapi">API Connection</Link>.

Par exemple, si vous souhaitez diffuser une série d'objets JSON depuis votre API, vous pouvez l'implémenter comme suit :
...
Le générateur FastAPI crée une infrastructure CDK ou Terraform en tant que code selon le `iacProvider` sélectionné. Vous pouvez l'utiliser pour déployer votre API.
...
:::attention
Si vous avez sélectionné `None` pour `auth` lors de l'exécution du générateur, vous pourriez voir des échecs de vérification Checkov comme :
...
<Infrastructure>
<Fragment slot="cdk">
Puisque les opérations FastAPI sont définies en Python et l'infrastructure CDK en TypeScript, nous instrumentons la génération de code pour fournir des métadonnées au construct CDK afin d'offrir une interface type-safe pour les intégrations.

Une cible `generate:<ApiName>-metadata` est ajoutée au `project.json` des constructs communs pour faciliter cette génération, émettant un fichier comme `packages/common/constructs/src/generated/my-api/metadata.gen.ts`. Ce fichier étant généré au build, il est ignoré dans le contrôle de version.
...
<Fragment slot="terraform">
:::remarque
Nous ne supportons pas les intégrations type-safe pour Terraform, donc aucune cible de génération de code n'est configurée si vous avez choisi Terraform comme `iacProvider`.
:::
</Fragment>
</Infrastructure>
...
# Attache la politique à un rôle IAM (ex: pour utilisateurs authentifiés)
...
- Rechargement automatique sur modification du code
- Documentation interactive de l'API sur `/docs` ou `/redoc`
- Schéma OpenAPI sur `/openapi.json`

## Invocation de votre API FastAPI

Pour invoquer votre API depuis un site React, vous pouvez utiliser le <Link path="guides/api-connection/react-fastapi">générateur `api-connection`</Link>.