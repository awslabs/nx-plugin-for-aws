...
3. Cognito Auth ajouté via le <Link path="/guides/react-website-auth">générateur `ts#website#auth`</Link> si vous vous connectez à une API qui utilise l'authentification Cognito ou IAM
...
Si vous avez <Link path="/guides/fastapi#streaming">configuré votre FastAPI pour diffuser des réponses</Link>, votre hook `useQuery` mettra automatiquement à jour ses données à mesure que de nouveaux morceaux du flux arrivent.
...
Si vous avez <Link path="/guides/fastapi#streaming">configuré votre FastAPI pour diffuser des réponses</Link>>, le client généré inclura des méthodes type-safe pour itérer de manière asynchrone sur les morceaux de votre flux en utilisant la syntaxe `for await`.
