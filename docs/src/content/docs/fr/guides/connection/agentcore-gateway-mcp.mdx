...
démarre une passerelle locale plus chaque serveur MCP attaché sur son port local assigné. La passerelle locale expose un point de terminaison MCP unique qui agrège les outils des serveurs attachés. Les agents connectés à la passerelle via les générateurs de connexion de passerelle <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> ou <Link path="guides/connection/py-agent-gateway">Python</Link> pointent vers elle lors de l'exécution avec `LOCAL_DEV=true`.
