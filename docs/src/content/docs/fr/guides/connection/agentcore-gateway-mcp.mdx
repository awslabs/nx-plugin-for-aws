...
- packages/\<gateway>
  ...
  - serve-local.ts `ATTACHED_MCP_SERVERS` mis à jour pour que la passerelle locale agrège le serveur MCP

</FileTree>

La cible `<gateway>-serve-local` du projet de passerelle gagne une dépendance sur la cible `<mcp>-serve-local` du serveur MCP, donc l'exécution de la passerelle localement démarre également le serveur MCP. Le serveur MCP est également enregistré dans le `serve-local.ts` du projet de passerelle afin que la passerelle locale agrège ses outils.
...
Le `name` de la cible (`my-mcp-server` ci-dessus) est utilisé comme préfixe pour les noms d'action Cedar — voir la <Link path="guides/agentcore-gateway">section Rédaction de politiques</Link>. `policy_dependencies` garantit que les politiques Cedar référençant les actions de cette cible sont créées après que la cible a enregistré ces actions.
</Fragment>
</Infrastructure>
