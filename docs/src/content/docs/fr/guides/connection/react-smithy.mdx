...
2. D'un backend d'API Smithy TypeScript fonctionnel (généré via le générateur <Link path="/guides/ts-smithy-api">`ts#api`</Link> avec `--framework=smithy`)
3. D'une authentification Cognito ajoutée via le générateur <Link path="/guides/react-website-auth">`ts#website#auth`</Link> si vous connectez une API utilisant l'authentification Cognito ou IAM
...
Si vous travaillez simultanément sur votre application React et votre API Smithy, utilisez la cible `dev` de l'application React (un alias pour `serve-local`). Cela régénérera automatiquement le client à chaque changement d'API, avec rechargement à chaud du site et du serveur Smithy local :
