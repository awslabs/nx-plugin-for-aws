...
2. Un backend d'API TypeScript Smithy fonctionnel (généré à l'aide du <Link path="/guides/ts-smithy-api">générateur `ts#api`</Link> avec `--framework=smithy`)
3. Cognito Auth ajouté via le <Link path="/guides/react-website-auth">générateur `ts#website#auth`</Link> si vous connectez une API qui utilise l'authentification Cognito ou IAM
