...
- **IAM** (par défaut) : Utilise les URL présignées AWS SigV4 pour authentifier la connexion WebSocket. Les identifiants sont obtenus à partir du pool d'identités Cognito configuré avec l'authentification de votre site web. En mode `serve-local`, la signature est automatiquement ignorée lorsque Cognito n'est pas configuré
...
3. Pour l'authentification IAM, la signature de requête est automatiquement ignorée en mode `serve-local` lorsque Cognito n'est pas configuré
