...
description: Connecter un site web React à un Python Agent
when:
...
Nx Plugin for AWS fournit un générateur pour intégrer rapidement votre <Link path="guides/py-agent">Python Agent</Link> avec un site web React. Il configure tous les éléments nécessaires pour se connecter à votre agent via un client généré par OpenAPI avec typage sécurisé, incluant la prise en charge de l'authentification AWS IAM et Cognito.

:::tip[Agents AG-UI]
Si votre Python Agent a été généré avec `protocol=ag-ui`, le générateur de connexion configure à la place [CopilotKit](https://docs.copilotkit.ai/aws-strands) et un `HttpAgent` [`@ag-ui/client`](https://docs.ag-ui.com/) sur le site web React. Consultez le guide <Link path="guides/connection/react-agui">React vers AG-UI Agent</Link> pour plus de détails.
:::
...
2. Un Python Agent avec `protocol=http` (généré avec le <Link path="guides/py-agent">générateur `py#agent`</Link>)
...
- [Documentation TanStack Query](https://tanstack.com/query/v5)