...
4. Une authentification Cognito ajoutée via le <Link path="/guides/react-website-auth">générateur `ts#website#auth`</Link> si vous connectez une API utilisant l'authentification Cognito ou IAM
...
Les souscriptions ne sont prises en charge que lorsque l'API tRPC utilise `rest-lambda` (API REST) comme type de calcul. Les API HTTP d'API Gateway ne prennent pas en charge le streaming de réponse.
:::
