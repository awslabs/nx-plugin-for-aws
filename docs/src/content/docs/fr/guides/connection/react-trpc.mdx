...
- **`ServerlessApiGatewayRestApi`** — lien HTTP avec `httpSubscriptionLink` via Server-Sent Events pour les souscriptions tRPC.
- **`ServerlessApiGatewayHttpApi`** — lien HTTP simple (pas de souscriptions).
- **`BedrockAgentCoreRuntimeWebSocket`** — lien WebSocket vers l'URL du runtime AgentCore, avec handshake signé SigV4 (IAM) ou sous-protocole JWT encodé en base64url (Cognito). L'ARN du runtime est lu depuis `runtimeConfig.apis.<ApiName>` et converti en URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` à la volée.
...
De plus, il installe les dépendances requises :
  ...
  - `event-source-polyfill` (si utilisation d'un backend API REST, pour le support des souscriptions)
