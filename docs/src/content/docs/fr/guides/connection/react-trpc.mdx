...
Le générateur inspecte le `computeType` du backend et produit le client correspondant :

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Un lien HTTP avec `httpSubscriptionLink` via Server-Sent Events pour les souscriptions tRPC.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
Un lien HTTP simple. Les souscriptions ne sont pas prises en charge car les API HTTP d'API Gateway ne prennent pas en charge le streaming de réponse.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Un lien WebSocket vers l'URL du runtime AgentCore, avec une poignée de main signée SigV4 (IAM) ou un sous-protocole JWT encodé en base64url (Cognito). L'ARN du runtime est lu depuis `runtimeConfig.apis.<ApiName>` et converti en une URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` à la volée.
</TabItem>
</Tabs>
...
De plus, il installe les dépendances requises :
  ...
  - `event-source-polyfill` (si utilisation d'un backend REST API, pour le support des souscriptions)
  - `react-oidc-context` (si utilisation de l'authentification Cognito)
...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayHttpApi' }} description="Les backends HTTP API ne prennent pas en charge les souscriptions">
:::caution[Pas de souscriptions sur HTTP API]
Les API HTTP d'API Gateway ne prennent pas en charge le streaming de réponse. Sélectionnez `ServerlessApiGatewayRestApi` ou `BedrockAgentCoreRuntimeWebSocket` comme `computeType` de votre backend si vous avez besoin de souscriptions.
:::
</OptionFilter>

<OptionFilter when={{ computeType: ['ServerlessApiGatewayRestApi', 'BedrockAgentCoreRuntimeWebSocket'] }} description="Comment les souscriptions atteignent le client — le transport diffère selon le type de calcul du backend">
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
Le client généré utilise un `splitLink` qui route les opérations de souscription via `httpSubscriptionLink` (Server-Sent Events) et les requêtes/mutations standard via `httpLink`. Les souscriptions fonctionnent immédiatement sans configuration supplémentaire.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
Le client généré utilise `wsLink` vers l'URL du runtime AgentCore — toutes les opérations tRPC (requêtes, mutations et souscriptions) transitent via une seule connexion WebSocket. La poignée de main est authentifiée automatiquement : signée SigV4 pour l'authentification `IAM`, ou portant le JWT comme sous-protocole encodé en base64url pour `Cognito`.
</TabItem>
</Tabs>
