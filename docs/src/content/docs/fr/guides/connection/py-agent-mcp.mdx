...
1. Un projet Python avec un composant <Link path="guides/py-agent">Agent</Link>
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - app
      - \<mcp\_server\_name>\_client\_strands.py Client Strands par connexion pour chaque serveur MCP
...
Le client Strands est un gestionnaire de contexte, entré dans un bloc `with` autour de l'agent. Le paramètre `session_id` est transmis depuis l'appelant, garantissant la cohérence pour [Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
</TabItem>
