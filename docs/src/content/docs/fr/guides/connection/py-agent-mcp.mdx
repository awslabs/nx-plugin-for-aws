...
Le générateur `connection` peut connecter votre <Link path="guides/py-agent">Agent Python</Link> à un serveur MCP (soit <Link path="guides/ts-mcp-server">TypeScript</Link> soit <Link path="guides/py-mcp-server">Python</Link>).
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_transport.py Plomberie de transport AgentCore partagée
      - agentcore\_mcp\_transport.py Transport MCP indépendant du framework
      - agentcore\_mcp\_client\_\<framework>.py Client MCP encapsulant le transport pour le framework de votre agent
      - auth/ `httpx.Auth` SigV4 / transfert de session indépendant du framework
    - app
      - \_\_init\_\_.py Initialisation du package Python
      - \<mcp\_server\_name>\_client\_\<framework>.py Client par connexion pour chaque serveur MCP
...
Le client Strands est un gestionnaire de contexte, entré dans un bloc `with` autour de l'agent.
</TabItem>
...
Le client LangChain retourne une liste d'outils chargés via [`langchain-mcp-adapters`](https://docs.langchain.com/oss/python/langchain/mcp). Chaque outil ouvre une nouvelle session MCP par appel, de sorte que les outils restent utilisables pendant toute la durée de vie de l'agent — aucun bloc `with` n'est nécessaire.
</TabItem>
</Tabs>

L'ID de session AgentCore est propagé automatiquement au serveur MCP via l'en-tête `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` pour les deux frameworks, garantissant la cohérence pour [Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
