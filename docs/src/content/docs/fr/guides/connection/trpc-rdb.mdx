...
Le générateur `connection` connecte une <Link path="guides/trpc">API tRPC</Link> à un projet de <Link path="guides/ts-rdb">Base de Données Relationnelle</Link>, générant un plugin middleware tRPC type-safe qui rend un client Prisma disponible dans le contexte de vos procédures.
...
- packages/api/src
  - middleware
    - \<db-name>.ts Plugin tRPC exposant le client Prisma dans le contexte des procédures
...
Ajoutez le plugin généré à votre routeur tRPC afin que toutes les procédures l'utilisant aient accès à la base de données :
...
Pour permettre à votre API tRPC de se connecter à la base de données à l'exécution, les fonctions Lambda de l'API doivent être déployées dans le même VPC que la base de données et se voir accorder un accès réseau et IAM.
...
Déployez les fonctions Lambda de l'API dans un **sous-réseau privé avec sortie**, et non dans un sous-réseau privé isolé. À l'exécution, `getPrisma()` récupère les détails de connexion à la base de données depuis AWS AppConfig, qui est un point de terminaison de service AWS public nécessitant un accès Internet sortant.
...
Passez les sorties du module de base de données dans votre module API afin qu'il puisse atteindre la base de données et lire sa configuration d'exécution :
...
Le générateur configure la cible `serve-local` de votre API tRPC pour qu'elle dépende de la cible `serve-local` de la base de données, donc l'exécution de :
