...
L'ARN du runtime AgentCore du serveur MCP est automatiquement enregistré dans l'espace de noms `connection` de la <Link path="guides/runtime-config">Configuration Runtime</Link> par la construction CDK générée, afin que l'agent puisse le découvrir à l'exécution.
</Fragment>
...
L'ARN du runtime AgentCore du serveur MCP est automatiquement enregistré dans l'espace de noms `connection` de la <Link path="guides/runtime-config">Configuration Runtime</Link> par le module Terraform généré, afin que l'agent puisse le découvrir à l'exécution.
</Fragment>
</Infrastructure>
