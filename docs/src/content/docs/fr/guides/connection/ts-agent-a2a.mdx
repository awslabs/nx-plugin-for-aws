...
Le générateur `connection` peut connecter votre <Link path="guides/ts-agent">Agent Strands TypeScript</Link> à un agent [A2A](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) distant — soit <Link path="guides/ts-agent">TypeScript</Link> ou <Link path="guides/py-agent">Python</Link> — afin que votre agent puisse déléguer à un autre agent en tant qu'outil.
...
1. Un projet TypeScript avec un composant <Link path="guides/ts-agent">Agent</Link> (n'importe quel protocole)
2. Un projet avec un composant Agent généré avec `--protocol=A2A` et `--auth=IAM` (soit <Link path="guides/ts-agent">`ts#agent`</Link> ou <Link path="guides/py-agent">`py#agent`</Link>)
...
- packages/common/agent-connection
  - src
    - app
      - \<target-agent-name>-client-strands.ts Client Strands de haut niveau pour l'agent A2A connecté
    - core
      - agentcore-endpoints.ts Résolution ARN/URL indépendante du framework
      - agentcore-fetch.ts Fetch SigV4 / JWT / transfert de session indépendant du framework
      - agentcore-a2a-client-config.ts Configuration du client A2A indépendante du framework (`clientFactory` signé)
      - agentcore-a2a-client-strands.ts Client A2A Strands encapsulant la configuration
...
En coulisses, `RemoteAgentClientStrands.create(sessionId)` retourne un `A2AAgent` Strands configuré avec un `clientFactory` signant SigV4 lorsqu'il est déployé sur AWS, et un endpoint simple `http://localhost:<port>/` lorsque `SERVE_LOCAL=true`. La signature et la résolution des endpoints résident dans le fichier `agentcore-a2a-client-config.ts` indépendant du framework ; seul le mince `agentcore-a2a-client-strands.ts` dépend de Strands.
