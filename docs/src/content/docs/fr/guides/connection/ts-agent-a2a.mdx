...
En coulisses, `RemoteAgentClientStrands.create(sessionId)` retourne un `A2AAgent` Strands configuré avec un `clientFactory` signant SigV4 lorsqu'il est déployé sur AWS, et un endpoint simple `http://localhost:<port>/` lorsque `LOCAL_DEV=true`. La signature et la résolution des endpoints résident dans le fichier `agentcore-a2a-client-config.ts` indépendant du framework ; seul le mince `agentcore-a2a-client-strands.ts` dépend de Strands.
