...
2. Un projet avec un composant Strands Agent généré avec `--protocol=A2A` et `--auth=IAM` (soit <Link path="guides/ts-agent">`ts#agent`</Link> ou <Link path="guides/py-agent">`py#agent`</Link>)
