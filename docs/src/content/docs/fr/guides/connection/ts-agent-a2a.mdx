...
Sous le capot, `RemoteAgentClientStrands.create(sessionId)` retourne un `A2AAgent` Strands configuré avec une `clientFactory` signant SigV4 lorsqu'il est déployé sur AWS, et un point de terminaison `http://localhost:<port>/` simple lorsque `LOCAL_DEV=true`. La signature et la résolution du point de terminaison se trouvent dans le fichier `agentcore-a2a-client-config.ts` indépendant du framework ; seul le mince `agentcore-a2a-client-strands.ts` dépend de Strands.
