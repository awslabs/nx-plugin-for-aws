...
Le générateur `connection` peut connecter votre <Link path="guides/ts-agent">TypeScript Agent</Link> à un serveur MCP (soit <Link path="guides/ts-mcp-server">TypeScript</Link> soit <Link path="guides/py-mcp-server">Python</Link>).
...
1. Un projet TypeScript avec un composant <Link path="guides/ts-agent">Agent</Link>
...
- packages/common/agent-connection
  - src
    ...
    - core
      - agentcore-endpoints.ts Résolution ARN/URL agnostique du framework
      - agentcore-fetch.ts Fetch agnostique du framework avec SigV4 / JWT / transfert de session
      - agentcore-mcp-transport.ts Transport MCP agnostique du framework
