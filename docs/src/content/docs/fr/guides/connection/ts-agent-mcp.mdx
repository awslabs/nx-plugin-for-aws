...
Le générateur `connection` peut connecter votre <Link path="guides/ts-agent">TypeScript Strands Agent</Link> à un serveur MCP (soit <Link path="guides/ts-mcp-server">TypeScript</Link> soit <Link path="guides/py-mcp-server">Python</Link>).
