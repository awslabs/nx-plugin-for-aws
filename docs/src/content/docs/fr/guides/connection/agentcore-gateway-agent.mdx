...
Placer un agent derrière une Gateway est ce qui rend un agent déployé dans un VPC accessible : les appelants communiquent avec le point de terminaison public de la Gateway, et la Gateway atteint l'exécution en interne. Configurez les paramètres réseau de l'exécution de l'agent sur son construct/module (le module Terraform expose `enable_vpc`, `vpc_id` et `subnet_ids` ; le construct CDK accepte les props réseau de l'exécution).
:::
