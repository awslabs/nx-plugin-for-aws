...
Le générateur `connection` connecte un <Link path="guides/py-mcp-server">serveur MCP Python</Link> à un projet de <Link path="guides/py-rdb">base de données relationnelle Python</Link>, rendant la session de base de données disponible pour tous les outils enregistrés dans le serveur.
...
`my_scope_my_db` est le nom du module de votre projet de base de données — un seul module plat nommé `<scope>_<name>`, visible comme répertoire de package à l'intérieur du projet.

`.scalars()` décompresse les lignes de résultat en instances `ExampleModel`, de sorte que `model_dump()` est disponible sur chaque élément.
