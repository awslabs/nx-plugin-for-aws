...
1. Un projet Python avec un composant <Link path="guides/py-agent">Strands Agent</Link> (n'importe quel protocole)
2. Un projet avec un composant Strands Agent généré avec `--protocol=A2A` et `--auth=IAM` (soit <Link path="guides/ts-agent">`ts#agent`</Link> soit <Link path="guides/py-agent">`py#agent`</Link>)
