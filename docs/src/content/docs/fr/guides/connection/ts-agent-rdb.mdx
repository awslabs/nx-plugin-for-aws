...
:::caution
Lorsque `securityGroups` est omis, `RuntimeNetworkConfiguration.usingVpc` crée un groupe de sécurité avec un id de construct fixe (`SecurityGroup`) sur la portée donnée. Passer un groupe de sécurité explicite comme montré évite les collisions d'id de construct lorsque plusieurs runtimes (par exemple un agent et un serveur MCP) sont définis dans la même stack.
:::
