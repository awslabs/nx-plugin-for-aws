...
description: Connecter un Agent TypeScript à une Base de Données Relationnelle
when:
  sourceType: ts#agent
  targetType: ts#rdb
---
...
Le générateur `connection` relie un <Link path="guides/ts-agent">Agent TypeScript</Link> à un projet de <Link path="guides/ts-rdb">Base de Données Relationnelle</Link>, rendant un client Prisma disponible à l'intérieur de votre factory d'agent.
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="Exigences SSL lors de la Connexion sans RDS Proxy" />
