...
Le générateur `connection` connecte un <Link path="guides/ts-mcp-server">serveur MCP TypeScript</Link> à un projet de <Link path="guides/ts-rdb">base de données relationnelle</Link>, rendant un client Prisma disponible pour tous les outils enregistrés dans `createServer`.
...
Le client Prisma est récupéré dans `createServer` et disponible pour tous les outils et ressources enregistrés là :
...
Exécuter à nouveau le générateur avec une cible différente ajoute la deuxième base de données à côté de la première. Les deux sont récupérées dans `createServer` :
