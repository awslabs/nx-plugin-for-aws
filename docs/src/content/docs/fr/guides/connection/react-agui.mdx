...
1. Un site web React (généré avec le <Link path="guides/react-website">générateur `ts#website`</Link>)
...
3. Pour les agents déployés, Cognito Auth ajouté via le <Link path="/guides/react-website-auth">générateur `ts#website#auth`</Link>
