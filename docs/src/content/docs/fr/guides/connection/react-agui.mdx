...
:::caution[Agents TypeScript]
AG-UI n'est pas encore pris en charge pour les Agents TypeScript. Consultez [agents/sdk-typescript#347](https://github.com/agents/sdk-typescript/issues/347) pour connaître le statut actuel. Pour l'instant, utilisez le <Link path="guides/py-agent">générateur `py#agent`</Link> avec `protocol=AG-UI`.
:::
...
2. Un Python Agent avec `protocol=AG-UI` (généré avec le <Link path="guides/py-agent">générateur `py#agent`</Link>)
...
Instanciez les composants CopilotKit avec `agentId` pour sélectionner l'agent à utiliser. L'id est le nom de l'agent — le même que celui que vous avez choisi lors de l'exécution du <Link path="guides/py-agent">générateur `py#agent`</Link> — et vous pouvez également le trouver dans le fichier hook généré (par ex. la clé retournée par `packages/web/src/hooks/useAgui<AgentName>.tsx`).
