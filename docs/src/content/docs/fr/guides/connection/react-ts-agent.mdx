...
Nx Plugin for AWS fournit un générateur pour intégrer rapidement votre <Link path="guides/ts-agent">TypeScript Agent</Link> avec un site web React. Il configure tous les paramètres nécessaires pour se connecter à votre agent via tRPC sur WebSocket, y compris la prise en charge de l'authentification AWS IAM et Cognito. L'intégration fournit une sécurité de type de bout en bout complète entre votre frontend et le routeur tRPC de l'agent.
...
2. Un TypeScript Agent (généré en utilisant le <Link path="guides/ts-agent">générateur `ts#agent`</Link>)
