...
<FileTree>
  - src/\<name>/
    ...
    - files/
      - hello.ts.template Exemple de modèle rendu par le générateur stub
  - src/index.ts Mis à jour pour exporter votre générateur
  - generators.json Créé ou mis à jour pour définir vos générateurs
  - project.json Mis à jour pour enregistrer le générateur dans le projet
  - package.json Créé s'il est absent, et pointé vers `generators.json`
</FileTree>
...
Certaines options ne s'appliquent que pour certaines valeurs d'une autre. Utilisez `x-when` pour indiquer lesquelles, afin que le site de documentation omette l'option là où elle ne s'applique pas et indique la condition là où elle s'applique :
...
Plusieurs clés sont combinées avec AND et une liste de valeurs est combinée avec OR au sein d'une clé, correspondant à `<OptionFilter when={{…}}>` :
...
`x-when` documente l'option ; il n'applique rien, donc un générateur valide ou ignore toujours une option qui ne s'applique pas à l'exécution qui lui a été donnée.
...
- Utilisez `createTreeUsingTsSolutionSetup()` de `@aws/nx-plugin/sdk/utils/test` pour créer un système de fichiers virtuel. Il initialise les marqueurs d'espace de travail que notre preset écrit (`tsconfig.base.json`, `pnpm-workspace.yaml`, `biome.json`, un `package.json` racine avec `type: module`), dont tout générateur composant un générateur `@aws/nx-plugin` a besoin
