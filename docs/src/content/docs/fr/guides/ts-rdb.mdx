...
<FileTree>
  ...
  - scripts
    - pull-image.ts Télécharge l'image de conteneur de la base de données pour le développement local
...
La base de données déployée a l'architecture suivante. Par défaut, un [Amazon RDS Proxy](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/rds-proxy.html) se situe devant le cluster Aurora pour mutualiser les connexions et activer [l'authentification IAM](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.html) — voir [Désactiver RDS Proxy](#désactiver-rds-proxy) pour l'alternative. L'architecture est la même que vous sélectionniez le moteur PostgreSQL ou MySQL ; seule la variante du moteur Aurora diffère.
...
Arrêter `serve-local` (par exemple avec `Ctrl+C`) supprime automatiquement le conteneur de base de données local, mais préserve le volume nommé afin que vos données persistent entre les redémarrages.

:::caution[Windows]
En raison des limitations de gestion des signaux sur Windows, le conteneur n'est pas automatiquement supprimé lorsque `serve-local` est arrêté. Vous devrez le supprimer manuellement :
...
Pour des types de projets spécifiques, utilisez le générateur `connection` pour connecter automatiquement la base de données — il gère les imports, l'injection de contexte et les dépendances de développement local :

- <Link path="guides/connection/trpc-rdb">API tRPC → Base de données relationnelle</Link>
- <Link path="guides/connection/smithy-rdb">API Smithy → Base de données relationnelle</Link>
- <Link path="guides/connection/ts-agent-rdb">Agent TypeScript Strands → Base de données relationnelle</Link>
- <Link path="guides/connection/ts-mcp-server-rdb">Serveur MCP → Base de données relationnelle</Link>
...
Par défaut, l'image de conteneur de base de données locale générée correspond à la version du moteur Aurora par défaut. Si vous modifiez la version du moteur Aurora, il est recommandé d'utiliser également une version d'image de conteneur locale correspondante pour une compatibilité maximale. Consultez les notes de version AWS pour les [versions Aurora PostgreSQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraPostgreSQLReleaseNotes/aurorapostgresql-release-calendar.html) et les [versions Aurora MySQL](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraMySQLReleaseNotes/AuroraMySQL.Updates.30Updates.html) pour identifier la version de base de données communautaire correspondante.

L'image de base de données locale est configurée dans la cible `serve-local` du projet de base de données généré dans `project.json`. Mettez à jour l'argument d'image transmis à `scripts/start-container.ts` lorsque vous modifiez les versions du moteur.
...
Terraform n'utilise pas les politiques de suppression CDK. Par défaut, le module crée un instantané final lors de la suppression (`skip_final_snapshot = false`). Pour ignorer l'instantané final pour un environnement éphémère :
