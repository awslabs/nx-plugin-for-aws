...
L'image de base de données locale est configurée dans le champ `serveLocal.image` du fichier `config.json` généré à la racine de votre projet de base de données. Mettez à jour cette valeur lorsque vous modifiez les versions du moteur.
...
<CardGrid>
  ...
  />
  <ConnectionCard
    title="Agent TypeScript vers base de données relationnelle"
    description="Connecter un Agent TypeScript à une base de données relationnelle Aurora"
