...
L'image de base de données locale est configurée dans le champ `localDev.image` du fichier `config.json` généré à la racine de votre projet de base de données. Mettez à jour cette valeur lorsque vous modifiez les versions du moteur.
