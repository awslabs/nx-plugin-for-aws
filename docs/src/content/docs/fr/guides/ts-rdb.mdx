...
Le cluster Aurora exporte les journaux de base de données vers CloudWatch Logs (`postgresql` pour PostgreSQL, `error` et `slowquery` pour MySQL) et active Performance Insights sur l'instance d'écriture (chiffrée avec la clé KMS du cluster) afin que les requêtes soient observables pour le dépannage et l'audit. Notez que CloudWatch Logs et Performance Insights entraînent des coûts supplémentaires.
