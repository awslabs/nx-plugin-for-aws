...
Vous avez sélectionné `Any` pour votre `eventSource`, donc le décorateur `@event_parser` n'est pas appliqué et le gestionnaire reçoit un dictionnaire d'événement non typé. Régénérez avec un `eventSource` spécifique si vous souhaitez la validation Pydantic et l'accès aux champs typés.
</OptionFilter>
