...
Si vous avez choisi un `event` lors de la génération, votre fonction est instrumentée avec [`@event_parser` d'AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/). Exemple :
...
Si vous avez sélectionné `Any` pour votre `event`, le décorateur `@event_parser` n'est pas appliqué et le gestionnaire reçoit un dictionnaire d'événement non typé. Régénérez avec un `event` spécifique si vous souhaitez la validation Pydantic et l'accès aux champs typés.
</OptionFilter>
...
2. [`uv pip install`](https://docs.astral.sh/uv/reference/cli/#uv-pip-install) pour installer les dépendances pour la plateforme cible (`x86_64-manylinux_2_28`)

:::note[Changements d'architecture]
Si vous utilisez une [architecture ARM](https://docs.aws.amazon.com/lambda/latest/dg/foundation-arch.html), modifiez la cible de bundling en `aarch64-manylinux_2_28`.
:::
