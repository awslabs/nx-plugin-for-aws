...
Le plugin AWS pour Nx fournit un générateur pour intégrer rapidement votre <Link path="guides/trpc">API tRPC</Link> avec un site React. Il configure tous les éléments nécessaires pour connecter vos backends tRPC, incluant le support d'authentification AWS IAM/Cognito et une gestion appropriée des erreurs. Cette intégration offre une sécurité de typage de bout en bout entre votre frontend et vos backends tRPC.
...
3. Une API tRPC fonctionnelle (générée avec le générateur d'API tRPC)
4. Une authentification Cognito ajoutée via le <Link path="/guides/react-website-auth">générateur `ts#react-website-auth`</Link> si vous connectez une API utilisant Cognito ou l'authentification IAM
...
Utiliser les mises à jour optimistes pour améliorer l'expérience utilisateur :
...
L'intégration fournit une sécurité de typage complète de bout en bout. Votre IDE offrira une autocomplétion complète et une vérification des types pour tous vos appels d'API :
...
Les types sont automatiquement inférés depuis les définitions de votre routeur backend, garantissant que toute modification de l'API se reflète immédiatement dans le code frontend sans nécessiter de build.
