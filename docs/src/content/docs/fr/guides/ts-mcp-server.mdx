...
Générez un serveur [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) en TypeScript pour fournir du contexte aux Grands Modèles de Langage (LLMs), avec option de déploiement sur [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
:::note
Sauf si vous avez sélectionné `None` pour `computeType`, le générateur créera également un construct CDK pour déployer votre serveur MCP.
:::
...
Ceci démarrera l'inspector sur `http://localhost:6274`. Commencez en cliquant sur le bouton "Connect".
