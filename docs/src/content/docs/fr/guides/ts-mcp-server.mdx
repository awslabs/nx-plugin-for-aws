...
Générez un serveur TypeScript [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) pour fournir du contexte aux Grands Modèles de Langage (LLM), avec option de déploiement sur [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
:::tip
Utilisez d'abord le générateur <Link path="/guides/typescript-project">`ts#project`</Link> pour créer un projet avant d'y ajouter votre serveur MCP.
:::
  ...
  - your-project/
    - src/
      - mcp-server/ (ou nom personnalisé si spécifié)
        ...
        - Dockerfile Point d'entrée pour héberger votre serveur MCP (quand `computeType` est défini sur `BedrockAgentCoreRuntime`)
...
server.tool("toolName", "tool description",
  { param1: z.string(), param2: z.number() }, // Schéma d'entrée avec Zod
  async ({ param1, param2 }) => {
...
- Construit une image Docker depuis le `Dockerfile` exécutant ce serveur sur le port `8000`, conformément au [contrat de protocole MCP](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html#mcp-protocol-contract)

:::tip
L'image Docker utilise un tag (ex: `my-scope-my-project-mcp-server:latest`) que le construct CDK hérite, permettant de co-localiser votre `Dockerfile` avec votre projet MCP.
:::

### Observabilité

Votre serveur MCP est automatiquement configuré avec de l'observabilité via [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT), grâce à l'[auto-instrumentation](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) du SDK JavaScript dans votre `Dockerfile`.

Les traces sont disponibles dans la console CloudWatch via le menu "GenAI Observability". Activez [Transaction Search](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Transaction-Search.html) pour les visualiser.

Pour plus de détails, consultez la [documentation d'observabilité d'AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability-configure.html).