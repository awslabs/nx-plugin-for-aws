...
Le générateur configure une cible nommée `<your-server-name>-inspect`, qui démarre votre serveur MCP localement (via la cible `<your-server-name>-serve-local`, incluant toutes les dépendances connectées telles qu'une base de données locale) et lance l'[MCP Inspector](https://github.com/modelcontextprotocol/inspector) préconfiguré pour s'y connecter via le transport HTTP Streamable.
...
<CardGrid>
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'fr'}/guides/connection/ts-agent-mcp`}
    source="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'fr'}/guides/connection/ts-mcp-server-rdb`}
    source="mcp"
    target="aurora"
  />
  <ConnectionCard
  ...
  <ConnectionCard
    ...
    description="Agréger un serveur MCP derrière une passerelle AgentCore"
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'fr'}/guides/connection/agentcore-gateway-mcp`}
    source="agentcore"
    target="mcp"
  />
</CardGrid>
