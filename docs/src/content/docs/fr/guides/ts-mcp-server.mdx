...
:::tip
Commencez par utiliser le générateur <Link path="/guides/typescript-project">`ts#project`</Link> pour créer un projet auquel ajouter votre serveur MCP.
:::
...
:::caution
Le SDK MCP ne prend actuellement en charge que Zod v3. Pour permettre l'utilisation de Zod v4 dans d'autres générateurs, la dépendance sur Zod v3 est installée sous `zod-v3`. Pour les schémas d'outils MCP, vous devrez importer Zod comme suit :
