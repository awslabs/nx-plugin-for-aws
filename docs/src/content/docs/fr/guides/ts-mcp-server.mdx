...
Générez un serveur TypeScript [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) pour fournir du contexte aux modèles de langage de grande taille (LLM), avec option de déploiement sur [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
- Exécuter des outils (fonctions) effectuant des actions ou récupérant des informations
...
Vous pouvez générer un serveur MCP TypeScript de deux façons :
  ...
  - your-project/
    - src/
      - mcp-server/ (ou nom personnalisé si spécifié)
        ...
        - stdio.ts Point d'entrée pour le transport STDIO, utile pour les serveurs MCP locaux simples
...
Une fois [ce problème avec le SDK MCP](https://github.com/modelcontextprotocol/typescript-sdk/issues/164) résolu, vous pourrez supprimer la dépendance supplémentaire `zod-v3` et importer directement depuis `zod`.
:::
...
Pour compiler votre serveur MCP pour Bedrock AgentCore Runtime, une cible `<your-mcp-server>-bundle` est ajoutée à votre projet. Elle :

- Bundle votre serveur MCP en un seul fichier JavaScript avec [`esbuild`](https://esbuild.github.io/), utilisant `http.ts` comme point d'entrée pour un serveur MCP HTTP streamable
- Construit une image Docker depuis le `Dockerfile` qui exécute ce serveur bundle sur le port `8000`, conformément au [contrat de protocole MCP](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html#mcp-protocol-contract)

:::tip
L'image Docker est construite avec un tag (par exemple `my-scope-my-project-mcp-server:latest`), que le `Dockerfile` du construct CDK hérite, permettant à votre `Dockerfile` d'être colocalisé avec votre projet de serveur MCP.
:::
