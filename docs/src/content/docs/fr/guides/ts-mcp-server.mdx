...
### Portée des données d'outil par appelant

`createServer` reçoit un `McpRequestContext` décrivant la requête actuelle. Via HTTP Streamable, cela inclut l'identifiant de session AgentCore Runtime de l'appelant, qu'un <Link path="guides/ts-agent">agent</Link> transmet automatiquement à chaque appel MCP sortant.

Transmettez-le à un outil qui stocke des données par appelant :
...
:::caution
Délimitez les données stockées sur le contexte de requête plutôt que sur un argument d'outil. Un modèle peut être persuadé de passer n'importe quelle valeur, donc les arguments tels qu'un nom d'utilisateur ou de joueur ne constituent pas une limite de confiance. Le contexte est absent via STDIO, où il n'y a pas de requête.
:::
