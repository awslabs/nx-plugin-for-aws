...
<FileTree>
  - packages/\<nom>/
    ...
    - project.json Configuration de projet Nx avec les cibles de build, bundle et dev
    - src/
      - index.ts Point d'entrée du serveur MCP
      - server.ts Définition principale du serveur, incluant les outils et ressources
...
:::note
Reportez-vous à la <Link path="/guides/typescript-project">documentation du générateur de projet TypeScript</Link> pour plus de détails sur le résultat du générateur.
:::
...
Consultez les documentations suivantes pour configurer MCP avec des assistants IA spécifiques :
...
La tâche `bundle` utilise [esbuild](https://esbuild.github.io/) pour créer un fichier JavaScript unique utilisable avec les assistants IA :
...
Particulièrement utile pendant le développement, cela garantit que votre assistant IA utilise toujours la dernière version de votre serveur MCP.
