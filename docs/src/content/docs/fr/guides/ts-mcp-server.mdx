...
Vous avez sélectionné `None` pour `computeType`, donc aucun construct CDK ni module Terraform n'est généré — le serveur MCP est configuré uniquement pour une utilisation locale STDIO / HTTP.
</OptionFilter>
