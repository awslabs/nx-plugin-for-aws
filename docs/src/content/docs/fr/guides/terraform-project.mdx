...
[Terraform](https://www.terraform.io/) est un outil open source d'infrastructure en tant que code qui permet de créer, modifier et améliorer une infrastructure de manière sûre et prévisible.
...
<FileTree>
  ...
  - bootstrap Configuration bootstrap pour l'état distant
    ...
    - variables.tf Définition des variables bootstrap
...
:::tip
Les projets d'application incluent des capacités de déploiement complet avec gestion d'état distant, tandis que les projets bibliothèque sont conçus pour créer des modules Terraform réutilisables consommables par d'autres projets.
:::
...
Pour ajouter de nouveaux environnements, créez un nouveau fichier `src/env/<environment>.tfvars` avec les variables spécifiques et ajoutez de nouvelles entrées pour `apply, destroy, init, plan` dans le `project.json`. Par exemple, pour ajouter un environnement `prod` :
...
La cible bootstrap est uniquement disponible pour les projets de type application. Les projets bibliothèque ne nécessitent pas de gestion d'état distant car ils sont conçus comme modules réutilisables.
:::
...
Vous trouverez les résultats des tests de sécurité dans le dossier racine `dist`, sous `dist/packages/<my-terraform-project>/checkov`.
