...
[Terraform](https://www.terraform.io/) est un outil open source d'infrastructure as code qui vous permet de créer, modifier et améliorer votre infrastructure de manière sécurisée et prévisible.
...
Pour les projets de librairie (`--type=library`), le générateur crée une structure simplifiée pour des modules Terraform réutilisables :
...
:::tip
Les projets d'application incluent des capacités de déploiement complet avec gestion d'état distant, tandis que les projets de librairie sont conçus pour créer des modules Terraform réutilisables consommables par d'autres projets.
:::
...
Pour exécuter un module d'un projet séparé (librairie), vous pouvez procéder ainsi :
...
:::tip
Par défaut, toutes les cibles supposent l'environnement `dev`, mais vous pouvez cibler un environnement spécifique comme suit : <NxCommands commands={['run tf-infra:apply --configuration=prod']} />
...
La cible bootstrap est uniquement disponible pour les projets de type application. Les projets librairie ne nécessitent pas de gestion d'état distant car ils sont conçus comme des modules réutilisables.
:::
...
### Cibles communes (Application et Librairie)
...
Avant d'appliquer des changements, visualisez les modifications avec la cible `plan` :
