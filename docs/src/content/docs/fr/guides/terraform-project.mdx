...
Un simple `aws_s3_bucket` échoue à l'analyse [Checkov](#tests-de-sécurité) que `build` exécute, donc l'exemple déclare les ressources de versioning, de chiffrement et de blocage d'accès public que l'analyse attend, et supprime les vérifications restantes par ressource avec des commentaires `#checkov:skip` indiquant pourquoi. Examinez chaque suppression par rapport à vos propres exigences — voir [Tests de sécurité](#tests-de-sécurité) pour les deux mécanismes de suppression.
...
:::note[Noms de variables]
Chaque nom que vous définissez ici doit être déclaré dans `src/variables.tf` — le scaffold déclare `aws_region` et `environment`. Terraform ne fait qu'avertir d'une valeur pour une variable non déclarée, donc un nom mal orthographié est silencieusement ignoré plutôt qu'appliqué.
:::
</TabItem>
...
Comme `test`, cela exécute d'abord `terraform init -backend=false`, donc il installe les modules et les fournisseurs dont la validation a besoin sans configurer le backend S3. Il s'exécute donc sur un projet frais, avant <Link path="guides/terraform-project#bootstrap-de-létat-distant-projets-dapplication-uniquement">`bootstrap`</Link> et sans credentials AWS, tout comme `lint`, `test` et `checkov`.

#### Linting

Les projets Terraform utilisent [`terraform fmt`](https://developer.hashicorp.com/terraform/cli/commands/fmt) pour vérifier le formatage.
...
Chaque cible qui exécute `terraform init` réutilise un cache de fournisseurs sous `.terraform/plugin-cache` dans la racine de votre espace de travail, de sorte que les fournisseurs sont téléchargés une seule fois plutôt qu'à chaque exécution. Chaque projet obtient son propre répertoire là-bas : deux exécutions de `terraform init` remplissant un cache en même temps peuvent chacune calculer un hash différent pour le même fournisseur, que terraform rejette ensuite contre votre `.terraform.lock.hcl`. Consultez la [documentation Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) pour plus d'informations.

Définissez `TF_PLUGIN_CACHE_DIR` dans votre environnement pour pointer le script `init` fourni vers un cache que vous gérez vous-même — un volume partagé entre les espaces de travail, par exemple. Cela n'atteint que les cibles qui passent par le script, qui est le `init` d'un projet d'application. Toutes les autres cibles déclarent le chemin comme une entrée `env` dans `project.json`, et le bloc `env` de Nx l'emporte sur l'environnement ambiant, donc modifiez-le là aussi — sur `test` et `validate` pour l'un ou l'autre type de projet, et sur `init` également pour un projet de bibliothèque.
