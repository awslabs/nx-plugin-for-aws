...
Chaque nom que vous définissez ici doit être déclaré dans `src/variables.tf` — le générateur déclare `aws_region` et `environment`.
:::
</TabItem>
...
Chaque cible qui exécute `terraform init` réutilise un cache de fournisseurs sous `.terraform/plugin-cache` dans la racine de votre espace de travail, de sorte que les fournisseurs sont téléchargés une seule fois plutôt qu'à chaque exécution. Chaque projet obtient son propre répertoire là-bas. Consultez la [documentation Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) pour plus d'informations.

La cible `install-providers` est ce qui remplit ce cache, et chaque cible qui exécute `terraform init` en dépend :

<NxCommands commands={['install-providers tf-infra']} />

Vous ne l'exécuterez normalement pas vous-même. Elle existe parce que deux exécutions de `terraform init` remplissant un cache en même temps peuvent chacune calculer un hash différent pour le même fournisseur, que terraform rejette ensuite contre votre `.terraform.lock.hcl` avec "the cached package ... does not match any of the checksums recorded in the dependency lock file". Télécharger les fournisseurs une fois, avant les cibles qui en ont besoin, laisse ces cibles lire uniquement à partir d'un cache qui est déjà chaud.

Définissez `TF_PLUGIN_CACHE_DIR` dans votre environnement pour pointer le script `init` fourni vers un cache que vous gérez vous-même — un volume partagé entre les espaces de travail, par exemple. Notez que les cibles `install-providers`, `test` et `validate` lisent leur chemin depuis `project.json`, donc modifiez-le là aussi.
