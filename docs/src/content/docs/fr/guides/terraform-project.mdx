...
Checkov s'exécute dans le cadre de `build` et `assemble`, donc un `plan` interactif analyse votre infrastructure avant de l'appliquer.
...
`plan` dépend de `assemble` plutôt que de `build`, donc il produit les artefacts que vos modules référencent — les bundles Lambda et les métadonnées d'opérations générées — et exécute Checkov, sans également exécuter les portes de lint, test et vérification de type de chaque projet.

:::caution
Parce que `plan` n'exécute plus les tests de modules Terraform ou les tests unitaires de vos projets, exécutez `build` pour vérifier votre code avant d'appliquer quoi que ce soit qui vous importe, et conditionnez votre pipeline à cela :
