...
La cible `fmt` vérifie que votre code Terraform est formaté, et échoue si ce n'est pas le cas. Elle s'exécute dans le cadre de `build` :

<NxCommands commands={['fmt tf-infra']} />

Pour reformater votre code, exécutez avec la configuration `fix` :

<NxCommands commands={['fmt tf-infra --configuration=fix']} />

Une cible `lint` orchestre la vérification du format, de sorte que `run-many` atteint vos projets Terraform aux côtés de vos projets TypeScript et Python :

<NxCommands commands={['run-many --target lint --all --configuration=fix']} />

:::tip[Pourquoi la vérification est la valeur par défaut]
`terraform fmt` réécrit les fichiers qu'il lit — les fichiers mêmes que la cible `fmt` déclare comme ses entrées de cache. Écrire depuis la cible de base changerait sa propre clé de cache à chaque exécution, de sorte qu'elle ne pourrait jamais enregistrer un succès de cache. Vérifier à la place garde la cible cacheable, reflétant le fonctionnement des cibles `format` TypeScript et Python.
:::

Pour ignorer le formatage et le linting pendant le développement, utilisez la configuration `skip-lint` :
