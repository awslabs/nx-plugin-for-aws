...
<FileTree>
  ...
  - bootstrap Configuration du backend pour l'état distant
...
Le générateur crée également une bibliothèque `packages/common/terraform` qui ajoute initialement un module de métriques (crée une stack CloudFormation vide avec métadonnées) instrumenté automatiquement pour permettre à l'équipe `nx-plugin-for-aws` de suivre les métriques d'utilisation.
:::
</OptionFilter>
...
Les projets d'application incluent des capacités de déploiement complet avec gestion d'état distant, tandis que les projets de bibliothèque sont conçus pour créer des modules Terraform réutilisables consommables par d'autres projets.
:::
...
### Bucket d'assets pour les modules basés sur Lambda

<Snippet name="terraform/asset-bucket" parentHeading="Asset Bucket" />

### AppConfig de configuration d'exécution partagée

<Snippet name="terraform/runtime-config-appconfig" parentHeading="Runtime Configuration AppConfig" />
...
Par défaut, toutes les cibles utilisent l'environnement `dev`, mais vous pouvez cibler un environnement spécifique comme suit : <NxCommands commands={['apply tf-infra --configuration=prod']} />

Vous pouvez aussi changer d'environnement à tout moment car l'état segmente automatiquement le tfstate entre les environnements.
:::

<OptionFilter when={{ type: 'application' }} description="Remote state bootstrap is application-only">
### Bootstrap du backend distant (Projets application uniquement)

Avant de déployer votre infrastructure, vous devez initialiser le backend distant. Cela crée un bucket S3 pour stocker les fichiers d'état Terraform :

<NxCommands commands={['bootstrap tf-infra']} />

:::note[Configuration du profil AWS]
Assurez-vous que votre profil AWS CLI est configuré pour le compte/région où vous souhaitez déployer les ressources de bootstrap.
...
Exécutez des contrôles de sécurité sur votre infrastructure avec Checkov via la cible `test` :
...
Avant d'appliquer les changements, visualisez les modifications avec la cible `plan` :
...
Cette commande applique le plan créé par la cible `plan`. Pensez à exécuter `plan` d'abord pour examiner les changements.
:::

:::note[Alias Deploy]
Une cible `deploy` est également disponible comme alias pour `apply`. Vous pouvez utiliser indifféremment <NxCommands commands={['deploy tf-infra']} /> ou <NxCommands commands={['apply tf-infra']} />.
:::
...
:::caution[Opération destructrice]
Cela supprimera définitivement toutes les ressources gérées par cette configuration Terraform. À utiliser avec prudence !
:::

#### Destruction des ressources de bootstrap

Pour nettoyer les ressources de bootstrap (bucket S3 de stockage d'état) :
