...
Chaque cible qui exécute `terraform init` partage un cache de fournisseurs à `.terraform/plugin-cache` dans la racine de votre espace de travail. Consultez la [documentation Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) pour plus d'informations.
