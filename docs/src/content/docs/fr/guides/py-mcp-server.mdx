...
Une cible `docker` spécifique à votre serveur MCP est également ajoutée, qui copie le `Dockerfile` et les artefacts assemblés dans un répertoire de contexte Docker. Cela colocalise le `Dockerfile` avec la sortie de construction, permettant à CDK de construire l'image Docker directement en utilisant `AgentRuntimeArtifact.fromAsset`.
