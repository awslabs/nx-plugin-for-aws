...
:::tip
Pour exécuter votre serveur MCP (et toutes les dépendances connectées) sans l'inspecteur, utilisez la cible `dev` du projet. Si vous avez généré plusieurs composants dans le même projet, exécutez un seul serveur MCP seul avec sa cible `<your-server-name>-dev`.
:::
