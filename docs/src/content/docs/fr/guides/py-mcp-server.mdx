...
Générez un serveur [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) en Python pour fournir du contexte aux Grands Modèles de Langage (LLMs), avec option de déploiement sur [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
:::note
Sauf si vous avez sélectionné `None` pour `computeType`, le générateur créera également l'infrastructure CDK ou Terraform pour déployer votre serveur MCP.
:::
...
- La validation des types basée sur les annotations de type
...
Le moyen le plus simple de tester et utiliser un serveur MCP est d'utiliser l'inspector ou de le configurer avec un assistant IA (comme ci-dessus).
...
:::tip
L'image Docker est construite avec un tag (exemple `my-scope-my-project-mcp-server:latest`), référencé par votre infrastructure CDK ou Terraform, permettant de co-localiser votre `Dockerfile` avec votre projet de serveur MCP.
:::
