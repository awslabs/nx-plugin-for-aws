  ...
  - your-project/
    - your_module/
      - mcp_server/ (ou nom personnalisé si spécifié)
        ...
        - Dockerfile Point d'entrée pour l'hébergement du serveur MCP (exclu si `infra` est `None`)
...
:::tip
Pour inspecter le serveur en utilisant le transport STDIO à la place, utilisez la cible `<your-server-name>-inspect-stdio`, qui lance l'inspector contre une instance STDIO de votre serveur.
:::
...
Une cible `docker` spécifique à votre serveur MCP est également ajoutée, qui copie le `Dockerfile` et les artefacts assemblés dans un répertoire de contexte Docker. Cela colocalise le `Dockerfile` avec la sortie de build, permettant à CDK de construire l'image Docker directement en utilisant `AgentRuntimeArtifact.fromAsset`.

### Observabilité

<Snippet name="mcp/observability" parentHeading="Observabilité" />
</OptionFilter>
entHeading="Observabilité" />
</OptionFilter>
