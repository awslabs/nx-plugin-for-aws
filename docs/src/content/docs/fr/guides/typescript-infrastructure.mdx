...
Si vous générez avec `--stageConfig=true`, le générateur crée également deux packages partagés pour la gestion centralisée des identifiants (s'ils n'existent pas déjà) :
...
Cette section s'applique lorsque vous générez avec `stageConfig` :

<NxCommands commands={['g @aws/nx-plugin:ts#infra --name=infra --stageConfig=true']} />

Sans cela, le générateur produit une configuration plus simple où vous gérez vous-même les identifiants AWS (par exemple, en exportant `AWS_PROFILE` avant le déploiement).
:::
...
| `deploy`         | Déployer n'importe quel stage, en nommant le stage ou les stacks que vous voulez. Utilise le mode express sauf si vous générez avec `stageConfig`. |
...
La cible `deploy-sandbox` utilise le [mode express CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) par défaut (via le flag `--express` de CDK). Le mode express se termine une fois que la configuration des ressources est confirmée appliquée plutôt que d'attendre la stabilisation complète, avec la propagation se poursuivant en arrière-plan, pour des cycles d'itération nettement plus rapides.

Puisqu'il retourne avant que les ressources aient fini de se stabiliser, le mode express convient au stage sandbox sur lequel vous itérez, pas aux déploiements CI/CD ou en production — utilisez [`deploy-ci`](#déployer-vers-aws-dans-un-pipeline-cicd) pour ceux-ci. Pour attendre la stabilisation complète des ressources ici aussi, passez `--no-express` :
...
Le fait que cette cible utilise le mode express dépend de la façon dont vous avez généré le projet :

- **Avec `stageConfig`** elle attend la stabilisation complète des ressources, puisque la cible peut nommer n'importe quel stage, y compris votre stage de production. Passez `--express` pour opter pour le mode express pour un stage sur lequel vous itérez.
- **Sans `stageConfig`** elle utilise le mode express, comme le fait `deploy-sandbox`. Puisque le stage que vous nommez peut être un stage de production, passez `--no-express` pour attendre la stabilisation complète des ressources :

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Utilisez [`deploy-ci`](#déployer-vers-aws-dans-un-pipeline-cicd) depuis un pipeline à la place — il n'utilise jamais le mode express.
