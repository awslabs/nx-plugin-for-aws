...
Le stage sandbox est celui sur lequel agissent les cibles <Link path="guides/typescript-infrastructure#deploying-your-sandbox-stage">`deploy-sandbox`</Link> et <Link path="guides/typescript-infrastructure#tearing-down-your-sandbox-stage">`destroy-sandbox`</Link>.
...
| `deploy`         | Déployer n'importe quel stage, en nommant le stage ou les stacks que vous voulez. Attend la stabilisation complète. |
...
Puisque cette cible peut nommer n'importe quel stage, y compris votre stage de production, elle attend la stabilisation complète des ressources. Passez `--express` pour opter pour le mode express pour un stage sur lequel vous itérez.
...
Cette cible déploie un assemblage cloud pré-synthétisé plutôt que de synthétiser à la volée, évitant le non-déterminisme potentiel des changements de version de package et garantissant que chaque étape du pipeline déploie en utilisant le même assemblage cloud. Comme `deploy`, elle attend que chaque ressource se stabilise complètement avant de signaler le succès, et contrairement à `deploy` elle n'accepte aucune option `--express`.
