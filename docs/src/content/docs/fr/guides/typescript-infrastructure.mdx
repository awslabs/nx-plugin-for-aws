...
Vous pouvez ajouter d'autres stages, par exemple pour définir des stages `beta` et `prod` déployés dans des environnements distincts :

```ts title="src/main.ts"
new ApplicationStage(app, 'project-beta', {
  env: {
    account: '123456789012', // compte bêta
    region: 'us-west-2',
  },
});
...
Un Stage définit une collection de stacks à déployer ensemble pour composer votre application. Vous pouvez instancier autant de stacks que vous le souhaitez dans un stage, par exemple :
...
:::warning
Il est important de déclarer le site web _après_ les constructs d'API afin que la <Link path="guides/react-website#runtime-configuration">Configuration Runtime</Link> du site inclue toutes les configurations d'API.
:::

## Synthèse de votre infrastructure

Dans le cadre de votre cible `build`, en plus des <Link path="guides/typescript-project#building">cibles par défaut de compilation, lint et tests</Link>, votre projet d'infrastructure est _synthétisé_ en CloudFormation. Cela peut aussi être exécuté séparément via la cible `synth` :
...
Une cible `checkov` est ajoutée à votre projet pour exécuter des vérifications de sécurité sur votre infrastructure avec [Checkov](https://www.checkov.io/).

<NxCommands commands={['run <my-infra>:checkov']} />

Vous trouverez les résultats des tests de sécurité dans le dossier `dist` racine, sous `dist/packages/<my-infra-project>/checkov`.

:::note
Checkov est exécuté avec `uvx`, vous devez donc [installer `uv` si ce n'est pas déjà fait](https://docs.astral.sh/uv/getting-started/installation/).
:::
...
// Supprime CKV_AWS_XXX pour le construct ou ses descendants si c'est une instance de Bucket
...
Exécutez ensuite la cible deploy :
...
Cette cible diffère légèrement de `deploy` en garantissant le déploiement d'une cloud-assembly pré-synthétisée, plutôt qu'une synthèse à la volée. Cela évite les problèmes de non-déterminisme liés aux versions de packages, assurant que chaque étape du pipeline déploie la même cloud-assembly.
