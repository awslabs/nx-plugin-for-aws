...
`deploy-sandbox` et `deploy` dépendent de `^package`, donc ils construisent les artefacts qu'ils sont sur le point de déployer et rien d'autre. `deploy-ci` déploie un assemblage que votre pipeline a déjà construit, donc il n'a aucune dépendance de build.

:::caution
Parce que les cibles de déploiement ignorent les contrôles de lint, test et vérification de type, le déploiement n'échouera pas sur une erreur de lint ou un test unitaire échouant. Exécutez `build` avant de déployer quoi que ce soit qui vous importe, et conditionnez votre pipeline à cela :
