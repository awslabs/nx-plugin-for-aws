...
:::caution[Utilisez toujours `deploy-ci` pour la production]
`deploy` et `deploy-sandbox` utilisent le mode express et retournent avant que les ressources aient fini de se stabiliser, ce qui les rend inadaptés aux déploiements CI/CD et en production. Utilisez `deploy-ci` pour ceux-ci.
:::
