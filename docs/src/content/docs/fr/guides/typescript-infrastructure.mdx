...
`synth` dépend de `^assemble` plutôt que de `^build`, donc la synthèse construit les artefacts que vos stacks déploient — les bundles Lambda, les images de conteneur et les spécifications générées — sans exécuter également les contrôles de lint, test et vérification de type de chaque projet en amont. Consultez <Link path="guides/typescript-project#assembling">Assembling</Link> pour savoir comment `assemble` se rapporte à `build`.
...
:::caution
Parce que les cibles de déploiement ignorent les contrôles de lint, test et vérification de type, le déploiement n'échouera pas sur une erreur de lint ou un test unitaire échoué. Exécutez `build` avant de déployer quoi que ce soit qui vous importe, et conditionnez votre pipeline à cela :
...
Le mode express se termine une fois que la configuration des ressources est confirmée appliquée plutôt que d'attendre la stabilisation complète, avec la propagation se poursuivant en arrière-plan, pour des cycles d'itération nettement plus rapides. Il est destiné au développement et à l'itération.
:::
