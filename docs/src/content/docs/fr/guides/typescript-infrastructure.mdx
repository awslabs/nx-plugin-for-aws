...
:::tip[Commiter la config d'étape]
...
La cible `deploy` utilise le [mode express CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) par défaut (via le flag `--express` de CDK) pour accélérer les déploiements pendant le développement. Le mode express se termine une fois que la configuration des ressources est confirmée comme appliquée plutôt que d'attendre la stabilisation complète, la propagation se poursuivant en arrière-plan, pour des cycles d'itération nettement plus rapides.

Le mode express est destiné au développement et à l'itération. Pour déployer sans celui-ci et attendre la stabilisation complète des ressources, supprimez le flag `--express` de la commande de la cible `deploy` dans le `project.json` de votre projet d'infrastructure.

Pour les déploiements réels vers les environnements de production, utilisez la cible `deploy-ci` (voir [ci-dessous](#deploying-to-aws-in-a-cicd-pipeline)) dans le cadre d'un pipeline CI/CD. Cette cible n'utilise pas le mode express, elle attend donc toujours la stabilisation complète.
:::
