...
Si vous avez besoin d'un déploiement standard qui attend la stabilisation complète des ressources, désactivez-le avec `--no-express` :

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

La cible `deploy-ci` n'utilise pas le mode express, donc les déploiements de pipeline CI/CD attendent toujours la stabilisation complète.
:::
