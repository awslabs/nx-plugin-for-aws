...
<FileTree>
  - \<name>
    - \_\_init\_\_.py Exports du package (`get_engine`, `session_context`, `get_session`)
...
Importez `session_context` depuis votre package de base de données et utilisez-le comme gestionnaire de contexte pour obtenir une `Session` SQLModel :
...
Lors de la connexion directe au cluster Aurora (sans RDS Proxy), passez le chemin du bundle CA Amazon RDS en utilisant le paramètre optionnel `rds_ca` :
...
La valeur `rds_ca` doit pointer vers un bundle de certificats CA disponible dans le système de fichiers de votre environnement d'exécution. Pour Amazon RDS, utilisez le bundle CA global depuis :

```text
https://truststore.pki.rds.amazonaws.com/global/global-bundle.pem
```

La façon dont vous rendez ce fichier disponible dépend de l'environnement d'exécution qui se connecte à la base de données.

#### Images de conteneur d'exécution

Si vous préparez votre propre image de conteneur pour l'environnement d'exécution, ajoutez le bundle CA RDS dans votre Dockerfile et passez ce chemin à `session_context` :
...
Si vous déployez une fonction Lambda zippée, attachez la couche Lambda `RdsCaLayer` (CDK) ou le module Terraform `ca-layer`. La couche CDK utilise le bundling Docker pour ajouter le bundle CA, et la couche déployée le fournit à `/opt/global-bundle.pem`, que vous pouvez passer à `session_context` :
