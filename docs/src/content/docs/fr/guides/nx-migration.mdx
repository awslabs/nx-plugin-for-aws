...
- **Formater ce que vous écrivez.** `updateJson` et `writeJson` re-sérialisent un fichier entier, développant les tableaux en ligne dans une forme que la cible `format` fournie rejette — laissant l'utilisateur avec une construction échouée après une mise à niveau par ailleurs propre. Terminez avec `formatFilesInSubtree(tree)` afin que le diff reste limité aux lignes que vous vouliez modifier.
...
iques et hybrides sont appliquées par l'agent de codage local de l'utilisateur pendant `nx migrate --run-migrations` lorsqu'un agent est disponible.
