...
Les migrations sont regroupées par jour de création de leur échafaudage, de sorte que leur ordre approximatif reste visible d'un coup d'œil à mesure que la collection s'agrandit. Réexécuter le générateur pour une migration existante réutilise le jour où elle a été créée initialement plutôt que de créer un doublon sous la date du jour.
