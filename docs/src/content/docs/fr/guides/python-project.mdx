...
:::note[Noms entièrement qualifiés]
...
La cible `build` compilera, linttera, testera et vérifiera les types de votre projet.
...
Les projets Python utilisent [ty](https://docs.astral.sh/ty/) pour la vérification de types.

### Exécuter le vérificateur de types

La vérification de types s'exécute dans le cadre de la cible `build` de votre projet, mais vous pouvez aussi l'exécuter séparément via la cible `typecheck` :

<NxCommands commands={['run <project-name>:typecheck']} />

### Supprimer les erreurs de types
...
Pour configurer le comportement de la vérification de types dans votre projet, ajoutez une section `[tool.ty]` au fichier `pyproject.toml` de votre projet. Consultez la [référence de configuration ty](https://docs.astral.sh/ty/reference/configuration/) pour les options disponibles.
...
Ceci dispose d'une commande raccourcie depuis la racine de votre espace de travail :
