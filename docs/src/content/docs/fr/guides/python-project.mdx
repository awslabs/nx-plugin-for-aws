...
Votre projet dispose également d'une cible `assemble`, qui est le pendant de `build` pour les artefacts uniquement : elle produit tout ce que votre projet contribue à un déploiement (sa sortie compilée, et tout bundle, image de conteneur ou spécification générée qu'il déclare) sans exécuter les contrôles de lint, de test ou de vérification de type.

<NxCommands commands={['assemble <project-name>']} />

Les <Link path="guides/typescript-infrastructure#deploying-to-aws">cibles de déploiement</Link> dépendent de `assemble` plutôt que de `build`, de sorte que le déploiement ne construit que ce qu'il est sur le point de déployer. `build` reste inchangé et fait toujours tout, il reste donc ce que vous exécutez pour vérifier un projet — et ce que vous devriez exécuter en CI.

:::caution
Parce que `assemble` ignore les contrôles de qualité, un déploiement n'échoue plus sur une erreur de lint, un test unitaire échoué ou une erreur de type. Exécutez `build` (ou votre pipeline CI) pour vérifier votre code.
:::
