...
Votre projet dispose également d'une cible `assemble`, qui est le pendant de `build` axé uniquement sur les artefacts : elle produit tout ce que votre projet contribue à un déploiement (sa sortie compilée, et tout bundle, image de conteneur ou spécification générée qu'il déclare) sans exécuter les contrôles de lint, de test ou de vérification de type.

<NxCommands commands={['assemble <project-name>']} />

Les cibles de déploiement dépendent de `assemble` plutôt que de `build`, de sorte que le déploiement ne construit que ce qu'il est sur le point de déployer. `build` reste inchangé et fait toujours tout, il reste donc ce que vous exécutez pour vérifier un projet — et ce que vous devriez exécuter en CI.
