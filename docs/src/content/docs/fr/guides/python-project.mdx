...
L'option `type` sélectionne si vous obtenez une application (un projet déployable, par exemple une fonction lambda ou une image de conteneur) ou une bibliothèque (code réutilisable, importé par d'autres projets). Elle est définie par défaut sur `application`, donc passez `--type=library` pour une bibliothèque :
