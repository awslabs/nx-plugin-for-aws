...
Les fichiers de test sont exclus de l'entrée nommée `production` dans `nx.json`, qui couvre le répertoire `tests` ainsi que les fichiers `test_*.py`, `*_test.py` et `conftest.py` n'importe où dans votre projet.
