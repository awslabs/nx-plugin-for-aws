...
`getAppConfig` renvoie n'importe quelle valeur JSON valide, donc déclarez la forme que vous attendez et convertissez le résultat — sinon les accès aux propriétés ne compilent pas sous le `tsconfig` strict de l'espace de travail :
...
:::tip[Valider à la frontière]
Une conversion indique au compilateur la forme mais ne la vérifie pas à l'exécution. Pour une configuration que vous ne contrôlez pas, analysez-la plutôt avec un schéma [Zod](https://zod.dev/), de sorte qu'une valeur mal formée échoue bruyamment plutôt que de remonter comme `undefined` plus tard.
:::
