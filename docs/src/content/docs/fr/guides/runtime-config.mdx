...
- **`connection`** — configuration qui permet aux projets générés de se connecter entre eux :
  ...
  - **ARNs d'exécution des agents** — ajoutés par le générateur de connexion lorsque vous connectez un site web React à un Agent
...
1. **`core/runtime-config/appconfig`** — déclaré une fois, près du début du module racine. Crée l'application AppConfig, l'environnement, la stratégie de déploiement et un profil de configuration par namespace. Ses sorties `application_id` et `application_arn` sont transmises à chaque module qui lit la configuration d'exécution au moment de l'exécution (agents, serveurs MCP, fonctions Lambda, etc.).
...
2. **`core/runtime-config/entry`** — une invocation par contribution. Les modules d'API, d'agents et MCP générés appellent ceci en interne pour publier leurs URLs et ARNs. Appelez-le directement pour publier une configuration personnalisée.
...
3. **`core/runtime-config/appconfig-deployment`** — déclaré une fois, à la fin du module racine, avec `depends_on` couvrant chaque module qui contribue une entrée. Agrège les entrées contribuées en un JSON par namespace et crée la version de configuration hébergée + le déploiement vers l'application partagée.
...
<Fragment slot="terraform">
Référencez la sortie `application_id` du module partagé `runtime_config_appconfig` pour obtenir l'AppConfig Application ID, et ajoutez les déclarations de politique IAM appropriées :
...
:::note[Enregistrement automatique]
Les modules d'agents et MCP générés acceptent les entrées `appconfig_application_id` + `appconfig_application_arn` — transmettez les sorties du module partagé `runtime_config_appconfig` et ils configurent la variable d'environnement et les permissions IAM pour vous. Vous n'avez besoin de le faire manuellement que pour les fonctions Lambda ou ressources de calcul personnalisées.
:::
</Fragment>
</Infrastructure>
