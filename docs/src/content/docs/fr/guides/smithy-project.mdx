...
Sur macOS et Linux, la CLI est résolue par [mise](https://mise.jdx.dev/), que le build récupère à la demande, donc il n'y a rien à installer — il télécharge et met en cache la version épinglée la première fois que vous construisez. Sur Windows, c'est un prérequis que vous installez vous-même — voir <Link path="guides/ts-smithy-api#building-on-windows">Construction sur Windows</Link>.
