...
Les projets Smithy se construisent avec le [Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/index.html), qui valide votre modèle :

<NxCommands commands={['build my-shapes']} />

Sur macOS et Linux, le CLI est résolu par [mise](https://mise.jdx.dev/), que la construction récupère à la demande, donc il n'y a rien à installer — il télécharge et met en cache la version épinglée la première fois que vous construisez. Sur Windows, c'est un prérequis que vous installez vous-même — voir <Link path="guides/ts-smithy-api#building-on-windows">Building on Windows</Link>.
