...
La cible chat s'exécute de manière autonome — elle ne démarre **pas** l'agent pour vous. Par défaut, elle se connecte à votre agent en cours d'exécution localement, donc démarrez d'abord `<your-agent-name>-serve-local` (dans un terminal séparé) :
