...
description: Générer un TypeScript Agent pour construire des agents IA avec des outils et déployer sur Amazon Bedrock AgentCore Runtime
generator: ts#agent
---
...
Vous pouvez générer un TypeScript Agent de deux manières :
...
Le TypeScript Agent utilise [tRPC](https://trpc.io/) sur WebSocket, tirant parti du [support de streaming bidirectionnel d'AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-get-started-websocket.html) pour permettre une communication en temps réel et type-safe entre les clients et votre agent.

Puisque tRPC prend en charge les procédures Query, Mutation et Subscription sur WebSocket, vous pouvez définir autant de procédures que vous le souhaitez. Par défaut, une seule procédure de souscription nommée `invoke` est définie pour vous dans `router.ts`.
</OptionFilter>
...
La plupart des utilisateurs n'auront pas besoin de modifier ce fichier — modifiez `agent.ts` pour changer les outils ou le prompt système. Les agents A2A écoutent sur le port `9000` (contre `8080` pour HTTP), pour lequel le Dockerfile et l'infrastructure générés sont déjà configurés.
</OptionFilter>
...
La plupart des utilisateurs n'auront pas besoin de modifier `index.ts` — modifiez `agent.ts` pour changer les outils ou le prompt système. Les agents AG-UI écoutent sur le port `8080` (comme HTTP), pour lequel le Dockerfile et l'infrastructure générés sont déjà configurés.
</OptionFilter>
...
La cible de chat s'exécute de manière autonome. Par défaut, elle se connecte à votre agent exécuté localement, alors démarrez d'abord la cible `<your-agent-name>-dev` de l'agent (dans un terminal séparé) :
...
La communication de l'agent est transmise via tRPC sur WebSocket. En tant que tel, il est recommandé d'utiliser la fabrique de client type-safe générée dans `client.ts`.
...
Vous pouvez invoquer un agent exécuté localement en utilisant la méthode de fabrique `.local` de la fabrique de client.
...
Vous pouvez invoquer votre agent déployé en passant son ARN à la méthode de fabrique `withIamAuth` :
...
Utilisez la méthode de fabrique `withJwtAuth` pour vous authentifier avec le jeton d'accès JWT / Cognito.
...
Pour invoquer votre agent AG-UI depuis un site Web React, utilisez le <Link path="/guides/connection/react-agui">générateur `connection`</Link>, qui câble un client [CopilotKit](https://docs.copilotkit.ai/aws-strands) configuré pour votre agent déployé avec l'authentification correcte (IAM ou Cognito).
