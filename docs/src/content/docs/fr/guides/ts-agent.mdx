...
Si vous souhaitez que chaque utilisateur n'accède qu'à ses propres conversations, enregistrez qui possède chaque session dans votre propre application et vérifiez cette propriété à l'entrée. Un ID de session est alors quelque chose que votre application émet plutôt que quelque chose que l'appelant choisit.

**1. Créer des sessions via votre API**

Ajoutez une procédure qui démarre une conversation. Elle génère l'ID de session elle-même, résout l'appelant à partir de son propre contexte de requête, et stocke la paire — par exemple dans une table créée avec le générateur <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link> :
...
Votre agent reçoit déjà l'ID de session, et avec `auth=cognito` il reçoit également le JWT de l'appelant — l'autorisateur AgentCore Runtime le valide et l'infrastructure générée met en liste blanche l'en-tête `Authorization` jusqu'à votre code. Recherchez la session, comparez son propriétaire avec l'appelant, et refusez la requête s'ils diffèrent :
...
L'API décide qui possède une session ; l'agent l'applique. Vérifier aux deux endroits signifie qu'un ID de session qui fuit — copié depuis un navigateur, ou réutilisé d'une ancienne conversation — est toujours refusé lors de la prochaine invocation, et tout futur client de l'agent hérite de la même règle.
:::

:::caution[La session sélectionne également la microVM]
Un ID de session sélectionne la [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) qui sert la requête, donc son système de fichiers, `/tmp` et les caches en processus sont partagés par chaque requête résolvant vers cette session. Cela mérite attention si votre agent ou ses outils écrivent sur le disque local : deux appelants qui envoient le même ID de session partagent ces ressources même lorsque leurs conversations stockées sont séparées.

Générer des ID de session opaques dans votre API évite cela, car un appelant ne peut pas atterrir sur une session qui ne lui a pas été donnée. En dériver un à partir de valeurs fournies par l'utilisateur — un nom de conversation, un email, un nom de jeu — est prévisible et permet à deux appelants d'entrer en collision.
:::
