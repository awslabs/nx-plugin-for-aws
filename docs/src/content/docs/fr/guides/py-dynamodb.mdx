...
Pour des types de projets spécifiques, utilisez le générateur `connection` pour configurer automatiquement les dépendances de développement local afin que DynamoDB Local démarre automatiquement avec votre projet, et ajouter le package DynamoDB comme dépendance de workspace :
