...
Le générateur de connexion utilise la <Link path="guides/runtime-config">Configuration d'exécution</Link> pour transmettre des valeurs de déploiement (telles que les URL d'API, les paramètres Cognito et les ARN d'exécution des agents) entre les projets et composants générés au moment de l'exécution afin qu'ils puissent se découvrir et se connecter les uns aux autres.
:::