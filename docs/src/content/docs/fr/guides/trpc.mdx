...
:::tip
L'option `integrationStyle` est définie par défaut sur `Individual Functions`, qui crée une Lambda par procédure tRPC. Sélectionnez `Router` si vous préférez un seul gestionnaire Lambda partagé pour toute l'API.
:::
...
### Choix d'un style d'intégration

Le générateur tRPC supporte deux modèles d'infrastructure via l'option `integrationStyle` :

- `Individual Functions` est le paramètre par défaut. Il génère une fonction Lambda par procédure.
- `Router` génère une seule fonction Lambda partagée pour toute l'API et la réutilise pour chaque intégration.

`Individual Functions` vous offre une infrastructure plus granulaire et facilite l'ajout de permissions ou de configurations différentes par opération. `Router` réduit la prolifération de Lambdas et d'intégrations, ce qui peut être utile pour les APIs plus petites ou lorsque vous préférez un gestionnaire central unique.

Les deux styles d'intégration sont actuellement supportés pour `ServerlessApiGatewayRestApi` et `ServerlessApiGatewayHttpApi`.
