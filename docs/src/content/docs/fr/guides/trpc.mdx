...
### Choix d'un modèle d'intégration

Le générateur tRPC supporte deux modèles d'infrastructure via l'option `integrationPattern` :

- `isolated` est le paramètre par défaut. Il génère une fonction Lambda par procédure.
- `shared` génère une seule intégration Lambda `$router` par défaut pour toute l'API et la réutilise pour chaque procédure, sauf si vous fournissez des remplacements par procédure.

`isolated` vous offre une infrastructure plus granulaire et facilite l'ajout de permissions ou de configurations différentes par opération. `shared` réduit la prolifération de Lambdas et d'intégrations, tout en vous permettant de remplacer des procédures spécifiques par des intégrations dédiées si nécessaire.

Les deux modèles d'intégration sont actuellement supportés pour `ServerlessApiGatewayRestApi` et `ServerlessApiGatewayHttpApi`.
