...
Vous trouverez le point d'entrée de votre API dans `src/router.ts`. Ce fichier contient le gestionnaire Lambda qui route les requêtes vers des "procédures" basées sur l'opération invoquée. Chaque procédure définit l'entrée attendue, la sortie et l'implémentation.
...
Si vous appelez votre API depuis un site React, envisagez d'utiliser le générateur <Link path="guides/connection/react-trpc">Connexion API</Link> pour configurer le client.
