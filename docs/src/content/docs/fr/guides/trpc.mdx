...
<OptionFilter when={{ auth: 'Cognito' }} description="Supplying the identity construct for Cognito-authenticated CDK APIs">
```ts {9}
...
<OptionFilter when={{ auth: 'Cognito' }} description="Supplying Cognito config to Terraform-deployed APIs">
Pour connecter un User Pool et un client Cognito, fournissez les variables appropriées :
