...
<OptionFilter when={{ auth: ['IAM', 'Custom'] }} description="CDK usage for IAM or no authentication">
```ts {6-8}
...
<OptionFilter when={{ auth: ['IAM', 'Custom'] }} description="Terraform usage for IAM or no authentication">
```hcl {1-3, 8}
module "asset_bucket" {
...
<OptionFilter when={{ auth: 'Custom' }} description="Checkov failures you may see when auth is None">
