...
<FileTree>
  - src
    ...
    - schema Définitions de schémas utilisant Zod
...
Le générateur créera également une infrastructure CDK ou Terraform en tant que code qui peut être utilisée pour déployer votre API, résidant dans le répertoire `packages/common`.
...
Les schémas sont automatiquement validés par votre API tRPC à l'exécution, ce qui évite d'écrire manuellement une logique de validation personnalisée dans votre backend.

Zod fournit des utilitaires puissants pour combiner ou dériver des schémas comme `.merge`, `.pick`, `.omit` et plus encore. Vous pouvez trouver plus d'informations sur le [site de documentation de Zod](https://zod.dev/?id=basic-usage).

### Routeur et Procédures

Vous pouvez trouver le point d'entrée de votre API dans `src/router.ts`. Ce fichier contient le gestionnaire Lambda qui route les requêtes vers des "procédures" basées sur l'opération invoquée. Chaque procédure définit l'entrée attendue, la sortie et l'implémentation.
...
- `publicProcedure` définit une méthode publique sur l'API, incluant le middleware configuré dans `src/middleware`. Ce middleware inclut l'intégration d'AWS Lambda Powertools pour le logging, le tracing et les métriques.
...
L'utilisation de `query` pour définir l'implémentation indique que l'opération n'est pas mutable. Utilisez ceci pour définir des méthodes de récupération de données. Pour implémenter une opération mutable, utilisez plutôt la méthode `mutation`.
...
Vous pouvez regrouper des opérations ensemble en utilisant des routeurs imbriqués, par exemple :
...
:::warning
Cet exemple suppose que `auth` a été configuré à `IAM`. Pour l'authentification Cognito, le middleware d'identité est plus simple, extrayant les claims pertinents depuis l'`event`.
:::
...
Notez que nous définissons une propriété supplémentaire _optionnelle_ au contexte. tRPC gère le fait que ceci soit défini dans les procédures qui ont correctement configuré ce middleware.
...
Le générateur d'API tRPC crée une infrastructure CDK ou Terraform en tant que code basée sur votre `iacProvider` sélectionné. Vous pouvez l'utiliser pour déployer votre API tRPC.
...
Ceci configure l'infrastructure de votre API, incluant une API REST ou HTTP AWS API Gateway, des fonctions AWS Lambda pour la logique métier, et l'authentification basée sur votre méthode `auth` choisie.
...
# Attachez la politique à un rôle IAM (ex. pour les utilisateurs authentifiés)
