...
<FileTree>
  - src
    ...
    - procedures Procédures (ou opérations) exposées par votre API
      - echo.ts Procédure exemple
    - middleware
      - error.ts Middleware pour la gestion des erreurs
      - logger.ts Middleware pour configurer AWS Powertools pour les logs Lambda
...
Les schémas sont automatiquement validés par votre API tRPC à l'exécution, évitant d'écrire manuellement une logique de validation personnalisée dans le backend.

Zod fournit des utilitaires puissants pour combiner ou dériver des schémas comme `.merge`, `.pick`, `.omit` et plus. Vous trouverez plus d'informations sur le [site de documentation Zod](https://zod.dev/?id=basic-usage).
...
- `publicProcedure` définit une méthode publique sur l'API, incluant le middleware configuré dans `src/middleware`. Ce middleware inclut l'intégration d'AWS Lambda Powertools pour les logs, le tracing et les métriques.
...
L'utilisation de `query` pour définir l'implémentation indique que l'opération n'est pas mutation. Utilisez cela pour définir des méthodes de récupération de données. Pour implémenter une opération de mutation, utilisez plutôt la méthode `mutation`.
...
Si votre API grandit, vous pouvez souhaiter regrouper des opérations liées.
...
Les clients reçoivent alors ce regroupement d'opérations, par exemple invoquer l'opération `listUsers` ressemblerait à :
...
:::warning
Cet exemple suppose que `auth` est configuré sur `IAM`. Pour une authentification Cognito, le middleware d'identité est plus simple, extrayant les claims pertinents depuis l'`event`.
:::
...
Notez que nous définissons une propriété supplémentaire _optionnelle_ dans le contexte. tRPC gère le fait que cette propriété soit définie dans les procédures ayant correctement configuré ce middleware.
...
Ceci configure l'infrastructure de votre API, incluant une API Gateway REST ou HTTP AWS, des fonctions Lambda AWS pour la logique métier, et une authentification basée sur votre méthode `auth` choisie.
