...
Cet exemple présente le middleware d'identité pour l'authentification `IAM`, où nous recherchons l'appelant dans Cognito en utilisant le sub extrait de l'événement API Gateway. Pour l'authentification `Cognito`, le middleware d'identité est plus simple — extrayez les claims directement depuis l'`event`.
