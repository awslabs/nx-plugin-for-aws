...
:::tip[Type d'API]
Sélectionnez `ServerlessApiGatewayRestApi` (par défaut) comme `computeType` si vous souhaitez utiliser les [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) pour diffuser des réponses en streaming. Sélectionnez `BedrockAgentCoreRuntimeWebSocket` pour déployer le serveur tRPC en tant que conteneur sur Bedrock AgentCore avec des subscriptions WebSocket en streaming — voir [Déploiement sur Bedrock AgentCore](#deploying-to-bedrock-agentcore) ci-dessous.
:::

:::tip[Pattern d'intégration]
L'option `integrationPattern` est définie par défaut sur `isolated`, qui crée une Lambda par procédure tRPC. Sélectionnez `shared` si vous préférez un seul gestionnaire Lambda partagé pour toute l'API, avec des remplacements optionnels par procédure. L'option `integrationPattern` ne s'applique pas lorsque `computeType` est `BedrockAgentCoreRuntimeWebSocket`.
:::
...
Les subscriptions ne sont supportées que lorsque `computeType` est `ServerlessApiGatewayRestApi` (par défaut), car les API HTTP API Gateway ne supportent pas le streaming de réponses.
:::
...
## Déploiement sur Bedrock AgentCore

Définir `computeType` sur `BedrockAgentCoreRuntimeWebSocket` déploie le serveur tRPC en tant que serveur WebSocket conteneurisé sur [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) au lieu d'API Gateway + Lambda. AgentCore vous offre :

- **Subscriptions en streaming via WebSocket** — messagerie full duplex pour les subscriptions tRPC sans les timeouts de 30 secondes de Lambda ou API Gateway.
- **Affinité de session** — l'en-tête `x-amzn-bedrock-agentcore-runtime-session-id` est propagé dans votre contexte tRPC en tant que `ctx.sessionId`, vous permettant de gérer l'état par session.
- **Runtime géré** — AgentCore gère la mise à l'échelle, les vérifications de santé (`/ping`) et l'autorisation.

:::caution[Options d'authentification]
AgentCore ne supporte que l'authentification `IAM` ou `Cognito`. Sélectionner `auth=None` avec `computeType=BedrockAgentCoreRuntimeWebSocket` entraînera un échec de génération.
:::

### Comment ça fonctionne

Lorsque vous choisissez `BedrockAgentCoreRuntimeWebSocket`, le générateur émet :

- `src/handler-agentcore.ts` — un serveur Node WebSocket + HTTP qui expose `/ping` pour les vérifications de santé AgentCore et `/ws` pour les subscriptions tRPC via `@trpc/server/adapters/ws`.
- `Dockerfile` — conteneurise le serveur bundlé et inclut l'[AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) pour l'observabilité gérée.
- Un construct CDK (ou module Terraform) qui provisionne un `Runtime` Bedrock AgentCore, lui accorde la permission d'invoquer les modèles Bedrock et de lire la configuration du runtime, et publie l'ARN du runtime dans `runtimeConfig.apis.<YourApiName>` pour que le frontend puisse le récupérer.
- Une cible de build `docker` intégrée dans la cible `build` du projet afin que `nx build` produise l'image conteneur.

### Accès à l'ID de session

Chaque procédure reçoit `ctx.sessionId` lorsque l'API est déployée sur AgentCore :
...
export const chat = publicProcedure
  ...
  .subscription(async function* (opts) {
    ...
    // ...diffuser des chunks qui s'appuient sur `history`
  });
```

Localement, `nx serve` exécute un serveur HTTP + WebSocket autonome afin que les clients puissent se connecter de la même manière qu'en production. L'ID de session est par défaut un UUID aléatoire lorsque l'en-tête n'est pas présent.
...
:::warning[Hypothèse d'authentification]
Cet exemple suppose que `auth` était défini à `IAM`. Pour l'authentification Cognito, le middleware d'identité est plus simple, extrayant les claims pertinents depuis l'`event`.
:::
