...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
Par défaut, les agents Strands utilisent Claude Sonnet 4.6 sur Amazon Bedrock, mais vous pouvez personnaliser le fournisseur de modèle. Consultez la [documentation Strands sur les fournisseurs de modèles](https://strandsagents.com/docs/user-guide/concepts/model-providers/) pour les options de configuration :
...
<Snippet name="agent/dev-vs-serve" parentHeading="Développement local" />
...
Votre agent est automatiquement configuré avec l'observabilité en utilisant l'[AWS Distro for Open Telemetry](https://aws.amazon.com/otel/) (ADOT).
...
Ensuite, envoyez une requête POST à `/invocations` sur le port sur lequel votre agent local s'exécute. Remplacez le port attribué à votre agent — lisez-le depuis `metadata.ports` dans le `project.json` du projet, ou depuis le flag `--port` dans la commande de la cible `<your-agent-name>-dev`. Un premier agent HTTP dans un espace de travail neuf se voit attribuer `8081` :
...
:::caution[Utilisez `-dev`, pas `-serve`]
Utilisez `<your-agent-name>-dev` pour cela, pas `<your-agent-name>-serve`. `-serve` exécute le chemin de code déployé, il a donc besoin que `RUNTIME_CONFIG_APP_ID` soit défini — voir <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Sans cela, le streaming a déjà commencé au moment où l'agent tente de résoudre son stockage de session, donc la requête retourne un `200` vide sans erreur et l'échec n'apparaît que dans le journal du serveur.
:::
