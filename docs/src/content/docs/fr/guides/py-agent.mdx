...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain fournit un large écosystème d'[outils et intégrations](https://docs.langchain.com/oss/python/integrations/tools), chacun distribué dans son propre package. Le générateur ne les ajoute pas, donc installez d'abord celui dont vous avez besoin — par exemple `langchain-community`, d'où provient l'outil de recherche ci-dessous :

<NxCommands commands={['run <project-name>:add langchain-community']} />

Ensuite, importez les outils et ajoutez-les à `get_agent()` :
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
L'agent généré utilise le modèle Strands par défaut sur Amazon Bedrock. Pour le configurer, passez un `model` à l'`Agent`. Consultez la [documentation Strands sur les fournisseurs de modèles](https://strandsagents.com/docs/user-guide/concepts/model-providers/) pour les fournisseurs disponibles et leurs options :
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
Les agents LangChain utilisent un modèle [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse). L'agent généré lit l'identifiant du modèle et la région à partir des variables d'environnement `MODEL_ID` et `AWS_REGION`, que l'infrastructure définit pour l'agent déployé. Pour configurer davantage le modèle, ajoutez des arguments là où il est construit dans `get_agent()` :
...
Une cible `<your-agent-name>-serve` est également générée, qui exécute l'agent contre votre infrastructure déployée et nécessite donc que `RUNTIME_CONFIG_APP_ID` soit défini. Consultez le guide <Link path="guides/local-development">Développement local</Link> pour la différence entre `dev` et `serve`.
