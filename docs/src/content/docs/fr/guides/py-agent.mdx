...
Le générateur configure une cible Nx `<your-agent-name>-chat` qui vous place dans un chat terminal interactif avec votre agent, alimenté par [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli).

La cible de chat s'exécute de manière autonome — elle ne démarre **pas** l'agent pour vous. Par défaut, elle se connecte à votre agent en cours d'exécution localement, donc démarrez d'abord `<your-agent-name>-serve-local` (dans un terminal séparé) :
