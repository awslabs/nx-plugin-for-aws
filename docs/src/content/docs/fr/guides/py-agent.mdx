...
Pour les agents Strands, le générateur utilise [FastAPI](https://fastapi.tiangolo.com/) pour exposer un serveur HTTP par défaut. Alternativement, vous pouvez choisir le protocole [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) pour l'interopérabilité avec d'autres agents compatibles A2A, ou le protocole [AG-UI](https://docs.ag-ui.com/) pour une intégration frontend directe via [CopilotKit](https://docs.copilotkit.ai/).

:::note[LangChain supporte uniquement AG-UI]
Le framework `langchain` ne supporte actuellement que le protocole `ag-ui`. Choisissez le framework `strands` par défaut si vous avez besoin du protocole `http` ou `a2a`. Les agents LangChain peuvent toujours se connecter aux serveurs MCP, aux passerelles AgentCore et aux agents A2A via le <Link path="/guides/connection/py-agent-mcp">générateur `connection`</Link>.
:::
...
Les exemples ci-dessous utilisent Strands. Un agent LangChain fonctionne de la même manière, en utilisant `create_agent` avec le décorateur `@tool` de `langchain_core.tools` et un modèle `ChatBedrockConverse` (les variables d'environnement `MODEL_ID` et `AWS_REGION` définissent le modèle et la région). Consultez la [documentation LangChain](https://docs.langchain.com/oss/python/langchain/overview) pour plus de détails.
...
Pour utiliser des serveurs MCP créés avec les générateurs <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> ou <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>, vous pouvez utiliser le <Link path="/guides/connection/py-agent-mcp">générateur `connection`</Link>.
...
Pour un guide plus détaillé sur l'écriture d'agents Strands, consultez la [documentation Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Pour les agents LangChain, le <Link path="/guides/connection/py-agent-mcp">générateur `connection`</Link> intègre également les outils de serveur MCP, de passerelle AgentCore et d'agent A2A dans `create_agent`.
