...
- **Strands** : enveloppe votre `Agent` dans un `ag_ui_strands.StrandsAgent`, construit à l'intérieur d'un gestionnaire `lifespan` FastAPI (donc la construction se produit au démarrage du conteneur/session plutôt qu'au moment de l'import), et servi depuis une boucle FastAPI `/invocations` faite à la main.
- **LangChain** : enveloppe le graphe compilé dans un `ag_ui_langgraph.LangGraphAgent`, construit de la même manière à l'intérieur de `lifespan`, et servi depuis une boucle FastAPI `/invocations` faite à la main.
...
Cela utilise `uv run` pour exécuter votre agent via le [SDK Python Bedrock AgentCore](https://github.com/aws/bedrock-agentcore-sdk-python).
...
Consultez le guide [Guardrails](https://strandsagents.com/docs/user-guide/safety-security/guardrails/) de Strands pour plus de détails.
</TabItem>
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
```python
# agent.py
