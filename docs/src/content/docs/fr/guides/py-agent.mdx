...
Si vous avez sélectionné `None` pour `computeType`, aucun construct CDK ou module Terraform n'est généré — l'agent Strands ne peut être exécuté que localement. L'option `auth` est ignorée dans ce mode car il n'y a pas de point de terminaison hébergé à authentifier.
</OptionFilter>
