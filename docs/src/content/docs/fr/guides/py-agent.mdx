...
Si vous souhaitez que chaque utilisateur n'accède qu'à ses propres conversations, enregistrez qui possède chaque session dans votre propre application et vérifiez cette propriété à l'entrée. Un identifiant de session est alors quelque chose que votre application émet plutôt que quelque chose que l'appelant choisit.

**1. Créer des sessions via votre API**

Ajoutez une procédure qui démarre une conversation. Elle génère l'identifiant de session elle-même, résout l'appelant à partir de son propre contexte de requête, et stocke la paire — par exemple dans une table créée avec le générateur <Link path="guides/py-dynamodb">`py#dynamodb`</Link> :
...
L'API peut être dans n'importe quel langage ; l'exemple ci-dessus utilise le générateur <Link path="guides/trpc">`ts#trpc-api`</Link>.

Résolvez l'appelant à partir du contexte de requête vérifié plutôt qu'à partir d'un champ d'entrée, afin qu'il ne puisse pas être fourni par le navigateur. <Link path="guides/trpc">Implementing Custom Middleware</Link> montre comment faire cela pour les API authentifiées par IAM et Cognito.

Une deuxième procédure interrogeant cet index permet à l'interface utilisateur d'offrir à un utilisateur ses propres conversations sans jamais voir celles de quelqu'un d'autre.

**2. Valider la propriété dans l'agent**

Votre agent reçoit déjà l'identifiant de session, et avec `auth=cognito` il reçoit également le JWT de l'appelant — l'autorisateur AgentCore Runtime le valide et l'infrastructure générée autorise l'en-tête `Authorization` à passer à votre code. Recherchez la session, comparez son propriétaire avec l'appelant, et refusez la requête s'ils diffèrent :
...
Décoder la revendication sans vérifier sa signature est suffisant ici, puisque AgentCore Runtime a validé le jeton lors de l'autorisation entrante.

:::tip[Pourquoi valider dans l'agent ainsi que dans l'API]
L'API décide qui possède une session ; l'agent l'applique. Vérifier aux deux endroits signifie qu'un identifiant de session qui fuit — copié depuis un navigateur, ou réutilisé d'une ancienne conversation — est toujours refusé lors de la prochaine invocation, et tout futur client de l'agent hérite de la même règle.
:::

:::caution[La session sélectionne également la microVM]
Un identifiant de session sélectionne la [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) qui sert la requête, donc son système de fichiers, `/tmp` et les caches en cours de processus sont partagés par chaque requête résolvant à cette session. Cela mérite attention si votre agent ou ses outils écrivent sur le disque local : deux appelants qui envoient le même identifiant de session partagent ces ressources même lorsque leurs conversations stockées sont séparées.

Générer des identifiants de session opaques dans votre API évite cela, car un appelant ne peut pas atterrir sur une session qui ne lui a pas été donnée. En dériver un à partir de valeurs fournies par l'utilisateur — un nom de conversation, un email, un nom de jeu — est prévisible et permet à deux appelants d'entrer en collision.
:::
