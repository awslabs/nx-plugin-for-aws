...
Si vous avez sélectionné `None` pour `infra`, aucun construct CDK ou module Terraform n'est généré — l'agent Strands ne peut être exécuté que localement. L'option `auth` est ignorée dans ce mode car il n'y a pas de point de terminaison hébergé à authentifier.
</OptionFilter>
...
Cette commande utilise `uv run` pour exécuter votre Agent en utilisant le [SDK Python Bedrock AgentCore](https://github.com/aws/bedrock-agentcore-sdk-python).
...
Pour invoquer votre agent Strands depuis un site web React, vous pouvez utiliser le <Link path="/guides/connection/react-py-agent">générateur `connection`</Link>, qui configure automatiquement un client avec l'authentification appropriée (IAM ou Cognito).
...
r>
/connection/react-agui">générateur `connection`</Link>, qui configure un client [CopilotKit](https://docs.copilotkit.ai/aws-strands) pour votre agent déployé avec l'authentification appropriée (IAM ou Cognito).

<RunGenerator generator="connection" />

Consultez le <Link path="/guides/connection/react-agui">guide du générateur `connection`</Link> pour plus de détails sur la configuration de la connexion.
</OptionFilter>
tps://docs.copilotkit.ai/aws-strands) pour votre agent déployé avec l'authentification appropriée (IAM ou Cognito).
