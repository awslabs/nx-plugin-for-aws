...
Étant donné que l'adaptateur `ag-ui-strands` d'AG-UI clone un `Agent` modèle par fil de conversation, `get_session_manager` est câblé en tant que `session_manager_provider` sur le `StrandsAgentConfig` passé à `StrandsAgent` dans `main.py`, de sorte qu'un nouveau `SessionManager` est résolu pour chaque fil plutôt que d'être intégré dans l'agent modèle partagé.
</OptionFilter>

<OptionFilter when={{ protocol: ['http', 'a2a'] }} description="HTTP/A2A session wiring">
Étant donné que `with_session_id` met déjà en cache une instance `Agent` par session, `get_session_manager` est appelé directement à l'intérieur de l'appel `Agent(session_manager=get_session_manager())` de `get_agent` dans `agent.py`.
</OptionFilter>
...
- **`s3`** : L'agent déployé utilise un `S3CheckpointSaver` avec le bucket de session provisionné, stockant les points de contrôle et les écritures en attente sous le préfixe `checkpoints/`. Cette classe (adaptée de [`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3), sous licence MIT) se trouve dans `s3_checkpoint_saver_langchain.py` dans le projet de connexion d'agent partagé.
...
- **`in-memory`** (par défaut) : Aucune table ou bucket n'est provisionné. L'état de conversation est conservé en mémoire uniquement pendant la durée de vie du processus en cours d'exécution et ne survit pas aux redémarrages ou à la réduction d'échelle.
