...
<OptionFilter not when={{ eventSource: 'Any' }} description="eventSource fournit un schéma Zod typé pour l'événement entrant">
...
<OptionFilter when={{ eventSource: 'Any' }} description="eventSource=Any — pas de middleware parser, l'événement est typé comme `any`">
Vous avez sélectionné `Any` pour votre `eventSource`, ce qui signifie que le middleware `parser` n'est pas configuré et que le paramètre `event` est typé comme `any`. Si vous souhaitez ultérieurement bénéficier de la sécurité de type à la compilation et de la validation à l'exécution, régénérez la fonction avec un `eventSource` spécifique.
</OptionFilter>
