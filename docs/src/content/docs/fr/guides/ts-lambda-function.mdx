...
Si vous avez sélectionné `Any` pour votre `eventSource`, le middleware `parser` n'est pas configuré et le paramètre `event` est typé comme `any`. Régénérez la fonction avec un `eventSource` spécifique si vous souhaitez bénéficier de la sécurité de type à la compilation et de la validation à l'exécution.
</OptionFilter>
