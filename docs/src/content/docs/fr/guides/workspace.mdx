...
Du point de vue de Node, cela signifie un seul fichier de verrouillage à la racine avec un seul `node_modules` contenant toutes les dépendances. Chaque projet Node déclare les dépendances d'exécution que son code source importe dans son propre `package.json`, tandis que les outils de build/test partagés se trouvent dans les `devDependencies` du `package.json` racine. Ajoutez une dépendance d'exécution d'un projet avec le flag filter/workspace de votre gestionnaire de packages (par exemple `pnpm add <pkg> --filter <project>`), et les outils de développement partagés à la racine.

Pour les gestionnaires de packages avec support de catalogue ([pnpm](https://pnpm.io/catalogs), [yarn](https://yarnpkg.com/features/catalogs) et [bun](https://bun.com/docs/install/catalogs)), les versions des dépendances sont enregistrées dans le catalogue et référencées avec le protocole `catalog:`, conservant une source unique de vérité pour les versions dans le `package.json` de chaque projet. Pour les espaces de travail npm, nous recommandons [syncpack](https://github.com/JamieMason/syncpack) pour aligner les versions déclarées dans plusieurs fichiers `package.json`.
...
KER`. Consultez le <Link path="guides/docker-bundling">guide de bundling Docker</Link> pour plus de détails.
