...
Cela signifie que vous n'avez pas besoin d'exécuter la vérification explicitement, bien que vous puissiez toujours le faire avec la cible NxCommands `license-check` :
...
Pour désactiver de manière permanente, supprimez la clé `license.dependencies` de votre configuration dans `aws-nx-plugin.config.mts`. Vous pouvez également ré-exécuter le générateur `license` avec `--dependencyCheck=false` pour le générer sans cette fonctionnalité — le flag du générateur est toujours nommé `--dependencyCheck` et contrôle si le générateur écrit le bloc `dependencies`.