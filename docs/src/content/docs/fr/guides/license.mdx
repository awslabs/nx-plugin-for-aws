...
Utilisez `exceptions` pour les paquets qui échouent à la vérification — soit parce que leur licence ne figure pas dans la liste autorisée, soit parce qu'ils sont livrés sans métadonnées de licence détectables.

Chaque entrée accepte :

- `package` (requis) : le nom du paquet auquel l'exception s'applique
- `reason` (requis) : pourquoi l'exception a été accordée, afin que les réviseurs puissent le voir
- `version` (optionnel) : restreindre l'exception à une seule version, afin qu'une version ultérieure soit vérifiée à nouveau
- `spdx` (optionnel) : la licence à traiter comme déclarée par le paquet, pour un paquet qui livre du texte de licence sans le déclarer dans ses métadonnées. C'est ce que le hook `onDependency` ci-dessous rapporte
...
Les collecteurs découvrent les dépendances et extraient les métadonnées de licence. Les collecteurs intégrés sont `npmCollector()` (analyse `node_modules`) et `pythonCollector()` (analyse les environnements virtuels Python), tous deux exportés depuis `@aws/nx-plugin/sdk/license`. Le générateur de licence configure `npmCollector()` par défaut et ajoute `pythonCollector()` lorsque des projets Python sont présents.
