...
Si vous souhaitez explorer le développement AWS assisté par l'IA avec plus de structure et de fiabilité, essayez le Plugin Nx pour AWS MCP Server. Vous pouvez le configurer dans votre assistant IA préféré (Kiro, Kiro CLI, Cline, Claude Code, etc.) avec la configuration suivante du serveur MCP :
