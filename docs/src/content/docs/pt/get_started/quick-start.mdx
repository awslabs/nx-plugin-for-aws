...
Este guia orienta você através dos conceitos básicos de instalação e uso do `@aws/nx-plugin` para construir projetos rapidamente na AWS.
...
Adicionaremos uma API tRPC, Site React, Autenticação Cognito e Infraestrutura CDK neste guio rápido. Dependendo do tipo de projeto que você está construindo, você pode escolher qualquer combinação de geradores para inicializar rapidamente seu projeto. Confira os __Guias__ na barra de navegação à esquerda para ver a lista completa de opções.
...
Isso estruturará um novo site React em `packages/demo-website`.
