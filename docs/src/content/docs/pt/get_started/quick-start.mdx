...
Este guia orienta você através dos fundamentos da instalação e uso do `@aws/nx-plugin` para construir projetos rapidamente na AWS. Teste.

:::tip
Para um tutorial mais detalhado sobre a construção de uma aplicação full-stack, confira o <Link path="get_started/tutorials/dungeon-game/overview">Tutorial de Aventura no Calabouço</Link>.
:::
...
:::tip
Você será questionado sobre seu provedor de infraestrutura como código (IaC) preferido, seja [CDK](https://docs.aws.amazon.com/cdk/) ou [Terraform](https://developer.hashicorp.com/terraform). Você pode pular a pergunta executando o comando com `--iacProvider`, ex:
