...
### Remover os Recursos de Bootstrap

Os recursos que você fez bootstrap são compartilhados por todos os projetos que implantam nessa conta e região AWS, e custam quase nada para deixar no lugar. Remova-os apenas se nada mais for implantado lá.
...
O bucket de assets é retido propositalmente, então exclua-o separadamente. Ele é versionado, o que `aws s3 rb --force` não consegue lidar, então remova todas as versões e marcadores de exclusão primeiro:
...
Cada `delete-objects` reporta um erro em vez de não fazer nada quando não há mais nada desse tipo para remover, o que é seguro ignorar.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Seu Estado do Terraform Também É Excluído]
Isso esvazia e exclui o bucket S3 que contém seu estado do Terraform, então execute-o apenas quando nada que você se importe ainda estiver implantado a partir desse estado.
:::
</Fragment>
</Infrastructure>
