...
Execute o seguinte comando para criar um <Link path="guides/workspace">workspace Nx</Link> com o gerenciador de pacotes de sua preferência:

<CreateNxWorkspaceCommand workspace="my-project" />

:::note[Repositórios Git Existentes]
Se o diretório do workspace já estiver dentro de um repositório Git, a inicialização do Git é ignorada para que seu repositório existente permaneça intocado.
:::

:::tip[Escolhendo um provedor IaC]
Você será questionado sobre seu provedor de infraestrutura como código (IaC) preferido, [CDK](https://docs.aws.amazon.com/cdk/) ou [Terraform](https://developer.hashicorp.com/terraform). Você pode pular a pergunta executando o comando com `--iac`, ex:
...
# Bucket de ativos compartilhado — armazena zips de implantação Lambda para cada módulo lambda / API
module "asset_bucket" {
