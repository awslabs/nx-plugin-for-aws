...
Você pode experimentar padrões GritQL no [Playground GritQL](https://docs.grit.io/playground), ou instalar o CLI GritQL para testar padrões localmente:
...
Se você completou o <Link path="get_started/tutorials/dungeon-game/overview">tutorial da aventura na masmorra</Link>, ou já tem outro workspace Nx existente que usa uma API tRPC, você pode pular esta etapa.
:::
...
Temos um conjunto de "smoke tests" que executam geradores em um workspace novo e garantem que tudo seja construído. No mínimo, seu novo gerador deve ser adicionado a **ambas** as matrizes de geradores para que seja exercitado pelos smoke tests:
...
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — um gerador oculto que compõe todos os outros em processo em uma única tree.

Ambas valem a pena manter. A matriz CLI testa o caminho que os usuários realmente seguem. A matriz composta é enviada com o plugin, então cada versão lançada carrega a matriz dos geradores que _ela_ tinha — o que permite que testes que atualizam um workspace mais antigo o estruturem com a matriz da própria versão. Como ela chama os geradores diretamente em vez de através da CLI, suas opções são verificadas quanto ao tipo em relação ao schema de cada gerador, então uma nova opção obrigatória ou um enum alterado falha na construção lá em vez de reduzir silenciosamente a cobertura.

Observe que a matriz de geradores apenas executa os geradores e constrói o workspace — ela não instancia nenhuma infraestrutura. Para geradores que implantam infraestrutura, considere estender os testes e2e de implantação (`e2e/src/smoke-tests/cdk-deploy.spec.ts` e `terraform-deploy.spec.ts`) para realmente implantar seus recursos (via `cdk deploy` / `terraform apply`), e então adicione uma asserção a `deploy-invocations.ts` que invoca o recurso implantado e verifica se ele se comporta conforme esperado.
