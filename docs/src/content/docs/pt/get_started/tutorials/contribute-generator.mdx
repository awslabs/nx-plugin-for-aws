...
Temos um conjunto de "smoke tests" que executam geradores em um workspace novo e garantem que tudo seja construído. No mínimo, seu novo gerador deve ser adicionado à matriz de geradores em `e2e/src/smoke-tests/generator-matrix.ts` para que seja exercitado pelos smoke tests.
