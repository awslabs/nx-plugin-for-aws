...
Temos um conjunto de "smoke tests" que executam geradores em um workspace novo e garantem que tudo seja construído. No mínimo, seu novo gerador deve ser adicionado à matriz de geradores em `packages/nx-plugin/src/internal/test-matrix/generator.ts` para que seja exercitado pelos smoke tests.

A matriz é em si um gerador (oculto) que compõe todos os outros, então ela é enviada com o plugin. Isso significa que cada versão lançada carrega a matriz dos geradores que _ela_ tinha, o que permite que testes que atualizam um workspace mais antigo o estruturem com a matriz da própria versão. Como ela chama os geradores diretamente em vez de através da CLI, as opções são verificadas quanto ao tipo em relação ao schema de cada gerador — então uma nova opção obrigatória ou um enum alterado falhará na construção aqui em vez de reduzir silenciosamente a cobertura.
