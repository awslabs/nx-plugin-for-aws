...
<FileTree>
  - packages/nx-plugin/src/trpc/procedure
    ...
    - generator.ts Função que o Nx executa como nosso gerador
</FileTree>
    ...
    ```json
    {
      ...
      "properties": {
        ...
        "procedure": {
          ...
          "type": "string",
          "x-prompt": "Como você gostaria de nomear seu novo procedimento?",
          "x-priority": "important",
        },
        "type": {
          "description": "Tipo de procedimento a ser gerado",
          "type": "string",
          "x-prompt": "Qual tipo de procedimento você gostaria de gerar?",
          "x-priority": "important",
          "default": "query",
          "enum": ["query", "mutation"]
        }
...
Em seguida, vamos conectar nosso gerador atualizando `packages/nx-plugin/generators.json`:
...
Primeiro, definiremos nosso template em `packages/nx-plugin/src/trpc/procedure/files/procedures/__procedureNameKebabCase__.ts.template`:
...
:::tip
Quando `generateFiles` processa nosso template, ele substituirá referências a `__<variável>__` em nomes de arquivos/diretórios pelos valores fornecidos, além de remover `.template` do nome do arquivo.
...
Precisamos garantir que passaremos essas variáveis para `generateFiles`, assim como o diretório para gerar os arquivos, especificamente o local dos arquivos fonte (ou seja, `sourceRoot`) do projeto tRPC selecionado pelo usuário, que podemos extrair da configuração do projeto.

Vamos atualizar nosso gerador para fazer isso:
...
Em seguida, queremos que nosso gerador conecte o novo procedimento ao router. Isso significa ler e atualizar o código fonte do usuário!

Usamos manipulação de AST TypeScript para atualizar as partes relevantes do arquivo fonte. Existem helpers chamados `replace` e `destructuredImport` para facilitar isso.
...
Agora que implementamos nosso gerador, vamos compilá-lo para garantir que esteja disponível para testes em nosso projeto dungeon-adventure.
...
Para testar nosso gerador, vamos vincular nosso plugin local ao código do <Link path="get_started/tutorials/dungeon-game/overview">dungeon-adventure</Link>.

::::note
Não se preocupe se você não completou o tutorial do dungeon-adventure, isso também funcionará em qualquer codebase habilitado para Nx.
::::

No seu codebase, vamos vincular nosso `@aws/nx-plugin` local:
...
Vamos executar nosso novo gerador:

<RunGenerator generator="ts#trpc-api#procedure" />

:::note
Se você não vir o novo gerador na lista no VSCode, talvez precise atualizar o workspace Nx:
...
Escreva alguns testes unitários para o gerador. São relativamente simples de implementar, seguindo o fluxo geral:
...
Se ninguém contribuiu com este gerador para o `@aws/nx-plugin`, envie um pull request! Se já houver um lá, talvez pense em outro gerador que possa contribuir.