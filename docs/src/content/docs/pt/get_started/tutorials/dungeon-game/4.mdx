...
Você pode então abrir o site local em um navegador, onde será solicitado que faça login e siga as instruções para criar um novo usuário. Após concluir, você deverá ver o site base:
...
:::tip
Você pode executar <NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} /> para iniciar também quaisquer servidores de API locais conectados, permitindo que você itere tanto na API tRPC quanto no site simultaneamente.
:::
...
O layout padrão configurado é mais adequado para aplicativos empresariais no estilo SaaS do que para um jogo. Vamos reconfigurar o layout e retematizá-lo para se parecer mais com um jogo de masmorra.
...
:::tip
Você notará que a integração com nossa Game API já está configurada - não precisamos fazer nada manualmente pois usamos o gerador `api-connection`. Por favor, +1 [nesta issue do GitHub](https://github.com/awslabs/nx-plugin-for-aws/issues/326) se desejar uma experiência similar para agentes.
:::
