...
:::note
Sempre que fizer alterações em seu FastAPI, você precisará reconstruir seu projeto para ver essas alterações refletidas no client gerado em seu website.
...
Após a conclusão da implantação, você verá algumas saídas similares às seguintes _(alguns valores foram omitidos)_:
...
<li>
<Drawer title="curl habilitado com Sigv4" trigger="Chamando a API implantada diretamente usando curl com Sigv4">
...
Você pode adicionar a seguinte função ao seu perfil do PowerShell ou simplesmente colar no mesmo sessão do PowerShell onde deseja executar o comando.
```powershell
...
```
    <Aside type="caution">
    Use o valor de saída `dungeon-adventure-infra-sandbox-Application.StoryApiStoryApiUrlXXX` do deploy CDK para substituir o placeholder de URL destacado e defina a região adequadamente.
    </Aside>
  </TabItem>
</Tabs>
