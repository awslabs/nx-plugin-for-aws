...
<Tabs>
  <TabItem label="Local">
  O gerador de agentes fornece uma CLI de chat interativo de terminal (`agent-chat`) que usa um cliente TypeScript type-safe gerado a partir da especificação OpenAPI do agente. Como alteramos a forma de entrada do agente para aceitar `playerName`, `genre` e `actions`, precisamos atualizar o script de chat gerado para fornecer esses campos.

  Atualize `packages/story/scripts/agent/chat.ts`:
  ...
  Em seguida, inicie um chat interativo com seu agente Story em execução local:
    ...
    O destino `agent-chat` inicia automaticamente o `agent-serve-local` (que inicia o agente e quaisquer dependências do servidor MCP com `SERVE_LOCAL=true`) e executa a geração da especificação OpenAPI + cliente antes de colocá-lo no chat.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat` depende de `agent-serve-local`, que inicia o agente com `SERVE_LOCAL=true` e também inicia quaisquer dependências do servidor MCP localmente. Se você preferir que o agente se conecte ao seu servidor MCP **implantado** (via `RUNTIME_CONFIG_APP_ID`), execute o destino `agent-serve` em um terminal separado e invoque o agente manualmente via `curl` contra seu endpoint `/invocations`.
    :::
  </TabItem>
