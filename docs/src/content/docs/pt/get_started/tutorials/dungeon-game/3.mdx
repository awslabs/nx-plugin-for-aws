...
- `main.py` adiciona um `session_manager_provider` que cria um `S3SessionManager` por `thread_id` — isso é o que faz o histórico de conversação persistir entre turnos para o mesmo `(player, genre)`, e o coloca no bucket do qual o `queryActions` da Game API lê.
