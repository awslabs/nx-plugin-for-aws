...
- Construção do agente com um prompt de sistema e ferramentas do MCP server
...
<ul>
<li>Iniciando uma instância local do servidor Agent e invocando com `curl`</li>
...
<Tabs>
  ...
  <TabItem label="Deployed">
    Para testar o agente implantado, autentique via Cognito para obter um token JWT:
    ...
    Crie um usuário de teste e obtenha o token:
...
Em caso de sucesso, você verá eventos sendo streamados como:
