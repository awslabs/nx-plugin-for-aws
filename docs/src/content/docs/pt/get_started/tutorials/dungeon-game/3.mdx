...
<Tabs>
  ...
  Inicie seu servidor Agent local executando o seguinte comando:
    ...
    O destino `agent-serve` inicia o agente localmente, mas se conecta ao seu servidor MCP **implantado** (via `RUNTIME_CONFIG_APP_ID`). Se você preferir executar o servidor MCP localmente também, use `agent-serve-local` em vez disso, que inicia todos os serviços dependentes e define `SERVE_LOCAL=true` para conexões HTTP diretas.
    :::

    Assim que o servidor Agent estiver em execução (você não verá nenhuma saída), chame-o executando o seguinte comando:
...
Se o comando for executado com sucesso, você deverá começar a ver o texto inicial da história sendo transmitido como JSON Lines:
