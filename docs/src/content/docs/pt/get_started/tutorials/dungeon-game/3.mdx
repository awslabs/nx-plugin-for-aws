...
<Aside type="caution">
Certifique-se que sua conta AWS habilitou o acesso ao [modelo padrão da Strands](https://strandsagents.com/latest/documentation/docs/user-guide/concepts/model-providers/amazon-bedrock/) dentro do Bedrock para sua região alvo seguindo os passos do [guia oficial](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html). No momento desta escrita, o modelo é o Anthropic Claude Sonnet 4.
</Aside>

O Story Agent é um agente [Strands](https://strandsagents.com/) que, dado um `Game` e uma lista de `Action`s como contexto, progride em uma história. Configuraremos o agente para interagir com nosso Inventory MCP Server e gerenciar os itens disponíveis do jogador.

:::note
Para simplificar este tutorial, implementamos o Agente de forma "stateless" onde cada invocação é uma sessão independente incluindo todo o histórico. Para produção, considere integrar com [Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html). Mais documentação sobre uso com Strands está [disponível aqui](https://github.com/aws/bedrock-agentcore-sdk-python/tree/main/src/bedrock_agentcore/memory/integrations/strands).
:::
...
Após conclusão, você verá saídas similares a estas _(valores sensíveis omitidos)_:
...
<Tabs>
  ...
  Inicie o servidor local com:
    ...
    :::caution
    Use o output `InventoryMcpArn` do CDK deploy acima, especificando também sua `AWS_REGION` alvo.
    :::
  ...
  <TabItem label="Deployed">
    Para testar o agente implantado, autentique via Cognito para obter um token JWT. Primeiro configure variáveis:
    ...
    Crie um usuário de teste e capture o token:
    ...
    :::caution
    Use o output `UserIdentityUserIdentityUserPoolIdXXX` para `POOL_ID` e confirme o Client ID correspondente.
    :::
    ...
    :::note
    A codificação da URL é necessária para formatar corretamente a ARN no endpoint da API.
    :::
  </TabItem>
</Tabs>

Se bem sucedido, você verá eventos streamados como:
