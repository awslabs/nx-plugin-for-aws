...
:::note[O histórico de conversação vive com o agente]
Não persistimos mensagens de chat através da API do Jogo. No <Link path="get_started/tutorials/dungeon-game/3">Módulo 3</Link>, o Story Agent conecta um `FileSessionManager` que escreve o histórico de cada thread no disco, indexado pelo id da thread que a UI envia a cada turno — assim, cada par `(jogador, gênero)` retoma sua própria história sem que a UI precise reproduzir as mensagens.
:::
