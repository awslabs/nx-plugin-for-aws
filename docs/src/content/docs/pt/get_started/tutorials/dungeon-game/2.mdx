...
A dependência `@aws-sdk/client-s3` já está no workspace, então não há nada novo para instalar. O nome do bucket vem do mesmo mecanismo de Configuração de Runtime usado para `tables` — nós o registramos sob um novo namespace `buckets` na pilha de infraestrutura abaixo.
  </TabItem>
</Tabs>
