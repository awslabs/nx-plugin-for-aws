...
:::note[Sem procedimento de histórico de chat]
Você pode ter esperado um procedimento para salvar cada turno de usuário/assistente do chat. Não precisamos de um: o agente AG-UI está hospedado no Bedrock AgentCore Runtime, que persiste o histórico de conversação por sessão automaticamente, desde que o mesmo thread id seja usado.
:::
