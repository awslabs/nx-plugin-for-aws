...
Para definir as entradas e saídas da nossa API, vamos criar nosso schema usando [Zod](https://zod.dev/) dentro do diretório `packages/game-api/src/schema` da seguinte forma:
...
:::note
Observe que como cada procedimento é servido por uma função lambda individual, podemos seguir o princípio de menor privilégio e atribuir apenas as permissões de leitura/escrita necessárias com base na implementação do procedimento.
:::
  </TabItem>
</Tabs>
...
:::caution
Você pode precisar fazer o bootstrap inicial da sua conta AWS executando:
...
Uma vez concluída a implantação, você verá algumas saídas similares às seguintes _(alguns valores foram omitidos)_:
...
Para fazer uma requisição curl autenticada com sigv4, você pode invocar o `acurl` como nos exemplos abaixo:
...
Você pode adicionar a seguinte função ao seu perfil do PowerShell ou simplesmente colar no mesmo session do PowerShell onde deseja executar o comando.
```powershell
...
<Tabs>
  <TabItem label="Local">
    Inicie seu servidor local `game-api` executando o comando:
    ...
    Com o servidor em execução, você pode chamá-lo usando:
...
```
    <Aside type="caution">
    Use o valor de saída do CDK `dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX` para substituir o placeholder destacado e ajustar a região conforme necessário.
    </Aside>
  </TabItem>
</Tabs>
