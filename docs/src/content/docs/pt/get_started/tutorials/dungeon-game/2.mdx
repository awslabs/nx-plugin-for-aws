...
Vamos implementar nosso banco de dados no DynamoDB e usaremos a biblioteca cliente [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) do DynamoDB para simplificar o processo. O `queryActions` da Game API também precisa do cliente S3 para ler os turnos de conversação que o agente escreveu. Instale todos eles com:
