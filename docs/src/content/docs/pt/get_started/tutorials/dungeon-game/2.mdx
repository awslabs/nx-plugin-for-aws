...
1. `createGame` - criará uma nova instância do jogo.
...
Vamos implementar nosso banco de dados no DynamoDB usando a biblioteca [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/). Para começar, primeiro instale o `electrodb` executando:

<InstallCommand pkg="electrodb @aws-sdk/client-dynamodb" />

<Aside>
Todas as dependências são adicionadas ao `package.json` raiz pois o `@aws/nx-plugin` segue o princípio de [single version policy](https://nx.dev/concepts/decisions/dependency-management#single-version-policy). Para mais informações, consulte o <Link path="guides/typescript-project#dependencies">guia ts#projeto</Link>.
</Aside>

Agora crie os seguintes arquivos na pasta `packages/game-api/backend/src/entities` para definir nossas entidades ElectroDB conforme o diagrama ER:
...
O ElectroDB é poderoso e nos permite definir tipos, fornecer valores padrão como os timestamps acima. Além disso, segue o padrão [single-table design](https://electrodb.dev/en/core-concepts/single-table-relationships/) recomendado para DynamoDB.
...
Este plugin cria o `DynamoDBClient` e injeta no contexto.
  </TabItem>
...
Estendemos o `IMiddlewareContext` com `IDynamoDBContext`.
  </TabItem>
...
Plugin do DynamoDB adicionado.

<Aside>
Usamos `unstable_concat` que é seguro. O prefixo `unstable_` existe por ser uma nova API. Veja [concat guide](https://trpc.io/docs/server/middlewares#concat).
</Aside>
  </TabItem>
</Tabs>

### Definindo os procedimentos

Agora vamos implementar os métodos da API. Faça as alterações em `packages/game-api/backend/src/procedures`:
...
Exclua também o arquivo `echo.ts` (de `packages/game-api/backend/src/procedures`).

### Configuração do roteador

Atualize o seguinte arquivo para conectar as APIs:
...
Atualize `packages/infra/src` para criar a tabela DynamoDB e conceder permissões:
...
A primeira implantação leva ~8 minutos. Demais implantações ~2 minutos.

:::tip
Para alterações em funções Lambda, use `--hotswap` após compilar:
...
Teste a API usando:
<ul>
<li>Servidor local tRPC com `curl`</li>
<li>
...
<Tabs>
  <TabItem label="Local">
    ...
    <Aside type="caution">
    Use o valor de `ElectroDbTableTableNameXXX` do deploy.
    </Aside>
...
```
    <Aside type="caution">
    Use o `GameApiGameApiUrlXXX` do deploy.
    </Aside>
  </TabItem>
</Tabs>
...
Parabéns! Sua primeira API com tRPC está funcionando! 🎉🎉🎉