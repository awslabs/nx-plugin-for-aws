...
O nome da tabela DynamoDB é registrado na <Link path="guides/runtime-config">Configuração de Runtime</Link> sob o namespace `tables`, armazenado no AWS AppConfig. As funções Lambda e o servidor MCP o recuperam em runtime usando Lambda Powertools, em vez de precisar que seja passado como uma variável de ambiente. Um benefício dessa abordagem é que conforme o número de tabelas ou outros identificadores de infraestrutura cresce, você precisa apenas de uma única variável de ambiente `RUNTIME_CONFIG_APP_ID` ao executar localmente.
:::
  </TabItem>
</Tabs>
