...
- packages/
  - story/
    - dungeon\_adventure\_story/ módulo Python
      - agent/
        - main.py ponto de entrada da aplicação FastAPI AG-UI para Bedrock AgentCore Runtime
...
Como escolhemos `protocol=AG-UI`, o gerador envolve nosso agente Strands em um adaptador [AG-UI](https://ag-ui.com/). `create_strands_app` constrói uma aplicação FastAPI que expõe um único endpoint `/invocations` - instâncias de agente por sessão são criadas automaticamente, e o histórico de conversação é mantido em memória por `threadId` durante o tempo de vida do servidor. Não precisaremos modificar `main.py` em nenhum lugar deste tutorial.
