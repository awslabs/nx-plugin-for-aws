...
# One Agent per session — preserves conversation history.
...
def _get_cached_agent(session_id: str) -> Agent:
    if session_id not in _agents:
        _agents[session_id] = _agent_stack.enter_context(get_agent())
