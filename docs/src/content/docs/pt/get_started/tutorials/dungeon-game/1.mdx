...
<Aside type="tip">É uma melhor prática garantir que todos os arquivos não preparados sejam commitados no Git antes de executar qualquer gerador. Isso permite ver o que mudou após executar o gerador via `git diff`</Aside>
...
> {
  /**
   * Mapa de nomes de operação para suas integrações do API Gateway
   */
...
- packages/
  ...
  - story_api/
    ...
    - pyproject.toml
    - project.json
- .python-version versão Python fixa do uv
- pyproject.toml
- uv.lock
</FileTree>
...
Este hook pode ser usado para fazer requisições autenticadas à API `StoryApi`. Como visto na implementação, ele usa o `StoryApi` que é gerado no build time, então você verá um erro em seu IDE até construirmos nosso código. Para mais detalhes sobre como o cliente é gerado ou como consumir a API, consulte o <Link path="guides/api-connection/react-fastapi">guia React para FastAPI</Link>.
