...
O gerador `ts#dynamodb` fornece um target `serve-local` que executa o [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) em um contêiner. Combinado com o gerador `connection` (que executamos abaixo), isso significa que **o jogo inteiro roda em sua máquina sem chamadas AWS** até que você escolha fazer o deploy no final do tutorial. Você precisará de [um motor de contêiner](https://docs.docker.com/get-started/get-docker/) como Docker ou Finch instalado e em execução.
:::
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - gsi.ts fonte de verdade para Global Secondary Indexes
      - entities/
  ...
  - common/constructs/
    - src/
      ...
      - core/
        - dynamodb.ts construct genérico de tabela DynamoDB
</FileTree>
...
:::note[Shadcn UI]
Selecionamos `--ux=shadcn` para que o website gerado use componentes [shadcn/ui](https://ui.shadcn.com/) estilizados com Tailwind — todo o código de rota usa primitivos shadcn como `Card`, `Button` e `Input`, e o gerador `connection` posteriormente conecta uma superfície de chat [CopilotKit](https://docs.copilotkit.ai/) com tema shadcn correspondente.
:::
...
Este é o ponto de entrada onde o React é montado. A estilização vem de tokens Tailwind v4 importados via `styles.css`. O `@tanstack/react-router` é configurado no modo de [roteamento baseado em arquivo](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing): enquanto o servidor de desenvolvimento estiver em execução, qualquer arquivo que você criar em `routes/` é detectado automaticamente e a árvore de rotas é regenerada. Geradores posteriores (auth, connection) farão patch AST neste arquivo para envolver `<App />` em providers adicionais.
...
Graças à [inferência de Typescript](https://trpc.io/docs/concepts) do tRPC, `useGameApi` não precisa de uma etapa de build para que mudanças no backend cheguem ao frontend — edições no router ou em qualquer procedimento são detectadas pelo verificador de tipos instantaneamente.
</Aside>
...
- packages/
  - game-ui/
    - src/
      ...
      - hooks/
        - **useAguiStoryAgent.tsx** Constrói um `HttpAgent`, injeta o token bearer do Cognito e preenche o `threadId` para o session id de 33 caracteres do AgentCore
...
- Lê o ARN de runtime do agente da Runtime Configuration, constrói a URL de invocação do AgentCore e anexa o token bearer do Cognito mais o cabeçalho de session id do AgentCore.
...
Como o `agent-serve-local` do Story Agent já depende do `mcp-server-serve-local` do servidor MCP de inventário (via a conexão `story → inventory` acima), e tanto a Game API quanto o servidor MCP agora dependem de `dungeon-db:serve-local`, executar o target `serve-local` de qualquer projeto inicia todas as dependências necessárias — incluindo o DynamoDB Local — na ordem correta. Vamos confiar nisso durante o resto do tutorial.
</Aside>
...
<Aside type="note" title="Ambientes não interativos">
Em um shell não interativo (CI, por exemplo) não há prompt — o build falha com um erro "workspace is out of sync". Execute <NxCommands commands={['sync']} /> para aplicar os geradores de sincronização pendentes, depois faça o build novamente, ou defina `sync.applyChanges` como `true` em `nx.json` para aplicar as mudanças automaticamente em cada build. Você encontrará essa mesma verificação de fora de sincronia sempre que as referências do projeto mudarem (por exemplo, após adicionar procedimentos no próximo módulo).
</Aside>
