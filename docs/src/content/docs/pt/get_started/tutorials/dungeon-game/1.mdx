...
Vamos conectar nossa Game UI ao Story Agent para que possamos chamar a API do agente de nosso website React com um cliente type-safe, auto-gerado.
...
- packages/
  - story/
    - scripts/
      - **agent\_openapi.py** Script para gerar a especificação OpenAPI da aplicação FastAPI do agente
    - project.json Atualizado com target `agent-openapi`
  - game-ui/
    - src/
      - components/
        - **StoryAgentProvider.tsx** Provider para o cliente OpenAPI type-safe
        - **QueryClientProvider.tsx** Provider do TanStack React Query client (se ainda não estiver presente)
      - hooks/
        - **useStoryAgent.tsx** Hook para proxy de opções do TanStack Query
        - **useStoryAgentClient.tsx** Hook para o cliente API vanilla
      - generated/
        - story-agent/ Gerado em tempo de build a partir da especificação OpenAPI
          - types.gen.ts
          - client.gen.ts
          - options-proxy.gen.ts
    - project.json Atualizado com targets de geração do cliente
    - **main.tsx** Atualizado para adicionar StoryAgentProvider

</FileTree>

O gerador:
- Cria um script de geração de especificação OpenAPI com escopo de agente no projeto story
- Gera um cliente TypeScript type-safe a partir da especificação OpenAPI do agente em tempo de build
- Adiciona um provider, hooks e cliente ao website para chamar a API do agente
- Configura o `serve-local` para iniciar automaticamente o servidor local do agente e regenerar o cliente em mudanças
