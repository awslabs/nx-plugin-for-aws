...
- packages/
  - story/
    - dungeon_adventure_story/ módulo python
      - agent/
        - init.py configura o app FastAPI e middleware
...
- packages/
  - dungeon-db/
    ...
    - project.json adiciona os targets `serve-local` e `pull-image`
  - common/
    - scripts/
      - src/
        - dynamodb/
...
- packages/
  ...
  - game-ui/
    ...
    - ...
  - common/
    - shadcn/ biblioteca shadcn/ui compartilhada (tokens de tema, `Button`, `Card`, `Input`, `Sidebar`, …) importada por todo website `uxProvider=Shadcn`
      - src/components/ui/*
...
- Atualiza o target `serve-local` para iniciar automaticamente o servidor MCP ao rodar localmente
...
O gerador:
- Detecta o `uxProvider` do website React (Shadcn aqui) e fornece componentes de chat correspondentes.
...
Como o `agent-serve-local` do Agente de História já depende do `mcp-server-serve-local` do servidor MCP de inventário (via a conexão `story → inventory` acima), e tanto a Game API quanto o servidor MCP agora dependem de `dungeon-db:serve-local`, executar o target `serve-local` de qualquer projeto inicia todas as dependências que ele precisa na ordem correta.
</Aside>
