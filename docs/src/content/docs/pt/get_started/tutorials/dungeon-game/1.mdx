...
:::note[Shadcn UI]
Selecionamos `--uxProvider=Shadcn` para que o website gerado use componentes [shadcn/ui](https://ui.shadcn.com/) estilizados com Tailwind — todo o código de rota usa primitivos shadcn como `Card`, `Button` e `Input`, e o gerador `connection` posteriormente conecta uma superfície de chat [CopilotKit](https://docs.copilotkit.ai/) com tema shadcn correspondente.
:::
...
Este é o ponto de entrada onde o React é montado. Como escolhemos `--uxProvider=Shadcn`, não há `I18nProvider` do Cloudscape — a estilização vem de tokens Tailwind v4 importados via `styles.css`. O `@tanstack/react-router` é configurado no modo de [roteamento baseado em arquivo](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing): enquanto o servidor de desenvolvimento estiver em execução, qualquer arquivo que você criar em `routes/` é detectado automaticamente e a árvore de rotas é regenerada. Geradores posteriores (auth, connection) farão patch AST neste arquivo para envolver `<App />` em providers adicionais.
...
O hook `useGameApi` é diferente do `useStoryAgent` pois não requer um build para que mudanças sejam refletidas, graças ao uso de [inferência de Typescript](https://trpc.io/docs/concepts) pelo tRPC. Isso permite que desenvolvedores façam mudanças no backend que são instantaneamente refletidas no frontend.
</Aside>
...
Vamos conectar nossa Game UI ao Story Agent. Como o agente fala AG-UI, o gerador `connection` conecta o [CopilotKit](https://docs.copilotkit.ai/) em vez de gerar um cliente OpenAPI — você obtém um componente de chat com tema e um `HttpAgent` `@ag-ui/client` pronto para renderizar.
...
- Preenche o `threadId` para o mínimo de 33 caracteres do AgentCore — é por isso que nossa rota do Módulo 4 pode construir com segurança ``{player}-{genre}`` como um thread id estável.
