...
<Aside type="tip" title="Commite Frequentemente">É uma melhor prática garantir que todos os arquivos não stageados estejam commitados no Git antes de executar quaisquer geradores. Isso permite ver o que mudou após executar o gerador via `git diff`.</Aside>
...
- packages/
  - story/
    - dungeon\_adventure\_story/ módulo Python
      - agent/
        - main.py ponto de entrada da aplicação Starlette AG-UI para Bedrock AgentCore Runtime
        - agent.py define o agente Strands e suas ferramentas
        - session.py captura o id de sessão do runtime AgentCore de entrada
...
Vamos analisar os arquivos-chave em detalhes:
...
def get_agent():
    ...
    yield Agent(
        ...
        system_prompt="""
Você é um mago matemático.
Use suas ferramentas para tarefas matemáticas.
...
`agent.py` define um agente Strands de exemplo. Substituiremos o system prompt e as ferramentas pela lógica de dungeon-adventure no <Link path="get_started/tutorials/dungeon-game/3">Módulo 3</Link>.
...
class _LazyStrandsApp:
    """Builds the inner Strands app on first ``/invocations`` request so that
    ``/ping`` health checks stay lightweight and the inbound AgentCore session
    id is captured before any downstream MCP / A2A client reads it."""
...
Como escolhemos `protocol=AG-UI`, o gerador envolve nosso agente Strands em um adaptador [AG-UI](https://ag-ui.com/) exposto via uma aplicação Starlette. O AgentCore roteia cada `runtimeSessionId` para uma microVM dedicada, então um `Agent` de longa duração por container é o tempo de vida correto - `main.py` o constrói preguiçosamente na primeira requisição `/invocations` após capturar o id de sessão de entrada. Quando o armazenamento de sessão gerenciado pelo AgentCore está anexado, `agent.py` conecta um `FileSessionManager` contra `SESSION_STORAGE_PATH` para que o agente possa retomar o histórico completo da conversação após um stop/resume. Não precisaremos modificar `main.py` em nenhum lugar deste tutorial.
...
Este é o ponto de entrada onde o React é montado. Inicialmente apenas configura um `@tanstack/react-router` em uma configuração de [`roteamento baseado em arquivo`](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing) - enquanto o servidor de desenvolvimento estiver em execução, criar um arquivo em `src/routes/` adiciona automaticamente uma rota e regenera `routeTree.gen.ts`. Isso mantém `<Link to="...">` totalmente type-safe.
...
Um componente será renderizado ao navegar para a rota `/`. Como escolhemos `uxProvider=Shadcn`, o gerador também cria um sidebar + app shell em `components/AppLayout` usando primitivos [shadcn/ui](https://ui.shadcn.com/) compartilhados em `packages/common/shadcn`.
...
<Aside title="Hooks com tipos seguros">
O hook `useGameApi` não requer um build para que mudanças sejam refletidas, graças ao uso de [inferência de TypeScript](https://trpc.io/docs/concepts) pelo tRPC. Isso permite que desenvolvedores façam mudanças no backend e as vejam instantaneamente refletidas no frontend.
</Aside>
...
Vamos conectar nossa Game UI ao Story Agent para que o website React possa conversar com o agente através do protocolo AG-UI.
...
Como o Story Agent foi gerado com `protocol=AG-UI`, o gerador `connection` conecta o website usando [CopilotKit](https://copilotkit.ai/) (que fala o protocolo [AG-UI](https://ag-ui.com/)). Estes arquivos são gerados/atualizados:
...
- packages/
  - game-ui/
    - src/
      - components/
        - **AguiProvider.tsx** CopilotKitProvider que registra cada agente AG-UI
        - **copilot/** Componentes de chat CopilotKit temáticos (bolhas de assistente/usuário, input, cursor) - escolhidos automaticamente com base no provedor UX do website (Shadcn aqui)
      - hooks/
        - **useAguiStoryAgent.tsx** Hook por agente que constrói um `HttpAgent` AG-UI autenticado com Cognito direcionado ao runtime AgentCore
      - **main.tsx** Atualizado para adicionar `<AguiProvider>` ao redor do app

</FileTree>

O gerador:
- Adiciona um `AguiProvider` compartilhado (se ainda não existir) que envolve o app no `CopilotKitProvider` do CopilotKit, e registra o hook de cada agente AG-UI
- Gera um hook por agente (`useAguiStoryAgent`) que resolve o ARN do runtime AgentCore do `runtime-config.json`, constrói um `HttpAgent` autenticado, e anexa o cabeçalho `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` em cada requisição
- Fornece primitivos de chat temáticos em `src/components/copilot/` para que o chat pareça nativo ao provedor UX do website

Para mais detalhes, consulte o <Link path="guides/connection/react-py-strands-agent">guia de conexão React para Python Strands Agent</Link>.
</Drawer>
...
Esta mensagem indica que o NX detectou alguns arquivos que podem ser atualizados automaticamente. Neste caso, está se referindo aos arquivos `tsconfig.json` que não têm referências de projeto TypeScript configuradas nos projetos referenciados.
...
<Aside type="tip" title="Shells não interativos">
Se você estiver executando em um shell não interativo (CI ou um ambiente headless), você não verá o prompt - `build` falhará imediatamente com "The workspace is out of sync". Nesse caso, execute <NxCommands commands={['sync']} /> manualmente uma vez e, em seguida, execute o build novamente.
</Aside>
