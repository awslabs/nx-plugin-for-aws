...
O gerador `ts#dynamodb` fornece um target `serve-local` que executa o [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html) em um contêiner. Combinado com o gerador `connection` (que executamos abaixo), isso significa que **o jogo inteiro roda em sua máquina sem um deployment** até o final do tutorial.
:::
