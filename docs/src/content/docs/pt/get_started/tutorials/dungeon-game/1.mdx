...
Como o `agent-serve-local` do Story Agent já depende do `mcp-server-serve-local` do servidor MCP de inventário (via a conexão `story → inventory` acima), e tanto a Game API quanto o servidor MCP agora dependem de `dungeon-db:serve-local`, executar o target `serve-local` de qualquer projeto inicia todas as dependências necessárias na ordem correta.
</Aside>
