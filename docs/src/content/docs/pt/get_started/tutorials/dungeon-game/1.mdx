...
:::note[CDK como Provedor IaC]
Usamos `--iac=cdk` pois usaremos CDK para infraestrutura como código neste tutorial. O Nx Plugin para AWS também suporta `terraform`.
:::
...
- tsconfig.json
- aws-nx-plugin.config.mts configuração para o Nx Plugin para AWS
</FileTree>

<Aside type="tip" title="Faça Commit Frequentemente">É uma boa prática garantir que todos os seus arquivos não staged estejam commitados no Git antes de executar qualquer gerador. Isso permite que você veja o que mudou após executar seu gerador via `git diff`.</Aside>

## Tarefa 2: Estruturar o Dungeon Adventure Game

Com o workspace em vigor, estruturamos os sub-projetos do jogo — a Game API, Story Agent, servidor MCP de Inventário, banco de dados do jogo e website — juntamente com as conexões que os conectam. Existem duas maneiras de fazer isso:
...
O diagrama abaixo _é_ o workspace Dungeon Adventure: cada projeto, componente e conexão que você construirá neste módulo. Clique em **Copy commands** para pegar toda a série e, em seguida, execute-os de dentro do diretório `dungeon-adventure` que você criou na Tarefa 1.

<EmbeddedGraph preset="dungeon-adventure" workspace="dungeon-adventure" iac="cdk" skipWorkspace />

<Drawer title="Step By Step" trigger="Step By Step: execute cada gerador você mesmo e veja o que ele produz">

Em vez de copiar os comandos todos de uma vez, você pode executar cada gerador individualmente. Esta é a melhor maneira de entender o que cada gerador adiciona ao seu workspace. Execute cada gerador por vez, de dentro do diretório `dungeon-adventure` que você criou na Tarefa 1.
...
<details>
<summary>Examine os arquivos gerados do `ts#api` em detalhes</summary>

Abaixo está uma lista de todos os arquivos gerados pelo gerador `ts#api`. Vamos examinar alguns dos arquivos-chave destacados na árvore de arquivos:
<FileTree>
- packages/
  - common/
    - constructs/
      - src/
        ...
        - core/ constructs cdk genéricos
          - api/
            ...
            - trpc-utils.ts utilitários para constructs de API tRPC cdk
  ...
  - game-api/ API tRPC
    - src/
      - client/ cliente vanilla tipicamente usado para chamadas ts máquina a máquina
        - index.ts
...
Todas as definições de schema tRPC são definidas usando [Zod](https://zod.dev/) e são exportadas como tipos typescript via sintaxe `z.TypeOf`.
...
// Tipo union de string para todos os nomes de operação da API
...
> {
  /**
   * Mapa de nomes de operação para suas integrações API Gateway
   */
...
> extends RestApi<Operations, TIntegrations> {
  ...
  /**
   * Cria integrações padrão para todas as operações, que implementam cada operação como
   ...
   *
   * @param grantee - O principal IAM para conceder permissões a
   */
  public grantInvokeAccess(grantee: IGrantable) {
    ...
    // usuários) e recursos (ex: quais caminhos de api podem ser invocados por qual principal) se necessário.
...
Você verá alguns novos arquivos aparecerem na sua árvore de arquivos.
<details>
<summary>Examine os arquivos gerados do `py#project` em detalhes</summary>
...
Isso cria um agente Strands de exemplo e define uma ferramenta de subtração.
...
Isso configura um `AgentRuntimeArtifact` CDK que faz upload da sua imagem Docker do agente para o ECR e a hospeda usando AgentCore Runtime.
...
<details>
<summary>Examine os arquivos gerados do `ts#project` em detalhes</summary>
...
<details>
<summary>Examine os arquivos gerados do `ts#mcp-server` em detalhes</summary>
...
O estado do nosso jogo — jogos salvos e o inventário de cada jogador — reside no [Amazon DynamoDB](https://aws.amazon.com/dynamodb/). Crie um projeto DynamoDB chamado `DungeonDb` com o gerador `ts#dynamodb`:
...
Você verá alguns novos arquivos aparecerem na sua árvore de arquivos.
<details>
<summary>Examine os arquivos gerados do `ts#dynamodb` em detalhes</summary>
...
- packages/
  - dungeon-db/
    - config.json configuração do DynamoDB incluindo porta, nome da tabela, configurações do contêiner e Global Secondary Indexes
    - src/
...
:::note[Shadcn UI]
Selecionamos `--ux=shadcn` para que o website gerado use componentes [shadcn/ui](https://ui.shadcn.com/) estilizados com Tailwind — todo o código de rota usa primitivos shadcn como `Card`, `Button` e `Input`, e o gerador `connection` mais tarde conecta uma superfície de chat [CopilotKit](https://docs.copilotkit.ai/) com tema shadcn correspondente.
:::

Você verá alguns novos arquivos aparecerem na sua árvore de arquivos.

<details>
<summary>Examine os arquivos gerados do `ts#website` em detalhes</summary>

O `ts#website` gera estes arquivos. Vamos examinar alguns dos arquivos-chave destacados na árvore de arquivos:
...
- packages/
  ...
  - game-ui/
    ...
    - src/
      - components/
        - AppLayout/
          - index.tsx layout geral da página usando shadcn `SidebarProvider` + cabeçalho
        - app-sidebar.tsx sidebar shadcn padrão com itens de navegação
...
Este é o ponto de entrada onde o React é montado. A estilização vem de tokens Tailwind v4 importados via `styles.css`. O `@tanstack/react-router` é configurado no modo de [roteamento baseado em arquivo](https://tanstack.com/router/v1/docs/framework/react/routing/file-based-routing): enquanto o servidor de desenvolvimento estiver rodando, qualquer arquivo que você criar em `routes/` é detectado automaticamente e a árvore de rotas é regenerada. Geradores posteriores (auth, connection) farão patch AST neste arquivo para envolver `<App />` em providers adicionais.
...
Um componente será renderizado ao navegar para a rota `/`. O `@tanstack/react-router` gerenciará a `Route` para você sempre que você criar/mover este arquivo (desde que o servidor de desenvolvimento esteja rodando).
...
<details>
<summary>Examine os arquivos gerados do `ts#website#auth` em detalhes</summary>

O gerador `ts#website#auth` atualiza/gera estes arquivos. Vamos examinar alguns dos arquivos-chave destacados na árvore de arquivos:
...
Graças à [inferência Typescript](https://trpc.io/docs/concepts) do tRPC, `useGameApi` não precisa de uma etapa de build para que mudanças no backend cheguem ao frontend — edições no router ou em qualquer procedimento são detectadas pelo verificador de tipo instantaneamente.
</Aside>
...
O arquivo `main.tsx` foi atualizado via uma transformação AST para injetar os providers tRPC.
...
<details>
<summary>Examine os arquivos de conexão Story Agent → Inventory MCP</summary>
    ...
    - agent\_connection/
      - dungeon\_adventure\_agent\_connection/
        - core/
          ...
          - auth/ `httpx.Auth` independente de framework para SigV4 / encaminhamento de sessão
        - app/
...
- Atualiza o target `dev` para iniciar automaticamente o servidor MCP ao rodar localmente

Para mais detalhes, consulte o <Link path="guides/connection/py-agent-mcp">guia de conexão Python Agent para MCP</Link>.
</details>

###### UI do Jogo: Conectar ao Story Agent

Vamos conectar nossa Game UI ao Story Agent. Como o agente fala AG-UI, o gerador `connection` conecta o [CopilotKit](https://docs.copilotkit.ai/): um componente de chat com tema e um `HttpAgent` `@ag-ui/client` pronto para renderizar.
...
<details>
<summary>Examine os arquivos de conexão UI → Story Agent</summary>
...
- packages/
  - game-ui/
    - src/
      - components/
        - **AguiProvider.tsx** `CopilotKitProvider` com todos os agentes AG-UI conectados registrados
        - copilot/
      ...
      - hooks/
        - **useAguiStoryAgent.tsx** Constrói um `HttpAgent`, injeta o token bearer Cognito e preenche `threadId` para o session id de 33 caracteres do AgentCore
...
- Registra todos os agentes conectados em um único `CopilotKitProvider` — executar novamente para outro agente apenas adiciona outro hook.
- Lê o ARN de runtime do agente da Configuração de Runtime, constrói a URL de invocação do AgentCore e anexa o token bearer Cognito mais o cabeçalho de session id do AgentCore.
...
Como o `agent-dev` do Story Agent já depende do `mcp-server-dev` do servidor MCP de Inventário (via a conexão `story → inventory` acima), e tanto a Game API quanto o servidor MCP agora dependem de `dungeon-db:dev`, executar o target `dev` de qualquer projeto inicia todas as dependências que ele precisa na ordem correta.
</Aside>
...
<details>
<summary>Examine os arquivos gerados do `ts#infra` em detalhes</summary>
...
Vamos instanciar nossos constructs CDK para construir nosso jogo de aventura de dungeon.
...
O comando `run-many` executará um target em múltiplos sub-projetos listados (`--all` os seleciona todos). Isso garante que as dependências sejam executadas na ordem correta.

Você também pode disparar um build (ou qualquer outra tarefa) para um target de projeto único executando o target diretamente no projeto. Por exemplo, para fazer build do projeto `@dungeon-adventure/infra`, execute o seguinte comando:
...
O Nx depende de [caching](https://nx.dev/concepts/how-caching-works) para que você possa reutilizar artefatos de builds anteriores e acelerar o desenvolvimento. É necessária alguma configuração para que isso funcione corretamente e pode haver casos onde você queira executar um build **sem usar o cache**. Para fazer isso, simplesmente adicione o argumento `--skip-nx-cache` ao seu comando. Por exemplo:
...
Selecione a opção **Yes, sync the changes and run the tasks** para prosseguir. Você deve notar que todos os seus erros de importação relacionados à IDE são resolvidos automaticamente, pois o gerador de sincronização adicionará as referências typescript faltantes automaticamente!
