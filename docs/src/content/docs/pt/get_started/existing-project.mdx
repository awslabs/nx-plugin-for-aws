...
- package.json scripts de conveniência para tarefas comuns, além das devDependencies `@aws/nx-plugin`, `nx`, `@nx/js`, `@nx/workspace`, `typescript` e Biome
- biome.json configuração padrão de formatador e linter do Biome
- .mcp.json configura o servidor MCP para agentes de codificação suportados a menos que desabilitado (também equivalentes .cursor/, .kiro/, .gemini/, .vscode/ e .codex/). Ele executa o bin `aws-nx-mcp` do próprio `@aws/nx-plugin` do workspace, então o servidor sempre corresponde à versão do plugin instalada
</FileTree>
...
, altere o `type` raiz — mas essa é uma migração do seu código existente, não algo que `init` faz por você.
</Aside>
