...
Este gerador cria infraestrutura como código CDK ou Terraform com base no `iac` selecionado. Você pode usar isso para implantar sua função.
...
Certifique-se de que a fonte de evento corresponda à opção `event` selecionada para garantir que o evento seja processado corretamente dentro de sua função handler.
:::
