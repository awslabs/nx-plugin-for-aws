...
:::note[Correspondência de origem de eventos]
...
O módulo de função prepara seu zip de implantação em um bucket S3 de ativos compartilhado em vez de fazer upload inline via a API do Lambda (que tem um limite de requisição de 70 MB). Instancie o módulo `core/asset-bucket` uma vez por implantação e passe sua saída `bucket_name` para cada módulo lambda / API via a entrada `asset_bucket_name`:
...
O módulo `core/asset-bucket` provisiona um bucket S3 com criptografia gerenciada pelo S3 (AES256), versionamento, uma política de bucket somente TLS, bloqueio de acesso público e registro de acesso S3 habilitado (os logs são entregues a um bucket de logs de acesso irmão). Reutilize a mesma instância do módulo em todos os módulos Lambda / API em sua implantação — cada chamador escreve sob um prefixo de chave com escopo de módulo, portanto não há colisões.
