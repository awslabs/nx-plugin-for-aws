...
:::tip
Usamos o target `dev` aqui (um alias para `serve-local`), que também inicia servidores locais para APIs conectadas via `connection` e recarrega automaticamente se houver mudanças no website, modelo ou backend! Isso permite testar a API e o website localmente antes mesmo de escrever código CDK.
:::
