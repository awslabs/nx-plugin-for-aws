...
Este tipo de projeto utilizava [`create-react-app`](https://github.com/facebook/create-react-app), que está agora depreciado. Para migrar o website neste guia, usaremos o <Link path="/guides/react-website">gerador `ts#website`</Link>, que utiliza tecnologias mais modernas e suportadas, como [Vite](https://vite.dev/).
...
O gerador de website React acima não inclui autenticação Cognito por padrão como o `CloudscapeReactTsWebsiteProject`. Em vez disso, isso é adicionado explicitamente via o <Link path="/guides/react-website-auth">gerador `ts#website#auth`</Link>.
...
Como estamos usando [roteamento baseado em arquivo](https://tanstack.com/router/latest/docs/framework/react/routing/file-based-routing), podemos usar o servidor de desenvolvimento local do website para gerenciar automaticamente a geração da configuração de rotas.
...
:::tip
Usamos o target `dev` aqui, que também inicia servidores locais para quaisquer APIs conectadas via `connection` e recarrega automaticamente se houver mudanças no website, modelo ou backend! Isso permite testar a API e o website localmente antes mesmo de escrever código CDK.
:::
...
1. Importe o novo cliente gerado em vez do antigo:
    ...
    Note também que importamos os hooks relevantes diretamente do `@tanstack/react-query`, pois o cliente gerado fornece métodos para gerar opções para hooks do TanStack Query, em vez de wrappers de hooks.
