...
Se você selecionou `agentcore` para `infra`, a infraestrutura CDK ou Terraform relevante é gerada, que você pode usar para implantar seu servidor MCP no [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/agents-tools-runtime.html).
...
Passe as saídas do módulo compartilhado <Link path="/guides/runtime-config#writing-configuration">`runtime_config_appconfig`</Link> para o módulo do servidor MCP:
...
[Docker](https://www.docker.com/) é necessário para construir e implantar seu servidor MCP. Certifique-se de ter instalado e iniciado o Docker para evitar erros como:
