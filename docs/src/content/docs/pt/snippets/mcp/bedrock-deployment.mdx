...
Você pode conceder acesso para invocar seu servidor MCP no Bedrock AgentCore Runtime usando o método `grantInvokeAccess`. Por exemplo, você pode desejar que um agente gerado com o gerador <Link path="/pt/guides/py-agent">`py#agent`</Link> chame seu servidor MCP:
