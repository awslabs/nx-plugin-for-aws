...
Você pode conceder acesso para invocar seu servidor MCP no Bedrock AgentCore Runtime usando o método `grantInvokeAccess`. Por exemplo, você pode permitir que um agent gerado com o gerador <Link path="/guides/py-agent">`py#agent`</Link> chame seu servidor MCP:
