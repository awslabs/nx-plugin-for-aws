...
Implante as funções Lambda da API em **sub-redes privadas com egress**, não em sub-redes privadas isoladas. `RUNTIME_CONFIG_APP_ID` deve apontar para a <Link path="guides/runtime-config">configuração de tempo de execução</Link> compartilhada do AppConfig, não para uma saída do módulo do banco de dados. Inclua o namespace `database` ao instanciá-lo para que a entrada de configuração de tempo de execução do módulo do banco de dados seja implantada:
