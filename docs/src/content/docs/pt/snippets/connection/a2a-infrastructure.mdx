...
O ARN do runtime AgentCore do agente remoto é automaticamente registrado no namespace `connection` da <Link path="guides/runtime-config">Runtime Configuration</Link> pelo construto CDK gerado, para que o agente host possa descobri-lo em tempo de execução.
</Fragment>
...
O ARN do runtime AgentCore do agente remoto é automaticamente registrado no namespace `connection` da <Link path="guides/runtime-config">Runtime Configuration</Link> pelo módulo Terraform gerado, para que o agente host possa descobri-lo em tempo de execução.
</Fragment>
</Infrastructure>