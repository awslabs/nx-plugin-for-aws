...
<Fragment slot="terraform">
A configuração de VPC já é suportada pelo módulo gerado — defina `enable_vpc` junto com `vpc_id` e `subnet_ids`, e o módulo implanta a função Lambda em sua VPC atrás de um grupo de segurança que ele cria para você:
...
Para opções que o módulo não expõe, edite o recurso `aws_lambda_function` no módulo Terraform gerado diretamente.
</Fragment>
</Infrastructure>
