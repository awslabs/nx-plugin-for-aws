...
O suporte do gerador atualmente varia por backend:

- `ts#trpc-api` suporta tanto `isolated` quanto `shared` para `ServerlessApiGatewayRestApi` e `ServerlessApiGatewayHttpApi` através da opção `integrationPattern`.
- `py#fast-api` e `ts#smithy-api` atualmente geram `isolated`.

Você também pode alternar um construto CDK de API gerado existente entre os dois padrões editando o campo `pattern` em seu método `defaultIntegrations`:

```ts {3}
return IntegrationBuilder.rest({
  // Altere isto para 'isolated' ou 'shared'
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Os módulos Terraform já usam o padrão de roteador por padrão, com uma única função Lambda processando todas as operações da API.
</Fragment>
</Infrastructure>
...
#### Padrão de Roteador Personalizado

<Infrastructure>
<Fragment slot="cdk">
Se você precisa personalizar o padrão de roteador além dos padrões gerados, pode livremente editar o método `defaultIntegrations` de sua API para configurar as integrações da forma que preferir:
