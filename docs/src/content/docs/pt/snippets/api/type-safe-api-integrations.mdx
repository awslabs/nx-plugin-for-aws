...
Se você especificar `environment` em suas opções padrão, isso irá __sobrescrever__ quaisquer variáveis de ambiente que estejam definidas dentro do seu construto de API gerado. Geralmente é preferível usar o método `addEnvironment` para funções Lambda. Você pode facilmente conseguir isso iterando através de suas integrações:
