...
Em seguida, registre-a em `server.ts`:
...
### Definindo Escopo de Dados de Ferramenta por Chamador

`createServer` recebe um `McpRequestContext` descrevendo a requisição atual. Sobre Streamable HTTP, isso inclui o id de sessão do AgentCore Runtime do chamador, que um <Link path="guides/ts-agent">agente</Link> encaminha automaticamente em cada chamada MCP de saída.

Passe-o para uma ferramenta que armazena dados por chamador:
...
:::caution
Defina o escopo dos dados armazenados no contexto da requisição em vez de em um argumento da ferramenta. Um modelo pode ser persuadido a passar qualquer valor que desejar, então argumentos como um nome de usuário ou jogador não são um limite de confiança. O contexto está ausente sobre STDIO, onde não há requisição.
:::
