...
Gerar um servidor TypeScript do [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) para fornecer contexto a Modelos de Linguagem de Grande Porte (LLMs), e opcionalmente implantá-lo no [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
<FileTree>
  - your-project/
    ...
    - package.json Atualizado com entrada bin e dependências do MCP
...
:::tip
A imagem docker é construída usando uma tag (por exemplo `my-scope-my-project-mcp-server:latest`), da qual o construct CDK `Dockerfile` herda, permitindo que seu `Dockerfile` fique colocado junto com seu projeto de servidor MCP.
:::
