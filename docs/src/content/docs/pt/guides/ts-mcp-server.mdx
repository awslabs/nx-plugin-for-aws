...
:::tip
Seu `Dockerfile` fica colocado junto ao projeto do servidor MCP. Para CDK, o constructo usa `buildContexts` para construir a imagem diretamente deste Dockerfile. Para Terraform, a imagem docker é pré-construída usando uma tag (por exemplo `my-scope-my-project-mcp-server:latest`) que é referenciada pela sua infraestrutura Terraform.
:::
