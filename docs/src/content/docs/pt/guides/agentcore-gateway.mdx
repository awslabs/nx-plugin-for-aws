...
O gerador adiciona um alvo `<name>-serve-local` ao projeto Gateway. Este alvo é um agregador contínuo: quando conectado a servidores MCP via o <Link path="guides/connection/agentcore-gateway-mcp">gerador `agentcore-gateway#mcp-connection`</Link>, executá-lo inicia todos os servidores MCP anexados juntos para multiplexação local.
