...
Passe o ARN da função como uma variável de template (veja <a href="#template-variables">Variáveis de template</a> abaixo) em vez de codificá-lo diretamente ou usar correspondência de padrões. Execute re-synth ou re-plan para implantar a nova política.
...
Adicione novas variáveis onde as políticas são renderizadas, por exemplo, para passar o ARN da função de execução de um agente para o exemplo `ts-agent-divide.cedar` acima:

<Infrastructure>
<Fragment slot="cdk">
Em `packages/common/constructs/src/app/gateways/<name>/<name>.ts`, adicione ao objeto passado para `ejs.render`:
...
O atributo `id` do principal contém o ARN IAM do chamador. Os chamadores geralmente chegam como ARNs de assumed-role do STS com um sufixo de sessão gerado, então passe o ARN exato da função como uma variável de template e combine o sufixo de sessão com `like`:
