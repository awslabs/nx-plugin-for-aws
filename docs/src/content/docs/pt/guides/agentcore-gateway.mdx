...
<FileTree>
  - packages/\<name>/
    ...
    - package.json Declara as dependências que `local-dev.ts` importa
    - tsconfig.json Configuração TypeScript usada pelo alvo `typecheck`
    - project.json Adiciona os alvos `serve`, `dev`, `build`, `lint`, `format` e `typecheck`
</FileTree>
...
Selecione `cognito` quando o Gateway frontaliza um agente `auth: cognito`: o Gateway encaminha o token do chamador para o runtime do agente, então ele precisa ter recebido um. Veja <Link path="guides/connection/agentcore-gateway-agent">Gateway para Agente</Link>.
