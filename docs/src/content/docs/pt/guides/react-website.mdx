...
`$id` no caminho `to` é um [parâmetro de caminho](https://tanstack.com/router/latest/docs/framework/react/guide/path-params), fornecido através de `params`. Troque o `<button>` simples pelo componente de botão do seu kit de UX conforme necessário.
...
O target `checkov` passa `--config-file`, o que desativa a descoberta própria do `checkov` de um `.checkov.yaml` — um arquivo de configuração colocado em qualquer outro lugar é ignorado.
:::
...
Seu site é gerado com um `RuntimeConfigProvider` (em `src/components/RuntimeConfig`) que carrega `runtime-config.json` antes de renderizar sua aplicação, e um hook `useRuntimeConfig` para ler valores dele:
...
O provider retorna a uma configuração vazia quando nenhum `runtime-config.json` é servido, então um site sem nada implantado ainda é renderizado. `IRuntimeConfig` é tipado como `any` por padrão — restrinja-o em `src/components/RuntimeConfig/index.tsx` se você quiser acesso type-safe.
