...
A configuração de sua infraestrutura é fornecida ao seu website via <Link href="guides/runtime-config">Configuração de Runtime</Link>. Isso permite que seu website acesse detalhes como URLs de API que só são conhecidos após a implantação da aplicação.
...
:::note
Para detalhes sobre como a configuração de runtime é armazenada no AWS AppConfig e como consumidores do lado do servidor (funções Lambda, agentes) podem recuperá-la, consulte o guia de <Link path="guides/runtime-config">Configuração de Runtime</Link>.
:::
