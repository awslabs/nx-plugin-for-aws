...
<OptionFilter when={{ uxProvider: 'Cloudscape' }} description="Cloudscape-specific note">
:::note[Sistema de design Cloudscape]
Este gerador configura o [Cloudscape](http://cloudscape.design/) por padrão, mas você pode selecionar `None` como `uxProvider` se preferir usar uma biblioteca de componentes ou sistema de design diferente.
:::
</OptionFilter>

<OptionFilter when={{ uxProvider: 'Shadcn' }} description="Shadcn-specific note">
:::note[shadcn/ui]
Você selecionou [shadcn/ui](https://ui.shadcn.com/) como seu `uxProvider`. Componentes e estilos gerados ficam ao lado do TanStack Router / Vite — consulte a documentação do Shadcn para adicionar mais componentes.
:::
</OptionFilter>

<OptionFilter when={{ uxProvider: 'None' }} description="No UX provider — bring your own component library">
:::note[Sem Provedor UX]
Você selecionou `None` para `uxProvider`, então nenhuma biblioteca de componentes ou sistema de design está configurado. Você é livre para adicionar a biblioteca de componentes de sua escolha.
:::
</OptionFilter>
...
Quando executado neste modo e nenhum `runtime-config.json` estiver presente, se você configurou Autenticação Cognito (via gerador <Link path="/guides/react-website-auth">`ts#react-website#auth`</Link>), o login será ignorado e requisições para servidores locais não incluirão headers de autenticação.
