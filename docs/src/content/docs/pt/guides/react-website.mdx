...
#### Domínio Personalizado & TLS

Por padrão, a distribuição usa o nome de domínio padrão do CloudFront (`*.cloudfront.net`) e seu certificado padrão, que não suporta a imposição de uma versão mínima de TLS de 1.2. Para servir seu website a partir do seu próprio domínio, forneça um [certificado ACM](https://docs.aws.amazon.com/acm/latest/userguide/acm-overview.html) (que deve residir em `us-east-1` para uso com CloudFront) e seus nomes de domínio — uma versão mínima de TLS de 1.2 é então imposta para os visualizadores:

<Infrastructure>
<Fragment slot="cdk">
Passe as props `certificate` e `domainNames` no construct do website gerado em `packages/common/constructs/src/app/static-websites`:
...
<Fragment slot="terraform">
Defina as variáveis `custom_domain_names` e `acm_certificate_arn` no módulo do website gerado em `packages/common/terraform/src/app/static-websites`:
...
O gerador de website React cria infraestrutura como código CDK ou Terraform baseado no `iac` selecionado. Você pode usar isso para implantar seu website.
...
<Fragment slot="terraform">
Para implantar seu website, recomendamos usar o gerador <Link path="/guides/terraform-project">`terraform#project`</Link> para criar um projeto Terraform.
