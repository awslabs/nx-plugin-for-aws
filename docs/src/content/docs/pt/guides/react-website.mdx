...
:::warning
Quando executado neste modo e nenhum `runtime-config.json` estiver presente, se você configurou Autenticação Cognito (via <Link path="/guides/react-website-auth">gerador CloudScape Website Auth</Link>), o login será ignorado e requisições para servidores locais não incluirão headers de autenticação.
