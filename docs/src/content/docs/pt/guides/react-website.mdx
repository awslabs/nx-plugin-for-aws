...
O target `serve-local` inicia um servidor de desenvolvimento local para seu website (com [Vite `MODE`](https://vite.dev/guide/env-and-mode) definido como `serve-local`), além de iniciar quaisquer servidores locais para APIs conectadas via gerador <Link path="/guides/connection">Connection</Link>.
...
:::note[Script `dev`]
Se nenhum script `dev` existir no `package.json` raiz quando este gerador for executado, um script `dev` é adicionado que invoca `serve-local` para o website gerado, o que significa que você também pode iniciar o servidor de desenvolvimento local com:

<PackageManagerShortCommand commands={["dev"]} />

Isso torna o primeiro website React que você gera o target `dev` padrão para o workspace. Websites subsequentes não sobrescrevem o script `dev` existente — você pode invocar seus targets `serve-local` diretamente ou atualizar o script manualmente.
:::
