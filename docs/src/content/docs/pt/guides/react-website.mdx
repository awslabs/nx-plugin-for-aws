...
O target `serve-local` (com alias `dev`) inicia um servidor de desenvolvimento local para seu website (com [Vite `MODE`](https://vite.dev/guide/env-and-mode) definido como `serve-local`), além de iniciar quaisquer servidores locais para APIs que você conectou ao seu website via <Link path="/guides/connection">gerador Connection</Link>.
...
:::tip[Comportamento do Serve-Local]
Ao executar com `serve-local`, você pode especificar quaisquer variáveis de ambiente necessárias para suas APIs apontarem para outros recursos AWS implantados, por exemplo:
