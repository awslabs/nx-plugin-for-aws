...
Quando `protocol` está definido como `A2A`, o ponto de entrada usa o [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent) em vez do tRPC:
...
O `A2AAgent` do SDK Strands envolve um endpoint A2A remoto para que você possa chamá-lo como um `Agent` local. `invoke` resolve para um `AgentResult`; chamar `.toString()` nele junta os blocos de texto da resposta do agente remoto.
...
As solicitações para um agente A2A implantado no AgentCore (incluindo a busca do cartão do agente) devem ser assinadas com SigV4. Passe um `clientFactory` para `A2AAgent` que usa um fetch com assinatura SigV4 tanto para o resolvedor de cartão quanto para o transporte JSON-RPC:
...
:::note[Permissões IAM]
A função IAM do chamador precisa de `bedrock-agentcore:InvokeAgentRuntime` e `bedrock-agentcore:GetAgentCard` no ARN do runtime de destino. O helper `grantInvokeAccess` do construto gerado concede ambos.
:::

:::note[Dependências extras]
O exemplo de cliente SigV4 acima requer `@a2a-js/sdk`, `aws4fetch` e `@aws-sdk/credential-providers`. Adicione-os ao seu `package.json` antes de executar o exemplo.
:::

#### Usando um Agente A2A como Ferramenta

`A2AAgent` implementa a mesma interface `InvokableAgent` que um `Agent` local, então você pode envolvê-lo em uma `tool` Strands e passá-lo para outro agente. É assim que o template de agente HTTP/tRPC gerado delega para um agente A2A remoto:

```typescript
import { Agent, tool } from '@strands-agents/sdk';
import { z } from 'zod';

const remote = /* A2AAgent construído com clientFactory como acima */;
...
Em sua pilha CDK, conceda ao agente host acesso de invocação ao agente remoto — isso conecta ambas as ações IAM que o cliente A2A precisa:
