...
O gerador configura um destino Nx `<your-agent-name>-chat` que depende de `<your-agent-name>-serve-local`. Executá-lo inicia o agente localmente e o coloca em um chat de terminal interativo alimentado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Para agentes **A2A**, o destino simplesmente executa `agent-chat-cli` como um binário — nenhum código é fornecido em seu workspace.

Para agentes **HTTP** (tRPC sobre WebSocket), o gerador emite um pequeno `scripts/<your-agent-name>/chat.ts` que envolve o `<Agent>Client.local({ url })` gerado como um `ChatAdapter` e o passa para o `chatLoop` do `agent-chat-cli`.
