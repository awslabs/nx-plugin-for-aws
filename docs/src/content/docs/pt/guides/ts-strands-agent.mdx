...
O gerador também fornece uma CLI de chat interativa mínima em `scripts/<your-agent-name>/chat.ts` e um destino correspondente `<your-agent-name>-chat`. Este destino depende de `<your-agent-name>-serve-local`, então executá-lo inicia o agente localmente e o coloca em um chat de terminal interativo com ele:

<NxCommands commands={['run your-project:agent-chat']} />

A CLI usa o cliente apropriado para o protocolo (`AgentClient.local` para HTTP/tRPC ou `@a2a-js/sdk` para A2A) e transmite as respostas do agente token por token via [`@clack/prompts`](https://github.com/bombshell-dev/clack).
