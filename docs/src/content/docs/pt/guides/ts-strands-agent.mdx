...
Por padrão, seu Strands Agent será protegido usando autenticação IAM, simplesmente implante-o sem nenhum argumento:
...
Quando você seleciona autenticação `Cognito`, o gerador cria automaticamente o construto `UserIdentity` e configura o agente para usar o Cognito para autenticação.
...
O construto `UserIdentity` é gerado automaticamente quando você seleciona autenticação `Cognito`.
</Fragment>
<Fragment slot="terraform">
