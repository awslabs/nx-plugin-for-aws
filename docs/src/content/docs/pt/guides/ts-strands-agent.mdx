...
:::caution[AG-UI ainda não suportado para TypeScript]
O [protocolo AG-UI](https://docs.ag-ui.com/) ainda não é suportado para Strands Agents TypeScript. Se você precisa de suporte AG-UI, por favor use o gerador <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> com `protocol=AG-UI`. Consulte [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347) para o status do suporte AG-UI para TypeScript.
:::
...
O gerador configura um destino chamado `<your-agent-name>-invoke` que inicia um chat interativo com seu agente a partir do terminal. Por padrão, ele se conecta ao servidor local:

<NxCommands commands={['agent-invoke your-project']} />

Para conversar com um agente implantado, defina `RUNTIME_CONFIG_APP_ID` como o ID da aplicação AppConfig do seu workspace — o CLI resolve o ARN do runtime e invoca o endpoint implantado:
...
O script CLI está localizado em `scripts/<agent>/cli.ts` e seleciona automaticamente o protocolo correto (HTTP/A2A) para seu agente.
