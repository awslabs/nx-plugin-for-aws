...
:::tip
Seu `Dockerfile` é co-localizado com seu projeto Strands Agent. Para CDK, o construto usa `buildContexts` para construir a imagem diretamente a partir deste Dockerfile. Para Terraform, a imagem docker é pré-construída usando uma tag (por exemplo `my-scope-my-project-agent:latest`) que é referenciada pela sua infraestrutura Terraform.
:::
