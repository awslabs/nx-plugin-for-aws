...
Para invocar seu Strands Agent a partir de um site React, você pode fazer uso do <Link path="/guides/connection/react-ts-agent">gerador `connection`</Link>, que configura automaticamente um cliente tRPC WebSocket com a autenticação correta (IAM ou Cognito).
