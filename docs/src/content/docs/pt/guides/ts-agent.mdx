...
**1. Crie sessões através da sua API**

Adicione um procedimento que inicia uma conversa. Ele gera o ID da sessão por si mesmo, resolve o chamador de seu próprio contexto de solicitação e armazena o par — por exemplo, em uma tabela criada com o gerador <Link path="guides/ts-dynamodb">`ts#dynamodb`</Link>:
...
Resolva o chamador do contexto de solicitação verificado, em vez de um campo de entrada, para que não possa ser fornecido pelo navegador. <Link path="guides/trpc">Implementing Custom Middleware</Link> mostra como fazer isso para APIs autenticadas com IAM e Cognito.
...
**2. Valide a propriedade no agente**

Seu agente já recebe o ID da sessão e, com `auth=cognito`, também recebe o JWT do chamador — o autorizador do AgentCore Runtime o valida e a infraestrutura gerada permite que o cabeçalho `Authorization` passe para seu código. Procure a sessão, compare seu proprietário com o chamador e recuse a solicitação se eles diferirem:
