...
Isso usa `tsx --watch` para reiniciar automaticamente o servidor quando os arquivos mudam. O agente estará disponível em `http://localhost:8081` (ou a porta atribuída se você tiver vários agentes — leia-a de `metadata.ports` no `project.json` do projeto).
...
Em seguida, invoque-o usando o método de fábrica `.local` da fábrica de cliente.
...
Substitua a porta atribuída ao seu agente — leia-a de `metadata.ports` no `project.json` do projeto.
...
Use `<your-agent-name>-dev` para isso, não `<your-agent-name>-serve`. `-serve` executa o caminho de código implantado, então precisa de `RUNTIME_CONFIG_APP_ID` definido — veja <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Sem ele, toda invocação falha com `RUNTIME_CONFIG_APP_ID is not set`.
:::
