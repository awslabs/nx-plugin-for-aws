...
Primeiro use o gerador <Link path="/pt/guides/typescript-project">`ts#project`</Link> para criar um projeto ao qual adicionar seu Agent.
:::
...
Para consumir Servidores MCP que você criou usando os geradores <Link path="/pt/guides/py-mcp-server">`py#mcp-server`</Link> ou <Link path="/pt/guides/ts-mcp-server">`ts#mcp-server`</Link>, você pode usar o <Link path="/pt/guides/connection/ts-agent-mcp">gerador `connection`</Link>.

<RunGenerator generator="connection" />

Consulte o <Link path="/pt/guides/connection/ts-agent-mcp">guia do gerador `connection`</Link> para detalhes sobre como a conexão é configurada.
...
A maioria dos usuários não precisará modificar este arquivo — edite `agent.ts` para alterar ferramentas ou o prompt do sistema. Agentes A2A escutam na porta `9000` (vs `8080` para HTTP), para a qual o Dockerfile e a infraestrutura gerados já estão configurados.
</OptionFilter>
...
Agentes AG-UI são projetados para serem consumidos diretamente por um frontend. Use o <Link path="/pt/guides/connection/react-agui">gerador `connection`</Link> para conectar seu site React ao agente com um provedor [CopilotKit](https://docs.copilotkit.ai/aws-strands) e cliente [AG-UI HttpAgent](https://docs.ag-ui.com/).

A maioria dos usuários não precisará modificar `index.ts` — edite `agent.ts` para alterar ferramentas ou o prompt do sistema. Agentes AG-UI escutam na porta `8080` (igual ao HTTP), para a qual o Dockerfile e a infraestrutura gerados já estão configurados.
</OptionFilter>
...
Para agentes autenticados por Cognito, forneça um token de acesso Cognito via variável de ambiente `AGENT_ACCESS_TOKEN`, que é enviado como um bearer token:
...
Você pode encontrar rastreamentos no Console AWS do CloudWatch, selecionando "GenAI Observability" no menu. Observe que para que os rastreamentos sejam preenchidos, você precisará habilitar [Transaction Search](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Transaction-Search.html).
...
O `accessTokenProvider` deve retornar o token usado para autenticar a solicitação. Você pode, por exemplo, obter um token dentro deste método para garantir que credenciais novas sejam reutilizadas quando o tRPC reiniciar uma conexão WebSocket. O exemplo abaixo demonstra o uso do AWS SDK para obter o token do Cognito:
...
Para invocar seu Agent a partir de um site React, você pode usar o <Link path="/pt/guides/connection/react-ts-agent">gerador `connection`</Link>, que configura automaticamente um cliente tRPC WebSocket com a autenticação correta (IAM ou Cognito).

<RunGenerator generator="connection" />

Consulte o <Link path="/pt/guides/connection/react-ts-agent">guia do gerador `connection`</Link> para detalhes sobre como a conexão é configurada.
</OptionFilter>

<OptionFilter when={{ protocol: 'a2a' }} description="A2A delegation details">
### Invocando um Agent A2A como Ferramenta

Para delegar trabalho deste agente para um agente A2A remoto (seja <Link path="/pt/guides/ts-agent">TypeScript</Link> ou <Link path="/pt/guides/py-agent">Python</Link>), use o <Link path="/pt/guides/connection/ts-agent-a2a">gerador `connection`</Link>. Ele fornece um cliente autenticado por SigV4 para o agente de destino e transforma por AST o `agent.ts` deste agente para registrar o agente A2A remoto como uma `tool` Strands.

<RunGenerator generator="connection" />

Consulte o <Link path="/pt/guides/connection/ts-agent-a2a">guia do gerador `connection`</Link> para detalhes sobre como a conexão é configurada.
</OptionFilter>

<OptionFilter when={{ protocol: 'ag-ui' }} description="AG-UI / React connection details">
### Invocando um Agent AG-UI

Para invocar seu agente AG-UI a partir de um site React, use o <Link path="/pt/guides/connection/react-agui">gerador `connection`</Link>, que conecta um cliente [CopilotKit](https://docs.copilotkit.ai/aws-strands) configurado para seu agente implantado com a autenticação correta (IAM ou Cognito).

<RunGenerator generator="connection" />

Consulte o <Link path="/pt/guides/connection/react-agui">guia do gerador `connection`</Link> para detalhes sobre como a conexão é configurada.
</OptionFilter>
