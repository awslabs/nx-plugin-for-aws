...
Se você selecionou `None` para `computeType`, nenhum construto CDK ou módulo Terraform é gerado — o Agent só pode ser executado localmente. A opção `auth` é ignorada neste modo, pois não há endpoint hospedado para autenticar.
</OptionFilter>
