...
Para executar seu Agent (e tudo conectado a ele) localmente, use o destino `dev` do projeto:

<NxCommands commands={['dev your-project']} />

Se você adicionou múltiplos componentes ao seu projeto (agentes, servidores MCP, etc.), isso inicia todos eles. Para executar apenas este agente, direcione seu destino `<your-agent-name>-dev`:

<NxCommands commands={['agent-dev your-project']} />

Isso usa `tsx --watch` para reiniciar automaticamente o servidor quando os arquivos mudam. O agente estará disponível em `http://localhost:8081` (ou a porta atribuída se você tiver múltiplos agentes).

### Chat com seu Agente

O gerador configura um destino Nx `<your-agent-name>-chat` que o coloca em um chat de terminal interativo com seu agente.

O destino de chat é executado de forma independente. Por padrão, ele se conecta ao seu agente em execução local, então inicie o destino `<your-agent-name>-dev` do agente primeiro (em um terminal separado):
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="Coloque um agent na frente com um AgentCore Gateway como destino de tempo de execução"
