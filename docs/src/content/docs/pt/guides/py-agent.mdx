...
O ID da sessão chega do chamador, então por si só identifica uma conversa, mas não a quem a conversa pertence. O AgentCore Runtime autoriza uma invocação contra o ARN do recurso do runtime do agente, em vez de contra uma sessão individual, o que deixa o agente livre para decidir o que uma sessão significa para sua aplicação.

Se você deseja que cada usuário acesse apenas suas próprias conversas, registre quem possui cada sessão em sua própria aplicação e verifique essa propriedade na entrada. Um ID de sessão é então algo que sua aplicação emite, em vez de algo que o chamador escolhe.

**1. Criar sessões através da sua API**

Adicione um procedimento que inicia uma conversa. Ele gera o ID da sessão por si mesmo, resolve o chamador a partir de seu próprio contexto de requisição e armazena o par — por exemplo, em uma tabela criada com o gerador <Link path="guides/py-dynamodb">`py#dynamodb`</Link>:

| Atributo | Exemplo | Notas |
| --- | --- | --- |
...
A API pode estar em qualquer linguagem; o exemplo acima usa o gerador <Link path="guides/trpc">`ts#trpc-api`</Link>.

Resolva o chamador a partir do contexto de requisição verificado, em vez de um campo de entrada, para que não possa ser fornecido pelo navegador. <Link path="guides/trpc">Implementing Custom Middleware</Link> mostra como fazer isso para APIs autenticadas por IAM e Cognito.

Um segundo procedimento consultando esse índice permite que a UI ofereça a um usuário suas próprias conversas sem nunca ver as de outra pessoa.

**2. Validar propriedade no agente**

Seu agente já recebe o ID da sessão e, com `auth=cognito`, também recebe o JWT do chamador — o autorizador do AgentCore Runtime o valida e a infraestrutura gerada permite que o cabeçalho `Authorization` passe para seu código. Procure a sessão, compare seu proprietário com o chamador e recuse a requisição se eles diferirem:
...
Decodificar a reivindicação sem verificar sua assinatura é suficiente aqui, já que o AgentCore Runtime validou o token durante a autorização de entrada.

:::tip[Por que validar no agente e também na API]
A API decide quem possui uma sessão; o agente a aplica. Verificar em ambos os lugares significa que um ID de sessão que vaza — copiado de um navegador ou reutilizado de uma conversa antiga — ainda é recusado na próxima invocação, e qualquer cliente futuro do agente herda a mesma regra.
:::

:::caution[A sessão também seleciona a microVM]
Um ID de sessão seleciona a [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html) que atende a requisição, então seu sistema de arquivos, `/tmp` e caches em processo são compartilhados por cada requisição que resolve para essa sessão. Isso merece atenção se seu agente ou suas ferramentas gravam no disco local: dois chamadores que enviam o mesmo ID de sessão compartilham esses recursos mesmo quando suas conversas armazenadas são separadas.

Gerar IDs de sessão opacos em sua API evita isso, porque um chamador não pode chegar a uma sessão que não lhe foi dada. Derivar um de valores fornecidos pelo usuário — um nome de conversa, um e-mail, um nome de jogo — é previsível e permite que dois chamadores colidam.
:::
