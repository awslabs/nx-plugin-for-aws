...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain fornece um grande ecossistema de [ferramentas e integrações](https://docs.langchain.com/oss/python/integrations/tools), cada uma distribuída em seu próprio pacote. O gerador não adiciona esses pacotes, então instale o que você precisa primeiro — por exemplo `langchain-community`, de onde vem a ferramenta de pesquisa abaixo:

<NxCommands commands={['run <project-name>:add langchain-community']} />

Em seguida, importe as ferramentas e adicione-as a `get_agent()`:
...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
O agente gerado usa o modelo Strands padrão no Amazon Bedrock. Para configurá-lo, passe um `model` para o `Agent`. Consulte a [documentação Strands sobre provedores de modelo](https://strandsagents.com/docs/user-guide/concepts/model-providers/) para os provedores disponíveis e suas opções:
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
Agentes LangChain usam um modelo [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse). O agente gerado lê o id do modelo e a região das variáveis de ambiente `MODEL_ID` e `AWS_REGION`, que a infraestrutura define para o agente implantado. Para configurar o modelo ainda mais, adicione argumentos onde ele é construído em `get_agent()`:
...
Um target `<your-agent-name>-serve` também é gerado, que executa o agente contra sua infraestrutura implantada e, portanto, requer que `RUNTIME_CONFIG_APP_ID` seja definido. Consulte o guia <Link path="guides/local-development">Desenvolvimento Local</Link> para a diferença entre `dev` e `serve`.
