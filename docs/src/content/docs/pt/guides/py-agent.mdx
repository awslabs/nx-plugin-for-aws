...
Se você selecionou `None` para `computeType`, nenhum construct CDK ou módulo Terraform é gerado — o Agente só pode ser executado localmente. A opção `auth` é ignorada neste modo, pois não há endpoint hospedado para autenticar.
</OptionFilter>
