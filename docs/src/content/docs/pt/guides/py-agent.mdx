...
Para um guia mais detalhado sobre escrita de agentes Strands, consulte a [documentação do Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Para agentes LangChain, o <Link path="/guides/connection/py-agent-mcp">gerador `connection`</Link> também conecta ferramentas de servidor MCP e agente A2A no `create_agent`.
