...
O ponto de entrada do servidor difere por framework (Strands produz um `Agent` gerenciado por contexto, enquanto LangChain dirige um grafo `create_agent` compilado), mas o contrato externo para cada protocolo é o mesmo.

Todos os protocolos expõem `/ping` para o contrato de verificação de integridade do runtime AgentCore. Agentes A2A escutam na porta `9000`; agentes HTTP e AG-UI escutam na porta `8080`. O Dockerfile e a infraestrutura gerados são configurados para você.
...
- Endpoint de verificação de integridade (`/ping`)
...
O `main.py` gerado expõe um único endpoint POST que transmite eventos [AG-UI](https://docs.ag-ui.com/) sobre Server-Sent Events (SSE), bem como `/ping` para a verificação de integridade do runtime AgentCore. A conexão depende do framework:
...
Para agentes autenticados com Cognito, forneça um token de acesso Cognito através da variável de ambiente `AGENT_ACCESS_TOKEN`, que é enviado como um token bearer:
...
Para invocar um Agente em execução local através do target `<your-agent-name>-serve`, você pode enviar uma solicitação POST simples para `/invocations` na porta em que seu agente local está sendo executado. Por exemplo, com `curl`:
...
Para Autenticação IAM, a solicitação deve ser assinada usando AWS Signature Version 4 (SigV4).
...
Para delegar trabalho deste agente para um agente A2A remoto (seja <Link path="/guides/ts-agent">TypeScript</Link> ou <Link path="/guides/py-agent">Python</Link>), use o <Link path="/guides/connection/py-agent-a2a">gerador `connection`</Link>. Ele fornece um cliente autenticado com SigV4 para o agente de destino e transforma por AST o `agent.py` deste agente para registrar o agente A2A remoto como um delegado decorado com `@tool`.
