...
<Snippet name="agent/dev-vs-serve" parentHeading="Desenvolvimento Local" />
...
:::caution[Use `-dev`, não `-serve`]
Use `<your-agent-name>-dev` para isso, não `<your-agent-name>-serve`. `-serve` executa o caminho de código implantado, então precisa que `RUNTIME_CONFIG_APP_ID` esteja definido — veja <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>. Sem ele, o streaming já começou quando o agente tenta resolver seu armazenamento de sessão, então a requisição retorna um `200` vazio sem erro e a falha aparece apenas no log do servidor.
:::
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'pt'}/guides/connection/py-agent-a2a`}
    source="strands"
    sourceBadge="python"
    target="a2a"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'pt'}/guides/connection/ts-agent-a2a`}
    source="strands"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'pt'}/guides/connection/py-agent-dynamodb`}
    source="strands"
    sourceBadge="python"
    target="dynamodb"
    targetBadge="python"
  ...
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'pt'}/guides/connection/py-agent-gateway`}
    source="strands"
    sourceBadge="python"
    target="agentcore"
  />
  <ConnectionCard
    ...
    href={`/nx-plugin-for-aws/${Astro.currentLocale || 'pt'}/guides/connection/agentcore-gateway-agent`}
    source="agentcore"
    target="strands"
  />
</CardGrid>
