...
- **Strands**: envolve seu `Agent` em um `ag_ui_strands.StrandsAgent`, construído dentro de um manipulador `lifespan` do FastAPI (para que a construção aconteça na inicialização do contêiner/sessão em vez de no momento da importação), e servido a partir de um loop FastAPI `/invocations` feito manualmente.
- **LangChain**: envolve o grafo compilado em um `ag_ui_langgraph.LangGraphAgent`, construído da mesma forma dentro de `lifespan`, e servido a partir de um loop FastAPI `/invocations` feito manualmente.
...
Consulte a documentação do [`ChatBedrockConverse`](https://docs.langchain.com/oss/python/integrations/chat/bedrock_converse) para os campos de `guardrail_config`.
</TabItem>
</Tabs>
...
<CardGrid>
  <ConnectionCard
    ...
    description="Chame um Python Agent de um website React"
  ...
  <ConnectionCard
    ...
    description="Chame um Agent que expõe o protocolo AG-UI de um website React via CopilotKit"
  ...
  <ConnectionCard
    ...
    description="Coloque um agente na frente de um AgentCore Gateway como um destino de runtime"
