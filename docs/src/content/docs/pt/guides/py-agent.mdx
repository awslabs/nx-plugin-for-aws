...
Para agentes Strands, o gerador usa [FastAPI](https://fastapi.tiangolo.com/) para expor um servidor HTTP por padrão. Alternativamente, você pode escolher o protocolo [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) para interoperabilidade com outros agentes compatíveis com A2A, ou o protocolo [AG-UI](https://docs.ag-ui.com/) para integração direta com frontend via [CopilotKit](https://docs.copilotkit.ai/).

:::note[LangChain suporta apenas AG-UI]
O framework `langchain` atualmente suporta apenas o protocolo `ag-ui`. Escolha o framework padrão `strands` se você precisar do protocolo `http` ou `a2a`. Agentes LangChain ainda podem se conectar a servidores MCP, AgentCore Gateways e agentes A2A através do <Link path="/guides/connection/py-agent-mcp">gerador `connection`</Link>.
:::
...
Os exemplos abaixo usam Strands. Um agente LangChain funciona da mesma forma, usando `create_agent` com o decorador `@tool` de `langchain_core.tools` e um modelo `ChatBedrockConverse` (as variáveis de ambiente `MODEL_ID` e `AWS_REGION` definem o modelo e a região). Consulte a [documentação do LangChain](https://docs.langchain.com/oss/python/langchain/overview) para detalhes.
...
Para consumir servidores MCP criados usando os geradores <Link path="/guides/py-mcp-server">`py#mcp-server`</Link> ou <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>, você pode usar o <Link path="/guides/connection/py-agent-mcp">gerador `connection`</Link>.
...
Para um guia mais detalhado sobre escrita de agentes Strands, consulte a [documentação do Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/). Para agentes LangChain, o <Link path="/guides/connection/py-agent-mcp">gerador `connection`</Link> também conecta ferramentas de servidor MCP, AgentCore Gateway e agente A2A ao `create_agent`.
