...
- project.json Um novo target é adicionado ao build para gerar o cliente tipado
...
Durante o build, um cliente tipado é gerado a partir da especificação OpenAPI do FastAPI. Isso adicionará três novos arquivos à sua aplicação React:
...
:::tip
Por padrão, o cliente gerado é ignorado do controle de versão. Se preferir incluí-lo, você pode remover a entrada do arquivo `.gitignore` da sua aplicação React, mas note que quaisquer alterações manuais nos arquivos `.gen.ts` serão sobrescritas quando o projeto for construído.
:::
...
:::note
Sempre que fizer alterações no FastAPI, você precisa reconstruir seu projeto para que as mudanças sejam refletidas no cliente gerado. Por exemplo:

<NxCommands commands={['run-many --target build --all']} />
:::

:::tip
Se estiver desenvolvendo simultaneamente na aplicação React e no FastAPI, você pode usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar o cliente sempre que fizer mudanças na API:
...
Os hooks gerados incluem suporte para mutations usando o hook `useMutation` do TanStack Query. Isso fornece uma forma limpa de lidar com operações de criação, atualização e exclusão com estados de carregamento, tratamento de erros e atualizações otimistas.
...
const createItem = useMutation({
  ...
  onSuccess: (data) => {
    // Executa quando a mutation é bem-sucedida
  ...
  onSettled: () => {
    // Executa quando a mutation é finalizada (sucesso ou erro)
...
### Paginação com Consultas Infinitas

Para endpoints que aceitam um parâmetro `cursor` como entrada, os hooks gerados fornecem suporte para consultas infinitas usando o hook `useInfiniteQuery` do TanStack Query. Isso facilita a implementação de funcionalidades "carregar mais" ou scroll infinito.
...
:::tip
Se você tem uma API paginada com parâmetro de paginação com nome diferente de `cursor`, você pode [personalizar usando a extensão OpenAPI `x-cursor`](#custom-pagination-cursor).
:::
...
A integração inclui tratamento de erros embutido com respostas de erro tipadas. Um tipo `<operation-name>Error` é gerado e encapsula as possíveis respostas de erro definidas na especificação OpenAPI. Cada erro possui propriedades `status` e `error`, e ao verificar o valor de `status` você pode identificar um tipo específico de erro.
...
<Steps>
  ...
  3. Chunks subsequentes são recebidos
      ...
      - `fetchStatus` permanece `'fetching'`
      - `data` é atualizado com cada novo chunk assim que é recebido

  4. O stream é finalizado
...
Se você <Link path="guides/fastapi#streaming">configurou seu FastAPI para stream de respostas</Link>, o cliente gerado incluirá métodos tipados para iterar assincronamente sobre os chunks do stream usando sintaxe `for await`.
...
Por padrão, operações no FastAPI que usam os métodos HTTP `PUT`, `POST`, `PATCH` e `DELETE` são consideradas mutations, e as demais consultas.
...
Os hooks e métodos do cliente gerados são organizados automaticamente com base nas tags OpenAPI dos endpoints do FastAPI. Isso ajuda a manter as chamadas de API organizadas e facilita encontrar operações relacionadas.
...
Os hooks gerados serão agrupados por estas tags:
...
<Drawer title="Tratar estados de carregamento usando o cliente diretamente" trigger="Clique aqui para ver um exemplo usando o cliente vanilla diretamente.">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
...
function ItemForm() {
  ...
  const createItem = useMutation({
    ...api.createItem.mutationOptions(),
    // ✅ Erro de tipo se o callback onSuccess não tratar o tipo correto de resposta
    onSuccess: (data) => {
  ...
  // UI de erro pode usar estreitamento de tipo para tratar diferentes erros
  if (createItem.error) {
