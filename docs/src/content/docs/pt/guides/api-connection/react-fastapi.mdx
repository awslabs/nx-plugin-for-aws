...
:::tip[Dica]
Por padrão, o cliente gerado é ignorado no controle de versão. Se preferir incluí-lo, você pode remover a entrada do arquivo `.gitignore` da sua aplicação React, mas note que quaisquer alterações manuais nos arquivos `.gen.ts` serão sobrescritas ao construir o projeto.
:::
...
:::tip[Dica]
Se estiver trabalhando simultaneamente na aplicação React e no FastAPI, use o target `serve-local` da aplicação React que regenerará automaticamente o cliente quando a API mudar, além de recarregar o site e o servidor FastAPI local:
...
Use o método `queryOptions` para obter as opções necessárias para chamar sua API usando o hook `useQuery` do TanStack Query:
...
Você pode usar as propriedades `isLoading` e `fetchStatus` para determinar o estado atual do stream. Um stream segue este ciclo de vida:

<Steps>
  1. A requisição HTTP para iniciar o stream é enviada
...
<Drawer title="Streaming usando o cliente da API diretamente" trigger="Clique aqui para um exemplo usando o cliente vanilla diretamente.">
...
<Drawer title="Operações agrupadas usando o cliente da API diretamente" trigger="Clique aqui para um exemplo usando o cliente diretamente.">
```tsx
import { useState, useEffect } from 'react';
import { useMyApiClient } from './hooks/useMyApiClient';
...
Você pode personalizar respostas de erro no FastAPI definindo classes de exceção customizadas, handlers de exceção e especificando modelos de resposta para códigos de status de erro diferentes. O cliente gerado lidará automaticamente com esses tipos de erro personalizados.
    ...
    if not is_valid(item):
        raise ValidationException(
            ValidationError(
                ...
                field_errors=["name é obrigatório"]
            )
        )
...
function ItemComponent() {
  ...
  // Query com tratamento de erros tipado
...
<Drawer title="Lidar com estados de carregamento usando o cliente diretamente" trigger="Clique aqui para um exemplo usando o cliente vanilla diretamente.">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
