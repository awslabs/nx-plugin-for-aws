...
- project.json Um novo target é adicionado ao build que gera o cliente tipado
...
O cliente tipado gerado pode ser usado para chamar seu FastAPI a partir da aplicação React. Recomenda-se usar os hooks TanStack Query, mas você pode usar o cliente vanilla se preferir.
...
:::tip[Dica]
Se estiver trabalhando ativamente tanto na aplicação React quanto no FastAPI, use o target `serve-local` da aplicação React, que regenerará automaticamente o cliente sempre que a API mudar, além de recarregar automaticamente o site e o servidor FastAPI local:

<NxCommands commands={['run <WebsiteProject>:serve-local']} />

Para controle mais granular, você pode usar o target `watch-generate:<ApiName>-client` da sua aplicação React para regenerar o cliente sempre que fizer alterações na API:
...
const createItem = useMutation({
  ...
  onSettled: () => {
    ...
    // Bom lugar para invalidar consultas que possam ser afetadas
...
Para endpoints que aceitam um parâmetro `cursor` como entrada, os hooks gerados fornecem suporte para infinite queries usando o hook `useInfiniteQuery` do TanStack Query. Isso facilita a implementação de funcionalidades de "carregar mais" ou scroll infinito.
...
:::tip[Dica]
Se você tem uma API paginada cujo parâmetro de paginação tem um nome diferente de `cursor`, você pode [personalizá-lo usando a extensão OpenAPI `x-cursor`](#custom-pagination-cursor).
:::
...
Se você <Link path="guides/fastapi#streaming">configurou seu FastAPI para transmitir respostas</Link>, seu hook `useQuery` atualizará automaticamente seus dados conforme novos chunks do stream chegam.
...
Se você <Link path="guides/fastapi#streaming">configurou seu FastAPI para transmitir respostas</Link>, o cliente gerado incluirá métodos tipados para iterar assincronamente sobre chunks do stream usando a sintaxe `for await`.
...
Por padrão, operações no seu FastAPI que usam os métodos HTTP `PUT`, `POST`, `PATCH` e `DELETE` são consideradas mutations, e as demais são consideradas consultas.
...
@app.get(
    "/items",
    openapi_extra={
        # Desabilita paginação baseada em cursor para este endpoint
        "x-cursor": False
    }
)
...
function ItemsAndUsers() {
  ...
  const handleCreateItem = async () => {
    try {
      // Criar item usando o método agrupado
...
function ItemComponent() {
  ...
  const getItem = useQuery({
    ...
    onError: (error) => {
      // O erro é tipado com base nas respostas do seu FastAPI
      switch (error.status) {
        case 404:
...
function ItemComponent() {
  ...
  // Buscar item com tratamento de erros
  useEffect(() => {
    const fetchItem = async () => {
      try {
        setLoading(true);
  ...
  // Criar item com tratamento de erros
...
function ItemList() {
  ...
  const deleteMutation = useMutation({
    ...
    onSettled: () => {
      // Sempre refaz a consulta após erro ou sucesso para garantir sincronização
      queryClient.invalidateQueries({ queryKey: api.listItems.queryKey() });
    },
  });
...
function ItemForm() {
  ...
  // UI de erro pode usar narrowing de tipos para lidar com diferentes tipos de erro
  if (createItem.error) {
...
function ItemForm() {
  ...
  const handleSubmit = async (data: CreateItemInput) => {
    try {
      ...
      // ✅ O tipo de erro inclui todas as possíveis respostas de erro
      const err = e as CreateItemError;
      switch (err.status) {
        case 400:
