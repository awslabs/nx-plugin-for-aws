...
- src
  - components
    - \<ApiName>ClientProvider.tsx Configura os clientes tRPC e vinculações aos schemas do backend. ApiName será resolvido para o nome da API
...
A integração inclui tratamento de erros embutido que processa adequadamente erros tRPC:
  ...
  const deleteMutation = useMutation(
    trpc.users.delete.mutationOptions({
      onMutate: async (userId) => {
        ...
        // Remove o usuário otimisticamente
...
Os tipos são inferidos automaticamente do router e definições de schema do seu backend, garantindo que quaisquer mudanças na API sejam imediatamente refletidas no código frontend sem necessidade de build.
...
Você também pode consultar diretamente a [documentação TanStack Query](https://tanstack.com/query/v5).