...
O target `fmt` verifica se seu código Terraform está formatado e falha se não estiver. Ele é executado como parte do `build`:

<NxCommands commands={['fmt tf-infra']} />

Para reformatar seu código, execute com a configuração `fix`:

<NxCommands commands={['fmt tf-infra --configuration=fix']} />

Um target `lint` orquestra a verificação de formato, então `run-many` alcança seus projetos Terraform junto com seus projetos TypeScript e Python:

<NxCommands commands={['run-many --target lint --all --configuration=fix']} />

:::tip[Por que a verificação é o padrão]
`terraform fmt` reescreve os arquivos que lê — os mesmos arquivos que o target `fmt` declara como suas entradas de cache. Escrever a partir do target base mudaria sua própria chave de cache em cada execução, então nunca poderia registrar um acerto de cache. Verificar em vez disso mantém o target cacheável, espelhando como os targets `format` de TypeScript e Python funcionam.
:::

Para pular formatação e linting durante o desenvolvimento, use a configuração `skip-lint`:
...
Defina `TF_PLUGIN_CACHE_DIR` no seu ambiente para apontar o script `init` fornecido para um cache que você gerencia — um volume compartilhado entre workspaces, por exemplo. Note que o target `test` lê seu caminho do `project.json`, então altere-o lá também.
