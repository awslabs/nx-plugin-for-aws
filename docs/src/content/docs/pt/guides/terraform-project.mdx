...
#### Cache de Provedores Compartilhado

Cada target que executa `terraform init` compartilha um cache de provedores em `.terraform/plugin-cache` na raiz do seu workspace, através do [`TF_PLUGIN_CACHE_DIR`](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) do Terraform. Os provedores são baixados uma vez e vinculados a partir daí posteriormente, o que é mais importante para o target `test`: ele mantém seu diretório de trabalho em `dist`, então um `dist` limpo ou uma falha de cache faria o re-download do provedor AWS — várias centenas de megabytes — em cada execução.

O diretório é criado automaticamente antes de cada `terraform init`, e `.terraform` está no gitignore. Diferentemente de `.nx/cache`, ele não é removido por `nx reset`, então um reset não descarta os provedores. Em CI, faça cache de `.terraform/plugin-cache` entre jobs para obter a mesma economia lá.

O `env` do Nx tem precedência sobre seu shell, então definir `TF_PLUGIN_CACHE_DIR` no seu ambiente não altera onde esses targets procuram. Para apontá-los para outro lugar — um local compartilhado entre vários workspaces, por exemplo — altere `TF_PLUGIN_CACHE_DIR` no `project.json` do seu projeto, e o caminho em `scripts/plugin-cache.ts` para os targets `bootstrap`, `bootstrap-destroy` e `init`. Os caminhos em `project.json` são relativos ao `cwd` do target, já que o Nx não expande `{workspaceRoot}` dentro de `env`.

:::note[Arquivos de Lock]
O cache não altera quais versões de provedores você obtém: `.terraform.lock.hcl` ainda as fixa, e o Terraform registra os mesmos hashes independentemente de um provedor ter vindo do cache ou do registro. Faça commit do arquivo de lock como de costume.
:::
