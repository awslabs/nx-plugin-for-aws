...
Checkov é executado como parte de `build` e `package`, então um `plan` interativo verifica sua infraestrutura antes de você aplicá-la.
...
`plan` depende de `package` em vez de `build`, então ele produz os artefatos que seus módulos referenciam — os bundles Lambda e metadados de operações gerados — e executa Checkov, sem também executar os gates de lint, test e type-check de cada projeto.

:::caution
Como `plan` não executa mais os testes de módulo Terraform ou os testes unitários de seus projetos, execute `build` para verificar seu código antes de aplicar qualquer coisa que você se importe, e controle seu pipeline com ele:
