...
O [Terraform](https://www.terraform.io/) é uma ferramenta de infraestrutura como código open-source que permite criar, alterar e melhorar infraestruturas de forma segura e previsível.
...
<FileTree>
  ...
  - src
    ...
    - providers.tf Configuração de providers com backend S3
...
:::tip
Projetos de aplicação incluem capacidades completas de deploy com gerenciamento de estado remoto, enquanto projetos de biblioteca são projetados para criar módulos Terraform reutilizáveis que podem ser consumidos por outros projetos.
:::
...
Se você quiser executar um módulo de um projeto separado (lib), pode fazê-lo da seguinte forma:
...
Isso atualizará automaticamente o grafo do Nx para adicionar uma dependência entre sua aplicação consumidora e sua lib.
...
:::tip
Por padrão, todos os targets assumem o ambiente `dev`, porém você pode especificar um ambiente usando: <NxCommands commands={['run tf-infra:apply --configuration=prod']} />
...
Para projetos de aplicação, antes de fazer deploy da infraestrutura, você precisa executar o bootstrap do backend de estado remoto. Isso cria um bucket S3 para armazenar seus arquivos de estado do Terraform:
...
O target bootstrap só está disponível para projetos do tipo aplicação. Projetos de biblioteca não requerem gerenciamento de estado remoto pois são projetados para serem módulos reutilizáveis.
:::

## Targets Disponíveis

Os targets disponíveis dependem do tipo de projeto:
...
Execute verificações de segurança na sua infraestrutura usando Checkov com o target `test`:
...
Antes de aplicar mudanças, veja o que o Terraform fará executando o target `plan`:
...
Recupere valores de output da sua configuração Terraform:
...
:::caution
Isso irá excluir permanentemente todos os recursos gerenciados por esta configuração Terraform. Use com cautela!
:::
...
Para mais informações sobre o Terraform, consulte a [Documentação do Terraform](https://www.terraform.io/docs) e a [Documentação do AWS Provider](https://registry.terraform.io/providers/hashicorp/aws/latest/docs).