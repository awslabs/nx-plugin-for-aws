...
Um projeto sem arquivos de teste é um sucesso sem operação, então você pode adicionar testes quando precisar deles. Este target não faz parte do `build`, já que executar testes baixa os providers — invoque-o diretamente, ou a partir do CI onde o Terraform está disponível.
