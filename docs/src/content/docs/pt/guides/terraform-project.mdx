...
Cada nome que você definir aqui deve ser declarado em `src/variables.tf` — o gerador declara `aws_region` e `environment`.
:::
</TabItem>
...
Cada target que executa `terraform init` reutiliza um cache de provedores em `.terraform/plugin-cache` na raiz do seu workspace, então os provedores são baixados uma vez em vez de a cada execução. Cada projeto obtém seu próprio diretório lá. Consulte a [documentação do Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) para mais informações.

O target `install-providers` é o que preenche esse cache, e cada target que executa `terraform init` depende dele:

<NxCommands commands={['install-providers tf-infra']} />

Você normalmente não o executará por conta própria. Ele existe porque duas execuções de `terraform init` preenchendo um cache ao mesmo tempo podem cada uma calcular um hash diferente para o mesmo provedor, que o terraform então rejeita contra seu `.terraform.lock.hcl` com "the cached package ... does not match any of the checksums recorded in the dependency lock file". Baixar os provedores uma vez, antes dos targets que precisam deles, deixa esses targets apenas lendo de um cache que já está aquecido.

Defina `TF_PLUGIN_CACHE_DIR` no seu ambiente para apontar o script `init` fornecido para um cache que você gerencia — um volume compartilhado entre workspaces, por exemplo. Observe que os targets `install-providers`, `test` e `validate` leem seu caminho do `project.json`, então altere-o lá também.
