...
Cada target que executa `terraform init` compartilha um cache de provedores em `.terraform/plugin-cache` na raiz do seu workspace. Consulte a [documentação do Terraform](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache) para mais informações.
