...
O [Terraform](https://www.terraform.io/) é uma ferramenta de código aberto para infraestrutura como código que permite criar, alterar e melhorar infraestrutura de forma segura e previsível.
...
<FileTree>
  ...
  - scripts Auxiliares Node executados pelos targets nx `bootstrap` e `init`
    - aws-config.ts Resolve conta + região via cadeia de credenciais do AWS SDK
    - bootstrap.ts Puxa/envia o tfstate do bootstrap e executa `terraform apply`
    - init.ts Executa `terraform init` com a configuração do backend S3
...
:::tip[Ambiente padrão]
Por padrão, todos os targets assumem o ambiente `dev`, porém você pode especificar um ambiente como segue: <NxCommands commands={['apply tf-infra --configuration=prod']} />
...
Antes de implantar sua infraestrutura, você precisará executar o bootstrap do backend de estado remoto. Isso cria um bucket S3 para armazenar seus arquivos de estado Terraform:

<NxCommands commands={['bootstrap tf-infra']} />

:::note[Configuração do perfil AWS]
...
:::tip[Aprovação automática]
O comando acima aplica o plano criado pelo target `plan`. Certifique-se de executar `plan` primeiro para revisar as mudanças.
:::

:::note[Alias Deploy]
Um target `deploy` também está disponível como alias para `apply`. Você pode usar tanto <NxCommands commands={['deploy tf-infra']} /> quanto <NxCommands commands={['apply tf-infra']} /> de forma intercambiável.
:::
...
Para mais informações sobre o Terraform, consulte a [Documentação do Terraform](https://www.terraform.io/docs) e a [Documentação do Provedor AWS](https://registry.terraform.io/providers/hashicorp/aws/latest/docs).rmações
