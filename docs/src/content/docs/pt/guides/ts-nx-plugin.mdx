---
title: TypeScript Nx Plugin
description: Gere seu próprio Nx Plugin! Construa geradores personalizados automaticamente disponibilizados para codificação assistida por IA via MCP
generator: ts#nx-plugin
---

import { FileTree } from '@astrojs/starlight/components';
import RunGenerator from '@components/run-generator.astro';
import NxCommands from '@components/nx-commands.astro';
import GeneratorParameters from '@components/generator-parameters.astro';
import Link from '@components/link.astro';
import Snippet from '@components/snippet.astro';
import InstallCommand from '@components/install-command.astro';

Gere um [Nx Plugin](https://nx.dev/extending-nx/intro/getting-started) TypeScript completo com um servidor [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) integrado. Este gerador cria uma base para construir geradores personalizados que podem ser usados perfeitamente por assistentes de IA para geração automatizada de código e scaffolding de projetos.

## Uso

### Gerar um Nx Plugin

Você pode gerar um TypeScript Nx Plugin de duas maneiras:

<RunGenerator generator="ts#nx-plugin" />

### Opções

<GeneratorParameters generator="ts#nx-plugin" />

## Saída do Gerador

O gerador criará a seguinte estrutura de projeto:

<FileTree>
- your-plugin/
  - src/
    - index.ts Entry point for your plugin
    - mcp-server/
      - index.ts Exports your server
      - server.ts Main MCP server configuration
      - stdio.ts Entry point for your MCP server with STDIO transport
      - http.ts Entry point for your MCP server with Streamable HTTP transport
      - schema.ts Shared schemas for the MCP server's tools
      - utils.ts Utility functions for the MCP server
      - resources/
        - GENERAL_GUIDANCE.md Guidance served by the `general-guidance` tool
      - tools/
        - create-workspace-command.ts Tool for workspace creation guidance
        - general-guidance.ts Tool for general Nx and plugin guidance
        - list-generators.ts Tool to list available generators
        - generator-guide.ts Tool for detailed generator information
  - generators.json Nx generator configuration (initially empty)
  - package.json Plugin package configuration
  - project.json Nx project configuration with build and package targets
  - tsconfig.json TypeScript configuration, referencing the two below
  - tsconfig.lib.json TypeScript configuration for your plugin's source
  - tsconfig.spec.json TypeScript configuration for your plugin's tests
  - vitest.config.mts Vitest configuration for your plugin's tests
  - README.md Documentation for your plugin, served by the `generator-guide` tool
  - LICENSE Written by the <Link path="/guides/license">`license`</Link> generator's sync, when configured
</FileTree>

## Trabalhando com Seu Nx Plugin

### Adicionando Geradores

Depois de ter seu projeto de plugin, você pode adicionar geradores usando o gerador <Link path="/guides/nx-generator">`ts#nx-generator`</Link>:

<RunGenerator generator="ts#nx-generator" requiredParameters={{ project: 'your-plugin' }} />

Isso adicionará um novo gerador ao seu plugin.

:::tip[Documentação do Gerador]
Leia o <Link path="/guides/nx-generator">guia `ts#nx-generator`</Link> para detalhes sobre como implementar geradores.
:::

Certifique-se de escrever um `README.md` detalhado para seu gerador, pois ele é usado pela ferramenta `generator-guide` do MCP Server.

### Targets do Projeto

O plugin gerado inclui vários targets importantes:

#### Target Build
Compila seu código TypeScript e prepara o plugin para uso:

<NxCommands commands={['build your-plugin']} />

#### Target Package
Cria um pacote distribuível pronto para publicação no NPM:

<NxCommands commands={['package your-plugin']} />

O target package inclui todos os recursos necessários:
- Arquivos JavaScript compilados
- Arquivos de declaração TypeScript
- Os manifestos `generators.json`, `executors.json` e `migrations.json` que o Nx lê de um plugin publicado
- Arquivos de documentação e licença

## Integração com MCP Server

O plugin configura um servidor MCP usando o gerador <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>.

### Ferramentas

O plugin inclui um servidor MCP completo que fornece aos assistentes de IA as seguintes ferramentas:

- `general-guidance`: Obtenha as melhores práticas para trabalhar com Nx e seu plugin
- `create-workspace-command`: Aprenda como criar novos workspaces que podem usar seu plugin
- `list-generators`: Liste todos os geradores disponíveis em seu plugin
- `generator-guide`: Obtenha informações detalhadas sobre geradores específicos, incluindo schemas e uso

### Configurando com Assistentes de IA

<Snippet name="mcp/configuration-ts" parentHeading="Configurando com Assistentes de IA" />

### Personalizando o MCP Server

Você pode estender o servidor MCP modificando `server.ts` para adicionar ferramentas ou recursos adicionais específicos ao domínio do seu plugin.

Para mais detalhes sobre como escrever MCP Servers, consulte o <Link path="/guides/ts-mcp-server">guia `ts#mcp-server`</Link>.

## Publicando Seu Plugin

### Preparando para Publicação

1. **Atualize as Informações do Pacote**: Edite o [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) do seu plugin com nome, versão, descrição e informações de repositório adequados. Também remova `"private": true`.

2. **Construa o Pacote**:
   <NxCommands commands={['package your-plugin']} />

   O pacote é colocado em `dist/<raiz do projeto do seu plugin>/package` — para um plugin gerado com `--name=nx-plugin --directory=tools`, isso é `dist/tools/nx-plugin/package`.

3. **Teste Localmente**: Você pode testar seu plugin localmente instalando-o em outro workspace:
   ```bash
   npm install /path/to/your/workspace/dist/tools/nx-plugin/package
   ```

### Publicando no NPM

Quando seu plugin estiver pronto:

```bash
cd dist/tools/nx-plugin/package
npm publish
```

### Usando Plugins Publicados

Após a publicação, outros podem instalar e usar seu plugin:

<InstallCommand pkg="your-plugin-name" />

Em seguida, use seus geradores:

<RunGenerator namespace="your-plugin-name" generator="your-generator-name" />

#### Configure Assistentes de IA com seu Plugin Publicado

O servidor MCP do seu plugin publicado pode ser configurado da seguinte forma:

```json {3-6}
{
  "mcpServers": {
    "aws-nx-mcp": {
      "command": "npx",
      "args": ["-y", "-p", "your-plugin-name", "your-plugin-name-mcp-server"]
    }
  }
}
```

