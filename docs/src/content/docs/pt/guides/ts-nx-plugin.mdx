...
Depois de ter seu projeto de plugin, você pode adicionar geradores usando o gerador <Link path="/pt/guides/nx-generator">`ts#nx-generator`</Link>:
...
Leia o <Link path="/pt/guides/nx-generator">guia `ts#nx-generator`</Link> para detalhes sobre como implementar geradores.
:::
...
O plugin configura um servidor MCP usando o gerador <Link path="/pt/guides/ts-mcp-server">`ts#mcp-server`</Link>.
...
Para mais detalhes sobre como escrever MCP Servers, consulte o <Link path="/pt/guides/ts-mcp-server">guia `ts#mcp-server`</Link>.

## Publicando Seu Plugin

### Preparando para Publicação

1. **Atualize as Informações do Pacote**: Edite o [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json) do seu plugin com nome, versão, descrição e informações de repositório adequados.
