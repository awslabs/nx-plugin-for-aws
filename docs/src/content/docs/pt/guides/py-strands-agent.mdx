...
O gerador configura um alvo Nx `<your-agent-name>-chat` que depende de `<your-agent-name>-serve-local`. Executá-lo inicia o agente localmente e o coloca em um chat interativo de terminal alimentado por [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli):

<NxCommands commands={['run your-project:agent-chat']} />

Para agentes **A2A** e **AG-UI**, o alvo de chat simplesmente invoca `agent-chat-cli` como um binário contra o agente em execução local — nenhum código é fornecido em seu workspace.

Para agentes **HTTP**, o gerador emite:
...
- Um alvo Nx `<your-agent-name>-generate-client` que executa `open-api#ts-client` para produzir um cliente TypeScript com segurança de tipos em `scripts/<your-agent-name>/generated/`
- `scripts/<your-agent-name>/chat.ts` — um adaptador fino que importa o cliente gerado e o conecta ao `chatLoop` do `agent-chat-cli`

O script de chat é um pequeno wrapper em torno do cliente gerado. Quando você personalizar a forma de entrada do agente (por exemplo, adicionar novos campos a `InvokeInput`), atualize o `sendMessage` do adaptador para passar os novos campos e o resto funciona automaticamente.
