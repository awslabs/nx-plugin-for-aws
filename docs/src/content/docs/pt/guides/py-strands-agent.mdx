...
O gerador fornece uma opção `auth` para configurar autenticação para seu Agente Strands. Você pode escolher entre autenticação `IAM` (padrão) ou `Cognito` ao gerar seu agente.
...
Quando você seleciona autenticação `Cognito`, o gerador cria automaticamente o construct `UserIdentity` e configura o agente para usar Cognito para autenticação.

<Infrastructure>
<Fragment slot="cdk">
O construct gerado aceita uma prop `identity` que configura autenticação Cognito:
...
O construct `UserIdentity` é gerado automaticamente quando você seleciona autenticação `Cognito`.
</Fragment>
<Fragment slot="terraform">
