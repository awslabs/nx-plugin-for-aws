...
Quando `protocol` é definido como `AG-UI`, o ponto de entrada usa a integração [ag-ui-strands](https://docs.ag-ui.com/), que expõe seu agente via protocolo AG-UI para integração direta com frontend usando [CopilotKit](https://docs.copilotkit.ai/aws-strands):
...
O gerador configura um alvo chamado `<your-agent-name>-invoke` que inicia um chat interativo com seu agente a partir do terminal. Ele é fornecido como um script TypeScript (via `tsx`) e seleciona automaticamente o protocolo correto (HTTP, A2A, AG-UI) para seu agente. Por padrão, ele se conecta ao servidor local:

<NxCommands commands={['agent-invoke your-project']} />

Para conversar com um agente implantado, defina `RUNTIME_CONFIG_APP_ID` como o ID da aplicação AppConfig do seu workspace — a CLI resolve o ARN do runtime e invoca o endpoint implantado com requisições assinadas com SigV4:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Para agentes autenticados com Cognito, passe um JWT com `--accessToken`:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

O script da CLI está localizado em `scripts/<agent>/cli.ts` no seu projeto.
