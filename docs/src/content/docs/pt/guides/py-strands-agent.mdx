...
O gerador também fornece uma CLI de chat interativa mínima em `scripts/<your-agent-name>/chat.ts` e um alvo correspondente `<your-agent-name>-chat`. Este alvo depende de `<your-agent-name>-serve-local`, então executá-lo inicia o agente localmente e o coloca em um chat interativo de terminal com ele:

<NxCommands commands={['run your-project:agent-chat']} />

A CLI é um script TypeScript (executado via `tsx`) que usa o cliente apropriado para o protocolo para transmitir respostas token por token via [`@clack/prompts`](https://github.com/bombshell-dev/clack):

- **HTTP**: `POST /invocations` com streaming JSONL
- **A2A**: `@a2a-js/sdk` `ClientFactory` com `sendMessageStream`
