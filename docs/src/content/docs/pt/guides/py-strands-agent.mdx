...
:::tip
Seu `Dockerfile` fica co-localizado com seu projeto do Agente Strands. Para CDK, o construct usa `buildContexts` para construir a imagem diretamente a partir deste Dockerfile. Para Terraform, a imagem Docker é pré-construída usando uma tag (por exemplo `my-scope-my-project-agent:latest`) que é referenciada por sua infraestrutura Terraform.
:::
