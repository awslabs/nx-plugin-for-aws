...
Você pode definir um modelo Pydantic para produzir dados estruturados em vez disso. Para fazer isso, você precisará serializar os modelos pydantic produzidos por `handle_invoke`:
...
:::caution
Note aqui que o servidor fornecido (e o método `serialize_stream` acima) não está atualmente em conformidade com o padrão Server-Sent Events (SSE), de modo que, se desejado, o Agente possa ser invocado por nosso cliente TypeScript gerado a partir de um frontend, que atualmente não espera eventos de streaming no formato SSE. Por favor, dê +1 [neste problema do GitHub](https://github.com/awslabs/nx-plugin-for-aws/issues/369) se isso afetar você para nos ajudar a priorizar a resolução disso.
:::
