...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    Adicione a dependência com [`--save-catalog`](https://pnpm.io/settings#catalogmode), para que o pnpm registre a versão no catálogo e a referencie:
    ...
    :::note
    Definir [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) em `pnpm-workspace.yaml` torna isso o padrão para cada `pnpm add`. Isso também faz com que `pnpm add some-npm-package@<version>` falhe sempre que o catálogo fixar uma versão diferente, o que impede que `nx migrate` atualize o Nx — então o workspace gerado deixa isso não definido.
    :::
  </TabItem>
  <TabItem label="yarn">
