...
<FileTree>
  ...
  - vitest.config.mts a configuração compartilhada do Vitest que o próprio vitest.config.mts do seu projeto estende (criado pelo primeiro projeto TypeScript)
  - package.json ferramentas compartilhadas de build e teste são adicionadas
  - pnpm-workspace.yaml o projeto é adicionado ao workspace, e versões de dependências ao [catálogo](#catálogos) (o arquivo equivalente para o seu gerenciador de pacotes)
  - .gitignore caminhos de saída de build e teste são ignorados
...
O `cwd` é o que permite que o `rolldown` encontre sua configuração: sem ele, o comando é executado a partir da raiz do workspace, onde `rolldown.config.ts` não existe.
...
Seu projeto também tem um alvo `assemble`, que produz o que seu projeto contribui para uma implantação (por exemplo, sua saída compilada ou empacotada), sem executar as verificações de lint, teste ou type-check.

<NxCommands commands={['assemble <project-name>']} />

Os <Link path="/pt/guides/typescript-infrastructure#deploying-to-aws">alvos de deploy</Link> dependem de `assemble`, então o deploy constrói apenas o que está prestes a implantar.

:::caution[Use `build` no seu pipeline]
`assemble` é usado para iteração mais rápida durante o desenvolvimento. Execute `build` no seu pipeline de CI para garantir que seu projeto passe pelas verificações de qualidade antes de uma implantação em produção.
:::
