...
:::tip[Funções Lambda]
Se você está construindo uma função AWS Lambda, confira o gerador <Link path="/guides/ts-lambda-function">`ts#lambda-function`</Link>, pois ele configura o empacotamento para você, além de gerar infraestrutura e adicionar observabilidade e type-safety.
:::
...
Os <Link path="/guides/typescript-infrastructure#deploying-to-aws">alvos de deploy</Link> dependem de `assemble`, então o deploy constrói apenas o que está prestes a implantar.
