...
Seu projeto também tem um alvo `package`, que é o irmão somente de artefatos do `build`: ele produz o que seu projeto contribui para uma implantação (sua saída compilada e qualquer bundle, imagem de contêiner ou especificação gerada que ele declara) sem executar as verificações de lint, teste ou type-check.

<NxCommands commands={['package <project-name>']} />

Os <Link path="/pt/guides/typescript-infrastructure#implantando-na-aws">alvos de deploy</Link> dependem de `package` em vez de `build`, então a implantação constrói apenas o que está prestes a implantar. `build` permanece inalterado e ainda faz tudo, então continua sendo o que você executa para verificar um projeto — e o que você deve executar no CI.

:::caution
Como `package` pula as verificações de qualidade, uma implantação não falha mais em um erro de lint ou em um teste unitário com falha. Execute `build` (ou seu pipeline de CI) para verificar seu código.
:::
