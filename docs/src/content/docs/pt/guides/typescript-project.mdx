...
No pnpm, o workspace também define [`catalogMode: strict`](https://pnpm.io/settings#catalogmode) para que `pnpm add` registre novas dependências no catálogo automaticamente. yarn e bun não possuem configuração equivalente, então adicione novas dependências com o protocolo `catalog:` para registrá-las no catálogo, por exemplo `yarn workspace @my-scope/my-library add some-npm-package@catalog:` ou `bun add some-npm-package@catalog: --cwd packages/my-library`.
