...
No pnpm, o workspace também define [`catalogMode: strict`](https://pnpm.io/settings#catalogmode), então `pnpm add` registra a versão no catálogo e a referencia automaticamente.

yarn e bun não possuem equivalente, e seu protocolo `catalog:` apenas *referencia* uma entrada de catálogo existente — ele não pode criar uma. Para adicionar uma nova dependência no yarn ou bun, primeiro registre sua versão no catálogo, depois a referencie do projeto:

- **yarn** — adicione a versão sob `catalog` em `.yarnrc.yml`, depois `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — adicione a versão sob `catalog` no `package.json` raiz, depois `bun add some-npm-package@catalog: --cwd packages/my-library`
