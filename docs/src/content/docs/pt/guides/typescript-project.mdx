...
**Ferramentas de build e teste** (por exemplo `vitest`, `tsx`, `rolldown`, `@nx/*`, `typescript`, `@biomejs/biome`) são compartilhadas em todo o workspace, então ficam nas `devDependencies` do `package.json` **raiz** em vez de em cada projeto. Essas ferramentas são referenciadas apenas em arquivos de configuração, scripts de build e testes, que a regra de lint isenta.
...
npm não possui recurso de catálogo, então as versões são declaradas diretamente em cada `package.json`. Para mantê-las consistentes entre projetos em um workspace npm, recomendamos [syncpack](https://github.com/JamieMason/syncpack).
:::
...
Se você está publicando seu projeto TypeScript no NPM, seu `package.json` deve declarar as dependências que seu projeto referencia, junto com pontos de entrada (`main`/`exports`). Como em tempo de build seu projeto resolve dependências instaladas via `package.json` da raiz do workspace, a regra `noUndeclaredDependencies` do Biome avisará se seu projeto importar um pacote que não está listado em seu `package.json`.
