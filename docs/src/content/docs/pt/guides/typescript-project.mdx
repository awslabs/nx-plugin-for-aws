...
Seu projeto também tem um alvo `assemble`, que é o irmão somente de artefatos do `build`: ele produz o que quer que seu projeto contribua para uma implantação (sua saída compilada e qualquer bundle, imagem de contêiner ou especificação gerada que ele declare) sem executar as verificações de lint, teste ou type-check.

<NxCommands commands={['assemble <project-name>']} />

Os <Link path="/pt/guides/typescript-infrastructure#deploying-to-aws">alvos de deploy</Link> dependem de `assemble` em vez de `build`, então o deploy constrói apenas o que está prestes a implantar. `build` permanece inalterado e ainda faz tudo, então continua sendo o que você executa para verificar um projeto — e o que você deve executar no CI.

:::caution
Como `assemble` pula as verificações de qualidade, um deploy não falha mais em um erro de lint ou em um teste unitário com falha. Execute `build` (ou seu pipeline de CI) para verificar seu código.
:::
