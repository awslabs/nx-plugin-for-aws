...
Com catálogos desabilitados, os geradores escrevem versões de dependências diretamente no `package.json` de cada projeto, e manter versões alinhadas entre projetos é sua responsabilidade (por exemplo usando [syncpack](https://github.com/JamieMason/syncpack)). Quando catálogos estão habilitados no pnpm, o workspace também define [`catalogMode: prefer`](https://pnpm.io/settings#catalogmode) para que `pnpm add` continue adicionando novas dependências ao catálogo.
