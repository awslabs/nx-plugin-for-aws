...
:::tip[Não precisa de package.json]
...
Este deve declarar as dependências que seu projeto referencia. Como em tempo de build seu projeto resolve dependências instaladas via `package.json` da raiz do workspace, a regra `noUndeclaredDependencies` do Biome avisará se seu projeto importar um pacote que não está listado em seu `package.json`.
