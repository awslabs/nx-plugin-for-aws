...
:::tip
Seu `Dockerfile` fica colocado junto ao projeto do servidor MCP. Para CDK, o construct usa `buildContexts` para construir a imagem diretamente a partir deste Dockerfile. Para Terraform, a imagem docker é pré-construída usando uma tag (exemplo `my-scope-my-project-mcp-server:latest`) que é referenciada por sua infraestrutura Terraform.
:::
