...
Gere um servidor Python do [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) para fornecer contexto a Modelos de Linguagem de Grande Porte (LLMs), e opcionalmente implante-o no [Amazon Bedrock AgentCore](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/).
...
<FileTree>
  - your-project/
    ...
    - pyproject.toml Atualizado com dependências do MCP
...
- Constrói uma imagem docker a partir do `Dockerfile` que executa seu servidor MCP na porta `8000`, conforme o [contrato de protocolo MCP](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-service-contract.html#mcp-protocol-contract)

:::tip
A imagem docker é construída usando uma tag (por exemplo `my-scope-my-project-mcp-server:latest`), da qual o `Dockerfile` do constructo CDK herda, permitindo que seu `Dockerfile` fique junto com seu projeto de servidor MCP.
:::
