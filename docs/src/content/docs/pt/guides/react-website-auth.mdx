...
`mfa` aceita `Mfa.OFF` / `Mfa.OPTIONAL` / `Mfa.REQUIRED` no CDK, e `"OFF"` / `"OPTIONAL"` / `"ON"` no Terraform. `mfaSecondFactor.sms` / `mfa_second_factor_sms` e o equivalente `otp` habilitam ou desabilitam cada método de segundo fator independentemente; eles não têm efeito quando `mfa` está desativado.

:::caution[Terraform vincula SMS MFA à verificação de número de telefone]
Ao contrário do construct CDK, o provedor AWS do Terraform não tem como oferecer SMS como método MFA independentemente de usá-lo para verificação de número de telefone. Definir `mfa_second_factor_sms = false` também desabilita a verificação de número de telefone baseada em SMS (`auto_verified_attributes` remove `phone_number`).
:::
