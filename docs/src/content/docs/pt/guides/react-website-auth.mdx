...
O cliente do User Pool gerado permite `http://localhost:4200` e `http://localhost:4300` como URLs de callback/logout OAuth (para desenvolvimento local contra o pool implantado), além da URL de distribuição do CloudFront. Mantenha esta lista de permissões mínima: para implantações de produção reforçadas, considere remover as URLs localhost (ou usar um cliente separado para desenvolvimento local) assim que você não executar mais localmente contra o User Pool implantado.
:::
