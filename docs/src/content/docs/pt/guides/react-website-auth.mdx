...
:::caution[SMS MFA está vinculado à verificação de número de telefone]
O Cognito rejeita a verificação de número de telefone baseada em SMS a menos que SMS também seja um método MFA habilitado sempre que MFA não estiver desativado, tanto no CDK quanto no Terraform. Desabilitar `mfaSecondFactor.sms` / `mfa_second_factor_sms` enquanto `mfa` está ativado, portanto, também desabilita a verificação de número de telefone baseada em SMS para o user pool.
:::
