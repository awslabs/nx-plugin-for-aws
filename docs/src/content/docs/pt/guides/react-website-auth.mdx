...
Depois de observar as avaliações de risco para seus usuários, você pode mudar para a aplicação de função completa para responder automaticamente a atividades arriscadas (por exemplo, exigir MFA ou bloquear login), e então configurar a resposta por nível de risco via [autenticação adaptativa](https://docs.aws.amazon.com/cognito/latest/developerguide/cognito-user-pool-settings-adaptive-authentication.html):
...
Ao contrário das opções `enableWaf` e MFA abaixo, o modo de aplicação não é exposto como uma prop ou variável, então esta edição se aplica a todos os `UserIdentity` em seu workspace.
...
`mfa` aceita `"OFF"` / `"OPTIONAL"` / `"ON"`. `mfa_second_factor_sms` e `mfa_second_factor_otp` habilitam ou desabilitam cada método de segundo fator independentemente; eles não têm efeito quando `mfa` é `"OFF"`.
</Fragment>
</Infrastructure>

:::caution[MFA por SMS está vinculado à verificação de número de telefone]
O Cognito rejeita a verificação de número de telefone baseada em SMS a menos que o SMS também seja um método MFA habilitado sempre que o MFA não estiver desativado, tanto no CDK quanto no Terraform. Desabilitar `mfaSecondFactor.sms` / `mfa_second_factor_sms` enquanto `mfa` está ativado, portanto, também desabilita a verificação de número de telefone baseada em SMS para o user pool.
:::

#### Web Application Firewall (WAF)

Por padrão, o User Pool é associado a uma Web ACL do [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) usando os grupos de regras gerenciadas `AWSManagedRulesCommonRuleSet` e `AWSManagedRulesKnownBadInputsRuleSet`. Você pode desabilitar isso se desejar gerenciar sua própria Web ACL ou não precisar de uma:
...
O cliente User Pool gerado permite a URL de distribuição do CloudFront (e quaisquer nomes de domínio personalizados configurados nele) como URLs de callback/logout OAuth, além das próprias URLs de servidor de desenvolvimento local e preview de cada website conectado (por exemplo, `http://localhost:4200` e `http://localhost:4300` para o primeiro website gerado em um workspace) para desenvolvimento local contra o pool implantado. Cada website gerado com `ts#website` recebe suas próprias portas distintas, então um workspace com vários websites terá mais de um par `http://localhost:<port>` na lista de permissões aqui.
...
<Fragment slot="terraform">
Edite as `callback_urls`/`logout_urls` em `packages/common/terraform/src/core/user-identity/identity/identity.tf`.
</Fragment>
</Infrastructure>
:::
...
Para conceder aos usuários autenticados acesso para executar certas ações, como conceder permissões para invocar uma API, você pode adicionar declarações de política IAM à função autenticada do identity pool:
