...
Adicione um target `trivy` que `dependsOn` do seu target `docker`. Ele salva a imagem construída em um tarball e a escaneia através de um bind mount relativo ao workspace, para que o mesmo comando funcione tanto com `docker` quanto com `finch`:
...
:::tip[Trivy não faz parte do `build` — execute-o no CI]
O escaneamento é intencionalmente **não** conectado ao `build`. O resultado do Trivy depende de seu banco de dados de vulnerabilidades, que é atualizado constantemente: um escaneamento que passa hoje pode falhar amanhã quando um novo CVE é publicado contra uma imagem base ou dependência que você não alterou. Acoplá-lo ao `build` tornaria os builds locais e de CI não-determinísticos e bloquearia trabalhos não relacionados.

Em vez disso, execute-o como uma etapa dedicada em seu pipeline de CI (e localmente antes de um lançamento):

<PackageManagerShortCommand commands={['trivy']} />

Quando um escaneamento começa a falhar em um CVE recém-publicado, atualize o pacote afetado ou a imagem base em seu `Dockerfile`, ou suprima-o em `.trivyignore` se ainda não houver correção (veja abaixo).
:::
...
O bloco `data.external.docker_digest` garante que o `null_resource` seja re-executado sempre que o hash da imagem local mudar, disparando um novo push a cada mudança significativa no código. A imagem é enviada sob uma tag imutável baseada em conteúdo derivada de seu digest, para que o repositório ECR possa usar mutabilidade de tag `IMMUTABLE` e rejeitar qualquer tentativa de sobrescrever uma tag existente.
