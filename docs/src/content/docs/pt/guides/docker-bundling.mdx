...
Fixe cada dependência instalada dentro do `Dockerfile` a uma versão exata (como acima) — caso contrário, `npm install` silenciosamente pegará uma versão mais recente na próxima vez que você construir, produzindo imagens que se comportam de forma diferente dependendo de quando foram construídas. O mesmo conselho se aplica à imagem base: prefira um digest ou tag específica (por exemplo `node:22.14.0`) em vez de uma tag flutuante como `lts`.
:::
