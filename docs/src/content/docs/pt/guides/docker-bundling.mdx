...
Crie um `Dockerfile` no diretório de origem do seu projeto. O arquivo não faz nada além de `COPY` do bundle para uma imagem base Node, além de `npm install` de quaisquer pacotes `external` que não puderam ser empacotados. Coloque a etapa `RUN npm install` **antes** do `COPY`, para que o Docker possa fazer cache da camada `node_modules` instalada e só re-executá-la quando a lista de dependências realmente mudar. Montar um cache no diretório de cache do npm permite que builds repetidos reutilizem pacotes baixados em vez de buscá-los novamente:
