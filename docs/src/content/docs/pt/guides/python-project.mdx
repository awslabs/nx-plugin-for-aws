...
Quando você usa seu projeto Python como código de runtime (por exemplo, como o handler para uma função AWS lambda), você precisará criar um bundle do código-fonte e todas as suas dependências. Os geradores <Link path="guides/python-lambda-function">`py#lambda-function`</Link>, <Link path="guides/fastapi">`py#api`</Link> e <Link path="guides/py-mcp-server">`py#mcp-server`</Link> adicionam isso para você. Para adicionar um manualmente, adicione targets como o seguinte ao seu arquivo `project.json`, correspondendo à forma que esses geradores fornecem:
