...
Arquivos de teste são excluídos da entrada nomeada `production` em `nx.json`, que cobre o diretório `tests` assim como arquivos `test_*.py`, `*_test.py` e `conftest.py` em qualquer lugar do seu projeto.
