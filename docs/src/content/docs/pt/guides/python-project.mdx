...
A opção `type` seleciona se você obtém uma aplicação (um projeto implantável, por exemplo uma função lambda ou imagem de contêiner) ou uma biblioteca (código reutilizável, importado por outros projetos). O padrão é `application`, então passe `--type=library` para uma biblioteca:
...
`assemble` é usado para iteração mais rápida durante o desenvolvimento. Execute `build` no seu pipeline de CI para garantir que seu projeto passe pelas verificações de qualidade antes de um deployment de produção.
:::
