...
Seu projeto também tem um target `package`, que é o irmão somente de artefatos do `build`: ele produz o que quer que seu projeto contribua para uma implantação (sua saída compilada, e qualquer bundle, imagem de contêiner ou especificação gerada que ele declare) sem executar as verificações de lint, teste ou verificação de tipos.

<NxCommands commands={['package <project-name>']} />

Os <Link path="guides/typescript-infrastructure#deploying-to-aws">targets de deploy</Link> dependem de `package` em vez de `build`, então a implantação constrói apenas o que está prestes a implantar. `build` permanece inalterado e ainda faz tudo, então continua sendo o que você executa para verificar um projeto — e o que você deve executar em CI.

:::caution
Como `package` pula as verificações de qualidade, uma implantação não falha mais em um erro de lint, um teste unitário com falha ou um erro de tipo. Execute `build` (ou seu pipeline de CI) para verificar seu código.
:::
