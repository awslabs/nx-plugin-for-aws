...
Os targets de deploy dependem de `assemble` em vez de `build`, então a implantação constrói apenas o que está prestes a implantar. `build` permanece inalterado e ainda faz tudo, então continua sendo o que você executa para verificar um projeto — e o que você deve executar no CI.
