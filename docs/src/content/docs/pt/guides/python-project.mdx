...
Seu projeto também tem um target `assemble`, que é o irmão somente de artefatos do `build`: ele produz o que seu projeto contribui para uma implantação (sua saída compilada e qualquer bundle, imagem de contêiner ou especificação gerada que ele declara) sem executar as verificações de lint, teste ou verificação de tipos.

<NxCommands commands={['assemble <project-name>']} />

Os <Link path="guides/typescript-infrastructure#deploying-to-aws">targets de deploy</Link> dependem de `assemble` em vez de `build`, então a implantação constrói apenas o que está prestes a implantar. `build` permanece inalterado e ainda faz tudo, então continua sendo o que você executa para verificar um projeto — e o que você deve executar na CI.

:::caution
Como `assemble` pula as verificações de qualidade, uma implantação não falha mais em um erro de lint, um teste unitário com falha ou um erro de tipo. Execute `build` (ou seu pipeline de CI) para verificar seu código.
:::
