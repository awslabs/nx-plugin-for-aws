...
Você selecionou `Any` para o `eventSource`, o que significa que o middleware `parser` não está configurado e o parâmetro `event` é tipado como `any`. Se você posteriormente quiser segurança de tipos em tempo de compilação e validação em runtime, regenere a função com um `eventSource` específico.
</OptionFilter>
