...
Se você escolheu um `event` ao gerar sua função Lambda, seu handler será instrumentado com o middleware [`parser` do AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Por exemplo:
...
Se você selecionou `Any` para o `event`, o middleware `parser` não está configurado e o parâmetro `event` é tipado como `any`. Regenere a função com um `event` específico se você quiser segurança de tipos em tempo de compilação e validação em runtime.
</OptionFilter>
