...
Se você selecionou `Any` para o `eventSource`, o middleware `parser` não está configurado e o parâmetro `event` é tipado como `any`. Regenere a função com um `eventSource` específico se você quiser segurança de tipos em tempo de compilação e validação em runtime.
</OptionFilter>
