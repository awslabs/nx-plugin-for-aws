...
5. **Bundling com esbuild** para pacotes de deploy otimizados
...
Métricas CloudWatch são coletadas automaticamente para cada requisição via middleware `logMetrics`. Você pode adicionar métricas personalizadas:
...
Se você escolheu um `eventSource` ao gerar sua função lambda, ela será instrumentada com o middleware [`parser` do AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/). Exemplo:
...
:::caution
Se não quiser que seu handler lance erro quando o evento não conformar com o schema, você pode usar a opção [`safeParse`](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#safe-parsing).
:::

:::tip
Se tiver dados personalizados aninhados em um evento, como um stream DynamoDB ou evento EventBridge, você pode se beneficiar usando [Envelopes](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#envelopes) para garantir segurança de tipos nesses dados.
:::

Se não deseja tipar seu evento, você pode selecionar `Any` para o `eventSource`, resultando em um tipo `any` para o parâmetro event.

## Bundling

O gerador configura automaticamente [esbuild](https://esbuild.github.io/) para pacotes de deploy Lambda otimizados:

Faça bundling de uma função específica com:

<NxCommands commands={['run <project-name>:bundle-<function-name>']} />

Faça bundling de todas as funções lambda no projeto com:
