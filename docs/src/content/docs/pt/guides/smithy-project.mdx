...
No macOS e Linux, a CLI é resolvida pelo [mise](https://mise.jdx.dev/), que o build busca sob demanda, então não há nada para instalar — ele baixa e armazena em cache a versão fixada na primeira vez que você constrói.
...
Como a CLI é instalada globalmente em vez de fixada no seu workspace, atualizar o workspace com `nx migrate` **não** a atualiza. Quando uma migração move as dependências Smithy para frente, atualize sua Smithy CLI global para uma versão correspondente você mesmo — a CLI não pode carregar um modelo construído contra um `smithy-model` mais recente.
:::

### Usando a Smithy CLI diretamente

Por padrão, macOS e Linux resolvem a CLI através do `mise` e Windows usa uma CLI instalada globalmente, mas você pode escolher qualquer abordagem em qualquer plataforma editando o comando do alvo `compile` no `project.json` do seu projeto.
...
:::caution
Quando você invoca a CLI diretamente, sua versão não está mais fixada no workspace, então `nx migrate` não pode movê-la para frente — atualize sua Smithy CLI global independentemente ao atualizar o workspace.
:::
