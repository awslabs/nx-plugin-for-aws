...
No macOS e Linux, a CLI é resolvida pelo [mise](https://mise.jdx.dev/), que o build busca sob demanda, então não há nada para instalar — ele baixa e armazena em cache a versão fixada na primeira vez que você constrói. No Windows, é um pré-requisito que você instala por conta própria — veja <Link path="guides/ts-smithy-api#building-on-windows">Construindo no Windows</Link>.
