...
A CLI é resolvida pelo [mise](https://mise.jdx.dev/), adicionado ao seu workspace como uma dependência de desenvolvimento, então não há nada para instalar — ele baixa e armazena em cache a versão fixada na primeira vez que você constrói.

:::caution[Windows]
`mise` não publica nenhum pacote Windows para npm, então siga o [guia de instalação da Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) para instalá-la você mesmo antes de construir.
:::

:::tip[Construindo em CI]
`mise` lê os metadados de lançamento da CLI da API do GitHub, cujo limite de taxa não autenticado é compartilhado por endereço IP e facilmente esgotado em um executor de CI — o download então falha com `403 Forbidden`. Defina `GITHUB_TOKEN` em seu ambiente de build para aumentá-lo. Construções subsequentes usam o cache local e não fazem nenhuma requisição.
:::
