...
Algumas opções só se aplicam para certos valores de outra. Use `x-when` para especificar quais, para que o site de documentação omita a opção onde ela não se aplica e declare a condição onde ela se aplica:
...
Múltiplas chaves são combinadas com AND e uma lista de valores é combinada com OR dentro de uma chave, correspondendo a `<OptionFilter when={{…}}>`:
...
`x-when` documenta a opção; não impõe nada, então um gerador ainda valida ou ignora uma opção que não se aplica à execução que foi fornecida.
