...
<FileTree>
  - src/\<name>/
    ...
    - schema.d.ts Tipos TypeScript para o schema
    - generator.ts Implementação básica do generator
...
:::tip
Recomendamos gerar um projeto TypeScript dedicado para todos os seus generators usando primeiro o generator `ts#project`. Por exemplo:
...
O arquivo `schema.json` define as opções aceitas pelo seu generator. Ele segue o formato [JSON Schema](https://json-schema.org/) com extensões específicas do Nx.
...
"project": {
  ...
  "x-prompt": "Em qual projeto deseja adicionar o componente?",
  "x-dropdown": "projects"
}
```
...
Isso permite executar o generator como `nx g your-generator meu-componente` ao invés de `nx g your-generator --name=meu-componente`.
...
Opções podem ter prioridades `"important"` ou `"internal"`. Isso ajuda o Nx a ordenar propriedades na extensão VSCode.
...
Para mais detalhes sobre schemas, consulte a [documentação de Opções do Nx Generator](https://nx.dev/extending-nx/recipes/generator-options).
...
- Tornar propriedades obrigatórias ou opcionais (use o sufixo `?` para opcionais)
...
Um generator é uma função que modifica um sistema de arquivos virtual (a `Tree`), lendo e escrevendo arquivos para fazer as alterações desejadas.

Aqui estão algumas operações comuns que você pode realizar no seu generator:
...
if (nodes.length > 0) {
  ...
  // Escrever conteúdo atualizado de volta na tree
  tree.write(
    'path/to/version.ts',
    ts
      .createPrinter({
...
:::note
Se não visualizar seu generator na UI do plugin VSCode, você pode atualizar seu Nx Workspace com:
...
- Use snapshots para conteúdo complexo de arquivos
...
Quando executado em nosso repositório, este generator gerará os seguintes arquivos:

<FileTree>
  - packages/nx-plugin/src/\<name>/
    - schema.json Schema para entrada do generator
    - schema.d.ts Tipos TypeScript para o schema
