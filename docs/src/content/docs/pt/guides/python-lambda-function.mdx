...
Se você selecionou `Any` para seu `eventSource`, o decorator `@event_parser` não é aplicado e o handler recebe um dict de evento sem tipagem. Regenere com um `eventSource` específico se desejar validação Pydantic e acesso a campos tipados.
</OptionFilter>
