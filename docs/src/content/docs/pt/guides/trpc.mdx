...
:::tip[Tipo de API]
Selecione `ServerlessApiGatewayRestApi` (padrão) como seu `computeType` se desejar usar [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) para transmitir respostas em streaming. Selecione `BedrockAgentCoreRuntimeWebSocket` para implantar o servidor tRPC como um container no Bedrock AgentCore com subscriptions WebSocket em streaming — veja [Implantando no Bedrock AgentCore](#implantando-no-bedrock-agentcore) abaixo.
:::

:::tip[Padrão de integração]
A opção `integrationPattern` tem como padrão `isolated`, que cria um Lambda por procedimento tRPC. Selecione `shared` se preferir um único handler Lambda compartilhado para toda a API, com opção de substituições por procedimento. A opção `integrationPattern` não se aplica quando `computeType` é `BedrockAgentCoreRuntimeWebSocket`.
:::
...
<FileTree>
  ...
  - Dockerfile Imagem de container para implantações AgentCore (somente tipo de computação AgentCore)
...
Subscriptions são suportadas apenas quando `computeType` é `ServerlessApiGatewayRestApi` (padrão), pois API Gateway HTTP APIs não suportam streaming de resposta.
:::

tRPC subscriptions permitem transmitir dados do servidor para o cliente usando [Server-Sent Events (SSE)](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events). Quando você seleciona `ServerlessApiGatewayRestApi` como seu tipo de computação, o gerador configura automaticamente a infraestrutura necessária para streaming, bem como um handler Lambda de streaming e o helper de schema `ZodAsyncIterable`.
...
## Implantando no Bedrock AgentCore

Definir `computeType` como `BedrockAgentCoreRuntimeWebSocket` implanta o servidor tRPC como um servidor WebSocket containerizado no [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime.html) em vez de API Gateway + Lambda. AgentCore oferece:

- **Subscriptions em streaming via WebSocket** — mensagens full duplex para subscriptions tRPC sem timeouts de 30 segundos do Lambda ou API Gateway.
- **Afinidade de sessão** — o cabeçalho `x-amzn-bedrock-agentcore-runtime-session-id` é propagado para o contexto tRPC como `ctx.sessionId`, permitindo gerenciar estado por sessão.
- **Runtime gerenciado** — AgentCore gerencia escalonamento, verificações de saúde (`/ping`) e autorização.

:::caution[Opções de autenticação]
AgentCore suporta apenas autenticação `IAM` ou `Cognito`. Selecionar `auth=None` com `computeType=BedrockAgentCoreRuntimeWebSocket` falhará na geração.
:::

### Como funciona

Quando você escolhe `BedrockAgentCoreRuntimeWebSocket`, o gerador emite:

- `src/handler-agentcore.ts` — um servidor Node WebSocket + HTTP que expõe `/ping` para verificações de saúde do AgentCore e `/ws` para subscriptions tRPC via `@trpc/server/adapters/ws`.
- `Dockerfile` — containeriza o servidor empacotado e inclui o [AWS Distro for OpenTelemetry](https://aws-otel.github.io/docs/getting-started/js-sdk/trace-metric-auto-instr) para observabilidade gerenciada.
- Um construct CDK (ou módulo Terraform) que provisiona um `Runtime` Bedrock AgentCore, concede permissão para invocar modelos Bedrock e ler configuração de runtime, e publica o ARN do runtime em `runtimeConfig.apis.<YourApiName>` para o frontend capturar.
- Um target de build `docker` conectado ao target `build` do projeto para que `nx build` produza a imagem de container.

### Acessando o id de sessão

Cada procedimento recebe `ctx.sessionId` quando a API é implantada no AgentCore:
...
Localmente, `nx serve` executa um servidor HTTP + WebSocket standalone para que clientes possam se conectar da mesma forma que fazem em produção. O id de sessão tem como padrão um UUID aleatório quando o cabeçalho não está presente.
...
:::warning[Premissa de autenticação]
...
Para React, use o gerador <Link path="guides/connection/react-trpc">Connection</Link>.
