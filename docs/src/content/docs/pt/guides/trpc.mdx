...
### Escolhendo um Padrão de Integração

O gerador tRPC suporta dois padrões de infraestrutura via opção `integrationPattern`:

- `isolated` é o padrão. Gera uma função Lambda por procedimento.
- `shared` gera uma única integração Lambda `$router` padrão para toda a API e a reutiliza para cada procedimento, a menos que você forneça substituições por procedimento.

`isolated` oferece infraestrutura mais granular e facilita anexar diferentes permissões ou configurações por operação. `shared` reduz a proliferação de Lambdas e integrações, enquanto ainda permite substituir procedimentos específicos com integrações dedicadas quando necessário.

Ambos os padrões de integração são atualmente suportados para `ServerlessApiGatewayRestApi` e `ServerlessApiGatewayHttpApi`.
