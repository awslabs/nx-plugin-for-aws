...
:::tip
Selecione `ServerlessApiGatewayRestApi` (padrão) como seu `computeType` se desejar usar [tRPC Subscriptions](https://trpc.io/docs/server/subscriptions) para transmitir respostas em streaming.
:::

:::tip
A opção `integrationStyle` tem como padrão `Individual Functions`, que cria um Lambda por procedimento tRPC. Selecione `Router` se preferir um único handler Lambda compartilhado para toda a API.
:::
...
### Escolhendo um Estilo de Integração

O gerador tRPC suporta dois padrões de infraestrutura via opção `integrationStyle`:

- `Individual Functions` é o padrão. Gera uma função Lambda por procedimento.
- `Router` gera uma única função Lambda compartilhada para toda a API e a reutiliza para cada integração.

`Individual Functions` oferece infraestrutura mais granular e facilita anexar diferentes permissões ou configurações por operação. `Router` reduz a proliferação de Lambdas e integrações, o que pode ser útil para APIs menores ou quando você prefere um handler central único.

Ambos os estilos de integração são atualmente suportados para `ServerlessApiGatewayRestApi` e `ServerlessApiGatewayHttpApi`.
