...
Quando você implanta com `auth: 'cognito'`, o autorizador Cognito User Pools do API Gateway verifica o JWT que o chamador fornece no cabeçalho `Authorization` e coloca as reivindicações verificadas no evento Lambda em `event.requestContext.authorizer.claims`. Nosso middleware apenas lê essas reivindicações — sem chamadas extras do AWS SDK, sem verificação manual de JWT.
