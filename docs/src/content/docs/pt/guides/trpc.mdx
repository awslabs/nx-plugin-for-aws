...
O tracer AWS Lambda Powertools é configurado em `src/middleware/tracer.ts` e acessado via `opts.ctx.tracer`. Use para adicionar traces com AWS X-Ray para fornecer insights detalhados sobre o desempenho e fluxo das requisições da API. Por exemplo:
...
Crie um cliente type-safe para invocar a API de outro backend usando o cliente em `src/client/index.ts`:
