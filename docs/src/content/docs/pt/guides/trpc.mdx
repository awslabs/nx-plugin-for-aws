...
Este exemplo demonstra middleware de identidade para autenticação `IAM`, onde buscamos o chamador no Cognito usando o sub extraído do evento do API Gateway. Para autenticação `Cognito`, o middleware de identidade é mais direto — extraia as claims diretamente do `event`.
...
Se você está chamando sua API de um website React, considere usar o gerador <Link path="guides/connection/react-trpc">Connection</Link> para configurar o cliente.
