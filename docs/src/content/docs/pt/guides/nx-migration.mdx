...
- **Formate o que você escreve.** `updateJson` e `writeJson` re-serializam um arquivo inteiro, expandindo arrays inline em uma forma que o alvo `format` fornecido rejeita — deixando o usuário com uma build falhando após uma atualização que de outra forma seria limpa. Finalize com `formatFilesInSubtree(tree)` para que o diff permaneça limitado às linhas que você pretendia mudar.
