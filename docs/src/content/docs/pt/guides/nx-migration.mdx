...
As migrações são agrupadas pelo dia em que foram criadas, para que sua ordem aproximada permaneça visível rapidamente à medida que a coleção cresce.
...
rate --run-migrations` quando um está disponível.
