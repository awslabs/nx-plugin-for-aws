...
Esta seção se aplica quando você gera com `stageConfig`. Sem ela, o gerador produz uma configuração mais simples onde você gerencia as credenciais AWS por conta própria (por exemplo, exportando `AWS_PROFILE` antes de implantar).
:::
...
| `deploy`         | Implantar qualquer stage durante o desenvolvimento, nomeando o stage ou stacks que você deseja. Usa modo express. |
...
Os targets `deploy-sandbox` e `deploy` usam o [modo express do CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) por padrão (através da flag `--express` do CDK). O modo express termina assim que a configuração do recurso é confirmada como aplicada, em vez de esperar pela estabilização completa, com a propagação continuando em segundo plano, para ciclos de iteração significativamente mais rápidos.

Para aguardar a estabilização completa dos recursos, passe `--no-express`:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox` e `deploy` são apenas para desenvolvimento]
Como o modo express retorna antes que os recursos tenham terminado de estabilizar, `deploy-sandbox` e `deploy` são destinados para desenvolvimento e iteração, não para CI/CD ou implantações em produção. Use [`deploy-ci`](#implantando-na-aws-em-um-pipeline-cicd) em seu pipeline — ele nunca usa modo express, então sempre aguarda a estabilização completa.
:::
...
Este target difere dos targets `deploy` e `deploy-sandbox` de duas maneiras. Primeiro, ele implanta um cloud assembly pré-sintetizado em vez de sintetizar em tempo real, evitando potencial não-determinismo de mudanças de versão de pacotes e garantindo que cada estágio do pipeline implante usando o mesmo cloud assembly. Segundo, ele não usa modo express, então aguarda que cada recurso estabilize completamente antes de reportar sucesso.

:::caution[Sempre use `deploy-ci` para produção]
`deploy` e `deploy-sandbox` usam modo express e retornam antes que os recursos tenham terminado de estabilizar, o que os torna inadequados para CI/CD e implantações em produção. Use `deploy-ci` para esses casos.
:::
