...
`synth` depende de `^package` em vez de `^build`, então sintetizar constrói os artefatos que seus stacks implantam — os bundles Lambda, imagens de contêiner e especificações geradas — sem também executar as verificações de lint, teste e type-check de cada projeto upstream. Consulte <Link path="guides/typescript-project#empacotamento">Empacotamento</Link> para saber como `package` se relaciona com `build`.
...
`deploy-sandbox` e `deploy` dependem de `^package`, então eles constroem os artefatos que estão prestes a implantar e nada mais. `deploy-ci` implanta um assembly que seu pipeline já construiu, então não tem dependências de build.

:::caution
Como os targets de implantação ignoram as verificações de lint, teste e type-check, a implantação não falhará em um erro de lint ou em um teste unitário com falha. Execute `build` antes de implantar qualquer coisa que você se importe, e bloqueie seu pipeline nisso:
...
O modo express termina assim que a configuração do recurso é confirmada como aplicada, em vez de esperar pela estabilização completa, com a propagação continuando em segundo plano, para ciclos de iteração significativamente mais rápidos. É destinado para desenvolvimento e iteração.
:::
