...
| `deploy`         | Implantar qualquer stage, nomeando o stage ou stacks que você deseja. Usa modo express a menos que você gere com `stageConfig`. |
...
Se este target usa modo express depende de como você gerou o projeto:

- **Com `stageConfig`** ele aguarda a estabilização completa dos recursos, já que o target pode nomear qualquer stage incluindo o de produção. Passe `--express` para optar pelo modo express para um stage no qual você está iterando.
- **Sem `stageConfig`** ele usa modo express, como `deploy-sandbox` faz. Como o stage que você nomeia pode ser um de produção, passe `--no-express` para aguardar a estabilização completa dos recursos:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  Use [`deploy-ci`](#implantando-na-aws-em-um-pipeline-cicd) de um pipeline em vez disso — ele nunca usa modo express.
...
Este target difere dos targets `deploy` e `deploy-sandbox` de duas maneiras. Primeiro, ele implanta um cloud assembly pré-sintetizado em vez de sintetizar em tempo real, evitando potencial não-determinismo de mudanças de versão de pacotes e garantindo que cada estágio do pipeline implante usando o mesmo cloud assembly. Segundo, ele nunca usa modo express, então sempre aguarda que cada recurso estabilize completamente antes de reportar sucesso.
