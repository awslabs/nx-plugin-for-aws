...
:::tip
Sua infraestrutura é um projeto TypeScript, portanto você pode consultar a <Link path="guides/typescript-project">documentação de projetos TypeScript</Link> para mais detalhes sobre o uso geral.
:::
...
Se você usou os geradores <Link path="guides/trpc">tRPC API</Link> ou <Link path="guides/fastapi">FastAPI</Link> para criar APIs, notará que já existem alguns constructs disponíveis em `packages/common/constructs` para implantá-los.
...
Se você usou o gerador <Link path="guides/react-website">React website</Link>, notará que já existe um construct em `packages/common/constructs` para implantá-lo. Por exemplo:
...
:::warning
É importante garantir que o website seja declarado _após_ quaisquer constructs de API para que a <Link path="guides/react-website#runtime-configuration">Configuração de Runtime</Link> do website inclua todas as configurações da API.
:::

## Sintetizando sua Infraestrutura

Como parte do target `build`, além de executar os <Link path="guides/typescript-project#building">targets padrão de compilação, lint e teste</Link>, seu projeto de infraestrutura é _sintetizado_ para CloudFormation. Isso também pode ser executado isoladamente através do target `synth`:
...
:::tip
O comando acima implanta _todas_ as stacks definidas em `main.ts`. Você pode optar por implantar uma stack específica, especialmente se tiver configurado múltiplos estágios de uma aplicação:
...
Este target difere ligeiramente do `deploy` padrão, pois garante que a cloud-assembly pré-sintetizada seja implantada, em vez de sintetizar durante a execução. Isso evita problemas potenciais de não-determinismo devido a versões de pacotes, garantindo que todos os estágios do pipeline usem a mesma cloud-assembly.
