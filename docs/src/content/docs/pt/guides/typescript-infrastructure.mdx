...
:::note[Configuração por estágios]
...
:::tip[Confirmar configuração de estágio]
...
Com CDK, o construct de website pode ser declarado em qualquer ponto da sua stack — a configuração de runtime é resolvida preguiçosamente no momento da síntese. Para Terraform, certifique-se de que o módulo de website tenha `depends_on` apropriado para módulos que adicionam ao <Link path="guides/react-website#runtime-configuration">Runtime Config</Link>.
:::
...
Um target `checkov` é adicionado ao seu projeto para executar verificações de segurança na sua infraestrutura usando o [Checkov](https://www.checkov.io/).
...
:::caution[Regras Checkov suprimidas]
...
O target `deploy` usa o [modo express do CloudFormation](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html) por padrão (via flag `--express` do CDK) para acelerar implantações durante o desenvolvimento. O modo express finaliza assim que a configuração do recurso é confirmada como aplicada, em vez de esperar pela estabilização completa, com a propagação continuando em segundo plano, para ciclos de iteração significativamente mais rápidos.

O modo express é destinado ao desenvolvimento e iteração. Para implantar sem ele e esperar pela estabilização completa dos recursos, remova a flag `--express` do comando do target `deploy` no `project.json` do seu projeto de infraestrutura.

Para implantações reais em estágios de produção, use o target `deploy-ci` (veja [abaixo](#deploying-to-aws-in-a-cicd-pipeline)) como parte de um pipeline CI/CD. Este target não usa o modo express, então ele sempre espera pela estabilização completa.
:::
