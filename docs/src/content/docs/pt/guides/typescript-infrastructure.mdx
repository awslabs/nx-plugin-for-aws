...
Se você precisar de uma implantação padrão que espera pela estabilização completa dos recursos, desative com `--no-express`:

<NxCommands commands={['deploy <my-infra> <my-infra>-sandbox/* --no-express']} />

O target `deploy-ci` não usa o modo express, então as implantações de pipeline CI/CD sempre esperam pela estabilização completa.
:::
