...
:::note
Com o CDK, o construct do website pode ser declarado em qualquer ponto da sua stack — a configuração de runtime é resolvida de forma preguiçosa no momento da síntese. Para o Terraform, certifique-se de que o módulo do website tenha `depends_on` apropriado para módulos que adicionam ao <Link path="guides/react-website#runtime-configuration">Runtime Config</Link>.
:::
