...
- **Conexão** — `BaseModel.Meta` define `host` de `config.json` e define `region`, `aws_access_key_id` e `aws_secret_access_key` quando `is_local()` é `True`, apontando o PynamoDB para a instância local do DynamoDB. Na AWS, estes são deixados indefinidos para que o PynamoDB use a cadeia de credenciais padrão.
