...
Quando executado dessa forma, o `runtime-config.json` do website é automaticamente sobrescrito (via [`MODE` do Vite](https://vite.dev/guide/env-and-mode), definido como `local-dev`) para que aponte para seus servidores em execução local em vez de URLs implantadas.
