...
`my_scope_my_db` é o nome do módulo do seu projeto de banco de dados — um único módulo plano nomeado `<scope>_<name>`, visível como o diretório do pacote dentro do projeto.

`.scalars()` desempacota as linhas de resultado em instâncias de `ExampleModel`, que o FastAPI pode serializar.
...
O exemplo abaixo referencia os recursos VPC do guia de implantação do <Link path="guides/py-rdb">Banco de Dados Relacional Python</Link>: as sub-redes `aws_subnet.database` não têm rota de internet, enquanto as sub-redes `aws_subnet.private` têm saída NAT.
