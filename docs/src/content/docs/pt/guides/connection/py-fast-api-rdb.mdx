...
O exemplo abaixo referencia os recursos VPC do guia de implantação <Link path="guides/py-rdb">Banco de Dados Relacional Python</Link>: as sub-redes `aws_subnet.database` não têm rota de internet, enquanto as sub-redes `aws_subnet.private` têm saída NAT.
...
Implante as funções Lambda da API em **sub-redes privadas com saída**, não em sub-redes isoladas privadas. `appconfig_application_id`/`appconfig_application_arn` vêm da aplicação AppConfig de <Link path="guides/runtime-config">configuração de tempo de execução</Link> compartilhada declarada uma vez em seu módulo raiz, não do módulo do banco de dados — passá-los define `RUNTIME_CONFIG_APP_ID` nas funções Lambda e concede a elas acesso de leitura à aplicação.
...
Ao invocar a API diretamente, observe que a saída `api_endpoint` é a URL base do API Gateway sem o caminho do estágio — anexe `/prod` (ou use a saída `stage_invoke_url`) ao chamar a API implantada.
