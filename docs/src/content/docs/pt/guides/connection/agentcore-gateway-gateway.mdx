...
description: Conecte um AgentCore Gateway a outro AgentCore Gateway
when:
...
- packages/\<source-gateway>
  - project.json o destino `dev` do Gateway de origem ganha uma dependência no destino `dev` do gateway de destino
  - local-dev.ts `ATTACHED_MCP_SERVERS` atualizado para que o gateway local agregue o gateway de destino

</FileTree>

O destino `dev` do projeto Gateway de origem ganha uma dependência no destino `dev` do Gateway de destino, então executar o Gateway de origem localmente também inicia o gateway de destino (e, transitivamente, cada servidor MCP anexado a ele). O gateway de destino também é registrado no `local-dev.ts` do projeto Gateway de origem para que o gateway local agregue suas ferramentas.

## Adicionando o destino do gateway à sua pilha

O gerador **não pode** conectar automaticamente o destino do gateway à sua infraestrutura porque não sabe qual pilha ou módulo instancia os Gateways. Adicione uma única chamada para `gateway.addGateway(targetGateway)` você mesmo.

<Infrastructure>
<Fragment slot="cdk">
Na pilha onde você instancia os Gateways, registre o gateway de destino como um destino do gateway de origem:
...
O `name` do destino (`inner-gateway` acima) prefixa nomes de ação Cedar no gateway externo — veja a <Link path="guides/agentcore-gateway">seção Escrevendo Políticas</Link>. A entrada `additional_iam_policy_statements` concede à função de execução do gateway externo acesso de invocação ao gateway interno, o que é necessário tanto para buscar as ferramentas do gateway interno no momento da criação do destino quanto para rotear chamadas em tempo de execução. `policy_dependencies` garante que as políticas Cedar que referenciam as ações deste destino sejam criadas após o destino tê-las registrado.

:::note[Ordenação de destinos]
O AgentCore busca as ferramentas do gateway interno quando o `aws_bedrockagentcore_gateway_target` é criado, então o gateway interno já deve estar servindo-as. Defina o `tool_dependencies` do gateway interno para os ids de seus próprios destinos: seu `gateway_url` então flui através de uma sonda de prontidão que pesquisa `tools/list` até que essas ferramentas sejam servidas, então o destino do gateway externo aguarda o gateway interno estar pronto.
:::
</Fragment>
</Infrastructure>

## Políticas Cedar através de gateways encadeados
