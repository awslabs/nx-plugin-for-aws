...
Se seu agente usa autenticação IAM, a função autenticada do Cognito Identity Pool deve receber permissão para invocar o agente. Você pode usar o método `grantInvokeAccess` que concede tanto as permissões `InvokeAgentRuntime` quanto `InvokeAgentRuntimeWithWebSocketStream`:
