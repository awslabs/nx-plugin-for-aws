...
- **IAM** (padrão): Usa URLs pré-assinadas AWS SigV4 para autenticar a conexão WebSocket. As credenciais são obtidas do Cognito Identity Pool configurado com a autenticação do seu site. No modo `serve-local`, a assinatura é automaticamente ignorada quando o Cognito não está configurado
...
3. Para autenticação IAM, a assinatura de requisição é automaticamente ignorada no modo `serve-local` quando o Cognito não está configurado
