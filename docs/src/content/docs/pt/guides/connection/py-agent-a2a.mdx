...
1. Um projeto Python com um componente <Link path="guides/py-agent">Strands Agent</Link> (qualquer protocolo)
2. Um projeto com um componente Strands Agent gerado com `--protocol=A2A` e `--auth=IAM` (seja <Link path="guides/ts-agent">`ts#agent`</Link> ou <Link path="guides/py-agent">`py#agent`</Link>)
