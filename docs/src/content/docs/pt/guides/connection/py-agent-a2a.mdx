...
1. Um projeto Python com um componente <Link path="guides/py-agent">Agent</Link> (qualquer protocolo)
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_a2a\_client\_strands.py Cliente A2A Strands encapsulando a configuração
      - auth/ `httpx.Auth` SigV4 / encaminhamento de sessão independente de framework
    - app
      - \<target\_agent\_name>\_client\_strands.py Cliente Strands por conexão para cada agente A2A
