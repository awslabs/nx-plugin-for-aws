...
1. Um projeto TypeScript com um componente <Link path="guides/ts-agent">Agent</Link>
...
- packages/common/agent-connection
  - src
    ...
    - core
      - agentcore-endpoints.ts Resolução de ARN/URL agnóstica de framework
      - agentcore-fetch.ts Fetch agnóstico de framework com SigV4 / JWT / encaminhamento de sessão
      - agentcore-mcp-transport.ts Transporte MCP agnóstico de framework
      - agentcore-mcp-client-strands.ts Cliente MCP Strands que envolve o transporte
...
- Atualiza o alvo `dev` do agente para depender do alvo serve do servidor MCP
