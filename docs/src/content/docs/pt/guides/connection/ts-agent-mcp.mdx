...
O gerador `connection` pode conectar seu <Link path="guides/ts-agent">TypeScript Strands Agent</Link> a um servidor MCP (seja <Link path="guides/ts-mcp-server">TypeScript</Link> ou <Link path="guides/py-mcp-server">Python</Link>).
