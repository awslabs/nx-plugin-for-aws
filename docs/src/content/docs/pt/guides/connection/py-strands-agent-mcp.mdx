...
O ARN do runtime AgentCore do servidor MCP é automaticamente registrado no namespace `connection` da <Link path="guides/runtime-config">Configuração de Runtime</Link> pelo construtor CDK gerado, para que o agente possa descobri-lo em tempo de execução.
</Fragment>
