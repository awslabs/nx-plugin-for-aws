...
O gerador `connection` conecta um <Link path="guides/ts-strands-agent">TypeScript Strands Agent</Link> a um projeto de <Link path="guides/ts-rdb">Banco de Dados Relacional</Link>, disponibilizando um cliente Prisma dentro da sua fábrica de agente.
...
O cliente Prisma é instanciado dentro de `getAgent()`. Como o gerador `ts#strands-agent` configura um único Agent por sessão, o cliente também é reutilizado durante o tempo de vida da sessão.
