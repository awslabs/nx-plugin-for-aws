...
O gerador configura o agente para que ele se autentique no Gateway com IAM SigV4 (via assinatura de requisição `httpx`) quando implantado, e se conecte ao gateway local iniciado pelo projeto Gateway quando executado localmente.
...
- Modifica o `agent.py` do seu agente para importar o cliente Gateway e registrar suas ferramentas em `tools`
...
O gerador transforma o `agent.py` do seu agente para usar o cliente Gateway:
...
- **Modo implantado** (`LOCAL_DEV` não definido): ferramentas apontadas para o endpoint MCP do Gateway, assinadas com SigV4.
- **Modo local** (`LOCAL_DEV=true`): ferramentas HTTP simples apontadas para o gateway local iniciado pelo target `dev` do projeto Gateway.

O ID da sessão é propagado automaticamente para servidores MCP downstream via o cabeçalho `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id`.
...
Para executar o agente localmente **contra o Gateway implantado** (por exemplo, para exercitar políticas Cedar), use o target `serve` do agente. Sem `LOCAL_DEV` definido, o cliente resolve a URL do Gateway implantado a partir da configuração de tempo de execução e assina requisições com SigV4 usando suas credenciais AWS locais:
