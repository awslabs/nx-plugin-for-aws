...
- **Modo implantado** (`SERVE_LOCAL` não definido): ferramentas apontadas para o endpoint MCP do Gateway, assinado com SigV4.
- **Modo local** (`SERVE_LOCAL=true`): ferramentas HTTP simples apontadas para o gateway local iniciado pelo alvo `serve-local` do projeto Gateway.
