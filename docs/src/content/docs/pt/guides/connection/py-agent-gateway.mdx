...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py Cliente MCP Strands para o Gateway implantado
      - auth/ `httpx.Auth` SigV4 / encaminhamento de sessão independente de framework
    - app/
      - \<gateway\_snake>\_client\_strands.py Wrapper de cliente Strands por Gateway
...
`MyGatewayClientStrands.create()` retorna um único cliente gerenciável por contexto cujo `list_tools_sync()` produz todas as ferramentas disponíveis através do Gateway:

- **Modo implantado** (`SERVE_LOCAL` não definido): um único `MCPClient` apontado para o endpoint MCP do Gateway, assinado com SigV4.
- **Modo local** (`SERVE_LOCAL=true`): um `MCPClient` HTTP simples apontado para o gateway local iniciado pelo alvo `serve-local` do projeto Gateway.
