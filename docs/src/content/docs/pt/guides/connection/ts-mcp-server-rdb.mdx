...
Executar o gerador novamente com um destino diferente adiciona o segundo banco de dados ao lado do primeiro. Ambos são obtidos dentro de `createServer`:
...
O construct do servidor MCP gerado implementa `IGrantable` e `IConnectable`, então você pode conceder acesso de rede e IAM ao banco de dados diretamente no construct.
...
<Snippet name="connection/ts-mcp-server-rdb-ssl-requirements" parentHeading="Requisitos de SSL ao Conectar Sem RDS Proxy" />
