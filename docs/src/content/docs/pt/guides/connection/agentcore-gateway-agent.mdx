...
Implantações de Runtime VPC do AgentCore são suportadas apenas em certas zonas de disponibilidade. Se uma implantação falhar com `subnets are in unsupported availability zones`, restrinja a VPC (ou a seleção de sub-rede) às zonas suportadas para sua região.
:::
</Fragment>
