...
:::caution[Agentes Cognito precisam de um Gateway Cognito]
Um agente `auth: cognito` é invocado encaminhando o token que o Gateway validou na entrada, então ele requer um Gateway gerado com `auth: cognito` também — um Gateway `auth: iam` autentica seus chamadores com SigV4 e não tem token bearer para encaminhar. O gerador rejeita esse emparelhamento. Um Gateway Cognito pode colocar na frente um agente `auth: iam`, que ele invoca com seu próprio papel.
:::
