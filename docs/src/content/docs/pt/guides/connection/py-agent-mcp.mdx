...
1. Um projeto Python com um componente <Link path="guides/py-agent">Agent</Link>
...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      - agentcore\_endpoints.py Resolução de ARN/URL agnóstica de framework
      - agentcore\_mcp\_transport.py Transporte MCP agnóstico de framework
      - agentcore\_mcp\_client\_\<framework>.py Cliente MCP que envolve o transporte para o framework do seu agente
      - auth/ `httpx.Auth` agnóstico de framework para SigV4 / encaminhamento de sessão
    - app
...
O ID de sessão do AgentCore é propagado para o servidor MCP automaticamente via o cabeçalho `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` para ambos os frameworks, garantindo consistência para [Bedrock AgentCore Observability](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability.html).
