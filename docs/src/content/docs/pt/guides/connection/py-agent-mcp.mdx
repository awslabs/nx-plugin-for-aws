...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    ...
    - core
      ...
      - agentcore\_mcp\_client\_strands.py Cliente MCP Strands que envolve o transporte
      - auth/ `httpx.Auth` agnóstico de framework para SigV4 / encaminhamento de sessão
    - app
      - \<mcp\_server\_name>\_client\_strands.py Cliente Strands por conexão para cada servidor MCP
