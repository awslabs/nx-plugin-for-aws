...
`create()` sempre retorna um único `McpClient`. Quando implantado (`SERVE_LOCAL` não definido), ele aponta para o endpoint MCP do Gateway e autentica com SigV4. Quando `SERVE_LOCAL=true`, é um cliente multiplex que distribui para todos os servidores MCP em `ATTACHED_MCP_SERVERS`, então o mesmo `agent.ts` funciona uniformemente em ambos os modos.
