...
Se estiver trabalhando ativamente na aplicação React e na API Smithy simultaneamente, use o target `dev` da aplicação React (um alias para `serve-local`) que regenerará automaticamente o cliente quando a API mudar, além de recarregar automaticamente o site e o servidor local da API Smithy:
