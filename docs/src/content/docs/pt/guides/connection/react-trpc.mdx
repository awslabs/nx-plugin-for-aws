...
O gerador inspeciona o `computeType` do backend e produz o client correspondente:

- **`ServerlessApiGatewayRestApi`** — link HTTP com `httpSubscriptionLink` sobre Server-Sent Events para subscriptions tRPC.
- **`ServerlessApiGatewayHttpApi`** — link HTTP simples (sem subscriptions).
- **`BedrockAgentCoreRuntimeWebSocket`** — link WebSocket contra a URL do runtime AgentCore, com handshake assinado por SigV4 (IAM) ou subprotocolo JWT codificado em base64url (Cognito). O ARN do runtime é lido de `runtimeConfig.apis.<ApiName>` e convertido para uma URL `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` em tempo real.
