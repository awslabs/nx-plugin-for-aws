...
4. Cognito Auth adicionado via <Link path="/guides/react-website-auth">gerador `ts#website#auth`</Link> se conectando a uma API que usa autenticação Cognito ou IAM
