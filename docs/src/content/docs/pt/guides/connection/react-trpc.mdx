...
Subscriptions são suportadas apenas quando a API tRPC usa `rest-lambda` (REST API) como tipo de computação. API Gateway HTTP APIs não suportam streaming de resposta.
:::
