...
| ------------ | ------------------------------------------------------------------------ |
| `Cloudscape` | Mensagens do assistente e do usuário são renderizadas dentro de componentes Cloudscape `ChatBubble` com `Avatar`s gen-AI, correspondendo ao [padrão de chat de IA generativa do Cloudscape](https://cloudscape.design/patterns/genai/generative-AI-chat/). A barra de ferramentas de mensagens se torna um `ButtonGroup` do Cloudscape no slot `actions` da bolha (copiar, curtir/descurtir, regenerar), Markdown é renderizado através do renderizador padrão do CopilotKit dentro da bolha, o indicador de digitação se torna uma `LoadingBar` gen-AI, e o input é substituído por `PromptInput` do Cloudscape. Construído a partir de `@cloudscape-design/components` e `@cloudscape-design/chat-components`. |
| `Shadcn` | Mensagens do assistente são renderizadas com um avatar `Sparkles` e uma bolha estilo card suave `bg-muted` usando estilização Tailwind `prose`; uma barra de ferramentas `Button` revelada ao passar o mouse fornece copiar/curtir/regenerar. Mensagens do usuário são renderizadas alinhadas à direita em uma bolha `bg-primary` com um avatar `User`. O input é substituído por um `Textarea` arredondado + `Button` de ícone em formato de pílula (Enter envia, Shift+Enter insere uma nova linha; cresce automaticamente até 8 linhas), e o indicador de digitação é uma animação de pulsação de três pontos. Usa primitivos shadcn (`Avatar`, `Button`, `Textarea`) do pacote compartilhado `common-shadcn`. |
| `None` (ou qualquer outro) | Nenhum tema aplicado — o wrapper simplesmente re-exporta os componentes padrão do CopilotKit. |
