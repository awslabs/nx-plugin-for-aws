...
AG-UI ainda não é suportado para TypeScript Agents. Veja [agents/sdk-typescript#347](https://github.com/agents/sdk-typescript/issues/347) para o status mais recente. Por enquanto, use o <Link path="guides/py-agent">gerador `py#agent`</Link> com `protocol=AG-UI`.
:::
...
2. Um Python Agent com `protocol=AG-UI` (gerado usando o <Link path="guides/py-agent">gerador `py#agent`</Link>)
