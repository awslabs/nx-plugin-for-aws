...
- src
  - components
    ...
    - copilot
      - index.tsx Re-exporta `CopilotChat`, `CopilotSidebar` e `CopilotPopup` com padrões de slot que correspondem ao `ux` do seu website (Cloudscape, Shadcn, ou nenhum tema).
      - *ThemeComponents*.tsx Componentes de tema por slot (ex: `CloudscapeAssistantMessage.tsx`, `ShadcnChatInput.tsx`). Apenas fornecidos quando `ux` é `cloudscape` ou `shadcn`.
  - hooks
...
- **Desenvolvimento local**: `dev` substitui o valor para a URL local do agente (ex: `http://localhost:8081`)
...
Importe os componentes de chat do módulo gerado `./components/copilot` para que o tema que corresponde ao `ux` do seu website seja aplicado automaticamente:
...
O gerador lê `metadata.ux` do seu projeto de website React e fornece um módulo wrapper temático em `src/components/copilot/index.tsx` para que os componentes de chat correspondam ao resto da sua UI sem nenhuma configuração extra:

| `ux` | Estilização aplicada a `CopilotChat` / `CopilotSidebar` / `CopilotPopup` |
| ------------ | ------------------------------------------------------------------------ |
