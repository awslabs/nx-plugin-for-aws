...
Se estiver trabalhando simultaneamente na aplicação React e no FastAPI, use o target `serve-local` da aplicação React que regenerará automaticamente o cliente sempre que a API mudar, além de recarregar automaticamente o site e o servidor FastAPI local:
