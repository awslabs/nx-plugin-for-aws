...
3. Cognito Auth adicionado via o <Link path="/guides/react-website-auth">gerador `ts#website#auth`</Link> se estiver conectando uma API que usa autenticação Cognito ou IAM
...
Se você <Link path="/guides/fastapi#streaming">configurou seu FastAPI para fazer stream de respostas</Link>, seu hook `useQuery` atualizará automaticamente seus dados conforme novos chunks do stream chegarem.
...
Se você <Link path="/guides/fastapi#streaming">configurou seu FastAPI para fazer stream de respostas</Link>>, o cliente gerado incluirá métodos type-safe para iterar assincronamente sobre chunks em seu stream usando sintaxe `for await`.
