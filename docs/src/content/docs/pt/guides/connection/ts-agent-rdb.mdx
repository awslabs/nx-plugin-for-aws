...
description: Conecte um TypeScript Agent a um Banco de Dados Relacional
when:
  sourceType: ts#agent
  targetType: ts#rdb
---
...
O gerador `connection` conecta um <Link path="guides/ts-agent">TypeScript Agent</Link> a um projeto de <Link path="guides/ts-rdb">Banco de Dados Relacional</Link>, disponibilizando um cliente Prisma dentro da sua factory de agente.
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="Requisitos de SSL ao Conectar Sem RDS Proxy" />
