...
O `name` do destino (`my-mcp-server` acima) é usado como prefixo para nomes de ações Cedar — veja a <Link path="guides/agentcore-gateway">seção Escrevendo Políticas</Link>. `policy_dependencies` garante que as políticas Cedar que referenciam as ações deste destino sejam criadas após o destino ter registrado essas ações.
</Fragment>
</Infrastructure>
