...
inicia um gateway local mais cada servidor MCP anexado em sua porta local atribuída. O gateway local expõe um único endpoint MCP que agrega as ferramentas dos servidores anexados. Agentes conectados ao Gateway via os geradores de conexão de gateway <Link path="guides/connection/ts-agent-gateway">TypeScript</Link> ou <Link path="guides/connection/py-agent-gateway">Python</Link> apontam para ele ao executar com `LOCAL_DEV=true`.
