...
:::caution[Fixe `boto3` na versão acima]
Outros geradores adicionam `boto3` fixado exatamente nesta versão. Como seu workspace compartilha um único ambiente virtual UV e lockfile, adicioná-lo sem fixação (`add boto3`) resolve a versão mais recente e escreve uma restrição `boto3>=<latest>`, que `uv` não pode satisfazer junto com a fixação. Todos os alvos Python no workspace então falham ao resolver dependências.
:::
