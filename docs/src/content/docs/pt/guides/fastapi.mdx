...
:::tip
Recomenda-se especificar modelos de resposta para operações da API para melhor geração de código ao usar o `connection`. <Link path="guides/connection/react-fastapi#errors">Veja detalhes aqui</Link>.
:::
...
Para consumir a API de um site React, use o <Link path="guides/connection/react-fastapi">`connection`</Link>.