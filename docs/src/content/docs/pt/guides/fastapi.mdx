...
- Adicionar a extensão OpenAPI `x-streaming: true` se usar o gerador <Link path="guides/connection/react-fastapi">Connection</Link>.
...
Para consumir streams, utilize o <Link path="guides/connection/react-fastapi#consuming-a-stream">Gerador de Conexão</Link>, que fornece um método type-safe para iterar sobre os chunks transmitidos.
