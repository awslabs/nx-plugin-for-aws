...
:::tip
Recomenda-se especificar modelos de resposta para operações da API para melhor geração de código ao usar o gerador `connection`. <Link path="guides/connection/react-fastapi#errors">Veja detalhes aqui</Link>.
:::
...
- Adicionar a extensão OpenAPI `x-streaming: true` se usar <Link path="guides/connection/react-fastapi">API Connection</Link>.
...
Para consumir streams, utilize o <Link path="guides/connection/react-fastapi#consuming-a-stream">Gerador de Conexão de API</Link>, que fornece um método type-safe para iterar sobre os chunks transmitidos.
...
Para consumir a API de um site React, use o gerador <Link path="guides/connection/react-fastapi">`connection`</Link>.