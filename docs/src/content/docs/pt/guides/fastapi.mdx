...
:::tip[Tipo de API]
Selecione `rest-lambda` (padrão) como seu `infra` se você pretende construir operações de streaming.
:::
...
- scripts
  - generate_open_api.py Script para gerar um schema OpenAPI a partir do app FastAPI
...
O gerador FastAPI cria código de infraestrutura CDK ou Terraform baseado no `iac` selecionado. Use para implantar sua FastAPI.
...
O módulo da API armazena seu zip de implantação Lambda em um bucket S3 de assets compartilhado — veja o <Link path="/guides/terraform-project">guia de infraestrutura Terraform</Link> para detalhes. Instancie o módulo `core/asset-bucket` uma vez por implantação e passe sua saída `bucket_name` para cada módulo de API / Lambda via a entrada `asset_bucket_name`:
...
Não oferecemos suporte a integrações type-safe para Terraform e, portanto, nenhum alvo de geração de código é configurado se você selecionou Terraform para seu `iac`.
:::
</Fragment>
</Infrastructure>
