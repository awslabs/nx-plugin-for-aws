...
O gerador FastAPI cria um novo FastAPI com configuração de infraestrutura AWS CDK ou Terraform. O backend gerado usa AWS Lambda para implantação serverless, exposto via uma API AWS API Gateway. Ele configura [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/) para observabilidade, incluindo logging, rastreamento AWS X-Ray e Cloudwatch Metrics.
...
1. Integração AWS Lambda Powertools para observabilidade
...
<OptionFilter when={{ infra: 'rest-lambda' }} description="Registro de acesso — APIs REST registram solicitações no CloudWatch por padrão">
### Registro de acesso
...
:::tip[Hot Reloading]
Se você está trabalhando ativamente tanto em sua infraestrutura CDK quanto no FastAPI juntos, você pode usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar esses tipos toda vez que você fizer alterações na API:
...
`nx watch` requer o [daemon Nx](https://nx.dev/concepts/nx-daemon), que é habilitado por padrão mas desligado dentro de CI, contêineres Docker e ambientes sandboxed. Onde está desligado, o comando sai com `Daemon is not running` — defina `NX_DAEMON=true` para habilitá-lo.
:::
</Fragment>
<Fragment slot="terraform">
