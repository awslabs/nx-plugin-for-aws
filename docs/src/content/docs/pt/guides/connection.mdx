...
O gerador de conexão faz uso de <Link path="guides/runtime-config">Configuração de Runtime</Link> para passar valores de tempo de implantação (como URLs de API, configurações do Cognito e ARNs de runtime de agente) entre projetos e componentes gerados em tempo de execução para que eles possam se descobrir e conectar uns aos outros.
:::