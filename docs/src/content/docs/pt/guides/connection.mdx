...
O gerador connection faz uso da <Link path="guides/runtime-config">Configuração de Runtime</Link> para passar valores de tempo de implantação (como URLs de API, configurações do Cognito e ARNs de runtime de agentes) entre projetos e componentes gerados em runtime para que possam se descobrir e conectar uns aos outros.
:::

:::tip[Desenvolvimento Local]
Projetos conectados podem ser executados em sua máquina com os targets `serve` e `dev`. Consulte o guia de <Link path="guides/local-development">Desenvolvimento Local</Link> para detalhes.
:::
