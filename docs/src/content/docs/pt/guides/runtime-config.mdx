...
- **`connection`** — configuração que permite que projetos gerados se conectem uns aos outros:
  ...
  - **ARNs de runtime de agentes** — adicionados pelo gerador de conexão quando você conecta um website React a um Strands Agent
- **`agentcore`** — ARNs de runtime do AgentCore para agentes e servidores MCP. Registrados automaticamente pelos constructs de agente/MCP e usados para descoberta server-side (agente → agente via A2A, agente → servidor MCP).

O namespace `connection` também é implantado como um arquivo `runtime-config.json` no bucket S3 do seu website, permitindo a descoberta client-side de recursos de backend. O namespace `agentcore` é apenas server-side (via AppConfig), portanto os ARNs de runtime de agentes não são expostos ao frontend, a menos que você conecte explicitamente um website a um agente.
...
1. **`core/runtime-config/appconfig`** — declarado uma vez, próximo ao topo do módulo raiz. Cria a aplicação AppConfig, ambiente, estratégia de implantação e um configuration profile por namespace. Suas saídas `application_id` e `application_arn` são passadas para cada módulo que lê a configuração de runtime em tempo de execução (agentes, servidores MCP, funções Lambda, etc.).
...
2. **`core/runtime-config/entry`** — uma invocação por contribuição. Módulos gerados de API, agente e MCP chamam isso internamente para publicar suas URLs e ARNs. Chame-o diretamente para publicar configuração personalizada.
...
3. **`core/runtime-config/appconfig-deployment`** — declarado uma vez, no final do módulo raiz, com `depends_on` cobrindo cada módulo que contribui com uma entrada. Agrega as entradas contribuídas em um JSON por namespace e cria a hosted configuration version + deployment contra a aplicação compartilhada.
...
Os constructs de API e agente gerados fazem isso automaticamente — você só precisa fazer isso manualmente se criar suas próprias funções Lambda ou recursos de computação que precisam ler a configuração de runtime.
:::
</Fragment>
<Fragment slot="terraform">
...
:::note[Registro automático]
Os módulos de agente e MCP gerados aceitam as entradas `appconfig_application_id` + `appconfig_application_arn` — passe as saídas do `runtime_config_appconfig` compartilhado e eles conectam a variável de ambiente e as permissões IAM para você. Você só precisa fazer isso manualmente para funções Lambda personalizadas ou recursos de computação.
:::
</Fragment>
</Infrastructure>

:::tip[Desenvolvimento Local]
Se você usar runtime config com namespaces personalizados (por exemplo, `tables`, `features`), você só precisa definir uma única variável de ambiente `RUNTIME_CONFIG_APP_ID` localmente para apontar para sua aplicação AppConfig implantada. Isso permite que suas APIs ou agentes locais recuperem a mesma configuração do ambiente implantado sem precisar replicar todas as variáveis de ambiente individuais.
:::

## Acesso Server-Side via AppConfig

Consumidores server-side, como funções Lambda e agentes, podem recuperar a configuração de runtime do **AWS AppConfig** usando [AWS Lambda Powertools](https://docs.aws.amazon.com/powertools/).
...
Para websites, o namespace `connection` é implantado como um arquivo `runtime-config.json` no bucket S3. Consulte o guia <Link path="guides/react-website#runtime-configuration">React Website Runtime Configuration</Link> para detalhes sobre como acessar esses valores do seu código frontend.