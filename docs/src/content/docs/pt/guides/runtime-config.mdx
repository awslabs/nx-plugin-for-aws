...
Cada construct gerado (APIs, agentes) cria sua própria instância de módulo `appconfig` autocontida. Todas são idênticas — cada uma lê o mesmo conjunto de arquivos de namespace e produz a mesma aplicação AppConfig. Isso garante que cada módulo seja implantável de forma independente.
:::
</Fragment>
</Infrastructure>
...
:::note[Registro automático]
Os módulos de API e agente gerados lidam com isso automaticamente — cada um cria sua própria instância de módulo `appconfig` e conecta a variável de ambiente e as permissões IAM. Você só precisa fazer isso manualmente para funções Lambda personalizadas ou recursos de computação.
:::
</Fragment>
</Infrastructure>
