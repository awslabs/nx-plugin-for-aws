...
Ao usar a CLI do Prisma, execute os comandos a partir do diretório do pacote de banco de dados gerado (por exemplo, `packages/mydb`) para que o Prisma possa encontrar `prisma/schema.prisma` e `prisma.config.ts`.
:::

Você também pode executar a partir da raiz do workspace com o alvo `prisma`:
...
Use o alvo `prisma` gerado, que inicia automaticamente um banco de dados local via Docker antes de executar:
...
:::note
Consulte o <Link path="/guides/trpc">guia do gerador `ts#trpc-api`</Link> para mais detalhes sobre o padrão de middleware.
...
Use o alvo `prisma` para executar qualquer comando da CLI do Prisma contra o banco de dados local. Ele inicia automaticamente o banco de dados via `serve-local` antes de executar, então não há necessidade de configurar um banco de dados local separadamente.

<NxCommands commands={['run <project>:prisma migrate dev']} />

<NxCommands commands={['run <project>:prisma migrate deploy']} />

O alvo `prisma` define `SERVE_LOCAL=true` automaticamente, que `prisma.config.ts` usa para construir a URL de conexão do banco de dados a partir das credenciais locais configuradas no momento da geração.
