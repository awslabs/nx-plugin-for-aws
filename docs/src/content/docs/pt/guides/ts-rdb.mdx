...
O cluster Aurora exporta logs de banco de dados para o CloudWatch Logs (`postgresql` para PostgreSQL, `error` e `slowquery` para MySQL) e habilita o Performance Insights na instância de gravação (criptografado com a chave KMS do cluster) para que as consultas sejam observáveis para solução de problemas e auditoria. Observe que o CloudWatch Logs e o Performance Insights incorrem em custos adicionais.
