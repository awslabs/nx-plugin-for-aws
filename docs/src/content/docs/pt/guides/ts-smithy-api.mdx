...
A opção `integrationPattern` tem como padrão `isolated`, que cria uma Lambda por operação Smithy. Selecione `shared` se você preferir um único handler Lambda compartilhado para toda a API, com substituições opcionais por operação.
:::
...
Como este gerador cria infraestrutura como código baseada no `iac` escolhido, ele criará um projeto em `packages/common` que inclui os constructs CDK ou módulos Terraform relevantes.
...
A API Smithy implantada possui a seguinte arquitetura, com um Web ACL [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) na frente do estágio do API Gateway:
...
### Adicionando uma Biblioteca de Formas

Se você tiver várias APIs Smithy que compartilham os mesmos tipos de dados, você pode definir esses tipos uma vez em uma biblioteca de formas em vez de duplicá-los em cada modelo. Uma biblioteca de formas é um projeto Smithy sem serviço — apenas formas reutilizáveis — que qualquer número de projetos Smithy pode depender.
...
O modelo da sua API pode então referenciar suas formas com `use`:
...
Consulte o <Link path="guides/smithy-project">guia de projeto Smithy</Link> para saber como criar uma biblioteca de formas e conectá-la como uma dependência do modelo da sua API.
...
Um projeto gerado no Windows faz commit de um target `compile` que invoca `smithy` diretamente, então qualquer outra pessoa trabalhando nele — incluindo no macOS ou Linux — precisa da Smithy CLI no `PATH` também. Para fazer essas máquinas resolverem a CLI através do `mise`, mude o target para o comando `mise` como [descrito abaixo](#choosing-how-the-cli-is-resolved).

### Escolhendo como a CLI é resolvida

macOS e Linux resolvem a CLI através do `mise` e Windows usa uma CLI instalada globalmente, mas você pode escolher qualquer uma em qualquer plataforma editando o comando do target `compile` no `project.json` do projeto de modelo.
...
O gerador cria infraestrutura CDK ou Terraform baseada no `iac` selecionado.
...
:::note[Compilar antes de implantar]
...
Não suportamos integrações tipadas para Terraform, portanto nenhum target de geração de código é configurado se você selecionou Terraform como `iac`.
:::
</Fragment>
</Infrastructure>
...
Para invocar sua API de um website React, você pode usar o gerador <Link path="guides/connection/react-smithy">`connection`</Link>, que fornece geração de cliente tipado a partir do seu modelo Smithy.
