...
No macOS e Linux, a CLI é resolvida pelo [mise](https://mise.jdx.dev/), que o build busca sob demanda, então não há nada para instalar — ele baixa e armazena em cache a versão fixada na primeira vez que você faz o build. No Windows, a CLI é um pré-requisito que você instala por conta própria, e atualizar o workspace não a atualiza — veja <Link path="guides/smithy-project#building-on-windows">Building on Windows</Link> e <Link path="guides/smithy-project#using-the-smithy-cli-directly">Using the Smithy CLI directly</Link> no guia de projeto Smithy.
