...
O gerador de API TypeScript Smithy cria uma nova API usando Smithy para definição de serviço e o [Smithy TypeScript Server SDK](https://github.com/awslabs/smithy-typescript) para implementação. O gerador fornece infraestrutura como código CDK ou Terraform para implantar seu serviço no AWS Lambda, exposto via uma API REST do AWS API Gateway. Ele fornece desenvolvimento de API com segurança de tipos e geração automática de código a partir de modelos Smithy. O handler gerado usa [AWS Lambda Powertools for TypeScript](https://docs.powertools.aws.dev/lambda/typescript/latest/) para observabilidade, incluindo logging, rastreamento AWS X-Ray e CloudWatch Metrics
...
A opção `integrationPattern` tem como padrão `isolated`, que cria um Lambda por operação Smithy. Selecione `shared` se você preferir um único handler Lambda compartilhado para toda a API, com substituições opcionais por operação.
:::
...
Veja o <Link path="guides/smithy-project">guia de projeto Smithy</Link> para como criar uma biblioteca de shapes e conectá-la como uma dependência do modelo da sua API.
...
Você deve construir o contexto você mesmo tanto em `handler.ts` (o ponto de entrada da função Lambda) quanto em `local-server.ts` (o ponto de entrada para executar localmente via o alvo `serve`).
:::
...
Você pode referenciar o logger das suas implementações de operações via o contexto:
...
O rastreamento AWS X-Ray é configurado automaticamente via o middleware `captureLambdaHandler`.
...
As métricas do CloudWatch são coletadas automaticamente para cada requisição via o middleware `logMetrics`.
...
Para autenticação `IAM`, procuramos o chamador no Cognito usando o sub extraído do evento do API Gateway. A busca usa o cliente Cognito Identity Provider, que não é uma dependência de um backend Smithy gerado, então instale-o primeiro no projeto backend:
...
Você não precisa de `aws-jwt-verify` ou qualquer outra biblioteca de verificação JWT aqui — o autorizador Cognito User Pools do API Gateway já verificou a assinatura, emissor, escopos e expiração no momento em que seu Lambda é executado. Se alguma dessas verificações falhar, o API Gateway retorna `401 Unauthorized` e seu handler nunca é invocado.
:::
</OptionFilter>
...
O projeto de modelo Smithy usa o [Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/index.html) para construir os artefatos Smithy e gerar o TypeScript Server SDK:

<NxCommands commands={['build <model-project>']} />

No macOS e Linux, o CLI é resolvido pelo [mise](https://mise.jdx.dev/), que o build busca sob demanda, então não há nada para instalar — ele baixa e armazena em cache a versão fixada na primeira vez que você faz o build.

Este processo:

1. **Compila o modelo Smithy** e o valida
2. **Gera especificação OpenAPI** a partir do modelo Smithy
3. **Cria TypeScript Server SDK** com interfaces de operação com segurança de tipos
...
`mise` não publica nenhum pacote Windows para npm, então no Windows o Smithy CLI é um pré-requisito que você instala você mesmo. Instale-o uma vez seguindo o [guia de instalação do Smithy CLI](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html) (por exemplo `winget install smithy` ou `scoop install smithy`), e certifique-se de que `smithy` está no seu `PATH`. Um projeto Smithy gerado no Windows executa `smithy` diretamente em vez de através do `mise`.

:::caution[Atualizando o CLI]
Como o CLI é instalado globalmente em vez de fixado no seu workspace, atualizar o workspace com `nx migrate` **não** o atualiza. Quando uma migração move as dependências Smithy para frente, atualize seu Smithy CLI global para uma versão correspondente você mesmo.
:::

Alternativamente, desenvolva dentro do [WSL](https://learn.microsoft.com/en-us/windows/wsl/install), onde o build executa o caminho Linux e `mise` resolve o CLI para você — nada para instalar.

Um projeto gerado no Windows confirma um alvo `compile` que invoca `smithy` diretamente, então qualquer outra pessoa trabalhando nele — incluindo no macOS ou Linux — precisa do Smithy CLI no seu `PATH` também. Para que essas máquinas resolvam o CLI através do `mise`, mude o alvo para o comando `mise` como [descrito abaixo](#escolhendo-como-o-cli-é-resolvido).

### Escolhendo como o CLI é resolvido

macOS e Linux resolvem o CLI através do `mise` e Windows usa um CLI instalado globalmente, mas você pode escolher qualquer um em qualquer plataforma editando o comando do alvo `compile` no `project.json` do projeto de modelo.

Para usar um Smithy CLI instalado globalmente em vez do `mise`, substitua o prefixo `mise` por um `smithy` simples:
...
:::tip[Hot Reloading]
O servidor local não apenas recarregará quando você fizer alterações TypeScript no seu código backend, mas recarregará quando você fizer alterações no seu projeto de modelo Smithy, permitindo que você itere continuamente no seu modelo Smithy e servidor juntos.
:::
...
2. API Gateway REST API como o gatilho da função
...
<OptionFilter when={{ auth: 'custom' }} description="Uso do Lambda Authorizer personalizado no CDK">
...
Os resultados do autorizador são armazenados em cache por 5 minutos, chaveados no cabeçalho `Authorization`, então um token revogado pode ser aceito até que seu resultado em cache expire. Reduza a duração do cache na infraestrutura gerada se você precisar de revogação mais rápida.
:::
</OptionFilter>
</Fragment>
<Fragment slot="terraform">
...
:::note[Autenticação Cognito]
Se você selecionou autenticação `Cognito`, você precisará fornecer a configuração Cognito:
...
<Snippet name="api/custom-domain" parentHeading="Domínio personalizado" />
...
As operações Smithy são declaradas em PascalCase (`Echo`), e seu backend as exporta sob esse nome. As chaves de integração CDK são a forma **camelCase** do mesmo nome, então uma operação `Echo` é `echo` em todos os lugares no seu stack:
...
As saídas chaveadas por operação do módulo gerado e seu `operations.json` usam a forma **camelCase** do nome da operação Smithy, então uma operação Smithy `Echo` é chaveada `echo`:
...
:::tip[Hot Reloading]
Se você está trabalhando ativamente tanto na sua infraestrutura CDK quanto na API Smithy juntas, você pode usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar esses tipos toda vez que você fizer alterações no modelo:
...
<OptionFilter when={{ auth: 'iam' }} description="APIs autenticadas por IAM apenas">
