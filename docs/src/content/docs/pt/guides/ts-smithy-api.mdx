...
Para invocar sua API de um website React, você pode usar o gerador <Link path="guides/connection/react-smithy">`connection`</Link>, que fornece geração de cliente tipado a partir do seu modelo Smithy.m outros no seu workspace. As seguintes conexões envolvem este projeto:
