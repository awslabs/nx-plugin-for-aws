...
O gerador de API Smithy TypeScript cria uma nova API usando Smithy para definição de serviços e o [Smithy TypeScript Server SDK](https://github.com/awslabs/smithy-typescript) para implementação. O gerador fornece infraestrutura como código via CDK ou Terraform para implantar seu serviço na AWS Lambda, exposto através de uma API REST do AWS API Gateway. Oferece desenvolvimento de API com tipagem segura através de geração automática de código a partir de modelos Smithy. O handler gerado utiliza [AWS Lambda Powertools for TypeScript](https://docs.powertools.aws.dev/lambda/typescript/latest/) para observabilidade, incluindo logging, rastreamento com AWS X-Ray e métricas no CloudWatch.
...
- **backend/** Implementação TypeScript do backend
  ...
  - src/
    ...
    - local-server.ts Servidor local para desenvolvimento
...
O projeto comum de infraestrutura como código tem a seguinte estrutura:
...
As implementações das operações estão localizadas no diretório `src/operations/` do projeto backend. Cada operação é implementada usando os tipos gerados do TypeScript Server SDK (gerado em tempo de build a partir do seu modelo Smithy).
...
Você pode definir um contexto compartilhado para suas operações em `context.ts`:
...
O Smithy fornece tratamento de erros embutido. Você pode definir erros personalizados no seu modelo Smithy:
...
1. **Compila o modelo Smithy** e valida-o
...
O projeto backend usa [Rolldown](https://rolldown.rs/) para empacotar o handler de serviço em um pacote de implantação para AWS Lambda.

<NxCommands commands={['run <backend-project>:bundle']} />

## Desenvolvimento Local

O gerador configura um servidor local com hot reloading:

<NxCommands commands={['run <backend-project>:serve']} />

:::tip
O servidor local não apenas recarrega automaticamente quando você faz alterações no código TypeScript do backend, mas também recarrega quando você altera o projeto de modelo Smithy, permitindo iteração contínua entre o modelo Smithy e o servidor.
:::
...
<Infrastructure>
<Fragment slot="cdk">
Como as operações são definidas no Smithy, usamos geração de código para fornecer metadados ao construct CDK para integrações tipadas.
...
:::tip
Se você está trabalhando ativamente na infraestrutura CDK e na API Smithy simultaneamente, pode usar [`nx watch`](https://nx.dev/nx-api/nx/documents/watch) para regenerar esses tipos toda vez que fizer alterações no modelo:
...
Para invocar sua API de um site React, você pode usar o gerador <Link path="guides/api-connection/react-smithy">`api-connection`</Link>, que fornece geração de cliente tipada a partir do seu modelo Smithy.