...
- **`iac.provider`** — o provedor de infraestrutura como código padrão (`CDK` ou `Terraform`) usado por geradores que emitem infraestrutura (por exemplo, `ts#infra`, `ts#trpc-api`, `py#fast-api`). Geradores que aceitam uma flag `--iac` têm como padrão `Inherit`, que lê este valor.
