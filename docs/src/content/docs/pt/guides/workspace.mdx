...
Novos workspaces definem `parallel` em `nx.json`, que controla quantas tarefas o Nx executa simultaneamente:
...
O workspace vem com um arquivo `aws-nx-plugin.config.mts` na raiz. Os geradores leem este arquivo para escolher padrões sensatos, então você não precisa passar as mesmas flags toda vez:
...
- **`packageManager.catalogs`** — se os geradores devem registrar versões de dependências no catálogo do gerenciador de pacotes e referenciá-las com o protocolo `catalog:` (veja [Política de Versão Única](#política-de-versão-única)). Defina como `false` para que os geradores escrevam intervalos de versão diretos no `package.json` de cada projeto. Não tem efeito no npm, que não possui catálogo.

O <Link path="guides/license">gerador de licença</Link> adiciona uma chave `license` a este mesmo arquivo para configurar seu próprio comportamento.
