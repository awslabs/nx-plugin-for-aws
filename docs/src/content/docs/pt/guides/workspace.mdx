...
O padrão do próprio Nx é 3, o que deixa núcleos ociosos nesses workspaces — cada projeto contribui com vários targets (`lint`, `compile`, `test`, e frequentemente `bundle` e `docker`), produzindo um grafo de tarefas amplo com bastante trabalho independente. Aumentar isso além de 8 tende a não ajudar, já que o caminho crítico do grafo se torna o limite em vez dos núcleos disponíveis.
