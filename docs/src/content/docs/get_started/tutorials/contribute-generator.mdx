...
Let's create a new generator to contribute to `@aws/nx-plugin`. Our objective will be to generate a new procedure for our tRPC API.
...
We provide a generator generator so you can quickly scaffold your new generator! You can run this generator as follows:
...
<FileTree>
  - packages/nx-plugin/src/trpc/procedure
    ...
    - generator.spec.ts Tests for our generator
</FileTree>

Let's update the schema to add the properties we'll need for our generator:
...
You will notice our generator has already been hooked up in `packages/nx-plugin/generators.json`:
...
To test our generator, we'll link our local plugin to our [dungeon-adventure](/nx-plugin-for-aws/get_started/tutorials/dungeon-game/overview) codebase.
