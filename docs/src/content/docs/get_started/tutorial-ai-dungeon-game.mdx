...
Now let's create the following files within our `packages/game-api/backend/src` folder to define our ElectroDB entities as per the above ER Diagram:

<Tabs>
  <TabItem label="entities/game.ts">
```typescript
...
```
  ...
  <TabItem label="entities/action.ts">
```typescript
import { Entity } from 'electrodb';
import { DynamoDBClient } from '@aws-sdk/client-dynamodb';
...
Now that our entities are defined, we need to define our API inputs and outputs. To do that, let's create our schema using [Zod](https://zod.dev/) within the `packages/game-api/schema/schema/src` project as follows:
