...
We can test our API by either calling the deployed API directly with something like [`awscurl`](https://docs.aws.amazon.com/prometheus/latest/userguide/AMP-compatible-APIs.html) or we can start a local instance of the tRPC backend and invoke the API's using `curl`.
...
```bash "https://xxx.region.amazonaws.com/"
awscurl --service execute-api --region ap-southeast-2 -X GET \
  https://xxx.region.amazonaws.com/games.query\?input\={} 
```
    <Aside type="caution">
    Use the CDK deploy output value of `infra-sandbox.GameApiGameApiUrlXXX` to replace the highlighted placeholder.
    </Aside>
  </TabItem>
</Tabs>
