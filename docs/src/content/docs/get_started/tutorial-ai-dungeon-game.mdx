...
<SheetWrapper client:load title="ts#trpc-api generated files" trigger="Click here to examine these files in more detail.">
...
You should see some new files have appeared in your file tree.
<SheetWrapper client:load title="py#fast-api generated files" trigger="Click here to examine these files in more detail.">
...
Here is an example of the generator performing an AST transform which preserves all existing code and performs an update. Here you can see the `StoryApi` was added to the `IRuntimeConfig` definition which means when this is eventually consumed by our frontend, we have type safety!
