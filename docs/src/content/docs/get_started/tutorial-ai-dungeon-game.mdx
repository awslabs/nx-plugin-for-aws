...
<Aside type="tip">
If you encounter any lint errors, you can run the following command to autmatically fix them.
...
<Tabs>
  <TabItem label="procedures/queryActions.ts">
```typescript
...
```
  ...
  <TabItem label="procedures/queryGames.ts">
```typescript
...
```
  ...
  <TabItem label="procedures/saveAction.ts">
```typescript
...
```
  ...
  <TabItem label="procedures/saveGame.ts">
```typescript
import { createGameEntity } from '../entities/game.js';
...
import { APIGatewayProxyEventV2WithIAMAuthorizer } from 'aws-lambda';
-import { echo } from './procedures/echo.js';
+import { queryActions } from './procedures/queryActions.js';
+import { saveAction } from './procedures/saveAction.js';
+import { queryGames } from './procedures/queryGames.js';
+import { saveGame } from './procedures/saveGame.js';
...
This is however not recommended given that you may choose to seperate out your deployment stages as seperate stacks `i.e. infra-prod`. In this case the `--all` flag will attempt to deploy all stacks which would not be what you want.

</Drawer>

<NxCommands commands={['nx run @dungeon-adventure/infra:deploy infra-sandbox']} />

Once the deployment completes, we can test our API by either calling the deployed API directly with something like [`awscurl`](https://docs.aws.amazon.com/prometheus/latest/userguide/AMP-compatible-APIs.html) or we can start a local instance of the tRPC backend and invoke the API's using `curl`.

<Tabs>
  <TabItem label="Local">
You can get the `TABLE_NAME` from the CDK deploy outputs.
<NxCommands env={{TABLE_NAME:"{TABLE_NAME}"}} commands={["run @dungeon-adventure/game-api-backend:serve"]} postCommands={['curl -X GET http://localhost:2022/games.query\?input="\\{\\}"']} />
  </TabItem>
  <TabItem label="Deployed">
You can get the `GameApiUrl` from the CDK deploy outputs.
```bash
awscurl --service execute-api --region ap-southeast-2 -X GET https://{GameApiUrl}/games.query\?input\={} 
```
  </TabItem>
</Tabs>
