...
<Link path="/guides/ts-agent">`ts#agent`</Link>, <Link path="/guides/py-agent">`py#agent`</Link>, <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link>, <Link path="/guides/py-mcp-server">`py#mcp-server`</Link>와 같은 제너레이터는 호스트 아키텍처와 관계없이 `linux/arm64` (Graviton) 이미지를 생성합니다. `x86_64` 호스트에서 `arm64` 이미지를 빌드하거나 Apple Silicon에서 `amd64` 이미지를 빌드하려면 로컬 Docker 설치에서 [멀티 플랫폼 빌드](https://docs.docker.com/build/building/multi-platform/) 지원이 필요합니다.
