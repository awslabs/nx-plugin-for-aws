...
이 생성기는 Cognito 사용자 풀과 관련 ID 풀을 생성하기 위한 CDK 인프라를 구성하고, 사용자 로그인 흐름을 처리하는 호스팅된 UI, 그리고 CloudScape 웹사이트와의 통합을 구성합니다.
...
웹사이트 구성 요소 _이전에_ `UserIdentity` 구성 요소를 스택에 추가해야 합니다:
...
`UserIdentity` 구성 요소는 웹사이트가 인증을 위해 올바른 Cognito 사용자 풀을 가리킬 수 있도록 필요한 <Link path="guides/cloudscape-website#runtime-configuration">런타임 구성</Link>을 자동으로 추가합니다.

### 인증된 사용자에게 액세스 권한 부여

API 호출과 같은 특정 작업을 수행할 수 있는 권한을 인증된 사용자에게 부여하려면 ID 풀의 인증된 역할에 IAM 정책 문을 추가할 수 있습니다:
