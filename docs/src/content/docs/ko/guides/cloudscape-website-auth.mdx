...
이 생성기는 Cognito 사용자 풀, 연동 Identity Pool, 사용자 로그인 흐름을 처리하는 호스팅 UI, 그리고 CloudScape 웹사이트와의 통합을 구성하는 CDK 인프라를 설정합니다.
...
<FileTree>
  - src
    ...
    - main.tsx CognitoAuth 컴포넌트 적용을 위해 업데이트됨
</FileTree>
...
웹사이트 Construct를 선언하기 **전에** 스택에 `UserIdentity` Construct를 추가해야 합니다:
...
`UserIdentity` Construct는 웹사이트가 인증을 위해 올바른 Cognito 사용자 풀을 가리킬 수 있도록 필수 <Link path="guides/cloudscape-website#runtime-configuration">런타임 구성</Link>을 자동으로 추가합니다.

### 인증 사용자 접근 권한 부여

API 호출 권한 부여와 같이 인증된 사용자에게 특정 작업 수행 권한을 주려면 Identity Pool의 인증된 역할에 IAM 정책 명세를 추가할 수 있습니다:
