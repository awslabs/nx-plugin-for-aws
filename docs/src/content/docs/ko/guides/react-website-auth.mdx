...
이 생성기는 기존 정적 웹사이트 아키텍처에 Amazon Cognito 사용자 풀(로그인용)과 Identity Pool(로그인한 사용자를 범위가 지정된 IAM 자격 증명으로 페더레이션하기 위한)을 추가합니다:
...
:::caution[프로덕션 환경에서 localhost 콜백 URL 제거]
생성된 사용자 풀 클라이언트는 CloudFront 배포 URL을 OAuth 콜백/로그아웃 URL로 허용하며, 배포된 풀에 대한 로컬 개발을 위해 `http://localhost:4200` 및 `http://localhost:4300`도 허용합니다.

**프로덕션 스테이지에서는 `http://localhost` 콜백/로그아웃 URL을 제거**하여 허용 목록을 실제 애플리케이션 오리진으로만 제한하는 것이 권장됩니다.
