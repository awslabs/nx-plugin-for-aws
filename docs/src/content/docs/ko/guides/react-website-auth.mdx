...
:::caution[SMS MFA는 전화번호 확인과 연결됩니다]
Cognito는 MFA가 꺼져 있지 않을 때마다 SMS가 활성화된 MFA 방법이 아닌 경우 SMS 기반 전화번호 확인을 거부합니다. 이는 CDK와 Terraform 모두에서 동일합니다. 따라서 `mfa`가 켜져 있는 동안 `mfaSecondFactor.sms` / `mfa_second_factor_sms`를 비활성화하면 사용자 풀에 대한 SMS 기반 전화번호 확인도 비활성화됩니다.
:::
