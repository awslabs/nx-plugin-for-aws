...
:::note[콜백 URL]
생성된 사용자 풀 클라이언트는 `http://localhost:4200` 및 `http://localhost:4300`을 OAuth 콜백/로그아웃 URL로 허용합니다(배포된 풀에 대한 로컬 개발용). 또한 CloudFront 배포 URL도 허용합니다. 이 허용 목록을 최소화하세요: 강화된 프로덕션 배포의 경우, 배포된 사용자 풀에 대해 로컬에서 더 이상 실행하지 않는 경우 localhost URL을 제거하거나 로컬 개발용 별도 클라이언트를 사용하는 것을 고려하세요.
:::
