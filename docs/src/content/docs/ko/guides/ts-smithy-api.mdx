...
macOS와 Linux에서는 CLI가 빌드가 필요에 따라 가져오는 [mise](https://mise.jdx.dev/)에 의해 해결되므로 별도로 설치할 필요가 없습니다 — 처음 빌드할 때 고정된 버전을 다운로드하고 캐시합니다. Windows에서는 CLI가 직접 설치해야 하는 전제 조건이며, 워크스페이스를 업그레이드해도 CLI는 업그레이드되지 않습니다 — Smithy 프로젝트 가이드의 <Link path="guides/smithy-project#building-on-windows">Windows에서 빌드하기</Link> 및 <Link path="guides/smithy-project#using-the-smithy-cli-directly">Smithy CLI 직접 사용하기</Link>를 참조하세요.
