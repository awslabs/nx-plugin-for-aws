...
:::tip[통합 패턴]
`integrationPattern` 옵션은 기본값이 `isolated`로, Smithy 오퍼레이션당 하나의 Lambda를 생성합니다. 전체 API에 대해 단일 공유 Lambda 핸들러를 선호하는 경우 `shared`를 선택하세요. 오퍼레이션별 선택적 재정의도 가능합니다.
:::
...
- **model/** Smithy 모델 프로젝트
  ...
  - ssdk.rolldown.config.mjs 생성된 TypeScript Server SDK 번들링
  - src/
...
이 생성기는 선택한 `iac` 기반으로 코드형 인프라스트럭처를 생성하므로, `packages/common`에 관련 CDK 구문 또는 Terraform 모듈을 포함하는 프로젝트가 생성됩니다.
...
배포된 Smithy API는 다음과 같은 아키텍처를 가지며, API Gateway 스테이지 앞에 [AWS WAFv2](https://docs.aws.amazon.com/waf/latest/developerguide/waf-chapter.html) Web ACL이 위치합니다:
...
동일한 데이터 타입을 공유하는 여러 Smithy API가 있는 경우, 각 모델에서 중복하는 대신 shape library에서 해당 타입을 한 번만 정의할 수 있습니다. Shape library는 서비스가 없는 Smithy 프로젝트로, 재사용 가능한 shape만 포함하며 여러 Smithy 프로젝트가 의존할 수 있습니다.

<Link path="guides/smithy-project">`smithy#project`</Link> 생성기로 생성할 수 있습니다:

<RunGenerator generator="smithy#project" requiredParameters={{ name: 'my-shapes', type: 'shapes' }} />

그러면 API의 모델에서 `use`를 사용하여 해당 shape를 참조할 수 있습니다:
...
Shape library를 생성하고 API 모델의 의존성으로 연결하는 방법은 <Link path="guides/smithy-project">Smithy 프로젝트 가이드</Link>를 참조하세요.
...
macOS와 Linux에서는 CLI가 빌드가 필요에 따라 가져오는 [mise](https://mise.jdx.dev/)에 의해 해결되므로 별도로 설치할 필요가 없습니다 — 처음 빌드할 때 고정된 버전을 다운로드하고 캐시합니다.
...
`mise`는 npm에 Windows 패키지를 제공하지 않으므로, Windows에서는 Smithy CLI를 직접 설치해야 합니다. [Smithy CLI 설치 가이드](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html)를 따라 한 번 설치하고(예: `winget install smithy` 또는 `scoop install smithy`), `smithy`가 `PATH`에 있는지 확인하세요. Windows에서 생성된 Smithy 프로젝트는 `mise`를 통하지 않고 `smithy`를 직접 실행합니다.

:::caution[CLI 업그레이드]
CLI가 워크스페이스에 고정되지 않고 전역으로 설치되기 때문에, `nx migrate`로 워크스페이스를 업그레이드해도 CLI는 업그레이드되지 **않습니다**. 마이그레이션이 Smithy 의존성을 업데이트하면, 전역 Smithy CLI도 일치하는 버전으로 직접 업그레이드하세요.
:::

또는 [WSL](https://learn.microsoft.com/en-us/windows/wsl/install) 내에서 개발하면 빌드가 Linux 경로로 실행되고 `mise`가 CLI를 자동으로 해결합니다 — 별도 설치가 필요 없습니다.

Windows에서 생성된 프로젝트는 `smithy`를 직접 호출하는 `compile` 타겟을 커밋하므로, macOS나 Linux를 포함한 다른 작업자도 `PATH`에 Smithy CLI가 필요합니다. 이러한 머신에서 `mise`를 통해 CLI를 해결하려면 [아래 설명](#choosing-how-the-cli-is-resolved)대로 타겟을 `mise` 명령으로 전환하세요.

### CLI 해결 방법 선택하기

macOS와 Linux는 `mise`를 통해 CLI를 해결하고 Windows는 전역 설치된 CLI를 사용하지만, 모델 프로젝트의 `project.json`에서 `compile` 타겟의 명령을 편집하여 모든 플랫폼에서 둘 중 하나를 선택할 수 있습니다.

`mise` 대신 전역 설치된 Smithy CLI를 사용하려면 `mise` 접두사를 `smithy`로 바꾸세요:
...
생성기는 선택한 `iac` 기반으로 CDK 또는 Terraform 인프라스트럭처를 생성합니다.
...
`UserIdentity` 구문은 <Link path="/guides/react-website-auth">`ts#website#auth` 생성기</Link>로 생성 가능
:::
</OptionFilter>
</Fragment>
...
API 모듈은 공유 S3 자산 버킷에 Lambda 배포 zip을 스테이징합니다 — 자세한 내용은 <Link path="/guides/terraform-project">Terraform 인프라스트럭처 가이드</Link>를 참조하세요. 배포당 한 번 `core/asset-bucket` 모듈을 인스턴스화하고 `bucket_name` 출력을 `asset_bucket_name` 입력을 통해 모든 API / Lambda 모듈에 전달하세요:
...
<Fragment slot="terraform">
:::note[Terraform 제한사항]
Terraform의 경우 타입 안전 통합을 지원하지 않으므로 `iac`로 Terraform 선택 시 코드 생성 타겟이 구성되지 않습니다.
:::
</Fragment>
</Infrastructure>
...
React 웹사이트에서 API를 호출하려면 <Link path="guides/connection/react-smithy">`connection`</Link> 생성기를 사용하여 Smithy 모델에서 타입 안전 클라이언트를 생성할 수 있습니다.
