...
CLI는 워크스페이스에 개발 의존성으로 추가된 [mise](https://mise.jdx.dev/)에 의해 해결되므로 별도로 설치할 필요가 없습니다 — 처음 빌드할 때 고정된 버전을 다운로드하고 캐시합니다.
...
:::tip[Building in CI]
`mise`는 GitHub API에서 CLI의 릴리스 메타데이터를 읽는데, 인증되지 않은 속도 제한은 IP 주소당 공유되며 CI 러너에서 쉽게 소진됩니다 — 그러면 다운로드가 `403 Forbidden`으로 실패합니다. 빌드 환경에서 `GITHUB_TOKEN`을 설정하여 이를 해제하세요. 이후 빌드는 로컬 캐시를 사용하며 요청을 전혀 하지 않습니다.
:::
