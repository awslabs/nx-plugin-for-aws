...
:::caution[Windows]
`mise`는 npm에 Windows 패키지를 게시하지 않으므로 빌드하기 전에 Smithy CLI를 직접 설치해야 합니다:
...
다른 옵션은 [Smithy CLI 설치 가이드](https://smithy.io/2.0/guides/smithy-cli/cli_installation.html)를 참조하세요.
:::
