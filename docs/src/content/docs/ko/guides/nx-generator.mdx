...
:::tip[GritQL 플레이그라운드]
[GritQL 플레이그라운드](https://docs.grit.io/playground)에서 GritQL 패턴을 테스트하거나, GritQL CLI를 로컬에 설치하여 파일에 대해 패턴을 테스트할 수 있습니다:
...
export const myGenerator = async (tree: Tree, schema: MyGeneratorSchema) => {
  ...
  generateFiles(
    tree,
    joinPathFragments(import.meta.dirname, 'files'), // 템플릿 디렉토리
    'path/to/output', // 출력 디렉토리
    data,
  );
};
