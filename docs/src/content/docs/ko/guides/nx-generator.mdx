...
일부 옵션은 다른 옵션의 특정 값에만 적용됩니다. `x-when`을 사용하여 이를 지정하면,
문서 사이트에서 적용되지 않는 곳에서는 옵션을 제외하고 적용되는 곳에서는 조건을 명시합니다:
...
여러 키는 AND로 연결되고 키 내의 값 목록은 OR로 연결되며, 이는
`<OptionFilter when={{…}}>`와 일치합니다:
...
`x-when`은 옵션을 문서화하지만 아무것도 강제하지 않으므로, generator는 여전히
주어진 실행에 적용되지 않는 옵션을 검증하거나 무시해야 합니다.
...
- `@aws/nx-plugin/sdk/utils/test`의 `createTreeUsingTsSolutionSetup()`을 사용하여 가상 파일 시스템을 생성합니다. 이는 우리 preset이 작성하는 워크스페이스 마커(`tsconfig.base.json`, `pnpm-workspace.yaml`, `biome.json`, `type: module` 루트 `package.json`)를 시드하며, `@aws/nx-plugin` generator를 구성하는 모든 generator에 필요합니다
