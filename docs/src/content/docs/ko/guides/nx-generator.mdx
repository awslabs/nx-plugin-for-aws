...
`schema.json` 파일은 생성기가 허용하는 옵션들을 정의합니다. 이 파일은 Nx 확장 기능이 포함된 [JSON Schema](https://json-schema.org/) 형식을 따릅니다.
...
`x-prompt` 속성을 추가하여 CLI에서 생성기를 실행할 때 표시되는 프롬프트를 사용자 정의할 수 있습니다:
...
기존 워크스페이스 프로젝트 중에서 선택할 수 있게 하는 일반적인 패턴:
...
옵션은 `"important"` 또는 `"internal"` 우선순위를 가질 수 있습니다. 이는 Nx VSCode 확장에서 속성 순서를 정하는 데 도움이 됩니다.
...
스키마에 대한 자세한 내용은 [Nx 생성기 옵션 문서](https://nx.dev/extending-nx/recipes/generator-options)를 참조하세요.
...
:::caution
`schema.json`을 수정할 때마다 `schema.d.ts`도 일치하도록 업데이트해야 합니다. 다음 사항을 포함합니다:
...
생성기는 가상 파일 시스템(`Tree`)을 변형하는 함수로, 파일을 읽고 써서 원하는 변경 사항을 만듭니다.
...
:::note
VSCode 플러그인 UI에 생성기가 보이지 않으면 다음 명령으로 Nx 워크스페이스를 새로 고침하세요:
...
생성기의 단위 테스트는 간단하게 구현할 수 있습니다. 일반적인 패턴 예시:
...
- 복잡한 파일 내용에 스냅샷 사용
- 오류 조건 테스트로 생성기 안정성 확인

## @aws/nx-plugin에 생성기 기여하기

`ts#nx-generator`를 사용하여 `@aws/nx-plugin` 내부에 생성기 스캐폴딩도 가능합니다.

이 생성기를 레포지토리에서 실행하면 다음 파일들이 생성됩니다:

<FileTree>
  - packages/nx-plugin/src/\<name>/
    - schema.json 생성기 입력 스키마
    - schema.d.ts 스키마의 TypeScript 타입
    - generator.ts 생성기 구현
    - generator.spec.ts 생성기 테스트
  - docs/src/content/docs/guides/
    - \<name>.mdx 생성기 문서 페이지
  - packages/nx-plugin/generators.json 생성기 포함하도록 업데이트
</FileTree>

이후 생성기 구현을 시작할 수 있습니다.
