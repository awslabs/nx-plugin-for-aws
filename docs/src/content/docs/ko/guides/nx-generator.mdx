...
TypeScript 프로젝트에 [Nx Generator](https://nx.dev/recipes/generators/local-generators)를 추가하여 컴포넌트 스캐폴딩이나 특정 프로젝트 구조 강제화와 같은 반복 작업을 자동화할 수 있도록 도와줍니다.
...
[GritQL](https://docs.grit.io/)을 사용하여 제너레이터에서 소스 코드를 선언적으로 검색하고 변환할 수 있습니다. GritQL은 TypeScript, JavaScript, Python, HCL (Terraform) 등 여러 언어를 지원하므로 전체 스택에서 동일한 패턴 구문을 사용할 수 있습니다.

AWS용 Nx 플러그인은 두 가지 헬퍼를 제공합니다:

- `applyGritQL(tree, filePath, pattern)` — 파일에 GritQL 재작성 패턴을 적용하고 변경 여부를 나타내는 `Promise<boolean>`을 반환합니다
- `matchGritQL(tree, filePath, pattern)` — 파일 내 어디에서든 GritQL 패턴이 일치하는지 확인하고 `Promise<boolean>`을 반환합니다
...
```typescript
// Python: print 문을 logging 호출로 교체
await applyGritQL(
  tree,
  'src/handler.py',
...
GritQL 패턴은 백틱으로 구분된 코드 스니펫과 와일드카드로 사용되는 `$메타변수`를 사용합니다. 재작성에는 `=>`를, 조건에는 `where` 절을 사용하세요.

:::tip
[GritQL Playground](https://docs.grit.io/playground)에서 GritQL 패턴을 테스트하거나, GritQL CLI를 로컬에 설치하여 파일에 대해 패턴을 테스트할 수 있습니다:
...
:::tip
AI 코딩 에이전트를 사용하여 제너레이터 작성을 돕는다면, [Context7 MCP server](https://context7.com/)를 추가하여 최신 문서를 제공하세요. `gritql`과 `docs.grit.io`를 조회하여 GritQL 패턴 구문 참조를 얻고, GritQL CLI에 대한 액세스를 제공하여 패턴을 직접 테스트할 수 있도록 프롬프트하세요.
:::
