...
- `is_local()` — `LOCAL_DEV=true`일 때 `True`를 반환하며, 로컬과 AWS 동작을 전환하는 데 사용됩니다.
- `get_table_name()` — DynamoDB 테이블 이름을 반환합니다. `LOCAL_DEV=true`일 때는 `config.json`의 `localDev.tableName`에서 테이블 이름을 읽고, 그렇지 않으면 `RUNTIME_CONFIG_APP_ID` 환경 변수를 사용하여 AWS AppConfig에서 이름을 가져와 후속 호출을 위해 캐시합니다.
