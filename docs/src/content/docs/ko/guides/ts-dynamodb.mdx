...
description: TypeScript DynamoDB 프로젝트 생성하기
...
생성된 프로젝트는 단일 DynamoDB 테이블에서 타입 안전 엔티티 모델링을 위해 [ElectroDB](https://electrodb.dev/)를 사용하며, [DynamoDB의 단일 테이블 설계](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/data-modeling-foundations.html)를 따릅니다. 생성된 예제 엔티티를 시작점으로 사용하여 `src/entities/` 아래에 엔티티 파일을 추가하거나 업데이트하세요.
...
GSI는 프로젝트 루트의 `config.json`에서 `tableConfig.globalSecondaryIndexes` 키 아래에 정의됩니다. GSI 키에 대한 [단일 테이블 설계](https://electrodb.dev/en/core-concepts/single-table-relationships/) 명명 규칙을 따라 각 GSI에 대한 항목을 추가하세요:
...
<Snippet name="runtime-config-app-id-note" parentHeading="테이블에 연결하기" />

## 테이블 배포하기

<Snippet name="dynamodb/deploying-table" parentHeading="테이블 배포하기" />
