...
생성된 프로젝트는 [Prisma ORM](https://www.prisma.io/docs/orm)을 사용하여 데이터베이스 스키마를 정의하고 타입 안전 클라이언트를 생성합니다. 워크플로는 모델 우선 방식입니다: 데이터베이스 프로젝트의 `prisma/models/` 디렉토리에 Prisma 모델 파일을 추가하거나 업데이트한 다음, 해당 모델 변경 사항으로부터 마이그레이션을 생성합니다.
...
자세한 내용은 공식 [Prisma 데이터 모델링 가이드](https://www.prisma.io/docs/orm/core-concepts/data-modeling#data-modeling-with-prisma-orm)를 참조하세요.
...
이렇게 하면 스키마가 변경될 때마다 `prisma/migrations`에 새로운 마이그레이션 폴더가 생성됩니다:
...
이 로컬 개발 플로우에서 `migrate deploy`는 마이그레이션 파일을 로컬 데이터베이스에 적용합니다. AWS에 데이터베이스를 배포하지는 않습니다.

### Prisma 명령 실행

생성된 `prisma` 타겟은 Prisma CLI를 노출하므로 로컬 데이터베이스에 대해 Prisma가 지원하는 모든 명령을 실행할 수 있습니다. 사용 가능한 명령은 [Prisma CLI 참조](https://www.prisma.io/docs/orm/reference/prisma-cli-reference)를 참조하세요.

<NxCommands commands={['run <project>:prisma <prisma-command>']} />

### Prisma Studio 사용

[Prisma Studio](https://www.prisma.io/studio)는 로컬 데이터베이스를 위한 시각적 편집기입니다. 테이블 탐색, 레코드 검사 및 편집, 데이터 필터링, 관계 추적, 내장 SQL 콘솔을 통한 원시 SQL 실행에 사용할 수 있습니다. 개발 중 마이그레이션 검증 및 테스트 데이터 시딩에 유용합니다. 다음 명령으로 실행하세요:

<NxCommands commands={['run <project>:prisma studio']} />

### 로컬 데이터베이스 중지

`dev`를 중지하면(예: `Ctrl+C`로) 로컬 데이터베이스 컨테이너가 자동으로 제거되지만, 명명된 볼륨은 보존되어 재시작 시에도 데이터가 유지됩니다.
...
모든 TypeScript 프로젝트에서 데이터베이스 패키지에서 `getPrisma`를 가져와 호출하여 타입 안전 Prisma 클라이언트를 얻을 수 있습니다:
...
<Snippet name="runtime-config-app-id-note" parentHeading="데이터베이스 연결" />
...
Node.js 20 이상 런타임을 사용하는 압축된 Lambda 함수의 경우, `NODE_EXTRA_CA_CERTS`를 설정하여 Amazon RDS CA 번들을 로드하세요:
...
자세한 내용은 AWS Lambda [Amazon RDS 연결을 위한 SSL/TLS 요구사항](https://docs.aws.amazon.com/lambda/latest/dg/services-rds.html#services-rds-tls)을 참조하세요. RDS Proxy를 사용하는 경우 데이터베이스에 연결하는 런타임에서 RDS CA 번들을 구성할 필요가 없습니다.
...
:::note
이는 PostgreSQL에는 필요하지 않습니다. PostgreSQL 어댑터는 `allowExitOnIdle: true`로 구성된 `pg` 연결 풀을 사용하여 명시적인 연결 해제 없이 이벤트 루프가 자연스럽게 종료될 수 있도록 합니다. 이 제한으로 인해 MySQL용 `getPrisma()`는 클라이언트를 캐시하거나 웜 Lambda 호출 간에 연결을 재사용하지 않습니다. 클라이언트를 모듈 수준에서 캐시하고 동일한 연결 풀을 재사용하는 PostgreSQL 어댑터와 달리 매번 새로운 연결이 열립니다.
:::
...
:::caution
15분 이상 실행되는 작업에 주의하세요. 원래 IAM 토큰이 만료된 후 MySQL 클라이언트가 새 연결을 열어야 하는 경우 인증이 실패합니다.
:::
