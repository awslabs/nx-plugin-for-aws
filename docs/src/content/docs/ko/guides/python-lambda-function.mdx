...
생성기는 선택한 `iac` 기반으로 함수를 배포하기 위한 인프라 코드를 생성합니다:
...
람다 함수 생성 시 `event`를 선택한 경우, [AWS Lambda Powertools의 `@event_parser`](https://docs.powertools.aws.dev/lambda/python/latest/utilities/parser/)로 함수가 계측됩니다. 예시:
...
`event`로 `Any`를 선택한 경우, `@event_parser` 데코레이터가 적용되지 않으며 핸들러는 타입이 지정되지 않은 이벤트 딕셔너리를 받습니다. Pydantic 검증 및 타입이 지정된 필드 액세스를 원하는 경우 특정 `event`로 재생성하세요.
</OptionFilter>
...
2. Lambda 배포를 위한 대상 플랫폼(`x86_64-manylinux_2_28`)에 종속성을 설치하기 위한 [`uv pip install`](https://docs.astral.sh/uv/reference/cli/#uv-pip-install)

:::note[아키텍처 변경]
[람다 아키텍처를 ARM으로 변경](https://docs.aws.amazon.com/lambda/latest/dg/foundation-arch.html)하는 경우 번들 대상을 `aarch64-manylinux_2_28`로 수정해야 합니다.
:::
