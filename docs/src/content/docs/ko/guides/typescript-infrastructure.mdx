...
`stageConfig` 옵션을 설정한 경우, 생성기는 중앙 집중식 자격 증명 관리를 위한 두 개의 공유 패키지도 생성합니다(아직 존재하지 않는 경우):
...
:::note[스테이지 구성]
이 섹션은 `stageConfig`로 생성한 경우에 적용됩니다. 이 옵션 없이는 생성기가 더 간단한 설정을 생성하며, AWS 자격 증명을 직접 관리해야 합니다(예: 배포 전에 `AWS_PROFILE`을 내보내기).
:::
...
CDK에 대한 자세한 내용은 [CDK 개발자 가이드](https://docs.aws.amazon.com/cdk/v2/guide/core_concepts.html)와 [API 참조 문서](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)를 참조하세요.l)와 [API 참조 문서](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-construct-library.html)를 참조하세요.