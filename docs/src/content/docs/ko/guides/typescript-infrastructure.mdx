...
| `deploy`         | 개발 중 원하는 스테이지 또는 스택을 지정하여 모든 스테이지 배포. Express 모드를 사용합니다.               |
...
`deploy-sandbox` 및 `deploy` 타겟은 기본적으로 [CloudFormation express 모드](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)를 사용합니다(CDK의 `--express` 플래그를 통해). Express 모드는 전체 안정화를 기다리는 대신 리소스 구성이 적용되었음이 확인되면 완료되며, 전파는 백그라운드에서 계속되어 반복 주기가 훨씬 빨라집니다.

대신 전체 리소스 안정화를 기다리려면 `--no-express`를 전달하세요:

<NxCommands commands={['deploy-sandbox <my-infra> --no-express']} />
:::

:::caution[`deploy-sandbox`와 `deploy`는 개발 전용입니다]
Express 모드는 리소스가 안정화를 완료하기 전에 반환되므로, `deploy-sandbox`와 `deploy`는 개발 및 반복을 위한 것이며 CI/CD 또는 프로덕션 배포에는 적합하지 않습니다. 파이프라인에서는 [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline)를 사용하세요 — express 모드를 사용하지 않으므로 항상 완전한 안정화를 기다립니다.
:::
...
이 타겟은 `deploy` 및 `deploy-sandbox` 타겟과 두 가지 면에서 다릅니다. 첫째, 즉석에서 합성하는 대신 사전 합성된 클라우드 어셈블리를 배포하여, 패키지 버전 변경으로 인한 잠재적인 비결정성을 방지하고 모든 파이프라인 스테이지가 동일한 클라우드 어셈블리를 사용하여 배포하도록 보장합니다. 둘째, express 모드를 사용하지 않으므로 성공을 보고하기 전에 모든 리소스가 완전히 안정화될 때까지 기다립니다.

:::caution[프로덕션에는 항상 `deploy-ci` 사용]
`deploy`와 `deploy-sandbox`는 express 모드를 사용하고 리소스가 안정화를 완료하기 전에 반환되므로, CI/CD 및 프로덕션 배포에는 적합하지 않습니다. 이러한 경우에는 `deploy-ci`를 사용하세요.
:::
