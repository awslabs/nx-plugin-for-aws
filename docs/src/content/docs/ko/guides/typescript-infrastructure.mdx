...
:::note[CDK 웹사이트 컨스트럭트]
CDK의 경우 웹사이트 구문은 스택의 어느 지점에서나 선언할 수 있습니다 — 런타임 구성은 합성 시점에 지연 평가됩니다. Terraform의 경우 웹사이트 모듈이 <Link path="guides/react-website#runtime-configuration">런타임 구성</Link>에 추가하는 모듈에 대한 적절한 `depends_on`을 가지도록 해야 합니다.
:::
...
`deploy` 타겟은 개발 중 배포 속도를 높이기 위해 기본적으로 [CloudFormation express 모드](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)를 사용합니다(CDK의 `--express` 플래그를 통해). Express 모드는 전체 안정화를 기다리지 않고 리소스 구성이 적용된 것으로 확인되면 완료되며, 전파는 백그라운드에서 계속 진행되어 반복 주기가 훨씬 빨라집니다.

Express 모드는 개발 및 반복 작업을 위한 것입니다. Express 모드 없이 배포하고 전체 리소스 안정화를 기다리려면 인프라 프로젝트의 `project.json`에 있는 `deploy` 타겟의 명령에서 `--express` 플래그를 제거하세요.

프로덕션 스테이지에 대한 실제 배포의 경우 CI/CD 파이프라인의 일부로 `deploy-ci` 타겟을 사용하세요([아래](#deploying-to-aws-in-a-cicd-pipeline) 참조). 이 타겟은 express 모드를 사용하지 않으므로 항상 전체 안정화를 기다립니다.
:::
