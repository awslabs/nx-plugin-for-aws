...
| `deploy`         | 원하는 스테이지 또는 스택을 지정하여 모든 스테이지 배포. 완전한 안정화를 기다립니다.                 |
...
이 타겟은 프로덕션 스테이지를 포함한 모든 스테이지를 지정할 수 있으므로, 전체 리소스 안정화를 기다립니다. 반복하는 스테이지에 대해 express 모드를 선택하려면 `--express`를 전달하세요.
...
이 타겟은 즉석에서 합성하는 대신 사전 합성된 클라우드 어셈블리를 배포하여, 패키지 버전 변경으로 인한 잠재적인 비결정성을 방지하고 모든 파이프라인 스테이지가 동일한 클라우드 어셈블리를 사용하여 배포하도록 보장합니다. `deploy`와 마찬가지로 성공을 보고하기 전에 모든 리소스가 완전히 안정화될 때까지 기다리며, `deploy`와 달리 `--express` 옵트아웃을 허용하지 않습니다.
