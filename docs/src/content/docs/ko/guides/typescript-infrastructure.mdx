...
최상위 `src/main.ts` 파일이 `<namespace>-sandbox` 이름의 CDK [스테이지](https://docs.aws.amazon.com/cdk/v2/guide/stages.html)를 인스턴스화하는 것을 확인할 수 있습니다. 이 스테이지는 개발 및 테스트용으로 사용됩니다.
...
<Link path="guides/trpc">tRPC API</Link> 또는 <Link path="guides/fastapi">FastAPI</Link> 생성기를 사용한 경우 `packages/common/constructs`에 배포용 구문이 이미 존재합니다.
...
<Link path="guides/react-website">CloudScape 웹사이트</Link> 생성기를 사용한 경우 `packages/common/constructs`에 배포용 구문이 존재합니다:
...
`build` 타겟의 일부로 <Link path="guides/typescript-project#building">기본 컴파일, 린트, 테스트</Link> 외에 인프라 프로젝트를 CloudFormation으로 _합성_합니다. `synth` 타겟을 실행하여 별도로 수행할 수도 있습니다:
