...
`--stageConfig=true`로 생성하면, 생성기는 중앙 집중식 자격 증명 관리를 위한 두 개의 공유 패키지도 생성합니다(아직 존재하지 않는 경우):
...
:::note[스테이지 구성]
이 섹션은 `stageConfig`로 생성한 경우에 적용됩니다:

<NxCommands commands={['g @aws/nx-plugin:ts#infra --name=infra --stageConfig=true']} />

이 옵션 없이는 생성기가 더 간단한 설정을 생성하며, AWS 자격 증명을 직접 관리합니다(예: 배포 전에 `AWS_PROFILE`을 내보내는 방식).
:::
...
`synth`는 `^assemble`에 의존하므로, 합성은 Lambda 번들, 컨테이너 이미지 및 생성된 사양과 같이 스택이 배포하는 아티팩트를 빌드하지만, 모든 업스트림 프로젝트의 린트, 테스트 및 타입 검사 게이트를 실행하지는 않습니다. `assemble`에 대한 자세한 내용은 <Link path="guides/typescript-project#assembling">Assembling</Link>을 참조하세요.
...
| 타겟             | 사용 용도                                                                                                                     |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `deploy-sandbox` | 개발 중 자체 샌드박스 스테이지 배포. 스테이지 인수가 필요하지 않습니다. Express 모드를 사용합니다.                           |
| `deploy`         | 원하는 스테이지 또는 스택을 지정하여 모든 스테이지 배포. `stageConfig`로 생성하지 않은 경우 Express 모드를 사용합니다.      |
| `deploy-ci`      | 사전 합성된 클라우드 어셈블리를 사용하여 CI/CD 파이프라인에서 배포. 완전한 안정화를 기다립니다.                             |

`deploy-sandbox`와 `deploy`는 `^assemble`에 의존하므로, 배포하려는 아티팩트만 빌드합니다. `deploy-ci`는 파이프라인이 이미 빌드한 어셈블리를 배포하므로, 빌드 의존성이 전혀 없습니다.
...
`deploy-sandbox` 타겟은 기본적으로 [CloudFormation express 모드](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)를 사용합니다(CDK의 `--express` 플래그를 통해). Express 모드는 전체 안정화를 기다리는 대신 리소스 구성이 적용되었음이 확인되면 완료되며, 전파는 백그라운드에서 계속되어 반복 주기가 훨씬 빨라집니다.

리소스가 안정화를 완료하기 전에 반환되므로, express 모드는 반복하는 샌드박스 스테이지에 적합하며, CI/CD 또는 프로덕션 배포에는 적합하지 않습니다 — 이러한 경우에는 [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline)를 사용하세요. 여기서도 전체 리소스 안정화를 기다리려면 `--no-express`를 전달하세요:
...
이 타겟이 express 모드를 사용하는지 여부는 프로젝트를 생성한 방법에 따라 다릅니다:

- **`stageConfig`로 생성한 경우** 프로덕션 스테이지를 포함한 모든 스테이지를 지정할 수 있으므로 전체 리소스 안정화를 기다립니다. 반복하는 스테이지에 대해 express 모드를 선택하려면 `--express`를 전달하세요.
- **`stageConfig` 없이 생성한 경우** `deploy-sandbox`와 마찬가지로 express 모드를 사용합니다. 지정하는 스테이지가 프로덕션일 수 있으므로, 전체 리소스 안정화를 기다리려면 `--no-express`를 전달하세요:

  <NxCommands commands={['deploy <my-infra> <my-infra>-prod/* --no-express']} />

  대신 파이프라인에서 [`deploy-ci`](#deploying-to-aws-in-a-cicd-pipeline)를 사용하세요 — express 모드를 절대 사용하지 않습니다.
...
이 타겟은 `deploy` 및 `deploy-sandbox` 타겟과 두 가지 면에서 다릅니다. 첫째, 즉석에서 합성하는 대신 사전 합성된 클라우드 어셈블리를 배포하여, 패키지 버전 변경으로 인한 잠재적인 비결정성을 방지하고 모든 파이프라인 스테이지가 동일한 클라우드 어셈블리를 사용하여 배포하도록 보장합니다. 둘째, express 모드를 사용하지 않으므로 성공을 보고하기 전에 모든 리소스가 완전히 안정화될 때까지 항상 기다립니다.
