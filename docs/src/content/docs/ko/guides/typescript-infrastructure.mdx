...
`synth`는 `^build`가 아닌 `^assemble`에 의존하므로, 합성은 스택이 배포하는 아티팩트(Lambda 번들, 컨테이너 이미지 및 생성된 사양)를 빌드하지만 모든 업스트림 프로젝트의 린트, 테스트 및 타입 검사 게이트를 실행하지는 않습니다. `assemble`이 `build`와 어떻게 관련되는지는 <Link path="guides/typescript-project#assembling">어셈블링</Link>을 참조하세요.
...
`deploy-sandbox`와 `deploy`는 `^assemble`에 의존하므로, 배포하려는 아티팩트만 빌드하고 다른 것은 빌드하지 않습니다. `deploy-ci`는 파이프라인이 이미 빌드한 어셈블리를 배포하므로, 빌드 의존성이 전혀 없습니다.

:::caution
배포 타겟은 린트, 테스트 및 타입 검사 게이트를 건너뛰므로, 린트 오류나 실패한 단위 테스트에서 배포가 실패하지 않습니다. 중요한 것을 배포하기 전에 `build`를 실행하고, 파이프라인에서 이를 게이트로 설정하세요:
