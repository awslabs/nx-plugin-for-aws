...
:::note
CDK의 경우 웹사이트 구문은 스택의 어느 지점에서든 선언할 수 있습니다 — 런타임 구성은 합성 시간에 지연 평가됩니다. Terraform의 경우 웹사이트 모듈이 <Link path="guides/react-website#runtime-configuration">런타임 구성</Link>에 추가하는 모듈에 대해 적절한 `depends_on`을 갖도록 해야 합니다.
:::
