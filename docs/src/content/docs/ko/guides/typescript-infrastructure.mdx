...
<Link path="guides/trpc">tRPC API</Link> 또는 <Link path="guides/fastapi">FastAPI</Link> 생성기를 사용하여 API를 생성한 경우 `packages/common/constructs`에 배포를 위한 구성 요소가 이미 존재하는 것을 확인할 수 있습니다.

예를 들어 `my-api`라는 tRPC API를 생성한 경우, 단순히 구성을 가져와 인스턴스화하면 배포에 필요한 모든 인프라가 추가됩니다:
...
<Link path="guides/cloudscape-website">CloudScape 웹사이트</Link> 생성기를 사용한 경우 `packages/common/constructs`에 배포용 구성 요소가 이미 존재합니다. 예시:
...
:::warning
웹사이트 <Link path="guides/cloudscape-website#runtime-configuration">런타임 설정</Link>에 모든 API 설정이 포함되도록 하려면 API 구성을 선언한 _후에_ 웹사이트를 선언해야 합니다.
:::

## 인프라 합성

`build` 타겟의 일부로 <Link path="guides/typescript-project#building">기본 컴파일, 린트 및 테스트 타겟</Link>을 실행하는 동시에 인프라 프로젝트가 CloudFormation으로 _합성_됩니다. 이는 `synth` 타겟을 실행하여 독립적으로 수행할 수도 있습니다:
...
AWS 계정에 CDK 애플리케이션을 처음 배포할 경우 부트스트랩이 필요합니다.
...
:::tip
위 명령은 `main.ts`에 정의된 _모든_ 스택을 배포합니다. 특히 애플리케이션의 여러 단계를 구성한 경우 개별 스택을 대상으로 지정할 수 있습니다:
