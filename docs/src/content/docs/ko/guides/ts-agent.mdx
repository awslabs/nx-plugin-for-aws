...
`<your-agent-name>-serve` 타겟도 생성되며, 이는 배포된 인프라에 대해 에이전트를 실행하므로 `RUNTIME_CONFIG_APP_ID`를 설정해야 합니다. `dev`와 `serve`의 차이점은 <Link path="guides/local-development">Local Development</Link> 가이드를 참조하세요.
...
그런 다음 클라이언트 팩토리의 `.local` 팩토리 메서드를 사용하여 호출합니다.
