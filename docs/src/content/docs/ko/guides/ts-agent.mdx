...
생성기는 [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)로 구동되는 대화형 터미널 채팅으로 에이전트와 대화할 수 있는 `<your-agent-name>-chat` Nx 타겟을 구성합니다.

채팅 타겟은 독립적으로 실행됩니다 — 에이전트를 자동으로 시작하지 **않습니다**. 기본적으로 로컬에서 실행 중인 에이전트에 연결하므로, 먼저 별도의 터미널에서 `<your-agent-name>-serve-local`을 시작하세요:
