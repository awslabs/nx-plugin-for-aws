...
:::tip[프로젝트 전제 조건]
먼저 <Link path="/guides/typescript-project">`ts#project`</Link> 제너레이터를 사용하여 Agent를 추가할 프로젝트를 생성하세요.
:::
...
제너레이터는 기존 TypeScript 프로젝트에 다음 파일을 추가합니다. 생성되는 파일은 선택한 `protocol`에 따라 달라집니다:
...
엔트리 포인트는 tRPC 대신 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)를 사용합니다:
...
엔트리 포인트는 [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands)를 사용하여 [CopilotKit](https://docs.copilotkit.ai/aws-strands)과 호환되는 [AG-UI 프로토콜](https://docs.ag-ui.com/) (SSE over POST)을 통해 에이전트를 노출합니다:
...
`infra`에 대해 `none`을 선택한 경우, CDK 구성 또는 Terraform 모듈이 생성되지 않으며 Agent는 로컬에서만 실행할 수 있습니다. 인증할 호스팅된 엔드포인트가 없으므로 이 모드에서는 `auth` 옵션이 무시됩니다.
</OptionFilter>
...
- **AG-UI**: [CopilotKit](https://docs.copilotkit.ai/aws-strands)과의 직접적인 프론트엔드 통합을 위해 [`@ag-ui/aws-strands`](https://www.npmjs.com/package/@ag-ui/aws-strands)를 통한 [AG-UI 프로토콜](https://docs.ag-ui.com/) (SSE over POST)을 사용합니다. 스트리밍, 도구 호출 시각화 및 상태 관리를 갖춘 풍부한 채팅 UI를 원할 때 가장 적합합니다.
...
<Link path="/guides/py-mcp-server">`py#mcp-server`</Link> 또는 <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> 제너레이터를 사용하여 생성한 MCP 서버를 사용하려면 <Link path="/guides/connection/ts-agent-mcp">`connection` 제너레이터</Link>를 활용할 수 있습니다.

<RunGenerator generator="connection" />

연결 설정 방법에 대한 자세한 내용은 <Link path="/guides/connection/ts-agent-mcp">`connection` 제너레이터 가이드</Link>를 참조하세요.
...
생성된 `index.ts`는 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)를 Express 앱에 마운트하여 생성된 에이전트가 `/ping` 헬스 체크와 함께 A2A 프로토콜 엔드포인트를 노출하도록 합니다. AgentCore에 배포되면 엔트리 포인트는 AppConfig에서 런타임의 공개 ARN을 해결하고 에이전트 카드에 광고합니다.
...
AG-UI 에이전트는 프론트엔드에서 직접 사용되도록 설계되었습니다. <Link path="/guides/connection/react-agui">`connection` 제너레이터</Link>를 사용하여 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 프로바이더 및 [AG-UI HttpAgent](https://docs.ag-ui.com/) 클라이언트로 React 웹사이트를 에이전트에 연결하세요.
...
프로젝트에 여러 컴포넌트(에이전트, MCP 서버 등)를 추가한 경우 모두 시작됩니다. 이 에이전트만 실행하려면 `<your-agent-name>-dev` 타겟을 대상으로 지정하세요:
...
제너레이터는 에이전트와 대화형 터미널 채팅을 할 수 있는 `<your-agent-name>-chat` Nx 타겟을 구성합니다.
...
제너레이터는 모든 프로토콜에 대해 `scripts/<your-agent-name>/chat.ts`를 생성합니다. 에이전트의 입력 형태가 발전함에 따라 커스터마이징할 수 있습니다. 기본적으로 로컬 에이전트에 연결되거나 `RUNTIME_CONFIG_APP_ID`가 설정된 경우 배포된 에이전트에 연결됩니다(아래 [배포된 에이전트와 채팅](#chat-with-your-deployed-agent) 참조).

<OptionFilter when={{ infra: 'agentcore' }} description="Deployed agent chat details">
#### 배포된 에이전트와 채팅

Bedrock AgentCore에 배포된 에이전트와 채팅하려면 `RUNTIME_CONFIG_APP_ID` 환경 변수를 배포의 AppConfig 애플리케이션 ID(배포된 스택에서 `RuntimeConfigApplicationId`로 출력됨)로 설정하세요. 채팅 스크립트는 런타임 구성에서 에이전트의 런타임 ARN을 해결하고 배포된 엔드포인트에 연결합니다:
...
제너레이터는 에이전트 소스 디렉토리에서 번들 출력 디렉토리로 `Dockerfile`을 복사하는 `<your-agent-name>-docker` 타겟을 구성합니다. 이는 `Dockerfile`을 번들된 아티팩트와 함께 배치하여 CDK가 `AgentRuntimeArtifact.fromAsset`을 사용하여 Docker 이미지를 직접 빌드할 수 있도록 합니다.
...
- **`s3`** (기본값): CDK/Terraform 인프라는 세션 데이터를 위한 전용 S3 버킷을 프로비저닝하며, 전용 KMS 키로 암호화되고 모든 공개 액세스가 차단됩니다. 서버 액세스 로그는 동일한 키를 통해 CloudWatch Logs 로그 그룹으로 전달됩니다. 에이전트의 IAM 역할에는 버킷에 대한 읽기/쓰기/목록/삭제 액세스 권한과 키에 대한 복호화/데이터 키 생성 액세스 권한이 부여되며, 버킷 이름은 AppConfig 런타임 구성에 에이전트의 ARN과 함께 등록됩니다.
- **`in-memory`**: 버킷이 프로비저닝되지 않습니다. 대화 상태는 실행 중인 프로세스의 수명 동안만 메모리에 유지되며 재시작이나 스케일 인 시 유지되지 않습니다.

이는 생성된 `session.ts`에 구현되어 있으며, 현재 세션에 대한 `SessionManager`를 해결하는 `getSessionManager()` 함수를 내보냅니다.

<OptionFilter when={{ protocol: 'ag-ui' }} description="AG-UI session wiring">
AG-UI는 대화 스레드당 템플릿 에이전트를 복제하므로 `getSessionManager`는 `index.ts`의 `StrandsAgent` 구성에서 `sessionManagerProvider`로 연결되어 공유 템플릿 에이전트에 구워지는 대신 각 스레드에 대해 새로운 `SessionManager`가 해결됩니다.
</OptionFilter>

<OptionFilter when={{ protocol: ['http', 'a2a'] }} description="HTTP/A2A session wiring">
`withSessionId`는 이미 세션당 하나의 `Agent` 인스턴스를 캐시하므로 `getSessionManager`는 `agent.ts`의 `getAgent`의 `new Agent({ sessionManager: await getSessionManager() })` 호출 내에서 직접 호출됩니다.
</OptionFilter>

세션 ID 자체는 AgentCore Runtime 세션에서 가져오며(A2A/AG-UI의 경우 `x-amzn-bedrock-agentcore-runtime-session-id` 헤더를 통해, HTTP/tRPC의 경우 WebSocket 연결 컨텍스트를 통해 전파됨) [`AsyncLocalStorage`](https://nodejs.org/api/async_context.html#class-asynclocalstorage) 기반 컨텍스트에 바인딩되어 `getCurrentSessionId()`가 요청의 어디에서나 이를 해결할 수 있습니다 — <Link path="/guides/connection">`connection` 제너레이터</Link>를 통해 연결된 다운스트림 MCP 또는 A2A 클라이언트를 포함하여 전체 호출 체인이 일관된 세션을 공유하도록 합니다.
...
예를 들어 워크스페이스에 클라이언트를 가져오는 `scripts/test.ts`라는 파일을 만들 수 있습니다:
...
ARN을 `withIamAuth` 팩토리 메서드에 전달하여 배포된 에이전트를 호출할 수 있습니다:
...
:::note[AWS 자격 증명]
이는 기본 AWS 자격 증명 제공자 체인을 사용하여 요청을 인증하므로 위의 코드가 실행되는 환경에는 런타임을 호출할 수 있는 액세스 권한이 있는 AWS 자격 증명이 구성되어 있어야 합니다.
:::
</TabItem>
...
`accessTokenProvider`는 요청을 인증하는 데 사용되는 토큰을 반환해야 합니다. 예를 들어 tRPC가 WebSocket 연결을 재시작할 때 새로운 자격 증명이 재사용되도록 이 메서드 내에서 토큰을 얻을 수 있습니다. 아래는 AWS SDK를 사용하여 Cognito에서 토큰을 얻는 방법을 보여줍니다:
...
React 웹사이트에서 Agent를 호출하려면 올바른 인증(IAM 또는 Cognito)으로 tRPC WebSocket 클라이언트를 자동으로 설정하는 <Link path="/guides/connection/react-ts-agent">`connection` 제너레이터</Link>를 활용할 수 있습니다.

<RunGenerator generator="connection" />

연결 설정 방법에 대한 자세한 내용은 <Link path="/guides/connection/react-ts-agent">`connection` 제너레이터 가이드</Link>를 참조하세요.
</OptionFilter>

<OptionFilter when={{ protocol: 'a2a' }} description="A2A delegation details">
### A2A Agent를 도구로 호출

이 에이전트에서 원격 A2A 에이전트(<Link path="/guides/ts-agent">TypeScript</Link> 또는 <Link path="/guides/py-agent">Python</Link>)로 작업을 위임하려면 <Link path="/guides/connection/ts-agent-a2a">`connection` 제너레이터</Link>를 사용하세요. 대상 에이전트에 대한 SigV4 인증 클라이언트를 제공하고 이 에이전트의 `agent.ts`를 AST 변환하여 원격 A2A 에이전트를 Strands `tool`로 등록합니다.

<RunGenerator generator="connection" />

연결 설정 방법에 대한 자세한 내용은 <Link path="/guides/connection/ts-agent-a2a">`connection` 제너레이터 가이드</Link>를 참조하세요.
</OptionFilter>

<OptionFilter when={{ protocol: 'ag-ui' }} description="AG-UI / React connection details">
### AG-UI Agent 호출

React 웹사이트에서 AG-UI 에이전트를 호출하려면 올바른 인증(IAM 또는 Cognito)으로 배포된 에이전트에 대해 구성된 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 클라이언트를 연결하는 <Link path="/guides/connection/react-agui">`connection` 제너레이터</Link>를 사용하세요.

<RunGenerator generator="connection" />

연결 설정 방법에 대한 자세한 내용은 <Link path="/guides/connection/react-agui">`connection` 제너레이터 가이드</Link>를 참조하세요.
</OptionFilter>
...
<Link path="guides/connection">`connection`</Link> 제너레이터를 사용하여 이 프로젝트를 워크스페이스의 다른 프로젝트와 통합하세요. 다음 연결은 이 프로젝트와 관련이 있습니다:
