...
React 웹사이트에서 Strands Agent를 호출하려면 <Link path="/guides/connection/react-ts-agent">`connection` 생성기</Link>를 활용할 수 있으며, 이는 올바른 인증(IAM 또는 Cognito)으로 tRPC WebSocket 클라이언트를 자동으로 설정합니다.
