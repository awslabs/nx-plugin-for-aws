...
이는 `tsx --watch`를 사용하여 파일이 변경될 때 서버를 자동으로 재시작합니다. 에이전트는 `http://localhost:8081`(또는 여러 에이전트가 있는 경우 할당된 포트 — 프로젝트의 `project.json`에서 `metadata.ports`를 읽으세요)에서 사용할 수 있습니다.

<Snippet name="agent/dev-vs-serve" parentHeading="로컬 개발" />
...
그런 다음 클라이언트 팩토리의 `.local` 팩토리 메서드를 사용하여 호출하세요.
...
에이전트에 할당된 포트로 대체하세요 — 프로젝트의 `project.json`에서 `metadata.ports`를 읽으세요.
...
:::caution[`-serve`가 아닌 `-dev` 사용]
이를 위해 `<your-agent-name>-serve`가 아닌 `<your-agent-name>-dev`를 사용하세요. `-serve`는 배포된 코드 경로를 실행하므로 `RUNTIME_CONFIG_APP_ID`가 설정되어야 합니다 — <Link path="guides/ts-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>를 참조하세요. 이것이 없으면 모든 호출이 `RUNTIME_CONFIG_APP_ID is not set`으로 실패합니다.
:::
