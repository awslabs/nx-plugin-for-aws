...
`api-connection` 제너레이터는 React 웹사이트와 FastAPI 백엔드의 통합을 빠르게 설정할 수 있는 방법을 제공합니다. 타입 세이프한 방식으로 FastAPI 백엔드 연결에 필요한 모든 구성을 설정하며, 클라이언트 및 [TanStack Query](https://tanstack.com/query/v5) 훅 생성, AWS IAM 및 Cognito 인증 지원, 적절한 오류 처리를 포함합니다.
...
제너레이터는 TanStack Query의 `useQuery` 훅으로 API를 호출할 수 있는 `use<ApiName>` 훅을 제공합니다.
...
<Drawer title="일반 클라이언트 직접 사용 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx {5,13}
import { useState, useEffect } from 'react';
...
<Drawer title="일반 클라이언트로 뮤테이션 사용 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
...
<Drawer title="일반 클라이언트로 페이지네이션 사용 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
import { useMyApiClient } from './hooks/useMyApiClient';
...
<Drawer title="일반 클라이언트로 오류 처리 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx {9,15}
function MyComponent() {
  const api = useMyApiClient();
...
<Drawer title="일반 클라이언트로 스트리밍 사용 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
...
<Drawer title="일반 클라이언트로 그룹화된 작업 사용 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
import { useMyApiClient } from './hooks/useMyApiClient';
...
<Drawer title="일반 클라이언트로 커스텀 오류 처리 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
...
<Drawer title="일반 클라이언트로 로딩 상태 처리 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
...
<Drawer title="일반 클라이언트로 낙관적 업데이트 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
...
<Drawer title="일반 클라이언트로 타입 안전성 예시" trigger="일반 클라이언트 직접 사용 예시 보기">
```tsx
function ItemForm() {
  const api = useMyApiClient();
  const [error, setError] = useState<CreateItemError | null>(null);
