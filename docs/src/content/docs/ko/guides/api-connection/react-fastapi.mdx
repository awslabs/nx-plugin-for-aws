...
`api-connection` 제너레이터는 React 웹사이트와 FastAPI 백엔드의 통합을 빠르게 설정할 수 있는 방법을 제공합니다. 타입 세이프한 방식의 클라이언트 및 [TanStack Query](https://tanstack.com/query/v5) 훅 생성, AWS IAM 및 Cognito 인증 지원, 적절한 에러 핸들링을 포함해 FastAPI 백엔드 연결에 필요한 모든 구성을 설정합니다.
...
2. 작동 중인 FastAPI 백엔드 (FastAPI 제너레이터로 생성된)
3. Cognito 또는 IAM 인증을 사용하는 API에 연결하는 경우 <Link path="/guides/react-website-auth">`ts#react-website-auth` 제너레이터</Link>로 추가된 Cognito 인증
...
- src
  ...
  - hooks
    ...
    - use\<ApiName>Client.tsx API 호출을 위한 일반 클라이언트 인스턴스화 훅 추가
...
제너레이터는 또한 웹사이트 인프라에 Runtime Config를 추가해 FastAPI의 API URL이 웹사이트에서 사용 가능하고 `use<ApiName>.tsx` 훅에 자동으로 구성되도록 합니다.
...
- src
  - generated
    - \<ApiName>
      ...
      - client.gen.ts API 호출을 위한 타입 세이프 클라이언트
      - options-proxy.gen.ts TanStack Query로 API 상호작용을 위한 훅 옵션 생성 메서드 제공

</FileTree>

:::tip
기본적으로 생성된 클라이언트는 버전 관리에서 제외됩니다. 체크인하려면 React 애플리케이션의 `.gitignore`에서 해당 항목을 제거할 수 있지만, `.gen.ts` 파일을 수동으로 수정하면 프로젝트 빌드 시 덮어쓰여집니다.
:::

## 생성된 코드 사용

생성된 타입 세이프 클라이언트로 React 애플리케이션에서 FastAPI를 호출할 수 있습니다. TanStack Query 훅을 통해 클라이언트를 사용하는 것이 권장되지만, 일반 클라이언트도 사용 가능합니다.
...
더 세밀한 제어를 원하면 React 애플리케이션의 `watch-generate:<ApiName>-client` 타겟을 사용해 API 변경 시마다 클라이언트 재생성 가능:
...
function MyComponent() {
  ...
  if (item.isLoading) return <div>로드 중...</div>;
  if (item.isError) return <div>오류: {item.error.message}</div>;

  return <div>항목: {item.data.name}</div>;
}
```

<Drawer title="API 클라이언트 직접 사용" trigger="직접 클라이언트 사용 예시 보기">
```tsx {5,13}
import { useState, useEffect } from 'react';
...
생성된 훅은 TanStack Query의 `useMutation` 훅을 사용한 뮤테이션 지원을 포함합니다. 로딩 상태, 에러 핸들링, 낙관적 업데이트를 위한 깔끔한 방법을 제공합니다.
...
function CreateItemForm() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    createItem.mutate({ name: '새 항목', description: '새로운 항목' });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      >
        {createItem.isPending ? '생성 중...' : '항목 생성'}
      </button>

      {createItem.isSuccess && (
        <div className="success">
          생성된 항목 ID: {createItem.data.id}
        </div>
      )}
...
<Drawer title="직접 클라이언트로 뮤테이션 사용" trigger="직접 클라이언트 사용 예시 보기">
```tsx
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button
        ...
        disabled={isLoading}
      >
        {isLoading ? '생성 중...' : '항목 생성'}
      </button>

      {createdItem && (
        <div className="success">
          생성된 항목 ID: {createdItem.id}
        </div>
      )}
...
생성된 훅은 API가 커서 기반 페이지네이션을 지원하면 자동으로 처리합니다. `nextCursor` 값이 응답에서 추출되어 다음 페이지 호출에 사용됩니다.

:::tip
`cursor` 외의 페이지네이션 파라미터를 사용하는 경우 [`x-cursor` OpenAPI 벤더 확장으로 커스터마이징](#custom-pagination-cursor) 가능합니다.
:::

<Drawer title="직접 클라이언트로 페이지네이션 사용" trigger="직접 클라이언트 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
import { useMyApiClient } from './hooks/useMyApiClient';
...
통합에는 타입 지정된 에러 응답이 포함됩니다. OpenAPI 명세에 정의된 가능한 에러 응답을 캡슐화하는 `<operation-name>Error` 타입이 생성됩니다. 각 에러는 `status`와 `error` 속성을 가지며, `status` 값을 확인해 특정 에러 유형을 식별할 수 있습니다.
...
function MyComponent() {
  ...
  return <button onClick={handleClick}>항목 생성</button>;
}
```

<Drawer title="직접 클라이언트로 에러 핸들링" trigger="직접 클라이언트 사용 예시 보기">
```tsx {9,15}
function MyComponent() {
  const api = useMyApiClient();
  ...
  const handleClick = async () => {
    try {
      await api.createItem({ name: '새 항목' });
    } catch (e) {
...
<Link path="guides/fastapi#streaming">스트림 응답을 구성한 FastAPI</Link>가 있는 경우, `useQuery` 훅은 새 스트림 청크 도착 시 자동으로 데이터를 업데이트합니다.
...
<Drawer title="직접 클라이언트로 스트리밍 사용" trigger="직접 클라이언트 사용 예시 보기">

<Link path="guides/fastapi#streaming">스트림 응답을 구성한 FastAPI</Link>가 있는 경우, 생성된 클라이언트는 `for await` 구문으로 스트림 청크를 비동기 반복하는 타입 세이프 메서드를 포함합니다.
...
생성된 훅과 클라이언트 메서드는 FastAPI 엔드포인트의 OpenAPI 태그 기반으로 자동 구성됩니다. 관련 작업을 쉽게 찾고 관리할 수 있게 도와줍니다.
...
function ItemsAndUsers() {
  ...
  return (
    <div>
      ...
      <button onClick={handleCreateItem}>항목 추가</button>
...
<Drawer title="직접 클라이언트로 그룹화된 작업 사용" trigger="직접 클라이언트 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
import { useMyApiClient } from './hooks/useMyApiClient';
...
function ItemsAndUsers() {
  ...
  const handleCreateItem = async () => {
    try {
      const newItem = await api.items.create({ name: '새 항목' });
      setItems(prevItems => [...prevItems, newItem]);
    } catch (error) {
      console.error('항목 생성 오류:', error);
    }
  };
...
FastAPI에서 커스텀 예외 클래스, 예외 핸들러, 응답 모델을 정의해 에러 응답을 커스터마이징할 수 있습니다. 생성된 클라이언트는 자동으로 이 커스텀 에러 타입을 처리합니다.
...
def get_item(item_id: str) -> Item:
    ...
    if not item:
        raise NotFoundException(message=f"항목 ID {item_id} 없음")
    return item
...
def create_item(item: Item) -> Item:
    if not is_valid(item):
        raise ValidationException(
            ValidationError(
                message="잘못된 항목 데이터",
                field_errors=["name 필수"]
            )
        )
    return save_item(item)
```

#### React에서 커스텀 에러 타입 사용

생성된 클라이언트는 커스텀 에러 타입을 자동 처리해 타입 검사 및 에러 응답 처리 가능:
...
<Drawer title="직접 클라이언트로 커스텀 에러 처리" trigger="직접 클라이언트 사용 예시 보기">
```tsx
import { useState, useEffect } from 'react';
...
:::tip
FastAPI에서 에러 응답 정의 시 항상 `responses` 파라미터로 상태 코드별 모델 지정. 이렇게 하면 생성된 클라이언트가 적절한 타입 정보를 가집니다.
:::
...
<Drawer title="직접 클라이언트로 로딩 상태 처리" trigger="직접 클라이언트 사용 예시 보기">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
...
function ItemList() {
  ...
  if (itemsQuery.isError) {
    return <ErrorMessage message="항목 불러오기 실패" />;
  }
...
<Drawer title="직접 클라이언트로 낙관적 업데이트" trigger="직접 클라이언트 사용 예시 보기">
```tsx
function ItemList() {
  const api = useMyApiClient();
  const [items, setItems] = useState([]);
...
<Drawer title="직접 클라이언트로 타입 안전성" trigger="직접 클라이언트 사용 예시 보기">
```tsx
function ItemForm() {
  const api = useMyApiClient();
  const [error, setError] = useState<CreateItemError | null>(null);
