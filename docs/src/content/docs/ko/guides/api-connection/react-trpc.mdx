...
- src
  - components
    - TrpcClients
      - index.tsx
      - TrpcProvider.tsx 여러 tRPC API에 재사용되는 Provider
...
통합에는 tRPC 에러를 적절히 처리하는 내장 에러 처리 기능이 포함되어 있습니다:
...
`cursor`라는 이름의 입력 속성이 있는 프로시저에 대해서만 무한 쿼리를 사용할 수 있습니다.
