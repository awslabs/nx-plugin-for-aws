...
tRPC 생성기는 `integrationPattern` 옵션을 통해 두 가지 인프라 패턴을 지원합니다:

- `isolated`가 기본값입니다. 프로시저당 하나의 Lambda 함수를 생성합니다.
- `shared`는 전체 API에 대해 단일 기본 `$router` Lambda 통합을 생성하고 프로시저별 재정의를 제공하지 않는 한 모든 프로시저에 재사용합니다.

`isolated`는 더 세분화된 인프라를 제공하며 작업별로 다른 권한이나 구성을 첨부하기 쉽게 만듭니다. `shared`는 Lambda 및 통합 분산을 줄이며, 필요할 때 전용 통합으로 특정 프로시저를 재정의할 수 있습니다.

두 통합 패턴 모두 현재 `ServerlessApiGatewayRestApi` 및 `ServerlessApiGatewayHttpApi`에 대해 지원됩니다.
