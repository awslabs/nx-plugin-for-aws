...
:::tip
`integrationStyle` 옵션은 기본적으로 `Individual Functions`로 설정되며, 이는 tRPC 프로시저당 하나의 Lambda를 생성합니다. 전체 API에 대해 단일 공유 Lambda 핸들러를 선호하는 경우 `Router`를 선택하세요.
:::
...
tRPC 생성기는 `integrationStyle` 옵션을 통해 두 가지 인프라 패턴을 지원합니다:

- `Individual Functions`가 기본값입니다. 프로시저당 하나의 Lambda 함수를 생성합니다.
- `Router`는 전체 API에 대해 단일 공유 Lambda 함수를 생성하고 모든 통합에 재사용합니다.

`Individual Functions`는 더 세분화된 인프라를 제공하며 작업별로 다른 권한이나 구성을 첨부하기 쉽게 만듭니다. `Router`는 Lambda 및 통합 분산을 줄이며, 이는 소규모 API나 단일 중앙 핸들러를 선호하는 경우에 유용할 수 있습니다.

두 통합 스타일 모두 현재 `ServerlessApiGatewayRestApi` 및 `ServerlessApiGatewayHttpApi`에 대해 지원됩니다.
