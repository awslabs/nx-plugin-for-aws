...
tRPC API 생성기는 AWS CDK 인프라가 설정된 새 tRPC API를 생성합니다. 생성된 백엔드는 서버리스 배포를 위해 AWS Lambda를 사용하며 [Zod](https://zod.dev/)를 통한 스키마 검증을 포함합니다. 로깅, AWS X-Ray 추적, Cloudwatch 메트릭을 포함한 관측성을 위해 [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/typescript/latest/)가 설정됩니다.
...
개요적으로 tRPC API는 특정 프로시저에 요청을 위임하는 라우터로 구성됩니다. 각 프로시저는 Zod 스키마로 정의된 입력과 출력을 가집니다.
...
- `query`는 API 구현을 정의하는 함수를 받습니다. 이 구현은 `opts`를 수신하며, 여기에는 작업에 전달된 `input`과 미들웨어에서 설정된 `opts.ctx`의 다른 컨텍스트가 포함됩니다. `query`에 전달된 함수는 `output` 스키마를 준수하는 출력을 반환해야 합니다.
...
AWS Lambda Powertools 로거는 `src/middleware/logger.ts`에 구성되며 API 구현에서 `opts.ctx.logger`로 접근할 수 있습니다. 이를 사용해 CloudWatch Logs에 로깅하거나 모든 구조화된 로그 메시지에 포함할 추가 값을 제어할 수 있습니다:
...
AWS Lambda Powertools 메트릭은 `src/middleware/metrics.ts`에 구성되며 API 구현에서 `opts.ctx.metrics`로 접근할 수 있습니다. AWS SDK를 임포트하거나 사용하지 않고도 CloudWatch에 메트릭을 기록할 수 있습니다:
...
AWS Lambda Powertools 추적기는 `src/middleware/tracer.ts`에 구성되며 API 구현에서 `opts.ctx.tracer`로 접근할 수 있습니다. 이를 사용해 API 요청의 성능과 흐름에 대한 상세한 통찰력을 제공하는 AWS X-Ray 추적을 추가할 수 있습니다:
...
미들웨어를 구현해 프로시저에 제공되는 컨텍스트에 추가 값을 제공할 수 있습니다.
...
이 추가 _옵셔널_ 속성이 올바르게 이 미들웨어를 구성한 프로시저에서 정의되도록 tRPC가 관리합니다.
...
이 경우 Cognito 사용자의 주체 ID("sub")를 API Gateway 이벤트에서 추출하고 Cognito에서 사용자 세부 정보를 검색하려 합니다. 구현은 이벤트가 REST API 또는 HTTP API에서 제공되는지에 따라 약간 다릅니다:
...
export const createIdentityPlugin = () => {
  ...
  return t.procedure.use(async (opts) => {
    ...
    if (!Users || Users.length !== 1) {
      throw new TRPCError({
        code: 'FORBIDDEN',
        message: `주체 ID ${sub}로 사용자를 찾을 수 없음`,
      });
    }
...
tRPC API 생성기는 `common/constructs` 폴더에 API 배포용 CDK 구문을 생성합니다. CDK 애플리케이션에서 다음과 같이 사용할 수 있습니다:
...
이것은 AWS API Gateway REST/HTTP API, 비즈니스 로직용 AWS Lambda 함수, 선택한 `auth` 메서드 기반 인증을 포함한 API 인프라를 설정합니다.
...
`UserIdentity` 구문은 <Link path="/guides/cloudscape-website-auth">`ts#cloudscape-website-auth` 생성기</Link>로 생성할 수 있습니다.
:::
...
`serve` 타겟을 사용해 API용 로컬 서버를 실행할 수 있습니다:
...
React 웹사이트에서 API를 호출하는 경우 <Link path="guides/api-connection/react-trpc">API Connection</Link> 생성기를 사용해 클라이언트를 구성하는 것을 고려하세요.
