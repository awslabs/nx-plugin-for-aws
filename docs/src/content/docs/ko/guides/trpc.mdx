...
`auth: 'cognito'`로 배포하면 API Gateway Cognito User Pools 권한 부여자가 호출자가 `Authorization` 헤더에 제공하는 JWT를 확인하고 확인된 클레임을 `event.requestContext.authorizer.claims`의 Lambda 이벤트에 배치합니다. 우리의 미들웨어는 이러한 클레임을 읽기만 하면 됩니다 — 추가 AWS SDK 호출이나 수동 JWT 확인이 필요하지 않습니다.
