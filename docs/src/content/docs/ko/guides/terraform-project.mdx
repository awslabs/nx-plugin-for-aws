...
생성기는 또한 `packages/common/terraform` 라이브러리를 생성하며, 초기에는 메트릭 모듈(메타데이터가 포함된 빈 Cloudformation 스택 생성)을 추가하여 `nx-plugin-for-aws` 팀이 사용 메트릭을 추적할 수 있도록 자동 계측됩니다.
:::
</OptionFilter>
...
라이브러리 프로젝트(`--type=library`)의 경우 생성기는 재사용 가능한 Terraform 모듈을 위한 단순한 구조를 생성합니다:
...
:::tip[애플리케이션 vs 라이브러리]
애플리케이션 프로젝트는 원격 상태 관리를 포함한 전체 배포 기능을 제공하는 반면, 라이브러리 프로젝트는 다른 프로젝트에서 사용할 수 있는 재사용 가능한 Terraform 모듈 생성을 위해 설계되었습니다.
:::

## Terraform 인프라 구현

`src/main.tf` 내부에 Terraform 인프라를 작성할 수 있습니다. 예시:
...
새 환경을 추가하려면 환경별 변수가 포함된 `src/env/<environment>.tfvars` 파일을 생성하고 `project.json`의 `apply, destroy, init, plan`에 새 환경 구성 항목을 추가하세요. 예를 들어 `prod` 환경을 추가한다고 가정해 보겠습니다:
...
:::tip[기본 환경]
기본적으로 모든 타겟은 `dev` 환경을 가정하지만, 다음과 같이 특정 환경을 지정할 수 있습니다: <NxCommands commands={['apply tf-infra --configuration=prod']} />
...
인프라 배포 전에 원격 상태 백엔드를 부트스트랩해야 합니다. 이는 Terraform 상태 파일을 저장할 S3 버킷을 생성합니다:

<NxCommands commands={['bootstrap tf-infra']} />

:::note[AWS 프로파일 구성]
부트스트랩 리소스를 배포할 계정/리전에 AWS CLI 프로필이 구성되어 있는지 확인하세요.

부트스트랩 타겟은 애플리케이션 유형 프로젝트에서만 사용 가능합니다. 라이브러리 프로젝트는 재사용 가능한 모듈로 설계되어 원격 상태 관리가 필요하지 않습니다.
:::
</OptionFilter>
...
`fmt` 타겟을 사용하여 Terraform 코드를 포맷팅하세요:

<NxCommands commands={['fmt tf-infra']} />

#### 보안 테스트

`test` 타겟으로 Checkov를 사용하여 인프라 보안 검사를 실행하세요:
...
변경 사항 적용 전 `plan` 타겟을 실행하여 Terraform 실행 계획을 확인하세요:
...
`init` 타겟으로 Terraform 작업 디렉토리를 초기화하세요:

<NxCommands commands={['run tf-infra:init']} />

#### AWS에 배포

계획 수립 후 `apply` 타겟을 사용하여 인프라를 AWS에 배포하세요:

<NxCommands commands={['apply tf-infra']} />

:::tip[자동 승인]
위 명령어는 `plan` 타겟으로 생성된 계획을 적용합니다. 변경 사항을 검토하려면 먼저 `plan`을 실행하세요.
:::

:::note[배포 별칭]
`deploy` 타겟은 `apply`의 별칭으로도 사용 가능합니다. <NxCommands commands={['deploy tf-infra']} /> 또는 <NxCommands commands={['apply tf-infra']} />를 상호 교환하여 사용할 수 있습니다.
:::

#### 출력 값 확인

Terraform 구성에서 출력 값을 확인하세요:

<NxCommands commands={['output tf-infra']} />

#### 인프라 삭제

인프라를 철거해야 할 때 `destroy` 타겟을 사용하세요:

<NxCommands commands={['destroy tf-infra']} />

:::caution[파괴적 작업]
이 명령은 해당 Terraform 구성이 관리하는 모든 리소스를 영구 삭제합니다. 주의해서 사용하세요!
:::

#### 부트스트랩 리소스 삭제

부트스트랩 리소스(S3 상태 저장 버킷)를 정리하려면:
