...
Checkov는 `build`와 `package` 모두의 일부로 실행되므로 대화형 `plan`은 적용하기 전에 인프라를 스캔합니다.
...
`plan`은 `build`가 아닌 `package`에 의존하므로 모듈이 참조하는 아티팩트(Lambda 번들 및 생성된 작업 메타데이터)를 생성하고 Checkov를 실행하지만, 모든 프로젝트의 린트, 테스트 및 타입 체크 게이트는 실행하지 않습니다.
