...
:::tip
애플리케이션 프로젝트는 원격 상태 관리를 포함한 완전한 배포 기능을 제공하며, 라이브러리 프로젝트는 다른 프로젝트에서 사용할 수 있는 재사용 가능한 Terraform 모듈 생성을 위해 설계되었습니다.
:::

## Terraform 인프라 구현

`src/main.tf` 내부에 Terraform 인프라 코드를 작성할 수 있습니다. 예시:
...
이렇게 하면 Nx 그래프에 소비 애플리케이션과 라이브러리 간의 의존성이 자동으로 추가됩니다.

### 환경 설정

환경별 변수는 `src/env/*.tfvars` 파일에서 구성할 수 있습니다.

새 환경을 추가하려면 `src/env/<environment>.tfvars` 파일을 생성하고 환경별 변수를 입력한 후 `project.json`의 `apply, destroy, init, plan` 항목에 새 환경 구성 항목을 추가하세요. 예를 들어 `prod` 환경을 추가하는 경우:
...
`test` 타겟으로 Checkov를 사용하여 인프라 보안 검사를 실행합니다:
...
계획 수립 후 `apply` 타겟을 사용하여 인프라를 AWS에 배포할 수 있습니다:

<NxCommands commands={['run tf-infra:apply']} />

:::tip
위 명령어는 `plan` 타겟으로 생성된 계획 파일을 적용합니다. 변경 사항을 검토하려면 먼저 `plan`을 실행하세요.
:::
...
:::caution
이 명령은 해당 Terraform 설정이 관리하는 모든 리소스를 영구적으로 삭제합니다. 주의해서 사용하세요!
:::
