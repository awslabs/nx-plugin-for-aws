...
`terraform init`을 실행하는 모든 타겟은 워크스페이스 루트의 `.terraform/plugin-cache`에 있는 프로바이더 캐시를 공유합니다. 자세한 내용은 [Terraform 문서](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache)를 참조하세요.
