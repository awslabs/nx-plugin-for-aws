...
Terraform 인프라 생성기는 Terraform 인프라 프로젝트를 생성합니다. 생성된 애플리케이션은 [Checkov](https://www.checkov.io/) 보안 검사를 통해 보안 모범 사례를 포함합니다.

## 사용 방법

### Terraform 프로젝트 생성

새 Terraform 프로젝트를 두 가지 방식으로 생성할 수 있습니다:
...
이렇게 하면 Nx 그래프가 자동으로 업데이트되어 애플리케이션과 라이브러리 간 종속성이 추가됩니다.

### 환경 구성

환경별 변수는 `src/env/*.tfvars` 파일에서 구성할 수 있습니다.

새 환경을 추가하려면 환경별 변수가 포함된 `src/env/<environment>.tfvars` 파일을 생성하고 `project.json`의 `apply, destroy, init, plan`에 새 환경 구성 항목을 추가하세요. 예를 들어 `prod` 환경을 추가하는 경우:
...
:::tip
기본적으로 모든 타겟은 `dev` 환경을 가정합니다. 특정 환경을 대상으로 하려면: <NxCommands commands={['run tf-infra:apply --configuration=prod']} />
...
부트스트랩 타겟은 애플리케이션 유형 프로젝트에서만 사용 가능하며, 라이브러리 프로젝트는 재사용 모듈로 설계되어 원격 상태 관리가 필요하지 않습니다.
:::
...
`validate` 타겟으로 Terraform 구성을 검증할 수 있습니다:

<NxCommands commands={['run tf-infra:validate']} />

#### 코드 포맷팅

`fmt` 타겟으로 Terraform 코드를 포맷팅합니다:

<NxCommands commands={['run tf-infra:fmt']} />

#### 보안 테스트

`test` 타겟으로 Checkov를 사용해 인프라 보안 검사를 실행합니다:
...
변경 사항 적용 전 `plan` 타겟으로 Terraform 실행 계획을 확인할 수 있습니다:
...
계획 수립 후 `apply` 타겟으로 인프라를 AWS에 배포할 수 있습니다:

<NxCommands commands={['run tf-infra:apply']} />

:::tip
위 명령은 `plan` 타겟으로 생성된 계획을 적용합니다. 변경 사항 검토를 위해 먼저 `plan`을 실행하세요.
:::

#### 출력 값 확인

Terraform 구성에서 출력 값을 확인합니다:
...
:::caution
이 명령은 Terraform 구성으로 관리되는 모든 리소스를 영구 삭제합니다. 주의해서 사용하세요!
:::

#### 부트스트랩 리소스 삭제

상태 저장을 위한 S3 버킷 등 부트스트랩 리소스를 정리하려면:
