...
<FileTree>
  ...
  - scripts nx `bootstrap`, `bootstrap-destroy` 및 `init` 타겟에서 실행되는 Node 헬퍼
    ...
    - env.ts 공유 프로바이더 캐시에서 `terraform init`을 가리킴
...
단순한 `aws_s3_bucket`은 `build`가 실행하는 [Checkov](#보안-테스트) 스캔에 실패하므로, 예제는 스캔이 예상하는 버전 관리, 암호화 및 퍼블릭 액세스 차단 리소스를 선언하고, 리소스별로 나머지 검사를 이유를 명시한 `#checkov:skip` 주석으로 억제합니다. 각 억제를 자신의 요구 사항에 맞게 검토하세요 — 두 가지 억제 메커니즘에 대해서는 [보안 테스트](#보안-테스트)를 참조하세요.
...
:::note[변수 이름]
여기에서 설정하는 모든 이름은 `src/variables.tf`에 선언되어야 합니다 — 스캐폴드는 `aws_region`과 `environment`를 선언합니다. Terraform은 선언되지 않은 변수의 값에 대해서만 경고하므로, 철자가 틀린 이름은 적용되지 않고 조용히 무시됩니다.
:::
</TabItem>
...
`test`와 마찬가지로, 이것은 먼저 `terraform init -backend=false`를 실행하므로 S3 백엔드를 구성하지 않고 검증에 필요한 모듈과 프로바이더를 설치합니다. 따라서 <Link path="guides/terraform-project#원격-상태-부트스트랩-애플리케이션-프로젝트만-해당">`bootstrap`</Link> 전에 새 프로젝트에서 실행되며, `lint`, `test` 및 `checkov`와 마찬가지로 AWS 자격 증명 없이 실행됩니다.

#### 린팅

Terraform 프로젝트는 [`terraform fmt`](https://developer.hashicorp.com/terraform/cli/commands/fmt)를 사용하여 포맷팅을 확인합니다.
...
:::tip[단축 명령]
워크스페이스 루트에서 다음 단축 명령을 사용할 수 있습니다:
...
테스트 파일이 없는 프로젝트는 성공으로 처리되므로 필요할 때 테스트를 추가할 수 있습니다. `build`는 이 타겟을 실행하므로 테스트는 일반 빌드의 일부로 실행됩니다.
...
`terraform init`을 실행하는 모든 타겟은 워크스페이스 루트의 `.terraform/plugin-cache` 아래에 있는 프로바이더 캐시를 재사용하므로 프로바이더는 매번 실행할 때마다가 아니라 한 번만 다운로드됩니다. 각 프로젝트는 거기에 자체 디렉터리를 가지고 있습니다: 동시에 하나의 캐시를 채우는 두 개의 `terraform init` 실행은 각각 동일한 프로바이더에 대해 다른 해시를 계산할 수 있으며, terraform은 이를 `.terraform.lock.hcl`에 대해 거부합니다. 자세한 내용은 [Terraform 문서](https://developer.hashicorp.com/terraform/cli/config/config-file#provider-plugin-cache)를 참조하세요.

환경에서 `TF_PLUGIN_CACHE_DIR`을 설정하여 제공된 `init` 스크립트가 직접 관리하는 캐시(예: 워크스페이스 간에 공유되는 볼륨)를 가리키도록 할 수 있습니다. 이는 스크립트를 거치는 타겟에만 도달하며, 이는 애플리케이션 프로젝트의 `init`입니다. 다른 모든 타겟은 `project.json`에서 경로를 `env` 항목으로 선언하고, Nx의 `env` 블록이 주변 환경보다 우선하므로 거기에서도 변경해야 합니다 — 두 프로젝트 타입 모두에서 `test` 및 `validate`에서, 그리고 라이브러리 프로젝트의 경우 `init`에서도 마찬가지입니다.
...
`plan`은 `assemble`에 의존하므로 Lambda 번들 및 생성된 작업 메타데이터와 같이 모듈이 참조하는 아티팩트를 생성하지만, lint, test 및 type-check 게이트는 실행하지 않습니다.
