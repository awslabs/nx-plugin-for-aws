...
:::tip[AG-UI 에이전트]
Python Strands Agent가 `protocol=AG-UI`로 생성된 경우, 연결 생성기는 대신 React 웹사이트에 [CopilotKit](https://docs.copilotkit.ai/aws-strands)과 [`@ag-ui/client`](https://docs.ag-ui.com/) `HttpAgent`를 연결합니다. 자세한 내용은 <Link path="guides/connection/react-agui">React to AG-UI Agent</Link> 가이드를 참조하세요.
:::
...
2. `protocol=HTTP`를 사용하는 Python Strands Agent (<Link path="guides/py-strands-agent">`py#strands-agent` 생성기</Link>를 사용하여 생성)
