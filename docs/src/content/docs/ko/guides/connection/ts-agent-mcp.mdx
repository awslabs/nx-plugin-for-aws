...
`connection` 생성기는 <Link path="guides/ts-agent">TypeScript Strands Agent</Link>를 MCP 서버(<Link path="guides/ts-mcp-server">TypeScript</Link> 또는 <Link path="guides/py-mcp-server">Python</Link>)에 연결할 수 있습니다.
