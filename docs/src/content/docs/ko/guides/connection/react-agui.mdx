...
AG-UI와 AgentCore Runtime은 각각 대화를 다르게 식별하며, 생성된 훅은 이들을 연결합니다:

- **`threadId`** — AG-UI 대화 식별자로, 요청 본문에 전송됩니다. CopilotKit은 명시적인 `threadId`를 전달하지 않는 한 채팅당 무작위 UUID를 생성합니다.
- **Session ID** — AgentCore Runtime 세션으로, `X-Amzn-Bedrock-AgentCore-Runtime-Session-Id` 헤더에 전송됩니다. 요청을 처리하는 [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html)을 선택하며, 에이전트의 `session.ts` / `session.py`가 대화 상태를 키로 사용하는 값입니다.
...
두 값 모두 브라우저에서 오므로 누구의 것인지 말하지 않고 대화의 이름만 지정합니다. 각 사용자를 자신의 대화로 제한하려면 <Link path="guides/ts-agent">세션을 소유자로 제한하기</Link>를 참조하세요.
