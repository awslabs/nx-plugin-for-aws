...
- packages/common/agent\_connection
  - \<scope>\_agent\_connection
    - core/
      ...
      - agentcore\_gateway\_mcp\_client\_strands.py 배포된 Gateway용 Strands MCP 클라이언트
      - auth/ 프레임워크 독립적인 SigV4 / 세션 전달 `httpx.Auth`
    - app/
      - \<gateway\_snake>\_client\_strands.py Gateway별 Strands 클라이언트 래퍼
...
`MyGatewayClientStrands.create()`는 컨텍스트 관리 가능한 단일 클라이언트를 반환하며, 이 클라이언트의 `list_tools_sync()`는 Gateway를 통해 사용 가능한 모든 도구를 제공합니다:
