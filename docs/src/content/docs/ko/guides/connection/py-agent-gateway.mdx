...
에이전트를 로컬에서 **배포된 Gateway에 대해** 실행하려면(예: Cedar 정책을 테스트하기 위해) 다음을 설정하세요:
...
- **Cedar 정책 평가 없음.** 정책에 관계없이 모든 도구가 에이전트에 표시됩니다. 배포된 Gateway에 대해 정책을 테스트하려면 `GATEWAY_URL_OVERRIDE`를 사용하세요.
