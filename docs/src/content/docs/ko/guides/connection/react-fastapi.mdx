...
3. Cognito 또는 IAM 인증을 사용하는 API에 연결하는 경우 <Link path="/guides/react-website-auth">`ts#website#auth` 제너레이터</Link>를 통해 추가된 Cognito 인증
...
:::tip[자동 재생성]
React 애플리케이션과 FastAPI를 동시에 개발 중인 경우 React 애플리케이션의 `dev` 타겟(`serve-local`의 별칭)을 사용하면 API 변경 시 클라이언트 자동 재생성과 웹사이트/로컬 FastAPI 서버 핫 리로드가 가능합니다:
