...
<Link path="ko/guides/fastapi#streaming">FastAPI가 응답을 스트리밍하도록 구성</Link>한 경우, `useQuery` 훅은 스트림의 새 청크가 도착할 때마다 자동으로 데이터를 업데이트합니다.
...
<Link path="ko/guides/fastapi#streaming">FastAPI가 응답을 스트리밍하도록 구성</Link>한 경우, 생성된 클라이언트에는 `for await` 구문을 사용하여 스트림의 청크를 비동기적으로 반복하는 타입 안전 메서드가 포함됩니다.
