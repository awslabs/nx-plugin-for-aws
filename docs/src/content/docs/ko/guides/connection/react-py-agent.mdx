---
title: "React에서 Python Agent로 연결"
...
Nx Plugin for AWS는 <Link path="guides/py-agent">Python Agent</Link>를 React 웹사이트와 빠르게 통합할 수 있는 생성기를 제공합니다. 이는 AWS IAM 및 Cognito 인증 지원을 포함하여 타입 안전 OpenAPI 생성 클라이언트를 통해 에이전트에 연결하는 데 필요한 모든 구성을 설정합니다.

:::tip[AG-UI 에이전트]
Python Agent가 `protocol=AG-UI`로 생성된 경우, 연결 생성기는 대신 React 웹사이트에 [CopilotKit](https://docs.copilotkit.ai/aws-strands)과 [`@ag-ui/client`](https://docs.ag-ui.com/) `HttpAgent`를 연결합니다. 자세한 내용은 <Link path="guides/connection/react-agui">React to AG-UI Agent</Link> 가이드를 참조하세요.
:::
...
2. `protocol=HTTP`를 사용하는 Python Agent (<Link path="guides/py-agent">`py#agent` 생성기</Link>를 사용하여 생성)
...
React 웹사이트를 소스 프로젝트로, Python Agent가 포함된 프로젝트를 타겟 프로젝트로 선택하라는 메시지가 표시됩니다. 타겟 프로젝트에 여러 컴포넌트(예: 여러 에이전트 또는 다른 컴포넌트 유형)가 포함된 경우 명확하게 구분하기 위해 `targetComponent`를 지정하라는 메시지가 표시됩니다.
...
빌드 시점에 Python Agent의 FastAPI 앱을 검사하여 OpenAPI 사양을 생성합니다. 이 사양은 <Link path="guides/connection/react-fastapi">React to FastAPI 연결</Link>과 동일한 패턴을 따라 TanStack Query hooks가 포함된 타입 안전 TypeScript 클라이언트를 생성하는 데 사용됩니다.
