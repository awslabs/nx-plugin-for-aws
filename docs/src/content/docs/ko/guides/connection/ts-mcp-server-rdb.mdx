---
title: MCP Server에서 관계형 데이터베이스로
description: TypeScript MCP Server를 관계형 데이터베이스에 연결
when:
...
`connection` 생성기는 <Link path="guides/ts-mcp-server">TypeScript MCP Server</Link>를 <Link path="guides/ts-rdb">관계형 데이터베이스</Link> 프로젝트에 연결하여, `createServer` 내부에 등록된 모든 도구에서 Prisma 클라이언트를 사용할 수 있게 합니다.
...
다른 타겟으로 생성기를 다시 실행하면 첫 번째 데이터베이스와 함께 두 번째 데이터베이스가 추가됩니다. 둘 다 `createServer` 내부에서 가져와집니다:
...
<Snippet name="connection/ts-mcp-server-rdb-ssl-requirements" parentHeading="RDS Proxy 없이 연결할 때의 SSL 요구 사항" />
