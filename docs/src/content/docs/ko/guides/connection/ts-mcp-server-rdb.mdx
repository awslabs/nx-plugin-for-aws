...
:::caution
`securityGroups`가 생략되면, `RuntimeNetworkConfiguration.usingVpc`는 주어진 스코프에서 고정된 구성 ID(`SecurityGroup`)로 보안 그룹을 생성합니다. 표시된 것처럼 명시적인 보안 그룹을 전달하면 여러 런타임(예: 에이전트와 MCP 서버)이 동일한 스택에 정의될 때 구성 ID 충돌을 방지할 수 있습니다.
:::
