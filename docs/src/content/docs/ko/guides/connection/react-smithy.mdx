...
2. 작동하는 Smithy TypeScript API 백엔드 (<Link path="/guides/ts-smithy-api">`ts#api` 제너레이터</Link>를 `--framework=smithy`와 함께 사용하여 생성)
3. Cognito 또는 IAM 인증을 사용하는 API 연결 시 <Link path="/guides/react-website-auth">`ts#website#auth` 제너레이터</Link>로 추가된 Cognito 인증
...
:::tip[자동 재생성]
React 애플리케이션과 Smithy API를 동시에 개발 중인 경우 React 애플리케이션의 `dev` 타겟(`serve-local`의 별칭)을 사용하면 API 변경 시 클라이언트를 자동 재생성하고 웹사이트와 로컬 Smithy API 서버를 핫 리로드합니다:
...
Smithy API가 `Custom` 인증(Lambda Authorizer)을 사용하는 경우 생성된 클라이언트 프로바이더를 편집하여 인증자가 예상하는 인증 헤더를 추가해야 합니다. 생성된 `<ApiName>Provider.tsx`에서 `fetch` 구성을 찾아 요청 헤더에 토큰 또는 API 키를 추가하세요.
