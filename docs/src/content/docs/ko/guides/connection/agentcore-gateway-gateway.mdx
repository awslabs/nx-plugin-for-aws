...
소스 Gateway 프로젝트의 `<source-gateway>-dev` 대상은 대상 Gateway의 `<target-gateway>-dev` 대상에 대한 종속성을 얻으므로, 소스 Gateway를 로컬에서 실행하면 대상 게이트웨이(및 전이적으로 연결된 모든 MCP 서버)도 시작됩니다. 대상 게이트웨이는 소스 Gateway 프로젝트의 `local-dev.ts`에도 등록되어 로컬 게이트웨이가 해당 도구를 집계합니다.
...
:::note[대상 순서]
AgentCore는 `aws_bedrockagentcore_gateway_target`이 생성될 때 내부 게이트웨이의 도구를 가져오므로 내부 게이트웨이가 이미 해당 도구를 제공하고 있어야 합니다. 내부 게이트웨이의 `tool_dependencies`를 자체 대상의 ID로 설정하세요. 그러면 `gateway_url`이 준비 프로브를 통해 흐르며, 이 프로브는 해당 도구가 제공될 때까지 `tools/list`를 폴링하므로 외부 게이트웨이의 대상이 내부 게이트웨이가 준비될 때까지 대기합니다.
:::
</Fragment>
</Infrastructure>
