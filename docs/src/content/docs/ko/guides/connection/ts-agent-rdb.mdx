---
title: TypeScript Agent에서 관계형 데이터베이스로
description: TypeScript Agent를 관계형 데이터베이스에 연결
when:
  sourceType: ts#agent
  targetType: ts#rdb
---
...
`connection` 생성기는 <Link path="guides/ts-agent">TypeScript Agent</Link>를 <Link path="guides/ts-rdb">관계형 데이터베이스</Link> 프로젝트에 연결하여 에이전트 팩토리 내에서 Prisma 클라이언트를 사용할 수 있게 합니다.
...
Prisma 클라이언트는 `getAgent()` 내에서 인스턴스화됩니다. `ts#agent` 생성기가 세션당 단일 Agent를 구성하므로 클라이언트도 세션의 수명 동안 재사용됩니다:
...
에이전트를 데이터베이스와 동일한 VPC 내에서 실행하고, `additional_iam_policy_statements`를 통해 `rds-db:connect`를 부여하며, 보안 그룹 규칙 쌍으로 네트워크 경로를 엽니다. `aws_vpc.main` 및 `aws_subnet` 리소스는 데이터베이스 배포 가이드에 정의되어 있습니다:
...
<Snippet name="connection/strands-agent-rdb-ssl-requirements" parentHeading="RDS Proxy 없이 연결할 때의 SSL 요구 사항" />

## 로컬 개발

<NxCommands commands={["<agent-name>-dev <project-name>"]} />

이것은 에이전트와 연결된 모든 데이터베이스를 시작합니다. `LOCAL_DEV=true` 환경 변수는 각 Prisma 클라이언트가 Aurora 대신 로컬 Docker 데이터베이스에 연결하도록 합니다.
