...
4. Cognito 또는 IAM 인증을 사용하는 API 연결 시 <Link path="/guides/react-website-auth">`ts#website#auth` 제너레이터</Link>를 통해 추가된 Cognito 인증
...
백엔드에서 구독 프로시저를 정의하는 방법에 대한 정보는 <Link path="guides/trpc">tRPC API 제너레이터 가이드</Link>를 참조하세요.
...
tRPC API가 `Custom` 인증(Lambda Authorizer)을 사용하는 경우, 생성된 클라이언트 프로바이더에는 인증자가 요구하는 인증 헤더를 추가해야 하는 플레이스홀더 `headers`가 포함됩니다. 생성된 `<ApiName>ClientProvider.tsx`에서 `// TODO: Add headers required by your custom authorizer` 주석을 찾아 토큰 또는 API 키 로직으로 교체하세요.
