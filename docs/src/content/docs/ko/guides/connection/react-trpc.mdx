...
제너레이터는 백엔드의 `computeType`을 검사하고 일치하는 클라이언트를 생성합니다:

<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
tRPC 구독을 위해 Server-Sent Events를 통한 `httpSubscriptionLink`가 포함된 HTTP 링크입니다.
</TabItem>
<TabItem label="HTTP API" _filter={{ computeType: 'ServerlessApiGatewayHttpApi' }}>
일반 HTTP 링크입니다. API Gateway HTTP API는 응답 스트리밍을 지원하지 않으므로 구독이 지원되지 않습니다.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
AgentCore 런타임 URL에 대한 WebSocket 링크로, SigV4 서명 핸드셰이크(IAM) 또는 base64url 인코딩된 JWT 서브프로토콜(Cognito)을 사용합니다. 런타임 ARN은 `runtimeConfig.apis.<ApiName>`에서 읽어 즉시 `wss://bedrock-agentcore.<region>.amazonaws.com/runtimes/.../ws` URL로 변환됩니다.
</TabItem>
</Tabs>
...
추가적으로 다음 의존성을 설치합니다:
  ...
  - `event-source-polyfill` (REST API 백엔드 사용 시, 구독 지원용)
  - `react-oidc-context` (Cognito 인증 사용 시)

## 생성된 코드 사용

### tRPC 옵션 프록시 훅 사용

제너레이터는 `useQuery` 및 `useMutation`과 같은 TanStack Query 훅과 함께 사용할 수 있는 [tRPC 옵션 프록시](https://trpc.io/docs/client/tanstack-react-query)를 반환하는 `use<ApiName>` 훅을 제공합니다:
...
`use<ApiName>Client` 훅은 명령형 API 호출과 구독에 유용한 [바닐라 tRPC 클라이언트](https://trpc.io/docs/client/vanilla)에 대한 접근을 제공합니다:
...
<OptionFilter when={{ computeType: 'ServerlessApiGatewayHttpApi' }} description="HTTP API 백엔드는 구독을 지원하지 않습니다">
:::caution[HTTP API에서는 구독 불가]
API Gateway HTTP API는 응답 스트리밍을 지원하지 않습니다. 구독이 필요한 경우 백엔드 `computeType`으로 `ServerlessApiGatewayRestApi` 또는 `BedrockAgentCoreRuntimeWebSocket`을 선택하세요.
:::
</OptionFilter>

<OptionFilter when={{ computeType: ['ServerlessApiGatewayRestApi', 'BedrockAgentCoreRuntimeWebSocket'] }} description="구독이 클라이언트에 도달하는 방법 — 전송 방식은 백엔드 컴퓨트 타입에 따라 다릅니다">
<Tabs syncKey="trpc-compute">
<TabItem label="REST API" _filter={{ computeType: 'ServerlessApiGatewayRestApi' }}>
생성된 클라이언트는 구독 작업을 `httpSubscriptionLink` (Server-Sent Events)를 통해 라우팅하고 일반 쿼리/뮤테이션을 `httpLink`를 통해 라우팅하는 `splitLink`를 사용합니다. 추가 구성 없이도 구독이 즉시 작동합니다.
</TabItem>
<TabItem label="AgentCore" _filter={{ computeType: 'BedrockAgentCoreRuntimeWebSocket' }}>
생성된 클라이언트는 AgentCore 런타임 URL에 대해 `wsLink`를 사용합니다 — 모든 tRPC 작업(쿼리, 뮤테이션, 구독)이 단일 WebSocket 연결을 통해 전송됩니다. 핸드셰이크는 자동으로 인증됩니다: `IAM` 인증의 경우 SigV4 서명, `Cognito`의 경우 base64url 인코딩된 서브프로토콜로 JWT를 전달합니다.
</TabItem>
</Tabs>

백엔드에서 구독 프로시저를 정의하는 방법에 대한 정보는 <Link path="guides/trpc">`ts#trpc-api` 제너레이터 가이드</Link>를 참조하세요.

#### useSubscription 훅 사용

옵션 프록시의 `subscriptionOptions`와 함께 `useSubscription` 훅을 사용하여 구독을 소비할 수 있습니다:
...
- [tRPC TanStack React Query 통합](https://trpc.io/docs/client/tanstack-react-query)
- [tRPC 바닐라 클라이언트](https://trpc.io/docs/client/vanilla)
- [TanStack Query 공식 문서](https://tanstack.com/query/v5)