...
생성된 프로젝트는 [SQLModel](https://sqlmodel.tiangolo.com/)을 사용하여 데이터베이스 스키마를 정의합니다. 워크플로는 모델 우선입니다: 데이터베이스 프로젝트의 `<name>/models/` 디렉터리 아래에 SQLModel 테이블 클래스를 추가하거나 업데이트한 다음, 해당 모델 변경 사항으로부터 마이그레이션을 생성합니다.
...
다른 개발자가 생성한 마이그레이션 파일을 가져온 경우, 로컬 데이터베이스에 적용합니다:
...
데이터베이스 패키지에서 `session_context`를 가져와 비동기 컨텍스트 매니저로 사용하여 `AsyncSession`을 얻습니다:
...
`my_scope_my_db`는 데이터베이스 프로젝트의 모듈 이름입니다 — `<scope>_<name>`으로 명명된 단일 플랫 모듈이며, 프로젝트 내부의 패키지 디렉터리로 표시됩니다.
...
Amazon RDS CA 번들이 런타임의 시스템 신뢰 저장소에 있어야 합니다.
