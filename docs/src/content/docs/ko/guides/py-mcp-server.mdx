...
이 명령은 `uv run uvicorn --reload`를 사용하여 HTTP 전송으로 MCP 서버를 실행하고, 파일이 변경되면 자동으로 재시작합니다.

각 MCP 서버는 `8000`부터 시작하는 자체 포트가 할당되므로 동일한 워크스페이스에서 여러 서버를 나란히 실행할 수 있습니다. `project.json`의 `<your-server-name>-serve` 타겟에서 서버가 수신 대기하는 포트를 확인하세요.
