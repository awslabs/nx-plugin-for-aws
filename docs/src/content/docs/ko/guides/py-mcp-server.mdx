...
생성기는 `<your-server-name>-inspect` 대상을 구성하며, 이는 MCP 서버를 로컬에서 시작하고(`<your-server-name>-dev` 대상을 통해 로컬 데이터베이스와 같은 연결된 종속성 포함) 스트리밍 가능 HTTP 전송을 통해 연결하도록 사전 구성된 [MCP 인스펙터](https://github.com/modelcontextprotocol/inspector)를 실행합니다.
...
:::tip
인스펙터 없이 MCP 서버(및 연결된 종속성)를 실행하려면 프로젝트의 `dev` 대상을 사용하세요. 동일한 프로젝트에 여러 컴포넌트를 생성한 경우 `<your-server-name>-dev` 대상으로 단일 MCP 서버를 개별적으로 실행할 수 있습니다.
:::
