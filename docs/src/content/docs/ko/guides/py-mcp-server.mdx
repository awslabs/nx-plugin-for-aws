...
`computeType`을 `None`으로 선택한 경우 CDK 구성 또는 Terraform 모듈이 생성되지 않습니다 — MCP 서버는 로컬 STDIO / HTTP 사용만을 위해 구성됩니다. 호스팅되는 엔드포인트가 없어 인증할 필요가 없으므로 이 모드에서는 `auth` 옵션이 무시됩니다.
</OptionFilter>
