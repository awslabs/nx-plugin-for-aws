...
3. 결과 번역을 PR 브랜치에 다시 커밋하며, 커밋 메시지로 `docs: update translations`를 사용합니다. 이렇게 하면 후속 증분 실행이 기준 커밋을 감지하고 그 이후 변경된 파일만 다시 번역할 수 있습니다. 다른 메시지를 사용하려면 `scripts/translate.config.json`에 `translationCommitMessage` 키를 추가하고 해당 값을 대신 일치시키세요.
