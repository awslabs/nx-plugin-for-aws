...
`protocol`이 `A2A`로 설정되면 진입점은 tRPC 대신 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)를 사용합니다:
...
`protocol=A2A`일 때, 생성된 `index.ts`는 [Strands A2A Express Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)를 Express 앱에 마운트하여 생성된 에이전트가 `/ping` 상태 확인과 함께 A2A 프로토콜 엔드포인트를 노출하도록 합니다. AgentCore에 배포되면 진입점은 AppConfig에서 런타임의 공개 ARN을 확인하고 에이전트 카드에 이를 알립니다.
...
Strands SDK의 `A2AAgent`는 원격 A2A 엔드포인트를 래핑하여 로컬 `Agent`처럼 호출할 수 있도록 합니다. `invoke`는 `AgentResult`로 해석되며, 여기에 `.toString()`을 호출하면 원격 에이전트의 응답에서 텍스트 블록을 결합합니다.
...
#### IAM 인증으로 AgentCore에서 호출

AgentCore에 배포된 A2A 에이전트에 대한 요청(에이전트 카드 가져오기 포함)은 SigV4 서명이 필요합니다. 카드 리졸버와 JSON-RPC 전송 모두에 대해 SigV4 서명 fetch를 사용하는 `clientFactory`를 `A2AAgent`에 전달하세요:
...
:::note[IAM 권한]
호출자의 IAM 역할에는 대상 런타임 ARN에 대한 `bedrock-agentcore:InvokeAgentRuntime` 및 `bedrock-agentcore:GetAgentCard` 권한이 모두 필요합니다. 생성된 구성의 `grantInvokeAccess` 헬퍼는 두 권한을 모두 부여합니다.
:::

:::note[추가 종속성]
위의 SigV4 클라이언트 예제에는 `@a2a-js/sdk`, `aws4fetch` 및 `@aws-sdk/credential-providers`가 필요합니다. 예제를 실행하기 전에 `package.json`에 추가하세요.
:::

#### A2A 에이전트를 도구로 사용

`A2AAgent`는 로컬 `Agent`와 동일한 `InvokableAgent` 인터페이스를 구현하므로, Strands `tool`로 래핑하여 다른 에이전트에 전달할 수 있습니다. 생성된 HTTP/tRPC 에이전트 템플릿이 원격 A2A 에이전트에 위임하는 방식입니다:
...
CDK 스택에서 호스트 에이전트에 원격 에이전트에 대한 호출 액세스 권한을 부여하세요 — 이는 A2A 클라이언트에 필요한 두 IAM 작업을 모두 연결합니다:
