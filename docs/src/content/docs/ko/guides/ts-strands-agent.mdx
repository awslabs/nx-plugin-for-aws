...
생성기는 `<your-agent-name>-serve-local`에 의존하는 `<your-agent-name>-chat` Nx 타겟을 구성합니다. 실행하면 에이전트를 로컬에서 시작하고 [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)로 구동되는 대화형 터미널 채팅으로 진입합니다:

<NxCommands commands={['run your-project:agent-chat']} />

**A2A** 에이전트의 경우, 타겟은 단순히 `agent-chat-cli`를 바이너리로 실행합니다 — 워크스페이스에 코드가 생성되지 않습니다.

**HTTP** (tRPC over WebSocket) 에이전트의 경우, 생성기는 생성된 `<Agent>Client.local({ url })`를 `ChatAdapter`로 래핑하고 `agent-chat-cli`의 `chatLoop`에 전달하는 작은 `scripts/<your-agent-name>/chat.ts`를 생성합니다.
