...
생성기는 또한 `scripts/<your-agent-name>/chat.ts` 아래에 최소한의 대화형 채팅 CLI와 해당하는 `<your-agent-name>-chat` 타겟을 제공합니다. 이 타겟은 `<your-agent-name>-serve-local`에 의존하므로, 실행하면 에이전트를 로컬에서 시작하고 대화형 터미널 채팅으로 진입합니다:

<NxCommands commands={['run your-project:agent-chat']} />

CLI는 프로토콜에 적합한 클라이언트(HTTP/tRPC의 경우 `AgentClient.local` 또는 A2A의 경우 `@a2a-js/sdk`)를 사용하고 [`@clack/prompts`](https://github.com/bombshell-dev/clack)를 통해 에이전트 응답을 토큰 단위로 스트리밍합니다.
