...
:::caution[TypeScript에서 AG-UI가 아직 지원되지 않음]
[AG-UI 프로토콜](https://docs.ag-ui.com/)은 TypeScript Strands Agent에서 아직 지원되지 않습니다. AG-UI 지원이 필요한 경우 `protocol=AG-UI`와 함께 <Link path="/guides/py-strands-agent">`py#strands-agent`</Link> 생성기를 사용하세요. TypeScript AG-UI 지원 상태는 [strands-agents/sdk-typescript#347](https://github.com/strands-agents/sdk-typescript/issues/347)을 참조하세요.
:::
...
생성기는 터미널에서 에이전트와 대화형 채팅을 시작하는 `<your-agent-name>-invoke`라는 타겟을 구성합니다. 기본적으로 로컬 서버에 연결됩니다:

<NxCommands commands={['agent-invoke your-project']} />

대신 배포된 에이전트와 채팅하려면 `RUNTIME_CONFIG_APP_ID`를 워크스페이스의 AppConfig 애플리케이션 ID로 설정하세요 — CLI가 런타임 ARN을 확인하고 배포된 엔드포인트를 호출합니다:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke
```

Cognito 인증 에이전트의 경우 `--accessToken`으로 JWT를 전달하세요:

```bash
RUNTIME_CONFIG_APP_ID=<appconfig-app-id> pnpm nx run your-project:agent-invoke -- --accessToken <JWT>
```

CLI 스크립트는 `scripts/<agent>/cli.ts`에 있으며 에이전트에 적합한 프로토콜(HTTP/A2A)을 자동으로 선택합니다.
