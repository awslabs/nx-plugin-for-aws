...
:::tip
간소화된 설정을 위해 <Link path="/guides/connection/react-strands-agent">연결 생성기</Link>를 사용하여 React 웹사이트를 Strands Agent에 연결하세요. 이는 올바른 인증으로 tRPC WebSocket 클라이언트를 자동으로 설정합니다.
