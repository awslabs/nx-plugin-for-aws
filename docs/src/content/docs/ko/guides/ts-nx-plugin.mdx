...
- 컴파일된 JavaScript 파일
- TypeScript 선언 파일
- 게시된 플러그인에서 Nx가 읽는 `generators.json`, `executors.json` 및 `migrations.json` 매니페스트
- 문서 및 라이선스 파일
...
1. **패키지 정보 업데이트**: 플러그인의 [`package.json`](https://docs.npmjs.com/cli/v10/configuring-npm/package-json)을 적절한 이름, 버전, 설명 및 저장소 정보로 편집합니다. 또한 `"private": true`를 제거하세요.

2. **패키지 빌드**:
   <NxCommands commands={['package your-plugin']} />

   패키지는 `dist/<your-plugin's project root>/package`에 생성됩니다. 예를 들어 `--name=nx-plugin --directory=tools`로 생성된 플러그인의 경우 `dist/tools/nx-plugin/package`입니다.
