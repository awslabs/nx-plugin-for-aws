...
플러그인 프로젝트가 생성되면 <Link path="/ko/guides/nx-generator">`ts#nx-generator`</Link> 제너레이터를 사용하여 제너레이터를 추가할 수 있습니다:
...
:::tip[제너레이터 문서]
제너레이터 구현 방법에 대한 자세한 내용은 <Link path="/ko/guides/nx-generator">`ts#nx-generator` 가이드</Link>를 읽어보세요.
:::
...
플러그인은 <Link path="/ko/guides/ts-mcp-server">`ts#mcp-server`</Link> 제너레이터를 사용하여 MCP 서버를 구성합니다.
...
MCP Server 작성에 대한 자세한 내용은 <Link path="/ko/guides/ts-mcp-server">`ts#mcp-server` 가이드</Link>를 참조하세요.
