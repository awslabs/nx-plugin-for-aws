...
일부 기본 구성 라이선스 헤더 내용 및 형식이 `aws-nx-plugin.config.mts`에 추가되어 여러 파일 유형에 적절한 헤더를 작성합니다. 필요에 따라 이를 추가로 사용자 지정할 수 있습니다. 아래 [구성 섹션](#configuration)을 참조하세요.
...
:::note
지속적 통합 빌드 작업이 라이선스 동기화 문제로 실패하지 않도록 라이선스 동기화 생성기가 만든 변경 사항을 버전 관리에 확인하세요.
:::
...
추가로 저작권 소유자와 저작권 연도를 구성할 수 있으며, 이는 일부 `LICENSE` 파일에 포함됩니다:
...
- `lineStart`: 라이선스 내용의 각 줄에 앞에 추가되는 텍스트
- `lineEnd`: 라이선스 내용의 각 줄 끝에 추가되는 텍스트
- `blockEnd`: 라이선스 내용 뒤에 작성되는 텍스트 (예: 블록 주석 종료)

#### 사용자 정의 주석 구문

기본적으로 지원되지 않는 파일 유형의 경우, 동기화 생성기가 이러한 파일 유형의 기존 라이선스 헤더를 식별하는 방법을 알려주는 사용자 정의 주석 구문을 지정할 수 있습니다.
...
글로브 패턴을 사용하여 특정 프로젝트나 파일의 동기화를 제외할 수 있습니다:
  ...
  license: {
    files: {
      exclude: [
        ...
        // LICENSE 파일은 동기화하지 않고 package.json 및/또는 pyproject.toml은 동기화
