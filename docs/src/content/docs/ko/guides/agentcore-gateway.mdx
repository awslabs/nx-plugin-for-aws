...
[Amazon Bedrock AgentCore Gateway](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html) 프로젝트를 생성합니다. AgentCore Gateway는 하나 이상의 MCP 서버 대상을 단일 MCP 엔드포인트 뒤에 집계하고, 인바운드 요청을 인증하며(IAM 또는 Cognito), [Cedar 정책 엔진](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/policy.html)에 대해 모든 도구 호출을 평가하고, MCP 서버로의 아웃바운드 트래픽에 IAM SigV4로 서명하는 관리형 진입점입니다.
...
<FileTree>
  - packages/\<name>/
    - policies/ Cedar 정책 소스 파일 (`cedarPolicy: false`일 때 생략됨)
      - permit-all.cedar 인증된 호출자를 허용하는 기본 Cedar 정책 (동일한 AWS 계정의 IAM 호출자 또는 모든 Cognito 사용자 — [인증](#authentication) 참조)
...
- MCP 프로토콜용으로 구성된 `AgentCore::Gateway`로 인바운드 IAM 인증(기본값) 또는 Cognito JWT 인증 사용 (<a href="#authentication">인증</a> 참조)
...
`Cognito`를 선택하면 Gateway는 Cognito 사용자 풀을 가리키는 Custom JWT 권한 부여자로 구성됩니다. 호출자는 JWT 베어러 토큰을 제시하여 인증하며, 토큰의 `sub` 클레임은 Cedar 정책에서 `AgentCore::OAuthUser` 주체로 사용할 수 있습니다. 호출자가 Cognito를 통해 인증하는 최종 사용자인 경우(예: 웹사이트 또는 비AWS 클라이언트) 이 옵션을 사용하세요.
