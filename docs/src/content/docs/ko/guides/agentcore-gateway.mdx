...
로컬 게이트웨이는 배포된 Gateway와 일치하도록 도구에 `<target>___<tool>` 접두사가 붙은 모든 연결된 MCP 서버(<Link path="guides/connection/agentcore-gateway-mcp">`agentcore-gateway#mcp-connection` 생성기</Link>를 통해 연결됨)를 집계하는 단일 MCP 엔드포인트를 노출합니다.
</OptionFilter>

<OptionFilter when={{ protocol: 'http' }} description="Path-routing proxy local gateway">
로컬 게이트웨이는 배포된 Gateway의 경로 기반 라우팅과 일치하도록 `/<targetName>/...` 경로를 각 연결된 에이전트의 로컬 서버(<Link path="guides/connection/agentcore-gateway-agent">`agentcore-gateway#agent-connection` 생성기</Link>를 통해 연결됨)로 프록시합니다.
</OptionFilter>
