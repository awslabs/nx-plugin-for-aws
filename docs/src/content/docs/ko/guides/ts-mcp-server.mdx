...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/)는 AI 어시스턴트가 외부 도구 및 리소스와 상호 작용할 수 있도록 하는 오픈 표준입니다. LLM이 다음을 수행할 수 있는 일관된 방법을 제공합니다:
...
<FileTree>
  - packages/\<name>/
    ...
    - project.json 빌드, 번들링, 개발 타겟을 포함한 Nx 프로젝트 설정
    - src/
      - index.ts MCP 서버 진입점
...
이 명령은 `dist/packages/your-mcp-server/bundle/index.js` 경로에 번들 버전을 생성합니다 (디렉토리 설정에 따라 경로가 다를 수 있음).
...
`/path/to/workspace/dist/packages/your-mcp-server/bundle/index.js`를 실제 번들링된 MCP 서버 경로로 변경하세요.

:::caution
서버 연결 시 `ENOENT node` 오류가 발생하면 터미널에서 `which node` 명령으로 확인한 전체 node 경로를 지정해야 할 수 있습니다.
:::
...
이 생성기는 <Link path="/guides/typescript-project">TypeScript 프로젝트 생성기</Link>를 기반으로 하며 해당 타겟을 상속받고 다음과 같은 추가 타겟을 제공합니다:
...
이 기능은 개발 중에 AI 어시스턴트가 최신 버전의 MCP 서버를 사용하도록 보장해주므로 특히 유용합니다.
