...
생성기는 `<your-server-name>-inspect` 타겟을 구성하며, 이는 MCP 서버를 로컬에서 시작하고(`<your-server-name>-serve-local` 타겟을 통해, 로컬 데이터베이스와 같은 연결된 종속성 포함) Streamable HTTP 전송을 통해 연결하도록 사전 구성된 [MCP 검사기](https://github.com/modelcontextprotocol/inspector)를 실행합니다.
