  ...
  - your-project/
    - src/
      - mcp-server/ (사용자 지정 이름 지정 가능)
        ...
        - Dockerfile MCP 서버 호스팅용 진입점 (`computeType`이 `BedrockAgentCoreRuntime`으로 설정된 경우)
...
[STDIO 전송](https://modelcontextprotocol.io/docs/concepts/transports#standard-input%2Foutput-stdio)을 직접 사용하려면 `<your-server-name>-serve-stdio` 타겟을 사용할 수 있습니다.
...
- Streamable HTTP MCP 서버용 진입점으로 `http.ts`를 사용하여 [`esbuild`](https://esbuild.github.io/)로 단일 JavaScript 파일 번들 생성
