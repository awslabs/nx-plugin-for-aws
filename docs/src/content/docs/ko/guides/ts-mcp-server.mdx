...
리소스는 AI 어시스턴트에 컨텍스트를 제공합니다. 도구와 마찬가지로 각 리소스는 `resources/` 디렉터리 아래의 자체 파일에 위치하며 `register<Name>Resource` 함수를 내보냅니다. 이 함수를 `server.ts`에서 호출합니다. 파일에서 정적 리소스나 동적 리소스를 추가할 수 있습니다:
...
Astro.currentLocale || 'en'}/guides/connection/agentcore-gateway-mcp`}
    source="agentcore"
    target="mcp"
  />
</CardGrid>
