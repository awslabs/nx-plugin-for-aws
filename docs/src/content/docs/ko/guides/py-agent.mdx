...
Strands 에이전트 작성에 대한 심층 가이드는 [Strands 문서](https://strandsagents.com/docs/user-guide/quickstart/overview/)를 참조하세요. LangChain 에이전트의 경우 [`connection` 생성기](/guides/connection/py-agent-mcp)도 MCP 서버 및 A2A 에이전트 도구를 `create_agent`에 연결합니다.
