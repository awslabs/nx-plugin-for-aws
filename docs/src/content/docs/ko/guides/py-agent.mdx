---
title: "파이썬 에이전트"
description: "도구를 사용하여 AI 에이전트를 구축하고 Amazon Bedrock AgentCore 런타임에 배포하기 위한 파이썬 에이전트 생성"
generator: py#agent
---
...
두 가지 방법으로 Python 에이전트를 생성할 수 있습니다:
...
진입점은 [CopilotKit](https://docs.copilotkit.ai/)과의 직접적인 프론트엔드 통합을 위해 [AG-UI](https://docs.ag-ui.com/) 프로토콜을 통해 에이전트를 노출합니다. Strands 에이전트는 [ag-ui-strands](https://docs.ag-ui.com/) 통합을 사용하고, LangChain 에이전트는 [ag-ui-langgraph](https://docs.ag-ui.com/)를 사용합니다:
  ...
  - your-project/
    - your_module/
      - agent/ (지정한 경우 사용자 정의 이름 사용)
        ...
        - Dockerfile 에이전트 호스팅용 진입점 (`infra`이 `None`으로 설정된 경우 제외)
    - pyproject.toml 프레임워크 및 AG-UI 종속성으로 업데이트
    - project.json 에이전트 서빙 타겟으로 업데이트
</FileTree>
...
`infra`으로 `None`을 선택한 경우 CDK 컨스트럭트나 Terraform 모듈이 생성되지 않습니다 — 에이전트는 로컬에서만 실행할 수 있습니다. 인증할 호스팅 엔드포인트가 없으므로 이 모드에서는 `auth` 옵션이 무시됩니다.
</OptionFilter>
...
도구는 AI 에이전트가 작업 수행을 위해 호출할 수 있는 함수입니다. 두 프레임워크 모두 도구 정의를 위해 데코레이터 기반 접근 방식을 사용하고, 함수 이름과 docstring에서 도구 이름과 설명을 파생하며, 타입 힌트에서 입력 스키마를 생성합니다.
...
<TabItem label="LangChain" _filter={{ framework: 'langchain' }}>
LangChain은 대규모 [도구 및 통합](https://docs.langchain.com/oss/python/integrations/tools) 생태계를 제공합니다. 관련 통합 패키지를 설치한 다음 도구를 `create_agent`에 전달하세요:
...
<Link path="/guides/py-mcp-server">`py#mcp-server`</Link> 또는 <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> 생성기를 사용하여 생성한 MCP 서버의 경우 <Link path="/guides/connection/py-agent-mcp">`connection` 생성기</Link>를 활용할 수 있으며, 이는 두 프레임워크 모두에 대해 MCP 서버의 도구를 에이전트에 연결합니다.
...
에이전트 작성에 대한 심층 가이드는 [Strands](https://strandsagents.com/docs/user-guide/quickstart/overview/) 또는 [LangChain](https://docs.langchain.com/oss/python/langchain/overview) 문서를 참조하세요.
...
- **Strands**: `Agent`를 `ag_ui_strands.StrandsAgent`로 래핑하고 `create_strands_app()`을 통해 FastAPI 앱을 생성합니다.
- **LangChain**: 컴파일된 그래프를 `ag_ui_langgraph.LangGraphAgent`로 래핑하고 수동으로 작성된 FastAPI `/invocations` 루프에서 제공합니다.

대부분의 사용자는 이 파일을 수정할 필요가 없습니다 — 도구나 시스템 프롬프트를 변경하려면 `agent.py`를 편집하세요.

:::tip[React 웹사이트에 연결]
AG-UI 에이전트는 프론트엔드에서 직접 사용되도록 설계되었습니다. <Link path="/guides/connection/react-agui">`connection` 생성기</Link>를 사용하여 [CopilotKit](https://docs.copilotkit.ai/) 프로바이더와 [AG-UI HttpAgent](https://docs.ag-ui.com/) 클라이언트로 React 웹사이트를 에이전트에 연결하세요.
:::
</OptionFilter>
...
생성기는 개발 및 테스트를 위해 에이전트를 로컬에서 시작하는 `<your-agent-name>-serve` 타겟을 구성합니다.

<NxCommands commands={['agent-serve your-project']} />

이 명령은 [Bedrock AgentCore Python SDK](https://github.com/aws/bedrock-agentcore-sdk-python)를 사용하여 에이전트를 실행하기 위해 `uv run`을 사용합니다.

### 에이전트와 채팅하기

생성기는 에이전트와의 대화형 터미널 채팅으로 진입하는 `<your-agent-name>-chat` Nx 타겟을 구성합니다.
...
**HTTP** 에이전트의 경우, 채팅 스크립트는 에이전트의 OpenAPI 스펙에서 생성된 타입 안전 TypeScript 클라이언트를 사용합니다. 생성기는 다음도 생성합니다:
...
## Bedrock AgentCore Runtime에 에이전트 배포

<Snippet name="agent/bedrock-deployment" parentHeading="Bedrock AgentCore Runtime에 에이전트 배포" />

### 번들 및 도커 타겟

Bedrock AgentCore Runtime용 에이전트 빌드를 위해 프로젝트에 `bundle` 타겟이 추가됩니다. 이 타겟은:

- `uv export`를 사용하여 Python 종속성을 `requirements.txt` 파일로 내보냄
- 대상 플랫폼(`aarch64-manylinux_2_28`)용 종속성을 `uv pip install`로 설치

또한 에이전트 전용 `docker` 타겟이 추가되어 `Dockerfile` 및 번들된 아티팩트를 도커 컨텍스트 디렉토리로 복사합니다. 이렇게 하면 `Dockerfile`이 빌드 출력과 함께 배치되어 CDK가 `AgentRuntimeArtifact.fromAsset`을 사용하여 Docker 이미지를 직접 빌드할 수 있습니다.
...
React 웹사이트에서 에이전트를 호출하려면 <Link path="/guides/connection/react-py-agent">`connection` 생성기</Link>를 활용할 수 있습니다. 이 생성기는 올바른 인증(IAM 또는 Cognito)이 설정된 클라이언트를 자동으로 설정합니다.
...
이 에이전트에서 원격 A2A 에이전트(<Link path="/guides/ts-agent">TypeScript</Link> 또는 <Link path="/guides/py-agent">Python</Link>)로 작업을 위임하려면 <Link path="/guides/connection/py-agent-a2a">`connection` 생성기</Link>를 사용하세요. 이 생성기는 대상 에이전트에 대한 SigV4 인증 클라이언트를 제공하고 이 에이전트의 `agent.py`를 AST 변환하여 원격 A2A 에이전트를 `@tool` 데코레이터가 적용된 위임자로 등록합니다.
...
React 웹사이트에서 AG-UI 에이전트를 호출하려면 <Link path="/guides/connection/react-agui">`connection` 생성기</Link>를 사용하세요. 이 생성기는 올바른 인증(IAM 또는 Cognito)이 설정된 배포된 에이전트용 [CopilotKit](https://docs.copilotkit.ai/) 클라이언트를 연결합니다.
...
<CardGrid>
  <ConnectionCard
    ...
    description="React 웹사이트에서 Python 에이전트 호출"
  ...
  <ConnectionCard
    ...
    description="CopilotKit을 통해 React 웹사이트에서 AG-UI 프로토콜을 노출하는 에이전트 호출"
  ...
  <ConnectionCard
    ...
    description="Python 에이전트를 MCP 서버에 연결"
  ...
  <ConnectionCard
    ...
    description="Python 에이전트를 원격 A2A 에이전트에 연결"
  ...
  <ConnectionCard
    ...
    description="TypeScript 에이전트를 원격 A2A 에이전트에 연결"
  ...
  <ConnectionCard
    ...
    description="Python 에이전트를 DynamoDB 테이블에 연결"
  ...
  <ConnectionCard
    ...
    description="Python 에이전트를 AgentCore Gateway에 연결"
