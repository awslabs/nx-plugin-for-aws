---
title: "Python 에이전트"
description: "도구를 사용하여 AI 에이전트를 구축하고 Amazon Bedrock AgentCore 런타임에 배포하기 위한 Python 에이전트 생성"
generator: py#agent
---
...
아래 예제는 Strands를 사용합니다. LangChain 에이전트는 `langchain_core.tools`의 `@tool` 데코레이터와 `ChatBedrockConverse` 모델을 사용하는 `create_agent`를 사용하여 동일한 방식으로 작동합니다(`MODEL_ID` 및 `AWS_REGION` 환경 변수가 모델과 리전을 설정합니다). 자세한 내용은 [LangChain 문서](https://docs.langchain.com/oss/python/langchain/overview)를 참조하세요.
...
Strands 에이전트 작성에 대한 심층 가이드는 [Strands 문서](https://strandsagents.com/docs/user-guide/quickstart/overview/)를 참조하세요. LangChain 에이전트의 경우 <Link path="/guides/connection/py-agent-mcp">`connection` 생성기</Link>도 MCP 서버 및 A2A 에이전트 도구를 `create_agent`에 연결합니다.
