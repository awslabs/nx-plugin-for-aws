...
각 사용자가 자신의 대화에만 접근하도록 하려면, 자체 애플리케이션에서 각 세션의 소유자를 기록하고 진입 시 해당 소유권을 확인하세요. 그러면 세션 ID는 호출자가 선택하는 것이 아니라 애플리케이션이 발급하는 것이 됩니다.

**1. API를 통해 세션 생성하기**

대화를 시작하는 프로시저를 추가하세요. 이는 세션 ID 자체를 생성하고, 자체 요청 컨텍스트에서 호출자를 확인하고, 쌍을 저장합니다 — 예를 들어 <Link path="guides/py-dynamodb">`py#dynamodb`</Link> 생성기로 생성된 테이블에:
...
API는 어떤 언어로든 작성할 수 있습니다. 위 예시는 <Link path="guides/trpc">`ts#trpc-api`</Link> 생성기를 사용합니다.

브라우저에서 제공할 수 없도록 입력 필드가 아닌 검증된 요청 컨텍스트에서 호출자를 확인하세요. <Link path="guides/trpc">커스텀 미들웨어 구현하기</Link>는 IAM 및 Cognito 인증 API 모두에 대해 이를 수행하는 방법을 보여줍니다.

해당 인덱스를 쿼리하는 두 번째 프로시저를 사용하면 UI가 다른 사람의 대화를 보지 않고 사용자에게 자신의 대화를 제공할 수 있습니다.

**2. 에이전트에서 소유권 검증하기**

에이전트는 이미 세션 ID를 받고 있으며, `auth=cognito`를 사용하면 호출자의 JWT도 받습니다 — AgentCore Runtime 권한 부여자가 이를 검증하고 생성된 인프라가 `Authorization` 헤더를 코드로 허용 목록에 추가합니다. 세션을 조회하고, 소유자를 호출자와 비교하고, 다르면 요청을 거부하세요:
...
AgentCore Runtime이 인바운드 권한 부여 중에 토큰을 검증했으므로 서명을 검증하지 않고 클레임을 디코딩하는 것으로 충분합니다.

:::tip[API와 에이전트 모두에서 검증하는 이유]
API는 세션의 소유자를 결정하고, 에이전트는 이를 시행합니다. 두 곳에서 모두 확인하면 브라우저에서 복사되거나 이전 대화에서 재사용된 세션 ID가 유출되더라도 다음 호출 시 여전히 거부되며, 에이전트의 향후 클라이언트는 동일한 규칙을 상속합니다.
:::

:::caution[세션은 microVM도 선택합니다]
세션 ID는 요청을 처리하는 [microVM](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-how-it-works.html)을 선택하므로, 파일 시스템, `/tmp` 및 프로세스 내 캐시는 해당 세션으로 확인되는 모든 요청에서 공유됩니다. 에이전트나 도구가 로컬 디스크에 쓰는 경우 주의할 가치가 있습니다. 동일한 세션 ID를 보내는 두 호출자는 저장된 대화가 분리되어 있어도 해당 리소스를 공유합니다.

API에서 불투명한 세션 ID를 생성하면 호출자가 제공받지 않은 세션에 도달할 수 없으므로 이를 방지할 수 있습니다. 사용자 제공 값(대화 이름, 이메일, 게임 이름)에서 파생하는 것은 예측 가능하며 두 호출자가 충돌할 수 있습니다.
:::
