...
Strands 에이전트의 경우, 생성기는 기본적으로 [FastAPI](https://fastapi.tiangolo.com/)를 사용하여 HTTP 서버를 노출합니다. 또는 다른 A2A 호환 에이전트와의 상호 운용성을 위해 [Agent-to-Agent (A2A)](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-a2a.html) 프로토콜을 선택하거나, [CopilotKit](https://docs.copilotkit.ai/)을 통한 직접적인 프론트엔드 통합을 위해 [AG-UI](https://docs.ag-ui.com/) 프로토콜을 선택할 수 있습니다.

:::note[LangChain은 AG-UI만 지원]
`langchain` 프레임워크는 현재 `ag-ui` 프로토콜만 지원합니다. `http` 또는 `a2a` 프로토콜이 필요한 경우 기본 `strands` 프레임워크를 선택하세요. LangChain 에이전트는 여전히 <Link path="/guides/connection/py-agent-mcp">`connection` 생성기</Link>를 통해 MCP 서버, AgentCore 게이트웨이 및 A2A 에이전트에 연결할 수 있습니다.
:::
...
아래 예제는 Strands를 사용합니다. LangChain 에이전트는 동일한 방식으로 작동하며, `langchain_core.tools`의 `@tool` 데코레이터와 `ChatBedrockConverse` 모델을 사용하는 `create_agent`를 사용합니다(`MODEL_ID` 및 `AWS_REGION` 환경 변수가 모델과 리전을 설정합니다). 자세한 내용은 [LangChain 문서](https://docs.langchain.com/oss/python/langchain/overview)를 참조하세요.
...
<Link path="/guides/py-mcp-server">`py#mcp-server`</Link> 또는 <Link path="/guides/ts-mcp-server">`ts#mcp-server`</Link> 생성기를 사용하여 생성한 MCP 서버의 경우 <Link path="/guides/connection/py-agent-mcp">`connection` 생성기</Link>를 활용할 수 있습니다.
...
Strands 에이전트 작성에 대한 심층 가이드는 [Strands 문서](https://strandsagents.com/docs/user-guide/quickstart/overview/)를 참조하세요. LangChain 에이전트의 경우, <Link path="/guides/connection/py-agent-mcp">`connection` 생성기</Link>도 MCP 서버, AgentCore 게이트웨이 및 A2A 에이전트 도구를 `create_agent`에 연결합니다.
