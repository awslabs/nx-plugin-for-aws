...
<Tabs syncKey="agent-framework">
<TabItem label="Strands" _filter={{ framework: 'strands' }}>
기본적으로 Strands 에이전트는 Amazon Bedrock의 Claude Sonnet 4.6을 사용하지만, 모델 공급자를 커스터마이즈할 수 있습니다. 구성 옵션은 [Strands 모델 공급자 문서](https://strandsagents.com/docs/user-guide/concepts/model-providers/)를 참조하세요:
...
에이전트는 생성 시 워크스페이스 풀에서 할당된 포트에서 수신합니다. 프로젝트의 `project.json`에서 `metadata.ports`를 읽거나, `<your-agent-name>-dev` 타겟의 명령에서 `--port` 플래그를 확인하세요. 아래 예제는 새 워크스페이스에서 첫 번째 HTTP 에이전트에 할당되는 포트인 `8081`을 사용합니다.
...
- `scripts/<your_agent_name>_openapi.py` — 에이전트의 OpenAPI 스펙을 내보내는 작은 스크립트 (에이전트 이름을 `snake_case`로 사용하여 명명됨)
...
그런 다음 로컬 에이전트가 실행 중인 포트의 `/invocations`에 POST 요청을 보내세요. 에이전트에 할당된 포트로 대체하세요 — 프로젝트의 `project.json`에서 `metadata.ports`를 읽거나, `<your-agent-name>-dev` 타겟의 명령에서 `--port` 플래그를 확인하세요. 새 워크스페이스에서 첫 번째 HTTP 에이전트는 `8081`이 할당됩니다:
...
:::caution[`-dev` 사용, `-serve` 아님]
이를 위해 `<your-agent-name>-serve`가 아닌 `<your-agent-name>-dev`를 사용하세요. `-serve`는 배포된 코드 경로를 실행하므로 `RUNTIME_CONFIG_APP_ID` 설정이 필요합니다 — <Link path="guides/py-agent#-dev-vs--serve">`-dev` vs `-serve`</Link>를 참조하세요. 이것이 없으면 에이전트가 세션 스토리지를 확인하려고 할 때 이미 스트리밍이 시작되어 요청이 오류 없이 빈 `200`을 반환하고 실패는 서버 로그에만 나타납니다.
:::
