...
- **`s3`**: 배포된 에이전트는 프로비저닝된 세션 버킷과 함께 `S3CheckpointSaver`를 사용하여 `checkpoints/` 접두사 아래에 체크포인트 및 보류 중인 쓰기를 저장합니다. 이 클래스([`langgraph-checkpoint-s3`](https://github.com/Isa-rentacs/langgraph-checkpoint-s3)에서 적응, MIT 라이선스)는 공유 에이전트 연결 프로젝트의 `s3_checkpoint_saver_langchain.py`에 있습니다.
...
- **`in-memory`** (기본값): 테이블이나 버킷이 프로비저닝되지 않습니다. 대화 상태는 실행 중인 프로세스의 수명 동안만 메모리에 유지되며 재시작이나 스케일 인 시 유지되지 않습니다.
