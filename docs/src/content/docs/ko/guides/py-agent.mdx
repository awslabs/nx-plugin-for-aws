...
- **Strands**: `Agent`를 `ag_ui_strands.StrandsAgent`로 래핑하고, FastAPI `lifespan` 핸들러 내부에서 빌드되며(따라서 import 시점이 아닌 컨테이너/세션 시작 시 생성됨), 수동으로 작성된 FastAPI `/invocations` 루프에서 제공됩니다.
- **LangChain**: 컴파일된 그래프를 `ag_ui_langgraph.LangGraphAgent`로 래핑하고, 동일한 방식으로 `lifespan` 내부에서 빌드되며, 수동으로 작성된 FastAPI `/invocations` 루프에서 제공됩니다.
...
프로젝트에 여러 컴포넌트(에이전트, MCP 서버 등)를 추가한 경우, 이 명령은 모든 것을 시작합니다. 이 에이전트만 실행하려면 `<your-agent-name>-dev` 타겟을 대상으로 지정하세요:

<NxCommands commands={['agent-dev your-project']} />

이것은 [Bedrock AgentCore Python SDK](https://github.com/aws/bedrock-agentcore-sdk-python)를 사용하여 에이전트를 실행하기 위해 `uv run`을 사용합니다.
...
<CardGrid>
  ...
  <ConnectionCard
    ...
    description="런타임 대상으로 AgentCore Gateway로 에이전트를 프론트하기"
