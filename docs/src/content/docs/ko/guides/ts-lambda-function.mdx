...
`eventSource`로 `Any`를 선택했습니다. 이는 `parser` 미들웨어가 연결되지 않으며 `event` 매개변수가 `any` 타입으로 지정됨을 의미합니다. 나중에 컴파일 타임 타입 안전성과 런타임 유효성 검증을 원하는 경우, 특정 `eventSource`로 함수를 다시 생성하세요.
</OptionFilter>
