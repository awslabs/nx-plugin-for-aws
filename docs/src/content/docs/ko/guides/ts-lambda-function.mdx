...
<Infrastructure>
<Fragment slot="cdk">
생성기는 함수 배포에 사용할 수 있는 CDK 구성을 생성하며, 이는 `packages/common/constructs/src/app/lambda-functions` 디렉토리에 위치합니다.
</Fragment>
<Fragment slot="terraform">
생성기는 함수 배포에 사용할 수 있는 Terraform 모듈을 생성하며, 이는 `packages/common/terraform/src/app/lambda-functions/<function-name>` 디렉토리에 위치합니다.
</Fragment>
</Infrastructure>
...
`captureLambdaHandler` 미들웨어를 통해 AWS X-Ray 추적이 자동 구성됩니다. 추적에 커스텀 서브세그먼트를 추가할 수 있습니다:
...
:::tip
DynamoDB 스트림이나 EventBridge 이벤트와 같이 이벤트 내 중첩된 커스텀 데이터가 있는 경우, [Envelopes](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/#envelopes)를 사용하여 해당 데이터에 대한 유형 안전성을 제공할 수 있습니다.
:::

이벤트 유형을 지정하지 않으려면 `eventSource`로 `Any`를 선택하면 이벤트 매개변수가 `any` 유형으로 설정됩니다.

## 번들링

생성기는 최적화된 Lambda 배포 패키지를 위해 [esbuild](https://esbuild.github.io/)를 자동 구성합니다:
