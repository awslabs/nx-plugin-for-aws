...
생성기는 선택한 `iac` 기반으로 함수 배포를 위한 인프라 코드를 생성합니다:
...
람다 함수 생성 시 `event`를 선택한 경우, [AWS Lambda Powertools의 `parser` 미들웨어](https://docs.powertools.aws.dev/lambda/typescript/latest/utilities/parser/)가 함수에 적용됩니다. 예시:
...
`event`로 `Any`를 선택한 경우, `parser` 미들웨어가 연결되지 않으며 `event` 매개변수가 `any` 타입으로 지정됩니다. 컴파일 타임 타입 안전성과 런타임 유효성 검증을 원하는 경우 특정 `event`로 함수를 다시 생성하세요.
</OptionFilter>
