...
`UserIdentity`는 [Managed Login (version 2)](https://docs.aws.amazon.com/cognito/latest/developerguide/managed-login.html)로 User Pool 도메인을 구성합니다. Managed Login은 **App Client당 브랜딩 스타일**이 필요하므로 새 프록시 클라이언트에 대해 하나를 생성해야 합니다 — 그렇지 않으면 호스팅된 로그인 페이지가 `403`을 반환합니다. 기본 스타일을 위해 `useCognitoProvidedValues: true`와 함께 `CfnManagedLoginBranding`을 사용하세요.
...
`identity.userPool.userPoolId`, `proxyClient.userPoolClientId` 및 `identity.userPoolDomain.baseUrl()`을 프록시에 전달합니다. `UserIdentity` 구성은 항상 도메인을 생성하므로 추가할 필요가 없습니다.
</Fragment>
<Fragment slot="terraform">
```hcl
# ts#website#auth로 생성된 동일한 User Pool의 기밀 App Client
...
User Pool id, 이 클라이언트의 id 및 기존 User Pool 도메인을 프록시 모듈에 전달합니다.
</Fragment>
</Infrastructure>
:::
