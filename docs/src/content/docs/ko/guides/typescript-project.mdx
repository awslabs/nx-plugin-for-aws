...
pnpm에서는 워크스페이스가 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)도 설정하므로 `pnpm add`가 버전을 카탈로그에 기록하고 자동으로 참조합니다.

yarn과 bun에는 동등한 기능이 없으며, `catalog:` 프로토콜은 기존 카탈로그 항목만 *참조*할 수 있고 새로 생성할 수는 없습니다. yarn 또는 bun에서 새 의존성을 추가하려면 먼저 카탈로그에 버전을 기록한 다음 프로젝트에서 참조하세요:

- **yarn** — `.yarnrc.yml`의 `catalog` 아래에 버전을 추가한 다음 `yarn workspace @my-scope/my-library add some-npm-package@catalog:`
- **bun** — 루트 `package.json`의 `catalog` 아래에 버전을 추가한 다음 `bun add some-npm-package@catalog: --cwd packages/my-library`
