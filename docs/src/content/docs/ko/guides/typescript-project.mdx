...
<Tabs syncKey="package-manager">
  <TabItem label="pnpm">
    [`--save-catalog`](https://pnpm.io/settings#catalogmode)와 함께 의존성을 추가하면 pnpm이 버전을 카탈로그에 기록하고 참조합니다:
    ...
    :::note
    `pnpm-workspace.yaml`에서 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)를 설정하면 모든 `pnpm add`에 대해 이것이 기본값이 됩니다. 또한 카탈로그가 다른 버전을 고정할 때마다 `pnpm add some-npm-package@<version>`이 실패하게 되어 `nx migrate`가 Nx를 업그레이드하는 것을 방지합니다 — 따라서 생성된 워크스페이스는 이를 설정하지 않은 상태로 둡니다.
    :::
  </TabItem>
  <TabItem label="yarn">
