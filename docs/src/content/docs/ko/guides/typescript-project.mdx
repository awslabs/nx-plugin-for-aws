...
pnpm에서는 워크스페이스가 [`catalogMode: strict`](https://pnpm.io/settings#catalogmode)도 설정하여 `pnpm add`가 새 의존성을 카탈로그에 자동으로 기록합니다. yarn과 bun에는 동등한 설정이 없으므로, 카탈로그에 기록하려면 `catalog:` 프로토콜로 새 의존성을 추가하세요. 예: `yarn workspace @my-scope/my-library add some-npm-package@catalog:` 또는 `bun add some-npm-package@catalog: --cwd packages/my-library`.
