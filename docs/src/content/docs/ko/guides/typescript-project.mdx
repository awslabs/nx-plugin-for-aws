...
이 파일은 프로젝트가 참조하는 모든 의존성을 선언해야 합니다. 빌드 시 프로젝트가 워크스페이스 루트 `package.json`을 통해 설치된 의존성을 확인하므로, Biome의 `noUndeclaredDependencies` 규칙은 프로젝트가 `package.json`에 나열되지 않은 패키지를 임포트하는 경우 경고를 표시합니다.
...
이렇게 하면 빌드 중에 lint 대상을 완전히 건너뜁니다.
