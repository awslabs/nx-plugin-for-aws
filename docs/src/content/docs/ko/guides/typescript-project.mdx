...
프로젝트에는 `package` 타겟도 있습니다. 이는 `build`의 아티팩트 전용 형제 타겟으로, 린트, 테스트 또는 타입 체크 게이트를 실행하지 않고 프로젝트가 배포에 기여하는 것(컴파일된 출력 및 선언된 번들, 컨테이너 이미지 또는 생성된 사양)을 생성합니다.

<NxCommands commands={['package <project-name>']} />

<Link path="guides/typescript-infrastructure#deploying-to-aws">배포 타겟</Link>은 `build`가 아닌 `package`에 의존하므로 배포는 배포하려는 것만 빌드합니다. `build`는 변경되지 않고 여전히 모든 것을 수행하므로 프로젝트를 확인하기 위해 실행하는 것 — 그리고 CI에서 실행해야 하는 것 — 으로 남아 있습니다.

:::caution
`package`는 품질 게이트를 건너뛰기 때문에 배포는 더 이상 린트 오류나 실패한 단위 테스트에서 실패하지 않습니다. 코드를 확인하려면 `build`(또는 CI 파이프라인)를 실행하세요.
:::
