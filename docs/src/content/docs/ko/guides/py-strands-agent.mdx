...
:::tip[React 웹사이트에 연결]
AG-UI 에이전트는 <Link path="/guides/connection/react-agui">`react` → AG-UI agent connection generator</Link>를 사용하여 React 프론트엔드에 연결할 수 있으며, 풍부한 채팅 경험을 위한 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 컴포넌트를 설정합니다.
:::
...
:::tip[React 웹사이트에 연결]
AG-UI 에이전트는 프론트엔드에서 직접 사용되도록 설계되었습니다. <Link path="/guides/connection/react-agui">`react` → AG-UI agent connection generator</Link>를 사용하여 [CopilotKit](https://docs.copilotkit.ai/aws-strands) 프로바이더와 [AG-UI HttpAgent](https://docs.ag-ui.com/) 클라이언트로 React 웹사이트를 에이전트에 연결하세요.
:::
