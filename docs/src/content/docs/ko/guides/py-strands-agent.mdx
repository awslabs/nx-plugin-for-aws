...
생성기는 `<your-agent-name>-serve-local`에 의존하는 `<your-agent-name>-chat` Nx 타겟을 구성합니다. 실행하면 에이전트가 로컬에서 시작되고 [`agent-chat-cli`](https://www.npmjs.com/package/agent-chat-cli)로 구동되는 대화형 터미널 채팅으로 진입합니다:

<NxCommands commands={['run your-project:agent-chat']} />

**A2A** 및 **AG-UI** 에이전트의 경우, 채팅 타겟은 로컬에서 실행 중인 에이전트에 대해 `agent-chat-cli`를 바이너리로 호출하기만 하며 — 워크스페이스에 코드가 생성되지 않습니다.

**HTTP** 에이전트의 경우, 생성기는 다음을 생성합니다:

- `scripts/<your-agent-name>_openapi.py` — 에이전트의 OpenAPI 스펙을 내보내는 작은 스크립트
- 이를 실행하는 `<your-agent-name>-openapi` Nx 타겟
- `open-api#ts-client`를 실행하여 `scripts/<your-agent-name>/generated/` 아래에 타입 안전 TypeScript 클라이언트를 생성하는 `<your-agent-name>-generate-client` Nx 타겟
- `scripts/<your-agent-name>/chat.ts` — 생성된 클라이언트를 가져와 `agent-chat-cli`의 `chatLoop`에 연결하는 얇은 어댑터

채팅 스크립트는 생성된 클라이언트를 감싸는 작은 래퍼입니다. 에이전트의 입력 형태를 사용자 정의할 때(예: `InvokeInput`에 새 필드 추가), 어댑터의 `sendMessage`를 업데이트하여 새 필드를 전달하면 나머지는 자동으로 작동합니다.
