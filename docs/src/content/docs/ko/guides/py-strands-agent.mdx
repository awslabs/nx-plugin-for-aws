...
도구를 사용하여 AI 에이전트를 구축하고 [Amazon Bedrock AgentCore Runtime](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/)에 배포할 수 있는 Python [Strands Agent](https://strandsagents.com/)를 생성합니다.

## Strands란?

[Strands](https://strandsagents.com/latest/documentation/docs/)는 AI 에이전트 구축을 위한 가벼우면서 프로덕션 준비가 된 Python 프레임워크입니다. 주요 기능은 다음과 같습니다:

- **가벼움과 커스터마이징 가능**: 간단한 에이전트 루프로 개발자 방해 최소화
- **프로덕션 준비 완료**: 완전한 관찰성, 추적 및 확장을 위한 배포 옵션
...
:::note
`computeType`을 `None`으로 선택하지 않은 경우, 생성기는 Strands 에이전트 배포를 위한 CDK 또는 Terraform 인프라도 생성합니다.
:::
...
기본적으로 Strands 에이전트는 Claude 4 Sonnet을 사용하지만 모델 제공자를 커스터마이즈할 수 있습니다. 구성 옵션은 [Strands 모델 제공자 문서](https://strandsagents.com/latest/documentation/docs/user-guide/quickstart/#model-providers)를 참조하세요:
...
`agent.py`의 `get_agent` 메서드를 수정하여 MCP 클라이언트를 생성하고 도구를 추가할 수 있습니다. 다음 예제는 IAM(SigV4) 인증을 사용하는 방법을 보여줍니다:
...
위 IAM 인증 예제를 사용하려면 인프라에서 두 가지를 구성해야 합니다. 첫째, MCP 서버의 AgentCore Runtime ARN을 위한 환경 변수를 추가하고, 둘째 에이전트에 MCP 서버 호출 권한을 부여해야 합니다. 이는 다음과 같이 구현할 수 있습니다:
...
생성기는 AgentCore의 에이전트가 구현해야 하는 기본 HTTP 계약을 관리하기 위해 [Bedrock AgentCore Python SDK](https://github.com/aws/bedrock-agentcore-sdk-python)를 구성합니다.
...
생성기는 개발 및 테스트를 위해 로컬에서 Strands 에이전트를 시작하는 `<your-agent-name>-serve` 타겟을 구성합니다.
...
<Infrastructure>
<Fragment slot="cdk">
생성기는 생성기 실행 시 선택한 `name`을 기반으로 이름이 지정된 CDK 구성을 생성합니다(기본값은 `<ProjectName>Agent`).

CDK 애플리케이션에서 이 구성을 사용할 수 있습니다:
...
<Fragment slot="terraform">
생성기는 생성기 실행 시 선택한 `name`을 기반으로 이름이 지정된 Terraform 모듈을 생성합니다(기본값은 `<ProjectName>-agent`).

Terraform 프로젝트에서 이 모듈을 사용할 수 있습니다:
...
:::note
Bedrock AgentCore Runtime에 배포할 때 AWS 계정에서 적절한 파운데이션 모델이 활성화되어 있어야 합니다. 기본적으로 Strands 에이전트는 Claude 4 Sonnet을 사용하지만 위에서 설명한 대로 커스터마이즈할 수 있습니다.

Amazon Bedrock에서 모델 접근을 활성화하려면 [여기 지침](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html)을 따르세요.
:::
...
- `uv pip install`을 사용하여 대상 플랫폼(`aarch64-manylinux2014`)에 종속성 설치
...
:::tip
도커 이미지는 태그(예: `my-scope-my-project-agent:latest`)로 빌드되며, 이는 CDK 또는 Terraform 인프라에서 참조되어 Strands 에이전트 프로젝트와 함께 `Dockerfile`을 배치할 수 있게 합니다.
:::
...
기본적으로 Strands 에이전트는 IAM 인증으로 보호됩니다. 인수 없이 배포할 수 있습니다:
...
에이전트 호출 권한은 `grantInvoke` 메서드로 부여할 수 있습니다. 예시:
...
호출 권한을 부여하려면 `module.my_project_agent.agent_core_runtime_arn` 출력을 참조하는 다음과 같은 정책을 추가해야 합니다:
...
<Infrastructure>
<Fragment slot="cdk">
JWT 인증을 구성하려면 에이전트 구성에 `authorizerConfiguration` 속성을 전달할 수 있습니다. 다음은 Cognito 사용자 풀과 클라이언트를 구성하는 예시입니다:
...
<Fragment slot="terraform">
JWT 인증을 구성하려면 에이전트 모듈의 `customJWTAuthorizer` 변수를 다음과 같이 수정할 수 있습니다:
...
에이전트는 [AWS Distro for Open Telemetry](https://aws.amazon.com/otel/)(ADOT)를 사용하여 `Dockerfile`에 자동 계측을 구성함으로써 관찰성이 자동으로 구성됩니다.

CloudWatch AWS 콘솔에서 "GenAI Observability"를 선택하여 추적을 확인할 수 있습니다. 추적이 표시되려면 [트랜잭션 검색](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Transaction-Search.html)을 활성화해야 합니다.

자세한 내용은 [AgentCore 관찰성 문서](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/observability-configure.html)를 참조하세요.