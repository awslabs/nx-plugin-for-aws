...
생성기는 Strands 에이전트의 인증을 구성하기 위한 `auth` 옵션을 제공합니다. 에이전트 생성 시 `IAM`(기본값) 또는 `Cognito` 인증 중에서 선택할 수 있습니다.
...
Bedrock AgentCore Runtime에서 에이전트 호출 권한을 부여하려면 `grantInvokeAccess` 메서드를 사용할 수 있습니다. 예시:
...
`Cognito` 인증을 선택하면 생성기가 자동으로 `UserIdentity` 컨스트럭트를 생성하고 Cognito 인증을 사용하도록 에이전트를 구성합니다.

<Infrastructure>
<Fragment slot="cdk">
생성된 컨스트럭트는 Cognito 인증을 구성하는 `identity` prop을 받습니다:
...
`UserIdentity` 컨스트럭트는 `Cognito` 인증을 선택하면 자동으로 생성됩니다.
</Fragment>
<Fragment slot="terraform">
생성된 모듈은 Cognito 인증을 위해 `user_pool_id` 및 `user_pool_client_ids` 변수를 받습니다:
