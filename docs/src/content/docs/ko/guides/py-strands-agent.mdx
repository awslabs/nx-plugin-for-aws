...
생성기는 `scripts/<your-agent-name>/chat.ts` 아래에 최소한의 대화형 채팅 CLI와 해당하는 `<your-agent-name>-chat` 타겟도 제공합니다. 이 타겟은 `<your-agent-name>-serve-local`에 의존하므로 실행하면 에이전트가 로컬에서 시작되고 대화형 터미널 채팅으로 진입합니다:

<NxCommands commands={['run your-project:agent-chat']} />

CLI는 프로토콜에 적합한 클라이언트를 사용하여 [`@clack/prompts`](https://github.com/bombshell-dev/clack)를 통해 토큰 단위로 응답을 스트리밍하는 TypeScript 스크립트(`tsx`를 통해 실행)입니다:

- **HTTP**: JSONL 스트리밍을 사용하는 `POST /invocations`
- **A2A**: `sendMessageStream`을 사용하는 `@a2a-js/sdk` `ClientFactory`
