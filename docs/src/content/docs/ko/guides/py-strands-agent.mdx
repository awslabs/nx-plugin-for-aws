...
`protocol`이 `A2A`로 설정되면 진입점은 FastAPI 대신 [Strands A2A Server](https://strandsagents.com/docs/user-guide/concepts/multi-agent/agent-to-agent)를 사용합니다:
    ...
    - app
      - agents
        - \<project-name>
          ...
          - Dockerfile CDK 컨스트럭트에서 사용하는 패스스루 도커 파일
</FileTree>
</Fragment>
...
에이전트의 서버 프로토콜은 통신 방식을 결정합니다. 두 옵션 모두 [FastAPI](https://fastapi.tiangolo.com/)에서 제공되며 진입점이 다릅니다:
...
두 프로토콜 모두 AgentCore 런타임 헬스 체크 계약을 위해 `/ping`을 노출합니다. A2A 에이전트는 포트 `9000`(HTTP의 경우 `8080`)에서 수신하며, 생성된 Dockerfile 및 인프라가 자동으로 구성됩니다.
...
`protocol=A2A`인 경우 생성된 `main.py`는 `/ping`도 노출하는 상위 FastAPI 앱에 `A2AServer.to_fastapi_app()`을 마운트합니다. AgentCore에 배포되면 진입점은 AppConfig에서 런타임의 공개 ARN을 확인하고 에이전트 카드에 광고합니다.
...
Strands SDK의 `A2AAgent`는 원격 A2A 엔드포인트를 래핑하여 로컬 `Agent`처럼 호출할 수 있게 합니다.
...
#### IAM 인증을 사용한 AgentCore 호출

AgentCore에 배포된 A2A 에이전트에 대한 요청(에이전트 카드 가져오기 포함)은 SigV4 서명이 필요합니다. SigV4 인증 플로우를 사용하는 `httpx.AsyncClient`와 함께 `client_config`를 `A2AAgent`에 전달합니다:
...
:::note[IAM 권한]
호출자의 IAM 역할에는 대상 런타임 ARN에 대해 `bedrock-agentcore:InvokeAgentRuntime` 및 `bedrock-agentcore:GetAgentCard` 권한이 모두 필요합니다. 생성된 컨스트럭트의 `grantInvokeAccess` 헬퍼는 두 권한을 모두 부여합니다.
:::

:::note[추가 종속성]
A2A 생성기는 `strands-agents[a2a]`를 자동으로 추가합니다. HTTP 에이전트에 A2A 클라이언트 코드를 추가하는 경우 `pyproject.toml`에 `strands-agents[a2a]` 및 `httpx`를 추가하세요.
:::

#### A2A 에이전트를 도구로 사용

`A2AAgent`는 로컬 `Agent`처럼 `AgentBase`를 상속하므로 `@tool`로 래핑하여 다른 에이전트에 전달할 수 있습니다:

```python
from strands import Agent, tool

remote = # 위와 같이 client_config로 빌드된 A2AAgent

async def ask_remote_agent(prompt: str) -> str:
    """원격 A2A 에이전트에 질문을 위임하고 응답을 반환합니다."""
    result = await remote.invoke_async(prompt)
    return str(result)

ask_remote = tool(
    name="ask_remote_agent",
    description="원격 A2A 에이전트에 질문을 위임하고 응답을 반환합니다.",
)(ask_remote_agent)

agent = Agent(
    system_prompt="적절한 경우 ask_remote_agent에 위임합니다.",
    tools=[ask_remote],
)
```

CDK 스택에서 호스트 에이전트에 원격 에이전트에 대한 호출 액세스를 부여합니다 — 이는 A2A 클라이언트에 필요한 두 IAM 작업을 모두 연결합니다:
