...
`protocol`이 `AG-UI`로 설정되면 진입점은 [ag-ui-strands](https://docs.ag-ui.com/) 통합을 사용하며, [CopilotKit](https://docs.copilotkit.ai/aws-strands)과의 직접적인 프론트엔드 통합을 위해 AG-UI 프로토콜을 통해 에이전트를 노출합니다:
...
`protocol=AG-UI`인 경우 생성된 `main.py`는 Strands `Agent`를 `ag_ui_strands.StrandsAgent`로 래핑하고 `create_strands_app()`을 통해 FastAPI 앱을 생성합니다. 결과 앱은 Server-Sent Events(SSE)를 통해 [AG-UI](https://docs.ag-ui.com/) 이벤트를 스트리밍하는 단일 POST 엔드포인트와 AgentCore 런타임 헬스 체크를 위한 `/ping`을 노출합니다.
...
생성기는 터미널에서 에이전트와 대화형 채팅을 시작하는 `<your-agent-name>-invoke`라는 타겟을 구성합니다. TypeScript 스크립트(`tsx`를 통해)로 제공되며 에이전트에 적합한 프로토콜(HTTP, A2A, AG-UI)을 자동으로 선택합니다. 기본적으로 로컬 서버에 연결됩니다:

<NxCommands commands={['agent-invoke your-project']} />

대신 배포된 에이전트와 채팅하려면 `RUNTIME_CONFIG_APP_ID`를 워크스페이스의 AppConfig 애플리케이션 ID로 설정하세요 — CLI가 런타임 ARN을 확인하고 SigV4 서명된 요청으로 배포된 엔드포인트를 호출합니다:
...
CLI 스크립트는 프로젝트의 `scripts/<agent>/cli.ts`에 있습니다.
