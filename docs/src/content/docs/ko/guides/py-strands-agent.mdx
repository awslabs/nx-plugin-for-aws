...
<Infrastructure>
<Fragment slot="cdk">
에이전트를 위한 CDK 컨스트럭트가 생성되며, 생성기 실행 시 선택한 `name`을 기반으로 이름이 지정됩니다(기본값은 `<ProjectName>Agent`).
...
:::note
이 컨스트럭트는 [`@aws-cdk/aws-bedrock-agentcore-alpha` 모듈](https://docs.aws.amazon.com/cdk/api/v2/docs/aws-bedrock-agentcore-alpha-readme.html)을 사용합니다.
:::
</Fragment>
<Fragment slot="terraform">
생성기 실행 시 선택한 `name`을 기반으로 이름이 지정된 테라폼 모듈을 생성합니다(기본값은 `<ProjectName>-agent`).
