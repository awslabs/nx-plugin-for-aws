...
구조화된 데이터를 yield하도록 Pydantic 모델을 정의할 수 있습니다. 이를 위해서는 `handle_invoke`가 yield하는 pydantic 모델을 직렬화해야 합니다:
...
:::caution
여기서 제공된 서버(및 위의 `serialize_stream` 메서드)는 현재 Server-Sent Events(SSE) 표준을 준수하지 않습니다. 따라서 원하는 경우 프론트엔드에서 생성된 TypeScript 클라이언트로 에이전트를 호출할 수 있지만, 현재는 SSE 형식의 스트리밍 이벤트를 예상하지 않습니다. 이것이 영향을 미치는 경우 [이 GitHub 이슈](https://github.com/awslabs/nx-plugin-for-aws/issues/369)에 +1을 눌러 우선순위 지정에 도움을 주세요.
:::
