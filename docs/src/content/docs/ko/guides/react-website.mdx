...
:::note[shadcn/ui]
`uxProvider`로 [shadcn/ui](https://ui.shadcn.com/)를 선택하셨습니다. 생성된 컴포넌트와 스타일은 TanStack Router / Vite와 함께 제공됩니다 — 추가 컴포넌트를 추가하려면 Shadcn 문서를 참조하세요.
:::
</OptionFilter>

<OptionFilter when={{ uxProvider: 'None' }} description="No UX provider — bring your own component library">
:::note[UX Provider 없음]
`uxProvider`로 `None`을 선택하셨으므로 컴포넌트 라이브러리나 디자인 시스템이 연결되어 있지 않습니다. 원하는 컴포넌트 라이브러리를 자유롭게 추가할 수 있습니다.
:::
</OptionFilter>
