...
`serve-local` 타겟은 [Vite `MODE`](https://vite.dev/guide/env-and-mode)를 `serve-local`로 설정한 로컬 개발 서버를 시작하며, <Link path="/guides/connection">Connection 생성기</Link>로 연결된 API의 로컬 서버도 함께 실행합니다.
...
:::note[`dev` 스크립트]
이 생성기 실행 시 루트 `package.json`에 `dev` 스크립트가 없으면, 생성된 웹사이트의 `serve-local`을 호출하는 `dev` 스크립트가 추가됩니다. 즉, 다음 명령어로도 로컬 개발 서버를 시작할 수 있습니다:

<PackageManagerShortCommand commands={["dev"]} />

이렇게 하면 처음 생성한 React 웹사이트가 워크스페이스의 기본 `dev` 타겟이 됩니다. 이후 생성되는 웹사이트는 기존 `dev` 스크립트를 덮어쓰지 않으므로, 해당 웹사이트의 `serve-local` 타겟을 직접 호출하거나 스크립트를 수동으로 업데이트할 수 있습니다.
:::
