...
:::warning
이 모드 실행 시 `runtime-config.json`이 없고 Cognito 인증(<Link path="/guides/react-website-auth">CloudScape 웹사이트 인증 생성기</Link> 통해 구성된 경우)이 구성된 경우, 로그인이 생략되며 로컬 서버 요청에 인증 헤더가 포함되지 않습니다.
