...
<FileTree>
  ...
  - src
    ...
    - hooks
      - useAppLayout.tsx 중첩 컴포넌트에서 AppLayout 조정을 위한 훅 (Cloudscape 전용)
    - routes
...
<Infrastructure>
<Fragment slot="cdk">
`packages/common/constructs/src/app/static-websites`의 생성된 웹사이트 construct에서 `certificate` 및 `domainNames` props를 전달하세요:
...
<Fragment slot="terraform">
`packages/common/terraform/src/app/static-websites`의 생성된 웹사이트 모듈에서 `custom_domain_names` 및 `acm_certificate_arn` 변수를 설정하세요:
...
또한 CloudFront 배포를 가리키는 DNS 레코드(예: Route 53)를 생성해야 합니다.

## React 웹사이트 구현

[React 문서](https://react.dev/learn)는 React 개발 기초를 배우기에 좋은 시작점입니다.

<OptionFilter when={{ ux: 'cloudscape' }} description="Cloudscape component docs pointer">
[Cloudscape 문서](https://cloudscape.design/components/)에서 사용 가능한 컴포넌트와 사용 방법을 확인할 수 있습니다.
</OptionFilter>

<OptionFilter when={{ ux: 'shadcn' }} description="Shadcn component docs pointer">
[shadcn/ui 문서](https://ui.shadcn.com/docs)에서 사용 가능한 컴포넌트와 사용 방법을 확인할 수 있습니다.
</OptionFilter>
...
웹사이트는 기본적으로 [TanStack Router](https://tanstack.com/router/v1)가 구성되어 있습니다. 새로운 라우트 추가가 간편합니다:
...
인프라의 구성 정보는 <Link href="guides/runtime-config">런타임 구성</Link>을 통해 웹사이트에 제공됩니다. 이를 통해 배포 시점에만 알 수 있는 API URL 등의 정보에 접근할 수 있습니다.

### 인프라

<Infrastructure>
<Fragment slot="cdk">
`RuntimeConfig` CDK construct를 사용하여 CDK 인프라에 구성 값을 추가하고 조회할 수 있습니다. `@aws/nx-plugin` 생성기(<Link path="guides/trpc">`ts#api`</Link>, <Link path="guides/fastapi">`py#api`</Link> 등)로 생성된 CDK construct는 자동으로 `RuntimeConfig`에 적절한 값을 추가합니다.

웹사이트 CDK construct는 런타임 구성의 `connection` 네임스페이스를 S3 버킷 루트에 `runtime-config.json` 파일로 배포합니다.
...
export class ApplicationStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    // 웹사이트는 어느 시점에나 선언 가능 — 런타임 구성은 지연 평가됨
...
:::note[CDK 웹사이트 컨스트럭트]
CDK에서는 웹사이트 construct를 스택의 어느 시점에나 선언할 수 있습니다. 런타임 구성은 synth 시점에 지연 평가되므로 선언 순서와 관계없이 모든 값이 포함됩니다.
:::
</Fragment>
<Fragment slot="terraform">
...
웹사이트 Terraform 모듈은 런타임 구성의 `connection` 네임스페이스를 S3 버킷 루트에 `runtime-config.json` 파일로 배포합니다.
...
:::note[런타임 구성]
런타임 구성이 AWS AppConfig에 어떻게 저장되고 서버 측 소비자(Lambda 함수, 에이전트)가 어떻게 조회할 수 있는지에 대한 자세한 내용은 <Link path="guides/runtime-config">런타임 구성</Link> 가이드를 참조하세요.
:::
...
로컬 개발을 위한 표준 명령어는 `dev`로, 웹사이트와 연결된 API의 로컬 서버를 하나의 명령어로 시작합니다:

<NxCommands commands={['dev <my-website>']} />

`serve` 타겟은 애플리케이션의 어느 부분을 로컬에서 실행하고 어느 부분을 배포된 AWS 인프라를 가리킬지 제어해야 할 때 사용할 수 있습니다. 여러 컴포넌트가 있는 프로젝트에서 `dev`가 어떻게 동작하는지를 포함하여 연결된 프로젝트 간 로컬 개발에 대한 전반적인 개요는 <Link path="guides/local-development">로컬 개발</Link> 가이드를 참조하세요.
...
`dev` 타겟은 [Vite `MODE`](https://vite.dev/guide/env-and-mode)를 `local-dev`로 설정한 로컬 개발 서버를 시작하며, <Link path="/guides/connection">Connection 생성기</Link>로 연결된 API의 로컬 서버도 함께 실행합니다.
...
이 타겟은 웹사이트와 API를 동시에 작업하며 인프라 배포 없이 빠른 반복을 원할 때 유용합니다.
...
이 명령은 웹사이트(및 `dev` 타겟이 있는 다른 프로젝트)의 로컬 개발 서버를 한 번에 시작합니다. 이 웹사이트만 실행하려면 해당 웹사이트의 `dev` 타겟을 사용하세요(예: `nx dev <my-website>`).
:::

:::warning[모의 인증]
이 모드에서 실행 시 `runtime-config.json`이 없고 <Link path="/guides/react-website-auth">`ts#website#auth` 생성기</Link>로 Cognito 인증을 구성한 경우, 로그인이 생략되며 로컬 서버 요청에 인증 헤더가 포함되지 않습니다.

`dev`에서 로그인 및 인증을 사용하려면 인프라를 배포하고 런타임 구성을 로드해야 합니다.
:::

:::tip[Dev 동작]
`dev` 실행 시 API가 다른 배포된 AWS 리소스를 가리키도록 필요한 환경 변수를 지정할 수 있습니다. 예를 들어:
...
`build` 타겟을 사용해 웹사이트를 빌드할 수 있습니다. 이는 `bundle`, `compile`, `test`, `lint` 타겟을 실행하여 타입 검사, 번들링, 테스트, 린팅을 수행합니다.

<NxCommands commands={['build <my-website>']} />

`bundle` 타겟은 Vite를 사용해 루트 `dist/packages/<my-website>/bundle` 디렉토리에 프로덕션 번들을 생성합니다. 이는 웹사이트 인프라에서 사용하는 배포 가능한 아티팩트입니다. 단독으로 실행할 수 있습니다:
...
React 웹사이트 생성기는 선택한 `iac` 기반으로 CDK 또는 Terraform 인프라 코드를 생성합니다. 이를 통해 웹사이트를 배포할 수 있습니다.
...
<Fragment slot="terraform">
웹사이트 배포를 위해 Terraform 프로젝트 생성 시 <Link path="/guides/terraform-project">`terraform#project` 생성기</Link> 사용을 권장합니다.
