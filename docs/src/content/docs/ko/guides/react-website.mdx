...
<Fragment slot="terraform">
Terraform의 경우 런타임 구성은 runtime-config 모듈을 통해 관리됩니다. `@aws/nx-plugin` 생성기(<Link path="guides/trpc">`ts#api`</Link>, <Link path="guides/fastapi">`py#api`</Link> 등)로 생성된 Terraform 모듈은 런타임 구성에 적절한 값을 자동으로 추가합니다.
