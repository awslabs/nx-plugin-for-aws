...
export class ApplicationStack extends Stack {
  constructor(scope: Construct, id: string) {
    ...
    // 웹사이트는 어느 시점에든 선언 가능 — 런타임 구성은 지연 평가됨
...
:::note
CDK를 사용하면 웹사이트 construct를 스택의 어느 시점에든 선언할 수 있습니다. 런타임 구성은 synth 시점에 지연 평가되므로 선언 순서와 관계없이 모든 값이 포함됩니다.
:::
</Fragment>
<Fragment slot="terraform">
...
:::note
런타임 구성이 AWS AppConfig에 저장되는 방식과 서버 측 소비자(Lambda 함수, 에이전트)가 이를 조회하는 방법에 대한 자세한 내용은 <Link path="guides/runtime-config">런타임 구성</Link> 가이드를 참조하세요.
:::
