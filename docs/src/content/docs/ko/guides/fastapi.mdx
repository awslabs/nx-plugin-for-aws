...
- 각 응답 청크의 반환 타입 선언
- <Link path="guides/api-connection/react-fastapi">API Connection</Link> 사용을 계획하는 경우 OpenAPI 벤더 확장 `x-streaming: true` 추가
...
Cognito User Pool 및 Client는 적절한 Terraform 리소스나 모듈을 사용해 설정할 수 있습니다.
:::
...
API를 공개로 유지하려는 경우 [suppression comment 추가](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html)가 가능합니다.
:::
</Fragment>
</Infrastructure>
