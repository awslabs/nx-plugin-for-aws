...
- 각 응답 청크의 반환 타입 선언
- <Link path="guides/connection/react-fastapi">Connection</Link> 생성기 사용을 계획하는 경우 OpenAPI 벤더 확장 `x-streaming: true` 추가
...
스트리밍 응답을 소비하려면 <Link path="guides/connection/react-fastapi#consuming-a-stream">Connection Generator</Link>를 사용하여 스트리밍 청크를 반복하는 타입 세이프 메서드를 제공할 수 있습니다.
