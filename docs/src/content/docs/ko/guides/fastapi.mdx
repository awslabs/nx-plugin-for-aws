...
resource "aws_iam_policy" "my_api_invoke_policy" {
  ...
  description = "스트리밍 FastAPI Lambda Function URL 호출을 허용하는 정책"
...
스트리밍 응답을 소비하려면 <Link path="guides/api-connection/react-fastapi#consuming-a-stream">API Connection Generator</Link>를 사용하여 스트리밍 청크를 반복하는 타입 세이프 메서드를 제공할 수 있습니다.
