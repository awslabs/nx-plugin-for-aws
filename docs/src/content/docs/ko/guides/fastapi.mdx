...
FastAPI 생성자는 AWS CDK 또는 Terraform 인프라 설정이 포함된 새로운 FastAPI를 생성합니다. 생성된 백엔드는 서버리스 배포를 위해 AWS Lambda를 사용하며 AWS API Gateway API를 통해 노출됩니다. [AWS Lambda Powertools](https://docs.powertools.aws.dev/lambda/python/latest/)를 설정하여 로깅, AWS X-Ray 추적, Cloudwatch 메트릭을 포함한 관측 가능성을 제공합니다.
...
def read_item(item_id: int):
    # 새로운 서브세그먼트 생성
    with tracer.provider.in_subsegment("아이템-상세-조회"):
        # 로직 구현
        return {"item_id": item_id}
```
...
:::tip
`api-connection` 생성기를 사용하는 경우 더 나은 코드 생성을 위해 API 작업에 응답 모델을 지정하는 것이 좋습니다. <Link path="guides/api-connection/react-fastapi#errors">자세한 내용 참조</Link>.
:::
...
# 예시: 정책을 역할에 연결 (필요에 따라 주석 해제 및 수정)
...
스트리밍 응답을 소비하려면 <Link path="guides/api-connection/react-fastapi#consuming-a-stream">API 연결 생성기</Link>를 사용해 스트리밍 청크를 반복 처리하는 타입 세이프 메서드를 활용할 수 있습니다.
...
적절한 Terraform 리소스나 모듈을 사용해 Cognito 사용자 풀 및 클라이언트를 설정할 수 있습니다.
:::
...
API를 공개하려는 경우 [suppression comment 추가](https://www.checkov.io/2.Basics/Suppressing%20and%20Skipping%20Policies.html)로 해결할 수 있습니다.
:::
</Fragment>
</Infrastructure>
...
`common/constructs`의 `project.json`에 `generate:<ApiName>-metadata` 대상이 추가되어 `packages/common/constructs/src/generated/my-api/metadata.gen.ts`와 같은 파일을 생성합니다. 빌드 시 생성되므로 버전 관리에서 제외됩니다.
