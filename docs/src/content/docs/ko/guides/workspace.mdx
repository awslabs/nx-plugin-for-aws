...
- **`packageManager.catalogs`** — 제너레이터가 패키지 매니저의 카탈로그에 종속성 버전을 기록하고 `catalog:` 프로토콜로 참조할지 여부([단일 버전 정책](#단일-버전-정책) 참조). `false`로 설정하면 제너레이터가 각 프로젝트의 `package.json`에 직접 버전 범위를 작성합니다. 카탈로그가 없는 npm에는 영향을 미치지 않습니다.
