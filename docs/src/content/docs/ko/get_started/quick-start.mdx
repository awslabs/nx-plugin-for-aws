...
:::tip[IaC 제공자 선택]
선호하는 코드형 인프라(IaC) 제공자로 [CDK](https://docs.aws.amazon.com/cdk/) 또는 [Terraform](https://developer.hashicorp.com/terraform) 중 하나를 선택하라는 메시지가 표시됩니다. `--iac` 옵션과 함께 명령을 실행하여 프롬프트를 건너뛸 수 있습니다:
...
이 빠른 시작 가이드에서는 tRPC API, React 웹사이트, Cognito 인증, 그리고 CDK 또는 Terraform 인프라를 추가합니다. 구축하는 프로젝트 유형에 따라 제너레이터의 조합을 선택하여 프로젝트를 빠르게 부트스트랩할 수 있습니다. 왼쪽 탐색 바의 __Generators__를 확인하여 전체 옵션 목록을 보거나, <Link path="get_started/graph-builder">그래프 빌더</Link>를 사용하여 워크스페이스를 시각적으로 구성해 보세요.
...
:::tip[AI로 빌드하기]
<Link path="get_started/building-with-ai">Nx Plugin for AWS MCP Server</Link>가 이미 워크스페이스에 구성되어 있으므로, CLI 명령을 직접 입력하는 대신 Kiro나 Claude와 같은 좋아하는 AI 코딩 에이전트에게 이러한 프로젝트를 빌드하도록 요청할 수 있습니다.
:::
...
웹사이트는 Amazon Cognito로 보호되며, 자체 가입은 기본적으로 비활성화되어 있으므로 배포한 사용자 풀에 첫 번째 사용자를 생성합니다. 배포 출력 또는 [Amazon Cognito 콘솔](https://console.aws.amazon.com/cognito/)에서 사용자 풀 ID를 찾을 수 있습니다:
...
`--message-action SUPPRESS`를 제거하면 사용자에게 직접 공유하는 대신 Cognito가 제공한 주소로 임시 비밀번호를 이메일로 보냅니다. Cognito 콘솔의 사용자 풀 **Users** 탭에서도 사용자를 생성할 수 있습니다.

이 사용자로 처음 로그인하면 새 비밀번호를 선택하라는 메시지가 표시된 다음, SMS로 전송된 코드 또는 인증 앱의 시간 기반 일회용 비밀번호를 사용하여 다단계 인증을 설정하라는 메시지가 표시됩니다.

:::tip[자체 가입]
각 사용자를 직접 생성하는 대신 사용자가 로그인 페이지에서 직접 등록하도록 하려면 `--allowSignup=true` 옵션과 함께 <Link path="guides/react-website-auth">`ts#website#auth`</Link> 제너레이터를 실행하세요.
:::

## 6단계: 배포된 클라우드 리소스로 웹사이트 테스트
...
웹사이트는 `http://localhost:4200`에서 사용할 수 있으며, API 및 인증을 위해 배포한 리소스를 가리킵니다. 생성한 사용자로 로그인하여 실제로 작동하는 것을 확인하세요.

웹사이트는 CloudFront에서도 제공됩니다. 배포 도메인 이름을 열어 AWS에서 완전히 실행되는 버전을 사용하세요.
