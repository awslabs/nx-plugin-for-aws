...
:::note[보존된 리소스]
Cognito 사용자 풀은 삭제 보호가 활성화된 상태로 생성되므로 자동으로 제거되지 않습니다. 더 이상 필요하지 않으면 사용자 풀과 Cognito가 SMS 메시지를 보내는 데 사용하는 IAM 역할을 수동으로 삭제하세요.
:::

### 부트스트랩 리소스 제거

부트스트랩한 리소스는 해당 AWS 계정 및 리전에 배포하는 모든 프로젝트에서 공유되며, 그대로 두어도 비용이 거의 들지 않습니다. 다른 것이 배포되지 않는 경우에만 제거하세요.

<Infrastructure>
<Fragment slot="cdk">
`us-east-1`을 포함하여 부트스트랩한 모든 리전에서 부트스트랩 스택을 삭제합니다. 이렇게 하면 부트스트랩 역할과 컨테이너 이미지 리포지토리가 제거됩니다:

```sh
aws cloudformation delete-stack --stack-name CDKToolkit --region <region>
```

자산 버킷은 의도적으로 보존되므로 별도로 삭제하세요. 버전이 지정되어 있어 `aws s3 rb --force`로 처리할 수 없으므로 먼저 모든 버전과 삭제 마커를 제거하세요:
...
각 `delete-objects`는 제거할 해당 종류의 항목이 남아 있지 않을 때 아무것도 하지 않는 대신 오류를 보고하는데, 이는 무시해도 안전합니다.
</Fragment>
<Fragment slot="terraform">
<NxCommands commands={['bootstrap-destroy infra']} />

:::caution[Terraform 상태도 삭제됩니다]
이 명령은 Terraform 상태를 보관하는 S3 버킷을 비우고 삭제하므로, 해당 상태에서 배포된 중요한 것이 더 이상 없을 때만 실행하세요.
:::
</Fragment>
</Infrastructure>
...
- <Link path="get_started/existing-project">Add to an Existing Project</Link> — 기존 코드베이스에 플러그인을 도입합니다.
