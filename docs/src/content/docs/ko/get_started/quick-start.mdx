...
:::tip[풀스택 튜토리얼]
풀스택 애플리케이션 구축을 위한 더 심화된 튜토리얼은 <Link path="get_started/tutorials/dungeon-game/overview">Dungeon Adventure Tutorial</Link>을 확인하세요.
:::
...
:::tip[IaC 제공자 선택]
워크스페이스를 생성하면 선호하는 코드형 인프라(IaC) 제공자로 [CDK](https://docs.aws.amazon.com/cdk/) 또는 [Terraform](https://developer.hashicorp.com/terraform) 중 하나를 선택하라는 메시지가 표시됩니다. `--iac`를 전달하여 프롬프트를 건너뛸 수 있습니다:
...
컨테이너 이미지를 빌드하는 제너레이터에서 사용하는 컨테이너 엔진을 선택하라는 메시지도 표시됩니다. 기본값(`infer`)을 수락하면 Docker가 설치되어 있으면 Docker를 선택하고, 그렇지 않으면 Finch를 선택합니다.
:::

### AI 사용

<Steps>
1. 원하는 패키지 매니저로 <Link path="guides/workspace">Nx 워크스페이스</Link>를 생성하고, 생성된 디렉터리를 엽니다:
...
3. 애플리케이션을 요청합니다:
    ...
    어시스턴트는 MCP 서버를 통해 제너레이터를 발견하고 종속성이 필요한 순서대로 실행합니다. 변경 사항을 검토한 다음 2단계로 계속 진행하세요.
</Steps>

:::tip[어시스턴트용 플러그인 또는 파워 설치]
Claude Plugin과 Kiro Power는 MCP 서버를 스티어링 문서 및 워크플로 가이드와 함께 번들로 제공하여 MCP 서버 단독보다 더 나은 결과를 얻을 수 있습니다. <Link path="get_started/building-with-ai">Building with AI</Link>를 참조하여 설치하거나 워크스페이스별이 아닌 전역으로 MCP 서버를 구성하세요.
:::
...
2. 애플리케이션을 스캐폴딩하는 제너레이터를 실행합니다. 아래 버튼을 사용하여 모두 복사하거나, 다이어그램 아래에 설명된 대로 각각을 직접 실행하세요.
...
:::note[보존된 리소스]
Cognito 사용자 풀은 삭제 보호가 활성화된 상태로 생성되므로 자동으로 제거되지 않습니다. 더 이상 필요하지 않은 경우 사용자 풀(및 Cognito가 SMS 메시지를 보내는 데 사용하는 IAM 역할)을 수동으로 삭제하세요.
:::
