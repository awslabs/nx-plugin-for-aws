...
:::note[보존된 리소스]
Cognito 사용자 풀은 삭제 보호가 활성화된 상태로 생성되므로 자동으로 제거되지 않습니다(destroy 명령이 실패를 보고합니다). 더 이상 필요하지 않으면 사용자 풀과 Cognito가 SMS 메시지를 보내는 데 사용하는 IAM 역할을 수동으로 삭제하세요.
:::
</Fragment>
</Infrastructure>
...
이 명령은 Terraform 상태를 보관하는 S3 버킷을 비우고 삭제하므로, 배포된 모든 인프라를 해체한 후에만 실행하세요.
:::
</Fragment>
</Infrastructure>
