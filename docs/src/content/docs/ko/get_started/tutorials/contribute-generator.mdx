...
새로운 워크스페이스에서 제너레이터를 실행하고 모든 것이 빌드되는지 확인하는 "smoke test" 모음이 있습니다. 최소한 새 제너레이터를 `packages/nx-plugin/src/internal/test-matrix/generator.ts`의 제너레이터 매트릭스에 추가하여 smoke test에서 실행되도록 해야 합니다.

매트릭스 자체는 다른 모든 제너레이터를 구성하는 (숨겨진) 제너레이터이므로 플러그인과 함께 제공됩니다. 즉, 각 릴리스 버전은 _해당_ 버전이 가지고 있던 제너레이터의 매트릭스를 포함하므로, 이전 워크스페이스를 업그레이드하는 테스트가 해당 버전의 자체 매트릭스로 스캐폴드할 수 있습니다. CLI를 통하지 않고 제너레이터를 직접 호출하기 때문에 옵션은 각 제너레이터의 스키마에 대해 타입 검사되므로, 새로운 필수 옵션이나 변경된 enum은 적용 범위를 조용히 줄이는 대신 여기서 빌드를 실패시킵니다.
