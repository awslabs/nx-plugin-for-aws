...
새로운 워크스페이스에서 제너레이터를 실행하고 모든 것이 빌드되는지 확인하는 "smoke test" 모음이 있습니다. 최소한 새 제너레이터를 `e2e/src/smoke-tests/generator-matrix.ts`의 제너레이터 매트릭스에 추가하여 smoke test에서 실행되도록 해야 합니다.
