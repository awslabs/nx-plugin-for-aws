...
:::tip[파일 포맷팅]
제너레이터 끝에 `formatFilesInSubtree`를 호출하여 생성하거나 수정한 파일들이 워크스페이스의 [Biome](https://biomejs.dev/) 구성에 따라 포맷팅되도록 합니다.
:::
...
[GritQL](https://docs.grit.io/)을 사용하여 선언적으로 소스 코드를 검색하고 변환합니다. `addDestructuredImport` 헬퍼는 명명된 임포트를 추가하고, `applyGritQL`은 GritQL 패턴을 적용하여 라우터의 객체 리터럴에 프로시저를 추가합니다.
...
:::tip[GritQL 패턴]
위 코드 스니펫에서 `applyGritQL`은 [GritQL](https://docs.grit.io/) 패턴을 사용하여 `router({ ... })` 호출을 찾고 새 프로시저를 객체 리터럴에 추가합니다. `where` 절은 멱등성을 보장합니다 — 프로시저가 이미 존재하면 변경이 이루어지지 않습니다.

[GritQL 플레이그라운드](https://docs.grit.io/playground)에서 GritQL 패턴을 시험해 볼 수 있으며, GritQL CLI를 설치하여 로컬에서 패턴을 테스트할 수도 있습니다:
...
:::note[자체 프로젝트 사용]
<Link path="get_started/tutorials/dungeon-game/overview">던전 어드벤처 튜토리얼</Link>을 완료했거나 tRPC API를 사용하는 기존 Nx 워크스페이스가 있다면 이 단계를 건너뛸 수 있습니다.
:::
...
새로운 워크스페이스에서 제너레이터를 실행하고 모든 것이 빌드되는지 확인하는 "smoke test" 모음이 있습니다. 최소한 새 제너레이터를 **두** 제너레이터 매트릭스에 모두 추가하여 smoke test에서 실행되도록 해야 합니다:

- `e2e/src/smoke-tests/generator-matrix.ts` — 사용자가 하는 것과 똑같이 CLI를 통해 각 제너레이터를 한 번에 하나씩 실행합니다.
- `packages/nx-plugin/src/internal/test-matrix/generator.ts` — 단일 트리에서 다른 모든 제너레이터를 인프로세스로 구성하는 숨겨진 제너레이터입니다.

둘 다 유지할 가치가 있습니다. CLI 매트릭스는 사용자가 실제로 거치는 경로를 테스트합니다. 구성된 매트릭스는 플러그인과 함께 제공되므로, 각 릴리스 버전은 _해당_ 버전이 가지고 있던 제너레이터의 매트릭스를 포함합니다 — 이를 통해 이전 워크스페이스를 업그레이드하는 테스트가 해당 버전의 자체 매트릭스로 스캐폴드할 수 있습니다. CLI를 통하지 않고 제너레이터를 직접 호출하기 때문에 옵션은 각 제너레이터의 스키마에 대해 타입 검사되므로, 새로운 필수 옵션이나 변경된 enum은 적용 범위를 조용히 줄이는 대신 거기서 빌드를 실패시킵니다.

제너레이터 매트릭스는 제너레이터를 실행하고 워크스페이스를 빌드하기만 하며 인프라를 인스턴스화하지는 않습니다. 인프라를 배포하는 제너레이터의 경우, 배포 e2e 테스트(`e2e/src/smoke-tests/cdk-deploy.spec.ts` 및 `terraform-deploy.spec.ts`)를 확장하여 실제로 리소스를 배포하고(`cdk deploy` / `terraform apply`를 통해), `deploy-invocations.ts`에 배포된 리소스를 호출하고 예상대로 동작하는지 확인하는 어설션을 추가하는 것을 고려하세요.
