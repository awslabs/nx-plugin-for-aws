...
이 튜토리얼에서는 AI 기반 던전 어드벤처 게임을 구축하게 됩니다. 본 튜토리얼은 `@aws/nx-plugin` 또는 관련 기술에 대한 사전 지식을 요구하지 않습니다. 여기서 학습하게 될 기술들은 `@aws/nx-plugin` 기반 애플리케이션 구축에 필수적인 핵심 요소들이며, 튜토리얼을 완벽히 이해하면 `@aws/nx-plugin`에 대한 깊은 이해와 [NX](https://nx.dev/) 사용법의 기초를 습득할 수 있습니다.

<Aside>이 튜토리얼은 실제 제작을 통해 배우는 것을 선호하며 즉각적인 결과물을 만들고 싶은 분들을 위해 설계되었습니다. 단계별 개념 학습을 선호한다면 개별 컴포넌트 가이드를 참조하세요</Aside>

본 튜토리얼을 완료한 사용자는 다음을 수행할 수 있는 지식을 얻게 됩니다:

- `@aws/nx-plugin`을 사용한 새로운 애플리케이션 생성
- NX를 활용한 코드베이스 관리/빌드
- tRPC와 FastAPI를 모두 사용한 API 구축
- 타입 안전 파일 기반 라우팅을 지원하는 Tanstack router를 이용한 새 페이지 생성
- 백엔드 API 호출을 위한 Tanstack query 활용
- 목적에 맞게 생성된 코드 수정
- CDK 인프라 생성 및 배포
...
- React/Vite 프론트엔드 웹사이트:
  - 안전한 API 호출을 위한 Amazon Cognito/Identity Pools 통합
...
<Prerequisites />
- 대상 리전에서 [이 가이드](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html)에 설명된 단계를 통해 AWS 계정이 Bedrock 내 Anthropic Claude 3.5 Sonnet v2 모델 접근 권한을 보유했는지 확인하세요.