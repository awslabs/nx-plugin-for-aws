---
title: 에이전틱 AI 던전 게임
description: "@aws/nx-plugin을 사용하여 에이전틱 AI 기반 던전 어드벤처 게임을 구축하는 방법에 대한 안내"
---
...
이 튜토리얼을 통해 `@aws/nx-plugin`을 사용하여 에이전틱 AI 기반 던전 어드벤처 게임을 구축하게 됩니다. 본 튜토리얼은 `@aws/nx-plugin` 또는 관련 기술에 대한 사전 지식을 전제로 하지 않습니다.
이 튜토리얼에서 배우게 될 기술들은 다음을 돕습니다:
- 모든 `@aws/nx-plugin` 기반 애플리케이션 구축,
- `@aws/nx-plugin`에 대한 깊은 이해 제공, 그리고
- [NX](https://nx.dev/) 프레임워크 사용법의 기초 습득.

<Aside title="실습 학습">이 튜토리얼은 실습을 통해 배우기를 선호하며 빠르게 실질적인 결과물을 만들고 싶은 분들을 위해 설계되었습니다. 단계별 개념 학습을 선호하시는 경우 왼쪽 네비게이션을 사용하여 개별 구성 요소 가이드를 참조하세요.</Aside>
...
이 튜토리얼에서는 `@aws/nx-plugin`을 사용하여 에이전틱 AI 기반 던전 어드벤처 게임을 구축하게 됩니다.
...
- 플레이어 인벤토리 관리를 위해 에이전트가 사용하는 [Amazon Bedrock AgentCore](https://aws.amazon.com/bedrock/agentcore/)에 호스팅된 TypeScript MCP 서버.
...
- 이 튜토리얼은 애플리케이션을 배포하고 Amazon Bedrock을 호출하므로 대상 AWS 계정에 구성된 [AWS Credentials](https://docs.aws.amazon.com/sdkref/latest/guide/access.html) 필요
- 로컬 DynamoDB 개발 및 AgentCore 컴포넌트 빌드를 위해 [Docker](https://www.docker.com/) (또는 [Finch](https://github.com/runfinch/finch)) 필요
