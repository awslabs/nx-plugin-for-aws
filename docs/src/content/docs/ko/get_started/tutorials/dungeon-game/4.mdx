...
:::tip
대신 Nx 명령을 실행하여 연결된 API 로컬 서버를 시작할 수 있으며, 이를 통해 tRPC API와 웹사이트를 동시에 반복 개발할 수 있습니다. <NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} env={{TABLE_NAME:"dungeon-adventure-infra-sandbox-Application-ElectroDbTableXXX-YYY"}} />
:::
