...
<Link path="get_started/tutorials/dungeon-game/1">모듈 1</Link>에서 `GameUI`를 Story Agent에 연결했습니다. 에이전트가 `--protocol=AG-UI`로 생성되었기 때문에, connection 제너레이터는 일반적인 OpenAPI 클라이언트 대신 CopilotKit `AguiProvider`와 에이전트별 훅(`useAguiStoryAgent`)을 연결했습니다. 단일 `<CopilotChat />` 컴포넌트로 채팅을 렌더링하고 `threadId`로 구동할 수 있습니다(AgentCore 세션 ID로 사용).

`packages/game-ui/src/routes/game/` 아래에 두 개의 새 파일을 생성하세요:
