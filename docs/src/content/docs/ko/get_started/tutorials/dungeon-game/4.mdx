...
:::tip[Nx 명령어 사용]
대신 Nx 명령을 실행하여 연결된 API 로컬 서버를 시작할 수 있으며, 이를 통해 tRPC API와 웹사이트를 동시에 반복 개발할 수 있습니다. <NxCommands commands={["serve-local game-ui"]} env={{RUNTIME_CONFIG_APP_ID:"xxxx"}} />

`RUNTIME_CONFIG_APP_ID` 환경 변수에는 CDK 배포 출력 값인 `RuntimeConfigApplicationId`를 사용하세요.
:::
...
<Link path="get_started/tutorials/dungeon-game/1#task-7-connect-your-components">모듈 1</Link>에서 `connection` 제너레이터를 사용하여 Game UI를 Story Agent에 연결했던 것을 기억하세요. 이를 통해 에이전트와 상호작용하기 위한 타입 안전한 OpenAPI 클라이언트와 훅 및 프로바이더가 설정되었습니다.

connection 제너레이터는 다음을 생성했습니다:
- `main.tsx`에서 앱을 감싸는 `StoryAgentProvider` 컴포넌트
- TanStack Query 통합을 위한 `useStoryAgent` 훅
- 직접 클라이언트 액세스를 위한 `useStoryAgentClient` 훅
- 에이전트의 OpenAPI 사양에서 생성된 TypeScript 타입

게임 컴포넌트에서 `useStoryAgentClient` 훅을 사용하여 에이전트로부터 스토리 응답을 스트리밍할 것입니다.
