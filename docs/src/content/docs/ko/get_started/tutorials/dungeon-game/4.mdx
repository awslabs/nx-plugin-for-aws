...
`deploy` 타겟은 기본적으로 [CloudFormation express mode](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/cloudformation-express-mode.html)를 사용하며, 이는 완전한 안정화를 기다리지 않고 구성이 적용되는 즉시 각 리소스 작업을 완료합니다. 첫 번째 배포는 완료하는 데 약 4분이 걸리며, 일부 리소스(예: CloudFront 배포)는 이후 잠시 동안 백그라운드에서 계속 프로비저닝될 수 있습니다. 이후 배포는 더 빠릅니다.
