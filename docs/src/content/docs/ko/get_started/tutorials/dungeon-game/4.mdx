...
UI 구성을 시작하기 위해 로컬 개발 서버를 배포된 샌드박스에 연결하겠습니다. 다음 명령어를 실행하세요:
...
<Aside type="caution">
_이 모듈 진행 동안 개발 서버를 계속 실행할 예정이며, 변경 사항이 자동으로 핫 리로드됩니다._
</Aside>

:::tip
<NxCommands commands={["run @dungeon-adventure/game-ui:serve-local"]} /> 명령어를 사용하면 연결된 API 로컬 서버도 함께 시작되어 tRPC API와 웹사이트를 동시에 반복 개발할 수 있습니다.
:::

### 새 '/game' 라우트 생성

`@tanstack/react-router`의 기능을 활용해 타입 안전한 새 라우트를 생성해 보겠습니다. 다음 위치에 빈 파일을 생성하세요: `packages/game-ui/src/routes/game/index.tsx`. 파일이 즉시 업데이트되는 것을 확인할 수 있습니다.
...
기본 라우트를 새 `/game` 라우트로 업데이트하겠습니다. `to` 필드를 수정할 때 타입 안전한 라우트 목록에서 선택할 수 있는 것을 확인하세요.
...
기본 구성된 레이아웃은 SaaS 스타일 비즈니스 애플리케이션에 더 적합합니다. 던전 스타일 게임에 맞게 레이아웃을 재구성하고 테마를 변경하겠습니다.
...
스토리 에이전트와 상호작용할 클라이언트 초기화 훅을 생성하겠습니다.
...
이 코드는 다음 기능을 수행합니다:
- `runtime-config.json`에서 에이전트 ARN 추출
- ARN으로 AgentCore Runtime 호출 URL 구성
- 로그인 사용자의 JWT 토큰과 임의 세션 ID로 에이전트 호출
- 스트리밍 에이전트 메시지 청크를 위한 비동기 반복자 반환

:::tip
`api-connection` 제너레이터를 사용했기 때문에 Game API 통합이 자동으로 구성됩니다. 에이전트에 유사한 경험을 원하시면 [이 GitHub 이슈](https://github.com/awslabs/nx-plugin-for-aws/issues/326)에 +1을 남겨주세요.
:::

### 게임 페이지

API를 호출할 게임 페이지를 생성하고 게임 구현을 완성하겠습니다. `packages/game-ui/src/routes/game` 경로의 파일을 업데이트하세요:
...
<Aside type="tip">
`$playerName` 구문은 `@tanstack/react-router`가 `playerName`을 [경로 매개변수](https://tanstack.com/router/v1/docs/framework/react/guide/path-params)로 처리하도록 지시합니다. 또한 `validateSearch` 메서드를 구현해 `genre` 쿼리 매개변수가 장르 열거형에 강력하게 타입 지정되도록 합니다.
</Aside>
</TabItem>
</Tabs>
...
<Drawer title="빌드 및 배포" trigger="원하는 경우 코드를 Cloudfront에 빌드 및 배포할 수 있습니다.">
...
배포 후 CDK 배포 출력에서 Cloudfront URL을 확인할 수 있습니다.
