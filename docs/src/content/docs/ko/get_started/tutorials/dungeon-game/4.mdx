...
`game-ui:serve-local`은 `game-api:serve-local` 및 `dungeon_adventure.story:agent-serve-local`에 대한 `dependsOn`을 가지고 있으므로, Nx는 모든 프로젝트의 로컬 서버를 병렬로 시작합니다. 모듈 1에서 설정한 연결을 통해 이들은 차례로 Inventory MCP 서버와 DynamoDB Local을 부팅합니다. 컨테이너 엔진이 실행 중인지 확인한 다음 브라우저에서 개발 서버를 여세요.
...
첫 번째 배포는 완료하는 데 약 8분이 걸립니다. 이후 배포는 더 빠릅니다.
...
핵심은 `RUNTIME_CONFIG_APP_ID` 환경 변수입니다. 프로젝트가 `SERVE_LOCAL=true` **없이** 실행될 때, 런타임 구성 조회는 이 애플리케이션 ID를 사용하여 AWS AppConfig에서 구성을 가져옵니다 — CDK 출력의 `RuntimeConfigApplicationId` 값입니다.
...
축하합니다. 에이전트 기반 던전 어드벤처 게임을 성공적으로 구축, 테스트 및 배포하셨습니다! 🎉🎉🎉 구축, 테스트 및 배포하셨습니다! 🎉🎉🎉�하셨습니다! 🎉🎉🎉