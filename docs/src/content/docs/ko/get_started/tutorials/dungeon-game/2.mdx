...
:::note[채팅 기록 프로시저 불필요]
채팅의 각 사용자/어시스턴트 턴을 저장하는 프로시저가 필요할 것으로 예상했을 수 있습니다. 하지만 필요하지 않습니다: AG-UI 에이전트는 Bedrock AgentCore Runtime에서 호스팅되며, 동일한 스레드 ID가 사용되는 한 세션당 대화 기록을 자동으로 유지합니다.
:::
