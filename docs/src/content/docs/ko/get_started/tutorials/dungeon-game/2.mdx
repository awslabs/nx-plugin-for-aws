...
:::note[대화 기록은 에이전트에 저장됩니다]
Game API를 통해 채팅 메시지를 저장하지 않습니다. <Link path="get_started/tutorials/dungeon-game/3">모듈 3</Link>에서 Story Agent는 `FileSessionManager`를 연결하여 각 스레드의 기록을 디스크에 작성하며, UI가 매 턴마다 보내는 스레드 ID를 키로 사용합니다. 따라서 각 `(플레이어, 장르)` 쌍은 UI가 메시지를 재생할 필요 없이 자체 스토리를 재개합니다.
:::
