...
먼저 Game API를 구현해 보겠습니다. 이를 위해 총 4개의 API를 생성해야 합니다:
...
API 입력과 출력을 정의하기 위해 [Zod](https://zod.dev/)를 사용하여 `packages/game-api/src/schema` 디렉토리 내에 다음과 같이 스키마를 생성합니다:
...
이 프로젝트에서 사용하지 않을 `packages/game-api/src/schema/echo.ts` 파일은 삭제해도 됩니다.
...
<Aside>
모든 종속성은 [단일 버전 정책](https://nx.dev/concepts/decisions/dependency-management#single-version-policy) 원칙을 따르는 `@aws/nx-plugin`에 따라 루트 `package.json`에 추가됩니다. 자세한 내용은 <Link path="guides/typescript-project#dependencies">ts#프로젝트 가이드</Link>를 참조하세요.
</Aside>
...
<Aside>
ElectroDB는 [컬렉션](https://electrodb.dev/en/modeling/collections/)을 지원하지만, 본 튜토리얼에서는 간결함을 위해 사용하지 않습니다.
</Aside>

### tRPC 컨텍스트에 DynamoDB 클라이언트 추가

각 프로시저에서 DynamoDB 클라이언트에 접근해야 하므로 컨텍스트를 통해 단일 클라이언트 인스턴스를 전달할 수 있도록 설정합니다. 이를 위해 `packages/game-api/src` 내에서 다음 변경 사항을 적용합니다:
...
마지막 단계로 DynamoDB 테이블을 생성하고 Game API에서 작업 수행 권한을 부여하기 위해 인프라를 업데이트합니다. `packages/infra/src`를 다음과 같이 수정합니다:
...
:::tip
람다 함수 코드 변경 사항을 반복적으로 수정하는 경우, 코드베이스 빌드 후 `--hotswap` 플래그를 사용하여 배포하면 훨씬 짧은(2-3초) 배포 시간이 소요됩니다.

<NxCommands commands={['run @dungeon-adventure/infra:deploy dungeon-adventure-infra-sandbox/* --hotswap']} />
:::

배포가 완료되면 다음과 유사한 출력을 확인할 수 있습니다 _(일부 값은 생략됨)_:
...
<ul>
<li>tRPC 백엔드의 로컬 인스턴스를 시작하고 `curl`로 API를 호출합니다.</li>
<li>
<Drawer title="Sigv4 활성화 curl" trigger="배포된 API를 sigv4 활성화 curl로 호출하기">

<Tabs>
  <TabItem label="Bash/Linux/macOS">
`.bashrc` 파일에 다음 스크립트를 추가하거나(그리고 `source` 실행) 동일한 터미널에 직접 붙여넣을 수 있습니다.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
다음 예제와 같이 `acurl`을 호출하여 sigv4 인증 curl 요청을 수행할 수 있습니다:
...
```
    <Aside type="caution">
    CDK 배포 출력 값 `dungeon-adventure-infra-sandbox-Application.GameApiGameApiEndpointXXX`을 사용하여 하이라이트된 플레이스홀더를 대체하고 지역을 적절히 설정하세요.
    </Aside>
  </TabItem>
</Tabs>
