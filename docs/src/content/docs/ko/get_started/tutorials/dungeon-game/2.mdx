...
`@aws-sdk/client-s3` 의존성은 이미 워크스페이스에 있으므로 새로 설치할 것이 없습니다. 버킷 이름은 `tables`에 사용된 것과 동일한 런타임 구성 메커니즘에서 가져오며, 아래 인프라 스택에서 새로운 `buckets` 네임스페이스에 등록됩니다.
  </TabItem>
</Tabs>
...
:::tip[런타임 구성]
DynamoDB 테이블 이름은 <Link path="guides/runtime-config">런타임 구성</Link>의 `tables` 네임스페이스에 등록되어 AWS AppConfig에 저장됩니다. 람다 함수와 MCP 서버는 환경 변수로 전달받는 대신 Lambda Powertools를 사용하여 런타임에 이를 가져옵니다. 이 접근 방식의 장점은 테이블이나 다른 인프라 식별자의 수가 증가하더라도 로컬에서 실행할 때 단일 `RUNTIME_CONFIG_APP_ID` 환경 변수만 필요하다는 것입니다.
:::
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="로컬">
    ...
    CDK 배포 출력 값 `dungeon-adventure-infra-sandbox-Application.RuntimeConfigApplicationId`을 사용하여 하이라이트된 플레이스홀더를 대체하세요.
    </Aside>
