...
[Zod](https://zod.dev/)를 사용하여 API 입력/출력 스키마를 `packages/game-api/src/schema` 디렉토리에 다음과 같이 정의합니다:
...
<Aside type="tip">
위에서 볼 수 있듯이 각 Zod 스키마 정의에 `z.TypeOf` 구문을 사용하여 인터페이스를 함께 내보냅니다. 이렇게 하면 중복 작업 없이 Zod 정의를 TypeScript 인터페이스로 변환할 수 있습니다!
</Aside>
...
DynamoDB에 데이터베이스를 구현할 것이며, [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDB 클라이언트 라이브러리를 사용하여 작업을 단순화합니다. 시작하려면 다음 명령어로 `electrodb`와 DynamoDB 클라이언트를 설치해야 합니다:
...
정의한 프로시저를 API에 연결합니다. 다음 파일을 업데이트합니다:
...
이제 사용되지 않는 `packages/inventory/src/mcp-server` 내의 `tools` 및 `resources` 디렉토리를 삭제할 수 있습니다.

### 인프라구조

마지막 단계로 DynamoDB 테이블을 생성하고 Game API에서 작업 권한을 부여하기 위해 인프라를 업데이트합니다. `packages/infra/src`를 다음과 같이 수정합니다:
...
<li>
<Drawer title="Sigv4 활성화된 curl 사용" trigger="배포된 API를 Sigv4 활성화된 curl로 호출">
...
Sigv4 인증 curl 요청은 다음 예시처럼 `acurl`을 호출하여 수행할 수 있습니다:
...
PowerShell 프로필에 다음 함수를 추가하거나 현재 세션에 직접 붙여넣을 수 있습니다.
```powershell
