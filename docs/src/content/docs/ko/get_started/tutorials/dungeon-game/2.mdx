...
DynamoDB에 데이터베이스를 구현할 것이며, [ElectroDB](https://electrodb.dev/en/core-concepts/introduction/) DynamoDB 클라이언트 라이브러리를 사용하여 작업을 단순화합니다. Game API의 `queryActions`는 에이전트가 작성한 대화 턴을 읽기 위해 S3 클라이언트도 필요합니다. 다음 명령어로 모두 설치합니다:
