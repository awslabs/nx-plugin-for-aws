...
<Tabs>
  <TabItem label="Local">
  에이전트 생성기는 에이전트의 OpenAPI 스펙에서 생성된 타입 안전 TypeScript 클라이언트를 사용하는 대화형 터미널 채팅 CLI(`agent-chat`)를 제공합니다. 에이전트의 입력 형태를 `playerName`, `genre`, `actions`를 받도록 변경했으므로, 생성된 채팅 스크립트를 업데이트하여 해당 필드를 제공해야 합니다.

  `packages/story/scripts/agent/chat.ts`를 업데이트하세요:
  ...
  그런 다음 로컬에서 실행 중인 Story 에이전트와 대화형 채팅을 시작하세요:
    ...
    `agent-chat` 타겟은 자동으로 `agent-serve-local`을 시작하고(`SERVE_LOCAL=true`로 에이전트와 MCP 서버 종속성을 시작함) OpenAPI 스펙 + 클라이언트 생성을 실행한 후 채팅으로 진입합니다.

    :::note[`agent-serve` vs `agent-serve-local`]
    `agent-chat`은 `agent-serve-local`에 의존하며, 이는 `SERVE_LOCAL=true`로 에이전트를 시작하고 MCP 서버 종속성도 로컬에서 시작합니다. 에이전트가 (`RUNTIME_CONFIG_APP_ID`를 통해) **배포된** MCP 서버에 연결하도록 하려면, 별도의 터미널에서 `agent-serve` 타겟을 실행한 다음 `/invocations` 엔드포인트에 대해 `curl`로 에이전트를 수동으로 호출하세요.
    :::
  </TabItem>
