...
<Aside type="caution">
AWS 계정이 [이 가이드](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html)에 설명된 단계를 통해 대상 리전에서 Bedrock의 [Strands 기본 모델](https://strandsagents.com/latest/documentation/docs/user-guide/concepts/model-providers/amazon-bedrock/) 접근을 활성화했는지 확인하세요. 본 문서 작성 시점 기준으로는 Anthropic Claude Sonnet 4 모델입니다.
</Aside>

스토리 에이전트는 [Strands](https://strandsagents.com/) 에이전트로, `Game`과 컨텍스트용 `Action` 목록을 입력받아 스토리를 진행합니다. 인벤토리 MCP 서버와 상호작용하여 플레이어의 사용 가능한 아이템을 관리하도록 에이전트를 구성할 것입니다.
...
- 시스템 프롬프트와 MCP 서버 도구를 포함한 에이전트 생성
...
<ul>
<li>로컬 에이전트 서버 인스턴스를 시작하고 `curl`로 호출</li>
...
<Tabs>
  ...
  다음 명령어로 로컬 에이전트 서버를 시작하세요:
    ...
    에이전트 서버가 실행되면(콘솔 출력이 없음) 다음 명령어로 호출하세요:
  ...
  <TabItem label="Deployed">
    ...
    :::caution
    `POOL_ID`에는 CDK 배포 출력값 `UserIdentityUserIdentityUserPoolIdXXX`를 사용하고 해당 Client ID가 있는지 확인하세요.
    :::
...
명령이 성공적으로 실행되면 다음과 유사한 스트리밍 이벤트를 확인할 수 있습니다:
