...
<Tabs>
  <TabItem label="Local">
  다음 명령어를 실행하여 로컬 에이전트 서버를 시작하세요:
    ...
    `agent-serve` 타겟은 에이전트를 로컬에서 시작하지만 (`RUNTIME_CONFIG_APP_ID`를 통해) **배포된** MCP 서버에 연결합니다. MCP 서버도 로컬에서 실행하려면 `agent-serve-local`을 사용하세요. 이는 모든 종속 서비스를 시작하고 직접 HTTP 연결을 위해 `SERVE_LOCAL=true`를 설정합니다.
    :::

    에이전트 서버가 실행되면(출력이 표시되지 않음), 다음 명령어를 실행하여 호출하세요:
...
명령이 성공적으로 실행되면 초기 스토리 텍스트가 JSON Lines로 스트리밍되는 것을 볼 수 있습니다:
