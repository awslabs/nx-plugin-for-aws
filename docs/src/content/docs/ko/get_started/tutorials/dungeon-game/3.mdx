...
API를 생성하기 전에 몇 가지 추가 종속성을 설치해야 합니다.
...
이전에 설정한 <Link path="get_started/tutorials/dungeon-game/1#game-ui-infrastructure">인프라</Link>는 모든 API가 Lambda 함수와 통합되는 API Gateway를 가정합니다. `story_api`의 경우 스트리밍 응답을 지원하지 않는 API Gateway 대신 [응답 스트리밍이 구성된 Lambda Function URL](https://docs.aws.amazon.com/lambda/latest/dg/configuration-response-streaming.html)을 사용합니다.
...
배포 완료 후 다음과 유사한 출력이 표시됩니다 _(일부 값 생략)_:
...
`.bashrc` 파일에 다음 스크립트를 추가하거나(추가 후 `source` 실행) 명령어 실행 터미널에 직접 붙여넣으세요.
```bash
// ~/.bashrc
acurl () {
    REGION=$1
...
```
    <Aside type="caution">
    CDK 배포 출력값 `dungeon-adventure-infra-sandbox-Application.StoryApiStoryApiUrlXXX`을 사용하여 URL 플레이스홀더를 교체하고 리전을 적절히 설정하세요.
    </Aside>
  </TabItem>
</Tabs>
