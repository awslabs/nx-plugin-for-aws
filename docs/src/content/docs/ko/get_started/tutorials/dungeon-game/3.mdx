...
<Aside type="caution">
[이 가이드](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access-modify.html)에 설명된 단계를 통해 **Anthropic Claude 3.5 Sonnet v2** 모델에 대한 액세스 권한을 부여했는지 확인하세요.
</Aside>

StoryApi는 `Game`과 컨텍스트를 위한 `Action` 목록이 주어지면 스토리를 진행하는 단일 API `generate_story`로 구성됩니다. 이 API는 Python/FastAPI로 스트리밍 API로 구현되며, 생성된 코드를 목적에 맞게 수정하는 방법을 추가로 보여줍니다.
...
:::note
FastAPI를 변경할 때마다 웹사이트의 생성된 클라이언트에 반영된 변경 사항을 보려면 프로젝트를 다시 빌드해야 합니다.

다시 빌드하기 전에 아래에서 몇 가지 더 변경하겠습니다.
:::
