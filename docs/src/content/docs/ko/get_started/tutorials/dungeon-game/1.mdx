...
## 작업 2: Dungeon Adventure Game 스캐폴딩하기
...
- **단계별 방법** — 아래 섹션을 펼쳐 각 제너레이터를 직접 실행하고 각각이 생성하는 내용을 정확히 확인합니다.

아래 다이어그램은 Dungeon Adventure 워크스페이스 그 자체입니다: 이 모듈에서 구축할 모든 프로젝트, 컴포넌트 및 연결입니다. **Copy commands**를 눌러 전체 시리즈를 가져온 다음 작업 1에서 생성한 `dungeon-adventure` 디렉토리 내에서 실행하세요.
