...
- packages/
  - story/
    - dungeon\_adventure\_story/ Python 모듈
      - agent/
        - main.py AG-UI FastAPI 앱 진입점 (Bedrock AgentCore 런타임용)
...
`protocol=AG-UI`를 선택했기 때문에 생성기는 Strands 에이전트를 [AG-UI](https://ag-ui.com/) 어댑터로 래핑합니다. `create_strands_app`은 단일 `/invocations` 엔드포인트를 노출하는 FastAPI 앱을 빌드합니다. 세션별 에이전트 인스턴스는 자동으로 생성되며, 대화 기록은 서버 수명 동안 `threadId`별로 메모리에 유지됩니다. 이 튜토리얼에서는 `main.py`를 수정할 필요가 없습니다.
