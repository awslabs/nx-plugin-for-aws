...
이 튜토리얼에서는 인프라스트럭처 코드로 CDK를 사용하기 위해 `--iac=cdk`를 사용합니다. AWS용 Nx 플러그인은 `Terraform`도 지원합니다.
:::
...
`py#agent` 생성기를 사용하여 Strands 에이전트를 프로젝트에 추가하려면:
...
:::tip[로컬 우선 개발]
`ts#dynamodb` 생성기는 컨테이너에서 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)을 실행하는 `serve-local` 대상을 제공합니다. 아래에서 실행할 `connection` 생성기와 결합하면 **전체 게임이 튜토리얼 마지막에 배포하기로 선택할 때까지 AWS 호출 없이 로컬 머신에서 실행됩니다**. Docker 또는 Finch와 같은 [컨테이너 엔진](https://docs.docker.com/get-started/get-docker/)이 설치되어 실행 중이어야 합니다.
:::
...
- packages/
  - dungeon-db/
    ...
    - src/
      ...
      - gsi.ts Global Secondary Index의 소스
      - entities/
...
:::note[Shadcn UI]
생성된 웹사이트가 Tailwind로 스타일링된 [shadcn/ui](https://ui.shadcn.com/) 컴포넌트를 사용하도록 `--ux=shadcn`을 선택합니다. 모든 라우트 코드는 `Card`, `Button`, `Input`과 같은 shadcn 프리미티브를 사용하며, 나중에 `connection` 생성기가 일치하는 shadcn 테마의 [CopilotKit](https://docs.copilotkit.ai/) 채팅 인터페이스를 연결합니다.
:::
...
자세한 내용은 <Link path="guides/connection/py-agent-mcp">Python Strands 에이전트에서 MCP로의 연결 가이드</Link>를 참조하세요.
</Drawer>
...
### Game API 및 Inventory MCP 서버를 데이터베이스에 연결

Game API와 Inventory MCP 서버 모두 DynamoDB 테이블을 읽고 쓰므로 `DungeonDb` 프로젝트에 연결해 보겠습니다. `connection` 생성기는 대상이 `ts#dynamodb` 프로젝트임을 감지하고 각 소스 프로젝트의 `serve-local` 대상이 DynamoDB Local을 자동으로 시작하도록 연결합니다.
...
Story Agent의 `agent-serve-local`이 이미 Inventory MCP 서버의 `mcp-server-serve-local`에 의존하고(위의 `story → inventory` 연결을 통해), Game API와 MCP 서버 모두 이제 `dungeon-db:serve-local`에 의존하므로, 어떤 프로젝트의 `serve-local` 대상을 실행하든 DynamoDB Local을 포함하여 필요한 모든 종속성을 올바른 순서로 시작합니다. 튜토리얼의 나머지 부분에서 이를 활용할 것입니다.
</Aside>
...
<Aside type="note" title="비대화형 환경">
비대화형 셸(예: CI)에서는 프롬프트가 표시되지 않고 대신 "workspace is out of sync" 오류와 함께 빌드가 실패합니다. <NxCommands commands={['sync']} />를 실행하여 대기 중인 동기화 생성기를 적용한 다음 다시 빌드하거나, `nx.json`에서 `sync.applyChanges`를 `true`로 설정하여 모든 빌드에서 변경 사항을 자동으로 적용합니다. 프로젝트 참조가 변경될 때마다(예: 다음 모듈에서 프로시저를 추가한 후) 동일한 동기화 확인이 발생합니다.
</Aside>
