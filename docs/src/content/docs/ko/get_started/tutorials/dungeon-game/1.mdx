...
이 파일은 에이전트의 진입점입니다. `--protocol=AG-UI`를 선택했기 때문에 생성기는 Strands `Agent`를 [`ag_ui_strands`](https://docs.copilotkit.ai/aws-strands/integration)의 `StrandsAgent`로 래핑하고 [AG-UI 프로토콜](https://docs.copilotkit.ai/aws-strands/protocol)을 사용하는 FastAPI 앱에 마운트합니다. 이것이 React 웹사이트에서 CopilotKit이 통신할 대상입니다. <Link path="get_started/tutorials/dungeon-game/3">모듈 3</Link>에서는 각 스레드 ID가 자체 온디스크 `FileSessionManager`를 갖고 대화 기록이 턴 간에 유지되도록 `session_manager_provider`를 추가할 것입니다.
...
:::note[Shadcn UI]
생성된 웹사이트가 Tailwind로 스타일링된 [shadcn/ui](https://ui.shadcn.com/) 컴포넌트를 사용하도록 `--uxProvider=Shadcn`을 선택합니다. 모든 라우트 코드는 `Card`, `Button`, `Input`과 같은 shadcn 프리미티브를 사용하며, 나중에 `connection` 생성기가 일치하는 shadcn 테마의 [CopilotKit](https://docs.copilotkit.ai/) 채팅 인터페이스를 연결합니다.
:::
...
Game UI를 Story Agent에 연결해 보겠습니다. 에이전트가 AG-UI를 사용하기 때문에 `connection` 생성기는 OpenAPI 클라이언트를 생성하는 대신 [CopilotKit](https://docs.copilotkit.ai/)를 연결합니다. 테마가 적용된 채팅 컴포넌트와 렌더링 준비가 된 `@ag-ui/client` `HttpAgent`를 얻게 됩니다.
...
- `threadId`를 AgentCore의 33자 최소값으로 패딩합니다. 이것이 모듈 4 라우트가 안전하게 ``{player}-{genre}``를 안정적인 스레드 ID로 빌드할 수 있는 이유입니다.
