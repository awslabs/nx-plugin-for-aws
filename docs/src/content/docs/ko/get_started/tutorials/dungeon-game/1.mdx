...
:::tip[로컬 우선 개발]
`ts#dynamodb` 생성기는 컨테이너에서 [DynamoDB Local](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DynamoDBLocal.html)을 실행하는 `serve-local` 대상을 제공합니다. 아래에서 실행할 `connection` 생성기와 결합하면 **전체 게임이 튜토리얼 마지막에 배포하기로 선택할 때까지 AWS 호출 없이 로컬 머신에서 실행됩니다**.
:::
