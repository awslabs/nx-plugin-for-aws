...
tRPC API에 대한 새로운 프로시저를 생성하는 새로운 제너레이터를 만들어 보겠습니다.
...
:::note
제너레이터에는 `Tree`와 스키마에서 정의한 옵션이 입력으로 제공됩니다. `Tree`는 본질적으로 가상 파일 시스템으로, 프로젝트 파일을 읽고 쓸 수 있습니다. 사용자가 "드라이 런" 모드에서 제너레이터를 실행할 때 파일 시스템을 직접 수정하지 않으려고 합니다.
:::
