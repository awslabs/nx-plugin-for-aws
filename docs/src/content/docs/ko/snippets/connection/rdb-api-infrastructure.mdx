...
API Lambda 함수를 프라이빗 격리 서브넷이 아닌 **외부 연결이 가능한 프라이빗 서브넷**에 배포하세요. `RUNTIME_CONFIG_APP_ID`는 데이터베이스 모듈의 출력이 아닌 공유 <Link path="guides/runtime-config">런타임 구성</Link> AppConfig 애플리케이션을 가리켜야 합니다. 데이터베이스 모듈의 런타임 구성 항목이 배포되도록 인스턴스화할 때 `database` 네임스페이스를 포함하세요:
