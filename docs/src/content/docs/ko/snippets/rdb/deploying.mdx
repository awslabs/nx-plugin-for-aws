...
관계형 데이터베이스 생성기는 선택한 `iac`에 따라 CDK 또는 Terraform 인프라를 생성합니다.
...
마이그레이션 Lambda는 컨테이너 이미지로 배포되며 공유 자산 레지스트리에 게시됩니다. 배포당 한 번 `core/asset-ecr` 모듈을 인스턴스화하고 모든 데이터베이스 모듈에 `repository_url`을 전달하세요:
...
이는 RDS Proxy, 관리자 자격 증명, create-db-user Lambda, 런타임 구성 등록 및 마이그레이션 Lambda가 포함된 Aurora 클러스터를 프로비저닝합니다.
...
<Drawer title="VPC 구성 예시" trigger="VPC 구성 예시를 보려면 여기를 클릭하세요.">
