...
이 프로젝트 타입은 현재 더 이상 사용되지 않는 [`create-react-app`](https://github.com/facebook/create-react-app)을 활용했습니다. 이 가이드에서 웹사이트를 마이그레이션하기 위해 보다 현대적이고 지원되는 기술, 즉 [Vite](https://vite.dev/)를 사용하는 <Link path="/guides/react-website">`ts#website` 생성기</Link>를 사용할 것입니다.
