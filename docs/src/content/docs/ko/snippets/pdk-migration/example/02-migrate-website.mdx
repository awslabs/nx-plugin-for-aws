...
`@aws-northstar/ui`는 `ace-builds`에 의존하는 코드 에디터 컴포넌트를 번들로 제공하며, Vite가 해결할 수 없는 webpack 전용 import를 사용합니다. 쇼핑 리스트 애플리케이션에서는 이 컴포넌트를 사용하지 않으므로 `packages/website/vite.config.mts`의 기존 `build` 옵션 내 `external` 구성에 추가하여 번들에서 제외합니다:
