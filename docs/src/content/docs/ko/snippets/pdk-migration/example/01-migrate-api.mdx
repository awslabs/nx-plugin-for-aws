...
따라서 <Link path="/guides/ts-smithy-api">`ts#smithy-api` 생성기</Link>를 사용하여 동등한 기능을 제공할 수 있습니다.

:::note
OpenAPI나 TypeSpec을 모델링 언어로 사용했거나 Python/Java로 핸들러를 구현한 경우 [FAQ](#frequently-asked-questions)에서 가능한 옵션을 참조하세요.
:::

#### TypeScript Smithy API 생성하기

<Link path="/guides/ts-smithy-api">`ts#smithy-api` 생성기</Link>를 실행하여 `packages/api`에 API 프로젝트를 설정합니다:

<RunGenerator generator="ts#smithy-api" noInteractive requiredParameters={{ name: 'api', namespace: 'com.aws', auth: 'IAM' }} />

이렇게 하면 `model` 프로젝트와 `backend` 프로젝트가 생성됩니다. `model` 프로젝트는 Smithy 모델을 포함하며, `backend`는 서버 구현을 포함합니다.

백엔드는 [TypeScript용 Smithy 서버 생성기](https://smithy.io/2.0/languages/typescript/ts-ssdk/index.html)를 사용합니다. 이에 대해서는 아래에서 자세히 살펴보겠습니다.

#### Smithy 모델 마이그레이션

Smithy API 프로젝트의 기본 구조가 준비되었으므로 모델을 마이그레이션할 수 있습니다:

<Steps>

1. `packages/api/model/src`에 생성된 예제 Smithy 파일 삭제
1. PDK 프로젝트의 `packages/api/model/src/main/smithy` 디렉토리에서 새 프로젝트의 `packages/api/model/src` 디렉토리로 모델 복사
...
1. `main.smithy`의 서비스에 Smithy TypeScript 서버 SDK 사용 시 필수인 `ValidationException` 오류 추가:
...
1. 생성된 클라이언트에 페이지네이션 정보를 제공하는 트레이트를 정의할 `extensions.smithy` 파일을 `packages/api/model/src`에 추가:
...
1. `get-shopping-lists.smithy`의 `GetShoppingLists` 연산에 새 `@cursor` 트레이트 추가:
    ...
    Nx Plugin for AWS(<Link path="guides/api-connection/react-smithy">`api-connection`</Link> 생성기 통해)에서 제공하는 클라이언트 생성기를 사용하는 경우 `@paginated` 연산에도 `@cursor`를 사용해야 합니다.

1. 마지막으로 모든 연산에서 `@handler` 트레이트 제거. `ts#smithy-api`를 사용하면 이 트레이트가 생성하는 람다 함수 CDK 구성 및 번들링 대상을 사용하지 않으며, 모든 람다 함수에 단일 번들을 사용합니다.

</Steps>

이제 모델 변경 사항을 확인하고 생성된 서버 코드가 작동하는지 빌드를 실행해 보겠습니다. 백엔드 프로젝트(`@shopping-list/api`)에 일부 오류가 발생하지만 다음 단계에서 해결할 것입니다.
...
Type Safe API와 `ts#smithy-api` 생성기의 주요 차이점은 핸들러 구현 방식입니다. `ts#smithy-api`는 [TypeScript용 Smithy 서버 생성기](https://smithy.io/2.0/languages/typescript/ts-ssdk/index.html)를 사용하며, Type Safe API의 자체 생성된 핸들러 래퍼(`api/generated/typescript/runtime` 프로젝트)를 사용하지 않습니다.

우리의 람다 핸들러는 `@aws-sdk/client-dynamodb` 패키지에 의존하므로 먼저 설치합니다:
...
핸들러 마이그레이션은 다음 일반 단계를 따를 수 있습니다:

<Steps>

1. PDK 프로젝트의 `packages/api/handlers/typescript/src` 디렉토리에서 새 프로젝트의 `packages/api/backend/src/operations` 디렉토리로 핸들러 복사

1. `my-api-typescript-runtime` 임포트 제거 및 생성된 TypeScript 서버 SDK에서 연산 타입과 `ServiceContext` 임포트:
...
1. 입력 파라미터 참조 업데이트. SSDK는 Smithy 모델과 정확히 일치하는 타입을 제공하므로(바디 파라미터와 별도로 경로/쿼리/헤더 파라미터를 그룹화하지 않음) 입력 참조를 적절히 수정:
...
1. `Response` 사용 제거. SSDK에서는 일반 객체를 반환합니다.
    ...
    또한 `Response`를 throw하거나 반환하지 않고 SSDK의 생성된 오류를 throw:
...
<Drawer title="쇼핑 리스트 핸들러 마이그레이션" trigger="튜토리얼의 세 가지 쇼핑 리스트 연산에 대한 전체 Before/After 예제 보기">
...
/**
 * DeleteShoppingList 연산용 타입 안전 핸들러
 */
export const deleteShoppingList: DeleteShoppingListChainedHandlerFunction = async (request) => {
  LoggingInterceptor.getLogger(request).info(
 ...
 * deleteShoppingListHandler 메서드는 타입 안전 핸들러를 래핑하고 입출력 마샬링 관리
 */
export const handler = deleteShoppingListHandler(
  ...INTERCEPTORS,
  deleteShoppingList,
...
/**
 * GetShoppingLists 연산용 타입 안전 핸들러
 */
 ...
 * getShoppingListsHandler 메서드는 타입 안전 핸들러를 래핑하고 입출력 마샬링 관리
 */
 ...
 * putShoppingListHandler 메서드는 타입 안전 핸들러를 래핑하고 입출력 마샬링 관리
 */
...
Smithy API 프로젝트를 생성할 때 `packages/api`에 추가하기 위해 이름을 `api`로 지정했습니다. 이제 Smithy API가 `service Api` 대신 `service MyApi`를 정의하므로 `getApiServiceHandler` 인스턴스를 `getMyApiServiceHandler`로 업데이트해야 합니다.

`handler.ts`에 다음 변경사항 적용:
...
지금까지의 마이그레이션이 작동하는지 확인하기 위해 프로젝트 빌드:
