...
1. `get-shopping-lists.smithy`의 `GetShoppingLists` 연산에 새 `@cursor` 트레이트 추가:
    ...
    Nx Plugin for AWS(<Link path="guides/connection/react-smithy">`api-connection`</Link> 생성기 통해)의 클라이언트 생성기를 사용하는 경우 `@paginated` 연산에도 `@cursor`를 사용해야 합니다.
...
쇼핑 목록 애플리케이션의 람다 핸들러는 `@aws-sdk/client-dynamodb` 패키지를 사용하므로 `@shopping-list/api` 프로젝트에 설치합니다:
...
`ts#smithy-api` 생성기는 예제 `Echo` 연산을 스캐폴딩합니다. 모델에서 이를 제거했으므로 `backend/src/operations/echo.ts`의 해당 핸들러를 삭제합니다. 마이그레이션된 연산은 아래 `service.ts`에서 등록할 것입니다.
