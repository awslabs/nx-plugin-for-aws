...
`isolated`는 작업별로 더 세밀한 권한과 구성을 제공합니다. `shared`는 선택적 재정의를 여전히 허용하면서 Lambda 및 API Gateway 통합 확산을 줄입니다.

생성기 지원은 현재 백엔드에 따라 다릅니다:
...
- `py#fast-api`와 `ts#smithy-api`는 현재 `isolated`를 생성합니다.

기존 생성된 CDK API 구문의 `defaultIntegrations` 메서드에서 `pattern` 필드를 편집하여 두 패턴 간에 전환할 수도 있습니다:
...
<Infrastructure>
<Fragment slot="cdk">
생성된 기본값을 넘어 라우터 패턴을 사용자 정의해야 하는 경우, API의 `defaultIntegrations` 메서드를 자유롭게 편집하여 원하는 방식으로 통합을 연결할 수 있습니다:
...
export class MyApi<...> extends ... {
  ...
  public static defaultIntegrations = (scope: Construct) => {
    const router = new NodejsFunction(scope, 'Router', {
      ...
    });
