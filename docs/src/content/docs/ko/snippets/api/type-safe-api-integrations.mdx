...
- `isolated`는 작업당 하나의 Lambda 함수를 생성합니다. 이는 생성된 API의 기본값입니다.
- `shared`는 단일 기본 라우터 Lambda를 생성하고 특정 통합을 재정의하지 않는 한 모든 작업에서 재사용합니다.

`isolated`는 작업별로 더 세분화된 권한과 구성을 제공합니다. `shared`는 선택적 재정의를 허용하면서도 Lambda 및 API Gateway 통합 확산을 줄입니다.

현재 생성기 지원은 백엔드에 따라 다릅니다:

- `ts#trpc-api`는 `integrationPattern` 옵션을 통해 `ServerlessApiGatewayRestApi` 및 `ServerlessApiGatewayHttpApi`에 대해 `isolated`와 `shared`를 모두 지원합니다.
- `py#fast-api` 및 `ts#smithy-api`는 현재 `isolated`를 생성합니다.

기존 생성된 CDK API 구문의 `defaultIntegrations` 메서드에서 `pattern` 필드를 편집하여 두 패턴 간 전환할 수도 있습니다:

```ts {3}
return IntegrationBuilder.rest({
  // 'isolated' 또는 'shared'로 변경
  pattern: 'isolated',
  ...
});
```
...
<Fragment slot="terraform">
Terraform 모듈은 기본적으로 라우터 패턴을 사용하며, 모든 API 작업을 처리하는 단일 Lambda 함수를 사용합니다.
</Fragment>
</Infrastructure>
...
<Infrastructure>
<Fragment slot="cdk">
생성된 기본값을 넘어 라우터 패턴을 사용자 정의해야 하는 경우, API의 `defaultIntegrations` 메서드를 자유롭게 편집하여 원하는 방식으로 통합을 구성할 수 있습니다:
...
원한다면 다른 방식으로 코드를 수정할 수도 있습니다. 예를 들어 `router` 함수를 메서드 내부에서 생성하는 대신 `defaultIntegrations`의 매개변수로 정의하는 것을 선호할 수 있습니다.
</Fragment>
<Fragment slot="terraform">
Terraform 모듈은 자동으로 라우터 패턴을 사용합니다 - 이것이 기본이자 유일하게 지원되는 접근 방식입니다. 생성된 모듈은 모든 API 작업을 처리하는 단일 Lambda 함수를 생성합니다.
