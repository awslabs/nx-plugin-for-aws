...
기본 옵션에서 `environment`를 지정하면 생성된 API 구문 내에 정의된 모든 환경 변수를 __덮어씁니다__. 일반적으로 Lambda 함수에는 `addEnvironment` 메서드를 사용하는 것이 좋습니다. 통합을 반복하여 이를 쉽게 달성할 수 있습니다:
...
예를 들어 `pattern`을 `'shared'`로 설정하면 통합당 하나씩 대신 단일 함수를 생성합니다:
