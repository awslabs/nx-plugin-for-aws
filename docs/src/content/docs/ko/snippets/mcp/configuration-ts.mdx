...
:::caution
서버 연결 시 `ENOENT npx` 오류가 발생하면 터미널에서 `which npx`를 실행해 얻은 전체 경로를 지정해야 할 수 있습니다.
:::
...
:::tip
Amazon Q Developer과 같은 일부 AI 어시스턴트는 워크스페이스 수준의 MCP 서버 설정을 지원하며, 특정 프로젝트와 관련된 MCP 서버를 정의할 때 특히 유용합니다.
:::