...
MCP 서버는 공유 AWS AppConfig 애플리케이션에서 런타임 구성을 읽습니다. 자세한 내용은 <Link path="/guides/terraform-project#shared-runtime-config-appconfig">Terraform 인프라스트럭처 가이드</Link>를 참조하세요. 배포당 한 번 `core/runtime-config/appconfig` 모듈을 인스턴스화하고 그 출력을 모든 에이전트 / MCP 모듈에 전달합니다:

```terraform {1-5, 10-11}
# 공유 runtime-config AppConfig (배포당 한 번 선언)
...
루트 모듈 끝에 `module "runtime_config_appconfig_deployment"`를 한 번 선언하세요 (런타임 구성 항목을 제공하는 모든 모듈을 포함하는 `depends_on` 사용). 이는 제공된 항목을 집계하고 AppConfig 호스팅 구성 버전 + 배포를 생성합니다. <Link path="/guides/terraform-project#shared-runtime-config-appconfig">전체 패턴</Link>을 참조하세요.
</Fragment>
</Infrastructure>
