...
이 생성기는 선택한 `iac` 기반으로 CDK 또는 Terraform 인프라 코드를 생성합니다. 이를 통해 함수를 배포할 수 있습니다.
...
:::note[이벤트 소스 매칭]
핸들러 함수 내에서 이벤트가 올바르게 처리되도록 선택한 `event` 옵션과 이벤트 소스가 일치하는지 확인하세요.
:::
...
이 생성기는 `common/terraform` 폴더에 함수 배포를 위한 Terraform 모듈을 생성합니다. 배포당 한 번 공유 `core/asset-bucket` 모듈을 인스턴스화하고 `asset_bucket_name` 입력에 `bucket_name`을 전달하세요:
