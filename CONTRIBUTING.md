...
export const DECLARED_DEPENDENCIES = declareDependencies({
  ts: ['@modelcontextprotocol/sdk', 'express', ...SHARED_CONSTRUCTS_DEPENDENCIES],
  py: ['mcp', 'boto3'],
});
```
...
- **Declare every branch.** A declaration is static, so a conditional dependency (`auth === 'cognito'`, a database engine) must appear regardless of which branch runs.
...
- **Name the export `DECLARED_DEPENDENCIES`.** The migration reads it by that name.
- **Cover what you delegate to.** A helper taken through `MustDeclare` is checked by the compiler, but one that merely exports its own `DECLARED_DEPENDENCIES` (the AG-UI react connection, for instance) is not — spread it, as `ts#agent#react-connection` does.
