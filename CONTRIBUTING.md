...
At upgrade time `ownedDependencies` (`owned-dependencies.ts`) works out which generators the workspace has run — from the `generator` recorded on each project and its components, plus `aws-nx-plugin.config.mts` standing in for `init`, which creates no project — then reads each one's declaration and syncs only that union. Nothing extra is recorded in the workspace, so this works for workspaces generated by earlier releases.

Ownership is per workspace, not per manifest: if a generator that owns `zod` has run anywhere, every `zod` in the workspace is synced. Narrowing further would mean recording per-manifest state in the user's repo, which this deliberately avoids.
...
**Overrides are synced too**, in `syncOverrides`. A generator pins a package under an override when a dependency's own range would otherwise resolve a second, incompatible copy (`ts#mcp-server`'s `zod`, `ts#rdb`'s `@types/pg`). Devkit doesn't manage those fields, so the pin would otherwise stay on the version it was generated with. Every field a package manager reads is covered — npm's `overrides`, yarn and bun's `resolutions`, pnpm 10's `pnpm.overrides` and pnpm 11's `overrides` in `pnpm-workspace.yaml` — including nested and parent-scoped keys, where the package is the key's last segment.

When writing an override in a generator, **declare both yarn descriptor forms**: classic honours only the `**/`-prefixed one in a workspace, while berry honours only the bare one and *deletes* a glob descriptor on install (`YN0057`). Writing one form alone leaves the dedupe silently inert under the other major.
...
The version sync has a smoke lane of its own (`smoke-test --name=version-sync`), which runs it over a real generated workspace rather than a synthetic tree — so it covers the manifests, pyprojects and `.tf` files the generators actually write, and ownership resolved from the metadata they really recorded. `internal#roll-back-versions` puts the workspace behind first, deriving every version it writes from the current tables so nothing is hardcoded, and `internal#sync-vended-versions` runs the migration without needing a release. It runs under **yarn**, since the generators only write a resolution there. Add a case to it when you add a surface the sync touches.
