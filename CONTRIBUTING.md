...
First build and link the local packages:

```bash
# In this repo
pnpm package:all
cd dist/packages/nx-plugin && pnpm link --global && cd -
```
...
```bash
cd ~/test
npx -y create-nx-workspace@latest my-migration-test --preset=@aws/nx-plugin --ci=skip
...
Now swap in the local build and run the migration:

```bash
pnpm link --global @aws/nx-plugin
...
4. **Re-run it** — running the migration a second time must change nothing.
