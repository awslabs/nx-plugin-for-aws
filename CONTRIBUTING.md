...
The `migrate` e2e test then covers every migration together: it creates a workspace on a released version, scaffolds a representative recipe with that version's generators, and upgrades it to the local build with the real `nx migrate` cycle — asserting the deterministic migrations apply, prompt migrations are deferred to `tools/ai-migrations/`, a customised user-owned file is left alone, re-running changes nothing, and the workspace still builds. Run it with:

```bash
pnpm nx run @aws/nx-plugin-e2e:smoke-test --name=migrate
```

It migrates from the latest release by default; set `NX_E2E_MIGRATE_VERSIONS=<n>` to cover the last _n_ releases.

Unit tests prove a migration does what you wrote it to do, and the e2e test proves the collection as a whole keeps a workspace green. Neither tells you whether the migrated workspace matches what today's generators would produce — so also **test your migration manually against a real workspace generated by the latest published version**, since that is the state your users are upgrading from.
